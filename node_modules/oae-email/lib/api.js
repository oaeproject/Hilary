 ...
 */
var debug = true;
var debugEmailSendCounter = new Counter();
 ...
 */
var EmailAPI = (module.exports = new EmitterAPI.EventEmitter());
...
var init = (module.exports.init = function(emailSystemConfig, callback) {
  ...
  deduplicationInterval =
    emailSystemConfig.deduplicationInterval || 7 * 24 * 60 * 60;
  ...
  var EmailRedback = redback.use(Redis.getClient(), {
    namespace: 'oae-email:redback',
  });
  ...
  if (debug) {
    ...
    if (emailSystemConfig.transport === 'SMTP') {
      log().info(
        { data: emailSystemConfig.smtpTransport },
        'Configuring SMTP email transport.',
      );
      emailTransport = nodemailer.createTransport(
        emailSystemConfig.smtpTransport,
      );
...
var refreshTemplates = (module.exports.refreshTemplates = function(callback) {
  ...
  _getTemplatesForModules(__dirname + '/../..', modules, function(
    err,
    _templates,
  ) {
    if (err) {
 ...
 * @param  {Object}     [callback.err]      An error that occurred, if any
 */
var sendEmail = (module.exports.sendEmail = function(
  templateModule,
  templateId,
  recipient,
  data,
  ...
  validator
    .check(templateModule, { code: 400, msg: 'Must specify a template module' })
    .notEmpty();
  validator
    .check(templateId, { code: 400, msg: 'Must specify a template id' })
    .notEmpty();
  validator
    .check(null, {
      code: 400,
  ...
  if (!metaTemplate) {
    var noMetaTemplateErr = {
      code: 500,
      msg: 'No email metadata template existed for user',
    };
    log().error(
      {
  ...
  var templateCtx = _.extend({}, data, {
    ...
    shared: sharedLogic,
    instance: {
      name: TenantsConfig.getValue(
        recipient.tenant.alias,
        'instance',
        'instanceName',
      ),
      URL: TenantsConfig.getValue(
        recipient.tenant.alias,
        'instance',
        'instanceURL',
      ),
  ...
  var metaRendered = UIAPI.renderTemplate(
    metaTemplate,
    templateCtx,
    recipient.locale,
  );
  ...
  try {
    ...
    return callback({
      code: 500,
      msg: 'Error parsing email metadata template for recipient',
    });
  }
  ...
  if (htmlTemplate) {
    try {
      htmlContent = UIAPI.renderTemplate(
        htmlTemplate,
        templateCtx,
        recipient.locale,
      );
  ...
  if (txtTemplate) {
    try {
      txtContent = UIAPI.renderTemplate(
        txtTemplate,
        templateCtx,
        recipient.locale,
      );
  ...
  if (htmlContent || txtContent) {
    ...
    return callback({
      code: 500,
      msg: 'Could not parse a suitable content template for user',
    });
  }
  ...
  var fromName =
    EmailConfig.getValue(tenant.alias, 'general', 'fromName') ||
    tenant.displayName;
  ...
  /*!
     ...
     */
  var _decrCallback = function(err) {
    _decr();
    callback(err);
  };
 ...
 */
var whenAllEmailsSent = (module.exports.whenAllEmailsSent = function(callback) {
...
var _sendEmail = function(emailInfo, opts, callback) {
  ...
  Locking.acquire(lockKey, deduplicationInterval, function(err, token) {
    if (err) {
      log().error(
        { err: err, emailInfo: emailInfo },
        'Unable to lock email id',
      );
      return callback(err);
    } else if (!token) {
      ...
      return callback({
        code: 403,
        msg: 'This email has already been sent out',
      });
    }

    // Ensure we're not sending out too many emails to a single user within the last timespan
    EmailRateLimiter.count(emailInfo.to, throttleConfig.timespan, function(
      err,
      count,
    ) {
      if (err) {
        ...
        return callback({
          code: 500,
          msg: 'Failed to perform email throttle check',
        });
      ...
      EmailRateLimiter.add(emailInfo.to, function(err) {
        ...
        emailTransport.sendMail(emailInfo, function(err, info) {
          if (err) {
            log().error(
              { err: err, to: emailInfo.to, subject: emailInfo.subject },
 ...
 * @api private
 */
var _getTemplatesForTemplateIds = function(
  basedir,
  module,
  templateIds,
  callback,
  ...
  var templateMetaPath = _templatesPath(
    basedir,
    module,
    templateId + '.meta.json.jst',
  );
  var templateHtmlPath = _templatesPath(
    basedir,
    module,
    templateId + '.html.jst',
  );
  var templateTxtPath = _templatesPath(
    basedir,
    module,
    templateId + '.txt.jst',
  );
  ...
  _getCompiledTemplate(templateMetaPath, function(err, metaTemplate) {
    ...
    _getCompiledTemplate(templateHtmlPath, function(err, htmlTemplate) {
      ...
      _getCompiledTemplate(templateTxtPath, function(err, txtTemplate) {
        ...
        var sharedLogic = {};
        try {
          var templateSharedPath = _templatesPath(
            basedir,
            module,
            templateId + '.shared',
          );
        ...
        return _getTemplatesForTemplateIds(
          basedir,
          module,
          templateIds,
          callback,
  ...
  fs.exists(templatePath, function(exists) {
    ...
    fs.readFile(templatePath, 'utf8', function(err, templateContent) {
      ...
      if (templateContent) {
        ...
        return callback({
          code: 500,
          msg: 'Template file ' + templatePath + ' had no content',
        });
      }
    });
  });
