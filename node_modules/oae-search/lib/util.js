 ...
 */
module.exports.getQueryParam = function(val, defaultVal) {
 ...
 * @return  {String}                `val` if it is valid. `defaultVal` otherwise.
 */
module.exports.getResourceTypeParam = function(val, defaultVal) {
    defaultVal = defaultVal || SearchConstants.general.RESOURCE_TYPE_ALL;
    return (val && _.contains(SearchConstants.general.RESOURCE_TYPES, val)) ? val : defaultVal;
}
 ...
 * @return  {String}                `val` if it is valid. `defaultVal` otherwise.
 */
module.exports.getSortParam = function(val, defaultVal) {
 ...
 * @param   {String}        searchType          The type of search for which to transform the results (e.g., general)
 ...
 */
module.exports.transformSearchResults = function(ctx, searchType, transformers, results, callback) {
   var hits = results.hits;
    if (!hits || !hits.hits || hits.hits.length === 0) {
        ...
        var docIdOrdering = {};
        var hits = hits.hits;
        for (var i = 0, numHits = hits.length; i < numHits; i++) {
            var doc = hits[i];
            var id = doc._id;
            var type = doc._type;
        ...
        for (var type in docsByType) {
            transformers[type](ctx, searchType, docsByType[type], _monitorTransformers);
        }
    }
