...
var canUpdateConfigValue = function(ctx, moduleId, feature, element) {
    if (!cachedGlobalSchema[moduleId] ||
        ...
        return {'code': 400, 'msg': 'The configuration to change does not exist.'};
    }
    ...
    return {'code': 401, 'msg': 'Only global administrators can change this config value'};
};
...
var updateConfig = module.exports.updateConfig = function(ctx, tenantAlias, configValues, callback) {
    ...
    for (var i = 0; i < configFields.length; i++) {
        ...
        var canUpdateValue = canUpdateConfigValue(ctx, element[0], element[1], element[2]);
        if (canUpdateValue.code) {
            return callback(canUpdateValue);
        }
    }
    if (validator.hasErrors()) {
        return callback(validator.getFirstError());
