 ...
 *
 * @param  {RestContext}        restContext     The REST context to use when adding the content items to the folder
 * @param  {String}             folderId        The id of the folder to which to try and add the content items
 ...
 * @param  {String[]}           folderIds       The ids of the folders to which to add the content item
 * @param  {String}             contentId       The id of the content item to add to the folders
 * @param  {Function}           callback        Invoked when the content item has been successfully added to all the folders
...
/**
 * Remove a set of content items to a folder, ensuring that all items have been removed succesfully
 *
 * @param  {RestContext}        restContext     The REST context to use when removing the content items to the folder
 ...
 * @param  {Function}           callback        Invoked when the content items have been successfully removed
    ...
    _getAllFoldersInLibraries(allMemberInfos, function(principalFoldersLibrariesBeforeCreate) {
        ...
        RestAPI.Folders.createFolder(restContext, displayName, description, visibility, managerIds, viewerIds, function(err, createdFolder) {
            ...
            RestAPI.User.getMe(restContext, function(err, me) {
                ...
                // Add the current user member info to ensure the folder gets added to their
                // library
                allMemberInfos.push({
                    'restContext': restContext,
                    'profile': me
                ...
                assertFullFolderMembersEquals(restContext, createdFolder.id, expectedMemberRoles, function() {
                    ...
                    _getAllFoldersInLibraries(allMemberInfos, function(principalFoldersLibrariesAfterCreate) {
                        ...
                        _purgeFoldersLibraries(_.union(managerIds, viewerIds), function() {
                            _getAllFoldersInLibraries(allMemberInfos, function(principalFoldersLibrariesAfterCreate) {
                                _.each(allMemberInfos, function(memberInfo) {
                                    assert.ok(_.chain(principalFoldersLibrariesAfterCreate[memberInfo.profile.id]).pluck('id').contains(createdFolder.id).value());
                                });
 ...
 * @throws {AssertionError}                     Thrown if the request did not fail in the expected manner
 */
var assertUpdateFolderContentVisibility = module.exports.assertUpdateFolderContentVisibility = function(restContext, folderId, visibility, httpCode, callback) {
...
var assertDeleteFolderFails = module.exports.assertDeleteFolderFails = function(restContext, folderId, httpCode, callback) {
    RestAPI.Folders.deleteFolder(restContext, folderId, function(err) {
        assert.ok(err);
        assert.strictEqual(err.code, httpCode);
        return callback();
    });
 ...
 * @throws {AssertionError}                     Thrown if the request did not fail in the expected manner
 */
var assertDeleteFolderSucceeds = module.exports.assertDeleteFolderSucceeds = function(restContext, folderId, callback) {
    RestAPI.Folders.deleteFolder(restContext, folderId, function(err) {
        assert.ok(!err);

        // Wait till all the libraries have been purged before returning to the caller
        FoldersLibrary.whenAllPurged(callback);
    });
};
 ...
 * @param  {String}             folderId        The id of the folder whose content items to get
 ...
 * @param  {String}             principalId         The id of the principal from whose library a folder should be removed
 ...
 * @throws {AssertionError}                     Thrown if the request did not succeed
 */
var assertShareFolderSucceeds = module.exports.assertShareFolderSucceeds = function(restContext, folderId, viewers, callback) {
    ...
    getAllFolderMembers(restContext, folderId, null, function(membersBeforeShare) {
        ...
        _getAllFoldersInLibraries(viewerInfos, function(principalFoldersLibrariesBeforeShare) {
            ...
            RestAPI.Folders.shareFolder(restContext, folderId, viewerIds, function(err) {
                assert.ok(!err);
                LibraryAPI.Index.whenUpdatesComplete(function() {

                    // Ensure the members of the folder were updated as we would expect
                    getAllFolderMembers(restContext, folderId, null, function(membersAfterShare) {
 ...
 * @throws {AssertionError}                     Thrown if the request did not fail in the expected manner
 */
var assertShareFolderFails = module.exports.assertShareFolderFails = function(restContext, folderId, viewerIds, httpCode, callback) {
    RestAPI.Folders.shareFolder(restContext, folderId, viewerIds, function(err) {
        assert.ok(err);
        assert.strictEqual(err.code, httpCode);
        return callback();
 ...
 * @throws {AssertionError}                     Thrown if the request did not fail in the expected manner
 */
var assertUpdateFolderMembersFails = module.exports.assertUpdateFolderMembersFails = function(restContext, folderId, memberUpdates, httpCode, callback) {
    RestAPI.Folders.updateFolderMembers(restContext, folderId, memberUpdates, function(err) {
        assert.ok(err);
        assert.strictEqual(err.code, httpCode);
        return callback();
    });
 ...
 * @throws {AssertionError}                         Thrown if the request did not succeed
 */
var assertUpdateFolderMembersSucceeds = module.exports.assertUpdateFolderMembersSucceeds = function(restContext, folderId, memberUpdateInfos, callback) {
    ...
    _getAllFoldersInLibraries(memberInfos, function(principalFoldersLibrariesBeforeUpdate) {
        ...
        RestAPI.Folders.updateFolderMembers(restContext, folderId, roleChange, function(err) {
            assert.ok(!err);

            // Iterate the folder members and ensure all the members have the specified role, or are
            // removed depending on the specified role
            getAllFolderMembers(restContext, folderId, null, function(membersAfterUpdate) {
 ...
 * @param  {Content[]}          expectedContent     The set of content items that are supposed to come back
 * @param  {Function}           callback            Standard callback function
 * @throws {AssertionError}                         Thrown if any assertions failed
 */
var assertFolderSearchEquals = module.exports.assertFolderSearchEquals = function(restContext, folderId, q, expectedContent, callback) {
    SearchTestsUtil.searchAll(restContext, 'folder-content', [folderId], {'q': q}, function(err, results) {
        assert.ok(!err);
...
var assertGeneralFolderSearchEquals = module.exports.assertGeneralFolderSearchEquals = function(restContext, q, expectedFolders, missingFolders, callback) {
    SearchTestsUtil.searchAll(restContext, 'general', null, {'resourceTypes': 'folder', 'q': q}, function(err, results) {
        assert.ok(!err);
 ...
 *
 * @param  {RestContext}        restContext         The REST context to use when searching for folders
 * @param  {String}             principalId         The id of the principal whose folder library should be searched through
 * @param  {String}             q                   The term to search on
 * @param  {Folder[]}           expectedFolders     The set of folders that are supposed to come back
 * @param  {Folder[]}           missingFolders      Folders that should not be included in the result set
 * @param  {Function}           callback            Standard callback function
...
var assertFolderLibrarySearch = module.exports.assertFolderLibrarySearch = function(restContext, principalId, q, expectedFolders, missingFolders, callback) {
    SearchTestsUtil.searchAll(restContext, 'folder-library', [principalId], {'q': q}, function(err, results) {
        assert.ok(!err);
        _assertSearchResults(results, expectedFolders, missingFolders);
        return callback();
    });
...
/**
 * Assert the given search results meet a given set of expectations
 *
...
var assertFolderSearchFails = module.exports.assertFolderSearchFails = function(restContext, folderId, httpCode, callback) {
    SearchTestsUtil.searchAll(restContext, 'folder-content', [folderId], null, function(err, results) {
        assert.ok(err);
        assert.strictEqual(err.code, httpCode);
        return callback();
    });
 ...
 * @param  {Function}       callback                Invoked when the full folders library has been fetched
 ...
 * @param  {Function}       callback                Invoked when the full folder members list has been fetched
 * @param  {Folder[]}       callback.contentItems   A list of all users and groups who are members of the folder
 * @param  {Object[]}       callback.responses      All the raw web responses that were received for each page request
 */
 ...
 * @param  {Function}       callback        Invoked when the folders are created
 * @api private
 */
 ...
 * @param  {Function}       callback        Invoked when the folder is created
 * @api private
 */
 ...
 * @param  {Function}   callback                                Invoked when the folders libraries have been collected
