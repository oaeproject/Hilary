 ...
 */
module.exports.createUser = function(ctx, displayName, opts, callback) {
    opts = opts || {};
    ...
    opts.publicAlias = opts.publicAlias || '';
    ...
    var id = AuthzUtil.toId('u', tenantId, ShortId.generate());
    ...
    Cassandra.runQuery('INSERT INTO Principals (principalId, tenant, displayName, visibility, locale, timezone, publicAlias) VALUES (?, ?, ?, ?, ?, ?, ?) USING CONSISTENCY QUORUM', 
            [id, tenantId, displayName, opts.visibility, opts.locale, opts.timezone, opts.publicAlias], function (err) {
        if (err) {
            return callback(err);
        }
        return callback(null, new PrincipalsModel.User(tenantId, id, displayName, opts));
    });
};
 ...
 */
module.exports.getMe = function(ctx, callback) {
    if (!ctx.user()) {
    ...
    PrincipalsAPI.getBasicProfile(ctx, ctx.user().id, function(err, data) {
        if (err) {
            return callback(err);
        }
