 ...
 *     - A small image (original format except for gif)
 *     - A medium image (original format except for gif)
 *     - A large image (original format except for gif)
...
var generatePreviewsFromImage = module.exports.generatePreviewsFromImage = function(ctx, path, options, callback) {
    ...
    var extension = ImageUtil.getImageExtension(path, '.jpg');
    var fixedPath = util.format('%s/fixed%s', ctx.baseDir, extension);
    var opts = {
...
var _resize = function(ctx, path, size, callback) {
    ...
    ImageUtil.resizeImage(inputPath, size, function(err, file) {
        if (err) {
            return callback(err);
        }
        ...
        var extension = ImageUtil.getImageExtension(path, '.jpg');
        var outputPath = ctx.baseDir + '/' + prefix + size.size + extension;
        IO.moveFile(file.path, outputPath, function(err) {
            if (err) {
                return callback(err);
            }
...
var _cropThumbnail = function(ctx, path, cropMode, callback) {
    ...
    _convertToJPG(ctx, path, function(err, jpgPath) {
        if (err) {
            return callback(err);
        }
        ...
        _cropIntelligently(ctx, jpgPath, PreviewConstants.SIZES.IMAGE.THUMBNAIL, PreviewConstants.SIZES.IMAGE.THUMBNAIL, opts, 'thumbnail.jpg', function(err, thumbnailPath) {
            if (err) {
                return callback(err);
            ...
            _cropIntelligently(ctx, jpgPath, PreviewConstants.SIZES.IMAGE.WIDE_WIDTH, PreviewConstants.SIZES.IMAGE.WIDE_HEIGHT, opts, 'wide.jpg', function(err, widePath) {
                if (err) {
                    return callback(err);
                }
 ...
 * @param  {Function}           callback                Standard callback method
 * @param  {Object}             callback.err            Standard error object, if any
 * @param  {String}             callback.path           The converted JPG image
 * @api private
 */
var _convertToJPG = function(ctx, path, callback) {
    var conversionPath = path;
    if (path.lastIndexOf('.gif') === (path.length - 4)) {
        // If we're dealing with a GIF, we use the first frame
        conversionPath  = path + '[0]';
    }
    gm(conversionPath).size(function (err, size) {
         ...
         */
        var outputPath = util.format('%s/noAnimations.jpg', ctx.baseDir);
        var cmd = util.format('gm convert -size %dx%d xc:white -compose over %s -flatten %s', size.width, size.height, path, outputPath);
        exec(cmd, {'timeout': 2000}, function (err, stdout, stderr) {
            if (err) {
                log().error({'err': err}, 'Unable to convert input image to JPG');
                return callback({'code': 500, 'msg': err});
            }
