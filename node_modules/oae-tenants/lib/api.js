...
Pubsub.on('oae-tenants', function (message) {
    ...
    getTenant(alias, function(err, tenant) {
        if (err) {
 ...
 */
emitter.on('start', _cacheTenants);
emitter.on('stop', _cacheTenants);
emitter.on('refresh', _cacheTenants);
...
/**
 * Get all available tenants from cache
 *
 * @param  {Boolean}        [excludeDisabled]   Whether or not disabled tenants should be included as well. By default, all tenants will be returned
...
var getTenants = module.exports.getTenants = function(excludeDisabled) {
    if (excludeDisabled) {
        // Only return the active tenants
        var filteredTenants = {};
        _.each(tenants, function(tenant, tenantAlias) {
            if (tenant.active) {
                filteredTenants[tenantAlias] = tenant;
            }
        });
 ...
 * @param  {Function}       callback            Standard callback function
 * @param  {Object}         callback.err        Error object containing the error code and message
 * @param  {Tenant}         callback.tenant     The tenant object that is associated to the provided alias
 */
var getTenant = module.exports.getTenant = function(tenantAlias, callback) {
    var tenant = tenants[tenantAlias];
    if (tenant) {
        return callback(null, tenant);
    } else {
        return callback({'code': 404, 'msg': 'There is no tenant with alias ' + tenantAlias});
    }
};
...
var _cacheTenants = function(callback) {
    ...
    globalTenant = new Tenant(serverConfig.globalAdminAlias, 'Global admin server', serverConfig.globalAdminHost, true, false);
    ...
    Cassandra.runQuery('SELECT * FROM Tenant USING CONSISTENCY QUORUM', false, function(err, rows) {
        ...
        for (var i = 0; i < rows.length; i++) {
            ...
            // Cache all tenants that have not been deleted
            if (!tenant.deleted) {
...
var createTenant = module.exports.createTenant = function(ctx, alias, displayName, host, callback) {
    ...
    // TODO: Make sure that there are no spaces in here
    ...
    getTenant(alias, function(err, tenant) {
        if (!tenant) {
            ...
            if (!tenantsByHost[alias]) {
                // Create the tenant
                var tenant = new Tenant(alias, displayName, host, true, false);
...
var updateTenant = module.exports.updateTenant = function(ctx, alias, tenantUpdates, callback) {
    ...
    _.each(tenantUpdates, function(updateValue, updateField) {
        ...
        if (updateField === 'displayName') {
            ...
            validator.check(profileFields['displayName'], {'code': 400, 'msg': 'A hostname cannot be empty'}).notEmpty();
        }
    });
    if (validator.hasErrors()) {
        return callback(validator.getFirstError());
