...
var PreviewContext = module.exports.PreviewContext = function(config, contentId, revisionId) {
    ...
    that.cleanup = function() {
        ...
        var dirs = [];
        var files = [];
        readdirp({'root': that.baseDir, 'lstat': true})
            .on('data', function(entry) {
                files.push(entry.fullPath);
                if (dirs.indexOf(entry.fullParentDir) === -1) {
                    dirs.push(entry.fullParentDir);
                }
            }).on('end', function(){
                var rmDir = function() {
                    if (_.isEmpty(dirs)) {
                        ...
                        fs.rmdir(dirs.pop(), rmDir);
                    }
                };
                var cleanDirs = _.after(files.length, function() {
                ...
                _.each(files, function(file) {
                    fs.unlink(file, cleanDirs);
                });
            }).on('error', function(err) {
                log().error('Error cleaning up temporary files: ' + err);
            });
    };
        ...
        RestAPI.Admin.getSignedToken(globalRestContext, tenantAlias, function(err, token) {
            ...
            RestAPI.Admin.loginWithSignedToken(restCtx, token, function(err, body, response) {
                if (err) {
                    ...
                    restCtx.username = globalRestContext.username;
                    // Use this context
