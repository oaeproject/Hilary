...
var _produceContentSearchDocuments = function(resources, callback) {
    ...
    // For indexing resources that have content items attached, produce the document. For those that don't,
    // aggregate the ids so the content items may be fetched
    var contentIdsToFetch = [];
    _.each(resources, function(resource) {
        if (resource.content) {
            docs.push(_produceContentSearchDocument(resource.content));
        } else {
    ...
    ContentDAO.Content.getMultipleContentItems(contentIdsToFetch, function(err, contentItems) {
        if (err) {
            return callback(err);
        }

        // Produce documents out of all the fetched content items
        _.each(contentItems, function(content) {
            // Ensure that there is still a content item there.
            // A fast sequence of create/delete could result in a content item
            // being already deleted before we reach this point.
            if (content && content.tenant) {
...
var _produceContentSearchDocument = function(content) {
    ...
    if (content.resourceSubType === 'collabdoc' && content.collabdoc && content.collabdoc.html) {
        fullText += ' ' + content.collabdoc.html;
    }
    ...
    var doc = {
        ...
        'profilePath': content.profilePath,
        'q_high': content.displayName,
        'q_low': fullText,
        'sort': fullText,
        '_extra': {'lastModified': content.lastModified}
    };
...
var _transformContentDocuments = function(ctx, docs, callback) {
    ...
    _.each(docs, function(doc, docId) {
        ...
        if (doc.fields.thumbnailUrl && result.lastModified) {
            result.thumbnailUrl = ContentUtil.getThumbnailUrl(ctx, result.id, result.lastModified);
        }
