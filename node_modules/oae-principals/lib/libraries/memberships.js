 ...
 */
PrincipalsEmitter.when(PrincipalsConstants.events.CREATED_GROUP, function(ctx, group, memberChangeInfo, callback) {
    _touchMembershipLibraries(group, null, memberChangeInfo, function(err) {
        if (err) {
            log().warn({
                'err': err,
                'groupId': group.id,
 ...
 * belongs
 */
PrincipalsEmitter.on(PrincipalsConstants.events.UPDATED_GROUP, function(ctx, updatedGroup, oldGroup) {
    _touchMembershipLibraries(updatedGroup, oldGroup.lastModified, null, function(err) {
        if (err) {
            log().warn({
                'err': err,
 ...
 */
PrincipalsEmitter.when(PrincipalsConstants.events.LEFT_GROUP, function(ctx, group, memberChangeInfo, callback) {
    _touchMembershipLibraries(group, null, memberChangeInfo, function(err) {
        if (err) {
            log().warn({
                'err': err,
 ...
 */
PrincipalsEmitter.when(PrincipalsConstants.events.JOINED_GROUP, function(ctx, group, oldGroup, memberChangeInfo, callback) {
    ...
    _touchMembershipLibraries(group, oldGroup.lastModified, memberChangeInfo, function(err) {
        if (err) {
            log().warn({
                'err': err,
                'group': group,
 ...
 */
PrincipalsEmitter.when(PrincipalsConstants.events.UPDATED_GROUP_MEMBERS, function(ctx, group, oldGroup, memberChangeInfo, callback) {
 ...
 * @api private
 */
var _touchMembershipLibraries = function(group, oldLastModified, memberChangeInfo, callback) {
    var addedMemberIds = (memberChangeInfo) ? _.pluck(memberChangeInfo.members.added, 'id') : [];
    var removedMemberIds = (memberChangeInfo) ? _.pluck(memberChangeInfo.members.removed, 'id') : [];
    ...
    _getGroupAncestorsIncludingDeleted(group, function(err, ancestorGroups) {
        if (err) {
            return callback(err);
        }
        ...
        _insertMembershipsLibraries(groups, addedMemberIds, function(err, explodedInsertedPrincipals) {
            if (err) {
                return callback(err);
            }
 ...
 * @api private
 */
var _insertMembershipsLibraries = function(groups, addedMemberIds, callback) {
    ...
    _getAllGroupChildren(addedMemberIds, [], function(err, allChildren) {
        if (err) {
            return callback(err);
        }

        // The principals for which the groups will be inserted in their libraries
        var principalIds = allChildren.concat(addedMemberIds);
 ...
 * @api private
 */
var _updateMembershipsLibraries = function(group, excludePrincipalIds, callback) {
    // Get the exploded members list of the group we've updated excluding any
    // principals we've dealth with earlier
    _getAllGroupChildren([group.id], excludePrincipalIds, function(err, allChildren) {
        if (err) {
            return callback(err);
        }
...
var _removeMembershipsLibraries = function(removedMemberIds, groups, callback) {
    ...
    _getAllGroupChildren(removedMemberIds, [], function(err, allChildren) {
        if (err) {
            return callback(err);
        }
 ...
 * @api private
 */
var _getGroupAncestorsIncludingDeleted = function(group, callback) {
    ...
    AuthzAPI.getPrincipalMembershipsGraph(group.id, function(err, graph) {
        ...
        PrincipalsDAO.getPrincipals(membershipIds, ['principalId', 'tenantAlias', 'lastModified', 'visibility'], function(err, parentGroupsByGroupId) {
            if (err) {
                return callback(err);
            }
 ...
 * @api private
 */
var _getAllGroupChildren = function(principalIds, excludePrincipals, callback, _groupsToExplode, _allChildren) {
    ...
    var groupId = _groupsToExplode.shift();
    ...
    AuthzAPI.getAuthzMembers(groupId, null, 10000, function(err, members) {
        ...
        _.each(members, function(member) {
            ...
            if (AuthzUtil.isGroupId(member.id) && !_.contains(_allChildren, member.id) && !_.contains(_groupsToExplode, member.id) && !_.contains(excludePrincipals, member.id)) {
