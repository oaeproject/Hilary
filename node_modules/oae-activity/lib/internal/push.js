...
var _subscribe = function (socket, message) {
    ...
    authzHandler(socket.ctx, data.stream.resourceId, data.token, function(err) {
        ...
        _bindQueue(activityStreamId, function(err) {
            ...
            socket.transformers = socket.transformers || {};
            socket.transformers[activityStreamId] = data.transformer || ActivityConstants.transformerTypes.INTERNAL;
            ...
            log().trace({'sid': socket.id, 'activityStreamId': activityStreamId}, 'Registered a client for a stream');
            _writeResponse(socket, message.id);
        });
    });
};
...
var _handlePushActivity = function(data, callback) {
    ...
    _.each(socketsPerStream[activityStreamId], function(socket) {
        ...
        var transformer = socket.transformers[activityStreamId] || ActivityConstants.transformerTypes.INTERNAL;
        ActivityTransformer.transformActivities(socket.ctx, activities, transformer, function(err) {
            if (err) {
