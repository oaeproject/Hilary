...
var init = (module.exports.init = function(_serverConfig, callback) {
  ...
  OAE.tenantServer.use(function(req, res, next) {
    if (
      ...
      req.headers.host === serverConfig.shibbolethSPHost
    ) {
    ...
    if (!req.tenant) {
      ...
      return res
        .status(418)
        .send('This hostname is not associated to any tenant');
    }
    ...
    if (!req.tenant.active) {
      ...
      return res
        .status(503)
        .send('This server is currently disabled. Please check back later.');
    }
 ...
 */
var getTenantByEmail = (module.exports.getTenantByEmail = function(
  emailOrDomain,
) {
 ...
 */
var getTenantsForEmailDomains = (module.exports.getTenantsForEmailDomains = function(
  emailsOrDomains,
) {
  var mappedTenants = {};
...
var _updateCachedTenant = function(tenantAlias, callback) {
  callback =
    ...
    function(err) {
      if (err) {
        log().error(
          { err: err, tenantAlias: tenantAlias },
          'Failed to re-cache the specified tenant',
        );
      }
    };
  ...
  // Get the available tenants
  Cassandra.runQuery(
    'SELECT * FROM "Tenant" WHERE "alias" = ?',
    [tenantAlias],
    function(err, rows) {
      if (err) {
        return callback(err);
      ...
      _.each(tenant.emailDomains, function(emailDomain) {
        var conflictingTenantAlias = tenantEmailDomainIndex.update(
          tenantAlias,
          emailDomain,
        );
...
var _createTenant = function(alias, displayName, host, opts, callback) {
  ...
  validator
    ...
    .notContains(':');
  validator
    .check(displayName, { code: 400, msg: 'Missing tenant displayName' })
    .notEmpty();
...
var updateTenant = (module.exports.updateTenant = function(
  ...
  if (!ctx.user() || !ctx.user().isAdmin(ctx.user().tenant.alias)) {
    return callback({
      code: 401,
      msg: 'Unauthorized users cannot update tenants',
    });
  }
  ...
  validator
    .check(getTenant(alias), {
      code: 404,
      msg: util.format(
        'Tenant with alias "%s" does not exist and cannot be updated',
        alias,
      ),
    })
    .notNull();
  ...
  _.each(tenantUpdates, function(updateValue, updateField) {
    validator
      .check(updateField, {
        code: 400,
        msg: util.format(
          '"%s" is not a recognized tenant update field',
          updateField,
        ),
      })
    ...
    if (updateField === 'displayName') {
      validator
        .check(updateValue, { code: 400, msg: 'A displayName cannot be empty' })
        .notEmpty();
      ...
      validator
        .check(updateValue, { code: 400, msg: 'A hostname cannot be empty' })
        .notEmpty();
      validator
 ...
 * @param  {Object}       callback.err    An error that occurred, if any
 */
var disableTenants = (module.exports.disableTenants = function(
  ctx,
  aliases,
  disabled,
  callback,
  ...
  Cassandra.runBatchQuery(queries, function(err) {
    ...
    async.mapSeries(
      aliases,
      function(eachAlias, transformed) {
        // disable or restore users from those tenancies too
        UserAPI.deleteOrRestoreUsersByTenancy(
          ctx,
          eachAlias,
          disabled,
          function(err) {
 ...
 * @api private
 */
var _setLandingPageBlockAttribute = function(
  ctx,
  block,
  blockName,
  attributeName,
  ...
  block[attributeName] = TenantsConfig.getValue(
    ctx.tenant().alias,
    blockName,
    attributeName,
  );
 ...
 * @api private
 */
var _validateEmailDomains = function(
  validator,
  emailDomains,
  updateTenantAlias,
) {
  _.each(emailDomains, function(emailDomain) {
    ...
    validator
      .check(emailDomain, { code: 400, msg: 'Invalid email domain' })
      .isHost();

    var matchingTenantAlias = tenantEmailDomainIndex.conflict(
      updateTenantAlias,
      emailDomain,
    );
...
var _copyTenant = function(tenant) {
  ...
  var tenantCopy = _storageHashToTenant(
    tenant.alias,
    _tenantToStorageHash(tenant),
  );
