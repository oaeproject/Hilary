...
/*!
 * Post a content-update activity when a user updates a content item's metadata.
 */
...
ActivityAPI.registerActivityObjectProducer('content', function(activityType, entityType, resource, callback) {
    ...
    ContentAPI._getContent(resource.resourceId, function(err, content) {
        ...
        TenantsAPI.getTenantByAlias(content.tenant, function(err, tenant) {
            ...
            return callback(null, new ActivityModel.ActivityObject('content', globalId, opts), propagation);
        });
    });
});

ActivityAPI.registerActivityObjectProducer('content-comment', function(activityType, entityType, resource, callback) {
    // TODO: When _getContent is no longer exposed on ContentAPI, there should be something like a ContentDAO.getContent available to do get content w/o permission checks instead
    ...
    ContentAPI._getContent(comment.contentId, function(err, content) {
        ...
        TenantsAPI.getTenantByAlias(content.tenant, function(err, tenant) {
            ...
            return callback(null, new ActivityModel.ActivityObject('content-comment', globalId, opts), propagation);
        });
    });
});
...
ActivityAPI.registerActivityRouter('content', function(activityType, entityType, activityObject, callback) {
    // Extract the OAE ID from the activity object to get the content id
    return _routeContent(activityType, entityType, activityObject[ActivityConstants.properties.OAE_ID], callback);
});

ActivityAPI.registerActivityRouter('content-comment', function(activityType, entityType, activityObject, callback) {
    // Extract the parent content id from the activity object, as the routes are identical
    return _routeContent(activityType, entityType, activityObject[ContentConstants.activity.PROP_OAE_CONTENT_ID], callback);
});
