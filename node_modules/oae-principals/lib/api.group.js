...
var getFullGroupProfile = module.exports.getFullGroupProfile = function(ctx, groupId, callback) {
    getGroup(ctx, groupId, function(err, group) {
        ...
        var accessCheck = function(roles) {
            ...
            var isAdmin =  (ctx.user() && ctx.user().isTenantAdmin(group.tenant.alias));
...
var _canView = function(ctx, group, hasRole) {
    ...
    } else if (group.visibility === PrincipalsConstants.visibility.LOGGEDIN && TenantsUtil.isLoggedIn(ctx, group.tenant.alias)) {
        return true;

    // If the group is set to private I can still view it if the joinable setting has been set to 'yes'.
    } else if (group.visibility === PrincipalsConstants.visibility.PRIVATE && TenantsUtil.isLoggedIn(ctx, group.tenant.alias) && group.joinable === 'yes') {
        return true;
    // If the user is admin of the group's tenant, they have access
    } else if (ctx.user() && ctx.user().isAdmin(group.tenant.alias)) {
        return true;
    }
...
var leaveGroup = module.exports.leaveGroup = function(ctx, groupId, callback) {
    ...
    AuthzAPI.hasAnyRole(ctx.user().id, groupId, function(err, hasAnyRole) {
        ...
        PrincipalsDAO.getPrincipal(groupId, function(err, group) {
            ...
            // Make sure we're not the last manager
            _checkNewGroupMembers(ctx, group, changeMembers, function(err) {
                if (err) {
                    return callback(err);
                }
...
var _canJoin = function(ctx, group, hasRole) {
    ...
    } else if (TenantsUtil.canInteract(ctx.user().tenant.alias, group.tenant.alias) && group.visibility === 'public') {
        return true;
 ...
 */
var createGroup = module.exports.createGroup = function(ctx, displayName, description, visibility, joinable, members, otherValues, callback) {
    ...
    validator.check(displayName, {'code': 400, 'msg': 'A displayName can be at most 1000 characters long'}).isShortString();
    ...
    canCreateGroup(ctx, groupId, function(err) {
        ...
        // Create the group.
        Cassandra.runQuery('INSERT INTO Principals (principalId, tenantAlias, displayName, description, visibility, joinable) VALUES (?, ?, ?, ?, ?, ?) USING CONSISTENCY QUORUM',
            [groupId, tenantAlias, displayName, description, visibility, joinable], function (err) {
            if (err) {
                return callback(err);
            }
            ...
            var group = new Group(tenantAlias, groupId, displayName, opts);
            _setGroupMembers(ctx, group, members, function(err) {
...
var updateGroup = module.exports.updateGroup = function(ctx, groupId, profileFields, callback) {
    ...
    fieldNames.forEach(function(fieldName) {
        ...
        if (fieldName === 'visibility') {
            ...
            validator.check(profileFields['displayName'], {'code': 400, 'msg': 'A displayName can be at most a 1000 characters long'}).isShortString();
        } else if (fieldName === 'description' && profileFields['description']) {
            validator.check(profileFields['description'], {'code': 400, 'msg': 'A description can be at most 10000 characters long'}).isMediumString();
        }
    });
