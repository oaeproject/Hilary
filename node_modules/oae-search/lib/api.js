...
var _aggregateMembershipsDocuments = function(membershipUpdates, callback, membershipsDocs) {
    ...
    if (AuthzUtil.isUserId(resourceId)) {
        // If the resource is a user, we're actually indexing the list of users that they follow
        FollowingDAO.getFollowing(resourceId, null, 10000, function(err, followingUserIds) {
            if (err) {
                return callback(err);
            }

            membershipsDocs.push(_createMembershipsDocument(resourceId, followingUserIds));
            return _aggregateMembershipsDocuments(membershipUpdates, callback, membershipsDocs);
        });
    } else {
        // If the resource is a group, we're indexing all of its memberships
        //
        // Filter the memberships to "group" types. This means that we will only be able to search through group members. If we
