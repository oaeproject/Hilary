 ...
 * @param  {Object}             roles                       The roles, keyed by a share target (e.g., principal id or email address), whose value is the role the target should have on the resource
...
var create = module.exports.create = function(ctx, roles, createFn, callback) {
    ...
    _getTargetRoles(roles, function(err, targetRolesByTargetId) {
        if (err) {
            return callback(err);
        }

        var targetRoles = _.values(targetRolesByTargetId);
 ...
 * @param  {String[]}           targetIds                   The share targets (e.g., principal ids and email addresses) to share with
 * @param  {String}             role                        The share role for this resource (e.g., "viewer", "member")
 * @param  {Function}           callback                    Standard callback function
 * @param  {Object}             callback.err                An error that occurred, if any
 * @param  {MemberChangeInfo}   callback.memberChangeInfo   The member change info object that describes the resource member updates
 * @param  {EmailChangeInfo}    callback.emailChangeInfo    The email change info object that describes the resource invitation updates
 */
...
var acceptInvitation = module.exports.acceptInvitation = function(ctx, token, callback) {
    ...
    _acceptInvitation(ctx, token, function(err, email, invitationHashes, changesByResourceId) {
        if (err) {
            return callback(err);
        }
        ...
        PrincipalsDAO.getPrincipals(inviterUserIds, null, function(err, inviterUsersById) {
            ...
            _ensureVerifiedEmail(ctx, email, function() {
                ...
                // made
                ResourceActions.emit(ResourceConstants.events.ACCEPTED_INVITATION, ctx, invitationHashes, changesByResourceId, inviterUsersById, function(errs, results) {
                    if (errs) {
...
var _ensureVerifiedEmail = function(ctx, email, callback) {
    ...
    PrincipalsUtil.verifyEmailAddress(ctx, ctx.user(), email, function(err) {
        if (err) {
            // Whine in the logs if this fails, but it's not critical so we can still otherwise
            // succeed
            log().warn({
                'err': err,
                'user': ctx.user(),
        ...
        AuthzInvitationsDAO.getAllInvitationsByEmail(email, function(err, invitationHashes) {
            ...
            _applyAllMemberChanges(memberRolesByResourceId, function(err, changesByResourceId) {
                ...
                AuthzInvitationsDAO.deleteInvitationsByEmail(email, function(err) {
                    ...
                    return callback(null, email, invitationHashes, changesByResourceId);
                });
            });
        });
    });
 ...
 *
 * @param  {Object}         roles                           A merged representation of `MemberRoles` and `EmailRoles` that indicate the role changes being applied
 * @param  {Function}       callback                        Standard callback function
 * @param  {Object}         callback.err                    An error that occurred, if any
 * @param  {ShareTarget[]}  callback.targetRolesByTargetId  The share targets, keyed by their original target id, with their desired roles applied
 */
var _getTargetRoles = function(roles, callback) {
    _getTargets(_.keys(roles), function(err, targetsByTargetId) {
        if (err) {
            return callback(err);
        }

        // Apply the roles to each share target
        _.each(targetsByTargetId, function(target, targetId) {
            target.role = roles[targetId];
        });

        return callback(null, targetsByTargetId);
    });
};
 ...
 * @param  {Function}       callback                    Standard callback function
 * @param  {Object}         callback.err                An error that occurred, if any
 * @param  {Object}         callback.targetsByTargetId  An object keyed by the target id whose value is the actual share targets expanded from the parsed target ids
 * @api private
 */
var _getTargets = function(targetIds, callback) {
    var targetsByTargetId = _.chain(targetIds)
        .map(function(targetId) {
            var target = AuthzUtil.parseShareTarget(targetId);
            if (target) {
                return [targetId, AuthzUtil.parseShareTarget(targetId)];
            }
        })
        .compact()
        .object()
    ...
    // Get the emails of all the email-only targets, we will see if they are already associated
    // to any accounts
    var emails = _.chain(targetsByTargetId)
    ...
    OaeUtil.invokeIfNecessary(!_.isEmpty(emails), PrincipalsDAO.getUserIdsByEmails, emails, function(err, userIdsByEmails) {
        ...
        _.each(emails, function(email) {
            ...
            if (_.size(userIds) === 1) {
                targetsByTargetId[email].principalId = _.first(userIds);
            }
        });

        var principalIds = _.chain(targetsByTargetId).pluck('principalId').uniq().compact().value();
        if (_.isEmpty(principalIds)) {
            // If there are no principal profiles to expand, just return
            return callback(null, targetsByTargetId);
        }

        // Expand all our principals into principal profiles
        PrincipalsDAO.getExistingPrincipals(principalIds, null, function(err, principalsById) {
            ...
            // profile
            _.each(targetsByTargetId, function(target) {
 ...
 *
 * @param  {Object}     memberChangesByResourceId               An object keyed by resource id, whose values are the `MemberRoles` objects to apply to change their access
 * @param  {Function}   callback                                Standard callback function
 * @param  {Object}     callback.err                            An error that occurred, if any
 * @param  {Object}     callback.memberChangeInfosByResourceId  An object keyed by resource id, whose values are the `MemberChangeInfo` objects that describe the canonical changes to apply to the member roles
 * @api private
 */
var _applyAllMemberChanges = function(memberChangesByResourceId, callback) {
    ...
    var memberChangeInfosByResourceId = {};
    var _done = _.chain(memberChangesByResourceId)
        .size()
        .after(function() {
            return callback(null, memberChangeInfosByResourceId);
        })
        .value();
    ...
    _.each(memberChangesByResourceId, function(changes, resourceId) {
        ...
        AuthzAPI.computeMemberRolesAfterChanges(resourceId, changes, {'promoteOnly': true}, function(err, changes, roles, members) {
            ...
            AuthzAPI.updateRoles(resourceId, changes, function(err) {
                ...
                memberChangeInfosByResourceId[resourceId] = {
                    'changes': changes,
                    'roles': roles,
                    'members': members
                };
