 ...
 */
var routeActivity = (module.exports.routeActivity = function(
  activitySeed,
  callback,
) {
  callback =
    ...
    function(err) {
      if (err) {
        log().error(
          { err: err, activitySeed: activitySeed },
  ...
  _produceAllEntities(activitySeed, function(err, actor, object, target) {
    ...
    _produceAllRoutes(activitySeed, actor, object, target, function(
      err,
      routes,
    ) {
      if (err) {
      ...
      _.each(routes, function(route) {
        ...
        allRoutedActivities[route.resourceId] =
          allRoutedActivities[route.resourceId] || {};
        ...
        var stream = ActivityRegistry.getRegisteredActivityStreamType(
          route.streamType,
        );
        if (stream && stream.visibilityBucketing) {
          ...
          if (_canRouteToPublicStream(actor, object, target)) {
            ...
            var publicGroupEntity = _getGroupEntity(actor, object, target);
          ...
          if (_canRouteToLoggedinStream(actor, object, target)) {
            ...
            var loggedinGroupEntity = _getGroupEntity(actor, object, target);
      ...
      log().trace(
        {
          ...
          allRoutedActivities: allRoutedActivities,
        },
        'Finished routed activities',
      );

      // Queue the activity entities to be collected and aggregated
      _queueActivities(activitySeed, allDeliveredActivities, function(
        err,
        buckets,
      ) {
        if (err) {
        ...
        ActivityEmitter.emit(
          ActivityConstants.events.ROUTED_ACTIVITIES,
          allRoutedActivities,
        );
 ...
 * @api private
 */
var _produceAllRoutes = function(
  activitySeed,
  actor,
  object,
  target,
  ...
  var associationsSession = new ActivityModel.AssociationsSession(
    ActivityRegistry.getRegisteredActivityEntityAssociations(),
    actor,
    object,
    target,
  );
  var activityConfig = ActivityRegistry.getRegisteredActivityTypes()[
    activitySeed.activityType
  ];
  ...
  var actorRouteConfig = _getRouterConfigForObjectType(
    activityConfig,
    ActivityConstants.entityTypes.ACTOR,
  );
  var objectRouteConfig = _getRouterConfigForObjectType(
    activityConfig,
    ActivityConstants.entityTypes.OBJECT,
  );
  var targetRouteConfig = _getRouterConfigForObjectType(
    activityConfig,
  ...
  _produceRoutes(associationsContexts.actor, actor, actorRouteConfig, function(
    err,
    actorRoutes,
  ) {
    if (err) {
    ...
    _produceRoutes(
      ...
      function(err, objectRoutes) {
        ...
        _produceRoutes(
          ...
          target,
          targetRouteConfig,
          function(err, targetRoutes) {
            if (err) {
              return callback(err);
            }

            // Determine the propagation rules for the actor
            _producePropagation(associationsContexts.actor, actor, function(
              err,
              ...
              _producePropagation(associationsContexts.object, object, function(
                err,
                ...
                _producePropagation(
                  associationsContexts.target,
                  target,
                  function(err, targetPropagationRules) {
                    if (err) {
                      return callback(err);
                    }
                    ...
                    var allRoutes = _.chain(actorRoutes)
                      .union(objectRoutes, targetRoutes)
                      .compact()
                      .filter(function(route) {
                        if (
                          route.streamType ===
                            ActivityConstants.streams.NOTIFICATION ||
 ...
 * @api private
 */
var _canRouteToPublicStream = function(actor, object, target) {
  return (
 ...
 * @api private
 */
var _canRouteToLoggedinStream = function(actor, object, target) {
  return (
    (actor['oae:visibility'] === 'public' ||
      actor['oae:visibility'] === 'loggedin') &&
    (!object ||
      object['oae:visibility'] === 'public' ||
      object['oae:visibility'] === 'loggedin') &&
    (!target ||
      target['oae:visibility'] === 'public' ||
      target['oae:visibility'] === 'loggedin')
  );
};
 ...
 * @api private
 */
var _getGroupEntity = function(actor, object, target) {
  if (object && AuthzUtil.isGroupId(object.id)) {
    return object;
  } else if (target && AuthzUtil.isGroupId(target.id)) {
    return target;
  }
  return null;
};
 ...
 * @api private
 */
var _applyPropagations = function(
  objectType,
  propagations,
  routes,
  associationsContexts,
  ...
  _applyPropagation(
    ...
    entity,
    entityRoutes,
    function(err, includeRoutes, excludeRoutes) {
      if (err) {
        log().warn(
          { err: err, propagation: propagation, entity: entity },
 ...
 * @api private
 */
var _applyPropagation = function(
  objectType,
  propagation,
  routes,
  associationsContexts,
  ...
  /*!
     ...
     */
  var _applyAssociationsCtx = function(
    associationsCtx,
    associationName,
    callback,
  ) {
    associationsCtx.get(associationName, function(err, associations) {
      if (err) {
        log().warn(
          { err: err, association: associationName },
      ...
      _.each(associations, function(associationRoute) {
  ...
  if (propagation.type === ActivityConstants.entityPropagation.ALL) {
    ...
    _.each(routes, function(route) {
      var routeTenantAlias = AuthzUtil.getResourceFromId(route.resourceId)
        .tenantAlias;
    ...
    propagation.type === ActivityConstants.entityPropagation.INTERACTING_TENANTS
  ) {
    ...
    _.each(entityRoutes, function(entityRoute) {
    ...
    _applyAssociationsCtx(associationsContexts[objectType], 'self', function(
      err,
    ) {
      if (err) {
        return callback(err);
    ...
    propagation.type === ActivityConstants.entityPropagation.ASSOCIATION
  ) {
    // Apply the specified association of the item
    _applyAssociationsCtx(
      associationsContexts[objectType],
    ...
    propagation.type ===
    ActivityConstants.entityPropagation.EXTERNAL_ASSOCIATION
  ) {
    /*!
    ...
    return callback(
      new Error(
        util.format('Received invalid propagation type "%s"', propagation.type),
      ),
    );
  }
};
...
var _producePropagation = function(associationsCtx, entity, callback) {
  ...
  var entityTypeConfig = ActivityRegistry.getRegisteredActivityEntityTypes()[
    entity.objectType
  ];
  if (!entityTypeConfig || !_.isFunction(entityTypeConfig.propagation)) {
    return callback(null, [
      { type: ActivityConstants.entityPropagation.ROUTES },
    ]);
  }
...
var _produceRoutes = function(associationsCtx, entity, routeConfig, callback) {
  ...
  var routeStreamAssociations = function() {
    ...
    _routeAssociations(
      associationsCtx,
      stream.associationNames.slice(),
      function(err, streamRoutes) {
        if (err) {
          return callback(err);
        }
 ...
 * @api private
 */
var _routeAssociations = function(
  associationsCtx,
  associationNames,
  callback,
  _routes,
...
var _getBucketNumber = function(route, activitySeed) {
  ...
  return ActivityBuckets.getBucketNumber(
    route + activitySeed.activityType,
    numberOfBuckets,
  );
};
