...
OAE.tenantRouter.on('post', '/api/user/create', function(req, res) {
    ...
    // Although technically the `email` field is optionaly when creating a user account,
    // it has to be there when signing up through the REST api
    if (!opts.email) {
        return res.send(400, 'An email address is required when creating an account');
    }
...
/**
 * @REST postUserGuestVerify
 *
 * Verify a guest token
 *
 * @Server      tenant
 * @Method      POST
 * @Path        /user/guest/verify
 * @FormParam   {string}        token                       The token to verify
 * @FormParam   {string}        username                    The username for the guest
 * @FormParam   {string}        password                    The password for the guest
 * @HttpResponse                200                         The token was valid and the login credentials were associated with the user account
 * @HttpResponse                400                         A token, username or password was not specified
 * @HttpResponse                404                         The token was not associated with a user
 */
OAE.tenantRouter.on('post', '/api/user/guest/verify', function(req, res) {
    PrincipalsAPI.verifyGuestToken(req.ctx, req.body.token, req.body.username, req.body.password, function(err, user) {
        if (err) {
            return res.send(err.code, err.msg);
        }

        // If the user is anonymous we log them into the system
        if (!req.ctx.user()) {
            AuthenticationUtil.handleLogin('local', user, '/me', req, res);

        // Otherwise, we send a 200
        } else {
            res.send(200);
        }
    });
 ...
 * @HttpResponse                404                         The token was not associated with a user
 */
OAE.tenantRouter.on('post', '/api/user/email/verify', function(req, res) {
    PrincipalsAPI.verifyEmailToken(req.ctx, req.body.token, function(err, user) {
        if (err) {
            return res.send(err.code, err.msg);
        }
...
/**
 * @REST postUserIdEmailSend
 *
 * Send a new email token to a user
 *
 * @Server      admin,tenant
 * @Method      POST
 * @Path        /user/email/{userId}/trigger
...
OAE.globalAdminRouter.on('post', '/api/user/:userId/email/send', _handleResendEmailToken);
OAE.tenantRouter.on('post', '/api/user/:userId/email/send', _handleResendEmailToken);
