...
const TinCanConfig = require('oae-config').config('oae-tincanapi');
...
describe('TinCanAPI', () => {
  ...
  afterEach(callback => {
    ConfigTestUtil.updateConfigAndWait(
      camAdminRestContext,
      null,
      { 'oae-tincanapi/lrs/enabled': false },
      err => {
        assert.ok(!err);
        return callback();
      }
      ...
      ActivityTestsUtil.collectAndGetActivityStream(
        ...
        (err, activityStream) => {
          ...
          ConfigTestUtil.updateConfigAndWait(
            camAdminRestContext,
            null,
            { 'oae-tincanapi/lrs/enabled': true },
            err => {
              assert.ok(!err);
              ...
              onRequest = function(req) {
                ...
                _.each(req.body, (val, key) => {
                  ...
                  setTimeout(() => {
                    ...
                    assert.strictEqual(
                      val.object.definition.name['en-US'],
                      testLinks[val.object.id].displayName
                    );
                    assert.strictEqual(
                      val.object.definition.description['en-US'],
                      testLinks[val.object.id].description
                    );
                  }, 2000);
                });
        ...
        (err, activityStream) => {
          ...
          onRequest = function(req) {
            assert.fail(
              null,
              null,
              'No statements should be sent when LRS integration is disabled'
            );
          };
