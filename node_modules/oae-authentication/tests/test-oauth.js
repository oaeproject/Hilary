...
describe('Authentication', function() {
  ...
  describe('OAuth', function() {
    ...
    var _setupOAuth = function(callback) {
      TestsUtil.generateTestUsers(localAdminRestContext, 1, function(
        err,
        users,
        simong,
      ) {
    ...
    describe('Authentication', function() {
      ...
      it('verify access token check', function(callback) {
        var oauthRestContext = _generateOAuthRestContext(
          global.oaeTests.tenants.localhost.host,
          'InvalidAccessToken',
        );
                  ...
                  function(err) {
                    ...
                    RestAPI.Config.getTenantConfig(
                      ...
                      function(err, config) {
                        assert.ok(!err);
                        assert.strictEqual(
                          config['oae-authentication']['shibboleth']['enabled'],
                          true,
                        );
      ...
      it('verify the same access token gets issued when requesting an access token', function(callback) {
        _setupOAuth(function(simong, client, firstAccessToken) {
          ...
          oauth.getOAuthAccessToken(
            '',
            { grant_type: 'client_credentials' },
            function(err, newAccessToken, refreshToken, results) {
              assert.ok(!err);
                                ...
                                function(err, data) {
                                  ...
                                  RestAPI.OAuth.getClients(
                                    ...
                                    function(err, data) {
                                      ...
                                      var clientDisplayName = TestsUtil.generateRandomText(
                                        1,
                                      );
                                      RestAPI.OAuth.createClient(
                                        localAdminRestContext,
