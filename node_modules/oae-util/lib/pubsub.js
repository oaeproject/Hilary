 ...
 */
var connectClient = function(config) {
    // Open a socket.
    ...
    if (config.pass && config.pass !== '') {
        client.auth(config.pass, function(err, res) {
            if (err) {
                console.warn('Couldn\'t authenticate with redis.');
            }
        });
    }
    ...
    client.select(dbIndex, function(err, res) {
        if (err) {
            console.warn('Couldn\'t select the redis DB index {}', dbIndex);
        }
    });
...
module.exports.init = function(config) {
    ...
    if (redisManager === null) {
        ...
        redisManager = connectClient(config);
        redisSubscriber = connectClient(config);
        redisPublisher = connectClient(config);
