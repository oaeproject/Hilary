...
var log = require('oae-logger').logger('test-interact-search');
...
describe('Interact Search', function() {
    ...
    before(function(callback) {
        // Create the rest contexts with which we can perform requests
    ...
    it('verify resource types selection', function(callback) {

        TestsUtil.generateTestUsers(camAdminRestContext, 1, function(err, users, mrvisser) {
            assert.ok(!err);

            // Create one of each resource type: group, content, discussion
            RestAPI.Group.createGroup(mrvisser.restContext, 'group', 'group', 'public', 'yes', [], [], function(err, group) {
                assert.ok(!err);
                RestAPI.Content.createLink(mrvisser.restContext, 'link', 'link', 'public', 'http://google.ca', [], [], function(err, link) {
                    assert.ok(!err);
                    RestAPI.Discussions.createDiscussion(mrvisser.restContext, 'discussion', 'discussion', 'public', null, null, function(err, discussion) {
                        assert.ok(!err);

                        /*!
                         * Create the opts parameter for a specific resource type
                         *
                         * @param  {String}     [resourceType]  The resource type that should be returned. If unspecified, return all.
                         * @return {Object}                     The `opts` object for an interact search
                         */
                        var _getOpts = function(resourceType) {
                            return {
                                'q': '*',
                        ...
                        // Ensure only the group shows up for group resource type
                        SearchTestsUtil.searchAll(mrvisser.restContext, 'interact', null, _getOpts('group'), function(err, results) {
                            assert.ok(!err);
                            assert.ok(!_getDocById(results, mrvisser.user.id));
                            assert.ok(_getDocById(results, group.id));
                            assert.ok(!_getDocById(results, link.id));
                            assert.ok(!_getDocById(results, discussion.id));

                            // Ensure only the user shows up for user resource type
                            SearchTestsUtil.searchAll(mrvisser.restContext, 'interact', null, _getOpts('user'), function(err, results) {
                                assert.ok(!err);
                                assert.ok(_getDocById(results, mrvisser.user.id));
                                assert.ok(!_getDocById(results, group.id));
                                assert.ok(!_getDocById(results, link.id));
                                ...
                                // Ensure only the content item shows up for content resource type
                                SearchTestsUtil.searchAll(mrvisser.restContext, 'interact', null, _getOpts('content'), function(err, results) {
                                    assert.ok(!err);
                                    assert.ok(!_getDocById(results, mrvisser.user.id));
                                    assert.ok(!_getDocById(results, group.id));
                                    assert.ok(_getDocById(results, link.id));
                                    ...
                                    // Ensure only the discussion shows up for discussion resource type
                                    SearchTestsUtil.searchAll(mrvisser.restContext, 'interact', null, _getOpts('discussion'), function(err, results) {
                                        assert.ok(!err);
                                        assert.ok(!_getDocById(results, mrvisser.user.id));
                                        assert.ok(!_getDocById(results, group.id));
                                        assert.ok(!_getDocById(results, link.id));
                                        ...
                                        // Ensure all show up for no resource type
                                        SearchTestsUtil.searchAll(mrvisser.restContext, 'interact', null, _getOpts(), function(err, results) {
                                            assert.ok(!err);
                                            assert.ok(_getDocById(results, mrvisser.user.id));
                                            assert.ok(_getDocById(results, group.id));
                                            assert.ok(_getDocById(results, link.id));
     ...
     */
    it('verify private groups are not returned', function(callback) {

        /*!
         * Generate a random group display name for interact search test
         */
        var _generateGroupDisplayName = function() {
            return util.format('interact group %s', TestsUtil.generateRandomText(5));
        };

        TestsUtil.generateTestUsers(camAdminRestContext, 1, function(err, users) {
            assert.ok(!err);
            var user = _.values(users)[0];
            RestAPI.Group.createGroup(camAdminRestContext, _generateGroupDisplayName(), 'Private unjoinable group', 'private', 'no', [], [], function(err, privateUnJoinableGroup) {
                assert.ok(!err);
                RestAPI.Group.createGroup(camAdminRestContext, _generateGroupDisplayName(), 'Private joinable group', 'private', 'yes', [], [], function(err, privateJoinableGroup) {
                    assert.ok(!err);
                    RestAPI.Group.createGroup(user.restContext, _generateGroupDisplayName(), 'Private member group', 'private', 'yes', [], [], function(err, privateMemberGroup) {
                        assert.ok(!err);
                        RestAPI.Group.createGroup(camAdminRestContext, _generateGroupDisplayName(), 'Loggedin joinable group', 'public', 'yes', [], [], function(err, loggedinJoinableGroup) {
                            assert.ok(!err);
                            RestAPI.Group.createGroup(camAdminRestContext, _generateGroupDisplayName(), 'Public joinable group', 'public', 'yes', [], [], function(err, publicJoinableGroup) {
                                assert.ok(!err);
                                ...
                                SearchTestsUtil.searchAll(user.restContext, 'interact', null, opts, function(err, data) {
                                    assert.ok(!err);
                                    ...
                                    SearchTestsUtil.searchAll(camAdminRestContext, 'interact', null, opts, function(err, data) {
                                        assert.ok(!err);
    ...
    it('verify anonymous users cannot access the feed', function(callback) {
        RestAPI.Search.search(anonymousRestContext, 'interact', null, {'q': '*'}, function(err, data) {
            assert.ok(err);
            ...
            TestsUtil.generateTestGroups(creator.restContext, 3, function(err, groups) {
                ...
                TestsUtil.generateGroupHierarchy(creator.restContext, [topGroupId, intermediateGroupId, leafGroupId], 'member', function(err) {
                    ...
                    RestAPI.Group.setGroupMembers(creator.restContext, leafGroupId, memberRole, function(err) {
                        ...
                        SearchTestsUtil.searchAll(member.restContext, 'interact', null, opts, function(err, data) {
                            assert.ok(!err);
    ...
    it('verify tenant separation', function(callback) {
        TestsUtil.setupMultiTenantPrivacyEntities(function(publicTenant, publicTenant1, privateTenant, privateTenant1) {
            ...
            SearchTestsUtil.searchAll(publicTenant.publicUser.restContext, 'interact', null, opts, function(err, data) {
                assert.ok(!err);
                ...
                SearchTestsUtil.searchAll(privateTenant.publicUser.restContext, 'interact', null, opts, function(err, data) {
                    assert.ok(!err);
                    ...
                    SearchTestsUtil.searchAll(privateTenant.adminRestContext, 'interact', null, opts, function(err, data) {
                        assert.ok(!err);
