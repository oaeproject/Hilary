...
var TWO_DAYS_IN_MS = 2 * 24 * ONE_HOUR_IN_MS;
...
ActivityEmitter.on(ActivityConstants.events.DELIVERED_ACTIVITIES, function(deliveredActivities) {
    var emailRecipientIds = _.chain(deliveredActivities)
        .keys()
        .filter(_isEmailRecipientId)
        .filter(function(emailRecipientId) {
    ...
    _getEmailRecipientResources(emailRecipientIds, function(err, recipients) {
        if (err) {
            scheduledEmailsCounter.decr();
            return log().error({'err': err, 'emailRecipientIds': usersToMail}, 'Failed to get the email preference field for all the users in this activity');
        }

        // Filter out recipients that should not get emails
        var recipientsToQueue = _.filter(recipients, function(emailRecipient) {
            // Users without a valid email address or who've opted out of email notifications should not get emails
            return (!emailRecipient.deleted && emailRecipient.email && emailRecipient.emailPreference !== PrincipalsConstants.emailPreferences.NEVER);
        });

        var emailBuckets = {};
        _.each(recipientsToQueue, function(recipient) {
            var bucketId = _createEmailBucketIdForRecipient(recipient);
        ...
        ActivityDAO.saveQueuedUserIdsForEmail(emailBuckets, function(err) {
            ...
            log().trace({'recipientsToQueue': recipientsToQueue}, 'Queued mail for users');
 ...
 */
PrincipalsEmitter.on(PrincipalsConstants.events.UPDATED_USER, function(ctx, newUser, oldUser) {
    ...
    if (newUser.emailPreference === oldUser.emailPreference || oldUser.emailPreference === PrincipalsConstants.emailPreferences.NEVER) {
        return ActivityEmitter.emit(ActivityConstants.events.UPDATED_USER, ctx, newUser, oldUser);
    }

    // Take the user out of the old bucket
    var oldBucketId = _createEmailBucketIdForRecipient(oldUser);
    ActivityDAO.unqueueUsersForEmail(oldBucketId, [oldUser.id], function(err) {
        ...
        var newBucketId = _createEmailBucketIdForRecipient(newUser);
        var emailBucket = {};
...
var collectAllBuckets = module.exports.collectAllBuckets = function(callback) {
    ...
    var collectionExpiry = ActivitySystemConfig.getConfig().mail.pollingFrequency;
...
var _collectDailyBucket = function(bucketNumber, callback) {
    if (_isDailyCycle()) {
        ...
        var dateWithAnHour = new TZ.Date(now.getTime() + (60 * 60 * 1000), 'UTC');
        return collectMails(bucketNumber, PrincipalsConstants.emailPreferences.DAILY, null, dateWithAnHour.getHours(), callback);
    } else {
...
var _collectWeeklyBucket = function(bucketNumber, callback) {
    if (_isWeeklyCycle()) {
        var now = new TZ.Date('UTC');
        var dateWithAnHour = new TZ.Date(now.getTime() + (60 * 60 * 1000), 'UTC');
        return collectMails(bucketNumber, PrincipalsConstants.emailPreferences.WEEKLY, dateWithAnHour.getDay(), dateWithAnHour.getHours(), callback);
    } else {
        return callback(null, true);
    }
};
...
var collectMails = module.exports.collectMails = function(bucketNumber, emailPreference, dayOfWeek, hour, callback) {
    ...
    var oldestActivityTimestamp = _getCollectionCycleStart(emailPreference);
    _collectMails(bucketId, oldestActivityTimestamp, null, function(err, users) {
        if (err) {
            return callback(err);
        }

        Telemetry.incr(util.format('sent.%s.count', emailPreference), users.length);
 ...
 * @param  {Function}   callback                    Standard callback function
 * @param  {User}       callback.recipients         The recipients that received an email
 * @api private
 */
var _collectMails = function(bucketId, oldestActivityTimestamp, start, callback, _collectedRecipients) {
    ...
    ActivityDAO.getQueuedUserIdsForEmail(bucketId, start, MAX_COLLECTION_BATCH_SIZE, function(err, recipientIds, nextToken) {
        if (err) {
            return callback(err);
        ...
        var recipientIdsByActivityStreamIds = _.chain(recipientIds)
        ...
        ActivityDAO.getActivitiesFromStreams(_.keys(recipientIdsByActivityStreamIds), oldestActivityTimestamp, function(err, activitiesPerStream) {
            if (err) {
                return callback(err);
            }

            // Will hold the activities (keyed per stream) who have no activities within the grace period
            var activitiesPerMailableStreams = {};

            // Will hold the user IDs that should receive an email during this collection phase
            var recipientIdsToMail = [];

            // 3. Filter the activity streams to those who have no activities within the grace period
            var threshold = Date.now() - (ActivitySystemConfig.getConfig().mail.gracePeriod * 1000);
            _.each(activitiesPerStream, function(activities, activityStreamId) {
                var hasRecentActivity = _.find(activities, function(activity) {
            ...
            ActivityDAO.resetAggregationForActivityStreams(_.keys(activitiesPerMailableStreams), function(err) {
                if (err) {
                    return callback(err);
                }

                // 5. Remove the users we'll email from the buckets
                ActivityDAO.unqueueUsersForEmail(bucketId, recipientIdsToMail, function(err) {
                    if (err) {
                        return callback(err);
                    }
                    ...
                    _.each(activitiesPerMailableStreams, function(activities, activityStreamId) {
                    ...
                    ActivityDAO.deleteActivities(activitiesPerStreamToDelete, function(err) {
                        ...
                        _getEmailRecipientResources(recipientIdsToMail, function(err, recipients) {
                            if (err) {
                                log().error({'err': err, 'recipientIds': recipientIdsToMail}, 'Failed to get the recipients when sending email');
                                return callback(err);
                            }

                            // Keep track of whom we need to email
                            var recipientsToMail = _.filter(recipients, function(recipient) {
                                ...
                                var emailActivityStreamId = ActivityUtil.createActivityStreamId(recipient.id, ActivityConstants.streams.EMAIL);
                                return (recipient.emailPreference !== PrincipalsConstants.emailPreferences.NEVER && !_.isEmpty(activitiesPerMailableStreams[emailActivityStreamId]));
                            });

                            // Transform the recipients array so the activities are included
                            var toMail = _.map(recipientsToMail, function(recipient) {
                                var emailActivityStreamId = ActivityUtil.createActivityStreamId(recipient.id, ActivityConstants.streams.EMAIL);
                                return {'recipient': recipient, 'activities': activitiesPerMailableStreams[emailActivityStreamId]};
                            });
...
var _mailAll = function(toMail, callback) {
    ...
    var to = toMail.pop();
    _mail(to.recipient, to.activities, function(err) {
        if (err) {
 ...
 * @api private
 */
var _mail = function(recipient, activities, callback) {
    ...
    var aggregatedActivities = _aggregate(recipient.id, activities);
    ...
    var timezone = TenantConfig.getValue(recipient.tenant.alias, 'timezone', 'timezone');
    var ctx = new Context(tenant, recipient);
    ActivityTransformer.transformActivities(ctx, aggregatedActivities, ActivityConstants.transformerTypes.ACTIVITYSTREAMS, function(err) {
        ...
        if (AuthzUtil.isEmail(recipient.id)) {
            var invitationTokenParam = encodeURIComponent(recipient.token);
            var invitationEmailParam = encodeURIComponent(recipient.email);
            var signupRedirectUrl = util.format('/me?invitationToken=%s&invitationEmail=%s',
                invitationTokenParam, invitationEmailParam);
            var invitationUrl = util.format('%s/signup?url=%s', baseUrl, encodeURIComponent(signupRedirectUrl));
        }
        ...
        var adaptedActivities = ActivityAdapter.adapt(recipient.id, recipient, aggregatedActivities, Sanitization, {
        ...
        var emailHash = util.format('%s#', recipient.id);
...
var _getEmailRecipientResources = function(emailRecipientIds, callback) {
    var recipientIdsPartitioned = _.partition(emailRecipientIds, AuthzUtil.isUserId);
    var userIds = _.first(recipientIdsPartitioned);
    var emails = _.last(recipientIdsPartitioned);

    OaeUtil.invokeIfNecessary(!_.isEmpty(emails), AuthzInvitationsDAO.getTokensByEmails, emails, function(err, tokensByEmail) {
        if (err) {
            return callback(err);
        }

        var emailResources = _.map(emails, function(email) {
            return {
                'id': email,
...
var _isEmailRecipientId = function(resourceId) {
    return (AuthzUtil.isUserId(resourceId) || AuthzUtil.isEmail(resourceId));
};
 ...
 * @api private
 */
var _createEmailBucketIdForRecipient = function(recipient) {
    var timezone = TenantConfig.getValue(recipient.tenant.alias, 'timezone', 'timezone');
    var bucketNumber = _getBucketNumber(recipient.id);
 ...
 * @api private
 */
var _getCollectionCycleStart = function(emailPreference) {
