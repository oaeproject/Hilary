...
var LibraryAPI = reequire('oae-library');
...
var invokeGroupDeleteHandlers = module.exports.invokeGroupDeleteHandlers = function(group) {
    return _invokeGroupHandlers(groupDeleteLog, _groupDeleteHandlers, group);
};
...
var invokeGroupRestoreHandlers = module.exports.invokeGroupRestoreHandlers = function(group) {
    return _invokeGroupHandlers(groupRestoreLog, _groupRestoreHandlers, group);
};
...
PrincipalsAPI.on(PrincipalsConstants.events.DELETED_GROUP, function(ctx, group) {
    ...
    return invokeGroupDeleteHandlers(group);
});
...
PrincipalsAPI.on(PrincipalsConstants.events.RESTORED_GROUP, function(ctx, group) {
    ...
    return invokeGroupRestoreHandlers(group);
});
 ...
 * deleted
 */
registerGroupDeleteHandler('memberships-library', function(group, membershipsGraph, membersGraph, callback) {
    var userIdsToInvalidate = _.chain(membersGraph.getNodes()).filter(PrincipalsUtil.isUser).value();
    return _invalidateMembershipLibraries(userIdsToInvalidate, callback);
});

/*!
 * Register a restore handler that purges all memberships libraries containing a group that is
 * being restored
 */
registerGroupRestoreHandler('memberships-library', function(group, membershipsGraph, membersGraph, callback) {
    var userIdsToInvalidate = _.chain(membersGraph.getNodes()).filter(PrincipalsUtil.isUser).value();
    return _invalidateMembershipLibraries(userIdsToInvalidate, callback);
});

/**
 * Generic invalidation method to handle invalidation of group membership information when a group
 * is deleted or restored
 *
 * @param  {Group}          group               The group that was deleted
 * @param  {AuthzGraph}     membershipsGraph    The graph of group memberships of the deleted group
 * @param  {AuthzGraph}     membersGraph        The graph of group members of the deleted group
 * @param  {Function}       callback            Standard callback function
 * @param  {Object[]}       callback.errs       All errs that occurred while trying to invalidate the group membership components
 */
var _handleInvalidateGroupOperation = function(group, membershipsGraph, membersGraph, callback) {
    var userIdsToInvalidate = _.chain(membersGraph.getNodes()).filter(PrincipalsUtil.isUser).value();

    // Invalidate the membership libraries of all users whose memberships are impacted by this
    var allErrs = null;
    _invalidateMembershipsLibraries(userIdsToInvalidate, function(errs) {
        if (errs) {
            allErrs = _.union(allErrs, errs);
        }

        // Rebuild the search documents that are impacted by the deletion of this group
        _invalidateSearch(group, membershipsGraph, membersGraph, function(errs) {
            if (errs) {
                allErrs = _.union(allErrs, errs);
            }
...
/**
 * Convenience function to invalidate the memberships libraries of all user ids that are specified
 *
 * @param  {String[]}   userIds         The ids of the users whose memberships libraries to purge
 * @param  {Function}   callback        Standard callback function
 * @param  {Object[]}   callback.errs   All errors that occurred, if any
 */
var _invalidateMembershipsLibraries = function(userIds, callback, _errs, _userIds) {
    _userIds = _userIds || userIds.slice();
    ...
    LibraryAPI.Index.purge(PrincipalsConstants.library.MEMBERSHIPS_INDEX_NAME, userId, function(err) {
        ...
        return _invalidateMembershipsLibraries(userIds, callback, _errs, _userIds);
    });
};

/**
 * Convenience function to invoke the search tasks required to invalidate search documents necessary
 * on the basis that the given `group` was either deleted or restored
 *
 * @param  {Group}          group               The group that was deleted
 * @param  {AuthzGraph}     membershipsGraph    The graph of group memberships of the deleted group
 * @param  {AuthzGraph}     membersGraph        The graph of group members of the deleted group
 ...
 */
var _invalidateSearch = function(group, membershipsGraph, membersGraph, callback) {
    // All members (direct and indirect) of the group that was deleted (groups and users) need to
    // have their memberships search documents invalidated
    var groupAndUserIds =
        _.chain(membersGraph.getNodes())
            ...
            .value();
    var groupIds = groupAndUserIds[0];
    var userIds = groupAndUserIds[1];

    // Create the index tasks that will tell search which resource's memberships document to update
    var groupIndexTasks = _.map(groupIds, function(groupId) {
        return {'id': groupId};
    });

    var userIndexTasks = _.map(userIds, function(userId) {
        return {'id': userId};
    });
    ...
    // Finally, invoke the group and user memberships update tasks
    var allErrs = null;
    SearchAPI.postIndexTask('group', groupIndexTasks, membershipsIndexOp, function(err) {
        if (err) {
            allErrors = _.union(allErrs, [err]);
        }

        SearchAPI.postIndexTask('user', userIndexTasks, membershipsIndexOp, function(err) {
            if (err) {
                allErrors = _.union(allErrors, [err]);
            }
...
var _invokeGroupHandlers = function(log, handlers, group) {
    ...
    AuthzAPI.getPrincipalMembershipsGraph(group.id, function(err, membershipsGraph) {
        if (err) {
            return groupDeleteLog().error({'err': err, 'groupId': group.id}, 'An unexpected error occurred while getting the authz memberships graph');
        }

        AuthzAPI.getAuthzMembersGraph([group.id], function(err, membersGraph) {
            if (err) {
                return groupDeleteLog().error({'err': err, 'groupId': group.id}, 'An unexpected error occurred while getting the authz members graph');
            }

            return _invokeHandlers(log, handlers, group, membershipsGraph, membersGraph);
        });
    });
};
...
var _invokeHandlers = function(log, handlers, principal /*, args... */) {
    ...
    _.each(handlers, function(handler, name) {
        ...
        var thisHandlerArgs = handlerArgs.slice();
        thisHandlerArgs.push(function(errs) {
            if (!_.isEmpty(errs)) {
