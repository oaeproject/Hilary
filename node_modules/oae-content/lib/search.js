...
var init = module.exports.init = function(callback) {
    return MessageBoxSearch.registerMessageSearchDocument('content_comment', function(resources, callback) {
...
SearchAPI.registerSearch('relatedcontent', require('./search/searches/relatedcontent'));
...
// Binds the local events to the task queue to assign indexing jobs

ContentAPI.on(ContentConstants.events.CREATED_CONTENT, function(ctx, content, revision, members) {
    TaskQueue.submit(SearchConstants.mq.TASK_INDEX_DOCUMENT, {
        'resources': [{
            'id': content.id,
            'index': {
                'resource': 'content',
...
ContentAPI.on(ContentConstants.events.DELETED_CONTENT, function(ctx, contentId) {
    TaskQueue.submit(SearchConstants.mq.TASK_DELETE_DOCUMENT, {'id': contentId});
});
...
ContentAPI.on(ContentConstants.events.DELETED_COMMENT, function(ctx, comment, content) {
    return MessageBoxSearch.deleteMessageSearchDocument('content_comment', content.id, comment);
});
...
var _produceContentCommentDocuments = function(resources, callback, _documents) {
    ...
    if (AuthzUtil.getResourceFromId(resource.id).resourceType !== 'c') {
        // Produce only children for content items
        return _produceContentCommentDocuments(resources, callback, _documents);
    } else if (resource.comments) {
        var documents = MessageBoxSearch.createMessageSearchDocuments('content_comment', resource.id, resource.comments);
    ...
    MessageBoxSearch.createAllMessageSearchDocuments('content_comment', resource.id, resource.id, function(err, documents) {
        if (err) {
...
var _transformContentDocuments = function(ctx, docs, callback) {
    ...
    _.each(docs, function(doc, docId) {
        var result = _.extend({}, doc.fields, {
            'id': doc['_id'],
            'lastModified': doc.fields._extra.lastModified
        });
        ...
        if (doc.fields.thumbnailUrl && result.lastModified) {
            result.thumbnailUrl = ContentUtil.getDownloadUrl(ctx, doc.fields.thumbnailUrl);
        }
...
SearchAPI.registerReindexAllHandler('content', function(callback) {
    ...
    var _onEach = function(contentRows, done) {
        ...
        TaskQueue.submit(SearchConstants.mq.TASK_INDEX_DOCUMENT, {'resources': contentResources});
