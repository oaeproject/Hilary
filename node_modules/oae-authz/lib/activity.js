...
ActivityAPI.registerActivityEntityType('email', {
  transformer: {
    activitystreams: function(ctx, activityEntities, callback) {
      ...
      var transformedEntities = _.mapObject(activityEntities, function(
        entities,
        activityId,
      ) {
 ...
 */
ActivityAPI.registerActivityEntityAssociation('email', 'self', function(
  associationsCtx,
  entity,
  callback,
) {
 ...
 * was invited into
 */
ResourceActions.on(ResourceConstants.events.INVITED, function(
  ctx,
  invitations,
  emailTokens,
) {
  ...
  _.chain(invitations)
    .map(function(invitation) {
      ...
      var targetResource = ActivityModel.ActivitySeedResource.fromResource(
        invitation.resource,
      );
