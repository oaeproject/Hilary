...
describe('Preview processor', function() {
    ...
    describe('Preview generation', function() {
        ...
        before(function(callback) {
            ...
            // Connect too the queue, but DO NOT accept any tasks.
            var options = {
                'subscribe': {
                    'subscribe': false,
                    'prefetchCount': 1
            ...
            MQ.getOrCreateQueue(PreviewConstants.MQ.TASK_GENERATE_PREVIEWS, {}, function(queue) {

                // Now that we're bound to the queue.. purge it.
                queue.purge().addCallback(function(ok) {
                    if (!ok) {
                        return callback(new Error('Error purging queue.'));
                    }
                    ...
                    PreviewAPI.enable(function(err) {
                        ...
                        MQTestUtil.whenTasksEmpty(PreviewConstants.MQ.TASK_GENERATE_PREVIEWS, callback);
                    });
                });
            });
        });
            ...
            MQTestUtil.whenTasksEmpty(PreviewConstants.MQ.TASK_GENERATE_PREVIEWS, function() {
                TestsUtil.generateTestUsers(signedAdminRestContext, 1, function(err, response) {
                    ...
                    if (type === 'file') {
                        ...
                        RestAPI.Content.createCollabDoc(restCtx, 'Test document', 'Test document', 'private', [], [], contentCreated);
                    }
                });
            });
        };
        ...
        /**
         * Verifies the collaboration document processor works.
         */
        it('verify collaboration document processing works', function(callback) {
            // Ignore this test if the PP is disabled.
            if (!defaultConfig.previews.enabled) {
                return callback();
            }
