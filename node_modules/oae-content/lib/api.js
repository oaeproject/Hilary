...
var _getFullContentProfile = function(ctx, contentObj, isManager, callback) {
    ...
    /**
     * Continue retrieving the full content profile after membership has been determined
     */
    var continueGetFullContentProfile = function() {
    ...
    if (isManager) {
        ...
        continueGetFullContentProfile();
    // If not a manager do an extra check to see if the user is a member
    } else {
        _isMember(ctx, contentObj, function(err, isMember) {
            if (err) {
                return callback(err);
            }
