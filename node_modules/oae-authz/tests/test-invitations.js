...
describe('Invitations', function() {
    ...
    var _assertAdaptedActivities = function(inviterUserInfo, invitedUserInfo, otherUserInfo, resources, assertions, callback) {
        ...
        EmailTestUtil.collectAndFetchAllEmails(function() {
            ActivityTestUtil.collectAndGetActivityStream(inviterUserInfo.restContext, inviterUserInfo.user.id, null, function(err, result) {
                ...
                inviterUserInfo = _.extend({}, inviterUserInfo, {'summary': _getTranslatedSummaryForUserInfo(inviterUserInfo, activity)});
                invitedUserInfo = _.extend({}, invitedUserInfo, {'summary': _getTranslatedSummaryForUserInfo(invitedUserInfo, activity)});
                otherUserInfo = _.extend({}, otherUserInfo, {'summary': _getTranslatedSummaryForUserInfo(otherUserInfo, activity)});
    ...
    var _assertStandardInvitationAcceptSummaries = function(inviterUserInfo, invitedUserInfo, otherUserInfo, resources) {
    ...
    var _getTranslatedSummaryForUserInfo = function(userInfo, activity) {
        var adaptedSummary = _adapt(userInfo, activity)[0].summary;
        return UIAPI.translate(adaptedSummary.i18nKey, null, adaptedSummary.i18nArguments);
    };

    var _adapt = function(userInfo, activity) {
        var adapter = UIAPI.getActivityAdapter();
        return adapter.adapt(userInfo.user.id, userInfo.user, [clone(activity)], Sanitization);
    };
