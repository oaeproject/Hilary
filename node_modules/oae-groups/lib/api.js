    ...
    module.exports.getGroup(group_id, function(err, group) {
        ...
        OAE.runQuery('SELECT * FROM Principals WHERE principal_id IN (?)', [principalsToAdd], function (err, rows) {
            ...
            for (var j = 0; j < rows.rowCount();j++) {
                if (rows[j]._colCount === 1) {
                    // This is an invalid principal!
                    requestPrincipals[rows[j].colHash.principal_id] = false;
                    all_valid = false;
                }
            }
            ...
            OAE.runQuery('SELECT * FROM MemberOf WHERE principal_id IN (?)', [principalsToAdd], function (err, rows) {
                ...
                OAE.runQuery('SELECT * FROM MemberOf WHERE principal_id  = ?', [group_id], function (err, rows) {
                    ...
                    var q = [];
                    var parameters = [];
                    for (var k = 0; k < principalsToAdd.length;k++) {
                        q.push("?=?");
                        parameters.push(principalsToAdd[k]);
                        parameters.push("");
                    }
                    ...
                    OAE.runQuery("UPDATE GroupMembers SET " + q.join(', ') + " WHERE group_id = ?", parameters, function(err) {
                        ...
                        explodePrincipals(principalsToAdd, false, function(err, users) {
                            ...
                            OAE.runBatchQuery(queries, function(err) {
                                if (err) {
                                    return callback({'code': 500, 'msg': err}, null);
                                }
 ...
 * @param  {String}   title       The title for this group. This will be stemmed down to something that we can use in Cassandra and prefixed with g:tenant.id:
...
module.exports.createGroup = function(tenant, title, description, callback) {
    ...
    module.exports.getGroup(id, function(err, group) {
        ...
        // Create the group.
        OAE.runQuery('INSERT INTO Principals (principal_id, tenant, group_title, group_description) VALUES (?, ?, ?, ?)', [id, tenant.id, title, description], function (err) {
            if (err) {
