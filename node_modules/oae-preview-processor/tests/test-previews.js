...
describe('Preview processor', function() {
    ...
    describe('Preview Reprocessing', function() {
        ...
        /**
         * Test that verifies that a push notification for content updates include the right previews object
         */
        it('verify content update push notifications include the right previews object', function(callback) {
            // Ignore this test if the PP is disabled
            if (!defaultConfig.previews.enabled) {
                return callback();
            }
            ...
            TestsUtil.generateTestUsers(signedAdminRestContext, 1, function(err, users, mrvisser) {
                ...
                RestAPI.User.getMe(mrvisser.restContext, function(err, mrvisserFullMeData) {
                    ...
                    // Ensure everything is cleared out of the PP queue before we enable the PP
                    PreviewTestUtil.purgePreviewsQueue(function() {
                        PreviewAPI.enable(function(err) {
                            assert.ok(!err);

                            // Create a link that we can update
                            RestAPI.Content.createLink(mrvisser.restContext, 'Apereo', 'Apereo', 'private', 'http://www.apereo.org',  [], [], function(err, contentObj) {
                                assert.ok(!err);

                                // Wait untill we're done generating a preview for this link
                                MQTestUtil.whenTasksEmpty(PreviewConstants.MQ.TASK_GENERATE_PREVIEWS, function() {
                                    ...
                                    RestAPI.Content.getContent(mrvisser.restContext, contentObj.id, function(err, contentObj) {
                                        ...
                                        ActivityTestsUtil.getFullySetupPushClient(data, function(client) {
                                            ...
                                            // We don't need the preview processor any further
                                            PreviewAPI.disable(function(err) {
                                                assert.ok(!err);

                                                // Update the link
                                                RestAPI.Content.updateContent(mrvisser.restContext, contentObj.id, {'displayName': 'Yahoo!', 'link': 'http://www.yahoo.com', 'description': 'Yahoo Portal'}, function(err, updatedLink) {
                                                    assert.ok(!err);
                                                });

                                                client.on('message', function(message) {
                                                    if (message.activity && message.activity['oae:activityType'] === 'content-update') {
                                                        // Assert the previews status is set back to pending
                                                        assert.strictEqual(message.activity.object.previews.status, 'pending');

                                                        // Assert the new metadata is correct as well
                                                        assert.strictEqual(message.activity.object.displayName, 'Yahoo!');
                                                        assert.strictEqual(message.activity.object.description, 'Yahoo Portal');
                                                        return callback();
                                                    }
                                                });
                                            });
