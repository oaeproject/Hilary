 ...
 */
var isEmail = (module.exports.isEmail = AuthzUtil.isEmail);
 ...
 */
var getEmailSubject = (module.exports.getEmailSubject = function(
  util,
  recipient,
  activities,
) {
  if (isEmail(recipient.id)) {
    ...
    var actors = _getAllEntities(activities, 'actor');
    if (_.size(actors) === 1) {
      return util.i18n.translate(
        '__MSG__ACTIVITY_EMAIL_SUBJECT_INVITE_ACTOR_1__',
        {
          actor1DisplayName: actors[0].displayName,
        },
      );
    } else if (_.size(actors) === 2) {
      return util.i18n.translate(
        '__MSG__ACTIVITY_EMAIL_SUBJECT_INVITE_ACTOR_2__',
        {
          actor1DisplayName: actors[0].displayName,
      ...
      return util.i18n.translate(
        '__MSG__ACTIVITY_EMAIL_SUBJECT_INVITE_ACTOR_3+__',
        {
          actor1DisplayName: actors[0].displayName,
    ...
    var emailPreference = recipient.emailPreference;
    ...
    if (emailPreference === 'immediate') {
      if (activities.length === 1) {
        ...
        message = util.i18n.translate(
          '__MSG__ACTIVITY_EMAIL_SUBJECT_MULTIPLE__',
        );
      }
 ...
 */
var getEmailSummary = (module.exports.getEmailSummary = function(
  util,
  recipient,
  activities,
  baseUrl,
  ...
  if (isEmail(recipient.id)) {
    ...
    if (emailPreference === 'immediate') {
      ...
      _.each(activities, function(activity) {
        if (
          actor !== null &&
          actor['oae:id'] !== activity.originalActivity.actor['oae:id']
        ) {
          isSingleActor = false;
        }
      ...
      if (isSingleActor && actor.objectType !== 'collection') {
        var key =
          '__MSG__ACTIVITY_EMAIL_SUMMARY_IMMEDIATE_SINGLE_ACTOR_SINGLE_ACTIVITY__';
        if (activities.length > 1) {
          key =
            '__MSG__ACTIVITY_EMAIL_SUMMARY_IMMEDIATE_SINGLE_ACTOR_MULTIPLE_ACTIVITIES__';
        }
        ...
        var url = util.url.ensureAbsoluteLink(
          actor['oae:profilePath'],
          baseUrl,
        );
        var actorLink =
          '<span>' + util.html.encodeForHTML(actor.displayName) + '</span>';
        if (url) {
          actorLink =
            '<a href="' +
            url +
            '">' +
            util.html.encodeForHTML(actor.displayName) +
            '</a>';
        }
 ...
 */
var _getAllEntities = function(activities, entityType) {
  var seenIds = {};
  var entities = [];
  _.each(activities, function(activity) {
    var entity = activity.originalActivity[entityType];

    // If the entity is a collection, collect all of them
    if (entity.objectType === 'collection') {
      _.each(entity[ActivityConstants.properties.OAE_COLLECTION], function(
        entity,
      ) {
