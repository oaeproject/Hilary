...
var _createContent = function(ctx, contentId, revisionId, resourceSubType, displayName, description, visibility, roles, folderIds, otherValues, revisionData, callback) {
    ...
    validator.check(null, {'code': 400, 'msg': 'The additional members should be specified as an object'}).isObject(roles);
    validator.check(null, {'code': 401, 'msg': 'You have to be logged in to be able to create a content item'}).isLoggedInUser(ctx);
    ...
    canManageFolders(ctx, folderIds, function(err, folders) {
        ...
        ResourceActions.create(ctx, roles, createFn, function(err, content, revision, changes, members) {
            if (err) {
                return callback(err);
            }

            _updateContentLibraries(content, [], function(err, content) {
                if (err) {
                    return callback(err);
                }

                ContentMembersLibrary.insert(content, members, function(err) {
                    if (err) {
                        log().warn({
                            'err': err,
                            'contentId': content.id,
                            'principalIds': _.pluck(members, 'id')
                    ...
                    _addContentItemToFolders(ctx, content, folders, function(err) {
                        ...
                        return callback(null, content, revision);
                    });
                });
            });
        });
...
var shareContent = module.exports.shareContent = function(ctx, contentId, principalIds, callback) {
    ...
    _.each(principalIds, function(principalId) {
        validator.check(principalId, {'code': 400, 'msg': 'An invalid user or group id has been specified'}).isPrincipalId();
    });
    ...
    ContentDAO.Content.getContent(contentId, function(err, content) {
        ...
        ResourceActions.share(ctx, content, principalIds, AuthzConstants.role.VIEWER, function(err, changes, roles, addedMembers) {
            if (err) {
                return callback(err);
            }
            ...
            _updateContentLibraries(content, [], function(err, content) {
                ...
                ContentMembersLibrary.insert(content, addedMembers, function(err) {
                    if (err) {
                        log().warn({
                            'err': err,
                            'contentId': content.id,
                    ...
                    ContentAPI.emit(ContentConstants.events.UPDATED_CONTENT_MEMBERS, ctx, content, changes, addedMemberIds, [], []);
                    return callback();
                });
            });
        });
...
var setContentPermissions = module.exports.setContentPermissions = function(ctx, contentId, changes, callback) {
    ...
    var validator = new Validator();
    validator.check(contentId, {'code': 400, 'msg': 'A content id must be provided'}).isResourceId();
    validator.check(_.keys(changes).length, {'code': 400, 'msg': 'You should specify at least 1 user/group to set content permissions on'}).min(1);
    validator.check(null, {'code': 401, 'msg': 'You have to be logged in to be able to create a content item'}).isLoggedInUser(ctx);
    if (validator.hasErrors()) {
        return callback(validator.getFirstError());
    }
    ...
    getContent(ctx, contentId, function(err, content) {
        ...
        _.each(changes, function(role, principalId) {
            if (role !== false) {
                validator.check(principalId, {'code': 400, 'msg': util.format('An invalid principal id has been specified: %s', principalId)}).isPrincipalId();
                validator.check(role, {'code': 400, 'msg': util.format('Invalid role "%s" specified. Must be one of %s', role, validRoles.join(', '))}).isIn(validRoles);
            }
        });
        if (validator.hasErrors()) {
        ...
        ResourceActions.setRoles(ctx, content, changes, function(err, changes, roles, members) {
            if (err) {
                return callback(err);
            }
            ...
            _updateContentLibraries(content, removedMemberIds, function(err, content) {
                if (err) {
                    return callback(err);
                }

                // If setting the content permissions results in any new members, we should insert them into the content members library
                ContentMembersLibrary.insert(content, members.added, function(err) {
                    if (err) {
                        log().warn({
                            'err': err,
                            'contentId': content.id,
                    ...
                    ContentMembersLibrary.remove(content, removedMemberIds, function(err) {
                        ...
                        ContentAPI.emit(ContentConstants.events.UPDATED_CONTENT_MEMBERS, ctx, content, changes, addedMemberIds, updatedMemberIds, removedMemberIds);
                        return callback();
                    });
                });
            });
...
var _updateContentLibraries = function(content, removedMemberIds, callback) {
    ContentDAO.Content.updateContentLibraries(content, removedMemberIds, function(err, newContent) {
        ...
        return callback(err, newContent || content);
    });
};
...
var removeContentFromLibrary = module.exports.removeContentFromLibrary = function(ctx, libraryOwnerId, contentId, callback) {
    ...
    ContentDAO.Content.getContent(contentId, function(err, content) {
        ...
        PrincipalsDAO.getPrincipal(libraryOwnerId, function(err, libraryOwner) {
            ...
            AuthzPermissions.canRemoveRole(ctx, libraryOwner, content, function(err, changes, roles) {
                if (err) {
                    return callback(err);
                }

                // All validation checks have passed, finally persist the role change and update the user library
                AuthzAPI.updateRoles(contentId, changes, function(err) {
                    if (err) {
                        return callback(err);
                    }
                    ...
                    LibraryAPI.Index.remove(ContentConstants.library.CONTENT_LIBRARY_INDEX_NAME, [entry], function(err) {
                        ...
                        ContentAPI.emit(ContentConstants.events.UPDATED_CONTENT_MEMBERS, ctx, content, changes, [], [], [libraryOwnerId]);
                        return callback();
                    });
                });
            });
...
var _getExistingPrincipals = function(principalIds, fields, callback) {
    PrincipalsDAO.getPrincipals(principalIds, fields, function(err, principals) {
        if (err) {
            return callback(err);
        } else if (_.keys(principals).length !== principalIds.length) {
            return callback({'code': 400, 'msg': 'One or more principals do not exist'});
        }
