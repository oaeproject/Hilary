...
var pushCountsToRedis = function() {
    ...
    for (var i = 0; i < modules.length; i++) {
        ...
        for (var j = 0; j < keys.length;j++) {
            ...
            if (counters[module][key] > 0) {
                totalItems++;
                redisTodo++;
                redisClient.hincrby('telemetry', module + '.' + key, counters[module][key], function(err, res) {
                    if (err) {
                    ...
                    redisTodo--;
                    if (redisTodo === 0) {
                        setTimeout(pushCountsToRedis, Config.telemetry.circonus.redisInterval);
                    }
                });
    ...
    // but it can happen on startup or during periods where absolutely nothing happens.
    if (totalItems === 0) {
        setTimeout(pushCountsToRedis, Config.telemetry.circonus.redisInterval);
    }
};
...
var pushHistogramsToCirconus = function() {
    ...
    if (histogramsToSend !== 0) {
        ...
        request({
            ...
            setTimeout(pushHistogramsToCirconus, Config.telemetry.circonus.circonusInterval);
        });
    } else {
        setTimeout(pushHistogramsToCirconus, Config.telemetry.circonus.circonusInterval);
    }
