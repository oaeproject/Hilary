...
describe('Utilities', function() {
    ...
    describe('Cassandra', function() {
        ...
        it('verify paged column query start and end', function(callback) {
            ...
            Cassandra.createColumnFamily('VerifyPagedColumnQueryStartAndEnd', 'CREATE COLUMNFAMILY VerifyPagedColumnQueryStartAndEnd (keyId text PRIMARY KEY)', function(err, created) {
                ...
                Cassandra.runQuery(query.query, query.parameters, function(err) {
                    ...
                    Cassandra.runPagedColumnQuery('VerifyPagedColumnQueryStartAndEnd', 'keyId', 'key', null, 8, {'end': 'a'}, function(err, columns, nextToken, startMatched) {
                        assert.ok(!err);
                        assert.ok(columns);
                        ...
                        Cassandra.runPagedColumnQuery('VerifyPagedColumnQueryStartAndEnd', 'keyId', 'key', 'a', 8, {'end': 'a'}, function(err, columns, nextToken, startMatched) {
                            assert.ok(!err);
                            assert.ok(columns);
                            ...
                            Cassandra.runPagedColumnQuery('VerifyPagedColumnQueryStartAndEnd', 'keyId', 'key', 'a', 8, {'end': 'b'}, function(err, columns, nextToken, startMatched) {
                                assert.ok(!err);
                                assert.ok(columns);
                                assert.strictEqual(startMatched, true);

                                assert.equal(columns.length, 1);
                                assert.equal(columns[0].name, 'b');
                                assert.ok(!columns[1]);

                                // Verify inclusive end works with bounded start, multiple results full page (forward)
                                Cassandra.runPagedColumnQuery('VerifyPagedColumnQueryStartAndEnd', 'keyId', 'key', null, 8, {'end': 'j'}, function(err, columns, nextToken, startMatched) {
                                    assert.ok(!err);
                                    assert.ok(columns);
                                    assert.strictEqual(startMatched, false);
                                    ...
                                    assert.equal(columns[2].name, 'c');
                                    assert.equal(columns[3].name, 'd');
                                    assert.equal(columns[4].name, 'e');
                                    assert.equal(columns[5].name, 'f');
                                    assert.equal(columns[6].name, 'g');
                                    assert.equal(columns[7].name, 'h');
                                    assert.equal(nextToken, 'h');
                                    assert.ok(!columns[8]);

                                    // Verify inclusive end works with bounded start, multple results partial page (forward)
                                    Cassandra.runPagedColumnQuery('VerifyPagedColumnQueryStartAndEnd', 'keyId', 'key', 'g', 8, {'end': 'j'}, function(err, columns, nextToken, startMatched) {
                                        assert.ok(!err);
                                        assert.ok(columns);
                                        assert.strictEqual(startMatched, true);

                                        assert.equal(columns.length, 3);
                                        assert.equal(columns[0].name, 'h');
                                        assert.equal(columns[1].name, 'i');
                                        assert.equal(columns[2].name, 'j');
                                        assert.ok(!nextToken);
                                        assert.ok(!columns[3]);


                                        // Verify inclusive end works with unbounded start (reversed)
                                        Cassandra.runPagedColumnQuery('VerifyPagedColumnQueryStartAndEnd', 'keyId', 'key', null, 8, {'reversed': true, 'end': 'm'}, function(err, columns, nextToken, startMatched) {
                                            assert.ok(!err);
                                            assert.ok(columns);
                                            assert.strictEqual(startMatched, false);

                                            assert.equal(columns.length, 1);
                                            assert.equal(columns[0].name, 'm');
                                            assert.ok(!columns[1]);

                                            // Verify start === end (reversed)
                                            Cassandra.runPagedColumnQuery('VerifyPagedColumnQueryStartAndEnd', 'keyId', 'key', 'm', 8, {'reversed': true, 'end': 'm'}, function(err, columns, nextToken, startMatched) {
                                                assert.ok(!err);
                                                assert.ok(columns);
                                                assert.strictEqual(startMatched, true);
                                                ...
                                                Cassandra.runPagedColumnQuery('VerifyPagedColumnQueryStartAndEnd', 'keyId', 'key', 'm', 8, {'reversed': true, 'end': 'l'}, function(err, columns, nextToken, startMatched) {
                                                    assert.ok(!err);
                                                    assert.ok(columns);
                                                    assert.strictEqual(startMatched, true);

                                                    assert.equal(columns.length, 1);
                                                    assert.equal(columns[0].name, 'l');
                                                    assert.ok(!columns[1]);

                                                    // Verify inclusive end works with bounded start, multiple results full page (forward)
                                                    Cassandra.runPagedColumnQuery('VerifyPagedColumnQueryStartAndEnd', 'keyId', 'key', null, 8, {'reversed': true, 'end': 'c'}, function(err, columns, nextToken, startMatched) {
                                                        assert.ok(!err);
                                                        assert.ok(columns);
                                                        assert.strictEqual(startMatched, false);
                                                        ...
                                                        assert.equal(columns.length, 8);
                                                        assert.equal(columns[0].name, 'm');
                                                        assert.equal(columns[1].name, 'l');
                                                        assert.equal(columns[2].name, 'k');
                                                        assert.equal(columns[3].name, 'j');
                                                        assert.equal(columns[4].name, 'i');
                                                        assert.equal(columns[5].name, 'h');
                                                        assert.equal(columns[6].name, 'g');
                                                        assert.equal(columns[7].name, 'f');
                                                        assert.equal(nextToken, 'f');
                                                        assert.ok(!columns[8]);

                                                        // Verify inclusive end works with bounded start, multple results partial page (forward)
                                                        Cassandra.runPagedColumnQuery('VerifyPagedColumnQueryStartAndEnd', 'keyId', 'key', 'f', 8, {'reversed': true, 'end': 'c'}, function(err, columns, nextToken, startMatched) {
                                                            assert.ok(!err);
                                                            assert.ok(columns);
                                                            assert.strictEqual(startMatched, true);

                                                            assert.equal(columns.length, 3);
                                                            assert.equal(columns[0].name, 'e');
                                                            assert.equal(columns[1].name, 'd');
                                                            assert.equal(columns[2].name, 'c');
                                                            assert.ok(!nextToken);
                                                            assert.ok(!columns[3]);
        ...
        it('verify paged column query paging forward and reverse', function(callback) {
            ...
            Cassandra.createColumnFamily('VerifyPagedColumnQueryPaging', 'CREATE COLUMNFAMILY VerifyPagedColumnQueryPaging (keyId text PRIMARY KEY)', function(err, created) {
                ...
                Cassandra.runQuery(query.query, query.parameters, function(err) {
                    ...
                    // Page by 8
                    Cassandra.runPagedColumnQuery('VerifyPagedColumnQueryPaging', 'keyId', 'key', null, 8, null, function(err, columns, nextToken, startMatched) {
                        assert.ok(!err);
                        assert.ok(columns);
                        assert.strictEqual(startMatched, false);
                        ...
                        // Start from 'h'
                        Cassandra.runPagedColumnQuery('VerifyPagedColumnQueryPaging', 'keyId', 'key', 'h', 8, null, function(err, columns, nextToken, startMatched) {
                            assert.ok(!err);
                            assert.ok(columns);
                            assert.strictEqual(startMatched, true);

                            assert.equal(columns.length, 5);
                            assert.equal(columns[0].name, 'i');
                            assert.equal(columns[1].name, 'j');
                            assert.equal(columns[2].name, 'k');
                            assert.equal(columns[3].name, 'l');
                            assert.equal(columns[4].name, 'm');
                            assert.ok(!nextToken);
                            assert.ok(!columns[5]);

                            // Start from 'm'
                            Cassandra.runPagedColumnQuery('VerifyPagedColumnQueryPaging', 'keyId', 'key', 'm', 8, null, function(err, columns, nextToken, startMatched) {
                                assert.ok(!err);
                                assert.ok(columns);
                                assert.strictEqual(startMatched, true);
                                ...
                                Cassandra.runPagedColumnQuery('VerifyPagedColumnQueryPaging', 'keyId', 'key', null, 8, {'reversed': true}, function(err, columns, nextToken, startMatched) {
                                    assert.ok(!err);
                                    assert.ok(columns);
                                    assert.strictEqual(startMatched, false);
                                    ...
                                    // Start from 'f'
                                    Cassandra.runPagedColumnQuery('VerifyPagedColumnQueryPaging', 'keyId', 'key', 'f', 8, {'reversed': true}, function(err, columns, nextToken, startMatched) {
                                        assert.ok(!err);
                                        assert.ok(columns);
                                        assert.strictEqual(startMatched, true);
                                        ...
                                        assert.equal(columns[1].name, 'd');
                                        assert.equal(columns[2].name, 'c');
                                        assert.equal(columns[3].name, 'b');
                                        assert.equal(columns[4].name, 'a');
                                        assert.ok(!nextToken);
                                        assert.ok(!columns[5]);

                                        // Start from 'a'
                                        Cassandra.runPagedColumnQuery('VerifyPagedColumnQueryPaging', 'keyId', 'key', 'a', 8, {'reversed': true}, function(err, columns, nextToken, startMatched) {
                                            assert.ok(!err);
                                            assert.ok(columns);
                                            assert.strictEqual(startMatched, true);
        ...
        it('verify multi-byte character in paged column query', function(callback) {
            Cassandra.createColumnFamily('VerifyMultiBytePagedColumnQuery', 'CREATE COLUMNFAMILY VerifyMultiBytePagedColumnQuery (keyId text PRIMARY KEY, colOne text, colTwo text)', function(err, created) {
                assert.ok(!err);
                assert.ok(created);

                var stringWithMultiByte = 'Foo O’bar';

                Cassandra.runQuery('INSERT INTO VerifyMultiBytePagedColumnQuery (keyId, colOne) VALUES (?, ?) USING CONSISTENCY QUORUM', ['key1', stringWithMultiByte], function(err) {
                    assert.ok(!err);

                    Cassandra.runPagedColumnQuery('VerifyMultiBytePagedColumnQuery', 'keyId', 'key1', null, 10, null, function(err, columns) {
                        assert.ok(!err);
                        assert.equal(columns.length, 1);
                        assert.equal(columns[0].value, stringWithMultiByte);
                        return callback();
                    });
                });
            });
