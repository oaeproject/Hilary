...
runner.addPhase(20, 'second', 128, 'second');
...
runner.addDataDirectory('./data/csv');

// Add a suite
var suite = fs.readFileSync('./suites/' + argv.suite, 'utf-8');
var tests = suite.split('\n');
for (var i = 0; i < tests.length; i++) {
    var test = tests[i].split(',');
    var probability = test[1];
    var test = require('./tests/' + test[0]);
    test.test(runner, probability);
}

// Dump or run the test
var xml = runner.to_xml();
xml = xml.replace(/%/g, '%%');
console.log(xml);