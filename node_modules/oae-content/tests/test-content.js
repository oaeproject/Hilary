...
describe('Content', function() {
    ...
    describe('Create content', function() {
        ...
        it('verify create collaborative document', function(callback) {
            setUpUsers(function(contexts) {
                // Create one as anon user
                RestAPI.Content.createDocument(anonymousRestContext, 'Test Content 1', 'Test content description 1', 'public', [], [], function(err, contentObj) {
                    assert.ok(err);
                    assert.ok(!contentObj);

                    // Create one with all required fields
                    RestAPI.Content.createDocument(contexts['nicolaas'].restContext, 'Test Content 2', 'Test content description 2', 'public', [], [], function(err, contentObj) {
                        assert.ok(!err);
                        assert.ok(contentObj.id);

                        // Create one without description
                        RestAPI.Content.createDocument(contexts['nicolaas'].restContext, 'Test Content 3', null, 'public', [], [], function(err, contentObj) {
                            assert.ok(!err);
                            assert.ok(contentObj.id);

                            // Create one without title
                            RestAPI.Content.createDocument(contexts['nicolaas'].restContext, null, 'Test content description 4', 'public', [], [], function(err, contentObj) {
                                assert.ok(err);
                                assert.ok(!contentObj);

                                // Create one without permission
                                RestAPI.Content.createDocument(contexts['nicolaas'].restContext, 'Test Content 5', 'Test content description 5', null, [], [], function(err, contentObj) {
                                    assert.ok(!err);
                                    assert.ok(contentObj.id);
         ...
         * to verify access, and then get the content as a logged in user and an anonymous user to verify they don't have access
         */
        it('verify create content with default members document', function(callback) {
            setUpUsers(function(contexts) {
                // Create a private content item and share with 2 people
                RestAPI.Content.createDocument(contexts['nicolaas'].restContext, 'Test Content 2', 'Test content description 2', 'private', [contexts['simon'].user.id], [contexts['bert'].user.id], function(err, contentObj) {
                    assert.ok(!err);
                    assert.ok(contentObj.id);
