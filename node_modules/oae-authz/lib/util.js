...
var isGroupId = module.exports.isGroupId = function(groupId) {
    return (_.isString(groupId) && groupId.indexOf(AuthzConstants.resourceTypes.GROUP + ':') === 0);
};
...
var isUserId = module.exports.isUserId = function(userId) {
    return (_.isString(userId) && userId.indexOf(AuthzConstants.principalTypes.USER + ':') === 0);
};
...
var computeRoleChanges = module.exports.computeRoleChanges = function(rolesBefore, roleChanges, opts) {
    opts = opts || {};
    opts.onlyAdd = opts.onlyAdd || false;
    ...
    // If we're only taking into consideration adds, remove any role changes that are removals or
    // already exist in the roles prior to applying the changes
    if (opts.onlyAdd) {
        _.each(result.changes, function(role, id) {
            if (role === false || result.roles.before[id]) {
