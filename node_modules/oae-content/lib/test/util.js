 ...
 */
var assertCreateLinkSucceeds = module.exports.assertCreateLinkSucceeds = function(restContext, displayName, description, visibility, link, managerIds, viewerIds, folderIds, callback) {
    PrincipalsTestUtil.assertGetMeSucceeds(restContext, function(me) {
        RestAPI.Content.createLink(restContext, displayName, description, visibility, link, managerIds, viewerIds, folderIds, function(err, content) {
            assert.ok(!err);
            ...
            _.each(managerIds, function(id) {
                roleChanges[id] = 'manager';
            });

            _.each(viewerIds, function(id) {
                roleChanges[id] = 'viewer';
            });

            // Ensure the members have the expected roles
            getAllContentMembers(restContext, content.id, null, function(result) {
                AuthzTestUtil.assertMemberRolesEquals({}, roleChanges, AuthzTestUtil.getMemberRolesFromResults(result));

                AuthzTestUtil.assertGetInvitationsSucceeds(restContext, 'content', content.id, function(result) {
 ...
 * @throws {AssertionError}                     Thrown if any assertions fail
 */
var assertCreateLinkFails = module.exports.assertCreateLinkFails = function(restContext, displayName, description, visibility, link, managerIds, viewerIds, folderIds, httpCode, callback) {
    RestAPI.Content.createLink(restContext, displayName, description, visibility, link, managerIds, viewerIds, folderIds, function(err, content) {
        assert.ok(err);
 ...
 * @throws {AssertionError}                 Thrown if any assertions fail
 */
var assertGetContentFails = module.exports.assertGetContentFails = function(restContext, contentId, httpCode, callback) {
    RestAPI.Content.getContent(restContext, contentId, function(err, content) {
        assert.ok(err);
        assert.strictEqual(err.code, httpCode);
        assert.ok(!content);
        return callback();
 ...
 * @throws {AssertionError}                     Thrown if any assertions fail
 */
var assertDeleteContentSucceeds = module.exports.assertDeleteContentSucceeds = function(restContext, contentId, callback) {
    RestAPI.Content.deleteContent(restContext, contentId, function(err) {
        assert.ok(!err);
...
var assertUpdateContentMembersFails = module.exports.assertUpdateContentMembersFails = function(managerRestContext, actorRestContext, contentId, roleChanges, httpCode, callback) {
    ...
    getAllContentMembers(managerRestContext, contentId, null, function(result) {
        var memberRolesBefore = AuthzTestUtil.getMemberRolesFromResults(result);

        AuthzTestUtil.assertGetInvitationsSucceeds(managerRestContext, 'content', contentId, function(result) {
            ...
            RestAPI.Content.updateMembers(actorRestContext, contentId, roleChanges, function(err) {
                ...
                AuthzTestUtil.assertGetInvitationsSucceeds(managerRestContext, 'content', contentId, function(result) {
                    AuthzTestUtil.assertEmailRolesEquals(emailRolesBefore, null, AuthzTestUtil.getEmailRolesFromResults(result.results));

                    getAllContentMembers(managerRestContext, contentId, null, function(result) {
                        AuthzTestUtil.assertMemberRolesEquals(memberRolesBefore, null, AuthzTestUtil.getMemberRolesFromResults(result));
        ...
        AuthzTestUtil.assertGetInvitationsSucceeds(managerRestContext, 'content', contentId, function(result) {
            ...
            RestAPI.Content.updateMembers(actorRestContext, contentId, roleChanges, function(err) {
                ...
                AuthzTestUtil.assertGetInvitationsSucceeds(managerRestContext, 'content', contentId, function(result) {
                    ...
                    getAllContentMembers(managerRestContext, contentId, null, function(result) {
                        AuthzTestUtil.assertMemberRolesEquals(memberRolesBefore, roleChanges, AuthzTestUtil.getMemberRolesFromResults(result));
 ...
 * @throws {AssertionError}                     Thrown if there is an error ensuring that the request fails in the specified manner
 */
var assertShareContentFails = module.exports.assertShareContentFails = function(managerRestContext, actorRestContext, contentId, memberIds, httpCode, callback) {
    // Get the members library so we can ensure it does not change after the failure
    getAllContentMembers(managerRestContext, contentId, null, function(result) {
        var memberRolesBefore = AuthzTestUtil.getMemberRolesFromResults(result);
        ...
        AuthzTestUtil.assertGetInvitationsSucceeds(managerRestContext, 'content', contentId, function(result) {
            ...
            // Perform the update and ensure it fails as expected
            RestAPI.Content.shareContent(actorRestContext, contentId, memberIds, function(err) {
                assert.ok(err);
                assert.strictEqual(err.code, httpCode);
 ...
 */
var assertShareContentSucceeds = module.exports.assertShareContentSucceeds = function(managerRestContext, actorRestContext, contentId, memberIds, callback) {
    // Ensure the members library is currently built
    getAllContentMembers(managerRestContext, contentId, null, function(result) {
        var memberRolesBefore = AuthzTestUtil.getMemberRolesFromResults(result);
 ...
 */
var assertGetAllContentLibrarySucceeds = module.exports.assertGetAllContentLibrarySucceeds = function(restContext, principalId, opts, callback, _contentItems, _responses, _nextToken) {
    ...
    opts = opts || {};
    opts.batchSize = opts.batchSize || 25;
    assertGetContentLibrarySucceeds(restContext, principalId, {'start': _nextToken, 'limit': opts.batchSize}, function(result) {
        _responses.push(result);
        return assertGetAllContentLibrarySucceeds(restContext, principalId, opts, callback, _.union(_contentItems, result.results), _responses, result.nextToken);
    });
};
 ...
 */
var assertGetContentLibrarySucceeds = module.exports.assertGetContentLibrarySucceeds = function(restContext, principalId, opts, callback) {
    opts = opts || {};
    RestAPI.Content.getLibrary(restContext, principalId, opts.start, opts.limit, function(err, result) {
        assert.ok(!err);
        assert.ok(result);
        return callback(result);
    });
 ...
 */
var generateTestLinks = module.exports.generateTestLinks = function(restContext, total, callback) {
    var contentItems = [];
    ...
    RestAPI.User.getMe(restContext, function(err) {
        ...
        _.each(_.range(total), function(i) {
            RestAPI.Content.createLink(restContext, 'test displayname', 'test descr', 'public', 'google.com', [], [], [], function(err, contentItem) {
                assert.ok(!err);
 ...
 */
var publishCollabDoc = module.exports.publishCollabDoc = function(contentId, userId, callback) {
    var data = {
        'contentId': contentId,
        'userId': userId
    };
    TaskQueue.submit(ContentConstants.queue.ETHERPAD_PUBLISH, data, function(err) {
        assert.ok(!err);
    });
 ...
 * @param  {Object}         callback.user..         An object containing the user profile and a rest context for the next user of the set of users that was created
 */
var createCollabsheet = module.exports.createCollabsheet = function(adminRestContext, nrOfUsers, nrOfJoinedUsers, callback) {
    TestsUtil.generateTestUsers(adminRestContext, nrOfUsers, function(err, users) {
        assert.ok(!err);
        ...
        // Create a collaborative document where all the users are managers
        var name = TestsUtil.generateTestUserId('collabdoc');
        RestAPI.Content.createCollabsheet(userValues[0].restContext, name, 'description', 'public', userIds, [], [], [], function(err, contentObj) {
            assert.ok(!err);
            ...
            // Join the collabsheet for `nrOfJoinedUsers` users
            var joinCollabsheet = function(i) {
                var restCtx = userValues[i].restContext;
                RestAPI.Content.joinCollabDoc(restCtx, contentObj.id, function(err, data) {
                    assert.ok(!err);
                    callCallback();
