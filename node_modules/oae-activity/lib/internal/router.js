                ...
                _producePropagation(associationsContexts.actor, actor, function(err, actorPropagationRules) {
                    ...
                    _producePropagation(associationsContexts.object, object, function(err, objectPropagationRules) {
                        ...
                        _producePropagation(associationsContexts.target, target, function(err, targetPropagationRules) {
                            ...
                            _applyPropagations('actor', actorPropagationRules, allRoutes, associationsContexts, actor, actorRoutes, function(err, includedRoutes) {
                                if (err) {
                                    return callback(err);
                                ...
                                _applyPropagations('object', objectPropagationRules, includedRoutes, associationsContexts, object, objectRoutes, function(err, includedRoutes) {
                                    if (err) {
                                        return callback(err);
                                    } else if (includedRoutes.length === 0) {
                                    ...
                                    _applyPropagations('target', targetPropagationRules, includedRoutes, associationsContexts, target, targetRoutes, function(err, includedRoutes) {
                                        if (err) {
                                            return callback(err);
                                        }
 ...
 *
 * @param  {String}                 objectType                      The type of activity object ("actor", "object" or "target") whose propagation is to be applied
...
var _applyPropagation = function(objectType, propagation, routes, associationsContexts, entity, entityRoutes, callback) {
    ...
    if (propagation.type === ActivityConstants.entityPropagation.ALL) {
        ...
        // Apply a specified association that is of an object that is external to the item itself (e.g., the target group specifies
        // that it is OK to propagate to the members of the "object" entity -- a content item or another group)
        var associationsCtx = associationsContexts[propagation.objectType];
        if (!associationsCtx) {
            // There is no entity that is of this object type (e.g., no object or no target), therefore we apply no exclusions
            return callback(null, routes, []);
        }

        _applyAssociationsCtx(associationsCtx, propagation.association, function(err) {
            if (err) {
                return callback(err);
            }
