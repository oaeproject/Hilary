...
var createFolder = (module.exports.createFolder = function(
  ...
  var insertGroupIdIndexQuery = Cassandra.constructUpsertCQL(
    'FoldersGroupId',
    'groupId',
    groupId,
    { folderId: folderId },
  );
  var insertFolderQuery = Cassandra.constructUpsertCQL(
    'Folders',
    'id',
    folderId,
    storageHash,
  ...
  Cassandra.runBatchQuery(
    [insertGroupIdIndexQuery, insertFolderQuery],
    function(err) {
      if (err) {
        return callback(err);
      }
 ...
 */
var getFoldersByIds = (module.exports.getFoldersByIds = function(
  folderIds,
  callback,
) {
 ...
 */
var getFoldersByGroupIds = (module.exports.getFoldersByGroupIds = function(
  groupIds,
  callback,
) {
  ...
  Cassandra.runQuery(
    ...
    function(err, rows) {
      ...
      var folderIdsByGroupIds = _.chain(rows)
  ...
  getFoldersByIds([folderId], function(err, folders) {
    if (err) {
      ...
      return callback({
        code: 404,
        msg: util.format(
          'A folder with the id "%s" could not be found',
          folderId,
        ),
      });
    }
...
var updateFolder = (module.exports.updateFolder = function(
  ...
  Cassandra.runQuery(query.query, query.parameters, function(err) {
    ...
    return callback(
      null,
      _createUpdatedFolderFromStorageHash(folder, storageHash),
    );
  });
});
 ...
 */
var getContentItems = (module.exports.getContentItems = function(
  folderGroupId,
  opts,
  callback,
) {
  ...
  AuthzAPI.getRolesForPrincipalAndResourceType(
    ...
    function(err, roles, nextToken) {
      ...
      ContentDAO.Content.getMultipleContentItems(ids, opts.fields, function(
        err,
        contentItems,
      ) {
        if (err) {
 ...
 * @param  {Folder}     callback.folder     The updated folder
 */
var setPreviews = (module.exports.setPreviews = function(
  folder,
  previews,
  callback,
) {
  var query = Cassandra.constructUpsertCQL('Folders', 'id', folder.id, {
    previews: previews,
  });
  Cassandra.runQuery(query.query, query.parameters, function(err) {
    if (err) {
    ...
    return callback(
      null,
      _createUpdatedFolderFromStorageHash(folder, { previews: previews }),
    );
  });
});
