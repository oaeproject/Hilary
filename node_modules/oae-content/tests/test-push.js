                    ...
                    ActivityTestsUtil.getFullySetupClient(data, function(client) {
                        ...
                        RestAPI.Content.createLink(simong.restContext, 'content', 'A piece of content', 'private', 'http://www.google.com', [branden.user.id], [], function(err, contentObj) {
                            ...
                            RestAPI.Content.getContent(simong.restContext, contentObj.id, function(err, contentObj) {
                                ...
                                client.subscribe(contentObj.id, null, contentObj.signature, function(err) {
                                    assert.equal(err.code, 400);

                                    // Ensure we get a 400 error with a missing resource id
                                    client.subscribe(null, 'activity', contentObj.signature, function(err) {
                                        assert.equal(err.code, 400);

                                        // Ensure we get a 400 error with an invalid token
                                        client.subscribe(contentObj.id, 'activity', {'signature': 'foo', 'lastModified': Date.now()}, function(err) {
                                            assert.equal(err.code, 400);
                                            client.subscribe(contentObj.id, 'activity', {'expires': Date.now() + 10000, 'lastModified': Date.now()}, function(err) {
                                                assert.equal(err.code, 400);
                                                client.subscribe(contentObj.id, 'activity', {'expires': Date.now() + 10000, 'signature': 'foo'}, function(err) {
                                                    assert.equal(err.code, 400);

                                                    // Ensure we get a 401 error with an incorrect signature
                                                    client.subscribe(contentObj.id, 'activity', {'expires': Date.now() + 10000, 'signature': 'foo', 'lastModified': Date.now()}, function(err) {
                                                        ...
                                                        RestAPI.Content.getContent(branden.restContext, contentObj.id, function(err, contentObjForBranden) {
                                                            assert.ok(!err);
                                                            client.subscribe(contentObj.id, 'activity', contentObjForBranden.signature, function(err) {
                                                                assert.equal(err.code, 401);
                                                                callback();
                                                            });
                                                        });
    ...
    describe('Notifications', function() {
        ...
        it('verify content updates trigger a push notification', function(callback) {
            setupFixture(function(contexts, contentObj, client) {
                client.on('message', function(message) {
                    if (message.resourceId === contentObj.id && message.streamType === 'activity') {
                ...
                client.on('message', function(message) {
                    if (message.resourceId === contentObj.id && message.streamType === 'activity') {
                        ...
                        RestAPI.Content.getContent(contexts['branden'].restContext, contentObj.id, function(err, contentObj) {
                            assert.ok(!err);
                            assert.equal(message.activity.object['oae:revisionId'], contentObj.latestRevisionId);
        ...
        it('verify a new comment triggers a push notification', function(callback) {
            setupFixture(function(contexts, contentObj, client) {
                ...
                client.on('message', function(message) {
                    if (message.resourceId === contentObj.id && message.streamType === 'message') {
                        ActivityTestsUtil.assertActivity(message.activity, ContentConstants.activity.ACTIVITY_CONTENT_COMMENT, ActivityConstants.verbs.POST, contexts['branden'].user.id, messageId, contentObj.id);
                ...
                // Create a message
                RestAPI.Content.createComment(contexts['branden'].restContext, contentObj.id, 'Cup a Soup', null, function(err, message) {
                    assert.ok(!err);
