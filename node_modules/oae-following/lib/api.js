...
var getFollowers = module.exports.getFollowers = function(ctx, userId, start, limit, callback) {
    ...
    validator.check(limit, {'code': 400, 'msg': 'You must specify a valid limit'}).isInt();
    if (start) {
        validator.check(start, {'code': 400, 'msg': 'If you specify a start parameter, it must be a valid user id from which to start'}).isUserId();
    }
...
/**
 * Get the users who the specified user is following
 *
 * @param  {Context}    ctx                 The context of the current user
...
/**
 * Make the user in context follow the user identified by `followedUserId`
 *
...
var follow = module.exports.follow = function(ctx, followedUserId, callback) {
    ...
    // Get the user with which we can perform permission checks
...
/**
 * Unfollow the user identified by `unfollowedUserId`
 *
 * @param  {Context}    ctx                 The context of the current request
