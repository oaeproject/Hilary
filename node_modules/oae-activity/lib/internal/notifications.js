...
ActivityEmitter.on(ActivityConstants.events.DELIVERED_NOTIFICATIONS, function(deliveredNotifications) {
    ...
    // Note: It is important to ensure that incrementNotificationsUnread does not do a raw invalidation of a
    // user in cache, as the users are always read using PrincipalsDAO immediately after. This method should
    // have a way to update the user in cache rather than invalidate.
    incrementNotificationsUnread(userIdsIncrBy, function(err) {
        ...
        var registeredActivityTypes = ActivityRegistry.getRegisteredActivityTypes();
        var tenantsForContext = TenantsAPI.getTenantsForContext();
        _.each(deliveredNotifications, function(activities, userId) {

            // Only send an email if it is configured to do so.
            ...
            // Clone the activities since they will be tranformed in-place. This avoids hosing other potential
            // listeners on this event.
            activitiesToEmail = clone(activitiesToEmail);

            // Create an API context that can be used to transform the emailed activities
            PrincipalsDAO.getPrincipal(userId, function(err, user) {
                ...
                ActivityTransformer.transformActivities(ctx, activitiesToEmail, function(err) {
                    ...
                    _.each(activitiesToEmail, function(activity) {
                        ...
                        EmailAPI.sendEmail(templateModule, templateId, user, {'activity': activity}, null);
                    });
                });
            });
        });
...
var _getRegisteredActivityOptions = function(activity) {
    ...
    return registeredActivityTypes[activityType];
};
