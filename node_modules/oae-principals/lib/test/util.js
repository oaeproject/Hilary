 ...
 * @param  {Function}       callback                Standard callback function
 * @throws {AssertionError}                         An assertion error is thrown if there are any errors adding the users to the groups
 */
...
/*
 * Sign into a guest account for a given guest token
 *
 ...
 * @throws {AssertionError}                             An assertion error is thrown when an unexpected error occurs
 */
var signIntoGuestAccountWithToken = module.exports.signIntoGuestAccountWithToken = function(token, callback) {
...
/**
 * Execute a callback function when a guest email is sent
 *
 * @param  {String}     [email]             The email address to which the token should've been sent
 * @param  {Function}   callback            Standard callback function
 * @param  {String}     callback.token      The guest token that was sent to the user
 */
var whenGuestEmailSent = module.exports.whenGuestEmailSent = function(email, callback) {
    EmailAPI.once('debugSent', function(message) {
        // Verify the email address
        if (email) {
            assert.strictEqual(message.to[0].address, email);
        ...
        // Verify a token is passed in both the html and text email
        assert.ok(message.html.match(/\?token=(guest-[a-zA-Z0-9]{32})/));
        assert.ok(message.text.match(/\?token=(guest-[a-zA-Z0-9]{32})/));

        // Parse the token out
        var token = message.text.match(/\?token=(guest-[a-zA-Z0-9]{32})/)[1];
        assert.ok(token);
