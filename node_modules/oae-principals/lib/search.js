 ...
 */
PrincipalsAPI.on(PrincipalsConstants.events.VERIFIED_EMAIL, function(
  ctx,
  user,
) {
  SearchAPI.postIndexTask('user', [{ id: user.id }], {
 ...
 * memberships documents
 */
PrincipalsAPI.on(PrincipalsConstants.events.LEFT_GROUP, function(
  ctx,
  group,
  memberChangeInfo,
) {
 ...
 * @api private
 */
var _produceUserSearchDocuments = function(
  resources,
  callback,
  _documents,
  _errs,
  ...
  PrincipalsDAO.getPrincipal(resource.id, function(err, user) {
    if (err) {
      ...
      return _produceUserSearchDocuments(
        resources,
        callback,
        _documents,
        _errs,
 ...
 * @api private
 */
var _produceGroupSearchDocuments = function(
  resources,
  callback,
  _documents,
  _errs,
  ...
  PrincipalsDAO.getPrincipal(resource.id, function(err, group) {
    if (err) {
      _errs = _.union(_errs, [err]);
      return _produceGroupSearchDocuments(
        resources,
        callback,
        _documents,
        _errs,
...
var _transformUserDocuments = function(ctx, docs, callback) {
  ...
  _.each(docs, function(doc, docId) {
    ...
    if (user.picture.mediumUri) {
      result.thumbnailUrl = ContentUtil.getSignedDownloadUrl(
        ctx,
        user.picture.mediumUri,
      );
    }
...
SearchAPI.registerReindexAllHandler('principal', function(callback) {
  ...
  var _onEach = function(principalRows, done) {
    ...
    if (!_.isEmpty(userResources)) {
      SearchAPI.postIndexTask('user', userResources, {
        resource: true,
        children: true,
      });
    }
    ...
    if (!_.isEmpty(groupResources)) {
      SearchAPI.postIndexTask('group', groupResources, {
        resource: true,
        children: true,
      });
    }
 ...
 * @api private
 */
var _handleInvalidateSearch = function(
  group,
  membershipsGraph,
  membersGraph,
  callback,
