...
var getGroupMembers = module.exports.getGroupMembers = function(context, group_id, start, limit, callback) {
    ...
    getGroup(context, group_id, function(err, group) {
        ...
        Cassandra.runQuery("SELECT first " + paging.limit + " ?..\'\' FROM GroupMembers WHERE group_id = ?", [paging.start, group_id], function (err, rows) {
            if (err) {
                return callback({'code': 500, 'msg': err});
            }

            var members = [];
            for (var i = paging.startIndex, j = rows[0].count;i<j;i++) {
...
var memberOf = module.exports.memberOf = function(context, start, limit, callback) {
    ...
    getGroupMembershipAncestry(context.user().id, function(err, groups) {
        if (!err) {
            ...
            PrincipalsUtil.getPrincipals(context, groupsArray, function(err, principals) {
                if (err) {
                    return callback(err);
                }
                var principalList = [];
                for (var i = 0, j = groupsArray.length; i < j; i++) {
                    principalList.push(principals[groupsArray[i]]);
                }
                callback(false, principalList);
            });
        } else {
            callback(err);
        }
