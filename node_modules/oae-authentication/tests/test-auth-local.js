...
describe('Authentication', function() {
    ...
    before(function(callback) {
        ...
        anonymousGtRestContext = TestsUtil.createAnonymousRestContext(global.oaeTests.tenants.gt.baseUrl);
    ...
    describe('Admin login', function() {
        ...
        it('verify global administrator authentication', function(callback) {
            var globalAdminRestContext = TestsUtil.createAdminRestContext('http://localhost:2000');
    ...
    describe('#getOrCreateUser', function() {
        ...
        it('verify getOrCreateUser', function(callback) {
            ...
            AuthenticationAPI.getOrCreateUser(ctx, 'twitter', userId, 'Nicolaas Matthijs', null, function(err, userObj) {
                assert.ok(!err, JSON.stringify(err));
                assert.ok(userObj);
                
                // Get the user again through the same function
                AuthenticationAPI.getOrCreateUser(ctx, 'twitter', userId, 'Branden Visser', null, function(err, userObj) {
                    assert.ok(!err, JSON.stringify(err));
                    assert.ok(userObj);
    ...
    describe('#createUser', function() {
        ...
        it ('verify create without tenant id', function(callback) {
            var ctx = new Context(global.oaeTests.tenants.cam);
            var userId = TestsUtil.generateTestUserId();
            var loginId = new LoginId(null, 'local', userId, { password: 'password' });
        ...
        it ('verify create without external id', function(callback) {
            var ctx = new Context(global.oaeTests.tenants.cam);
            var loginId = new LoginId(ctx.tenant().alias, 'local', null, { password: 'password' });
            AuthenticationAPI.createUser(ctx, loginId, 'Branden Visser', null, function(err, userObj) {
                assert.ok(err);
                assert.equal(err.code, 400);
                callback();
        ...
        it ('verify create local without password', function(callback) {
            var ctx = new Context(global.oaeTests.tenants.cam);
            var userId = TestsUtil.generateTestUserId();
            var loginId = new LoginId(ctx.tenant().alias, 'local', userId);
            AuthenticationAPI.createUser(ctx, loginId, 'Branden Visser', null, function(err, userObj) {
                assert.ok(err);
                assert.equal(err.code, 400);
                callback();
        ...
        it ('verify create local with short password', function(callback) {
            var ctx = new Context(global.oaeTests.tenants.cam);
            var userId = TestsUtil.generateTestUserId();
            var loginId = new LoginId(ctx.tenant().alias, 'local', userId, { password: '12345' });
            AuthenticationAPI.createUser(ctx, loginId, 'Branden Visser', null, function(err, userObj) {
                assert.ok(err);
                assert.equal(err.code, 400);
                callback();
        ...
        it ('verify create user with local loginId', function(callback) {
            ...
            var userId = TestsUtil.generateTestUserId();
            var userRestContext = new RestContext(global.oaeTests.tenants.cam.baseUrl, userId, 'password');
            var loginId = new LoginId(ctx.tenant().alias, 'local', userId, { password: 'password' });
            AuthenticationAPI.createUser(ctx, loginId, 'Branden Visser', null, function(err, userObj) {
                assert.ok(!err);
                assert.ok(userObj);
        ...
        it ('verify create user with non-local loginId', function(callback) {
            ...
            var userId = TestsUtil.generateTestUserId();
            var userRestContext = new RestContext(global.oaeTests.tenants.cam.baseUrl, userId, 'password');
            var loginId = new LoginId(ctx.tenant().alias, 'twitter', userId);
            AuthenticationAPI.createUser(ctx, loginId, 'Branden Visser', null, function(err, userObj) {
                assert.ok(!err);
                assert.ok(userObj);

                // verify the mapping exists
                AuthenticationAPI.getUserIdFromLoginId(ctx.tenant().alias, 'twitter', userId, function(err, userId) {
                    assert.ok(!err);
    ...
    describe('#associateLoginId', function() {
        ...
        it('verify associate login id to self', function(callback) {
            ...
            PrincipalsAPI.createUser(ctx, 'Branden Visser', null, function(err, user) {
                ...
                AuthenticationAPI.associateLoginId(ctx, loginId, user.id, function(err) {
                    ...
                    AuthenticationAPI.getUserIdFromLoginId(ctx.tenant().alias, 'local', userId, function(err, userId) {
                        assert.ok(!err);
        ...
        it('verify associate multiple login ids to self', function(callback) {
            ...
            PrincipalsAPI.createUser(ctx, 'Branden Visser', null, function(err, user) {
                ...
                // associate a login id to the user
                AuthenticationAPI.associateLoginId(ctx, new LoginId(ctx.tenant().alias, 'local', userId, { password: 'password' }), user.id, function(err) {
                    assert.ok(!err);

                    // associate a second twitter id to the user
                    AuthenticationAPI.associateLoginId(ctx, new LoginId(ctx.tenant().alias, 'twitter', twitterId), user.id, function(err) {
                        assert.ok(!err);
                        ...
                        AuthenticationAPI.getUserIdFromLoginId(ctx.tenant().alias, 'local', userId, function(err, userId) {
                            ...
                            AuthenticationAPI.getUserIdFromLoginId(ctx.tenant().alias, 'twitter', twitterId, function(err, userId) {
                                assert.ok(!err);
                                assert.equal(user.id, userId);
                                callback();
                            });
        ...
        it('verify another user cannot associate login id', function(callback) {
            ...
            PrincipalsAPI.createUser(ctx, 'Branden Visser', null, function(err, mrvisser) {
                ...
                PrincipalsAPI.createUser(ctx, 'Bert Pareyn', null, function(err, bert) {
                    ...
                    // associate a login id to the user
                    AuthenticationAPI.associateLoginId(bertCtx, new LoginId(ctx.tenant().alias, 'local', mrvisserUsername, { password: 'password' }), mrvisser.id, function(err) {
                        assert.ok(err);
                        assert.equal(err.code, 401);

                        // verify the local login id is mapped
                        AuthenticationAPI.getUserIdFromLoginId(ctx.tenant().alias, 'local', mrvisserUsername, function(err, mrvisserId) {
                            assert.ok(!err);
        ...
        it('verify cannot associate multiple login ids of same type', function(callback) {
            ...
            PrincipalsAPI.createUser(ctx, 'Branden Visser', null, function(err, user) {
                ...
                AuthenticationAPI.associateLoginId(ctx, new LoginId(ctx.tenant().alias, 'local', userId, { password: 'password' }), user.id, function(err) {
                    ...
                    // associate a second twitter id to the user
                    AuthenticationAPI.associateLoginId(ctx, new LoginId(ctx.tenant().alias, 'local', userId2, { password: 'password' }), user.id, function(err) {
                        assert.ok(err);
                        assert.equal(err.code, 400);
                        ...
                        AuthenticationAPI.getUserIdFromLoginId(ctx.tenant().alias, 'local', userId, function(err, userId) {
                            ...
                            AuthenticationAPI.getUserIdFromLoginId(ctx.tenant().alias, 'local', userId2, function(err, userId) {
                                assert.ok(!err);
                                assert.ok(!userId);
                                callback();
                            });
        ...
        it('verify admin re-associate login id', function(callback) {
            ...
            var mrvisserUsername = TestsUtil.generateTestUserId() + ':withcolon';
            var mrvisserLoginId = new LoginId(global.oaeTests.tenants.cam.alias, 'twitter', mrvisserUsername);
            var bertUsername = TestsUtil.generateTestUserId() + ':withcolon';
            var bertLoginId = new LoginId(global.oaeTests.tenants.cam.alias, 'local', bertUsername, { password: 'password' });
            AuthenticationAPI.createUser(adminCtx, mrvisserLoginId, 'Branden Visser', null, function(err, mrvisser) {
                ...
                AuthenticationAPI.createUser(adminCtx, bertLoginId, 'Bert Pareyn', null, function(err, bert) {
                    ...
                    AuthenticationAPI.associateLoginId(adminCtx, new LoginId(adminCtx.tenant().alias, 'twitter', mrvisserUsername), bert.id, function(err) {
                        assert.ok(!err);

                        AuthenticationAPI.getUserIdFromLoginId(adminCtx.tenant().alias, 'twitter', mrvisserUsername, function(err, userId) {
                            assert.ok(!err);
        ...
        it('verify non-admin cannot re-associate login id', function(callback) {
            ...
            var mrvisserLoginId = new LoginId(tenant.alias, 'twitter', mrvisserUsername);
            var bertUsername = TestsUtil.generateTestUserId();
            var bertLoginId = new LoginId(tenant.alias, 'local', bertUsername, { password: 'password' });
            AuthenticationAPI.createUser(adminCtx, mrvisserLoginId, 'Branden Visser', null, function(err, mrvisser) {
                assert.ok(!err);
                ...
                AuthenticationAPI.createUser(adminCtx, bertLoginId, 'Bert Pareyn', null, function(err, bert) {
                    ...
                    AuthenticationAPI.associateLoginId(bertCtx, new LoginId(tenant.alias, 'twitter', mrvisserUsername), bert.id, function(err) {
                        assert.ok(err);
                        assert.ok(err.code, 401);

                        AuthenticationAPI.getUserIdFromLoginId(tenant.alias, 'twitter', mrvisserUsername, function(err, userId) {
                            assert.ok(!err);
