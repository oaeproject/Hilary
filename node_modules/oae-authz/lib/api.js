 ...
 * @param {Function(err, role)} callback        A function specifying the role of the principal. If the principal has no role or there is an
 *                                              error performing the check, role will be null
 ...
 * @param {Function(err, roles)}    callback        A callback function invoked when the process completes
 * @param {Object}                  callback.err    An error that occurred, if any
 * @param {Object}                  callback.roles  A JSON Object, keyed by all the roles the principal has on the resource. The value of
 *                                                  all the entries is simply `true`
 */
 ...
 * @param {String}                  start               The starting entry on which to begin returning results (used for paging). **This is not a
 *                                                      number**. If null, then the results will simply begin from the start. If the resource
 *                                                      uuid is provided, then results will begin from that resource's location, exclusively.
 ...
 * @param {Object}                  callback.err        An error that occurred, if any
 * @param {Object}                  callback.entries    A JSON object of the format provided in the summary. Contains all resource instances and
 *                                                      the role the principal has on them
 */
...
var getMemberOf = module.exports.getMemberOf = function(principalUuid, callback) {
    ...
    _getGroupMembershipAncestry(principalUuid, function(err, groups) {
        Telemetry.append('getMemberOf', Date.now()-start);
        if (!err) {
