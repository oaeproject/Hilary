...
//var TenantsTestUtil = require('oae-tenants');
...
if (process.env['OAE_COVERING']) {
    ...
    config.files.localStorageDirectory = '../../files';
}
...
var setUpTenants = function(callback) {
    ...
    global.oaeTests.tenants.global = new Tenant(config.servers.globalAdminAlias, 'Global tenant', config.servers.globalAdminHost, true, false);

    // Mock a global admin, so we can create some tenants.
    //var mockUserId = 'u:' + config.servers.globalAdminAlias + ':admin';
    //var globalAdmin = new User(config.servers.globalAdminAlias, mockUserId, 'The global admin User', {
    //    visibility: 'private',
    //    isGlobalAdmin: true,
    //    isTenantAdmin: true
    //});
    var globalRestContext = TestsUtil.createGlobalAdminRestContext();

    // Create the Cambridge tenant
    RestAPI.Tenants.createTenant(globalRestContext, 'camtest', 'Cambridge University Test', 'cambridge.oae.com', function(err, tenant) {
        if (err) {
            log().error({err: err});
            return callback(err);
        }
        ...
        RestAPI.Tenants.createTenant(globalRestContext, 'gttest', 'Georgia Tech Test', 'gt.oae.com', function(err, tenant) {
            if (err) {
                return callback(err);
            }
            ...
            RestAPI.Tenants.createTenant(globalRestContext, 'localhost', 'Tenant with a hostname set to localhost', 'localhost:2001', function(err, tenant) {
                if (err) {
                    return callback(err);
                }
...
var setupTenantAdmin = function(tenant, callback) {
    ...
    var context = new Context(tenant, adminUser);

    AuthenticationAPI.createUser(context, adminLoginId, adminUser.displayName, null, function(err, createdUser) {
        if (err) {
            return callback(err);
        }
        PrincipalsAPI.setTenantAdmin(context, createdUser.id, true, callback);
    });
};
