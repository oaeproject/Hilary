 ...
 */
DiscussionsAPI.when(DiscussionsConstants.events.CREATED_DISCUSSION, function(
  ctx,
  discussion,
  memberChangeInfo,
  callback,
 ...
 */
DiscussionsAPI.when(DiscussionsConstants.events.DELETED_DISCUSSION, function(
  ctx,
  discussion,
  removedMemberIds,
  callback,
...
DiscussionsAPI.when(
  ...
  function(ctx, discussion, memberChangeInfo, opts, callback) {
    var addedMemberIds = _.pluck(memberChangeInfo.members.added, 'id');
    ...
    _removeLibrary(removedMemberIds, discussion, function(err) {
      ...
      OaeUtil.invokeIfNecessary(touchDiscussion, _touch, discussion, function(
        err,
        touchedDiscussion,
      ) {
        if (err) {
        ...
        _insertLibrary(addedMemberIds, discussion, function(err) {
          ...
          var libraryUpdateIds = _.chain(memberChangeInfo.roles.before)
            .keys()
...
var _testDiscussionUpdateThreshold = function(discussion) {
  return (
    !discussion.lastModified ||
    Date.now() - discussion.lastModified >
      LIBRARY_UPDATE_THRESHOLD_SECONDS * 1000
  );
};
