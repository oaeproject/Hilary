...
describe('Profile pictures', () => {
  ...
  const _getTextStream = function() {
    const file = __dirname + '/data/speech.txt';
    return fs.createReadStream(file);
  };
    ...
    _createUser(ctx => {
      ...
      RestAPI.User.uploadPicture(ctx, ctx.user.id, _getPictureStream, null, err => {
        ...
        TestsUtil.generateTestGroups(ctx, 1, function() {
          const groupId = _.first(arguments).group.id;
  ...
  const _verifySearchThumbnails = function(
    ...
    SearchTestsUtil.searchAll(restContext, 'members-library', [groupId], null, (err, results) => {
      ...
      assert.strictEqual(users[publicUserId].hasOwnProperty('thumbnailUrl'), canPublic);
      assert.strictEqual(users[loggedinUserId].hasOwnProperty('thumbnailUrl'), canLoggedIn);
      assert.strictEqual(users[privateUserId].hasOwnProperty('thumbnailUrl'), canPrivate);
      callback();
    });
  };
