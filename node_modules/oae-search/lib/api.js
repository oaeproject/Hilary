 ...
 *
 * @param  {String}     name                            The name of the document mapping
 ...
 * @throws {Error}                                      Thrown if there is a search document already registered with this name
 */
var registerChildSearchDocument = module.exports.registerChildSearchDocument = function(name, options, callback) {
    if (childSearchDocuments[name]) {
        throw new Error('Child search document mapping with name "' + name + '" already exists');
    }
 ...
 *
 * @param  {Object}             data                                            The task data
 * @param  {Object[]}           data.resources                                  An array of resources that are to be indexed. This is partially dependent on the resource type's custom document producer, but has some internal fields to control how resources are indexed
 * @param  {Object}             data.resources[i].index                         Specifies how to index the resource item
 * @param  {String}             [data.resources[i].resource]                    Should specify the type of resource being indexed (e.g., content, discussion, user, etc...). If not specified, the top-level resource document will **not** be indexed
 * @param  {Object|Boolean}     [data.resources[i].children]                    Specifies how to index the child documents that are associated to the parent resource item. If not specified, no children will be indexed. If `true`, **all** known child documents will be indexed
 * @param  {Boolean}            [data.resources[i].children.<document_type>]    If `true` for some registered `<document type>` (e.g., resource_members, content_comment, etc...), then documents of the specified type(s) for the parent resource will be indexed
 * @param  {Function}           callback                                        Invoked when indexing has completed
 * @param  {Object}             callback.err                                    An error that occurred, if any
 * @api private
 */
var _handleIndexDocumentTask = function(data, callback) {
    ...
    _.each(data.resources, function(resource) {
        ...
        resource.index.children = resource.index.children || {};

        // If the children property is set to boolean true, it indicates all known child documents
        // should be indexed for the resource
        if (resource.index.children === true) {
            resource.index.children = {};
            _.each(childSearchDocuments, function(options, documentType) {
                resource.index.children[documentType] = true;
            });
        }

        // Keep track of all core resource documents that need to be indexed
        if (resource.index.resource) {
            resourcesToIndex[resource.index.resource] = resourcesToIndex[resource.index.resource] || [];
            resourcesToIndex[resource.index.resource].push(resource);
        }

        // Keep track of all child documents that need to be indexed
        _.chain(resource.index.children).keys().each(function(documentType) {
