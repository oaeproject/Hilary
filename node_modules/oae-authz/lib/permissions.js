...
var canShare = module.exports.canShare = function(ctx, resource, targets, role, callback) {
    ...
    resolveEffectivePermissions(ctx, resource, function(err, permissions) {
        ...
        _validateRoleChanges(ctx, resource, targets, {'promoteOnly': true}, function(err, memberChangeInfo, emailChangeInfo) {
            ...
            var emailResourcesAdded = _.map(emailChangeInfo.emails.added, _emailToResource);
            var addedPrincipals = _.union(memberChangeInfo.members.added, emailResourcesAdded);
...
var _validateRoleChanges = function(ctx, resource, targetRoles, opts, callback) {
    ...
    _computeMemberRolesAfterChanges(resource, principalTargetRoles, opts, function(err, memberChanges, memberRoles, members) {
        if (err) {
            return callback(err);
        }

        _computeInvitationRolesAfterChanges(resource, emailTargetRoles, opts, function(err, emailChanges, emailRoles, emails) {
            if (err) {
                return callback(err);
            }

            // Determine the resources we need to check against for interaction. This is only all
            // new email/principal associations being made to the system, excluding principals that
            // the client validated with knowledge of the principal email address
            var checkForInteraction = _.chain(emails.added)
                .map(_emailToResource)
                .union(members.added)
                .filter(function(resource) {
                    return (!validatedPrincipalIds[resource.id]);
                })
                .value();
            canInteract(ctx, checkForInteraction, function(err) {
                if (err && err.code === 401) {
