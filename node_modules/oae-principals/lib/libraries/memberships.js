...
LibraryAPI.Index.registerLibraryIndex(PrincipalsConstants.library.MEMBERSHIPS_INDEX_NAME, {
    'pageResources'(libraryId, start, limit, callback) {
        ...
        _getAllGroupMembershipsFromAuthz(libraryId, (err, groupIdRoles) => {
            ...
            PrincipalsDAO.getPrincipals(_.keys(groupIdRoles), ['principalId', 'tenantAlias', 'visibility', 'lastModified'], (err, groups) => {
                if (err) {
                    return callback(err);
                }
...
var _getAllGroupMembershipsFromLibrary = function(principalId, callback, _groupIds, _nextToken) {
    ...
    LibraryAPI.Index.list(PrincipalsConstants.library.MEMBERSHIPS_INDEX_NAME, principalId, AuthzConstants.visibility.PRIVATE, opts, (err, entries, nextToken) => {
        if (err) {
            return callback(err);
        }
...
var _getGroupAncestorsIncludingDeleted = function(group, callback) {
    ...
    AuthzAPI.getPrincipalMembershipsGraph(group.id, (err, graph) => {
        ...
        PrincipalsDAO.getPrincipals(membershipIds, ['principalId', 'tenantAlias', 'lastModified', 'visibility'], (err, parentGroupsByGroupId) => {
            if (err) {
                return callback(err);
            }
