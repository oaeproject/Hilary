...
var getBasicProfile = module.exports.getBasicProfile = function(ctx, userId, callback) {
    getUser(ctx, userId, function(err, user) {
        ...
        // To avoid littering the codebase with if (foo) { data.foo = foo }
        // we OR all falsy values to undefined.
        // Keys with undefined as value will be removed when outputting JSON in the REST endpoints
        data.smallPicture = user.smallPicture || undefined;
        data.mediumPicture = user.mediumPicture || undefined;
        data.largePicture = user.largePicture || undefined;

        data.profile.displayName = user.displayName || undefined;
        data.profile.publicAlias = user.publicAlias || undefined;
