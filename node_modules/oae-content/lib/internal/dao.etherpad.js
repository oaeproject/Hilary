 ...
 * @param  {Object}     callback.err    An error that occurred, if any
 */
var saveAuthorId = (module.exports.saveAuthorId = function(
  authorId,
  userId,
  callback,
) {
  ...
  Redis.getClient().set(key, userId, function(err) {
    if (err) {
      log().error(
        { err: err, authorId: authorId, userId: userId },
        'Failed to store Etherpad author mapping',
      );
      return callback({
        code: 500,
        msg: 'Failed to store Etherpad author mapping',
      });
    }
