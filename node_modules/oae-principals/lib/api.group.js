 ...
 * 
 * @param {String}          currentUserUuid The user performing this action
 ...
 */
module.exports.addGroupMember = function(currentUserUuid, groupUuid, memberUuid, role, callback) {
    callback = callback || function() {};
    role = role || ROLE_MEMBER;

    // ensure that the user is allowed to perform the action
    if (permissionsAPI.isAllowed(currentUserUuid, Constants.ROLE_MANAGER, groupUuid, function(err, isAllowed) {
        if (!err) {
            if (isAllowed) {
                ...
                callback({code: 401, msg: 'User '+currentUserUuid+' not authorized to add member to group '+groupUuid});
            }
        } else {
            callback(err);
        }
 ...
 *
 * @param {String}                  currentUserUuid The UUID of the principal performing this action
 ...
 */
module.exports.addGroupMembers = function(currentUserUuid, groupUuid, principalsToAdd, role, callback) {
    callback = callback || function() {};
    ...
    module.exports.getGroup(groupUuid, function(err, group) {
        ...
        permissionsAPI.isAllowed(currentUserUuid, Constants.ROLE_MANAGER, groupUuid, function(err, isAllowed) {
            if (!err) {
                if (isAllowed) {
                    ...
                    cassandra.runQuery('SELECT * FROM Principals WHERE principal_id IN (?)', [principalsToAdd], function (err, rows) {
                        ...
                        addMembers(groupUuid, principalsToAdd, callback);
                    });
                } else {
                    callback({code: 401, msg: 'User '+currentUserUuid+' not authorized to add members to group '+groupUuid});
                }
            } else {
                callback(err);
            }
 ...
 * @param {String}              currentUserUuid The UUID of the user performing the action
 ...
 */
module.exports.createGroup = function(tenant, currentUserUuid, title, description, callback) {
    callback = callback || function() {};
    var groupId = title.replace(/[:;<>#%{}|~`@%&!$,.="'\^\+\/\?\(\)\*\s\\\\\[\]]/gi, '-');
    var id = roleUtil.toUuid('g', tenant.alias, groupId);
    ...
    module.exports.getGroup(id, function(err, group) {
        ...
        cassandra.runQuery('INSERT INTO Principals (principal_id, tenant, group_title, group_description) VALUES (?, ?, ?, ?)', [id, tenant.alias, title, description], function (err) {
            if (err) {
                ...
                addMembers(id, [currentUserUuid], Constants.ROLE_MANAGER, function(err) {
                    if (!err) {
                        callback(null, id);
                    } else {
                        callback(err);
