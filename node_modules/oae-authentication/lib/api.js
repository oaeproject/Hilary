 ...
 * create the user with the provided password and make them a global admin
 *
 ...
 * @param  {Boolean}    callback.created    `true` if a new user was created with the provided username, `false` otherwise
 */
var getOrCreateGlobalAdminUser = module.exports.getOrCreateGlobalAdminUser = function(ctx, username, password, displayName, callback) {
    var validator = new Validator();
    ...
    var profileInfo = {'visibility': AuthzConstants.visibility.PRIVATE};
    var providerProperties = {'password': password};

    getOrCreateUser(ctx, AuthenticationConstants.providers.LOCAL, username, providerProperties, displayName, profileInfo, function(err, user, created) {
        if (err) {
            return callback(err);
        } else if (!created) {
 ...
 * @param  {Object}     providerProperties  The arbitrary properties associated the auto provider (e.g., `password` for local auth)
 ...
 * @param  {Function}   callback            Standard callback function
 * @param  {Object}     callback.err        An error that occured, if any
 * @param  {User}       callback.user       The user associated with the provided external id
