 ...
 */
var SearchAPI = module.exports = new events.EventEmitter();
 ...
 * @param  {Object}    producer.callback.errors     A set of errors that occurred while creating the documents, if any
 ...
 * @param  {Object}     options.producer.callback.errors    Any errors that occurred while creating the documents, if any
...
var _ensureSearchSchema = function(callback, _names) {
    if (!_names) {
        return client.putMapping(SearchConstants.search.MAPPING_RESOURCE, require('./schema/resourceSchema'), null, function(err) {
            if (err) {
                return callback(err);
            }
...
var _produceAllResourceDocuments = function(resourcesToIndex, callback, _resourceTypes, _documents) {
    ...
    if (searchDocumentProducer) {
        searchDocumentProducer(resourcesToIndex[resourceType], function(errors, documents) {
...
var _produceAllChildDocuments = function(resourceChildrenToIndex, callback, _documentTypes, _documents) {
    ...
    if (childSearchDocumentProducer) {
        childSearchDocumentProducer(resourceChildrenToIndex[documentType], function(errors, documents) {
            // Some resources might have triggered an error. We log those here,
            // but we try to include any documents that were generated
            _.each(errors, function(err) {
                log().error({'err': err}, 'Error producing child search documents from resources');
            });
