 ...
 * - `{private: true, tenantOverride: true, suppress: true}`    Values that only the global admin can view, you don't want to expose to tenant admins and end-users and want the tenant to be able to override (e.g. MediaCore integration)
 * - `{private: true, tenantOverride: false, suppress: true}`   Values that only the global admin can view, you don't want to expose to tenant admins and end-users and don't want the tenant to be able to override (e.g. Google Authentication)
 * - `{private: false, tenantOverride: true, suppress: false}`  Values that global and tenant admins can view, you want to expose to tenant admins and end-users and want the tenant to be able to override (e.g. default visibility for content)
 * - `{private: false, tenantOverride: false, suppress: true}`  Values that global and tenant admins can view, you don't want to expose to end-users and don't want tenants to be able to override (e.g. stopping a tenant)
 *
 ...
 * @param  {Boolean}    [options.private]   Whether or not this config element is only viewable by global admin users. Defaults to `false`
 * @api private
 */
var BaseField = function(type, name, description, defaultValue, options) {
    ...
    var field = {
        ...
        'private': options.private === true ? true : false
    };
 ...
 * @param  {Boolean}    [options.private]  Whether or not a configuration value can only be retrieved by a global administrator. If this is not explicitly set, private will be set to false
 */
