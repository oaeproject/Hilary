                                                ...
                                                client.subscribe(discussion.id, 'activity', {'expires': Date.now() + 10000, 'signature': 'foo', 'lastModified': Date.now()}, function(err) {
                                                    ...
                                                    RestAPI.Discussions.getDiscussion(branden.restContext, discussion.id, function(err, discussionForBranden) {
                                                        ...
                                                        client.subscribe(discussion.id, 'activity', discussionForBranden.signature, function(err) {
                                                            ...
                                                            // Sanity check
                                                            client.subscribe(discussion.id, 'activity', {'expires': discussion.signature.expires, 'signature': discussion.signature.signature}, function(err) {
                                                                assert.ok(!err);
                                                                return callback();
                                                            });
                                                        });
    ...
    describe('Notifications', function() {
        ...
        it('verify a new message triggers a push notification', function(callback) {
            setupFixture(function(contexts, discussion, client) {
                ...
                // Create a message
                RestAPI.Discussions.createMessage(contexts['branden'].restContext, discussion.id, 'Cup a Soup', null, function(err, discussionMessage) {
                    assert.ok(!err);
                    ...
                    client.on('message', function(message) {
                        ActivityTestsUtil.assertActivity(message.activity, DiscussionsConstants.activity.ACTIVITY_DISCUSSION_MESSAGE, ActivityConstants.verbs.POST, contexts['branden'].user.id, discussionMessage.id, discussion.id);
