...
describe('Authentication', function() {
    ...
    /**
     * Can be used to wrap some logic that requires an authentication strategy to be enabled
     *
     ...
     * @param  {Function}   enabledCallback.done    Standard function that you will have to invoke onces you're done with the strategy and it can be reset again
     * @param  {Function}   resetCallback           Standard function that will be invoked when the strategy has been reset
     */
    var enableStrategy = function(strategy, enabledCallback, resetCallback) {
        var strategyStatus = Config.getValue(global.oaeTests.tenants.global.alias, strategy, 'enabled');

        // Enable strategy
        var configUpdate = {};
        configUpdate['oae-authentication/' + strategy + '/enabled'] = true;
        ConfigTestUtil.updateConfigAndWait(globalAdminRestContext, null, configUpdate, function(err) {
            assert.ok(!err);
        });
        ...
        AuthenticationAPI.once(AuthenticationConstants.events.REFRESHED_STRATEGIES, function(tenant) {
            ...
            enabledCallback(function() {
                ...
                configUpdate['oae-authentication/' + strategy + '/enabled'] = strategyStatus;
                ConfigTestUtil.updateConfigAndWait(globalAdminRestContext, null, configUpdate, function(err) {
                    assert.ok(!err);
                });
                AuthenticationAPI.once(AuthenticationConstants.events.REFRESHED_STRATEGIES, function(tenant) {
    ...
    var verifyForward = function(strategy, host, callback) {
        enableStrategy(strategy, function(done) {
            var options = {
     ...
     *
     * @param  {String}     strategy    The authentication strategy to test
     * @param  {String}     method      The HTTP method to use
     * @param  {Object}     parameters  The parameters (either querystring or POST parameters) to send in the request
     * @param  {Function}   callback    Standard callback function
     */
    var verifyCallbackTampering = function(strategy, method, parameters, callback) {
        enableStrategy(strategy, function(done) {
            var options = {
                'uri': anonymousCamRestContext.host + '/api/auth/' + strategy + '/callback',
                'headers': {
                    'host': anonymousCamRestContext.hostHeader
                },
                'method': method,
            ...
            request(options, function(err, res, body) {
                ...
                assert.equal(body, 'Authentication denied');
                return done();
            });
        }, callback);
    };
    ...
    describe('External authentication', function() {
        ...
        it('verify the Shibboleth Service Provider metadata can be retrieved', function(callback) {
            ...
            ConfigTestUtil.updateConfigAndWait(globalAdminRestContext, 'camtest', configUpdate, function(err) {
                assert.ok(!err);
                ...
                enableStrategy('shibboleth', function(done) {
                    ...
                    var options = {
                        'uri': anonymousCamRestContext.host + '/api/auth/shibboleth/metadata',
                        'headers': {
                            'host': anonymousCamRestContext.hostHeader
                        }
                    };
                    request(options, function(err, res, body) {
                        ...
                        xml2js.parseString(body, function (err, result) {
                            ...
                            var redirectUrl = 'https://' + global.oaeTests.tenants.cam.host + '/api/auth/shibboleth/callback';
                            ...
                            ConfigTestUtil.clearConfigAndWait(globalAdminRestContext, 'camtest', _.keys(configUpdate), function(err) {
                                assert.ok(!err);
