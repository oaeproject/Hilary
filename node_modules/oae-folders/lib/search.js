...
FoldersAPI.on(FoldersConstants.events.CREATED_FOLDER, function(
  ...
  SearchAPI.postIndexTask(
    'folder',
    [{ id: folder.groupId, folderId: folder.id }],
    {
      resource: true,
      children: {
  ...
  AuthzSearch.fireMembershipUpdateTasks(
...
FoldersAPI.on(FoldersConstants.events.UPDATED_FOLDER, function(
  ...
  SearchAPI.postIndexTask(
    'folder',
    [{ id: updatedFolder.groupId, folderId: updatedFolder.id }],
    {
      resource: true,
    },
  );
 ...
 */
FoldersAPI.on(FoldersConstants.events.UPDATED_FOLDER_PREVIEWS, function(
  folder,
) {
  SearchAPI.postIndexTask(
    'folder',
...
FoldersAPI.on(FoldersConstants.events.UPDATED_FOLDER_MEMBERS, function(
  ...
  var principalIds = _.chain(memberChangeInfo.members.added)
 ...
 */
FoldersAPI.on(FoldersConstants.events.DELETED_COMMENT, function(
  ctx,
  message,
  folder,
  deleteType,
...
var _produceFolderMessageDocuments = function(resources, callback, _documents) {
  ...
  MessageBoxSearch.createAllMessageSearchDocuments(
    ...
    function(err, documents) {
      if (err) {
        log().warn(
          { err: err, resource: resource },
          'An error occurred producing message search documents',
        );
      }
...
SearchAPI.registerReindexAllHandler('folder', function(callback) {
  ...
  var _onEach = function(folderRows, done) {
    ...
    log().info(
      'Firing re-indexing task for %s folders',
      folderResources.length,
    );

    if (!_.isEmpty(folderResources)) {
      SearchAPI.postIndexTask('folder', folderResources, {
        resource: true,
        children: true,
      });
    }
