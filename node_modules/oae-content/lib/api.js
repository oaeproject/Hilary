...
/**
 * Get the members to a folder and return the roles with the members folder added
 *
 * @param  {Context}        ctx                 Standard context object containing the current user and the current tenant
 * @param  {String}         folderId            The ids of the folders to which this content item should be added
 * @param  {Object}         roles               Object where the keys represent principal ids that need to be added to the content upon creation and the values represent the role that principal will have. Possible values are "viewer" and "manager", as well as "editor" for collabdocs
 * @param  {String}         resourceSubType     The content item type. One of `file`, `collabdoc`, `link`
 * @param  {Function}       callback            Standard callback function
 * @param  {Object}         callback.err        An error that occurred, if any
 * @param  {Content}        callback.roles      Object where the keys represent principal ids that need to be added to the content upon creation and the values represent the role that principal will have. Possible values are "viewer" and "manager", as well as "editor" for collabdocs
 * @api private
 */
var _getFolderMembers = function(ctx, folder, roles, resourceSubType, callback) {

    if (!folder[0]) {
        return callback(null, roles);
    }

    // Get the members of the folder to add them to the content
    require('oae-folders').getFolderMembers(ctx, folder[0].id, null, null, function(err, memberList) {
        ...
        _.each(memberList, function(member) {
            if (ctx.user().id !== member.profile.id) {

                // If a member is a folder viewer and the content is a collabdoc, the member will become editor on the content
                if ((member.role === AuthzConstants.role.VIEWER) && (resourceSubType === 'collabdoc')) {
                    roles[member.profile.id] = AuthzConstants.role.EDITOR;
                } else {
                    roles[member.profile.id] = member.role;
                }
            }
        });
