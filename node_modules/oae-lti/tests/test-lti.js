        ...
        it('verify that a LTI tool can not be created in a deleted group', function(callback) {
            RestAPI.Group.createGroup(johnRestContext, 'This is a group', null, 'public', 'yes', [], [], function(err, group) {
                ...
                RestAPI.Group.deleteGroup(johnRestContext, group.id, function(err) {
                    ...
                    var secret = 'secret';
                    var key = '12345';
                    RestAPI.LtiTool.createLtiTool(johnRestContext, group.id, toolId, launchUrl, secret, key, 'LTI tool title', 'LTI tool description', function(err, toolObject) {
                        assert.ok(err);
    ...
    describe('Get LTI tools for a group', function() {
        ...
        it('verify retrieving LTI tools for a group', function(callback) {
            RestAPI.Group.createGroup(johnRestContext, 'This is a group', null, 'public', 'yes', [], [], function(err, group) {
                ...
                var title = 'LTI tool title';
                var description = 'LTI tool description';
                RestAPI.LtiTool.createLtiTool(johnRestContext, group.id, '12345', 'http://lti.launch1.url', secret, '12345', title, description, function(err, tool1) {
                    assert.ok(!err);
                    RestAPI.LtiTool.createLtiTool(johnRestContext, group.id, '12346', 'http://lti.launch2.url', secret, '12346', title, description, function(err, tool2) {
                        assert.ok(!err);
                        // Get the LTI tools for the group
                        RestAPI.LtiTool.getLtiTools(johnRestContext, group.id, function(err, ltiTools) {
                            assert.ok(!err);
                            assert.equal(ltiTools.length, 2);
                            assert.equal(ltiTools[0].groupId, group.id);
                            // Check that OAUTH secret is not included in the returned object
                            assert.ok(!ltiTools[0].secret);
                            var toolIds = _.pluck(ltiTools, 'toolId');
                            ...
                            RestAPI.Group.deleteGroup(johnRestContext, group.id, function(err) {
                                ...
                                RestAPI.LtiTool.getLtiTools(johnRestContext, group.id, function(err, ltiTools) {
                                    ...
                                    RestAPI.Group.createGroup(johnRestContext, 'This is a group', null, 'public', 'yes', [], [], function(err, group) {
                                        RestAPI.LtiTool.getLtiTools(johnRestContext, group.id, function(err, ltiTools) {
                                            assert.ok(!err);
                                            assert.equal(ltiTools.length, 0);
                                            return callback();
                                        });
                                    });
                                });
    ...
    describe('Delete LTI tool', function() {
        ...
        it('verify that a non-manager can not delete LTI tool', function(callback) {
            RestAPI.Group.createGroup(johnRestContext, 'This is a group', null, 'public', 'yes', [], [], function(err, group) {
                ...
                RestAPI.LtiTool.createLtiTool(johnRestContext, group.id, toolId, launchUrl, secret, key, 'LTI tool title', 'LTI tool description', function(err, toolObject) {
                    assert.ok(!err);
                    RestAPI.LtiTool.deleteLtiTool(anonymousRestContext, group.id, toolId, function(err) {
                        assert.ok(err) ;
                        assert.equal(err.code, 401);
                        assert.equal(err.msg, 'The current user does not have access to manage this resource');
                        return callback();
