...
var init = module.exports.init = function() {
    ...
    Pubsub.on('oae-tenant-networks', function(message) {
 ...
 */
var createTenantNetwork = module.exports.createTenantNetwork = function(displayName, callback) {
    var tenantNetwork = new TenantNetwork(ShortId.generate(), displayName);

    // Create a query that can be used to create the tenant network
    var tenantNetworkQuery = Cassandra.constructUpsertCQL('TenantNetwork', 'id', tenantNetwork.id, tenantNetwork, 'QUORUM');
    if (!tenantNetworkQuery) {
        ...
        return callback({'code': 500, 'msg': 'An unexpected error occurred while creating the tenant network'});
    }

    // Execute the create query
    Cassandra.runQuery(tenantNetworkQuery.query, tenantNetworkQuery.parameters, function(err) {
        if (err) {
            return callback(err);
        }
 ...
 */
var getAllTenantNetworks = module.exports.getAllTenantNetworks = function(callback) {
    // Verify the cache is populated
    _ensureCache(function(err) {
        if (err) {
            return callback(err);
        }
 ...
 */
var getTenantNetwork = module.exports.getTenantNetwork = function(id, callback) {
    _ensureCache(function(err) {
        if (err) {
            return callback(err);
        } else if (!_cacheTenantNetworks[id]) {
            return callback({'code': 404, 'msg': util.format('Attempted to access non-existing tenant network: "%s"', id)});
        }
 ...
 */
var updateTenantNetwork = module.exports.updateTenantNetwork = function(id, displayName, callback) {
    // Ensure the tenant network we're updating exists
    getTenantNetwork(id, function(err, tenantNetwork) {
        if (err) {
            return callback(err);
        }

        var query = Cassandra.constructUpsertCQL('TenantNetwork', 'id', id, {'displayName': displayName}, 'QUORUM');
        Cassandra.runQuery(query.query, query.parameters, function(err) {
            ...
            return callback(null, _.extend({}, tenantNetwork, {'displayName': displayName}));
        });
    });
};
 ...
 * @param  {Object}     callback.err    An error that occurred, if any
 */
var deleteTenantNetwork = module.exports.deleteTenantNetwork = function(id, callback) {
    ...
    getTenantNetwork(id, function(err, tenantNetwork) {
        ...
        Cassandra.runBatchQuery(deleteQueries, 'QUORUM', function(err) {
            if (err) {
                return callback(err);
            }
 ...
 * @param  {Object}     callback.err            An error that occurred, if any
 */
var addTenantAliases = module.exports.addTenantAliases = function(tenantNetworkId, tenantAliases, callback) {
    // Ensure the tenant network exists
    getTenantNetwork(tenantNetworkId, function(err, tenantNetwork) {
        if (err) {
            return callback(err);
        } else if (_.isEmpty(tenantAliases)) {
            return callback();
        }

        // Convert the tenantAliases into a hash so the aliases are column names and the value is some arbitrary value
        var tenantAliasesHash = {};
        _.each(tenantAliases, function(tenantAlias) {
            tenantAliasesHash[tenantAlias] = '1';
        });

        // Create and execute the query to persist all tenant aliases
        var query = Cassandra.constructUpsertCQL('TenantNetworkTenants', 'tenantNetworkId', tenantNetworkId, tenantAliasesHash, 'QUORUM');
        return Cassandra.runQuery(query.query, query.parameters, function(err) {
            if (err) {
                return callback(err);
            }
 ...
 */
var getAllTenantNetworkTenantAliases = module.exports.getAllTenantNetworkTenantAliases = function(callback) {
    _ensureCache(function(err) {
        if (err) {
            return callback(err);
        }
...
/**
 * Get a list of tenant aliases that belong to a specified tenant network.
 *
 * @param  {String}     id                      The id of the tenant network to fetch
 * @param  {Function}   callback                Standard callback function
 * @param  {Object}     callback.err            An error that occurred, if any
 * @param  {String[]}   callback.tenantAliases  The tenant aliases that belong to this tenant network
 */
var getTenantNetworkTenantAliases = module.exports.getTenantNetworkTenantAliases = function(id, callback) {
    // First get the tenant network to ensure it exists
    getTenantNetwork(id, function(err, tenantNetwork) {
        if (err) {
            return callback(err);
        }

        // Default to an array if the provided tenant network has no aliases associated to it
        return callback(null, clone(_cacheTenantAliasesByTenantNetworkId[id]) || []);
    });
};
 ...
 * @param  {Object}     callback.err        An error that occurred, if any
 */
var removeTenantAliases = module.exports.removeTenantAliases = function(tenantNetworkId, tenantAliases, callback) {
    // Ensure the tenant network exists
    getTenantNetwork(tenantNetworkId, function(err, tenantNetwork) {
        if (err) {
            return callback(err);
        ...
        // For every entry in the tenantAliases array, we create a placeholder that we will use in our delete Cassandra query
        var placeholders = _.map(tenantAliases, function(tenantAlias) { return '?'; });

        // Create and execute the delete query
        var cql = util.format('DELETE %s FROM TenantNetworkTenants USING CONSISTENCY QUORUM WHERE tenantNetworkId = ?', placeholders.join(','));
        var params = _.union(tenantAliases, tenantNetworkId);
        return Cassandra.runQuery(cql, params, function(err) {
            if (err) {
                return callback(err);
            }
 ...
 * @api private
 */
var _getAllTenantNetworksFromCassandra = function(callback) {
    Cassandra.runQuery('SELECT * FROM TenantNetwork USING CONSISTENCY QUORUM', null, function(err, rows) {
        if (err) {
            return callback(err);
        }

        // Convert all rows into tenant networks, filtering out all empty rows / tombstones
        var tenantNetworks = _.chain(rows).map(_rowToTenantNetwork).compact().indexBy('id').value();
 ...
 * @api private
 */
var _getAllTenantNetworkTenantAliasesFromCassandra = function(tenantNetworkIds, callback) {
    if (_.isEmpty(tenantNetworkIds)) {
        return callback(null, {});
    }

    // For each tenantNetworkId, add a ? to the placeholders array
    Cassandra.runQuery('SELECT * FROM TenantNetworkTenants USING CONSISTENCY QUORUM WHERE tenantNetworkId IN (?)', [tenantNetworkIds], function(err, rows) {
        if (err) {
            return callback(err);
        }

        var tenantNetworkAliases = {};
        _.each(rows, function(row) {
            var rowHash = Cassandra.rowToHash(row);
            tenantNetworkAliases[rowHash.tenantNetworkId] = _rowToTenantAliases(row);
        });
 ...
 * @api private
 */
var _rowToTenantNetwork = function(row) {
...
/**
 * Convert a Cassandra row into a list of tenant aliases.
 *
 * @param  {Row}        row     A Cassandra row that was queried from TenantNetworkTenants
 * @param  {String[]}           A list of tenant aliases that was represented by the row of data
 * @api private
 */
var _rowToTenantAliases = function(row) {
    row = Cassandra.rowToHash(row);

    // Get all the keys (column names) of the row, without the "tenantNetworkId" column
    var tenantAliases = _.chain(row).keys().without('tenantNetworkId').value();
 ...
 * @api private
 */
var _ensureCache = function(callback) {
    if (_cacheTenantNetworks) {
        return callback();
    }

    // Load all known tenant networks from Cassandra
    _getAllTenantNetworksFromCassandra(function(err, tenantNetworks) {
        if (err) {
            return callback(err);
        }

        // Load all known tenant network tenant associations from Cassandra
        _getAllTenantNetworkTenantAliasesFromCassandra(_.keys(tenantNetworks), function(err, tenantNetworkTenantAliases) {
            if (err) {
                return callback(err);
            }
            ...
            _.each(_cacheTenantAliasesByTenantNetworkId, function(tenantAliases, tenantNetworkId) {
                _.each(tenantAliases, function(tenantAlias) {
 ...
 * @api private
 */
var _invalidateAllCaches = function() {
    _invalidateLocalCache();
 ...
 * @api private
 */
var _invalidateLocalCache = function() {
