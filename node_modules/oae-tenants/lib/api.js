...
emitter.on('start', function(tenant) {
    _startTenant(tenant, function(err, disabled) {
        if (err) {
            log().error({err: err}, 'Could not initialize tenant %s', alias);
        } else {
            if (disabled) {
                log().info({tenant: tenant.alias}, 'Tenant (%s) on port %d initialized (disabled)', alias);
            } else {
                log().info({tenant: tenant.alias}, 'Tenant (%s) initialized.', alias);
            }
        }
    });
});

emitter.on('stop', function(alias) {
    _stopTenant(tenant, function(err) {
        if (err) {
            log().error({err: err, tenant: tenant.alias}, 'Error stopping tenant %s', alias);
        } else {
            log().info({tenant: tenant.alias}, 'Tenant %s stopped.', alias);
        }
    });
});
...
var initializeTenantMiddleware = module.exports.initializeTenantMiddleware = function(callback) {
    ...
    OAE.globalAdminServer.use(function(req, res, next) {
        req.tenant = {
            ...
            'alias': 'admin' //TODO Use config value.
        };
        return next();
    });
...
var createTenant = module.exports.createTenant = function(ctx, alias, name, host, callback) {
    ...
    validator.check(host, {'code': 400, 'msg': 'Missing hostname'}).notEmpty();
