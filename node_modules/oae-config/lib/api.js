...
var initConfig = module.exports.initConfig = function(_config, callback) {
    ...
    cacheSchema(function() {
        cacheAllTenantConfigs(function(err) {
            if (err) {
                return callback(err);
            }

            // Now that the config schema has been initialized, we can start listening to tenant
            // create events to resolve new tenant configs on the fly
...
var cacheTenantConfig = function(tenantAlias, callback) {
    ...
    Cassandra.runQuery('SELECT "configKey", "value", WRITETIME("value") AS "writetime" FROM "Config" WHERE "tenantAlias" = ?', [tenantAlias], function(err, rows) {
        if (err) {
            return callback(err);
        }
        ...
        _.each(rows, function(row) {
            persistentConfig[row.configKey] = {
                'value': row.value,
                'timestamp': new Date(Math.floor(row.writetime / 1000))
            };
        });
