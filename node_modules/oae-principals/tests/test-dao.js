    ...
    describe('#updatePrincipal', function() {
        ...
        it('verify updatePrincipal does not allow setting of restricted fields', function(callback) {
            TestsUtil.generateTestUsers(camAdminRestContext, 1, function(err, users, mrvisser) {
                ...
                PrincipalsDAO.updatePrincipal(mrvisser.id, {'publicAlias': 'haha'}, null, function(err) {
                    assert.ok(!err);
                    ...
                    PrincipalsDAO.getPrincipal(mrvisser.id, function(err, mrvisserRaw) {
                        ...
                        PrincipalsDAO.updatePrincipal(mrvisser.id, {'admin:tenant': true}, null, function(err) {
                            assert.ok(err);
                            assert.equal(err.code, 400);
                            assert.equal(err.msg, 'Attempted to update an invalid property');

                            PrincipalsDAO.updatePrincipal(mrvisser.id, {'admin:global': true}, null, function(err) {
                                assert.ok(err);
                                assert.equal(err.code, 400);
                                assert.equal(err.msg, 'Attempted to update an invalid property');

                                PrincipalsDAO.updatePrincipal(mrvisser.id, {'admin:tenant': true, 'admin:global': true}, null, function(err) {
                                    assert.ok(err);
                                    assert.equal(err.code, 400);
                                    assert.equal(err.msg, 'Attempted to update an invalid property');
