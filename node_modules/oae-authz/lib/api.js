 ...
 * @param  {Function}   callback                        Standard callback function
 * @param  {Object}     callback.err                    An error that occurred, if any
 * @param  {Object}     callback.rolesAfterChanges      An object that describes the roles on the resource after the changes have been applied. The key is the id of the principal, and the value is the role of the principal
 ...
 * @param  {Object}     callback.rolesBeforeChanges     An object that describes the roles on the resource before the changes have been applied. The key is the id of the principal, and the value is the role of the principal
 */
var computeMemberRolesAfterChanges = module.exports.computeMemberRolesAfterChanges = function(resourceId, permissionChanges, callback) {
    getAuthzMembers(resourceId, null, 10000, function(err, rolesAfterChangesArray) {
        if (err) {
            return callback(err);
        }

        // Turn the id->role array into a hash, we don't care about order
        var rolesBeforeChanges = {};
        _.each(rolesAfterChangesArray, function(memberIdWithRole) {
            rolesBeforeChanges[memberIdWithRole.id] = memberIdWithRole.role;
        });

        var rolesAfterChanges = _.extend({}, rolesBeforeChanges);
        var newMembers = [];
        var updatedMembers = [];
        var removedMembers = [];
        ...
        _.each(permissionChanges, function(change, memberId) {
            if (change === false) {
                if (rolesAfterChanges[memberId]) {
                    // The member `memberId` will be removed
                    delete rolesAfterChanges[memberId];
                    removedMembers.push(memberId);
                }
            } else {
                if (rolesAfterChanges[memberId] && rolesAfterChanges[memberId] !== change) {
                    // The member `memberId` will have a changed role
                    rolesAfterChanges[memberId] = change;
                    updatedMembers.push(memberId);
                } else if (!rolesAfterChanges[memberId]) {
        ...
        return callback(null, rolesAfterChanges, newMembers, updatedMembers, removedMembers, rolesBeforeChanges);
    });
};
...
var resolveEffectiveRole = module.exports.resolveEffectiveRole = function(user, resource, rolesPriority, callback) {
    if (user && _.isFunction(user.user)) {
        throw new Error('Needs to be updated to pass in user instead of ctx');
    }
    ...
    resolveImplicitRole(user, resource, rolesPriority, function(err, implicitRole, canInteract) {
        ...
        getAllRoles(user.id, _getAuthzId(resource), function(err, roles) {
            if (err) {
                return callback(err);
...
var resolveImplicitRole = module.exports.resolveImplicitRole = function(principal, resource, rolesPriority, callback) {
    ...
    var principalId = _getAuthzId(principal);
    var resourceId = _getAuthzId(resource);
