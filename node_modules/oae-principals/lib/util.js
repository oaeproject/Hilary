...
var getPrincipalFromRow = function(ctx, row) {
    if (module.exports.isGroup(row.get('principalId').value)) {
        ...
        var principalResource = AuthzUtil.getResourceFromId(user.id);
        if (ctx.user() && ctx.user().isAdmin && ctx.user().isAdmin(principalResource.tenantId)) {
            return user;
        }

        // Hide the sensitive profile information if needed.
        if ((user.visibility === VISIBILITY_OPTIONS.LOGGEDIN && !ctx.user()) ||
            (user.visibility === VISIBILITY_OPTIONS.PRIVATE && (!ctx.user() || ctx.user().id !== user.id))) {
            user.hide();
        }
 ...
 * @return {Boolean}                    True if the provided password matches the stored password, false if they are different
 */
 ...
 */
var cleanName = module.exports.cleanName = function(name) {
