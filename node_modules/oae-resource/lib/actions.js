...
var acceptInvitation = module.exports.acceptInvitation = function(ctx, token, callback) {
    var validator = new Validator();
    validator.check(null, {'code': 401, 'msg': 'Only authenticated users can accept an invitation'});
    if (validator.hasErrors()) {
        return callback(validator.getFirstError());
    }
        ...
        AuthzInvitationsDAO.getAllInvitationsByEmail(email, function(err, invitationHashes) {
            ...
            _applyAllMemberChanges(memberRolesByResourceId, function(err, changesByResourceId) {
                ...
                AuthzInvitationsDAO.deleteInvitations(email, function(err) {
                    ...
                    // been made
                    ResourceActions.emit(ResourceConstants.events.ACCEPTED_INVITATION, ctx, invitationHashes, changesByResourceId, function() {
                        return callback();
                    });
                });
            });
    ...
    _.each(memberChangesByResourceId, function(changes, resourceId) {
        ...
        AuthzAPI.computeMemberRolesAfterChanges(resourceId, changes, {'promotionOnly': true}, function(err, changes, roles, members) {
            if (err) {
                ...
                // Ignore this as the change was rejected, it should not be applied
                return _done();
            }
