...
var getClients = module.exports.Clients.getClients = function(ctx, userId, callback) {
    ...
    if (ctx.user().id !== userId && !ctx.user().isAdmin(userTenantAlias)) {
        return callback({'code': 401, 'msg': 'Only administrators can request another user their registered clients'});
    }
 ...
 * They can create a client *for another* user if they choose to do so. That client will be scoped,
 * to that user so the client application will *NOT* have tenant administrator privileges.
 *
 * @param  {Context}    ctx                 The context of the current request
