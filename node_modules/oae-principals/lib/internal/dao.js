...
/**
 * Iterate through all the principals. This will return just the principal properties that are specified in the `properties`
 * parameter, and only `batchSize` principals at a time. On each iteration of `batchSize` principals, the `onEach` callback
 * will be executed. When completed (e.g., there are 0 principals left to iterate through), the `onEach` callback will be
 * executed with 3rd parameter `true`. Principals will be fetched in a completely random order.
 *
 ...
 * @param  {Object}     onEach.principals   An object holding the raw principal data that was fetched from storage
...
var iterateAll = module.exports.iterateAll = function(properties, batchSize, onEach) {
    ...
    Cassandra.iterateAll(properties, 'Principals', 'principalId', batchSize, 'ONE', function(err, rows, complete) {
        if (err) {
            return onEach(err);
        ...
        // Invoke the callback with the hashed rows
        var hashedRows = [];
