    ...
    describe('Routes', function() {
        ...
        it('verify that publishing a collaborative document generates a notification', function(callback) {
            TestsUtil.generateTestUsers(camAdminRestContext, 3, function(err, users, simon, branden, nico) {
                ...
                RestAPI.Content.createCollabDoc(simon.restContext, name, 'description', 'public', [branden.user.id], [nico.user.id], function(err, contentObj) {
                    assert.ok(!err);
                    RestAPI.Content.joinCollabDoc(branden.restContext, contentObj.id, function(err, data) {
                        ...
                        etherpadClient.setText(args, function(err) {
                            assert.ok(!err);
                            Etherpad.addRecentAuthor(contentObj.id, contentObj.etherpadPadId, branden.user.id, function(err) {
                                assert.ok(!err);

                                // Set the email of simon so he gets an email
                                RestAPI.User.updateUser(simon.restContext, simon.user.id, {'email': 'simong@email.blah'}, function(err) {
                                    assert.ok(!err);
