...
module.exports.Group = function(tenant, id, alias, opts) {
    ...
    that.setPicture = function(size, uri) {
        ...
        var sig = Signature.createExpiresAtSignature(uri, expiresAt);
        that[size + 'Picture'] = util.format('/api/download/signed?uri=%s&signature=%s&expires=%s', uri, sig.signature, sig.expires);
        that[size + 'PictureUri'] = uri;
    };

    if (opts.smallPictureUri) {
        that.setPicture('small', opts.smallPictureUri);
    }
    if (opts.mediumPictureUri) {
        that.setPicture('medium', opts.mediumPictureUri);
    }
    if (opts.largePictureUri) {
        that.setPicture('large', opts.largePictureUri);
    }
