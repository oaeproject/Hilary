...
describe('Activity Email', function() {
    ...
    describe('Templates', function() {
         ...
         * configuration
         */
        it('verify footer template', function(callback) {
            TestsUtil.generateTestUsers(camAdminRestContext, 3, function(err, users, simong, nico, mrvisser) {
                ...
                EmailTestUtil.collectAndFetchAllEmails(function() {
                    ...
                    var hostingOrganizationURL = util.format('http://www.host.oaeproject.org/%s', TestsUtil.generateRandomText(1));

                    var instanceConfigFields = ['oae-tenants/instance/instanceName', 'oae-tenants/instance/instanceURL'];
                    var hostingOrganizationConfigFields = ['oae-tenants/instance/hostingOrganization', 'oae-tenants/instance/hostingOrganizationURL'];

                    var updateConfigInstance = {
                        'oae-tenants/instance/instanceName': instanceName,
                        'oae-tenants/instance/instanceURL': instanceURL
                    };

                    var updateConfigHostingOrganization = {
                     ...
                     */
                    var _assertEmailFooter = function(message, assertions) {
                        ...
                        if (assertions.expectInstanceName) {
                            assert.notEqual(message.html.indexOf(instanceName), -1);
                            assert.notEqual(message.text.indexOf(instanceName), -1);
                        } else {
                        ...
                        if (assertions.expectHostingOrganizationName) {
                            assert.notEqual(message.html.indexOf(hostingOrganization), -1);
                            assert.notEqual(message.text.indexOf(hostingOrganization), -1);
                        } else {
                        ...
                        if (assertions.expectHostingOrganizationURL) {
                            assert.notEqual(message.html.indexOf(util.format('<a\n href=\"%s\"\n ', Sanitization.encodeForHTMLAttribute(hostingOrganizationURL))), -1);
                    ...
                    ConfigTestUtil.clearConfigAndWait(globalAdminRestContext, null, _.union(instanceConfigFields, hostingOrganizationConfigFields), function(err) {
                        assert.ok(!err);

                        // Create a link, let Nico manage it
                        RestAPI.Content.createLink(mrvisser.restContext, 'Google', 'Google', 'public', 'http://www.google.ca', [nico.user.id], [], [], function(err, link) {
                            assert.ok(!err);
                            ...
                            EmailTestUtil.collectAndFetchAllEmails(function(messages) {
                                ...
                                ConfigTestUtil.updateConfigAndWait(globalAdminRestContext, null, updateConfigInstance, function(err) {
                                    assert.ok(!err);
                                    RestAPI.Content.updateContent(mrvisser.restContext, link.id, {'displayName': 'Update 1'}, function(err, link) {
                                        ...
                                        EmailTestUtil.collectAndFetchAllEmails(function(messages) {
                                            ...
                                            ConfigTestUtil.clearConfigAndWait(globalAdminRestContext, null, instanceConfigFields, function(err) {
                                                assert.ok(!err);
                                                ConfigTestUtil.updateConfigAndWait(globalAdminRestContext, null, updateConfigHostingOrganization, function(err) {
                                                    assert.ok(!err);
                                                            ...
                                                            ConfigTestUtil.updateConfigAndWait(globalAdminRestContext, null, updateConfigInstance, function(err) {
                                                                ...
                                                                RestAPI.Content.updateContent(mrvisser.restContext, link.id, {'displayName': 'Update 3'}, function(err, link) {
                                                                    ...
                                                                    EmailTestUtil.collectAndFetchAllEmails(function(messages) {
                                                                        ...
                                                                        var urlFields = ['oae-tenants/instance/instanceURL', 'oae-tenants/instance/hostingOrganizationURL'];
                                                                        ConfigTestUtil.clearConfigAndWait(globalAdminRestContext, null, urlFields, function(err) {
                                                                            assert.ok(!err);
                                                                            RestAPI.Content.updateContent(mrvisser.restContext, link.id, {'displayName': 'Update 3'}, function(err, link) {
                                                                                assert.ok(!err);
                                                                                EmailTestUtil.collectAndFetchAllEmails(function(messages) {
     ...
     * profiles
     */
    it('verify email is not delivered to users who have since deleted/restored their profile', function(callback) {
                            ...
                            PrincipalsTestUtil.assertDeleteUserSucceeds(camAdminRestContext, camAdminRestContext, mrvisser.user.id, function() {
                                ContentTestUtil.assertCreateLinkSucceeds(simong.restContext, 'displayName', 'description', 'public', 'http://www.google.ca/firstlink', null, [group.id], null, function(link2) {
                                    EmailTestUtil.collectAndFetchAllEmails(function(messages) {
                                        ...
                                        // Restore mrvisser and ensure the email is not send to mrvisser even if he has been restored because
                                        // he lost all those rights when his profiled was delete
                                        PrincipalsTestUtil.assertRestoreUserSucceeds(camAdminRestContext, mrvisser.user.id, function() {
                                            ContentTestUtil.assertCreateLinkSucceeds(simong.restContext, 'displayName', 'description', 'public', 'http://www.google.ca/firstlink', null, [group.id], null, function(link2) {
                                                EmailTestUtil.collectAndFetchAllEmails(function(messages) {
                                                    assert.strictEqual(messages.length, 1);

                                                    // Ensure the recipients is nico
                                                    var recipient = _.chain(messages)
                                                        .pluck('headers')
                                                        .pluck('to')
                                                        .value()
                                                        .sort();
                                                    assert.deepEqual(recipient, [nico.user.email]);
     ...
     */
    it('verify email markdown parsing', function(callback) {
        ...
        TestsUtil.generateTestUsers(camAdminRestContext, 2, function(err, users, user1, user2) {
            ...
            RestAPI.User.updateUser(user1.restContext, user1.user.id, {'emailPreference': 'immediate'}, function(err) {
                assert.ok(!err);

                // Create a discussion on which to comment
                RestAPI.Discussions.createDiscussion(user1.restContext, 'Test Discussion', 'Test Description', 'public', null, [user2.user.id], function(err, discussion) {
                    assert.ok(!err);
                    ...
                    var htmlBody = [
                        ...
                        '<p', /* style inserted here */ '><code', /* style inserted here */ '>Preformatted Text</code></p>',
                        '<h1', /* style inserted here */ '>First Level Heading</h1>',
                        '<p', /* style inserted here */ '>Paragraph with<br>line break</p>',
                        '<p', /* style inserted here */ '>&lt;script&gt;alert(&quot;XSS attack&quot;)&lt;/script&gt;</p>'
                    ];

                    RestAPI.Discussions.createMessage(user2.restContext, discussion.id, markdownBody, null, function(err, comment) {
                        assert.ok(!err);
                        ...
                        EmailTestUtil.collectAndFetchAllEmails(function(messages) {
                            ...
                            _.each(htmlBody, function(htmlFragment) {
                                assert.notEqual(mail.html.indexOf(htmlFragment), -1, htmlFragment);
                            });
     ...
     */
    it('verify email date timezones', function(callback) {
        TestsUtil.generateTestUsers(camAdminRestContext, 2, function(err, users, simong, nico) {
            assert.ok(!err);

            var alias = TenantsTestUtil.generateTestTenantAlias();
            var host = TenantsTestUtil.generateTestTenantHost();
            TestsUtil.createTenantWithAdmin(alias, host, function(err, tenant, restContext, admin) {
                assert.ok(!err);

                TestsUtil.generateTestUsers(restContext, 1, function(err, users, bert) {
                    assert.ok(!err);
                    ...
                    ConfigTestUtil.updateConfigAndWait(restContext, null, {'oae-tenants/timezone/timezone': 'Etc/GMT+5'}, function(err) {
                        ...
                        RestAPI.Content.createLink(simong.restContext, 'Google', 'Google', 'public', 'http://www.google.ca', [], [nico.user.id, bert.user.id], [], function(err, link) {
                            ...
                            EmailTestUtil.collectAndFetchAllEmails(function(messages) {
                                ...
                                var utcMessage = _.find(messages, function(message) { return (message.to[0].address === nico.user.email); });
                                var plus5Message = _.find(messages, function(message) { return (message.to[0].address === bert.user.email); });

                                // Parse the dates out of the messages
                                var utcDate = utcMessage.html.match(/\d\d\d\d-\d\d-\d\d \d\d:\d\d:\d\d/g)[0];
                                var plus5Date = plus5Message.html.match(/\d\d\d\d-\d\d-\d\d \d\d:\d\d:\d\d/g)[0];

                                // Assert that the offsets are correct
                                var utcHour = parseInt(utcDate.substr(11, 2), 10);
                                var plus5Hour = parseInt(plus5Date.substr(11, 2), 10);
                                assert.equal((utcHour + 5) % 24, plus5Hour);
                                return callback();
                            });
                        });
                    });
    ...
    it('verify basic checks in email template', function(callback) {
        TestsUtil.generateTestUsers(camAdminRestContext, 2, function(err, users, simong, nico) {
            ...
            refreshConfiguration(null, false, false, {}, function(config) {
                ...
                var displayName = TestsUtil.generateRandomText(3);
                    ...
                    RestAPI.Content.createLink(simong.restContext, 'Link #2', 'Google', 'public', 'http://www.google.ca', [], [nico.user.id], function(err, link) {
                        ...
                        ActivityTestUtil.collectAndGetActivityStream(nico.restContext, null, null, function(err, activityStream) {
                            ...
                            EmailTestUtil.collectAndFetchAllEmails(function(messages) {
                                ...
                                EmailTestUtil.waitForEmail(function(messages) {
                                    assert.strictEqual(messages.length, 1);
    ...
    it('verify email for 2 items being created for a group do not result in lines that are too long', function(callback) {
        TestsUtil.generateTestUsers(camAdminRestContext, 3, function(err, users, simong, nico) {
            ...
            TestsUtil.generateTestGroups(simong.restContext, 1, function(group) {
                ...
                PrincipalsTestUtil.assertSetGroupMembersSucceeds(simong.restContext, simong.restContext, group.id, members, function() {
