...
const log = require('oae-logger').logger('oae-discussions');
...
const Tenant = require('oae-tenants/lib/model').Tenant;
...
const DiscussionsConstants = require('./constants').DiscussionsConstants;
...
DiscussionsAPI.on(DiscussionsConstants.events.CREATED_DISCUSSION, (ctx, discussion) => {
    ...
    const objectResource = new ActivityModel.ActivitySeedResource('discussion', discussion.id, {discussion});
    const activitySeed = new ActivityModel.ActivitySeed(DiscussionsConstants.activity.ACTIVITY_DISCUSSION_CREATE, millis, ActivityConstants.verbs.CREATE, actorResource, objectResource);
...
DiscussionsAPI.on(DiscussionsConstants.events.UPDATED_DISCUSSION, (ctx, newDiscussion, oldDiscussion) => {
    const millis = Date.now();
    const actorResource = new ActivityModel.ActivitySeedResource('user', ctx.user().id, {'user': ctx.user()});
    const objectResource = new ActivityModel.ActivitySeedResource('discussion', newDiscussion.id, {'discussion': newDiscussion});
    ...
    const activitySeed = new ActivityModel.ActivitySeed(activityType, millis, ActivityConstants.verbs.UPDATE, actorResource, objectResource);
    ActivityAPI.postActivity(ctx, activitySeed);
});
...
DiscussionsAPI.on(DiscussionsConstants.events.CREATED_DISCUSSION_MESSAGE, (ctx, message, discussion) => {
    const millis = Date.now();
    const actorResource = new ActivityModel.ActivitySeedResource('user', ctx.user().id, {'user': ctx.user()});
    const objectResource = new ActivityModel.ActivitySeedResource('discussion-message', message.id, {'discussionId': discussion.id, message});
    const targetResource = new ActivityModel.ActivitySeedResource('discussion', discussion.id, {discussion});
    const activitySeed = new ActivityModel.ActivitySeed(DiscussionsConstants.activity.ACTIVITY_DISCUSSION_MESSAGE, millis, ActivityConstants.verbs.POST, actorResource, objectResource, targetResource);
    ActivityAPI.postActivity(ctx, activitySeed);
});
...
DiscussionsAPI.on(DiscussionsConstants.events.UPDATED_DISCUSSION_MEMBERS, (ctx, discussion, memberChangeInfo, opts) => {
    ...
    const millis = Date.now();
    const actorResource = new ActivityModel.ActivitySeedResource('user', ctx.user().id, {'user': ctx.user()});
    const discussionResource = new ActivityModel.ActivitySeedResource('discussion', discussion.id, {discussion});
...
const _discussionProducer = function(resource, callback) {
    const discussion = (resource.resourceData && resource.resourceData.discussion) ? resource.resourceData.discussion : null;
...
var _createPersistentDiscussionActivityEntity = function(discussion) {
    return new ActivityModel.ActivityEntity('discussion', discussion.id, discussion.visibility, {discussion});
};
...
const _discussionMessageProducer = function(resource, callback) {
    const discussionId = resource.resourceData.discussionId;
    const message = resource.resourceData.message;
...
var _transformPersistentDiscussionActivityEntity = function(ctx, entity) {
    const discussion = entity.discussion;
 ...
 */
ActivityAPI.registerActivityEntityAssociation('discussion', 'message-contributors', (associationsCtx, entity, callback) => {
