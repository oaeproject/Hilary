 ...
 * @param  {String}     callback.visibility The maximum visibility (as enumerated in `LibraryConstants.visibility.ALL_PRIORITY`) of library the user in context can see from the target library owner
 */
var resolveTargetLibraryVisibility = module.exports.resolveTargetLibraryVisibility = function(ctx, libraryOwnerId, callback) {
    ...
    if (ctx.user() && (ctx.user().isGlobalAdmin() || ctx.user().isTenantAdmin(libraryTenantAlias))) {
        return callback(null, LibraryConstants.visibility.PRIVATE);
    }

    // User is not an admin of the target library tenant, so we'll have to do some acrobatics to resolve the visibility
    // We'll need to determine the visibility of the library owner first.
    PrincipalsUtil.getPrincipal(ctx, libraryOwnerId, function(err, principal) {
        if (err) {
            return callback(err);
        }
        ...
        if (PrincipalsUtil.isUser(libraryOwnerId)) {
            ...
            else if (principal.visibility === LibraryConstants.visibility.PRIVATE || (principal.visibility === LibraryConstants.visibility.LOGGEDIN && !TenantsUtil.isLoggedIn(ctx, libraryTenantAlias))) {
                return callback({'code': 401, 'msg': 'This user\'s library is not visible.'});
            }
            // We give the user the public stream in case the user is not logged in to the user's tenant
                ...
                return callback(null, LibraryConstants.visibility.PUBLIC);
            // We give the user the logged in stream in case the current user is not the same as the library we're looking at
                ...
                return callback(null, LibraryConstants.visibility.LOGGEDIN);
            }
        ...
        // Check if we are looking at a group library
            ...
            if (!TenantsUtil.isLoggedIn(ctx, libraryTenantAlias)) {
                if (principal.visibility === LibraryConstants.visibility.PUBLIC) {
                    return callback(null, LibraryConstants.visibility.PUBLIC);
                } else {
                    return callback({'code': 401, 'msg': 'This group\'s library is not visible to anonymous users.'});
                }
            }
            ...
            AuthzAPI.hasAnyRole(ctx.user().id, libraryOwnerId, function(err, hasAnyRole) {
                ...
                if (principal.visibility === LibraryConstants.visibility.PRIVATE && !hasAnyRole) {
                    return callback({'code': 401, 'msg': 'This user\'s library is not visible.'});
                }

                // If the user has a role we can show them the private stream.
                else if (hasAnyRole) {
                    return callback(null, LibraryConstants.visibility.PRIVATE);
                }

                // If the user is not a member, but is authenticated to the group's tenant, we give him the logged in stream
                else if (!hasAnyRole && TenantsUtil.isLoggedIn(ctx, libraryTenantAlias)) {
                    return callback(null, LibraryConstants.visibility.LOGGEDIN);
...
var resolveEffectiveLibraryVisibilities = module.exports.resolveEffectiveLibraryVisibilities = function(libraryOwnerIds, resource, callback) {
    ...
    _.each(libraryOwnerIds, function(principalId) {
        ...
        if (effectiveVisibility === LibraryConstants.visibility.LOGGEDIN) {
            ...
            if (principalTenantAlias !== resource.tenant.alias) {
                effectiveVisibility = LibraryConstants.visibility.PRIVATE;
            }
        }
