 ...
 * @param  {String}     role                        The role asked by the user
 * @param  {Function}   callback                    Standard callback function
 * @param  {Object}     callback.err                An error that occured, if any
 */
var createRequestJoinGroup = module.exports.createRequestJoinGroup = function(ctx, groupId, role, callback) {

    var validator = new Validator();
    validator.check(role, {'code': 400, 'msg': role + ' is not a recognized role group'}).isIn([AuthzConstants.role.MEMBER, AuthzConstants.role.MANAGER]);
    validator.check(null, {'code': 401, 'msg': 'You have to be logged in to be able to ask to join a group'}).isLoggedInUser(ctx);
    validator.check(groupId, {'code': 402, 'msg': 'A valid group id must be provided'}).isGroupId();
    if (validator.hasErrors()) {
        return callback(validator.getFirstError());
    }

    // If the request exist, return 
    PrincipalsDAO.createRequestJoinGroup(ctx.user().id, groupId, role, function(err, request) {
        if (err) {
            return callback(err);
        }
        ...
        PrincipalsDAO.getPrincipal(groupId, function(err, group) {
            ...
            // Notify managers that someone ask to be part of this group
            return PrincipalsEmitter.emit(PrincipalsConstants.events.REQUEST_TO_JOIN_GROUP, ctx, group, callback);
        });
    });
};

/**
 * Get all request related to a group
 *
 * @param  {Context}    ctx                         Standard context object containing the current user and the current tenant
 * @param  {String}     groupId                     The group id
 * @param  {Function}   callback                    Standard callback function
 * @param  {Object}     callback.err                An error that occured, if any
 */
var getRequestsJoinGroup = module.exports.getRequestsJoinGroup = function(ctx, groupId, start, limit, callback) {
    limit = OaeUtil.getNumberParam(limit, 10, 1);
    ...
    // If the request exist, return 
    PrincipalsDAO.getRequestsJoinGroup(groupId, function(err, requests) {
        if (err) {
            return callback(err);
        }
        ...
        var principals = [];
        _.each(JSON.parse(requests.principalId), function(r, i, arr) {
            var id = Object.keys(r);
            id = id[0];
            
            PrincipalsDAO.getPrincipal(id, function(err, principal) {
                if (err) {
                    return callback(err);
                }
                var obj = {};
                obj.role = r[id];
                obj.profile = principal;
                principals.push(obj)

                if (i === arr.length - 1) {
                    return callback(null, principals, null);
                }
            });
        });
    });
 ...
 * @param  {Object}     callback.err                An error that occured, if any
 */
var getRequestJoinGroup = module.exports.getRequestJoinGroup = function(ctx, groupId, callback) {
    ...
    // If the request exist, return 
    PrincipalsDAO.getRequestJoinGroup(ctx.user().id, groupId, function(err, request) {
        if (err) {
            return callback(err);
        }
...
/**
 * Update a request from the data base. A user can update his own request. 
 *
 * @param  {Context}    ctx                         Standard context object containing the current user and the current tenant
 * @param  {String}     groupId                     The group id
 * @param  {String}     role                        The role asked by the user
 ...
 * @param  {Object}     callback.err                An error that occured, if any
 */
var updateRequestJoinGroup = module.exports.updateRequestJoinGroup = function(ctx, groupId, role, callback) {
    ...
    PrincipalsDAO.getRequestJoinGroup(ctx.user().id, groupId, function(err, request) {
        ...
        PrincipalsDAO.getPrincipal(groupId, function(err, group) {
            ...
            PrincipalsDAO.updateRequestJoinGroup(ctx.user().id, groupId, role, function(err) {
                if (err) {
                    return callback(err);
                }
                return callback(null);
...
/**
 * Remove a request from the data base. A user can remove his own request. 
 * Each time a manager accept or reject a request, the request must be removed from the database
 *
 * @param  {Context}    ctx                         Standard context object containing the current user and the current tenant
 * @param  {String}     groupId                     The group id
 * @param  {Function}   callback                    Standard callback function
 * @param  {Object}     callback.err                An error that occured, if any
 */
var removeJoinGroupByRequest = module.exports.removeJoinGroupByRequest = function(ctx, groupId, callback) {
    ...
    PrincipalsDAO.getRequestJoinGroup(ctx.user().id, groupId, function(err, request) {
        ...
        // Remvove the request
        PrincipalsDAO.removeJoinGroupByRequest(ctx.user().id, groupId, function(err) {
            if (err) {
                return callback(err);
            }
 ...
 * @param  {Object}     callback.err    An error that occurred, if any
 */
var acceptJoinGroupByRequest = module.exports.acceptJoinGroupByRequest = function(ctx, groupId, principalId, role, callback) {
    ...
    var constRole = '';
    if (role === 'manager') {
        constRole = AuthzConstants.role.MANAGER;
    } else if (role === 'member') {
        constRole = AuthzConstants.role.MEMBER;
    }

    var changes = {};
    changes[principalId] = constRole;

    // Add the user in the group
    setGroupMembers(ctx, groupId, changes, function(err) {
        if (err) {
            return callback(err);
        }

        // Remove the request
        PrincipalsDAO.removeJoinGroupByRequest(principalId, groupId, function(err) {
            if (err) {
                return callback(err);
            }
 ...
 * @param  {Object}     callback.err                An error that occured, if any
 */
var rejectJoinGroupByRequest = module.exports.rejectJoinGroupByRequest = function(ctx, groupId, principalId, callback) {
    ...
    AuthzAPI.hasRole(ctx.user().id, groupId, AuthzConstants.role.MANAGER, function(err, isManager) {
        if (err) {
            return callback(err);
        } else if (!isManager) {
            return callback({'code': 400, 'msg': 'You must be a manager of this group to validate a request'});
        }
