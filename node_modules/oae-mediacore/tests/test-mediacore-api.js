    ...
    beforeEach(function(callback) {
        ...
        TestsUtil.createTestServer(function(_mediaCoreApp, _server, _port) {
            ...
            var update = {
                ...
                'oae-mediacore/mediacore/url': util.format('http://127.0.0.1:%s', port),
                'oae-mediacore/mediacore/keyId': 'key',
                'oae-mediacore/mediacore/secret': 'secret',
                'oae-mediacore/mediacore/collectionId': 12345
            };
            ConfigTestUtil.updateAllConfigAndWait(globalAdminRestContext, null, update, function(err) {
                assert.ok(!err);
                return callback();
            });
        });
                ...
                RestAPI.Content.createFile(user.restContext, 'test video', null, 'private', _getVideoStream, null, null, function(err, content) {
                    ...
                    RestAPI.MediaCore.getEmbedCode(user.restContext, 'not-an-id', function(err) {
                        ...
                        RestAPI.MediaCore.getEmbedCode(user.restContext, 'c:camtest:non-existing-content', function(err) {
                            ...
                            // Verfy we get 400 when fetching embed code for content item who doesn't have a mediaCoreId
                        ...
                        MediaCoreDAO.saveContentRevisionId(mediaCoreId, content.id, content.latestRevisionId, function(err) {
                            ...
                            RestAPI.MediaCore.notifyEncodingComplete(user.restContext, mediaCoreId, function(err) {
                                ...
                                RestAPI.Content.getContent(user.restContext, content.id, function(err, content) {
                                    ...
                                    // Verify the URIs
                                    assert.equal(content.previews.smallUri, 'remote:http://path/to/large/image');
                                    assert.equal(content.previews.mediumUri, 'remote:http://path/to/720p/image');
                                    assert.equal(content.previews.largeUri, 'remote:http://path/to/720p/image');
                                    assert.equal(content.previews.wideUri, 'remote:http://path/to/720p/image');
