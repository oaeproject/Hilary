...
var createFolder = module.exports.createFolder = function(ctx, displayName, description, visibility, members, callback) {
    ...
    _getExistingPrincipals(principalIds, null, function(err, principals) {
        ...
        AuthzPermissions.canInteract(ctx, principals, function(err) {
            ...
            var managerIds = _.filter(principalIds, function(principalId) {
                return (members[principalId] === FoldersConstants.roles.MANAGER);
            });
...
var shareFolder = module.exports.shareFolder = function(ctx, folderId, principalIds, callback) {
    ...
    FoldersDAO.getFolder(folderId, function(err, folder) {
        ...
        _getExistingPrincipals(principalIds, null, function(err, principals) {
            ...
            AuthzPermissions.canShare(ctx, folder, {'principals': principals}, function(err, newMembers, memberRoles) {
                if (err) {
                    return callback(err);
                } else if (_.isEmpty(newMembers)) {
                    // If the target user already has the item in their library, this will be empty
                    // and we shouldn't perform any updates
                    return callback();
                }
                ...
                var roleChanges = {};
                _.each(newMemberIds, function(newMemberId) {
                    roleChanges[newMemberId] = AuthzConstants.role.VIEWER;
                });

                // Get all the members who already had this folder in their library so we can update
                // their libraries
                var allMemberIdsBeforeShare = _.chain(memberRoles).keys().difference(newMemberIds).value();
...
var setFolderPermissions = module.exports.setFolderPermissions = function(ctx, folderId, permissionChanges, callback) {
    ...
    FoldersDAO.getFolder(folderId, function(err, folder) {
        ...
        _getExistingPrincipals(principalIds, null, function(err, principals) {
            ...
            AuthzPermissions.canSetRoles(ctx, folder, {'principalRoles': principalRoles}, function(err, membershipAfterChanges, newMembers, updatedMembers, removedMembers) {
                if (err) {
                    return callback(err);
                }
...
var removeFolderFromLibrary = module.exports.removeFolderFromLibrary = function(ctx, principalId, folderId, callback) {
    ...
    FoldersDAO.getFolder(folderId, function(err, folder) {
        ...
        PrincipalsDAO.getPrincipal(principalId, function(err, principal) {
            ...
            AuthzPermissions.canRemoveRole(ctx, principal, folder, function(err) {
                if (err) {
                    return callback(err);
                }
