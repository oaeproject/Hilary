 ...
 */
var UIAPI = (module.exports = new EmitterAPI.EventEmitter());
var emitter = UIAPI;
...
var init = (module.exports.init = function(_uiDirectory, _hashes, callback) {
  ...
  if (process.env['NODE_ENV'] !== 'production') {
    watch.createMonitor(uiDirectory, { ignoreDotFiles: true }, function(
      monitor,
    ) {
  ...
  if (/^\/node_modules\/(.*?)\/manifest.json$/.test(filename)) {
    ...
    _cacheSkinVariables(function(err) {
      if (err) {
        log().error(
          { err: err },
          'Could not cache the skin variables after a file update.',
        );
      }
    });
  ...
  readdirp({
    ...
    .on('data', function(entry) {
      ...
      try {
        ...
        log().error(
          { err: err, widgetId: widgetId, path: entry.fullPath },
    ...
    .on('warn', function(err) {
      log().warn(
        { err: err },
        'A non-fatal error occured whilst caching a widget manifest',
      );
    })
    .on('error', function(err) {
      log().error(
        { err: err },
        'A fatal error occured whilst caching a widget manifest',
      );
    });
};
 ...
 */
var getStaticBatch = (module.exports.getStaticBatch = function(
  files,
  callback,
) {
  var validator = new Validator();
  validator
    .check(null, { code: 400, msg: 'The files parameter must be an array' })
    .isArray(files);
 ...
 */
var getLogo = (module.exports.getLogo = function(ctx, callback) {
 ...
 */
var getSkinVariables = (module.exports.getSkinVariables = function(
  ctx,
  tenantAlias,
  callback,
) {
  ...
  if (!ctx.user() || !ctx.user().isAdmin(tenantAlias)) {
    return callback({
      code: 401,
      msg: 'Only administrators can retrieve the skin variables',
    });
  }
 ...
 */
var getTenantSkinVariables = (module.exports.getTenantSkinVariables = function(
  tenantAlias,
) {
  ...
  var resolvedVariables = _.extend({}, defaultVariables, tenantVariables);
 ...
 * @api private
 */
var _renderDynamicValue = function(obj, key, tenantAlias) {
  var value = obj[key];
  if (_.isString(value)) {
 ...
 */
var uploadLogoFile = (module.exports.uploadLogoFile = function(
  ctx,
  file,
  tenantAlias,
  callback,
  ...
  if (!ctx.user() || !ctx.user().isAdmin(tenantAlias)) {
    return callback({
      code: 401,
      msg: 'Only administrators can upload new logos for tenants',
    });
  }
  ...
  ContentUtil.getStorageBackend(ctx).store(
    ...
    function(err, uri) {
      ...
      var signedUrl = ContentUtil.getSignedDownloadUrl(ctx, uri, -1, -1);
...
var _cacheI18nKeys = function(callback) {
  _cacheI18nKeysInDirectory('/shared/oae/bundles', null, function(err) {
    ...
    return _cacheI18nKeysInDirectory(
      '/node_modules',
      _widgetDirectoryFilter,
      callback,
    );
  });
};
...
var _cacheI18nKeysInDirectory = function(directory, directoryFilter, callback) {
  ...
  readdirp({
    ...
    .on('warn', function(err) {
      log().warn(
        { err: err },
        'A non-fatal error occured whilst reading the i18n bundles',
      );
    })
    .on('error', function(err) {
      log().error(
        { err: err },
        'A fatal error occured whilst reading the i18n bundles',
      );
    })
    .on('end', done);
};
...
var _cacheBundleFile = function(bundlePath, callback) {
  ...
  getStaticFile(bundlePath, function(err, data) {
    if (err) {
      log().error(
        { err: err, path: bundlePath },
        'Could not read an i18n bundle',
      );
 ...
 */
var renderTemplate = (module.exports.renderTemplate = function(
  template,
  data,
  locale,
) {
  ...
  _.extend(data.util, {
    html: {
      ...
      /*!
             ...
             */
      toText: function(str, retainLinks) {
    ...
    i18n: {
      ...
      /*!
             ...
             */
      formatDate: function(date, dateFormat, timezone) {
        ...
        var kludgeDate = new TZ.Date(date.valueOf(), timezone);
        kludgeDate.setMinutes(
          kludgeDate.getMinutes() + kludgeDate.getTimezoneOffset() * 2,
        );

        // Return a properly formatted date
        var formatted = Globalize.format(
          kludgeDate,
          dateFormat,
          globalizeLocale,
        );
...
var compileTemplate = (module.exports.compileTemplate = function(template) {
  ...
  template = template.replace(/<%\s*include\s*(.*?)\s*%>/g, function(
    match,
    path,
  ) {
 ...
 */
var getHashedPath = (module.exports.getHashedPath = function(path) {
 ...
 */
var getIso3166CountryInfo = (module.exports.getIso3166CountryInfo = function() {
