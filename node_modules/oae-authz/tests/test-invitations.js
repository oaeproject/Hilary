...
describe('Invitations', function() {
    ...
    var _testInvitationsPartialRemoveRoles = function(resourceType, callback) {
        ...
        TestsUtil.generateTestUsers(camAdminRestContext, 2, function(err, users, userSetRoles, userAccept) {
            assert.ok(!err);

            // Create 2 resources with the email address invited
            fns.createSucceeds(userSetRoles.restContext, 'public', [], [email], function(resource1) {
                fns.createSucceeds(userSetRoles.restContext, 'public', [], [email], function(resource2) {

                    // Remove the email from one of the resources
                    var roles = _.oaeObj(email, false);
                    fns.setRolesSucceeds(userSetRoles.restContext, userSetRoles.restContext, resource1.id, roles, function() {

                        // Accept the email invitation as the invited email user, ensuring they get access
                        // to just the one resource (resource2)
                        var assertions = {'role': memberRole, 'membersSize': 2, 'librarySize': 1};
                        _assertAcceptEmailInvitation(userSetRoles, userAccept, [resource2], assertions, function() {
                            return callback();
                        });
                    });
                });
