...
var log = require('oae-logger').logger('content');
...
var createFile = module.exports.createFile = function(ctx, name, description, visibility, additionalMembers, file, callback) {
    callback = callback || function() {};
    _createContent(ctx, 'file', name, description, visibility, additionalMembers, {'file': file}, function(err, content) {
        callback(err, content);

        if (!err) {
            emitter.emit(ContentConstants.events.created, content);
        }
    });
};
...
var _createContent = function(ctx, contentType, name, description, visibility, additionalMembers, otherValues, callback) {
    ...
    if (contentType === 'file') {
        if (!otherValues.file) {
            return callback({'code': 400, 'msg': 'Missing file parameter'});
        }
        validator.check(otherValues.file.mime, {'code': 400, 'msg': 'Missing mimetype on the file object.'}).notEmpty();
        validator.check(otherValues.file.size, {'code': 400, 'msg': 'Missing size on the file object.'}).notEmpty();
        validator.check(otherValues.file.filename, {'code': 400, 'msg': 'Missing filename on the file object.'}).notEmpty();
    }
    if (validator.hasErrors()) {
        return callback(validator.getFirstError());
    }
    ...
    // Remove the file object if there is any.
    var file = null;
    if (otherValues.file) {
        file = otherValues.file;
        delete otherValues.file;
        otherValues.mime = file.mime;
        otherValues.size = file.size;
        otherValues.filename = file.filename;
    }
    ...
    Cassandra.runQuery(q.query, q.parameters, function(err) {
        ...
        _setContentPermissions(ctx, contentObj, additionalMembers, function(err) {
            ...
            var createdRevision = function(err, revision) {
                if (err) {
                    return callback(err);
                }
                ...
                var opts = {
                    ...
                    'openStream': function() {
                        ...
                        stream.on('error', function(err) {
                            log().error({'err': err}, 'Error reading from stream %s', file.path);
                        });
                        return stream;
                    }
                };
                contentObj.filename = file.filename;
                contentObj.size = file.size;
                contentObj.mime = file.mime;
                _getBackend(ctx).store(ctx, opts, function(err, uri) {
                    if (err) {
                        return callback(err);
                    }
                    // Create the revision.
                    delete opts.openStream;
                    _createRevision(ctx, contentObj.contentId, uri, opts, createdRevision);
                });
            } else if (contentType === 'sakaidoc') {
                // TODO: Flesh out
                var uri = 'sakaidoc:' + contentObj.contentId;
                _createRevision(ctx, contentObj.contentId, uri, {}, createdRevision);
            } else {
                // Falltrough for links.
                callback(null, contentObj);
            }
        });
    });
...
var deleteContent = module.exports.deleteContent = function(ctx, contentId, callback) {
    ...
    canManage(ctx, contentId, function(err, canManage, contentObj) {
        ...
        Cassandra.runQuery('DELETE FROM Content WHERE contentId = ?', [contentId], function(err, rows) {
            ...
            // Async file body deletion.
            // If it's a file, delete the file body.
            if (contentObj.contentType === 'file') {
                _getRevisions(ctx, contentId, null, 1000, function(err, revisions) {
                    if (err) {
                        return log().error({'err': err}, 'An error occured while removing files.');
                    } else if (revisions.length === 0) {
                        return log().error('A file (%s) was attempted to be removed that had no revision.', contentId);
                    }

                    var removeFile = function() {
                        var revision = revisions.shift();
                        _getBackend(ctx, revision.uri).remove(ctx, revision.uri, function(err) {
                            if (err) {
                                return callback(err);
                            }
                            if (revisions.length !== 0) {
                                ...
                                // All files are gone, delete the revisions in Cassandra.
                                _deleteRevisions(ctx, contentId, revisions);
                            }
                        });
                    };
                    removeFile();
...
var _hasAccess = function(ctx, contentObj, callback) {
    var contentTenant = AuthzUtil.getResourceFromId(contentObj.contentId).tenantId;
    // Global admins or tenant admins for this piece of content can see everything,
    // regardless of the content visibility.
    if (ctx.user() && ctx.user().isAdmin(contentTenant)) {
        callback(null, contentObj);
    // If the content is public, I always get access
    } else if (contentObj.visibility === ContentConstants.visibility.PUBLIC) {
        callback(null, contentObj);
    ...
    canManage(ctx, contentId, function(err, isManager, contentObj) {
        ...
        _getBackend(ctx).store(ctx, opts, function(err, uri) {
            ...
            _createRevision(ctx, contentObj.contentId, uri, opts, function(err, revision) {
                ...
                // Update the recent meta data on the content item.
                ...
                Cassandra.runQuery(q.query, q.parameters, function(err) {
                    ...
                    callback(err);
                    emitter.emit(ContentConstants.events.updated, contentObj);
                });
            });
        });
    });
...
/**
 * Attaches preview items to a piece of content.
 ...
 * @param {String}      [revisionId]    The revision ID these previews should be attached to. Can be null and they will be attached to the latest one.
 * @param {File[]}      files           An array of express files object.
 * @param {Function}    callback        Standard callback method
 * @param {Object}      callback.err    Standard error object (if any.)
 */
var addPreviewItems = module.exports.addPreviewItems = function(ctx, contentId, revisionId, files, callback) {
    // Check for authentication before we do any expensive checks.
    var contentTenant = AuthzUtil.getResourceFromId(contentId).tenantId;
    if (!ctx.user() || !ctx.user().isAdmin(contentTenant)) {
        return callback({'code': 401, 'msg': 'Unauthorized'});
    }

    getRevision(ctx, contentId, revisionId, function(err, revision, contentObj) {
        if (err) {
            return callback(err);
        }
        ...
        fileKeys.forEach(function(key) {
            var options = {
                ...
                'size': files[key].size,
                'openStream': function() {
                    var stream = fs.createReadStream(files[key].path);
                    stream.on('error', function(err) {
                        log().error({'err': err}, 'Error reading from stream %s', files[key].path);
                    });
                    return stream;
                }
            };
            ...
            _getBackend(ctx).store(ctx, options, function(err, uri) {
                if (err) {
                    return callback(err);
                }
                ...
                values[options.filename] = uri;
                todo--;
                if (todo === 0) {
                    var q = Cassandra.constructUpsertCQL('ContentPreviews', 'contentId', contentId, values, 'QUORUM');
                    Cassandra.runQuery(q.query, q.parameters, function(err) {
                        if (err) {
                            return callback(err);
                        }
...
/**
 * Get the link to a preview item.
 * @param  {Context}    ctx             The current context.
 * @param  {String}     contentId       The content ID that the preview item is associated with.
 * @param  {String}     item            The preview item that needs to be retrieved.
 * @param  {String}     signature       A signature that van validate this call.
 * @param  {Number}     expires         When the signature expires.
 * @param  {Function}   callback        Standard callback method.
 * @param  {Object}     callback.err    Standard error object (if any)
 * @param  {Object}     callback.link   Object that holds a `code` and `link`.
 */
var getPreviewItem = module.exports.getPreviewItem = function(ctx, contentId, item, signature, expires, callback) {
    var validator = new Validator();
    validator.check(contentId, {'code': 400, 'msg': 'Missing content ID'}).notEmpty();
    validator.check(item, {'code': 400, 'msg': 'Missing item ID'}).notEmpty();
    validator.check(signature, {'code': 400, 'msg': 'Missing signature'}).notEmpty();
    validator.check(expires, {'code': 400, 'msg': 'Missing expires'}).notEmpty();
    validator.check(expires, {'code': 400, 'msg': 'Missing expires'}).isInt();
    validator.check(signature, {'code': 401, 'msg': 'Invalid signature.'}).verifyExperingSignature(contentId, expires);
    if (validator.hasErrors()) {
        return callback(validator.getFirstError());
    }

    Cassandra.runQuery('SELECT ? FROM ContentPreviews WHERE contentId = ?', [item, contentId], function(err, rows) {
        if (err) {
            return callback(err);
        }

        // Cassandra always returns the key as a column so the _colCount will always be 1.
        var col = rows[0].get(item);
        if (!col || col.name !== item) {
            console.log(rows[0]);
            return callback({'code': 404, 'msg': "Couldn't find item " + item + " for contentId: " + contentId});
        }

        var uri = col.value;
        var link = _getBackend(ctx, uri).getDownloadLink(ctx, uri);
        callback(null, link);
    });
};

/**
 * Get the entire list of preview items that is associated to a content item.
 * @param {Context}     ctx             The current context.
    ...
    hasAccess(ctx, contentId, function(err, hasAccess) {
        ...
        Cassandra.runQuery('SELECT * FROM ContentPreviews WHERE contentId = ?', [contentId], function(err, rows) {
            ...
            if (rows[0]._colCount <= 1) {
                return callback({'code': 404, 'msg': "Couldn't find preview items for contentId: " + contentId});
            }

            // Generate a signature that expires in 5 minutes.
            var signature = Signature.createExpiringSignature(contentId, 5);

            // Dump the files
            var hash = Cassandra.rowToHash(rows[0]);
            delete hash.contentId;
            var files = _.keys(hash);
            var result = {
                'files': files,
                'signature': signature
            };
 ...
 */
var _getBackend = function(ctx, uri) {
    ...
    if (!_backends[backendName]) {
        try {
            ...
            // TODO: Find a better way to deal with an unfound storage backend.
...
/**
 * Creates and stores a revision in Cassandra.
 *
 * @param {Context}     ctx                 Current execution context.
 ...
 */
var createRevision = module.exports.createRevision = function(ctx, contentId, uri, opts, callback) {
    opts = opts || {};
    callback = callback || function(err) {
        if (err) {
            log().error({'err': err}, 'Error creating revision for %s', contentId);
        }
    };

    var validator = new Validator();
    validator.check(contentId, {'code': 400, 'msg': 'There must be a contentId associated to a revision'}).notEmpty();
    validator.check(uri, {'code': 400, 'msg': 'The URI cannot be empty'}).notEmpty();
    validator.check(null, {'code': 401, 'msg': 'You have to be logged in to be able to create a revision'}).isLoggedInUser(ctx);
    if (validator.hasErrors()) {
        return callback(validator.getFirstError());
    }
    ...
    _canManage(ctx, contentId, function(err, isManager) {
        ...
        _createRevision(ctx, contentId, uri, opts, callback);
    });
};
...
/**
 * Internal method to delete revisions for a content item.
 * @param {Context}     ctx                 Current execution context.
 * @param {String}      contentId           The contentId this revision is for.
 * @param {Object}      [opts]              Extra options that should be stored on the revision.
 * @param {Function}    [callback]          An optional callback method.
 * @param {Object}      callback.err        An error object (if any)
 */
var _deleteRevisions = function(ctx, contentId, revisions, callback) {
    callback = callback || function(err) {
        if (err) {
            log().error({'err': err}, 'An error occurred when trying to remove all revisions for %s', contentId);
        }
    };
    ...
    for (var i = 0; i < revisions.length; i++) {
        parameters.push(revisions[i].created);
        q.push('?');

        // One query per revision.
        queries.push({
            'query': 'DELETE FROM Revisions WHERE revisionId=?',
            'parameters': [revisions[i].revisionId]
        });
    }

    // One query to remove each revision from a piece of content.
    parameters.push(contentId);
    queries.push({
        'query': 'DELETE ' + q.join(',') + ' FROM RevisionByContent WHERE contentId=?',
...
var getRevisions = module.exports.getRevisions = function(ctx, contentId, start, limit, callback) {
    ...
    var validator = new Validator();
    validator.check(contentId, {'code': 400, 'msg': 'A contentId must be provided'}).notEmpty();
    validator.check(limit, {'code': 400, 'msg': 'A valid limit should be passed in'}).isInt();
    if (validator.hasErrors()) {
        return callback(validator.getFirstError());
    }
 ...
 * @param  {Context}        ctx                 Standard context object, representing the currently logged user and its tenant
 * @param  {String}         contentId           The id of the object for which we want to get the revision
 * @param  {String}         revisionId          The id of the revision that needs to be retrieved.
 * @param  {Function}       callback            Standard callback function with arguments `err`, `revision` and `contentObj`
 * @param  {Object}         callback.err        Error object containing the error message
 * @param  {Revision}       callback.revision   The retrieved revision
 * @param  {Revision}       callback.contentObj The associated content object
 */
var getRevision = module.exports.getRevision = function(ctx, contentId, revisionId, callback) {
    ...
    if (!revisionId) {
        // Get the latest one.
        return getLatestRevision(ctx, contentId, callback);
    }
    ...
    getContent(ctx, contentId, function(err, contentObj) {
        ...
        Cassandra.runQuery('SELECT * FROM Revisions WHERE RevisionId = ?', [revisionId], function (err, rows) {
            ...
            callback(null, revisionObj, contentObj);
        });
    });
};
...
var _getRevisions = function(ctx, contentId, start, limit, callback) {
    ...
    Cassandra.runQuery('SELECT FIRST ' + paging.limit + ' REVERSED ?..\'\' FROM RevisionByContent USING CONSISTENCY QUORUM WHERE contentId = ?', [paging.start, contentId], function (err, rows) {
        ...
        Cassandra.runQuery('SELECT * FROM Revisions WHERE revisionId IN (?)', [revisionsToRetrieve], function(err, rows) {
            ...
            var revisions = [];
            for (var i = 0; i < rows.length; i++) {
                var revision = _rowToRevision(contentId, rows[i]);
            ...
            PrincipalsUtil.getPrincipals(ctx, userIds, function(err, users) {
                ...
                for (var r = 0; r < revisions.length; r++) {
                    if (users[revisions[r].createdBy]) {
                        ...
                        revisions[r].link = _getBackend(ctx, revisions[r].uri).getDownloadLink(ctx, revisions[r].uri);
                    }
                }
...
var _rowToRevision = function(contentId, row) {
    ...
    return new Revision(contentId, hash.revisionId, hash.created, hash.createdBy, hash.uri, opts);
};
