...
var publishCollabDoc = module.exports.publishCollabDoc = function(ctx, contentId, callback) {
    ...
    canManage(ctx, contentId, function(err, canManage, contentObj) {
        ...
        // We only go through this cycle if we're a recent author of this document
        // This is to avoid generating revisions and/or activities by users who didn't actually make any changes
        Etherpad.isRecentAuthor(contentObj.id, contentObj.etherpadPadId, ctx.user().id, function(err, isRecent) {
            if (err) {
                return callback(err);
            } else if (!isRecent) {
                // This user didn't make any changes which means we can return early
                return callback(null, contentObj);
            }
 ...
 */
var getSignedPreviewDownload = module.exports.getSignedPreviewDownload = function(ctx, contentId, revisionId, previewItem, signatureData, callback) {
    var validator = new Validator();
    ...
    ContentDAO.Previews.getContentPreview(revisionId, previewItem, function(err, preview) {
        ...
        return callback(null, {'filename': previewItem, 'downloadStrategy': downloadStrategy});
    });
};
...
var verifySignedDownloadQueryString = module.exports.verifySignedDownloadQueryString = function(ctx, qs, callback) {
    ...
    return callback(null, {'downloadStrategy': downloadStrategy, 'filename': Path.basename(uri)});
};
