...
describe('Activity Email', function() {
    ...
    beforeEach(function(callback) {
        ...
        EmailTestsUtil.clearEmailCollections(function() {
            refreshConfiguration(false, false, {}, function(config) {
                return callback();
            });
        });
    });
     ...
     * @api private
     */
    var refreshConfiguration = function(expectDaily, expectWeekly, extraConfig, callback) {
                        ...
                        RestAPI.Content.createLink(simong.restContext, 'Google', 'Google', 'public', 'http://www.google.ca', [], [nico.user.id], function(err, fourthLink) {
                            ...
                            RestAPI.Discussions.createDiscussion(simong.restContext, 'First discussion', 'descr', 'public', null, [nico.user.id], function(err, firstDiscussion) {
                                ...
                                EmailTestsUtil.collectAndFetchEmails(function(messages) {
                                    ...
                                    assert.ok(messages[0]['_message'].html);
                                    ...
                                    var contentIndex = messages[0]['_message'].html.indexOf(firstLink.profilePath);
                                    var discussionIndex = messages[0]['_message'].html.indexOf(firstDiscussion.profilePath);
                                    ...
                                    RestAPI.Discussions.createDiscussion(simong.restContext, 'Second discussion', 'descr', 'public', null, [nico.user.id, mrvisser.user.id], function(err, secondDiscussion) {
                                        ...
                                        EmailTestsUtil.collectAndFetchEmails(function(messages) {
                                            ...
                                            _.each(messages, function(message) {
                                                ...
                                                assert.equal(messages[0]['_message'].html.indexOf(firstLink.profilePath), -1);
                                                assert.equal(messages[0]['_message'].html.indexOf(secondLink.profilePath), -1);
                                                assert.equal(messages[0]['_message'].html.indexOf(thirdLink.profilePath), -1);
                                                assert.equal(messages[0]['_message'].html.indexOf(fourthLink.profilePath), -1);
                                                assert.equal(messages[0]['_message'].html.indexOf(firstDiscussion.profilePath), -1);
    ...
    it('verify aggregation in a stream is stopped when an email is sent', function(callback) {
        TestsUtil.generateTestUsers(camAdminRestContext, 3, function(err, users, nico, branden,  simong) {
            ...
            RestAPI.Content.createLink(simong.restContext, 'Link #1', 'Google', 'public', 'http://www.google.be', [], [branden.user.id], function(err, firstContentObj) {
                ...
                EmailTestsUtil.collectAndFetchEmails(function(messages) {
                    assert.equal(messages.length, 1);
                    assert.equal(messages[0]['_message'].to, branden.user.email);
                    assert.ok(messages[0]['_message'].html.indexOf(firstContentObj.displayName) > 0);
                    assert.ok(messages[0]['_message'].html.indexOf(firstContentObj.profilePath) > 0);
                    ...
                    RestAPI.Content.createLink(simong.restContext, 'Link #2', 'Google', 'public', 'http://www.google.be', [], [branden.user.id], function(err, secondContentObj) {
                        ...
                        EmailTestsUtil.collectAndFetchEmails(function(messages) {
                            assert.equal(messages.length, 1);
                            assert.equal(messages[0]['_message'].to, branden.user.email);
                            assert.equal(messages[0]['_message'].html.indexOf(firstContentObj.displayName), -1);
                            assert.equal(messages[0]['_message'].html.indexOf(firstContentObj.profilePath), -1);
                            assert.ok(messages[0]['_message'].html.indexOf(secondContentObj.displayName) > 0);
                            ...
                            RestAPI.Discussions.createDiscussion(nico.restContext, 'Discussion', 'Discussion description', 'public', [], [branden.user.id], function(err, discussion) {
                                ...
                                EmailTestsUtil.collectAndFetchEmails(function(messages) {
                                    ...
                                    assert.equal(messages[0]['_message'].to, branden.user.email);
                                    assert.equal(messages[0]['_message'].html.indexOf(firstContentObj.displayName), -1);
                                    assert.equal(messages[0]['_message'].html.indexOf(secondContentObj.displayName), -1);
                                    assert.ok(messages[0]['_message'].html.indexOf(discussion.displayName) > 0);
                            ...
                            refreshConfiguration(false, false, {}, function(config) {
                                ...
                                RestAPI.Content.createLink(simong.restContext, 'Google', 'Google', 'public', 'http://www.google.be', [], [neverMailUser.user.id, immediateMailUser.user.id, dailyMailUser.user.id, weeklyMailUser.user.id], function(err, firstLink) {
                                    ...
                                    EmailTestsUtil.collectAndFetchEmails(function(messages) {
                                        ...
                                        refreshConfiguration(true, false, {}, function(config) {
                                            ...
                                            RestAPI.Content.createLink(simong.restContext, 'Google', 'Google', 'public', 'http://www.google.be', [], [neverMailUser.user.id, immediateMailUser.user.id, dailyMailUser.user.id, weeklyMailUser.user.id], function(err, secondLink) {
                                                ...
                                                EmailTestsUtil.collectAndFetchEmails(function(messages) {
                                                    ...
                                                    refreshConfiguration(false, true, {}, function(config) {
                                                        ...
                                                        RestAPI.Content.createLink(simong.restContext, 'Google', 'Google', 'public', 'http://www.google.be', [], [neverMailUser.user.id, immediateMailUser.user.id, dailyMailUser.user.id, weeklyMailUser.user.id], function(err, thirdLink) {
                                                            ...
                                                            EmailTestsUtil.collectAndFetchEmails(function(messages) {
                                                                ...
                                                                refreshConfiguration(true, true, {}, function(config) {
    ...
    it('verify sending emails uses the configured tenant default timezone', function(callback) {
        ...
        refreshConfiguration(true, false, {}, function(config) {
            ...
            var createTenant = function() {
                ...
                // Create a tenant
                var alias = TestsUtil.generateRandomText(1);
                ...
                EmailTestsUtil.clearEmailCollections(function() {
                    ...
                    TestsUtil.generateTestUsers(camAdminRestContext, 1, function(err, users, simong) {
                        ...
                        RestAPI.Content.createLink(simong.restContext, 'Google', 'Google', 'public', 'http://www.google.ca', [], userIds, function(err, link) {
                            ...
                            EmailTestsUtil.collectAndFetchEmails(function(messages) {
                                assert.equal(messages.length, 1);
                                assert.equal(messages[0]['_message'].to, receivingUser);
            ...
            ConfigTestUtil.updateConfigAndWait(restContext, null, {'oae-principals/user/timezone': 'Etc/GMT+5'}, function(err) {
                ...
                TestsUtil.generateTestUsers(restContext, 2, function(err, users, simong, nico) {
                    ...
                    RestAPI.User.updateUser(nico.restContext, nico.user.id, {'emailPreference': 'daily'}, function(err) {
                        ...
                        // Configure the mail collector so daily mails are collected later
                        refreshConfiguration(false, false, {}, function(config) {
                            ...
                            RestAPI.Content.createLink(simong.restContext, 'Google', 'Google', 'public', 'http://www.google.ca', [], [nico.user.id], function(err, link) {
                                ...
                                // As Nico is in a UTC+5 timezone, his mail should not be collected now
                                EmailTestsUtil.collectAndFetchEmails(function(messages) {
                                    assert.equal(messages.length, 0);

                                    // If we manually collect the daily emails that are scheduled for the UTC+5 timezone, Nico's mail should be sent out
                    ...
                    RestAPI.User.updateUser(nico.restContext, nico.user.id, {'emailPreference': 'weekly'}, function(err) {
                        ...
                        ConfigTestUtil.updateConfigAndWait(restContext, null, {'oae-principals/user/timezone': 'Etc/GMT+5'}, function(err) {
                            ...
                            RestAPI.Content.createLink(simong.restContext, 'Google', 'Google', 'public', 'http://www.google.ca', [], [nico.user.id], function(err, link) {
                                ...
                                ActivityTestsUtil.collectAndGetActivityStream(nico.restContext, nico.user.id, null, function(err) {
                                    assert.ok(!err);
    ...
    /**
     * Test that verifies that old activities are not included in an email when they are not situated in the email interval
     */
    it('verify old activities in the email stream are not included', function(callback) {
        // Set the email polling frequency to 1 second so we can test that "old" activities
        // are not included in new collection cycles. Unfortunately we can't do this through
        // the ActivityAPI as that does a minimum check
        var config = ActivitySystemConfig.getConfig();
        config.mail.pollingFrequency = 1;

        TestsUtil.generateTestUsers(camAdminRestContext, 2, function(err, users, simong, nico) {
            assert.ok(!err);

            // Set Nico his email preference to never
            RestAPI.User.updateUser(nico.restContext, nico.user.id, {'emailPreference': 'never'}, function(err) {
                assert.ok(!err);

                // When we retrieve the emails for the immediate feed (see lower) we get all the emails in the
                // email stream from up to 1 hour ago. If we want to verify that we don't include older activities
                // we need to create an activity with a created timestamp that is older than 1 hour. Unfortunately,
                // there's no other way than to temporarily mock the `Date.now` function
                var now = Date.now();
                Date.now = function() {
                    return now - 3 * 60 * 60 * 1000;
                };

                RestAPI.Content.createLink(simong.restContext, 'Google', 'Canadian Google', 'public', 'http://www.google.ca', [], [nico.user.id], function(err, link) {
                    assert.ok(!err);

                    // Deliver the activity to the email stream
                    ActivityTestsUtil.collectAndGetActivityStream(nico.restContext, null, null, function(err, activityStream) {
                        assert.ok(!err);

                        Date.now = _originalDateNow;

                        // Change Nico's email preference to immediate
                        RestAPI.User.updateUser(nico.restContext, nico.user.id, {'emailPreference': 'immediate'}, function(err) {
                            assert.ok(!err);

                            // In the immediate collection cycle we collect all the activities that were delivered
                            // to an email stream up untill `pollingFrequency` milliseconds ago. To assert that this
                            // functionality is working, we wait a bit longer than the pollingFrequence, trigger another
                            // activity and collect all the emails. Only the last activity should be included in the email
                            setTimeout(RestAPI.Discussions.createDiscussion, 1050, simong.restContext, 'discussion', 'discussion', 'public', [], [nico.user.id], function(err, discussion) {
                                assert.ok(!err);

                                // Collect the emails
                                EmailTestsUtil.collectAndFetchEmails(function(messages) {
                                    assert.strictEqual(messages.length, 1);
                                    assert.strictEqual(messages[0]['_message'].to, nico.user.email);

                                    // Assert the link is not included in the email
                                    assert.strictEqual(messages[0]['_message'].html.indexOf(link.profilePath), -1);
                                    assert.strictEqual(messages[0]['_message'].html.indexOf(link.displayName), -1);

                                    // Assert the discussion is present
                                    assert.notEqual(messages[0]['_message'].html.indexOf(discussion.profilePath), -1);
                                    assert.notEqual(messages[0]['_message'].html.indexOf(discussion.displayName), -1);
                                    return callback();
                                });
                            });
                        });
                                        ...
                                        EmailTestsUtil.collectAndFetchEmails(function(messages) {
                                            ...
                                            RestAPI.Content.createLink(simong.restContext, 'Google', 'Google', 'public', 'http://www.google.ca', [], [branden.user.id, bert.user.id], function(err, thirdLink) {
                                                ...
                                                EmailTestsUtil.collectAndFetchEmails(function(messages) {
                                                    ...
                                                    var brandenMessage = _.find(messages, function(message) { return (message['_message'].to === branden.user.email); });
                                                    var bertMessage = _.find(messages, function(message) { return (message['_message'].to === bert.user.email); });
                                                    assert.notEqual(brandenMessage['_message'].subject, bertMessage['_message'].subject);
                                                    return callback();
                                                });
                                            });
                                        });
                            ...
                            refreshConfiguration(true, true, {}, function(config) {
                                ...
                                RestAPI.Content.createLink(simong.restContext, 'Google', 'Google', 'public', 'http://www.google.ca', [], [nico.user.id, branden.user.id, bert.user.id, stuart.user.id], function(err, link) {
                                    ...
                                    ActivityTestsUtil.collectAndGetActivityStream(nico.restContext, null, null, function(err, activityStream) {
                                        ...
                                        ActivityTestsUtil.markNotificationsAsRead(nico.restContext, function(result) {
                                            ActivityTestsUtil.markNotificationsAsRead(branden.restContext, function(result) {
                                                ActivityTestsUtil.markNotificationsAsRead(bert.restContext, function(result) {
                                                    ActivityTestsUtil.markNotificationsAsRead(stuart.restContext, function(result) {
                                                        ...
                                                        EmailTestsUtil.collectAndFetchEmails(function(messages) {
                                                            assert.equal(messages.length, 2);
                                                            _.each(messages, function(message) {
                                                                assert.ok(_.contains([branden.user.email, bert.user.email], message['_message'].to));
                                                            });
                    ...
                    ActivityTestsUtil.collectAndGetActivityStream(nico.restContext, null, null, function(err, activityStream) {
                        ...
                        ActivityAPI.once(ActivityConstants.events.UPDATED_USER, function() {
                            ...
                            EmailTestsUtil.collectAndFetchEmails(function(messages) {
                                ...
                                // Note: we cannot sanity check that he receives the daily email by using refreshConfiguration
                                // as that moves the hour on which the daily email gets collected.
                                return callback();
                            });
                        });
                    });
    ...
    it('verify two distinct aggregated activities aggregate during email collection', function(callback) {
        TestsUtil.generateTestUsers(camAdminRestContext, 3, function(err, users, simong, nico, branden) {
            ...
            refreshConfiguration(false, false, {'collectionPollingFrequency': -1, 'aggregateIdleExpiry': 1}, function(config) {
                ...
                RestAPI.Content.createLink(simong.restContext, 'Link A', 'Link A', 'public', 'http://www.google.com', [], [], function(err, contentObj) {
                    ...
                    RestAPI.Content.createComment(nico.restContext, contentObj.id, 'Comment A', null, function(err, commentA) {
                        ...
                        RestAPI.Content.createComment(branden.restContext, contentObj.id, 'Comment B', null, function(err, commentB) {
                            ...
                            ActivityTestsUtil.collectAndGetActivityStream(simong.restContext, null, null, function(err, activityStream) {
                                assert.ok(!err);
                                    ...
                                    RestAPI.Content.createComment(branden.restContext, contentObj.id, 'Comment D', null, function(err, commentD) {
                                        ...
                                        EmailTestsUtil.collectAndFetchEmails(function(messages) {
                                            ...
                                            var simongMessage = _.find(messages, function(message) {
                                                return (message['_message'].to === simong.user.email);
                                            });
                                            // Assert that the correct content item is included in the email
                                            assert.equal(simongMessage['_message'].html.match(contentObj.profilePath).length, 1);

                                            // Assert that all 4 comments are in the email
                                            assert.equal(simongMessage['_message'].html.match(/activity-comment-container/g).length, 4);
                                            assert.ok(simongMessage['_message'].html.indexOf(commentA.body) > 0);
                                            assert.ok(simongMessage['_message'].html.indexOf(commentB.body) > 0);
                                            assert.ok(simongMessage['_message'].html.indexOf(commentC.body) > 0);
                                            assert.ok(simongMessage['_message'].html.indexOf(commentD.body) > 0);
                                            return callback();
                                        });
                                    });
                                });
                ...
                RestAPI.Content.createLink(simong.restContext, displayName, 'Google', 'public', 'http://www.google.ca', [], [nico.user.id], function(err, link) {
                    ...
                    ActivityTestsUtil.collectAndGetActivityStream(nico.restContext, null, null, function(err, activityStream) {
                        ...
                        EmailTestsUtil.collectAndFetchEmails(function(messages) {
                            ...
                            var text = messages[0]['_message'].body;
                            ...
                            assert.ok(text.indexOf(link.profilePath) !== -1);
