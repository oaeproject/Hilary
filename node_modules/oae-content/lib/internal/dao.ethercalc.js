 ...
 */
const hasUserEditedSpreadsheet = (module.exports.hasUserEditedSpreadsheet = function(
    contentId,
    userId,
    callback
) {
    ...
    client.exists(key, function(err, exists) {
        if (err) {
            log().error(
                { err: err, contentId: contentId, userId: userId },
                "Failed to check whether user has edited Ethercalc spreadsheet"
            );
            return callback({
                code: 500,
                msg:
        ...
        if (exists) {
            client.lrange(key, 0, -1, function(err, replies) {
                if (err) {
                    log().error(
                        { err: err, contentId: contentId, userId: userId },
                        "Failed to fetch editors for Ethercalc spreadsheet"
                    );
                    return callback({
                        code: 500,
 ...
 * @param  {Object}     callback.err        An error that occurred, if any
 */
const setEditedBy = (module.exports.setEditedBy = function(
    contentId,
    userId,
    callback
) {
    ...
    Redis.getClient().rpush(key, userId, function(err) {
        if (err) {
            log().error(
                { err: err, contentId: contentId, userId: userId },
                "Failed to store Ethercalc user edits"
            );
            return callback({
                code: 500,
