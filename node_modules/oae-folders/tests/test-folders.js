...
describe('Folders', function() {
    ...
    describe('Update Folder', function() {
                                ...
                                FoldersTestUtil.assertUpdateFolderFails(simong.restContext, folder.id, {'not': 'right'}, 400, function() {
                                    ...
                                    FoldersTestUtil.assertUpdateFolderFails(simong.restContext, folder.id, {'displayName': longDisplayName}, 400, function() {
                                        ...
                                        FoldersTestUtil.assertUpdateFolderFails(simong.restContext, folder.id, {'description': longDescription}, 400, function() {
                                            ...
                                            // Try to update the visibility without passing the `applyVisibilityOn` property
                                            FoldersTestUtil.assertUpdateFolderFails(simong.restContext, folder.id, {'visibility': 'private'}, 400, function() {

                                                // Try to update the visibility but passing an unknown `applyVisibilityOn` value
                                                FoldersTestUtil.assertUpdateFolderFails(simong.restContext, folder.id, {'visibility': 'private', 'applyVisibilityOn': 'all the things'}, 400, function() {

                                                    // Invalid visibility
                                                    FoldersTestUtil.assertUpdateFolderFails(simong.restContext, folder.id, {'visibility': 'noowpe', 'applyVisibilityOn': 'folder'}, 400, function() {
        ...
        it('verify update folder authorization', function(callback) {
            TestsUtil.generateTestUsers(camAdminRestContext, 3, function(err, users, simong, nico, bert) {
                ...
                FoldersTestUtil.assertCreateFolderSucceeds(simong.restContext, 'test displayName', 'test description', 'public', [], [nico], function(folder) {
                    ...
                    FoldersTestUtil.assertUpdateFolderFails(camAnonymousRestContext, folder.id, {'visibility': 'private', 'applyVisibilityOn': 'folder'}, 401, function() {

                        // Unrelated users cannot update folders
                        FoldersTestUtil.assertUpdateFolderFails(bert.restContext, folder.id, {'visibility': 'private', 'applyVisibilityOn': 'folder'}, 401, function() {

                            // Members cannot update folders
                            FoldersTestUtil.assertUpdateFolderFails(nico.restContext, folder.id, {'visibility': 'private', 'applyVisibilityOn': 'folder'}, 401, function() {

                                // Tenant admins from other tenants cannot update the folder
                                FoldersTestUtil.assertUpdateFolderFails(gtAdminRestContext, folder.id, {'visibility': 'private', 'applyVisibilityOn': 'folder'}, 401, function() {                                
            ...
            TestsUtil.generateTestUsers(camAdminRestContext, 1, function(err, users, simong) {
                ...
                FoldersTestUtil.assertCreateFolderSucceeds(simong.restContext, 'test displayName', 'test description', 'public', [], [], function(folder) {
                    ...
                    var updates = {
                        ...
                        'description': 'mega',
                        'visibility': 'private',
                        'applyVisibilityOn': 'folder'
                    };
                    RestAPI.Folders.updateFolder(simong.restContext, folder.id, updates, function(err, data) {
                        assert.ok(!err);
                        assert.ok(data.folder);
                        assert.strictEqual(data.folder.displayName, updates.displayName);
                        assert.strictEqual(data.folder.description, updates.description);
                        assert.strictEqual(data.folder.visibility, updates.visibility);

                        // Sanity-check the full folder profile was returned
                        assert.strictEqual(data.folder.canShare, true);
                        assert.strictEqual(data.folder.canManage, true);
                        assert.strictEqual(data.folder.canAddItem, true);
                        assert.ok(_.isObject(data.folder.createdBy));
                        assert.strictEqual(data.folder.createdBy.id, simong.user.id);

                        // Sanity-check no content failed to update (as there is none)
                        assert.ok(_.isArray(data.failedContent));
                        assert.strictEqual(data.failedContent.length, 0);
         ...
         */
        it('verify updating a folder\'s visibility can update the content inside the folder', function(callback) {
    ...
    describe('Delete Folder', function() {
        ...
        it('verify delete folder removes the folder authz group from the content items', function(callback) {
            TestsUtil.generateTestUsers(camAdminRestContext, 2, function(err, users, simong) {
                assert.ok(!err);

                // Generate a test folder
                FoldersTestUtil.generateTestFolders(simong.restContext, 1, function(folder) {

                    // Create a content item and stick it in the folder
                    RestAPI.Content.createLink(simong.restContext, 'test', 'test', 'private', 'http://www.google.ca', null, null, [], function(err, link) {
                        assert.ok(!err);
                        FoldersTestUtil.assertAddContentItemsToFolderSucceeds(simong.restContext, folder.id, [link.id], function() {

                            // Sanity-check the folder is an authz member of the content item
                            AuthzAPI.getAuthzMembers(link.id, null, 10, function(err, members) {
                                assert.ok(!err);
                                assert.strictEqual(members.length, 2);
                                assert.ok(_.find(members, function(member) { return (member.id === folder.groupId); }));

                                // Delete the folder
                                FoldersTestUtil.assertDeleteFolderSucceeds(simong.restContext, folder.id, function() {

                                    // The folder should no longer be an authz member of the piece of content
                                    AuthzAPI.getAuthzMembers(link.id, null, 10, function(err, members) {
                                        assert.ok(!err);
                            ...
                            RestAPI.Folders.getManagedFolders(simong.restContext, function(err, folders) {
                                ...
                                FoldersTestUtil.assertUpdateFolderMembersSucceeds(nico.restContext, nicosFolder.id, _memberUpdate(simong, 'manager'), function() {
                                    ...
                                    RestAPI.Folders.getManagedFolders(simong.restContext, function(err, folders) {
                                        ...
                                        FoldersTestUtil.assertDeleteFolderSucceeds(simong.restContext, simonsFolder.id, function() {
