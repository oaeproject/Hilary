...
var checkGroupMembershipsForUser = function(principalId, groupIds, callback) {
    ...
    Cassandra.runQuery('SELECT FIRST 2 * FROM AuthzMembershipsCache USING CONSISTENCY QUORUM WHERE principalId = ?', [principalId], function(err, rows) {
        if (err) {
            return callback(err);
        }
        ...
        // There are no columns, we need to explode
        } else {
