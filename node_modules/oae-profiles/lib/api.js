...
var setSection = module.exports.setSection = function(context, principal_id, sectionName, sectionVisibility, sectionData, overwrite, callback) {
    ...
    if (context.user().id === principal_id || context.user().isAdmin(principalResource.tenantId)) {
        ...
        getSection(context, principal_id, sectionName, function(err, oldProfileSection) {
            ...
            _setSection(principal_id, profileSection, callback);
        });
    } else {
...
var _setSection = function(principal_id, profileSection, callback) {
    ...
    Cassandra.runQuery("UPDATE Profiles USING CONSISTENCY QUORUM SET ?=?, ?=? WHERE principal_id=?", [columnVisibilityName, profileSection.visibility, columnDataName, data, principal_id], function(err) {
        if (err) {
...
var getSection = module.exports.getSection = function(context, principal_id, sectionName, callback) {
    ...
    Cassandra.runQuery("SELECT ?, ? FROM Profiles USING CONSISTENCY QUORUM WHERE principal_id = ?", [columnVisibilityName, columnDataName, principal_id], function(err, rows) {
        if (err) {
            return callback({'code': 500, 'msg': err});
        }
        if (rows[0].count <= 1) {
...
var getAllSections = module.exports.getAllSections = function(context, principal_id, callback) {
    ...
    Cassandra.runQuery("SELECT * FROM Profiles USING CONSISTENCY QUORUM WHERE principal_id = ?", [principal_id], function(err, rows) {
        if (err) {
            return callback({'code': 500, 'msg': err});
        }
...
var _hasAccess = function(context, profileSection, callback) {
    ...
    var principalResource = AuthzUtil.getResourceFromUuid(profileSection.principal_id);
    if (context.user().id === profileSection.principal_id || context.user().isAdmin(principalResource.tenantId) || profileSection.visibility !== Constants.visibility.PRIVATE) {
        callback(false, true);
    } else {
        return callback({'code': 403, 'msg': 'You don\'t have access to this profile section.'});
    }
