...
var LibraryIndexer = require('oae-index').LibraryIndexer;
 ...
 */
var createContent = module.exports.createContent = function(contentId, createdBy, resourceSubType, displayName, description, visibility, additionalMembers, otherValues, callback) {
    ...
    Cassandra.runQuery('DELETE FROM Content WHERE contentId = ?', [contentObj.id], function(err) {
        ...
        getAllContentMembers(contentObj.id, function(err, members) {
            ...
            AuthzAPI.updateRoles(contentObj.id, updateMembers, function(err) {
                ...
                LibraryIndexer.remove(membersToRemove, contentObj.id, contentObj.lastModified, function(err) {
                    if (err) {
...
var getLibraryItems = module.exports.getLibraryItems = function(principalId, visibility, start, limit, callback) {
    ...
    LibraryIndexer.list(principalId, visibility, start, limit, function(err, resourceIds, nextToken) {
        if (err) {
            return callback(err);
        }
...
var _updateLibraries = function(contentObj, oldLastModified, newLastModified, removedMembers, callback) {
    ...
    getAllContentMembers(contentObj.id, function(err, members) {
        if (err) {
            ...
            return LibraryIndexer.insert(members, contentObj.id, contentObj.visibility, newLastModified, callback);
        }
        ...
        _.each(members, function(member) {
            if (!_.contains(removedMembers, member)) {
        ...
        return LibraryIndexer.update(membersToUpdate, contentObj.id, contentObj.visibility, newLastModified, oldLastModified, function(err) {
            if (err) {
                errs.push(err);
            }
            ...
            if (removedMembers && removedMembers.length > 0) {
                ...
                return LibraryIndexer.remove(removedMembers, contentObj.id, oldLastModified, function(err) {
                    if (err) {
                        errs.push(err);
                    }
...
var _rowToContent = function(row) {
    ...
    var contentObj = new Content(hash.tenantAlias, hash.contentId, hash.visibility, hash.displayName, hash.description, hash.resourceSubType, hash.createdBy, hash.created, hash.lastModified);
    if (contentObj.resourceSubType === 'file') {
        ...
        contentObj.etherpadHtml = hash.etherpadHtml;
