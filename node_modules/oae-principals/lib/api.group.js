...
var setGroupMembers = module.exports.setGroupMembers = function(ctx, groupId, members, callback) {
    ...
    getGroup(ctx, groupId, function(err, group) {
        ...
        /**
         * Handle the callback of changing the group members.
         *
         * @param   {Object}    err     An error that occurred, if any
         */
        var _handleSetGroupMembers = function(err) {
            if (err) {
                return callback(err);
            }

            PrincipalsEmitter.emit(PrincipalsConstants.events.UPDATED_GROUP_MEMBERS, ctx, group.id, members);
            return callback();
        };

        var memberIds = _.keys(members);
        // We consider the operation an unjoin if there is only one operation, and it is the user removing themself from the group
        var isUnjoin = (memberIds.length === 1 && memberIds[0] === ctx.user().id && members[memberIds[0]] === false);
        // We consider the operation a join if there is only one operation, and it is the user adding themself to the group as 'member'
        var isJoin = (memberIds.length === 1 && memberIds[0] === ctx.user().id && members[memberIds[0]] === Constants.roles.MEMBER);
        if (isUnjoin) {
            // A user can always unjoin themselves
            _setGroupMembers(ctx, group, members, _handleSetGroupMembers);
        } else if (isJoin && group.joinable === Joinable.YES) {
            // A user can always join a joinable group
            _setGroupMembers(ctx, group, members, _handleSetGroupMembers);
        } else {
            // If it is not a valid self-serve action, we need to validate the user's permission to do this
...
var createGroup = module.exports.createGroup = function(ctx, alias, name, description, visibility, joinable, members, otherValues, callback) {
    ...
    canCreateGroup(ctx, groupId, function(err) {
        ...
        Cassandra.runQuery('INSERT INTO Principals (principalId, alias, tenant, name, description, visibility, joinable) VALUES (?, ?, ?, ?, ?, ?, ?) USING CONSISTENCY QUORUM',
            [groupId, alias, tenant.alias, name, description, visibility, joinable], function (err) {
            if (err) {
                return callback(err);
            }
            ...
            var group = new Group(tenant.alias, groupId, alias, name, description, visibility, joinable);
    ...
    getGroup(ctx, groupId, function(err, oldGroup) {
        ...
        _canManage(ctx, oldGroup, function(err, canManage) {
            ...
            Cassandra.runQuery(q.query, q.parameters, function(err) {
                ...
                // Merge the updated profile fields to make the updated group
...
var canViewMembers = module.exports.canViewMembers = function(ctx, group, callback) {
    ...
    } else if (group.visibility === Visibility.LOGGEDIN && TenantsUtil.isLoggedIn(ctx, group.tenant)) {
