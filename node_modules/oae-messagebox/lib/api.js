...
var getRecentContributions = module.exports.getRecentContributions = function(messageBoxId, start, limit, callback) {
    ...
    // By using a consistency of `ONE` we will only query 1 Cassandra node. We can handle a little inaccuracy in this method,
    // so the possibility that we don't have the latest information isn't that bad.
    var opts = {
        'reversed': true,
        'consistency': 'ONE'
    };
    Cassandra.runPagedColumnQuery('MessageBoxRecentContributions', 'messageBoxId', messageBoxId, start, limit, opts, function(err, columns) {
        if (err) {
            return callback(err);
        }

        var recentContributions = _.pluck(columns, 'name');
...
var _getThreadKeysFromMessageBox = function(messageBoxId, start, limit, callback) {
    ...
    Cassandra.runPagedColumnQuery('MessageBoxMessages', 'messageBoxId', messageBoxId, start, limit, {'reversed': true}, function(err, columns, nextToken) {
        if (err) {
            return callback(err);
        }

        var threadKeys = _.pluck(columns, 'name');
