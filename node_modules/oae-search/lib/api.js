...
var _ensureSearchSchema = function(callback, _names) {
    if (!_names) {
        return client.putMapping(SearchConstants.resourceMappingName, require('./schema/resourceSchema'), {'_source': false}, function(err) {
            if (err) {
                return callback(err);
            }
...
var _handleDeleteDocumentTask = function(data, callback) {
    ...
    // Setup the parent resource document and all potential children documents to be deleted
    var deletes = _.map(childSearchDocuments, function(options, documentType) {
        return {'documentType': documentType, 'documentId': data.id};
    });
    deletes.push({'documentType': 'resource', 'documentId': data.id});

    // Delete the resource document, plus all its children
    _deleteAll(deletes, callback);
};

var _deleteAll = function(deletes, callback, _errs) {
    if (_.isEmpty(deletes)) {
        return callback(_errs);
    }

    var del = deletes.shift();
    client.del(del.documentType, del.documentId, function(err) {
        if (err) {
            log().error({'err': err, 'document': del}, 'Error deleting a document from the search index');
        }

        return _deleteAll(deletes, callback, _errs);
    });
};
    ...
    if (searchDocumentProducer) {
        searchDocumentProducer(resourcesToIndex[resourceType], function(err, documents) {
            ...
            documents = _.map(documents, function(doc) {
                var newDoc = _.extend({}, doc, {'_type': SearchConstants.resourceMappingName, 'resourceType': resourceType});
                if (newDoc['_extra']) {
