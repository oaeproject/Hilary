...
var getDirectRoles = module.exports.getDirectRoles = function(principalIds, resourceId, callback) {
    ...
    _.each(principalIds, function(pId) {
        validator.check(pId, {'code': 400, 'msg': 'Invalid principal id provided.'}).isPrincipalId();
    });
...
var getPrincipalMemberships = module.exports.getPrincipalMemberships = function(principalId, start, limit, callback) {
    ...
    Cassandra.runQuery('SELECT FIRST ' + paging.limit + ' ?..\'\' FROM AuthzMembershipsCache USING CONSISTENCY QUORUM WHERE principalId = ?', [paging.start, principalId], function (err, rows) {
        ...
        var row = rows[0];
        // Check if the cached row has been found
        if (row.count >= 1) {
            var memberships = [];
            for (var i = paging.startIndex, j = row.count; i < j; i++) {
            ...
            return callback(null, memberships);
        // If the user's memberships haven't been exploded yet, we explode them and perform the
        // paging on the entire exploded set
        } else {
