...
var init = module.exports.init = function(config) {
    _pdftkPath = config.previews.binaries.pdftk;
};
...
var _splitPDF = function(ctx, path, pagesDir, callback) {
    var cmd = _pdftkPath + ' "' + path + '" burst output ' + pagesDir + '/page.%d.pdf';
     // Execute the command.
    log().trace('%s - executing %s', ctx.contentId, cmd);
    var child = exec(cmd, { 'timeout': 120000 }, function (err, stdout, stderr) {
        if (err) {
