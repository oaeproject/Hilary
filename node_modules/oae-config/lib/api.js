...
var _getPersistentTenantConfig = function(alias, callback) {
    Cassandra.runQuery('SELECT "tenantAlias", "configKey", "value", writetime("value") as wt FROM "Config" WHERE "tenantAlias" = ?', [alias], function(err, rows) {
        if (err) {
            return callback(err);
        }
...
var _rowsToConfig = function(rows) {
    ...
    _.each(rows, function(row) {
        ...
        var value = row.value;
        var timestamp = Cassandra.bigintTimestampToDate(row.wt);
        ...
        if (_.isString(value)) {
            ...
            if (element) {
                try {
                    value = _deserializeConfigValue(element, row.value);

                    persistentConfig[row.tenantAlias] = persistentConfig[row.tenantAlias] || {};
                    persistentConfig[row.tenantAlias][key] = {
                        'value': value,
                        'timestamp': timestamp
                    };
                } catch (err) {
