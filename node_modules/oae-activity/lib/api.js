 ...
 */
var getActivityStream = module.exports.getActivityStream = function(ctx, principalId, start, limit, transformerType, callback) {
    transformerType = transformerType || ActivityConstants.transformerTypes.ACTIVITYSTREAMS;

    var validator = new Validator();
    validator.check(principalId, {'code': 400, 'msg': 'You can only view activity streams for a principal'}).isPrincipalId();
    ...
    PrincipalsUtil.getPrincipal(ctx, principalId, function(err, principal) {
        ...
        LibraryAuthz.resolveTargetLibraryAccess(ctx, principalId, principal, function(err, hasAccess, visibility) {
            if (err) {
                return callback(err);
            } else if (!hasAccess) {
 ...
 */
var getNotificationStream = module.exports.getNotificationStream = function(ctx, userId, start, limit, transformerType, callback) {
    transformerType = transformerType || ActivityConstants.transformerTypes.ACTIVITYSTREAMS;
...
var _getActivityStream = function(ctx, activityStreamId, start, limit, transformerType, callback) {
    ActivityDAO.getActivities(activityStreamId, start, limit, function(err, activities, nextToken) {
        ...
        ActivityTransformer.transformActivities(ctx, activities, transformerType, function(err) {
            ...
            ActivityEmitter.emit(ActivityConstants.events.GET_ACTIVITY_STREAM, ctx, activityStreamId, start, limit, transformerType, activities);
 ...
 */
var removeActivityStream = module.exports.removeActivityStream = function(ctx, principalId, callback) {       
    var callback = callback || function() {};

    if (ctx.user().isGlobalAdmin() || ctx.user().isAdmin() || ctx.user().isTenantAdmin()) {

        var activityTypes = ['#activity', '#email', '#activity#public', '#activity#private', '#activity#loggedin', '#notification'];

        async.eachSeries(activityTypes, function(activityType, innerCallback) {
            ...
            ActivityDAO.getActivities(principalId + activityType, null, null, function(err, activities) {
                ...
                ActivityDAO.deleteActivities(activities, function(err, data) {
                    if (err) {
                        return callback(err);
                    } 
