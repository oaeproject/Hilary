...
var getFullContentProfile = module.exports.getFullContentProfile = function(ctx, contentId, callback) {
    ...
    getContent(ctx, contentId, function(err, contentObj) {
        ...
        _isExplicitManager(ctx, contentObj, function(err, isManager) {
            if (err) {
                return callback(err);
            }
...
/**
 * Adds the `isManager` and `isMember` flag, `createdBy` user object, `canShare` flag and the `latestRevision` in case it's a collaborative document.
 *
 * @param  {Context}    ctx                         Standard context object, representing the currently logged in user and its tenant
...
var _getFullContentProfile = function(ctx, contentObj, isManager, callback) {
    ...
    PrincipalsUtil.getPrincipal(ctx, contentObj.createdBy, function(err, createdBy) {
        ...
        // Get the access the user has to the content
        _getContentAccess(ctx, contentObj, isManager, function(err, contentObj) {
            if (err) {
                return callback(err);
            }
...
/**
 * Add extra properties on the content object, depending on the access the user has on the content item
 *
 * @param  {Context}    ctx                                  Standard context object, representing the currently logged in user and its tenant
 * @param  {Content}    contentObj                           Retrieved content profile
 * @param  {Boolean}    isManager                            Whether or not the current user is a manager of the piece of content.
 * @param  {Function}   callback                             Standard callback function
 * @param  {Object}     callback.err                         Error object containing the error message
 * @param  {Content}    callback.contentProfile              Retrieved content profile
 * @param  {Boolean}    callback.contentProfile.isManager    Whether or not the current user is an explicit manager of the piece of content
 * @param  {Boolean}    callback.contentProfile.isMember     Whether or not the current user is an explicit member of the piece of content
 * @param  {Boolean}    callback.contentProfile.canShare     Whether or not the current user can share the content
 */
var _getContentAccess = function(ctx, contentObj, isManager, callback) {
    // Store the isManager property.
    contentObj.isManager = isManager;

    // Check if the user can share this content item
    _canShareContent(ctx, contentObj, null, function(err, canShare) {
        if (err) {
            return callback(err);
        }
        ...
        // If the user is a manager they're automatically also a member
        if (isManager) {
            contentObj.isMember = true;
            return callback(null, contentObj);
        // If not a manager do an extra check to see if the user is a member
        } else {
            _isExplicitMember(ctx, contentObj, function(err, isMember) {
                if (err) {
                    return callback(err);
                }

                contentObj.isMember = isMember;
                return callback(null, contentObj);
            });
        }
    });
...
var joinCollabDoc = module.exports.joinCollabDoc = function(ctx, contentId, callback) {
    ...
    canManage(ctx, contentId, function(err, canManage, contentObj) {
        ...
        // Join the pad
        return Etherpad.joinPad(ctx, contentObj.id, contentObj.etherpadPadId, contentObj.etherpadGroupId, callback);
    });
};
...
var canManage = module.exports.canManage = function(ctx, contentId, callback) {
    ...
    getContent(ctx, contentId, function(err, contentObj) {
        ...
        _isManager(ctx, contentObj, function(err, canManage) {
            if (err) {
                return callback(err);
            }
 ...
 * @api private
 */
var _isManager = function(ctx, contentObj, callback) {
    if (!ctx.user()) {
        return callback(null, false);
...
/**
 * Internal function that checks whether or not the current user is an explicit manager of a piece of content. This function assumes that
 * the piece of content passed in actually exists
 *
 * @param  {Context}        ctx                 Standard context object, representing the currently logged in user and its tenant
 * @param  {Content}        contentObj          The content object we want to check
 * @param  {Function}       callback            Standard callback function takes arguments `err` and `canManage`
 * @param  {Object}         callback.err        Error object containing the error message
 * @param  {Boolean}        callback.canManage  Whether or not the user is an explicit manager of the content
 * @api private
 */
var _isExplicitManager = function(ctx, contentObj, callback) {
    if (!ctx.user()) {
        return callback(null, false);
    } else {
        return AuthzAPI.hasRole(ctx.user().id, contentObj.id, ContentConstants.roles.MANAGER, callback);
...
/**
 * Internal function that checks whether or not the current user is a member of a piece of content. This function assumes that
 * the piece of content passed in actually exists
 *
 * @param  {Context}        ctx                 Standard context object, representing the currently logged in user and its tenant
 * @param  {Content}        contentObj          The content object we want to check
 * @param  {Function}       callback            Standard callback function takes arguments `err` and `isMember`
 * @param  {Object}         callback.err        Error object containing the error message
 * @param  {Boolean}        callback.isMember   Whether or not the user is a member of the content
 * @api private
 */
var _isMember = function(ctx, contentObj, callback) {
    if (!ctx.user()) {
        return callback(null, false);
    } else if (ctx.user().isAdmin(contentObj.tenant.alias)) {
        return callback(null, true);
    ...
    return AuthzAPI.hasRole(ctx.user().id, contentObj.id, ContentConstants.roles.VIEWER, callback);
};
 ...
 * @param  {Function}       callback            Standard callback function takes arguments `err` and `isMember`
 * @param  {Object}         callback.err        Error object containing the error message
 * @param  {Boolean}        callback.isMember   Whether or not the user is an explicit member of the content
 * @api private
 */
var _isExplicitMember = function(ctx, contentObj, callback) {
    if (!ctx.user()) {
        return callback(null, false);
    } else {
        return AuthzAPI.hasRole(ctx.user().id, contentObj.id, ContentConstants.roles.VIEWER, callback);
...
var deleteComment = module.exports.deleteComment = function(ctx, contentId, commentCreatedDate, callback) {
    ...
    getContent(ctx, contentId, function(err, content) {
        ...
        MessageBoxAPI.getMessages(contentId, [commentCreatedDate], null, function(err, messages) {
            ...
            _isManager(ctx, content, function(err, canManage) {
                if (err) {
                    return callback(err);
                } else if (!canManage) {
 ...
 * @param  {String}         [revisionId]        The id of the revision that needs to be retrieved. If unspecified, the latest revision will be retrieved
 * @param  {Function}       callback            Standard callback function
 * @param  {Object}         callback.err        Error object containing the error message
