...
describe('Content', function() {
     ...
     */ 
     var setUpGroups = function(contexts, callback) {
        ...
        // Create UI Dev Group
        PrincipalsAPI.createGroup(contexts[0], groups[0].name, groups[0].description, 'public', function(err, principal_id) {
            groups[0].id = principal_id;
            // Make Bert a member
            PrincipalsAPI.addGroupMember(contexts[0], groups[0].id, contexts[2].user().id, 'member', function(err) {
                // Create Back-end Dev Group
                PrincipalsAPI.createGroup(contexts[3], groups[1].name, groups[1].description, 'public', function(err, principal_id) {
                    groups[1].id = principal_id;
                    // Make Simon a member
                    PrincipalsAPI.addGroupMember(contexts[3], groups[1].id, contexts[1].user().id, 'member', function(err) {
                        // Create OAE Team Group
                        PrincipalsAPI.createGroup(contexts[4], groups[2].name, groups[2].description, 'public', function(err, principal_id) {
                             groups[2].id = principal_id;
                             // Make Stuart, UI Dev Group and Back-end Dev Group all members
                             PrincipalsAPI.addGroupMembers(contexts[4], groups[2].id, [groups[1].id, groups[0].id, contexts[5].user().id], 'member', function(err) {
                                callback(groups);
                             });
                        });
                    });
