...
OAE.tenantServer.post('/api/user/create', function(req, res) {
    ...
    if (user) {
        if (user.isAdmin(tenant.alias)) {
            // If the current user is an admin don't bother with reCaptcha
            return createUser();
        } else {
    ...
    // Check if the T&C has been agreed to (if applicable)
    var needsTermsAndConditionAgreement = PrincipalsConfig.getValue(tenant.alias, 'termsAndConditions', 'enabled');
    var acceptedTC = (req.body.acceptedTC === 'true');
    if (needsTermsAndConditionAgreement && acceptedTC !== true) {
