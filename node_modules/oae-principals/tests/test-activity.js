  ...
  describe('Activity Entity Model', function() {
                              ...
                              function(err) {
                                ...
                                ActivityTestsUtil.collectAndGetActivityStream(
                                  ...
                                  function(err, activityStream) {
                                    ...
                                    assert.equal(
                                      object['oae:collection'].length,
                                      2,
                                    );

                                    var publicUserActivityEntity = null;
                                    var privateUserActivityEnity = null;
                                    _.each(object['oae:collection'], function(
                                      user,
                                    ) {
                                      var resourceId = AuthzUtil.getResourceFromId(
                                        user['oae:id'],
                                      ).resourceId;
                                      if (
                                        user['oae:id'] === publicUser.user.id
                                      ) {
                                        ...
                                        user['oae:id'] === privateUser.user.id
                                      ) {
                                      ...
                                      function(err, body, response) {
                                        ...
                                        RestUtil.RestRequest(
                                          ...
                                          function(err, body, response) {
                                            ...
                                            signedDownloadUrl = url.parse(
                                              publicUserActivityEntity.image
                                                .url,
                                              true,
                                            );
                                            RestUtil.RestRequest(
    ...
    it('verify that a private unjoinable group is not delivered to a non-member user feed for a group-create activity', function(callback) {
      TestsUtil.generateTestUsers(camAdminRestContext, 3, function(
        ...
        RestAPI.Following.follow(branden.restContext, nico.user.id, function(
          err,
        ) {
          assert.ok(!err);

          RestAPI.Following.follow(nico.restContext, branden.user.id, function(
            err,
          ) {
            assert.ok(!err);
              ...
              function(err, privateGroup) {
                ...
                ActivityTestsUtil.collectAndGetActivityStream(
                  ...
                  function(err, response) {
                    ...
                    assert.equal(
                      response.items[1]['oae:activityType'],
                      'following-follow',
                    );
                      ...
                      function(err, publicGroup) {
                        ...
                        ActivityTestsUtil.collectAndGetActivityStream(
                          ...
                          function(err, response) {
                            ...
                            assert.equal(
                              response.items[0].object['oae:id'],
                              publicGroup.id,
                            );
      ...
      TestsUtil.generateTestUsers(camAdminRestContext, 2, function(
        ...
        RestAPI.Group.createGroup(
          ...
          function(err, group) {
            ...
            // Join the group
            RestAPI.Group.joinGroup(simong.restContext, group.id, function(
              err,
            ) {
              assert.ok(!err);
        ...
        RestAPI.Group.createGroup(
          ...
          function(err, group) {
            ...
            RestAPI.Group.joinGroup(jane.restContext, group.id, function(err) {
              ...
              // Join as branden
              RestAPI.Group.joinGroup(branden.restContext, group.id, function(
                err,
              ) {
                assert.ok(!err);
            ...
            RestAPI.Group.createGroup(
              ...
              function(err, group) {
                ...
                EmailTestsUtil.collectAndFetchAllEmails(function(messages) {
                  ...
                  assert.equal(
                    stringMessage.indexOf(simong.user.displayName),
                    -1,
                  );
                      ...
                      RestAPI.Group.joinGroup(
                        ...
                        function(err) {
                          ...
                          EmailTestsUtil.collectAndFetchAllEmails(function(
                            ...
                            // Ensure some data expected to be in the email is there
                            assert.notEqual(
                              stringMessage.indexOf(
                                simong.restContext.hostHeader,
                              ),
                              -1,
                            );
                            assert.notEqual(
                            ...
                            assert.equal(
                              stringMessage.indexOf(simong.user.email),
                              -1,
                            );
                            assert.equal(
                              stringMessage.indexOf(simong.user.locale),
                              -1,
                            );
                              ...
                              function(err) {
                                ...
                                EmailTestsUtil.collectAndFetchAllEmails(
                                  function(messages) {
                                    ...
                                    var stringMessage = JSON.stringify(
                                      messages[0],
                                    );
                                    var message = messages[0];
                                    ...
                                    assert.notEqual(
                                      stringMessage.indexOf(
                                        bert.user.displayName,
                                      ),
                                      -1,
                                    );
                                    assert.equal(
                                      stringMessage.indexOf(
                                        'swappedFromPublicAlias',
                                      ),
                                      -1,
                                    );
                                    ...
                                    assert.equal(
                                      stringMessage.indexOf(bert.user.email),
                                      -1,
                                    );
                                    assert.equal(
                                      stringMessage.indexOf(bert.user.locale),
                                      -1,
                                    );
