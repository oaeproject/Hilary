...
var initiateTransfer = module.exports.initiateTransfer = function (ctx, code, originalUserId, originalEmail, targetEmail, callback) { 
    ...
    _getTTLTransfer(ctx, function (err, TTL) {
        ...
        // Create transfer
        Cassandra.runQuery('INSERT INTO "Transfer" ("originalUserId", "code", "originalEmail", "targetEmail", "targetUserId", "status", "dateLastAction") VALUES (?, ?, ?, ?, ?, ?, ?) USING TTL ?', [originalUserId, code, originalEmail, targetEmail, '', TransferConstants.status.ONGOING, date.toString(), finalTTL], function(err) {
            if (err) {
                return callback(err);
            }
            Cassandra.runQuery('INSERT INTO "TransferByEmail" ("originalUserId", "code", "originalEmail", "targetEmail", "targetUserId", "status", "dateLastAction") VALUES (?, ?, ?, ?, ?, ?, ?) USING TTL ?', [originalUserId, code, originalEmail, targetEmail, '', TransferConstants.status.ONGOING, date.toString(), finalTTL], function(err) {
                if (err) {
                    return callback(err);
                }
