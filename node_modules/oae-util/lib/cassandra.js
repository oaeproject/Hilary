...
/**
 * Iterate through all the rows of a column family. This will return just the columnNames that are specified in the `columnNames`
 * parameter, and only `batchSize` rows at a time. On each iteration of `batchSize` rows, the `onEach` callback will be executed.
 * When completed (e.g., there are 0 rows left to iterate through), the `onEach` callback will be executed with 3rd parameter
 * `true`. Unless the column family schema is specified to have an ordered partitioner, the rows are fetched in a completely
 * random order.
 *
 ...
 * @param  {Number}     [batchSize]     The number of rows to fetch at a time. Defaults to 100.
 * @param  {String}     [consistency]   The Cassandra consistency to use when fetching rows. Defaults to ONE.
 ...
 * @param  {Rows}       onEach.rows     A helenus `Rows` object, holding all the rows fetched from storage in this iteration. If there is an error or if iteration is complete, this will not be specified. Check both `err` or `complete` to determine if this should be set to a value before trying to use it.
 * @param  {Boolean}    onEach.complete If `true`, indicates that there are no more items in the column family to iterate over. Otherwise, there will be more rows to iterate over.
 */
var iterateAll = module.exports.iterateAll = function(columnNames, columnFamily, keyColumnName, batchSize, consistency, onEach) {
    batchSize = (!isNaN(parseInt(batchSize, 10))) ? batchSize : DEFAULT_ITERATEALL_BATCH_SIZE;
    // Since this operation should *never* be executed in synchronicity with user requests, I'm pretty sure we are fine with using
    // a consistency of ONE by default.
    consistency = consistency || 'ONE';

    return _iterateAll(columnNames, columnFamily, keyColumnName, batchSize, consistency, onEach);
};

/**
 * Internal version of #iterateAll. The parameters and contract are the same, except this method does not apply any defaults and
 * there is one more parameter at the end `fromKey`, where the starting point can be specified. This is used for recursive
 * iteration.
 * @api private
 */
var _iterateAll = function(columnNames, columnFamily, keyColumnName, batchSize, consistency, onEach, fromKey) {
    ...
    runQuery(query.query, query.parameters, function(err, rows) {
        ...
        // Find the last key to use for the next iteration
        rows.forEach(function(row) {
            fromKey = row.key;
        });

        try {
            // Give the rows to the caller. We purposely invoke `onEach` before continuing iteration so that if onEach throws an exception
            // in the same process tick, iteration will finish.
            onEach(null, rows);
        } catch (ex) {
            return onEach(ex);
        }
        ...
        _iterateAll(columnNames, columnFamily, keyColumnName, batchSize, consistency, onEach, fromKey);
    });
};
