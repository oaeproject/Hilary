/*!
 * Copyright 2012 Sakai Foundation (SF) Licensed under the
 * Educational Community License, Version 2.0 (the "License"); you may
 * not use this file except in compliance with the License. You may
 * obtain a copy of the License at
 *
 *     http://www.osedu.org/licenses/ECL-2.0
 *
 * Unless required by applicable law or agreed to in writing,
 * software distributed under the License is distributed on an "AS IS"
 * BASIS, WITHOUT WARRANTIES OR CONDITIONS OF ANY KIND, either express
 * or implied. See the License for the specific language governing
 * permissions and limitations under the License.
 */


/*!
 * This backend is essentially the same as the Local Storage Backend.
 * The only difference is that it will respond with a 200 and the path to the file
 * when asked for a download link.
 * This is so files can be downloaded directly from the app server.
 *
 * This should only be used during unit tests and *NEVER* in production.
 */

var _ = require('underscore');

var LocalStorage = require('./local');

/**
 * @borrows Interface.store as TestStorageBackend.store
 */
var store = module.exports.store = function(ctx, file, options, callback) {
    LocalStorage.store(ctx, file, options, function(err, uri) {
        if (err) {
            return callback(err);
        }

        uri = uri.replace('local:', 'test:');
        return callback(null, uri);
    });
};

/**
 * @borrows Interface.get as TestStorageBackend.get
 */
var get = module.exports.get = function(ctx, uri, callback) {
    LocalStorage.get(ctx, uri, callback);
};

/**
 * @borrows Interface.remove as TestStorageBackend.remove
 */
var remove = module.exports.remove = function(ctx, uri, callback) {
    LocalStorage.remove(ctx, uri, callback);
};
/**
 * @borrows Interface.getDownloadLink as TestStorageBackend.getDownloadLink
 */
var getDownloadLink = module.exports.getDownloadLink = function(ctx, uri) {
    var file = LocalStorage.getRootDirectory() + '/' + uri.split(':')[1];
    return {'code': 200, 'url': file};
};
