 ...
 *
 * @param  {String[]}   modules         An array of modules that should be bootstrapped. These need to be located in the ./node_modules directory
 * @param  {Function}   callback        Standard callback function invoked when all modules have bootstrapped
 * @param  {Object}     callback.err    An error that occurred, if any
 * @api private
 */
var bootstrapModulesRest = function(modules, callback) {
    ...
    _.each(modules, function(module) {
        var path = util.format('node_modules/%s/lib/rest.js', module);
        if (!fs.existsSync(path)) {
            return complete();
        }
        log().info('REST services for %s have been registered', module);
        require(module + '/lib/rest');
        // Note that only modules with a rest.js will be documented, but swagger will parse all .js files in those modules
        Swagger.documentModule(module, complete);
    });
};
