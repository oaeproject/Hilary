 ...
 */
OAE.globalAdminServer.post('/api/user/import', function(req, res) {
    req.telemetryUrl = '/api/user/import';
    PrincipalsAPI.importUsers(req.ctx, req.body.tenantAlias, req.files.file, req.body.authenticationStrategy, function(err) {
        if (err) {
            return res.send(err.code, err.msg);
        }
...
OAE.tenantServer.post('/api/user/create', function(req, res) {
    ...
    if (user) {
        if (user.isAdmin(tenant.alias)) {
            ...
            // Non-admin logged in users cannot create accounts
            return res.send(401);
        }
    }
 ...
 * Import users using a CSV file
 */
OAE.tenantServer.post('/api/user/import', function(req, res) {
    req.telemetryUrl = '/api/user/import';
    PrincipalsAPI.importUsers(req.ctx, null, req.files.file, req.body.authenticationStrategy, function(err) {
        if (err) {
            return res.send(err.code, err.msg);
        }
 ...
 */
OAE.tenantServer.get('/api/user/termsAndConditions', function(req, res) {
    var locale = null;

    // Determine the locale for which we should retrieve the Terms and Conditions
    ...
    // 4. Otherwise we'll need to fall back to the `default` key. We will then retrieve the Terms and Conditions in the locale that the tenant admin used as the 'default'
    } else {
        locale = 'default';
    }
...
OAE.tenantServer.get('/api/user/:id/memberships', function(req, res) {
    ...
    PrincipalsAPI.memberOf(req.ctx, req.params.id, req.query.start, limit, function(err, memberships, nextToken) {
        if (err) {
            return res.send(err.code, err.msg);
        }
 ...
 */
OAE.tenantServer.post('/api/user/:id/termsAndConditions', function(req, res) {
    req.telemetryUrl = '/api/user/id/termsAndConditions';
