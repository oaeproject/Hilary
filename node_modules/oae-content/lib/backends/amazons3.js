...
var DownloadStrategy = require('../model').DownloadStrategy;
 ...
 */
var store = module.exports.store = function(tenantAlias, file, options, callback) {
    ...
    _getClient(tenantAlias).PutObject(options, function(err, data) {
...
var get = module.exports.get = function(tenantAlias, uri, callback) {
    ...
    _getClient(tenantAlias).GetObject(options, function(err, data) {
        if (err) {
            IO.destroyStream(writeStream);
...
var remove = module.exports.remove = function(tenantAlias, uri, callback) {
    ...
    _getClient(tenantAlias).DeleteObject(options, function(err, data) {
        if (err) {
 ...
 */
var getDownloadStrategy = module.exports.getDownloadStrategy = function(tenantAlias, uri) {
    ...
    var s3 = _getClient(tenantAlias);
    var bucketName = _getBucketName(tenantAlias);
    var amazonUri = BackendUtil.splitUri(uri).location;
    var stringToSign = util.format('GET\n\n\n%d\n/%s/%s', expires, bucketName, amazonUri);
    var signature = encodeURIComponent(s3.signature(stringToSign));

    // Construct the signed URL
    var keyId = s3.accessKeyId();
    var url = util.format('https://%s.s3.amazonaws.com/%s?AWSAccessKeyId=%s&Signature=%s&Expires=%s', bucketName, amazonUri, keyId, signature, expires);
 ...
 */
var _getBucketName = function(tenantAlias) {
 ...
 */
var _getClient = function(tenantAlias) {

    var accessKey = Config.getValue(tenantAlias, 'storage', 'amazons3-access-key');
    var secretKey = Config.getValue(tenantAlias, 'storage', 'amazons3-secret-key');
    var region = Config.getValue(tenantAlias, 'storage', 'amazons3-region');
