 ...
 */
var getGroupMembers = module.exports.getGroupMembers = function(context, group_id, retrieve_basicprofile, start, limit, callback) {
    // Defaults.
    limit = limit || 100;
    ...
    getGroup(context, group_id, function(err, group) {
        ...
        Cassandra.runQuery("SELECT first " + limit + " ?..\'\' FROM GroupMembers WHERE group_id = ?", [start, group_id], function (err, rows) {
            ...
            var members = [];
            for (var i = startColumnIndex, j = rows[0].count;i<j;i++) {
                var name = rows[0][i].name;
                if (name !== "group_id") {
                    members.push(name);
                }
            }
            ...
            PrincipalsUtil.getBasicProfile(members, callback);
        });
    });
};
 ...
 */
var memberOf = module.exports.memberOf = function(context, retrieve_basicprofile, start, limit, callback) {
    // Anonymous isn't member of any groups.
    if (context.user() === null) {
        return callback(false, []);
    }

    getGroupMembershipAncestry(context.user().id, function(err, groups) {
        if (!err) {
            var groupsArray = _.keys(groups);
            if (retrieve_basicprofile && groupsArray.length > 0) {
                PrincipalsUtil.getBasicProfile(groupsArray, callback);
            } else {
...
var addGroupMembers = module.exports.addGroupMembers = function(ctx, groupUuid, principalsToAdd, role, callback) {
    ...
    canManage(ctx, groupUuid, function(err) {
        if (!err) {
            ...
            Cassandra.runQuery('SELECT tenant FROM Principals WHERE principal_id IN (?)', [principalsToAdd], function (err, rows) {
                if (err) {
                    return callback({'code': 500, 'msg': err});
                }
...
var createGroup = module.exports.createGroup = function(ctx, title, description, visibility, callback) {
    ...
    var id = RolesUtil.toUuid(Constants.PRINCIPAL_TYPE_GROUP, tenant.alias, groupId);

    canCreateGroup(ctx, id, function(err) {
        if (!err) {
            // Create the group.
            Cassandra.runQuery('INSERT INTO Principals (principal_id, tenant, group_title, group_description, visibility) VALUES (?, ?, ?, ?, ?)',
 ...
 */
var getGroupUsers = module.exports.getGroupUsers = function(context, group_id, callback) {
    getGroup(context, group_id, function(err, group) {
        if (err) {
            return callback(err);
        }
        PrincipalsUtil.explodePrincipals([group_id], true, function(err, users) {
...
var setGroupVisibility = module.exports.setGroupVisibility = function(context, group_id, visibility, callback) {
    ...
    canManage(context, group_id, function(err) {
        ...
        Cassandra.runQuery('UPDATE Principals SET visibility = ? WHERE principal_id = ?', [visibility, group_id], function(err) {
            if (err) {
                return callback(err);
            }
            callback(err);
...
var _canManage = function(context, groupUuid, callback) {
    PermissionsAPI.isAllowed(userUuid(context), Constants.roles.MANAGER, groupUuid, function(err, isAllowed) {
        if (!err) {
            if (isAllowed) {
               return callback();
            } else {
