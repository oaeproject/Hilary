 ...
 * @param  {Number}     nrOfBuckets                 The number of buckets that should be collected
 ...
 * @param  {Boolean}    collector.empty             Whether or not the bucket is fully drained
 * @param  {Function}   [callback]                  Invoked when collection is complete
 * @param  {Object}     [callback.err]              An error that occurred, if any
 */
var collectAllBuckets = module.exports.collectAllBuckets = function(type, nrOfBuckets, maxConcurrentCollections, collectionExpiry, collector, callback) {
    ...
    var bucketNumbers = [];
    for (var i = 0; i < bucketsInfo[type].nrOfBuckets; i++) {
...
var _collectBuckets = function(type, bucketNumbers, callback, _errs) {
    ...
    _collectBucket(type, bucketNumber, function(err) {
        ...
        _collectBuckets(type, bucketNumbers, callback, _errs);
    });
};
...
var _collectBucket = function(type, bucketNumber, callback) {
    ...
    Locking.acquire(lockKey, bucketsInfo[type].collectionExpiry, function(err, lockId) {
        if (err) {
            return callback(err);
        } else if (lockId) {
            ...
            bucketsInfo[type].collector(bucketNumber, function(collectionErr, empty) {
                ...
                Locking.release(lockKey, lockId, function(releaseErr, hadLock) {
                    ...
                    if (!hadLock) {
                        ...
                        log().error({
                            'type': type,
                            'collectionExpiry': bucketsInfo[type].collectionExpiry
