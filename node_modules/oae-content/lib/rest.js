 ...
 * @FormParam   {string[]}          [folders]           Unique identifier(s) for folders where the collaborative document should be added to
 ...
 * @FormParam   {string[]}          [folders]           Unique identifier(s) for folders where the file should be added to
 ...
 * @FormParam   {string[]}          [folders]           Unique identifier(s) for folders where the link should be added to
...
OAE.tenantRouter.on('post', '/api/content/create', function(req, res) {
    ...
    /*!
     * Called once the content item has been created. This function will add the
     * created piece of content to a set of specified folders if any were specified
     */
    var _contentCreated = function(err, contentObj) {
        if (err) {
            return res.send(err.code, err.msg);
        }
        ...
        // Add the content item to each of the specified folders
        var folders = OaeUtil.toArray(req.body.folders);
        _addContentItemToFolders(req.ctx, contentObj, folders, function(err) {
            if (err) {
                return res.send(err.code, err.msg);
            }
    ...
    /*!
     * Add the created content item to a set of folders
     */
    var _addContentItemToFolders = function(ctx, contentObj, folders, callback) {
        if (folders.length === 0) {
            return callback();
        }

        var folderId = folders.pop();
        FoldersAPI.addContentItemsToFolder(ctx, folderId, [contentObj.id], function(err) {
            if (err) {
                return callback(err);
            }

            _addContentItemToFolders(ctx, contentObj, folders, callback);
        });
    };

    // Link creation
    if (req.body.resourceSubType === 'link') {
        ContentAPI.createLink(req.ctx, req.body.displayName, req.body.description, req.body.visibility, req.body.link, additionalMembers, _contentCreated);
    ...
    // File creation
        ...
        ContentAPI.createFile(req.ctx, req.body.displayName, req.body.description, req.body.visibility, additionalMembers, uploadedFile, _contentCreated);

    // Collaborative document creation
    } else if (req.body.resourceSubType === 'collabdoc') {
        ContentAPI.createCollabDoc(req.ctx, req.body.displayName, req.body.description, req.body.visibility, additionalMembers, _contentCreated);
