...
module.exports.testGetTenantByPort = function(test) {
    ...
    api.getTenantByPort(2001, function(err, tenant) {
        ...
        test.equal('oae.cam.ac.uk', tenant.baseUrl);
        api.getTenantByPort(2002, function(err, tenant) {
            ...
            test.equal('oae.gatech.edu', tenant.baseUrl);
            test.done();
        });
    });
...
module.exports.testCreateTenant = function(test) {
    ...
    api.createTenant('rsmarttest', 'rSmart Test', 'rSmart Description', 2003, "oae.rsmart.com", function(err, tenant) {
        test.ok(tenant);
        test.equal('rsmarttest', tenant.alias);
        test.equal('rSmart Test', tenant.name);
        test.equal('rSmart Description', tenant.description);
        test.equal('oae.rsmart.com', tenant.baseUrl);
...
module.exports.testCannotCreateDuplicatePort = function(test) {
    test.expect(7);
    api.createTenant('testCannotCreateDuplicatePort', 'Test Tenant', 'A tenant for testing', 2001, "duplicate.com", function(err, tenant) {
        test.ok(err);
        api.getTenantByPort(2001, function(err, tenant) {
            test.ok(!err);
            test.equal(2001, tenant.port);
...
module.exports.testCannotCreateDuplicateAlias = function(test) {
    test.expect(8);
    api.createTenant('camtest', 'Test Tenant', 'A tenant for testing', 2004, "duplicate.com", function(err, tenant) {
        test.ok(err);
