...
module.exports = function(tenant) {
    ...
    tenant.server.post('/api/user/create', function(req, res) {
        var createUser = function() {
            ...
            // users created through this endpoint use local authentication
            var loginId = new LoginId('local', tenant.alias, req.body.username);
            var opts = {
            ...
            PrincipalsAPI.createUser(req.ctx, loginId, req.body.password, req.body.displayName, opts, function(err, user) {
                if (err) {
                    return res.send(err.code, err.msg);
                }
        ...
        ConfigAPI.getConfig(req.ctx.tenant().alias, function(err, config) {
            ...
            recaptcha.checkAnswer(config['oae-principals']['recaptcha']['elements']['private-key'],
                                    req.connection.remoteAddress,
                                    req.body.recaptchaChallenge,
    ...
    tenant.server.get('/api/group/memberships/:id', function(req, res) {
        req.telemetryUrl = '/api/group/memberships/id';
