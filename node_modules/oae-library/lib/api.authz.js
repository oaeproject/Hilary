...
var resolveTargetLibraryVisibility = module.exports.resolveTargetLibraryVisibility = function(ctx, libraryOwnerId, callback) {
    ...
    var visibility = LibraryConstants.visibility.PRIVATE;
    ...
    if (PrincipalsUtil.isUser(libraryOwnerId)) {
        ...
        if (!TenantsUtil.isLoggedIn(ctx, libraryTenantAlias)) {
            visibility = LibraryConstants.visibility.PUBLIC;
        // We give the user the logged in stream in case the current user is not the same as the library we're looking at
            ...
            visibility = LibraryConstants.visibility.LOGGEDIN;
        }
    ...
    // Check if we are looking at a group library
        ...
        // We give the user the public stream in case the user is not logged in
        if (!ctx.user()) {
 ...
 * @param  {Function}   callback                    Invoked when the process completes
 * @param  {Object}     callback.err                An error that occurred, if any
 * @param  {Object}     callback.keyVisibilities    An object keyed by library owner id, whose value is the visibility level in which the resource belongs
 */
