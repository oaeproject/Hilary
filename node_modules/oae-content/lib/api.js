...
var createFile = module.exports.createFile = function(ctx, displayName, description, visibility, additionalMembers, file, callback) {
    ...
    var cleanUpCallback = _getCleanUpCallback(file, callback);
 ...
 * @api private
 */
var _getCleanUpCallback = function(file, callback) {
    return function() {
        ...
        if (err && file && file.path) {
            // Something went wrong with a request that has an uploaded file associated to it.
            // In that case we try to remove the file.
            fs.unlink(file.path, function(unlinkErr) {
                if (unlinkErr) {
                    log().error({'err': unlinkErr, 'file': file}, 'Could not remove the uploaded file.');
                }
