...
module.exports.updateUser = function(context, user_id, visibility, locale, timezone, firstName, lastName, displayName, callback) {
    ...
    module.exports.getUser(context, user_id, function(err, user) {
        ...
        if (context.user().id === user_id || context.user().isAdmin()) {
            update();
        } else {
            callback({'code': 403, 'msg': 'You cannot update this user his profile.'});
        }
    });
};
...
module.exports.setTenantAdmin = function(context, principal_id, isAdmin, callback) {
    if (context.user() && (context.user().isTenantAdmin() || context.user().isGlobalAdmin())) {
