 ...
 */
var PreviewContext = (module.exports.PreviewContext = function(
  config,
  contentId,
  revisionId,
) {
  ...
  var host =
    config.servers.serverInternalAddress || config.servers.globalAdminHost;
  ...
  that.login = function(callback) {
    ...
    RestAPI.Admin.getSignedTenantAuthenticationRequestInfo(
      ...
      function(err, requestInfo) {
        ...
        var restCtx = new RestContext(util.format('%s//%s', protocol, host), {
          hostHeader: hostHeader,
          strictSSL: strictSSL,
        });

        // Perform the actual login
        RestAPI.Admin.doSignedAuthentication(
          restCtx,
          requestInfo.body,
  ...
  that.getContentData = function(callback) {
    RestAPI.Content.getContent(that.tenantRestContext, contentId, function(
      err,
      content,
    ) {
      if (err) {
        log().error(
          { err: err, contentId: contentId },
          'Could not get the content profile.',
        );
        return callback(err);
      }
      ...
      RestAPI.Content.getRevision(
        ...
        function(err, revision) {
          if (err) {
            log().error(
              { err: err, contentId: contentId, revisionId: revisionId },
              'Could not get the revision',
            );
            return callback(err);
          }
      ...
      function(err) {
        if (err) {
          ...
          fs.unlink(path, function(unlinkErr) {
            if (unlinkErr) {
              log().error(
                { err: unlinkErr, contentId: contentId },
