...
var insert = module.exports.insert = function(indexName, entries, callback) {
    ...
    var indexEntries = _.map(entries, function(entry) {
        return {
            ...
            'visibility': _resolveLibraryBucket(entry.id, entry.resource)
        };
    });
 ...
 * @param  {String}         entries[i].resource.id          The id of the resource being inserted
 * @param  {String}         entries[i].resource.visibility  The visibility of the resource being inserted
 * @param  {String}         [entries[i].oldRank]            The old **string-ordered** rank of the resource to insert. Since this is string ordered, a rank of 11 for example is LOWER than 2. If not specified, defaults to 0
 * @param  {String}         [entries[i].newRank]            The new **string-ordered** rank of the resource to insert. Since this is string ordered, a rank of 11 for example is LOWER than 2. If not specified, defaults to 0
 * @param  {Function}       callback                        Standard callback function
 * @param  {Object}         callback.err                    An error that occurred, if any
 */
var update = module.exports.update = function(indexName, entries, callback) {
    ...
    _.each(entries, function(entry) {
        ...
        var oldValueKey = _createValueKey(entry.resource.id, entry.oldRank);
        var newValueKey = _createValueKey(entry.resource.id, entry.newRank);
        var value = '1';
        ...
        _.each(visibilityMasks, function(mask, bucketName) {
            queries.push({
                ...
                'parameters': [_createBucketKey(indexName, entry.id, bucketName), oldValueKey]
            });
            ...
            if (_.contains(mask, entry.resource.visibility)) {
                queries.push({
                    ...
                    'parameters': [value, _createBucketKey(indexName, entry.id, bucketName), newValueKey]
                });
            }
        });
    });
...
var remove = module.exports.remove = function(indexName, entries, callback) {
    ...
    // For each entry deleting, aggregate the query necessary to delete it from each visibility
    // bucket in the library index
    var queries = [];
    _.each(entries, function(entry) {
        var valueKey = _createValueKey(entry.resource.id, entry.rank);
        _.each(visibilityMasks, function(mask, bucketName) {
            queries.push({
                'query': 'DELETE FROM "LibraryIndex" WHERE "bucketKey" = ? AND "rankedResourceId" = ?',
                'parameters': [_createBucketKey(indexName, entry.id, bucketName), valueKey]
            });
        });
    });
    ...
    registeredIndex.pageResources(libraryId, _nextToken, 100, function(err, entries, nextToken) {
        ...
        var indexEntries = _.map(entries, function(entry) {
            return {
                ...
                'rank': entry.rank,
                'resourceId': entry.resource.id,
                'visibility': _resolveLibraryBucket(libraryId, entry.resource)
            };
        });
...
var _resolveLibraryBucket = function(libraryId, resource) {
    ...
    if (effectiveVisibility === AuthzConstants.visibility.LOGGEDIN) {
        ...
        var libraryOwnerTenantAlias = AuthzUtil.getResourceFromId(libraryId).tenantAlias;
        var resourceTenantAlias = AuthzUtil.getResourceFromId(resource.id).tenantAlias;
