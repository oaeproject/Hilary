...
describe('Tenants', function() {
    ...
    describe('Get tenant', function() {
            ...
            TenantsTestUtil.createTenantAndWait(globalAdminRestContext, tenantAlias, tenantDescription, tenantHost, {'emailDomain': tenantEmailDomain}, function(err, createdTenant) {
                ...
                RestAPI.Tenants.getTenant(anonymousRestContext, null, function(err, tenant) {
                    ...
                    RestAPI.Tenants.getTenant(globalAdminRestContext, tenantAlias, function(err, tenant) {
                        ...
                        var tenantByEmailDomain = TenantsAPI.getTenantByEmailDomain(tenantEmailDomain);
        ...
        it('verify get tenant by email domain', function(callback) {
            var randomHost = TenantsTestUtil.generateTestTenantHost();

            // Intialize tenant information for a tenant whose email domain expression is set to an
            // exact match of its host
            var exactMatchTenantAlias = TenantsTestUtil.generateTestTenantAlias();
            var exactMatchTenantDescription = TestsUtil.generateRandomText();
            var exactMatchTenantHost = randomHost;
            var exactMatchTenantOpts = {'emailDomain': exactMatchTenantHost};

            // Initialize tenant information for a tenant whose email domain expression is a suffix
            // match of its host
            var suffixTenantAlias = TenantsTestUtil.generateTestTenantAlias();
            var suffixTenantDescription = TestsUtil.generateRandomText();
            var suffixTenantHost = util.format('suffix.%s', randomHost);
            var suffixTenantOpts = {'emailDomain': util.format('*.%s', randomHost)};

            TenantsTestUtil.createTenantAndWait(globalAdminRestContext, exactMatchTenantAlias, exactMatchTenantDescription, exactMatchTenantHost, exactMatchTenantOpts, function(err, exactMatchTenant) {
                assert.ok(!err);
                TenantsTestUtil.createTenantAndWait(globalAdminRestContext, suffixTenantAlias, suffixTenantDescription, suffixTenantHost, suffixTenantOpts, function(err, suffixTenant) {
                    assert.ok(!err);

                    // Ensure we can get the one by exact match
                    var gotExactMatchTenant = TenantsAPI.getTenantByEmailDomain(exactMatchTenantOpts.emailDomain);
                    assert.ok(gotExactMatchTenant);
                    assert.strictEqual(gotExactMatchTenant.alias, exactMatchTenantAlias);
                    assert.strictEqual(gotExactMatchTenant.host, exactMatchTenantHost);
                    assert.strictEqual(gotExactMatchTenant.emailDomain, exactMatchTenantOpts.emailDomain);

                    // Ensure we can get a tenant by email domain that has the proper suffix
                    var gotSuffixTenant = TenantsAPI.getTenantByEmailDomain(util.format('something.%s', exactMatchTenantOpts.emailDomain));
                    assert.ok(gotSuffixTenant);
                    assert.strictEqual(gotSuffixTenant.alias, suffixTenantAlias);
                    assert.strictEqual(gotSuffixTenant.host, suffixTenantHost);
                    assert.strictEqual(gotSuffixTenant.emailDomain, suffixTenantOpts.emailDomain);

                    // Try something a little more subtle that should match
                    gotSuffixTenant = TenantsAPI.getTenantByEmailDomain(util.format('.%s', exactMatchTenantOpts.emailDomain));
                    assert.ok(gotSuffixTenant);
                    assert.strictEqual(gotSuffixTenant.alias, suffixTenantAlias);
                    assert.strictEqual(gotSuffixTenant.host, suffixTenantHost);
                    assert.strictEqual(gotSuffixTenant.emailDomain, suffixTenantOpts.emailDomain);

                    // Some subtle things that shouldn't match:

                    // One character off
                    assert.ok(!TenantsAPI.getTenantByEmailDomain(util.format('a%s', exactMatchTenantOpts.emailDomain)));

                    // Just missing the first character
                    assert.ok(!TenantsAPI.getTenantByEmailDomain(exactMatchTenantOpts.emailDomain.slice(1)));

                    // Missing the last character in an otherwise valid exact match
                    assert.ok(!TenantsAPI.getTenantByEmailDomain(exactMatchTenantOpts.emailDomain.slice(0, -1)));

                    // Missing the last character in an otherwise valid suffix match
                    assert.ok(!TenantsAPI.getTenantByEmailDomain(util.format('.%s', exactMatchTenantOpts.emailDomain).slice(0, -1)));
                    ...
                    _.chain(shouldMatchCambridge)
                        .map(TenantsAPI.getTenantByEmailDomain)
                        .pluck('alias')
    ...
    describe('Tenant actions', function() {
        ...
        it('verify cannot create tenant with conflicting email domain', function(callback) {
            ...
            var emailDomain = util.format('*.%s', uniqueString1);
            TenantsTestUtil.createTenantAndWait(globalAdminRestContext, uniqueString1, uniqueString1, uniqueString1, {'emailDomain': emailDomain}, function(err) {
                assert.ok(!err);
                ...
                TenantsTestUtil.createTenantAndWait(globalAdminRestContext, uniqueString2, uniqueString2, uniqueString2, {'emailDomain': uniqueString2}, function(err) {
                    ...
                    // Conflict domain where the exact match equals a potential match in emailDomain1
                    var emailDomain1ExactConflict = util.format('.%s', uniqueString1);
                    // Conflict domain where the email matches one that already exists
                    var emailDomain1SuffixConflict1 = util.format('*%s', uniqueString1.slice(1));
                    // Conflict domain where the email is matched by an existing domain expression (inverse of #1)
                    var emailDomain1SuffixConflict2 = util.format('*.something.%s', uniqueString1);

                    // Conflict domain where the exact match matches an exact match that already exists
                    var emailDomain2ExactConflict = uniqueString2;
                    // Conflict domain where the suffix matches an exact match that already exists
                    var emailDomain2SuffixConflict = util.format('*%s', uniqueString2.slice(1));

                    // Ensure exact match domain can't fall under the scope of the suffix
                    var uniqueString3 = TenantsTestUtil.generateTestTenantAlias();
                    TenantsTestUtil.createTenantAndWait(globalAdminRestContext, uniqueString3, uniqueString3, uniqueString3, {'emailDomain': emailDomain1ExactConflict}, function(err) {
                        assert.ok(err);
                        assert.strictEqual(err.code, 400);
                        TenantsTestUtil.createTenantAndWait(globalAdminRestContext, uniqueString3, uniqueString3, uniqueString3, {'emailDomain': emailDomain1SuffixConflict1}, function(err) {
                            assert.ok(err);
                            assert.strictEqual(err.code, 400);
                            TenantsTestUtil.createTenantAndWait(globalAdminRestContext, uniqueString3, uniqueString3, uniqueString3, {'emailDomain': emailDomain1SuffixConflict2}, function(err) {
                                assert.ok(err);
                                assert.strictEqual(err.code, 400);
                                TenantsTestUtil.createTenantAndWait(globalAdminRestContext, uniqueString3, uniqueString3, uniqueString3, {'emailDomain': emailDomain2ExactConflict}, function(err) {
                                    assert.ok(err);
                                    assert.strictEqual(err.code, 400);
                                    TenantsTestUtil.createTenantAndWait(globalAdminRestContext, uniqueString3, uniqueString3, uniqueString3, {'emailDomain': emailDomain2SuffixConflict}, function(err) {
                                        assert.ok(err);
                                        assert.strictEqual(err.code, 400);

                                        // Sanity check we can create a tenant with these values
                                        TenantsTestUtil.createTenantAndWait(globalAdminRestContext, uniqueString3, uniqueString3, uniqueString3, {'emailDomain': uniqueString3}, function(err) {
                                            assert.ok(!err);
                                            return callback();
                                        });
                                    });
        ...
        it('verify update email domains validation', function(callback) {
            ...
            TestsUtil.createTenantWithAdmin(uniqueString1, uniqueString1, function(err, tenant, tenantAdminRestContext) {
                ...
                TestsUtil.createTenantWithAdmin(uniqueString2, uniqueString2, function(err) {
                    ...
                    // Conflict uniqueString1 domain where the exact match equals a potential match in emailDomain1
                    var emailDomain1ExactConflict = util.format('.%s', uniqueString1);
                    // Conflict uniqueString1 domain where the email matches one that already exists
                    var emailDomain1SuffixConflict1 = util.format('*%s', uniqueString1.slice(1));
                    // Conflict uniqueString1 domain where the email is matched by an existing domain expression (inverse of #1)
                    var emailDomain1SuffixConflict2 = util.format('*.something.%s', uniqueString1);

                    // Conflict uniqueString2 domain where the exact match matches an exact match that already exists
                    var emailDomain2ExactConflict = uniqueString2;
                    // Conflict uniqueString2 domain where the suffix matches an exact match that already exists
                    var emailDomain2SuffixConflict = util.format('*%s', uniqueString2.slice(1));

                    // Ensure only global admin can update email domain
                    RestAPI.Tenants.updateTenant(tenantAdminRestContext, null, {'emailDomain': emailDomain}, function(err) {
                        assert.ok(err);
                        assert.strictEqual(err.code, 401);
                        RestAPI.Tenants.updateTenant(globalAdminRestContext, uniqueString1, {'emailDomain': emailDomain}, function(err) {
                            assert.ok(!err);
                            ...
                            RestAPI.Tenants.updateTenant(globalAdminRestContext, uniqueString2, {'emailDomain': emailDomain1ExactConflict}, function(err) {
                                ...
                                RestAPI.Tenants.updateTenant(globalAdminRestContext, uniqueString2, {'emailDomain': emailDomain1SuffixConflict1}, function(err) {
                                    ...
                                    RestAPI.Tenants.updateTenant(globalAdminRestContext, uniqueString2, {'emailDomain': emailDomain1SuffixConflict2}, function(err) {
                                        ...
                                        RestAPI.Tenants.updateTenant(globalAdminRestContext, uniqueString2, {'emailDomain': uniqueString2}, function(err) {
                                            assert.ok(!err);

                                            // Ensure we can update tenant 1's email domain to conflict with itself
                                            RestAPI.Tenants.updateTenant(globalAdminRestContext, uniqueString1, {'emailDomain': emailDomain1ExactConflict}, function(err) {
                                                assert.ok(!err);

                                                // Ensure we can't update tenant1's email domain to conflict with tenant 2's
                                                RestAPI.Tenants.updateTenant(globalAdminRestContext, uniqueString1, {'emailDomain': emailDomain2ExactConflict}, function(err) {
                                                    assert.ok(err);
                                                    assert.strictEqual(err.code, 400);
                                                    RestAPI.Tenants.updateTenant(globalAdminRestContext, uniqueString1, {'emailDomain': emailDomain2SuffixConflict}, function(err) {
                                                        assert.ok(err);
                                                        assert.strictEqual(err.code, 400);
                                                        ...
                                                        RestAPI.Tenants.getTenant(tenantAdminRestContext, null, function(err, tenant) {
                                                            assert.ok(!err);
                                                            assert.strictEqual(tenant.emailDomain, emailDomain1ExactConflict);
                                                            RestAPI.Tenants.getTenant(globalAdminRestContext, uniqueString2, function(err, tenant) {
                                                                assert.ok(!err);
                                                                assert.strictEqual(tenant.emailDomain, uniqueString2);
