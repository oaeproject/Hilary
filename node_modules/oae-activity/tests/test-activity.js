    ...
    describe('Activity API', function() {
        ...
        describe('#refreshConfiguration()', function() {
            ...
            it('verify activity ttl deletes an activity after the expiry time', function(callback) {
                ...
                ActivityAPI.refreshConfiguration(createDefaultConfig({'activityTtl': 2}), function(err) {
                    assert.ok(!err);
                    ...
                    RestAPI.User.createUser(camAdminRestContext, jackUsername, 'password', 'Jack McJackerson', null, function(err, jack) {
                        ...
                        RestAPI.Content.createLink(jackCtx, 'Google', 'Google', 'public', 'http://www.google.ca', [], [], function(err, link) {
                            ...
                            ActivityTestsUtil.collectAndGetActivityStream(jackCtx, null, null, function(err, activityStream) {
                                ...
                                setTimeout(ActivityTestsUtil.collectAndGetActivityStream, 2100, jackCtx, null, null, function(err, activityStream) {
                                    assert.ok(!err);
                                    assert.ok(activityStream);
                                    assert.ok(activityStream.items);
                                    assert.equal(activityStream.items.length, 0);
    ...
    describe('Activity Routing', function() {
                ...
                ConfigTestsUtil.updateConfigAndWait(TestsUtil.createGlobalAdminRestContext(), privateTenant1.tenant.alias, {'oae-tenants/tenantprivacy/tenantprivate': false}, function(err) {
                    ...
                    FollowingTestsUtil.followByAll(publicTenant0.publicUser.user.id, followers, function(err) {
                        ...
                        ConfigTestsUtil.updateConfigAndWait(TestsUtil.createGlobalAdminRestContext(), privateTenant1.tenant.alias, {'oae-tenants/tenantprivacy/tenantprivate': true}, function(err) {
                            ...
                            ActivityTestsUtil.collectAndGetActivityStream(publicTenant0.publicUser.restContext, null, null, function(err, response) {
                                assert.ok(!err);
                                ActivityTestsUtil.assertActivity(response.items[0], 'content-create', 'create', publicTenant0.publicUser.user.id, testId);

                                // Ensure a user from the same tenant who isn't a member got it
                                ActivityTestsUtil.collectAndGetActivityStream(publicTenant0.privateUser.restContext, null, null, function(err, response) {
                                    assert.ok(!err);
                                    ActivityTestsUtil.assertActivity(response.items[0], 'content-create', 'create', publicTenant0.publicUser.user.id, testId);

                                    // Ensure a user from another public tenant did not get it since they don't have access to the private object
                                    ActivityTestsUtil.collectAndGetActivityStream(publicTenant1.privateUser.restContext, null, null, function(err, response) {
                                        assert.ok(!err);

                                        // The last activity in their feed should be the follow activity from when they followed the publicTenant0 public user
                                        ActivityTestsUtil.assertActivity(response.items[0], 'following-follow', 'follow', publicTenant1.privateUser.user.id, publicTenant0.publicUser.user.id);

                                        // Ensure the user from the private tenant did not get it either
                                        ActivityTestsUtil.collectAndGetActivityStream(privateTenant1.publicUser.restContext, null, null, function(err, response) {
                                            assert.ok(!err);

                                            // The last activity in their feed should be the follow activity from when they followed the publicTenant0 public user
                                            ActivityTestsUtil.assertActivity(response.items[0], 'following-follow', 'follow', privateTenant1.publicUser.user.id, publicTenant0.publicUser.user.id);
                                            return callback();
                                        });
                                    });
                                });
            ...
            TestsUtil.setupMultiTenantPrivacyEntities(function(publicTenant0, publicTenant1, privateTenant0, privateTenant1) {
                ...
                FollowingTestsUtil.followByAll(publicTenant0.publicUser.user.id, followers, function(err) {
                    ...
                    ActivityTestsUtil.collectAndGetActivityStream(publicTenant0.publicUser.restContext, null, null, function(err, response) {
                        ...
                        ActivityTestsUtil.collectAndGetActivityStream(publicTenant0.privateUser.restContext, null, null, function(err, response) {
                            assert.ok(!err);
                            ActivityTestsUtil.assertActivity(response.items[0], 'following-follow', 'follow', publicTenant0.privateUser.user.id, publicTenant0.publicUser.user.id);
                            return callback();
                        });
                    });
                });
        ...
        it('verify activity association exclusion', function(callback) {
            TestsUtil.generateTestUsers(camAdminRestContext, 3, function(err, users, nico, branden, simon) {
                ...
                ActivityTestsUtil.collectAndGetActivityStream(branden.restContext, null, null, function(err, activityStream) {
                    assert.ok(!err);
                    ...
                    ActivityTestsUtil.collectAndGetActivityStream(nico.restContext, null, null, function(err, activityStream) {
                        ...
                        // Because Simon is excluded from the *empty set* (and NOT from the `all` set) he should've received an activity as well
                ...
                ActivityTestsUtil.collectAndGetActivityStream(anonymousCamRestContext, jack.id, null, function(err, activityStream) {
                    ...
                    ActivityTestsUtil.collectAndGetActivityStream(jackCtx, '  ', null, function(err, activityStream) {
                        ...
                        ActivityTestsUtil.collectAndGetActivityStream(jackCtx, 'c:cam:someContent', null, function(err, activityStream) {
                            ...
                            ActivityTestsUtil.collectAndGetActivityStream(jackCtx, 'c:cam:someContent', {'format': 'non-existing'}, function(err, activityStream) {
                                assert.ok(err);
                                assert.equal(err.code, 400);
                                assert.ok(!activityStream);
                ...
                RestAPI.Content.createLink(jack.restContext, 'A', 'A', 'public', 'http://www.google.ca', [], [], function(err, link) {
                    ...
                    ActivityTestsUtil.collectAndGetActivityStream(jack.restContext, null, null, function(err, activityStream) {
                        ...
                        RestAPI.Activity.getActivityStream(jack.restContext, jack.user.id, {'format': 'internal'}, function(err, activityStream) {
                            ...
                            assert.equal(activity.actor['timezone'], 'Etc/UTC');
                            ...
                            allowedActorProperties = ['oae:id', 'id', 'displayName', 'visibility', 'locale', 'timezone', 'publicAlias', 'picture', 'profilePath', 'resourceType', 'acceptedTC', 'objectType', 'tenant', 'email'];
                            _.each(activity.actor, function(value, key) {
        ...
        it('verify aggregation can be reset', function(callback) {
            TestsUtil.generateTestUsers(camAdminRestContext, 2, function(err, users, simong, mrvisser) {
                ...
                RestAPI.Content.createLink(simong.restContext, 'First item', 'A', 'public', 'http://www.google.be', [], [mrvisser.user.id], function(err, firstContentObj) {
                    assert.ok(!err);
                    ActivityTestsUtil.collectAndGetActivityStream(mrvisser.restContext, null, null, function(err, activityStream) {
                        assert.ok(!err);
                        // Sanity check that the create content activity is in mrvisser's activity stream
                        ActivityTestsUtil.assertActivity(activityStream.items[0], 'content-create', 'create', simong.user.id, firstContentObj.id);
                        ...
                        ActivityAggregator.resetAggregationForActivityStreams([mrvisser.user.id + '#activity'], function(err) {
                            ...
                            RestAPI.Content.createLink(simong.restContext, 'Second item', 'A', 'public', 'http://www.google.be', [], [mrvisser.user.id], function(err, secondContentObj) {
                                ...
                                ActivityTestsUtil.collectAndGetActivityStream(mrvisser.restContext, null, null, function(err, activityStream) {
                                    ...
                                    assert.equal(activityStream.items.length, 2);
                                    ActivityTestsUtil.assertActivity(activityStream.items[0], 'content-create', 'create', simong.user.id, secondContentObj.id);
                                    ActivityTestsUtil.assertActivity(activityStream.items[1], 'content-create', 'create', simong.user.id, firstContentObj.id);
                                    ...
                                    RestAPI.Content.createLink(simong.restContext, 'Third item', 'A', 'public', 'http://www.google.be', [], [mrvisser.user.id], function(err, thirdContentObj) {
                                        ...
                                        ActivityTestsUtil.collectAndGetActivityStream(mrvisser.restContext, null, null, function(err, activityStream) {
                                            assert.ok(!err);
                                            assert.equal(activityStream.items.length, 2);
                                            ActivityTestsUtil.assertActivity(activityStream.items[0], 'content-create', 'create', simong.user.id, [secondContentObj.id, thirdContentObj.id]);
                                            ActivityTestsUtil.assertActivity(activityStream.items[1], 'content-create', 'create', simong.user.id, firstContentObj.id);
                                            ...
                                            ActivityTestsUtil.collectAndGetNotificationStream(mrvisser.restContext, null, function(err, notificationStream) {
                                                ...
                                                ActivityTestsUtil.assertActivity(notificationStream.items[0], 'content-create', 'create', simong.user.id, [firstContentObj.id, secondContentObj.id, thirdContentObj.id]);
                                                ...
                                                ActivityAggregator.resetAggregationForActivityStreams([mrvisser.user.id + '#notification'], function(err) {
                                                    ...
                                                    RestAPI.Content.createLink(simong.restContext, 'Fourth item', 'A', 'public', 'http://www.google.be', [], [mrvisser.user.id], function(err, fourthContentObj) {
                                                        ...
                                                        ActivityTestsUtil.collectAndGetNotificationStream(mrvisser.restContext, null, function(err, notificationStream) {
                                                            ...
                                                            ActivityTestsUtil.assertActivity(notificationStream.items[0], 'content-create', 'create', simong.user.id, fourthContentObj.id);
                                                            ActivityTestsUtil.assertActivity(notificationStream.items[1], 'content-create', 'create', simong.user.id, [firstContentObj.id, secondContentObj.id, thirdContentObj.id]);
                                                            return callback();
                                                        });
                                                    });
                                                });
                ...
                RestAPI.Content.createLink(simong.restContext, 'A', 'A', 'public', 'http://www.google.ca', [], [], function(err, contentObj) {
                    ...
                    RestAPI.Content.createComment(simong.restContext, contentObj.id, 'Comment Comment', null, function(err, comment) {
                        ...
                        ActivityTestsUtil.collectAndGetActivityStream(simong.restContext, null, null, function(err, activityStream) {
                            ...
                            ActivityTestsUtil.assertActivity(activityStream.items[0], 'content-comment', 'post', simong.user.id, comment.id, contentObj.id);
