...
module.exports.config = function(moduleId) {
  ...
  return {
    ...
    getValue: function(tenantAlias, featureKey, elementKey) {
      var configValueInfo = _resolveConfigValueInfo(
        tenantAlias,
        moduleId,
        featureKey,
        elementKey,
...
var getSchema = (module.exports.getSchema = function(ctx, callback) {
  if (!ctx.user() || !ctx.user().isAdmin(ctx.tenant().alias)) {
    return callback({
      code: 401,
      msg: 'Only global and tenant admin can get the config schema',
    });
 ...
 */
var getTenantConfig = (module.exports.getTenantConfig = function(
  ctx,
  tenantAlias,
  callback,
) {
  ...
  validator
    .check(tenantAlias, { code: 400, msg: 'Missing tenant parameter' })
    .notEmpty();
  ...
  _.each(cachedGlobalSchema, function(module, moduleKey) {
    _.each(module, function(feature, featureKey) {
      _.each(feature.elements, function(element, elementKey) {
        ...
        tenantConfig[moduleKey][featureKey] =
          tenantConfig[moduleKey][featureKey] || {};
 ...
 * @api private
 */
var _resolveConfigValueInfo = function(
  tenantAlias,
  moduleKey,
  featureKey,
  elementKey,
  ...
  var adminConfigValues =
    cachedTenantConfigValues[config.servers.globalAdminAlias] || {};
  ...
  callback =
    ...
    function(err) {
      if (err) {
        log().error(
          { err: err },
          'An error occured whilst initializing the configuration',
        );
      }
    };
 ...
 * @param  {Object}     callback.err    An error that occurred, if any
 */
var updateTenantConfig = (module.exports.updateTenantConfig = function(
  tenantAlias,
  callback,
) {
  callback =
    ...
    function(err) {
      if (err) {
        log().error(
          { err: err, tenantAlias: tenantAlias },
          'An error occured while caching a tenant config',
        );
      }
    };
  ...
  var getModuleSchema = function(module) {
    ...
    IO.getFileListForFolder(dir, function(err, configFiles) {
      ...
      for (var c = 0; c < configFiles.length; c++) {
        ...
        cachedGlobalSchema[module] = _.extend(
          cachedGlobalSchema[module] || {},
          configFile,
        );
      }
          ...
          _.each(mod, function(feature, featureKey) {
            ...
            _.each(feature.elements, function(element, elementKey) {
              ...
              if (element.globalAdminOnly) {
                delete cachedTenantSchema[moduleKey][featureKey].elements[
                  elementKey
                ];
              }
            });
  ...
  callback =
    ...
    function(err) {
      if (err) {
        log().error(
          { err: err },
          'An error occured whilst caching all tenant configs',
        );
      }
    };
    ...
    if (_.isString(value)) {
      ...
      if (element) {
        try {
          ...
          persistentConfig[hash.tenantAlias] =
            persistentConfig[hash.tenantAlias] || {};
 ...
 * @api private
 */
var _canUpdateConfigValue = function(
  ctx,
  tenantAlias,
  moduleId,
  feature,
 ...
 * @param  {Object}     callback.err    An error that occurred, if any
 */
var updateConfig = (module.exports.updateConfig = function(
  ctx,
  tenantAlias,
  configValues,
  callback,
  ...
  if (!ctx.user() || !ctx.user().isAdmin(tenantAlias)) {
    return callback({
      code: 401,
      msg: 'Only authorized tenant admins can change config values',
    });
  }
  ...
  var validator = new Validator();
  validator
    .check(tenantAlias, { code: 400, msg: 'Missing tenantid' })
    .notEmpty();
  validator
  ...
  for (var i = 0; i < configFieldNames.length; i++) {
    ...
    validator
      .check(null, {
        code: 400,
        msg: util.format(
          'The configuration value for "%s" must be specified',
          configFieldName,
        ),
      })
    ...
    if (!_element(parts[0], parts[1], parts[2])) {
      ...
      !_canUpdateConfigValue(ctx, tenantAlias, parts[0], parts[1], parts[2])
    ) {
      return callback({
        code: 401,
        msg: util.format(
          'User is not allowed to update config value "%s"',
          configFieldName,
        ),
      });
    }
  ...
  _.each(configValues, function(value, key) {
    ...
    if (optionalKey) {
      var currentConfigValue = _resolveConfigValueInfo(
        tenantAlias,
        module,
        feature,
        element,
  ...
  _.each(aggregatedValues, function(value, key) {
    ...
    queries.push({
      query:
        'UPDATE "Config" SET "value" = ? WHERE "tenantAlias" = ? AND "configKey" = ?',
 ...
 * @param  {Object}     callback.err    An error that occurred, if any
 */
var clearConfig = (module.exports.clearConfig = function(
  ctx,
  tenantAlias,
  configFields,
  callback,
  ...
  var validator = new Validator();
  validator
    .check(tenantAlias, { code: 400, msg: 'Missing tenant alias' })
    .notEmpty();
  validator
    .check(configFields.length, {
      code: 400,
      msg:
        'Missing configuration. Example configuration: ["oae-authentication/twitter/enabled"]',
    })
    .min(1);
  ...
  var queries = _.map(rowChanges, function(value, columnName) {
    if (!value) {
      return {
        query:
          'DELETE FROM "Config" WHERE "tenantAlias" = ? AND "configKey" = ?',
