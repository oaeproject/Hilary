...
var generatePreviews = module.exports.generatePreviews = function(ctx, content, callback) {
    ...
    ZencoderUtil.job(config, tenant.alias, downloadPath, content.id, revision.id, _isVideo(mimeType), function(err, res, body) {
        ...
        ZencoderDAO.saveContentRevisionId(output.id.toString(), content.id, revision.id, function(err) {
            if (err) {
                return callback(err);
            }
        });
