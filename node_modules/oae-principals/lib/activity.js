...
ActivityAPI.registerActivityEntityProducer('user', function(activityType, entityType, resource, callback) {
    PrincipalsDAO.getPrincipal(resource.resourceId, function(err, user) {
        ...
        TenantsAPI.getTenantByAlias(user.tenant, function(err, userTenant) {
            ...
            return callback(null, new ActivityModel.ActivityEntity('user', globalId, opts));
        });
    });
});
...
ActivityAPI.registerActivityEntityProducer('group', function(activityType, entityType, resource, callback) {
    PrincipalsDAO.getPrincipal(resource.resourceId, function(err, group) {
        ...
        TenantsAPI.getTenantByAlias(group.tenant, function(err, groupTenant) {
            ...
            return callback(null, new ActivityModel.ActivityEntity('group', globalId, opts));
        });
    });
});
 ...
 */
ActivityAPI.registerActivityRouter('group', function(activityType, entityType, entity, callback) {
    // Deliver to all the group members, direct or indirect
    var groupId = entity[ActivityConstants.properties.OAE_ID];
    ActivityUtil.getAllAuthzMembersByRole(groupId, function(err, specify) {
        ...
        if (isMembershipActivity) {
            ...
            routes = _.uniq(_.flatten(_.values(specify)));
        }

        // Resolve propagation
        var propagation = {'type': ActivityConstants.entityPropagation.ALL};
        var visibility = entity[ActivityConstants.properties.OAE_VISIBILITY];
        var joinable = entity[ActivityConstants.properties.OAE_JOINABLE];
        if (visibility !== 'public' && visibility !== 'loggedin' && joinable !== 'yes') {
