...
var getUrlInfo = module.exports.getUrlInfo = function(ctx, expiresIn, callback) {
    ...
    expiresIn = OaeUtil.getNumberParam(expiresIn, TIME_FIVE_MINUTES_IN_SECONDS, TIME_ONE_MINUTE_IN_SECONDS, TIME_THREE_HOURS_IN_SECONDS) * 1000;

    // Extract configuration values
    var enabled = UservoiceConfig.getValue(ctx.tenant().alias, 'general', 'enabled');
    var baseUrl = UservoiceConfig.getValue(ctx.tenant().alias, 'general', 'baseUrl') || '';
    ...
    // Ensure User Voice is enabled and its subdomain is configured
    if (!enabled) {
...
/**
 * Generate an authentication token that can be used for secure communication with User Voice
 *
