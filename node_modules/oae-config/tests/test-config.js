...
describe('Configuration', function() {
    ...
    describe('Schema', function() {
        ...
        /**
         * Test that verifies that the configuration schema for a global admin has `private` values and
         * that the schema for a tenant admin excludes `private` values.
         */
        it('verify private', function(callback) {
            RestAPI.Config.getSchema(globalAdminRestContext, function(err, schema) {
                assert.ok(!err);
                assert.ok(schema);
            ...
            ConfigTestUtil.updateConfigAndWait(globalAdminRestContext, null, {'oae-authentication/twitter/enabled': false}, function(err) {
                ...
                RestAPI.Config.getTenantConfig(globalAdminRestContext, null, function(err, config) {
                    ...
                    RestAPI.Config.getTenantConfig(camAdminRestContext, null, function(err, config) {
                        ...
                        ConfigTestUtil.updateConfigAndWait(globalAdminRestContext, null, {'oae-mediacore/mediacore/collectionId': 'newID'}, function(err) {
                            assert.ok(!err);
                            ...
                            RestAPI.Config.getTenantConfig(globalAdminRestContext, null, function(err, config) {
                                assert.ok(!err);
                                assert.ok(config);
                                assert.equal(config['oae-mediacore']['mediacore']['collectionId'], 'newID');
    ...
    describe('Configuration options', function() {
        it('verify value with `private: true` and `tenantOverride: true` can only be viewed by the global admin', function(callback) {
            // Assert that `private` and `tenantOverride` are set for the configuration we'll be using in the test
            RestAPI.Config.getSchema(globalAdminRestContext, function(err, schema) {
                assert.equal(schema['oae-principals']['recaptcha'].elements['privateKey'].private, true);
        ...
        it('verify value with `private: true` and `tenantOverride: true` can only be overridden by the global admin', function(callback) {
            // Assert that `private` and `tenantOverride` are set for the configuration we'll be using in the test
            RestAPI.Config.getSchema(globalAdminRestContext, function(err, schema) {
                assert.equal(schema['oae-principals']['recaptcha'].elements['privateKey'].private, true);
                assert.equal(schema['oae-principals']['recaptcha'].elements['privateKey'].tenantOverride, true);
                ...
                ConfigTestUtil.updateConfigAndWait(camAdminRestContext, null, {'oae-principals/recaptcha/privateKey': 'new private key'}, function(err) {
                    assert.ok(err);
                    ...
                    RestAPI.Config.getTenantConfig(globalAdminRestContext, global.oaeTests.tenants.cam.alias, function(err, config) {
                        ...
                        ConfigTestUtil.updateConfigAndWait(johnRestContext, null, {'oae-principals/recaptcha/privateKey': 'new private key'}, function(err) {
                            assert.ok(err);
                            ...
                            RestAPI.Config.getTenantConfig(globalAdminRestContext, global.oaeTests.tenants.cam.alias, function(err, config) {
                                ...
                                ConfigTestUtil.updateConfigAndWait(anonymousCamRestContext, null, {'oae-principals/recaptcha/privateKey': 'new private key'}, function(err) {
                                    assert.ok(err);
                                    ...
                                    RestAPI.Config.getTenantConfig(globalAdminRestContext, global.oaeTests.tenants.cam.alias, function(err, config) {
                                        ...
                                        ConfigTestUtil.updateConfigAndWait(globalAdminRestContext, global.oaeTests.tenants.cam.alias, {'oae-principals/recaptcha/privateKey': 'new private key'}, function(err) {
                                            assert.ok(!err);

                                            // Verify that the configuration has changed
                                            RestAPI.Config.getTenantConfig(globalAdminRestContext, global.oaeTests.tenants.cam.alias, function(err, config) {
                                                assert.ok(!err);
                                                assert.equal(config['oae-principals']['recaptcha']['privateKey'], 'new private key');
                                                callback();
                                            });
                                        });
                                    });
        ...
        it('verify value with `private: true` and `tenantOverride: false` can only be viewed by the global admin', function(callback) {
            // Assert that `private` and `tenantOverride` are set for the configuration we'll be using in the test
            RestAPI.Config.getSchema(globalAdminRestContext, function(err, schema) {
                assert.equal(schema['oae-tenants']['actions'].elements['allowStop'].private, true);
                assert.equal(schema['oae-tenants']['actions'].elements['allowStop'].tenantOverride, false);

                // Private values don't return for anonymous users
                RestAPI.Config.getTenantConfig(anonymousCamRestContext, null, function(err, config) {
                    assert.ok(!err);
                    assert.equal(config['oae-tenants']['actions']['allowStop'], null);
                    ...
                    RestAPI.Config.getTenantConfig(johnRestContext, null, function(err, config) {
                        ...
                        RestAPI.Config.getTenantConfig(camAdminRestContext, null, function(err, config) {
                            ...
                            RestAPI.Config.getTenantConfig(globalAdminRestContext, null, function(err, config) {
                                assert.ok(!err);
                                assert.equal(config['oae-tenants']['actions']['allowStop'], false);
        ...
        it('verify value with `private: true` and `tenantOverride: false` can not be overridden on the tenant', function(callback) {
            // Assert that private and tenantOverride are set for the configuration we'll be using in the test
            RestAPI.Config.getSchema(globalAdminRestContext, function(err, schema) {
                assert.equal(schema['oae-tenants']['actions'].elements['allowStop'].private, true);
                assert.equal(schema['oae-tenants']['actions'].elements['allowStop'].tenantOverride, false);
                ...
                // Try overriding a private configuration value as a tenant admin on the tenant server
                ConfigTestUtil.updateConfigAndWait(camAdminRestContext, null, {'oae-tenants/actions/allowStop': true}, function(err) {
                    assert.ok(err);
                    ...
                    RestAPI.Config.getTenantConfig(globalAdminRestContext, global.oaeTests.tenants.cam.alias, function(err, config) {
                        ...
                        // Try overriding a private configuration value as a logged in user on the tenant server
                        ConfigTestUtil.updateConfigAndWait(johnRestContext, null, {'oae-tenants/actions/allowStop': true}, function(err) {
                            assert.ok(err);
                            ...
                            RestAPI.Config.getTenantConfig(globalAdminRestContext, global.oaeTests.tenants.cam.alias, function(err, config) {
                                ...
                                // Try overriding a private configuration value as an anonymous user on the tenant server
                                ConfigTestUtil.updateConfigAndWait(anonymousCamRestContext, null, {'oae-tenants/actions/allowStop': true}, function(err) {
                                    assert.ok(err);
                                    ...
                                    RestAPI.Config.getTenantConfig(globalAdminRestContext, global.oaeTests.tenants.cam.alias, function(err, config) {
                                        ...
                                        // Try overriding a private configuration value as the global administrator on the tenant server
                                        ConfigTestUtil.updateConfigAndWait(globalAdminRestContext, global.oaeTests.tenants.cam.alias, {'oae-tenants/actions/allowStop': true}, function(err) {
                                            assert.ok(err);
