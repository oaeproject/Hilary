...
var dataTypes = require('cassandra-driver').types.dataTypes;
var Row = require('cassandra-driver').types.Row;
...
module.exports.init = function(config, callback) {
    ...
    client = new cassandra.Client({ contactPoints: CONFIG.hosts,
        keyspace: CONFIG.keyspace,
        protocolOptions: { maxVersion: 3 },
        socketOptions: { readTimeout: 12000, connectTimeout: 12000 }
    });
...
var _reloadClient = function (keyspace) {
    client = new cassandra.Client({ contactPoints: CONFIG.hosts,
        keyspace: keyspace,
        protocolOptions: { maxVersion: 3 },
        socketOptions: { readTimeout: 12000, connectTimeout: 12000 }
...
var createKeyspace = module.exports.createKeyspace = function(name, callback) {
    ...
    keyspaceExists(name, function(err, exists) {
        ...
        if (!exists) {
            ...
            const query = `CREATE KEYSPACE "${name}" WITH REPLICATION = { 'class': '${options.strategyClass}', 'replication_factor': ${options.replication} }`;
...
var dropKeyspace = module.exports.dropKeyspace = function(name, callback) {
    callback = callback || function() {};
    runQuery(`DROP KEYSPACE "${name}"`, null, function(err) {
        if (err) {
            return callback(err);
        }
        callback(null, true);
...
var keyspaceExists = module.exports.keyspaceExists = function(name, callback) {
    // pool.getConnection()._client.describe_keyspace(name, function(err, definition) {
    // });
    ...
    client.execute(query, function (err, results) {
        if (results.rowLength === 0) {
            ...
            log().error({'err': err, 'name': name}, 'Error while describing cassandra keyspace');
...
var dropColumnFamily = module.exports.dropColumnFamily = function(name, callback) {
    ...
    columnFamilyExists(name, function(err, exists) {
        ...
        runQuery(`DROP TABLE "${name}"`, [], function(err) {
            if (err) {
                return callback(err);
            }
...
var runBatchQuery = module.exports.runBatchQuery = function(queries, callback) {
    ...
    queries = _.map(queries, function(eachQueryElement) {
        if(_.contains(eachQueryElement.parameters, null)) {
            console.log("TINHA NULL EU SABIA");
        }
            ...
            if (!returnKeyColumn) {
                ...
                _.each(row.keys(), (eachKey) => {
                    if (eachKey !== keyColumnName) {
                        newRowContent.push({ key: eachKey, value: row.get(eachKey), column: _.find(columns, (eachItem) => { return eachItem.name === eachKey }) });
                    }
                });
...
var executeQuery = function(query, parameters, callback) {
    ...
    // pool.cql(query, parameters, {'consistencyLevel': 2}, function(err, results) {
    // });

    client.execute(query, parameters, { prepare: true }, function (err, resultSet) {
        if (err) {
            console.log(err.stack);
...
var _describeKeyspace = function(keyspace, callback) {
    ...
    client.execute(query, function (err, definition) {
        if (err && err.name) {
 ...
 * @api private
 */
var _truncateLogParameters = function(err, query, params) {
    // We truncate the query string to a maximum
    var queryMaxStrLength = 300;
    ...
    var paramsStrMaxLength = 800;
    var paramsArrayMaxLength = 10;
    var paramStrMaxLength = 80;
    ...
    if (paramsStr && paramsStr.length > paramsStrMaxLength) {
        ...
        params = _.map(params.slice(0, paramsArrayMaxLength),
 ...
 * @api private
 */
var _truncateString = function (str, ifOverSize) {
    if (_.isString(str) && str.length > ifOverSize) {
        str = util.format('%s (and %s more)', str.slice(0, ifOverSize),
            str.length - ifOverSize);
    }
