...
describe('Signature', () => {
    ...
    describe('Expiring Signatures', (callback) => {
        ...
        it('verify expiring signature cannot be verified with different data or expires timestamp', (callback) => {
            ...
            assert.ok(!Signature.verifyExpiringSignature({'0': 'one', '1': 'zero'}, signatureData.expires, signatureData.signature));
            assert.ok(!Signature.verifyExpiringSignature({'0': 'zero', '1': 'one'}, signatureData.expires + 1, signatureData.signature));
            assert.ok(Signature.verifyExpiringSignature({'0': 'zero', '1': 'one'}, signatureData.expires, signatureData.signature));
            return callback();
        });
        ...
        it('verify expiring signature minimum and maximum duration guarantees', (callback) => {
            ...
            Date.now = function() { return now + (5 * 1000); };
            ...
            Date.now = function() { return now + (12 * 1000); };
            assert.ok(!Signature.verifyExpiringSignature(data, signatureData.expires, signatureData.signature));
        ...
        it('verify expiring signature remains the same when generated within the same rolling window', (callback) => {
            ...
            Date.now = function() { return Date.UTC(2013, 0, 25, 11, 39, 46); };
            ...
            Date.now = function() { return Date.UTC(2013, 0, 25, 11, 44, 46); };
            ...
            Date.now = function() { return Date.UTC(2013, 0, 25, 11, 55, 46); };
            ...
            assert.notEqual(signatureDataSecond.signature, signatureDataThird.signature);
