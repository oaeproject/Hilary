...
EventEmitter.prototype.emit = function(/*name, [args...], [callback]*/) {
  ...
  if (!_.isString(name)) {
    throw new Error(
      util.format(
        'Expected a string for event "name", but got: %s',
  ...
  // a function
  var consumerCallback = _.chain(args)
    .last()
    .isFunction()
    .value()
    ? args.pop()
    : function(errs) {
        if (errs) {
          log().error(
            { errs: errs, name: name },
            'Unhandled error(s) occurred processing `when` handlers',
          );
        }
      };
  ...
  var _handlerCallback = function(err, result) {
    if (err) {
...
EventEmitter.prototype.when = function(name, handler) {
  if (!_.isString(name)) {
    throw new Error(
      'Can only bind "when" handler for event whose name is a string',
    );
  } else if (!_.isFunction(handler)) {
    throw new Error(
      'Can only bind a function as the "when" handler for an event',
    );
  }
