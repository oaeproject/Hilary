...
describe('Users', function() {
                                        ...
                                        RestAPI.User.createUser(userRestContext, newUserId, 'password', 'Test User', null, function(err, userObj) {
                                            ...
                                            RestAPI.User.createUser(globalAdminRestContext, newUserId, 'password', 'Test User', {'visibility': 'public', 'tenantAlias': global.oaeTests.tenants.cam.alias}, function(err, userObj) {
                                                ...
                                                RestAPI.User.createUser(globalAdminRestContext, newUserId, 'password', 'Test User', {'visibility': 'public', 'tenantAlias': 'admin'}, function(err, userObj) {
                                                    ...
                                                    // Verify creating a global admin on the global tenant as a global admin
                                                    var newUserId = TestsUtil.generateTestUserId();
                                                    RestAPI.User.createUser(globalAdminRestContext, newUserId, 'password', 'Test User', {'visibility': 'public', 'tenantAlias': 'admin', 'isGlobalAdmin': true}, function(err, userObj) {
                                                        assert.ok(!err);
                                                        assert.ok(userObj.isGlobalAdmin);

                                                        // Verify creating a tenant admin on the global tenant as a global admin
                                                        var newUserId = TestsUtil.generateTestUserId();
                                                        RestAPI.User.createUser(globalAdminRestContext, newUserId, 'password', 'Test User', {'visibility': 'public', 'tenantAlias': 'admin', 'isTenantAdmin': true}, function(err, userObj) {
                                                            assert.ok(!err);
                                                            assert.ok(userObj.isTenantAdmin);

                                                            // Verify creating a global admin on a regular tenant as a global admin
                                                            var newUserId = TestsUtil.generateTestUserId();
                                                            RestAPI.User.createUser(globalAdminRestContext, newUserId, 'password', 'Test User', {'visibility': 'public', 'tenantAlias': global.oaeTests.tenants.cam.alias, 'isGlobalAdmin': true}, function(err, userObj) {
                                                                assert.ok(!err);
                                                                assert.ok(userObj.isGlobalAdmin);

                                                                // Verify creating a tenant admin on a regular tenant as a global admin
                                                                var newUserId = TestsUtil.generateTestUserId();
                                                                RestAPI.User.createUser(globalAdminRestContext, newUserId, 'password', 'Test User', {'visibility': 'public', 'tenantAlias': global.oaeTests.tenants.cam.alias, 'isTenantAdmin': true}, function(err, userObj) {
                                                                    assert.ok(!err);
                                                                    assert.ok(userObj.isTenantAdmin);

                                                                    // Verify creating a global admin on a regular tenant as a tenant administrator fails
                                                                    var newUserId = TestsUtil.generateTestUserId();
                                                                    RestAPI.User.createUser(camAdminRestContext, newUserId, 'password', 'Test User', {'visibility': 'public', 'isGlobalAdmin': true}, function(err, userObj) {
                                                                        assert.ok(err);
                                                                        assert.equal(err.code, 401);

                                                                        // Verify creating a tenant admin on a regular tenant as a tenant administrator
                                                                        var newUserId = TestsUtil.generateTestUserId();
                                                                        RestAPI.User.createUser(camAdminRestContext, newUserId, 'password', 'Test User', {'visibility': 'public', 'isTenantAdmin': true}, function(err, userObj) {
                                                                            assert.ok(!err);
                                                                            assert.ok(userObj.isTenantAdmin);
                                                                            callback();
                                                                        });
    ...
    describe('Update user', function() {
        ...
        it('verify update user', function(callback) {
            ...
            RestAPI.User.createUser(camAdminRestContext, testUserId, 'password', 'Test User', {'visibility': 'public', 'locale': 'en_GB', 'timezone': 'Europe/London'}, function(err, user) {
                assert.ok(!err);
                assert.ok(user);
                assert.equal(user.visibility, 'public');
                ...
                PrincipalsTestUtil.uploadAndCropPicture(testUserRestContext, user.id, _getPictureStream, {'x': 10, 'y': 10, 'width': 200}, function() {
                    ...
                    RestAPI.User.updateUser(testUserRestContext, user.id, updateValues, function (err, user) {
                        ...
                        RestAPI.User.getMe(testUserRestContext, function(err, me) {
                            ...
                            assert.equal(me.locale.locale, 'nl_NL');
                            assert.equal(me.locale.timezone.name, 'Europe/Amsterdam');
