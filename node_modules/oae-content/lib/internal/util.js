 ...
 */
var augmentContent = module.exports.augmentContent = function(ctx, contentObj) {
    ...
    contentObj.signature = signContent(ctx, contentObj.id, contentObj.lastModified);

    if (contentObj.previews) {
        if (contentObj.previews.thumbnailUri) {
            contentObj.previews.thumbnailUrl = getDownloadUrl(ctx, contentObj.previews.thumbnailUri, contentObj.id);
        }

        if (contentObj.previews.smallUri) {
            contentObj.previews.smallUrl = getDownloadUrl(ctx, contentObj.previews.smallUri, contentObj.id);
        }

        if (contentObj.previews.mediumUri) {
            contentObj.previews.mediumUrl = getDownloadUrl(ctx, contentObj.previews.mediumUri, contentObj.id);
        }

        if (contentObj.previews.largeUri) {
            contentObj.previews.largeUrl = getDownloadUrl(ctx, contentObj.previews.largeUri, contentObj.id);
        }
    }
};
