...
var previewCounter = new Counter();
 ...
 */
var whenPreviewsComplete = module.exports.whenPreviewsComplete = function(callback) {
    previewCounter.whenZero(function() {
 ...
 */
FoldersAPI.on(FoldersConstants.events.UPDATED_FOLDER, function(ctx, newFolder, oldFolder) {
 ...
 */
ContentAPI.emitter.on(ContentConstants.events.UPDATED_CONTENT_PREVIEW, function(content) {
...
ContentAPI.emitter.on(ContentConstants.events.DELETED_CONTENT, function(ctx, contentObj, members) {
    ...
    FoldersDAO.getFoldersByGroupIds(groupIds, function(err, folders) {
        if (err) {
            log().error({'err': err, 'contentId': contentObj.id}, 'Unable to regenerate folder preview after removing a piece of content');
            return;
        }
 ...
 * @api private
 */
var _reprocessFoldersThatContainContent = function(contentId) {
    previewCounter.incr();

    // Get all the folders this content item was part of
    FoldersAuthz.getFoldersForContent(contentId, function(err, folders) {
        if (err) {
            log().error({'err': err, 'contentId': contentId}, 'Unable to get the folders a piece of content resides in');
            previewCounter.decr();
            return;
        }
