 ...
 */
var getClient = module.exports.getClient = function(callback) {
     ...
     * @param  {String}    activityStreamId     The ID of the activity stream, ex: `activity`
     ...
     */
    client.subscribe = function(resourceId, activityStreamId, token, callback) {
        var payload = {
            'stream': {
     ...
     * A `close` event will be emitted once the socket is closed.
     */
 ...
 * @param  {String}     data.streams[i].activityStreamId    The id of the stream that should be retrieved, ex: `notification` or `activity`
 ...
 */
var getFullySetupClient = module.exports.getFullySetupClient = function(data, callback) {
    getClient(function(client) {
        client.authenticate(data.authentication.userId, data.authentication.tenantAlias, data.authentication.signature, function(err) {
            ...
            var register = function() {
                ...
                var stream = data.streams.pop();
                client.subscribe(stream.resourceId, stream.activityStreamId, stream.token, function(err) {
                    assert.ok(!err, 'Failed to register for feed ' + stream.resourceId + '#' + stream.activityStreamId);
                    register();
                });
            };
