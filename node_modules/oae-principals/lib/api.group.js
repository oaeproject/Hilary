...
var setGroupMembers = module.exports.setGroupMembers = function(ctx, groupId, memberChanges, callback) {
    ...
    PrincipalsDAO.getPrincipal(groupId, function(err, group) {
        ...
        var principalIds = _.keys(memberChanges);
        _getExistingPrincipals(principalIds, function(err, principals) {
            ...
            var principalRoles = _.map(principals, function(principal) {
                return {
                    'principal': principal,
                    'role': memberChanges[principal.id]
                };
            });

            AuthzPermissions.canSetRoles(ctx, group, {'principalRoles': principalRoles}, function(err, computedPermissions, newMembers, updatedMembers, removedMembers) {
                if (err) {
                    return callback(err);
                }
...
var leaveGroup = module.exports.leaveGroup = function(ctx, groupId, callback) {
    ...
    PrincipalsDAO.getPrincipal(groupId, function(err, group) {
        ...
        AuthzPermissions.canRemoveRole(ctx, ctx.user(), group, function(err, removedMembers, userRole) {
            if (err) {
                return callback(err);
            }
...
var createGroup = module.exports.createGroup = function(ctx, displayName, description, visibility, joinable, permissions, callback) {
    ...
    var currentUserId = _getUserId(ctx);
    permissions[currentUserId] = PrincipalsConstants.roles.MANAGER;
    ...
    _getExistingPrincipals(principalIds, function(err, principalsById) {
        ...
        var targets = {
            'principalRoles': _.map(principalsById, function(principal) {
                return {
                    'principal': principal,
                    'role': permissions[principal.id]
                };
            })
        };
        ...
        AuthzPermissions.canCreate(ctx, targets, function(err) {
            ...
            // Create the group
            PrincipalsDAO.createGroup(groupId, tenantAlias, displayName, description, visibility, joinable, currentUserId, function (err, group) {
                if (err) {
                    return callback(err);
                }
