...
module.exports = function(ctx, opts, callback) {
    ...
    validator.check(opts.email, {'code': 400, 'msg': 'An invalid email address has been provided'}).isEmail();
    if (validator.hasErrors()) {
        return callback(validator.getFirstError());
    }

    var query = SearchUtil.createEmailQuery(opts.email);
    var filterResources = SearchUtil.filterResources(['user']);
    var needsFilterByExplicitAccess = false;
    SearchUtil.filterScopeAndAccess(ctx, opts.scope, needsFilterByExplicitAccess, function(err, filterScopeAndAccess) {
        if (err) {
            return callback(err);
        }
