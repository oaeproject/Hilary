 ...
 * @param  {Object}    permissions          A hash where each key is a principal id and the value is one of `manager` or `member`
...
var createGroup = module.exports.createGroup = function(ctx, displayName, description, visibility, joinable, permissions, otherValues, callback) {
    ...
    _.each(permissions, function(role, principalId) {
        ...
        validator.check(role, {'code': 400, 'msg': 'Only `manager` or `member` are accepted as a role for a principal'}).isIn(['manager', 'member']);
    });
