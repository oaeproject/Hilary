 ...
 */
var augmentContent = module.exports.augmentContent = function(ctx, content, duration, offset, callback) {
    ...
    if (content.previews) {
        ...
        if (content.previews.videoKey  && typeof callback === 'function') {
            _getSignedVideoUrl(ctx, content.previews.videoKey, function(err, url) {
                if (err) {
                    log(ctx).error({'err': err}, 'Couldn\'t create signed Amazon S3 URL for video preview');
                    return callback(err);
                } else {
                    content.previews.video = url;
                    return callback();
                }
            });
        }
...
/**
 * Generate a signed url that can be used to show video previews in the browser for a video file stored in AWS S3
 *
 * @param  {Context}    ctx               Current request context
 * @param  {String}     key               The name of the file stored in S3
 * @param  {Function}   callback          Standard callback function
 * @param  {Object}     callback.err      An error that occurred, if any
 * @return {String}     callback.url      The signed S3 url that can be used to view the video
 * @api private
 */
var _getSignedVideoUrl = function(ctx, key, callback) {
    var config = ZencoderUtil.getConfig(ctx.tenant().alias);

    var s3 = new AWS.S3({
        'accessKeyId': config['amazons3-access-key'],
        'secretAccessKey': config['amazons3-secret-key'],
    ...
    var params = _.extend({}, headParams, {
        // The URL will be valid for an  hour
        'Expires': 60 * 60
    });

    s3.headObject(headParams, function(err, data) {
        if (err) {
            log(ctx).error({'err': err}, 'Couldn\'t connect to Amazon S3 bucket');
            return callback(err);
        }
...
var getSignedDownloadUrl = module.exports.getSignedDownloadUrl = function(ctx, uri, duration, offset) {
    ...
    var downloadStrategy = getStorageBackend(ctx, uri).getDownloadStrategy(ctx.tenant().alias, uri);
...
var createPersistentContentActivityEntity = module.exports.createPersistentContentActivityEntity = function(content) {
    ...
    return new ActivityModel.ActivityEntity('content', content.id, content.visibility, {'content': content});
};
