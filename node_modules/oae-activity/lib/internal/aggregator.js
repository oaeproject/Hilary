 ...
 *
 * @param  {String[]}   activityStreams     The set of activity streams that need to have their aggregation reset
 * @param  {Function}   callback            Standard callback function
 * @param  {Object}     callback.err        Standard error object, if any
 */
var resetAggregationForActivityStreams = module.exports.resetAggregationForActivityStreams = function(activityStreams, callback) {
    callback = callback || function() {};
    ActivityDAO.resetAggregationForActivityStreams(activityStreams, function(err) {
        if (err) {
            log().error({'err': err, 'activityStreams': activityStreams}, 'Failed to reset aggregation for activity streams');
            return callback(err);
        }

        ActivityEmitter.emit(ActivityConstants.events.RESET_AGGREGATION, activityStreams);
        return callback();
    });
};
        ...
        ActivityDAO.getAggregateStatus(allAggregateKeys, function(err, statusByAggregateKey) {
            ...
            ActivityDAO.deleteAggregateData(_.keys(expiredAggregates), function(err) {
                ...
                ActivityDAO.getAggregatedEntities(_.keys(activeAggregates), function(err, fetchedEntities) {
                     ...
                     *      active single- or multi-aggregate. This would make sure that we don't redliver an active aggregate, AND deliver
