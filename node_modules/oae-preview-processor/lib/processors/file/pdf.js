...
var _pdf2htmlLessTemplatePath = util.format(
  ...
  exec(cmd, { timeout: _pdf2htmlEXTimeout }, function(err, stdout, stderr) {
    if (err) {
      log().error(
        ...
        'Could not properly execute the pdf2htmlEX binary',
      );
      return callback({
        code: 500,
        msg: 'The path for the pdf2htmlEX binary is misconfigured',
      });
    }

    _pdf2htmlEXBinary = config.pdf2htmlEX.binary;
    _pdf2htmlEXTimeout = OaeUtil.getNumberParam(
      config.pdf2htmlEX.timeout,
      _pdf2htmlEXTimeout,
    );
    ...
    exec(cmd, { timeout: _pdftotextTimeout }, function(err, stdout, stderr) {
      if (err) {
        log().error(
          { err: err, stdout: stdout, stderr: stderr },
          'Could not properly execute the pdftotext binary',
        );
        return callback({
          code: 500,
          msg: 'The path for the pdftotext binary is misconfigured',
        });
      }

      _pdftotextBinary = config.pdftotext.binary;
      _pdftotextTimeout = OaeUtil.getNumberParam(
        config.pdftotext.timeout,
        _pdftotextTimeout,
      );
      ...
      fs.readFile(_pdf2htmlLessTemplatePath, 'utf8', function(
        err,
        templateContent,
      ) {
        if (err) {
          log().error(
            { err: err, stdout: stdout, stderr: stderr },
            'Could not read the pdf2htmlEX LESS template',
          );
          return callback({
            code: 500,
            msg: 'Could not read the pdf2htmlEX LESS template',
          });
        }
        ...
        if (templateContent) {
          try {
            ...
            log().error(
              { err: ex, templateContent: templateContent },
          ...
          return callback({
            code: 500,
            msg:
              'pdf2htmlEX LESS template file ' +
              _pdf2htmlLessTemplatePath +
              ' had no content',
          });
        }
      });
...
var previewPDF = (module.exports.previewPDF = function(ctx, path, callback) {
  ...
  fs.mkdir(pagesDir, function(err) {
    if (err) {
      log().error(
        { err: err, contentId: ctx.contentId },
...
var _convertPDFToHTMLPages = function(ctx, path, pagesDir, callback) {
  ...
  exec(cmd, { timeout: _pdf2htmlEXTimeout }, function(err, stdout, stderr) {
    if (err) {
      log().error(
        { err: err, cmd: cmd, stdout: stdout, stderr: stderr },
        'Could not convert page to html',
      );
    ...
    fs.readdir(pagesDir, function(err, files) {
      ...
      var htmlFiles = _.filter(files, function(file) {
        return (
          file.split('.')[0] === 'page' && file.split('.').pop() === 'html'
        );
      });
        ...
        fs.writeFile(
          ...
          function(err) {
            if (err) {
              ...
              return callback({
                code: 500,
                msg: 'Error saving the combined CSS file',
              });
            }
  ...
  gm(path + '[0]')
    ...
    .write(output, function(err) {
      if (err) {
        log().error(
          { err: err, contentId: ctx.contentId },
          'Could not convert a PDF page to a PNG',
        );
        return callback({
          code: 500,
          msg: 'Could not convert a PDF page to a PNG',
        });
      }

      // Crop a thumbnail out of the png file
      PreviewUtil.generatePreviewsFromImage(
        ctx,
        output,
        { cropMode: 'TOP' },
        callback,
...
var _convertToText = function(ctx, input, pagesDir, callback) {
  ...
  exec(cmd, options, function(err, stdout, stderr) {
    if (err) {
      log().error(
        { err: err, contentId: ctx.contentId, stdout: stdout, stderr: stderr },
        'Could not convert the PDF to plain text',
      );
      return callback({
        code: 500,
        msg: 'Could not convert the PDF to plain text',
      });
    }
    ...
    fs.readFile(output, { encoding: 'utf8' }, function(err, text) {
      if (err) {
        log().error(
          { err: err, contentId: ctx.contentId },
          'Could not read the generated plain text',
        );
        return callback({
          code: 500,
          msg: 'Could not read the generated plain text',
        });
      }
