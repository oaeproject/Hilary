...
var _storePicture = function(ctx, principal, file, callback) {
    ...
    ContentAPI.getStorageBackend(ctx).store(ctx, file, options, function(err, largePictureUri) {
        ...
        // Save in Cassandra.
        Cassandra.runQuery('UPDATE Principals SET largePictureUri = ? WHERE principalId = ?', [ largePictureUri, principal.id ], function(err) {
            if (err) {
                return callback(err);
            }

            principal.picture.largeUri = largePictureUri;
            principal.picture.large = PrincipalsUtil.getSignedPictureUrl(ctx.tenant().alias, largePictureUri);
...
var _saveUrisToCassandra = function(ctx, principal, smallPictureUri, mediumPictureUri, callback) {
    Cassandra.runQuery('UPDATE Principals SET smallPictureUri = ?, mediumPictureUri = ? WHERE principalId = ?', [ smallPictureUri, mediumPictureUri, principal.id ], function(err) {
        if (err) {
            return callback(err);
        }

        principal.picture.smallUri = smallPictureUri;
        principal.picture.mediumUri = mediumPictureUri;
        principal.picture.small = PrincipalsUtil.getSignedPictureUrl(ctx.tenant().alias, smallPictureUri);
        principal.picture.medium = PrincipalsUtil.getSignedPictureUrl(ctx.tenant().alias, mediumPictureUri);
