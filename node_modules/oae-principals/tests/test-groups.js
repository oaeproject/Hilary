...
describe('Groups', function() {
    ...
    describe('#canManage', function() {
        ...
        it('verify canManage returns false when checking if anonymous can manage a group', function(callback) {
            ...
            RestAPI.Group.createGroup(johnRestContext, TestsUtil.generateTestGroupId(), 'Group title', 'Group description', 'public', 'yes', [], [], function(err, publicGroupObj) {
                assert.ok(!err);

                RestAPI.Group.createGroup(johnRestContext, TestsUtil.generateTestGroupId(), 'Group title', 'Group description', 'loggedin', 'yes', [], [], function(err, loggedinGroupObj) {
                    assert.ok(!err);

                    RestAPI.Group.createGroup(johnRestContext, TestsUtil.generateTestGroupId(), 'Group title', 'Group description', 'private', 'yes', [], [], function(err, privateGroupObj) {
                        assert.ok(!err);
    ...
    describe('Update group', function() {
        ...
        it ('verify update non-existent group', function(callback) {
            ...
            RestAPI.Group.updateGroup(johnRestContext, groupId, {'joinable': 'yes'}, function(err) {
    ...
    describe('Get group members', function() {
        ...
        /**
         * Test that verifies that the amount of returned principals is bounded when fetching the group members.
         */
        it('verify parameter validation', function(callback) {
            ...
            TestsUtil.generateTestUsers(camAdminRestContext, 30, function(err, users) {
                assert.ok(!err);
                var groupId = TestsUtil.generateTestGroupId();
                RestAPI.Group.createGroup(johnRestContext, groupId, 'Group title', 'Group description', 'private', 'request', [], _.keys(users), function(err, newGroup) {
                    assert.ok(!err);
                    ...
                    RestAPI.Group.getGroupMembers(johnRestContext, newGroup.id, null, 'totally-not-a-number', function(err, members) {
                        ...
                        // A negative number (or 0) should result in 1 member being returned
                        RestAPI.Group.getGroupMembers(johnRestContext, newGroup.id, null, -10, function(err, members) {
                            assert.ok(!err);
                            assert.equal(members.results.length, 1);
                            ...
                            RestAPI.Group.getGroupMembers(johnRestContext, newGroup.id, null, 0, function(err, members) {
                                ...
                                // A too high number will result in 25 members being returned.
