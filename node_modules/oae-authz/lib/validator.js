...
Validator.prototype.isValidRole = function() {
    if (!_.isString(this.str) || _.isEmpty(this.str.trim())) {
...
Validator.prototype.isValidRoleChange = function() {
    if (this.str === false) {
        // 'false' is valid, it represents the removal of a role
        return;
    }
...
/**
 * Checks whether or not a string is specified and is a valid visibility. If the string is not a
 * member of the list of all available visibilities, it fails.
 *
 * Usage:
 * ```
 * var validator = new Validator();
 * validator.check(visibility, error).isValidVisibility();
 * ```
 */
Validator.prototype.isValidVisibility = function() {
    var allVisibilities = AuthzConstants.visibility.ALL_PRIORITY;
    if (!_.isString(this.str) || !_.contains(allVisibilities, this.str)) {
        this.error(this.msg || 'A visibility must be one of: %s', allVisibilities.join(', '));
    }
};
