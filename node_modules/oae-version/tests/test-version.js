...
describe('Version information', function() {
    ...
    it('should return the version information', function(callback) {
        ...
        TestsUtil.generateTestUsers(adminTenantRestContext, 1, function(err, users, user) {
            ...
            verifyVersionInformation(anonTenantRestContext, function() {
                verifyVersionInformation(userTenantRestContext, function() {
                    verifyVersionInformation(adminTenantRestContext, function() {

                        // Verify the version information on the global admin
                        verifyVersionInformation(anonGlobalRestContext, function() {
                            verifyVersionInformation(globalAdminRestContext, callback);
                        });
                    });
                });
            });
     ...
     */
    function verifyVersionInformation(restContext, callback) {
        RestAPI.Version.getVersion(restContext, function(err, version) {
            assert.ok(!err);
            assert.ok(_.isObject(version));
            assert.strictEqual(_.keys(version).length, 2);
            assert.ok(_.isString(version.hilary));
            assert.ok(_.isString(version['3akai-ux']));
            return callback();
        });
    }
});
