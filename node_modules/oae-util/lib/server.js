...
var setupRouter = module.exports.setupRouter = function(app) {
    ...
    /**
     * Use swagger to setup a route on the associated server
     *
     * @param  {Object}               spec            The swagger spec for the route
     * @throws {Error}                                Error thrown when arguments aren't of the proper type
     */
    that.addSwaggerEndpoint = function(spec) {
        if (_.isString(spec.method)) {
            _addApiMethod(spec);
        } else {
    ...
    /**
     * Add a swagger model to the server
     *
     * @param  {String}     name         The model name
     * @param  {Model}      model        See https://github.com/wordnik/swagger-spec/blob/master/versions/1.2.md#527-model-object
     */
    that.addSwaggerModel = function(name, model) {
        that.models[name] = model;
        that.models[name].id = name;
    };
    ...
    /**
     * Add an api method to the swagger spec
     *
     * @param  {Object}        spec     The swagger spec for the api method
     * @api private
     */
    var _addApiMethod = function(spec) {
        var apiRootPath = spec.path.split('/')[1];
        // Get the /api root resource
        var root = that.resources[apiRootPath] = that.resources[apiRootPath] || {
        ...
        _.each(root.apis, function (api) {
            if (api.path === spec.path && api.method === spec.method) {
    ...
    var _appendToApi = function(rootResource, api, spec) {
        ...
        if (validator.hasErrors()) {
            log().warn({'swaggerValidationErrors': validator.getErrors()}, 'Some swagger documentation could not be parsed, the server will start but those routes may be unavailable or undocumented');
        }
        ...
        rootResource.models = rootResource.models || {};
    };
