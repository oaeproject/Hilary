...
module.exports = function(tenant) {
    ...
    tenant.server.post('/api/group/create', function(req, res) {
        ...
        principalAPI.createGroup(req.ctx, req.body.name, req.body.description, req.body.visibility, req.body.joinable, memberHash, {}, function(err, groupId) {
            if (err) {
                return res.send(err.code, err.msg);
            }
    ...
    tenant.server.get('/api/group/:id', function(req, res) {
        principalAPI.getGroup(req.ctx, req.params.id, function(err, group) {
            if (err) {
                return res.send(err.code, err.msg);
            }
            ...
            if (req.ctx.user()) {
                AuthzAPI.getRole(req.ctx.user().id, group.id, function(err, role) {
                    if (err) {
                        return res.send(err.code, err.msg);
                    }
                    if (role) {
                        group.isMember = true;
                        group.isManager = (role === 'manager')
                    }
