...
/**
 * Gets the Etherpad author IDs of users who are in the pad right now
 *
 * @param  {String}     contentId           The OAE ID of the collaborative document
 * @param  {[String]}   padId               The Etherpad ID of the collaborative document
 * @param  {Function}   callback            Standard callback function
 * @param  {Object}     callback.err        Standard error object (if any)
 * @param  {String[]}   callback.authors    A set of authors that are online right now
 */
var getOnlineAuthors = module.exports.getOnlineAuthors = function(contentId, padId, callback) {
    ...
    client.padUsers({'padID': padId}, function(err, data) {
        if (err) {
            log().error({'err': err, 'contentId': contentId, 'padId': padId}, 'Could not get the users in a pad');
            return callback({'code': 500, 'msg': 'Could not get the users in a pad'});
        }
