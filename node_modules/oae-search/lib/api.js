...
var search = module.exports.search = function(ctx, searchType, opts, callback) {
    ...
    registeredSearch(ctx, opts, function(err, queryData) {
        ...
        // ensure we get the 'extra' field from the _source. * gives us all the stored fields, _source.extra gives us the 'extra' object
        // TODO: If we can somehow get an ES 'object' mapping to just 'store' all its data, this shouldn't be necessary. Then we could in theory disable the _source document (saves on space)
        queryData.fields = [ '*', '_source.extra' ];
        ...
        client.search(queryData, null, function(err, results) {
            ...
            // we pull the 'extra' field out of the _source and put it on the top-level fields object.
            var hits = results.hits.hits;
            for (var i = 0; i < hits.length; i++) {
                var hit = hits[i];
                hit.fields.extra = hit.fields['_source.extra'];
                delete hit.fields['_source.extra'];
            }
...
var _handleDeleteDocumentTask = function(data, callback) {
    ...
    var errs = [];
    client.delete(SearchConstants.resourceMembersMappingName, SearchUtil.getResourceMembersDocumentId(data.id), function(err) {
        if (err) {
            errs.push(err);
        }

        client.delete(SearchConstants.resourceMembershipsMappingName, SearchUtil.getResourceMembershipsDocumentId(data.id), function(err) {
            if (err) {
                errs.push(err);
            }

            client.delete(SearchConstants.resourceMappingName, data.id, function(err) {
                if (err) {
                    errs.push(err);
                }
...
var _handleIndexDocumentTask = function(data, callback) {
    ...
    if (documentProducer) {
        ...
        documentProducer(resourceUpdates, function(err, resourceDocs) {
            ...
            var resourceDocs = _.map(resourceDocs, function(doc) {
        ...
        log().warn('Ignoring %s documents of type "%s", which do not have an associated document producer.', resources.length, resourceType);
    }
};

var _aggregateMembersDocuments = function(memberUpdates, callback, memberDocs) {
    memberDocs = memberDocs || [];
    if (memberUpdates.length === 0) {
        return callback(null, memberDocs);
    }

    var memberUpdate = memberUpdates.shift();
    ...
    if (members) {
        ...
        return _aggregateMembersDocuments(memberUpdates, callback, memberDocs);
    } else {
...
var _createMembershipsDocument = function(resourceId, membershipIds) {
    return {
        '_parent': resourceId,
        '_type': SearchConstants.resourceMembershipsMappingName,
        'id': SearchUtil.getResourceMembershipsDocumentId(resourceId),
