...
module.exports.checkPassword = function(tenant, username, password, callback) {
    ...
    cassandra.runQuery("SELECT password FROM Principals WHERE principal_id = ?", [user_id], function(err, rows) {
        if (err) {
        ...
        utils.hashPassword(rows[0].colHash.password, function(hash) {
            if (hash !== password) {
                callback(false, null);
            } else {
                callback(false, {'principal_id': user_id});
            }
        });
    });
};