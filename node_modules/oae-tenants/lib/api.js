...
/**
 * Get a list of all available tenants from cache. The global admin tenant and deleted tenants will be excluded from the resulting tenant list
 *
...
var getTenant = module.exports.getTenant = function(tenantAlias) {
    return _copyTenant(tenants[tenantAlias && tenantAlias.toLowerCase()]);
};
...
var createTenant = module.exports.createTenant = function(ctx, alias, displayName, host, callback) {
    ...
    // Make sure the tenant alias is all lower-case
...
var deleteTenants = module.exports.deleteTenants = function(ctx, aliases, callback) {
    ...
    var validator = new Validator();
    validator.check(null, {'code': 401, 'msg': 'You must be a global admin user to disable a tenant'}).isGlobalAdministratorUser(ctx);
    validator.check(aliases.length, {'code': 400, 'msg': 'You must provide at least one alias to disable'}).min(1);
    _.each(aliases, function(alias) {
        validator.check(getTenant(alias), {'code': 404, 'msg': util.format('Tenant with alias "%s" does not exist and cannot be disabled', alias)}).notNull();
    });
    if (validator.hasErrors()) {
        return callback(validator.getFirstError());
    }
    ...
    Cassandra.runBatchQuery(queries, function(err) {
        ...
        // Broadcast the message across the cluster so we can stop the tenants
        _.each(aliases, function(alias) {
            Pubsub.publish('oae-tenants', 'stop ' + alias, callback);
        });
    });
};
...
var mapToTenant = function(row) {
    ...
    return new Tenant(hash.alias.toLowerCase(), hash.displayName, hash.host.toLowerCase(), {
        'active': hash.active,
