...
var renderTemplate = module.exports.renderTemplate = function(template, data, locale) {
    ...
    data.util = {
        ...
        'markdown': {
            /*!
             * Given markdown content, convert it to an appropriate HTML string
             *
             * @param  {String}     content     The markdown content to convert to HTML
             * @return {String}                 The HTML version of the content
             */
            'toHtml': function(str) {
                return marked(str, {'gfm': true, 'breaks': true, 'sanitize': true});
            },
        },
        ...
        'i18n': {
            ...
            'formatDate': function(date, dateFormat, timezone) {
                ...
                kludgeDate.setMinutes(kludgeDate.getMinutes() - kludgeDate.getTimezoneOffset());

                var globalized = Globalize.format(kludgeDate, dateFormat, globalizeLocale);

                // Append the timezone abbreviation to formats that include the time
                if (_.contains(['f', 'F', 't', 'T', 'S'], dateFormat)) {
                    globalized = util.format('%s %s', globalized, kludgeDate.getTimezoneAbbreviation());
                }

                // I would do something with date formats containing z, zz, or zzz but this happened https://github.com/jquery/globalize/issues/327
