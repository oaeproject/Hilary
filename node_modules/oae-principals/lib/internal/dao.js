...
var createRequestJoinGroup = module.exports.createRequestJoinGroup = function(principalId, groupId, callback) {
    ...
    Cassandra.runQuery('INSERT INTO "GroupJoinRequestsByGroup" ("groupId", "principalId", "created_at", "updated_at", "status") VALUES (?, ?, ?, ?, ?)', [groupId, principalId, lastModified, lastModified, PrincipalsConstants.requestStatus.PENDING], function(err) {
        if (err) {
            return callback(err);
        }
 ...
 * @api private
 */
var updateJoinGroupByRequest = module.exports.updateJoinGroupByRequest = function(principalId, groupId, status, callback) {
    var queries = [];
    var lastModified = Date.now().toString();

    // Create queries
    queries.push({'query': 'UPDATE "GroupJoinRequestsByGroup" SET "status" = ?, "updated_at" = ? WHERE "groupId" = ? AND "principalId" = ?', 'parameters': [status, lastModified, groupId, principalId]});
 ...
 * @api private
 */
var getJoinGroupRequest = module.exports.getJoinGroupRequest = function(groupId, principalId, callback) {
    Cassandra.runQuery('SELECT * FROM "GroupJoinRequestsByGroup" WHERE "groupId" = ? AND "principalId" = ?', [groupId, principalId], function (err, row) {
        if (err) {
            return callback(err);
        }
        if (_.isEmpty(row)) {
 ...
 * @api private
 */
var getJoinGroupRequests = module.exports.getJoinGroupRequests = function(groupId, callback) {
    Cassandra.runQuery('SELECT * FROM "GroupJoinRequestsByGroup" WHERE "groupId" = ?', [groupId], function (err, rows) {
        if (err) {
            return callback(err);
        }
        if (_.isEmpty(rows)) {
        ...
        var users = _.map(rows, function(row) { return Cassandra.rowToHash(row); });
