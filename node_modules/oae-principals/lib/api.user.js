...
var _createUser = function(ctx, user, email, callback) {
    PrincipalsDAO.createUser(user, function(err) {
        ...
        if (email && !user.email) {
            _sendEmailToken(ctx, user, email, null, function(err) {
                if (err) {
                    return callback(err);
                }
    ...
    PrincipalsDAO.getPrincipal(userId, function(err, oldUser) {
        ...
        _updateUser(ctx, oldUser, profileFields, function(err, newUser) {
            ...
            // If the email address changed but isn't verified, we have to send a verification email
                ...
                _sendEmailToken(ctx, newUser, newEmailAddress, null, function(err) {
                    if (err) {
                        return callback(err);
                    }
 ...
 * It will also not send out any email verification tokes if an email address were to change
 *
 * @param  {Context}        ctx             Standard context object containing the current user and the current tenant
...
var _sendEmailToken = function(ctx, user, email, token, callback) {
    ...
    PrincipalsDAO.storeEmailToken(user.id, email, token, function(err) {
        ...
        var fullTenant = TenantsAPI.getTenant(user.tenant.alias);
        var url = TenantsUtil.getBaseUrl(fullTenant) + '/email-verification?token=' + token;
 ...
 */
var hasPendingEmailToken = module.exports.hasPendingEmailToken = function(ctx, userId, callback) {
    var validator = new Validator();
    validator.check(userId, {'code': 400, 'msg': 'A valid user id must be provided'}).isUserId();
...
var deleteEmailToken = module.exports.deleteEmailToken = function(ctx, userId, callback) {
    ...
    hasPendingEmailToken(ctx, userId, function(err) {
        if (err) {
            return callback(err);
        }
        ...
        PrincipalsDAO.deleteEmailToken(userId, function(err) {
            ...
            PrincipalsEmitter.emit(PrincipalsConstants.events.DELETED_EMAIL_TOKEN, ctx, userId);
