...
var AuthenticationConfig = require('oae-config').config('oae-authentication');
 ...
 */
var isEnabled = module.exports.isEnabled = function(tenantAlias) {
...
var getServiceProviderUrl = module.exports.getServiceProviderUrl = function(ctx) {
    // The URL where the Shibboleth SP software (apache/mod_shib) is running
    var spURL = util.format('https://%s', getSPHost());

    // Generate a signature
    var data = {'tenantAlias': ctx.tenant().alias};
    var signature = Signature.createExpiringSignature(data, 60, 60);

    // Create and return the full URL
    return util.format('%s/api/auth/shibboleth/sp?tenantAlias=%s&signature=%s&expires=%s', spURL, ctx.tenant().alias, signature.signature, signature.expires);
};

/**
 * Validate the given `tenantAlias` and signature parameters. If the provided
 * parameters are valid the full tenant object will be returned. This method
 * will ensure that a tenant with the given `tenantAlias` exists and has enabled
 * Shibboleth as one of their authentication strategies.
 *
 ...
 * @oaram  {Tenant}     callback.tenant     The full tenant object for the given tenant alias
 */
var validateInitiateParameters = module.exports.validateInitiateParameters = function(tenantAlias, signature, expires, callback) {
    ...
    var data = {'tenantAlias': tenantAlias};
 ...
 * @oaram  {Tenant}     callback.tenant     The full tenant object for the given tenant alias
 */
var getShibbolethEnabledTenant = module.exports.getShibbolethEnabledTenant = function(tenantAlias, callback) {
...
/**
 * Get the URL where a user should be redirected to once mod_shib has
 ...
 * @return {String}                 The full URL where the user should be redirected to
 */
var getAuthenticatedUserRedirectUrl = module.exports.getAuthenticatedUserRedirectUrl = function(tenant, user) {
    ...
    var signature = Signature.createExpiringSignature(data, 60, 60);
    return util.format('https://%s/api/auth/shibboleth/callback?userId=%s&signature=%s&expires=%s', tenant.host, user.id, signature.signature, signature.expires);
};
 ...
 */
var getUser = module.exports.getUser = function(tenant, userId, signature, expires, callback) {
    var validator = new Validator();
    ...
    getShibbolethEnabledTenant(tenant.alias, function(err) {
        if (err) {
            return callback(err);
        }
        ...
        PrincipalsAPI.getUser(context, userId, callback);
    });
};
