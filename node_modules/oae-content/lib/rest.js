...
/*!
 * Download a specific version for a piece of content.
 */
OAE.tenantServer.get(/\/api\/content\/([%A-Za-z0-9:_-]*)\/download\/?([A-Za-z0-9:_-]*)/, function(req, res) {
    req.telemetryUrl = '/api/content/id/download/version';
    ContentAPI.getRevision(req.ctx, req.params[0], req.params[1], function(err, revision) {
        if (err) {
            return res.send(err.code, err.msg);
        }
...
var handleDownloadLink = function(res, link) {
    ...
    // A 204 suggest that nginx will be handling the download via the x-sendfile mechanisme.
    if (link.code === 204) {
