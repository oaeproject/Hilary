                ...
                RestAPI.Content.createLink(contexts['nicolaas'].restContext, 'Test Content 1', 'Test content description 1', 'public', 'http://www.oaeproject.org/', [], [], function(err, contentObj) {
                    ...
                    RestAPI.Content.updateMembers(contexts['nicolaas'].restContext, contentObj.id, permissions, function(err) {
                        ...
                        RestAPI.Content.getMembers(contexts['nicolaas'].restContext, contentObj.id, null, null, function(err, members) {
                            ...
                            // Ensure the user roles are all the same
                            var hash = _.indexBy(members.results, function(member) {
                                return member.profile.id;
                            });
                            assert.equal(hash[contexts['nicolaas'].user.id].role, 'manager');
                            assert.equal(hash[contexts['simon'].user.id].role, 'manager');
                            assert.equal(hash[contexts['stuart'].user.id].role, 'viewer');
                            assert.equal(hash[contexts['ian'].user.id].role, 'viewer');
                            ...
                            RestAPI.Content.updateMembers(contexts['nicolaas'].restContext, contentObj.id, permissions, function(err) {
                                ...
                                RestAPI.Content.getMembers(contexts['nicolaas'].restContext, contentObj.id, null, null, function(err, members) {
                                    ...
                                    assert.equal(hash[contexts['nicolaas'].user.id].role, 'manager');
                                    assert.equal(hash[contexts['ian'].user.id].role, 'viewer');
                                    assert.equal(hash[contexts['anthony'].user.id].role, 'viewer');
                                        ...
                                        RestAPI.Content.getMembers(contexts['nicolaas'].restContext, contentObj.id, null, null, function(err, members) {
                                            ...
                                            RestAPI.Content.getMembers(contexts['nicolaas'].restContext, contentObj.id, null, null, function(err, members) {
                                                ...
                                                RestAPI.Content.updateMembers(contexts['nicolaas'].restContext, contentObj.id, permissions, function(err) {
                                                    ...
                                                    // Verify ian and anthony have been removed
                                                    RestAPI.Content.getMembers(contexts['nicolaas'].restContext, contentObj.id, null, null, function(err, members) {
                                                        assert.ok(!err);
                                                        assert.equal(members.results.length, 1);
                                                        assert.equal(members.results[0].role, 'manager');
