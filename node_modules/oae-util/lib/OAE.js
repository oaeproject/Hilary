...
var MODULES_PATH = "oae-core";

module.exports = function(tenant){}

module.exports.getAvailableModules = function(callback) {
    var modules = {};
    IO.getFileListForFolder(MODULES_PATH, function(coreModules) {
        for (var m = 0; m < nodeModules.length; m++) {
            modules[m] = ""
        }
        IO.getFileListForFolder("node_modules", function(nodeModules) {
            for (var m = 0; m < nodeModules.length; m++) {
...
module.exports.runQuery = function(query, parameters, callback) {
    con.connect(function(connectErr){
        if (connectErr) {
            ...
            console.log("Connected to Cassandra");
            con.execute(query, parameters || [], function(queryErr, data) {
                if (queryErr) {
