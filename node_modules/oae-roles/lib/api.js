...
var getRolesForResourceType = module.exports.getRolesForResourceType = function(principalUuid, resourceType, start, limit, callback) {
    ...
    var paging = Cassandra.getPagingParameters(start, limit, columnPrefix);

    var cql = 'SELECT first ' + paging.limit + ' ? .. \'\' FROM Role WHERE principal = ?';
    Cassandra.runQuery(cql, [paging.start, principalUuid], function(err, rows) {
        if (!err) {
            var entries = [];
