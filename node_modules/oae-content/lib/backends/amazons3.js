 ...
 */
var store = module.exports.store = function(ctx, getStream, callback) {
    var opts = getStream();
    var date = new Date();
    var id = ShortId.generate();
    var uri = util.format('%d/%d/%d/%d/%d/%s/%s', date.getFullYear(), date.getMonth(), date.getDate(), date.getHours(), date.getMinutes(), id, opts.filename);

    log().trace('Streaming %s to S3.', uri);
...
var remove = module.exports.remove = function(ctx, uri, callback) {
    ...
    log().trace('Removing %s from S3.', uri);

    _getClient(ctx).DeleteObject(_getBucketName(ctx), uri, function(err) {
        if (err) {
