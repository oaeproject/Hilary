...
var getAggregatedEntities = module.exports.getAggregatedEntities = function(aggregateKeys, callback) {
    ...
    var multiGetIdentities = redisClient.multi();
    _.each(aggregateKeys, function(aggregateKey) {
        multiGetIdentities.hgetall(_createAggregateEntityCacheKey(aggregateKey, 'actors'));
        multiGetIdentities.hgetall(_createAggregateEntityCacheKey(aggregateKey, 'objects'));
        multiGetIdentities.hgetall(_createAggregateEntityCacheKey(aggregateKey, 'targets'));
    });

    multiGetIdentities.exec(function(err, results) {
        if (err) {
            return callback(err);
        }
        ...
        _fetchEntitiesByIdentities(_.keys(entityIdentities), function(err, identityEntities) {
            if (err) {
                return callback(err);
            }
            ...
            _.each(results, function(result, i) {
                if (result) {
                    ...
                    _.each(result, function(identity, entityKey) {
                        // Grab the entity from the indentity map that was fetched
                        aggregateEntities[aggregateKey][entityType][entityKey] = identityEntities[identity];
                    });
                }
            });
...
var _fetchEntitiesByIdentities = function(entityIdentities, callback) {
    ...
    redisClient.mget(entityIdentityCacheKeys, function(err, results) {
        ...
        _.each(results, function(entityStr, i) {
            if (entityStr) {
                try {
                    identityEntities[entityIdentities[i]] = JSON.parse(entityStr);
                } catch (ex) {
        ...
        return callback(null, identityEntities);
    });
};
    ...
    _.each(aggregates, function(aggregate, aggregateKey) {
        ...
        if (!_.isEmpty(aggregate.actors)) {
            ...
            _.each(aggregate.actors, function(actor, actorKey) {
                ...
                // Then push the identity reference
                hmsetActorArgs.push(actorKey, identity);

                // Remember this as we will need to persist it separately
                identityEntities[identity] = actor;
            });
        }
        ...
        if (!_.isEmpty(aggregate.objects)) {
            ...
            _.each(aggregate.objects, function(object, objectKey) {
                ...
                // Remember this as we will need to persist it separately
                identityEntities[identity] = object;
            });
        }
        ...
        if (!_.isEmpty(aggregate.targets)) {
            ...
            _.each(aggregate.targets, function(target, targetKey) {
                ...
                // Remember this as we will need to persist it separately
                identityEntities[identity] = target;
            });
        }
        ...
        // Now set the actual entity values
        var msetArgs = [];
        var toExpire = [];
        _.each(identityEntities, function(entity, identity) {
        ...
        log().trace({'msetArgs': msetArgs}, 'Persisting entities in reids by identity.');

        multi.mset(msetArgs);

        // We need to reset the expiry of each identity-entity pair to the aggregatedMaxExpiry to ensure that the entities
        // live as long as any aggregate referencing it needs it for aggregation
...
/**
 * Given an entity, create an identity hash that represents its unique contents.
 */
var _createEntityIdentity = function(entity, entityKey) {
    var md5sum = crypto.createHash('md5');

    // Simply using JSON.stringify is not perfect as object properties could technically be in a different order. Assuming
    // most objects are built in the same way this should be fine, however it could be better to do a "normalized" hash
