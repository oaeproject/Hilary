...
PrincipalsAPI.on(PrincipalsConstants.events.CREATED_GROUP, function(ctx, group, members) {
    ...
    _fireMembershipUpdateTasks(_.keys(members));
});
...
var _handleUpdateGroupMembers = function(ctx, group, principalIds) {
    ...
    // Fire additional tasks to update the memberships of the members
    _fireMembershipUpdateTasks(principalIds);
};

/**
 * Given a list of ids of principals whose membership has changed, fire the index tasks required to update the appropriate
 * membership documents.
 *
 * @param  {String[]}  principalIds    The list of principals whose membership in a group changed.
 * @api private
 */
var _fireMembershipUpdateTasks = function(principalIds) {
    if (_.isEmpty(principalIds)) {
        return;
    }

    // Fire iupdates to the resource_memberships for each principal
    TaskQueue.submit(SearchConstants.mq.TASK_INDEX_DOCUMENT, {
        'resources': _.map(principalIds, function(principalId) {
            return {
                'id': principalId,
                'index': {
                    'children': {
...
var _produceUserSearchDocument = function(user) {
    // Full text searches are done on the displayName and public alias of the user. We also sort by this content
    var fullText = _.compact([user.displayName, user.publicAlias]).join(' ');
    var searchDoc = {
...
var _transformGroupDocuments = function(ctx, docs, callback) {
    ...
    for (var i = 0; i < docIds.length; i++) {
        ...
        if (doc.fields.thumbnailUrl) {
            result.thumbnailUrl = PrincipalsUtil.getSignedPictureUrl(ctx, doc.fields.thumbnailUrl);
        }
        ...
        result.profilePath = '/group/' + result.tenant.alias + '/' + AuthzUtil.getResourceFromId(result.id).resourceId;
...
SearchAPI.registerReindexAllHandler('principal', function(callback) {
    ...
    var _onEach = function(principalRows, done) {
        ...
        if (!_.isEmpty(userResources)) {
            TaskQueue.submit(SearchConstants.mq.TASK_INDEX_DOCUMENT, {'resources': userResources});
        }

        if (!_.isEmpty(groupResources)) {
            TaskQueue.submit(SearchConstants.mq.TASK_INDEX_DOCUMENT, {'resources': groupResources});
        }
