...
describe('Users', function() {
    ...
    var verifyCropping = function(ctx, principalId, selectedArea, expectedHttpCode, callback) {
        RestAPI.Crop.cropPicture(ctx, principalId, selectedArea, function(err, data) {
            ...
            verifySize(data.smallPictureUri, 32, 32, function() {
                verifySize(data.mediumPictureUri, 128, 128, function() {
                    callback();
                });
            });
        });
        ...
        it('verify downloading picture', function(callback) {
            createUser(function(ctx) {
                ...
                RestAPI.User.uploadPicture(ctx, ctx.user.id, getPictureStream, selectedArea, function(err) {
                    ...
                    RestAPI.User.downloadPicture(ctx, ctx.user.id, 'small', function(err, body, request) {
                        assert.ok(!err);
                        assert.ok(request.statusCode, 204);
                        RestAPI.User.downloadPicture(ctx, ctx.user.id, 'medium', function(err, body, request) {
                            assert.ok(!err);
                            assert.ok(request.statusCode, 204);
                            RestAPI.User.downloadPicture(ctx, ctx.user.id, 'large', function(err, body, request) {
                                assert.ok(!err);
                                assert.ok(request.statusCode, 204);
                                callback();
                            });
