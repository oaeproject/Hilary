...
/**
 * Check whether or not the validated string is defined. Will result in
 * an error if the input string is `null` or `undefined`. However other
 * falsey values like `false` and `''` will not trigger a validation
 * error.
 *
 * Usage:
 * ```
 * var validator = new Validator();
 * validator.check(str, error).isDefined();
 * ```
 */
Validator.prototype.isDefined = function() {
    var isDefined = (this.str !== null && this.str !== undefined);
    if (!isDefined) {
...
/**
 * Check whether or not the provided value is strictly `true`.
 *
 * Usage:
 * ```
 * var validator = new Validator();
 * validator.check(null, error).isTrue(val);
 * ```
 *
 * @param  {Boolean}    val     Values that needs to be checked for validity
 */
Validator.prototype.isTrue = function(val) {
    if (val !== true) {
        this.error(this.msg || 'A non-true value has been passed in');
    }
    return this;
};
