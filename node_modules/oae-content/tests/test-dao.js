...
describe('Content DAO', function() {
  ...
  var getStream = function() {
    return fs.createReadStream(__dirname + '/data/apereo.jpg');
  };
      ...
      RestAPI.Content.createLink(
        ...
        function(err, link) {
          ...
          ContentDAO.Content.iterateAll(null, 100, _onEach, function(err) {
            ...
            ContentDAO.Content.iterateAll(['contentId', 'displayName'], 100, _onEach, function(
              err
            ) {
              assert.ok(!err, JSON.stringify(err, null, 4));
              assert.ok(foundLink, 'Expected to find the link we just created');
   ...
   */
  it('verify RevisionDAO getAllRevisionsForContent functionality', function(callback) {
    TestsUtil.generateTestUsers(camAdminRestContext, 1, function(err, users) {
      assert.ok(!err);
      ...
      RestAPI.Content.createFile(
        ...
        function(err, contentObj) {
          ...
          RestAPI.Content.updateFileBody(mrvisser.restContext, contentObj.id, getStream, function(
            err
          ) {
            assert.ok(!err);
            RestAPI.Content.updateFileBody(mrvisser.restContext, contentObj.id, getStream, function(
                ...
                function(err) {
                  ...
                  RestAPI.Content.updateFileBody(
                    ...
                    function(err) {
                      ...
                      ContentDAO.Revisions.getAllRevisionsForContent([contentObj.id], function(
                        err,
                        data
                      ) {
                        assert.ok(!err);
