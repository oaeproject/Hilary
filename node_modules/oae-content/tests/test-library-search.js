...
describe('Library Search', function() {
                    ...
                    RestAPI.User.createUser(camAdminRestContext, jackUsername, 'password', 'Jack McJackerson', null, function(err, jack) {
                        ...
                        RestAPI.User.createUser(camAdminRestContext, janeUsername, 'password', 'Jane McJaneFace', null, function(err, jane) {
                            ...
                            RestAPI.Content.createLink(doerRestContext, 'Apereo Website', 'The website of the Apereo Foundation', 'public', 'http://www.apereofoundation.org', [], [], function(err, content) {
                                ...
                                RestAPI.Content.shareContent(doerRestContext, content.contentId, [jack.id], function(err) {
                                    assert.ok(!err);

                                    // Verify anonymous can see the content item
                                    SearchTestsUtil.searchAll(anonymousRestContext, 'library', [jack.id], null, function(err, results) {
                                        assert.ok(!err);
                                        assert.equal(results.total, 1);
                                        assert.equal(results.results[0].id, content.contentId);

                                        // Verify tenant admin can see the content item
                                        SearchTestsUtil.searchAll(camAdminRestContext, 'library', [jack.id], {'_refresh': false}, function(err, results) {
                                            assert.ok(!err);
                                            assert.equal(results.total, 1);
                                            assert.equal(results.results[0].id, content.contentId);

                                            // Verify the target user can see the content item
                                            SearchTestsUtil.searchAll(jackRestContext, 'library', [jack.id], {'_refresh': false}, function(err, results) {
                                                assert.ok(!err);
                                                assert.equal(results.total, 1);
                                                assert.equal(results.results[0].id, content.contentId);

                                                // Verify a different loggedin user can see the content item
                                                SearchTestsUtil.searchAll(janeRestContext, 'library', [jack.id], {'_refresh': false}, function(err, results) {
                                                    assert.ok(!err);
                                                    assert.equal(results.total, 1);
                                                    assert.equal(results.results[0].id, content.contentId);

                                                    // Verify the cross-tenant user can see the content item
                                                    SearchTestsUtil.searchAll(darthVaderRestContext, 'library', [jack.id], {'_refresh': false}, function(err, results) {
                                                        assert.ok(!err);
                                                        assert.equal(results.total, 1);
                                                        assert.equal(results.results[0].id, content.contentId);
                                                        callback();
        ...
        it('verify all users see public group library items', function(callback) {
            ...
            RestAPI.User.createUser(camAdminRestContext, doerUsername, 'password', 'Doer', null, function(err, doer) {
                ...
                RestAPI.User.createUser(gtAdminRestContext, darthVaderUsername, 'password', 'Darth Vader', null, function(err, darthVader) {
                    ...
                    RestAPI.Group.createGroup(doerRestContext, groupAlias, groupAlias, groupAlias, 'public', 'no', [], [], function(err, group) {
                        assert.ok(!err);
                            ...
                            RestAPI.Group.setGroupMembers(doerRestContext, group.id, membership, function(err) {
                                ...
                                RestAPI.User.createUser(camAdminRestContext, janeUsername, 'password', 'Jane McJaneFace', null, function(err, jane) {
                                    ...
                                    RestAPI.Content.createLink(doerRestContext, 'Apereo Website', 'The website of the Apereo Foundation', 'public', 'http://www.apereofoundation.org', [], [], function(err, content) {
                                        ...
                                        RestAPI.Content.shareContent(doerRestContext, content.contentId, [group.id], function(err) {
                                            assert.ok(!err);

                                            // Verify anonymous can see it
                                            SearchTestsUtil.searchAll(anonymousRestContext, 'library', [group.id], null, function(err, results) {
                                                assert.ok(!err);
                                                assert.equal(results.total, 1);
                                                assert.equal(results.results[0].id, content.contentId);

                                                // Verify tenant admin can see it
                                                SearchTestsUtil.searchAll(camAdminRestContext, 'library', [group.id], {'_refresh': false}, function(err, results) {
                                                    assert.ok(!err);
                                                    assert.equal(results.total, 1);
                                                    assert.equal(results.results[0].id, content.contentId);

                                                    // Verify a member can see it
                                                    SearchTestsUtil.searchAll(jackRestContext, 'library', [group.id], {'_refresh': false}, function(err, results) {
                                                        assert.ok(!err);
                                                        assert.equal(results.total, 1);
                                                        assert.equal(results.results[0].id, content.contentId);

                                                        // Verify a loggedin non-member can see it
                                                        SearchTestsUtil.searchAll(janeRestContext, 'library', [group.id], {'_refresh': false}, function(err, results) {
                                                            assert.ok(!err);
                                                            assert.equal(results.total, 1);
                                                            assert.equal(results.results[0].id, content.contentId);

                                                            // Verify a cross-tenant user can see it
                                                            SearchTestsUtil.searchAll(darthVaderRestContext, 'library', [group.id], {'_refresh': false}, function(err, results) {
                                                                assert.ok(!err);
                                                                assert.equal(results.total, 1);
                                                                assert.equal(results.results[0].id, content.contentId);
                                                                callback();
            ...
            RestAPI.User.createUser(camAdminRestContext, doerUsername, 'password', 'Doer', null, function(err, doer) {
                ...
                RestAPI.User.createUser(gtAdminRestContext, darthVaderUsername, 'password', 'Darth Vader', null, function(err, darthVader) {
                    ...
                    RestAPI.User.createUser(gtAdminRestContext, sithUsername, 'password', 'Sithy McSithypants', null, function(err, sith) {
                        ...
                        RestAPI.Group.createGroup(doerRestContext, groupAlias, groupAlias, groupAlias, 'public', 'no', [], [sith.id], function(err, group) {
                            assert.ok(!err);
                                                ...
                                                SearchTestsUtil.searchAll(anonymousRestContext, 'library', [group.id], null, function(err, results) {
                                                    ...
                                                    SearchTestsUtil.searchAll(camAdminRestContext, 'library', [group.id], {'_refresh': false}, function(err, results) {
                                                        ...
                                                        SearchTestsUtil.searchAll(jackRestContext, 'library', [group.id], {'_refresh': false}, function(err, results) {
                                                            ...
                                                            SearchTestsUtil.searchAll(sithRestContext, 'library', [group.id], {'_refresh': false}, function(err, results) {
                                                                assert.ok(!err);
                                                                assert.equal(results.total, 1);
                                                                assert.equal(results.results[0].id, content.contentId);
    ...
    describe('Library Paging', function() {
            ...
            RestAPI.User.createUser(camAdminRestContext, doerUsername, 'password', 'Doer', null, function(err, doer) {
                ...
                RestAPI.Content.createLink(doerRestContext, 'Apereo Website', 'The website of the Apereo Foundation', 'public', 'http://www.apereofoundation.org', [], [], function(err, content) {
                    ...
                    RestAPI.Content.createLink(doerRestContext, 'Google Website', 'Google', 'public', 'http://www.google.ca', [], [], function(err, content) {
                        ...
                        SearchTestsUtil.searchRefreshed(doerRestContext, 'library', [doer.id], {'limit': 2, 'start': 0}, function(err, results) {
                            assert.ok(!err);
                            ...
                            RestAPI.Search.search(doerRestContext, 'library', [doer.id], {'limit': 1, 'start': 0}, function(err, results) {
                                assert.ok(!err);
                                assert.ok(results.results);
                                ...
                                RestAPI.Search.search(doerRestContext, 'library', [doer.id], {'limit': 1, 'start': 1}, function(err, results) {
                                    assert.ok(!err);
                                    assert.ok(results.results);
                                    assert.equal(results.results.length, 1);
