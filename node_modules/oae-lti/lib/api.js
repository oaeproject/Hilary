...
var LtiToolLaunchParams = require('oae-lti/lib/model').ToolLaunchParams;
var LtiLaunchParams = require('oae-lti/lib/model').LtiLaunchParams;
 ...
 */
var getLtiTool = module.exports.getLtiTool = function(ctx, id, groupId, callback) {
    PrincipalsApi.getFullGroupProfile(ctx, groupId, function(err, group) {
        if (err) {
            return callback(err);
        } else if (!group.isManager && !group.isMember) {
        ...
        PrincipalsApi.getMe(ctx, function(err, principal) {
            ...
            VersionAPI.getVersion(function(err, version) {
                ...
                LtiDAO.getLtiTool(id, groupId, function(err, tool) {
                    if (err) {
                        log().error({
                            'err': err
                    ...
                    var launchParams = {
                        ...
                        resource_link_id: tool.id, // unique key for LTI producer launch
                    ...
                    */
                    ...
                    var launchParams = new LtiLaunchParams(tool, version, group, groupId, principal);
 ...
 */
var addLtiTool = module.exports.addLtiTool = function(ctx, groupId, launchUrl, secret, consumerKey, opts, callback) {
    ...
    PrincipalsApi.getGroup(ctx, groupId, function(err, group) {
        ...
        PrincipalsApi.getMe(ctx, function(err, me) {
            if (err) {
                return callback(err);
            } else if (!me.isTenantAdmin && !me.isGlobalAdmin) {
                return callback({'code': 401, 'msg': 'The current user is not authorized to create an LTI tool'});
            }
            ...
            AuthzPermissions.canManage(ctx, group, function(err) {
                ...
                var id = AuthzUtil.toId('lti', group.tenant.alias, ShortId.generate());

                LtiDAO.createLtiTool(id, groupId, launchUrl, secret, consumerKey, opts.displayName, opts.description, function(err, tool) {
                    if (err) {
                        log().error({
                            'err': err,
                            'groupId': groupId,
 ...
 * @param  {Object}      callback.err        An error that occurred, if any
 */
var deleteLtiTool = module.exports.deleteLtiTool = function(ctx, id, groupId, callback) {
    // Ensure the target group exists and has not been deleted
    PrincipalsApi.getGroup(ctx, groupId, function(err, group) {
        if (err) {
            return callback(err);
        ...
        AuthzPermissions.canManage(ctx, group, function(err) {
            ...
            LtiDAO.deleteLtiTool(id, groupId, function(err) {
                if (err) {
                    log().error({
                        'err': err
