...
var generatePreviews = module.exports.generatePreviews = function(ctx, contentObj, callback) {
    ...
    ctx.download(function(err, path) {
        ...
        _fixOrientation(path, fixedPath, function(err) {
            if (err) {
                return callback(err);
            }
...
/**
 * Fixes the orientation for an input image and moves it to the specified output path.
 *
 * @param  {String}     inputPath       The path where the image that needs fixing is stored.
 * @param  {String}     outputPath      The path where the fixed image should be stored.
 * @param  {Function}   callback        Standard callback method.
 * @param  {Object}     callback.err    Standard error object (if any).
 * @api private
 */
var _fixOrientation = function(inputPath, outputPath, callback) {
    // Fix up the source image's orientation before we start doing any
    // calculations based on the width or height.
    ImageUtil.autoOrient(inputPath, function(err, fixedFile) {
        if (err) {
            return callback({'code': 500, 'msg': 'Could not auto orient based on EXIF data.'});
        }

        IO.moveFile(fixedFile.path, outputPath, callback);
    });
};
