...
var sendEmail = module.exports.sendEmail = function(templateModule, templateId, toUser, data, opts, callback) {
    ...
    var emailInfo = {
        'from': EmailConfig.getValue(toUser.tenant, 'general', 'from'),
        'to': toUser.email,
        'subject': renderedTemplates['meta.json'].subject,
        'generateTextFromHtml': true
    };

    // The 'from' property can be overridden by the meta.json, then we further override that with some hard values
    emailInfo = _.extend({'from': EmailConfig.getValue(toUser.tenant, 'general', 'from')}, renderedTemplates['meta.json'], {
        'to': toUser.email,
        'generateTextFromHtml': true
    });
...
var _compileTemplatesForTemplateIds = function(basedir, module, locale, templateIds, compiledTemplates, callback) {
    ...
    log().info({'module': module, 'locale': locale, 'templateId': templateId}, 'Compiling e-mail template');
