...
var remove = module.exports.remove = function(keys, resourceId, lastModified, callback) {
    ...
    _.each(visibilityMasks, function(mask, bucketName) {
        _.each(keys, function(key) {
            queries.push({
                'query': 'DELETE ? FROM IndexVisibilityMask WHERE bucketKey = ?',
                'parameters': [valueKey, _createBucketKey(key, bucketName)]
            });
        });
    });
...
var list = module.exports.list = function(key, visibility, start, limit, callback) {
    ...
    Cassandra.runPagedColumnQuery('IndexVisibilityMask', 'bucketKey', bucketKey, start, limit, {'reversed': true}, function(err, row) {
        if (err) {
            return callback(err);
        }
