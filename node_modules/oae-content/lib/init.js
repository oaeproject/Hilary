...
var Ethercalc  = require('./internal/ethercalc');
    ...
    ContentSearch.init(function(err) {
        ...
        mkdirp(config.files.uploadDir, function(err) {
            ...
            LocalStorage.init(config.files.localStorageDirectory, function(err) {
                ...
                TaskQueue.bind(ContentConstants.queue.ETHERPAD_PUBLISH, ContentAPI.handlePublish, null, function(err) {
                    if (err) {
                        return callback(err);
                    }
                    // Same for Ethercalc - no ack because ack breaks Ethercalc
                    TaskQueue.bind(ContentConstants.queue.ETHERCALC_PUBLISH, ContentAPI.ethercalcPublish, {subscribe: {ack: false}}, function(err) {
                        if (err) {
                            return callback(err);
                        }
                        TaskQueue.bind(ContentConstants.queue.ETHERCALC_EDIT, Ethercalc.setEdited, {subscribe: {ack: false}}, function(err) {
                            if (err) {
                                return callback(err);
                            }
...
var ensureSchema = function(callback) {
    Cassandra.createColumnFamilies({
        ...
        'Revisions': 'CREATE TABLE "Revisions" ("revisionId" text PRIMARY KEY, "contentId" text, "created" text, "createdBy" text, "filename" text, "mime" text, "size" text, "uri" text, "previewsId" text, "previews" text, "status" text, "largeUri" text, "mediumUri" text, "smallUri" text, "thumbnailUri" text, "wideUri" text, "etherpadHtml" text, "ethercalcHtml" text)',
