 ...
 * @param  {Boolean}        includePadHtml      Should etherpad content html be returned with each revision.
 ...
 */
var getRevisions = module.exports.getRevisions = function(contentId, start, limit, includePadHtml, callback) {
    ...
    Cassandra.runPagedColumnQuery('RevisionByContent', 'contentId', contentId, start, limit, {'reversed': true}, function(err, row) {
        ...
        getMultipleRevisions(revisionsToRetrieve, includePadHtml, callback);
    });
};
 ...
 * @param  {Revision[]}     callback.revisions  Array that contains an object for each revision.
 */
var getMultipleRevisions = module.exports.getMultipleRevisions = function(revisionIds, includePadHtml, callback) {
    var columns = '*';
    if (! includePadHtml) {
        // all columns except etherpadHtml
        columns = 'contentId,created,createdBy,downloadLink,filename,mediumUri,mediumUrl,mime,previews,previewsId,revisionId,size,status,thumbnailUri,thumbnailUrl,uri,wideUri';
    }
