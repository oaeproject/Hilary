...
describe('Profile pictures', function() {
    ...
    var verifySize = function(uri, width, height, callback) {
        ...
        assert.strictEqual(Path.extname(path), '.jpg');
     ...
     * @param  {Function}           callback            Standard callback function
     */
    var verifyCropping = function(restCtx, principal, selectedArea, expectedHttpCode, callback) {
            ...
            verifySize(data.picture.smallUri, PrincipalsConstants.picture.size.SMALL, PrincipalsConstants.picture.size.SMALL, function() {
                verifySize(data.picture.mediumUri, PrincipalsConstants.picture.size.MEDIUM, PrincipalsConstants.picture.size.MEDIUM, function() {
                    ...
                    if (PrincipalsUtil.isGroup(principal.id)) {
                        ...
                        assert.ok(!data.isMember);
                        assert.ok(!data.isManager);
                        assert.ok(!data.joinable);
                    }
                    callback();
                });
            });
    ...
    /*!
     * Test that verifies that pictures which are larger than 10 MB get rejected.
     */
    it('verify a picture cannot be more than 10 MB', function(callback) {
        createUser(function(ctx) {
            RestAPI.User.uploadPicture(ctx, ctx.user.id, TestsUtil.createFakeFileReadableStream('pic.png', 10 * 1024 * 1024 + 1), null, function(err) {
                assert.equal(err.code, 400);
                callback();
            });
        });
    ...
    it('verify cropping fails if the user hasn\'t uploaded a picture yet', function(callback) {
        createUser(function(ctx) {
            verifyCropping(ctx, ctx.user, createSelectedArea(10, 10, 200), 400, callback);
        });
    });
    ...
    it('verify cropping validation negative coordinates', function(callback) {
        createUser(function(ctx) {
            RestAPI.User.uploadPicture(ctx, ctx.user.id, getPictureStream, null, function(err) {
                assert.ok(!err);
                verifyCropping(ctx, ctx.user, createSelectedArea(-10, 10, 200), 400, function() {
                    verifyCropping(ctx, ctx.user, createSelectedArea(10, -10, 200), 400, function() {
                        verifyCropping(ctx, ctx.user, createSelectedArea(-10, -10, 200), 400, function() {
                            verifyCropping(ctx, ctx.user, createSelectedArea(10, 10, -200), 400, callback);
                        });
    ...
    it('verify cropping fails if x or y coord is outside of image', function(callback) {
        createUser(function(ctx) {
            RestAPI.User.uploadPicture(ctx, ctx.user.id, getPictureStream, null, function(err) {
                assert.ok(!err);
                verifyCropping(ctx, ctx.user, createSelectedArea(20000, 10, 100), 400, function() {
                    verifyCropping(ctx, ctx.user, createSelectedArea(10, 200000, 100), 400, callback);
                });
            });
        });
    });
    ...
    /*!
     * Verify that you can download a group picture.
     */
    it('verify downloading group picture', function(callback) {
        createUser(function(ctx) {
            TestsUtil.generateTestGroups(ctx, 2, function(err, groups) {
                ...
                RestAPI.Group.uploadPicture(ctx, groupA, getPictureStream, selectedArea, function(err) {
                    assert.ok(!err);
        ...
        createUsers(function(contexts) {
            TestsUtil.generateTestGroups(contexts['simon'].restContext, 1, function(err, groups) {
                ...
                RestAPI.Group.uploadPicture(contexts['nicolaas'].restContext, group.id, getPictureStream, null, function(err) {
                    assert.equal(err.code, 401);
                    verifyCropping(contexts['nicolaas'].restContext, group, createSelectedArea(10, 10, 200), 401, function() {
                            ...
                            RestAPI.Group.uploadPicture(contexts['nicolaas'].restContext, group.id, getPictureStream, null, function(err) {
                                ...
                                verifyCropping(contexts['nicolaas'].restContext, group, createSelectedArea(10, 10, 200), 401, function() {
                                    ...
                                    RestAPI.Group.setGroupMembers(contexts['simon'].restContext, group.id, members, function(err) {
                                        ...
                                        RestAPI.Group.uploadPicture(contexts['nicolaas'].restContext, group.id, getPictureStream, null, function(err) {
                                            assert.ok(!err);
                                            verifyCropping(contexts['nicolaas'].restContext, group, createSelectedArea(10, 10, 200), 200, callback);
                                        });
                                    });
                                });
                            });
