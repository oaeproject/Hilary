            ...
            PrincipalsDAO.getPrincipal(userId, function(err, user) {
                ...
                ActivityTransformer.transformActivities(ctx, activitiesToEmail,  ActivityConstants.transformerTypes.ACTIVITYSTREAMS, function(err) {
                    ...
                    _.each(activitiesToEmail, function(activity) {
                        ...
                        // Generate a unique id for this mail so we don't accidentally send it out twice
                        // We cannot use the activityId as each activity gets a new ID when routed and/or aggregated
                        var emailId = util.format('%s:%s:%s', user.id, activity['oae:activityType'], activity.published);

                        // Finally send the email
                        EmailAPI.sendEmail(templateModule, templateId, user, {'activity': activity}, {'id': emailId}, function(err) {
                            if (err) {
