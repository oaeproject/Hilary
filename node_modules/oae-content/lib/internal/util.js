 ...
 * @param  {String}     content           The content object that needs a signed S3 video URL
 * @api private
 */
var getSignedVideoUrl = module.exports.getSignedVideoUrl = function(ctx, content) {
    if (!content.previews.videoKey) {
        return callback();
    }

    var key = content.previews.videoKey;
    ...
    s3.getSignedUtl('getObject', params, function(err, url) {
        if (err) {
            return callback(err);
        }
