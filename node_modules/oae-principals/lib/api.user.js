 ...
 */
var getOrCreateGuestUser = module.exports.getOrCreateGuestUser = function(ctx, email, callback) {
...
/**
 * Given a set of strings, create (or retrieve) a guest account for the strings that
 * are email addresses. If the string is not an email address, it's expected to be
 * a user or group identifier. If this is not the case, a validation error will be
 * returned.
 *
 * @param  {Context}        ctx                         Standard context object containing the current user and the current tenant
 ...
 * @param  {Function}       callback                    Standard callback function
 * @param  {Object}         callback.err                An error that occurred, if any
 * @param  {Object}         callback.principalIds       Object mapping each principal of `principals` to a `principalId`
 */
var getOrCreateGuestUsers = module.exports.getOrCreateGuestUsers = function(ctx, principals, callback) {
    ...
    return _createGuestUsers(ctx, principals, callback);
};

/**
 * Given a set of strings, create (or retrieve) a guest account for the strings that
 * are email addresses. If the string is not an email address, it's expected to be
 * a user or group identifier and returned as is.
 *
 * @param  {Context}        ctx                         Standard context object containing the current user and the current tenant
 * @param  {String[]}       principals                  An array of principal ids to retrieve mixed with email address for which guest accounts should be created
 * @param  {Function}       callback                    Standard callback function
 ...
 * @api private
 */
var _createGuestUsers = function(ctx, principals, callback, _principalIds) {
    _principalIds = _principalIds || {};
    if (principals.length === 0) {
        return callback(null, _principalIds);
    }

    var principal = principals.pop();
    // If the `principal` is a user or group id, we can move on to the next one
    if (AuthzUtil.isUserId(principal) || AuthzUtil.isGroupId(principal)) {
        _principalIds[principal] = principal;
        return _createGuestUsers(ctx, principals, callback, _principalIds);
    }

    // The `principal` is an email
    getOrCreateGuestUser(ctx, principal, function(err, user) {
        if (err) {
            return callback(err);
        }

        _principalIds[principal] = user.id;
        _createGuestUsers(ctx, principals, callback, _principalIds);
    });
};

/**
 * Verify a guest token and associate a local login strategy with the user matching the token. A guest
 * token is sent to a user's email address when he's invited by another user. It allows us to map
 *
 * @param  {Context}    ctx                         Standard context object containing the current user and the current tenant
...
var verifyGuestToken = module.exports.verifyGuestToken = function(ctx, token, username, password, callback) {
    ...
    PrincipalsDAO.getUserByEmailToken(token, function(err, user) {
        ...
        require('oae-authentication').associateLoginId(userCtx, loginId, user.id, function(err) {
            ...
            PrincipalsDAO.validateEmailAddress(token, user, callback);
        });
    });
};
