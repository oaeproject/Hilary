...
FoldersAPI.on(FoldersConstants.events.CREATED_FOLDER, function(ctx, folder, members) {
    ...
    var extraMembers = _.chain(members)
        .keys()
...
ActivityAPI.registerActivityType(FoldersConstants.activity.ACTIVITY_FOLDER_COMMENT, {
    ...
    'streams': {
        'activity': {
            'router': {
                'actor': ['self'],
                'target': ['message-contributors', 'members', 'self']
            }
        },
        'notification': {
            'router': {
...
FoldersAPI.on(FoldersConstants.events.CREATED_MESSAGE, function(ctx, message, folder) {
    var millis = Date.now();
    var actorResource = new ActivityModel.ActivitySeedResource('user', ctx.user().id, {'user': ctx.user()});
    var objectResource = new ActivityModel.ActivitySeedResource('folder-message', message.id, {'folderId': folder.id, 'message': message});
    var targetResource = new ActivityModel.ActivitySeedResource('folder', folder.id, {'folder': folder});
 ...
 * @see ActivityAPI#registerActivityEntityType
 */
var _folderMessageProducer = function(resource, callback) {
    ...
    FoldersDAO.getFolder(folderId, function(err, folder) {
        ...
        MessageBoxUtil.createPersistentMessageActivityEntity(message, function(err, entity) {
            ...
            entity.objectType = 'folder-message';
 ...
 * @see ActivityAPI#registerActivityEntityType
 */
var _folderMessageTransformer = function(ctx, activityEntities, callback) {
    var transformedActivityEntities = {};
 ...
 * @see ActivityAPI#registerActivityEntityType
 */
var _folderMessageInternalTransformer = function(ctx, activityEntities, callback) {
    var transformedActivityEntities = {};
    _.keys(activityEntities).forEach(function(activityId) {
        transformedActivityEntities[activityId] = transformedActivityEntities[activityId] || {};
        _.keys(activityEntities[activityId]).forEach(function(entityId) {
...
ActivityAPI.registerActivityEntityType('folder-message', {
    'producer': _folderMessageProducer,
    'transformer': {
        'activitystreams': _folderMessageTransformer,
        'internal': _folderMessageInternalTransformer
    },
    'propagation': function(associationsCtx, entity, callback) {
...
/*!
 * Register an association that presents the folder for a folder-message entity
 */
ActivityAPI.registerActivityEntityAssociation('folder-message', 'self', function(associationsCtx, entity, callback) {
