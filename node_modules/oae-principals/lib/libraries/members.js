...
var _updatePrincipalInMembersLibraries = function(ctx, updatedPrincipal, oldPrincipal) {
    ...
    AuthzAPI.getAllRolesForPrincipalAndResourceType(updatedPrincipal.id, AuthzConstants.resourceTypes.GROUP, function(err, allRoles) {
        ...
        var entries = _.map(allRoles, function(groupRole) {
            return {
                'id': groupRole.id,
                'oldRank': _getMembersLibraryRank(oldPrincipal, groupRole.role),
                'newRank': _getMembersLibraryRank(updatedPrincipal, groupRole.role),
...
var _getMembersLibraryRank = function(principal, role) {
    var rank = 0;
    if (role === PrincipalsConstants.roles.MANAGER && _hasProfilePicture(principal)) {
        // Managers with profile pictures appear first
        rank = 3;
    } else if (_hasProfilePicture(principal)) {
