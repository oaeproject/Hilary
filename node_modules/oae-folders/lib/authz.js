 ...
 *
 * Because this function pulls the data straight from the Authz tables, there
 * is no way to differentiate between groups and folders. Therefor any group
 * that is a member of the content item will be returned as well. You will have
 * to query the `Folders` table to get just the folders
 *
...
var getFoldersForContent = module.exports.getFoldersForContent = function(contentId, callback) {
    AuthzAPI.getAuthzMembers(contentId, null, 10000, function(err, members) {
        ...
        return callback(null, authzPrincipalsToFolders(members));
    });
};
...
var canPostFolderMessage = module.exports.canPostFolderMessage = function(ctx, folder, callback) {
    ...
    // Try to determine the role of the current user on the folder implicitly
    AuthzAPI.resolveImplicitRole(ctx, folder.groupId, folder.tenant.alias, folder.visibility, FoldersConstants.roles.ALL_PRIORITY, function(err, implicitRole, canInteract) {
        if (err) {
            return callback(err);
        } else if (implicitRole === FoldersConstants.roles.MANAGER) {
...
/**
 * Determine whether or not the user in context can delete the provided message to the provided folder.
 *
 * @param  {Context}        ctx                     Standard context object containing the current user and the current tenant
 * @param  {Folder}         folder                  The folder to test for access
...
/**
 * Filter and map a set of authz principals to a set of light-weight folders. Keep
 * in mind that this function might map real groups to light-weight folders. Other than
 * checking if a folder exist in the database there is no way to determine whether an
 * authz principal is a group or a folder. In order to keep this operation light-weight
 * it has been decided to return real groups as well.
 *
 ...
 */
var authzPrincipalsToFolders = module.exports.authzPrincipalsToFolders = function(principals) {
