...
var job = module.exports.job = function(config, tenantAlias, path, contentId, revisionId, isVideo, callback) {
    config = getConfig(config, tenantAlias);
    ...
    var filename = revisionId ? revisionId.replace(/:/g, '_') : contentId.replace(/:/g, '_');
    ...
    var options = {
        ...
        private: true,
        output: {
            notifications: util.format('https://%sapi/zencoder/encodingNotification', tenant.host),
            base_url: baseUrl,
            public: true,
        }
    };
    ...
    log().info('Creating Zencoder job with options %s', options);
