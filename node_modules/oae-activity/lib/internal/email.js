...
ActivityEmitter.on(ActivityConstants.events.ROUTED_ACTIVITIES, function(activitySeed, allActivities) {
    ...
    PrincipalsDAO.getPrincipals(usersToMail, ['principalId', 'tenantAlias', 'email', 'emailPreference'], function(err, users) {
        ...
        _.each(users, function(user) {
            ...
            usersPreferences[user.id] = user.emailPreference;
        });
        ...
        ActivityDAO.queueUsersForEmail(usersPreferences, function(err) {
            ...
            log().info({'usersPreferences': usersPreferences, 'activitySeed': activitySeed}, 'Queued mail for users');
        });
    });
});
...
var collectBuckets = module.exports.collectBuckets = function(bucketNumbers, callback, errs) {
    ...
    _collectBucket(bucketNumber, function(err) {
        if (err) {
            ...
            Telemetry.incr('emailcollection.error.count');
            errs.push(err);
        }
...
var _handleImmediateEmails = function(bucketNumber, limit, callback) {
    _collectMails(bucketNumber, PrincipalsConstants.emailPreferences.IMMEDIATE, null, limit, callback);
};
...
var _handleDailyEmails = function(bucketNumber, limit, callback) {
    ...
    if ((now < collectAt && collectAt < end)) {
        return _collectMails(bucketNumber, PrincipalsConstants.emailPreferences.DAILY, null, limit, callback);
    } else {
...
var _handleWeeklyEmails = function(bucketNumber, limit, callback) {
    if (_isWeeklyCycle()) {
        return _collectMails(bucketNumber, PrincipalsConstants.emailPreferences.WEEKLY, null, limit, callback);
    } else {
        return callback(null, true);
    }
};
 ...
 * @api private
 */
var _collectMails = function(bucketNumber, emailPreference, start, limit, callback) {
    ...
    ActivityDAO.getQueuedUsersForEmail(bucketNumber, emailPreference, start, limit, function(err, users, nextToken) {
        if (err) {
            return callback(err);
        ...
        ActivityDAO.resetAggregationForActivityStreams(activityStreams, function(err) {
            ...
            ActivityDAO.unqueueUsersForEmail(bucketNumber, users, function(err) {
                if (err) {
                    return callback(err);
                }
