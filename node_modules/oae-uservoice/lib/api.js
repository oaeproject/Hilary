 ...
 */
var getUrlInfo = (module.exports.getUrlInfo = function(
  ctx,
  expiresIn,
  callback,
) {
  ...
  var enabled = UservoiceConfig.getValue(
    ctx.tenant().alias,
    'general',
    'enabled',
  );
  var baseUrl = UservoiceConfig.getValue(
    ctx.tenant().alias,
    'general',
    'baseUrl',
  );
  var subdomain = UservoiceConfig.getValue(
    ctx.tenant().alias,
    'general',
    'subdomain',
  );
  var ssoKey = UservoiceConfig.getValue(
    ctx.tenant().alias,
    'general',
    'ssoKey',
  );
  ...
  if (!enabled) {
    return callback({
      code: 400,
      msg: 'The UserVoice integration is not enabled',
    });
  } else if (!subdomain) {
    return callback({
      code: 400,
      msg: 'The UserVoice subdomain is not configured',
    });
  } else if (!baseUrl) {
    return callback({
      code: 400,
      msg: 'The UserVoice base URL is not configured',
    });
  }

  var result = { baseUrl: baseUrl };
 ...
 * @api private
 */
var _generateAuthenticationToken = function(
  user,
  subdomain,
  ssoKey,
  expiresIn,
  ...
  log().trace(
    { subdomain: subdomain, data: data, token: token },
    'Created encrypted token for UserVoice authentication',
  );
