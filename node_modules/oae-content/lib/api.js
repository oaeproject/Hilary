...
var getFullContentProfile = module.exports.getFullContentProfile = function(ctx, contentId, callback) {
    ...
    getContent(ctx, contentId, function(err, contentObj) {
        ...
        PrincipalsUtil.getPrincipal(ctx, contentObj.createdBy, function(err, createdBy) {
            ...
            /*!
             * Adds the `isManager` property to the `contentObj`.
             */
            var _setIsManager = function() {
...
var createLink = module.exports.createLink = function(ctx, displayName, description, visibility, link, additionalMembers, callback) {
    ...
    var contentId = _generateContentId(ctx);
    _createContent(ctx, contentId, 'link', displayName, description, visibility, additionalMembers, {'link': link}, {}, function(err, content, revision) {
        if (err) {
            return callback(err);
        }
...
var _createFile = function(ctx, displayName, description, visibility, additionalMembers, file, callback) {
    ...
    var contentId = _generateContentId(ctx);
    var options = {'resourceId': contentId};
    ...
    getStorageBackend(ctx).store(ctx, file, options, function(err, uri) {
        ...
        _createContent(ctx, contentId, 'file', displayName, description, visibility, additionalMembers, otherValues, revisionData, function(err, contentObj, revision) {
            if (err) {
                return callback(err);
            }
...
var createCollabDoc = module.exports.createCollabDoc = function(ctx, displayName, description, visibility, additionalMembers, callback) {
    ...
    Etherpad.createPad(contentId, function(err, ids) {
        ...
        _createContent(ctx, contentId, 'collabdoc', displayName, description, visibility, additionalMembers, ids, {}, function(err, content, revision) {
            if (err) {
                return callback(err);
            }
    ...
    canManage(ctx, contentId, function(err, canManage, contentObj) {
        ...
        Etherpad.getHTML(contentObj.id, contentObj.etherpadPadId, function(err, html) {
            ...
            _getRevision(ctx, contentId, contentObj.latestRevisionId, function(err, revision) {
                ...
                ContentDAO.Revisions.createRevision(contentId, ctx.user().id, {'etherpadHtml': html}, function(err, revision) {
                    if (err) {
 ...
 * @api private
 */
var _createContent = function(ctx, contentId, resourceSubType, displayName, description, visibility, additionalMembers, otherValues, revisionData, callback) {
    callback = callback || function() {};
    ...
    ContentDAO.Content.createContent(contentId, ctx.user().id, resourceSubType, displayName, description, visibility, additionalMembers, otherValues, revisionData, function(err, contentObj, revision) {
        if (err) {
            return callback(err);
        }
...
var _canShareContent = function(ctx, contentObj, principalIds, callback) {
    ...
    _resolveTenantBoundaryViolations(ctx, contentObj, principalIds, function(err, canShare, illegalPrincipalIds) {
        if (err) {
            return callback(err);
        ...
        _resolveImplicitRole(ctx, contentObj, function(implicitRole, canInteract) {
 ...
 *   * the piece of content and any of the principalIds
 *   * the current user and any of the principalIds
 *
 ...
 * @api private
 */
var _resolveTenantBoundaryViolations = function(ctx, content, principalIds, callback) {
    var user = ctx.user();
    // First test the easy stuff:
    //  * A direct violation between content.tenant and target.tenant
    //  * A direct violation between actor.tenant and target.tenant
    var illegalPrincipalIds = TenantsUtil.resolveTenantBoundaryViolations(user, content.tenant.alias, principalIds);
    if (illegalPrincipalIds.length > 0) {
        return callback(null, false, illegalPrincipalIds);
    }

    // Now, if any of the specified principalIds are part of an external tenant, we'll need to check their visibility.
    var externalPrincipals = _.filter(principalIds, function(principalId) {
        ...
        return targetTenant !== ctx.tenant().alias;
    });
    if (externalPrincipals.length > 0) {
        PrincipalsUtil.getPrincipals(ctx, externalPrincipals, function(err, principals) {
            ...
            var canShare = (nonPublicUsers.length === 0);
            var illegalPrincipalIds = (canShare) ? null : _.map(nonPublicUsers, function(user) { return user.id; });
            callback(null, canShare, illegalPrincipalIds);
        });
    } else {
...
/**
 * Given a user and a piece of content, determine the role the user has.
 * FIXME: This is essentially a copy of DiscussionsAPI.Authz._resolveImplicitRole and should be refactored.
 *
 * @param  {Context}    ctx                     The context of the current request
 * @param  {Content}    content                 The content to test access against
 * @param  {Function}   callback                Invoked when the process completes
 * @param  {Object}     callback.err            An error that occurred, if any
 ...
 */
var _resolveImplicitRole = function(ctx, content, callback) {
    var user = ctx.user();

    // Check admin access
    if (user && (user.isGlobalAdmin() || user.isTenantAdmin(content.tenant.alias))) {
        // The user is an administrator, give them the highest allowable access
        return callback(ContentConstants.roles.MANAGER, true);
    }

    // If we aren't an explicit manager, ability to interact is based on tenant privacy boundaries
    var canInteract = TenantsUtil.canInteract(ctx.tenant().alias, content.tenant.alias);

    // Check implicit member access
    if (content.visibility === ContentConstants.visibility.PUBLIC) {
        // Public resources can be seen by anyone
        return callback(ContentConstants.roles.VIEWER, canInteract);
    } else if (content.visibility === ContentConstants.visibility.LOGGEDIN && TenantsUtil.isLoggedIn(ctx, content.tenant.alias)) {
...
var _updateFileBody = function(ctx, contentId, file, callback) {
    ...
    canManage(ctx, contentId, function(err, isManager, contentObj) {
        ...
        getStorageBackend(ctx).store(ctx, file, options, function(err, uri) {
            ...
            ContentDAO.Revisions.createRevision(contentObj.id, ctx.user().id, opts, function(err, revision) {
                if (err) {
                    return callback(err);
                }
                ...
                ContentDAO.Content.updateContent(contentObj, opts, true, function(err, contentObj) {
                    if (err) {
                        return callback(err);
                    }
            ...
            _.each(revisions, function(revision) {
                if (users[revision.createdBy]) {
                    ...
                    if (revision.thumbnailUrl) {
                        var principalId = (ctx.user() ? ctx.user().id : '');
                        revision.thumbnailUrl = ContentUtil.getDownloadUrlForUri(principalId, ctx.tenant().alias, revision.thumbnailUrl);
                    }
                }
            });
...
var restoreRevision = module.exports.restoreRevision = function(ctx, contentId, revisionId, callback) {
    ...
    canManage(ctx, contentId, function(err, canManage, contentObj) {
        ...
        _getRevision(ctx, contentId, revisionId, function(err, revision) {
            ...
            // Create a new revision, by copying the other one.
            ContentDAO.Revisions.createRevision(contentId, ctx.user().id, revision, function(err, newRevision) {
                if (err) {
                    return callback(err);
                }
                 ...
                 * to the Content CF.
                 */
                var blacklist = ['revisionId', 'contentId', 'createdBy', 'created', 'etherpadHtml'];
                var updates = _.omit(revision, blacklist);

                // We also need to update the latest revisionID in the content CF.
                updates.latestRevisionId = newRevision.revisionId;

                ContentDAO.Content.updateContent(contentObj, updates, true, function(err) {
                    if (err) {
                        return callback(err);
                    }
...
var _augmentContent = function(ctx, contentObj) {
    ...
    if (contentObj.previews && contentObj.previews.thumbnailUri) {
        var principalId = (ctx.user() ? ctx.user().id : null);
        contentObj.previews.thumbnailUrl = ContentUtil.getDownloadUrlForUri(principalId, ctx.tenant().alias, contentObj.previews.thumbnailUri);
    }
};
 ...
 *
 * @param  {Context}    ctx     The context in which this new content ID should be generated. The tenant alias will be incorporated in the content ID.
 * @return {String}             The new content ID.
 * @api private
 */
var _generateContentId = function(ctx) {
