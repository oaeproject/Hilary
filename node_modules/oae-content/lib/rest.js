...
module.exports = function(tenant) {
    ...
    tenant.server.post('/api/content/create', function(req, res) {
        ...
        if (req.body.managers) {
            ...
            req.body.managers.forEach(function(manager) {
                additionalMembers[manager] = 'manager';
            });
        }
        if (req.body.viewers) {
            if (!Array.isArray(req.body.viewers)) {
                req.body.managers = [req.body.viewers];
            }
            req.body.viewers.forEach(function(viewer) {
                additionalMembers[viewer] = 'viewer';
            });
        }

        // Link creation
        if (req.body.contentType === 'link') {
            ContentAPI.createLink(req.ctx, req.body.visibility, req.body.name, req.body.description, req.body.link, function(err, contentObj) {
                if (err) {
        ...
        // File creation
            ...
            ContentAPI.createFile(req.ctx, req.body.visibility, req.body.name, req.body.description, function(err, contentObj) {
                if (err) {
                    return res.send(err.code, err.msg);
                }
                res.send(201, contentObj);
        ...
        // Sakai doc creation
            ...
            ContentAPI.createSakaiDoc(req.ctx, req.body.visibility, req.body.name, req.body.description, function(err, contentObj) {
                if (err) {
                    return res.send(err.code, err.msg);
                }
                res.send(201, contentObj);
