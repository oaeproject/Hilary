...
describe('Preview processor', function() {
  ...
  describe('Processor registration', function() {
    ...
    it('verify that processors can be unregistered', function(callback) {
      ...
      PreviewAPI.registerProcessor('verify-pp-20', {
        testval: 20,
        test: function(ctx, contentObj, callback) {
          callback(null, 20);
        },
      ...
      TestsUtil.generateTestUsers(signedAdminRestContext, 1, function(
        err,
        response,
      ) {
        assert.ok(!err);
        ...
        RestAPI.Content.createFile(
          ...
          function(err, contentObj) {
            ...
            PreviewAPI.getProcessor(mockCtx, contentObj, function(
              err,
              processor,
            ) {
              assert.ok(!err);
    ...
    it('verify parameter validation', function() {
      ...
      assert.throws(
        function() {
          PreviewAPI.registerProcessor('test', { test: function() {} });
        },
        Error,
        'A preview processor needs to specify an object that has a test and generatePreviews method.',
      );
  ...
  /*!
     ...
     */
  var _createContentAndWait = function(
    resourceSubType,
    link,
    stream,
    callback,
        ...
        TestsUtil.generateTestUsers(signedAdminRestContext, 1, function(
          ...
          if (resourceSubType === 'file') {
            ...
            RestAPI.Content.createCollabDoc(
              ...
              function(err, contentObj) {
                assert.ok(!err);
                RestAPI.Content.joinCollabDoc(restCtx, contentObj.id, function(
                  err,
                  data,
                ) {
                  assert.ok(!err);
  ...
  var _createContentAndAddToFolder = function(restContext, folder, callback) {
    ...
    _createContentAndWait('file', null, getImageStream, function(
  ...
  describe('Preview generation', function() {
    ...
    it('verify animated images get converted to single frame images for thumbnail images', function(callback) {
      ...
      _createContentAndWait('file', null, getImageGIFStream, function(
        restCtx,
        content,
      ) {
        assert.equal(content.previews.status, 'done');
    ...
    it('verify office processing works', function(callback) {
      ...
      _createContentAndWait('file', null, getOfficeStream, function(
        restCtx,
        content,
      ) {
        assert.equal(content.previews.status, 'done');
        ...
        _verifySignedUriDownload(
          ...
          function() {
            ...
            _verifySignedUriDownload(
              restCtx,
              content.previews.smallUrl,
              function() {
                assert.ok(content.previews.mediumUrl);
                _verifySignedUriDownload(
                  restCtx,
                  content.previews.mediumUrl,
                  function() {
                    callback();
                  },
                );
    ...
    it('verify pdf processing works', function(callback) {
      ...
      _createContentAndWait('file', null, getPDFStream, function(
        restCtx,
        content,
      ) {
        assert.equal(content.previews.status, 'done');
                  ...
                  function() {
                    ...
                    RestAPI.Content.getPreviewItems(
                      ...
                      function(err, previews) {
                        ...
                        assert.ok(
                          _.find(previews.files, function(file) {
    ...
    it('verify multiple pages pdf processing works', function(callback) {
      ...
      _createContentAndWait('file', null, getMultiplePagesPDFStream, function(
        restCtx,
        content,
      ) {
        assert.equal(content.previews.status, 'done');
              ...
              function(err) {
                ...
                MQTestUtil.whenTasksEmpty(
                  ...
                  function() {
                    ...
                    RestAPI.Content.getContent(restCtx, content.id, function(
                      err,
                      updatedContentObj,
                    ) {
                      assert.ok(!err);
      ...
      _createContentAndWait('file', null, getMultiplePagesPDFStream, function(
        ...
        RestAPI.Content.getRevision(
          ...
          function(err, revision) {
            ...
            RestUtil.RestRequest(restCtx, url, 'GET', params, function(
              err,
              body,
              response,
            ) {
    ...
    it('verify default link processing works', function(callback) {
      ...
      _createContentAndWait('link', 'http://www.google.com', null, function(
        restCtx,
        content,
      ) {
        assert.equal(content.previews.status, 'done');
                ...
                function() {
                  ...
                  _createContentAndWait(
                    ...
                    function(restCtx, content) {
                      ...
                      assert.ok(content.previews.thumbnailUrl);
                      assert.strictEqual(
                        content.previews.thumbnailUrl.indexOf(
                          '/api/download/signed',
                        ),
                        0,
                      );
                              ...
                              _verifySignedUriDownload(
                                ...
                                function() {
                                  ...
                                  _createContentAndWait(
                                    ...
                                    function(restCtx, content) {
                                      assert.equal(
                                        content.previews.status,
                                        'done',
                                      );
                                      assert.equal(
                                        content.previews.embeddable,
                                        false,
                                      );
    ...
    var _mockSlideShare = function(
      ...
      TestsUtil.createTestServer(function(app, server, port) {
        ...
        app.get('/api/2/get_slideshow', function(req, res) {
          ...
          if (returnError) {
            xml +=
              '<SlideShareServiceError>Something bad happened</SlideShareServiceError>';
    ...
    var _mockFlickr = function(serverStartedCallback) {
      TestsUtil.createTestServer(function(app, server, port) {
        ...
        // Pass control back so we can continue the unit test
        serverStartedCallback(expectedResponses, function(
    ...
    it('verify collaborative document processing works', function(callback) {
      ...
      _createContentAndWait('collabdoc', null, null, function(
        restCtx,
        content,
      ) {
        assert.equal(content.previews.status, 'done');
    ...
    it('verify zip files get ignored', function(callback) {
      ...
      _createContentAndWait('file', null, getZipStream, function(
        restCtx,
        content,
      ) {
        assert.equal(content.previews.status, 'ignored');
      ...
      TestsUtil.generateTestUsers(camAdminRestContext, 1, function(
        ...
        FoldersTestUtil.assertCreateFolderSucceeds(
          ...
          function(folder) {
            ...
            _createContentAndAddToFolder(simong.restContext, folder, function(
              content1,
              folder,
            ) {
              assert.ok(folder.previews);
                    ...
                    function() {
                      MQTestUtil.whenTasksEmpty(
                        ...
                        function() {
                          ...
                          Cassandra.runQuery('TRUNCATE "Content"', [], function(
                            err,
                          ) {
                            assert.ok(!err);
    ...
    it('verify reprocessing previews triggers an mq task', function(callback) {
      ...
      _reprocessWithHandler(globalAdminRestContext, filters, function(data) {
        ...
        _reprocessWithHandler(globalAdminRestContext, filters, function(data) {
          ...
          _reprocessWithHandler(globalAdminRestContext, filters, function(
            data,
          ) {
            assert.ok(data);
            assert.ok(data.filters);
          ...
          function(err) {
            ...
            RestAPI.Previews.reprocessPreviews(
              ...
              function(err) {
                ...
                RestAPI.Previews.reprocessPreviews(userRestCtx, null, function(
                  err,
                ) {
                  assert.ok(err);
    ...
    it('verify validation of reprocessing previews tasks', function(callback) {
      ...
      PreviewTestUtil.purgePreviewsQueue(function(err) {
        ...
        PreviewAPI.enable(function(err) {
          ...
          TaskQueue.unbind(PreviewConstants.MQ.TASK_GENERATE_PREVIEWS, function(
            err,
          ) {
            assert.ok(!err);
    ...
    it('verify a push notification is sent out on preview generation completion', function(callback) {
      ...
      PreviewTestUtil.purgePreviewsQueue(function(err) {
        ...
        TestsUtil.generateTestUsers(signedAdminRestContext, 1, function(
          ...
          RestAPI.User.getMe(mrvisser.restContext, function(
            err,
            mrvisserFullMeData,
          ) {
            assert.ok(!err);
              ...
              function(err, contentObj) {
                ...
                RestAPI.Content.getContent(
                  ...
                  function(err, contentObj) {
                    ...
                    ActivityTestsUtil.getFullySetupPushClient(data, function(
                      client,
                    ) {
                      ...
                      client.on('message', function(message) {
                        if (
                          ...
                          assert.equal(
                            message.activities[0].object.previews.status,
                            'done',
                          );

                          // Ensure that the full previews object is returned
                          assert.equal(
                            message.activities[0].object.previews.total,
                            4,
                          );
                          assert.ok(
                            message.activities[0].object.previews.largeUrl,
                          );
                          assert.ok(
                            message.activities[0].object.previews.mediumUrl,
                          );
                          assert.ok(
                            message.activities[0].object.previews.smallUrl,
                          );
                          assert.ok(
                            message.activities[0].object.previews.thumbnailUrl,
                          );
                          return callback();
                        }
                      });
                                    ...
                                    function() {
                                      ...
                                      RestAPI.Content.getRevisions(
                                        ...
                                        function(err, data) {
                                          ...
                                          assert.ok(data.results[0].previews);
                                          assert.equal(
                                            data.results[0].previews.status,
                                            'done',
                                          );
