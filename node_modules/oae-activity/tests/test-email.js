...
describe('Activity Email', function() {
    ...
    it('verify email markdown parsing', function(callback) {
        ...
        TestsUtil.generateTestUsers(camAdminRestContext, 2, function(err, users) {
            assert.ok(!err);
            var user0 = _.values(users)[0];
            var user1 = _.values(users)[1];

            // Make sure that the first user receives email messages
            RestAPI.User.updateUser(user0.restContext, user0.user.id, {'emailPreference': 'immediate'}, function(err) {
                assert.ok(!err);

                // Create a discussion on which to comment
                RestAPI.Discussions.createDiscussion(user0.restContext, 'Test Discussion', 'Test Description', 'public', null, [user1.user.id], function(err, discussion) {
                    assert.ok(!err);
                    ...
                    RestAPI.Discussions.createMessage(user1.restContext, discussion.id, markdownBody, null, function(err, comment) {
                        assert.ok(!err);
                        ...
                        EmailTestsUtil.collectAndFetchAllEmails(function(messages) {
                            ...
                            var mail = _.find(messages, function(message) { return (message.to[0].address === user0.user.email); })
                            assert.ok(mail);
