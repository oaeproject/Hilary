...
OAE.tenantRouter.on('post', '/api/discussion/create', function(req, res) {
    ...
    var memberIds = OaeUtil.getArrayParam(req.body.members);

    var roles = {};

    // Apply the manager roles
    _.each(managerIds, function(managerId) {
        roles[managerId] = DiscussionsConstants.roles.MANAGER;
    });

    // Apply the member roles
    _.each(memberIds, function(memberId) {
        roles[memberId] = DiscussionsConstants.roles.MEMBER;
    });
