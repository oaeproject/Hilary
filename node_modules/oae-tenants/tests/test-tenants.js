...
describe('Tenants', function() {
    ...
    describe('Tenant actions', function() {
                    ...
                    RestAPI.Tenants.getTenant(restContext, null, function(err, tenant) {
                        ...
                        RestAPI.Tenants.startTenant(globalAdminRestContext, testTenant.alias, function(err) {
                            ...
                            RestAPI.Tenants.getTenant(restContext, null, function(err, tenant) {
                                ...
                                assert.strictEqual(tenant.deleted, false);
                                return callback();
                            });
                        });
                    });
        ...
        it('verify delete tenant as anonymous user fails', function(callback) {
            TenantsTestUtil.generateTestTenants(globalAdminRestContext, 1, function(testTenant) {
                // Verify we cannot delete as the anonymous user
                RestAPI.Tenants.deleteTenant(anonymousGlobalRestContext, testTenant.alias, function(err) {
                    assert.ok(err);
                    assert.equal(err.code, 401);

                    // Sanity check we can delete as global admin
                    RestAPI.Tenants.deleteTenant(globalAdminRestContext, testTenant.alias, function(err) {
                        assert.ok(!err);
                        return callback();
                    });
                });
        ...
        it('verify delete non-existing tenant', function(callback) {
            // Delete tenant with no alias
            RestAPI.Tenants.deleteTenant(globalAdminRestContext, null, function(err) {
                assert.ok(err);
                assert.equal(err.code, 400);

                // Delete non-existing tenant
                RestAPI.Tenants.deleteTenant(globalAdminRestContext, TestsUtil.generateRandomText(), function(err) {
                    assert.ok(err);
                    assert.equal(err.code, 404);
                    return callback();
                });
