...
var Context = module.exports.Context = function(tenant, user) {
    ...
    this._isTenantAdmin = null;
    this._isGlobalAdmin = null;
     ...
     */
    this.isTenantAdmin = function(callback) {
        ...
        if (this._isTenantAdmin !== null) {
            return callback(null, this._isTenantAdmin);
        }

        // Nope, get it from the API.
        principalAPI.isTenantAdmin(tenant.alias, user, function(err, isAdmin) {
            if (!err) {
                this._isTenantAdmin = isAdmin;
            }
     ...
     */
    this.isGlobalAdmin = function(callback) {
        // Anonymous users are not admins.
        if (user === null) {
            return callback(null, false);
        }
        ...
        // Maybe we already performed the look-up and it's cached.
        if (this._isGlobalAdmin !== null) {
            return callback(null, this._isGlobalAdmin);
        }

        // Nope, get it from the API.
        principalAPI.isGlobalAdmin(user, function(err, isAdmin) {
            if (!err) {
                this._isGlobalAdmin = isAdmin;
            }
            callback(err, isAdmin);
        });
    };
