...
var store = module.exports.store = function(ctx, file, callback) {
    ...
    _ensureDirectoryExists(localDir, function(err) {
        ...
        var uri = util.format('%s/%s', dir, file.name);

        // Move the file.
        var newPath = util.format('%s/%s', localDir, file.name);
...
var get = module.exports.get = function(ctx, uri, callback) {
    ...
    var path = util.format("%s/%s", _getRootDir(ctx), uri.split(':')[1]);

    // Copy it to a temp folder.
    var filename = Path.basename(path);
    var tempPath = Temp.path({'suffix': filename});
    IO.copyFile(path, tempPath, function(err) {
        if (err) {
            return callback(err);
        }
        ...
        fs.stat(tempPath, function(err, stat) {
            if (err) {
                log().error({'err': err}, 'Could not stat the file: %s', tempPath);
