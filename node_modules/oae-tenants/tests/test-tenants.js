...
describe('Tenants', function() {
    ...
    describe('Tenant actions', function() {
        ...
        /**
         * Test that verifies that creating a tenant with the same hostname as the configured guest hostname is not allowed
         */
        it('verify creating a tenant with the guest tenant hostname as hostname is not allowed', function(callback) {
            var guestsHost = TenantsAPI.getGuestsTenant().host;
            TenantsTestUtil.createTenantAndWait(globalAdminRestContext, Math.random(), 'bladiebla', guestsHost, function(err) {
                assert.ok(err);
                assert.equal(err.code, 400);
                TenantsTestUtil.createTenantAndWait(globalAdminRestContext, Math.random(), 'bladiebla', guestsHost.toUpperCase(), function(err) {
                    assert.ok(err);
                    assert.equal(err.code, 400);
                    return callback();
                });
    ...
    describe('Update tenant', function() {
        ...
        /**
         * Test that verifies that updating a tenant's hostname to the guest host is not allowed
         */
        it('verify updating a tenant\'s hostname to the guest host is not allowed', function(callback) {
            // Create a tenant
            var tenantAlias = TenantsTestUtil.generateTestTenantAlias();
            var tenantDescription = TestsUtil.generateRandomText();
            var tenantHost = TenantsTestUtil.generateTestTenantHost();
            TenantsTestUtil.createTenantAndWait(globalAdminRestContext, tenantAlias, tenantDescription, tenantHost, function(err) {
                ...
                // Updating the hostname to the guest hostname should fail
                var guestsHost = TenantsAPI.getGuestsTenant().host;
                RestAPI.Tenants.updateTenant(globalAdminRestContext, tenantAlias, {'host': guestsHost}, function(err) {
                    assert.ok(err);
                    assert.equal(err.code, 400);

                    // Updating the hostname to any case of the guest hostname should fail
                    RestAPI.Tenants.updateTenant(globalAdminRestContext, tenantAlias, {'host': guestsHost.toUpperCase()}, function(err) {
                        assert.ok(err);
                        assert.equal(err.code, 400);
                        return callback();
                    });
