...
var getResetPasswordSecret =module.exports.getResetPasswordSecret = function(ctx, tenantAlias, username, callback){
	...
	_getUserIdFromLoginId(loginId, function(err, userId) {
        ...
        if (userId) {
			...
			// store it to cassandra database with 24hr valid time USING CONSISTENCY QUORUM AND TTL 86400 
			...
			PrincipalsAPI.getUser(ctx, userId, function(err, user) {
				...
				// TO DO: get the user's email address
				...
				// send the email to user(email address will probably be dispatched by sendEmail function)
				EmailAPI.sendEmail('oae-authentication', 'reset', user, emailData, null, function(err) {
			        if (err) {
			            log().error({'err': err}, 'Error in sending an Email to the user');
			            return callback(err);
			        }
			        return callback();
				});
		...
		// user doesn't exist
        	...
        	callback({'code': 400, 'msg': 'This login id does not exist and is not associated to any user'});
		}
	});
