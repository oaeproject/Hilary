 ...
 * @PathParam   {string}        userId                      The id of the user to check whether they have a pending email token
 ...
 */
var _hasPendingEmailToken = function(req, res) {
    PrincipalsAPI.hasPendingEmailToken(req.ctx, req.params.userId, function(err, email) {
        if (err) {
            return res.send(err.code, err.msg);
        }
...
OAE.globalAdminRouter.on('get', '/api/user/:userId/email/token', _hasPendingEmailToken);
OAE.tenantRouter.on('get', '/api/user/:userId/email/token', _hasPendingEmailToken);
