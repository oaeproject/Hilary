        ...
        Cassandra.runQuery('SELECT * FROM Config USING CONSISTENCY QUORUM WHERE tenantId IN (?)', [cqlParams], function(err, rows) {
            ...
            getDefaultConfiguration(function(defaultConfig) {
                ...
                for (var r = 0; r < rows.length; r++) {
                    if (rows[r].get('tenantId').value === config.servers.globalAdminAlias) {
                ...
                for (var rr = 0; rr < rows.length; rr++) {
                    if (rows[rr].get('tenantId').value !== config.servers.globalAdminAlias) {
                        cachedConfiguration[rows[rr].get('tenantId').value] = mergeStoredConfigIntoOriginal([rows[rr]], cachedConfiguration[config.servers.globalAdminAlias]);
                    }
                }
