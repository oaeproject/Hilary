    ...
    if (contentIds.length) {
        Cassandra.runQuery("SELECT * FROM Content WHERE contentId IN (?)", [contentIds], function(err, rows) {
            ...
            rows.forEach(function(row) {
                var contentObj = _rowToContent(row);
 ...
 */
var createLink = module.exports.createLink = function(ctx, visibility, name, description, link, additionalMembers, callback) {
    ...
    _createContent(ctx, visibility, name, description, 'link', {'link': link}, additionalMembers, callback);
};
 ...
 * @param  {Object}                  callback.content   JSON object containing the pool id of the created content
 */
var createFile = module.exports.createFile = function(ctx, visibility, name, description, additionalMembers, callback) {
    callback = callback || function() {};
    _createContent(ctx, visibility, name, description, 'file', {}, additionalMembers, callback);
};
 ...
 * @param  {Object}                  callback.content   JSON object containing the pool id of the created content
 */
var createSakaiDoc = module.exports.createSakaiDoc = function(ctx, visibility, name, description, additionalMembers, callback) {
    callback = callback || function() {};
    _createContent(ctx, visibility, name, description, 'sakaidoc', {}, additionalMembers, callback);
};
 ...
 * @param  {Object}                  callback.content   JSON object containing the pool id of the created content
 */
var _createContent = function(ctx, visibility, name, description, contentType, otherValues, additionalMembers, callback) {
    callback = callback || function() {};
    ...
    var q = Cassandra.constructInsertCQL('Content', parameters);
...
var _setContentPermissions = function(ctx, contentObj, newPermissions, callback) {
    ...
    PrincipalsUtil.getPrincipals(ctx, _.keys(newPermissions), function(err, principals, missingPrincipals) {
        if (err) {
            return callback(err);
...
var getContentMembers = module.exports.getContentMembers = function(ctx, contentId, start, limit, callback) {
    ...
    getContent(ctx, contentId, function(err, contentObj) {
        ...
        Cassandra.runQuery("SELECT first " + paging.limit + " ?..\'\' FROM ContentMembers WHERE contentId = ?", [paging.start, contentId], function (err, rows) {
            if (err) {
                return callback(err);
            }
...
var _updateContentLastModified = function(contentObj, callback) {
    ...
    Cassandra.runQuery('UPDATE Content SET ? = ? WHERE contentId = ?', ['lastModified', contentObj.lastModified, contentObj.contentId], callback);
};
...
var updateContentMetadata = module.exports.updateContentMetadata = function(ctx, contentId, profileFields, callback) {
    ...
    validator.check(fieldNames.length, {'code': 400, 'msg': 'You should at least specify a new name or description'}).min(1);
...
/**
 * Utility function that will be used to update metadata on a piece of content. Both the updateContentMetadata and
 * updateVisibility functions will call on this function. Once the Content CF has been updated, the libraries in
 * which the content shows will be updated as well. This function will assume that a valid contentId and valid
 * content metadata is passed in
 * @param  {Context}         ctx                 Standard context object, representing the currently logged user and its tenant
...
var _updateContentMetadata = function(ctx, contentId, metadata, callback) {
    ...
    canManage(ctx, contentId, function(err, canManage, contentObj) {
        ...
        var q = Cassandra.constructUpdateCQL('Content', metadata, 'contentId', contentId);
        Cassandra.runQuery(q.query, q.parameters, function(err) {
            if (err) {
                return callback(err);
            }
...
var _getLibraryItems = function(principalId, start, limit, callback) {
    ...
    Cassandra.runQuery("SELECT first " + paging.limit + " ?..\'\' FROM LibraryByPrincipal WHERE principalId = ?", [paging.start, principalId], function (err, rows) {
        if (err) {
            return callback(err);
        }
