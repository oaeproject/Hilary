...
describe('Collaborative documents', function() {
    ...
    it('verify basic parameter validation when publishing a collaborative document', function(callback) {
        ...
        ContentTestUtil.createCollabDoc(camAdminRestContext, 1, 1, function(contentObj, users, simon) {
            ...
            RestAPI.Content.createLink(simon.restContext, 'Test Content', 'Test description', 'public', 'http://www.oaeproject.org/', [], [], function(err, link) {
                assert.ok(!err);
                RestAPI.Content.publishCollabDoc(simon.restContext, link.id, function(err) {
                    assert.equal(err.code, 400);

                    // Check that the id is validated
                    RestAPI.Content.publishCollabDoc(simon.restContext, ' ', function(err) {
                        assert.equal(err.code, 400);
                        RestAPI.Content.publishCollabDoc(simon.restContext, 'invalid-id', function(err) {
                            assert.equal(err.code, 400);

                            // Sanity check
                            RestAPI.Content.publishCollabDoc(simon.restContext, contentObj.id, function(err, data) {
                                assert.ok(!err);
                                assert.ok(data);
                                return callback();
                            });
    ...
    /**
     * Test that verifies that you can only publish a collaborative document if you have manager permissions
     */
    it('verify publishing a pad respects the content permissions', function(callback) {
        // Create a test user and collaborative document where the user has joined the document
        ContentTestUtil.createCollabDoc(camAdminRestContext, 1, 1, function(contentObj, users, simon) {

            // Create a test user that's not affiliated with the collaborative document
            TestsUtil.generateTestUsers(camAdminRestContext, 1, function(err, users, branden) {
                ...
                editAndPublish(simon, contentObj, [text], function() {
                    ...
                    RestAPI.Content.publishCollabDoc(branden.restContext, contentObj.id, function(err, data) {
                        ...
                        RestAPI.Content.updateMembers(simon.restContext, contentObj.id, members, function(err) {
                            assert.ok(!err);
                            ...
                            RestAPI.Content.publishCollabDoc(branden.restContext, contentObj.id, function(err, data) {
                                ...
                                RestAPI.Content.updateMembers(simon.restContext, contentObj.id, members, function(err) {
                                    ...
                                    // Branden should now be able to access it and create revisions
                                    RestAPI.Content.joinCollabDoc(branden.restContext, contentObj.id, function(err, data) {
                                        assert.ok(!err);
                                        text = 'Lilalilalila';
                                        editAndPublish(branden, contentObj, [text], function() {
    ...
    var setEtherpadText = function(userId, contentObj, text, callback) {
        ...
        etherpadClient.setText(args, function(err) {
            ...
            // Make that user part of the recent authors
            Etherpad.addRecentAuthor(contentObj.id, contentObj.etherpadPadId, userId, callback);
        });
    };
    ...
    var editAndPublish = function(user, contentObj, texts, callback) {
        ...
        var doEditAndPublish = function() {
            ...
            setEtherpadText(user.user.id, contentObj, texts[done], function(err) {
                ...
                RestAPI.Content.publishCollabDoc(user.restContext, contentObj.id, function(err, data) {
                    assert.ok(!err);
                    assert.ok(data);
                    done++;
                    doEditAndPublish();
    ...
    it('verify empty revisions can be restored', function(callback) {
        ContentTestUtil.createCollabDoc(camAdminRestContext, 1, 1, function(contentObj, users, simon) {
            ...
            RestAPI.Content.publishCollabDoc(simon.restContext, contentObj.id, function(err) {
                assert.ok(!err);
