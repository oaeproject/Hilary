...
var createUser = module.exports.createUser = function(ctx, displayName, opts, callback) {
    ...
    var columns = ['principalId', 'tenant', 'displayName', 'visibility', 'locale', 'timezone', 'publicAlias'];
    var values = [id, tenantAlias, displayName, opts.visibility, opts.locale, opts.timezone, opts.publicAlias];

    // Only add the picture URIs if they have been filled out.
    if (opts.smallPictureUri) {
        columns.push('smallPictureUri');
        values.push(opts.smallPictureUri);
    }
    if (opts.mediumPictureUri) {
        columns.push('mediumPictureUri');
        values.push(opts.mediumPictureUri);
    }
    if (opts.largePictureUri) {
        columns.push('largePictureUri');
        values.push(opts.largePictureUri);
    }
    ...
    // Create the user
    Cassandra.runQuery('INSERT INTO Principals (' + columns.join(',') + ') VALUES (' + questionMarks.join(',') + ') USING CONSISTENCY QUORUM', values, function (err) {
        if (err) {
            return callback(err);
        }
...
var updateUser = module.exports.updateUser = function(ctx, userId, profileFields, callback) {
    ...
    if (profileFields['visibility']) {
        validator.check(profileFields['visibility'], {'code': 400, 'msg': 'An invalid visibility option has been passed on'}).isIn(_.values(PrincipalsConstants.visibility));
    }
