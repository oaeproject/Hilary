 ...
 * @param  {Object} config.previews.office          The namespace for all Office related configuration.
 * @param  {String} config.previews.office.binary   The path to the Libre Office executable. This should either be a direct path or the filename that's on the `PATH` environment.
 * @param  {Number} config.previews.office.timeout  Specifies the time (in ms) when the process is considered to be hanging and should be killed. (Default: 120000ms)
 */
var init = module.exports.init = function(config, callback) {
    ...
    log().trace('Executing %s to verify if the path to the office binary is correct.', cmd);
