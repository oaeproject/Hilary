 ...
 */
var transformActivities = (module.exports.transformActivities = function(
  ctx,
  activities,
  transformerType,
  callback,
  ...
  try {
    activities.forEach(function(activity) {
      ...
      _getActivityEntitiesByObjectType(
        activityId,
        activity.actor,
    ...
    log().error(
      { err: err, activities: logActivities, user: ctx.user() },
  ...
  if (objectTypes.length > 0) {
    objectTypes.forEach(function(objectType) {
      var transformer = _getEntityTypeTransformer(objectType, transformerType);
      transformer(ctx, activityEntitiesByObjectType[objectType], function(
        err,
        transformedActivityEntities,
      ) {
        if (err) {
        ...
        _.keys(transformedActivityEntities).forEach(function(activityId) {
          _.keys(transformedActivityEntities[activityId]).forEach(function(
            entityId,
          ) {
 ...
 * @api private
 */
var _getEntityTypeTransformer = function(objectType, transformerType) {
  var activityEntityType =
    ActivityRegistry.getRegisteredActivityEntityTypes()[objectType] || {};
  var transformer = activityEntityType.transformer;
 ...
 * @api private
 */
var _getActivityEntitiesByObjectType = function(
  activityId,
  entity,
  activityEntitiesByObjectType,
) {
  if (entity && entity.objectType !== 'collection') {
    ...
    entity[ActivityConstants.properties.OAE_COLLECTION].forEach(function(
      entity,
    ) {
      activityEntitiesByObjectType[entity.objectType] =
        activityEntitiesByObjectType[entity.objectType] || {};
 ...
 * @api private
 */
var _transformActivities = function(
  transformedActivityEntitiesByObjectType,
  activities,
) {
  activities.forEach(function(activity) {
...
var _transformEntity = function(
  ...
  if (entity.objectType !== 'collection') {
    return transformedActivityEntitiesByObjectType[entity.objectType][
      activityId
    ][entityId];
  } else {
    ...
    entity[ActivityConstants.properties.OAE_COLLECTION].forEach(function(
      ...
      var transformedEntity = _transformEntity(
        transformedActivityEntitiesByObjectType,
        activityId,
        collectionEntity,
      );
 ...
 * @api private
 */
var _defaultActivityEntityTransformer = function(
  ctx,
  activityEntities,
  callback,
) {
...
var _addTenantInformationToActivityEntity = function(entity) {
  if (entity) {
    ...
    if (entity[ActivityConstants.properties.OAE_ID]) {
      var tenantAlias = AuthzUtil.getResourceFromId(
        entity[ActivityConstants.properties.OAE_ID],
      ).tenantAlias;
