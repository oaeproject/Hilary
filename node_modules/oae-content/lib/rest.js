...
module.exports = function(tenant) {
    log().info('Initializing content rest endpoints for tenant %s', tenant.alias);
    ...
    tenant.server.post('/api/content/create', function(req, res) {
        ...
        // File creation
            ...
            // Upload was succesfull, deal with it.
            ContentAPI.createFile(req.ctx, req.body.name, req.body.description, req.body.visibility, req.body.additionalMembers, req.files.file, function(err, contentObj) {
                if (err) {
     ...
     */
    tenant.server.get('/api/content/:contentId/download', function(req, res) {
        req.telemetryUrl = '/api/content/id/download';
        ContentAPI.getLatestRevision(req.ctx, req.params.contentId, function(err, revision) {
            if (err) {
                return res.send(err.code, err.msg);
            }

            handleDownloadLink(res, revision.link);
        });
    });
     ...
     */
    tenant.server.post('/api/content/:contentId/newversion', function(req, res) {
     ...
     */
    tenant.server.post('/api/content/:contentId/previews', function(req, res) {
        req.telemetryUrl = '/api/content/id/previews';
        if (!req.files) {
            return res.send(400, 'Missing preview files');
        }

        ContentAPI.addPreviewItems(req.ctx, req.params.contentId, req.params.revisionId, req.files, function(err, files) {
            if (err) {
                return res.send(err.code, err.msg);
            }
            res.send(201);
     ...
     */
    tenant.server.get('/api/content/:contentId/previews', function(req, res) {
        req.telemetryUrl = '/api/content/id/previews';
     ...
     */
    tenant.server.get('/api/content/:contentId/previews/:item', function(req, res) {
        req.telemetryUrl = '/api/content/id/previews/item';
        ContentAPI.getPreviewItem(req.ctx, req.params.contentId, req.params.item, req.query.signature, req.query.expires, function(err, link) {
            if (err) {
                return res.send(err.code, err.msg);
            }

            handleDownloadLink(res, link);
        });
    });
     ...
     * Content revisions
     */
    tenant.server.get('/api/content/:contentId/revisions', function(req, res) {
 ...
 * @param {Object}      link    A link object that holds a url and code.
 */
var handleDownloadLink = function(res, link) {
    if (link.code === 204) {
        res.setHeader('X-Accel-Redirect', link.url);
        res.send(204);
    } else if (link.code === 301) {
