    ...
    describe('#computeMemberRolesAfterChanges()', function() {
        ...
        it('verify functionality when taking into account adds, removes and updates', function(callback) {
            ...
            AuthzAPI.updateRoles(groupId, rolesBefore, function(err) {
                ...
                AuthzAPI.computeMemberRolesAfterChanges(groupId, addBertChange, null, function(err, changes, roles, members) {
                    assert.ok(!err);
                    assert.deepEqual(changes, addBertChange);
                    assert.deepEqual(roles.before, rolesBefore);
                    assert.deepEqual(roles.after, _.extend({}, rolesBefore, addBertChange));
                    assert.deepEqual(members.addedIds, [bert]);
                    assert.deepEqual(members.updatedIds, []);
                    assert.deepEqual(members.removedIds, []);

                    // Verify a role change by making mrvisser a manager
                    var mrvisserManagerChange = makeMembershipChange(mrvisser, 'manager');
                    AuthzAPI.computeMemberRolesAfterChanges(groupId, mrvisserManagerChange, null, function(err, changes, roles, members) {
                        assert.ok(!err);
                        assert.deepEqual(changes, mrvisserManagerChange);
                        assert.deepEqual(roles.before, rolesBefore);
                        assert.deepEqual(roles.after, _.extend({}, rolesBefore, mrvisserManagerChange));
                        assert.deepEqual(members.addedIds, []);
                        assert.deepEqual(members.updatedIds, [mrvisser]);
                        assert.deepEqual(members.removedIds, []);
                        ...
                        AuthzAPI.computeMemberRolesAfterChanges(groupId, simonMemberChange, null, function(err, changes, roles, members) {
                            assert.ok(!err);
                            assert.deepEqual(changes, {});
                            assert.deepEqual(roles.before, rolesBefore);
                            assert.deepEqual(roles.after, rolesBefore);
                            assert.deepEqual(members.addedIds, []);
                            assert.deepEqual(members.updatedIds, []);
                            assert.deepEqual(members.removedIds, []);
                            ...
                            AuthzAPI.computeMemberRolesAfterChanges(groupId, simonRemoveChange, null, function(err, changes, roles, members) {
                                assert.ok(!err);
                                assert.deepEqual(changes, simonRemoveChange);
                                assert.deepEqual(roles.before, rolesBefore);
                                assert.deepEqual(roles.after, _.omit(rolesBefore, simong));
                                assert.deepEqual(members.addedIds, []);
                                assert.deepEqual(members.updatedIds, []);
                                assert.deepEqual(members.removedIds, [simong]);
                                ...
                                AuthzAPI.computeMemberRolesAfterChanges(groupId, unknownUserRemoveChange, null, function(err, changes, roles, members) {
                                    assert.ok(!err);
                                    assert.deepEqual(changes, {});
                                    assert.deepEqual(roles.before, rolesBefore);
                                    assert.deepEqual(roles.after, rolesBefore);
        ...
        it('verify functionality when taking into account only promotions', function(callback) {
            ...
            AuthzAPI.updateRoles(groupId, rolesBefore, function(err) {
                ...
                var addBertChange = makeMembershipChange(bert, 'member');
                AuthzAPI.computeMemberRolesAfterChanges(groupId, addBertChange, {'promoteOnly': true}, function(err, changes, roles, members) {
                    assert.ok(!err);
                    assert.deepEqual(changes, addBertChange);
                    assert.deepEqual(roles.before, rolesBefore);
                    assert.deepEqual(roles.after, _.extend({}, rolesBefore, addBertChange));
                    ...
                    AuthzAPI.computeMemberRolesAfterChanges(groupId, mrvisserEditorChange, {'promoteOnly': true}, function(err, changes, roles, members) {
                        assert.ok(!err);
                        assert.deepEqual(changes, mrvisserEditorChange);
                        assert.deepEqual(roles.before, rolesBefore);
                        assert.deepEqual(roles.after, _.extend({}, rolesBefore, mrvisserEditorChange));
                        assert.deepEqual(members.addedIds, []);
                        assert.deepEqual(members.updatedIds, [mrvisser]);
                        assert.deepEqual(members.removedIds, []);
                        ...
                        AuthzAPI.computeMemberRolesAfterChanges(groupId, stephenViewerChange, {'promoteOnly': true}, function(err, changes, roles, members) {
                            assert.ok(!err);
                            assert.deepEqual(changes, {});
                            assert.deepEqual(roles.before, rolesBefore);
                            assert.deepEqual(roles.after, rolesBefore);
                            ...
                            var simonMemberChange = makeMembershipChange(simong, 'member');
                            AuthzAPI.computeMemberRolesAfterChanges(groupId, simonMemberChange, {'promoteOnly': true}, function(err, changes, roles, members) {
                                assert.ok(!err);
                                assert.deepEqual(changes, {});
                                assert.deepEqual(roles.before, rolesBefore);
                                assert.deepEqual(roles.after, rolesBefore);
                                ...
                                // change
                                var simonRemoveChange = makeMembershipChange(simong, false);
                                AuthzAPI.computeMemberRolesAfterChanges(groupId, simonRemoveChange, {'promoteOnly': true}, function(err, changes, roles, members) {
                                    assert.ok(!err);
                                    assert.deepEqual(changes, {});
                                    assert.deepEqual(roles.before, rolesBefore);
                                    assert.deepEqual(roles.after, rolesBefore);
                                    ...
                                    var unknownUserRemoveChange = makeMembershipChange(unknownUser, false);
                                    AuthzAPI.computeMemberRolesAfterChanges(groupId, unknownUserRemoveChange, {'promoteOnly': true}, function(err, changes, roles, members) {
                                        assert.ok(!err);
                                        assert.deepEqual(changes, {});
                                        assert.deepEqual(roles.before, rolesBefore);
                                        assert.deepEqual(roles.after, rolesBefore);
