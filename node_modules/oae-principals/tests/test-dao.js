...
describe('Principals DAO', function() {
    ...
    describe('#deletePrincipal', function() {

        /**
         * Test that verifies only group principals can be deleted
         */
        it('verify deleting a user results in a 400 error', function(callback) {
            // Create a user that we can delete
            TestsUtil.generateTestUsers(camAdminRestContext, 1, function(err, users, mrvisser) {
                assert.ok(!err);
                var mrvisserBeforeDelete = mrvisser.user;

                // Delete the user, ensuring it fails
                PrincipalsDAO.deletePrincipal(mrvisserBeforeDelete.id, function(err) {
                    assert.ok(err);
                    assert.strictEqual(err.code, 400);

                    // Ensure subsequent accesses result in a principal that doesn't have a deleted
                    // flag set
                    PrincipalsDAO.getPrincipal(mrvisserBeforeDelete.id, function(err, mrvisserAfterDelete) {
                        assert.ok(!err);
                        assert.ok(!_.chain(mrvisserAfterDelete).keys().contains('deleted').value());
    ...
    describe('#restorePrincipal', function() {

        /**
         * Test that verifies only group principals can be restored
         */
        it('verify restoring a user results in a 400 error', function(callback) {
            // Create a user that we can restore
            TestsUtil.generateTestUsers(camAdminRestContext, 1, function(err, users, mrvisser) {
                assert.ok(!err);
                var mrvisserBeforeRestore = mrvisser.user;

                // Restore the user, ensuring it fails
                PrincipalsDAO.restorePrincipal(mrvisserBeforeRestore.id, function(err) {
                    assert.ok(err);
                    assert.strictEqual(err.code, 400);
                    return callback();
                });
