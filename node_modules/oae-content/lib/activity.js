...
/*!
 * Post a content-share activity when a user shares a content item.
 */
ContentAPI.on(ContentConstants.events.UPDATED_CONTENT_MEMBERS, function(ctx, contentId, permissionChanges) {
    ...
    for (var i = 0; i < principalIds.length; i++) {
        ...
        if (permissionChanges[principalId]) {
            var resourceType = (PrincipalsUtil.isGroup(principalId)) ? 'group' : 'user';
            targetResources.push(new ActivityModel.ActivitySeedResource(resourceType, principalId));
        }
    }

    // Post an activity for each principal this content item was shared with
    for (var i = 0; i < targetResources.length; i++) {
        ActivityAPI.postActivity(new ActivityModel.ActivitySeed(ContentConstants.activity.ACTIVITY_CONTENT_SHARE, millis, ActivityConstants.verbs.SHARE, actorResource, objectResource, targetResources[i]));
    }
});
...
ActivityAPI.registerActivityObjectProducer('content', function(activityType, entityType, resource, callback) {
...
ActivityAPI.registerActivityRouter('content', function(activityType, entityType, contentActivityObject, callback) {
    // Get lists of all the direct and indirect members of this content, keyed by their role
    ActivityUtil.getAllAuthzMembersByRole(contentActivityObject[ActivityConstants.properties.OAE_ID], function(err, membersByRole) {
        if (err) {
            return callback(err);
        }

        // When content is shared with another user, the 'viewers' of that content object are not interested
        if (activityType === ContentConstants.activity.ACTIVITY_CONTENT_SHARE && entityType === ActivityConstants.entityTypes.OBJECT) {
