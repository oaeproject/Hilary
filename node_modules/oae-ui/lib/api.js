    ...
    data.util = {
        'html': {
            ...
            'toText': function(str, retainLinks) {
                if (retainLinks) {
                    // Try to replace links with a plain-text variant
                    str = str.replace(/<a(.*?)href="(.*?)"(.*?)>(.*?)<\/a>/gm, function(match, pre, link, post, text) {
                        // Only replace the link tag if it's a link that is pointing to something
                        if (link && text && text[0] !== '#') {
                            return text + ' (' + link + ')';
                        }

                        // Return the link as-is
                        return util.format('<a%shref="%s"%s>%s</a>', pre, link, post, text);
                    });
                }
