...
var createUser = module.exports.createUser = function(ctx, tenantAlias, displayName, opts, callback) {
    ...
    opts.emailVerified = (opts.emailVerified || isAdmin);
    ...
    if (opts.email) {
        PrincipalsDAO.getUserByEmail(opts.email, function(err, existingUser) {
            if (err && err.code !== 404) {
                return callback(err);
...
var _createUser = function(ctx, user, callback) {
    PrincipalsDAO.createUser(user, function(err) {
        ...
        var isAdmin = (ctx.user() && ctx.user().isAdmin(user.tenant.alias));
...
/**
 * Get or create users accounts for a set of email addresses. If a user account has to be created
 * it will be created as a guest user. If the domain matches a configured email domain for a tenant
 * the user account will be created on that tenant. If not, it will be created on the `guests` tenant
 *
 * @param  {Context}        ctx                         Standard context object containing the current user and the current tenant
 * @param  {String[]}       emails                      An array of emails which user accounts should be created or retrieved
 * @param  {Function}       callback                    Standard callback function
 * @param  {Object}         callback.err                An error that occurred, if any
 * @param  {Object}         callback.users              Object mapping each email to a `user` object
 */
var getOrCreateGuestUsers = module.exports.getOrCreateGuestUsers = function(ctx, emails, callback, _users) {
    _users = _users || {};
    if (_.isEmpty(emails)) {
    ...
    _getOrCreateGuestUser(ctx, email, function(err, user) {
        if (err) {
            return callback(err);
        }

        _users[email] = user;
        getOrCreateGuestUsers(ctx, emails, callback, _users);
    });
};
 ...
 */
var _getOrCreateGuestUser = function(ctx, email, callback) {
    ...
    // Check if the email is already mapped to a user
    PrincipalsDAO.getUserByEmail(email, function(err, user) {
        if (err && err.code !== 404) {
            return callback(err);
        } else if (!err && user) {
            // Private tenancies can only interact with users from their own tenancy through email
            if (user.tenant.alias !== ctx.tenant().alias && TenantsUtil.isPrivate(ctx.tenant().alias)) {
                return callback({'code': 401, 'msg': 'Private tenants can not interact with guests'});
            }

            return callback(null, user);
        }

        // Private tenancies cannot create full-blown guest users
        if (TenantsUtil.isPrivate(ctx.tenant().alias)) {
            return callback({'code': 401, 'msg': 'Private tenants can not interact with guests'});
        }
        ...
        var opts = {
            'email': email,
            'emailPreference': PrincipalsConfig.getValue(ctx.tenant().alias, 'user', 'emailPreference'),

            // Note that we keep the email address as unverified. Setting this to `true` would allow the activity aggregator
            // to send emails to the guest, but could provide a potential risk for spamming people
            'emailVerified': false,
        ...
        // Get the tenant with whom the email address could be associated
        var emailDomain = email.split('@')[1];
        var tenant = TenantsUtil.getTenantForEmailDomain(emailDomain);

        if (!tenant) {
            return createGuestUser(ctx, displayName, opts, callback);
        }

        // If we do find a tenant that matches the guest's email domain,
        // we can simply create a user account in that tenant
        log().debug({'tenant': tenant.alias, 'email': email, 'inviter': ctx.user().id}, 'Creating guest user on specific tenant');
        createUser(ctx, tenant.alias, displayName, opts, function(err, user) {
            if (err) {
                return callback(err);
            }

            // A user account has been created on the tenant, but no login ID has been associated
            // with it yet. If the tenant is using external SSO like Shibboleth or CAS, the association
            // will happen when the user logs in for the first time as the email field should match.
            // However, some tenants do not use SSO (or rely on local authentication). If this is the case,
            // the email we send out will include a token they can use to associate a local login account
            createGuestTokenForUserIfNecesary(ctx, user, tenant, function(err, token) {
                if (err) {
                    return callback(err);
                }

                // In any case, we send the guest an email to invite him into the system
                sendGuestInvitiationEmail(ctx, user, tenant, token, callback);
            });
        });
    });
};

var createGuestUser = function(ctx, displayName, opts, callback) {
    log().debug({'email': opts.email, 'inviter': ctx.user().id}, 'Creating guest user on guests tenant');
    ...
    createUser(ctx, guestsTenant.alias, displayName, opts, function(err, user) {
        ...
        createGuestTokenForUser(ctx, user, guestsTenant, function(err, token) {
            if (err) {
                return callback(err);
            }

            // Send an email to the specified e-mail address
            sendGuestInvitiationEmail(ctx, user, guestsTenant, token, callback);
        });
    });
};

var createGuestTokenForUserIfNecesary = function(ctx, user, tenant, callback) {
    // If local authentication isn't enabled, we don't create a token for the guest as there's no
    // point in having them create a local account. They wouldn't be able to sign with it
    var localAuthenticationIsEnabled = AuthenticationConfig.getValue(tenant.alias, 'local', 'enabled');
    if (!localAuthenticationIsEnabled) {
        return callback();
    }

    // Otherwise we create the guest token
    return createGuestTokenForUser(ctx, user, tenant, callback);
};

var createGuestTokenForUser = function(ctx, user, tenant, callback) {
    // Generate a token that will be sent to the user's email address
    var token = util.format('guest-%s', _generateEmailToken());
...
var sendGuestInvitiationEmail = function(ctx, user, tenant, token, callback) {
    var data = {
        'tenant': tenant,
        'canCreateLocalAccount': (!_.isUndefined(token)),
        'invitee': user,
        'inviter': ctx.user(),
        'inviterDisplayName': ctx.user().displayName,
        'baseUrl': TenantsUtil.getBaseUrl(tenant),
        'skinVariables': require('oae-ui').getTenantSkinVariables(tenant.alias),
        'token': token
    };
...
/**
 * Verify a guest token and associate a local login strategy with the user matching the token. A guest
 * token is sent to a user's email address when he's invited by another user
 *
 * @param  {Context}    ctx                         Standard context object containing the current user and the current tenant
 ...
 * @param  {Function}   callback                    Standard callback function
 * @param  {Object}     callback.err                An error that occurred, if any
 * @param  {User}       callback.user               The user the token was associated with
 */
var verifyGuestToken = module.exports.verifyGuestToken = function(ctx, token, username, password, callback) {
    ...
    validator.check(token, {'code': 400, 'msg': 'A guest token must start with guest-'}).regex(/^guest-.*$/);
    ...
    // Ensure a user exists for the given token
    PrincipalsDAO.getUserByEmailToken(token, function(err, user) {
        if (err) {
            return callback(err);
        }

        // Associate the user with the new login
        // TODO: Get rid of cyclical dependencies
        ...
        // Ensure the login id is not used yet
        ...
        require('oae-authentication').associateLoginId(userCtx, loginId, user.id, function(err) {
            ...
            PrincipalsDAO.validateEmailAddress(token, user, function(err) {
                if (err) {
                    return callback(err);
                }
...
var updateUser = module.exports.updateUser = function(ctx, userId, profileFields, callback) {
    ...
    PrincipalsDAO.getPrincipal(userId, function(err, oldUser) {
        ...
        // If the user updated the email address we need to check if it's not associated to another user already
        if (profileFields.email && oldUser.email !== profileFields.email) {
            PrincipalsDAO.getUserByEmail(profileFields.email, function(err, user) {
                if (err && err.code !== 404) {
                    return callback(err);
                } else if (!err && user) {
                    return callback({'code': 400, 'msg': 'A user with that email address already exists'});
                }

                // If the user is an administrator we consider the email address to be verified
                profileFields.emailVerified = ctx.user().isAdmin(oldUser.tenant.alias);

                return _updateUser(ctx, oldUser, profileFields, callback);
            });
        } else {
            return _updateUser(ctx, oldUser, profileFields, callback);
        }
...
/**
 * Update a user. This function will *not* perform any authorisation checks
 *
 * @param  {Context}        ctx             Standard context object containing the current user and the current tenant
 * @param  {String}         userId          The id of the user to update
 * @param  {Object}         profileFields   Object that represent the profile fields that should be updated. Possible keys are `visibility`, `displayName`, `publicAlias`, `locale`, `email` and `emailPreference`
...
var _updateUser = function(ctx, oldUser, profileFields, callback) {
    ...
    PrincipalsDAO.updatePrincipal(oldUser.id, profileFields, oldUser.email, function(err) {
        if (err) {
            return callback(err);
        }
        ...
        getUser(ctx, oldUser.id, callback);
...
var sendEmailToken = module.exports.sendEmailToken = function(ctx, user, callback) {
    ...
    // Generate a token that will be sent to the user's email address
    var token = util.format('email-%s', _generateEmailToken());
...
var verifyEmailToken = module.exports.verifyEmailToken = function(ctx, token, callback) {
    var validator = new Validator();
    validator.check(token, {'code': 400, 'msg': 'A token must be provided'}).notEmpty();
    validator.check(token, {'code': 400, 'msg': 'A guest token must start with email-'}).regex(/^email-.*$/);
    if (validator.hasErrors()) {
        return callback(validator.getFirstError());
    }
    ...
    PrincipalsDAO.getUserByEmailToken(token, function(err, user) {
        ...
        // Remove the token and validate the email address
        PrincipalsDAO.validateEmailAddress(token, user, function(err) {
            if (err) {
                return callback(err);
            }
