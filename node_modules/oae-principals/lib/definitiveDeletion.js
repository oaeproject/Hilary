...
var AuthzInvitationDAO = require('oae-authz/lib/invitations/dao');
...
var transferDataToArchive = module.exports.transferDataToArchive = function (ctx, user, archiveUsers, callback) {
    ...
    async.eachSeries(types, function iterator(type, innerCallback) {
        ...
        _addToArchive(archiveUsers.archiveId, user, listOfIdElementToBeTransferred, function(err) {
            if (err) {
                return callback(err);
            }
...
var _sendEmail = function(ctx, data, archiveUser, userDeleted, callback) {
    ...
    callback = callback || function(err) {
        if (err) {
            log().error({'err': err, 'user': userDeleted.id}, 'Unable to send a user a verification email');
        }
    };
 ...
 * @param  {String}     principal       The user to delete 
 * @param  {String}     elementId       The id element to delete
 * @param  {Function}   callback        Standard callback function
 * @param  {Object}     callback.err    An error that occured, if any
 * @api private
 */
var _addToArchive = function(userArchiveId, principal, elementId, callback) {

    if (!userArchiveId || !principal.id) {
    ...
    var months = PrincipalsConfig.getValue(principal.tenant.alias, 'user', 'delete');

    var deletionDate = new Date();
    
    if (months) {
        deletionDate.setMonth(deletionDate.getMonth() + parseInt(months));
        deletionDate.setYear(deletionDate.getFullYear() + Math.trunc((deletionDate.getMonth() + parseInt(months)) / 12));
    } else {
    ...
    PrincipalsDAO.addDataToArchive(userArchiveId, principal.id, duplicationRemoved, deletionDate.toString(), function(err) {
        if (err) {
            return callback(err);
        }
        return callback();  
...
var _updateRoles = function(ctx, element, archiveUser, type, callback) {
    ...
    if (type === FOLDER) {
        ...
        ContentAPI.setContentPermissions(ctx, element.id, update, function(err ,update) {
                if (err) {
                    return callback(err);
                }
                return callback();
 ...
 * @param  {Object}     callback.err    An error that occured, if any
 */
var deleteUser = module.exports.deleteUser = function(ctx, user, alias, callback) {
    ...
    PrincipalsDAO.getArchivedUser(alias, function(err, userArchive) {
        ...
        PrincipalsDAO.getDataFromArchive(userArchive.archiveId, user.id, function(err, data) {
            if (err) {
                return callback(err);
            }   

            // Delete all rights on resources
            _deleteResourceRights(ctx, user, userArchive.archiveId, data, function(err) {
                if (err) {
                    return callback(err);
                } 
                ...
                removeProfilePicture(ctx, user, function(err) {
                    ...
                    FollowingAPI.deleteFollowers(ctx, user, function(err) {
                        ...
                        FollowingAPI.deleteFollowing(ctx, user, function(err) {
                            ...
                            AuthzInvitationDAO.deleteInvitationsByEmail(user.email, function(err) {
                                if (err) {
                                    return callback(err);
                                }

                                // Delete all activity
                                ActivityAPI.removeActivityStream(ctx, user.id, function(err) {
                                    if (err) {
                                        return callback(err);
                                    }

                                    // remove the principal from the table 'DataArchive'
                                    PrincipalsDAO.removePrincipalFromDataArchive(userArchive.archiveId, user.id, function(err) {
                                        if (err) {
                                            return callback(err);
                                        }
...
var removeProfilePicture = module.exports.removeProfilePicture = function(ctx, user, callback) {
    PrincipalsDAO.getPrincipal(user.id, function (err, principal) {
        ...
        if (_.isEmpty(principal.picture)) {
            ...
            var pathStorageBackend = ContentUtil.getStorageBackend(ctx, principal.picture.largeUri).getRootDirectory();
 ...
 * @api private
 */
var _deleteResourceRights = function(ctx, user, archiveId, data, callback) {   
