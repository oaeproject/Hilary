...
describe('Following Activity', function() {
    ...
    it('verify user gets notification and email when a user follows them', function(callback) {
        TestsUtil.generateTestUsers(camAdminRestContext, 2, function(err, users) {
            ...
            // Set the email for the followed user
            RestAPI.User.updateUser(followed.restContext, followed.user.id, {'email': 'email@email.blah'}, function(err) {
                assert.ok(!err);
                ...
                RestAPI.Following.follow(follower.restContext, followed.user.id, function(err) {
                    ...
                    EmailTestsUtil.collectAndFetchEmails(function(messages) {
                        ...
                        assert.equal(messages[0]._message.to, 'email@email.blah');
                        assert.notEqual(messages[0]._message.html.indexOf(follower.user.profilePath), -1);
        ...
        TestsUtil.generateTestUsers(camAdminRestContext, 4, function(err, users, simon, branden, nico, bert) {
            ...
            RestAPI.Following.follow(branden.restContext, simon.user.id, function(err) {
                ...
                RestAPI.Following.follow(nico.restContext, simon.user.id, function(err) {
                    ...
                    EmailTestsUtil.collectAndFetchEmails(function(messages) {
                        assert.equal(messages.length, 1);
                        assert.equal(messages[0]._message.to, simon.user.email);
                        assert.equal(messages[0]._message.html.match(/data-activity-id/g).length, 1);

                        // Assert both Branden and Nico are mentioned in the email
                        assert.ok(messages[0]._message.html.indexOf(branden.user.profilePath) > 0);
                        assert.ok(messages[0]._message.html.indexOf(nico.user.profilePath) > 0);
                        ...
                        RestAPI.Following.follow(bert.restContext, simon.user.id, function(err) {
                            ...
                            EmailTestsUtil.collectAndFetchEmails(function(messages) {
                                ...
                                assert.strictEqual(messages[0]._message.html.indexOf(branden.user.profilePath), -1);
                                assert.strictEqual(messages[0]._message.html.indexOf(nico.user.profilePath), -1);

                                // Assert Bert is mentioned in the email
                                assert.ok(messages[0]._message.html.indexOf(bert.user.profilePath) > 0);
                                return callback();
                            });
                        });
                    });
