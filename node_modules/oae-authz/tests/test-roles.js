...
describe('Authz-Roles', function() {
    ...
    var loadContentRoles = function(principalUuid, baseContentId, resourceType, numContentItems, role, callback) {
        ...
        AuthzAPI.setRole(principalUuid, resourceUuid, role, function(err) {
            if (err) {
                throw err;
            }
    ...
    describe('#getRole()', function() {
        ...
        it('verify role separation between tenants', function(callback) {
            ...
            AuthzAPI.setRole(principalUuidA, resourceUuid, 'manager', function(err) {
                assert.ok(!err);
                ...
                AuthzAPI.getRole(principalUuidB, resourceUuid, function(err, role) {
                    ...
                    AuthzAPI.setRole(principalUuidB, resourceUuid, 'viewer', function(err) {
                        assert.ok(!err);
    ...
    describe('#getRolesForResourceType()', function() {

        it('verify general functionality', function(callback) {
            var baseViewerContentId = 'contentIView';
            var baseManagerContentId = 'contentIManage';

            var principalUuid = AuthzUtil.toUuid(PrincipalTypes.USER, 'testGetRolesForResourceType', 'mrvisser');
            ...
            loadContentRoles(principalUuid, baseViewerContentId, ResourceTypes.CONTENT, 300, 'viewer', function() {
                loadContentRoles(principalUuid, baseManagerContentId, ResourceTypes.CONTENT, 300, 'manager', function() {
                    ...
                    // an aggregate to hold a unique set of all keys. Used to verify we get all the elements back
                    var aggregate = {};

                    // grab the first 250
                    AuthzAPI.getRolesForResourceType(principalUuid, ResourceTypes.CONTENT, null, 250, function(err, entries250_1) {
                        assert.ok(!err);
                        assert.equal(entries250_1.length, 250);

                        // get the next 250
                        AuthzAPI.getRolesForResourceType(principalUuid, ResourceTypes.CONTENT, entries250_1[249].uuid, 250, function(err, entries250_2) {
                            assert.ok(!err);
                            ...
                            AuthzAPI.getRolesForResourceType(principalUuid, ResourceTypes.CONTENT, entries250_2[249].uuid, 250, function(err, entries100) {
                                assert.ok(!err);
