        ...
        _updateContentLastModified(contentObj, function(err) {
            ...
            RolesAPI.setRoles(contentObj.contentId, newPermissions, function(err) {
                ...
                Cassandra.runBatchQuery(queries, function(err) {
                    ...
                    _updateLibraries(contentObj, oldLastModified, removedMembers, callback);
                });
            });
        });
    });
...
var updateContentProfile = module.exports.updateContentProfile = function(ctx, contentId, profileFields, callback) {
    ...
    canManage(ctx, contentId, function(err, canManage, contentObj) {
        ...
        for (var profileField in profileFields) {
            if (profileFields.hasOwnProperty(profileField)) {
                q.push("?=?");
                parameters.push(profileField);
                parameters.push(profileFields[profileField]);
            }
        }
        parameters.push(contentId);
        Cassandra.runQuery('UPDATE Content SET ' + q.join(', ') + ' WHERE contentId = ?', parameters, callback);
    });
};
