 ...
 * @Return      {Data}                                      Data of user
 */
OAE.tenantRouter.on('get', '/api/user/:userId/export/:exportType', function(req, res) {
    PrincipalsAPI.exportData(req.ctx, req.params.userId, req.params.exportType, function(err, zip) {
        if (err) {
            console.log(err)
            return res.status(err.code).send(err.msg);
        }

        var zipName = req.params.userId + Date.now() + ".zip";
    ...
    // Copy path from another code on oae (oae-content/lib/rest.js)
    // PrincipalsAPI.getPathExportData(req.ctx, req.params.userId, req.params.exportType, function(err, path) {
    //     console.log(path)
    ...
    //     res.setHeader('X-Sendfile', path);

    //     // Lighthttpd internal download
    //     res.setHeader('X-LIGHTTPD-send-file', path);
    ...
    //     res.setHeader('Content-Disposition', 'attachment; filename="30628887_2089261997753835_881891742331174912_n.png"');
    //     // res.setHeader('Content-Disposition', 'attachment; filename="u_d_Bk7Kxonnf1525245941680.zip"');

    //     // res.status(204).send(path);
        
    //     var filePath = ContentUtil.getStorageBackend(req.ctx, null).getRootDirectory();

    //     return res.download(filePath + '/c/d/H1/jw/De/Ah/H1jwDeAhz/rev-d-H1gjwwlA3f/30628887_2089261997753835_881891742331174912_n.png');
    // });
});
