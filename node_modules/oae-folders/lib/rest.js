...
OAE.tenantRouter.on('get', '/api/folder/managed', function(req, res) {
  FoldersAPI.getManagedFolders(req.ctx, function(err, results, nextToken) {
    ...
    return res.status(200).send({ results: results, nextToken: nextToken });
  });
});
...
OAE.tenantRouter.on('get', '/api/folder/:folderId', function(req, res) {
  FoldersAPI.getFullFolderProfile(req.ctx, req.params.folderId, function(
    err,
    folder,
  ) {
    if (err) {
...
OAE.tenantRouter.on('post', '/api/folder/:folderId', function(req, res) {
  FoldersAPI.updateFolder(req.ctx, req.params.folderId, req.body, function(
    err,
    updatedFolder,
  ) {
    if (err) {
 ...
 * @HttpResponse                    401                 You're not allowed to update this folder
 */
OAE.tenantRouter.on('post', '/api/folder/:folderId/contentvisibility', function(
  req,
  res,
) {
...
OAE.tenantRouter.on('delete', '/api/folder/:folderId', function(req, res) {
  ...
  FoldersAPI.deleteFolder(req.ctx, req.params.folderId, deleteContent, function(
    err,
    failedContent,
  ) {
    if (err) {
 ...
 */
OAE.tenantRouter.on('post', '/api/folder/:folderId/members', function(
  req,
  res,
) {
  ...
  FoldersAPI.setFolderPermissions(
    req.ctx,
    req.params.folderId,
    permissionUpdates,
    function(err) {
...
OAE.tenantRouter.on('get', '/api/folder/:folderId/members', function(req, res) {
  ...
  FoldersAPI.getFolderMembers(
    ...
    function(err, members, nextToken) {
      ...
      return res.status(200).send({ results: members, nextToken: nextToken });
    },
  );
});
 ...
 */
OAE.tenantRouter.on('get', '/api/folder/:folderId/invitations', function(
  req,
  res,
) {
  FoldersAPI.getFolderInvitations(req.ctx, req.params.folderId, function(
    err,
    invitations,
  ) {
    if (err) {
...
OAE.tenantRouter.on(
  ...
  function(req, res) {
    FoldersAPI.resendFolderInvitation(
      req.ctx,
      req.params.folderId,
      req.params.email,
      function(err) {
 ...
 */
OAE.tenantRouter.on('get', '/api/folder/library/:principalId', function(
  req,
  res,
) {
  ...
  FoldersAPI.getFoldersLibrary(
    ...
    req.query.start,
    limit,
    function(err, results, nextToken) {
      if (err) {
        return res.status(err.code).send(err.msg);
      }
...
OAE.tenantRouter.on(
  'delete',
  '/api/folder/library/:principalId/:folderId',
  function(req, res) {
    FoldersAPI.removeFolderFromLibrary(
      req.ctx,
      req.params.principalId,
      req.params.folderId,
      function(err) {
 ...
 */
OAE.tenantRouter.on('post', '/api/folder/:folderId/library', function(
  req,
  res,
) {
  var contentIds = OaeUtil.toArray(req.body.contentIds);
  FoldersAPI.addContentItemsToFolder(
    req.ctx,
    req.params.folderId,
    contentIds,
    function(err) {
 ...
 * @HttpResponse                            404                 The folder or one or more content items do not exist
 */
OAE.tenantRouter.on('delete', '/api/folder/:folderId/library', function(
  req,
  res,
) {
  var contentIds = OaeUtil.toArray(req.body.contentIds);
  FoldersAPI.removeContentItemsFromFolder(
    req.ctx,
    req.params.folderId,
    contentIds,
    function(err) {
 ...
 */
OAE.tenantRouter.on('get', '/api/folder/:folderId/messages', function(
  req,
  res,
) {
  var limit = OaeUtil.getNumberParam(req.query.limit, 10, 1, 25);
  FoldersAPI.getMessages(
    ...
    function(err, messages, nextToken) {
      ...
      return res.status(200).send({ results: messages, nextToken: nextToken });
    },
  );
});
 ...
 * @HttpResponse                404                 Could not find the specified folder
 */
OAE.tenantRouter.on('post', '/api/folder/:folderId/messages', function(
  req,
  res,
) {
...
OAE.tenantRouter.on(
  'delete',
  '/api/folder/:folderId/messages/:created',
  function(req, res) {
    FoldersAPI.deleteMessage(
      req.ctx,
      req.params.folderId,
      req.params.created,
      function(err, message) {
