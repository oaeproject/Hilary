...
var createFolder = module.exports.createFolder = function(ctx, displayName, description, visibility, roles, callback) {
    ...
    var managerIds = _.chain(roles)
        ...
        .filter(function(principalId) {
            return (roles[principalId] === FoldersConstants.roles.MANAGER);
        })
        .value();
    PrincipalsAPI.canManageAny(ctx, managerIds, function(err, canManageAny) {
        if (err && err.code !== 404) {
            ...
            roles[ctx.user().id] = FoldersConstants.roles.MANAGER;
        }

        var createFn = _.partial(FoldersDAO.createFolder, ctx.user().id, displayName, description, visibility);
        ResourceActions.create(ctx, roles, createFn, function(err, folder, members) {
            if (err) {
                return callback(err);
            }

            var changes = members.changes;

            // Insert the folder into the folders library of all the members
            var principalIds = _.keys(changes);
            FoldersFoldersLibrary.insert(principalIds, folder, function(err) {
                ...
                FoldersAPI.emit(FoldersConstants.events.CREATED_FOLDER, ctx, folder, changes);
...
var _deleteFolder = function(folder, callback) {
    ...
    AuthzAPI.getAuthzMembers(folder.groupId, null, 10000, function(err, memberRoles, nextToken) {
        ...
        AuthzAPI.updateRoles(folder.groupId, roleChanges, function(err) {
            ...
            // Remove the folder from each principal's folder library
...
var shareFolder = module.exports.shareFolder = function(ctx, folderId, principalIds, callback) {
    ...
    FoldersDAO.getFolder(folderId, function(err, folder) {
        ...
        ResourceActions.share(ctx, folder, principalIds, AuthzConstants.role.VIEWER, function(err, members) {
            if (err) {
                return callback(err);
            ...
            var allMemberIdsBeforeShare = _.keys(members.roles.before);
            ...
            OaeUtil.invokeIfNecessary(_testLibraryUpdateThreshold(folder), FoldersFoldersLibrary.update, allMemberIdsBeforeShare, folder, null, function(err, updatedFolder) {
                ...
                FoldersFoldersLibrary.insert(addedMemberIds, folder, function(err) {
                    ...
                    FoldersAPI.emit(FoldersConstants.events.UPDATED_FOLDER_MEMBERS, ctx, folder, members.changes, addedMemberIds, [], []);
                    return callback();
                });
            });
        });
...
var setFolderPermissions = module.exports.setFolderPermissions = function(ctx, folderId, changes, callback) {
    ...
    FoldersDAO.getFolder(folderId, function(err, folder) {
        ...
        ResourceActions.setRoles(ctx, folder, changes, function(err, members) {
            if (err) {
                return callback(err);
            }
            ...
            FoldersFoldersLibrary.insert(addedMemberIds, folder, function(err) {
                ...
                FoldersFoldersLibrary.remove(removedMemberIds, folder, function(err) {
                    ...
                    var memberIdsAfterUpdate = _.keys(members.roles.after);
                    OaeUtil.invokeIfNecessary(_testLibraryUpdateThreshold(folder), FoldersFoldersLibrary.update, memberIdsAfterUpdate, folder, null, function(err) {
                        ...
                        FoldersAPI.emit(FoldersConstants.events.UPDATED_FOLDER_MEMBERS, ctx, folder, members.changes, addedMemberIds, updatedMemberIds, removedMemberIds);
                        return callback();
                    });
                });
            });
...
var removeFolderFromLibrary = module.exports.removeFolderFromLibrary = function(ctx, principalId, folderId, callback) {
    ...
    FoldersDAO.getFolder(folderId, function(err, folder) {
        ...
        PrincipalsDAO.getPrincipal(principalId, function(err, principal) {
            ...
            AuthzPermissions.canRemoveRole(ctx, principal, folder, function(err, changes, roles) {
                if (err) {
                    return callback(err);
                }

                // All validation checks have passed, finally persist the role change and update the library
                AuthzAPI.updateRoles(folder.groupId, changes, function(err) {
                    if (err) {
                        return callback(err);
                    }
                    ...
                    FoldersFoldersLibrary.remove([principalId], folder, function(err) {
                        ...
                        FoldersAPI.emit(FoldersConstants.events.UPDATED_FOLDER_MEMBERS, ctx, folder, changes, [], [], [principalId]);
                        return callback();
                    });
                });
            });
...
var _getExistingPrincipals = function(principalIds, fields, callback) {
    PrincipalsDAO.getPrincipals(principalIds, fields, function(err, principals) {
        if (err) {
            ...
            return callback({'code': 400, 'msg': 'One or more principals did not exist'});
        }
