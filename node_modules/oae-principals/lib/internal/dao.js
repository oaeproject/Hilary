 ...
 * @param  {Object}      callback.err        An error that occurred, if any
 */
var updateLastLogin = module.exports.updateLastLogin = function(user, date, callback) {
    Cassandra.runQuery('UPDATE "Principals" SET "lastLogin" = ? WHERE "principalId" = ?', [date, user.id], function(err) {
        if (err) {
            return callback(err);
        }
        return callback(null);
