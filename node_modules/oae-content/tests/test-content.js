...
describe('Content', function() {
    ...
    before(function(callback) {
        ...
        RestAPI.User.getMe(camAdminRestContext, function(err, meObj) {
            // When covering, the tests are run from ~/Hilary/target/* which means all files
            // would be written in ~/Hilary/files.
            // We set the local files dir to the correct path so we don't mess up our own codebase.
            if (process.env['OAE_COVERING']) {
                var localDir = path.join(__dirname, '..', '..', '..', '..', '..', 'files');
                RestAPI.Config.setConfig(camAdminRestContext, null, 'oae-content/storage/local-dir', localDir, function() {
                    callback();
                });
            } else {
                callback();
    ...
    /**
     * @return {Stream}     A stream that points to a file that can be uploaded.
     */
        ...
        it ('verify download content', function(callback) {
            setUpUsers(function(contexts) {
                ...
                RestAPI.Content.createFile(contexts['nicolaas'].restContext, 'Test Content 1', 'Test content description 1', 'private', getFileStream,  [], [], function(err, contentObj) {
                    ...
                    // In the tests we're using local storage, so this should result in a 200 with the link as a body.
                    RestAPI.Content.download(contexts['nicolaas'].restContext, contentObj.contentId, false, function(err, body, response) {
                        var headerKeys = _.keys(response.headers);
                        assert.ok(!err);
                        assert.ok(headerKeys.indexOf('x-accel-redirect') !== -1)

                        // Try downloading it as Simon
                        RestAPI.Content.download(contexts['simon'].restContext, contentObj.contentId, false, function(err, body) {
                        ...
                        RestAPI.Content.createFile(contexts['nicolaas'].restContext, 'Test Content 2', 'Test content description 2', 'public', getFileStream, [], [], function(err, contentObj) {
                            ...
                            RestAPI.Content.updateFileBody(contexts['nicolaas'].restContext, contentObj.contentId, null, function(err, revision) {
                                ...
                                RestAPI.Content.updateFileBody(contexts['nicolaas'].restContext, contentObj.contentId, 'haha, no actual file body', function(err) {
                                    ...
                                    RestAPI.Content.updateFileBody(contexts['simon'].restContext, contentObj.contentId, null, function(err) {
                                        assert.ok(err);
            ...
            setUpUsers(function(contexts) {
                RestAPI.Content.createFile(contexts['nicolaas'].restContext, 'Test Content 2', 'Test content description 2', 'public', getFileStream, [], [], function(err, contentObj) {
                    ...
                    RestAPI.Content.getRevisions(contexts['nicolaas'].restContext, contentObj.contentId, null, null, function(err, revisions) {
                        ...
                        assert.equal(revisions[0].createdBy.username, contexts['nicolaas'].restContext.userId)
    ...
    describe('File previews', function() {
         ...
         * @param {Object}      callback.contexts   contexts object as returned by `setUpUsers`.
         * @param {Object}      callback.content    Content object as returned by `createFile`.
         * @param {Object}      callback.previews   Previews object as returned by `getPreviewItems`.
         */
        var createPreviews = function(callback) {
            setUpUsers(function(contexts) {
                RestAPI.Content.createFile(contexts['nicolaas'].restContext, 'Test Content 2', 'Test content description 2', 'private', getFileStream, [], [], function(err, contentObj) {
                    ...
                    var items = [getFileStream, getSakaiLogoStream];
                    RestAPI.Content.addPreviewItems(camAdminRestContext, contentObj.contentId, items, null, function(err) {
                        assert.ok(!err);
                        ...
                        RestAPI.Content.getPreviewItems(contexts['nicolaas'].restContext, contentObj.contentId, function(err, previews) {
                            ...
                            callback(contexts, contentObj, previews);
                        });
                    });
                });
            });
        ...
        it('verify uploading a preview', function(callback) {
            createPreviews(function(contexts, contentObj, previews) {
                // Only admins should be allowed to create previews.
                var items = [getFileStream, getSakaiLogoStream];
                RestAPI.Content.addPreviewItems(contexts['nicolaas'].restContext, contentObj.contentId, items, null, function(err) {
                    assert.equal(err.code, 401);

                    // Download one.
                    RestAPI.Content.downloadPreviewItem(contexts['nicolaas'].restContext, contentObj.contentId, previews.files[0], previews.signature.signature, previews.signature.expires, function(err, body, response) {
                        assert.ok(!err);
                        ...
                        RestAPI.Content.downloadPreviewItem(contexts['simon'].restContext, contentObj.contentId, previews.files[0], 'wrong', Date.now()+6000, function(err, body, response) {
                            assert.equal(err.code, 401);
                            assert.ok(!body);
                            callback();
                        });
        ...
        it('verify preview download parameter validation', function(callback) {
            createPreviews(function(contexts, contentObj, previews) {
                // Missing parameters.
                RestAPI.Content.downloadPreviewItem(contexts['nicolaas'].restContext, contentObj.contentId, previews.files[0], null, previews.signature.expires, function(err, body, response) {
                    assert.ok(err.code, 400);
                    assert.ok(!body);

                    RestAPI.Content.downloadPreviewItem(contexts['nicolaas'].restContext, contentObj.contentId, previews.files[0], previews.signature.signature, null, function(err, body, response) {
                        assert.ok(err.code, 400);
                        assert.ok(!body);

                        // Bad signature.
                        RestAPI.Content.downloadPreviewItem(contexts['nicolaas'].restContext, contentObj.contentId, previews.files[0], 'totally wrong', previews.signature.expires, function(err, body, response) {
                            assert.ok(err.code, 401);
                            assert.ok(!body);
                ...
                RestAPI.Content.createFile(contexts['nicolaas'].restContext, 'Test Content 2', 'Test content description 2', 'public', getFileStream, [], [], function(err, contentObj) {
                    ...
                    RestAPI.Content.getRevisions(contexts['nicolaas'].restContext, contentObj.contentId, null, 1000, function(err, revisions) {
                        ...
                        RestAPI.Content.deleteContent(contexts['nicolaas'].restContext, contentObj.contentId, function(err) {
                            ...
                            // Check if the file is removed on disk.
                            var checkFile = function() {
                                var revision = revisions.shift();
                                var path = Config.getValue('camtest', 'storage', 'local-dir') + '/' + revision.uri.split(':')[1];
                            ...
                            // Deleting the actual file bodies happens async from the request
                            // give it some time before we start checking.
