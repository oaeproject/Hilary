...
module.exports = function() {
    ...
    strategy.getPassportStrategy = function(tenant) {
        // Server config
        var host = AuthenticationConfig.getValue(tenant.alias, 'ldap', 'host');
        var port = AuthenticationConfig.getValue(tenant.alias, 'ldap', 'port');
        var ssl = AuthenticationConfig.getValue(tenant.alias, 'ldap', 'ssl');
        ...
        var protocol = (ssl ? 'ldaps' : 'ldap');

        var options = {
            'server': {
                'url': util.format('%s://%s:%s', protocol, host, port),
                'adminDn': adminDn,
        ...
        var passportStrategy = new LDAPStrategy(options, function(profile, done) {
            ...
            AuthenticationAPI.getOrCreateUser(ctx, AuthenticationConstants.providers.LDAP, externalId, displayName, opts, done);
        });
