...
var setUpGroups = function(contexts, callback) {
    var r = Math.floor(Math.random() * 10000);
    var groups = [new Group(contexts[4].tenant(), null, 'public', 'OAE Team ' + r, 'OAE Team Group'),
                  new Group(contexts[0].tenant(), null, 'public', 'UI Dev Team ' + r, 'UI Dev Group'),
                  new Group(contexts[3].tenant(), null, 'public', 'Back-end Dev Team ' + r, 'Back-end Dev Group')];
    // Create UI Dev Group
    PrincipalsAPI.createGroup(contexts[0], groups[0].title, groups[0].description, function(err, principal_id) {
        test.ok(!err);
        ...
        PrincipalsAPI.addGroupMember(contexts[0], groups[0].principal_id, contexts[2].user().id, 'member', function(err) {
            ...
            PrincipalsAPI.createGroup(contexts[3], groups[1].title, groups[1].description, function(err, principal_id) {
                test.ok(!err);
                ...
                PrincipalsAPI.addGroupMember(contexts[3], groups[1].principal_id, contexts[1].user().id, 'member', function(err) {
                    ...
                    // Create OAE Team Group
                     PrincipalsAPI.createGroup(contexts[4], groups[2].title, groups[2].description, function(err, principal_id) {
                        test.ok(!err);
...
// TODO: Test setPermissions on groups
...
// TODO: Multiple sharing with groups
...
// TODO: Public content
// Create users, groups with members and subgroup as member, subgroup with members
// Create a public piece of content
// 1) Set permissions on content --> Make UI dev team member, make Anthony a manager, make Simon a member
// Check that UI Dev Team, Bert, Nico and Simon have member access
// Check that it shows in UI Dev Team's library, not in Bert and Nico's library
// Check that it doesn't show in OAE Team's and Back-end team's library
// Check that Back-End Team and Branden don't have access
// 2) Share with the OAE Team group
// Check that everyone has got access
// Check that it shows in OAE Team and UI Dev team's library and not in the member's library
// 3) Make Back-end team manager
// Check that Simon and Branden are manager, check that Stuart is not a manager
module.exports.testPublicContentGroupAccess = function() {
    test.expect(1);
