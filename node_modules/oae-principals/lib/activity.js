 ...
 */
PrincipalsAPI.on(PrincipalsConstants.events.CREATED_GROUP, (ctx, group, memberChangeInfo) => {
    ...
    const activitySeed = new ActivityModel.ActivitySeed(PrincipalsConstants.activity.ACTIVITY_GROUP_CREATE, millis, ActivityConstants.verbs.CREATE, actorResource, objectResource);
 ...
 */
PrincipalsAPI.on(PrincipalsConstants.events.UPDATED_GROUP, (ctx, newGroup, oldGroup) => {
    const millis = Date.now();
    const actorResource = new ActivityModel.ActivitySeedResource('user', ctx.user().id, {'user': ctx.user()});
    const objectResource = new ActivityModel.ActivitySeedResource('group', newGroup.id, {'group': newGroup});
 ...
 */
PrincipalsAPI.on(PrincipalsConstants.events.UPDATED_GROUP_MEMBERS, (ctx, group, oldGroup, memberChangeInfo, opts) => {
    if (opts.invitation) {
    ...
    _.each(addedMemberIds, (memberId) => {
        ...
        const activitySeed = new ActivityModel.ActivitySeed(PrincipalsConstants.activity.ACTIVITY_GROUP_ADD_MEMBER, millis, ActivityConstants.verbs.ADD, actorResource, objectResource, targetResource);
        ActivityAPI.postActivity(ctx, activitySeed);
    });
    ...
    _.each(updatedMemberIds, (memberId) => {
        const objectResourceType = PrincipalsUtil.isGroup(memberId) ? 'group' : 'user';
        const objectResource = new ActivityModel.ActivitySeedResource(objectResourceType, memberId);
        const activitySeed = new ActivityModel.ActivitySeed(PrincipalsConstants.activity.ACTIVITY_GROUP_UPDATE_MEMBER_ROLE, millis, ActivityConstants.verbs.UPDATE, actorResource, objectResource, targetResource);
        ActivityAPI.postActivity(ctx, activitySeed);
    });
});
 ...
 */
PrincipalsAPI.on(PrincipalsConstants.events.JOINED_GROUP, (ctx, group, oldGroup, memberChangeInfo) => {
    const millis = Date.now();
    const actorResource = new ActivityModel.ActivitySeedResource('user', ctx.user().id, {'user': ctx.user()});
    const objectResource = new ActivityModel.ActivitySeedResource('group', group.id, {group});
...
const _userInternalTransformer = function(ctx, activityEntities, callback) {
    ...
    _.keys(activityEntities).forEach((activityId) => {
        ...
        _.keys(activityEntities[activityId]).forEach((entityId) => {
            const entity = activityEntities[activityId][entityId];
            transformedActivityEntities[activityId][entityId] = PrincipalsUtil.transformPersistentUserActivityEntityToInternal(ctx, entityId, entity.user);
        });
    });
    return callback(null, transformedActivityEntities);
};
...
const _groupInternalTransformer = function(ctx, activityEntities, callback) {
    ...
    _.keys(activityEntities).forEach((activityId) => {
        ...
        _.keys(activityEntities[activityId]).forEach((entityId) => {
            const entity = activityEntities[activityId][entityId];
            transformedActivityEntities[activityId][entityId] = PrincipalsUtil.transformPersistentGroupActivityEntityToInternal(ctx, entityId, entity.group);
        });
    });
    return callback(null, transformedActivityEntities);
};
