 ...
 * @param  {Object}     callback.err    An error that occurred, if any
 */
var fullyDeletePrincipal = module.exports.fullyDeletePrincipal = function(user, login, callback) {
    ...
    if (loginId && loginId.tenantAlias && loginId.provider && loginId.externalId) {
        queries.push({'query': 'DELETE FROM "AuthenticationLoginId" WHERE "loginId" = ?', 'parameters': [loginId.tenantAlias + ':' + loginId.provider + ':' + loginId.externalId]});
    }
...
var createArchivedUser = module.exports.createArchivedUser = function(alias, createdUserId, callback) {
    ...
    Cassandra.runQuery(query, parameters, function (err) {
        ...
        var userArchive = {'tenantAlias': alias, 'archiveId': createdUserId};
 ...
 * @param  [User]           callback.users      The list of users for the given tenancy
 */
var addDataToArchive = module.exports.addDataToArchive = function(archiveId, principalId, resourceId, date, callback) {

    var stringResource = resourceId.join();

    Cassandra.runQuery('INSERT INTO "DataArchive" ("archiveId", "principalId", "resourceId", "deletionDate", "fullyDeleted") VALUES (?, ?, ?, ?, ?)', [archiveId, principalId, stringResource, date, 'false'], function(err, rows) {
        if (err) {
            return callback(err);
        }
        return callback();
...
/**
 * Update the status from the DataArchive
 * In this way, we put the user has already fully deleted and we wont try to delete it a second time.
 *
 * @param  {String}         archiveId           The archive id ot the principal tenant 
 * @param  {String}         principalId         The principal id of the removed user
 * @param  {String}         deletionDate        The date of the deletion of the user
 * @param  {Function}       callback            Standard callback function
 * @param  {Object}         callback.err        An error that occurred, if any
 * @param  [User]           callback.users      The list of users for the given tenancy
 */
var updateStatusInDataArchive = module.exports.updateStatusInDataArchive = function(archiveId, principalId, deletionDate, callback) {
    
    Cassandra.runQuery('UPDATE "DataArchive" SET "fullyDeleted" = ? WHERE "archiveId" = ? AND "principalId" = ?', ['true', archiveId, principalId], function(err, rows) {
        if (err) {
            return callback(err);
        }
        return callback();
 ...
 * @param  {Function}       callback                Standard callback function
 * @param  {Object}         callback.err            An error that occurred, if any
 * @param  [Object]         callback.userArchive    The archive id ot the tenant
 */
 ...
 */
var getExpiredUser = module.exports.getExpiredUser = function(actualDate, callback) {
    ...
    Cassandra.runQuery('SELECT * FROM "DataArchive"', [], function(err, rows) {
        ...
        var users = _.map(rows, Cassandra.rowToHash);
        
        users = _.filter(users, function(user) { return (user.fullyDeleted == 'false') && (new Date(user.deletionDate) < actualDate); });
        return callback(null, users);
    });
};
 ...
 * @param  {Object}         callback.err        An error that occurred, if any
 */
var updateUserArchiveFlag = module.exports.updateUserArchiveFlag = function(principalId, callback) {
    Cassandra.runQuery('UPDATE "Principals" SET "isUserArchive" = ? WHERE "principalId" = ?', ['true', principalId], function(err, rows) {
        if (err) {
            return callback(err);
        }
        return callback();
