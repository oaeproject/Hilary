 ...
 * @param  {Object[]}               routes                  The list of routes to filter according to propagation rules. Each route is an object containing the `resourceId` and `streamType` for the activity. It will also include whether or not the stream is transient
 ...
 * @param  {Object[]}               entityRoutes            The routes that have been specified by the entity router. Each route is an object containing the `resourceId` and `streamType` for the activity. It will also include whether or not the stream is transient
 * @param  {Function}               callback                Standard callback function
 * @param  {Object}                 callback.err            An error that occurred, if any
 * @param  {Object[]}               callback.includedRoutes The routes to which the propagation rules indicate are safe to receive the entity. Each route is an object containing the `resourceId` and `streamType` for the activity. It will also include whether or not the stream is transient
 * @param  {Object[]}               callback.excludedRoutes The routes to which the propagation rules indicate are unsafe to receive the entity. Each route is an object containing the `resourceId` and `streamType` for the activity. It will also include whether or not the stream is transient
 * @api private
 */
 ...
 * @param  {Function}               callback                Standard callback function
 * @param  {Object}                 callback.err            An error that occurred, if any
 * @param  {Object[]}               callback.includedRoutes The routes to which the propagation rule indicates are safe to receive the entity. Each route is an object containing the `resourceId` and `streamType` for the activity. It will also include whether or not the stream is transient
 * @param  {Object[]}               callback.excludedRoutes The routes to which the propagation rule indicates are unsafe to receive the entity. Each route is an object containing the `resourceId` and `streamType` for the activity. It will also include whether or not the stream is transient
 * @api private
 */
var _applyPropagation = function(propagation, routes, associationsCtx, entity, entityRoutes, callback) {
    ...
    if (propagation.type === ActivityConstants.entityPropagation.ALL) {
        ...
        associationsCtx.get('self', function(err, associations) {
            // Although the self association will usually only return 1 route we allow for more than 1 route and index the associations into a hash to make the next loop O(n) instead of O(n^2)
            // If the entity doesn't have a self association the `associations` array will be undefined which will leave the `associationRoutesHash` empty
            // This will exclude all the routes for this propagation
            var associationRoutesHash = {};
            _.each(associations, function(associationRoute) { associationRoutesHash[associationRoute] = true; });

            // Split the routes into those who are in the self association and those who are not
            _.each(routes, function(route) {
                if (associationRoutesHash[route.resourceId] || (route.resourceId === entity['oae:id'])) {
                    includedRoutes.push(route);
                } else {
                    excludedRoutes.push(route);
                }
...
var _routeAssociations = function(associationsCtx, associationNames, callback, _routes) {
    ...
    var exclusionAssociation = false;
    if (associationName[0] === '^') {
