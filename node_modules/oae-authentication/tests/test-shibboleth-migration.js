...
describe('Shibboleth Migration', function() {
  ...
  before(function(callback) {
    ...
    fakeStream.on('error', function(err) {
      log().error(
        { err: err },
        'Error occurred when writing to the warnings file',
      );
    });
  ...
  it('verify new Shibboleth logins are created', function(callback) {
    TestsUtil.generateTestUsers(camAdminRestContext, 20, function(err, users) {
      assert.ok(!err);
      ...
      RestAPI.Tenants.getTenant(camAdminRestContext, null, function(
        ...
        Cassandra.runBatchQuery(queries, function(err) {
          ...
          // Run the migration
          ShibbolethMigrator.doMigration(tenantAlias, csvStream, function(
            err,
            errors,
          ) {
            assert.ok(!err);
  ...
  it('verify no Shibboleth logins are created for users without Google IDs', function(callback) {
    TestsUtil.generateTestUsers(gtAdminRestContext, 20, function(err, users) {
      assert.ok(!err);
      ...
      var googleUsers = _.sample(users, 10);
      var nonGoogleUsers = _.difference(users, googleUsers);

      RestAPI.Tenants.getTenant(gtAdminRestContext, null, function(
        err,
        tenant,
      ) {
        assert.ok(!err);
        ...
        Cassandra.runBatchQuery(queries, function(err) {
          ...
          ShibbolethMigrator.doMigration(tenantAlias, csvStream, function(
            ...
            _assertHaveShibbolethLoginIds(tenantAlias, googleUsers, function() {
              _assertHaveNoShibbolethLoginIds(
                tenantAlias,
                nonGoogleUsers,
                function() {
                  return callback();
