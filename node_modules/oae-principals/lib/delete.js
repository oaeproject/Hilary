 ...
 */
const registerGroupDeleteHandler = module.exports.registerGroupDeleteHandler = function(name, handler) {
 ...
 */
const registerGroupRestoreHandler = module.exports.registerGroupRestoreHandler = function(name, handler) {
 ...
 */
const invokeGroupDeleteHandlers = module.exports.invokeGroupDeleteHandlers = function(group) {
 ...
 * @param  {Group}  group   The group for which to invoke the delete handlers
 */
const invokeGroupRestoreHandlers = module.exports.invokeGroupRestoreHandlers = function(group) {
 ...
 */
const whenDeletesComplete = module.exports.whenDeletesComplete = function(callback) {
...
var _invokeGroupHandlers = function(log, handlers, group) {
    ...
    AuthzAPI.getPrincipalMembershipsGraph(group.id, (err, membershipsGraph) => {
        if (err) {
            return log().error({err, 'groupId': group.id}, 'An unexpected error occurred while getting the authz memberships graph');
        }

        AuthzAPI.getAuthzMembersGraph([group.id], (err, membersGraph) => {
            if (err) {
                return log().error({err, 'groupId': group.id}, 'An unexpected error occurred while getting the authz members graph');
            }
...
var _invokeHandlers = function(log, handlers, principal /* , args... */) {
    ...
    _.each(handlers, (handler, name) => {
        ...
        const thisHandlerArgs = handlerArgs.concat((errs) => {
            ...
            if (!_.isEmpty(errs)) {
                return log().error({'principalId': principal.id, 'handlerName': name, errs}, 'Error(s) occurred while trying to process a handler');
            }
