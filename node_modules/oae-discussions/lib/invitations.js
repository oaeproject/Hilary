 ...
 */
ResourceActions.when(ResourceConstants.events.ACCEPTED_INVITATION, function(
  ctx,
  invitationHashes,
  ...
  DiscussionsDAO.getDiscussionsById(discussionIds, null, function(
    ...
    _.each(discussions, function(discussion) {
      var invitationHash = _.findWhere(invitationHashes, {
      ...
      var invitation = Invitation.fromHash(
        invitationHash,
        discussion,
        inviterUser,
      );
      var memberChangeInfo = memberChangeInfosByResourceId[discussion.id];
...
DiscussionsAPI.when(DiscussionsConstants.events.DELETED_DISCUSSION, function(
  ...
  AuthzInvitationsDAO.deleteInvitationsByResourceId(discussion.id, function(
    err,
  ) {
    if (err) {
      log().warn(
...
var _isDiscussionId = function(discussionId) {
  return (
    AuthzUtil.isResourceId(discussionId) && discussionId.indexOf('d:') === 0
  );
};
