...
describe('Groups', function() {
    ...
    describe('Get group members', function() {
        ...
        /**
         * Test that verifies that group members are page appropriately and are sorted by rank
         */
        it('verify paging', function(callback) {
            // Create 10 users and get pages of 4.
            TestsUtil.generateTestUsers(camAdminRestContext, 10, function(err, createdUsers, userWithPicture) {
                assert.ok(!err);
                ...
                RestAPI.Group.createGroup(johnRestContext, 'Group title', 'Group description', 'private', 'request', [], userIds, function(err, newGroup) {
                    ...
                    // Give one user a picture, this should rank them higher in the members list
                    PrincipalsTestUtil.uploadAndCropPicture(userWithPicture.restContext, userWithPicture.user.id, _getPictureStream, {'x': 10, 'y': 10, 'width': 200}, function() {
                        ...
                        RestAPI.Group.getGroupMembers(johnRestContext, newGroup.id, null, 4, function(err, members) {
                            ...
                            // Add members to the allMemberIds list
                            allMemberIds = allMemberIds.concat(_.chain(members.results).pluck('profile').pluck('id').value());
                            ...
                            RestAPI.Group.getGroupMembers(johnRestContext, newGroup.id, members.nextToken, 4, function(err, members) {
                                ...
                                RestAPI.Group.getGroupMembers(johnRestContext, newGroup.id, members.nextToken, 4, function(err, members) {
                                    ...
                                    assert.strictEqual(members.nextToken, null);

                                    allMemberIds = allMemberIds.concat(_.chain(members.results).pluck('profile').pluck('id').value());

                                    // Ensure the first entry is the member with a picture, the
                                    // second entry is the manager without a picture, the rest are
                                    // in reverse lexigraphical order because Cassandra naturally
                                    // sorts them that way
                                    assert.strictEqual(allMemberIds[0], userWithPicture.user.id);
                                    assert.strictEqual(allMemberIds[1], johnRestContext.id);
                                    assert.deepEqual(allMemberIds.slice(2), allMemberIds.slice(2).sort().reverse());
