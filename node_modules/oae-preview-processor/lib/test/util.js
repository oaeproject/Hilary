 ...
 */
var purgePreviewsQueue = (module.exports.purgePreviewsQueue = function(
  callback,
) {
 ...
 * @throws {AssertionError}                 Thrown if an error occurs while purging the queue
 */
var purgeRegeneratePreviewsQueue = (module.exports.purgeRegeneratePreviewsQueue = function(
  callback,
) {
  // Unbind the old listener (if any) and bind a new one, so we can purge the queue
 ...
 * @throws {AssertionError}                 Thrown if an error occurs while purging the queue
 */
var purgeFoldersPreviewsQueue = (module.exports.purgeFoldersPreviewsQueue = function(
  callback,
) {
  // Unbind the old listener (if any) and bind a new one, so we can purge the queue
  TaskQueue.unbind(PreviewConstants.MQ.TASK_GENERATE_FOLDER_PREVIEWS, function(
    err,
  ) {
    assert.ok(!err);
    ...
    TaskQueue.bind(
      ...
      function(err) {
        ...
        // Purge anything that is in the queue
        MQ.purge(PreviewConstants.MQ.TASK_GENERATE_FOLDER_PREVIEWS, function(
          err,
        ) {
          assert.ok(!err);
