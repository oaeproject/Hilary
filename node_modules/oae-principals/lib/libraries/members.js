...
PrincipalsAPI.on(PrincipalsConstants.events.UPDATED_USER, function(ctx, updatedUser, oldUser) {
    // If there is no change in state of their profile picture, we don't have to make any updates
 ...
 */
PrincipalsAPI.when(PrincipalsConstants.events.UPDATED_GROUP_MEMBERS, function(ctx, group, memberChanges, newMemberIds, updatedMemberIds, removedMemberIds, callback) {
 ...
 */
PrincipalsAPI.when(PrincipalsConstants.events.JOINED_GROUP, function(ctx, group, joinRole, callback) {
    var entry = {
        'id': group.id,
 ...
 * have to do this with a synchronous `when` handler because the user generally won't care if the
 * members library update isn't finished within seconds
 */
PrincipalsAPI.on(PrincipalsConstants.events.LEFT_GROUP, function(ctx, group, leaveRole) {
    var entry = {
        'id': group.id,
