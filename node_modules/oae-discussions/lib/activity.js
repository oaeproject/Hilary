...
ActivityAPI.registerActivityEntityAssociation('discussion', 'members-by-role', function(associationsCtx, entity, callback) {
    ActivityUtil.getAllAuthzMembersByRole(entity['oae:id'], callback);
});
...
ActivityAPI.registerActivityEntityAssociation('discussion', 'message-contributors', function(associationsCtx, entity, callback) {
    MessageBoxAPI.getRecentContributions(entity['oae:id'], null, 100, callback);
});

/*!
 * Register an association that presents the discussion nessage, we return the ID of the discussion
 * as we're currently not storing activities PER comment
 */
...
/*!
 * Register an association that presents all the indirect members of the discussion to which the comment belongs
 */
ActivityAPI.registerActivityEntityAssociation('discussion-message', 'members', function(associationsCtx, entity, callback) {
    // It is possible that the parent discussion is not actually associated to the activity (e.g., an activity where a comment is edited),
    // therefore we only use the discussion's members if they exist in the associationsCtx
    associationsCtx.getSession().getByEntityId('discussion', entity.discussionId, 'members', function(err, members) {
        if (err) {
            return callback(err);
        } else if (members) {
