...
ConfigAPI.eventEmitter.on('update', function(tenantAlias) {
...
var cacheWidgetManifests = function() {
  ...
  readdirp({
    ...
    .on('warn', function(err) {
      log().warn({ err: err }, 'A non-fatal error occured whilst caching a widget manifest');
    })
    .on('error', function(err) {
      log().error({ err: err }, 'A fatal error occured whilst caching a widget manifest');
    });
};
 ...
 */
var uploadLogoFile = (module.exports.uploadLogoFile = function(ctx, file, tenantAlias, callback) {
  tenantAlias = tenantAlias || ctx.tenant().alias;
...
var _cacheI18nKeysInDirectory = function(directory, directoryFilter, callback) {
  ...
  readdirp({
    ...
    .on('warn', function(err) {
      log().warn({ err: err }, 'A non-fatal error occured whilst reading the i18n bundles');
    })
    .on('error', function(err) {
      log().error({ err: err }, 'A fatal error occured whilst reading the i18n bundles');
    })
    .on('end', done);
};
  ...
  _.extend(data.util, {
    ...
    i18n: {
      ...
      formatDate: function(date, dateFormat, timezone) {
        ...
        var formatted = Globalize.format(kludgeDate, dateFormat, globalizeLocale);
