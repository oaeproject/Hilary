...
// TODO: Activity
// TODO: Implement removing collection from library
// TODO: Implement gathering stats for content item visibility when collection visibility is changed from open to less open
...
var shareCollection = module.exports.shareCollection = function(ctx, collectionId, principalIds, callback) {
    ...
    _getCollection(collectionId, function(err, collection) {
        ...
        // Determine which members are actually newly added to this collection. We can only add those
        AuthzAPI.computeMemberRolesAfterChanges(collection.groupId, roleChanges, function(err, membershipAfterChanges, newMemberIds) {
            if (err) {
                return callback(err);
            ...
            CollectionsAuthz.canShareCollection(ctx, collection, newMemberIds, function(err, canShare, illegalPrincipalIds) {
                if (err) {
                    return callback(err);
                } else if (!_.isEmpty(illegalPrincipalIds)) {
                ...
                AuthzAPI.updateRoles(collection.groupId, roleChanges, function(err) {
                    ...
                    // Update the libraries that contain this collection, but only if we have not already updated it within the threshold. We provide `null` as the old last modified
                    // timestamp to trigger the update function to update the collection timestamp for us
                    OaeUtil.invokeIfNecessary(_testLibraryUpdateThreshold(collection), CollectionsCollectionsLibrary.update, allMemberIdsBeforeShare, collection, null, function(err, updatedCollection) {
                        if (err) {
                            log(ctx).warn({
                                'err': err,
                                'collectionId': collection.id,
                                'numberOfPrincipalIds': allMemberIdsBeforeShare.length
                            }, 'An error occurred while updating collection in member collection libraries after being shared');
                        }

                        // If the collection was updated for user libraries
                        collection = updatedCollection || collection;

                        // Insert the collection into the user libraries that we just shared with
                        CollectionsCollectionsLibrary.insert(newMemberIds, collection, function(err) {
                            if (err) {
                                ...
                                }, 'An error occurred while inserting a collection to in member collection libraries after being shared');
                            }

                            CollectionsAPI.emit(CollectionsConstants.events.UPDATED_COLLECTION_MEMBERS, ctx, collection, roleChanges, newMemberIds);
                            return callback();
                        });
                    });
                });
