...
describe('Group REST endpoints', function() {
    ...
    var loginId = function(tenantId, externalId) {
        return new LoginId('local', tenantId, externalId);
    }

    before(function(callback) {
        var r = Math.floor(Math.random()*100000);
        tenantCtx = new Context(global.oaeTests.tenants.cam, null);
        var tenantAlias = tenantCtx.tenant().alias;
        PrincipalAPI.createUser(tenantCtx, loginId(tenantAlias, 'john-' + r), 'password', 'John Doe', {}, function(err, id) {
            johnCtx = new RestAPI.Context(global.oaeTests.tenants.cam.baseUrl, id, 'john-' + r, 'password');
            callback();
        });
    });
    ...
    describe('#createGroup()', function() {
        ...
        it('verify missing alias', function(callback) {
            RestAPI.Groups.createGroup(johnCtx, undefined, 'Group name', 'Group description', 'public', 'yes', [], [], function(err, data) {
        ...
        it('verify alias with colons', function(callback) {
            RestAPI.Groups.createGroup(johnCtx, 'sneaky:alias', 'Group name', 'Group description', 'public', 'yes', [], [], function(err, data) {
                assert.equal(err.code, 400);
                callback();
            });
        });
        ...
        it('verify that missing visibility uses tenant default', function(callback) {
            var r = Math.floor(Math.random()*100000);
            RestAPI.Groups.createGroup(johnCtx, 'group-' + r, 'Group title', 'Group description', undefined, 'yes', [], [], function(err, data) {
                assert.ok(!err, err);
        ...
        it('verify that missing joinable uses tenant default', function(callback) {
            var r = Math.floor(Math.random()*100000);
            RestAPI.Groups.createGroup(johnCtx, 'group-' + r, 'Group title', 'Group description', 'public', undefined, [], [], function(err, data) {
                assert.ok(!err);
                assert.equal(data.joinable, 'yes')
                callback();
            });
        });
        ...
        it('verify that missing description is accepted', function(callback) {
            var r = Math.floor(Math.random()*100000);
            RestAPI.Groups.createGroup(johnCtx, 'group-' + r, 'Group title', null, 'public', undefined, [], [], function(err, data) {
                assert.ok(!err);
        ...
        it('verify that members can be specified on group creation', function(callback) {
            ...
            var r = Math.floor(Math.random()*100000);
            var tenantAlias = tenantCtx.tenant().alias;
            PrincipalAPI.createUser(tenantCtx, loginId(tenantAlias, 'jack-' + r), 'password', 'John Doe', {}, function(err, jack) {
                assert.ok(!err);
                PrincipalAPI.createUser(tenantCtx, loginId(tenantAlias, 'jane-' + r), 'password', 'Jane Doe', {}, function(err, jane) {
                    assert.ok(!err);
                    RestAPI.Groups.createGroup(johnCtx, 'group-' + r, 'Group title', 'Group description', 'public', 'yes', [jane], [jack], function(err, data) {
                        assert.ok(!err);
        ...
        it('verify that group creation fails if an invalid userId is specified as manager', function(callback) {
            var r = Math.floor(Math.random()*100000);
            RestAPI.Groups.createGroup(johnCtx, 'group-' + r, 'Group title', 'Group description', 'public', 'yes', ['totally-invalid'], [], function(err, data) {
                assert.equal(err.code, 400);
                callback();
            });
        });
        ...
        it('verify that group creation fails if an unknown user is specified as manager', function(callback) {
            var r = Math.floor(Math.random()*100000);
            RestAPI.Groups.createGroup(johnCtx, 'group-' + r, 'Group title', 'Group description', 'public', 'yes', ['u:camtest:totally-unknown'], [], function(err, data) {
                assert.equal(err.code, 400);
                callback();
            });
        });
        ...
        it('verify that group creation succeeds given a valid request', function(callback) {
            var r = Math.floor(Math.random()*100000);
            RestAPI.Groups.createGroup(johnCtx, 'group-' + r, 'Group title', 'Group description', 'public', 'yes', [], [], function(err, data) {
                assert.ok(!err);
                callback();
            });
        });
    ...
    describe('#getGroup()', function() {
        ...
        it('verify isMember and isManager', function(callback) {
            ...
            PrincipalAPI.createUser(tenantCtx, loginId(tenantAlias, 'jack-' + r), 'password', 'John Doe', {}, function(err, jack) {
                assert.ok(!err);
                var jackCtx = new RestAPI.Context(global.oaeTests.tenants.cam.baseUrl, jack, 'jack-' + r, 'password');

                PrincipalAPI.createUser(tenantCtx, loginId(tenantAlias, 'jane-' + r), 'password', 'Jane Doe', {}, function(err, jane) {
                    assert.ok(!err);
                    var janeCtx = new RestAPI.Context(global.oaeTests.tenants.cam.baseUrl, jane, 'jane-' + r, 'password');

                    PrincipalAPI.createUser(tenantCtx, loginId(tenantAlias, 'joe-' + r), 'password', 'Jane Doe', {}, function(err, joe) {
                        assert.ok(!err);
                        var joeCtx = new RestAPI.Context(global.oaeTests.tenants.cam.baseUrl, joe, 'joe-' + r, 'password');
    ...
    describe('#getGroupMembers()', function() {
        it('verify paging', function(callback) {
            ...
            for (var i = 0; i < 10; i++) {
                ...
                PrincipalAPI.createUser(tenantCtx, loginId(tenantCtx.tenant().alias, 'random-user-' + r), 'password', 'John Doe', {}, userCreated);
            }
        });
    ...
    describe('#updateGroup()', function() {
        ...
        it('verify updating as a non-manager is not allowed', function(callback) {
            ...
            RestAPI.Groups.createGroup(johnCtx, 'group-' + r, 'Group title', 'Group description', 'private', 'request', [], [], function(err, newGroup) {
                assert.ok(!err);
                PrincipalAPI.createUser(tenantCtx, loginId(tenantCtx.tenant().alias, 'jack-' + r), 'password', 'John Doe', {}, function(err, jack) {
                    assert.ok(!err);
                    var jackCtx = new RestAPI.Context(global.oaeTests.tenants.cam.baseUrl, jack, 'jack-' + r, 'password');
