        ...
        _cleanUpCSVFile(userCSV, function() {
            ...
            var processUser = function(data) {
                ...
                require('oae-authentication').getOrCreateUser(adminCtx, authenticationStrategy, externalId, displayName, opts, function(err, user) {
                    ...
                    // name to be the one provided in the CSV file. Alternatively, we update the user's display name when `forceProfileUpdate`
                    // has been provided, even when the existing display name is different than the external id. Note that the latter approach
                    // will override any display name changes that have been made by users.
                    } else if (user.displayName !== displayName && (forceProfileUpdate || user.displayName === externalId)) {
                        log(ctx).info({
