...
var test = (module.exports.test = function(ctx, contentObj, callback) {
  ...
  var key = PreviewConfig.getValue('admin', 'youtube', 'key');
  if (!key) {
    return callback(null, -1);
  }
  ...
  if (
    YOUTUBE_FULL_REGEX.test(contentObj.link) ||
    YOUTUBE_SHORT_REGEX.test(contentObj.link)
  ) {
...
var generatePreviews = (module.exports.generatePreviews = function(
  ...
  Youtube.videos.list({ part: 'snippet', id: id }, function(err, data) {
    ...
    if (data && data.items && data.items[0] && data.items[0].snippet) {
      ...
      // Download it
      var thumbnail =
        snippet.thumbnails.maxres || snippet.thumbnails['default'];
