 ...
 * together in order to find the final list of routes. If an association is prefixed with the `^` character however, its results will
 ...
 * in the router configuration is important.
 *
 ...
 *
 * Lets say you want to notify all the managers of a piece of content (object) except for the current collaborators:
 *
 * ```javascript
 * options.streams = {
 *      'notification': {
 *          'router': {
 *              'object': ['managers', '^current-collaborators']
 *          }
 *      }
 *  }
 * ```
 *
 * In this routing configuration, the `object` configuration states that the managers of the piece of content should be retrieved
 * and the current collaborators should be excluded from the result. Note that switching the parameters, will give different results.
 *
 * ```javascript
 *    ...
 *    'object': ['^current-collaborators', 'managers']
 *    ...
 * ```
 *
 * In this routing configuration, we would exclude the current collaborators from the empty set followed by adding all the managers.
 * The net result is that you would send a notification to *all* the managers.
 *
...
/**
 * Get the activity stream by the specified `resourceId` and `activityStreamType`.
 *
 ...
 * @param  {String}             activityStreamType       The name of the activity stream that should be retrieved
 ...
 */
var getActivityStream = module.exports.getActivityStream = function(ctx, resourceId, activityStreamType, start, limit, transformerType, callback) {
    var activityStream = getRegisteredActivityStreamType(activityStreamType);
    ...
    validator.check(activityStreamType, {'code': 400, 'msg': 'Must specify an activity stream'}).notEmpty();
    ...
    var authorizationHandler = activityStream.authorizationHandler || function(ctx, activityStreamType, token, callback) { return callback(); };
    authorizationHandler(ctx, resourceId, null, function(err) {
        if (err) {
            return callback(err);
        }

        return _getActivityStream(ctx, resourceId + '#' + activityStreamType, start, limit, transformerType, callback);
    });
};

/**
 * Mark all the notifications as read for the user authenticated to the given context.
 *
 * @param  {Context}    ctx             The context of the current session
 * @param  {Function}   callback        Invoked when the process completes
 * @param  {Object}     callback.err    An error that occurred, if any
