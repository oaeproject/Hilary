 ...
 * @param   {Content}   [content]           The content object we're indexing. If not specified, it will be fetched using the `contentId`.
 ...
 * @param   {String[]}  callback.members    A hash, where the keys are the principal ids, and the values are the role of the principal
 */
var getContentWithMembers = module.exports.getContentWithMembers = function(contentId, content, members, callback) {
    _getContentIfNecessary(contentId, content, function(err, content) {
        if (err) {
            return callback(err);
        }
        ...
        _getMembersIfNecessary(contentId, members, function(err, members) {
            ...
            return callback(null, content, members);
        });
    });
}
