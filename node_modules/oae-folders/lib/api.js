...
var ContentAuthz = require('oae-content/lib/authz');
 ...
 * @param  {Object}         [members]               An object whose keys are principal ids, or email address, and values are the role they should have on the folder. By default only the creator of the folder will be a manager
...
var createFolder = module.exports.createFolder = function(ctx, displayName, description, visibility, members, callback) {
    ...
    PrincipalsAPI.getPrincipalsAndCreateGuests(ctx, _.keys(members), function(err, principals) {
        if (err) {
            return callback(err);
        }

        var permissions = {};
        _.each(members, function(role, principal) {
            permissions[principals[principal].id] = role;
        });
        ...
        AuthzAPI.canInteract(ctx, ctx.tenant().alias, principals, function(err, canInteract, illegalPrincipalIds) {
            ...
            // Get the ids of the principals who should become managers
            var managerIds = _.chain(principals)
                .pluck('id')
                .filter(function(principalId) {
                    return (permissions[principalId] === FoldersConstants.roles.MANAGER);
                })
                .value();
            ...
            PrincipalsAPI.canManageAny(ctx, managerIds, function(err, canManageAny) {
                ...
                // We only make the current user a manager of the folder if he cannot
                // manage any of the specified managers
                if (!canManageAny) {
                    permissions[ctx.user().id] = FoldersConstants.roles.MANAGER;
                }
                ...
                FoldersDAO.createFolder(ctx.user().id, displayName, description, visibility, function(err, folder) {
                    ...
                    AuthzAPI.updateRoles(folder.groupId, permissions, function(err) {
                        if (err) {
                            return callback(err);
                        }

                        // Insert the folder into the folders library of all the members
                        FoldersFoldersLibrary.insert(_.keys(permissions), folder, function(err) {
                            if (err) {
                                log(ctx).warn({
                                    'err': err,
                                    'principalIds': memberIds,
                                    'folder': folder
                                }, 'An error occurred while inserting a created folder into member libraries');
                            }

                            FoldersAPI.emit(FoldersConstants.events.CREATED_FOLDER, ctx, folder, permissions);
    ...
    FoldersDAO.getFolder(folderId, function(err, folder) {
        ...
        FoldersAuthz.canManageFolder(ctx, folder, function(err, canManage) {
            ...
            FoldersDAO.updateFolder(folder, updates, function(err, updatedFolder) {
                ...
                FoldersAPI.emit(FoldersConstants.events.UPDATED_FOLDER, ctx, folder, updatedFolder);
...
var _deleteFolder = function(folder, callback) {
    ...
    AuthzAPI.getAuthzMembers(folder.groupId, null, 10000, function(err, memberRoles, nextToken) {
        ...
        AuthzAPI.updateRoles(folder.groupId, roleChanges, function(err) {
            ...
            // Remove the folder from each principal's folder
 ...
 * @param  {String[]}   principal       The ids or email addresses of the users and groups with whom to share the folder
 * @param  {Function}   callback        Standard callback function
 * @param  {Object}     callback.err    An error that occurred, if any
 */
var shareFolder = module.exports.shareFolder = function(ctx, folderId, principals, callback) {
    var validator = new Validator();
    ...
    validator.check(principals.length, {'code': 400, 'msg': 'The folder must at least be shared with 1 user, group or email address'}).min(1);
    if (validator.hasErrors()) {
        return callback(validator.getFirstError());
    }
    ...
    FoldersDAO.getFolder(folderId, function(err, folder) {
        ...
        PrincipalsAPI.getPrincipalsAndCreateGuests(ctx, principals, function(err, principals) {
            ...
            _.each(principals, function(principal) {
                roleChanges[principal.id] = FoldersConstants.roles.VIEWER;
            });

            // Determine which members are actually newly added to this folder. We can only add those
            AuthzAPI.computeMemberRolesAfterChanges(folder.groupId, roleChanges, function(err, membershipAfterChanges, newMemberIds, updatedMemberIds) {
                if (err) {
                    return callback(err);
                    ...
                    AuthzAPI.updateRoles(folder.groupId, roleChanges, function(err) {
                        ...
                        OaeUtil.invokeIfNecessary(_testLibraryUpdateThreshold(folder), FoldersFoldersLibrary.update, allMemberIdsBeforeShare, folder, null, function(err, updatedFolder) {
                            ...
                            FoldersFoldersLibrary.insert(newMemberIds, folder, function(err) {
                                ...
                                FoldersAPI.emit(FoldersConstants.events.UPDATED_FOLDER_MEMBERS, ctx, folder, roleChanges, newMemberIds, updatedMemberIds, []);
                                return callback();
                            });
                        });
                    });
 ...
 * @param  {Object}     permissionChanges   An object whose key is the user or group id to set, or the email address to invite, on the folder, and the value is the role you wish them to have. If the role of a user is set to `false`, then it indicates to remove the user from the folder
 * @param  {Function}   callback            Standard callback function
 * @param  {Object}     callback.err        An error that occurred, if any
 */
var setFolderPermissions = module.exports.setFolderPermissions = function(ctx, folderId, permissionChanges, callback) {
    ...
    FoldersDAO.getFolder(folderId, function(err, folder) {
        ...
        PrincipalsAPI.getPrincipalsAndCreateGuests(ctx, _.keys(permissionChanges), function(err, principals) {
            ...
            AuthzAPI.computeMemberRolesAfterChanges(folder.groupId, permissions, function(err, membershipAfterChanges, newMemberIds, updatedMemberIds, removedMemberIds) {
                if (err) {
                    return callback(err);
                }
                        ...
                        FoldersFoldersLibrary.insert(newMemberIds, folder, function(err) {
                            ...
                            FoldersFoldersLibrary.remove(removedMemberIds, folder, function(err) {
                                ...
                                OaeUtil.invokeIfNecessary(_testLibraryUpdateThreshold(folder), FoldersFoldersLibrary.update, updateMemberIds, folder, null, function(err) {
                                    ...
                                    FoldersAPI.emit(FoldersConstants.events.UPDATED_FOLDER_MEMBERS, ctx, folder, _.pick(permissions, updatedMemberIds), newMemberIds, updatedMemberIds, removedMemberIds);
                                    return callback();
                                });
                            });
                        });
...
var removeFolderFromLibrary = module.exports.removeFolderFromLibrary = function(ctx, principalId, folderId, callback) {
    ...
    FoldersDAO.getFolder(folderId, function(err, folder) {
        ...
        LibraryAPI.Authz.canRemoveFromLibrary(ctx, principalId, function(err, canRemove) {
            if (err) {
                return callback(err);
            } else if (!canRemove) {
            ...
            permissionChanges[principalId] = false;
            ...
            AuthzAPI.computeMemberRolesAfterChanges(folder.groupId, permissionChanges, function(err, membershipAfterChanges, newMemberIds, updatedMemberIds, removedMemberIds) {
                ...
                AuthzAPI.updateRoles(folder.groupId, permissionChanges, function(err) {
                    ...
                    FoldersFoldersLibrary.remove([principalId], folder, function(err) {
                        if (err) {
                            log().error({
                                'err': err,
                        ...
                        FoldersAPI.emit(FoldersConstants.events.UPDATED_FOLDER_MEMBERS, ctx, folder, permissionChanges, [], [], [principalId]);
                        return callback();
                    });
                });
            });
...
var createMessage = module.exports.createMessage = function(ctx, folderId, body, replyToCreatedTimestamp, callback) {
    ...
    FoldersDAO.getFolder(folderId, function(err, folder) {
        ...
        // Ensure the current user can view the folder
        FoldersAuthz.canPostFolderMessage(ctx, folder, function(err, canPost) {
            if (err) {
                return callback(err);
            } else if (!canPost) {
                return callback({'code': 401, 'msg': 'You are not authorized to post to this folder'});
            }
            ...
            MessageBoxAPI.createMessage(folderId, ctx.user().id, body, {'replyToCreated': replyToCreatedTimestamp}, function(err, message) {
                ...
                PrincipalsUtil.getPrincipal(ctx, ctx.user().id, function(err, createdBy) {
                    ...
                    FoldersAPI.emit(FoldersConstants.events.CREATED_COMMENT, ctx, message, folder);

                    // Pass the message to the caller but do not return yet as we still might have to update the folder libraries
                    callback(null, message);

                    // The following `if` block is an asynchronous block. Check to see if we are in a threshold to perform a
                    // folder lastModified update. If so, we will update the lastModified date and the folder's libraries
                    if (_testLibraryUpdateThreshold(folder)) {
                        // Try and get the principals whose libraries will be updated
                        AuthzAPI.getAuthzMembers(folder.groupId, null, 10000, function(err, memberRoles) {
                            if (err) {
                            ...
                            // Bump the `lastModified` timestamp and update the folder libraries for all of the members
                            var principalIds = _.pluck(memberRoles, 'id');
                            FoldersFoldersLibrary.update(principalIds, folder, null, function(err) {
                                if (err) {
                                    // There isn't much we can do at this point other than log an error
                                    log().warn({
                                        'err': err,
...
var deleteMessage = module.exports.deleteMessage = function(ctx, folderId, messageCreatedDate, callback) {
    ...
    FoldersDAO.getFolder(folderId, function(err, folder) {
        ...
        MessageBoxAPI.getMessages(folderId, [messageCreatedDate], {'scrubDeleted': false}, function(err, messages) {
            ...
            FoldersAuthz.canDeleteFolderMessage(ctx, folder, message, function(err, canDelete) {
                if (err) {
                    return callback(err);
                } else if (!canDelete) {
                    return callback({'code': 401, 'msg': 'You are not authorized to delete messages from this folder'});
                }
