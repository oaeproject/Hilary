...
ActivityAPI.registerActivityType(ContentConstants.activity.ACTIVITY_CONTENT_COMMENT,
    {
        ...
        'notifications': {
            'email': true,
            'emailTemplateModule': 'oae-content',
            'emailTemplateId': 'notify-content-message'
        }
    }
);

/*!
 * Post a content-message activity when a user comments on a content item
 */
ContentAPI.on(ContentConstants.events.CREATED_COMMENT, function(ctx, message, content) {
    var millis = Date.now();
    var actorResource = new ActivityModel.ActivitySeedResource('user', ctx.user().id, {'user': ctx.user()});
    var objectResource = new ActivityModel.ActivitySeedResource('content-message', message.id, {'contentId': content.id, 'message': message});
 ...
 */
ActivityAPI.registerActivityEntityProducer('content-message', function(activityType, entityType, resource, callback) {
    ...
    ContentDAO.Content.getContent(contentId, function(err, content) {
        ...
        message.visibility = content.visibility;
        MessageBoxUtil.createPersistentMessageActivityEntity(message, function(err, entity) {
            if (err) {
                return callback(err);
            }

            entity.objectType = ContentConstants.activity.ACTIVITY_CONTENT_COMMENT;
 ...
 */
ActivityAPI.registerActivityRouter('content-message', function(activityType, entityType, entity, callback) {
    // First collect the routes for the parent content item
    var contentId = entity.message.messageBoxId;
    _routeContent(activityType, entityType, contentId, entity.message.visibility, function(err, routes, propagation) {
        if (err) {
            return callback(err);
        }
...
var _routeContent = function(activityType, entityType, contentId, visibility, callback) {
    ...
    ActivityUtil.getAllAuthzMembersByRole(contentId, function(err, specify) {
        ...
        var isUpdateActivity = (activityType === ContentConstants.activity.ACTIVITY_CONTENT_UPDATE || activityType === ContentConstants.activity.ACTIVITY_CONTENT_REVISION);
 ...
 */
ActivityAPI.registerActivityEntityTransformer('content-message', function(ctx, activityEntities, callback) {
    var transformedActivityEntities = {};
    _.keys(activityEntities).forEach(function(activityId) {
        ...
        _.keys(activityEntities[activityId]).forEach(function(entityId) {
            ...
            var urlFormat = '/api/content/' + contentId + '/comments/%s';
            transformedActivityEntities[activityId][entityId] = MessageBoxUtil.transformPersistentMessageActivityEntity(ctx, entity.message, entity.messageContext, 'content-message', profilePath, urlFormat);
        });
    });
    return callback(null, transformedActivityEntities);
});
