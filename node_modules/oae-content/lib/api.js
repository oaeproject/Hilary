...
var _getComment = function(ctx, commentId, callback) {
    var validator = new Validator();
    validator.check(commentId, {'code': 400, 'msg': 'A comment id must be provided'}).not('null');
    validator.check(commentId, {'code': 400, 'msg': 'A comment id must be provided'}).notEmpty();
    ...
    Cassandra.runQuery('SELECT * FROM Comments USING CONSISTENCY QUORUM WHERE commentId = ?', [commentId], function (err, commentRows) {
        ...
        var comment = Cassandra.rowToHash(commentRows[0]);

        hasAccess(ctx, comment.contentId, function(err, hasAccess) {
            if (err || !hasAccess) {
                return callback(err);
            }

            Cassandra.runQuery('SELECT ? FROM CommentsByContent USING CONSISTENCY QUORUM WHERE contentId = ?', [comment.created, comment.contentId], function (err, rows) {
                callback(err, _rowsToComments(commentRows));
            });
        });
    });
};
...
/**
 * Creates a comment on the specified content item. Returns an error if saving the comment goes wrong.
 *
 * @param  {Context}    ctx             Standard context object, representing the currently logged user and its tenant
...
var createComment = module.exports.createComment = function(ctx, contentId, body, replyTo, callback) {
    ...
    // Parameter validation
    var validator = new Validator();
    validator.check(contentId, {'code': 400, 'msg': 'Invalid resource id provided.'}).isResourceId();
    ...
    hasAccess(ctx, contentId, function(err, hasAccess) {
        ...
        if (replyTo) {
            _getComment(ctx, replyTo, function(err, commentReplyingTo) {
                ...
                Cassandra.runQuery(q.query, q.parameters, function(err) {
                    ...
                    q = Cassandra.constructUpsertCQL('Comments', 'commentId', commentId, {
                        'body': body,
                        'createdBy': ctx.user().id,
                        'replyTo': commentReplyingTo.commentId,
                        'level': (commentReplyingTo.level + 1),
                        'contentId': contentId,
                        'created': created
                    }, 'QUORUM');
            ...
            var queries = [{
                'query': 'UPDATE Comments SET contentId = ?, body = ?, createdBy = ?, created = ? WHERE commentId = ?',
                'parameters': [contentId, body, ctx.user().id, created, commentId]
            }, {
...
var getComments = module.exports.getComments = function(ctx, contentId, start, limit, callback) {
    getContent(ctx, contentId, function(err, contentObj) {
        ...
        Cassandra.runQuery('SELECT FIRST ' + paging.limit + ' REVERSED ?..\'\' FROM CommentsByContent USING CONSISTENCY QUORUM WHERE contentId = ?', [paging.start, contentId], function (err, rows) {
            ...
            var commentIds = [];
            for (var i = paging.startIndex; i < rows[0].count; i++) {
                commentIds.push(rows[0][i].value);
            }
            ...
            Cassandra.runQuery('SELECT * FROM Comments USING CONSISTENCY QUORUM WHERE commentId IN (?)', [commentIds], function (err, commentsRows) {
                ...
                for (var ii = 0; ii < commentsRows.length; ii++) {
                    comments.push(Cassandra.rowToHash(commentsRows[ii]));
                }

                // Get information on the commenters
                var userIds = [];
                commentsRows.forEach(function(row){
                    userIds.push(row.get('createdBy').value);
                });

                PrincipalsUtil.getPrincipals(ctx, userIds, function(err, principals) {
                    comments = _rowsToComments(commentsRows);
 ...
 *
 * @param  {Context}    ctx             Standard context object, representing the currently logged user and its tenant
 * @param  {String}     commentId       The ID of the content to delete the comment from
 * @param  {String}     commentId       The ID of the comment
 * @param  {Function}   callback        Standard callback function takes argument `err`
 * @param  {Object}     callback.err    Error object containing the error message
 */
var deleteComment = module.exports.deleteComment = function(ctx, contentId, commentId, callback) {
    if (!ctx.user()) {
        return callback({'code': 401, 'msg': 'Only authorized users can delete comments'});
    }
    ...
    getContent(ctx, contentId, function(err, contentObj) {
        ...
        canManage(ctx, contentId, function(err, canManage, contentObj) {
            _getComment(ctx, commentId, function(err, comment) {
                ...
                var canDelete = canManage || ctx.user().id === comment[0].createdBy;
                if (!canDelete) {
                    return callback({'code': '400', 'msg': 'You don\t have the appropriate priviliges to delete this comment'});
                }

                // The content exists and we have access
                // Get the comment and the comment following it to check if there's a reply.
                getComments(ctx, contentId, comment[0].created, 2, function(err, comments) {
                    if (comments && comments.length && comments[0].replyTo === commentId) {
                        // If there is a reply, mark the message as deleted but don't delete
                        var q = Cassandra.constructUpsertCQL('Comments', 'commentId', commentId, {
                            'deleted': true
                        }, 'QUORUM');
                        Cassandra.runQuery(q.query, q.parameters, callback);
                    } else {
                        // If there's no reply, delete the message
                        Cassandra.runQuery('DELETE ? FROM CommentsByContent WHERE contentId = ?', [comment[0].created, comment[0].contentId], function(err) {
                            if (err) {
                                return callback(err);
                            }

                            Cassandra.runQuery('DELETE FROM Comments WHERE commentId = ?', [comment[0].commentId], function(err) {
                                callback(err);
                            });
                        });
                    }
...
/**
 * Creates an array of content comments and returns the array
 *
 * @param   {Row}        commentRow             Cassandra row retrieved from the Comments CF containing the reference to the comment and creation date
 * @return  {Comment}                           Returns a Comment object.
 * @api private
 */
var _rowsToComments = function(commentRow) {
    var commentArr = [];
    for (var i = 0; i < commentRow.length; i++) {
        commentRow[i] = Cassandra.rowToHash(commentRow[i]);
        var comment = new Comment(commentRow[i].body, commentRow[i].commentId, commentRow[i].contentId, commentRow[i].createdBy, commentRow[i].created, commentRow[i].lastModified, commentRow[i].level, commentRow[i].replyTo, commentRow[i].deleted);
