    ...
    describe('Activity Stream Permissions and Validation', function() {
        ...
        it('verify getActivityStream validation', function(callback) {
            ...
            RestAPI.User.createUser(camAdminRestContext, jackUsername, 'password', 'Jack McJackerson', null, function(err, jack) {
                ...
                // Try anonymous user
                RestAPI.Activity.getActivityStream(anonymousCamRestContext, jack.id, null, function(err, activityStream) {
                    assert.ok(err);
                    assert.equal(err.code, 401);
                    assert.ok(!activityStream);

                    // Try empty id
                    RestAPI.Activity.getActivityStream(jackCtx, '  ', null, function(err, activityStream) {
                        assert.ok(err);
                        assert.equal(err.code, 400);
                        assert.ok(!activityStream);

                        // Try invalid principal id
                        RestAPI.Activity.getActivityStream(jackCtx, 'c:cam:someContent', null, function(err, activityStream) {
                            assert.ok(err);
                            assert.equal(err.code, 400);
                            assert.ok(!activityStream);
