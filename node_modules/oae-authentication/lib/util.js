 ...
 * @param  {Object}     authObj             The authentication object that is returned by the strategy to be stored in `req.user`
 * @param  {User}       authObj.user        The user who will own the session
 * @param  {User}       [authObj.imposter]  If the user in session is being impostered, this is the user impostering the owner of the session
 * @param  {String}     strategyName        The name of the strategy used for authn
 */
var logAuthenticationSuccess = module.exports.logAuthenticationSuccess = function(req, authObj, strategyName) {
    var tenantAlias = req.tenant.alias;
    var user = authObj.user;
    var imposter = authObj.imposter;
...
var handleExternalCallback = module.exports.handleExternalCallback = function(strategyId, req, res, next) {
    ...
    passport.authenticate(strategyId, {}, function(err, user, challenges, status) {
        ...
        var authObj = {'user': user};
        logAuthenticationSuccess(req, authObj, strategyId);

        // Create a session for this user
        req.logIn(authObj, function(err) {
            if (err) {
                return errorHandler(err);
            }
