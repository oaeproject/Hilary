...
var createComment = module.exports.createComment = function(ctx, contentId, body, replyTo, callback) {
    ...
    hasAccess(ctx, contentId, function(err, hasAccess) {
        ...
        /**
         * Gets and returns the created comment
         *
         * @param {Object}    err    Error object coming from an error when saving the new comment
         */
        var returnCreatedComment = function(err) {
            if (err) {
                return callback(err);
            }
            ...
            _getComment(ctx, commentId, function(err, comment) {
                ...
                PrincipalsUtil.getPrincipal(ctx, comment.createdBy, function(err, principal) {
                    if (err) {
                        return callback(err);
                    }
        ...
        if (replyTo) {
            _getComment(ctx, replyTo, function(err, commentReplyingTo) {
                ...
                Cassandra.runQuery(q, [created, commentId, contentId], function(err) {
                    ...
                    Cassandra.runQuery(q, [commentId, body, ctx.user().id, commentReplyingTo.commentId, commentReplyingTo.level + 1, contentId, created], returnCreatedComment);
                });
            });
        } else {
            ...
            Cassandra.runBatchQuery(queries, 'QUORUM', returnCreatedComment);
        }
    });
};
