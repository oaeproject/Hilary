 ...
 *
 * @param  {Context}        ctx                 Standard context object containing the current user and the current tenant
 * @param  {Content}        content             The content object representing the content item we're checking access for
 * @param  {Function}       callback            Standard callback function
 * @param  {Object}         callback.err        An error that occurred, if any
 * @param  {Boolean}        callback.canView    Whether or not the user has access to the content
 */
var canViewContent = module.exports.canViewContent = function(ctx, content, callback) {
    var user = ctx.user();
    AuthzAPI.resolveImplicitRole(user, content, ContentConstants.roles.ALL_PRIORITY, function(err, implicitRole, canInteract) {
        if (err) {
            return callback(err);
        ...
        return AuthzAPI.hasAnyRole(user.id, content.id, callback);
    });
};
