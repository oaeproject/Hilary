...
var getTenantByEmailDomain = module.exports.getTenantByEmailDomain = function(emailDomain) {
    var tenant = _.chain(getTenants())
        .filter(function(tenant) {
            ...
            return (tenant.emailDomain && _matchesEmailDomain(emailDomain.toLowerCase(), tenant.emailDomain));
        })
        .first()
        .value();
...
var createTenant = module.exports.createTenant = function(ctx, alias, displayName, host, opts, callback) {
    ...
    _.each(opts, function(val, key) {
        ...
        if (key === 'emailDomain' && _.isString(val)) {
...
var _validateEmailDomain = function(validator, emailDomain, updateTenantAlias) {
    var matchingEmailDomain = _.chain(getTenants())
        ...
        .pluck('emailDomain')
        .compact()
        .filter(function(emailDomainExpr) {
            // There is a conflict if there is an existing expression that matches with the
            // expression, or vice versa
            return (_matchesEmailDomain(emailDomain, emailDomainExpr) ||
                _matchesEmailDomain(emailDomainExpr, emailDomain));
        })
        .first()
        .value();

    validator.check(matchingEmailDomain, {
        'code': 400,
...
/**
 * Determine if the given email domain expression matches the given email domain
 *
 * @param  {String}     emailDomain         The email domain to check
 * @param  {String}     emailDomainExpr     The email domain expression (e.g., *.cam.ac.uk)
 * @return {Boolean}                        Whether or not the domain expression matched the email
 * @api private
 */
var _matchesEmailDomain = function(emailDomain, emailDomainExpr) {
    var isSuffix = false;
    if (emailDomainExpr[0] === '*') {
        isSuffix = true;
        emailDomainExpr = emailDomainExpr.slice(1);
    }

    if (isSuffix) {
        return (emailDomain.indexOf(emailDomainExpr, emailDomain.length - emailDomainExpr.length) !== -1);
    } else {
        return (emailDomain === emailDomainExpr);
    }
};
