...
var storeMetadata = module.exports.storeMetadata = function(contentObj, revisionId, status, thumbnailUri, contentMetadata, previewMetadata, fileData, callback) {
    ...
    if (contentObj.previews.total && contentObj.previews.total > 0) {
        deleteQuery = {
            'query': 'DELETE FROM PreviewItems USING CONSISTENCY QUORUM WHERE revisionId = ?',
    ...
    if (nrOfFiles > 0) {
        // The keys of the file data are the Cassandra column names to ensure they are ordered properly. This is to support
