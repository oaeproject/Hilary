...
/**
 * Similar to Cassandra#runPagedQuery, but this will page through all the results and return all
 * the row pages that were queried
 *
 * @param  {String}     tableName               The name of the table to query
 * @param  {String}     keyColumnName           The name of the key column to query
 * @param  {String}     keyColumnValue          The value of the key column to query
 ...
 * @param  {Function}   callback                Standard callback function
 * @param  {Object}     callback.err            An error that occurred, if any
 * @param  {Rows[]}     callback.pages          A 2-dimensional array, containing each page of rows returned from the query
 */
var runAllPagesQuery = module.exports.runAllPagesQuery = function(tableName, keyColumnName, keyColumnValue, rangeColumnName, opts, callback, _nextToken, _pages) {
    _pages = _pages || [];
    opts = opts || {};
    ...
    runPagedQuery(tableName, keyColumnName, keyColumnValue, rangeColumnName, start, opts.batchSize, {'end': opts.end}, function(err, rows, nextToken) {
        if (err) {
            ...
            // Only put in pages that have something
            _pages.push(rows);
        }

        // Return to the caller if we've fetched all rows
        if (nextToken === null) {
            return callback(null, _pages);
        }

        // Subsequent iterations should not apply the `start` option
        if (opts.start) {
            opts = _.extend({}, opts);
            delete nextOpts.start
        }

        // Recursively fetch the next page
        return runAllPagesQuery(tableName, keyColumnName, keyColumnValue, rangeColumnName, opts, callback, nextToken, _pages);
    });
};
