...
var _transformUserDocuments = function(ctx, docs, callback) {
    ...
    for (var i = 0; i < docIds.length; i++) {
        ...
        if (user.picture.mediumUri) {
            result.thumbnailUrl = PrincipalsUtil.getSignedPictureUrl(ctx, ctx.tenant().alias, user.picture.mediumUri);
        }
...
SearchAPI.registerReindexAllHandler('principal', function(callback) {
    PrincipalsDAO.iterateAll(null, 1000, function(err, principals, complete) {
        if (err) {
            log().error({'err': err}, 'Error while re-indexing principals.');
            return callback(err);
        } else if (complete) {
            log().info('Re-indexing tasks for all principals has been submitted.');
            return callback();
        }
