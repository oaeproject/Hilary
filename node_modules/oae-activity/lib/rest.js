 ...
 * @api private
 */
var _handleGetActivities = function(resourceId, req, res) {
    var limit = OaeUtil.getNumberParam(req.query.limit, 10, 1, 25);
    var start = req.query.start;
    ActivityAPI.getActivityStream(req.ctx, resourceId, start, limit, req.query.format, function(err, activityStream) {
        if (err) {
 ...
 */
OAE.tenantRouter.on('get', '/api/activity/:resourceId', function(req, res) {
...
OAE.tenantRouter.on('get', '/api/notifications', function(req, res) {
    var limit = OaeUtil.getNumberParam(req.query.limit, 10, 1, 25);
    var userId = req.ctx.user() ? req.ctx.user().id : null;
    ActivityAPI.getNotificationStream(req.ctx, userId, req.query.start, limit, req.query.format, function(err, notificationStream) {
        if (err) {
            return res.send(err.code, err.msg);
        }
