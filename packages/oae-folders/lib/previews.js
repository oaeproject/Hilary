    ...
    MQTestUtil.whenTasksEmpty(PreviewConstants.MQ.TASK_GENERATE_FOLDER_PREVIEWS, () =>
      MQTestUtil.whenTasksEmpty(PreviewConstants.MQ.TASK_GENERATE_FOLDER_PREVIEWS_PROCESSING, () =>
        MQTestUtil.whenTasksEmpty(PreviewConstants.MQ.TASK_GENERATE_PREVIEWS, () =>
          MQTestUtil.whenTasksEmpty(PreviewConstants.MQ.TASK_GENERATE_PREVIEWS_PROCESSING, callback)
        )
      )
    );
  });
...
const _handleContentChange = function (ctx, folder, contentItems) {
  ...
  const contentItemsWithPreviews = _.filter(
    contentItems,
    (contentItem) => contentItem.previews && contentItem.previews.status === 'done'
  );
...
FoldersAPI.emitter.on(FoldersConstants.events.ADDED_CONTENT_ITEMS, (ctx, actionContext, folder, contentItems) =>
  _handleContentChange(ctx, folder, contentItems)
);
...
ContentAPI.emitter.on(ContentConstants.events.UPDATED_CONTENT, (ctx, newContentObject, oldContentObject) => {
  if (newContentObject.visibility !== oldContentObject.visibility) {
    _reprocessFoldersThatContainContent(newContentObject.id);
  }
});
...
const _reprocessFoldersThatContainContent = function (contentId) {
  ...
  FoldersAuthz.getFoldersForContent(contentId, (error, folders) => {
    if (error) {
      log().error({ err: error, contentId }, 'Unable to get the folders a piece of content resides in');
      previewCounter.decr();
      return;
    }
