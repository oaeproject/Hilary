...
var PrincipalsConstants = require('oae-principals/lib/constants')
  .PrincipalsConstants;
 ...
 */
ActivityEmitter.on(ActivityConstants.events.DELIVERED_ACTIVITIES, function(
  deliveredActivities,
) {
  ...
  _getEmailRecipientResources(emailRecipientIds, function(err, recipients) {
    if (err) {
      ...
      return log().error(
        { err: err, emailRecipientIds: usersToMail },
    ...
    var recipientsToQueue = _.filter(recipients, function(emailRecipient) {
      return (
        ...
        emailRecipient.emailPreference !==
          PrincipalsConstants.emailPreferences.NEVER
      );
    });
    ...
    ActivityDAO.saveQueuedUserIdsForEmail(emailBuckets, function(err) {
      if (err) {
        scheduledEmailsCounter.decr();
        return log().error(
          { err: err, deliveredActivities: deliveredActivities },
          'Unable to store the IDs of the users who need to receive mail',
        );
      }

      log().trace(
        { recipientsToQueue: recipientsToQueue },
        'Queued mail for users',
      );
 ...
 */
PrincipalsEmitter.on(PrincipalsConstants.events.UPDATED_USER, function(
  ctx,
  newUser,
  oldUser,
) {
  ...
  if (
    ...
    return ActivityEmitter.emit(
      ActivityConstants.events.UPDATED_USER,
      ctx,
      newUser,
      oldUser,
    );
  ...
  ActivityDAO.unqueueUsersForEmail(oldBucketId, [oldUser.id], function(err) {
    if (err) {
      ...
      newUser.emailPreference === PrincipalsConstants.emailPreferences.NEVER
    ) {
      return ActivityEmitter.emit(
        ActivityConstants.events.UPDATED_USER,
        ctx,
    ...
    ActivityDAO.saveQueuedUserIdsForEmail(emailBucket, function(err) {
      if (err) {
        log().error(
          { err: err, user: newUser.id },
          'Could not re-queue the user for email',
        );
      }
...
var collectAllBuckets = (module.exports.collectAllBuckets = function(callback) {
  ...
  var maxConcurrentCollections = ActivitySystemConfig.getConfig()
    .maxConcurrentCollections;
  var numberOfBuckets = ActivitySystemConfig.getConfig()
    .numberOfProcessingBuckets;
...
var _collectImmediateBucket = function(bucketNumber, callback) {
  collectMails(
    bucketNumber,
    PrincipalsConstants.emailPreferences.IMMEDIATE,
    null,
    null,
    callback,
  );
...
var _isDailyCycle = function() {
  ...
  var end = new TZ.Date(
    now.getTime() + config.mail.pollingFrequency * 1000,
    'UTC',
  );
 ...
 */
var collectMails = (module.exports.collectMails = function(
  bucketNumber,
  emailPreference,
  dayOfWeek,
  hour,
  ...
  var bucketId = _createEmailBucketId(
    bucketNumber,
    emailPreference,
    dayOfWeek,
    hour,
  ...
  _collectMails(bucketId, oldestActivityTimestamp, null, function(
    err,
    recipients,
  ) {
    if (err) {
    ...
    Telemetry.incr(
      util.format('sent.%s.count', emailPreference),
      recipients.length,
    );
    Telemetry.appendDuration(
      util.format('sent.%s.time', emailPreference),
      collectionStart,
    );
...
var _collectMails = function(
  ...
  ActivityDAO.getQueuedUserIdsForEmail(
    ...
    start,
    MAX_COLLECTION_BATCH_SIZE,
    function(err, recipientIds, nextToken) {
      if (err) {
        return callback(err);
      ...
      var recipientIdsByActivityStreamIds = _.chain(recipientIds)
        .map(function(recipientId) {
          return [
            ActivityUtil.createActivityStreamId(
              recipientId,
      ...
      ActivityDAO.getActivitiesFromStreams(
        ...
        oldestActivityTimestamp,
        function(err, activitiesPerStream) {
          if (err) {
            return callback(err);
          }
          ...
          var threshold =
            Date.now() -
            ActivitySystemConfig.getConfig().mail.gracePeriod * 1000;
          _.each(activitiesPerStream, function(activities, activityStreamId) {
            ...
            if (!hasRecentActivity) {
              ...
              recipientIdsToMail.push(
                recipientIdsByActivityStreamIds[activityStreamId],
              );
            }
          });
          ...
          ActivityDAO.resetAggregationForActivityStreams(
            ...
            function(err) {
              ...
              ActivityDAO.unqueueUsersForEmail(
                bucketId,
                recipientIdsToMail,
                function(err) {
                  if (err) {
                  ...
                  _.each(activitiesPerMailableStreams, function(
                    activities,
                    activityStreamId,
                  ) {
                  ...
                  ActivityDAO.deleteActivities(
                    ...
                    function(err) {
                      ...
                      _getEmailRecipientResources(recipientIdsToMail, function(
                        err,
                        recipients,
                      ) {
                        if (err) {
                          log().error(
                            { err: err, recipientIds: recipientIdsToMail },
                        ...
                        var recipientsToMail = _.filter(recipients, function(
                          recipient,
                        ) {
                          ...
                          var emailActivityStreamId = ActivityUtil.createActivityStreamId(
                            recipient.id,
                            ActivityConstants.streams.EMAIL,
                          );
                          return (
                            recipient.emailPreference !==
                              PrincipalsConstants.emailPreferences.NEVER &&
                            !_.isEmpty(
                              activitiesPerMailableStreams[
                                emailActivityStreamId
                              ],
                            )
                          );
                        });
                        ...
                        var toMail = _.map(recipientsToMail, function(
                          recipient,
                        ) {
                          var emailActivityStreamId = ActivityUtil.createActivityStreamId(
                            recipient.id,
                        ...
                        _mailAll(toMail, function(err) {
                          ...
                          _collectedRecipients = _collectedRecipients.concat(
                            recipientsToMail,
                          );
...
var _mail = function(recipient, activities, callback) {
  ...
  var timezone = TenantConfig.getValue(tenant.alias, 'timezone', 'timezone');
  var ctx = new Context(tenant, recipient);
  if (!tenant.active) {
    return callback(
      new Error(
        util.format(
          'Tried to email a user in a disabled tenancy %s',
          tenant.alias,
        ),
      ),
    );
  ...
  ActivityTransformer.transformActivities(
    ...
    function(err) {
      ...
      if (AuthzUtil.isEmail(recipient.id)) {
        ...
        var signupRedirectUrl = util.format(
 ...
 */
var whenEmailsScheduled = (module.exports.whenEmailsScheduled = function(
  handler,
) {
...
var _createActivityFromAggregate = function(aggregate) {
  var actor = ActivityAggregator.createActivityEntity(
    _.values(aggregate.actors),
  );
  var object = ActivityAggregator.createActivityEntity(
    _.values(aggregate.objects),
  );
  var target = ActivityAggregator.createActivityEntity(
    _.values(aggregate.targets),
  );
...
var _unrollActivities = function(aggregatedActivities) {
  ...
  _.each(aggregatedActivities, function(activity) {
    ...
    if (
      actorEntities.length <= 1 &&
      objectEntities.length <= 1 &&
    ...
    var target = null;
    while (
      !_.isEmpty(actorEntities) ||
      !_.isEmpty(objectEntities) ||
      !_.isEmpty(targetEntities)
    ) {
...
var _getEmailRecipientResources = function(emailRecipientIds, callback) {
  var recipientIdsPartitioned = _.partition(
    emailRecipientIds,
    AuthzUtil.isUserId,
  );
 ...
 * @api private
 */
var _isEmailRecipientId = function(routeId) {
...
var _createEmailBucketIdForRecipient = function(recipient) {
  var timezone = TenantConfig.getValue(
    recipient.tenant.alias,
    'timezone',
    'timezone',
  );
  ...
  if (
    recipient.emailPreference !== PrincipalsConstants.emailPreferences.IMMEDIATE
  ) {
    /*!
    ...
    var offsetInHours = Math.ceil(
      new TZ.Date(timezone).getTimezoneOffset() / 60,
    );
 ...
 * @api private
 */
var _createEmailBucketId = function(
  bucketNumber,
  emailPreference,
  dayOfWeek,
  hour,
  ...
  if (emailPreference === PrincipalsConstants.emailPreferences.IMMEDIATE) {
    return util.format(
      'oae-activity-email:%s:%s',
      bucketNumber,
      emailPreference,
    );
  } else if (emailPreference === PrincipalsConstants.emailPreferences.DAILY) {
    return util.format(
      'oae-activity-email:%s:%s:%d',
      bucketNumber,
      emailPreference,
      hour,
    );
