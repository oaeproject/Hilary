...
var previewPDF = module.exports.previewPDF = function(ctx, path, callback) {
    ...
    fs.mkdir(pagesDir, function(err) {
        ...
        _convertPDFToHTMLPages(ctx, path, pagesDir, function(err) {
            ...
            _convertToText(ctx, path, ctx.baseDir + '/plain.txt', function(err) {
                if (err) {
                    return callback(err);
                }
 ...
 * @param  {PreviewContext}      ctx             The preview context associated to this file
 * @param  {String}              input           The PDF file to convert
 * @param  {String}              output          The path where the txt file should be written
 * @param  {Function}            callback        Standard callback function
 * @param  {Object}              callback.err    An error that occurred, if any
 * @api private
 */
var _convertToText = function(ctx, input, output, callback) {
    var cmd = util.format('pdftotext -nopgbrk -q "%s" "%s"', input, output);
     // Execute the command
    log().trace({'contentId': ctx.contentId}, 'Executing %s', cmd);
    var options = {
        'timeout': _pdftkTimeout,
    ...
    exec(cmd, options, function (err, stdout, stderr) {
        if (err) {
            log().error({'err': err, 'contentId': ctx.contentId, 'stdout': stdout, 'stderr': stderr}, 'Could not convert a PDF page to plain text');
            return callback({'code': 500, 'msg': 'Could not convert a PDF page to a PNG'});
        }
