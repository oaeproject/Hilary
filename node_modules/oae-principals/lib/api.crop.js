...
var storePicture = module.exports.storePicture = function(ctx, principalId, file, callback) {
    ...
    canManagePrincipal(ctx, principalId, function(err, principal) {
        if (err) {
            return callback(err);
        }
        ...
        if (PrincipalsUtil.isUser(principalId)) {
            PrincipalsUtil.invalidateCachedUser(principalId, function(err) {
                if (err) {
                    log().error({'err': err}, 'Could not invalidate the user after storing a large picture.');
                    return callback(err);
                }
                // Store it.
...
var _storePicture = function(ctx, principal, file, callback) {
    ContentAPI.getStorageBackend(ctx).store(ctx, file, function(err, largePictureUri) {
        ...
        Cassandra.runQuery(q.query, q.parameters, function(err) {
            ...
            principal.largePicture = '/api/download/signed?uri=' + largePictureUri + '&signature=' + Signature.sign(largePictureUri);
            principal.largePictureUri = largePictureUri;
...
var _generateSizes = function(ctx, principal, x, y, width, callback) {
    ...
    ContentAPI.getStorageBackend(ctx).get(ctx, principal.largePictureUri, function(err, file) {
        ...
        cropAndResize(file.path, selectedArea, function(err, smallPicture, mediumPicture) {
            if (err) {
                return callback(err);
            }
            ...
            ContentAPI.getStorageBackend(ctx).store(ctx, smallPicture, function(err, smallPictureUri) {
                ...
                ContentAPI.getStorageBackend(ctx).store(ctx, mediumPicture, function(err, mediumPictureUri) {
                    ...
                    saveUris(principal, smallPictureUri, mediumPictureUri, callback);
                });
            });
        });
    });
 ...
 * @api private
 */
var cropAndResize = function(path, selectedArea, callback) {
 ...
 * @param {Function}    callback                Standard callback function.
 * @param {Object}      callback.err            An error object (if any.)
 * @param {String}      callback.tempPath       The path where the cropped file has been written to.
 * @api private
 */
var cropPicture = function(path, selectedArea, callback) {
    ...
    imageMagick(path).size(function (err, size) {
        ...
        // A user can crop out a 1x1 pixel as profile picture.
        // Crop outside the image is not allowed though.
 ...
 * If the principal is a user, the cache will be invalidated first.
 *
 ...
 * @api private
 */
var saveUris = function(principal, smallPictureUri, mediumPictureUri, callback) {
    // Invalidate the cache
    if (PrincipalsUtil.isUser(principal.id)) {
        PrincipalsUtil.invalidateCachedUser(principal.id, function(err) {
            if (err) {
                log().error({'err': err}, 'Could not invalidate the user after cropping the large picture.');
                return callback(err);
            }
            saveUrisToCassandra(principal, smallPictureUri, mediumPictureUri, callback);
        });
    }
    else {
        saveUrisToCassandra(principal, smallPictureUri, mediumPictureUri, callback);
 ...
 * @api private
 */
var saveUrisToCassandra = function(principal, smallPictureUri, mediumPictureUri, callback) {
    var values = {
    ...
    Cassandra.runQuery(q.query, q.parameters, function(err) {
        ...
        principal.smallPicture = '/api/download/signed?uri=' + smallPictureUri + '&signature=' + Signature.sign(smallPictureUri);
        principal.smallPictureUri = smallPictureUri;
        principal.mediumPicture = '/api/download/signed?uri=' + smallPictureUri + '&signature=' + Signature.sign(smallPictureUri);
        principal.mediumPictureUri = mediumPictureUri;
        callback(null, principal);
    });
};
