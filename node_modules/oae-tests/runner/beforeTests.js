...
var setUpTenants = function(callback) {
    ...
    RestAPI.Tenants.createTenant(globalAdminRestContext, 'camtest', 'Cambridge University Test', 'cambridge.oae.com', function(err, tenant) {
        if (err) {
            log().error({err: err});
            return callback(err);
        }
        ...
        RestAPI.Tenants.createTenant(globalAdminRestContext, 'gttest', 'Georgia Tech Test', 'gt.oae.com', function(err, tenant) {
            if (err) {
                return callback(err);
            }
            ...
            RestAPI.Tenants.createTenant(globalAdminRestContext, 'localhost', 'Tenant with a hostname set to localhost', 'localhost:2001', function(err, tenant) {
                if (err) {
                    return callback(err);
                }
