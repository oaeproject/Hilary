...
/**
 * Adds the following properties to the content profile
        ...
        _getAllRoles(ctx, contentObj, function(err, roles) {
            ...
            AuthzAPI.resolveImplicitRole(ctx, contentObj.id, contentObj.tenant.alias, contentObj.visibility, ContentConstants.roles.ALL_PRIORITY, function(err, implicitRole, canInteract) {
                ...
                _canShareContent(ctx, contentObj, roles, function(err, canShare) {
                    ...
                    // Add a signature that allows the user to subscribe to the content item's activity stream
                    ContentUtil.augmentContent(ctx, contentObj);
...
var createLinkedData = module.exports.createLinkedData = function(ctx, displayName, description, visibility, uri, additionalMembers, folders, callback) {
    callback = callback || function() {};

    // Setting content to default if no visibility setting is provided
    visibility = visibility || Config.getValue(ctx.tenant().alias, 'visibility', 'linked-data');

    // TODO: Check if we have this item, disambiguation, blabalbla
    ...
    var contentId = _generateContentId(ctx.tenant().alias);
    var revisionId = _generateRevisionId(contentId);
    _createContent(ctx, contentId, revisionId, 'linkeddata', displayName, description, visibility, additionalMembers, folders, {'uri': uri}, {}, function(err, content, revision) {
        if (err) {
            return callback(err);
        }
 ...
 */
var createFile = module.exports.createFile = function(ctx, displayName, description, visibility, file, extra, additionalMembers, folders, callback) {
    ...
    return _createFile(ctx, displayName, description, visibility, file, extra, additionalMembers, folders, cleanUpCallback);
};
 ...
 * @api private
 */
var _createFile = function(ctx, displayName, description, visibility, file, extra, additionalMembers, folders, callback) {
    callback = callback || function() {};
    ...
    ContentUtil.getStorageBackend(ctx).store(ctx, file, options, function(err, uri) {
        ...
        if (extra) {
            otherValues.extra = JSON.stringify(extra);
        }
