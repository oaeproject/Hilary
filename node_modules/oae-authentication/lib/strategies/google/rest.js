 ...
 */
OAE.tenantRouter.on('get', '/api/auth/google', function(req, res, next) {
    ...
    // Optionally limit authentication to a specific google apps account/domain
    var hostedDomain = AuthenticationConfig.getValue(req.ctx.tenant().alias, AuthenticationConstants.providers.GOOGLE, 'hostedDomain');
    if (hostedDomain) {
        options.hostedDomain = hostedDomain;
    }

    passport.authenticate(strategyId, options)(req, res, errorHandler);
});
