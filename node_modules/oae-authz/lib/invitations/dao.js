...
var getAllInvitationsByResourceId = module.exports.getAllInvitationsByResourceId = function(resourceId, callback, _invitations, _nextToken) {
    ...
    Cassandra.runQuery('SELECT * FROM "AuthzInvitations" WHERE "resourceId" = ? AND "email" > ? ORDER BY "email" ASC LIMIT 150', [resourceId, email], function(err, rows) {
        if (err) {
            return callback(err);
        } else if (_.isEmpty(rows)) {
...
var createInvitations = module.exports.createInvitations = function(resourceId, emailRoles, inviterUserId, callback) {
    getOrCreateTokensByEmails(_.keys(emailRoles), function(err, emailTokens) {
        ...
        var invitations = _.map(emailRoles, function(role, email) {
            return new Invitation(resourceId, email, inviterUserId, role);
        });
        var queries = _.chain(invitations)
            .map(function(invitation) {
                return [
                    Cassandra.constructUpsertCQL('AuthzInvitations', 'resourceId', invitation.resourceId, invitation.toStorage()),
                    Cassandra.constructUpsertCQL('AuthzInvitationsResourceIdByEmail', 'email', invitation.email, {'resourceId': invitation.resourceId})
                ];
            })
            .flatten()
            .value();
        ...
        Cassandra.runBatchQuery(queries, function(err) {
            ...
            return callback(null, emailTokens, invitations);
        });
    });
};
