...
var _search = function(ctx, opts, callback) {
    ...
    var query = null;
    if (queryParam === SearchConstants.query.ALL) {
        query = SearchUtil.createQueryStringQuery(opts.q);
    } else {
        query = {
            'bool': {
                'should': [
        ...
        if (_includesResourceType(opts, 'content')) {
            query.bool.should.push(SearchUtil.createHasChildQuery(ContentConstants.search.MAPPING_CONTENT_COMMENT, SearchUtil.createQueryStringQuery(opts.q, ['body^0.5']), 'max'));
        }

        // For discussions, include their messages
        if (_includesResourceType(opts, 'discussion')) {
            query.bool.should.push(SearchUtil.createHasChildQuery(DiscussionsConstants.search.MAPPING_DISCUSSION_MESSAGE, SearchUtil.createQueryStringQuery(opts.q, ['body^0.5']), 'max'));
        }
    }

    // The filter object for the Query DSL
    var filter = null;
    var typeFilter = SearchUtil.filterTerm('_type', SearchConstants.search.MAPPING_RESOURCE);

    // The resource type filter will filter by a particular document type (e.g., content, user, group)
    var resourceTypeFilter = (!opts.searchAllResourceTypes) ? SearchUtil.filterTerms('resourceType', opts.resourceTypes) : null;
    ...
    var accessFilter = (ctx.user() && ctx.user().isTenantAdmin(ctx.tenant().alias)) ?
            SearchUtil.filterTerm('tenantAlias', ctx.tenant().alias) : SearchUtil.createHasChildQuery(AuthzConstants.search.MAPPING_RESOURCE_MEMBERS, SearchUtil.filterTerms('direct_members', opts.access));
...
var _needsFilterByAccess = function(ctx, opts) {
    ...
    var includesContentOrGroups = (opts.searchAllResourceTypes || _.intersection(RESOURCE_TYPES_ACCESS_SCOPED, opts.resourceTypes).length > 0);
