        ...
        it('verify index updated group', function(callback) {
            ...
            RestAPI.User.createUser(camAdminRestContext, doerUsername, 'password', 'Doer', null, function(err, doer) {
                ...
                TestsUtil.generateTestGroups(doerRestContext, 1, function(err, groups) {
                    ...
                    // Verify that the group does not match on the term 'Xyz', this is to sanity check the search before searching for a valid post-update hit later
                    SearchTestsUtil.searchAll(doerRestContext, 'general', null, {'resourceTypes': 'group', 'q': 'Xyz'}, function(err, results) {
                        assert.ok(!err);
                        assert.ok(!_getDocById(results, xyzTeam.id));
        
                        // Update name to "Xyz Team"
                        RestAPI.Group.updateGroup(doerRestContext, xyzTeam.id, {'displayName': 'Xyz Team'}, function(err) {
                            assert.ok(!err);
        
                            // Verify that the group now appears with the search term 'Xyz'
                            SearchTestsUtil.searchAll(doerRestContext, 'general', null, {'resourceTypes': 'group', 'q': 'Xyz'}, function(err, results) {
                                assert.ok(!err);
                                assert.ok(_getDocById(results, xyzTeam.id));
                                callback();
