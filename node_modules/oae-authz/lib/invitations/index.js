 ...
 */
var getAllInvitations = (module.exports.getAllInvitations = function(
  ctx,
  resource,
  callback,
) {
  AuthzPermissions.canManage(ctx, resource, function(err) {
    ...
    AuthzInvitationsDAO.getAllInvitationsByResourceId(resourceAuthzId, function(
      err,
      invitationHashes,
    ) {
      if (err) {
 ...
 * @api private
 */
var _invitationsFromHashes = function(
  ctx,
  resource,
  invitationHashes,
  callback,
  ...
  var inviterUserIds = _.chain(invitationHashes)
    .pluck('inviterUserId')
    .uniq()
    .value();
  PrincipalsUtil.getPrincipals(ctx, inviterUserIds, function(
    err,
    principalsById,
  ) {
    if (err) {
