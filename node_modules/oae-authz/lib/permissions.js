 ...
 */
var resolveEffectivePermissions = (module.exports.resolveEffectivePermissions = function(
  ctx,
  resource,
  callback,
) {
  AuthzAPI.resolveEffectiveRole(
    ...
    function(err, effectiveRole, canInteract) {
      if (err) {
        return callback(err);
      }
...
var canManage = (module.exports.canManage = function(ctx, resource, callback) {
  ...
  AuthzAPI.resolveImplicitRole(
    ...
    resource,
    AuthzConstants.role.ALL_PRIORITY,
    function(err, implicitRole, canInteract) {
      if (err) {
        return callback(err);
 ...
 * @param  {Object}     callback.err    Thrown if the permission check fails
 */
var canManageMessage = (module.exports.canManageMessage = function(
  ctx,
  parentResource,
  message,
  callback,
...
var canView = (module.exports.canView = function(ctx, resource, callback) {
  ...
  AuthzAPI.resolveImplicitRole(
    ...
    function(err, implicitRole, canInteract) {
      ...
      AuthzAPI.hasAnyRole(user.id, AuthzUtil.getAuthzId(resource), function(
        err,
        hasAnyRole,
      ) {
        if (err) {
 ...
 */
var canShare = (module.exports.canShare = function(
  ctx,
  resource,
  targets,
  role,
  ...
  resolveEffectivePermissions(ctx, resource, function(err, permissions) {
    ...
    _validateRoleChanges(
      ...
      targets,
      { promoteOnly: true },
      function(err, memberChangeInfo, emailChangeInfo) {
        if (err) {
          return callback(err);
        ...
        _.each(addedPrincipals, function(addedPrincipal) {
          AuthzAPI.resolveImplicitRole(
            ...
            function(err, implicitRole, canInteract) {
              if (err) {
                invalidPrincipals[
                  addedPrincipal.id || addedPrincipal.email
                ] = err;
              } else if (!canInteract) {
...
var canJoin = (module.exports.canJoin = function(ctx, resource, callback) {
  ...
  if (!user) {
    ...
    return callback({
      code: 401,
      msg: 'The resource being joined is not joinable',
    });
  }
 ...
 */
var canRemoveRole = (module.exports.canRemoveRole = function(
  ctx,
  principal,
  resource,
  callback,
      ...
      function(err, memberChangeInfo) {
        if (err) {
          ...
          return callback({
            code: 400,
            msg:
              'The principal being removed is not currently a member of the resource',
          });
        } else if (
          ...
          return callback({
            code: 400,
            msg:
              'The requested change will result in leaving the resource without a manager',
          });
        }
 ...
 */
var canSetRoles = (module.exports.canSetRoles = function(
  ctx,
  resource,
  targetRoles,
  callback,
 ...
 */
var canCreate = (module.exports.canCreate = function(
  ctx,
  targetRoles,
  callback,
) {
  if (!ctx.user()) {
    return callback({
      code: 401,
      msg: 'Anonymous users are not authorized to create resources',
    });
  }

  _validateRoleChanges(ctx, null, targetRoles, { promoteOnly: false }, function(
    err,
    memberChangeInfo,
    emailChangeInfo,
  ) {
 ...
 */
var canInteract = (module.exports.canInteract = function(
  ctx,
  resources,
  callback,
) {
  ...
  var permissionErr = {
    code: 401,
    msg:
      'The current user does not have access to interact with these resources',
  };
  var resourceErrs = {};
  var _done = _.after(resources.length, function() {
    if (!_.isEmpty(resourceErrs)) {
      return callback(
        _.extend(permissionErr, { invalidResources: resourceErrs }),
      );
    }
 ...
 * @api private
 */
var _validateRoleChanges = function(
  ctx,
  resource,
  targetRoles,
  opts,
  ...
  if (
    ...
    !TenantsUtil.canInviteGuests(ctx.user().tenant.alias)
  ) {
    return callback({
      code: 401,
        ...
        function(err, emailChangeInfo) {
          ...
          canInteract(ctx, checkProfileInteraction, function(err) {
            var interactionErr = {
              code: 401,
              msg:
                'The current user does not have access to add the specified principals',
            };
            ...
            var invalidPrincipals = _.filter(checkTenantInteraction, function(
              resource,
            ) {
              return !TenantsUtil.canInteract(
    ...
    function(err, implicitRole, canInteract) {
      if (err) {
        ...
        if (!user) {
          ...
          AuthzUtil.isUserId(resource.id)
        ) {
 ...
 * @api private
 */
var _computeMemberRolesAfterChanges = function(
  resource,
  principalTargetRoles,
  opts,
  callback,
  ...
  AuthzAPI.computeMemberRolesAfterChanges(authzId, roleChanges, opts, function(
    err,
    idChangeInfo,
  ) {
    if (err) {
    ...
    var principalsById = _.chain(principalTargetRoles)
      .pluck('principal')
      .indexBy('id')
      .value();
    ...
    return callback(
      null,
      AuthzModel.MemberChangeInfo.fromIdChangeInfo(
        idChangeInfo,
        principalsById,
      ),
    );
 ...
 * @api private
 */
var _computeInvitationRolesAfterChanges = function(
  resource,
  emailTargetRoles,
  opts,
  callback,
