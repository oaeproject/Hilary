 ...
 * TODO
 */
var getContent = module.exports.getContent = function(currentUser, contentId, callback) {
    ...
    Cassandra.runQuery('SELECT * FROM Content WHERE contentId = ?', [contentId], function (err, rows) {
        ...
        _hasAccess(currentUser, contentObj, function(err, hasAccess) {
            if (err) {
                return callback(err);
            }
 ...
 * @param  {String}                  tenant            The tenant to which this piece of content will be asscoiated //TODO: Use context object
 ...
 */
var createContent = module.exports.createContent = function(tenant, currentUser, visibility, title, description, contentType, callback) {
    ...
    validator.check(tenant, {'code': 400, 'msg': 'A tenant must be associated to the piece of content'}).notEmpty();
    validator.check(null, {'code': 401, 'msg': 'You have to be logged in to be able to create a content item'}).isUser(currentUser);
    if (validator.hasErrors()) {
        return callback(validator.getErrors());
    }
    ...
    var contentId = RolesUtil.toUuid('c', tenant, ShortId.generate());
    var lastModified = new Date().getTime();
    description = description || '';

    // Create the content
    Cassandra.runQuery('INSERT INTO Content (contentId, tenant, visibility, title, description, contentType, lastModified) VALUES (?, ?, ?, ?, ?, ?, ?)', [contentId, tenant, visibility, title, description, contentType, lastModified], function (err) {
        if (err) {
            callback({'code': 500, 'msg': err});
        } else {
            var contentObj = new Content(tenant.alias, contentId, visibility, title, description, contentType, lastModified);
            // Now make the current user a manager of the content
            var newPermissions = {};
            newPermissions[currentUser.id] = ContentConstants.roles.MANAGER;
 ...
 * TODO
 */
module.exports.deleteContent = function(currentUser, contentId, callback) {
    callback = callback || function() {};

    // Parameter validation
    var validator = new Validator();
    validator.check(contentId, {'code': 400, 'msg': 'A content id must be provided'}).notEmpty();
    validator.check(null, {'code': 401, 'msg': 'You have to be logged in to be able to create a conten item'}).isUser(currentUser);
    if (validator.hasErrors()) {
        return callback(validator.getErrors());
    }

    // Fist check whether or not the current user is a manager of the piece of content
    canManage(currentUser, contentId, function(err, canManage) {
        if (err) {
            return callback(err);
        }
        // Get the members
 ...
 * TODO
 */
var shareContent = module.exports.shareContent = function(principal_id, content_id, role, callback) {
 ...
 * TODO
 */
var canManage = module.exports.canManage = function(currentUser, contentId, callback) {
    getContent(currentUser, contentId, function(err, contentObj) {
        if (err) {
            return callback(err);
        }
        _canManage(currentUser, contentId, function(err, canManage) {
            if (err) {
                return callback(err);
            }
            callback(null, true);
 ...
 * TODO
 */
var _canManage = function(currentUser, contentId, callback) {
    PermissionsAPI.isAllowed(currentUser.id, ContentConstants.roles.MANAGER, contentId, function(err, canManage) {
        if (err) {
            return callback(err);
        } else if (!canManage) {
 ...
 * TODO
 */
var hasAccess = module.exports.hasAccess = function(currentUser, contentId, callback) {
    getContent(currentUser, contentId, function(err, contentObj) {
        if (err) {
            return callback(err);
        }
 ...
 * TODO
 */
var _hasAccess = function(currentUser, contentObj, callback) {
    ...
    // If the content is private
        ...
        PermissionsAPI.isAllowedAny(currentUser.id, contentObj.contentId, function(err, hasAccess, two, three) {
            if (err) {
                return callback(err);
            } else if (!hasAccess) {
                return callback({'code': 401, 'msg': 'You are not a manager of this piece of content'});
 ...
 * TODO
 */
var setContentPermissions = module.exports.setContentPermissions = function(currentUser, contentId, newPermissions, callback) {
    callback = callback || {};
    // Parameter validation
    var validator = new Validator();
    validator.check(contentId, {'code': 400, 'msg': 'A content id must be provided'}).notEmpty();
    validator.check(null, {'code': 403, 'msg': 'You have to be logged in to be able to create a conten item'}).isUser(currentUser);
    if (validator.hasErrors()) {
        return callback(validator.getErrors());
    }
 ...
 * TODO
 */
var getContentMembers = module.exports.getContentMembers = function(currentUser, contentId, callback) {
 ...
 * TODO
 */
var updateContentProfile = module.exports.updateContentProfile = function(contentId, profileFields, callback) {
 ...
 * TODO
 */
var updatePrivacySettings = module.exports.updatePrivacySettings = function(contentId, visibility, callback) {
 ...
 * TODO
 */
var getLibraryItems = module.exports.getLibraryItems = function(currentUser, principalId, callback) {
    var validator = new Validator();
    ...
    // TODO: Check if I have access to the principalId

    if (!currentUser) {
        principalId = principalId + ':public';
    } else if (currentUser.id !== principalId) {
        principalId = principalId + ':loggedin';
    }

    Cassandra.runQuery("SELECT * FROM LibraryByPrinciple WHERE principalId = ?", [principalId], function (err, rows) {
        if (err) {
            return callback({'code': 500, 'msg': err});
        }
        ...
        // TODO: Add the actual profile info of the content
        callback(null, items);
    });
};
