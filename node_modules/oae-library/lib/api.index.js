 ...
 */
var registerLibraryIndex = (module.exports.registerLibraryIndex = function(
  name,
  options,
) {
...
var insert = (module.exports.insert = function(indexName, entries, callback) {
  ...
  var indexEntries = _.map(entries, function(entry) {
    return {
      ...
      value: entry.value,
      visibility: LibraryAuthz.resolveLibraryBucketVisibility(
        entry.id,
        entry.resource,
      ),
    };
...
var update = (module.exports.update = function(indexName, entries, callback) {
  ...
  _.each(entries, function(entry) {
    ...
    var oldRankedResourceId = _createRankedResourceId(
      entry.resource.id,
      entry.oldRank,
    );
    var newRankedResourceId = _createRankedResourceId(
      entry.resource.id,
      entry.newRank,
    );
    ...
    _.each(visibilityMasks, function(mask, bucketName) {
      queries.push({
        query:
          'DELETE FROM "LibraryIndex" WHERE "bucketKey" = ? AND "rankedResourceId" = ?',
        parameters: [
...
var remove = (module.exports.remove = function(indexName, entries, callback) {
  ...
  var queries = [];
  _.each(entries, function(entry) {
    var rankedResourceId = _createRankedResourceId(
      entry.resource.id,
      entry.rank,
    );
    _.each(visibilityMasks, function(mask, bucketName) {
 ...
 */
var whenUpdatesComplete = (module.exports.whenUpdatesComplete = function(
  callback,
) {
  ...
  _query(
    ...
    function(err, rows, nextToken) {
      ...
      _.each(rows, function(rawEntry) {
        numResults++;
        _splitRankedResourceId(rawEntry.rankedResourceId, function(
          resourceId,
          rank,
        ) {
      ...
      if (!_.isEmpty(rankedResourceIdsToDelete)) {
        ...
        var deleteQueries = _.map(rankedResourceIdsToDelete, function(
          rankedResourceId,
        ) {
          return {
            query:
...
var purge = (module.exports.purge = function(indexName, libraryId, callback) {
  ...
  log().trace(
    { indexName: indexName, libraryId: libraryId },
    'Purging library index',
  );
 ...
 */
var isStale = (module.exports.isStale = function(
  indexName,
  libraryId,
  visibility,
  callback,
  ...
  Cassandra.runPagedQuery(
    ...
    function(err, rows, nextToken) {
      ...
      try {
        ...
        return callback({
          err: 500,
          msg: 'An unexpected error occurred parsing library data',
        });
      }
...
var _build = function(indexName, libraryId, callback, _nextToken) {
  ...
  registeredIndex.pageResources(libraryId, _nextToken, 100, function(
    err,
    entries,
    nextToken,
  ) {
...
var _insert = function(indexName, indexEntries, callback) {
  ...
  _.each(indexEntries, function(indexEntry) {
    ...
    var visibilityIndex = AuthzConstants.visibility.ALL_PRIORITY.indexOf(
      indexEntry.visibility,
    );
    var visibilitiesToInsert = AuthzConstants.visibility.ALL_PRIORITY.slice(
      visibilityIndex,
    );
 ...
 * @api private
 */
var _adjustColumnsForSlugs = function(
  indexName,
  libraryId,
  start,
  limit,
