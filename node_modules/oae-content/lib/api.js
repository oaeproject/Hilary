...
var _createContent = function(ctx, contentType, displayName, description, visibility, additionalMembers, otherValues, callback) {
    ...
    var parameters = {
        ...
        'created': Date.now(),
        'previews': JSON.stringify({'status': 'pending'}) // FIXME: Once constructUpsertCQL supports objects, this can be a plain object.
    };
...
var setPreviewItems = module.exports.setPreviewItems = function(ctx, contentId, status, files, sizes, metadata, callback) {
    ...
    getContent(ctx, contentId, function(err, contentObj) {
        ...
        if (status === ContentConstants.previews.ERROR || todo === 0) {
            return _storePreviewData(ctx, contentObj, status, thumbnailUri, metadata, fileData, callback);
        }
        ...
        fileKeys.forEach(function(key) {
            ...
            getStorageBackend(ctx).store(ctx, files[key], function(err, uri) {
                ...
                if (todo === 0 && !called) {
                    ...
                    _storePreviewData(ctx, contentObj, status, thumbnailUri, metadata, fileData, callback);
                }
            });
        });
    });
...
var _storePreviewData = function(ctx, contentObj, status, thumbnailUri, metadata, fileData, callback) {
    ...
    if (contentObj.previews.total && contentObj.previews.total > 0) {
        queries.push({
            ...
            'parameters': [contentObj.contentId]
        });
    }
    ...
    if (nrOfFiles > 0) {
        queries.push(Cassandra.constructUpsertCQL('ContentPreviews', 'contentId', contentObj.contentId, fileData));
    }
    ...
    queries.push({
        ...
        'parameters': [previews, contentObj.contentId]
    });
...
var verifySignedPreviewItem = module.exports.verifySignedPreviewItem = function(ctx, contentId, previewItem, signature, expires, callback) {
    ...
    var body = principalId + ':' + contentId;
    validator.check(signature, {'code': 401, 'msg': 'Invalid signature.'}).verifyExpiringSignature(tenantAlias, expires, body);
    if (validator.hasErrors()) {
        return callback(validator.getFirstError());
    }
...
var getPreviewItems = module.exports.getPreviewItems = function(ctx, contentId, callback) {
    getContent(ctx, contentId, function(err, contentObj) {
        ...
        Cassandra.runQuery('SELECT * FROM ContentPreviews WHERE contentId = ?', [contentId], function(err, rows) {
            ...
            var signature = _signContent(ctx, contentId);
...
var getLibraryItems = module.exports.getLibraryItems = function(ctx, principalId, start, limit, callback) {
    ...
    if (PrincipalsUtil.isUser(principalId)) {
        ...
        return _getLibraryItems(principalId, start, limit, augment);
 ...
 */
var getThumbnailUrl = module.exports.getThumbnailUrl = function(ctx, contentId, signature) {
    if (!signature) {
        signature = _signContent(ctx, contentId);
    }
...
var _augmentContent = function(ctx, contentObj) {
    ...
    contentObj.signature = _signContent(ctx, contentObj.contentId);

    if (contentObj.previews.thumbnailUri) {
        contentObj.previews.thumbnailUrl = getThumbnailUrl(ctx, contentObj.contentId, contentObj.signature);
    }
};

/**
 * Creates the appropriate signature for a piece of content.
 * TODO: Include the current user his ID in the body.
 *
 * @param  {Context}    ctx         Standard context object, representing the currently logged in user and its tenant
 * @param  {String}     contentId   The ID of the piece of content that needs to be signed.
 * @return {Object}                 A signature.
 */
var _signContent = function(ctx, contentId) {
    var tenantAlias = (contentId) ? AuthzUtil.getResourceFromId(contentId).tenantAlias : '';
    var principalId = (ctx && ctx.user()) ? ctx.user().principalId : '';
    var settings = _getPreviewsExpireSettings(tenantAlias);
    var body = principalId + ':' + contentId;
    return Signature.createRollingSignature(tenantAlias, settings.unit, settings.value, settings.offsetUnit, settings.offsetValue, body);
};
 ...
 * @return {Object}             An object that holds `unit`, `value`, `offsetUnit` and `offsetValue` keys which can be used to create a rolling signature.
 */
var _getPreviewsExpireSettings = function(tenantAlias) {
    return {
        'unit': Config.getValue(tenantAlias, 'previews', 'unit'),
        'value': parseInt(Config.getValue(tenantAlias, 'previews', 'value'), 10),
        'offsetUnit': Config.getValue(tenantAlias, 'previews', 'offsetUnit'),
        'offsetValue': parseInt(Config.getValue(tenantAlias, 'previews', 'offsetValue'), 10)
    };
};