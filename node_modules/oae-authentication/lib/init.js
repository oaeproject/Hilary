...
var setupPassportSerializers = function(cookieSecret) {
  ...
  passport.serializeUser(function(oaeAuthInfo, done) {
    ...
    if (oaeAuthInfo.user) {
      ...
      if (oaeAuthInfo.imposter) {
        ...
        var strategyName = AuthenticationUtil.parseStrategyId(
          oaeAuthInfo.strategyId,
        ).strategyName;
        AuthenticationAPI.emit(
    ...
    var cookieData = _encryptCookieData(
  ...
  passport.deserializeUser(function(toDeserialize, callback) {
    ...
    PrincipalsDAO.getPrincipal(sessionData.userId, function(err, user) {
      ...
      PrincipalsDAO.getPrincipal(sessionData.imposterId, function(
        err,
        imposterUser,
      ) {
        if (err && err.code === 404) {
