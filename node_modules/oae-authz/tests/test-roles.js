...
describe('Authz-Roles', function() {
  ...
  var loadContentRoles = function(
    ...
    var resourceId = AuthzUtil.toId(
      resourceType,
      tenantAlias,
      baseContentId + '-' + numContentItems,
    );
    AuthzAPI.updateRoles(resourceId, makeChange(principalId, role), function(
      err,
    ) {
      if (err) {
        throw err;
  ...
  describe('#getAllRoles()', function() {
    ...
    it('verify non-principal id error', function(callback) {
      AuthzAPI.getAllRoles('c:cam:mrvisser', 'c:cam:Foo.docx', function(
        err,
        roles,
      ) {
        assert.ok(err);
    ...
    it('verify direct single role association', function(callback) {
      ...
      AuthzAPI.updateRoles(resourceId, makeChange(userId, 'viewer'), function(
        err,
      ) {
        assert.ok(!err);
        AuthzAPI.getAllRoles(userId, resourceId, function(err, roles) {
    ...
    it('verify indirect single role association', function(callback) {
      ...
      AuthzAPI.updateRoles(groupId, makeChange(userId, 'member'), function(
        err,
      ) {
        assert.ok(!err);
        AuthzAPI.updateRoles(
          resourceId,
          makeChange(groupId, 'viewer'),
          function(err) {
            assert.ok(!err);
            AuthzAPI.getAllRoles(userId, resourceId, function(err, roles) {
              assert.ok(!err);
    ...
    it('verify multi-indirect two role association', function(callback) {
      ...
      AuthzAPI.updateRoles(groupId1, makeChange(userId, 'member'), function(
        err,
      ) {
        assert.ok(!err);
        AuthzAPI.updateRoles(groupId2, makeChange(userId, 'member'), function(
          err,
        ) {
          assert.ok(!err);
          AuthzAPI.updateRoles(
            resourceId,
            makeChange(groupId1, 'viewer'),
            function(err) {
              assert.ok(!err);
              AuthzAPI.updateRoles(
                resourceId,
                makeChange(groupId2, 'manager'),
                function(err) {
                  assert.ok(!err);
                  AuthzAPI.getAllRoles(userId, resourceId, function(
                    err,
                    roles,
                  ) {
                    assert.ok(!err);
    ...
    it('verify circular group hierarchy three role association', function(callback) {
      ...
      AuthzAPI.updateRoles(groupId1, makeChange(userId, 'member'), function(
        ...
        assert.ok(!err);
        AuthzAPI.updateRoles(groupId2, makeChange(groupId1, 'member'), function(
          err,
        ) {
          assert.ok(!err);
          AuthzAPI.updateRoles(
            groupId3,
            makeChange(groupId2, 'member'),
            function(err) {
              assert.ok(!err);
              AuthzAPI.updateRoles(
                groupId1,
                makeChange(groupId3, 'member'),
                function(err) {
                  assert.ok(!err);
                  AuthzAPI.updateRoles(
                    resourceId,
    ...
    it('verify role separation between tenants', function(callback) {
      var principalIdA = AuthzUtil.toId(
        PrincipalTypes.USER,
        'testTenantSeparationA',
        'mrvisser',
      );
      var principalIdB = AuthzUtil.toId(
        PrincipalTypes.USER,
        'testTenantSeparationB',
        'mrvisser',
      );
      var resourceId = AuthzUtil.toId(
      ...
      AuthzAPI.updateRoles(
        resourceId,
        makeChange(principalIdA, 'manager'),
        function(err) {
          assert.ok(!err);
          ...
          AuthzAPI.getAllRoles(principalIdB, resourceId, function(err, roles) {
            ...
            AuthzAPI.updateRoles(
              resourceId,
              makeChange(principalIdB, 'viewer'),
              function(err) {
                assert.ok(!err);

                // ensure user from tenant A is still manager, not viewer
                AuthzAPI.hasRole(principalIdA, resourceId, 'manager', function(
                  err,
                  hasRole,
                ) {
                  assert.ok(!err);
                  ...
                  AuthzAPI.hasRole(principalIdB, resourceId, 'viewer', function(
                    err,
                    hasRole,
                  ) {
                    assert.ok(!err);
  ...
  describe('#hasRole()', function() {
    it('verify negative check without any roles', function(callback) {
      var principalId = AuthzUtil.toId(
        PrincipalTypes.USER,
        'testHasRoleWithout',
        'mrvisser',
      );
      AuthzAPI.hasRole(principalId, 'c:cam:nonExistent', 'manager', function(
        err,
        hasRole,
      ) {
        assert.ok(!err);
    ...
    it('verify positive check with a role', function(callback) {
      ...
      AuthzAPI.updateRoles(
        resourceId,
        makeChange(principalId, 'manager'),
        function(err) {
          assert.ok(!err);

          // verify that hasRole reports that the user has the manager role
          AuthzAPI.hasRole(principalId, resourceId, 'manager', function(
            err,
            hasRole,
          ) {
            assert.ok(!err);
  ...
  describe('#applyRoleChanges()', function() {
    it('verify update existing role', function(callback) {
      ...
      AuthzAPI.updateRoles(
        resourceId,
        makeChange(principalId, 'viewer'),
        function(err) {
          assert.ok(!err);
          AuthzAPI.hasRole(principalId, resourceId, 'viewer', function(
            err,
            hasRole,
          ) {
            assert.ok(!err);
    ...
    it('verify general functionality', function(callback) {
      var principalId1 = AuthzUtil.toId(
        PrincipalTypes.USER,
        'testHasRole',
        'mrvisser',
      );
      var principalId2 = AuthzUtil.toId(
        PrincipalTypes.USER,
        'testHasRole',
        'nm417',
      );
      var principalId3 = AuthzUtil.toId(
        PrincipalTypes.USER,
        'testHasRole',
        'simong',
      );
      var principalId4 = AuthzUtil.toId(
        PrincipalTypes.USER,
        'testHasRole',
        'PhysX',
      );
      var resourceId1 = AuthzUtil.toId(
        ResourceTypes.CONTENT,
        'testHasRole',
        'testHasRoleContent1',
      );
      var resourceId2 = AuthzUtil.toId(
        ResourceTypes.CONTENT,
        'testHasRole',
        'testHasRoleContent2',
      );
      var resourceId3 = AuthzUtil.toId(
        ResourceTypes.CONTENT,
        'testHasRole',
        'testHasRoleContent3',
      );
      ...
      AuthzAPI.updateRoles(resourceId1, roles, function(err) {
        assert.ok(!err);
        AuthzAPI.hasRole(principalId1, resourceId1, 'manager', function(
          err,
          hasRole,
        ) {
          assert.ok(!err);
          assert.ok(hasRole);
          AuthzAPI.hasRole(principalId2, resourceId1, 'manager', function(
            err,
            hasRole,
          ) {
            assert.ok(!err);
            ...
            AuthzAPI.updateRoles(resourceId2, roles, function(err) {
              assert.ok(!err);
              AuthzAPI.hasRole(principalId1, resourceId2, 'manager', function(
                err,
                hasRole,
              ) {
                assert.ok(!err);
                assert.ok(hasRole);
                AuthzAPI.hasRole(principalId2, resourceId2, 'manager', function(
                  err,
                  hasRole,
                ) {
                  assert.ok(!err);
                ...
                function() {
                  ...
                  AuthzAPI.getRolesForPrincipalsAndResourceType(
                    ...
                    function(err, entries) {
                      ...
                      assert.strictEqual(
                        _.keys(entries[principalId1]).length,
                        300,
                      );
                      assert.strictEqual(
                        _.keys(entries[principalId2]).length,
                        350,
                      );

                      // Verify that mrvisser is a viewer of all items, and that simong is a
                      // manager of all items
                      _.each(entries[principalId1], function(role, resourceId) {
                        assert.strictEqual(role, 'viewer');
                      });

                      _.each(entries[principalId2], function(role, resourceId) {
