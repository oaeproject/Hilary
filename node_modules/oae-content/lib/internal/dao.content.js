    ...
    Cassandra.runQuery('DELETE FROM Content WHERE contentId = ?', [contentObj.id], function(err) {
        ...
        getAllContentMembers(contentObj.id, function(err, members) {
            ...
            AuthzAPI.updateRoles(contentObj.id, updateMembers, function(err) {
                ...
                LibraryAPI.Index.remove(LIBRARY_INDEX_NAME, membersToRemove, contentObj.id, contentObj.lastModified, function(err) {
                    if (err) {
...
var getLibraryItems = module.exports.getLibraryItems = function(principalId, visibility, start, limit, callback) {
    ...
    LibraryAPI.Index.list(LIBRARY_INDEX_NAME, principalId, visibility, start, limit, null, function(err, resourceIds, nextToken) {
        if (err) {
            return callback(err);
        }

        getMultipleContentItems(resourceIds, function(err, contentItems) {
            if (err) {
                return callback(err);
            }
...
var _updateLibraries = function(contentObj, oldLastModified, newLastModified, removedMembers, callback) {
    ...
    getAllContentMembers(contentObj.id, function(err, members) {
        ...
        var libraryVisibility = DiscussionsAPI.Authz.resolveEffectiveLibraryVisibilities(memberIds, contentObj);
        ...
        if (!oldLastModified) {
            ...
            return LibraryAPI.Index.insert(LIBRARY_INDEX_NAME, libraryVisibility, contentObj.id, newLastModified, callback);
        }
        ...
        libraryVisibility = DiscussionsAPI.Authz.resolveEffectiveLibraryVisibilities(membersToUpdate, contentObj);

        // First apply the updates
        return LibraryAPI.Index.update(LIBRARY_INDEX_NAME, libraryVisibility, contentObj.id, newLastModified, oldLastModified, function(err) {
            if (err) {
                errs.push(err);
            }
            ...
            if (removedMembers && removedMembers.length > 0) {
                ...
                return LibraryAPI.Index.remove(LIBRARY_INDEX_NAME, removedMembers, contentObj.id, oldLastModified, function(err) {
                    if (err) {
                        errs.push(err);
                    }
