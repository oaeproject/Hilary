...
var ensureSchema = function(callback) {
    Cassandra.createColumnFamily('Tenant', 'CREATE COLUMNFAMILY Tenant (port text PRIMARY KEY, alias text, name text, baseUrl text, active boolean, deleted boolean)', callback);
};
...
var registerGlobalRestEndpoints = function(app) {
    ...
    app.server.get('/api/tenant/:tenantId', function(req, res) {
        TenantAPI.getTenantByAlias(req.params.tenantId, function(err, tenant) {
            if (err) {
    ...
    app.server.get('/api/tenant', function(req, res) {
        res.send(200, OAE.serverTenant);
    });

    app.server.post('/api/tenant', function(req, res) {
        TenantAPI.updateTenant(req.ctx, req.body.port, req.body.name, function(err) {
            if (err) {
                return res.send(err.code, err.msg);
            }
            res.send(200);
    ...
    app.server.post('/api/tenant/create', function(req, res) {
        TenantAPI.createTenant(req.ctx, req.body.id, req.body.name, req.body.port, req.body.baseurl, function(err) {
            if (err) {
                return res.send(err.code, err.msg);
            }
    ...
    app.server.post('/api/tenant/start', function(req, res) {
        ...
        TenantAPI.disableTenants(req.ctx, req.body.tenants, false, function(err) {
            if (err) {
                return res.send(err.code, err.msg);
            }
            res.send(200);
    ...
    app.server.post('/api/tenant/stop', function(req, res) {
        ...
        TenantAPI.disableTenants(req.ctx, req.body.tenants, true, function(err) {
            if (err) {
                return res.send(err.code, err.msg);
            }
            res.send(200);
    ...
    app.server.post('/api/tenant/delete', function(req, res) {
        TenantAPI.deleteTenants(req.ctx, req.body.tenants, function(err) {
            if (err) {
                return res.send(err.code, err.msg);
            }
            res.send(200);
