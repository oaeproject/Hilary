...
var cassandraLog = require('oae-logger').logger('oae-cassandra');
...
describe('Utilities', function() {
    ...
    describe('Cassandra', function() {
                    ...
                    Cassandra.runQuery(`INSERT INTO "testQuery" ("keyId", "c1", "c2") VALUES (?, ?, ?)`, ['key1', 'value1', 'value2'], function (err) {
                        ...
                        Cassandra.runQuery(`INSERT INTO "testQuery" ("keyId", "c1", "c2") VALUES (?, ?, ?)`, ['key2', 'value', null], function (err) {
                            ...
                            Cassandra.runQuery(`SELECT * FROM "testQuery" WHERE "keyId" = ?`, ['key1'], function (err, rows) {
                                ...
                                assert.equal(rows[0].get('keyId'), 'key1');
            ...
            Cassandra.createColumnFamily('testIterateAllAllColumns', 'CREATE TABLE "testIterateAllAllColumns" ("keyId" text PRIMARY KEY, "colOne" text, "colTwo" text)', function(err, created) {
                ...
                Cassandra.runBatchQuery(batch, function(err) {
                    ...
                    var _onEach = function(rows, done) {
                        ...
                        assert.equal(rows[0].get('keyId'), 'key1', 'Invalid value for keyId');
                        assert.equal(rows[0].get('colOne'), 'one', 'Invalid value for colOne');
                        assert.equal(rows[0].get('colTwo'), 'two', 'Invalid value for colTwo');
                ...
                Cassandra.runBatchQuery(batch, function(err) {
                    ...
                    Cassandra.iterateAll(null, 'testIterateAllPaging', 'keyId', {'batchSize': 1}, _onEach, function(err) {
                        ...
                        for (var i = 0;  i < 10; i++) {
                            ...
                            assert.equal(allRows[key].get('colOne'), 'colOne' + i, 'Invalid colOne value');
                            assert.equal(allRows[key].get('colTwo'), 'colTwo' + i, 'Invalid colTwo value');
                        }
                        ...
                        Cassandra.iterateAll(null, 'testIterateAllPaging', 'keyId', {'batchSize': 5}, _onEach, function(err) {
                            ...
                            for (var i = 0;  i < 10; i++) {
                                ...
                                assert.equal(allRows[key].get('colOne'), 'colOne' + i);
                                assert.equal(allRows[key].get('colTwo'), 'colTwo' + i);
                            }
                ...
                Cassandra.columnFamilyExists('testBatchQuery', function(err, exists) {
                    ...
                    Cassandra.runBatchQuery(queries, function(err) {
                        ...
                        Cassandra.runQuery('SELECT * FROM "testBatchQuery" WHERE "keyId" IN ?', [['key1', 'key2']], function (err, rows) {
                            ...
                            assert.equal(rows[0].keys().length, 3);
                            assert.equal(rows[0].values().length, 3);
                            assert.equal(rows[0].get('c1'), 'value1');
                            assert.equal(rows[0].get('c2'), 'value2');

                            assert.equal(rows[1].keys().length, 3);
                            assert.equal(rows[1].values().length, 3);
                            assert.equal(rows[1].get('c1'), 'value3');
                            assert.equal(rows[1].get('c2'), 'value4');
            ...
            Cassandra.createColumnFamily('testBooleans', 'CREATE TABLE "testBooleans" ("keyId" text PRIMARY KEY, "testbool" text, "testnumbool" text, "teststring" text)', function(err, created) {
                ...
                Cassandra.runQuery('INSERT INTO "testBooleans" ("keyId", "testbool", "testnumbool", "teststring") VALUES (?, ?, ?, ?)', ['testkey', 'true', '0', 'notaboolean'], function (err) {
                    ...
                    Cassandra.runQuery('SELECT "testbool", "testnumbool", "teststring" FROM "testBooleans" WHERE "keyId" = ?', ['testkey'], function (err, rows) {
                        assert.ok(!err);
                        assert.equal(typeof OaeUtil.castToBoolean(_.first(rows).get('testbool')), 'boolean');
                        assert.equal(typeof OaeUtil.castToBoolean(_.first(rows).get('testnumbool')), 'boolean');
                        assert.equal(typeof OaeUtil.castToBoolean(_.first(rows).get('teststring')), 'string');
                        callback();
                    });
                });
            });
            ...
            Cassandra.createColumnFamily('VerifyPagedColumnQueryStartAndEnd', 'CREATE TABLE "VerifyPagedColumnQueryStartAndEnd" ("keyId" text, "columnName" text, "value" text, PRIMARY KEY("keyId", "columnName")) WITH COMPACT STORAGE', function(err, created) {
                ...
                Cassandra.runBatchQuery(batch, function(err) {
                    ...
                    Cassandra.runPagedQuery('VerifyPagedColumnQueryStartAndEnd', 'keyId', 'key', 'columnName', null, 8, {'end': 'a'}, function(err, rows, nextToken, startMatched) {
                        ...
                        assert.equal(rows.length, 1);
                        assert.equal(rows[0].get('columnName'), 'a');
                        ...
                        Cassandra.runPagedQuery('VerifyPagedColumnQueryStartAndEnd', 'keyId', 'key', 'columnName', null, 8, {'end': 'j'}, function(err, rows, nextToken, startMatched) {
                            ...
                            assert.equal(rows[0].get('columnName'), 'a');
                            assert.equal(rows[1].get('columnName'), 'b');
                            assert.equal(rows[2].get('columnName'), 'c');
                            assert.equal(rows[3].get('columnName'), 'd');
                            assert.equal(rows[4].get('columnName'), 'e');
                            assert.equal(rows[5].get('columnName'), 'f');
                            assert.equal(rows[6].get('columnName'), 'g');
                            assert.equal(rows[7].get('columnName'), 'h');
                            ...
                            Cassandra.runPagedQuery('VerifyPagedColumnQueryStartAndEnd', 'keyId', 'key', 'columnName', null, 8, {'reversed': true, 'end': 'm'}, function(err, rows, nextToken, startMatched) {
                                ...
                                assert.equal(rows.length, 1);
                                assert.equal(rows[0].get('columnName'), 'm');
                                ...
                                Cassandra.runPagedQuery('VerifyPagedColumnQueryStartAndEnd', 'keyId', 'key', 'columnName', null, 8, {'reversed': true, 'end': 'c'}, function(err, rows, nextToken, startMatched) {
                                    ...
                                    assert.equal(rows.length, 8);
                                    assert.equal(rows[0].get('columnName'), 'm');
                                    assert.equal(rows[1].get('columnName'), 'l');
                                    assert.equal(rows[2].get('columnName'), 'k');
                                    assert.equal(rows[3].get('columnName'), 'j');
                                    assert.equal(rows[4].get('columnName'), 'i');
                                    assert.equal(rows[5].get('columnName'), 'h');
                                    assert.equal(rows[6].get('columnName'), 'g');
                                    assert.equal(rows[7].get('columnName'), 'f');
        ...
        it('verify a strict upper bound on range query results in one more item than requested with limit', function(callback) {
            Cassandra.createColumnFamily('VerifyCassandra7052', 'CREATE TABLE "VerifyCassandra7052" ("keyId" text, "column" text, "value" text, PRIMARY KEY ("keyId", "column")) WITH COMPACT STORAGE', function(err, created) {
                ...
                Cassandra.runBatchQuery(batch, function(err) {
                    ...
                    Cassandra.runQuery('SELECT "column" FROM "VerifyCassandra7052" WHERE "keyId" = ? AND "column" <= ? LIMIT 2', ['key', 'e'], function(err, rows) {
                        assert.ok(!err);
            ...
            Cassandra.createColumnFamily('VerifyMultiBytePagedColumnQuery', 'CREATE TABLE "VerifyMultiBytePagedColumnQuery" ("keyId" text, "column1" text, "value" text, PRIMARY KEY ("keyId", "column1")) WITH COMPACT STORAGE', function(err, created) {
                ...
                Cassandra.runQuery('INSERT INTO "VerifyMultiBytePagedColumnQuery" ("keyId", "column1", "value") VALUES (?, ?, ?)', ['key1', stringWithMultiByte, '1'], function(err) {
                    ...
                    Cassandra.runPagedQuery('VerifyMultiBytePagedColumnQuery', 'keyId', 'key1', 'column1', null, 10, null, function(err, rows, nextToken, startMatched) {
                        ...
                        assert.equal(rows[0].get('column1'), stringWithMultiByte);
                        return callback();
                    });
                });
            });
         ...
         */
        it('verify truncation of cassandra query log entries', function(callback) {
            // Create a large erroneous query whose query and parameters should
            // be truncated
            var invalidQuery = 'SELECT "LLLOOOOLLLLL" FROM "SOMETHING ELSE"';
            var invalidQueryLongerThan300 = invalidQuery + _.range(300).join('');

            var longQueryMoreThan10 = invalidQueryLongerThan300 + '? ? ? ? ? ? ? ? ? ? ? ?';
            var longParamsMoreThan10 = [
                'short enough',
            ...
            // the test
            Cassandra.runQuery(longQueryMoreThan10, longParamsMoreThan10, function(err) {
                assert.ok(err);
            ...
            var cassandraLoggerError = cassandraLog().error;
            cassandraLog().error = function(data, message) {
                try {
                    ...
                    assert.strictEqual(data.query, 'SELECT "LLLOOOOLLLLL" FROM "SOMETHING ELSE" (and 556 more)');
                    assert.strictEqual(data.parameters.length, 10);
                    assert.strictEqual(data.parameters[0], 'short enough');
                    assert.strictEqual(data.parameters[1], '01234567891011121314151617181920212223242526272829303132333435363738394041424344 (and 110 more)');
                    assert.strictEqual(data.parameters[2], '12345678910111213141516171819202122232425262728293031323334353637383940414243444 (and 109 more)');
                    assert.strictEqual(data.parameters[3], '23456789101112131415161718192021222324252627282930313233343536373839404142434445 (and 108 more)');
                    assert.strictEqual(data.parameters[4], '34567891011121314151617181920212223242526272829303132333435363738394041424344454 (and 107 more)');
                    assert.strictEqual(data.parameters[5], '45678910111213141516171819202122232425262728293031323334353637383940414243444546 (and 106 more)');
                    assert.strictEqual(data.parameters[6], '56789101112131415161718192021222324252627282930313233343536373839404142434445464 (and 105 more)');
                    assert.strictEqual(data.parameters[7], '67891011121314151617181920212223242526272829303132333435363738394041424344454647 (and 104 more)');
                    assert.strictEqual(data.parameters[8], '78910111213141516171819202122232425262728293031323334353637383940414243444546474 (and 103 more)');
                    assert.strictEqual(data.parameters[9], '89101112131415161718192021222324252627282930313233343536373839404142434445464748 (and 102 more)');
                } catch (ex) {
                ...
                // log information
                Cassandra.runQuery(invalidQueryLongerThan300, null, function(err) {
                    assert.ok(err);
                    assert.strictEqual(err.code, 500);
                    assert.strictEqual(err.msg, 'An error occurred executing a query');
                });
