...
describe('Activity', function() {
    ...
    describe('Activity Stream Aggregation', function() {
        ...
        it('verify aggregation max expiry time', function(callback) {
            ...
            ActivityAPI.refreshConfiguration(createDefaultConfig({'aggregateIdleExpiry': 5000, 'aggregateMaxExpiry': 1000}), function(err) {
                assert.ok(!err);
    ...
    describe('Activity Clustering', function() {
        ...
        it('verify bucket acquisition and release', function(callback) {
            ...
            ActivityCluster.acquireBucket(99, 5000, function(err, token) {
                ...
                ActivityCluster.acquireBucket(99, 5000, function(err, tokenBad) {
                    ...
                    // Release the bucket
                    ActivityCluster.releaseBucket(99, token, function(err) {
                        assert.ok(!err);
                        ...
                        ActivityCluster.acquireBucket(99, 5000, function(err, token2) {
                            ...
                            ActivityCluster.releaseBucket(99, token2, callback);
                        });
                    });
                });
            });
         ...
         * lock.
         */
        it('verify bucket expiry and stealing an expired lock', function(callback) {
            // Get a lock, make sure it works
            ActivityCluster.acquireBucket(99, 500, function(err, token) {
                assert.ok(!err);
                assert.ok(token);
                ...
                ActivityCluster.acquireBucket(99, 5000, function(err, tokenBad) {
                    ...
                    setTimeout(ActivityCluster.acquireBucket, 600, 99, 5000, function(err, tokenGood) {
                        assert.ok(!err);
                        assert.ok(tokenGood);

                        // Try and release with the wrong token, ensure we don't end up with an error
                        ActivityCluster.releaseBucket(99, token, function(err) {
                            assert.ok(!err);

                            // Make sure that the invalid release token failed to release the bucket
                            ActivityCluster.acquireBucket(99, 5000, function(err, tokenBad2) {
                                assert.ok(!err);
                                assert.ok(!tokenBad2);

                                // Release it successfully to continue.
                                ActivityCluster.releaseBucket(99, tokenGood, callback);
                            });
                        });
                    });
                });
         ...
         */
        it('verify releasing expired lock does not result in an error', function(callback) {
            // Get a lock, make sure it works
            ActivityCluster.acquireBucket(99, 500, function(err, token) {
                assert.ok(!err);
                assert.ok(token);
                ...
                ActivityCluster.acquireBucket(99, 5000, function(err, tokenBad) {
                    ...
                    setTimeout(ActivityCluster.releaseBucket, 600, 99, token, function(err) {
                        assert.ok(!err);
                        callback();
                    });
                });
