...
var context = require('oae-context'); // Due to a circular dependency, Context can't be required directly.
    ...
    rolesAPI.hasRole(principalUuid, resourceUuid, action, function(err, hasRole) {
        if (!err) {
            if (hasRole) {
                ...
                var fakeCtx = new context.Context(null, new User('', principalUuid));
                fakeCtx.isAdmin = function(callback) { callback(false, false); };
                fakeCtx.isTenantAdmin = function(callback) { callback(false, false); };
                fakeCtx.isGlobalAdmin = function(callback) { callback(false, false); };
                principalsAPI.memberOf(fakeCtx, null, null, function(err, groupList) {
                    var groups = _.keys(groupList);
                    if (!err) {
                        return internalIsAllowed(groups, action, resourceUuid, callback);
                    } else {
                        return callback(err);
                    }
                });
    ...
    rolesAPI.getRole(principalUuid, resourceUuid, function(err, role) {
        if (!err) {
            if (role) {
                ...
                fakeCtx.isTenantAdmin = function(callback) { callback(false, false); };
                fakeCtx.isGlobalAdmin = function(callback) { callback(false, false); };
                principalsAPI.memberOf(fakeCtx, null, null, function(err, groups) {
                    if (!err) {
 ...
 */
 module.exports.getAllPermissions = function(principalUuid, resourceUuid, callback) {
    ...
    principalsAPI.memberOf(fakeCtx, null, null, function(err, groups) {
        if (!err) {
            numPrincipals = _.keys(groups).length + 1;
            for (var group in groups) {
                if (groups.hasOwnProperty(group)) {
                    rolesAPI.getRole(groups[group].id, resourceUuid, aggregatePermissions);
                }
            };
            rolesAPI.getRole(principalUuid, resourceUuid, aggregatePermissions);
        } else {
            callback(err);
        }
    });
