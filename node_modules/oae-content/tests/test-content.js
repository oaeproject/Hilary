                ...
                RestAPI.Content.createFile(contexts['nicolaas'].restContext, 'Test Content 2', 'Test content description 2', 'public', getFileStream, [], [], function(err, contentObj) {
                    ...
                    RestAPI.Content.getRevisions(contexts['nicolaas'].restContext, contentObj.contentId, null, null, function(err, revisions) {
                        ...
                        RestAPI.Content.updateFileBody(contexts['nicolaas'].restContext, contentObj.contentId, null, function(err, revision) {
                            ...
                            RestAPI.Content.updateFileBody(contexts['nicolaas'].restContext, contentObj.contentId, 'haha, no actual file body', function(err, revision) {
                                assert.ok(err);
                                assert.ok(!revision);

                                // Try updating as a non-manager
                                RestAPI.Content.updateFileBody(contexts['simon'].restContext, contentObj.contentId, null, function(err, revision) {
                                    assert.ok(err);
                                    assert.ok(!revision);

                                    // Upload a new version.
                                    RestAPI.Content.updateFileBody(contexts['nicolaas'].restContext, contentObj.contentId, getSakaiLogoStream, function(err, revision) {
                                        assert.ok(!err);
                                        assert.equal(revision.opts.mime, 'image/png');
                                        assert.equal(revision.opts.filename, 'sakai-logo.png');
