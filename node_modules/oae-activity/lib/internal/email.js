...
ActivityEmitter.on(ActivityConstants.events.ROUTED_ACTIVITIES, function(activitySeed, allActivities) {
    ...
    PrincipalsDAO.getPrincipals(usersToMail, ['principalId', 'tenantAlias', 'email', 'emailPreference', 'timezone'], function(err, users) {
        ...
        var usersToQueue = _.chain(users).filter(function(user) {
            ...
            // Ensure that each user has a timezone. If a user has no timezone or the default UTC timezone
            // we use the configured default for his tenant
            if (!user.timezone || user.timezone === 'Etc/UTC') {
                user.timezone = PrincipalsConfig.getValue(user.tenant.alias, 'user', 'timezone');
            }
            return user;
        }).value();
...
PrincipalsAPI.on(PrincipalsConstants.events.UPDATED_USER, function(ctx, newUser, oldUser) {
    ...
    var bucketId = ActivityDAO.createEmailBucketIdForUser(oldUser.id, oldUser.emailPreference, oldUser.timezone);
