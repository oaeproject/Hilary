...
OAE.tenantRouter.on('get', '/api/user/:userId/export/:exportType', function(req, res) {
    PrincipalsAPI.exportData(req.ctx, req.params.userId, req.params.exportType, function(err, zipFile) {
        ...
        if (!fs.existsSync(filePath + '/downloadPersonalData')) {
            fs.mkdirSync(filePath + '/downloadPersonalData');
        }

        // Create and download the zip file
        zipFile
        .generateNodeStream({type:'nodebuffer',streamFiles:true})
        .pipe(fs.createWriteStream(filePath + '/downloadPersonalData/' + zipName))
        .on('finish', function () {
            res.download(filePath + '/downloadPersonalData/' + zipName);
        });

        // Remove the zip file one minute after the start of the download to be sure to keep it alive 
        // as long as the zip is not download
        setTimeout(function() { fs.unlinkSync(filePath + '/downloadPersonalData/' + zipName); }, 60000);
    });
});
