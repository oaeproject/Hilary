...
module.exports = function(client) {
     ...
     */
    MixpanelUtil.listen(ContentAPI, ContentConstants.events.GET_CONTENT_PROFILE, function(ctx, content) {
        var params = getBasicContentParameters(ctx, content);
     ...
     */
    MixpanelUtil.listen(ContentAPI, ContentConstants.events.GET_CONTENT_LIBRARY, function(ctx, principalId, visibility, start, limit, contentObjects) {
     ...
     */
    MixpanelUtil.listen(ContentAPI, ContentConstants.events.DOWNLOADED_CONTENT, function(ctx, content, revision) {
        var params = getBasicContentParameters(ctx, content);
     ...
     * Creating content
     */
    MixpanelUtil.listen(ContentAPI, ContentConstants.events.CREATED_CONTENT, function(ctx, content, revision, memberChangeInfo, folderIds) {
        var params = getBasicContentParameters(ctx, content);
    ...
    MixpanelUtil.listen(ContentAPI, ContentConstants.events.UPDATED_CONTENT, function(ctx, newContent, oldContent) {
        var params = getBasicContentParameters(ctx, newContent);
     ...
     */
    MixpanelUtil.listen(ContentAPI, ContentConstants.events.UPDATED_CONTENT_MEMBERS, function(ctx, content, memberChangeInfo, opts) {
        var params = getBasicContentParameters(ctx, content);
    ...
    MixpanelUtil.listen(ContentAPI, ContentConstants.events.UPDATED_CONTENT_PREVIEW, function(content) {
        ...
        if (content.previews.videoKey && content.previews.duration) {
            params.videoDuration = content.previews.duration;
        }
 ...
 * @api private
 */
var getBasicContentParameters = function(ctx, content) {
    var params = MixpanelUtil.getBasicParameters(ctx);
    params.id = content.id;
    params.resourceSubType = content.resourceSubType;
    params.visibility = content.visibility;
