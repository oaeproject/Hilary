...
var activityDaoGetAggregateStatusFn = ActivityDAO.getAggregateStatus;
...
describe('Activity', function() {
    ...
    before(function(callback) {
        ActivityTestUtil.refreshConfiguration({'processActivityJobs': true}, function(err) {
     ...
     */
    var _createDiscussion = function(actorRestContext, target, callback) {
        // Generate the discussions
        RestAPI.Discussions.createDiscussion(actorRestContext, 'title', 'description', 'public', null, null, function(err, publicDiscussion) {
            assert.ok(!err);
            RestAPI.Discussions.createDiscussion(actorRestContext, 'title', 'description', 'loggedin', null, null, function(err, loggedinDiscussion) {
                assert.ok(!err);
                RestAPI.Discussions.createDiscussion(actorRestContext, 'title', 'description', 'private', null, null, function(err, privateDiscussion) {
                    assert.ok(!err);

                    // Generate a share activity (as that has three entities)
                    RestAPI.Discussions.shareDiscussion(actorRestContext, publicDiscussion.id, [target], function(err) {
                        assert.ok(!err);
                        RestAPI.Discussions.shareDiscussion(actorRestContext, loggedinDiscussion.id, [target], function(err) {
                            assert.ok(!err);
                            RestAPI.Discussions.shareDiscussion(actorRestContext, privateDiscussion.id, [target], function(err) {
                                assert.ok(!err);
     ...
     */
    var _getEntities = function(activity, entity) {
        // No entity
        if (!activity[entity]) {
            return [];

        // A single entity
        } else if (activity[entity]['oae:id']) {
     ...
     */
    var _assertStream = function(restContext, resourceId, nrOfShareActivities, objectEntities, targetEntities, callback) {
        ActivityTestUtil.collectAndGetActivityStream(restContext, resourceId, null, function(err, result) {
            assert.ok(!err);

            var shareActivities = _.filter(result.items, function(activity) {
            ...
            var retrievedObjectEntities = [];
            var retrievedTargetEntities = [];
            _.each(shareActivities, function(activity) {
            ...
            assert.deepEqual(retrievedObjectEntities, objectEntities);
            assert.deepEqual(retrievedTargetEntities, targetEntities);
            return callback();
        });
    };
     ...
     */
    var _setupUsersAndDiscussions = function(callback) {
        TestsUtil.setupMultiTenantPrivacyEntities(function(publicTenant0, publicTenant1) {
            ...
            TestsUtil.generateTestUsers(publicTenant0.adminRestContext, 1, function(err, users, extraPublicUser) {
                assert.ok(!err);
                publicTenant0.extraPublicUser = extraPublicUser;

                // Ensure the user is public
                RestAPI.User.updateUser(publicTenant0.extraPublicUser.restContext, publicTenant0.extraPublicUser.user.id, {'visibility': 'public'}, function(err) {
                    assert.ok(!err);
                    ...
                    _createDiscussion(publicTenant0.publicUser.restContext, publicTenant0.extraPublicUser.user.id, function(publicDiscussion, loggedinDiscussion, privateDiscussion) {

                        publicTenant0.discussions.public2Extra = {'public': publicDiscussion, 'loggedin': loggedinDiscussion, 'private': privateDiscussion};
                        _createDiscussion(publicTenant0.publicUser.restContext, publicTenant0.loggedinUser.user.id, function(publicDiscussion, loggedinDiscussion, privateDiscussion) {
                            publicTenant0.discussions.public2Loggedin = {'public': publicDiscussion, 'loggedin': loggedinDiscussion, 'private': privateDiscussion};
                            _createDiscussion(publicTenant0.loggedinUser.restContext, publicTenant0.publicUser.user.id, function(publicDiscussion, loggedinDiscussion, privateDiscussion) {
                                publicTenant0.discussions.loggedin2Public = {'public': publicDiscussion, 'loggedin': loggedinDiscussion, 'private': privateDiscussion};
                                _createDiscussion(publicTenant0.loggedinUser.restContext, publicTenant0.extraPublicUser.user.id, function(publicDiscussion, loggedinDiscussion, privateDiscussion) {
                                    publicTenant0.discussions.loggedin2Extra = {'public': publicDiscussion, 'loggedin': loggedinDiscussion, 'private': privateDiscussion};
                                    _createDiscussion(publicTenant0.privateUser.restContext, publicTenant0.publicUser.user.id, function(publicDiscussion, loggedinDiscussion, privateDiscussion) {
                                        publicTenant0.discussions.private2Public = {'public': publicDiscussion, 'loggedin': loggedinDiscussion, 'private': privateDiscussion};
                                        _createDiscussion(publicTenant0.privateUser.restContext, publicTenant0.loggedinUser.user.id, function(publicDiscussion, loggedinDiscussion, privateDiscussion) {
                                            publicTenant0.discussions.private2Loggedin = {'public': publicDiscussion, 'loggedin': loggedinDiscussion, 'private': privateDiscussion};
                                            _createDiscussion(publicTenant0.privateUser.restContext, publicTenant0.extraPublicUser.user.id, function(publicDiscussion, loggedinDiscussion, privateDiscussion) {
                                                publicTenant0.discussions.private2Extra = {'public': publicDiscussion, 'loggedin': loggedinDiscussion, 'private': privateDiscussion};
     ...
     * @param  {Object}         callback.publicTenant1      The other tenant
     */
    var _setupGroupsAndDiscussions = function(callback) {
        TestsUtil.setupMultiTenantPrivacyEntities(function(publicTenant0, publicTenant1) {

            _createDiscussion(publicTenant0.adminRestContext, publicTenant0.publicGroup.id, function(publicDiscussion, loggedinDiscussion, privateDiscussion) {
                publicTenant0.publicGroup.discussions = {'public': publicDiscussion, 'loggedin': loggedinDiscussion, 'private': privateDiscussion};
                _createDiscussion(publicTenant0.adminRestContext, publicTenant0.loggedinGroup.id, function(publicDiscussion, loggedinDiscussion, privateDiscussion) {
                    publicTenant0.loggedinGroup.discussions = {'public': publicDiscussion, 'loggedin': loggedinDiscussion, 'private': privateDiscussion};
                    _createDiscussion(publicTenant0.adminRestContext, publicTenant0.privateGroup.id, function(publicDiscussion, loggedinDiscussion, privateDiscussion) {
                        publicTenant0.privateGroup.discussions = {'public': publicDiscussion, 'loggedin': loggedinDiscussion, 'private': privateDiscussion};
                        return callback(publicTenant0, publicTenant1);
                    });
                });
            });
            ...
            TestsUtil.setupMultiTenantPrivacyEntities(function(publicTenant0, publicTenant1, privateTenant0, privateTenant1) {
                ...
                FollowingTestsUtil.followByAll(publicTenant0.publicUser.user.id, followers, function(err) {
                    ...
                    ActivityTestUtil.collectAndGetActivityStream(publicTenant0.publicUser.restContext, null, null, function(err, response) {
                        ...
                        ActivityTestUtil.assertActivity(contentCreateActivity, 'content-create', 'create', publicTenant0.publicUser.user.id, testId);
                        ...
                        ActivityTestUtil.collectAndGetActivityStream(publicTenant0.privateUser.restContext, null, null, function(err, response) {
                            ...
                            var followActivity = _.findWhere(response.items, {'oae:activityType': 'following-follow'});
                            ActivityTestUtil.assertActivity(followActivity, 'following-follow', 'follow', publicTenant0.privateUser.user.id, publicTenant0.publicUser.user.id);
                            return callback();
                        });
                    });
                });
    ...
    describe('Activity Stream Permissions and Validation', function() {
        ...
        it('verify getActivityStream validation', function(callback) {
            TestsUtil.generateTestUsers(camAdminRestContext, 1, function(err, users, jack) {
                ...
                // Try empty id
                ActivityTestUtil.assertGetActivityStreamFails(jack.restContext, ' ', null, 400, function() {

                    // Try invalid principal id
                    ActivityTestUtil.assertGetActivityStreamFails(jack.restContext, 'c:cam:someContent', null, 400, function() {

                        // Try an invalid activity transformer
                        ActivityTestUtil.assertGetActivityStreamFails(jack.restContext, jack.user.id, {'format': 'non-existing'}, 400, function() {
         ...
         */
        it('verify public user activity stream', function(callback) {
            _setupUsersAndDiscussions(function(publicTenant0, publicTenant1) {
                ...
                _assertStream(publicTenant0.anonymousRestContext, publicTenant0.publicUser.user.id, 1, [publicTenant0.discussions.public2Extra.public.id], [publicTenant0.extraPublicUser.user.id], function() {
                    _assertStream(publicTenant1.anonymousRestContext, publicTenant0.publicUser.user.id, 1, [publicTenant0.discussions.public2Extra.public.id], [publicTenant0.extraPublicUser.user.id], function() {
                        _assertStream(publicTenant1.publicUser.restContext, publicTenant0.publicUser.user.id, 1, [publicTenant0.discussions.public2Extra.public.id], [publicTenant0.extraPublicUser.user.id], function() {
                            _assertStream(publicTenant1.loggedinUser.restContext, publicTenant0.publicUser.user.id, 1, [publicTenant0.discussions.public2Extra.public.id], [publicTenant0.extraPublicUser.user.id], function() {
                                _assertStream(publicTenant1.privateUser.restContext, publicTenant0.publicUser.user.id, 1, [publicTenant0.discussions.public2Extra.public.id], [publicTenant0.extraPublicUser.user.id], function() {
                                    _assertStream(publicTenant1.adminRestContext, publicTenant0.publicUser.user.id, 1, [publicTenant0.discussions.public2Extra.public.id], [publicTenant0.extraPublicUser.user.id], function() {
                                        ...
                                        _assertStream(publicTenant0.loggedinUser.restContext, publicTenant0.publicUser.user.id, 2, [publicTenant0.discussions.public2Extra.public.id, publicTenant0.discussions.public2Extra.loggedin.id, publicTenant0.discussions.public2Loggedin.public.id, publicTenant0.discussions.public2Loggedin.loggedin.id], [publicTenant0.extraPublicUser.user.id, publicTenant0.loggedinUser.user.id], function() {
                                            _assertStream(publicTenant0.privateUser.restContext, publicTenant0.publicUser.user.id, 2, [publicTenant0.discussions.public2Extra.public.id, publicTenant0.discussions.public2Extra.loggedin.id, publicTenant0.discussions.public2Loggedin.public.id, publicTenant0.discussions.public2Loggedin.loggedin.id], [publicTenant0.extraPublicUser.user.id, publicTenant0.loggedinUser.user.id], function() {
                                                ...
                                                var objectEntities = [
                                                    publicTenant0.discussions.public2Extra.public.id, publicTenant0.discussions.public2Extra.loggedin.id, publicTenant0.discussions.public2Extra.private.id,
                                                    publicTenant0.discussions.public2Loggedin.public.id, publicTenant0.discussions.public2Loggedin.loggedin.id, publicTenant0.discussions.public2Loggedin.private.id,
                                                    publicTenant0.discussions.loggedin2Public.public.id, publicTenant0.discussions.loggedin2Public.loggedin.id, publicTenant0.discussions.loggedin2Public.private.id,
                                                    publicTenant0.discussions.private2Public.public.id, publicTenant0.discussions.private2Public.loggedin.id, publicTenant0.discussions.private2Public.private.id
                                                ];
                                                _assertStream(publicTenant0.publicUser.restContext, publicTenant0.publicUser.user.id, 4, objectEntities, [publicTenant0.extraPublicUser.user.id, publicTenant0.loggedinUser.user.id, publicTenant0.publicUser.user.id], function() {
                                                    _assertStream(publicTenant0.adminRestContext, publicTenant0.publicUser.user.id, 4, objectEntities, [publicTenant0.extraPublicUser.user.id, publicTenant0.loggedinUser.user.id, publicTenant0.publicUser.user.id], function() {
                                                        return callback();
                                                    });
                                                });
                                            });
         ...
         */
        it('verify loggedin user activity stream', function(callback) {
            _setupUsersAndDiscussions(function(publicTenant0, publicTenant1) {

                // A loggedin user's activity feed is not accessible
                // by anonymous users or users from another tenant
                ActivityTestUtil.assertGetActivityStreamFails(publicTenant0.anonymousRestContext, publicTenant0.loggedinUser.user.id, null, 401, function() {
                    ActivityTestUtil.assertGetActivityStreamFails(publicTenant1.anonymousRestContext, publicTenant0.loggedinUser.user.id, null, 401, function() {
                        ActivityTestUtil.assertGetActivityStreamFails(publicTenant1.publicUser.restContext, publicTenant0.loggedinUser.user.id, null, 401, function() {
                            ActivityTestUtil.assertGetActivityStreamFails(publicTenant1.loggedinUser.restContext, publicTenant0.loggedinUser.user.id, null, 401, function() {
                                ActivityTestUtil.assertGetActivityStreamFails(publicTenant1.privateUser.restContext, publicTenant0.loggedinUser.user.id, null, 401, function() {
                                    ActivityTestUtil.assertGetActivityStreamFails(publicTenant1.adminRestContext, publicTenant0.loggedinUser.user.id, null, 401, function() {

                                        // Users from the same tenant will be given the logged in activity stream
                                        var objectEntities = [
                                            publicTenant0.discussions.loggedin2Public.public.id, publicTenant0.discussions.loggedin2Public.loggedin.id,
                                            publicTenant0.discussions.loggedin2Extra.public.id, publicTenant0.discussions.loggedin2Extra.loggedin.id
                                        ];
                                        _assertStream(publicTenant0.publicUser.restContext, publicTenant0.loggedinUser.user.id, 2, objectEntities, [publicTenant0.extraPublicUser.user.id, publicTenant0.publicUser.user.id], function() {
                                            _assertStream(publicTenant0.privateUser.restContext, publicTenant0.loggedinUser.user.id, 2, objectEntities, [publicTenant0.extraPublicUser.user.id, publicTenant0.publicUser.user.id], function() {
                                                ...
                                                objectEntities = [
                                                    publicTenant0.discussions.loggedin2Public.public.id, publicTenant0.discussions.loggedin2Public.loggedin.id, publicTenant0.discussions.loggedin2Public.private.id,
                                                    publicTenant0.discussions.loggedin2Extra.public.id, publicTenant0.discussions.loggedin2Extra.loggedin.id, publicTenant0.discussions.loggedin2Extra.private.id,
                                                    publicTenant0.discussions.public2Loggedin.public.id, publicTenant0.discussions.public2Loggedin.loggedin.id, publicTenant0.discussions.public2Loggedin.private.id,
                                                    publicTenant0.discussions.private2Loggedin.public.id, publicTenant0.discussions.private2Loggedin.loggedin.id, publicTenant0.discussions.private2Loggedin.private.id
                                                ];
                                                _assertStream(publicTenant0.loggedinUser.restContext, publicTenant0.loggedinUser.user.id, 4, objectEntities, [publicTenant0.extraPublicUser.user.id, publicTenant0.publicUser.user.id, publicTenant0.loggedinUser.user.id], function() {
                                                    _assertStream(publicTenant0.loggedinUser.restContext, publicTenant0.loggedinUser.user.id, 4, objectEntities, [publicTenant0.extraPublicUser.user.id, publicTenant0.publicUser.user.id, publicTenant0.loggedinUser.user.id], function() {
                                                        return callback();
                                                    });
                                                });
         ...
         */
        it('verify private user activity stream', function(callback) {
            _setupUsersAndDiscussions(function(publicTenant0, publicTenant1) {

                // A private user's activity feed is only accessible by the user themselve
                // or by a tenant administrator
                ActivityTestUtil.assertGetActivityStreamFails(publicTenant0.anonymousRestContext, publicTenant0.privateUser.user.id, null, 401, function() {
                    ActivityTestUtil.assertGetActivityStreamFails(publicTenant1.anonymousRestContext, publicTenant0.privateUser.user.id, null, 401, function() {
                        ActivityTestUtil.assertGetActivityStreamFails(publicTenant1.publicUser.restContext, publicTenant0.privateUser.user.id, null, 401, function() {
                            ActivityTestUtil.assertGetActivityStreamFails(publicTenant1.loggedinUser.restContext, publicTenant0.privateUser.user.id, null, 401, function() {
                                ActivityTestUtil.assertGetActivityStreamFails(publicTenant1.privateUser.restContext, publicTenant0.privateUser.user.id, null, 401, function() {
                                    ActivityTestUtil.assertGetActivityStreamFails(publicTenant1.adminRestContext, publicTenant0.privateUser.user.id, null, 401, function() {
                                        ActivityTestUtil.assertGetActivityStreamFails(publicTenant0.publicUser.restContext, publicTenant0.privateUser.user.id, null, 401, function() {
                                            ActivityTestUtil.assertGetActivityStreamFails(publicTenant0.loggedinUser.restContext, publicTenant0.privateUser.user.id, null, 401, function() {
                                                ...
                                                var objectEntities = [
                                                    publicTenant0.discussions.private2Public.public.id, publicTenant0.discussions.private2Public.loggedin.id, publicTenant0.discussions.private2Public.private.id,
                                                    publicTenant0.discussions.private2Loggedin.public.id, publicTenant0.discussions.private2Loggedin.loggedin.id, publicTenant0.discussions.private2Loggedin.private.id,
                                                    publicTenant0.discussions.private2Extra.public.id, publicTenant0.discussions.private2Extra.loggedin.id, publicTenant0.discussions.private2Extra.private.id,
                                                ];
                                                _assertStream(publicTenant0.privateUser.restContext, publicTenant0.privateUser.user.id, 3, objectEntities, [publicTenant0.extraPublicUser.user.id, publicTenant0.publicUser.user.id, publicTenant0.loggedinUser.user.id], function() {
         ...
         */
        it('verify public group activity stream', function(callback) {
            _setupGroupsAndDiscussions(function(publicTenant0, publicTenant1) {

                // Anonymous users and authenticated users from other tenants will receive
                // the public activity stream for a public group
                _assertStream(publicTenant0.anonymousRestContext, publicTenant0.publicGroup.id, 1, [publicTenant0.publicGroup.discussions.public.id], [publicTenant0.publicGroup.id], function() {
                    _assertStream(publicTenant1.anonymousRestContext, publicTenant0.publicGroup.id, 1, [publicTenant0.publicGroup.discussions.public.id], [publicTenant0.publicGroup.id], function() {
                        _assertStream(publicTenant1.publicUser.restContext, publicTenant0.publicGroup.id, 1, [publicTenant0.publicGroup.discussions.public.id], [publicTenant0.publicGroup.id], function() {
                            _assertStream(publicTenant1.loggedinUser.restContext, publicTenant0.publicGroup.id, 1, [publicTenant0.publicGroup.discussions.public.id], [publicTenant0.publicGroup.id], function() {
                                _assertStream(publicTenant1.privateUser.restContext, publicTenant0.publicGroup.id, 1, [publicTenant0.publicGroup.discussions.public.id], [publicTenant0.publicGroup.id], function() {
                                    _assertStream(publicTenant1.adminRestContext, publicTenant0.publicGroup.id, 1, [publicTenant0.publicGroup.discussions.public.id], [publicTenant0.publicGroup.id], function() {

                                        // Authenticated users on the same tenant will receive
                                        // the loggedin activity stream for a public group
                                        _assertStream(publicTenant0.loggedinUser.restContext, publicTenant0.publicGroup.id, 1, [publicTenant0.publicGroup.discussions.public.id, publicTenant0.publicGroup.discussions.loggedin.id], [publicTenant0.publicGroup.id], function() {
                                            _assertStream(publicTenant0.privateUser.restContext, publicTenant0.publicGroup.id, 1, [publicTenant0.publicGroup.discussions.public.id, publicTenant0.publicGroup.discussions.loggedin.id], [publicTenant0.publicGroup.id], function() {

                                                // Members and tenant administrators will receive
                                                // the private activity stream for a public group
                                                _assertStream(publicTenant0.publicUser.restContext, publicTenant0.publicGroup.id, 1, [publicTenant0.publicGroup.discussions.public.id, publicTenant0.publicGroup.discussions.loggedin.id, publicTenant0.publicGroup.discussions.private.id], [publicTenant0.publicGroup.id], function() {
                                                    _assertStream(publicTenant0.adminRestContext, publicTenant0.publicGroup.id, 1, [publicTenant0.publicGroup.discussions.public.id, publicTenant0.publicGroup.discussions.loggedin.id, publicTenant0.publicGroup.discussions.private.id], [publicTenant0.publicGroup.id], function() {
                                                        return callback();
                                                    });
                                                });
                                            });
         ...
         */
        it('verify loggedin group activity stream', function(callback) {
            _setupGroupsAndDiscussions(function(publicTenant0, publicTenant1) {

                // Anonymous users and authenticated users from other tenants
                // cannot access the activity stream of a loggedin group
                ActivityTestUtil.assertGetActivityStreamFails(publicTenant0.anonymousRestContext, publicTenant0.loggedinGroup.id, null, 401, function() {
                    ActivityTestUtil.assertGetActivityStreamFails(publicTenant1.anonymousRestContext, publicTenant0.loggedinGroup.id, null, 401, function() {
                        ActivityTestUtil.assertGetActivityStreamFails(publicTenant1.publicUser.restContext, publicTenant0.loggedinGroup.id, null, 401, function() {
                            ActivityTestUtil.assertGetActivityStreamFails(publicTenant1.loggedinUser.restContext, publicTenant0.loggedinGroup.id, null, 401, function() {
                                ActivityTestUtil.assertGetActivityStreamFails(publicTenant1.privateUser.restContext, publicTenant0.loggedinGroup.id, null, 401, function() {
                                    ActivityTestUtil.assertGetActivityStreamFails(publicTenant1.adminRestContext, publicTenant0.loggedinGroup.id, null, 401, function() {

                                        // Authenticated users on the same tenant will receive
                                        // the loggedin activity stream for a loggedin group
                                        _assertStream(publicTenant0.publicUser.restContext, publicTenant0.loggedinGroup.id, 1, [publicTenant0.loggedinGroup.discussions.public.id, publicTenant0.loggedinGroup.discussions.loggedin.id], [publicTenant0.loggedinGroup.id], function() {
                                            _assertStream(publicTenant0.privateUser.restContext, publicTenant0.loggedinGroup.id, 1, [publicTenant0.loggedinGroup.discussions.public.id, publicTenant0.loggedinGroup.discussions.loggedin.id], [publicTenant0.loggedinGroup.id], function() {

                                                // Members and tenant administrators will receive
                                                // the private activity stream for a loggedin group
                                                _assertStream(publicTenant0.loggedinUser.restContext, publicTenant0.loggedinGroup.id, 1, [publicTenant0.loggedinGroup.discussions.public.id, publicTenant0.loggedinGroup.discussions.loggedin.id, publicTenant0.loggedinGroup.discussions.private.id], [publicTenant0.loggedinGroup.id], function() {
                                                    _assertStream(publicTenant0.adminRestContext, publicTenant0.loggedinGroup.id, 1, [publicTenant0.loggedinGroup.discussions.public.id, publicTenant0.loggedinGroup.discussions.loggedin.id, publicTenant0.loggedinGroup.discussions.private.id], [publicTenant0.loggedinGroup.id], function() {
                                                        return callback();
                                                    });
                                                });
                                            });
         ...
         */
        it('verify private group activity stream', function(callback) {
            _setupGroupsAndDiscussions(function(publicTenant0, publicTenant1) {

                // A private group's activity feed is only accessible by members or by a tenant administrator
                ActivityTestUtil.assertGetActivityStreamFails(publicTenant0.anonymousRestContext, publicTenant0.privateGroup.id, null, 401, function() {
                    ActivityTestUtil.assertGetActivityStreamFails(publicTenant1.anonymousRestContext, publicTenant0.privateGroup.id, null, 401, function() {
                        ActivityTestUtil.assertGetActivityStreamFails(publicTenant1.publicUser.restContext, publicTenant0.privateGroup.id, null, 401, function() {
                            ActivityTestUtil.assertGetActivityStreamFails(publicTenant1.loggedinUser.restContext, publicTenant0.privateGroup.id, null, 401, function() {
                                ActivityTestUtil.assertGetActivityStreamFails(publicTenant1.privateUser.restContext, publicTenant0.privateGroup.id, null, 401, function() {
                                    ActivityTestUtil.assertGetActivityStreamFails(publicTenant1.adminRestContext, publicTenant0.privateGroup.id, null, 401, function() {
                                        ActivityTestUtil.assertGetActivityStreamFails(publicTenant0.publicUser.restContext, publicTenant0.privateGroup.id, null, 401, function() {
                                            ActivityTestUtil.assertGetActivityStreamFails(publicTenant0.loggedinUser.restContext, publicTenant0.privateGroup.id, null, 401, function() {

                                                // Members and tenant administrators will receive
                                                // the private activity stream for a private group
                                                _assertStream(publicTenant0.privateUser.restContext, publicTenant0.privateGroup.id, 1, [publicTenant0.privateGroup.discussions.public.id, publicTenant0.privateGroup.discussions.loggedin.id, publicTenant0.privateGroup.discussions.private.id], [publicTenant0.privateGroup.id], function() {
                                                    _assertStream(publicTenant0.adminRestContext, publicTenant0.privateGroup.id, 1, [publicTenant0.privateGroup.discussions.public.id, publicTenant0.privateGroup.discussions.loggedin.id, publicTenant0.privateGroup.discussions.private.id], [publicTenant0.privateGroup.id], function() {
                                                        return callback();
                                                    });
                                                });
                                            });
                ...
                RestAPI.Content.createLink(jack.restContext, 'A', 'A', 'public', 'http://www.google.ca', [], [], [], function(err, link) {
                    ...
                    ActivityTestUtil.collectAndGetActivityStream(jack.restContext, null, null, function(err, activityStream) {
                        ...
                        RestAPI.Activity.getActivityStream(jack.restContext, jack.user.id, {'format': 'internal'}, function(err, activityStream) {
                            ...
                            allowedActorProperties = ['oae:id', 'id', 'displayName', 'visibility', 'locale', 'publicAlias', 'picture', 'profilePath', 'resourceType', 'acceptedTC', 'objectType', 'tenant', 'email', 'emailPreference', 'created'];
                            _.each(activity.actor, function(value, key) {
    ...
    describe('Activity Stream Aggregation', function() {
         ...
         */
        it('verify an error in routed activity data does not permanently damage an aggregation bucket', function(callback) {
            // Start with empty collection buckets
            ActivityAggregator.collectAllBuckets(function(err) {
                assert.ok(!err);

                // Create jack and add a legitimate activity to his feed
                TestsUtil.generateTestUsers(camAdminRestContext, 1, function(err, users, jack) {
                    assert.ok(!err);
                    var routeEndsWith = util.format('%s#%s', jack.user.id, 'activity');
                    ...
                    ActivityDAO.getAggregateStatus = function(allAggregateKeys, callback) {
                        var brokenAggregateKeyPrefix = util.format('content-create#%s#activity', jack.user.id);
                        var brokenAggregateKeys = _.filter(allAggregateKeys, function(aggregateKey) {
                    ...
                    RestAPI.Content.createLink(jack.restContext, 'A', 'A', 'public', 'http://www.google.ca', [], [], [], function(err, linkA) {
                        ...
                        ActivityTestUtil.collectAndGetActivityStream(jack.restContext, null, null, function(err, activityStream) {
                            ...
                            RestAPI.Content.createComment(jack.restContext, linkA.id, 'Comment Comment', null, function(err, comment) {
                                assert.ok(!err);
                                ActivityTestUtil.collectAndGetActivityStream(jack.restContext, null, null, function(err, activityStream) {
                                    assert.ok(!err);
                                    assert.strictEqual(activityStream.items.length, 1);
