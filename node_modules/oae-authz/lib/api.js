 ...
 */
var getDirectRole = module.exports.getDirectRole = function(principalId, resourceId, callback) {
    ...
    _getDirectRole([principalId], resourceId, function(err, roles) {
        if (err) {
            return callback(err);
        }
 ...
 **/
var _getDirectRole = function(principalIds, resourceId, callback) {
...
var _getAllRoles = function(principalId, resourceId, callback) {
    ...
    getDirectRole(principalId, resourceId, function(err, directRole) {
        if (err) {
            return callback(err);
        } else if (directRole) {
        ...
        // Get the groups that are directly and indirectly associated to the resource
        _getResourceGroupHierarchy(resourceId, function(err, groups) {
            if (err) {
                return callback(err);
            }
            ...
            if (_.keys(groups).length === 0) {
                ...
                Cassandra.runQuery('SELECT ? FROM AuthzRoles USING CONSISTENCY QUORUM WHERE principalId = ?', [_.keys(groups), principalId], function(err, rows) {
                    if (err) {
                        return callback(err);
                    }
                    
                    // Check if any of the groups match and add their role to the roles array
                    var memberships = Cassandra.rowToHash(rows[0]);
                    for (var m in memberships) {
                        if (memberships[m]) {
                            // Add all of the roles that this group has
                            for (var r = 0; r < groups[m].roles.length; r++) {
                                if (allRoles.indexOf(groups[m].roles[r]) === -1) {
                                    allRoles.push(groups[m].roles[r]);
                                }
                            }
                        }
                    }
 ...
 * @param   {Function}     callback        Standard callback function
 * @param   {Object}       callback.err    An error that occurred, if any
 * @param   {Object}       callback.roles  A JSON object where the keys are the group ids of the directly and indirectly associated groups and the values are arrays specifying that groups direct and indirect roles on the resouce
 */
var _getResourceGroupHierarchy = function(resourceId, callback) {
    // Get the groups associated to the resource
    ...
    Cassandra.runQuery('SELECT ? .. ? FROM AuthzMembers USING CONSISTENCY QUORUM WHERE resourceId = ?', [start, end, resourceId], function(err, rows) {
        ...
        if (_.keys(groupHierarchy).length === 0) {
            callback(null, {});
        } else {
            
            // Explode the group memberships downwards
            var explodeGroupMemberships = function(groupIds) {
                Cassandra.runQuery('SELECT ? .. ? FROM AuthzMembers USING CONSISTENCY QUORUM WHERE resourceId IN (?)', [start, end, groupIds], function(err, rows) {
                    if (err) {
                        callback(err);
                    }

                    // Check if any of the returned groups need to be exploded further
                    var newToExplode = [];
                    for (var i = 0; i < rows.length; i++) {
                        var groups = Cassandra.rowToHash(rows[i]);
                        if (_.keys(groups).length > 0) {
                            for (var g in groups) {
                                // Fill in the children of the parent group
                                if (groupHierarchy[rows[i].key].children.indexOf(g) === -1) {
                                    groupHierarchy[rows[i].key].children.push(g);
                                }
                                // The group isn't part of the groupHierarchy yet.
                                if (!groupHierarchy[g]) {
                                    newToExplode.push(g);
                                    groupHierarchy[g] = {
                                        'roles': groupHierarchy[rows[i].key].roles.slice(),
                                        'children': []
                                    }
                                // If the group is already part of the groupHierarchy, we add the roles of the parent group. If any of them weren't in the
                                // roles list yet, we recursively loop over all of the group children to add the roles to all of its children
                                } else {
                                    var rolesNotOnChild = _.difference(groupHierarchy[rows[i].key].roles, groupHierarchy[g].roles);
                                    // If a new group has been find
                                    if (rolesNotOnChild.length > 0) {
                                        
                                        var addRolesToChild = function(groupId, rolesToAdd) {
                                            // Add the roles
                                            groupHierarchy[groupId].roles = groupHierarchy[groupId].roles.concat(rolesToAdd);
                                            // Add the roles to all of its children as well
                                            for (var c = 0; c < groupHierarchy[groupId].children.length; c++) {
                                                var rolesNotOnChild = _.difference(groupHierarchy[groupId].roles, groupHierarchy[groupHierarchy[groupId].children[c]].roles);
                                                if (rolesNotOnChild.length > 0) {
                                                    addRolesToChild(groupHierarchy[groupId].children[c], rolesNotOnChild);
                                                }
                                            }
                                        }
                                        addRolesToChild(g, rolesNotOnChild);
                                    }
                                }
                            }
                        }
                    ...
                    // If no groups need to be exploded, we can return
                    if (newToExplode.length > 0) {
                        explodeGroupMemberships(newToExplode);
                    } else {
            ...
            // Convert the direct group members to be able to take multiple roles (which will be filled
            // in because of indirect memberships) and the list of child groups
            for (var g in groupHierarchy) {
                groupHierarchy[g] = {
                    'roles': [groupHierarchy[g]],
                    'children': []
                }
            }
            // Start by exploding the groups directly associated to the resource
            explodeGroupMemberships(_.keys(groupHierarchy));
 ...
 */
var getPrincipalMemberships = module.exports.getPrincipalMemberships = function(principalId, callback) {
    var validator = new Validator();
    validator.check(principalId, {code: 400, msg: 'Invalid principal id provided'}).isPrincipalId();
    if (validator.hasErrors()) {
        return callback(validator.getFirstError());
    ...
    _getAuthzGroupMembershipAncestry(principalId, function(err, groups) {
        if (err) {
            return callback(err);
        }
...
/**
 * Get all principal roles associated to the resourceType for all the principals in the array of principal ids. This is similar
