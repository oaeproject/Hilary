...
var storePicture = module.exports.storePicture = function(ctx, principalId, file, callback) {
    ...
    _canManagePrincipal(ctx, principalId, function(err, principal) {
        ...
        _removePictures(ctx, principal, true, false, function(err) {
            if (err) {
                return cleanupOnError(err, file, callback);
            }
 ...
 * @param {Boolean}     removeLarge     Whether or not to remove the large picture.
 * @param {Booelan}     removeResized   Whether or not to remove the resized (small and medium) pictures.
 * @param {Function}    callback        Standard execution method.
 * @param {Object}      callback.err    Error object (if any.)
 */
var _removePictures = function(ctx, principal, removeLarge, removeResized, callback) {
    var toRemove = [];
    ...
    if (removeResized) {
         toRemove.push(principal.smallPictureUri, principal.mediumPictureUri);
    }
...
var _storePicture = function(ctx, principal, file, callback) {
    ContentAPI.getStorageBackend(ctx).store(ctx, file, function(err, largePictureUri) {
        ...
        Cassandra.runQuery(q.query, q.parameters, function(err) {
            ...
            principal.largePicture = PrincipalsUtil.getSignedPictureUrl(ctx, ctx.tenant().alias, largePictureUri);
...
var _saveUrisToCassandra = function(ctx, principal, smallPictureUri, mediumPictureUri, callback) {
    ...
    Cassandra.runQuery(q.query, q.parameters, function(err) {
        ...
        newPrincipal.smallPictureUri = smallPictureUri;
        newPrincipal.mediumPictureUri = mediumPictureUri;
        newPrincipal.smallPicture = PrincipalsUtil.getSignedPictureUrl(ctx, ctx.tenant().alias, smallPictureUri);
        newPrincipal.mediumPicture = PrincipalsUtil.getSignedPictureUrl(ctx, ctx.tenant().alias, mediumPictureUri);
