...
var store = module.exports.store = function(ctx, getStream, callback) {
    ...
    _writeBody(ctx, getStream, function(err, uri) {
        if (err) {
 ...
 * @param {Context}  ctx            The current execution context.
 * @param {Function} getStream      A function that returns an object that holds a `filename`, the `size`, the `mimetype` and the `stream`.
 * @param {Function} callback       A callback method.
 * @param {Object}   callback.err   An error object (if any)
 * @param {String}   callback.uri   The URI that can be used to retrieve a file.
 * @private
 */
var _writeBody = function(ctx, getStream, callback) {
    var rootDir = _getRootDir(ctx);
    var date = new Date();
