...
describe('Users', function() {
    ...
    describe('Create user', function() {
        ...
        it('verify create user', function(callback) {
            ...
            RestAPI.Tenant.createTenant(globalAdminRestContext, recaptchaTenantAlias, recaptchaTenantAlias, recaptchaTenantAlias, function(err, recaptchaTenant) {
                ...
                RestAPI.Config.updateConfig(globalAdminRestContext, recaptchaTenantAlias, 'oae-principals/recaptcha/enabled', true, function(err) {
                    assert.ok(!err);

                    RestAPI.User.createUser(recaptchaAnonymousRestContext, userId, 'password', 'Test User', {'visibility': 'public'}, function(err, userObj) {
                        assert.ok(err);
                        assert.equal(err.code, 400, JSON.stringify(err));
                        assert.ok(!userObj);
            ...
            RestAPI.User.createUser(camAdminRestContext, testUserId, 'password', 'Test User', {'visibility': 'public', 'locale': 'en_GB', 'timezone': 'Europe/London'}, function(err, userObj) {
                ...
                RestAPI.User.updateUser(testUserRestContext, userObj.id, updateValues, function (err) {
                    ...
                    RestAPI.User.getMe(testUserRestContext, function(err, meObj) {
                        ...
                        assert.equal(meObj.profile.publicAlias, 'publicalias');
                        ...
                        RestAPI.User.getUser(testUserRestContext, userObj.id, function(err, userObj) {
                            ...
                            assert.equal(userObj.profile.publicAlias, 'publicalias');
    ...
    describe('User visibility', function() {
        ...
        var verifyProfilePermissions = function(restContext, userToCheck, expectAccess, expectedDisplayName, expectedPublicAlias, expectedVisibility, checkDone) {
            ...
            RestAPI.User.getUser(restContext, userToCheck, function(err, userObj) {
                ...
                assert.equal(userObj.profile.displayName, expectedDisplayName);
                assert.equal(userObj.profile.publicAlias, expectedPublicAlias);
                if (!expectAccess) {
                    assert.equal(userObj.profile.timezone, undefined);
                    assert.equal(userObj.profile.locale, undefined);
                }
                checkDone();
            });
        };
        ...
        it('verify public user is visible beyond tenant scope', function(callback) {
            ...
            RestAPI.User.createUser(anonymousRestContext, usernameA, 'password', 'Public User', {'visibility': 'public'}, function(err, userA) {
                assert.ok(!err);
                ...
                RestAPI.Tenant.createTenant(globalAdminRestContext, tenantAliasB, tenantAliasB, tenantAliasB, function(err, tenantB) {
                    ...
                    RestAPI.User.createUser(restCtxB, usernameB, 'password', 'Private User', null, function(err, userB) {
                        ...
                        // restContext, userToCheck, expectAccess, expectedDisplayName, expectedPublicAlias, expectedVisibility, checkDone
        ...
        it('verify logedin user is hidden beyond tenant scope', function(callback) {
            var tenantAliasB = TestsUtil.generateTestUserId();
            var usernameA = TestsUtil.generateTestUserId();
            ...
            RestAPI.User.createUser(anonymousRestContext, usernameA, 'password', 'LoggedIn User', loggedInUserOpts, function(err, userA) {
                assert.ok(!err);
                var restCtxA = TestsUtil.createTenantRestContext(global.oaeTests.tenants.cam.host, usernameA, 'password');
