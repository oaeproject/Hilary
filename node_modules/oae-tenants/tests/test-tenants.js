...
describe('Tenants', function() {
    ...
    describe('Get tenant', function() {
        ...
        it('verify get all tenants', function(callback) {
            ...
            RestAPI.Tenant.getAllTenants(globalAdminRestContext, function(err, tenants) {
                assert.ok(!err);
                assert.ok(tenants);
                assert.equal(_.keys(tenants).length, 2);
                ...
                RestAPI.Tenant.createTenant(globalAdminRestContext, 'oxfordtest', 'Oxford University', 'oxford.oae.com', function(err) {
                    ...
                    RestAPI.Tenant.getAllTenants(globalAdminRestContext, function(err, tenants) {
                        assert.ok(tenants);
                        assert.equal(_.keys(tenants).length, 3);
                        assert.ok(tenants['camtest']);
                        assert.equal(tenants['camtest'].host, 'cambridge.oae.com');
                        assert.ok(tenants['gttest']);
                        assert.equal(tenants['gttest'].host, 'gt.oae.com');
    ...
    describe('Tenant actions', function() {
        ...
        it('verify creating tenant', function(callback) {
            RestAPI.Tenant.createTenant(globalAdminRestContext, 'nyutest', 'New York University', 'nyu.oae.com', function(err) {
                assert.ok(!err);
                // Get the tenant
        ...
        /**
         * Test that verifies that creating a tenant needs an alias, a name and a host specified
         */
    ...
    describe('Update tenant', function() {
        ...
        /**
         * Test that verifies that a tenant's name can not be updated without specifying the alias and name
         */
        it('verify update tenant name requires a alias and name', function(callback) {
            RestAPI.Tenant.updateTenant(camAdminRestContext, null, 'Anglia Ruskin University', function(err) {
                assert.ok(err);
                assert.equal(err.code, 400);

                RestAPI.Tenant.updateTenant(camAdminRestContext, 'camtest', null, function(err) {
                    assert.ok(err);
                    assert.equal(err.code, 400);
                    callback();
                });
