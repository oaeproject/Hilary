 ...
 *
 * @param  {Object[]}   resources           The resources representing the content item whose comments to index
 * @param  {Function}   callback            Standard callback function
 * @param  {Object}     callback.err        An error that occurred, if any
 * @param  {Object[]}   callback.documents  The content comment search documents
 * @api private
 */
 ...
 *
 * @param  {Object[]}   resources           The resources representing the content item whose body to index
 * @param  {Function}   callback            Standard callback method
 * @param  {Object}     callback.err        An error that occurred, if any
 * @param  {Object[]}   callback.documents  The content body search documents
 * @api private
 */
var _produceContentBodyDocuments = function(resources, callback, _documents) {
    _documents = _documents || [];
    if (_.isEmpty(resources)) {
        return callback(null, _documents);
    }
    ...
    ContentDAO.Revisions.getRevisions(resource.id, null, 1, null, function(err, revisions) {
        ...
        if (!revision.previews || !revision.previews.pageCount) {
            log().trace({'id': resource.id, 'previews': revision.previews}, 'No text to index');
            return callback(null, _documents);
        }


        // Call the callback only after we have all the pages
        var _done = _.after(revision.previews.pageCount, function() {
            // Generate a single string containing the entire body as HTML
            body = body.join('\n');

            // Generate a child document for the HTML body
            var childDoc = SearchUtil.createChildSearchDocument(ContentConstants.search.MAPPING_CONTENT_BODY, resource.id, {'content_body': body})

            // Add it and move on to the next document
            _documents.push(childDoc);
            return _produceContentBodyDocuments(resources, callback, _documents);
        });

        // Ensure that we don't accidentally pass errors more than once
        var errorCallback = _.once(callback);
        var tenantAlias = revision.previewsId.split(':')[1];

        /*!
         * Get the html preview of a page
         *
         * @param  {Number}     index   The page number to get
         */
        var _getPage = function(index) {
            ContentDAO.Previews.getContentPreview(revision.previewsId, util.format('page.%d.html', index), function(err, preview) {
                if (err) {
                ...
                ContentUtil.getStorageBackend(null, preview.uri).get(tenantAlias, preview.uri, function(err, file) {
                    ...
                    fs.readFile(file.path, function(err, data) {
                        if (err) {
                            log().error({'err': err}, 'Unable to read a page');
                        } else {
                            log().trace('Extracted text from page ' + index);
                            body[index - 1] = data;
                        }

                        // In all cases, the file should be removed again
                        fs.unlink(file.path, _done);
                    });
                });
            });
        };

        // Iterate over the pages adding them to the body
        _.range(1, revision.previews.pageCount + 1).forEach(_getPage);
    });
};
