...
OAE.tenantRouter.on('post', '/api/content/create', function(req, res) {
    ...
    /*!
     * Can be called once the content item has been created. This function will
     * add the created piece of content to a set of specified folders if any were specified
     */
    var _contentCreated = function(err, contentObj) {
        ...
        var folderIds = OaeUtil.toArray(req.body.folderIds);
        _addContentItemToFolders(req.ctx, contentObj, folderIds, function(err) {
            if (err) {
                return res.send(err.code, err.msg);
            }
     ...
     */
    var _addContentItemToFolders = function(ctx, contentObj, folderIds, callback) {
        if (folderIds.length === 0) {
            return callback();
        }

        var folderId = folderIds.pop();
        FoldersAPI.addContentItemsToFolder(ctx, folderId, [contentObj.id], function(err) {
            if (err) {
                return callback(err);
            }

            _addContentItemToFolders(ctx, contentObj, folderIds, callback);
        });
    };
