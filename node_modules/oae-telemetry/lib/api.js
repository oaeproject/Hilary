 ...
 */
var TelemetryAPI = (module.exports = new EmitterAPI.EventEmitter());
 ...
 */
var init = (module.exports.init = function(_telemetryConfig, callback) {
  ...
  if (telemetryConfig.enabled && telemetryConfig.publisher) {
    ...
    // full day
    _resetTelemetryCounts(function(err) {
      if (err) {
        return callback(err);
      }
      ...
      resetIntervalId = setInterval(
        _resetTelemetryCounts,
        telemetryConfig.resetInterval * 1000,
      );
...
module.exports.request = function(req, res) {
  ...
  res.on('finish', function() {
    if (req.telemetryUrl) {
      ...
      var requestCountName = util.format(
        '%s.%s.count',
        req.method,
        req.telemetryUrl,
      );
      var requestTimeName = util.format(
        '%s.%s.time',
        req.method,
        req.telemetryUrl,
      );
  ...
  _resetGlobalCounts(function() {
    ...
    Redis.getClient().del(_getTelemetryCountResetLock(), function(resetErr) {
      if (resetErr) {
        log().error(
          { err: resetErr },
          'Error trying to reset the count reset lock',
        );
      }

      Redis.getClient().del(_getTelemetryCountPublishLock(), function(
        publishErr,
      ) {
        if (publishErr) {
          log().error(
            { err: publishErr },
            'Error trying to reset the telemetry publish lock',
          );
        }
...
var getTelemetryData = (module.exports.getTelemetryData = function(callback) {
  _pushCountsToRedis(function() {
    _getCounts(function(err, countsHash) {
      if (err) {
        return callback(err);
      }
...
var _publishTelemetryData = function() {
  ...
  _pushCountsToRedis(function() {
    ...
    _lockAndGetCounts(function(countsHash) {
      var data = _mergeHistograms(countsHash);
 ...
 * @api private
 */
var _resetTelemetryCounts = function(callback) {
  callback = callback || function() {};
  Locking.acquire(
    _getTelemetryCountResetLock(),
    telemetryConfig.resetInterval,
    function(err, token) {
      if (err) {
        log().error(
          { err: err },
          'Error acquiring lock to reset telemetry data',
        );
        return callback();
      } else if (!token) {
      ...
      _resetLocalCounts();
      _resetGlobalCounts(function(err) {
        if (err) {
          log().error({ err: err }, 'Error resetting the telemetry data');
        } else {
  ...
  Locking.acquire(
    ...
    function(err, token) {
      if (err) {
        log().error(
          { err: err },
          'Error acquiring lock to publish telemetry counts',
        );
        return callback();
      } else if (!token) {
...
var _getCounts = function(callback) {
  Redis.getClient().hgetall(_getTelemetryCountHashKey(), function(
    err,
    countsHash,
  ) {
    if (err) {
 ...
 * @api private
 */
var _mergeHistograms = function(countsHash) {
  var mergedData = {};
...
var _applyTelemetryConfig = function(_telemetryConfig) {
  ...
  telemetryConfig.publishInterval = OaeUtil.getNumberParam(
    telemetryConfig.publishInterval,
    30,
    1,
  );
  telemetryConfig.resetInterval = OaeUtil.getNumberParam(
    telemetryConfig.resetInterval,
    86400,
    1,
  );
 ...
 * @api private
 */
var _resetLocalHistograms = function() {
  stats = stats || {};
  stats.histograms = {};
};
 ...
 * @api private
 */
var _resetLocalCounts = function() {
  stats = stats || {};
  stats.counts = {};
};
 ...
 * @api private
 */
var _getTelemetryCountPublishLock = function() {
 ...
 * @api private
 */
var _getTelemetryCountResetLock = function() {
