...
const FoldersConstants = require('oae-folders/lib/constants').FoldersConstants;
...
const _search = function(ctx, opts, callback) {
    ...
    SearchUtil.filterScopeAndAccess(ctx, opts.scope, _needsFilterByExplicitAccess(ctx, opts), (err, filterScopeAndAccess) => {
        if (err) {
            return callback(err);
        }
...
const _needsFilterByExplicitAccess = function(ctx, opts) {
    ...
    const includesContentOrGroups = (opts.searchAllResourceTypes || !_.chain(RESOURCE_TYPES_ACCESS_SCOPED).intersection(opts.resourceTypes).isEmpty().value());
