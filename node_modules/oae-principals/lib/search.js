 ...
 */
PrincipalsAPI.on(PrincipalsConstants.events.CREATED_USER, (ctx, user) => {
 ...
 */
PrincipalsAPI.on(PrincipalsConstants.events.UPDATED_USER, (ctx, user) => {
    SearchAPI.postIndexTask('user', [{'id': user.id}], {
        'resource': true
    });
});
 ...
 */
PrincipalsAPI.on(PrincipalsConstants.events.DELETED_USER, (ctx, user) => {
    SearchAPI.postIndexTask('user', [{'id': user.id}], {
        'resource': true
    });
});
 ...
 */
PrincipalsAPI.on(PrincipalsConstants.events.RESTORED_USER, (ctx, user) => {
    SearchAPI.postIndexTask('user', [{'id': user.id}], {
        'resource': true
    });
});
 ...
 */
PrincipalsAPI.on(PrincipalsConstants.events.VERIFIED_EMAIL, (ctx, user) => {
    SearchAPI.postIndexTask('user', [{'id': user.id}], {
        'resource': true
    });
});
 ...
 */
PrincipalsAPI.on(PrincipalsConstants.events.UPDATED_GROUP, (ctx, group) => {
    SearchAPI.postIndexTask('group', [{'id': group.id}], {
        'resource': true
    });
});
 ...
 * memberships documents
 */
PrincipalsAPI.on(PrincipalsConstants.events.LEFT_GROUP, (ctx, group, memberChangeInfo) => {
    _handleUpdateGroupMembers(ctx, group, _.keys(memberChangeInfo.changes));
});
...
const _handleInvalidateSearch = function(group, membershipsGraph, membersGraph, callback) {
    ...
    SearchAPI.postIndexTask('group', resourceGroupIndexTask, resourceIndexOp, (err) => {
        ...
        OaeUtil.invokeIfNecessary(!_.isEmpty(memberGroupIndexTasks), SearchAPI.postIndexTask, 'group', memberGroupIndexTasks, membershipsIndexOp, (err) => {
            if (err) {
                allErrs = _.union(allErrs, [err]);
            }

            // If there are user index tasks to invoke, do it
            OaeUtil.invokeIfNecessary(!_.isEmpty(memberUserIndexTasks), SearchAPI.postIndexTask, 'user', memberUserIndexTasks, membershipsIndexOp, (err) => {
                if (err) {
                    allErrs = _.union(allErrs, [err]);
                }
