 ...
 *
 * The email address will be set when a user has not set one himself or when the `forceProfileUpdate` parameter is `true`.
 *
 * @param  {Context}        ctx                     The current context
 ...
 * @param  {Boolean}        [forceProfileUpdate]    Whether or not the user's display name and/or email should be updated with the value specified in the CSV file, even when the display name is different than the external id or an email address has been set. By default, this will be set to `false`
    ...
    csv().from.path(userCSV.path).to.array(function(data) {
        ...
        _cleanUpCSVFile(userCSV, function() {
            ...
            var processUser = function(data) {
                ...
                require('oae-authentication').getOrCreateUser(adminCtx, authenticationStrategy, externalId, displayName, opts, function(err, user, status) {
                    if (err) {
...
var getMe = module.exports.getMe = function(ctx, callback) {
    if (!ctx.user()) {
        ...
        return callback({'code': 200, 'msg': anonMe});
    }
    ...
    getUser(ctx, ctx.user().id, function(err, data) {
        ...
        data.signature = Signature.createRollingSignature(ctx.user().tenant.alias, PrincipalsConstants.signature.EXPIRES, 10, ctx.user().id);
