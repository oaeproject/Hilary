...
ContentAPI.on(ContentConstants.events.UPDATED_CONTENT, function(ctx, newContentObj, oldContentObj) {
    if (newContentObj.visibility !== oldContentObj.visibility) {
        ...
        AuthzAPI.getAuthzMembers(newContentObj.id, null, 10000, function(err, members) {
            if (err) {
                log().error({'err': err, 'contentId': newContentObj.id}, 'Unable to get the members of a piece of content');
                return;
            }

            // Folders are masked as groups in AuthZ
            var groups = _.filter(members, function(member) {
                return AuthzUtil.isGroupId(member.id);
            });

            // Purge all the folder content libraries for the groups who are a
            // member of the content item. This can potential include proper
            // groups but is fairly harmless
            _.each(groups, function(group) {
                LibraryAPI.Index.purge(FoldersConstants.library.CONTENT_LIBRARY_INDEX_NAME, group.id, function(err) {
                    if (err) {
                        log().error({'err': err, 'contentId': contentId, 'groupId': group.id}, 'Unable to purge folder library');
                    }
...
/*!
 * Configure the folder content search endpoint
 */
