...
module.exports.config = function(moduleId) {
    ...
    return {
        ...
        /**
         * Get the timestamp when a value for an element has been last updated
         *
         * @param  {String}     tenantAlias           The alias of the tenant for which to get the last updated timestamp
         * @param  {String}     feature               The feature to get the element's last updated timestamp for. e.g. `twitter`
         * @param  {String}     element               The element to get the config last updated timestamp for. e.g. `enabled`
         * @return {Date}       cachedConfiguration   The timestamp when the element was last updated. If the element could not be found, the epoch date will be returned
         */
            ...
            _.each(module, function(feature, featureKey) {
                ...
                _.each(feature.elements, function(element, elementKey) {
                    ...
                    if (column) {
                        ...
                        cachedGlobalConfig[tenantAlias][moduleKey][featureKey][elementKey] = element.toJSONValue(column.value);
...
var updateConfig = module.exports.updateConfig = function(ctx, tenantAlias, configValues, callback) {
    ...
    // Convert the provided values to the correct DB data.
    var dbValues = {};
    var temp = {};
    _.each(configValues, function(newValue, key) {
        ...
        // The column name in cassandra is made out of the first three parts
        var columnName = module + '/' + feature + '/' + element;

        // We need the previous value to update some fields
        // However, because we can submit multiple values for a single element (ex: a value per locale for the internationalizable field)
        // we need to keep track of our updates
        var previousValue = temp[columnName] || cachedTenantConfig[tenantAlias][module][feature][element];

        // Convert this value to something that can be stored in a single column
        dbValues[columnName] = cachedGlobalSchema[module][feature].elements[element].toDBValue(previousValue, optionalKey, newValue);

        // Convert that output back in to a JSON value, so we can use it for the next key (if it needs it)
        temp[columnName] = cachedGlobalSchema[module][feature].elements[element].toJSONValue(dbValues[columnName]);
    });

    var q = Cassandra.constructUpsertCQL('Config', 'tenantAlias', tenantAlias, dbValues, 'QUORUM');
...
var clearConfig = module.exports.clearConfig = function(ctx, tenantAlias, configFields, callback) {
    ...
    for (var i = 0; i < configFields.length; i++) {
        ...
        var canUpdateValue = canUpdateConfigValue(ctx, tenantAlias, configField[0], configField[1], configField[2]);
        if (tenantAlias === config.servers.globalAdminAlias) {
            return callback({'code': 400, 'msg': 'Cannot clear config fields on the admin tenant'});
        }
        if (!canUpdateValue) {
            return callback({'code': 401, 'msg': 'User is not allowed to update config value'});
        }
    ...
    for (var j = 0; j < configFields.length; j++) {
        queries.push({
            ...
            'parameters': [configFields[j], tenantAlias]
        });
    }
