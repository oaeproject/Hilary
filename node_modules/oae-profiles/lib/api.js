...
var setSection = module.exports.setSection = function(context, principalId, sectionName, sectionVisibility, sectionData, overwrite, callback) {
    ...
    if (context.user().id === principalId || context.user().isAdmin(principalResource.tenantAlias)) {
...
var _hasAccess = function(context, profileSection, callback) {
    ...
    if (!context.user() && (profileSection.visibility !== Constants.visibility.PUBLIC)) {
        return callback({'code': 401, 'msg': 'You don\'t have access to this profile section. Try logging in.'});
    }

    var principalResource = AuthzUtil.getResourceFromId(profileSection.principalId);
    if (context.user().id === profileSection.principalId || context.user().isAdmin(principalResource.tenantAlias) || profileSection.visibility !== Constants.visibility.PRIVATE) {
        callback(null, true);
    } else {
        return callback({'code': 401, 'msg': 'You don\'t have access to this profile section.'});
    }
