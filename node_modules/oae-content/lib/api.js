...
var createContent = module.exports.createContent = function(ctx, visibility, name, description, contentType, callback) {
    ...
    // Create the content
    Cassandra.runQuery('INSERT INTO Content (contentId, tenant, visibility, name, description, contentType, lastModified) VALUES (?, ?, ?, ?, ?, ?, ?) USING CONSISTENCY QUORUM', [contentId, ctx.tenant().alias, visibility, name, description, contentType, lastModified], function (err) {
        if (err) {
...
var _updateContentMetadata = function(ctx, contentId, metadata, callback) {
    ...
    canManage(ctx, contentId, function(err, canManage, contentObj) {
        ...
        Cassandra.runQuery('UPDATE Content USING CONSISTENCY QUORUM SET ' + q.join(', ') + ' WHERE contentId = ?', parameters, function(err) {
            if (err) {
                return callback(err);
            }
