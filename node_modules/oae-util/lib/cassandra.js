 ...
 * @param  {Function}   callback    Callback method
 */
module.exports.runBatchQuery = function(queries, consistency, callback) {
    if (queries.length === 0) {
        return callback();
    }
 ...
 *
 * @param  {String} start           The column name to start from. Can be null. If it is not null this column will be included
 *                                  and the startIndex will be incremented by one. (ie: you can start from the *next* column.)
 * @param  {Number} limit           The amount of columns that should be retrieved. If this value is null/undefined,
 *                                  `25` will be used.
 * @param  {String} start_if_empty  If the `start` parameter is null or undefined this value will be used. 
 *                                  If this is left null or undefined an empty string will be used.
 * @return {Object}                 Returns an object that holds 3 keys.
 *                                   * start: The column name to start selecting from
 *                                   * limit: The number of columns that should be retrieved. If the `start` value
 *                                            is specified it will increase the `limit` value by one.
 *                                   * startIndex: From what index you should start reading as the `start` column will
 *                                                 be in the results.
 */
module.exports.getPagingParameters = function(start, limit, start_if_empty) {
    var limit = limit || 25;
    var startIndex = 0;
    start_if_empty = start_if_empty || '';
    if (start) {
        limit++;
        startIndex = 1;
    } else {
        start = start_if_empty;
    }
    return {
        'start': start,
        'limit': limit,
...
/**
 * Utility method that constructs an update CQL query with a flexible number of columns.
 * This creates a query with the following form: UPDATE testCF SET ? = ?, ? = ?, ? = ? WHERE testId = ?
 * @param  {String}     cf              The name of the column family in which we're doing an update
 * @param  {Object}     values          JSON object where the keys represent the column names of the columns that
 *                                      need to be updated and the values represent the new column values
 * @param  {String}     whereColumn     The column name to use in the WHERE clause
 * @param  {String}     whereValue      The column value to use in the WHERE clause
 * @return {Object}                     Returns a JSON object with a query key that contains the generated
 *                                      CQL query and a parameters key that contains the generated parameter array
 *                                      {query: CQLQuery, parameters: [parameterArray]}
 */
module.exports.constructUpdateCQL = function(cf, values, whereColumn, whereValue) {
    // Check that cf, whereColumn and whereValue have been provided 
    // Also check that values has at least 1 key-value pair
    if (!cf || !whereColumn || !whereValue || _.keys(values).length === 0) {
        return false;
    }
    // Construct the query
    var q = {
    ...
    q.query = 'UPDATE ' + cf + ' SET ' + tempQuery.join(', ') + ' WHERE ' + whereColumn + ' = ?';
    q.parameters.push(whereValue);
    return q;
};

/**
 * Utility method that constructs an insert CQL query with a flexible number of columns.
 * This creates a query with the following form: INSERT INTO testCF (column1, column2) VALUES (?, ?)
 * @param  {String}     cf              The name of the column family in which we're doing an insert
 * @param  {Object}     values          JSON object where the keys represent the column names of the columns that
 *                                      need to be inserted and the values represent the column values
 * @return {Object}                     Returns a JSON object with a query key that contains the generated
 *                                      CQL query and a parameters key that contains the generated parameter array
 *                                      {query: CQLQuery, parameters: [parameterArray]}
 */
module.exports.constructInsertCQL = function(cf, values) {
    // Check that cf has been provided 
    // Also check that values has at least 1 key-value pair
    if (!cf || _.keys(values).length === 0) {
        return false;
    }
    // Construct the query
    var q = {
    ...
    q.query = 'INSERT INTO ' + cf + ' (' + _.keys(values).join(', ') + ') VALUES (' + tempQuery.join(', ') + ')';
    q.parameters = _.values(values);
    return q;
};
