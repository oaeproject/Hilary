...
var MessageBoxConstants = require('oae-messagebox/lib/constants')
  .MessageBoxConstants;
 ...
 */
var getContent = (module.exports.getContent = function(
  ctx,
  contentId,
  callback,
) {
  ...
  validator
    .check(contentId, { code: 400, msg: 'A content id must be provided' })
    .isResourceId();
 ...
 */
var getFullContentProfile = (module.exports.getFullContentProfile = function(
  ctx,
  contentId,
  callback,
) {
...
var _getFullContentProfile = function(ctx, contentObj, isManager, callback) {
  ...
  PrincipalsUtil.getPrincipal(ctx, contentObj.createdBy, function(
    err,
    createdBy,
  ) {
    if (err) {
    ...
    AuthzPermissions.canShare(
      ...
      function(err) {
        ...
        if (contentObj.resourceSubType !== 'collabdoc') {
          ContentAPI.emit(
            ContentConstants.events.GET_CONTENT_PROFILE,
            ctx,
            contentObj,
          );
          return callback(null, contentObj);
        ...
        _getRevision(ctx, contentObj, contentObj.latestRevisionId, function(
          err,
          revision,
        ) {
          if (err) {
 ...
 */
var createLink = (module.exports.createLink = function(
  ctx,
  displayName,
  description,
  visibility,
  ...
  visibility =
    visibility || Config.getValue(ctx.tenant().alias, 'visibility', 'links');

  // Check if the link property is present. All other validation will be done in the _createContent function
  var validator = new Validator();
  validator
    .check(link, { code: 400, msg: 'A valid link must be provided' })
    .isUrl();
  validator
    .check(null, {
      code: 401,
 ...
 */
var createFile = (module.exports.createFile = function(
  ctx,
  displayName,
  description,
  visibility,
...
var _createFile = function(
  ...
  // Setting content to default if no visibility setting is provided
  visibility =
    visibility || Config.getValue(ctx.tenant().alias, 'visibility', 'files');
  ...
  validator
    .check(displayName, { code: 400, msg: 'A display name must be provided' })
    .notEmpty();
  validator
    .check(displayName, {
      code: 400,
  ...
  if (file) {
    validator
      .check(file.size, { code: 400, msg: 'Missing size on the file object' })
      .notEmpty();
    validator
      .check(file.size, { code: 400, msg: 'Invalid size on the file object' })
      .isInt();
    validator
      .check(file.size, { code: 400, msg: 'Invalid size on the file object' })
      .min(0);
    validator
      .check(file.name, { code: 400, msg: 'Missing name on the file object' })
      .notEmpty();
  }
  if (validator.hasErrors()) {
    return callback(validator.getFirstError());
 ...
 */
var createCollabDoc = (module.exports.createCollabDoc = function(
  ctx,
  displayName,
  description,
  visibility,
  ...
  visibility =
    visibility ||
    Config.getValue(ctx.tenant().alias, 'visibility', 'collabdocs');
...
var _createContent = function(
  ...
  validator
    .check(resourceSubType, {
      code: 400,
      msg:
        'A valid resourceSubType must be provided. This can be "file", "collabdoc" or "link"',
    })
  ...
  canManageFolders(ctx, folderIds, function(err, folders) {
    ...
    ResourceActions.create(ctx, roles, createFn, function(
      err,
      content,
      revision,
      memberChangeInfo,
      ...
      _addContentItemToFolders(ctx, content, folders, function(err) {
        if (err) {
          log().warn(
            { err: err, contentId: content.id, folders: folders },
...
var canManageFolders = function(ctx, folderIds, callback) {
  ...
  _.each(folderIds, function(folderId) {
    ...
    validator
      .check(folderId, { code: 400, msg: 'Invalid folder id specified' })
      .isResourceId();
  });
  ...
  require('oae-folders/lib/internal/dao').getFoldersByIds(folderIds, function(
    err,
    folders,
  ) {
    if (err) {
...
var handlePublish = (module.exports.handlePublish = function(data, callback) {
  ...
  PrincipalsDAO.getPrincipal(data.userId, function(err, user) {
    ...
    ContentDAO.Content.getContent(data.contentId, function(err, contentObj) {
      ...
      Etherpad.getHTML(contentObj.id, contentObj.etherpadPadId, function(
        err,
        html,
      ) {
        if (err) {
        ...
        ContentDAO.Revisions.getRevision(contentObj.latestRevisionId, function(
          err,
          revision,
        ) {
          if (err) {
            ...
            ContentAPI.emit(
              ContentConstants.events.EDITED_COLLABDOC,
              ctx,
              contentObj,
            );
            return callback();
          ...
          ContentDAO.Revisions.createRevision(
            ...
            function(err, revision) {
              if (err) {
                log().error(
                  { err: err, contentId: contentObj.id },
 ...
 */
var joinCollabDoc = (module.exports.joinCollabDoc = function(
  ctx,
  contentId,
  callback,
) {
  ...
  _canEdit(ctx, contentId, function(err, contentObj) {
    if (err) {
      ...
      return callback({
        code: 400,
        msg: 'This is not a collaborative document',
      });
    }
    ...
    Etherpad.joinPad(ctx, contentObj, function(err, data) {
      ...
      ContentDAO.Etherpad.saveAuthorId(
 ...
 * @param  {Object}    callback.err      An error that occurred, if any
 */
var deleteContent = (module.exports.deleteContent = function(
  ctx,
  contentId,
  callback,
) {
 ...
 * @param  {Object}    callback.err      An error that occurred, if any
 */
var shareContent = (module.exports.shareContent = function(
  ctx,
  contentId,
  principalIds,
  callback,
 ...
 * @param  {Object}          callback.err   An error that occurred, if any
 */
var setContentPermissions = (module.exports.setContentPermissions = function(
  ctx,
  contentId,
  changes,
  callback,
  ...
  getContent(ctx, contentId, function(err, content) {
    ...
    ResourceActions.setRoles(ctx, content, changes, function(
      err,
      memberChangeInfo,
    ) {
      if (err) {
 ...
 * @param  {Object}     callback.err    An error that occurred, if any
 */
var removeContentFromLibrary = (module.exports.removeContentFromLibrary = function(
  ctx,
  libraryOwnerId,
  contentId,
  callback,
  ...
  validator
    .check(null, {
      code: 401,
      msg:
        'You must be authenticated to remove a piece of content from a library',
    })
    .isLoggedInUser(ctx);
  validator
    .check(libraryOwnerId, {
      code: 400,
  ...
  ContentDAO.Content.getContent(contentId, function(err, content) {
    ...
    PrincipalsDAO.getPrincipal(libraryOwnerId, function(err, libraryOwner) {
      ...
      AuthzPermissions.canRemoveRole(ctx, libraryOwner, content, function(
        err,
        memberChangeInfo,
      ) {
        if (err) {
        ...
        AuthzAPI.updateRoles(contentId, memberChangeInfo.changes, function(
          err,
        ) {
          if (err) {
            return callback(err);
 ...
 */
var getContentMembersLibrary = (module.exports.getContentMembersLibrary = function(
  ctx,
  contentId,
  start,
  limit,
  ...
  validator
    ...
    .isResourceId();
  validator
    .check(limit, { code: 400, msg: 'A valid limit should be passed in' })
    .isInt();
  if (validator.hasErrors()) {
    return callback(validator.getFirstError());
  }
  ...
  ContentDAO.Content.getContent(contentId, function(err, content) {
    ...
    LibraryAPI.Authz.resolveTargetLibraryAccess(
      ...
      content.id,
      content,
      function(err, hasAccess, visibility) {
        if (err) {
          return callback(err);
        } else if (!hasAccess) {
          return callback({
            code: 401,
            msg:
              'You are not authorized to access the members of this content item',
          });
        }
        ...
        ContentMembersLibrary.list(
          ...
          function(err, memberIds, nextToken) {
            if (err) {
              return callback(err);
            ...
            AuthzAPI.getDirectRoles(memberIds, content.id, function(
              err,
              memberRoles,
            ) {
              if (err) {
              ...
              PrincipalsUtil.getPrincipals(ctx, memberIds, function(
                err,
                memberProfiles,
              ) {
                if (err) {
 ...
 */
var getContentInvitations = (module.exports.getContentInvitations = function(
  ctx,
  contentId,
  callback,
) {
 ...
 * @param  {Object}         callback.err    An error that occurred, if any
 */
var resendContentInvitation = (module.exports.resendContentInvitation = function(
  ctx,
  contentId,
  email,
  callback,
 ...
 */
var updateFileBody = (module.exports.updateFileBody = function(
  ctx,
  contentId,
  file,
  callback,
  ...
  callback =
    ...
    function(err) {
      if (err) {
        log().error(
          { err: err },
          'Error updating the filebody for %s',
          contentId,
        );
      }
    };
...
var _updateFileBody = function(ctx, contentId, file, callback) {
  ...
  validator
    ...
    .isResourceId();
  validator
    .check(file, { code: 400, msg: 'Missing file parameter.' })
    .notNull();
  if (file) {
    validator
      .check(file.size, { code: 400, msg: 'Missing size on the file object.' })
      ...
      function(err, uri) {
        ...
        ContentDAO.Revisions.createRevision(
          ...
          function(err, revision) {
            ...
            ContentDAO.Content.updateContent(contentObj, opts, true, function(
              err,
              updatedContentObj,
            ) {
              if (err) {
 ...
 * @param  {Object}      callback.err        An error that occurred, if any
 */
var setPreviewItems = (module.exports.setPreviewItems = function(
  ctx,
  contentId,
  revisionId,
  status,
  ...
  var validator = new Validator();
  validator
    .check(contentId, { code: 400, msg: 'Missing or invalid contentId' })
    .isResourceId();
  validator
    .check(revisionId, { code: 400, msg: 'Missing or invalid revisionId' })
    .isResourceId();
  validator
    .check(status, {
      code: 400,
  ...
  ContentDAO.Content.getContent(contentId, function(err, contentObj) {
    ...
    if (
      !ctx.user() ||
      !(
        ctx.user().isGlobalAdmin() ||
        ctx.user().isTenantAdmin(contentObj.tenant.alias)
      )
    ) {
    ...
    ContentDAO.Revisions.getRevision(revisionId, function(err, revision) {
      ...
      validator
        .check(revision.contentId, {
          code: 400,
          msg:
            'Specified revisionId does not belong to the specifed content item',
        })
        .equals(contentId);
      if (validator.hasErrors()) {
        return cleanUpCallback(validator.getFirstError());
      ...
      if (status === ContentConstants.previews.ERROR || todo === 0) {
        ...
        return ContentDAO.Previews.storeMetadata(
          ...
          function(err) {
            ContentAPI.emit(
              ContentConstants.events.UPDATED_CONTENT_PREVIEW,
              contentObj,
            );
            return callback(err);
          },
      ...
      var _finishIteration = function(err) {
        ...
        if (err) {
          ...
          log().trace(
            { data: fileData },
            'Storing %d content preview files',
            fileKeys.length,
          );
      ...
      fileKeys.forEach(function(key) {
        ...
        if (!size) {
          todo--;
          log().warn(
            'Ignoring file %s as it has no size associated to it',
            key,
          );
          return;
        }
 ...
 */
var getSignedPreviewDownloadInfo = (module.exports.getSignedPreviewDownloadInfo = function(
  ctx,
  contentId,
  revisionId,
  previewItem,
  ...
  var validator = new Validator();
  validator
    .check(contentId, { code: 400, msg: 'Missing content ID' })
    .isResourceId();
  validator
    .check(revisionId, { code: 400, msg: 'Missing revision ID' })
    .isResourceId();
  validator
    .check(previewItem, { code: 400, msg: 'Missing preview item' })
    .notEmpty();
  ...
  if (validator.hasErrors()) {
    ...
    return callback({
      code: 401,
      msg: 'Invalid content signature data for accessing previews',
    });
  }

  ContentDAO.Previews.getContentPreview(revisionId, previewItem, function(
    err,
    preview,
  ) {
    if (err) {
    ...
    var downloadStrategy = ContentUtil.getStorageBackend(
      ctx,
      preview.uri,
    ).getDownloadStrategy(ctx.tenant().alias, preview.uri);
    return callback(null, {
 ...
 */
var getPreviewItems = (module.exports.getPreviewItems = function(
  ctx,
  contentId,
  revisionId,
  callback,
  ...
  getContent(ctx, contentId, function(err, content) {
    ...
    ContentDAO.Previews.getContentPreviews(revisionId, function(err, previews) {
      ...
      var signature = Signature.createExpiringResourceSignature(
        ctx,
        content.id,
      );
      var result = {
 ...
 */
var updateContentMetadata = (module.exports.updateContentMetadata = function(
  ctx,
  contentId,
  profileFields,
  callback,
  ...
  validator
    .check(fieldNames.length, {
      code: 400,
      msg:
        'You should at least specify a new displayName, description, visibility or link',
    })
    .min(1);
  ...
  _canManage(ctx, contentId, function(err, oldContentObj) {
    ...
    if (profileFields.link) {
      if (oldContentObj.resourceSubType !== 'link') {
        return callback({
          code: 400,
          msg: 'This piece of content is not a link',
        });
 ...
 */
var createComment = (module.exports.createComment = function(
  ctx,
  contentId,
  body,
  ...
  validator
    ...
    .isResourceId();
  validator
    .check(body, { code: 400, msg: 'A comment must be provided' })
    .notEmpty();
  validator
    .check(body, {
      code: 400,
  ...
  getContent(ctx, contentId, function(err, contentObj) {
    ...
    MessageBoxAPI.createMessage(
      ...
      function(err, message) {
        ...
        PrincipalsUtil.getPrincipal(ctx, ctx.user().id, function(
          err,
          createdBy,
        ) {
          if (err) {
 ...
 * @param  {String}     callback.nextToken   The value to provide in the `start` parameter to get the next set of results
 */
var getComments = (module.exports.getComments = function(
  ctx,
  contentId,
  start,
  limit,
  ...
  getContent(ctx, contentId, function(err, contentObj) {
    ...
    MessageBoxAPI.getMessagesFromMessageBox(
      ...
      limit,
      null,
      function(err, comments, nextToken) {
        if (err) {
          return callback(err);
        }
 ...
 */
var deleteComment = (module.exports.deleteComment = function(
  ctx,
  contentId,
  commentCreatedDate,
  callback,
  ...
  getContent(ctx, contentId, function(err, content) {
    ...
    MessageBoxAPI.getMessages(contentId, [commentCreatedDate], null, function(
      err,
      messages,
    ) {
      if (err) {
        ...
        return callback({
          code: 404,
          msg: 'The specified comment does not exist',
        });
      }
...
var _deleteComment = function(ctx, content, commentToDelete, callback) {
  ...
  MessageBoxAPI.deleteMessage(
    ...
    function(err, deleteType, deletedComment) {
      if (err) {
        return callback(err);
      }
 ...
 * @param  {String}         callback.nextToken  The value to provide in the `start` parameter to get the next set of results
 */
var getContentLibraryItems = (module.exports.getContentLibraryItems = function(
  ctx,
  principalId,
  start,
  limit,
    ...
    LibraryAPI.Authz.resolveTargetLibraryAccess(
      ...
      function(err, hasAccess, visibility) {
        if (err) {
          ...
          return callback({
            code: 401,
            msg: 'You do not have access to this library',
          });
        }
        ...
        ContentDAO.Content.getContentLibraryItems(
          ...
          start,
          limit,
          function(err, contentObjects, nextToken) {
            if (err) {
              return callback(err);
            }
 ...
 * @param  {String}         callback.nextToken  The value to provide in the `start` parameter to get the next set of results
 */
var getRevisions = (module.exports.getRevisions = function(
  ctx,
  contentId,
  start,
  limit,
  ...
  var validator = new Validator();
  validator
    .check(contentId, { code: 400, msg: 'A contentId must be provided' })
    .isResourceId();
  validator
    .check(limit, { code: 400, msg: 'A valid limit should be passed in' })
    .isInt();
...
var _getRevisions = function(ctx, contentObj, start, limit, opts, callback) {
  // Page the query.
  ContentDAO.Revisions.getRevisions(contentObj.id, start, limit, opts, function(
    err,
    revisions,
    nextToken,
  ) {
    ...
    var userIds = _.map(revisions, function(revisions) {
 ...
 */
var getRevision = (module.exports.getRevision = function(
  ctx,
  contentId,
  revisionId,
  callback,
 ...
 */
var getRevisionDownloadInfo = (module.exports.getRevisionDownloadInfo = function(
  ctx,
  contentId,
  revisionId,
  callback,
  ...
  getContent(ctx, contentId, function(err, content) {
    if (err) {
      ...
      return callback({
        code: 400,
        msg: 'Only file content items can be downloaded',
      });
    }
    ...
    if (!revisionId) {
      return callback({
        code: 400,
        msg:
          'No revision id provided and content item does not have a latest revision id',
      });
    }
    ...
    ContentDAO.Revisions.getRevision(revisionId, function(err, revision) {
      if (err) {
        ...
        return callback({
          code: 400,
          msg:
            'The revision id provided is not associated with the specified content item',
        });
      }
      ...
      return callback(null, {
        ...
        strategy: ContentUtil.getStorageBackend(
          ctx,
          revision.uri,
        ).getDownloadStrategy(ctx.tenant().alias, revision.uri),
      });
    });
  });
});
  ...
  if (!revisionId) {
    ...
    _getRevisions(ctx, contentObj, null, 1, null, function(err, revisions) {
      if (err) {
        ...
        return callback({
          code: 404,
          msg: 'No revision found for ' + contentObj.id,
        });
      }
    ...
    ContentDAO.Revisions.getRevision(revisionId, function(err, revisionObj) {
      if (err) {
        ...
        return callback({
          code: 400,
          msg:
            'This revision ID is not associated with the specified piece of content.',
        });
      }
...
var _augmentRevision = function(ctx, revision, contentObj) {
  ...
  if (revision.thumbnailUri) {
    revision.thumbnailUrl = _getPictureDownloadUrlFromUri(
      ctx,
      revision.thumbnailUri,
      revision.id,
    );
  ...
  if (revision.mediumUri) {
    revision.mediumUrl = _getPictureDownloadUrlFromUri(
      ctx,
      revision.mediumUri,
      revision.id,
    );
 ...
 */
var restoreRevision = (module.exports.restoreRevision = function(
  ctx,
  contentId,
  revisionId,
  callback,
    ...
    ContentDAO.Revisions.getRevision(revisionId, function(err, revision) {
      if (err) {
        ...
        return callback({
          code: 400,
          msg:
            'The contentId specified is not the owner of the specified revisionId',
        });
      }
      ...
      ContentDAO.Revisions.createRevision(
        ...
        function(err, newRevision) {
          ...
          ContentDAO.Content.updateContent(contentObj, updates, true, function(
            err,
            newContentObj,
          ) {
            if (err) {
 ...
 */
var verifySignedDownloadQueryString = (module.exports.verifySignedDownloadQueryString = function(
  ctx,
  qs,
  callback,
) {
  ...
  if (!uri) {
    return callback({
      code: 401,
      msg: 'Invalid signature data for the provided download url',
    });
  }
  ...
  var downloadStrategy = ContentUtil.getStorageBackend(
    ctx,
    uri,
  ).getDownloadStrategy(ctx.tenant().alias, uri);
  return callback(null, {
    strategy: downloadStrategy,
...
var _getPictureDownloadUrlFromUri = function(ctx, uri, parentId) {
  try {
    ...
    log(ctx).warn(
      { parentId: parentId },
      'Could not find storage backend for uri: %s',
      uri,
    );
    return null;
  }
