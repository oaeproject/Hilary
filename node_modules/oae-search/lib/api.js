...
const SearchResult = require('oae-search/lib/model').SearchResult;
 ...
 */
const SearchAPI = module.exports = new EmitterAPI.EventEmitter();
 ...
 */
const registerSearchDocumentTransformer = module.exports.registerSearchDocumentTransformer = function(typeName, transformer) {
 ...
 */
const registerSearchDocumentProducer = module.exports.registerSearchDocumentProducer = function(typeName, producer) {
 ...
 */
const registerSearch = module.exports.registerSearch = function(typeName, queryBuilder, postProcessor) {
    if (searches[typeName]) {
        throw new Error('Search type ' + typeName + ' already exists');
    }
    searches[typeName] = {
        queryBuilder,
        'postProcessor': postProcessor || function(ctx, opts, results, callback) {
 ...
 */
const registerReindexAllHandler = module.exports.registerReindexAllHandler = function(handlerId, handler) {
 ...
 */
const registerChildSearchDocument = module.exports.registerChildSearchDocument = function(name, options, callback) {
 ...
 */
const refreshSearchConfiguration = module.exports.refreshSearchConfiguration = function(searchConfig, callback) {
 ...
 * @param  {Object}     callback.err    An error that occurred, if any
 */
const buildIndex = module.exports.buildIndex = function(destroy, callback) {
 ...
 */
const search = module.exports.search = function(ctx, searchType, opts, callback) {
        ...
        client.search(queryData, null, (err, elasticSearchResponse) => {
            ...
            _.each(elasticSearchResponse.hits.hits, (hit) => {
                try {
                    ...
                    log().warn({'err': parseErr, hit}, 'Failed to parse _extra field of search document into JSON. Ignoring');
                }
            });

            SearchUtil.transformSearchResults(ctx, searchDocumentTransformers, elasticSearchResponse, (err, transformedResults) => {
                if (err) {
                    return callback(err);
                }
 ...
 * @param  {Object}     callback.err    An error that occurred, if any
 */
const postReindexAllTask = module.exports.postReindexAllTask = function(ctx, callback) {
 ...
 * @param  {Object}             [callback.err]      An error that occurred, if any
 */
const postIndexTask = module.exports.postIndexTask = function(resourceType, resources, index, callback) {
    callback = callback || function(err) {
        if (err) {
            log().error({
                err,
 ...
 * @param  {Object}     callback.err    An error that occurred, if any
 */
const postDeleteTask = module.exports.postDeleteTask = function(id, children, callback) {
...
var _handleIndexDocumentTask = function(data, callback) {
    ...
    _.each(data.resources, (resource) => {
        ...
        _.chain(data.index.children).keys().each((documentType) => {
    ...
    if (searchDocumentProducer) {
        searchDocumentProducer(resourcesToIndex[resourceType], (errs, documents) => {
            ...
            documents = _.map(documents, (doc) => {
                const newDoc = _.extend({}, doc, {'_type': SearchConstants.search.MAPPING_RESOURCE, resourceType});
                if (newDoc._extra) {
