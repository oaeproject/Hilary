...
var generateTestUsers = module.exports.generateTestUsers = function(restCtx, total, callback, _createdUsers) {
    ...
    _ensureAuthenticated(restCtx, function(err) {
        ...
        RestAPI.User.createUser(restCtx, username, 'password', displayName, {'email': email}, function(err, user) {
            ...
            PrincipalsDAO.updatePrincipal(user.id, {'emailVerified': true}, null, function(err) {
                if (err) {
                    return callback(err);
                }
...
/**
 * Generate a number of random guests users that can be used inside of tests
 *
 * @param  {Number}         total               The total number of guest users that need to be created
 * @param  {Function}       callback            Standard callback function
 * @param  {Object}         callback.user1      An object with key `user` containing the User object, and `restContext` containing that guest user's rest context for one of the users that was created
 * @param  {Object}         [callback.user2]    Another user that was created
 * @param  {Object}         [callback....]      Each user that was generated as new callback arguments
 */
var generateGuestUsers = module.exports.generateGuestUsers =  function(total, callback, _createdUsers) {
    _createdUsers = _createdUsers || [];
    if (total === 0) {
    ...
    var email = generateTestEmailAddress(username);
    var anonymousGuestRestContext = createTenantRestContext('guests.oae.com');
    RestAPI.User.createUser(anonymousGuestRestContext, username, 'password', displayName, {'email': email}, function(err, user) {
        assert.ok(!err);

        // Verify the guest user's email
        PrincipalsDAO.updatePrincipal(user.id, {'emailVerified': true}, null, function(err) {
            if (err) {
                return callback(err);
            }
            ...
            _createdUsers.push({
                'user': user,
                'restContext': new RestContext(anonymousGuestRestContext.host, {
                    'hostHeader': anonymousGuestRestContext.hostHeader,
                    'username': username,
                    'userPassword': 'password',
                    'strictSSL': anonymousGuestRestContext.strictSSL
                })
            });

            // Recursively continue creating guest users
            return generateGuestUsers(total - 1, callback, _createdUsers);
        });
    });
};
...
var createTenantWithAdmin = module.exports.createTenantWithAdmin = function(tenantAlias, tenantHost, callback) {
    ...
    TenantsTestUtil.createTenantAndWait(adminCtx, tenantAlias, tenantAlias, tenantHost, function(err, tenant) {
        ...
        ConfigTestUtil.updateConfigAndWait(adminCtx, tenantAlias, {'oae-principals/recaptcha/enabled': false}, function(err) {
            ...
            RestAPI.User.createUser(anonymousCtx, 'administrator', 'administrator', 'Tenant Administrator', {'email': email}, function(err, tenantAdmin) {
                if (err) {
                    return callback(err);
                }

                PrincipalsDAO.updatePrincipal(tenantAdmin.id, {'emailVerified': true}, null, function(err) {
                    if (err) {
                        return callback(err);
                    }

                    tenantAdmin.emailVerified = true;
...
var _createUserWithVisibility = function(restCtx, visibility, callback) {
    ...
    RestAPI.User.createUser(restCtx, username, password, displayName, {'visibility': visibility, 'publicAlias': publicAlias, 'email': email}, function(err, user) {
        assert.ok(!err);
...
var createInitialTestConfig = module.exports.createInitialTestConfig = function() {
    ...
    config.servers.guestsTenantHost = 'guests.oae.com';
