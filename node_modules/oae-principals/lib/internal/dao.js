...
var updatePrincipals = module.exports.updatePrincipals = function(principalIds, profileFields, callback) {
    ...
    Cassandra.runBatchQuery(queries, 'QUORUM', function(err) {
        ...
        if (usersToInvalidate.length > 0) {
            invalidateCachedUsers(usersToInvalidate, callback);
        } else {
            return callback();
        }
    });
...
var getPrincipalFromCassandra = function(principalId, callback) {
    Cassandra.runQuery('SELECT * FROM Principals USING CONSISTENCY QUORUM WHERE principalId = ?', [principalId], function (err, rows) {
        ...
        // Do not store null or undefined values
...
var hashToUser = module.exports.hashToUser = function(hash) {
    ...
    var blacklist = ['id', 'principalId', 'resourceType', 'tenant', 'displayName', 'visibility', 'locale',
                     'smallPictureUri', 'mediumPictureUri', 'largePictureUri', 'email',
