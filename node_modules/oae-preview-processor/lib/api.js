 ...
 * @param  {Processor}  callback.processor  The processor who can generate previews for this piece of content. If no processor is equipped to deal with this piece of content, null will be returned.
 */
...
var _generatePreviews = function(ctx, callback) {
    ...
    getProcessor(ctx, ctx.content, function(err, processor) {
        if (err) {
            ...
            processor.generatePreviews(ctx, ctx.content, function(err, ignored, pending) {
                if (err) {
                    ctx.setStatus('error', callback);
                } else if (ignored) {
                    ctx.setStatus('ignored', callback);
                } else if (pending) {
                    ctx.setStatus('pending', callback);
                } else {
