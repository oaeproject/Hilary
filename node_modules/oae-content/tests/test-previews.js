...
describe('File previews', function() {
     ...
     * @param {Object}      callback.revisionId     The revisionId of the created content item.
     */
    var createPreviews = function(callback) {
        TestsUtil.generateTestUsers(globalAdminOnTenantRestContext, 2, function(err, users) {
            ...
            RestAPI.Content.createFile(contexts['nicolaas'].restContext, 'Test Content 2', 'Test content description 2', 'private', getFileStream, [], [], function(err, contentObj) {
                ...
                RestAPI.Content.getRevisions(contexts['nicolaas'].restContext, contentObj.id, null, 1, function(err, revisions) {
                    assert.ok(!err);
                    var revisionId = revisions[0].revisionId;

                    // Add some preview items.
                    RestAPI.Content.setPreviewItems(globalAdminOnTenantRestContext, contentObj.id, revisionId, 'done', suitable_files, suitable_sizes, {}, {}, function(err) {
                        assert.ok(!err);

                        // Get a list of preview items.
                        RestAPI.Content.getPreviewItems(contexts['nicolaas'].restContext, contentObj.id, function(err, previews) {
                            assert.ok(!err);
                            ...
                            RestAPI.Content.getContent(contexts['nicolaas'].restContext, contentObj.id, function(err, updatedContentObj) {
                                ...
                                callback(contexts, updatedContentObj, previews, revisionId);
                            });
                        });
                    });
                });
    ...
    it('verify uploading a preview', function(callback) {
        createPreviews(function(contexts, contentObj, previews, revisionId) {
            // Only global admins should be allowed to create previews.
            RestAPI.Content.setPreviewItems(anonymousRestContext, contentObj.id, revisionId, 'done', suitable_files, suitable_sizes, {}, {}, function(err) {
                assert.ok(err);
                assert.equal(err.code, 401);

                // Download one.
                RestAPI.Content.downloadPreviewItem(contexts['nicolaas'].restContext, contentObj.id, previews.files[0].filename, previews.signature, function(err, body, response) {
                    assert.ok(!err);
                    ...
                    RestAPI.Content.downloadPreviewItem(contexts['simon'].restContext, contentObj.id, previews.files[0].filename, previews.signature, function(err, body, response) {
                        assert.equal(err.code, 401);
                        assert.ok(!body);
                        callback();
                    });
    ...
    it('verify download non-existing previews is handled correctly', function(callback) {
         TestsUtil.generateTestUsers(globalAdminOnTenantRestContext, 1, function(err, users) {
            ...
            RestAPI.Content.createFile(simon.restContext, 'Test Content 2', 'Test content description 2', 'private', getFileStream, [], [], function(err, contentObj) {
                ...
                // Get a list of preview items.
                RestAPI.Content.getPreviewItems(simon.restContext, contentObj.id, function(err, previews) {
                    assert.ok(!err);
                    assert.equal(previews.files.length, 0);

                    // Downloading a preview item that doesn't exist, should result in a 404.
                    RestAPI.Content.downloadPreviewItem(simon.restContext, contentObj.id, 'does-not-exist.png', previews.signature, function(err, body, response) {
    ...
    it('verify setting preview status removes older preview items', function(callback) {
         createPreviews(function(contexts, contentObj, previews, revisionId) {
            RestAPI.Content.setPreviewItems(globalAdminOnTenantRestContext, contentObj.id, revisionId, 'error', {}, {}, {}, {}, function(err) {
                assert.ok(!err);
    ...
    it('verify uploading new preview items removes older preview items and the thumbnailUri', function(callback) {
         createPreviews(function(contexts, contentObj, previews, revisionId) {
            ...
            RestAPI.Content.setPreviewItems(globalAdminOnTenantRestContext, contentObj.id, revisionId, 'done', files, sizes, {}, {}, function(err) {
                assert.ok(!err);
    ...
    it('verify preview download parameter validation', function(callback) {
        createPreviews(function(contexts, contentObj, previews) {
            ...
            RestAPI.Content.downloadPreviewItem(contexts['nicolaas'].restContext, contentObj.id, previews.files[0].filename, previews.signature, function(err, body, response) {
                ...
                // Missing parameters.
                RestAPI.Content.downloadPreviewItem(contexts['nicolaas'].restContext, contentObj.id, previews.files[0].filename, { 'signature': previews.signature.signature, 'expires': previews.signature.expires, 'lastModified': '' }, function(err, body, response) {
                    assert.ok(err.code, 400);
                    assert.ok(!body);

                    RestAPI.Content.downloadPreviewItem(contexts['nicolaas'].restContext, contentObj.id, previews.files[0].filename, { 'signature': previews.signature.signature, 'expires': '', 'lastModified': previews.signature.lastModified }, function(err, body, response) {
                        assert.ok(err.code, 400);
                        assert.ok(!body);
                        ...
                        RestAPI.Content.downloadPreviewItem(contexts['nicolaas'].restContext, contentObj.id, previews.files[0].filename, { 'signature': previews.signature.signature, 'expires': '', 'lastModified': previews.signature.lastModified }, function(err, body, response) {
                            ...
                            // Bad signature.
                            RestAPI.Content.downloadPreviewItem(contexts['nicolaas'].restContext, contentObj.id, previews.files[0].filename, { 'signature': 'wrong', 'expires': previews.signature.expires, 'lastModified': previews.signature.lastModified }, function(err, body, response) {
