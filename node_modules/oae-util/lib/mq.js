...
let startupPurgeStatus = {};

let numMessagesInProcessing = 0;
let messagesInProcessing = {};

let MAX_NUM_MESSAGES_IN_PROCESSING = 1000;
let NUM_MESSAGES_TO_DUMP = 10;
 ...
 */
var MQ = (module.exports = new EmitterAPI.EventEmitter());
 ...
 */
let rejectMessage = (module.exports.rejectMessage = function(
  message,
  requeue,
  callback,
) {
...
var init = (module.exports.init = function(mqConfig, callback) {
  ...
  if (!connection) {
    ...
    let arrayOfHostsToConnectTo = _.map(
      [mqConfig.connection.host],
      eachHost => {
    ...
    log().warn(
      'Attempted to initialize an existing RabbitMQ connector. Ignoring',
    );
    return _createRedeliveryQueue(callback);
  }
});
 ...
 * @param  {Function}   callback            Standard callback function
 */
var declareExchange = (module.exports.declareExchange = function(
  exchangeName,
  exchangeOptions,
  callback,
) {
  if (!exchangeName) {
    log().error({
      ...
      err: new Error(
        'Tried to declare an exchange without providing an exchange name',
      ),
    });
    return callback({
      code: 400,
    ...
    log().error({
      exchangeName: exchangeName,
      err: new Error('Tried to declare an exchange twice'),
    });
  ...
  channel
    ...
    .catch(e => {
      log().error({
        exchangeName: exchangeName,
        err: new Error('Unable to declare an exchange'),
      });
    });
});
 ...
 * @param  {Function}   callback            Standard callback function
 */
var declareQueue = (module.exports.declareQueue = function(
  queueName,
  queueOptions,
  callback,
) {
  if (!queueName) {
    ...
    return callback({
      code: 400,
      msg: 'Tried to declare a queue without providing a name',
    });
  } else if (queues[queueName]) {
    log().error({
      queueName: queueName,
      queueOptions: queueOptions,
      err: new Error('Tried to declare a queue twice'),
    });
  ...
  channel
    ...
    .then(queue => {
      log().info(
        { queueName: queueName },
        'Created/Retrieved a RabbitMQ queue',
      );
    ...
    .catch(e => {
      log().error({
        queueName: queueName,
        err: new Error('Unable to declare a queue'),
      });
    });
});
 ...
 * @param  {Object}     callback.err    An error that occurred, if any
 */
var bindQueueToExchange = (module.exports.bindQueueToExchange = function(
  queueName,
  exchangeName,
  routingKey,
  callback,
 ...
 * @param  {Object}     callback.err    An error that occurred, if any
 */
var unbindQueueFromExchange = (module.exports.unbindQueueFromExchange = function(
  queueName,
  exchangeName,
  routingKey,
  callback,
  ...
  if (!queues[queueName]) {
    ...
    log().error({
      ...
      routingKey: routingKey,
      err: new Error(
        'Tried to unbind a queue from an exchange without providing a routingKey',
      ),
    });
  ...
  channel.unbindQueue(queueName, exchangeName, routingKey, {}).then(() => {
    log().trace(
      ...
      'Unbound a queue from an exchange',
    );
    callback();
  });
});
 ...
 * @param  {Object}     callback.err        An error that occurred, if any
 */
var subscribeQueue = (module.exports.subscribeQueue = function(
  queueName,
  subscribeOptions,
  listener,
  callback,
  ...
  if (!queues[queueName]) {
    ...
    return callback({
      code: 400,
      msg: 'Tried to subscribe to an unknown queue',
    });
  }
  ...
  channel
    .consume(
      ...
      function(msg) {
        ...
        var deliveryKey = util.format(
          '%s:%s',
          deliveryInfo.queue,
          deliveryInfo.deliveryTag,
        );
 ...
 * @param  {Object}    callback.err    An error that occurred, if any
 */
var unsubscribeQueue = (module.exports.unsubscribeQueue = function(
  queueName,
  callback,
) {
  ...
  if (!queue || !queue.queue) {
    log().warn({
      queueName: queueName,
      err: new Error(
        'Attempted to unbind listener from non-existant job queue. Ignoring',
      ),
    });
    return callback();
  }
  ...
  channel
    ...
    .then(ok => {
      ...
      if (!ok) {
        log().error(
          { queueName: queueName },
          'An unknown error occurred unsubscribing a queue',
        );
        return callback({
          code: 500,
          msg: 'An unknown error occurred unsubscribing a queue',
        });
      }
    ...
    .catch(e => {
      log().error({
        queueName: queueName,
        err: new Error('Unable to unsubscribe the queue'),
      });
    });
});
 ...
 */
var submit = (module.exports.submit = function(
  exchangeName,
  routingKey,
  data,
  options,
  ...
  if (!exchanges[exchangeName]) {
    ...
    return callback({
      code: 400,
      msg: 'Tried to submit a message to an unknown exchange',
    });
  } else if (!routingKey) {
    log().error({
      exchangeName: exchangeName,
      routingKey: routingKey,
      err: new Error(
        'Tried to submit a message without specifying a routingKey',
      ),
    });
    return callback({
      code: 400,
...
var purge = (module.exports.purge = function(queueName, callback) {
  ...
  if (!queues[queueName]) {
    log().error({
      queueName: queueName,
      err: new Error('Tried purging an unknown queue'),
    });
  ...
  channel
    ...
    .then(data => {
      if (!data) {
        ...
        return callback({
          code: 500,
          msg: 'Error purging queue: ' + queueName,
        });
      }
    ...
    .catch(e => {
      log().error({
        queueName: queueName,
        err: new Error('Unable to purge queue'),
      });
    });
});
...
var _createRedeliveryQueue = function(callback) {
  ...
  declareExchange(
    ...
    function(err) {
      ...
      declareQueue(
        MqConstants.REDELIVER_QUEUE_NAME,
        MqConstants.REDELIVER_QUEUE_OPTIONS,
        function(err) {
          if (err) {
            return callback(err);
          }
...
var _incrementProcessingTask = function(deliveryKey, data, deliveryInfo) {
  ...
  messagesInProcessing[deliveryKey] = {
    data: data,
    deliveryInfo: deliveryInfo,
  };
