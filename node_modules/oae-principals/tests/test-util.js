 ...
 */
var createUsers = function(test, callback) {
 ...
 */
var createGroups = function(test, userids, callback) {
    var r = Math.round(Math.random() * 1000000);
    var groupids = [];
 ...
 * @param  {Test}       test        Standard nodeunit test object
 */
module.exports.testIdentifiers = function(test) {
    var id = roleUtils.toUuid("g", "cam", "oae-team");
    test.ok(PrincipalsUtil.isGroup(id));

    id = roleUtils.toUuid("u", "cam", "simong");
    test.ok(PrincipalsUtil.isUser(id));
 ...
 * @param  {Test}       test        Standard nodeunit test object
 */
module.exports.getPrincipal = function(test) {
    test.expect(18);

    createUsers(test, function(userids) {
        createGroups(test, userids, function(groupids) {
            // Get an existing user
            PrincipalsUtil.getPrincipal(userids[0], function(err, user) {
                test.ok(!err);
                test.ok(user);
                test.equal(user.id, userids[0]);
                test.equal(user.firstName, 'Nicolaas');
                // Get a non-existing user
                PrincipalsUtil.getPrincipal('non-existing-user', function(err, user) {
                    test.ok(err);
                    test.ok(!user);
                    // Get an existing group
                    PrincipalsUtil.getPrincipal(groupids[0], function(err, group) {
                        test.ok(!err);
                        test.ok(group);
                        test.equal(group.principal_id, groupids[0]);
                        // Get a non-existing group
                        PrincipalsUtil.getPrincipal('non-existing-group', function(err, group) {
                            test.ok(err);
                            test.ok(!group);
                            test.done();
                        });
 ...
 * @param  {Test}       test        Standard nodeunit test object
 */
module.exports.getPrincipals = function(test) {
    test.expect(22);

    createUsers(test, function(userids) {
        createGroups(test, userids, function(groupids) {
            // Get existing users
            PrincipalsUtil.getPrincipals([userids[0], userids[1], userids[2]], function(err, users) {
                test.ok(!err);
                test.ok(users);
                test.equal(_.keys(users).length, 3);
                // Get existing groups
                PrincipalsUtil.getPrincipals([groupids[0], groupids[1]], function(err, groups) {
                    test.ok(!err);
                    test.ok(groups);
                    test.equal(_.keys(groups).length, 2);
                    // Get existing users/groups
                    PrincipalsUtil.getPrincipals([groupids[0], userids[0], userids[1], groupids[1], userids[3]], function(err, principals) {
                        test.ok(!err);
                        test.ok(principals);
                        test.equal(_.keys(principals).length, 5);
                        // Get existing users, of which some don't exist
                        PrincipalsUtil.getPrincipals([userids[0], 'non-existing-user', userids[2]], function(err, users) {
                            test.ok(err);
                            test.ok(!users);
                            // Get existing groups, of which some don't exist
                            PrincipalsUtil.getPrincipals([groupids[0], 'non-existing-group'], function(err, groups) {
                                test.ok(err);
                                test.ok(!users);
                                // Get existing users/groups, of which some don't exist
                                PrincipalsUtil.getPrincipals([groupids[0], userids[0], 'non-existing-user', userids[1], 'non-existing-group'], function(err, principals) {
                                    test.ok(err);
                                    test.ok(!principals);
                                    test.done();
                                });
