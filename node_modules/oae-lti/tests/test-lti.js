    ...
    describe('Create LTI tool', function() {
        ...
        it('verify that LTI tool creation succeeds given a valid request', function(callback) {
            RestAPI.Group.createGroup(johnRestContext, 'This is a group', null, 'public', 'yes', [], [], function(err, group) {
                ...
                var secret = 'secret';
                var key = '12345';
                RestAPI.LtiTool.createLtiTool(johnRestContext, group.id, launchUrl, secret, key, 'LTI tool title', 'LTI tool description', function(err, ltiTool) {
                    assert.ok(!err);
        ...
        it('verify that missing description is accepted', function(callback) {
            RestAPI.Group.createGroup(johnRestContext, 'This is a group', null, 'public', 'yes', [], [], function(err, group) {
                ...
                var secret = 'secret';
                var key = '12345';
                RestAPI.LtiTool.createLtiTool(johnRestContext, group.id, launchUrl, secret, key, 'LTI tool title', null, function(err, toolObject) {
                    assert.ok(!err);
                    assert.equal(toolObject.description, '');

                    // Verify that an empty description is acceptable as well
                    RestAPI.LtiTool.createLtiTool(johnRestContext, group.id, launchUrl, secret, key, 'LTI tool title', '', function(err, toolObject) {
                        assert.ok(!err);
                        assert.equal(toolObject.description, '');
                        return callback();
                    });
        ...
        it('verify that missing launchUrl is not accepted', function(callback) {
            RestAPI.Group.createGroup(johnRestContext, 'This is a group', null, 'public', 'yes', [], [], function(err, group) {
                ...
                var secret = 'secret';
                var key = '12345';
                RestAPI.LtiTool.createLtiTool(johnRestContext, group.id, '', secret, key, 'LTI tool title', null, function(err, toolObject) {
                    assert.ok(err);
        ...
        it('verify that missing OAUTH secret is not accepted', function(callback) {
            RestAPI.Group.createGroup(johnRestContext, 'This is a group', null, 'public', 'yes', [], [], function(err, group) {
                ...
                var launchUrl = 'http://lti.launch.url';
                var key = '12345';
                RestAPI.LtiTool.createLtiTool(johnRestContext, group.id, launchUrl, null, key, 'LTI tool title', null, function(err, toolObject) {
                    assert.ok(err);
                    assert.equal(err.code, 400);
        ...
        it('verify that missing OAUTH consumer key is not accepted', function(callback) {
            RestAPI.Group.createGroup(johnRestContext, 'This is a group', null, 'public', 'yes', [], [], function(err, group) {
                ...
                var launchUrl = 'http://lti.launch.url';
                var secret = 'secret';
                RestAPI.LtiTool.createLtiTool(johnRestContext, group.id, launchUrl, secret, null, 'LTI tool title', null, function(err, toolObject) {
                    assert.ok(err);
                    assert.equal(err.code, 400);
        ...
        it('verify that a LTI tool can not be created in a deleted group', function(callback) {
            RestAPI.Group.createGroup(johnRestContext, 'This is a group', null, 'public', 'yes', [], [], function(err, group) {
                assert.ok(!err);
                RestAPI.Group.deleteGroup(johnRestContext, group.id, function(err) {
                    assert.ok(!err);
                    var launchUrl = 'http://lti.launch.url';
                    var secret = 'secret';
                    var key = '12345';
                    RestAPI.LtiTool.createLtiTool(johnRestContext, group.id, launchUrl, secret, key, 'LTI tool title', 'LTI tool description', function(err, toolObject) {
                        assert.ok(err);
    ...
    describe('Get LTI tool', function() {
        ...
        it('verify retrieved LTI tool launch data', function(callback) {
            RestAPI.Group.createGroup(johnRestContext, 'This is a group', null, 'public', 'yes', [], [], function(err, group) {
                ...
                RestAPI.LtiTool.createLtiTool(johnRestContext, group.id, launchUrl, secret, key, title, description, function(err, tool) {
                    assert.ok(!err);
                    // Get the LTI tool and verify its model
                    RestAPI.LtiTool.getLtiTool(johnRestContext, group.id, tool.toolId, function(err, data) {
                        assert.ok(!err);
                        ...
                        assert.equal(ltiLaunchData['resource_link_id'], tool.toolId);
                        assert.equal(ltiLaunchData['resource_link_title'], title);
                        assert.equal(ltiLaunchData['resource_link_description'], description);
                        assert.equal(ltiLaunchData['user_id'], group.id + ':' + johnRestContext.user.id);
         ...
         */
        it('verify non-managers are given role \'Learner\'', function(callback) {
            RestAPI.Group.createGroup(johnRestContext, 'This is a group', null, 'public', 'yes', [], [], function(err, group) {
                RestAPI.LtiTool.createLtiTool(johnRestContext, group.id, 'http://lti.launch.url', 'secret', '12345', 'LTI tool title', 'LTI tool description', function(err, tool) {
                    assert.ok(!err);
                    // Get the LTI tool and verify its model
                    RestAPI.LtiTool.getLtiTool(anonymousRestContext, group.id, tool.toolId, function(err, data) {
                        assert.ok(!err);
                        var ltiLaunchData = data.launchParams;
        ...
        it('verify non existing LTI tool can not be retrieved', function(callback) {
            ...
            RestAPI.LtiTool.getLtiTool(johnRestContext, 'g:camtest:not-exists', '12345', function(err, ltiTool) {
                assert.ok(err);
                assert.equal(err.code, 404);
                assert.ok(!ltiTool);
    ...
    describe('Get LTI tools for a group', function() {
        ...
        it('verify retrieving LTI tools for a group', function(callback) {
            RestAPI.Group.createGroup(johnRestContext, 'This is a group', null, 'public', 'yes', [], [], function(err, group) {
                ...
                var title = 'LTI tool title';
                var description = 'LTI tool description';
                RestAPI.LtiTool.createLtiTool(johnRestContext, group.id, 'http://lti.launch1.url', secret, '12345', title, description, function(err, tool1) {
                    assert.ok(!err);
                    RestAPI.LtiTool.createLtiTool(johnRestContext, group.id, 'http://lti.launch2.url', secret, '12346', title, description, function(err, tool2) {
                        assert.ok(!err);
                        // Get the LTI tools for the group
                        RestAPI.LtiTool.getLtiTools(johnRestContext, group.id, function(err, ltiTools) {
                            assert.ok(!err);
                            ...
                            var toolIds = _.pluck(ltiTools.results, 'toolId');
                            assert.ok(_.contains(toolIds, tool1.toolId));
                            assert.ok(_.contains(toolIds, tool2.toolId));
                            return callback();
                        });
                    });
                });
        ...
        it('verify retrieving LTI tools are not retrieved for erroneous groups', function(callback) {
            ...
            RestAPI.LtiTool.getLtiTools(johnRestContext, notExists, function(err, ltiTools) {
                assert.ok(err);
                assert.equal(err.code, 404);
    ...
    describe('Delete LTI tool', function() {
        ...
        it('verify that LTI tools can be deleted', function(callback) {
            RestAPI.Group.createGroup(johnRestContext, 'This is a group', null, 'public', 'yes', [], [], function(err, group) {
                ...
                var secret = 'secret';
                var key = '12345';
                RestAPI.LtiTool.createLtiTool(johnRestContext, group.id, launchUrl, secret, key, 'LTI tool title', 'LTI tool description', function(err, tool) {
                    assert.ok(!err);
                    var toolId = tool.toolId;
                    // Assert the tool exists and can be fetched
                    RestAPI.LtiTool.getLtiTool(johnRestContext, group.id, toolId, function(err, data) {
                        assert.ok(!err);
                        var ltiLaunchData = data.launchParams;
                        assert.equal(ltiLaunchData['oauth_consumer_key'], key);
                        assert.equal(ltiLaunchData['resource_link_id'], toolId);
                        RestAPI.LtiTool.deleteLtiTool(johnRestContext, group.id, toolId, function(err) {
                            assert.ok(!err);
                            // Assert the tool can no longer be fetched
                            RestAPI.LtiTool.getLtiTool(johnRestContext, group.id, toolId, function(err, data) {
                                assert.ok(err) ;
                                assert.equal(err.code, 404);
                                assert.equal(err.msg, util.format('Could not find item %s for group %s', toolId, group.id));
                                return callback();
                            });
                        });
                    });
        ...
        it('verify that a non-manager can not delete LTI tool', function(callback) {
            RestAPI.Group.createGroup(johnRestContext, 'This is a group', null, 'public', 'yes', [], [], function(err, group) {
                ...
                RestAPI.LtiTool.createLtiTool(johnRestContext, group.id, launchUrl, secret, key, 'LTI tool title', 'LTI tool description', function(err, tool) {
                    assert.ok(!err);
                    RestAPI.LtiTool.deleteLtiTool(anonymousRestContext, group.id, tool.toolId, function(err) {
                        assert.ok(err) ;
                        assert.equal(err.code, 401);
                        assert.equal(err.msg, 'The current user does not have access to manage this resource');
                        return callback();
        ...
        it('verify that a LTI tool can not be deleted in a deleted group', function(callback) {
            RestAPI.Group.createGroup(johnRestContext, 'This is a group', null, 'public', 'yes', [], [], function(err, group) {
                ...
                RestAPI.LtiTool.createLtiTool(johnRestContext, group.id, 'http://lti.launch.url', 'secret', '12345', 'LTI tool title', 'LTI tool description', function(err, tool) {
                    ...
                    RestAPI.Group.deleteGroup(johnRestContext, group.id, function(err) {
                        assert.ok(!err);
                        RestAPI.LtiTool.deleteLtiTool(johnRestContext, group.id, tool.toolId, function(err) {
                            assert.ok(err) ;
                            assert.equal(err.code, 404);
