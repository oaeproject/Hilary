  ...
  describe('Tenant separation', function() {
    ...
    var verifyShare = function(
      ...
      RestAPI.User.getMe(targetRestContext, function(err, me) {
        ...
        RestAPI.Content.shareContent(actorUserRestContext, objectContent.id, [targetId], function(
          err
        ) {
          ...
          var targetId = AuthzUtil.isEmail(targetPrincipalId) ? me.id : targetPrincipalId;

          // Sanity check that the item appears in the library, if applicable
          RestAPI.Content.getLibrary(targetRestContext, targetId, null, 100, function(err, data) {
            assert.ok(!err);
                                                                ...
                                                                verifyShare(
                                                                  ...
                                                                  function() {
                                                                    ...
                                                                    verifyShare(
                                                                      publicTenantB.adminRestContext,
                                                                      publicTenantB.publicContent,
                                                                      privateTenantA.publicUser.user
                                                                        .id,
     ...
     * address
     */
    it('verify content sharing permutations from object to target by email address (users)', function(callback) {
      ContentTestUtil.setupMultiTenantPrivacyEntities(function(
        publicTenantA,
        publicTenantB,
        privateTenantA,
     ...
     */
    it('verify content sharing permutations from actor to target (users) with email only', function(callback) {
      ContentTestUtil.setupMultiTenantPrivacyEntities(function(
        publicTenantA,
        publicTenantB,
        privateTenantA
          ...
          RestAPI.Content.createLink(
            ...
            function(err, publicContentA) {
              ...
              RestAPI.Content.createLink(
                ...
                [],
                [],
                function(err, loggedInContentA) {
                  assert.ok(!err);

                  // Verify user A2 can see both public and logged in content items
                  RestAPI.Content.getLibrary(userA2.restContext, userA.user.id, null, 10, function(
                    err,
                    libraryA
                  ) {
                    assert.ok(!err);
                    ...
                    RestAPI.Content.getLibrary(userB.restContext, userA.user.id, null, 10, function(
                      err,
                      libraryA
                    ) {
                      assert.ok(!err);
                ...
                function(err, publicContentA) {
                  ...
                  RestAPI.Content.createLink(
                    ...
                    function(err, loggedInContentA) {
                      ...
                      RestAPI.Content.getLibrary(userA.restContext, groupA.id, null, 10, function(
                        err,
                        libraryA
                      ) {
                        assert.ok(!err);
                        ...
                        // Verify user B cannot see the loggedin content item, but can see the public content item
                        RestAPI.Content.getLibrary(userB.restContext, groupA.id, null, 10, function(
                          err,
                          libraryB
                        ) {
                          assert.ok(!err);
          ...
          function(err, contentObj) {
            ...
            ConfigTestUtil.updateConfigAndWait(
              ...
              function(err) {
                ...
                RestAPI.Content.updateMembers(actor.restContext, contentObj.id, update, function(
                  err
                ) {
                  assert.ok(!err);
                  // Verify that the user is still there and is a manager
                  RestAPI.Content.getMembers(actor.restContext, contentObj.id, null, 10, function(
                    err,
                    data
                  ) {
                    assert.ok(!err);
                    var sharedMember = _.find(data.results, function(member) {
                      ...
                      function(err) {
                        ...
                        RestAPI.Content.getMembers(
                          ...
                          function(err, data) {
                            assert.ok(!err);
                            assert.ok(
                              !_.find(data.results, function(member) {
                                return member.profile.id === sharedUser.id;
                              })
                            );
                            callback();
