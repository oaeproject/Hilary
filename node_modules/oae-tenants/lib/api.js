...
var initializeTenantMiddleware = module.exports.initializeTenantMiddleware = function(config, callback) {
    ...
    var globalTenant = new Tenant(config.servers.globalAdminAlias, 'Global admin server', config.servers.globalAdminPort, 'localhost');
    ...
    // This middleware adds the tenant to each request on the tenant server.
    ...
    getAllTenants(function(err, tenants) {
        ...
        var tenantAliases = _.keys(tenants);
        for (var t = 0; t < tenantAliases.length; t++) {
            tenantsByHost[tenants[tenantAliases[t]].host] = tenants[tenantAliases[t]];
            tenantsByAlias[tenantAliases[t]] = tenants[tenantAliases[t]];
        }
        callback();
    });
};
...
var updateTenant = module.exports.updateTenant = function(ctx, alias, name, callback) {
    ...
    Cassandra.runQuery('UPDATE Tenant USING CONSISTENCY QUORUM SET name = ? WHERE alias = ?', [name, alias], function(err) {
        if (err) {
            return callback(err);
        }
...
var getRunningTenantAliases = module.exports.getRunningTenantAliases = function() {
    var runningTenants = _.values(tenantsByHost);
    var aliases = [];
    for (var i = 0; i < runningTenants.length; i++) {
        var tenant = runningTenants[i];
        if (tenant.active) {
