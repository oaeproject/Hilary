 ...
 * Register an association that presents all the managers of a content item
 */
ActivityAPI.registerActivityEntityAssociation('content', 'managers-not-editors', function(associationsCtx, entity, callback) {
    associationsCtx.get('members-by-role', function(err, membersByRole) {
        if (err) {
            return callback(err);
        }

        var managers = membersByRole[ContentConstants.roles.MANAGER];

        // Filter out those managers who are are viewing the collaborative document
        if (entity.content.resourceSubType === 'collabdoc') {
            // TODO: Replace this with a proper presence API call
            Etherpad.getOnlineAuthors(entity.content.id, entity.content.etherpadPadId, function(err, onlineAuthors) {
                if (err) {
                    return callback(err);
                }

                ContentDAO.Etherpad.getUserIds(onlineAuthors, function(err, users) {
                    if (err) {
                        return callback(err);
                    }

                    return callback(null, _.difference(managers, _.values(users)));
                });
            });
        } else {
