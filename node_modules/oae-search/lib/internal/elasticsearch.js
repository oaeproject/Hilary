 ...
 *
 * @param   {String}    typeName        The type of document ot index
 * @param   {Object[]}  docs            An array of indexable documents
 * @param   {Function}  callback        Invoked whent he process completes
 * @param   {Object}    callback.err    An error that occurred, if any
 */
var bulk = module.exports.bulk = function(typeName, docs, callback) {
    log().trace({'typeName': typeName, 'documents': docs}, 'Indexing a bulk set of %s documents.', docs.length);
    var cmds = [];
    for (var i = 0; i < docs.length; i++) {
        var doc = docs[i];

        // first push the command metadata for the doc operation
        cmds.push({
            'index': {
                '_index': index,
                '_type': typeName,
        ...
        // next entry needs to be the doc data
        cmds.push(doc);

    }

    return _exec('bulk', client.bulk(cmds, null), callback);
}
