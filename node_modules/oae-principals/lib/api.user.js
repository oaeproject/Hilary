...
module.exports.createUser = function(ctx, username, password, visibility, locale, timezone, firstName, lastName, displayName, callback) {
    ...
    var id = AuthzUtil.toUuid('u', ctx.tenant().alias, cleanUserName);
 ...
 *                                          Possible keys are visibility, firstName, lastName,
    ...
    module.exports.getUser(ctx, userId, function(err, user) {
        ...
        if (ctx.user().id === userId || ctx.user().isAdmin(principalResource.tenantId)) {
            ...
            Cassandra.runQuery(q.query, q.parameters, function(err) {
                ...
                invalidateCachedUser(userId, callback);
            });
        } else {
...
module.exports.setUserVisibility = function(ctx, userId, visibility, callback) {
    ...
    if (ctx.user().id === userId || ctx.user().isAdmin(principalResource.tenantId)) {
        module.exports.getUser(ctx, userId, function(err, user) {
            ...
            Cassandra.runQuery('UPDATE Principals USING CONSISTENCY QUORUM SET visibility = ? WHERE principalId = ?', [visibility, userId], function(err) {
                if (err) {
                    return callback(err);
                }
                invalidateCachedUser(userId, callback);
...
var _setAdmin = function(ctx, adminType, isAdmin, principalId, callback) {
    ...
    module.exports.getUser(ctx, principalId, function(err, user) {
        ...
        Cassandra.runQuery("UPDATE Principals USING CONSISTENCY QUORUM SET ?=? WHERE principalId = ?", [adminType, isAdmin, principalId], function(err) {
            ...
            invalidateCachedUser(principalId, callback);
        });
    });
};
 ...
 */
var invalidateCachedUser = function(userId, callback) {
    Redis.getClient().del('user:' + userId, callback);
};
