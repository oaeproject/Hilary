...
/**
 * Returns the processor that is best equipped to generate previews for a piece of content.
 *
 * @param  {Context}    ctx                 Standard context object containing the current user and the current tenant
 * @param  {Content}    contentObj          The content object for which we need to find a processor.
 * @param  {Function}   callback            Standard callback function
 * @param  {Object}     callback.err        An error that occurred, if any
 * @param  {Processor}  callback.processor  The processor that can generate previews for this piece of content. If no processor is equipped to deal with this piece of content, null will be returned.
 */
...
var _generatePreviews = function(ctx, callback) {
    ...
    getProcessor(ctx, ctx.content, function(err, processor) {
        if (err) {
            ...
            ctx.setStatus(ContentConstants.previews.IGNORED, callback);
        }

         processor.generatePreviews(ctx, ctx.content, function(err, status) {
            if (err) {
                return ctx.setStatus(ContentConstants.previews.ERROR, callback);
            } else if (status && _.contains(ContentConstants.previews, status)) {
                // We received a valid preview status from the processor (most likely either 'ignored' or 'pending') so lets add it to the preview context
                return ctx.setStatus(status, callback);
            }
