 ...
 * @param  {Function}   callback            Standard callback function
 * @param  {Object}     callback.err        An error that occurred, if any
 * @param  {String}     callback.zip        Zip object contain all the data
 */
var exportData = module.exports.exportData = function(ctx, userId, exportType, callback) {
    ...
    PrincipalsDAO.getPrincipal(userId, function(err, principal) {
        ...
        zip.personalData = 'Personal id : ' + principal.id
        ...
        if (!_.isEmpty(principal.picture)) {
            ...
            zip.profilePicture = {'path': path + '/' + pathLargePicture[1], 'imageName': imageName};
        }    

        _exportContentData(ctx, userId, exportType, zip, function(err, zipFile) {
            if (err) {
                return callback(err);
            }

            return callback(null, zipFile, zip);
        });
    });
};

/**
 * Get content informations and put them into the personal data object
 *
 * @param  {Context}    ctx                     Standard context object containing the current user and the current tenant
 * @param  {String}     userId                  The id of the user for which to get his personal data
 * @param  {String}     exportType              Export type can be 'personal-data', 'content' or 'shared'
 ...
 */
var _exportContentData = function(ctx, userId, exportType, personalData, callback) {

    if (exportType === PrincipalsConstants.exportType.PERSONAL_DATA) {
        // Convert an object to a zip file
        _objectToZipFile(personalData, function(zipFile) {
            return callback(null, zipFile, personalData);
        });
        ...
        ContentAPI.getContentLibraryItems(ctx, userId, null, null, function(err, contents) {
            ...
            // get uploaded files
            _getUploadedFiles(ctx, contentsSplited.file, personalData, function(err, personalData) { 
                if (err) {
                    return callback(err);
                }

                // Convert links into txt file
                _linkToTxt(ctx, contentsSplited.link, personalData, function(err, personalData) { 
                    if (err) {
                        return callback(err);
                    }

                    // Convert collabdocs into txt file
                    _collabdocToTxt(ctx, contentsSplited.collabdoc, personalData, function(err, personalData) { 
                        if (err) {
                            return callback(err);
                        }
                        ...
                        MeetingsAPI.Meetings.getMeetingsLibrary(ctx, userId, null, null, function(err, meetings) {
                            ...
                            _meetingToTxt(ctx, meetings, personalData, function(err, personalData) {
                                if (err) {
                                    return callback(err);
                                }
                                ...
                                DiscussionsAPI.Discussions.getDiscussionsLibrary(ctx, userId, null, null, function(err, discussions) {
                                    ...
                                    // Convert meetings into txt file
                                    _discussionToTxt(ctx, discussions, personalData, function(err, personalData) {
                                        if (err) {
                                            return callback(err);
                                        }
...
/**
 * Get information about uploaded files and put them into the personal data object
 *
 * @param  {Context}    ctx                     Standard context object containing the current user and the current tenant
 ...
 * @param  {Objetc}     callback.personalData   Object contain all the data
 */
var _getUploadedFiles = function(ctx, uploadedFiles, personalData, callback) {
    if (_.isEmpty(uploadedFiles)) {
        return callback(null, personalData);
    }

    // Get files path
    var filePath = ContentUtil.getStorageBackend(ctx, null).getRootDirectory();

    personalData.upload = [];

    async.mapSeries(uploadedFiles, function(uploadedFile, outerCB) {
        ...
        personalData.upload.push({'path': filePath + '/' + path + '/' + nameWithoutSpace, 'title': nameWithoutSpace});
...
/**
 * Get information about collabdocs and put them into thepersonal data object
 *
 * @param  {Context}    ctx                     Standard context object containing the current user and the current tenant
 ...
 * @param  {Objetc}     callback.personalData   Object contain all the data
 */
var _collabdocToTxt = function(ctx, collabdocs, personalData, callback) {
    if (_.isEmpty(collabdocs)) {
        return callback(null, personalData);
    }

    var txtCollabdoc = '';
    personalData.collabdoc = [];

    async.mapSeries(collabdocs, function(collabdoc, outerCB) {
        ...
        ContentAPI.getComments(ctx, collabdoc.id, null, null, function(err, messages) {
            ...
            ContentAPI.getRevision(ctx, collabdoc.id, collabdoc.latestRevisionId, function(err, revision) {
                ...
                _getMessages(txtCollabdoc, messages, function(txtCollabdocWithMessages) {
                    _clearName(collabdoc.displayName, function(err, fileName) {
                        personalData.collabdoc.push({'text': txtCollabdocWithMessages, 'title': fileName + '.txt'});
                        outerCB();
                    });
                });
            });
...
/**
 * Get information about links and put them into the personal data object
 *
 * @param  {Context}    ctx                     Standard context object containing the current user and the current tenant
 ...
 * @param  {Function}   callback                Standard callback function
 * @param  {Object}     callback.err            An error that occurred, if any
 * @param  {Obejct}     callback.personalData   Object contain all the data
 */
var _linkToTxt = function(ctx, links, personalData, callback) {
    if (_.isEmpty(links)) {
        return callback(null, personalData);
    }
    ...
    async.mapSeries(links, function(link, outerCB) {
        ContentAPI.getComments(ctx, link.id, null, null, function(err, messages) {
            ...
            _getMessages(txtLink, messages, function(txtLinkWithMessages) {
                _clearName(link.displayName, function(err, fileName) {
                    personalData.link.push({'text': txtLinkWithMessages, 'title': fileName + '.txt'});
                    outerCB();
                });
            });
        });
...
/**
 * Get information about meetings and put them into the object
 *
 * @param  {Context}    ctx                     Standard context object containing the current user and the current tenant
 ...
 * @param  {Function}   callback                Standard callback function
 * @param  {Object}     callback.err            An error that occurred, if any
 * @param  {Object}     callback.personalData   Object contain all the data
 */
var _meetingToTxt = function(ctx, meetings, personalData, callback) {
    if (_.isEmpty(meetings)) {
        return callback(null, personalData);
    }

    var txtMeeting = '';
    personalData.meeting = [];

    async.mapSeries(meetings, function(meeting, outerCB) {
        MeetingsAPI.Meetings.getMessages(ctx, meeting.id, null, null, function(err, messages) {
            ...
            _getMessages(txtMeeting, messages, function(txtMeetingWithMessages) {
                _clearName(meeting.displayName, function(err, fileName) {
                    personalData.meeting.push({'text': txtMeetingWithMessages, 'title': fileName + '.txt'});
                    outerCB();
                });
            });
        });
...
/**
 * Get information about discussions and put them into an object
 *
 * @param  {Context}    ctx                     Standard context object containing the current user and the current tenant
 ...
 * @param  {Object}     callback.personalData   Object contain all the data
 */
var _discussionToTxt = function(ctx, discussions, personalData, callback) {
    if (_.isEmpty(discussions)) {
        return callback(null, personalData);
    }

    var txtDiscussion = '';
    personalData.discussion = [];

    async.mapSeries(discussions, function(discussion, outerCB) {
        DiscussionsAPI.Discussions.getMessages(ctx, discussion.id, null, null, function(err, messages) {
            ...
            _getMessages(txtDiscussion, messages, function(txtDiscussionWithMessages) {
                _clearName(discussion.displayName, function(err, fileName) {
                    personalData.discussion.push({'text': txtDiscussionWithMessages, 'title': fileName + '.txt'});
                    outerCB();
                });
            });
        });
 ...
 */
_getMessages = function(txt, messages, callback) {
    if (_.isEmpty(messages)) {
        return callback(txt);
    }
    txt += 'Comments : '
    async.mapSeries(messages, function(message, outerCB) {
        txt += '\n\t' + message.createdBy.displayName + ' says : ' + message.body;
        outerCB();
    }, function(err) {
        txt += '\n\n';
        return callback(txt);
 ...
 */
_clearName = function(nameResource, callback) {
    if (_.isEmpty(nameResource)) {
        return callback(null, 'no_name');
    }

    var fileName = nameResource.replace('http://', '');
    fileName = fileName.split('https://').join('');
    fileName = fileName.split('http://').join('');
 ...
 */
_objectToZipFile = function(personalData, callback) {

    var zipFile = new jszip();

    const compressPersonalData = (outerCallback) => {
        if (personalData.personalData) {
            zipFile.file('personal_data.txt', personalData.personalData);
    ...
    const compressProfilePicture = (outerCallback) => {
    ...
    const compressUploadedData = (outerCallback) => {
        if (personalData.upload) {

            var uploadFolder = zipFile.folder('upload_data');

            async.mapSeries(personalData.upload, function(uploadedFile, innerCallback) {
                fs.readFile(uploadedFile.path, function (err, data) {
                    if (err) {
                        return callback(err);
                    }
                    uploadFolder.file(uploadedFile.title, data, {base64: false, binary: true});
                    innerCallback();
                });
            }, function () {
                return outerCallback();
    ...
    const compressCollabDocs = (outerCallback) => {
        if (personalData.collabdoc) {
            
            var collabdocFolder = zipFile.folder('collabdoc_data');

            async.mapSeries(personalData.collabdoc, function(collabdoc, innerCallback) {
                collabdocFolder.file(collabdoc.title, collabdoc.text);
                innerCallback();
            }, function () {
                return outerCallback();
            });
    ...
    const compressLinks = (outerCallback) => {
        if (personalData.link) {
            
            var linkFolder = zipFile.folder('link_data');

            async.mapSeries(personalData.link, function(link, innerCallback) {
                linkFolder.file(link.title, link.text);
                innerCallback();
            }, function () {
                return outerCallback();
            });
    ...
    const compressMeetings = (outerCallback) => {
        if (personalData.meeting) {
            
            var meetingFolder = zipFile.folder('meeting_data');

            async.mapSeries(personalData.meeting, function(meeting, innerCallback) {
                meetingFolder.file(meeting.title, meeting.text);
                innerCallback();
            }, function () {
                return outerCallback();
            });
    ...
    const compressDiscussions = (outerCallback) => {
        if (personalData.discussion) {
            
            var discussionFolder = zipFile.folder('discussion_data');

            async.mapSeries(personalData.discussion, function(discussion, innerCallback) {
                discussionFolder.file(discussion.title, discussion.text);
                innerCallback();
            }, function () {
                return outerCallback();
            });
