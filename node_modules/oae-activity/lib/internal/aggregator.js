...
var resetAggregationForActivityStreams = (module.exports.resetAggregationForActivityStreams = function(
  ...
  ActivityDAO.resetAggregationForActivityStreams(activityStreamIds, function(
    err,
  ) {
    if (err) {
      log().error(
    ...
    ActivityEmitter.emit(
      ActivityConstants.events.RESET_AGGREGATION,
      activityStreamIds,
    );
    return callback();
  });
...
var _collectBucket = function(bucketNumber, callback) {
  ...
  log().trace(
    'Collecting batch of %s entries from bucket number %s.',
    limit,
    bucketNumber,
  );

  // Step #1: Get the next batch of queued activities to process
  ActivityDAO.getQueuedActivities(bucketNumber, limit, function(
    err,
    routedActivities,
    numToDelete,
  ) {
    ...
    ActivityDAO.deleteQueuedActivities(bucketNumber, numToDelete, function(
      err,
    ) {
      if (err) {
        return callback(err);
      ...
      ActivityDAO.getAggregateStatus(allAggregateKeys, function(
        err,
        statusByAggregateKey,
      ) {
        if (err) {
          return callback(err);
        }
        ...
        _.each(allAggregateKeys, function(aggregateKey) {
          ...
          if (
            status &&
            _isExpired(status, allAggregates[aggregateKey].published)
          ) {
        ...
        ActivityDAO.deleteAggregateData(_.keys(expiredAggregates), function(
          err,
        ) {
          if (err) {
            return callback(err);
          ...
          ActivityDAO.getAggregatedEntities(_.keys(activeAggregates), function(
            err,
            fetchedEntities,
          ) {
            if (err) {
            ...
            ActivityDAO.saveAggregatedEntities(allAggregates, function(err) {
              ...
              _.each(aggregatesToDeliver, function(
                aggregateToDeliver,
                aggregateKey,
              ) {
                var aggregate = allAggregates[aggregateKey];
                ...
                if (visitedActivities[flattenedActivity]) {
                  ...
                  activityId = ActivityDAO.createActivityId(
                    aggregate.published,
                  );
                  aggregate[
                    ActivityConstants.properties.OAE_ACTIVITY_ID
              ...
              ActivityDAO.deliverActivities(activityStreamUpdates, function(
                err,
              ) {
                if (err) {
                  return callback(err);
                ...
                _.each(activityStreamUpdates, function(
                  routedActivities,
                  route,
                ) {
                ...
                _.each(activitiesToDelete, function(activityToDelete, route) {
                  ...
                  ActivityDAO.indexAggregateData(
                    ...
                    function(err) {
                      ...
                      _.each(routedActivities, function(routedActivity) {
                        var activityStream = ActivityUtil.parseActivityStreamId(
                          routedActivity.route,
                        );
                        var resourceId = activityStream.resourceId;
                        var streamType = activityStream.streamType;
                        deliveredActivityInfos[resourceId] =
                          deliveredActivityInfos[resourceId] || {};
                        deliveredActivityInfos[resourceId][
                          streamType
                        ] = deliveredActivityInfos[resourceId][streamType] || {
                          numNewActivities:
                            numNewActivitiesByRoute[routedActivity.route] || 0,
                          activities: [],
                        };

                        deliveredActivityInfos[resourceId][
                          streamType
                        ].activities.push(routedActivity.activity);
                      });
 ...
 */
var createAggregates = (module.exports.createAggregates = function(
  routedActivities,
) {
  var aggregates = {};
  _.each(routedActivities, function(routedActivity) {
    ...
    var groupBy = activityTypes[activityType]
      ? activityTypes[activityType].groupBy
      : [];
    ...
    _.each(groupBy, function(pivot) {
      var pivotActorKey = _createPivotKey(activity.actor, pivot.actor);
      var pivotObjectKey = _createPivotKey(activity.object, pivot.object);
      var pivotTargetKey = _createPivotKey(activity.target, pivot.target);
      ...
      var hasNew = _contributesNewEntity(
        aggregate,
        actorKey,
        objectKey,
        targetKey,
 ...
 * @api private
 */
var _createPivotKey = function(entity, pivotSpec) {
  var key = '';
  if (pivotSpec) {
    if (pivotSpec === true) {
 ...
 * @api private
 */
var _contributesNewEntity = function(
  aggregate,
  actorKey,
  objectKey,
  targetKey,
  ...
  return (
    ...
    !aggregate.objects[objectKey] ||
    !aggregate.targets[targetKey]
  );
};
 ...
 */
var createActivityEntity = (module.exports.createActivityEntity = function(
  entities,
) {
  if (!entities) {
    return undefined;
  ...
  return new ActivityEntity('collection', undefined, undefined, { ext: ext });
});
...
var _isExpired = function(aggregateStatus, published) {
  var aggregateIdleExpiryInMs =
    ActivitySystemConfig.getConfig().aggregateIdleExpiry * 1000;
  var aggregateMaxExpiryInMs =
    ActivitySystemConfig.getConfig().aggregateMaxExpiry * 1000;

  var lastUpdateWasCollected =
    aggregateStatus.lastCollected &&
    aggregateStatus.lastCollected > aggregateStatus.lastUpdated;
  var lastUpdateIsIdleExpired =
    published - aggregateStatus.lastUpdated > aggregateIdleExpiryInMs;
  var createdMaxIsExpired =
    published - aggregateStatus.created > aggregateMaxExpiryInMs;
  return (
    lastUpdateWasCollected && (lastUpdateIsIdleExpired || createdMaxIsExpired)
  );
};
