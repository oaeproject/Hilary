 ...
 */
var registerFullUserProfileDecorator = (module.exports.registerFullUserProfileDecorator = function(
  namespace,
  decorator,
) {
 ...
 */
var createUser = (module.exports.createUser = function(
  ctx,
  tenantAlias,
  displayName,
  opts,
  ...
  if (!opts.locale) {
    ...
    if (!opts.locale) {
      opts.locale = PrincipalsConfig.getValue(
        tenantAlias,
        'user',
        'defaultLanguage',
      );
  ...
  opts.visibility =
    opts.visibility ||
    PrincipalsConfig.getValue(tenantAlias, 'user', 'visibility');
  opts.publicAlias = opts.publicAlias || displayName;
  opts.acceptedTC = opts.acceptedTC || false;
  opts.emailPreference =
    opts.emailPreference ||
    PrincipalsConfig.getValue(tenantAlias, 'user', 'emailPreference');
  ...
  opts.emailVerified =
    opts.emailVerified ||
    (ctx.user() && ctx.user().isAdmin(tenantAlias)) ||
    false;
  ...
  user.needsToAcceptTC = PrincipalsConfig.getValue(
    tenantAlias,
    'termsAndConditions',
    'enabled',
  );
...
var importUsers = (module.exports.importUsers = function(
  ...
  validator
    ...
    .notNull();
  validator
    .check(userCSV, { code: 400, msg: 'A CSV file must be provided' })
    .notNull();
  if (userCSV) {
    validator
      .check(userCSV.size, { code: 400, msg: 'Missing size on the CSV file' })
      .notEmpty();
    validator
      .check(userCSV.size, { code: 400, msg: 'Invalid size on the CSV file' })
      .isInt();
    validator
      .check(userCSV.size, { code: 400, msg: 'Invalid size on the CSV file' })
      .min(0);
    validator
      .check(userCSV.name, { code: 400, msg: 'Missing name on the CSV file' })
      .notEmpty();
  }
  validator
    ...
    .on('finish', function() {
      ...
      _cleanUpCSVFile(userCSV, function() {
        ...
        var processUser = function(data) {
          ...
          if (
            authenticationStrategy === AuthenticationConstants.providers.LOCAL
          ) {
          ...
          require('oae-authentication').getOrCreateUser(
            ...
            displayName,
            opts,
            function(err, user, loginId, created) {
              if (err) {
    ...
    fs.exists(userCSV.path, function(exists) {
      if (exists) {
        fs.unlink(userCSV.path, function(err) {
          if (err) {
            log().warn(
              { err: err, file: userCSV },
              'Could not remove the user import CSV file',
            );
          }
          callback();
        });
 ...
 */
var updateUser = (module.exports.updateUser = function(
  ctx,
  userId,
  profileFields,
  callback,
  ...
  var validator = new Validator();
  validator
    .check(userId, { code: 400, msg: 'A valid user id must be provided' })
    .notEmpty();
  validator
    .check(userId, { code: 400, msg: 'A valid user id must be provided' })
    .isUserId();
  ...
  validator
    .check(invalidKeys.length, {
      code: 400,
      msg: 'Restricted property was attempted to be set.',
    })
    .max(0);
  ...
  if (
    ctx.user().id !== userId &&
    !ctx.user().isAdmin(principalResource.tenantAlias)
  ) {
    return callback({
      code: 401,
  ...
  PrincipalsDAO.getPrincipal(userId, function(err, oldUser) {
    if (err) {
      ...
      return callback({
        code: 404,
        msg: util.format("Couldn't find principal: ", oldUser.id),
      });
    }
    ...
    var isEmailChange =
      !_.isUndefined(profileFields.email) &&
      profileFields.email !== oldUser.email;
    if (isEmailChange) {
  ...
  canDeleteUser(ctx, userId, function(err, canDelete, user) {
    ...
    PrincipalsDAO.deletePrincipal(userId, function(err) {
      ...
      return PrincipalsEmitter.emit(
        PrincipalsConstants.events.DELETED_USER,
        ctx,
        user,
        callback,
      );
 ...
 */
var deleteOrRestoreUsersByTenancy = (module.exports.deleteOrRestoreUsersByTenancy = function(
  ctx,
  tenantAlias,
  disableUsers,
  callback,
 ...
 * @api private
 */
var getAllUsersForTenant = (module.exports.getAllUsersForTenant = function(
  ctx,
  tenantAlias,
  callback,
) {
  ...
  if (!ctx.user() || !ctx.user().isAdmin(tenantAlias)) {
    log().warn(
      'A non-admin user tried to fetch all users for tenant %s',
      tenantAlias,
    );
    return callback({
      code: 401,
      msg: 'Only authorized admins can fetch all users for a tenant',
    });
  }
 ...
 * @param  {Object}     callback.err    An error that occured, if any
 */
var restoreUser = (module.exports.restoreUser = function(
  ctx,
  userId,
  callback,
) {
  ...
  canRestoreUser(ctx, userId, function(err, canRestore, user) {
    if (err) {
      ...
      return callback({
        code: 401,
        msg: 'You are not authorized to restore this user',
      });
    }
    ...
    PrincipalsDAO.restorePrincipal(userId, function(err) {
      ...
      return PrincipalsEmitter.emit(
        PrincipalsConstants.events.RESTORED_USER,
        ctx,
        user,
        callback,
      );
 ...
 */
var canDeleteUser = (module.exports.canDeleteUser = function(
  ctx,
  userId,
  callback,
) {
  ...
  PrincipalsDAO.getPrincipal(userId, function(err, user) {
    if (err) {
      ...
      ctx.user().id !== userId &&
      !ctx.user().isAdmin(user.tenant.alias)
    ) {
 ...
 * @param  {User}       callback.user       The user that was fetched to perform the checks. Will not be specified if the authorization fails
 */
var canRestoreUser = (module.exports.canRestoreUser = function(
  ctx,
  userId,
  callback,
) {
...
var getUser = (module.exports.getUser = function(ctx, userId, callback) {
  var validator = new Validator();
  validator
    .check(userId, { code: 400, msg: 'An invalid user id was provided' })
    .isUserId();
  if (validator.hasErrors()) {
    return callback(validator.getFirstError());
  }
 ...
 */
var getFullUserProfile = (module.exports.getFullUserProfile = function(
  ctx,
  userId,
  callback,
) {
  ...
  getUser(ctx, userId, function(err, user) {
    if (err) {
      ...
      return callback({
        code: 404,
        msg: util.format("Couldn't find principal: ", userId),
      });
    }
    ...
    var _finishDecorator = function() {
      ...
      if (numDecorators === 0) {
        ...
        PrincipalsEmitter.emit(
          PrincipalsConstants.events.GET_USER_PROFILE,
          ctx,
          user,
        );
        return callback(null, user);
    ...
    _.each(fullUserProfileDecorators, function(decorator, namespace) {
      ...
      decorator(ctx, clone(user), function(err, decoration) {
        if (err) {
          log().warn(
            { err: err },
            'Skipping decorator because of an error in the decoration method',
          );
          return _finishDecorator();
...
var getMe = (module.exports.getMe = function(ctx, callback) {
  ...
  getUser(ctx, ctx.user().id, function(err, data) {
    ...
    data.needsToAcceptTC = PrincipalsTermsAndConditionsAPI.needsToAcceptTermsAndConditions(
      ctx,
    );

    // Generate a signature that can be used to authenticate to one's self for push notifications
    data.signature = Signature.createExpiringResourceSignature(
      ctx,
      ctx.user().id,
    );
 ...
 * @param  {Object}    callback.err    An error that occurred, if any
 */
var setTenantAdmin = (module.exports.setTenantAdmin = function(
  ctx,
  userId,
  isAdmin,
  callback,
 ...
 * @param  {Object}    callback.err     An error that occurred, if any
 */
var setGlobalAdmin = (module.exports.setGlobalAdmin = function(
  ctx,
  userId,
  isAdmin,
  callback,
  ...
  if (
    ctx.user() &&
    _.isFunction(ctx.user().isGlobalAdmin) &&
...
var _setAdmin = function(ctx, adminType, isAdmin, principalId, callback) {
  if (!PrincipalsUtil.isUser(principalId)) {
    return callback({
      code: 400,
      msg: 'The provided principalId is not a user',
    });
  }
  ...
  getUser(ctx, principalId, function(err, user) {
    if (err) {
      ...
      return callback({
        code: 404,
        msg: util.format("Couldn't find principal: ", principalId),
      });
    }
...
var _sendEmailToken = function(ctx, user, email, token, callback) {
    ...
    function(err) {
      if (err) {
        log().error(
          ...
          'Unable to send a user a verification email',
        );
      }
    };
  ...
  PrincipalsDAO.storeEmailToken(user.id, email, token, function(err) {
    ...
    var data = {
      ...
      skinVariables: require('oae-ui').getTenantSkinVariables(
        ctx.tenant().alias,
      ),
      token: token,
    ...
    EmailAPI.sendEmail('oae-principals', 'verify', userToEmail, data, {
      hash: Date.now(),
    });
 ...
 * @param  {Object}     callback.err    An error that occurred, if any
 */
var resendEmailToken = (module.exports.resendEmailToken = function(
  ctx,
  userId,
  callback,
) {
  ...
  if (
    ...
    return callback({
      code: 401,
      msg: 'You are not authorized to resend an email token',
    });
  }
 ...
 * @param  {User}       callback.user   The updated user
 */
var verifyEmail = (module.exports.verifyEmail = function(
  ctx,
  userId,
  token,
  callback,
  ...
  validator
    ...
    .isUserId();
  validator
    .check(token, { code: 400, msg: 'A token must be provided' })
    .notEmpty();
  validator
  ...
  PrincipalsDAO.getPrincipal(userId, function(err, user) {
    ...
    PrincipalsDAO.getEmailToken(userId, function(err, email, persistedToken) {
      ...
      PrincipalsUtil.verifyEmailAddress(ctx, user, email, function(
        err,
        updatedUser,
      ) {
        if (err) {
 ...
 */
var getEmailToken = (module.exports.getEmailToken = function(
  ctx,
  userId,
  callback,
) {
  ...
  validator
    .check(null, {
      code: 401,
      msg:
        'You have to be logged in to be able to check for the existence of a pending email token',
    })
    .isLoggedInUser(ctx);
  if (validator.hasErrors()) {
    return callback(validator.getFirstError());
  ...
  if (
    ...
    return callback({
      code: 401,
      msg:
        'You are not authorized to check for the existence of a pending email token',
    });
  }
 ...
 * @param  {String}     callback.email      The email address for which there is a token
 */
var deleteEmailToken = (module.exports.deleteEmailToken = function(
  ctx,
  userId,
  callback,
) {
  ...
  validator
    .check(null, {
      code: 401,
      msg:
        'You have to be logged in to be able to delete a pending email token',
    })
    .isLoggedInUser(ctx);
  if (validator.hasErrors()) {
    return callback(validator.getFirstError());
  ...
  if (
    ...
    return callback({
      code: 401,
      msg: 'You are not authorized to delete a pending email token',
    });
  }
  ...
  getEmailToken(ctx, userId, function(err) {
    ...
    PrincipalsDAO.deleteEmailToken(userId, function(err) {
      ...
      PrincipalsEmitter.emit(
        PrincipalsConstants.events.DELETED_EMAIL_TOKEN,
        ctx,
        userId,
        function(errs) {
          if (errs) {
