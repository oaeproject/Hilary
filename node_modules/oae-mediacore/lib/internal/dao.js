 ...
 */
var getContentRevisionId = module.exports.getContentRevisionId = function(mediaCoreId, callback) {
    Cassandra.runQuery('SELECT * FROM "MediaCoreContentRevisionIdMap" WHERE "mediaCoreId" = ?', [mediaCoreId], function(err, rows) {
        if (err) {
            return callback(err);
        } else if (_.isEmpty(rows)) {
            return callback({'code': 404, 'msg': 'Could not find media core mapping for id ' + mediaCoreId});
        }
 ...
 * @param  {Object}     callback.err    An error that occurred, if any
 */
var saveContentRevisionId = module.exports.saveContentRevisionId = function(mediaCoreId, contentId, revisionId, callback) {
    var q = Cassandra.constructUpsertCQL('MediaCoreContentRevisionIdMap', 'mediaCoreId', mediaCoreId, {
        'contentId': contentId,
