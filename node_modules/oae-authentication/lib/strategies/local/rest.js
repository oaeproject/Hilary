...
/*!
 * Authenticate as a user to the global admin tenant
 */
OAE.globalAdminRouter.on('post', '/api/auth/login', [_handleLocalAuthentication, function(req, res, next) {
    ...
    return res.send(200, req.user.user);
}]);

/*!
 * Logout from the global admin tenant
 */
OAE.globalAdminRouter.on('post', '/api/auth/logout', _handleLogout);

/*!
 * Create a global admin user with mapped authentication credentials
 */
OAE.globalAdminRouter.on('post', '/api/auth/createGlobalAdminUser', function(req, res) {
    // Create the user as global admin
    AuthenticationAPI.createGlobalAdminUser(req.ctx, req.body.username, req.body.password, req.body.displayName, function(err, user) {
        if (err) {
...
OAE.globalAdminRouter.on('post', '/api/user/:id/password', function(req, res) {
    AuthenticationAPI.changePassword(req.ctx, req.params.id, null, req.body.newPassword, function(err, changed) {
        if (err) {
            return res.send(err.code, err.msg);
        }
...
OAE.tenantRouter.on('get', '/api/auth/exists/:username', function(req, res) {
    AuthenticationAPI.localUsernameExists(req.ctx, req.ctx.tenant().alias, req.params.username, function(err, exists) {
        if (err) {
            return res.send(err.code, err.msg);
        }
 ...
 * Checks whether or not a user id exists
 */
OAE.globalAdminRouter.on('get', '/api/auth/:tenant/exists/:username', function(req, res) {
    AuthenticationAPI.localUsernameExists(req.ctx, req.params.tenant, req.params.username, function(err, exists) {
        if (err) {
            return res.send(err.code, err.msg);
        }
