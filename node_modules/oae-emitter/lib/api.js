...
var log = require('oae-logger').logger('oae-emitter');
 ...
 */
var EventEmitter = module.exports.EventEmitter = function() {
 ...
 */
EventEmitter.prototype.emit = function(/*name, [args...], [callback]*/) {
    var self = this;
    ...
    var name = args.shift();
    if (!_.isString(name)) {
        throw new Error(util.format('Expected a string for event "name", but got: %s', JSON.stringify(name, null, 2)));
    }
    ...
    // a function
    var consumerCallback = (_.chain(args).last().isFunction().value()) ? args.pop() : function(errs) {
        if (errs) {
            log().error({'errs': errs, 'name': name}, 'Unhandled error(s) occurred processing `when` handlers');
        }
    };

    // If there are no _when handlers, invoke the consumer callback immediately
    var handlers = self._when[name];
    if (_.isEmpty(handlers)) {
        return consumerCallback();
    }

    // We will aggregate all errors returned by each handler into an array for the consumer callback
    var handlerErrs = null;
    ...
    var finalCallback = _.after(handlers.length, function() {
        return consumerCallback(handlerErrs);
    });
    ...
    var _handlerCallback = function(err) {
        if (err) {
...
EventEmitter.prototype.when = function(name, handler) {
    if (!_.isString(name)) {
        throw new Error('Can only bind "when" handler for event whose name is a string');
    } else if (!_.isFunction(handler)) {
        throw new Error('Can only bind a function as the "when" handler for an event');
    }
