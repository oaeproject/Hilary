 ...
 * @param  {Object}     options.pageResources.state                                 An arbitrary state object that will be provided for the next page of resources
 * @param  {String}     options.pageResources.limit                                 The suggested maximum number of items to return
 ...
 * @param  {Object}     [options.pageResources.callback.nextState]                  The object to use as the `state` parameter in the next invokation to get the next page of items. If unspecified, it indicates there are no more pages of resources
 */
 ...
 * @api private
 */
var _build = function(indexName, libraryId, callback, _nextState) {
    if (_nextState === null) {
        return callback();
    }
    ...
    registeredIndex.pageResources(libraryId, _nextState, 100, function(err, resources, nextState) {
        if (err) {
            return callback(err);
        }
        ...
        _insert(indexName, indexEntries, function(err) {
            ...
            // Recursively repeat with the nextState to get the next batch
            return _build(indexName, libraryId, callback, nextState);
        });
    });
};
