...
var getRevisions = module.exports.getRevisions = function(ctx, contentId, start, limit, callback) {
    ...
    getContent(ctx, contentId, function(err, contentObj) {
        ...
        _getRevisions(ctx, contentObj, start, limit, false, callback);
    });
};
 ...
 * @param  {Integer}        limit               Number of revisions to return. Will default to 10 if not provided
 * @param  {Boolean}        includePadHtml      Should the etherpad content html be included on each revision
 * @param  {Function}       callback            Standard callback function takes arguments `err` and `revisions`
 * @param  {Object}         callback.err        Error object containing the error message
 * @param  {Revision[]}     callback.revisions  Array that contains an object for each revision.
 * @api private
 */
var _getRevisions = function(ctx, contentObj, start, limit, includePadHtml, callback) {
    // Page the query.
    ContentDAO.Revisions.getRevisions(contentObj.id, start, limit, includePadHtml, function(err, revisions) {
        if (err) {
            return callback(err);
        }
...
var _getRevision = function(ctx, contentObj, revisionId, callback) {
    if (!revisionId) {
        // Get the latest one.
        _getRevisions(ctx, contentObj, null, 1, true, function(err, revisions) {
            if (err) {
                return callback(err);
            }
