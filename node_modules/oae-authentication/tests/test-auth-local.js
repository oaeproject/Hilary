...
describe('Authentication', function() {
                    ...
                    RestAPI.Authentication.changePassword(testUserRestContext, userObj.id, 'password', 'totally-new-password', function(err) {
                        ...
                        RestAPI.User.createUser(camAdminRestContext, usernameB, 'password', 'Test User', null, function(err, otherUserObj) {
                            ...
                            RestAPI.Authentication.changePassword(testUserRestContext, otherUserObj.id, 'password', 'totally-new-password', function(err) {
                                ...
                                RestAPI.Authentication.login(anonymousCamRestContext, usernameA, 'password', function(err) {
                                    assert.ok(err);
                                    assert.equal(err.code, 401);

                                    // Try logging in with the new password
                                    RestAPI.Authentication.login(anonymousCamRestContext, usernameA, 'totally-new-password', function(err) {
                                        assert.ok(!err);
    ...
    describe('#getOrCreateUser', function() {
        ...
        it('verify getOrCreateUser', function(callback) {
            ...
            AuthenticationAPI.getOrCreateUser(ctx, AuthenticationConstants.providers.TWITTER, userId, null, 'Nicolaas Matthijs', null, function(err, userObj) {
                assert.ok(!err);
                assert.ok(userObj);

                // Get the user again through the same function
                AuthenticationAPI.getOrCreateUser(ctx, AuthenticationConstants.providers.TWITTER, userId, null, 'Branden Visser', null, function(err, userObj) {
                    assert.ok(!err);
                    assert.ok(userObj);
    ...
    describe('#createUser', function() {
        ...
        it('verify create user with local loginId', function(callback) {
            ...
            AuthenticationAPI.createUser(ctx, loginId, 'Branden Visser', undefined, function(err, userObj) {
                ...
                RestAPI.Authentication.login(anonymousCamRestContext, username, 'password', function(err) {
                    assert.ok(!err);
                    // Verify that we are actually logged in
                    RestAPI.User.getMe(userRestContext, function(err, meObj) {
                        assert.ok(!err);
    ...
    describe('#createTenantAdminUser', function() {
        ...
        it('verify tenant administrator profile parameters', function(callback) {
            ...
            RestAPI.Authentication.createTenantAdminUserOnTenant(globalAdminRestContext, global.oaeTests.tenants.cam.alias, username, password, displayName, opts, function(err, user) {
                assert.ok(!err);
                ...
                // Ensure the user is a tenant administrator
                RestAPI.User.getUser(globalAdminRestContext, user.id, function(err, user) {
                    ...
                    username = TestsUtil.generateTestUserId();
                    RestAPI.Authentication.createTenantAdminUser(camAdminRestContext, username, password, displayName, opts, function(err, user) {
                        assert.ok(!err);
                        assert.equal(user.displayName, displayName);
                        assert.equal(user.email, opts.email);
                        assert.equal(user.locale, opts.locale);
        ...
        it('verify authorization of creating a tenant admin user on a tenant', function(callback) {
            ...
            TestsUtil.generateTestUsers(camAdminRestContext, 1, function(err, users, mrvisser) {
                ...
                RestAPI.Authentication.createTenantAdminUserOnTenant(anonymousGlobalRestContext, global.oaeTests.tenants.cam.alias, username, 'password', 'displayName', null, function(err, user) {
                    assert.ok(err);
                    assert.equal(err.code, 401);

                    // Ensure anonymous user tenant user cannot create a tenant admin on a tenant
                    RestAPI.Authentication.createTenantAdminUserOnTenant(anonymousCamRestContext, global.oaeTests.tenants.cam.alias, username, 'password', 'displayName', null, function(err, user) {
                        assert.ok(err);
                        assert.equal(err.code, 404);

                        // Ensure regular user tenant user cannot create a tenant admin on a tenant
                        RestAPI.Authentication.createTenantAdminUserOnTenant(mrvisser.restContext, global.oaeTests.tenants.cam.alias, username, 'password', 'displayName', null, function(err, user) {
                            assert.ok(err);
                            assert.equal(err.code, 404);

                            // Sanity check global admin can create a tenant administrator on another tenant
                            RestAPI.Authentication.createTenantAdminUserOnTenant(globalAdminRestContext, global.oaeTests.tenants.cam.alias, username, 'password', 'displayName', null, function(err, user) {
                                assert.ok(!err);
        ...
        it('verify authorization of creating a tenant admin user on the current tenant', function(callback) {
            ...
            TestsUtil.generateTestUsers(camAdminRestContext, 1, function(err, users, mrvisser) {
                ...
                RestAPI.Authentication.createTenantAdminUser(anonymousCamRestContext, username, 'password', 'displayName', null, function(err, user) {
                    assert.ok(err);
                    assert.equal(err.code, 401);

                    // Ensure regular users cannot create tenant admins
                    RestAPI.Authentication.createTenantAdminUser(mrvisser.restContext, username, 'password', 'displayName', null, function(err, user) {
                        assert.ok(err);
                        assert.equal(err.code, 401);

                        // Sanity check a tenant administrator can create a new tenant administrator on the tenant
                        RestAPI.Authentication.createTenantAdminUser(camAdminRestContext, username, 'password', 'displayName', null, function(err, user) {
                            assert.ok(!err);
        ...
        it('verify validation of creating a tenant admin user on a specified tenant', function(callback) {
            ...
            RestAPI.Authentication.createTenantAdminUserOnTenant(globalAdminRestContext, null, username, password, displayName, opts, function(err, user) {
                assert.ok(err);
                assert.equal(err.code, 404);

                // Ensure a username is required
                RestAPI.Authentication.createTenantAdminUserOnTenant(globalAdminRestContext, global.oaeTests.tenants.cam.alias, null, password, displayName, opts, function(err, user) {
                    assert.ok(err);
                    assert.equal(err.code, 400);

                    // Ensure a password is required
                    RestAPI.Authentication.createTenantAdminUserOnTenant(globalAdminRestContext, global.oaeTests.tenants.cam.alias, username, null, displayName, opts, function(err, user) {
                        assert.ok(err);
                        assert.equal(err.code, 400);

                        // Ensure a displayName is required
                        RestAPI.Authentication.createTenantAdminUserOnTenant(globalAdminRestContext, global.oaeTests.tenants.cam.alias, username, password, null, opts, function(err, user) {
                            assert.ok(err);
                            assert.equal(err.code, 400);

                            // Ensure target tenant cannot be the global admin tenant
                            RestAPI.Authentication.createTenantAdminUserOnTenant(globalAdminRestContext, 'admin', username, password, displayName, opts, function(err, user) {
                                assert.ok(err);
                                assert.equal(err.code, 400);
        ...
        /**
         * Test that verifies validation of creating a tenant administrator on the current
         */
        it('verify validation of creating a tenant admin user on the current tenant', function(callback) {
            ...
            // Ensure a username is required
            RestAPI.Authentication.createTenantAdminUser(camAdminRestContext, null, password, displayName, opts, function(err, user) {
                assert.ok(err);
                assert.equal(err.code, 400);

                // Ensure a password is required
                RestAPI.Authentication.createTenantAdminUser(camAdminRestContext, username, null, displayName, opts, function(err, user) {
                    assert.ok(err);
                    assert.equal(err.code, 400);

                    // Ensure a displayName is required
                    RestAPI.Authentication.createTenantAdminUser(camAdminRestContext, username, password, null, opts, function(err, user) {
                        assert.ok(err);
                        assert.equal(err.code, 400);

                        // Ensure target tenant cannot be the global admin tenant by requesting with the global administrator
                        RestAPI.Authentication.createTenantAdminUser(globalAdminRestContext, username, password, displayName, opts, function(err, user) {
                            assert.ok(err);
                            assert.equal(err.code, 404);
    ...
    describe('#createGlobalAdminUser', function() {
        ...
        it('verify global administrator profile parameters', function(callback) {
            ...
            RestAPI.Authentication.createGlobalAdminUser(globalAdminRestContext, username, password, displayName, opts, function(err, user) {
                assert.ok(!err);
                assert.equal(user.displayName, displayName);
                assert.equal(user.email, opts.email);
                assert.equal(user.locale, opts.locale);
        ...
        it('verify authorization of creating a global admin user', function(callback) {
            ...
            RestAPI.Authentication.createGlobalAdminUser(anonymousCamRestContext, userId, userId, userId, null, function(err, user) {
                assert.ok(err);
                assert.equal(err.code, 404);
                assert.ok(!user);

                // Ensure anonymous on global admin tenant cannot create a global admin user
                RestAPI.Authentication.createGlobalAdminUser(anonymousGlobalRestContext, userId, userId, userId, null, function(err, user) {
                    assert.ok(err);
                    assert.equal(err.code, 401);
                    assert.ok(!user);
                    ...
                    // only mounted on the global tenant
                    RestAPI.Authentication.createGlobalAdminUser(camAdminRestContext, userId, userId, userId, null, function(err, user) {
                        assert.ok(err);
                        assert.equal(err.code, 404);
                        assert.ok(!user);
                        ...
                        RestAPI.Authentication.login(testGlobalUserRestContext, userId, userId, function(err) {
                            ...
                            RestAPI.User.getMe(testGlobalUserRestContext, function(err, me) {
                                ...
                                RestAPI.Authentication.createGlobalAdminUser(globalAdminRestContext, userId, userId, userId, null, function(err, user) {
                                    assert.ok(!err);
                                    assert.ok(user);
        ...
        it('verify validation of creating a global admin user', function(callback) {
            ...
            RestAPI.Authentication.createGlobalAdminUser(globalAdminRestContext, null, username, username, null, function(err, user) {
                assert.ok(err);
                assert.equal(err.code, 400);
                assert.ok(!user);

                // Ensure you cannot create a global admin user without a password
                RestAPI.Authentication.createGlobalAdminUser(globalAdminRestContext, username, null, username, null, function(err, user) {
                    assert.ok(err);
                    assert.equal(err.code, 400);
                    assert.ok(!user);

                    // Ensure you cannot create a global admin user with a password that is too short
                    RestAPI.Authentication.createGlobalAdminUser(globalAdminRestContext, username, 'a', username, null, function(err, user) {
                        assert.ok(err);
                        assert.equal(err.code, 400);
                        assert.ok(!user);

                        // Ensure you cannot create a global admin user without a displayName
                        RestAPI.Authentication.createGlobalAdminUser(globalAdminRestContext, username, username, null, null, function(err, user) {
                            assert.ok(err);
                            assert.equal(err.code, 400);
                            assert.ok(!user);
                            ...
                            RestAPI.Authentication.login(testGlobalUserRestContext, username, username, function(err) {
                                ...
                                RestAPI.User.getMe(testGlobalUserRestContext, function(err, me) {
                                    ...
                                    RestAPI.Authentication.createGlobalAdminUser(globalAdminRestContext, username, username, username, null, function(err, user) {
                                        assert.ok(!err);
                                        assert.ok(user);

                                        // Ensure when we try and create one with the same loginId, we get a 400 error
                                        RestAPI.Authentication.createGlobalAdminUser(globalAdminRestContext, username, username, username, null, function(err, secondUser) {
                                            assert.ok(err);
                                            assert.equal(err.code, 400);
