...
LibraryAPI.Index.registerLibraryIndex(PrincipalsConstants.library.MEMBERSHIPS_INDEX_NAME, {
    'pageResources': function(libraryId, start, limit, callback) {
        // Query all the discussion ids ('c') to which the library owner is directly associated in this batch of paged resources
        AuthzAPI.getPrincipalMemberships(libraryId, start, limit, function(err, groupIds, nextToken) {
            if (err) {
                return callback(err);
            }
