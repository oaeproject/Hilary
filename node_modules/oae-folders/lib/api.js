 ...
 * * `updatedFolder(ctx, folder)`: A folder was deleted
 ...
 * @param  {String}         [updates.applyVisibilityOn]     Whether the visibility change should just be applied on the folder or on both the folder and the content in the folder. Options: `folder` or `folderAndContent
 * @param  {Function}       callback                        Standard callback function
 * @param  {Object}         callback.err                    An error that occurred, if any
...
var updateFolder = module.exports.updateFolder = function(ctx, folderId, updates, callback) {
    var allVisibilities = _.values(AuthzConstants.visibility);
    var applyVisibilityOptions = _.values(FoldersConstants.applyVisibilityOptions);
    ...
    if (updates.visibility) {
        ...
        validator.check(updates.applyVisibilityOn, {'code': 400, 'msg': 'An folder visibility apply option must be provided when changing the visibility. Must be one of: ' + applyVisibilityOptions.join(', ')}).notEmpty();
        validator.check(updates.applyVisibilityOn, {'code': 400, 'msg': 'An invalid folder visibility apply option has been provided. Must be one of: ' + applyVisibilityOptions.join(', ')}).isIn(applyVisibilityOptions);
    ...
    FoldersDAO.getFolder(folderId, function(err, folder) {
        ...
        FoldersAuthz.canManageFolder(ctx, folder, function(err, canManage) {
            ...
            FoldersDAO.updateFolder(folder, profileFields, function(err, updatedFolder) {
                ...
                /*!
                 * Gets called once all the content items have been updated
                 *
                 * @param  {Object}         err                 An error object, if any
                 * @param  {Content[]}      [failedContent]     The content items that could not be updated
                 * @api private
                 */
                var done = function(err, failedContent) {
                    failedContent = failedContent || [];

                    // Sign the folder previews (if any)
                    folder = _augmentFolder(ctx, folder);

                    // Regardless of any errors, we need to trigger an `updateFolder` event
                    // as the visibility of this folder has been persisted and has an influence
                    // on the search index
                    FoldersAPI.emit(FoldersConstants.events.UPDATED_FOLDER, ctx, folder, updatedFolder);

                    // If there was an error we return to the caller here
                    if (err) {
                        return callback(err);
                    }
                    ...
                    _getFullFolderProfile(ctx, updatedFolder, function(err, fullFolder) {
                        ...
                        return callback(err, fullFolder, failedContent);
                    });
                };

                // Determine if the visibility will change for this folder
                var visibilityChange = (profileFields.visibility && (folder.visibility !== profileFields.visibility));

                // Apply the visibility change on all the content in this folder if there
                // was a change and the user requested that the content items are changed as well
                if (visibilityChange && updates.applyVisibilityOn === FoldersConstants.applyVisibilityOptions.FOLDER_AND_CONTENT) {
                    return _applyVisibilityOnContentInFolder(ctx, folder, profileFields.visibility, done);

                // If the visibility didn't change, we can move on
                } else {
                    return done();
                }
            });
 ...
 * @param  {String}         newVisibility               The new visibility for the content items
 * @param  {Function}       callback                    Standard callback function
 * @param  {Object}         callback.err                Standard error object, if any
 * @param  {Content[]}      callback.failedContent      The content items that couldn't be updated
 * @api private
 */
var _applyVisibilityOnContentInFolder = function(ctx, folder, newVisibility, callback, _start, _failedContent) {
    ...
    FoldersContentLibrary.list(folder, 'private', {'start': _start, 'limit': 20}, function(err, contentIds, nextToken) {
        ...
        ContentDAO.Content.getMultipleContentItems(contentIds, null, function(err, contentItems) {
            ...
            contentItems = _.filter(contentItems, function(content) {
                return (content.visibility !== newVisibility);
            });
            ...
            var done = _.after(contentItems.length, function() {
                ...
                if (nextToken) {
                    _applyVisibilityOnContentInFolder(ctx, folder, newVisibility, callback, nextToken, _failedContent);
            ...
            _.each(contentItems, function(content) {
                ContentAPI.updateContentMetadata(ctx, content.id, {'visibility': newVisibility}, function(err) {
                    if (err) {
...
var deleteFolder = module.exports.deleteFolder = function(ctx, folderId, callback) {
    var validator = new Validator();
    validator.check(folder, {'code': 400, 'msg': 'A folder id must be provided'}).isResourceId();
    ...
    FoldersDAO.getFolder(folderId, function(err, folder) {
        ...
        FoldersAuthz.canManageFolder(ctx, folder, function(err, canManage) {
            ...
            AuthzAPI.getAuthzMembers(folder.groupId, null, 10000, function(err, memberRoles, nextToken) {
                ...
                var roleChanges = {};
                var memberIds = [];
                _.each(memberRoles, function(role, memberId) {
                ...
                AuthzAPI.updateRoles(folder.groupId, roleChanges, function(err) {
                    ...
                    FoldersFoldersLibrary.remove(memberIds, folder, function(err) {
                        ...
                        // TODO: Purge the folder content libraries
                        // TODO: Remove the folder from all the content items
                        ...
                        FoldersDAO.deleteFolder(folder.id, function(err) {
                            ...
                            FoldersAPI.emit(FoldersConstants.events.DELETED_FOLDER, ctx, discussionId);
                            return callback();
                        });
                    });
                });
