        ...
        _splitPDF(ctx, path, pagesDir, function(err, pageFilenames) {
            ...
            var resize = function() {
                ...
                _convertToPNG(ctx, pagePath, pngPath, PreviewConstants.SIZES.PDF.LARGE, function(err) {
                    ...
                    var desiredFilename = name + 'normal.png';
                    _downScale(ctx, pngPath, pagesDir, desiredFilename, PreviewConstants.SIZES.PDF.NORMAL, function(err, normalPagePath) {
                        if (err) {
                            return callback(err);
                        }
                        ...
                        desiredFilename = name + 'small.png';
                        _downScale(ctx, normalPagePath, pagesDir, desiredFilename, PreviewConstants.SIZES.PDF.SMALL, function(err, smallPagePath) {
                            if (err) {
                                return callback(err);
                            }
                            ...
                            if (pageFilenames.length === 0 || previewedPages === 250) {
                                ...
                                // We copy the page.1.thumbnail.png file to thumbnail.png
                                // and use it as a thumbnail.
                                var thumbnailPath = pagesDir + '/thumbnail.png';
                                IO.copyFile(pagesDir + '/page.1.small.png', thumbnailPath, function(err) {
                                    if (err) {
                                        return callback(err);
                                    }
 ...
 */
var _downScale = function(ctx, input, outputdir, desiredFilename, width, callback) {
    var cmd = util.format('MAGICK_TMPDIR="%s" OMP_NUM_THREADS=2 gm mogrify -density 150 -resize %sx -unsharp 0x0.5+0.75 -format "scaleddown.png" %s', outputdir, width, input);
     // Execute the command.
    log().trace({'contentId': ctx.contentId}, 'Executing %s', cmd);
    var child = exec(cmd, { 'timeout': _timeout }, function (err, stdout, stderr) {
        if (err) {
        ...
        var oldPath = outputdir + '/' + filename + '.scaleddown.png';
        var newPath = outputdir + '/' + desiredFilename;
