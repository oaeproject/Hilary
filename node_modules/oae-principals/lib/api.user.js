    ...
    getUser(ctx, userId, function(err, user) {
        ...
        if (ctx.user().id === userId || ctx.user().isAdmin(principalResource.tenantAlias)) {
            ...
            Cassandra.runQuery(q.query, q.parameters, function(err) {
                ...
                // asynchronously load and update the user.
                process.nextTick(function() {
                    module.exports.getUser(ctx, userId, function(err, user) {
                        if (err) {
                            log().error({ err: err }, 'Error loading user to be reindexed');
                            return;
                        }
                        _reindexUser(user);
                    });
...
/**
 * Index or reindex the given user.
 *
 * @param   {User}      user            The user to index.
 * @param   {Function}  callback        Invoked when the process completes.
 * @param   {Object}    callback.err    An error that occurred, if any
 */
var _reindexUser = function(user, callback) {
    callback = callback || function(err) {
        if (err) {
            log().error({
                err: err,
                user: user
            }, 'Error updating index for user.');
        }
    }

    PrincipalsSearchAPI.indexUser(user, callback);
};
