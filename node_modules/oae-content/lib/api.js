...
var createLink = module.exports.createLink = function(ctx, displayName, description, visibility, link, additionalMembers, callback) {
    ...
    validator.check(link, {'code': 400, 'msg': 'A link can be at most 2083 characters'}).len(1, 2083);
...
var _createFile = function(ctx, displayName, description, visibility, additionalMembers, file, callback) {
    ...
    validator.check(displayName, {'code': 400, 'msg': 'A content displayName must be provided'}).isShortString();
    if (description) {
        validator.check(description, {'code': 400, 'msg': 'A content description must be at most a medium-sized string'}).isMediumString();
    }
    if (file) {
...
var _createContent = function(ctx, contentId, revisionId, resourceSubType, displayName, description, visibility, additionalMembers, otherValues, revisionData, callback) {
    ...
    validator.check(displayName, {'code': 400, 'msg': 'A content displayName must be provided'}).isShortString();
    if (description) {
        validator.check(description, {'code': 400, 'msg': 'A content description must at most a medium-sized string'}).isMediumString();
    }
    validator.check(contentId, {'code': 400, 'msg': 'A content ID must be provided'}).isResourceId();
...
var updateContentMetadata = module.exports.updateContentMetadata = function(ctx, contentId, profileFields, callback) {
    ...
    for (var i = 0; i < fieldNames.length; i++) {
        ...
        if (fieldName === 'displayName') {
            ...
            validator.check(profileFields['description'], {'code': 400, 'msg': 'A description should be a medium-sized string'}).isMediumString();
        } else if (fieldName === 'link') {
            validator.check(link, {'code': 400, 'msg': 'A valid link should be provided'}).isUrl();
            validator.check(link, {'code': 400, 'msg': 'A link can be at most 2083 characters'}).len(1, 2083);
        }
    }
