/*!
 * Copyright 2014 Apereo Foundation (AF) Licensed under the
 * Educational Community License, Version 2.0 (the "License"); you may
 * not use this file except in compliance with the License. You may
 * obtain a copy of the License at
 *
 *     http://opensource.org/licenses/ECL-2.0
 *
 * Unless required by applicable law or agreed to in writing,
 * software distributed under the License is distributed on an "AS IS"
 * BASIS, WITHOUT WARRANTIES OR CONDITIONS OF ANY KIND, either express
 * or implied. See the License for the specific language governing
 * permissions and limitations under the License.
 */

var OAE = require('oae-util/lib/oae');
var TelemetryAPI = require('./api');

/**
* @REST adminGetTelemetry
*
* Get all the telemetry data
*
* @Server      admin
* @Method      GET
* @Path        /telemetry
* @Return      {Telemetry}                  Returns an object of telemetry data
*/
OAE.globalAdminRouter.on('get', '/api/telemetry', function(req, res) {
    if (req.ctx.user() && req.ctx.user().isGlobalAdmin()) {
        TelemetryAPI.getTelemetryData(function(err, data) {
            if (err) {
                return res.send(err.code, err.msg);
            }
            res.send(200, data);
        });
    } else {
        return res.send(401, 'Only global administrators are allowed to retrieve telemetry data');
    }
});

// Add the REST models for Tenants

/**
 * @RESTModel Telemetry
 *
 * @Required  []
 * @Property  {CassandraTelemetry}      cassandra       Cassandra telemetry data
 * @Property  {SearchTelemetry}         search          Search telemetry data
 * @Property  {ServerTelemetry}         server          Server telemetry data
 * @Property  {PushTelemetry}           push            Push telemetry data
 */

/**
 * @RESTModel CassandraTelemetry
 *
 * @Required  []
 * @Property  {number}          {cassandraStatistic}    Value of named cassandra statistic
 */

/**
 * @RESTModel SearchTelemetry
 *
 * @Required  []
 * @Property  {number}          {searchStatistic}       Value of named search statistic
 */

/**
 * @RESTModel SearchTelemetry
 *
 * @Required  []
 * @Property  {number}          {serverStatistic}       Value of named server statistic
 * @Property  {number[]}        {serverStatistics}      Values for named server statistic
 */

/**
 * @RESTModel PushTelemetry
 *
 * @Required  []
 * @Property  {number}          {pushStatistic}         Value of named push statistic
 */

