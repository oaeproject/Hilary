...
var assertCreateMembershipsGraphSucceeds = module.exports.assertCreateMembershipsGraphSucceeds = function(graph, callback, _ops) {
    if (!_ops) {
        var ops = _.chain(graph.getNodes())
            .pluck('id')
        ...
        return assertCreateMembershipGraphSucceeds(graph, callback, ops);
    ...
    AuthzAPI.updateRoles(op.id, op.roles, function(err) {
        assert.ok(!err);
        return assertCreateMembershipGraphSucceeds(graph, callback, _ops);
    });
};
