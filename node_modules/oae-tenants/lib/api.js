 ...
 */
var TenantsAPI = module.exports = new EmitterAPI.EventEmitter();
...
Pubsub.on('oae-tenants', function(message) {
    ...
    _updateCachedTenant(alias, function(err) {
        if (err) {
            log().error({'err': err, 'cmd': cmd, 'alias': alias},
                'An error occurred while refreshing a tenant after update');
        }
 ...
 */
ConfigAPI.on('update', function(alias) {
...
var init = module.exports.init = function(_serverConfig, callback) {
    ...
    _cacheTenants(function(err) {
        ...
        var guestTenantAlias = serverConfig.guestTenantAlias;
        var guestTenantHost = serverConfig.guestTenantHost;
        var guestTenant = getTenant(guestTenantAlias);
        if (guestTenant) {
            return callback();
        }

        // If the guest tenant doesn't exist yet, create it
        _createTenant(guestTenantAlias, 'Guest tenant', guestTenantHost, null, function(err) {
            if (err) {
                return callback(err);
            }
 ...
 */
var getNonInteractingTenants = module.exports.getNonInteractingTenants = function() {
 ...
 */
var getTenantByEmail = module.exports.getTenantByEmail = function(emailOrDomain) {
    var domain = _.last(emailOrDomain.split('@'));
    var tenant = _.find(tenantsSorted, function(tenant) {
        // Filter the list of all tenants down to just the one (should be only one due to
        // validation) whose email domain expression matches the specified email domain
        return (tenant.emailDomain && _matchesEmailDomainSuffix(domain.toLowerCase(), tenant.emailDomain));
    });

    // Default to the guest tenant
    tenant = tenant || TenantsAPI.getTenant(serverConfig.guestTenantAlias);
...
var _updateCachedTenant = function(tenantAlias, callback) {
    callback = callback || function(err) {
        if (err) {
            log().error({'err': err, 'tenantAlias': tenantAlias}, 'Failed to re-cache the specified tenant');
        }
    };
    ...
    Cassandra.runQuery('SELECT * FROM "Tenant" WHERE "alias" = ?', [tenantAlias], function(err, rows) {
        ...
        var tenant = _storageHashToTenant(tenantAlias, hash);
        ...
        var index = _.findIndex(tenantsSorted, function(tenant) {
 ...
 */
var createTenant = module.exports.createTenant = function(ctx, alias, displayName, host, opts, callback) {
    opts = opts || {};

    // Validate that the user in context is the global admin
    var validator = new Validator();
    validator.check(null, {'code': 401, 'msg': 'Only global administrators can create new tenants'}).isGlobalAdministratorUser(ctx);
 ...
 * @api private
 */
var _createTenant = function(alias, displayName, host, opts, callback) {
    opts = opts || {};
    ...
    // Make sure the tenant alias, host name and email domain are all lower-case
    alias = alias.toLowerCase();
    host = host.toLowerCase();

    // Ensure there are no conflicts
    validator.check(host, {'code': 400, 'msg': 'This hostname is reserved'}).not(serverConfig.shibbolethSPHost);
    validator.check(getTenant(alias), {'code': 400, 'msg': util.format('A tenant with the alias "%s" already exists', alias)}).isNull();
    validator.check(getTenantByHost(host), {'code': 400, 'msg': util.format('A tenant with the host "%s" already exists', host)}).isNull();

    // Ensure only valid optional fields are set
    _.each(opts, function(val, key) {
        validator.check(key, {'code': 400, 'msg': util.format('Invalid field: %s', key)}).isIn(['emailDomain']);
        if (key === 'emailDomain') {
    ...
    // Create the tenant
    var tenant = new Tenant(alias, displayName, host, opts);
    var q = Cassandra.constructUpsertCQL('Tenant', 'alias', alias, _tenantToStorageHash(tenant));
...
var updateTenant = module.exports.updateTenant = function(ctx, alias, tenantUpdates, callback) {
    ...
    _.each(tenantUpdates, function(updateValue, updateField) {
        validator.check(updateField, {'code': 400, 'msg': util.format('"%s" is not a recognized tenant update field', updateField)}).isIn(['displayName', 'host', 'emailDomain']);
...
var _validateEmailDomain = function(validator, emailDomain, updateTenantAlias) {
    var matchingEmailDomain = _.chain(tenantsSorted)
        .filter(function(tenant) {
            // If a tenant is being updated, don't try and match against its own existing email
            // domain
            return (tenant.alias !== updateTenantAlias);
        })
        .pluck('emailDomain')
        .compact()
        .find(function(emailDomainExpr) {
            // There is a conflict if there is an existing expression that matches with the
            // expression, or vice versa
            return (_matchesEmailDomainSuffix(emailDomain, emailDomainExpr) ||
                _matchesEmailDomainSuffix(emailDomainExpr, emailDomain));
        })
        .value();
...
/**
 * Determine if the given email domain is suffixed by the provided email domain suffix
 *
 * @param  {String}     emailDomain         The email domain to check
 * @param  {String}     emailDomainSuffix   The email domain suffix (e.g., cam.ac.uk)
 * @return {Boolean}                        Whether or not the domain expression matched the email
 * @api private
 */
var _matchesEmailDomainSuffix = function(emailDomain, emailDomainSuffix) {
    if (emailDomain === emailDomainSuffix) {
        // Exact match is a valid max on the domain suffix
        return true;
    }

    // Prepend the email domain suffix with a "." to avoid something like blacam.ac.uk matching the
    // suffix cam.ac.uk
    emailDomainSuffix = util.format('.%s', emailDomainSuffix);
    return (emailDomain.indexOf(emailDomainSuffix, emailDomain.length - emailDomainSuffix.length) !== -1);
};
...
var _copyTenant = function(tenant) {
    ...
    var tenantCopy = _storageHashToTenant(tenant.alias, _tenantToStorageHash(tenant));
