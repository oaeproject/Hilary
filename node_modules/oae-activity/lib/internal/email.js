...
// The amount of milliseconds that go in a day
var MS_IN_ONE_DAY = 24 * 60 * 60 * 1000;

// The amount of milliseconds that go in a week
var MS_IN_ONE_WEEK = 7 * MS_IN_ONE_DAY;
...
var collectMails = module.exports.collectMails = function(bucketNumber, emailPreference, dayOfWeek, hour, callback) {
    ...
    var oldestActivityTimestamp = _getOldestActivityTimestampForCollectionCycle(emailPreference);
...
/**
 * Internal method that collects the actual emails in a bucket.
 * This method will recursively drain a complete bucket.
 *
 * Steps:
...
var _collectMails = function(bucketId, oldestActivityTimestamp, start, callback, _collectedUsers) {
    ...
    ActivityDAO.getQueuedUserIdsForEmail(bucketId, start, MAX_COLLECTION_BATCH_SIZE, function(err, bucketId, userIds, nextToken) {
        if (err) {
            return callback(err);
...
var _mail = function(user, activities, callback) {
    ...
    ActivityTransformer.transformActivities(ctx, aggregatedActivities, ActivityConstants.transformerTypes.ACTIVITYSTREAMS, function(err) {
        ...
        var adaptedActivities = ActivityAdapter.adapt(user.id, user, aggregatedActivities);
...
/**
 * Get the timestamp for the oldest activity that should be included in the current collection cycle
 *
 ...
 * @api private
 */
var _getOldestActivityTimestampForCollectionCycle = function(emailPreference) {
    if (emailPreference === PrincipalsConstants.emailPreferences.IMMEDIATE) {
        var pollingFrequency = ActivitySystemConfig.getConfig().mail.pollingFrequency;
        return Date.now() - (pollingFrequency * 1000);
    } else if (emailPreference === PrincipalsConstants.emailPreferences.DAILY) {
        return Date.now() - MS_IN_ONE_DAY;
    } else if (emailPreference === PrincipalsConstants.emailPreferences.WEEKLY) {
        return Date.now() - MS_IN_ONE_WEEK;
    }
};
