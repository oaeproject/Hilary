 ...
 * @api private
 */
var createRequestJoinGroup = module.exports.createRequestJoinGroup = function(principalId, groupId, callback) {
    var queries = [];
    ...
    getRequestsJoinGroup(groupId, function(err, requests) {
        ...
        _addObjectToArray(principalId, list, function(newPrincipalString) {
            ...
            // Create query
            queries.push({'query': 'INSERT INTO "Request" ("groupId", "principalId") VALUES (?, ?)', 'parameters': [groupId, principalId]});
...
/**
 * Add a principal to the list of principals who made a request on a specific group
 *
 * @param  {String}         principalId                 Principal id to add to the list
 * @param  [list]           list                        List of principals id who make request
 * @param  {Function}       callback                    Standard callback function
 * @param  [PincipalsId]    callback.list               List of principals id who make request
 * @api private
 */
var _addObjectToArray = function(principalId, list, callback) {

    if (_.isEmpty(list)) {
        return callback(JSON.stringify([principalId]));
    } else {
        list.push(principalId);
        return callback(JSON.stringify(list));
    }
};

/**
 * Remove a principal to the list of principals who made a request on a specific group
 *
 * @param  {String}         principalId                 Principal id to remove to the list
 * @param  [list]           list                        List of principals id who make request
 * @param  {Function}       callback                    Standard callback function
 * @param  [PincipalsId]    callback.list               List of principals id who make request
 * @api private
