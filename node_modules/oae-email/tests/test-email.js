...
describe('Emails', function() {
     ...
     *
     * @return {String}     Random mail address
     */
    var _randomEmailAddres = function() {
        return _.random(0, 100000) + '@my.example.com';
    };
        ...
        it('verifies locale-specific template selection', function(callback) {
            TestsUtil.generateTestUsers(camAdminRestContext, 2, function(err, users) {
                ...
                EmailTestsUtil.sendEmail('oae-tests', 'TestCaOverride', mrvisser.user, null, null, function(err, message) {
                    ...
                    assert.equal(message.from, EmailConfig.getValue(mrvisser.user.tenant.alias, 'general', 'from'));
                    ...
                    EmailTestsUtil.sendEmail('oae-tests', 'TestCaOverride', nico.user, null, null, function(err, message) {
                        ...
                        message = message._message;
                        assert.equal(message.from, EmailConfig.getValue(nico.user.tenant.alias, 'general', 'from'));
                    ...
                    EmailTestsUtil.sendEmail('oae-tests', 'TestOverrideOverrides', mrvisser.user, null, null, function(err, message) {
                        ...
                        EmailTestsUtil.sendEmail('oae-tests', 'TestOverrideOverrides', nico.user, null, null, function(err, message) {
                            ...
                            EmailTestsUtil.sendEmail('oae-tests', 'TestOverrideOverrides', stuart.user, null, null, function(err, message) {
                                ...
                                message = message._message;
                                assert.equal(message.from, EmailConfig.getValue(stuart.user.tenant.alias, 'general', 'from'));
                                ...
                                EmailTestsUtil.sendEmail('oae-tests', 'TestOverrideOverrides', clay.user, null, null, function(err, message) {
                                    ...
                                    message = message._message;
                                    assert.equal(message.from, EmailConfig.getValue(clay.user.tenant.alias, 'general', 'from'));
    ...
    describe('Email configuration', function() {
        ...
        it('verify noreply from header', function(callback) {
            TestsUtil.generateTestUsers(camAdminRestContext, 2, function(err, camUsers) {
                ...
                // Update the simong user
                var simong = _.values(camUsers)[0];
                var simongEmail = 'simong@email.address.com';
                RestAPI.User.updateUser(simong.restContext, simong.user.id, {'email': simongEmail}, function(err) {
                    assert.ok(!err);

                    // Update the coenego user
                    var coenego = _.values(camUsers)[1];
                    var coenegoEmail = 'coenego@email.address.com';
                    RestAPI.User.updateUser(coenego.restContext, coenego.user.id, {'email': coenegoEmail}, function(err) {
                        assert.ok(!err);

                        // Set the `from` header in the email module
                        ConfigTestUtil.updateConfigAndWait(camAdminRestContext, null, {'oae-email/general/from': '"Cambridge" <noreply@cam.oae.com>'}, function(err) {
                            assert.ok(!err);
                            ...
                            RestAPI.Content.createLink(coenego.restContext, 'Google', 'Google', 'private', 'http://www.google.com', [], [simong.user.id], function(err, link) {
                                ...
                                // Assert that simong receives an email with `"Cambridge" <noreply@cam.oae.com>` as the the configured `from` header
                                EmailTestsUtil.collectAndFetchEmails(function(messages) {
                                    assert.ok(messages);
                                    assert.ok(messages.length);
                                    assert.strictEqual(messages[0]['_message']['to'], simongEmail);
                                    assert.strictEqual(messages[0]['_message']['from'], '"Cambridge" <noreply@cam.oae.com>');
                                    ...
                                    RestAPI.Config.clearConfig(camAdminRestContext, null, ['oae-email/general/from'], function(err) {
                                        ...
                                        RestAPI.Content.createComment(simong.restContext, link.id, 'I have never seen something like this before!', null, function(err, comment) {
                                            ...
                                            EmailTestsUtil.collectAndFetchEmails(function(messages) {
                                                assert.ok(messages);
                                                assert.ok(messages.length);
                                                assert.strictEqual(messages[0]['_message']['to'], coenegoEmail);
                                                var fromHeader = '"' + coenego.user.tenant.alias + '" <noreply@' + coenego.restContext.hostHeader + '>';
                                                assert.equal(messages[0]['_message']['from'], fromHeader);
                                                return callback();
                                            });
                                        });
                                    });
    ...
    describe('Email de-duplication', function() {
        ...
        it('verify a provided hash can be used to perform de-duplication', function(callback) {
            EmailTestsUtil.collectAndFetchEmails(function() {
                TestsUtil.generateTestUsers(camAdminRestContext, 1, function(err, users, simong) {
                    ...
                    simong.user.email = _randomEmailAddres();

                    // Send out the first e-mail
                    EmailTestsUtil.sendEmail('oae-tests', 'TestTemplate', simong.user, null, {'hash': 'u:cam:simong#123456'}, function(err, message) {
                        assert.ok(!err);
                        assert.ok(message);

                        // Re-using the same hash should result in a failure
                        EmailTestsUtil.sendEmail('oae-tests', 'TestTemplate', simong.user, null, {'hash': 'u:cam:simong#123456'}, function(err) {
                            assert.ok(err);
                            ...
                            EmailTestsUtil.sendEmail('oae-tests', 'TestTemplate', simong.user, {'data': 'test'}, {'hash': 'u:cam:simong#123456'}, function(err) {
                                assert.ok(err);
                                assert.equal(err.code, 403);

                                // Using another hash (but otherwise the same mail) should work
                                EmailTestsUtil.sendEmail('oae-tests', 'TestTemplate', simong.user, null, {'hash': 'u:cam:mrvisser#000000'}, function(err, message) {
                                    assert.ok(!err);
                                    assert.ok(message);
                                    return callback();
                                });
        ...
        it('verify omitting the hash uses the email info for de-duplication', function(callback) {
            TestsUtil.generateTestUsers(camAdminRestContext, 1, function(err, users, simong) {
                ...
                // Send out the first e-mail
                EmailTestsUtil.sendEmail('oae-tests', 'TestTemplate', simong.user, null, null, function(err, message) {
                    assert.ok(!err);
                    assert.ok(message);

                    // Sending out the same email should result in a failure
                    EmailTestsUtil.sendEmail('oae-tests', 'TestTemplate', simong.user, null, null, function(err) {
                        assert.ok(err);
                        assert.equal(err.code, 403);

                        // Sanity check that sending out a different email works
                        EmailTestsUtil.sendEmail('oae-tests', 'TestTemplate', simong.user, {'data': 'test'}, null, function(err) {
                            assert.ok(err);
                            assert.equal(err.code, 403);
                            return callback();
                        });
        ...
        it('verify the same email can be sent to multiple users', function(callback) {
            TestsUtil.generateTestUsers(camAdminRestContext, 2, function(err, users, simong, nico) {
                ...
                simong.user.email = _randomEmailAddres();
                nico.user.email = _randomEmailAddres();

                EmailTestsUtil.sendEmail('oae-tests', 'TestTemplate', simong.user, null, null, function(err, message) {
                    assert.ok(!err);
                    assert.ok(message);
                    EmailTestsUtil.sendEmail('oae-tests', 'TestTemplate', nico.user, null, null, function(err, message) {
                        assert.ok(!err);
                        assert.ok(message);
        ...
        it('verify email throttling', function(callback) {
            ...
            EmailAPI.init(_createDefaultConfig({'throttling': {'timespan': 2, 'count': 2}}), function(err) {
                TestsUtil.generateTestUsers(camAdminRestContext, 1, function(err, users, simong) {
                    ...
                    EmailTestsUtil.sendEmail('oae-tests', 'TestTemplate', simong.user, null, {'hash': _uniqueHash()}, function(err, message) {
                        assert.ok(!err);
                        EmailTestsUtil.sendEmail('oae-tests', 'TestTemplate', simong.user, null, {'hash': _uniqueHash()}, function(err, message) {
                            assert.ok(!err);
                            EmailTestsUtil.sendEmail('oae-tests', 'TestTemplate', simong.user, null, {'hash': _uniqueHash()}, function(err, message) {
