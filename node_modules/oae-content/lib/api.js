...
var _getBackend = function(ctx, uri) {
    ...
    if (uri) {
        ...
        // In case something went wrong in the config api, an error will be returned.
        // When the returned value is an object with a code key, we fall back to local storage.
        if (backendName.code) {
            log().warn({'err': err}, 'Config error, falling back to local storage.');
            backendName = 'local';
        }
    }
