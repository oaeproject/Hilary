...
Pubsub.on('oae-tenants', function(message) {
    ...
    if (cmd === 'refresh') {
        // For refresh, we have to reload all tenants. This is because if the hostname changed, we
        // need to know how to remove the tenant from the tenantsByHost cache. Since we don't have
        // the previous tenant info, we just refresh them all as it's easier
        return _cacheTenants(callback);
    } else {
        return _updateCachedTenant(alias, callback);
    }
});
...
var _cacheTenants = function(callback) {
    ...
    Cassandra.runQuery('SELECT * FROM "Tenant"', false, function(err, rows) {
        ...
        _.each(rows, function(row) {
            ...
            if (!tenant.active || tenant.deleted || TenantUtil.isPrivate(tenant.alias)) {
...
var _updateCachedTenant = function(tenantAlias, callback) {
    ...
    Cassandra.runQuery('SELECT * FROM "Tenant" WHERE "alias" = ?', [tenantAlias], function(err, rows) {
        ...
        // Keep a cache of all tenants that are private and disabled so we know which ones
        // cannot be interacted with
        if (tenant.isGlobalAdminServer || !tenant.active || tenant.deleted || TenantUtil.isPrivate(tenant.alias)) {
            tenantsNotInteractable[tenant.alias] = tenant;
        } else {
