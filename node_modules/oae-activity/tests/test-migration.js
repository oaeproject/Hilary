...
describe('Activity Migration', function() {
    ...
    var setUp = function(userId, callback) {
        ...
        activities[userId] = {}
        for (var i = 0; i < 5; i++) {
            var activityId = Date.now() + ':' + i;
            activities[userId][activityId] = _mockActivity(activityId, userId);
        }
        ...
        for (var i = 5; i < 10; i++) {
            var activityId = Date.now() + ':' + i;
            activities[userId + '#activity'][activityId] = _mockActivity(activityId, userId);
        }
