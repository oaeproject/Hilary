...
module.exports = function(tenant) {
    ...
    tenant.passport.deserializeUser(function(principal_id, done) {
        principalAPI.getUser(new Context(tenant, {'id': principal_id}), principal_id, function (err, user) {
            done(err, user);
        });
    });
    ...
    tenant.passport.use(new LocalStrategy({'passReqToCallback': true, 'failureRedirect': false},
    ...
    tenant.server.post('/api/auth/login', tenant.passport.authenticate('local'), function(req, res) {
        ...
        // We can't use req.ctx.user() as that will only get added later on.
        // req.user does exist however.
        if (!req.user) {
            return res.send(401);
        }
        principalAPI.getUser(req.ctx, req.user.id, function(err, user) {
            if (!err) {
