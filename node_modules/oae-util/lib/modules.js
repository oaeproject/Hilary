 ...
 */
var bootstrapModules = (module.exports.bootstrapModules = function(
  config,
  callback,
) {
  initAvailableModules(function(err, modules) {
    if (err) {
      return callback(err);
    } else if (_.isEmpty(modules)) {
      return callback(
        new Error('No modules to install, or error aggregating modules.'),
      );
    }
...
var bootstrapModulesInit = function(modules, config, callback) {
  var MODULE_INIT_FILE = '/lib/init.js';
  async.mapSeries(
    modules,
    function(moduleName, done) {
      var moduleInitPath =
        OaeUtil.getNodeModulesDir() + moduleName + MODULE_INIT_FILE;

      if (fs.existsSync(moduleInitPath)) {
        require(moduleName + MODULE_INIT_FILE)(config, function(err) {
          if (err) {
            log().error(err.stack);
            log().error(
              { err: err },
 ...
 * @api private
 */
var bootstrapModulesRest = function(modules, callback) {
  var complete = _.after(modules.length, callback);
  _.each(modules, function(module) {
    var path = OaeUtil.getNodeModulesDir() + module + '/lib/rest.js';
 ...
 */
var initAvailableModules = (module.exports.initAvailableModules = function(
  callback,
) {
  IO.getFileListForFolder(OaeUtil.getNodeModulesDir(), function(err, modules) {
    if (err) {
      return callback(err);
    }
