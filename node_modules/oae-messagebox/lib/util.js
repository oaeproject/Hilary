 ...
 * @param  {String}             objectType          The objectType for the entity
 ...
 */
var transformPersistentMessageActivityEntity = module.exports.transformPersistentMessageActivityEntity = function(ctx, message, context, objectType, profilePath, urlFormat) {
    var tenant = ctx.tenant();
    var entity = _transformMessageActivityEntity(ctx, message, objectType, urlFormat, profilePath);

    // Transform the parent if there is one
    if (context.parent) {
        entity[MessageBoxConstants.activity.PROP_OAE_MESSAGE_REPLY_TO] = _transformMessageActivityEntity(ctx, context.parent, objectType, urlFormat, profilePath);
    }
 ...
 * @api private
 */
var _transformMessageActivityEntity = function(ctx, message, objectType, urlFormat, profilePath) {
    var tenant = ctx.tenant();
    ...
    opts.ext[ActivityConstants.properties.OAE_VISIBILITY] = message.visibility;
    ...
    opts.ext[MessageBoxConstants.activity.PROP_OAE_MESSAGE_THREAD_KEY] = message.threadKey;

    return new ActivityModel.ActivityEntity(objectType, globalId, opts);
};
