 ...
 * @param  {String}             [transformer]           The type of transformer with which the activities should be transformed. One of `ActivityConstants.transformerTypes`. If left null, this will default to `ActivityConstants.transformerTypes.ACTIVITYSTREAMS`
 * @param  {Function}           callback                Invoked when the process completes
 * @param  {Object}             callback.err            An error that occurred, if any
 * @param  {ActivityStream}     callback.activities     The activity stream containing the requested set of activities
 */
var getActivityStream = module.exports.getActivityStream = function(ctx, resourceId, activtyStreamType, start, limit, transformer, callback) {
    var activityStream = getRegisteredActivityStreamType(activtyStreamType);
    transformer = transformer || ActivityConstants.transformerTypes.ACTIVITYSTREAMS;
    ...
    validator.check(transformer, {'code': 400, 'msg': 'Unknown activity transformer'}).isIn(_.values(ActivityConstants.transformerTypes));
    ...
    authorizationHandler(ctx, resourceId, null, function(err) {
        ...
        return _getActivityStream(ctx, resourceId + '#' + activtyStreamType, start, limit, transformer, callback);
    });
};
 ...
 * @param  {String}            transformer             The type of transformer with which the activities should be transformed. One of `ActivityConstants.transformerTypes`
 * @param  {Function}          callback                Invoked when the process completes
 * @param  {Object}            callback.err            An error that occurred, if any
 * @param  {ActivityStream}    callback.activityStream The activity stream
 * @api private
 */
var _getActivityStream = function(ctx, activityStreamId, start, limit, transformer, callback) {
    ActivityDAO.getActivities(activityStreamId, start, limit, function(err, activities, nextToken) {
        if (err) {
            return callback(err);
        }

        ActivityTransformer.transformActivities(ctx, activities, transformer, function(err) {
            if (err) {
                return callback(err);
            }
