                    ...
                    if (opt === colHash[1]) {
                        ...
                        for (var el in modules[ii].config.options[opt].elements) {
                            ...
                            if (el === colHash[2]) {
                                ...
                                modules[ii].config.options[opt].elements[el].value = Cassandra.castToBoolean(value);
                            }
                        }
                    } else {
                        modules[ii].config.options[opt].enabled = Cassandra.castToBoolean(modules[ii].config.options[opt].elements[opt + '-enabled'].value);
                    }
                }
            }
        }
    ...
    var doFilter = function(i) {
        if (modules[i]) {
            fs.exists('node_modules/' + modules[i] + '/config/config.json', function(exists) {
                if (exists) {
                    var configFile = require(modules[i] + '/config/config.json');
                    var configObj = {
                        'id': modules[i],
                        'config': configFile
                    };
