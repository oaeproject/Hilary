...
<<<<<<< HEAD
    _createContent(ctx, 'link', name, description, visibility, additionalMembers, {'link': link}, function(err, content) {
        if (!err) {
...
=======
    _createContent(ctx, 'link', displayName, description, visibility, additionalMembers, {'link': link}, callback);
>>>>>>> master
};
...
var createFile = module.exports.createFile = function(ctx, displayName, description, visibility, additionalMembers, file, callback) {
    ...
    var otherValues = {
        ...
        'size': file.size,
        'filename': file.name,
        'previewStatus': ContentConstants.previews.PENDING
    };
    _createContent(ctx, 'file', displayName, description, visibility, additionalMembers, otherValues, function(err, contentObj) {
        ...
        contentObj.previewStatus = ContentConstants.previews.PENDING;
...
<<<<<<< HEAD
    _createContent(ctx, 'sakaidoc', name, description, visibility, additionalMembers, { 'previewStatus': ContentConstants.previews.PENDING }, function(err, content) {
        if (!err) {
            emitter.emit(ContentConstants.events.CREATED_CONTENT, ctx, content, additionalMembers);
        }
        callback(err, content);
...
=======
    _createContent(ctx, 'sakaidoc', displayName, description, visibility, additionalMembers, {}, callback);
>>>>>>> master
};
...
var _hasAccess = function(ctx, contentObj, callback) {
    ...
    if (ctx.user() && ctx.user().isAdmin(contentObj.tenant)) {
        return callback(null, contentObj);
    }
 ...
 * @param {String}      status          One of ContentConstants.previews .
 * @param {File[]}      files           An array of express file objects. If there is no corresponding 'size_<filename>' in the metadata object, the file will be ignored.
 * @param {Object}      metadata        An object that holds the size for each file and optionally other metadata. For sizes the keys should be of the form 'size_<filename>' and the values one of 'small', 'normal' or 'large'. Other metadata needs to be prefixed by 'meta_'.
 * @param {String}      metadata.status The status of processing on this piece of content. One of ContentConstants.previews.
 * @param {Function}    callback        Standard callback method
 * @param {Object}      callback.err    Standard error object (if any.)
 */
var addPreviewItems = module.exports.addPreviewItems = function(ctx, contentId, status, files, metadata, callback) {
    files = files || {};
    metadata = metadata || {};

    // Check for authentication before we do any expensive checks.
    if (!ctx.user() || !ctx.user().isGlobalAdmin()) {
        return callback({'code': 401, 'msg': 'Only administrators can attach preview items to a content item.'});
    }

    var validator = new Validator();
    validator.check(contentId, {'code': 400, 'msg': 'Missing or invalid contentId'}).isResourceId();
    validator.check(metadata.status, {'code': 400, 'msg': 'The status parameter must be one of done or error'}).isIn(_.values(ContentConstants.previews));
    if (validator.hasErrors()) {
        return callback(validator.getFirstError());
    }
    ...
    getContent(ctx, contentId, function(err, contentObj) {
        ...
        if (status === ContentConstants.previews.ERROR || todo === 0) {
            return _setPreviewMetadata(ctx, contentObj, metadata.status, null, metadata, callback);
        }
        ...
        fileKeys.forEach(function(key) {
            ...
            getStorageBackend(ctx).store(ctx, files[key], function(err, uri) {
                todo--;
                if (err && !called) {
                    called = true;
                    return callback(err);
                ...
                values[files[key].name] = size + '#' + uri;
                ...
                if (todo === 0 && !called) {
                    ...
                    // Store the URIs in Cassandra.
                    log().trace({'files': values}, 'Storing %d content preview files.', fileKeys.length);
                    ...
                    Cassandra.runQuery(q.query, q.parameters, function(err) {
                        ...
                        // Store any other metadata that is passed in the request.
                        _setPreviewMetadata(ctx, contentObj, metadata.status, thumbnailUri, metadata, callback);
                    });
                }
            });
        });
 ...
 * @param {String}      status          The status of the preview generation. One of ContentConstants.previews .
 * @param {String}      thumbnailUri    The uri of a thumbnail
 * @param {Object}      metadata        Each key of the form `meta_<name>` in this object will be stored in cassandra in a column <name> with the hash entry's value as the column value.
 * @param {Function}    callback        Standard callback method
 * @param {Object}      callback.err    Standard error object (if any.)
 */
var _setPreviewMetadata = function(ctx, contentObj, status, thumbnailUri, metadata, callback) {
    var meta_keys = Object.keys(metadata);
    var values = {};
    var hasData = false;
    for (var i = 0; i < meta_keys.length; i++) {
        if (meta_keys[i].substr(0, 5) === 'meta_') {
            var propName = meta_keys[i].substr(5);
            values[propName] = metadata[meta_keys[i]];
            contentObj[propName] = metadata[meta_keys[i]];
            hasData = true;
        }
    }
    if (thumbnailUri) {
        values['thumbnailUri'] = thumbnailUri;
        contentObj.thumbnailUri = thumbnailUri;
    }
    values['previewStatus'] = status;
    var q = Cassandra.constructUpsertCQL('Content', 'contentId', contentObj.contentId, values, 'QUORUM');
    if (!q) {
        log().error('Could not create a correct query to set preview metadata.');
        return callback({'code': 500, 'msg': 'Could not create a correct query.'});
    }
    Cassandra.runQuery(q.query, q.parameters, function(err) {
        if (err) {
            return callback(err);
        ...
        if (status === ContentConstants.previews.DONE) {
            emitter.emit(ContentConstants.events.PREVIEWED_CONTENT, ctx, contentObj.contentId, contentObj);
        }
 ...
 */
var verifySignedPreviewItem = module.exports.verifySignedPreviewItem = function(ctx, contentId, previewItem, signature, expires, callback) {
    var validator = new Validator();
    ...
    validator.check(expires, {'code': 400, 'msg': 'Invalid expires parameter'}).isInt();
    validator.check(signature, {'code': 401, 'msg': 'Invalid signature.'}).verifyExpiringSignature(contentId, expires);
    if (validator.hasErrors()) {
        return callback(validator.getFirstError());
    }
...
/**
 * Constructs and returns a deleted Comment object.
 *
 * @param  {Context}   ctx        Standard context object, representing the currently logged in user and its tenant
 * @param  {Comment}   comment    The comment that was deleted (or marked as deleted)
 * @param  {Boolean}   deleted    `true` if the comment was deleted from Cassandra, `false` if the comment was marked as deleted
 * @api private
 */
var _constructDeletedComment = function(ctx, contentId, comment, deleted) {
    if (deleted){
        return new Comment(comment.commentId, null, contentId, ctx.user(), comment.created, 0, comment.replyTo, true);
    } else {
        return new Comment(comment.commentId, null, null, null, comment.created, comment.level, comment.replyTo, false);
    }
};
 ...
 * @param  {Function}   callback            Standard callback function takes argument `err`
 * @param  {Object}     callback.err        Error object containing the error message
 * @param  {Comment}    callback.deleted    Will return a Comment object with the `deleted` parameter set to `false` if the comment has been marked as deleted, but not deleted from Cassandra. This only happens when the comment has a reply.
 */
                ...
                getComments(ctx, contentId, comment.created, 1, function(err, comments) {
                    ...
                    if (comments.length > 0 && comments[0].replyTo === commentId) {
                        ...
                        Cassandra.runQuery(q, [true, commentId], function(err) {
                            callback(err, _constructDeletedComment(ctx, contentId, comment, false));
                        });
                    } else {
                        var queries = [{
                        ...
                        Cassandra.runBatchQuery(queries, 'QUORUM', function(err) {
                            ...
                            callback(null, _constructDeletedComment(ctx, contentId, comment, true));
                        });
                    }
                });
            });
...
var _rowToContent = function(row) {
    ...
    if (contentObj.contentType === 'file') {
        ...
        contentObj.previewStatus = hash.previewStatus;
        if (hash.pageCount) {
            contentObj.pageCount = parseInt(hash.pageCount, 10);
        }
        if (hash.thumbnailUri) {
            contentObj.thumbnailUri = hash.thumbnailUri;
            contentObj.thumbnailUrl = getThumbnailUrl(hash.contentId);
        }
 ...
 */
var getThumbnailUrl = module.exports.getThumbnailUrl = function(contentId) {
    var signature = Signature.createExpiringSignature(contentId, 5);
    return util.format('/api/content/%s/previews/thumbnail.png?signature=%s&expires=%s', contentId, signature.signature, signature.expires);
};
