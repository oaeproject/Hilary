        ...
        if (replyTo) {
            _getComment(ctx, replyTo, function(err, commentReplyingTo) {
                ...
                Cassandra.runQuery(q, [created, commentId, contentId], function(err) {
                    ...
                    q = 'UPDATE Comments USING CONSISTENCY QUORUM SET body = ?, createdBy = ?, replyTo = ?, level = ?, contentId = ?, created = ? WHERE commentId = ?';
                    Cassandra.runQuery(q, [body, ctx.user().id, commentReplyingTo.commentId, commentReplyingTo.level + 1, contentId, created, commentId], callback);
                });
            });
        } else {
            ...
            Cassandra.runQuery('SELECT * FROM Comments USING CONSISTENCY QUORUM WHERE commentId IN (?)', [commentIds], function (err, commentsRows) {
                ...
                for (var ii = 0; ii < comments.length; ii++) {
                    ...
                    if (comments[ii].deleted === true) {
                        delete comments[ii].body;
                        delete comments[ii].contentId;
                        delete comments[ii].createdBy;
                        delete comments[ii].created;
                        delete comments[ii].level;
                        delete comments[ii].replyTo;
                    }
                }
...
var deleteComment = module.exports.deleteComment = function(ctx, contentId, commentId, callback) {
    ...
    validator.check(commentId, {'code': 400, 'msg': 'A content id must be provided'}).notEmpty();
    if (validator.hasErrors()) {
        return callback(validator.getFirstError());
    }
    ...
    canManage(ctx, contentId, function(err, canManage, contentObj) {
        ...
        _getComment(ctx, commentId, function(err, comment) {
            ...
            getComments(ctx, contentId, comment.created, 2, function(err, comments) {
                if (err) {
                    return callback(err);
                }
