...
var childSearchDocumentMappings = {};
 ...
 *
 * By default each document will have its `_type` set to `SearchConstants.resourceMappingName`. If you've added a mapping for a new
 * type, you can specify it on a search document by giving it the appropriate value for the `_type` key.
 *
...
/**
 * Create a search document mapping in the OAE index that is a child of the main resource schema.
 *
 * @param  {String}     name            The name of the document mapping
 * @param  {Object}     schema          The ElasticSearch schema object
 ...
 */
var registerAndCreateChildSearchDocumentMapping = module.exports.registerAndCreateChildSearchDocumentMapping = function(name, schema, callback) {
    if (childSearchDocumentMappings[name]) {
        throw new Error('Child search document mapping with name "' + name + '" already exists');
    }
    childSearchDocumentMappings[name] = schema;
    return _createChildSearchDocumentMapping(name, schema, callback);
};
...
var _ensureSearchSchema = function(callback, _names) {
    if (!_names) {
        return client.putMapping(SearchConstants.resourceMappingName, require('./schemas/resourceSchema'), {'_source': false}, function(err) {
            if (err) {
                return callback(err);
            }

            return _ensureSearchSchema(callback, _.keys(childSearchDocumentMappings));
        });
    ...
    return _createChildSearchDocumentMapping(name, childSearchDocumentMappings[name], function(err) {
        if (err) {
            return callback(err);
        }
