 ...
 */
var getEmbedInfo = (module.exports.getEmbedInfo = function(
  ctx,
  contentId,
  callback,
) {
  ContentAPI.getContent(ctx, contentId, function(err, content) {
    if (err) {
      ...
      return callback({
        code: 400,
        msg: "This content doesn't have a MediaCore ID",
      });
    }
    ...
    var getEmbedInfoUrl = util.format(
      '/api2/media/%s/embedcode',
      content.previews.mediaCoreId,
    );
    MediaCoreUtil.signedRequest(
      ...
      function(err, res, body) {
        if (err) {
          return callback({
            code: 500,
            msg: 'Error communicating with MediaCore server',
          });
          ...
          return callback({
            code: 500,
            msg: util.format(
              'There was an unexpected error communicating with the media server',
            ),
          });
        }
        ...
        try {
          ...
          return callback({
            code: 500,
            msg: 'Error parsing MediaCore response as JSON',
          });
        }

        // The iframe src comes back unsigned, so we have to sign it and put the new url in
        embedInfo.html = embedInfo.html.replace(
          /src=["']*([^\s"']*)["']*[\s>]/,
          function(match, url) {
            url = url.split('?');
            return (
 ...
 * @param  {Object}     callback.err    An error that occurred, if any
 */
var updateThumbnails = (module.exports.updateThumbnails = function(
  mediaCoreId,
  callback,
) {
  ...
  MediaCoreDAO.getContentRevisionId(mediaCoreId, function(
    err,
    contentRevisionId,
  ) {
    if (err) {
      ...
      log().warn(
        'Attempted to update thumbnails for non-existing MediaCore ID: %s',
        mediaCoreId,
      );
      return callback({
        code: 404,
        msg: 'Non-existing MediaCore ID was provided',
      });
    }

    // Ensure the content item exists
    ContentDAO.Content.getContent(contentRevisionId.contentId, function(
      err,
      content,
    ) {
      if (err) {
      ...
      ContentDAO.Revisions.getRevision(contentRevisionId.revisionId, function(
        err,
        revision,
      ) {
        if (err) {
