...
ActivitySeedResource.fromResource = function(resource) {
    if (!resource.resourceType) {
        throw new Error('oops, not resource type: %s', JSON.stringify(resource, null, 2));
    }

    var resourceData = {};
    resourceData[resource.resourceType] = resource;
    return new ActivitySeedResource(resource.resourceType, resource.id, resourceData);
};
 ...
 */
var ActivityEntity = module.exports.ActivityEntity = function(objectType, id, visibility, opts) {
    opts = opts || {};
    var ext = opts.ext || {};
    delete opts.ext;
    return _.extend({}, ext, opts, {'objectType': objectType, 'id': id, 'oae:visibility': visibility});
};
