...
var updateDiscussion = (module.exports.updateDiscussion = function(
  ...
  Cassandra.runQuery(query.query, query.parameters, function(err) {
    ...
    return callback(
      null,
      _createUpdatedDiscussionFromStorageHash(discussion, storageHash),
    );
  });
});
 ...
 */
var getDiscussionsById = (module.exports.getDiscussionsById = function(
  discussionIds,
  fields,
  callback,
) {
  ...
  if (fields) {
    ...
    query = util.format(
      'SELECT %s FROM "Discussions" WHERE "id" IN ?',
      columns.join(','),
    );
  } else {
  ...
  Cassandra.runQuery(query, parameters, function(err, rows) {
    ...
    var orderedDiscussions = _.map(discussionIds, function(discussionId) {
