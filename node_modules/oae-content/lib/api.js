...
var createLink = module.exports.createLink = function(ctx, displayName, description, visibility, link, additionalMembers, callback) {
    ...
    _createContent(ctx, contentId, 'link', displayName, description, visibility, additionalMembers, {'link': link}, function(err, content) {
        if (!err) {
...
var _createFile = function(ctx, displayName, description, visibility, additionalMembers, file, callback) {
    ...
    getStorageBackend(ctx).store(ctx, file, options, function(err, uri) {
        ...
        ContentDAO.Revisions.createRevision(contentId, ctx.user().id, otherValues, function(err) {
            if (err) {
                return callback(err);
            }

            // Create the content object (remove the uri from the otherValues, as that should not be stored there).
            delete otherValues.uri;
            _createContent(ctx, contentId, 'file', displayName, description, visibility, additionalMembers, otherValues, function(err, contentObj) {
                if (err) {
                    return callback(err);
                }
...
var createCollabDoc = module.exports.createCollabDoc = function(ctx, displayName, description, visibility, additionalMembers, callback) {
    ...
    Etherpad.createPad(contentId, function(err, ids) {
        ...
        _createContent(ctx, contentId, 'collabdoc', displayName, description, visibility, additionalMembers, ids, function(err, content) {
            if (err) {
                return callback(err);
            }
...
var publishCollabDoc = module.exports.publishCollabDoc = function(ctx, contentId, callback) {
    ...
    canManage(ctx, contentId, function(err, canManage, contentObj) {
        ...
        Etherpad.getHTML(contentObj.id, contentObj.etherpadPadId, function(err, html) {
            ...
            var opts = {'etherpadHtml': html};
            ContentDAO.Revisions.createRevision(contentId, ctx.user().id, opts, function(err, revision) {
                if (err) {
                ...
                ContentDAO.Content.updateContent(contentObj, opts, true, function(err, newContentObj) {
                    if (err) {
                    ...
                    newContentObj.etherpadHtml = html;

                    // Emit an event for activities and preview processing
                    ContentAPI.emit(ContentConstants.events.UPDATED_CONTENT, ctx, newContentObj, contentObj);
 ...
 * @api private
 */
var _createContent = function(ctx, contentId, resourceSubType, displayName, description, visibility, additionalMembers, otherValues, callback) {
    callback = callback || function() {};
    ...
    ContentDAO.Content.createContent(contentId, ctx.user().id, resourceSubType, displayName, description, visibility, additionalMembers, otherValues, function(err, contentObj) {
        if (err) {
            return callback(err);
        }
    ...
    canManage(ctx, contentId, function(err, canManage, contentObj) {
        ...
        _getRevision(ctx, contentId, revisionId, function(err, revision) {
            ...
            ContentDAO.Revisions.createRevision(contentId, ctx.user().id, revision, function(err, newRevision) {
                 ...
                 * to the Content CF.
                 */
                var blacklist = ['revisionId', 'contentId', 'createdBy', 'created'];
                var profileFields = _.omit(revision, blacklist);

                ContentDAO.Content.updateContent(contentObj, profileFields, true, function(err) {
                    if (err) {
                        return callback(err);
                    }
                    ...
                    if (contentObj.resourceSubType === 'collabdoc') {
                        Etherpad.setHTML(contentObj.id, contentObj.etherpadPadId, profileFields.etherpadHtml, function(err) {
                            if (err) {
                                return callback(err);
                            }
