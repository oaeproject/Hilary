...
var canShare = module.exports.canShare = function(ctx, resource, targets, role, callback) {
    ...
    resolveEffectivePermissions(ctx, resource, function(err, permissions) {
        ...
        _validateRoleChanges(ctx, resource, targets, {'onlyAdd': true}, function(err, members, emails) {
            ...
            var addedPrincipals = _.union(members.members.added, emails.emailPrincipals.added);
...
var _validateRoleChanges = function(ctx, resource, targets, opts, callback) {
    ...
    // Expand all emails into transient resources
    targets.emailRoles = _.map(targets.emailRoles, function(emailRole) {
        return {
            'principal': _emailToResource(emailRole.email),
            'role': emailRole.role
        };
    });
    ...
    _computeMemberRolesAfterChanges(resource, targets.principalRoles, opts, function(err, memberChanges, memberRoles, members) {
        ...
        _computeInvitationRolesAfterChanges(resource, targets.emailRoles, opts, function(err, emailChanges, emailRoles, emailPrincipals) {
            if (err) {
                return callback(err);
            }

            var addedTargets = _.union(members.added, emailPrincipals.added);
            canInteract(ctx, addedTargets, function(err) {
                ...
                var emailResult = {
                    ...
                    'roles': emailRoles,
                    'emailPrincipals': emailPrincipals
                };
...
var _computeInvitationRolesAfterChanges = function(resource, emailPrincipalRoles, opts, callback) {
    if (_.isEmpty(emailPrincipalRoles)) {
        return callback(null, {}, {}, {});
    }

    var roleChanges = {};
    _.each(emailPrincipalRoles, function(emailPrincipalRole) {
        roleChanges[emailPrincipalRole.principal.email] = emailPrincipalRoles.role;
    });

    var authzId = (resource) ? AuthzUtil.getAuthzId(resource) : null;
    AuthzInvitationsUtil.computeInvitationRolesAfterChanges(resource, rolesChanges, opts, function(err, changes, emailRoles, emails) {
        if (err) {
            return callback(err);
        }

        var principalsByEmail = _.chain(emailPrincipalRoles).pluck('principal').indexBy('email').value();
        var getPrincipalByEmail = _.propertyOf(principalsByEmail);
        var emailResources = {
            'added': _.map(emails.added, getPrincipalByEmail),
            'updated': _.map(emails.updated, getPrincipalByEmail),
            'removed': _.map(emails.removed, getPrincipalByEmail)
        };

        return callback(null, changes, emailRoles, emailResources);
    });
};
