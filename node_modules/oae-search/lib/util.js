...
var getResourceTypeParam = module.exports.getResourceTypeParam = function(val, defaultVal) {
    defaultVal = _.contains(SearchConstants.general.RESOURCE_TYPES, defaultVal)?defaultVal:SearchConstants.general.RESOURCE_TYPE_ALL;
    if (SearchConstants.general.RESOURCE_TYPE_ALL == val ||
        _.contains(SearchConstants.general.RESOURCE_TYPES, val)) {
        return val;
    } else {
        return defaultVal;
    }
...
/**
 * Determine if the given parameter is a valid number parameter. If so, simply return the `val`, otherwise, return `defaultVal`.
 *
 * @param   {String}  val           The value to check
 * @param   {String}  [defaultVal]  The value to return if `val` is not valid. If `defaultVal` is not specified or is not a valid number, then it defaults to `undefined`
 * @return  {String}                `val` if it is valid. `defaultVal` otherwise
 */
var getNumberParam = module.exports.getNumberParam = function(val, defaultVal) {
    if (!_isNumber(defaultVal)) {
...
var createQuery = module.exports.createQuery = function(query, filter, opts) {
    ...
    opts = {
        'from': getNumberParam(opts.start),
        'size': getNumberParam(opts.limit),
        'sort': [
            {'_score': {'order': 'desc'}},
...
/**
 * Determines whether or not the given value is a valid number (excluding NaN).
 *
 * @param   {String|Number|Boolean|Object}    val   The value to check.
