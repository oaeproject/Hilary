 ...
 */
var _updatePrincipalInMembersLibraries = function(ctx, updatedPrincipal, oldPrincipal) {
    ...
    AuthzAPI.getAllRolesForPrincipalAndResourceType(updatedPrincipal.id, AuthzConstants.resourceTypes.GROUP, function(err, allRoles) {
        if (err) {
...
PrincipalsEmitter.when(PrincipalsConstants.events.JOINED_GROUP, function(ctx, group, oldGroup, memberChangeInfo, callback) {
    var joinRole = memberChangeInfo.changes[ctx.user().id];
    var entry = {
        'id': group.id,
 ...
 */
PrincipalsEmitter.on(PrincipalsConstants.events.LEFT_GROUP, function(ctx, group, memberChangeInfo) {
    var leaveRole = memberChangeInfo.roles.before[ctx.user().id];
    var entry = {
        'id': group.id,
...
var _getMembersLibraryRank = function(groupId, principal, role) {
    var libraryTenantAlias = AuthzUtil.getResourceFromId(groupId).tenantAlias;
 ...
 * @api private
 */
var _handleInvalidateMembersLibrary = function(group, membershipsGraph, membersGraph, callback) {
