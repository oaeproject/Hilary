 ...
 */
var canAddItemsToFolder = (module.exports.canAddItemsToFolder = function(
  ctx,
  folder,
  contentItems,
  callback,
  ...
  if (!ctx.user()) {
    return callback({
      code: 401,
      msg: 'You must be authenticated to add items to a folder',
    });
  }
 ...
 */
var getFoldersForContent = (module.exports.getFoldersForContent = function(
  contentId,
  callback,
) {
  AuthzAPI.getAuthzMembers(contentId, null, 10000, function(err, members) {
    if (err) {
      log().error(
        ...
        'Unable to get the members of a piece of content',
      );
      return callback(err);
    }
 ...
 */
var getContentInFolder = (module.exports.getContentInFolder = function(
  folder,
  callback,
) {
  // Get the content items in this folder from the canonical source
  AuthzAPI.getAllRolesForPrincipalAndResourceType(folder.groupId, 'c', function(
    err,
    roles,
  ) {
    if (err) {
