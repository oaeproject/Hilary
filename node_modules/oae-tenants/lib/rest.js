...
OAE.globalAdminServer.post('/api/tenant', function(req, res) {
    req.telemetryUrl = '/api/tenant';
    TenantsAPI.updateTenant(req.ctx, req.body.alias, req.body.name, function(err) {
        if (err) {
            return res.send(err.code, err.msg);
        }
        res.send(200);
...
OAE.globalAdminServer.post('/api/tenant/create', function(req, res) {
    ...
    TenantsAPI.createTenant(req.ctx, req.body.alias, req.body.name, req.body.host, function(err) {
        if (err) {
            return res.send(err.code, err.msg);
        }
        res.send(200, 'New tenant "' + req.body.name + '" has been fired up on host ' + req.body.host);
    });
});
...
OAE.tenantServer.post('/api/tenant', function(req, res) {
    req.telemetryUrl = '/api/tenant';
    TenantsAPI.updateTenant(req.ctx, req.ctx.tenant().alias, req.body.name, function(err) {
        if (err) {
            return res.send(err.code, err.msg);
        }
        res.send(200);
