...
/**
 * Query a set of principal rows from the database, ensuring each one exists
 *
 ...
 *
 * @param  {String[]|String}    email           The email address that is associated with users
 * @param  {Function}           callback        Standard callback function
 * @param  {Object}             callback.err    An error object, if any
 * @param  {User[]}             callback.users  The users that are associated with the email address
 */
var getUsersByEmails = module.exports.getUsersByEmails = function(emails, callback) {
    if (_.isString(emails)) {
        return getUsersByEmails([emails], callback);
    }
