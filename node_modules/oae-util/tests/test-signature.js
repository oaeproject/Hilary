...
describe('Signature', function() {
  ...
  describe('Simple Signing', function() {
     ...
     */
    it('verify signature cannot be verified with different data', function(callback) {
      var signature = Signature.sign({ '0': 'zero', '1': 'one' });
     ...
     */
    it('verify data signature is consistent regardless of key ordering', function(callback) {
      var signature = Signature.sign({ '0': 'zero', '1': 'one' });
     ...
     * objects
     */
    it('verify signing cannot be tampered with different permutations of similar data objects', function(callback) {
      assert.ok(
        !Signature.verify({ '': '', a: '' }, Signature.sign({ a: '' })),
      );
      assert.ok(
        !Signature.verify({ abc: 'def' }, Signature.sign({ abcd: 'ef' })),
      );
      assert.ok(
        !Signature.verify({ abc: 'def' }, Signature.sign({ '': 'abcdef' })),
      );
  ...
  describe('Expiring Signatures', function(callback) {
    /**
     * Test that verifies the expiring signature can be verified and cannot be tampered with
     */
    it('verify expiring signature cannot be verified with different data or expires timestamp', function(callback) {
      var signatureData = Signature.createExpiringSignature({
        '0': 'zero',
        '1': 'one',
      });
      assert.ok(
     ...
     */
    it('verify expiring signature minimum and maximum duration guarantees', function(callback) {
      var now = Date.now();
      var data = { data: 'value' };
      ...
      var signatureData = Signature.createExpiringSignature(data, 1, 10);
     ...
     */
    it('verify expiring signature remains the same when generated within the same rolling window', function(callback) {
      var data = { data: 'value' };
      ...
      var signatureDataFirst = Signature.createExpiringSignature(
        data,
        15 * 60,
        15 * 60,
      );
      ...
      var signatureDataSecond = Signature.createExpiringSignature(
        data,
        15 * 60,
        15 * 60,
      );
      ...
      assert.strictEqual(
        signatureDataFirst.expires,
        signatureDataSecond.expires,
      );
      assert.strictEqual(
        signatureDataFirst.signature,
        signatureDataSecond.signature,
      );
      ...
      var signatureDataThird = Signature.createExpiringSignature(
        data,
        15 * 60,
        15 * 60,
      );
      assert.notEqual(signatureDataSecond.expires, signatureDataThird.expires);
      assert.notEqual(
        signatureDataSecond.signature,
        signatureDataThird.signature,
      );
