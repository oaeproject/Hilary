...
describe('Authentication', function() {
    ...
    describe('Password reset', function() {
         ...
         */
        it('verify that when a user request a password reset, a secret is generated and can be used to set the new password', function(callback) {
            // Create a test user
            TestsUtil.generateTestUsers(camAdminRestContext, 1, function(err, users, user) {
                assert.ok(!err);
                ...
                RestAPI.Authentication.logout(userRestContext, function(err, body, response) {
                    ...
                    RestAPI.Authentication.getResetPasswordSecret(anonymousCamRestContext, username, function(err, body, response) {
                        assert.ok(!err);
                        var loginId = new LoginId(global.oaeTests.tenants.cam.alias, 'local', username);
                        // Ensure secret is saved correctly in db
                        Cassandra.runQuery('SELECT "secret" FROM "AuthenticationLoginId" WHERE "loginId" = ?', [loginId.tenantAlias + ':' + loginId.provider + ':' + loginId.externalId], function(err, rows) {
                            assert.ok(!err);
                            var secret = rows[0].get('secret');
                            var newPassword = 'newPassword';
                            // Check that a new password can be set
                            RestAPI.Authentication.resetPassword(anonymousCamRestContext, username, secret.value, newPassword, function(err) {
                                assert.ok(!err);
                                // Check user can login with new password
                                RestAPI.Authentication.login(anonymousCamRestContext, username, newPassword, function(err) {
                                    assert.ok(!err);
                                    // Verify that we are actually logged in
                                    RestAPI.User.getMe(user.restContext, function(err, meObj) {
                                        assert.ok(!err);
        ...
        /**
         * Test that verifies that an invalid password can't be set
         */
        it('verify that when a user request a password reset, an invalid password can not be set', function(callback) {
            // Create a test user
            TestsUtil.generateTestUsers(camAdminRestContext, 1, function(err, users, user) {
                assert.ok(!err);
                var userRestContext = user.restContext;
