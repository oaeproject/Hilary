...
var importUsers = module.exports.importUsers = function(ctx, tenantAlias, userCSV, authenticationMethod, callback) {
    ...
    var adminCtx = new Context(TenantsAPI.getTenant(tenantAlias), ctx.user());
    ...
    csv().from.path(userCSV.path).to.array(function(data) {
        ...
        var processUser = function(data) {
            ...
            var displayName = user[2] + ' ' + user[1];
            var email = user[3];
            require('oae-authentication').getOrCreateUser(adminCtx, authenticationMethod, externalId, displayName, {
                'email': email
            }, function(err, user) {
                if (user.displayName === externalId && user.displayName !== displayName) {
                    updateUser(adminCtx, user.id, {
                        'displayName': displayName
                    }, function(err) {
                ...
                console.log('Processed ' + displayName);
                console.log('Number of users left to process: ' + data.length);
                if (data.length !== 0) {
                    processUser(data);
                }
            });
