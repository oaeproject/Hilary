...
var log = require('oae-logger').logger('folders-authz');
...
var canAddItemsToFolder = module.exports.canAddItemsToFolder = function(ctx, folder, contentItems, callback) {
    ...
    // Ensure that the current user has access to all the content items
    ContentAuthz.canViewAllContent(ctx, contentItems.slice(), function(err, canViewAll, illegalContentItems) {
        if (err) {
            return callback(err);
        } else if (!canViewAll) {
