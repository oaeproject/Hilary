...
module.exports = function(config, callback) {
    ...
    // Add the terms and conditions filter
    setupTCMiddleware();
...
/**
 * Adds middleware that will check if the user has accepted the
 * terms and conditions for the current tenant if it has been enabled.
 *
 * @api private
 */
var setupTCMiddleware = function() {
    // Allow POSTs to /api/auth/ and /api/user/
    var allowedUrls = /^\/api\/(auth)|(user)\//;

    OAE.tenantServer.use(function(req, res, next) {
        var ctx = req.ctx;
        var user = ctx.user();

        // The T&C middleware is only applicable on logged-in users who try to interact with the system
        // other than authentication and updating their profile
        if (user && req.method === 'POST' && !allowedUrls.test(req.path) && PrincipalsAPI.needsToAcceptTermsAndConditions(req.ctx)) {
            return res.send(409, 'You need to accept the terms and conditions before you can interact with this tenant');
        }
