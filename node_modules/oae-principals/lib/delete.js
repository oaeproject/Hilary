...
var _handleInvalidateGroupOperation = function(group, membershipsGraph, membersGraph, callback) {
    var userIdsToInvalidate = _.chain(membersGraph.getNodes()).pluck('id').filter(PrincipalsUtil.isUser).value();
...
var _invalidateSearch = function(group, membershipsGraph, membersGraph, callback) {
    ...
    SearchAPI.postIndexTask('group', resourceGroupIndexTask, resourceIndexOp, function(err) {
        ...
        // If there group index tasks to invoke, do it
 ...
 * deleted
 */
registerGroupDeleteHandler('memberships-library', _handleInvalidateGroupOperation);

/*!
 * Register a restore handler that purges all memberships libraries containing a group that is
 * being restored
 */
registerGroupRestoreHandler('memberships-library', _handleInvalidateGroupOperation);
