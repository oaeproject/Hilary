...
describe('Memberships Search', function() {
     ...
     */
    it('verify memberships search validation', function(callback) {
        SearchTestsUtil.searchAll(anonymousRestContext, 'memberships', ['not-a-valid-user-id'], null, function(err, results) {
            assert.ok(err);
            assert.equal(err.code, 400);
            assert.ok(!results);

            // When a user or group doesn't exist, we do not return a 404. Not a technical spec but rather just not necessary at this point
            // to query the db to find if the principal exists, as that is otherwise not necessary.
            SearchTestsUtil.searchAll(anonymousRestContext, 'memberships', ['u:camtest:not-an-existing-user'], null, function(err, results) {
                assert.ok(!err);
                assert.strictEqual(results.results.length, 0);

                SearchTestsUtil.searchAll(anonymousRestContext, 'memberships', ['g:camtest:not-an-existing-group'], null, function(err, results) {
                    assert.ok(!err);
                    assert.strictEqual(results.results.length, 0);
