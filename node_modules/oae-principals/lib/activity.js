 ...
 */
ActivityAPI.registerActivityRouter('user', function(activityType, entityType, resource, callback) {
    ...
    return callback(null, [resource.resourceId]);
});

/*!
 * Routes a group entity when an activity has occurred.
 */
ActivityAPI.registerActivityRouter('group', function(activityType, entityType, resource, callback) {
    // Deliver to all the group members, direct or indirect
    ActivityUtil.getAllAuthzMembersByRole(resource.resourceId, function(err, membersByRole) {
        if (err) {
            return callback(err);
        }
