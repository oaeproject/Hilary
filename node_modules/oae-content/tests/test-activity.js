                                        ...
                                        RestAPI.Content.shareContent(jackCtx, link.contentId, [jane.id], function(err) {
                                            ...
                                            RestAPI.Content.updateContent(jackCtx, link.contentId, {'description': 'Super awesome link'}, function(err) {
                                                ...
                                                ActivityTestsUtil.collectAndGetActivityStream(jackCtx, jack.id, null, function(err, activityStream) {
                                                    ...
                                                    ActivityTestsUtil.collectAndGetActivityStream(camAdminRestContext, managerGroup.id, {'_delay': false}, function(err, activityStream) {
                                                        assert.ok(!err);
                                                        assert.ok(_getActivity(activityStream, 'content-create', 'object', link.contentId));
                                                        assert.ok(_getActivity(activityStream, 'content-share', 'target', jane.id));
                                                        assert.ok(_getActivity(activityStream, 'content-update', 'object', link.contentId));
                                                        ...
                                                        ActivityTestsUtil.collectAndGetActivityStream(camAdminRestContext, viewerGroup.id, {'_delay': false}, function(err, activityStream) {
                                                            assert.ok(!err);
                                                            assert.ok(_getActivity(activityStream, 'content-create', 'object', link.contentId));
                                                            assert.ok(!_getActivity(activityStream, 'content-share', 'target', jane.id));
                                                            assert.ok(_getActivity(activityStream, 'content-update', 'object', link.contentId));
                                                            ...
                                                            ActivityTestsUtil.collectAndGetActivityStream(camAdminRestContext, managerGroupMember.id, {'_delay': false}, function(err, activityStream) {
                                                                assert.ok(!err);
                                                                assert.ok(_getActivity(activityStream, 'content-create', 'object', link.contentId));
                                                                assert.ok(_getActivity(activityStream, 'content-share', 'target', jane.id));
                                                                assert.ok(_getActivity(activityStream, 'content-update', 'object', link.contentId));
    ...
    describe('Activity Entity Models', function() {
        ...
        it('verify the content entity model contains the correct content information', function(callback) {
            ...
            var _assertStandardLinkModel = function(entity, contentId) {
                ...
                assert.equal(entity['oae:contentType'], 'link');
            ...
            RestAPI.User.createUser(camAdminRestContext, jackUsername, 'password', 'Jack McJackerson', null, function(err, jack) {
                ...
                RestAPI.Content.createLink(jackCtx, 'Google', 'Google', 'public', 'http://www.google.ca', [], [], function(err, link) {
                    ...
                    ActivityTestsUtil.collectAndGetActivityStream(jackCtx, jack.id, null, function(err, activityStream) {
                        ...
                        _assertStandardLinkModel(entity, link.contentId);
                        assert.ok(!entity.image);
                        assert.ok(!entity['oae:previews']);

                        // Get the global admin context on the camtest tenant
                        RestAPI.Authentication.loginOnTenant(globalAdminRestContext, global.oaeTests.tenants.localhost.alias, function(err, globalTenantAdminRestContext) {
                            assert.ok(!err, JSON.stringify(err, null, 4));

                            // Set the preview to error status
                            RestAPI.Content.setPreviewItems(globalTenantAdminRestContext, link.contentId, 'error', {}, {}, {}, function(err) {
                                assert.ok(!err, JSON.stringify(err, null, 4));
                                ...
                                ActivityTestsUtil.collectAndGetActivityStream(jackCtx, jack.id, {'_delay': false}, function(err, activityStream) {
                                    ...
                                    RestAPI.Content.setPreviewItems(globalTenantAdminRestContext, link.contentId, 'ignored', {}, {}, {}, function(err) {
                                        assert.ok(!err);
                                        ...
                                        ActivityTestsUtil.collectAndGetActivityStream(jackCtx, jack.id, {'_delay': false}, function(err, activityStream) {
                                            ...
                                            RestAPI.Content.setPreviewItems(globalTenantAdminRestContext, link.contentId, 'done', suitable_files, suitable_sizes, {}, function(err) {
                                                assert.ok(!err);
                                                ...
                                                ActivityTestsUtil.collectAndGetActivityStream(jackCtx, jack.id, {'_delay': false}, function(err, activityStream) {
                                                    ...
                                                    assert.notEqual(entity.image.url.indexOf(link.contentId), -1);
        ...
        it('verify content-update activities are pivoted by object', function(callback) {
            ...
            RestAPI.User.createUser(camAdminRestContext, jackUsername, 'password', 'Jack McJackerson', null, function(err, jack) {
                ...
                RestAPI.Content.createLink(jackCtx, 'Google', 'Google', 'public', 'http://www.google.ca', [], [], function(err, googleLink) {
                    ...
                    RestAPI.Content.updateContent(jackCtx, googleLink.contentId, {'displayName': 'The Google'}, function(err) {
                        assert.ok(!err);

                        // Update it a second time as jack, we use this to make sure we don't get duplicates in the aggregation
                        RestAPI.Content.updateContent(jackCtx, googleLink.contentId, {'displayName': 'Google'}, function(err) {
                            assert.ok(!err);

                            // Update it with a different user, this should be a second entry in the collection
                            RestAPI.Content.updateContent(camAdminRestContext, googleLink.contentId, {'displayName': 'Google'}, function(err) {
                                assert.ok(!err);
