...
var storePicture = module.exports.storePicture = function(ctx, principalId, file, callback) {
    ...
    _canManagePrincipal(ctx, principalId, function(err, principal) {
        ...
        // Detect the mimetype of the file, as the one that Express gives us is pulled from the HTTP request.
        // This makes it an untrustworthy piece of information as some browsers are notoriously bad at providing
        // the correct mimetype and it can be spoofed. If the mimetype can be extracted, we default back to
        // application/octet-stream.
        mime(file.path, function(err, type) {
            if (err) {
                log().warn({'err': err, 'file': file, 'principalId': principalId}, 'Could not determine the file type, defaulting to application/octet-stream');
                type = 'application/octet-stream';
            }

            // Override the file type provided by the browsers
            file.type = type;

            // Only images can be uploaded
            if (!_.contains(['image/jpg', 'image/jpeg', 'image/png', 'image/gif', 'image/bmp'], type)) {
...
/**
 * Stores a large picture for a principal and attaches it to the User or Group object in Cassandra.
 * This function assumes that all the parameters have been validated previously.
 * It will auto orient the picture so it can be displayed correctly in all browsers.
 *
 * @param  {Context}     ctx                 Context that holds the current user and tenant
...
var _storePicture = function(ctx, principal, file, callback) {
    ...
    ImageUtil.autoOrient(file.path, {'removeInput': true}, function(err, orientedFile) {
        ...
        var options = {
            ...
            'filename': 'large' + ImageUtil.getImageExtension(file.filename, '.jpg')
        };
        ContentAPI.getStorageBackend(ctx).store(ctx, orientedFile, options, function(err, largePictureUri) {
            if (err) {
                return cleanupOnError(err, orientedFile, callback);
...
var _canManagePrincipal = function(ctx, principalId, callback) {
    ...
    // We'll need to check a group explicitly
        ...
        GroupAPI.canManage(ctx, principalId, function(err, canManage, group) {
            ...
            // Return the updated group profile
            GroupAPI.getFullGroupProfile(ctx, principalId, callback);
        });
    } else {
