 ...
 * @param  {Context}        ctx                     The context of the current request
 * @param  {Resource}       resource                The resource on which to check permission
 * @param  {Object}         [targets]               An object indicating the targets on which to perform checks
 * @param  {String[]}       [targets.emails]        The email address to which invitations for a share should be sent
 * @param  {Principal[]}    [targets.principals]    The existing principals that the current user is sharing with
 * @param  {Function}       callback                Standard callback function
 * @param  {Object}         callback.err            Thrown if the permission check fails
 * @param  {Principal[]}    callback.newMembers     The members that actually get added as a result of the share. This must be used as the final list of permission changes to make to avoid having a share result in user demotion
 * @param  {Object}         callback.memberRoles    An object keyed by principal id whose value are roles of the final membership state of the resource after the shares are applied
 */
var canShare = module.exports.canShare = function(ctx, resource, targets, callback) {
    targets = targets || {};
    targets.principals = targets.principals || [];
    targets.emails = targets.emails || [];
...
var canSetRoles = module.exports.canSetRoles = function(ctx, resource, targets, callback) {
    canManage(ctx, resource, function(err) {
        ...
        _validateRoleChanges(ctx, resource, targets, function(err, memberRoles, newMembers, updatedMembers, removedMembers) {
            if (err) {
                return callback(err);
            } else if (!_.chain(memberRoles).values().contains(AuthzConstants.role.MANAGER).value()) {
                return callback({'code': 400, 'msg': 'The requested change will result in leaving the resource without a manager'});
...
var canCreate = module.exports.canCreate = function(ctx, targets, callback) {
    ...
    _validateRoleChanges(ctx, null, targets, function(err, memberRoles, newMembers) {
        if (err) {
            return callback(err);
        }
 ...
 * @param  {Object}         [callback.err.invalidPrincipals]        If any of the target principals couldn't be associated, this object will be keyed by all principals that failed, and the value wil be the error that caused the failure
 * @param  {Object}         callback.memberRoles                    An object keyed by principal id whose value are roles of the final membership state of the resource after the role changes are applied with the principals
 * @param  {Principal[]}    callback.newMembers                     The member principals being added as a result of this change
 * @param  {Principal[]}    callback.updatedMembers                 The member principals whose roles are being updated as a result of this change
 * @param  {Principal[]}    callback.removedMembers                 The member principals who are being removed as a result of this change
 * @param  {Object}         callback.memberRolesBeforeChanges       The full resource membership before the changes are made
 * @api private
 */
var _validateRoleChanges = function(ctx, resource, targets, callback) {
    targets = targets || {};
    targets.principalRoles = targets.principalRoles || [];
    targets.emailRoles = targets.emailRoles || [];

    var emailResources = _.chain(targets.emailRoles)
        .pluck('email')
    ...
    var roleChanges = {};
    _.each(targets.principalRoles, function(principalRole) {
        roleChanges[principalRole.principal.id] = principalRole.role;
    });

    _computeMemberRolesAfterChanges(resource, roleChanges, function(err, memberRoles, newMemberIds, updatedMemberIds, removedMemberIds, memberRolesBeforeChanges) {
        if (err) {
            return callback(err);
        }

        // Map all member ids to their principal resources
        var principals = _.pluck(targets.principalRoles, 'principal');
...
/**
 * Same as `AuthzAPI.computeMemberRolesAfterChanges` except if the `resource` parameter is
 * unspecified, it will treat the computation as though the resource currently has no membership
 * list.
 *
 * @param  {Resource}   resource        The resource whose memberships to compute using the sample changes
 * @param  {Object}     roleChanges     See `AuthzAPI.computeMemberRolesAfterChanges`
 * @param  {Function}   callback        See `AuthzAPI.computeMemberRolesAfterChanges`
 * @api private
 */
var _computeMemberRolesAfterChanges = function(resource, roleChanges, callback) {
    if (!resource) {
        // If there is no resource, filter down to only members being added as members being removed
        // doesn't really mean anything
        var newMemberRoles = {};
        _.each(roleChanges, function(role, principalId) {
            if (_.isString(role)) {
                newMemberRoles[principalId] = role;
            }
        });
        return callback(null, newMemberRoles, _.keys(newMemberRoles), [], [], {});
    }

    return AuthzAPI.computeMemberRolesAfterChanges(_getAuthzId(resource), roleChanges, callback);
};
...
/**
 * Get the authz id from the resource. The authz id is an id that is used by the resource to
 * represent its membership information in the authz API (e.g., for folders, it is its groupId)
 *
 * @param  {Resource}   resource    The resource whose authz id to get
 * @return {String}                 The authz id of the resource
 * @api private
 */
