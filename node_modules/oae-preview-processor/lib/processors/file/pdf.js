...
var init = module.exports.init = function(config, callback) {
    ...
    var cmd = util.format('%s --help', config.binary);
    ...
    fs.mkdir(pagesDir, function(err) {
        ...
        _splitPDF(ctx, path, pagesDir, function(err, pageFilenames) {
            ...
            var resize = function() {
                ...
                // Convert the pdf to html and test for errors.
                var cmd = util.format('pdf2htmlEX --dest-dir %s %s', pagesDir, pagePath);
                exec(cmd,{'10000':10000}, function (err, stdout, stderr) {
                    if(err){
                    ...
                    var htmlPath=pagePath.substr(0,pagePath.length -3) + 'html';
                    ctx.addPreview(htmlPath,'html');
