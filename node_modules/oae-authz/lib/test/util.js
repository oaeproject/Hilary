...
var assertGetInvitationsSucceeds = module.exports.assertGetInvitationsSucceeds = function(restCtx, resourceType, resourceId, callback) {
    RestAPI.Invitations.getInvitations(restCtx, resourceType, resourceId, function(err, result) {
        assert.ok(!err);
        return callback(result);
    });
};

var assertGetInvitationsFails = module.exports.assertGetInvitationsFails = function(restCtx, resourceType, resourceId, httpCode, callback) {
    RestAPI.Invitations.getInvitations(restCtx, resourceType, resourceId, function(err, result) {
        assert.ok(err);
...
var assertMemberRolesEquals = module.exports.assertMemberRolesEquals = function(before, delta, after) {
    ...
    var resourceIds = _.chain(delta).keys().filter(AuthzUtil.isPrincipalId).value();
    delta = _.pick(delta, resourceIds);
...
var assertEmailRolesEquals = module.exports.assertEmailRolesEquals = function(before, delta, after) {
    ...
    var emails = _.chain(delta)
        ...
        .filter(function(email) {
            return (!AuthzUtil.isPrincipalId(email));
        })
        .value();
    delta = _.pick(delta, emails);
    _assertDeltaEquals(before, delta, after);
};
