...
var createPad = module.exports.createPad = function(contentObj, callback) {
    ...
    log().info({'contentId': contentObj.id}, 'Creating group.');
    client.createGroupIfNotExistsFor(args, function(err, groupData) {
        ...
        log().info({'contentId': contentObj.id, 'groupID': groupData.groupID}, 'Creating group pad.');
        client.createGroupPad(groupPad, function(err, padData) {
            ...
            log().info({'contentId': contentObj.id, 'groupID': groupData.groupID, 'padID': padData.padID}, 'Created an etherpad group and pad.');
 ...
 * @param  {Function}   callback        Standard callback method.
 * @param  {Object}     callback.err    Standard error object (if any.)
 * @param  {String}     callback.html   The HTML for this pad.
 */
var getHTML = module.exports.getHTML = function(contentObj, callback) {
    log().info({'contentId': contentObj.id}, 'Getting HMTL');
    getClient(contentObj.id).getHTML({'padID': contentObj.collabdoc.padID}, function(err, data) {
        if (err) {
            log().error({'err': err, 'name': name, 'contentId': contentObj.id}, 'Could not grab the HTML from etherpad.');
...
var joinPad = module.exports.joinPad = function(ctx, contentObj, callback) {
    ...
    client.createAuthorIfNotExistsFor(args, function(err, author) {
        ...
        log().info(session, 'Creating a session');
        client.createSession(session, function(err, data) {
            ...
            // Construct the URL.
            var host = getServerHost(contentObj.id);
            var url = 'http://' + host + '/p/' + contentObj.collabdoc.padID;
...
/**
 * Get the public hostname where users can reach the pads tied to a specific piece of content.
 *
 * @param  {String} contentId   The ID of the piece of content.
 * @return {String}             The hostname of the server serving the pad for this content.
 */
var getServerHost = module.exports.getServerHost = function(contentId) {
    return _getServer(contentId).host;
};
