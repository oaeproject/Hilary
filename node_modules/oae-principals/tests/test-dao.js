...
describe('Principals DAO', function() {
    ...
    it('verify PrincipalsDAO iterateAll functionality', function(callback) {
        TestsUtil.generateTestUsers(camAdminRestContext, 1, function(err, users) {
            ...
            PrincipalsDAO.iterateAll(null, 100, function(err, principalRows, complete) {
                assert.ok(!err, JSON.stringify(err, null, 4));

                // Ensure we found the user when complete
                if (complete) {
                    ...
                    // We aren't complete, so we'll have more iterations. Do not continue to the next part of the test yet
                    return;
                }
                ...
                PrincipalsDAO.iterateAll(['principalId', 'displayName'], 100, function(err, principalRows, complete) {
                    assert.ok(!err, JSON.stringify(err, null, 4));
