 ...
 */
var refreshConfiguration = (module.exports.refreshConfiguration = function(
  config,
  callback,
) {
  config = _.extend(
 ...
 */
var collectAndGetActivityStream = (module.exports.collectAndGetActivityStream = function(
  restCtx,
  resourceId,
  opts,
  callback,
 ...
 */
var collectAndGetNotificationStream = (module.exports.collectAndGetNotificationStream = function(
  restCtx,
  opts,
  callback,
) {
 ...
 */
var assertGetActivityStreamFails = (module.exports.assertGetActivityStreamFails = function(
  restCtx,
  resourceId,
  opts,
  code,
  ...
  RestAPI.Activity.getActivityStream(restCtx, resourceId, opts, function(
    err,
    activityStream,
  ) {
    assert.ok(err);
 ...
 */
var markNotificationsAsRead = (module.exports.markNotificationsAsRead = function(
  restContext,
  callback,
) {
  var result = null;

  ActivityAPI.once(ActivityConstants.events.RESET_AGGREGATION, function(
    activityStreams,
  ) {
 ...
 */
var assertFeedContainsActivity = (module.exports.assertFeedContainsActivity = function(
  restContext,
  activityStreamId,
  activityType,
  verb,
  ...
  collectAndGetActivityStream(restContext, activityStreamId, null, function(
    err,
    response,
  ) {
    assert.ok(!err);
    var activity = _.findWhere(response.items, {
      'oae:activityType': activityType,
    });
    assert.ok(activity);
 ...
 */
var assertFeedDoesNotContainActivity = (module.exports.assertFeedDoesNotContainActivity = function(
  restContext,
  activityStreamId,
  activityType,
  callback,
 ...
 * @throws {Error}                              An assertion error gets thrown if the activity was not found or some of its properties are not what was expected
 */
var _assertNotificationStreamContainsActivity = (module.exports.assertNotificationStreamContainsActivity = function(
  restContext,
  activityType,
  verb,
  actorId,
  ...
  collectAndGetNotificationStream(restContext, null, function(
    err,
    notificationStream,
  ) {
    assert.ok(!err);
    var activity = _.findWhere(notificationStream.items, {
      'oae:activityType': activityType,
    });
    assert.ok(activity);
    assertActivity(activity, activityType, verb, actorId, objectId, targetId);
 ...
 * @throws {Error}                              An assertion error gets thrown if the activity was found
 */
var _assertNotificationStreamDoesNotContainActivity = (module.exports.assertNotificationStreamDoesNotContainActivity = function(
  restContext,
  activityType,
  callback,
) {
 ...
 */
var assertActivity = (module.exports.assertActivity = function(
  activity,
  activityType,
  verb,
  actorEntityId,
...
var _assertActivityEntity = function(activityEntity, entityId) {
  if (!entityId) {
    ...
    _.each(
      activityEntity[ActivityConstants.properties.OAE_COLLECTION],
      function(activityEntity) {
        assert.ok(
          _.contains(
            entityIds,
            activityEntity[ActivityConstants.properties.OAE_ID],
          ),
        );
      },
    );
 ...
 */
var parseActivityHtml = (module.exports.parseActivityHtml = function(html) {
  var $ = cheerio.load(html);

  var activities = [];
  $('tr.activity-row').each(function() {
    var $activity = $(this);
    var $summary = $activity.find(
      '.activity-summary-container > div:first-child',
    );

    // Get the id and the summary of this activity
    var activityId = $activity.attr('data-activity-id');
    var summary = {
      html: $summary.html(),
      text: $summary.text(),
      links: [],
...
var getPushClient = (module.exports.getPushClient = function(callback) {
  ...
  var client = new EmitterAPI.EventEmitter();
  ...
  socket.on('data', function(msg) {
    ...
    try {
      ...
      assert.fail(
        err,
        null,
        'Did not expect malformed JSON as a response from the server',
      );
    }
 ...
 */
var getFullySetupPushClient = (module.exports.getFullySetupPushClient = function(
  data,
  callback,
) {
 ...
 */
var waitForPushActivity = (module.exports.waitForPushActivity = function(
  client,
  activityType,
  verb,
  actorId,
  ...
  var _onMessage = function(message) {
    var targetActivity = _.find(message.activities, function(activity) {
      if (
        activity[ActivityConstants.properties.OAE_ACTIVITY_TYPE] !==
        activityType
      ) {
        return false;
      } else if (activity.verb !== verb) {
        return false;
      } else if (
        activity.actor[ActivityConstants.properties.OAE_ID] !== actorId
      ) {
        return false;
      } else if (
        objectId &&
        activity.object[ActivityConstants.properties.OAE_ID] !== objectId
      ) {
        return false;
      } else if (targetId) {
        if (!activity.target) {
          ...
          activity.target[ActivityConstants.properties.OAE_ID] !== targetId
        ) {
          return false;
        }
      }
