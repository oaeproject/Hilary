...
var bootstrapModulesInit = function(modules, config, callback) {
    ...
    async.mapSeries(modules, function (moduleName, done) {
        var moduleInitPath = OaeUtil.getNodeModulesDir() + moduleName + MODULE_INIT_FILE;

        if(fs.existsSync(moduleInitPath)) {
            require(moduleName + MODULE_INIT_FILE)(config, function(err) {
                if (err) {
                    log().error(err.stack);
                    log().error({err: err}, 'Error initializing module %s', moduleName);
                    return callback(err);
                }
