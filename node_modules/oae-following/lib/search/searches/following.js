...
module.exports = function(ctx, opts, callback) {
    ...
    validator.check(opts.userId, {'code': 400, 'msg': 'Must specificy an id of a user to search their following list'}).isUserId();
    if (validator.hasErrors()) {
        return callback(validator.getFirstError());
    }

    PrincipalsDAO.getPrincipal(opts.userId, function(err, user) {
        if (err) {
            return callback(err);
        }

        FollowingAuthz.canViewFollowing(ctx, user, function(err, canViewFollowing) {
            if (err) {
                return callback(err);
            } else if (!canViewFollowing) {
                return callback({'code': 401, 'msg': 'You are not authorized to view the list of users this user follows'});
            }
...
var _search = function(ctx, opts, callback) {
    ...
    var filter = SearchUtil.filterAnd(
        ...
        SearchUtil.filterTerms('resourceType', ['user']),
        SearchUtil.createHasChildQuery(FollowingConstants.search.MAPPING_RESOURCE_FOLLOWERS, SearchUtil.filterTerms('followers', [opts.userId]))
    );
