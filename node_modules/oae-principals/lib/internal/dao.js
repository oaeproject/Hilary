...
var deletePrincipal = module.exports.deletePrincipal = function(principalId, callback) {
    ...
    // Currently, only deleting groups is supported
    validator.check(principalId, {'code': 400, 'msg': 'Only group principals can be deleted'}).isGroupId();
    if (validator.hasErrors()) {
        return callback(validator.getFirstError());
    }
    ...
    AuthzDelete.isDeleted([principalId], function(err, wasDeleted) {
        ...
        AuthzDelete.setDeleted(principalId, function(err) {
            ...
            Cassandra.runQuery('UPDATE "Principals" SET "deleted" = ? WHERE "principalId" = ?', [deleted, principalId], function(err) {
                if (err) {
...
var restorePrincipal = module.exports.restorePrincipal = function(principalId, callback) {
    ...
    // Currently, only deleting groups is supported
    validator.check(principalId, {'code': 400, 'msg': 'Only group principals can be restored'}).isGroupId();
    if (validator.hasErrors()) {
        return callback(validator.getFirstError());
    }
    ...
    AuthzDelete.isDeleted([principalId], function(err, wasDeleted) {
        ...
        AuthzDelete.unsetDeleted(principalId, function(err) {
            ...
            Cassandra.runQuery('DELETE "deleted" FROM "Principals" WHERE "principalId" = ?', [principalId], function(err) {
                if (err) {
 ...
 * @param  {String}     [token]         The token that was used to verify ownership of the email address
 * @param  {Function}   callback        Standard callback function
 * @param  {Object}     callback.err    An error object, if any
 * @param  {User}       callback.user   The updated user
 */
var setEmailAddress = module.exports.setEmailAddress = function(user, email, token, callback) {
    var queries = [];

    // Mark the email address as verified
    var principalsUpdate = {
        'email': email,
        'emailVerified': true,
        'emailChange': ''
    };
...
var _getUserFromRow = function(row) {
    ...
    return _hashToUser(Cassandra.rowToHash(row));
};
