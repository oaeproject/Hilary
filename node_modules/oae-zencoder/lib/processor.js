...
/**
 * Set the default config. This can be overridden for individual tenants in the admin interface
 *
...
var generatePreviews = module.exports.generatePreviews = function(ctx, content, callback) {
    ...
    var mimeType = revision.mime;
    log().info(config.apiKey)

    if (revision.previews && revision.previews.zencoderId) {
        return callback();
    }

    log().info(ctx, content);

    ctx.download(function(err, downloadPath) {
        log().info('Generating previews for video file %s', content.id);
        var tenantAlias = content.tenant.alias;
    
        // Create a Zencoder job
        ZencoderUtil.job(config, tenantAlias, downloadPath, content.id, revision.id, _isVideo(mimeType), function(err, res, body) {
            ...
            ZencoderDAO.saveContentRevisionId(output.id, ctx.contentId, ctx.revisionId, function(err) {
                if (err) {
                    return callback(err);
                }
            });
