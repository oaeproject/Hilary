...
var init = module.exports.init = function(callback) {
    ...
    SearchAPI.registerChildSearchDocument(AuthzConstants.search.MAPPING_RESOURCE_MEMBERS, membersChildSearchDocumentOptions, function(err) {
        if (err) {
            return callback(err);
        }

        return SearchAPI.registerChildSearchDocument(AuthzConstants.search.MAPPING_RESOURCE_MEMBERSHIPS, membershipsChildSearchDocumentOptions, callback);
    });
};
 ...
 */
var fireMembershipUpdateTasks = module.exports.fireMembershipUpdateTasks = function(memberIds) {
...
var _produceResourceMembersDocuments = function(resources, callback, _documents) {
    ...
    _getMemberIds(resource, function(err, memberIds) {
        ...
        _documents.push(SearchUtil.createChildSearchDocument(AuthzConstants.search.MAPPING_RESOURCE_MEMBERS, resource.id, {'direct_members': memberIds}));
...
var _produceResourceMembershipsDocuments = function(resources, callback, _documents) {
    ...
    _getMembershipIds(resource, function(err, membershipIds) {
        ...
        _documents.push(SearchUtil.createChildSearchDocument(AuthzConstants.search.MAPPING_RESOURCE_MEMBERSHIPS, resource.id, {'direct_memberships': membershipIds}));
...
var _getMembershipIds = function(resource, callback) {
    ...
    AuthzAPI.getRolesForPrincipalsAndResourceType([resource.id], AuthzConstants.resourceTypes.GROUP, 10000, function(err, principalGroupRole) {
        if (err) {
            return callback(err);
        }
        ...
        var memberships = {};
        _.each(principalGroupRole, function(groupRole, principalId) {
            _.each(groupRole, function(role, groupId) {
