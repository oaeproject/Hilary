 ...
 */
var writeConfig = module.exports.writeConfig = function(ctx, tenantId, configValues, callback) {
    if (!ctx.user() || !ctx.user().isAdmin(tenantId)) {
    ...
    for (var i = 0; i < _.keys(configValues).length; i++) {
        ...
        validator.check(configValues[_.keys(configValues)[i]].toString(), {'code': 400, 'msg': 'The configuration value for "' + _.keys(configValues)[i] + '" cannot be null'}).notNull();
...
var mergeStoredConfigIntoOriginal = function(cassandraConfig, moduleConfig) {
    ...
    /**
     * Checks if the config element matched with the colHash has elements and returns them if so, if there are no elements false is returned.
     * @param  {Array}             colHash    colHash looks like: ['moduleid', 'optionid', 'elementid']. e.g. ['oae-authentication', 'twitter', 'enabled'] and is used to get to the correct elements in the configuration
     * @return {Object|Boolean}               Returns the config elements if there are any, if no elements `false` is returned.
     */
    var getConfigElement = function(colHash) {
        if (moduleConfig[colHash[0]] && moduleConfig[colHash[0]][colHash[1]] && moduleConfig[colHash[0]][colHash[1]].elements[colHash[2]]) {
            return moduleConfig[colHash[0]][colHash[1]].elements[colHash[2]];
        } else {
            return false;
        }
    };
    ...
    for (var r = 0; r < cassandraConfig.length; r++) {
        ...
        for (var c = 0; c < cassandraConfig[r].count; c++) {
            if (cassandraConfig[r][c].name !== 'tenantId') {
                ...
                var configElement = getConfigElement(colHash);
                if (configElement) {
                    ...
                    if (schema && _.isObject(parsedValue)) {
                    ...
                    // If the type is of `treestructure` we don't extend but just assign
                    // When there's more than one type that doesn't require the default configuration to be merged with the saved values we can introduce an array to keep track.
                    if (configElement.type === 'treestructure') {
                        configElement.defaultValue = parsedValue;
                    } else {
                        cloneextend.extend(configElement, {'defaultValue': parsedValue});
                    }
                }
            }
        }
