 ...
 * @param  {Object}     callback.embedInfo          A JSON object with embedding info for MediaCore-hosted content
 * @param  {String}     callback.embedInfo.html     The HTML source that can be used to embed this content item's MediaCore video in a page
 * @param  {String}     callback.embedInfo.url      The URL to the video hosted by MediaCore
 */
var getEmbedInfo = module.exports.getEmbedInfo = function(ctx, contentId, callback) {
    ContentAPI.getContent(ctx, contentId, function(err, content) {
        ...
        var getEmbedInfoUrl = util.format('/api2/media/%s/embedcode', content.previews.mediaCoreId);
        MediaCoreUtil.signedRequest(content.tenant.alias, 'get', getEmbedInfoUrl, null, null, function(err, res, body) {
            if (err) {
            ...
            embedInfo.html = embedInfo.html.replace(/src=["']*([^\s"']*)["']*[\s>]/, function(match, url) {
                url = url.split('?');
