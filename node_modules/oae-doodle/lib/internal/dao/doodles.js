...
exports.removeUser = function (loodleId, userId, currentUserToken, callback) {
    ...
    async.waterfall([
        ...
        function (user, done) {
            _removeUser(loodleId, user.id, currentUserToken, done);
        }
    ], callback)

};

var _removeUser = function (loodleId, userId, token, callback) {

    // '/loodle/:loodleId/user/:userId';
...
var addUserIfNeeded = exports.addUserIfNeeded = function (loodleId, userId, currentUserToken, callback) {
    ...
    async.waterfall([
        ...
        function getUserData (token, done) {
            UserDAO.get(token, function (err, data) {
                user = data;
                return done(err, data);
            });
        },
        ...
        function checkIfInLoodle (userData, done) {
            UserDAO.isInLoodle(userData.id, loodleId, token, done);
        },
        // If not add him
        function AddIfNeeded (isInLoodle, done) {
            if (isInLoodle) return done();
            addUserToLoodle(currentUserToken, loodleId, userId, done);
        }
    ], callback);

};

var getUserDataFromUserId = function (userId, callback) {

    async.waterfall([
        // Get user token
        async.apply(UserDAO.getToken, userId),
        // Get user data from mirror account
        UserDAO.get
    ], callback);
 ...
 * @param callback
 */
var addUserToLoodle = exports.addUserToLoodle = function (currentUserToken, loodleId, userId, callback) {
    ...
    async.series([
        ...
        function addUser (done) {
            _addUser(loodleId, user.id, currentUserToken, done);
        }
    ], callback);
