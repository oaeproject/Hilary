...
var getOrCreateUser = function(tenant, username, firstName, lastName, displayName, callback) {
    ...
    principalAPI.getTenantUser(tenant, username, function(err, user) {
        if (err && err.code === 404) {
            ...
            principalAPI.createUser(tenant, username, password, "public", firstName, lastName, displayName, function(err, principal_id) {
                if (err) {
                    return callback(err);
                }
                else {
                    // Get the meta data for this user.
                    principalAPI.getTenantUser(tenant, username, callback);
                }
            });
        } else if (err) {
            return callback(err);
