 ...
 * @param  {Function}   callback                        Standard callback function
 * @param  {Object}     callback.err                    An error that occurred, if any
 * @param  {String[]}   callback.memberships            An array of sorted group ids that represents the full list of direct and indirect memberships for the provided principal
 * @param  {String[]}   callback.indirectMemberships    An array of sorted group ids that represents only the groups in which the principal is a member indirectly
 * @api private
 */
...
var getIndirectPrincipalMemberships = module.exports.getIndirectPrincipalMemberships = function(principalId, start, limit, callback) {
    ...
    // Groups their indirect member ship isn't cached, so we'll need to resolve this by exploding it
    if (AuthzUtil.isGroupId(principalId)) {
        _getIndirectPrincipalMembershipsByExplosion(principalId, start, limit, callback);
    } else {
        _getIndirectPrincipalMembershipsFromCache(principalId, start, limit, callback);
    }
};
