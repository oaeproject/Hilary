...
var createDiscussion = module.exports.createDiscussion = function(ctx, displayName, description, visibility, roles, opts, callback) {
    ...
    ResourceActions.create(ctx, roles, createFn, function(err, discussion, members) {
        if (err) {
            return callback(err);
        }

        // Index the discussion into user libraries
        _insertLibrary(_.keys(members.changes), discussion, function(err) {
            if (err) {
                return callback(err);
            }

            DiscussionsAPI.emit(DiscussionsConstants.events.CREATED_DISCUSSION, ctx, discussion, members.changes);
...
var shareDiscussion = module.exports.shareDiscussion = function(ctx, discussionId, principalIds, callback) {
    ...
    _getDiscussion(discussionId, function(err, discussion) {
        ...
        ResourceActions.share(ctx, discussion, principalIds, AuthzConstants.role.MEMBER, function(err, members) {
            if (err) {
                return callback(err);
            } else if (_.isEmpty(members.changes)) {
                return callback();
            }

            var addedMemberIds = _.keys(members.changes);
            ...
            OaeUtil.invokeIfNecessary(_testDiscussionUpdateThreshold(discussion), _touch, discussion, function(err, touchedDiscussion) {
                ...
                _insertLibrary(addedMemberIds, discussion, function(err) {
                    ...
                    DiscussionsAPI.emit(DiscussionsConstants.events.UPDATED_DISCUSSION_MEMBERS, ctx, discussion, members.changes, addedMemberIds);
                    return callback();
                });
            });
        });
...
var setDiscussionPermissions = module.exports.setDiscussionPermissions = function(ctx, discussionId, changes, callback) {
    ...
    validator.check(discussionId, {'code': 400, 'msg': 'A valid discussion id must be provided'}).isResourceId();
    validator.check(null, {'code': 400, 'msg': 'Must specify at least one permission change to apply'}).isObject(changes);
    validator.check(_.keys(changes).length, {'code': 400, 'msg': 'You must specify at least one permission change'}).min(1);
    ...
    _getDiscussion(discussionId, function(err, discussion) {
        ...
        ResourceActions.setRoles(ctx, discussion, changes, function(err, members) {
            if (err) {
                return callback(err);
            }

            var addedMemberIds = _.pluck(members.members.added, 'id');
            var updatedMemberIds = _.pluck(members.members.updated, 'id');
            var removedMemberIds = _.pluck(members.members.removed, 'id');
            ...
            _removeLibrary(removedMemberIds, discussion, function(err) {
                ...
                OaeUtil.invokeIfNecessary(touchDiscussion, _touch, discussion, function(err, touchedDiscussion) {
                    ...
                    _insertLibrary(addedMemberIds, discussion, function(err) {
                        ...
                        var libraryUpdateIds = _.keys(members.roles.before);
            ...
            DiscussionsAPI.emit(DiscussionsConstants.events.UPDATED_DISCUSSION_MEMBERS, ctx, discussion, members.changes, addedMemberIds, updatedMemberIds, removedMemberIds);
            return callback(null, members.roles.after);
        });
    });
};
...
var removeDiscussionFromLibrary = module.exports.removeDiscussionFromLibrary = function(ctx, libraryOwnerId, discussionId, callback) {
    ...
    _getDiscussion(discussionId, function(err, discussion) {
        ...
        PrincipalsDAO.getPrincipal(libraryOwnerId, function(err, principal) {
            ...
            AuthzPermissions.canRemoveRole(ctx, principal, discussion, function(err, changes, roles) {
                if (err) {
                    return callback(err);
                }

                // All validation checks have passed, finally persist the role change and update the user library
                AuthzAPI.updateRoles(discussionId, changes, function(err) {
                    if (err) {
                        return callback(err);
                    }

                    DiscussionsAPI.emit(DiscussionsConstants.events.UPDATED_DISCUSSION_MEMBERS, ctx, discussion, changes, [], [], [libraryOwnerId]);
