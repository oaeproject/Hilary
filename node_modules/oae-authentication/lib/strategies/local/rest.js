...
/**
 * Handles the callback where a user has successfully authenticated to the system
 *
 * @param  {Request}    req             The express request object
 * @param  {User}       req.user.user   The authenticated user
 * @param  {Response}   res             The express response object
 * @api private
 */
...
/**
 * Handles the request to determine if a username exists
 *
 * @param  {Request}    req                         The express request object
...
var _handleLocalUsernameExists = function(req, res) {
    AuthenticationAPI.localUsernameExists(req.ctx, req.params.tenantAlias, req.params.username, function(err, exists) {
        ...
        var responseCode = (exists) ? 200 : 404;
...
OAE.globalAdminRouter.on('post', '/api/auth/createGlobalAdminUser', function(req, res) {
    ...
    AuthenticationAPI.getOrCreateGlobalAdminUser(req.ctx, req.body.username, req.body.password, req.body.displayName, function(err, user, created) {
        if (err) {
            return res.send(err.code, err.msg);
        } else if (!created) {
