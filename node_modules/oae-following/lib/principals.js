 ...
 */
PrincipalsAPI.registerFullUserProfileDecorator('following', function(ctx, user, callback) {
    if (!ctx.user()) {
        return callback();
    ...
    FollowingDAO.isFollowing(ctx.user().id, [user.id], function(err, following) {
        if (err) {
            return callback(err);
        }

        // If the user is following them, we can't follow anymore, so we have all our answers
        var isFollowing = following[user.id];
        if (isFollowing) {
        ...
        FollowingAuthz.canFollow(ctx, user, function(err, canFollow) {
            if (err) {
                return callback(err);
            }
 ...
 * @api private
 */
var _createFullUserProfileDecoration = function(canFollow, isFollowing) {
    return {
