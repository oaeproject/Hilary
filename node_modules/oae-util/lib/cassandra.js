...
module.exports.init = function(config, callback) {
    ...
    client.connect({'address': config.hosts}, function(err) {
        ...
        createKeyspace(keyspace, function(err) {
            ...
            // TODO: Create the keyspace if it doesn't exist yet
...
var createKeyspace = module.exports.createKeyspace = function(name, callback) {
    ...
    runQuery(util.format('CREATE KEYSPACE "%s" WITH REPLICATION = %s', name, replication), null, function(err) {
...
var dropKeyspace = module.exports.dropKeyspace = function(name, callback) {// TODO: Implement
    callback = callback || function() {};
    runQuery(util.format('DROP KEYSPACE "%s"', name), null, function(err) {
        if (err) {
            return callback(err);
        }
...
var columnFamilyExists = module.exports.columnFamilyExists = function(name, callback) {
    ...
    runQuery(cql, [CONFIG.keyspace, name], function(err, rows) {
        if (err) {
            return callback(err);
        }
...
var dropColumnFamily = module.exports.dropColumnFamily = function(name, callback) {
    ...
    columnFamilyExists(name, function(err, exists) {
        ...
        return runQuery(util.format('DROP TABLE "%s"', name), [], callback);
    });
};
...
var runInQuery = module.exports.runInQuery = function(queryWithoutWhere, parametersWithoutWhere, key, inArray, callback) {
    ...
    return runQuery(util.format('%s WHERE "%s" IN (%s)', queryWithoutWhere, key, inPlaceholders.join(',')), parameters, callback);
};
...
var prepare = function(query, callback) {
    if (cachedPreparedQueries[query]) {
        ...
        client.prepare(query, function(err, preparedQuery) {
            ...
            // TODO: Rather than caching everything, be a little smarter about how many queries
            // should be cached (or pruning very infrequent queries or ..)
...
var executeQuery = function(query, parameters, callback) {
    ...
    prepare(query, function(err, preparedQuery) {
        if (err) {
            log().error({
                'err': err,
                'query': query
