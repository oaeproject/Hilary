        ...
        try {
            ...
            _.each(doc.endpoints, function(endpoint, nickname) {
                ...
                _.each(endpoint.pathParams, function(pathParam) {
                    endpoint.parameters.push(SwaggerParam.path(pathParam.name, pathParam.description, _convertRJDArrayDefToSwagger(pathParam.type), pathParam.allowableValues, pathParam.defaultValue));
                });
                _.each(endpoint.queryParams, function(queryParam) {
                    endpoint.parameters.push(SwaggerParam.query(queryParam.name, queryParam.description, _convertRJDArrayDefToSwagger(queryParam.type), queryParam.required, queryParam.allowMultiple, queryParam.allowableValues, queryParam.defaultValue));
                });
            ...
            log().warn('Could not parse restjsdoc in ' + filename + ' ' + err.message);
        }
        return callback();
    });
};
