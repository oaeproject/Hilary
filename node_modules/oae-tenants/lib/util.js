...
var getAllTenantsForInteraction = module.exports.getAllTenantsForInteraction = function(tenantAlias) {
    ...
    _.each(tenants, function(tenant, alias) {
        if (!isPrivate(alias)) {
...
/**
 * Get the tenant for which the provided domain matches the configured tenant email domain
 *
 * @param  {String}         domain      The domain to test against
 * @return {Tenant}                     The first tenant that matched the given domain
 */
var getTenantForEmailDomain = module.exports.getTenantForEmailDomain = function(domain) {
    // An empty domain can't match any tenant
    if (!domain) {
        return null;
    }

    // Find the first tenant that matches the given domain
    var tenant = _.find(TenantsAPI.getTenants(true), function(tenant, tenantAlias) {
        // Get the configured email domain regex
        var emailDomainRegex = TenantsConfig.getValue(tenantAlias, 'domains', 'email');

        // If a domain regex was configured, we match it against the passed in domain
        return (emailDomainRegex && domain.match(emailDomainRegex));
    });

    // Return a copy of the tenant object
    return clone(tenant);
};
...
var canInteract = module.exports.canInteract = function(tenantAliasOne, tenantAliasOther) {
    // You can always interact with a guests tenant
    var guestsTenant = TenantsAPI.getGuestsTenant();
    if (guestsTenant.alias === tenantAliasOne || guestsTenant.alias === tenantAliasOther) {
        return true;
    }
