...
/**
 * Regenerate a preview for a folder if the adedd or removed content items
...
ContentAPI.on(ContentConstants.events.UPDATED_CONTENT_PREVIEW, function(content) {
    ...
    AuthzAPI.getAuthzMembers(content.id, null, 10000, function(err, members) {
        if (err) {
            log().error({'err': err, 'contentId': content.id}, 'Unable to get the members of a piece of content');
            return;
        }

        _reprocessFolders(members);
    });
});
 ...
 *
 * @param  {String[]}   principalIds    A set of principal IDs that may or may not contain group ids that map to folders
 */
var _reprocessFolders = function(principalIds) {
    // Folders are masked as groups in AuthZ
    var groupIds = _.chain(principalIds)
