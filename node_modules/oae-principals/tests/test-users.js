...
describe('Principals', function() {
    ...
    describe('Users', function() {
        ...
        var createUser = function(callback) {
            ...
            PrincipalsAPI.createUser(anonymousContext, loginId, password, displayName, opts, function(err, id) {
                if (err) {
                    assert.fail("Creating a new user should be possible.");
                    return assert.done();
                }

                PrincipalsAPI.getUserIdByLoginId(loginId, anonymousContext.tenant().alias, function(err, userId) {
                    assert.ok(!err, err);
                    assert.equal(id, userId);

                    PrincipalsAPI.getUser(anonymousContext, id, function(err, user) {
                        assert.equal(displayName, user.displayName);
                        assert.equal(opts.visibility, user.visibility);
                        assert.equal(opts.locale, user.locale);
                        assert.equal(opts.timezone, user.timezone);

                        // Try to create one again.
                        PrincipalsAPI.createUser(anonymousContext, loginId, password, displayName, opts, function(err, foo) {
                            if (!err) {
        ...
        it('Test check password', function(callback) {
            createUser(function(createdUser) {
                PrincipalsAPI.checkPassword(createdUser.id, 'password', function(err, pwUser) {
                    assert.ok(!err);
                    assert.equal(createdUser.id, pwUser.id);
                    PrincipalsAPI.checkPassword('u:cam:none_existing_user', 'password', function(err, pwUser) {
        ...
        it('Verify that a tenant admin can only update users within his own tenant.', function(callback) {
            ...
            createUser(function(john) {
                ...
                createUser(function(jack) {
                    ...
                    PrincipalsAPI.createUser(anonymousContextTenantB, new LoginId('local', 'jane'), 'password', 'displayName', {}, function(err, id) {
                                        ...
                                        PrincipalsAPI.getUser(jackContext, jack.id, function(err, user) {
                                            ...
                                            PrincipalsAPI.changePassword(johnContext, jack.id, '', 'foobar', function(err) {
                                                ...
                                                PrincipalsAPI.updateUser(johnContext, jane.id, {
                                                    ...
                                                    PrincipalsAPI.changePassword(johnContext, jane.id, 'wrong-password', 'foobar', function(err) {
                                                        assert.equal(err.code, 401);
                                                        callback();
                                                    });
                                                });
