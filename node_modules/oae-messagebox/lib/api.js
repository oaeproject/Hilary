...
var _replaceLinks = function(body) {
    ...
    var aliases = _.keys(tenants);
    var hosts = _.map(tenants, function(tenant) {
        return tenant.host;
    });

    // Construct a regex that will find OAE links in the body
    var regex = new RegExp('https?://(' + hosts.join('|') +
        ')/(content|discussion|folder|group)/(' + aliases.join('|') +
        ')/([a-zA-Z0-9_-]+)', 'gi');

    // Replace any matched URLs with relative links in markdown format
    return body.replace(regex, '[/$2/$3/$4](/$2/$3/$4)');
};
...
var updateMessageBody = module.exports.updateMessageBody = function(messageBoxId, created, newBody, callback) {
    ...
    var messageId = _createMessageId(messageBoxId, created);
    Cassandra.runQuery('UPDATE "Messages" SET "body" = ? WHERE "id" = ?', [_replaceLinks(newBody), messageId], function(err) {
        if (err) {
            return callback(err);
        }
