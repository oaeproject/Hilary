    ...
    describe('Member of', function() {
        ...
        it('verify paging', function(callback) {
            TestsUtil.generateTestUsers(camAdminRestContext, 1, function(err, users) {
                ...
                TestsUtil.generateTestGroups(nicolaas.restContext, 5, function(err, groups) {
                    assert.ok(!err);

                    // Get the first 2
                    RestAPI.Group.memberOf(nicolaas.restContext, nicolaas.user.id, null, 2, function(err, memberships) {
                        assert.ok(!err);
                        assert.equal(memberships.results.length, 2);
                        assert.equal(memberships.nextToken, memberships.results[1].id);

                        // Get the next 2
                        RestAPI.Group.memberOf(nicolaas.restContext, nicolaas.user.id, memberships.nextToken, 2, function(err, memberships) {
                            assert.ok(!err);
                            assert.equal(memberships.results.length, 2);
                            assert.equal(memberships.nextToken, memberships.results[1].id);
                            ...
                            RestAPI.Group.memberOf(nicolaas.restContext, nicolaas.user.id, memberships.nextToken, 2, function(err, memberships) {
                                assert.ok(!err);
                                assert.equal(memberships.results.length, 1);
                                assert.ok(!memberships.nextToken);
                                callback();
                            });
                        });
                    });
