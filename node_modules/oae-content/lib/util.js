...
var ContentInternalUtil = require('./internal/util');
 ...
 * @param   {Object}            previews    An object that contains the most recent `lastModified` date of the content item and its `previews` object
 * @return  {ActivityEntity}                The activity entity that represents the given content item
 */
var transformPersistentContentActivityEntity = module.exports.transformPersistentContentActivityEntity = function(ctx, content, previews) {
    ...
    if (previews && previews.previews.status === 'done') {
        var width = PreviewConstants.SIZES.IMAGE.THUMBNAIL;
        var thumbnailUrl = ContentInternalUtil.getThumbnailUrl(ctx, content.id, previews.lastModified);
        opts.image = new ActivityModel.ActivityMediaLink(thumbnailUrl, width, width);
        opts.ext[ContentConstants.activity.PROP_OAE_PREVIEWS] = previews.previews;
    }
...
var _transformCommentActivityEntity = function(ctx, comment) {
    var tenant = ctx.tenant();
    var baseUrl = TenantsUtil.getBaseUrl(tenant);
    var commentUrl = baseUrl + '/content/' + comment.contentId;
    var profilePath = '/content/' + comment.contentId;
    ...
    var globalId = 'http//' + tenant.host + '/api/content/' + comment.contentId + '/comments/' + comment.commentId;
