...
describe('Activity Email', function() {
    ...
    /**
     * Test that verifies that a user does not receive any email when he marks his notifications as read
     */
    it('verify no emails are sent out when a user marks their notifications as read', function(callback) {
        TestsUtil.generateTestUsers(camAdminRestContext, 2, function(err, users, simong, nico) {
            assert.ok(!err);

            // Trigger an activity for Nico
            RestAPI.Content.createLink(simong.restContext, 'Google', 'Google', 'public', 'http://www.google.ca', [], [nico.user.id], function(err, link) {
                assert.ok(!err);

                // Collect the activities, this should result in an activity in the email activity stream
                ActivityTestsUtil.collectAndGetActivityStream(nico.restContext, null, null, function(err, activityStream) {
                    assert.ok(!err);

                    // Mark the notifications as read, this should remove the activity in the email activity stream
                    ActivityTestsUtil.markNotificationsAsRead(nico.restContext, function(result) {

                        // Assert no email is sent out
                        EmailTestsUtil.collectAndFetchEmails(function(messages) {
