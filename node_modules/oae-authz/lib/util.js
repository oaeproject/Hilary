 ...
 */
var isUserId = module.exports.isUserId = function(userId) {
 ...
 */
var isEmail = module.exports.isEmail = function(email) {
 ...
 */
var parseShareTarget = module.exports.parseShareTarget = function(shareTargetStr) {
    if (!_.isString(shareTargetStr)) {
        return null;
    }

    var shareTargetSplit = shareTargetStr.split(':');
    if (isEmail(shareTargetSplit[0])) {
        ...
        if (shareTargetSplit.length === 1) {
            return {'email': shareTargetSplit[0]};
        } else {
            var email = shareTargetSplit[0];
            var userId = shareTargetSplit.slice(1).join(':');
            if (isUserId(userId)) {
                return {'email': email, 'principalId': userId};
            }
        }
 ...
 */
var isResource = module.exports.isResource = function(resource) {
    return (
        _.isObject(resource) &&
 ...
 */
var isPrincipalId = module.exports.isPrincipalId = function(principalId) {
 ...
 */
var isResourceId = module.exports.isResourceId = function(resourceId) {
 ...
 */
var isRole = module.exports.isRole = function(role) {
 ...
 */
var isVisibility = module.exports.isVisibility = function(visibility) {
 ...
 */
var getGroupIds = module.exports.getGroupIds = function(principals) {
    return _.chain(principals).pluck('id').filter(isGroupId).value();
};
 ...
 */
var filterDeletedIds = module.exports.filterDeletedIds = function(ids, callback) {
    AuthzDelete.isDeleted(ids, function(err, deletedIds) {
        if (err) {
            return callback(err);
        }

        return callback(null, _.filter(ids, function(id) {
            return (!deletedIds[id]);
        }));
    });
};

var computeRoleChanges = module.exports.computeRoleChanges = function(rolesBefore, roleChanges, opts) {
    opts = opts || {};
    ...
    var rolesPriority = AuthzConstants.role.ALL_PRIORITY;
    if (opts.promoteOnly) {
        _.each(result.changes, function(role, id) {
            if (role === false) {
                delete result.changes[id];
            } else if (result.roles.before[id]) {
                var beforeIndex = _.indexOf(rolesPriority, result.roles.before[id]);
                var changeIndex = _.indexOf(rolesPriority, role);
    ...
    _.each(result.changes, function(change, id) {
 ...
 */
var getAuthzId = module.exports.getAuthzId = function(resource) {
