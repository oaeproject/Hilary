...
describe('Content Activity', function() {
    ...
    before(function(callback) {
        ...
        suitable_files = {
            'file.small.jpg': getSakaiLogoStream,
            'file.medium.jpg': getFileStream,
            'thumbnail.png': getSakaiLogoStream,
            'wide.png': getFileStream
        };
        suitable_sizes = {
                        ...
                        RestAPI.Content.createComment(jackCtx, link.id, 'Comment B', null, function(err, commentB) {
                            ...
                            RestAPI.Content.createComment(jackCtx, link.id, 'Reply Comment A', commentA.created, function(err, replyCommentA) {
                                ...
                                ActivityTestsUtil.collectAndGetActivityStream(jackCtx, jack.id, null, function(err, activityStream) {
                                     ...
                                     * @param   {Comment}           [replyToComment]            Indicates the entity should have this comment as its oae:messageReplyTo. If unspecified, the entity should have no parent.
                                     */
                                    var _validateComment = function(entity, comment, replyToComment) {
                                        assert.equal(entity['objectType'], 'content-message');
                                        assert.equal(entity['content'], comment.body);
                                        assert.equal(entity['oae:id'], comment.id);
                                        assert.equal(entity['oae:profilePath'], '/content/' + link.id);
                                        assert.ok(entity['url'].indexOf(link.id) !== -1);
                                        assert.ok(entity['id'].indexOf('content/' + link.id + '/comments/' + comment.created) !== -1);
                                        assert.equal(entity['published'], comment.created);
                                        assert.equal(entity['oae:messageThreadKey'], comment.threadKey);
                                        ...
                                        if (replyToComment) {
                                            _validateComment(entity['oae:messageReplyTo'], replyToComment);
                                        } else {
                                            assert.ok(!entity['oae:messageReplyTo']);
                                        }
                                    };
    ...
    describe('Activity Aggregation', function() {
        ...
        /*
         * The content-message activity demonstrates an activity that has all 3 entities, but only aggregates on 1 of them. This means that
         * 2 of the entity types are aggregated as more activities are generated instead of just one.
         */

        /**
         * Test that verifies that when content-message activities are aggregated, both actor and object entities are collected into the activity
         * for display.
         */
        it('verify that content-message activity aggregates both actor and object entities while pivoting on target', function(callback) {
            var jackUsername = TestsUtil.generateTestUserId('jack');
                    ...
                    RestAPI.Content.createComment(jackCtx, link.id, 'Test Comment A', null, function(err) {
                        ...
                        RestAPI.Content.createComment(camAdminRestContext, link.id, 'Test Comment B', null, function(err) {
                            ...
                            ActivityTestsUtil.collectAndGetActivityStream(jackCtx, null, null, function(err, activityStream) {
                                ...
                                var activity = activityStream.items[0];
                                assert.equal(activity['oae:activityType'], 'content-message');
    ...
    describe('Email', function() {
         ...
         * scrubbed.
         */
        it('verify content-message email and privacy', function(callback) {
                        ...
                        RestAPI.Content.createLink(mrvisser.restContext, 'Google', 'Google', 'public', 'http://www.google.ca', [], [], function(err, link) {
                            ...
                            RestAPI.Content.createComment(simong.restContext, link.id, 'Nice link, would click again', null, function(err, simongComment) {
                                ...
                                EmailTestsUtil.collectAndFetchEmails(function(messages) {
                                    // There should be exactly one message, the one sent to mrvisser (manager of content item receives content-message notification)
