...
describe('General Search', function() {
    ...
    describe('Search Scopes', function() {
        ...
        /**
         * Perform a general search using the "_share" scope, ensuring the target user or group is
         * in the search results
         *
         * @param  {RestContext}    restContext         The REST context with which to send the request
         * @param  {Object}         resource            The resource being shared
         * @param  {User|Group}     target              The target user or group to search for
         * @param  {Object}         [opts]              Optional arguments
         * @param  {Boolean}        [opts.canManage]    Whether or not the current user in context can manage the resource. Default: `false`
         * @param  {Function}       callback            Invoked when all assertions have passed
         * @throws {AssertionError}                     Thrown if any assertions fail
         */
        var _assertSearchShareContains = function(restContext, resource, target, opts, callback) {
            opts = _.extend({'q': target.displayName}, opts);
            return SearchTestsUtil.assertSearchShareContains(restContext, resource, opts, [target.id], callback);
        };
         ...
         * @throws {AssertionError}                     Thrown if any assertions fail
         */
        var _assertSearchShareNotContains = function(restContext, resource, target, opts, callback) {
            opts = _.extend({'q': target.displayName}, opts);
            return SearchTestsUtil.assertSearchShareNotContains(restContext, resource, opts, [target.id], callback);
        };
        ...
        /**
         * Test that verifies that, when using the "share" search scope as a non-manager of a
         * resource in a different tenant, that only users who can interact with the resource
         * are returned
         */
        it('verify non-managers share search for resource in different tenant only has those that can interact with the resource', function(callback) {
            TestsUtil.setupMultiTenantPrivacyEntities(function(publicTenant0, publicTenant1, privateTenant0, privateTenant1) {

                // Public external resource can be shared to all that the actor can interact with
                _assertSearchShareContains(publicTenant0.publicUser.restContext, publicTenant1.publicGroup, publicTenant0.publicGroup, null, function() {
                    _assertSearchShareContains(publicTenant0.publicUser.restContext, publicTenant1.publicGroup, publicTenant0.loggedinGroup, null, function() {
                        _assertSearchShareContains(publicTenant0.publicUser.restContext, publicTenant1.publicGroup, publicTenant1.publicGroup, null, function() {

                            // Loggedin external resource can be shared to only those in the same tenant
                            _assertSearchShareNotContains(publicTenant0.publicUser.restContext, publicTenant1.loggedinGroup, publicTenant0.publicGroup, null, function() {
                                _assertSearchShareNotContains(publicTenant0.publicUser.restContext, publicTenant1.loggedinGroup, publicTenant0.loggedinGroup, null, function() {
                                    _assertSearchShareContains(publicTenant0.publicUser.restContext, publicTenant1.loggedinGroup, publicTenant1.publicGroup, null, function() {

                                        // Private resource can't be shared anywhere, which works
                                        // out well because they don't have access to share it
                                        _assertSearchShareNotContains(publicTenant0.publicUser.restContext, publicTenant1.privateGroup, publicTenant0.publicGroup, null, function() {
                                            _assertSearchShareNotContains(publicTenant0.publicUser.restContext, publicTenant1.privateGroup, publicTenant0.loggedinGroup, null, function() {
                                                _assertSearchShareNotContains(publicTenant0.publicUser.restContext, publicTenant1.privateGroup, publicTenant1.publicGroup, null, function() {
                                                    return callback();
                                                });
                                            });
                                        });
        ...
        /**
         * Test that verifies that, when using the "share" search scope as a non-manager of a
         * resource, that only users who can interact with the resource are returned
         */
        it('verify non-managers share search for resource in same tenant only has those that can interact with the resource', function(callback) {
            TestsUtil.setupMultiTenantPrivacyEntities(function(publicTenant0, publicTenant1, privateTenant0, privateTenant1) {

                // Public resource can be shared to all that the actor can interact with
                _assertSearchShareContains(publicTenant0.publicUser.restContext, publicTenant0.publicGroup, publicTenant0.publicGroup, null, function() {
                    _assertSearchShareContains(publicTenant0.publicUser.restContext, publicTenant0.publicGroup, publicTenant0.loggedinGroup, null, function() {
                        _assertSearchShareContains(publicTenant0.publicUser.restContext, publicTenant0.publicGroup, publicTenant1.publicGroup, null, function() {

                            // Loggedin resource can be shared to only those in the same tenant
                            _assertSearchShareContains(publicTenant0.publicUser.restContext, publicTenant0.loggedinGroup, publicTenant0.publicGroup, null, function() {
                                _assertSearchShareContains(publicTenant0.publicUser.restContext, publicTenant0.loggedinGroup, publicTenant0.loggedinGroup, null, function() {
                                    _assertSearchShareNotContains(publicTenant0.publicUser.restContext, publicTenant0.loggedinGroup, publicTenant1.publicGroup, null, function() {

                                        // Private resource can't be shared anywhere, which works
                                        // out well because they don't have access to share it
                                        _assertSearchShareNotContains(publicTenant0.publicUser.restContext, publicTenant0.privateGroup, publicTenant0.publicGroup, null, function() {
                                            _assertSearchShareNotContains(publicTenant0.publicUser.restContext, publicTenant0.privateGroup, publicTenant0.loggedinGroup, null, function() {
                                                _assertSearchShareNotContains(publicTenant0.publicUser.restContext, publicTenant0.privateGroup, publicTenant1.publicGroup, null, function() {
                                                    return callback();
                                                });
                                            });
                                        });
        ...
        /**
         * Test that verifies that the _share scope only searches share targets with which the
         * user performing the search can interact
         */
        it('verify the share search scope searches only users and groups with which the actor user can interact', function(callback) {
            TestsUtil.setupMultiTenantPrivacyEntities(function(publicTenant0, publicTenant1, privateTenant0, privateTenant1) {
                // Use a public group as the sharing resource
                var resource = publicTenant0.publicGroup;

                // Anonymous users cannot use the _share scope without a 401 error
                SearchTestsUtil.assertSearchShareFails(publicTenant0.anonymousRestContext, resource, null, 401, function() {

                    // Public and loggedin items from the current public tenant should be searched
                    _assertSearchShareContains(publicTenant0.publicUser.restContext, resource, publicTenant0.publicUser.user, null, function() {
                        _assertSearchShareContains(publicTenant0.publicUser.restContext, resource, publicTenant0.loggedinUser.user, null, function() {
                            _assertSearchShareNotContains(publicTenant0.publicUser.restContext, resource, publicTenant0.privateUser.user, null, function() {

                                // A private user cannot search themself for interaction
                                _assertSearchShareNotContains(publicTenant0.privateUser.restContext, resource, publicTenant0.privateUser.user, null, function() {

                                    // Private joinable groups from the current tenant should be searched when searched with the tenant admin
                                    _assertSearchShareContains(publicTenant0.adminRestContext, resource, publicTenant0.privateGroup, null, function() {

                                        // Private joinable groups from the current tenant should not be searched when searched as a regular user
                                        _assertSearchShareNotContains(publicTenant0.publicUser.restContext, resource, publicTenant0.privateGroup, null, function() {

                                            // Sanity check that under _network search, the private joinable group does get searched when searching as a regular user
                                            var privateGroup = publicTenant0.privateGroup;
                                            SearchTestsUtil.assertSearchContains(publicTenant0.publicUser.restContext, 'general', null, {'scope': '_network', 'q': privateGroup.displayName}, [privateGroup.id], function() {

                                                // Only public users and groups from another public tenant should be searched
                                                _assertSearchShareContains(publicTenant0.publicUser.restContext, resource, publicTenant1.publicUser.user, null, function() {
                                                    _assertSearchShareNotContains(publicTenant0.publicUser.restContext, resource, publicTenant1.loggedinUser.user, null, function() {
                                                        _assertSearchShareNotContains(publicTenant0.publicUser.restContext, resource, publicTenant1.privateUser.user, null, function() {

                                                            // Nothing from an external private tenant should be searched
                                                            _assertSearchShareNotContains(publicTenant0.publicUser.restContext, resource, privateTenant0.publicUser.user, null, function() {
                                                                _assertSearchShareNotContains(publicTenant0.publicUser.restContext, resource, privateTenant0.loggedinUser.user, null, function() {
                                                                    _assertSearchShareNotContains(publicTenant0.publicUser.restContext, resource, privateTenant0.privateUser.user, null, function() {

                                                                        // Public and loggedin items from the current private tenant should be searched. Since we are are
                                                                        // isolating the actor -> target user interaction, we set `canManage` to true which will bypass
                                                                        // the resource -> target user checks
                                                                        _assertSearchShareContains(privateTenant0.publicUser.restContext, resource, privateTenant0.publicUser.user, {'canManage': true}, function() {
                                                                            _assertSearchShareContains(privateTenant0.publicUser.restContext, resource, privateTenant0.loggedinUser.user, {'canManage': true}, function() {
                                                                                _assertSearchShareNotContains(privateTenant0.publicUser.restContext, resource, privateTenant0.privateUser.user, {'canManage': true}, function() {

                                                                                    // Nothing from an external public tenant should be searched when searching from a
                                                                                    // private tenant. Since we are are isolating the actor -> target user interaction,
                                                                                    // we set `canManage` to true which will bypass the resource -> target user checks
                                                                                    _assertSearchShareNotContains(privateTenant0.publicUser.restContext, resource, publicTenant0.publicUser.user, {'canManage': true}, function() {
                                                                                        _assertSearchShareNotContains(privateTenant0.publicUser.restContext, resource, publicTenant0.loggedinUser.user, {'canManage': true}, function() {
                                                                                            _assertSearchShareNotContains(privateTenant0.publicUser.restContext, resource, publicTenant0.privateUser.user, {'canManage': true}, function() {

                                                                                                // Nothing from an external private tenant should be searched when
                                                                                                // searching from a private tenant. Since we are are isolating the
                                                                                                // actor -> target user interaction, we set `canManage` to true which
                                                                                                // will bypass the resource -> target user checks
                                                                                                _assertSearchShareNotContains(privateTenant0.publicUser.restContext, resource, privateTenant1.publicUser.user, null, function() {
                                                                                                    _assertSearchShareNotContains(privateTenant0.publicUser.restContext, resource, privateTenant1.loggedinUser.user, null, function() {
                                                                                                        _assertSearchShareNotContains(privateTenant0.publicUser.restContext, resource, privateTenant1.privateUser.user, null, function() {
                                                                                                            return callback();
                                                                                                        });
                                                                                                    });
                                                                                                });
