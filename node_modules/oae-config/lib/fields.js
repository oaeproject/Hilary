 ...
 *
 * - `{tenantOverride: true, valueVisibility: 'private'}`       Values only viewable by the admin interface that sets them and overridable by admins
 * - `{tenantOverride: true, valueVisibility: 'admin'}`         Values only viewable by admins and overridable by admins
 * - `{tenantOverride: true, valueVisibility: 'public'}`        Values viewable by everyone and overridable by admins
 * - `{tenantOverride: false, valueVisibility: 'private'}`      Values only viewable by the admin interface that sets them and not overridable by tenant admins
 * - `{tenantOverride: false, valueVisibility: 'admin'}`        Values only viewable by admins and not overridable by tenant admins
 * - `{tenantOverride: false, valueVisibility: 'public'}`       Values viewable by everyone and not overridable by tenant admins
 *
 ...
 * @param  {Boolean}    [options.tenantOverride]      Whether or not tenant admins should be able to override this value for a tenant. Defaults to `true`. Note that global administrators can always override tenant values.
 * @param  {String}     [options.valueVisibility]     Determens who can see the value of the configuration option. 'private': The value is only viewable by the admin interface where its set, 'admin': The value is only viewable by admins, 'public': The value is viewable by all, including end users
 * @api private
 */
var BaseField = function(type, name, description, defaultValue, options) {
    ...
    var field = {
        ...
        'valueVisibility': options.valueVisibility ? options.valueVisibility : 'public'
    };
 ...
 * @param  {Object}     [options]                  List of additional configuration options
 * @param  {Boolean}    [options.tenantOverride]   Whether or not tenant admins should be able to override this value for a tenant. Defaults to `true`. Note that global administrators can always override tenant values.
 * @param  {String}     [options.valueVisibility]  Determines who can see the value of the configuration option. 'private': The value is only viewable by the admin interface where its set, 'admin': The value is only viewable by admins, 'public': The value is viewable by all, including end users
 */
