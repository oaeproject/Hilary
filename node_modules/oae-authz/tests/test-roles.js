...
describe('Authz-Roles', function() {
    ...
    var loadContentRoles = function(principalUuid, baseContentId, resourceType, numContentItems, role, callback) {
        ...
        AuthzAPI.updateRoles(resourceUuid, makeChange(principalUuid, role), function(err) {
            if (err) {
                throw err;
            }
    ...
    describe('#getDirectRole()', function() {
        ...
        it('verify role separation between tenants', function(callback) {
            ...
            AuthzAPI.updateRoles(resourceUuid, makeChange(principalUuidA, 'manager'), function(err) {
                assert.ok(!err);

                // verify tenant B user does not have a direct or indirect role on that content
                AuthzAPI.getDirectRole(principalUuidB, resourceUuid, function(err, role) {
                    assert.ok(!err);
                    assert.ok(!role);
                    AuthzAPI.getAllRoles(principalUuidB, resourceUuid, function(err, roles) {
                        assert.ok(!err);
                        assert.equal(roles.length, 0);

                        // add 'viewer' for security context B
                        AuthzAPI.updateRoles(resourceUuid, makeChange(principalUuidB, 'viewer'), function(err) {
                            assert.ok(!err);
    ...
    describe('#getAllRoles()', function() {
        ...
        it('verify direct single role association', function(callback) {
            ...
            AuthzAPI.updateRoles(resourceUuid, makeChange(userUuid, 'viewer'), function(err) {
                assert.ok(!err);
                AuthzAPI.getAllRoles(userUuid, resourceUuid, function(err, roles) {
                    assert.ok(!err);
        ...
        it('verify indirect single role association', function(callback) {
            ...
            AuthzAPI.updateRoles(groupUuid, makeChange(userUuid, 'member'), function(err) {
                assert.ok(!err);
                AuthzAPI.updateRoles(resourceUuid, makeChange(groupUuid, 'viewer'), function(err) {
                    assert.ok(!err);
                    AuthzAPI.getAllRoles(userUuid, resourceUuid, function(err, roles) {
                        assert.ok(!err);
                        assert.equal(roles.length, 1);
        ...
        it('verify indirect two role association', function(callback) {
            ...
            AuthzAPI.updateRoles(groupUuid, makeChange(userUuid, 'member'), function(err) {
                ...
                AuthzAPI.updateRoles(resourceUuid, changes, function(err) {
                    assert.ok(!err);
                    AuthzAPI.getAllRoles(userUuid, resourceUuid, function(err, roles) {
                        assert.ok(!err);
        ...
        it('verify multi-indirect two role association', function(callback) {
            ...
            AuthzAPI.updateRoles(groupUuid1, makeChange(userUuid, 'member'), function(err) {
                assert.ok(!err);
                AuthzAPI.updateRoles(groupUuid2, makeChange(userUuid, 'member'), function(err) {
                    assert.ok(!err);
                    AuthzAPI.updateRoles(resourceUuid, makeChange(groupUuid1, 'viewer'), function(err) {
                        assert.ok(!err);
                        AuthzAPI.updateRoles(resourceUuid, makeChange(groupUuid2, 'manager'), function(err) {
                            assert.ok(!err);
                            AuthzAPI.getAllRoles(userUuid, resourceUuid, function(err, roles) {
                                assert.ok(!err);
                                assert.equal(roles.length, 2);
        ...
        it('verify circular group hierarchy three role association', function(callback) {
            ...
            AuthzAPI.updateRoles(groupUuid1, makeChange(userUuid, 'member'), function(err) {
                assert.ok(!err);
                AuthzAPI.updateRoles(groupUuid2, makeChange(groupUuid1, 'member'), function(err) {
                    assert.ok(!err);
                    AuthzAPI.updateRoles(groupUuid3, makeChange(groupUuid2, 'member'), function(err) {
                        assert.ok(!err);
                        AuthzAPI.updateRoles(groupUuid1, makeChange(groupUuid3, 'member'), function(err) {
                            assert.ok(!err);
                            AuthzAPI.updateRoles(resourceUuid, makeChange(groupUuid1, 'viewer'), function(err) {
                                assert.ok(!err);
                                AuthzAPI.updateRoles(resourceUuid, makeChange(groupUuid2, 'manager'), function(err) {
                                    assert.ok(!err);
                                    AuthzAPI.updateRoles(resourceUuid, makeChange(groupUuid3, 'editor'), function(err) {
                                        assert.ok(!err);
                                        AuthzAPI.getAllRoles(userUuid, resourceUuid, function(err, roles) {
                                            assert.ok(!err);
    ...
    describe('#hasRole()', function() {
        ...
        it('verify positive check with a role', function(callback) {
            ...
            AuthzAPI.updateRoles(resourceUuid, makeChange(principalUuid, 'manager'), function(err) {
                assert.ok(!err);
        ...
        it('verify negative check when role has been removed', function(callback) {
            ...
            AuthzAPI.updateRoles(resourceUuid, makeChange(principalUuid, 'manager'), function(err) {
                ...
                AuthzAPI.hasRole(principalUuid, resourceUuid, 'manager', function(err, hasRole) {
                    ...
                    AuthzAPI.updateRoles(resourceUuid, makeChange(principalUuid, false), function(err) {
                        assert.ok(!err);
    ...
    describe('#applyRoleChanges()', function() {
        ...
        it('verify update existing role', function(callback) {
            ...
            AuthzAPI.updateRoles(resourceUuid, makeChange(principalUuid, 'viewer'), function(err) {
                assert.ok(!err);
        ...
        it('verify general functionality', function(callback) {
            ...
            AuthzAPI.updateRoles(resourceUuid1, roles, function(err) {
                assert.ok(!err);
                AuthzAPI.hasRole(principalUuid1, resourceUuid1, 'manager', function(err, hasRole) {
                    ...
                    AuthzAPI.hasRole(principalUuid2, resourceUuid1, 'manager', function(err, hasRole) {
                        ...
                        AuthzAPI.updateRoles(resourceUuid2, roles, function(err) {
                            assert.ok(!err);
                            AuthzAPI.hasRole(principalUuid1, resourceUuid2, 'manager', function(err, hasRole) {
                                ...
                                AuthzAPI.hasRole(principalUuid2, resourceUuid2, 'manager', function(err, hasRole) {
                                    ...
                                    AuthzAPI.updateRoles(resourceUuid3, roles, function(err) {
                                        assert.ok(!err);
        ...
        it('verify validation', function(callback) {
            ...
            AuthzAPI.updateRoles(null, roles, function(err) {
                assert.ok(err);
                // Try applyRolesChanges with empty roles
                AuthzAPI.updateRoles(resourceUuid1, {}, function(err) {
                    assert.ok(err);
                    callback();
                });
            });
