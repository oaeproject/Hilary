 ...
 */
var doMigration = (module.exports.doMigration = function(
  tenantAlias,
  stream,
  callback,
) {
    ...
    _getExternalIds(userHashes, function(err, usersWithIds) {
      ...
      _mapUsersToShibLogin(usersWithIds, function(err, mappedUsers) {
        ...
        if (_.isEmpty(mappedUsers)) {
          ...
          log().info(
            '%s users were migrated to Shibboleth logins.',
            mappedUsers.length,
          );
        }
  ...
  Cassandra.runQuery(
    ...
    function(err, results) {
      if (err) {
        log().error(
          { err: err },
          'Failed to update AuthenticationUserLoginId table in Cassandra',
        );
        _writeErrorRow(
          userHash,
          'Failed to update AuthenticationUserLoginId for this user',
        );
        return callback(err);
      }
      Cassandra.runQuery(
        ...
        function(err, results) {
          if (err) {
            log().error(
              { err: err },
              'Failed to update AuthenticationLoginId table in Cassandra',
            );
            _writeErrorRow(
              userHash,
              'Failed to update AuthenticationLoginId for this user',
            );
            return callback(err);
          }

          log().info(
            'Created Shibboleth login record for user %s',
            userHash.displayName,
          );
          return callback();
        },
      );
