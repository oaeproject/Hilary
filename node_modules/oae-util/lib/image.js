 ...
 */
var autoOrient = (module.exports.autoOrient = function(
  inputPath,
  opts,
  callback,
) {
  ...
  var outputPath =
    opts.outputPath ||
    temp.path({ suffix: getImageExtension(inputPath, '.jpg') });
  gm(inputPath)
    .noProfile()
    .autoOrient()
    .write(outputPath, function(err) {
      if (err) {
        fs.unlink(outputPath, function(err) {
          if (err) {
            log().warn(
              { err: err, path: outputPath },
              'Could not unlink a file',
            );
          }
        });
        log().error(
          { err: err },
          'Could not auto orient the image %s',
          inputPath,
        );
      ...
      fs.stat(outputPath, function(err, stat) {
        if (err) {
          ...
          log().error(
            { err: err },
            'Could not get the file system information about %s',
            outputPath,
          );
          return callback({
            code: 500,
        ...
        fs.unlink(inputPath, function(err) {
          if (err) {
            ...
            return callback({
              code: 500,
              msg: 'Could not unlink the input image',
            });
          }
 ...
 */
var cropAndResize = (module.exports.cropAndResize = function(
  imagePath,
  selectedArea,
  sizes,
  callback,
  ...
  validator
    .check(sizes, { code: 400, msg: 'The desired sizes array is missing' })
    .notNull();
  if (sizes) {
    validator
      .check(sizes.length, {
 ...
 */
var cropImage = (module.exports.cropImage = function(
  imagePath,
  selectedArea,
  callback,
) {
...
var _cropImage = function(imagePath, selectedArea, callback) {
  ...
  gm(imagePath).size(function(err, size) {
    if (err) {
      log().error(
        { err: err },
        'Could not get the image size for the large image',
      );
      return callback({
        code: 500,
        msg: 'Could not get the image size for the large image',
      });
    }
    ...
    if (
      ...
      return callback({
        code: 400,
        msg: 'You cannot crop outside of the image',
      });
    }
        ...
        fs.stat(tempPath, function(err, stat) {
          if (err) {
            ...
            return callback({
              code: 500,
              msg:
                'Could not retrieve the file information for the cropped file',
            });
          }
          var file = {
            path: tempPath,
 ...
 */
var resizeImage = (module.exports.resizeImage = function(
  imagePath,
  size,
  callback,
) {
  ...
  validator
    ...
    .notNull();
  validator
    .check(null, { code: 400, msg: 'The size must be specified' })
    .isObject(size);
  if (size) {
    validator
      .check(size.width, {
...
var _resizeImage = function(imagePath, size, callback) {
  var suffix =
    size.width + 'x' + size.height + getImageExtension(imagePath, '.jpg');
    ...
    .write(tempPath, function(err) {
      ...
      fs.stat(tempPath, function(err, stat) {
        if (err) {
          ...
          log().error(
            { err: err },
            'Could not get the file system information for %s',
            tempPath,
          );
          return callback({
            code: 500,
 ...
 */
var getImageExtension = (module.exports.getImageExtension = function(
  source,
  fallback,
) {
 ...
 */
var convertToJPG = (module.exports.convertToJPG = function(
  inputPath,
  callback,
) {
  var validator = new Validator();
  ...
  gm(conversionPath).size(function(err, size) {
    if (err) {
      log().error(
        { err: err },
        'Unable to get size of the image that should be converted to JPG',
      );
      return callback({ code: 500, msg: err });
    }
    ...
    exec(cmd, { timeout: 4000 }, function(err, stdout, stderr) {
      ...
      if (err) {
        log().error(
          { err: err },
          'Unable to convert input image to JPG (Took %sms)',
          durationMs,
        );
        return callback({
          code: 500,
          msg: 'Failed converting input image to JPG',
        });
      }

      log().trace(
        'Finished converting image into a JPG (Took %sms)',
        durationMs,
      );
      ...
      fs.stat(outputPath, function(err, stat) {
        if (err) {
          ...
          return callback({
            code: 500,
            msg:
              'Could not retrieve the file information for the converted file',
          });
        }
        var file = {
          path: outputPath,
