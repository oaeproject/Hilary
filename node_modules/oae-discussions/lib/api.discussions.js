...
var createDiscussion = module.exports.createDiscussion = function(ctx, displayName, description, visibility, members, opts, callback) {
    ...
    members[ctx.user().id] = AuthzConstants.role.MANAGER;

    var createFn = _.partial(DiscussionsDAO.createDiscussion, ctx.user().id, displayName, description, visibility, null);
    ResourceActions.create(ctx, members, createFn, function(err, discussion) {
        if (err) {
            return callback(err);
        }
...
var shareDiscussion = module.exports.shareDiscussion = function(ctx, discussionId, principalIds, callback) {
    ...
    _getDiscussion(discussionId, function(err, discussion) {
        ...
        ResourceActions.share(ctx, discussion, principalIds, AuthzConstants.role.MEMBER, function(err, newMembers, memberRoles) {
            if (err) {
                return callback(err);
            }

            var newMemberIds = _.pluck(newMembers, 'id');
            var roleChanges = _.pick(memberRoles, newMemberIds);
            ...
            OaeUtil.invokeIfNecessary(_testDiscussionUpdateThreshold(discussion), _touch, discussion, function(err, touchedDiscussion) {
                ...
                _insertLibrary(newMemberIds, discussion, function(err) {
                    ...
                    DiscussionsAPI.emit(DiscussionsConstants.events.UPDATED_DISCUSSION_MEMBERS, ctx, discussion, roleChanges, newMemberIds);
                    return callback();
                });
            });
        });
...
var setDiscussionPermissions = module.exports.setDiscussionPermissions = function(ctx, discussionId, permissionChanges, callback) {
    ...
    _getDiscussion(discussionId, function(err, discussion) {
        ...
        ResourceActions.setRoles(ctx, discussion, permissionChanges, function(err, membershipAfterChanges, newMembers, updatedMembers, removedMembers) {
            if (err) {
                return callback(err);
            }
