...
describe('Content', function() {
        ...
        ContentAPI.createLink(contexts[0], privacy, 'Test Content 1', 'Test content description 1', 'http://www.sakaiproject.org/', {}, function(err, contentObj) {    
            ...
            checkPieceOfContent(contexts[0], contexts[0].user().id, contentObj, true, true, true, function() {
                ...
                ContentAPI.getContentMembers(contexts[0], contentObj.contentId, null, null, function(err, members) {
                    ...
                    assert.equal(members[0].profile.id, contexts[0].user().id);
                        ...
                        ContentAPI.setContentPermissions(contexts[0], contentObj.contentId, permissions, function(err) {
                            ...
                            checkPieceOfContent(contexts[1], contexts[1].user().id, contentObj, true, true, true, function() {
                                ContentAPI.getContentMembers(contexts[0], contentObj.contentId, null, null, function(err, members) {
                                    ...
                                    assert.equal(members[0].profile.id, contexts[0].user().id);
                                    assert.equal(members[0].role, 'manager');
                                    assert.equal(members[1].profile.id, contexts[1].user().id);
                                    ...
                                    ContentAPI.setContentPermissions(contexts[0], contentObj.contentId, permissions, function(err) {
                                        ...
                                        checkPieceOfContent(contexts[2], contexts[2].user().id, contentObj, true, false, true, function() {
                                            ContentAPI.getContentMembers(contexts[0], contentObj.contentId, null, null, function(err, members) {
                                                ...
                                                assert.equal(members[1].profile.id, contexts[1].user().id);
                                                assert.equal(members[1].role, 'manager');
                                                assert.equal(members[2].profile.id, contexts[2].user().id);
                                                assert.equal(members[2].role, 'viewer');
                                                            ...
                                                            ContentAPI.setContentPermissions(contexts[0], contentObj.contentId, permissions, function(err) {
                                                                ...
                                                                checkPieceOfContent( contexts[1], contexts[1].user().id, contentObj, privacy === 'private' ? false : true, false, false, function() {
                                                                    ContentAPI.getContentMembers(contexts[0], contentObj.contentId, null, null, function(err, members) {
                                                                        ...
                                                                        assert.equal(members[0].profile.id, contexts[0].user().id);
                                                                        assert.equal(members[0].role, 'manager');
                                                                        assert.equal(members[1].profile.id, contexts[2].user().id);
            ...
            ContentAPI.createLink(contexts[0], 'public', 'Test Content 1', 'Test content description 1', 'http://www.sakaiproject.org/', {}, function(err, contentObj) {    
                ...
                ContentAPI.setContentPermissions(contexts[0], contentObj.contentId, permissions, function(err) {
                    ...
                    ContentAPI.getContentMembers(contexts[0], contentObj.contentId, function(err, members) {
                        assert.ok(!err);
                        assert.equal(_.keys(members).length, 4);
                        ...
                        assert.equal(members[contexts[3].user().id], 'viewer');
                        ...
                        ContentAPI.setContentPermissions(contexts[0], contentObj.contentId, permissions, function(err) {
                            ...
                            ContentAPI.getContentMembers(contexts[0], contentObj.contentId, function(err, members) {
                                ...
                                assert.equal(members[contexts[0].user().id], 'manager');
                                assert.equal(members[contexts[3].user().id], 'viewer');
                                assert.equal(members[contexts[4].user().id], 'viewer');
    ...
    // TODO: we should also have tests for concurrent updates to content. e.g., 10 individual bursts of 2-3 concurrent updates, 
    // then a look at the library afterward after each burst to ensure there are no duplicate library entries.
