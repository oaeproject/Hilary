...
module.exports.registerLibrarySearch = function(searchName, resourceTypes, options) {
    ...
    SearchAPI.registerSearch(searchName, function(ctx, opts, callback) {
        ...
        validator.check(opts.libraryOwnerId, {'code': 400, 'msg': 'Must specificy an id of a library to search'}).isValidId();
        ...
        options.getLibraryOwner(opts.libraryOwnerId, function(err, libraryOwner) {
            ...
            LibraryAPI.Authz.resolveTargetLibraryAccess(ctx, opts.libraryIndexedId, libraryOwner.visibility, function(err, canAccess, visibility) {
                if (err) {
                    return callback(err);
                } else if (!canAccess) {
                ...
                var query = {
                    'bool': {
                        'should': [
                            ...
                            // Search on content comments or discussion messages
                            SearchUtil.createHasChildQuery(ContentConstants.search.MAPPING_CONTENT_COMMENT, SearchUtil.createQueryStringQuery(opts.q, ['body']), 'max'),
                            SearchUtil.createHasChildQuery(DiscussionsConstants.search.MAPPING_DISCUSSION_MESSAGE, SearchUtil.createQueryStringQuery(opts.q, ['body']), 'max'),

                            // Search on content bodies
                            SearchUtil.createHasChildQuery(ContentConstants.search.MAPPING_CONTENT_BODY, SearchUtil.createQueryStringQuery(opts.q, ['content_body']), 'max', 2)
                        ],
...
var _defaultLibraryFilter = function(resourceTypes, visibility) {
    return function(ctx, libraryOwner, opts, callback) {
        ...
        var baseFilter = SearchUtil.filterAnd(
            ...
            SearchUtil.createHasChildQuery(AuthzConstants.search.MAPPING_RESOURCE_MEMBERS, SearchUtil.filterTerms('direct_members', [opts.libraryIndexedId]))
        );
