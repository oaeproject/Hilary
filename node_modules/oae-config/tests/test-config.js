...
describe('Config Aggregator', function() {
    ...
    describe('Configuration', function() {
        ...
        it('verify get global config', function(callback) {
            RestAPI.Config.getConfig(globalAdminRestContext, 'global', function(err, config) {
                assert.ok(!err);
                assert.ok(config);
        ...
        it('verify set global config value', function(callback) {
            RestAPI.Config.setConfig(globalAdminRestContext, 'global', 'oae-authentication/twitter/enabled', false, function(err) {
                assert.ok(!err);
        ...
        it('verify set tenant config value', function(callback) {
            RestAPI.Config.setConfig(camAdminRestContext, 'global', 'oae-authentication/twitter/enabled', true, function(err) {
                assert.ok(!err);

                // Validate that the change has been made and has overriden the global config
                RestAPI.Config.getConfig(camAdminRestContext, 'global', function(err, config) {
                    assert.ok(!err);
                    assert.ok(config);
        ...
        it('verify validation', function(callback) {
            // Missing configField
            RestAPI.Config.setConfig(globalAdminRestContext, 'global', null, false, function(err) {
                assert.ok(err);
                
                // Missing configValue
                RestAPI.Config.setConfig(globalAdminRestContext, 'global', 'oae-authentication/twitter/enabled', null, function(err) {
                    assert.ok(err);
                    
                    // Try changing the config as a non-admin
                    RestAPI.Config.setConfig(anonymousCamRestContext, 'global', 'oae-authentication/twitter/enabled', 'moops', function(err) {
                        assert.ok(err);
                        callback();
                    });
                });
        ...
        it('Test retrieving a single config value succeeds', function(callback) {
            assert.equal(AuthenticationConfig.getValue('camtest', 'twitter', 'enabled'), true);
            callback();
        });

        it('Test retrieving a single config value requires all parameters to be present', function(callback) {
