 ...
 *
 * @param  {String}     _uiDirectory    The path to the directory containing the UI repository.
 * @param  {Object}     _hashes         A mapping that will map pre-optimized paths to hashed, post-optimized static asset paths
...
var init = module.exports.init = function(_uiDirectory, _hashes, callback) {
    ...
    cacheSkinVariables(function(err) {
        if (err) {
            return callback(err);
        }
...
ConfigAPI.on('update', function(tenantAlias) {
    ...
    generateSkin(tenantAlias, function(err) {
        if (err) {
...
/**
 * When a UI production build is done, most file names will be hashed to ensure that user caches are "busted" when
 * the file changes, allowing for "infinite" length caching of the resources. This function will return the path to
 * the hashed resource based on the original unhashed path.
 *
 * @param  {String}     path            Unhashed path to the resource for which we want to retrieve the hashed path
 * @return {string}                     Hashed path for the resource representing the unhashed path. If the unhashed path cannot be found, `false` will be returned.
 */
var getHashedResource = module.exports.getHashedResource = function(path) {
    // When working with a non-production build, the hashes object will be empty
    if (hashes) {
        return hashes[path];
    }
 ...
 * When a UI production build is done, most file names will be hashed to ensure that user caches are "busted" when
 * the file changes, allowing for "infinite" length caching of the resources. This function will return the path to
 * the original unhashed resource based on the hashed path.
 *
 * @param  {String}     path            Hashed path to the resource for which we want to retrieve the unhashed path
 * @return {string}                     Unhashed path for the resource represented by the hashed path. If the hashed path cannot be found, `false` will be returned.
 */
var getOriginalResource = module.exports.getOriginalResource = function(path) {
    var originalPath = false;

    // When working with a non-production build, the hashes object will be empty
    if (hashes) {
        _.each(hashes, function(hashedPath, unhashedPath) {
            if (hashedPath === path) {
                originalPath = unhashedPath;
            }
        });
    }
...
var getSkin = module.exports.getSkin = function(ctx, callback) {
    ...
    if (cachedSkins[tenantAlias]) {
        ...
        generateSkin(tenantAlias, callback);
    }
};
...
var getSkinVariables = module.exports.getSkinVariables = function(ctx, tenantAlias, callback) {
    ...
    var applyTenantValues = function(err, skinVariables) {
        ...
        var tenantVariables = getTenantSkinVariableValues(tenantAlias);
    ...
    if (cachedSkinVariables) {
        ...
        cacheSkinVariables(applyTenantValues);
    }
};
 ...
 * @api private
 */
var generateSkin = function(tenantAlias, callback) {

    // Get all the default variables in the skin, as well as the tenant overrides
    var allVariables = getDefaultSkinVariableValues();
    var tenantVariables = getTenantSkinVariableValues(tenantAlias);
    ...
    parseLessFile(allVariables, function(err, tree) {
        if (err) {
            return callback(err);
        }
 ...
 * @api private
 */
var parseLessFile = function(variables, callback) {
    variables = variables || {};
    ...
    getStaticFile(UIConstants.paths.BASE_SKIN, function(err, skin) {
        ...
        variables = replaceOptimizedPaths(variables);
 ...
 * @api private
 */
var cacheSkinVariables = function(callback) {
    parseLessFile(null, function(err, tree) {
        if (err) {
            return callback(err);
        }
 ...
 * @api private
 */
var getTenantSkinVariableValues = function(tenantAlias) {
    try {
 ...
 * @api private
 */
var getDefaultSkinVariableValues = function() {
    if (!cachedSkinVariables) {
        return null;
    }
 ...
 *
 * @param  {Object}    skinVariables   An object of variableName->variableValue of skin values that we should try and map to the optimized paths
 * @return {Object}                    An object of variableName->variableValue that contains the remapped URL variable values
 * @api private
 */
var replaceOptimizedPaths = function(skinVariables) {
