 ...
 * TODO
 */
module.exports.getFullContentProfile = function() {
 ...
 * TODO
 */
var createLink = module.exports.createLink = function(ctx, visibility, name, description, callback) {
 ...
 * TODO
 */
var createSakaiDoc = module.exports.createSakaiDoc = function() {
 ...
 */
var _createContent = function(ctx, visibility, name, description, contentType, callback) {
    callback = callback || function() {};
    ...
    // Create the content
    Cassandra.runQuery('INSERT INTO Content (contentId, tenant, visibility, name, description, contentType, lastModified) VALUES (?, ?, ?, ?, ?, ?, ?)', [contentId, ctx.tenant().alias, visibility, name, description, contentType, lastModified], function (err) {
        if (err) {
            callback({'code': 500, 'msg': err});
        } else {
            var contentObj = new Content(ctx.tenant().alias, contentId, visibility, name, description, contentType, lastModified);
...
var getContentMembers = module.exports.getContentMembers = function(ctx, contentId, callback) {
    ...
    getContent(ctx, contentId, function(err, contentObj) {
        ...
        _getContentMembers(contentId, callback);
    });
};
...
/**
 * Update a content item's name or description. This can only be done by the manager of that piece of content.
 * @param  {Context}         ctx                 Standard context object, representing the currently logged user and its tenant
 ...
 *                                               e.g. {'name': 'New content name', 'description': 'New content description'}
 * @param  {Function(err)}   callback            Standard callback function
 * @param  {Object}          callback.err        Error object containing the error message
 */
var updateContentProfile = module.exports.updateContentProfile = function(ctx, contentId, profileFields, callback) {
    ...
    fieldNames.forEach(function(fieldName) {
        validator.check(fieldName, {'code': 400, 'msg': fieldName + ' is not a recognized content profile field'}).isIn(['name', 'description']);
    });
    validator.check(null, {'code': 401, 'msg': 'You have to be logged in to be able to create a content item'}).isLoggedInUser(ctx);
    if (validator.hasErrors()) {
        return callback(validator.getErrors());
...
/**
 * Update a content item's visibility setting. This can only be done by the manager of that piece of content.
 * @param  {Context}         ctx                 Standard context object, representing the currently logged user and its tenant
 * @param  {String}          contentId           The id of the content object for which we want to update the content profile
 * @param  {String}          visibility          The new visibility setting. This can be 'private', 'loggedin' or 'public'
 * @param  {Function(err)}   callback            Standard callback function
 * @param  {Object}          callback.err        Error object containing the error message
 */
var updatePrivacySettings = module.exports.updateVisibility = function(ctx, contentId, visibility, callback) {
    callback = callback || function() {};
    ...
    // Do the actual update
    _updateContentMetadata(ctx, contentId, {'visibility': visibility}, callback);
};

/**
 * Utility function that will be used to update metadata on a piece of content. Both the updateContentProfile and
    ...
    canManage(ctx, contentId, function(err, canManage, contentObj) {
        ...
        for (var m in metadata) {
            if (metadata.hasOwnProperty(m)) {
                q.push("?=?");
                parameters.push(m);
                parameters.push(metadata[m]);
                contentObj[m] = metadata[m];
            }
        }
        parameters.push(contentId);
        Cassandra.runQuery('UPDATE Content SET ' + q.join(', ') + ' WHERE contentId = ?', parameters, function(err) {
            if (err) {
                return callback(err);
            }
...
var _rowToContent = function (row) {
    ...
    return new Content(hash.tenant, hash.contentId, hash.visibility, hash.name, hash.description, hash.contentType, hash.lastModified);
};
