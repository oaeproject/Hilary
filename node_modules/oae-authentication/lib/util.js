...
Cassandra.runQuery('UPDATE "Principals" SET "lastConnected" = ? WHERE "principalId" = ?', [LastConnected.toString() , user.id], function(err) {
        if (err) {
            log().error({'err': err}, 'Error changing a LastConnected');
    ...
    return function(err) {
        if (err) {
            ...
            // Generic error
                ...
                log().error({'err': err, 'host': req.hostname}, 'An error occurred during login');
 ...
 */
var handleExternalGetOrCreateUser = module.exports.handleExternalGetOrCreateUser = function(req, authProvider, externalId, providerProperties, displayName, opts, callback) {
    if (!opts.email) {
    ...
    return require('oae-authentication').getOrCreateUser(ctx, authProvider, externalId, providerProperties, displayName, opts, callback);
};
