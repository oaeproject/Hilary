...
describe('Tenants', function() {
        ...
        it('verify guest tenant is flagged in the me feed', function(callback) {
            ...
            PrincipalsTestUtil.assertGetMeSucceeds(guestRestContext, function(me) {
                ...
                TestsUtil.generateTestUsers(camAdminRestContext, 1, function(err, users, user) {
                    assert.ok(!err);
                    PrincipalsTestUtil.assertGetMeSucceeds(user.restContext, function(me) {
    ...
    describe('Get tenants by an email address', function() {
        ...
        it('verify validation', function(callback) {
            RestAPI.Tenants.getTenantsByEmailAddress(camAdminRestContext, [], function(err, tenants) {
        ...
        it('verify tenants can be looked up through an email address', function(callback) {
            ...
            TestsUtil.setupMultiTenantPrivacyEntities(function(publicTenant0, publicTenant1, privateTenant0, privateTenant1) {
                ...
                var emails = [publicTenant0.privateUser.user.email, privateTenant1.privateUser.user.email];
                RestAPI.Tenants.getTenantsByEmailAddress(publicTenant0.publicUser.restContext, emails, function(err, tenants) {
                    assert.ok(!err);
    ...
    describe('Tenant actions', function() {
        ...
        it('verify create tenant', function(callback) {
            ...
            TenantsTestUtil.createTenantAndWait(globalAdminRestContext, tenantAlias, tenantDescription, tenantHost, {'countryCode': 'ca'}, function(err, tenant) {
                assert.ok(!err);
                assert.ok(tenant);
                assert.strictEqual(tenant.alias, tenantAlias);
                assert.strictEqual(tenant.host, tenantHost.toLowerCase());
        ...
        it('verify create tenant validation', function(callback) {
            ...
            TenantsTestUtil.createTenantAndWait(globalAdminRestContext, null, 'AAR', tenantHost, null, function(err) {
                assert.ok(err);
                assert.equal(err.code, 400);

                // Try creating a tenant with an invalid alias, using spaces in the alias
                TenantsTestUtil.createTenantAndWait(globalAdminRestContext, 'American Academic of Religion', 'AAR', tenantHost, null, function(err) {
                    assert.ok(err);
                    assert.equal(err.code, 400);

                    // Try creating a tenant with an invalid alias, using a colon in the alias
                    TenantsTestUtil.createTenantAndWait(globalAdminRestContext, 'aar:test', 'AAR', tenantHost, null, function(err) {
                        assert.ok(err);
                        assert.equal(err.code, 400);

                        // Try creating a tenant with an alias that's already taken
                        TenantsTestUtil.createTenantAndWait(globalAdminRestContext, 'camtest', 'Cambridge University', tenantHost, null, function(err) {
                            assert.ok(err);
                            assert.equal(err.code, 400);

                            // Try creating a tenant with no displayName
                            TenantsTestUtil.createTenantAndWait(globalAdminRestContext, tenantAlias, null, tenantHost, null, function(err) {
                                assert.ok(err);
                                assert.equal(err.code, 400);

                                // Try creating a tenant with no base URL
                                TenantsTestUtil.createTenantAndWait(globalAdminRestContext, tenantAlias, 'AAR', null, null, function(err) {
                                    assert.ok(err);
                                    assert.equal(err.code, 400);
                                                                        ...
                                                                        TenantsTestUtil.updateTenantAndWait(tenant4AdminRestContext, null, {'displayName': 'Cambridge University Test', 'host': 'cambridge.oae.com'}, function(err) {
                                                                            ...
                                                                            RestAPI.Tenants.getTenant(tenant4AdminRestContext, null, function(err, tenant) {
                                                                                ...
                                                                                RestAPI.Tenants.getTenant(camAdminRestContext, null, function(err, tenant) {
                                                                                    ...
                                                                                    TenantsTestUtil.updateTenantAndWait(camAdminRestContext, null, {'countryCode': 'ca'}, function(err) {
                                                                                        RestAPI.Tenants.getTenant(camAdminRestContext, null, function(err, tenant) {
                                                                                            assert.ok(!err);
                                                                                            assert.strictEqual(tenant.countryCode, 'CA');

                                                                                            // Unset the country code, ensuring it changed
                                                                                            TenantsTestUtil.updateTenantAndWait(camAdminRestContext, null, {'countryCode': ''}, function(err) {
                                                                                                RestAPI.Tenants.getTenant(camAdminRestContext, null, function(err, tenant) {
                                                                                                    assert.ok(!err);
