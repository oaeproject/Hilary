...
var generatePreviews = module.exports.generatePreviews = function(ctx, callback) {
    ...
    ctx.download(function(err, path) {
        ...
        var sizes = [
            { 'width': PreviewConstants.SIZES.MEDIUM, 'height': PreviewConstants.SIZES.MEDIUM, 'name': 'medium' },
            { 'width': PreviewConstants.SIZES.SMALL, 'height': PreviewConstants.SIZES.SMALL, 'name': 'small' }
        ];
 ...
 * @param {Object[]}            sizes           An array of size object. Each object should have a `width`, `height` and `name` key. The name key will be used in the filename.
 * @param {Function}            callback        Standard callback method.
 * @param {Object}              callback.err    Standard error object (if any.)
 */
 ...
 * @param {String}              size.name       The name for this image.
 * @param {Function}            callback        Standard callback method.
 * @param {Object}              callback.err    Standard error object (if any.)
 * @api private
 */
var _resize = function(ctx, path, size, callback) {
    log().trace('%s - Resizing image %s to %s x %s', ctx.contentId, path, size.width, size.height);
    ImageUtil.resizeImage(path, size, function(err, file) {
        if (err) {
            return callback(err);
        }

        // Grab the filename and strip of the extension (if any)
        var filename = Path.basename(path);
        var to = filename.lastIndexOf('.') !== -1 ? filename.lastIndexOf('.') : filename.length;
        filename = filename.substr(0, to);

        // Move the resized image to the previews directory for this piece of content,
        var resizedPath = ctx.dirs.previews + '/' + filename + '.' + size.name + '.png';
        shell.mv(file.path, resizedPath);

        // and add it to the set of previews that should be attached to it.
        ctx.addPreview(resizedPath, size.name);
        callback();
    });
};
...
var cropThumbnail = module.exports.cropThumbnail = function(ctx, path, callback) {
    log().trace('%s - Cropping image: %s', ctx.contentId, path);
    gm(path).size(function (err, size) {
        ...
        if (width === height) {
            ImageUtil.resizeImage(path, { 'width': PreviewConstants.SIZES.THUMBNAIL, 'height': PreviewConstants.SIZES.THUMBNAIL }, function(err, thumbnailFile) {
                if (err) {
                    return callback(err);
                }
                // Move the files to the preview directory
                var thumbnailPath = ctx.dirs.base + '/thumbnail.png';
                shell.mv(thumbnailFile.path, thumbnailPath);
            ...
            // crop it.
            ImageUtil.cropAndResize(path, selectedArea, [ PreviewConstants.SIZES.THUMBNAIL ], function(err, files) {
                if (err) {
                    return callback(err);
                }
                ...
                var thumbnailPath = ctx.dirs.base + '/thumbnail.png';
                shell.mv(files[PreviewConstants.SIZES.THUMBNAIL].path, thumbnailPath);
