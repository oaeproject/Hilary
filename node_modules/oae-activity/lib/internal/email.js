...
ActivityEmitter.on(ActivityConstants.events.DELIVERED_ACTIVITIES, function(deliveredActivities) {
    ...
    // Iterate over the delivered activities and get those who are delivered to users their email stream
    _.each(deliveredActivities, function(streamTypeActivities, resourceId) {
        if (AuthzUtil.isUserId(resourceId)) {
            _.each(streamTypeActivities, function(activityInfo, streamType) {
                if (streamType === 'email') {
                    usersToMail.push(resourceId);
                }
            });
        }
    });

    // Some users might've received multiple notifications, reduce the array to the unique values
    // so we don't send an email twice
    usersToMail = _.uniq(usersToMail);
 ...
 * @api private
 */
var _collectMails = function(bucketId, oldestActivityTimestamp, start, callback, _collectedUsers) {
    // Keep track of whom we sent a mail to
    _collectedUsers = _collectedUsers || [];

    // 1. Get all the users who are queued for email delivery
    ActivityDAO.getQueuedUserIdsForEmail(bucketId, start, MAX_COLLECTION_BATCH_SIZE, function(err, userIds, nextToken) {
        if (err) {
            return callback(err);
        ...
        // 2. Get the activities from the email streams
        ...
        ActivityDAO.getActivitiesFromStreams(activityStreamIds, oldestActivityTimestamp, function(err, activitiesPerStream) {
            if (err) {
                return callback(err);
            }
            ...
            _.each(activitiesPerStream, function(activities, activityStreamId) {
                ...
                if (!hasRecentActivity) {
                    ...
                    userIdsToMail.push(ActivityUtil.parseActivityStreamId(activityStreamId).resourceId);
                }
            });
            ...
            ActivityDAO.resetAggregationForActivityStreams(_.keys(activitiesPerMailableStreams), function(err) {
                ...
                ActivityDAO.unqueueUsersForEmail(bucketId, userIdsToMail, function(err) {
                    if (err) {
                        return callback(err);
                    }
                    ...
                    ActivityDAO.deleteActivities(activitiesPerStreamToDelete, function(err) {
                        ...
                        PrincipalsDAO.getPrincipals(userIdsToMail, null, function(err, users) {
                            if (err) {
                                log().error({'err': err, 'userIds': userIdsToMail}, 'Failed to get the users when sending email');
                                return callback(err);
                            }
                            ...
                            var usersToMail = _.filter(users, function(user) {
                                ...
                                return (user.emailPreference !== PrincipalsConstants.emailPreferences.NEVER && !_.isEmpty(activitiesPerMailableStreams[emailActivityStreamId]));
                            });

                            // Transform the users array so the activities are included
                            var toMail = _.map(usersToMail, function(user) {
                                var emailActivityStreamId = ActivityUtil.createActivityStreamId(user.id, ActivityConstants.streams.EMAIL);
                                return {'user': user, 'activities': activitiesPerMailableStreams[emailActivityStreamId]};
                            });
                            ...
                            _mailAll(toMail, function(err) {
                                ...
                                _collectedUsers = _collectedUsers.concat(usersToMail);

                                if (nextToken) {
                                    _collectMails(bucketId, oldestActivityTimestamp, nextToken, callback, _collectedUsers);
                                } else {
                                    return callback(null, _collectedUsers);
                                }
                            });
                        });
                    });
...
var _mail = function(user, activities, callback) {
    ...
    ActivityTransformer.transformActivities(ctx, aggregatedActivities, ActivityConstants.transformerTypes.ACTIVITYSTREAMS, function(err) {
        ...
        var adaptedActivities = ActivityAdapter.adapt(user.id, user, aggregatedActivities, Sanitization);
