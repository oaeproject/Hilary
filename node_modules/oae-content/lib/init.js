    ...
    ContentSearch.init(function(err) {
        ...
        mkdirp(config.files.uploadDir, function(err) {
            if (err && err.code !== "EEXIST") {
                log().error(
                    { err: err },
                    "Could not create the directory where uploaded files can be stored."
                );
                return callback(err);
            }
            ...
            if (config.files.cleaner.enabled) {
                ...
                Cleaner.start(
                    ...
                    config.files.cleaner.interval
                );
            }

            LocalStorage.init(config.files.localStorageDirectory, function(
                err
            ) {
                if (err) {
                    return callback(err);
