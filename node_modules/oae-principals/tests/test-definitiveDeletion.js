...
describe('Delete user', function() {
    ...
    describe('Delete user - Principals', function() {
         ...
         */
        it('Verify if the DAO function get the correct expired users', function(callback) {
            ...
            TestsUtil.generateTestUsers(camAdminRestContext, 2, function(err, users, userToDelete, userArchive) {
                ...
                // Delete User - step 1
                PrincipalsTestUtil.assertDeleteUsersSucceeds(camAdminRestContext, userToDelete, userArchive, function(err, resUserArchive) { 
                    assert.ok(!err); 
                    ...
                    PrincipalsDAO.getExpiredUser(actualDate, function(err, expiredUsers) {
                        assert.ok(!err);
                        assert.ok(!_.find(expiredUsers, function(expiredUser) { return expiredUser.principalId == userToDelete.user.id; }));

                        var months = PrincipalsConfig.getValue(userArchive.user.tenant.alias, 'user', 'delete');
                        
                        if (months) {
                            actualDate.setMonth(actualDate.getMonth() + parseInt(months) + 1);
                            actualDate.setYear(actualDate.getFullYear() + Math.trunc((actualDate.getMonth() + parseInt(months) + 1) / 12));
                        } else {
                            actualDate.setMonth(actualDate.getMonth() + DEFAULT_MONTH + 1);
                            actualDate.setYear(actualDate.getFullYear() + Math.trunc((actualDate.getMonth() + DEFAULT_MONTH + 1) / 12));
                        }

                        // Get expired principals
                        // The deleted user should be considered as an expired user because the date in the database is outdated
                        PrincipalsDAO.getExpiredUser(actualDate, function(err, otherExpiredUsers) {
                            assert.ok(!err);
                            assert.ok(_.find(otherExpiredUsers, function(otherExpiredUser) { return otherExpiredUser.principalId == userToDelete.user.id; }));

                            // Delete User - step 2
                            PrincipalsDefinitiveDeletionAPI.deleteUser(camAdminRestContext, userToDelete.user, resUserArchive.tenantAlias, function(err) {
                                assert.ok(!err); 

                                // Get expired principals
                                // The deleted user shouldn't appear because the user is already marked has fully deleted 
                                PrincipalsDAO.getExpiredUser(actualDate, function(err, moreExpiredUsers) {
                                    assert.ok(!err);
                                    assert.ok(!_.find(moreExpiredUsers, function(moreExpiredUser) { return moreExpiredUser.principalId == userToDelete.user.id; }));
                                    return callback();
                                });
                            });
                        });
         ...
         */
        it('Verify if the deletion fails when we try to delete a user archive', function(callback) {
            ...
            TestsUtil.generateTestUsers(camAdminRestContext, 2, function(err, users, userToDelete, userArchive) {
                ...
                PrincipalsDAO.updateUserArchiveFlag(userArchive.user.id, function(err) { 
                    assert.ok(!err); 

                    // Delete User - step 1
                    PrincipalsTestUtil.assertDeleteUserFails(camAdminRestContext, userArchive.user.id, 401, function(err) { 
                        assert.ok(!err); 
         ...
         */
        it('Verify if user still alive && marked like removed after the deletion', function(callback) {
         ...
         */
        it('Verify if user is removed from data base after the fully deletion', function(callback) {
                ...
                PrincipalsTestUtil.assertDeleteUsersSucceeds(camAdminRestContext, userToDelete, userArchive, function(err, resUserArchive) { 
                    ...
                    PrincipalsDefinitiveDeletionAPI.deleteUser(camAdminRestContext, userToDelete.user, resUserArchive.tenantAlias, function(err) {
                        ...
                        PrincipalsDAO.getPrincipalFromCassandra(userToDelete.user.id, function(err, user) {
                            ...
                            PrincipalsDAO.getDataFromArchive(userArchive.user.id, userToDelete.user.id, function(err, user) {
                                assert.ok(err);
                                assert.ok(!user);
         ...
         */
        it('Verify if the profile picture has been deleted from the file system after the fully deletion', function(callback) {
    ...
    describe('Delete user - step 1 : "first deletion"', function() {
        
        /**
         * Test that verifies definitive delation remove user roles on link
         */
        it('Verify if the user role is removed from the link', function(callback) {
            // Generate a deleted user to test with
            TestsUtil.generateTestUsers(camAdminRestContext, 4, function(err, users, user, otherUser, userToDelete, userArchive) {
                assert.ok(!err);
                        ...
                        PrincipalsTestUtil.generateRightContent(userToDelete, user, 'manager', links[0], function(err) {
                            ...
                            PrincipalsTestUtil.generateRightContent(userToDelete, user, 'viewer', links[1], function(err) {
                                ...
                                PrincipalsTestUtil.generateRightContent(user, userToDelete, 'viewer', link[0], function(err) {
                                    ...
                                    // Delete User
                                    PrincipalsTestUtil.assertDeleteUsersSucceeds(camAdminRestContext, userToDelete, userArchive, function(err, userArchive) {
                                        assert.ok(!err); 
                                             ...
                                             AuthzAPI.hasAnyRole(userArchive.archiveId, links[0].id, function(err, hasRole) {
                                                ...
                                                AuthzAPI.hasRole(user.user.id, links[0].id, 'manager', function(err, hasRole) {
                                                    ...
                                                    AuthzAPI.hasAnyRole(userToDelete.user.id, links[1].id, function(err, hasRole) {
                                                        assert.equal(hasRole, false);
                                                        AuthzAPI.hasRole(userArchive.archiveId, links[1].id, 'manager', function(err, hasRole) {
                                                            assert.equal(hasRole, true);
         ...
         */
        it('Verify if the user role is removed from the collabdoc', function(callback) {
            // Generate a deleted user to test with
            TestsUtil.generateTestUsers(camAdminRestContext, 4, function(err, users, user, otherUser, userToDelete, userArchive) {
                assert.ok(!err);
                                                     ...
                                                     AuthzAPI.hasAnyRole(userArchive.archiveId, collabdocs[0].id, function(err, hasRole) {
                                                        ...
                                                        AuthzAPI.hasRole(user.user.id, collabdocs[0].id, 'manager', function(err, hasRole) {
                                                            ...
                                                            AuthzAPI.hasAnyRole(userToDelete.user.id, collabdocs[1].id, function(err, hasRole) {
                                                                assert.equal(hasRole, false);
                                                                AuthzAPI.hasRole(userArchive.archiveId, collabdocs[1].id, 'manager', function(err, hasRole) {
                                                                    assert.equal(hasRole, true);
                                                                    AuthzAPI.hasRole(user.user.id, collabdocs[1].id, 'editor', function(err, hasRole) {
                                                                        assert.equal(hasRole, true);
         ...
         */
        it('Verify if the user role is removed from the file', function(callback) {
            // Generate a deleted user to test with
            TestsUtil.generateTestUsers(camAdminRestContext, 4, function(err, users, user, otherUser, userToDelete, userArchive) {
                assert.ok(!err);
                                             ...
                                             AuthzAPI.hasAnyRole(userArchive.archiveId, files[0].id, function(err, hasRole) {
                                                ...
                                                AuthzAPI.hasRole(user.user.id, files[0].id, 'manager', function(err, hasRole) {
                                                    ...
                                                    AuthzAPI.hasAnyRole(userToDelete.user.id, files[1].id, function(err, hasRole) {
                                                        assert.equal(hasRole, false);
                                                        AuthzAPI.hasRole(userArchive.archiveId, files[1].id, 'manager', function(err, hasRole) {
                                                            assert.equal(hasRole, true);
         ...
         */
        it('Verify if the user role is removed from the meeting', function(callback) {
           // Generate a deleted user to test with
            TestsUtil.generateTestUsers(camAdminRestContext, 4, function(err, users, user, otherUser, userToDelete, userArchive) {
                assert.ok(!err);
                                             ...
                                             AuthzAPI.hasAnyRole(userArchive.archiveId, meetings[0].id, function(err, hasRole) {
                                                ...
                                                AuthzAPI.hasRole(user.user.id, meetings[0].id, 'manager', function(err, hasRole) {
                                                    ...
                                                    AuthzAPI.hasAnyRole(userToDelete.user.id, meetings[1].id, function(err, hasRole) {
                                                        assert.equal(hasRole, false);
                                                        AuthzAPI.hasRole(userArchive.archiveId, meetings[1].id, 'manager', function(err, hasRole) {
                                                            assert.equal(hasRole, true);
                                                            AuthzAPI.hasRole(user.user.id, meetings[1].id, 'member', function(err, hasRole) {
                                                                assert.equal(hasRole, true);
         ...
         */
        it('Verify if the user role is removed from the discussion', function(callback) {
                                             ...
                                             AuthzAPI.hasAnyRole(userArchive.archiveId, discussions[0].id, function(err, hasRole) {
                                                ...
                                                AuthzAPI.hasRole(user.user.id, discussions[0].id, 'manager', function(err, hasRole) {
                                                    ...
                                                    AuthzAPI.hasAnyRole(userToDelete.user.id, discussions[1].id, function(err, hasRole) {
                                                        assert.equal(hasRole, false);
                                                        AuthzAPI.hasRole(userArchive.archiveId, discussions[1].id, 'manager', function(err, hasRole) {
                                                            assert.equal(hasRole, true);
                                                            AuthzAPI.hasRole(user.user.id, discussions[1].id, 'member', function(err, hasRole) {
                                                                assert.equal(hasRole, true);
         ...
         */
        it('Verify if the user role is removed from the group', function(callback) {
                                            ...
                                            AuthzAPI.hasAnyRole(userArchive.archiveId, groups[0].id, function(err, hasRole) {
                                                ...
                                                AuthzAPI.hasRole(user.user.id, groups[0].id, 'manager', function(err, hasRole) {
                                                    ...
                                                    AuthzAPI.hasAnyRole(userToDelete.user.id, groups[1].id, function(err, hasRole) {
                                                        assert.equal(hasRole, false);
                                                        AuthzAPI.hasRole(userArchive.archiveId, groups[1].id, 'manager', function(err, hasRole) {
                                                            assert.equal(hasRole, true);
                                                            AuthzAPI.hasRole(user.user.id, groups[1].id, 'member', function(err, hasRole) {
                                                                assert.equal(hasRole, true);
         ...
         */
        it('Verify if the user role is removed from the folder', function(callback) {
                                             ...
                                             AuthzAPI.hasAnyRole(userArchive.archiveId, folders[0].groupId, function(err, hasRole) {
                                                ...
                                                AuthzAPI.hasRole(user.user.id, folders[0].groupId, 'manager', function(err, hasRole) {
                                                    ...
                                                    AuthzAPI.hasAnyRole(userToDelete.user.id, folders[1].groupId, function(err, hasRole) {
                                                        assert.equal(hasRole, false);
                                                        AuthzAPI.hasRole(userArchive.archiveId, folders[1].groupId, 'manager', function(err, hasRole) {
                                                            assert.equal(hasRole, true);
                                                            AuthzAPI.hasRole(user.user.id, folders[1].groupId, 'viewer', function(err, hasRole) {
                                                                assert.equal(hasRole, true);
         ...
         */
        it('Verify the veracity of the data contained in the DataArchive table', function(callback) {
         ...
         * Test that verifies the data in the table DataArchive
         */
        it('Verify if discussions are in the library of userArchive after the deletion of a user', function(callback) {
         ...
         */
        it('Verify if groups are in the library of userArchive after the deletion of a user', function(callback) {
         ...
         * Test that verifies if elements are in the library
         */
        it('Verify if contents are in the library of userArchive after the deletion of a user', function(callback) {
         ...
         * Test that verifies if elements are in the library
         */
        it('Verify if meetings are in the library of userArchive after the deletion of a user', function(callback) {
            // Generate a deleted user to test with
            TestsUtil.generateTestUsers(camAdminRestContext, 2, function(err, users, userToDelete, userArchive) {
                assert.ok(!err);
         ...
         * Test that verifies if elements are in the library
         */
        it('Verify if folders are in the library of userArchive after the deletion of a user', function(callback) {
            // Generate a deleted user to test with
            TestsUtil.generateTestUsers(camAdminRestContext, 2, function(err, users, userToDelete, userArchive) {
                assert.ok(!err);
    ...
    describe('Delete user - step 2 "fully deletion"', function() {

        /**
         * Test that verifies removing followers 
         */
        it('Verify if fully deletion remove followers', function(callback) {
            // Generate a deleted user to test with
            TestsUtil.generateTestUsers(camAdminRestContext, 1, function(err, users, userArchive) {
                ...
                FollowingTestsUtil.createFollowerAndFollowed(camAdminRestContext, function(follower, followed) {
                    ...
                    FollowingTestsUtil.assertFollows(follower.user.id, follower.restContext, followed.user.id, followed.restContext, function() {
                        ...
                        PrincipalsTestUtil.assertDeleteUsersSucceeds(camAdminRestContext, followed, userArchive, function(err, userArchive) { 
                            assert.ok(!err); 
                            assert.ok(userArchive);

                            // Delete user
                            PrincipalsDefinitiveDeletionAPI.deleteUser(camAdminRestContext, followed.user, userArchive.tenantAlias, function(err) {
                                assert.ok(!err); 

                                // Ensure the follower and following feeds indicate they are no longer following
                                PrincipalsTestUtil.assertDoesNotFollow(follower.user.id, followed.user.id, callback);
                            });
                        });
                    });
                });
         ...
         */
        it('Verify if fully deletion remove following', function(callback) {
            // Generate a deleted user to test with
            TestsUtil.generateTestUsers(camAdminRestContext, 1, function(err, users, userArchive) {
                assert.ok(!err);
                ...
                FollowingTestsUtil.createFollowerAndFollowed(camAdminRestContext, function(follower, followed) {
                    ...
                    FollowingTestsUtil.assertFollows(follower.user.id, follower.restContext, followed.user.id, followed.restContext, function() {
                        ...
                        // Delete user
                        PrincipalsTestUtil.assertDeleteUsersSucceeds(camAdminRestContext, follower, userArchive, function(err, userArchive) { 
                            assert.ok(!err); 
                            assert.ok(userArchive);

                            // Delete user
                            PrincipalsDefinitiveDeletionAPI.deleteUser(camAdminRestContext, follower.user, userArchive.tenantAlias, function(err) {
                                assert.ok(!err); 
         ...
         */
        it('Verify if fully deletion gives the good rights to the user archive on meeting && verify the table DataArchive', function(callback) {
            ...
            TestsUtil.generateTestUsers(camAdminRestContext, 4, function(err, users, user, otherUser, userToDelete, userArchive) {
                ...
                PrincipalsTestUtil.generateMeetings(userToDelete.restContext, userToDelete.user, 'private', 1, function(err, meeting) {
                    ...
                    PrincipalsTestUtil.assertDeleteUsersSucceeds(camAdminRestContext, userToDelete, userArchive, function(err, resUserArchive) { 
                        ...
                        // Get Data and compare it with the id list
                        PrincipalsDAO.getDataFromArchive(userArchive.archiveId, userToDelete.user.id, function(err, data) {
                            assert.ok(!err); 
                            assert.deepEqual(data.resourceId, meeting[0].id);
         ...
         */
        it('Verify if fully deletion gives the good rights to the user archive on content && verify the table DataArchive', function(callback) {
                    ...
                    PrincipalsTestUtil.generateLinks(userToDelete.restContext, 'private', 1,  function(err, link) {
                        ...
                        PrincipalsTestUtil.generateCollabdocs(userToDelete.restContext, 'private', 1,  function(err, collabdoc) {
                            ...
                            PrincipalsTestUtil.assertDeleteUsersSucceeds(camAdminRestContext, userToDelete, userArchive, function(err, resUserArchive) { 
                                ...
                                PrincipalsDAO.getDataFromArchive(userArchive.archiveId, userToDelete.user.id, function(err, data) {
                                    assert.ok(!err); 
                                    var listContentFromArchive = data.resourceId.split(',');
                                    var listContent = [link[0].id, file[0].id, collabdoc[0].id];
                                    assert.deepEqual(listContentFromArchive.sort(), listContent.sort());
         ...
         */
        it('Verify if fully deletion gives the good rights to the user archive on group && verify the table DataArchive', function(callback) {
            ...
            TestsUtil.generateTestUsers(camAdminRestContext, 4, function(err, users, user, otherUser, userToDelete, userArchive) {
                ...
                PrincipalsTestUtil.generateGroups(userToDelete.restContext, 'private', 1,  function(err, group) {
                    ...
                    PrincipalsTestUtil.assertDeleteUsersSucceeds(camAdminRestContext, userToDelete, userArchive, function(err, resUserArchive) { 
                        ...
                        PrincipalsDAO.getDataFromArchive(userArchive.archiveId, userToDelete.user.id, function(err, data) {
                            assert.ok(!err); 
                            assert.deepEqual(data.resourceId, group[0].id);
         ...
         */
        it('Verify if fully deletion gives the good rights to the user archive on discussion && verify the table DataArchive', function(callback) {
            ...
            TestsUtil.generateTestUsers(camAdminRestContext, 4, function(err, users, user, otherUser, userToDelete, userArchive) {
                ...
                PrincipalsTestUtil.generateDiscussions(userToDelete.restContext, 'private', 1,  function(err, discussion) {
                    ...
                    PrincipalsTestUtil.assertDeleteUsersSucceeds(camAdminRestContext, userToDelete, userArchive, function(err, resUserArchive) { 
                        ...
                        PrincipalsDAO.getDataFromArchive(userArchive.archiveId, userToDelete.user.id, function(err, data) {
                            assert.ok(!err); 
                            assert.deepEqual(data.resourceId, discussion[0].id);
         ...
         */
        it('Verify if fully deletion give the good rights to the user archive on folder && verify the table DataArchive', function(callback) {
            ...
            TestsUtil.generateTestUsers(camAdminRestContext, 4, function(err, users, user, otherUser, userToDelete, userArchive) {
                ...
                PrincipalsTestUtil.generateFolders(userToDelete, 'private', 1,  function(err, folder) {
                    ...
                    PrincipalsTestUtil.assertDeleteUsersSucceeds(camAdminRestContext, userToDelete, userArchive, function(err, resUserArchive) { 
                        ...
                        PrincipalsDAO.getDataFromArchive(userArchive.archiveId, userToDelete.user.id, function(err, data) {
                            assert.ok(!err); 
                            assert.deepEqual(data.resourceId, folder[0].id);
         ...
         */
        it('Verify if fully deletion doesn\'t remove the content if a manager is added to the content', function(callback) {
         ...
         * Test that verifies rights of user archive on meeting
         */
        it('Verify if fully deletion doesn\'t remove the content if a manager is added to the meeting', function(callback) {
         ...
         * Test that verifies rights of user archive on discussion
         */
        it('Verify if fully deletion doesn\'t remove the content if a manager is added to the discussion', function(callback) {
         ...
         * Test that verifies rights of user archive on group
         */
        it('Verify if fully deletion doesn\'t remove the content if a manager is added to the group', function(callback) {
         ...
         * Test that verifies rights of user archive on folder
         */
        it('Verify if fully deletion doesn\'t remove the content if a manager is added to the folder', function(callback) {
    ...
    describe('Delete user - Verify if when a user is deleted, a mail to each users is sent && verify the veracity of the data on ... ', function() {

        /**
         * Test that verifies definitive delation send one email to each users
         */
        it('... files', function(callback) {
            // Generate a deleted user to test with
            TestsUtil.generateTestUsers(camAdminRestContext, 4, function(err, users, user, otherUser, userToDelete, userArchive) {
                assert.ok(!err);
                                ...
                                PrincipalsTestUtil.generateRightContent(userToDelete, otherUser, 'viewer', files[1], function(err) {
                                    ...
                                    PrincipalsTestUtil.generateRightContent(user, userToDelete, 'viewer', file[0], function(err) {
                                        ...
                                        PrincipalsTestUtil.generateRightContent(userToDelete, user, 'viewer', files[2], function(err) {
                                            ...
                                            // Delete User
                                            PrincipalsTestUtil.assertDeleteUsersSucceeds(camAdminRestContext, userToDelete, userArchive, function(err, userArchive, listEmail) { 
                                                assert.ok(!err); 
                                                listEmail.sort();
                                                // Verify email
         ...
         * Test that verifies definitive delation send one email to each users
         */
        it('... collabdoc', function(callback) {
