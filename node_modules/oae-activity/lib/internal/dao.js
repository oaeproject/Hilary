...
/**
 * Get a list of activities for a set of activity streams
 *
 ...
 */
var getActivitiesFromMultipleStreams = module.exports.getActivitiesFromMultipleStreams = function(activityStreamIds, callback) {
...
/**
 * Delete activities from multiple activity streams in one go
 *
 * @param  {Object}     activitiesPerStream     A list of activities keyed by their activity stream id which you want to remove
 * @param  {Function}   callback                Standard callback function
 * @param  {Object}     callback.err            Standard error object, if any
 */
var deleteActivitiesFromMultipleStreams = module.exports.deleteActivitiesFromMultipleStreams = function(activitiesPerStream, callback) {
    var queries = [];
    _.each(activitiesPerStream, function(activities, activityStreamId) {
        _.each(activities, function(activity) {
            queries.push({
                ...
                'parameters': [activityStreamId, activity['oae:activityId']]
            });
        });
    });
...
var resetAggregationForActivityStreams = module.exports.resetAggregationForActivityStreams = function(activityStreamIds, callback) {
    ...
    getActiveAggregateKeysForActivityStreams(activityStreamIds, function(err, activeAggregateKeysForActivityStreams) {
        ...
        // Delete all the active aggregate keys
...
/**
 * Queues a set of users for email collection.
 * You can specifiy whether a user should have his email delivered in the next collection cycle or in the daily interval.
 *
 * Users will be divided up in buckets so multiple machines can collect a bucket and send out mail.
 * A user will always end up in the same bucket. This is to ensure that a single machine can send out an e-mail
 * that contains *ALL* the relevant activities for that user.
 *
 * Buckets will be stored in Cassandra.
 *
 * @param  {Object}     users           Keys a user's id to their mail preference
 * @param  {Function}   callback        Standard callback function
 * @param  {Object}     callback.err    Standard error object, if any
 */
var queueUsersForEmail = module.exports.queueUsersForEmail = function(users, callback) {
    var buckets = {};
    _.each(users, function(userDetails, userId) {
        var bucket = _createEmailBucketForUser(userId, userDetails.emailPreference, userDetails.timezone);
        buckets[bucket] = buckets[bucket] || [];
        buckets[bucket].push(userId);
    });

    var queries = [];
    _.each(buckets, function(userIds, bucket) {
        _.each(userIds, function(userId) {
            queries.push({'query': 'INSERT INTO "EmailBuckets" ("bucket", "userId") VALUES (?, ?)', 'parameters': [bucket, userId]});
        });
    });
 ...
 *
 * @param  {Number}     bucketNumber        Which bucket to collect
 ...
 * @param  {Function}   callback            Standard callback function
 * @param  {Object}     callback.err        Standard error object, if any
 * @param  {Object[]}   callback.users      A set of objects, each object contains a `userId` and a `preference`
 ...
 */
var getQueuedUsersForEmail = module.exports.getQueuedUsersForEmail = function(bucketNumber, emailPreference, hour, start, limit, callback) {
    var bucket = _createEmailBucket(bucketNumber, emailPreference, hour);
    Cassandra.runPagedQuery('EmailBuckets', 'bucket', bucket, 'userId', start, limit, {'end': 'z'}, function(err, rows, nextToken) {
        if (err) {
            return callback(err);
        }
        ...
        var users = [];
        _.each(rows, function(row) {
            if (row.get('bucket') && row.get('userId')) {
                var bucket = row.get('bucket').value;
                var preference = _getEmailPreferenceFromBucket(bucket);
                var userId = row.get('userId').value;
                users.push({'userId': userId, 'preference': preference});
            }
        });
...
/**
 * Removes a set of users from a bucket.
 *
 * @param  {Number}     bucketNumber    Which bucket to remove the users from
 * @param  {Object[]}   users           Which users should be removed, as returned by `getQueuedUsersForEmail`
 * @param  {Function}   callback        Standard callback function
 * @param  {Object}     callback.err    Standard error object, if any
 */
var unqueueUsersForEmail = module.exports.unqueueUsersForEmail = function(bucketNumber, users, hour, callback) {
    var queries = [];
    _.each(users, function(user) {
        var bucket = _createEmailBucket(bucketNumber, user.preference, hour);
        queries.push({'query': 'DELETE FROM "EmailBuckets" WHERE "bucket" = ? AND "userId" = ?', 'parameters': [bucket, user.userId]});
    });
    Cassandra.runBatchQuery(queries, callback);
};

/**
 * Given a user ID and his email preference, return the approriate email bucket
 *
 * @param  {String}     userId              The ID of the user for whom to retrieve the email bucket
 * @param  {String}     emailPreference     The user his email preference. One of {@see PrincipalsConstants.emailPreferences}
 * @param  {String}     timezone            The user's timezone. Defaults to `UTC`
 * @return {String}                         The email bucket that this user should go in
 * @api private
 */
var _createEmailBucketForUser = function(userId, emailPreference, timezone) {
    timezone = timezone || 'UTC';
    var bucketNumber = _getBucketNumber(userId);

    var hour = 0;
    // Take the user's timezone into account for daily and/or weekly mails
    if (emailPreference !== PrincipalsConstants.emailPreferences.IMMEDIATE) {
        var mailConfig = ActivitySystemConfig.getConfig().mail;
        var userTimezoneOffset = (new TZ.Date(timezone).getTimezoneOffset() / 60) % 60;
        hour = (mailConfig[emailPreference].hour + userTimezoneOffset + 24) % 24;
    }

    return _createEmailBucket(bucketNumber, emailPreference, hour);
};

/**
 * Given a bucket number and an email preference, return the approriate email bucket
 *
 ...
 * @param  {Number}     hour                What hour the email bucket should be delivered
 * @return {String}                         The email bucket that this user should go in
 * @api private
 */
var _createEmailBucket = function(bucketNumber, emailPreference, hour) {
...
/**
 * Given an email bucket, retrieve the e-mail preference
 *
 * @param  {String}     bucket      The bucket from which to retrieve the email preference
 * @return {String}                 The email preference
 * @api private
 */
var _getEmailPreferenceFromBucket = function(bucket) {
