...
/**
 * Get content informations and put them into the zip object
 *
 * @param  {Context}    ctx                 Standard context object containing the current user and the current tenant
 * @param  {String}     userId              The id of the user for which to get his personal data
 * @param  {String}     exportType          Export type can be 'personal-data', 'content' or 'shared'
 * @param  {object}     zip                 An object containing all the data
 * @param  {Function}   callback            Standard callback function
 * @param  {Object}     callback.err        An error that occurred, if any
 * @param  {Jszip}      callback.zip        Zip object contain all the data
 */
var _exportContentData = function(ctx, userId, exportType, zip, callback) {
    ...
    ContentAPI.getContentLibraryItems(ctx, userId, null, null, function(err, contents) {
        ...
        // get uploaded files
        _getUploadedFiles(ctx, contentsSplited.file, zip, function(err, zip) { 
            if (err) {
                return callback(err);
            }

            // Convert links into txt file
            _linkToTxt(ctx, contentsSplited.link, zip, function(err, zip) { 
                if (err) {
                    return callback(err);
                }

                // Convert collabdocs into txt file
                _collabdocToTxt(ctx, contentsSplited.collabdoc, zip, function(err, zip) { 
                    if (err) {
                        return callback(err);
                    }
                    ...
                    MeetingsAPI.Meetings.getMeetingsLibrary(ctx, userId, null, null, function(err, meetings) {
                        ...
                        _meetingToTxt(ctx, meetings, zip, function(err, zip) {
                            if (err) {
                                return callback(err);
                            }
                            ...
                            DiscussionsAPI.Discussions.getDiscussionsLibrary(ctx, userId, null, null, function(err, discussions) {
                                ...
                                // Convert meetings into txt file
                                _discussionToTxt(ctx, discussions, zip, function(err, zip) {
                                    if (err) {
                                        return callback(err);
                                    }

                                    // Convert an object to a zip file
                                    _objectToZipFile(zip, function(zipFile) {
                                        return callback(null, zipFile, zip);
                                    });
                                });
                            });
...
/**
 * Get information about uploaded files and put them into the zip object
 *
 * @param  {Context}    ctx                 Standard context object containing the current user and the current tenant
 ...
 * @param  {Jszip}      callback.zip        Zip object contain all the data
 */
var _getUploadedFiles = function(ctx, uploadedFiles, zip, callback) {
    ...
    async.mapSeries(uploadedFiles, function(uploadedFile, outerCB) {
        ...
        zip.upload.push({'path': filePath + '/' + path + '/' + nameWithoutSpace, 'title': nameWithoutSpace});
...
/**
 * Get information about collabdocs and put them into the zip object
 *
 * @param  {Context}    ctx                 Standard context object containing the current user and the current tenant
 ...
 * @param  {Jszip}      callback.zip        Zip object contain all the data
 */
var _collabdocToTxt = function(ctx, collabdocs, zip, callback) {
            ...
            ContentAPI.getRevision(ctx, collabdoc.id, collabdoc.latestRevisionId, function(err, revision) {
                ...
                _getMessages(txtCollabdoc, messages, function(txtCollabdocWithMessages) {
                    _clearName(collabdoc.displayName, function(err, fileName) {
                        zip.collabdoc.push({'text': txtCollabdocWithMessages, 'title': fileName + '.txt'});
                        outerCB();
                    });
                });
            });
...
/**
 * Get information about links and put them into the zip object
 *
 * @param  {Context}    ctx                 Standard context object containing the current user and the current tenant
 ...
 * @param  {Jszip}      callback.zip        Zip object contain all the data
 */
var _linkToTxt = function(ctx, links, zip, callback) {
    if (_.isEmpty(links)) {
        return callback(null, zip);
    }
        ...
        ContentAPI.getComments(ctx, link.id, null, null, function(err, messages) {
            ...
            _getMessages(txtLink, messages, function(txtLinkWithMessages) {
                _clearName(link.displayName, function(err, fileName) {
                    zip.link.push({'text': txtLinkWithMessages, 'title': fileName + '.txt'});
                    outerCB();
                });
            });
        });
...
/**
 * Get information about meetings and put them into the zip object
 *
 * @param  {Context}    ctx                 Standard context object containing the current user and the current tenant
 ...
 * @param  {Jszip}      callback.zip        Zip object contain all the data
 */
var _meetingToTxt = function(ctx, meetings, zip, callback) {
        ...
        MeetingsAPI.Meetings.getMessages(ctx, meeting.id, null, null, function(err, messages) {
            ...
            _getMessages(txtMeeting, messages, function(txtMeetingWithMessages) {
                _clearName(meeting.displayName, function(err, fileName) {
                    zip.meeting.push({'text': txtMeetingWithMessages, 'title': fileName + '.txt'});
                    outerCB();
                });
            });
        });
...
/**
 * Get information about discussions and put them into the zip object
 *
 * @param  {Context}    ctx                 Standard context object containing the current user and the current tenant
 ...
 * @param  {Jszip}      callback.zip        Zip object contain all the data
 */
var _discussionToTxt = function(ctx, discussions, zip, callback) {
        ...
        DiscussionsAPI.Discussions.getMessages(ctx, discussion.id, null, null, function(err, messages) {
            ...
            _getMessages(txtDiscussion, messages, function(txtDiscussionWithMessages) {
                _clearName(discussion.displayName, function(err, fileName) {
                    zip.discussion.push({'text': txtDiscussionWithMessages, 'title': fileName + '.txt'});
                    outerCB();
                });
            });
        });
...
/**
 * Build the jszip object 
 *
 * @param  {object}     zip                 An object containing all the data
 * @param  {Function}   callback            Standard callback function
 * @param  {Object}     callback.err        An error that occurred, if any
 * @param  {String}     callback.jszip      Zip object contain all data
 */
_objectToZipFile = function(zip, callback) {
    ...
    async.series([
        ...
        function (CB) {
            ...
            if (zip.personalData) {
                zipFile.file('personal_data.txt', zip.personalData);
                CB();
            } else {
                CB();
            }
        ...
        function (CB) {
            if (zip.profilePicture) {

                fs.readFile(zip.profilePicture.path, function (err, data) {
                    if (err) {
                        return callback(err);
                    }
                    zipFile.file(zip.profilePicture.imageName, data, {base64: false, binary: true});
                    CB();
                });
            } else {
                CB();
        ...
        function (CB) {
            if (zip.upload) {
                ...
                async.mapSeries(zip.upload, function(uploadedFile, outerCB) {
        ...
        function (CB) {
            if (zip.collabdoc) {
                ...
                async.mapSeries(zip.collabdoc, function(collabdoc, outerCB) {
        ...
        function (CB) {
            if (zip.link) {
                ...
                async.mapSeries(zip.link, function(link, outerCB) {
        ...
        function (CB) {
            if (zip.meeting) {
                ...
                async.mapSeries(zip.meeting, function(meeting, outerCB) {
        ...
        function (CB) {
            if (zip.discussion) {
                ...
                async.mapSeries(zip.discussion, function(discussion, outerCB) {
