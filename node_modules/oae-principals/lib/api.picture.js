...
var _storeLargePicture = function(ctx, principalId, file, callback) {
    ...
    ImageUtil.autoOrient(file.path, {'removeInput': true}, function(err, orientedFile) {
        ...
        ImageUtil.convertToJPG(orientedFile.path, function(err, convertedFile) {
            ...
            ContentUtil.getStorageBackend(ctx).store(ctx.tenant().alias, convertedFile, options, function(err, largePictureUri) {
                if (err) {
...
var generateSizes = module.exports.generateSizes = function(ctx, principalId, x, y, width, callback) {
    ...
    validator.check(width, {'code': 400, 'msg': 'The width value must be a positive integer greater than or equal to 10'}).min(10);
    if (validator.hasErrors()) {
...
var _generateSizes = function(ctx, principal, x, y, width, callback) {
    ...
    ContentUtil.getStorageBackend(ctx, principal.picture.largeUri).get(ctx.tenant().alias, principal.picture.largeUri, function(err, file) {
        if (err) {
            return callback(err);
        }
...
var _storeCroppedPictures = function(ctx, principal, files, callback) {
    ...
    var options = _getProfilePictureStorageOptions(principal.id, now, 'small', ImageUtil.getImageExtension(smallImage.name, '.jpg'));
    backend.store(ctx.tenant().alias, smallImage, options, function(err, smallPictureUri) {
        if (err) {
            return callback(err);
        }
        ...
        options = _getProfilePictureStorageOptions(principal.id, now, 'medium', ImageUtil.getImageExtension(mediumImage.name, '.jpg'));
        backend.store(ctx.tenant().alias, mediumImage, options, function(err, mediumPictureUri) {
            if (err) {
                return callback(err);
            }
