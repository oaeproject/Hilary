...
var getPreviewUris = module.exports.getPreviewUris = function(revisionIds, callback) {
    ...
    Cassandra.runInQuery('SELECT "revisionId", "thumbnailUri", "wideUri" FROM "Revisions"', null, 'revisionId', revisionIds, function(err, rows) {
        if (err) {
            return callback(err);
        }
        ...
        rows.forEach(function(row) {
            ...
            var thumbnailUri = row.thumbnailUri
            if (thumbnailUri) {
                previews[revisionId].thumbnailUri = thumbnailUri;
            }

            var wideUri = row.wideUri
            if (wideUri) {
