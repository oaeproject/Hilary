...
ActivityEmitter.on(ActivityConstants.events.DELIVERED_ACTIVITIES, function(deliveredActivities) {
    ...
    incrementNotificationsUnread(userIdsIncrBy, function(err) {
        ...
        _decrementNotifications(nrOfNotifications);
    });
});
...
var markNotificationsRead = module.exports.markNotificationsRead = function(ctx, callback) {
    // Anonymous users don't have notifications
    if (!ctx.user()) {
        return callback({'code': 401, 'msg': 'You cannot mark notifications for anonymous users as read'});
    }
    ...
    ActivityDAO.clearNotificationsUnreadCount(ctx.user().id, function(err) {
        if (err) {
            return callback(err);
        }

        // Update the notifications values in the basic profile
        PrincipalsDAO.updatePrincipal(ctx.user().id, profileFields, function(err) {
            if (err) {
                return callback(err);
            }
            ...
            var notificationActivityStreamId = ActivityUtil.createActivityStreamId(ctx.user().id, 'notification');
            ...
            if (ctx.user().emailPreference === PrincipalsConstants.emailPreferences.IMMEDIATE) {
                var emailActivityStreamId = ActivityUtil.createActivityStreamId(ctx.user().id, 'email');
                ActivityDAO.clearActivityStream(emailActivityStreamId, function(err) {
                    if (err) {
                        log().error({'err': err}, 'Could not clear the email activity stream');
                    }
                });
            }
        });
