...
var assertUpdateUserSucceeds = module.exports.assertUpdateUserSucceeds = function(restCtx, userId, update, callback) {
    ...
    RestAPI.User.getUser(restCtx, userId, function(err, user) {
        ...
        RestAPI.User.getMe(restCtx, function(err, me) {
            ...
            // If an administrator performs the update, there won't be a verification email
            var expectVerificationEmail = (isEmailChange && !isAdmin);
            ...
            OaeUtil.invokeIfNecessary(expectVerificationEmail, onceVerificationEmailSent, update.email, function(_token) {
                token = _token;
                done();
            });
        });
...
var assertResendEmailTokenSucceeds = module.exports.assertResendEmailTokenSucceeds = function(restCtx, userId, callback) {
    // Get the user's profile so we have access to their email address
    RestAPI.User.getUser(restCtx, userId, function(err, user) {
        assert.ok(!err);
        ...
        onceVerificationEmailSent(user.email, function(_token) {
            token = _token;
            done();
        });
    });
 ...
 */
var assertUserEmailMapping = module.exports.assertUserEmailMapping = function(email, expectedUserIds, callback) {
 ...
 */
var onceVerificationEmailSent = module.exports.onceVerificationEmailSent = function(email, callback) {
