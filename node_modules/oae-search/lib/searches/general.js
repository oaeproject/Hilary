 ...
 * @param   {String}        opts.pathParams[0]  The resource type to search
...
module.exports = function(ctx, opts, callback) {
    ...
    opts = opts || {};
    opts.resourceType = SearchUtil.getResourceTypeParam(opts.pathParams[0]);
    opts.includeExternal = (opts.includeExternal === 'true');

    // The query option needs to be resolved as we will determine whether or not the filter by access based on it
    opts.q = SearchUtil.getQueryParam(opts.q);

    Telemetry.incr('general.count');
    if (_needsFilterByAccess(ctx, opts)) {
        ...
        Telemetry.incr('general.access.count');
