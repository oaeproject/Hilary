...
// Cached config schema, will include all configuration options
var cachedSchema = {};
// Cached config values for global admin users, will include all configuration values
var cachedGlobalConfig = {};
// Cached config values for tenant admin users, will include all configuration values except for the ones marked as `valueVisibility: private`
var cachedTenantConfig = {};
// Cached config values for non-admin users, will include all configuration values marked as `valueVisibility: public`
var cachedPublicConfig = {};
...
var getTenantConfig = module.exports.getTenantConfig = function(ctx, tenantAlias, callback) {
    ...
    // configuration without suppressed or private values.
    ...
    callback(null, cachedPublicConfig[tenantAlias]);
};
    ...
    Cassandra.runQuery('SELECT * FROM Config USING CONSISTENCY QUORUM WHERE tenantAlias = ?', [tenantAlias], function(err, rows) {
        ...
        _.each(cachedSchema, function(module, moduleKey) {
            ...
            _.each(module, function(feature, featureKey) {
                ...
                cachedPublicConfig[tenantAlias][moduleKey][featureKey] = {};
                ...
                _.each(feature.elements, function(element, elementKey) {
                    ...
                    if (column) {
                        ...
                        try {
                            ...
                            cachedLastUpdated[tenantAlias][moduleKey][featureKey][elementKey] = column.timestamp;
                        } catch (err) {
                            log().warn({'err': err, 'value': column.value, 'key': key}, 'Could not parse a stored config value');
                        }
                    }

                    // If the config value is available to tenant admins or it was overridden by a tenant admin, add it to the cached tenant config
                    if (element.valueVisibility !== 'private' || overridden) {
                        ...
                        cachedTenantConfig[tenantAlias][moduleKey][featureKey][elementKey] = '';
                    }

                    // If the value can be seen by everyone, add it to the cached suppressed config object
                    if (element.valueVisibility === 'public') {
                        cachedPublicConfig[tenantAlias][moduleKey][featureKey][elementKey] = cachedGlobalConfig[tenantAlias][moduleKey][featureKey][elementKey];
                    }
                });
            });
        });
 ...
 * Tenant administrators cannot update values marked as `valueVisibility: private`; regular users cannot update any configuration values.
 *
 * @param  {Context}   ctx           Standard context object, representing the currently logged in user and its tenant
...
var canUpdateConfigValue = function(ctx, tenantAlias, moduleId, feature, element) {
    ...
    if (!cachedSchema[moduleId] ||
          !cachedSchema[moduleId][feature] ||
          !cachedSchema[moduleId][feature].elements ||
          !cachedSchema[moduleId][feature].elements[element]) {
        return false;
    }
    ...
    var configElement = cachedSchema[moduleId][feature].elements[element];
    ...
    if (!configElement.tenantOverride) {
        return false;
    }
...
var updateConfig = module.exports.updateConfig = function(ctx, tenantAlias, configValues, callback) {
    ...
    // Convert the aggregated keys to something we can store
    var storageValues = {};
    _.each(aggregatedValues, function(value, key) {
        if (_.isObject(value)) {
            storageValues[key] = JSON.stringify(value);
        } else {
            storageValues[key] = value;
        }
    });

    var q = Cassandra.constructUpsertCQL('Config', 'tenantAlias', tenantAlias, storageValues, 'QUORUM');
...
/**
 * Clear a configuration value for a particular tenant so it goes back to using
 * the global setting
 *
 * @param  {Context}    ctx                The current context.
 * @param  {String}     tenantAlias        The alias of the tenant to store config for
 * @param  {String[]}   configFields       The config elements whose values should be cleared in the form 'oae-authentication/twitter/enabled'
 * @param  {Function}   callback           Standard callback function
 * @param  {Object}     callback.err       Error object containing error code and message
 */
var clearConfig = module.exports.clearConfig = function(ctx, tenantAlias, configFields, callback) {
    ...
    var validator = new Validator();
    validator.check(tenantAlias, {'code': 400, 'msg': 'Missing tenantid'}).notEmpty();
    validator.check(tenantAlias, {'code': 400, 'msg': 'Cannot clear config fields on the admin tenant'}).not(config.servers.globalAdminAlias);
    ...
    var queries = [];
    _.each(rowChanges, function(value, columnName) {
        if (!value) {
            queries.push({
                'query': 'DELETE ? FROM Config WHERE tenantAlias = ?',
                'parameters': [columnName, tenantAlias]
            });
        } else {
            var storageValues = {};
            storageValues[columnName] = JSON.stringify(value);
            queries.push(Cassandra.constructUpsertCQL('Config', 'tenantAlias', tenantAlias, storageValues));
        }
    });
...
/**
 * Given a `module`, `feature` and `element`, return a string that can be used as a column name
 *
