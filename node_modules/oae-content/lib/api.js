...
var _getComment = module.exports._getComment = function(ctx, contentId, created, callback) {
    Cassandra.runQuery('SELECT ? FROM CommentsByContent USING CONSISTENCY QUORUM WHERE contentId = ?', [created, contentId], function (err, rows) {
        if (err) {
            return callback(err);
        }
        ...
        Cassandra.runQuery('SELECT * FROM Comments USING CONSISTENCY QUORUM WHERE commentId = ?', [comment[created]], function (err, detailRows) {

            // Get comment from the result set
            comment = Cassandra.rowToHash(detailRows[0]);

            // Get information on the commenter
            var userId = detailRows[0].get('createdBy').value;
            PrincipalsUtil.getPrincipal(ctx, userId, function(err, principal) {
                comment = new Comment(comment.body, comment.commentId, principal, created, comment.lastModified, comment.level, comment.replyTo);
 ...
 * @param  {Object}     callback.err    Error object containing the error message
 */
var postComment = module.exports.postComment = function(ctx, contentId, body, callback) {
    callback = callback || function() {};
    ...
    var q = Cassandra.constructUpsertCQL('CommentsByContent', 'contentId', contentId, parameters, 'QUORUM');
    Cassandra.runQuery(q.query, q.parameters, function(err) {
        ...
        q = Cassandra.constructUpsertCQL('Comments', 'commentId', commentId, {'body': body, 'createdBy': ctx.user().id}, 'QUORUM');
 ...
 * @param  {String}     body            The updated comment for the content item
 * @param  {Function}   callback        Standard callback function takes argument `err`
 * @param  {Object}     callback.err    Error object containing the error message
 */
var updateComment = module.exports.updateComment = function(ctx, contentId, created, body, callback) {
    callback = callback || function() {};
    ...
    _getComment(ctx, contentId, created, function(err, commentToUpdate) {
        q = Cassandra.constructUpsertCQL('Comments', 'commentId', commentToUpdate.commentId, {'body': body, 'lastModified': new Date().getTime()}, 'QUORUM');
        Cassandra.runQuery(q.query, q.parameters, callback);
    });
};
 ...
 * @param  {String}     body            The reply on a comment to place on the content item
 * @param  {Function}   callback        Standard callback function takes argument `err`
 * @param  {Object}     callback.err    Error object containing the error message
 */
var postReply = module.exports.postReply = function(ctx, contentId, created, body, callback) {
    callback = callback || function() {};
    ...
    _getComment(ctx, contentId, created, function(err, commentReplyingTo) {
        ...
        Cassandra.runQuery(q.query, q.parameters, function(err) {
            ...
            q = Cassandra.constructUpsertCQL('Comments', 'commentId', commentId, {
                'body': body,
                'createdBy': ctx.user().id,
                'replyTo': created,
                'level': (level + 1)
...
var getComments = module.exports.getComments = function(ctx, contentId, start, limit, callback) {
    ...
    Cassandra.runQuery('SELECT FIRST ' + paging.limit + ' REVERSED ?..\'\' FROM CommentsByContent USING CONSISTENCY QUORUM WHERE contentId IN (?)', [paging.start, contentId], function (err, pagedRows) {
        ...
        Cassandra.runQuery('SELECT * FROM Comments USING CONSISTENCY QUORUM WHERE commentId IN (?)', [commentIds], function (err, detailRows) {
            ...
            detailRows.forEach(function(row){
                userIds.push(row.get('createdBy').value);
            });

            PrincipalsUtil.getPrincipals(ctx, userIds, function(err, principals) {
                // Aggregate the data to return to the user
                for (var i = 0; i < detailRows.length; i++) {
                    var comment = new Comment(comments[i].body, comments[i].commentId, principals[comments[i].createdBy], timestamps[i], comments[i].lastModified, comments[i].level, comments[i].replyTo);
...
var deleteComment = module.exports.deleteComment = function(ctx, contentId, created, callback) {
    ...
    _getComment(ctx, contentId, created, function(err, comment) {
        ...
        Cassandra.runQuery('DELETE ? FROM CommentsByContent WHERE contentId = ?', [created, contentId], function(err) {

            if (err) {
                return callback(err);
            }

            Cassandra.runQuery('DELETE FROM Comments WHERE commentId = ?', [comment.created], function(err) {
                callback(err);
            });
        });
    });
