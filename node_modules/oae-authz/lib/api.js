 ...
 * @param  {Function}                   callback                    Standard callback function
 * @param  {Object}                     callback.err                An error that occurred, if any
 * @param  {Array.<Array.<String>>}     callback.ancestryByDepth    An array of arrays. The top-level array represents depth to which the principal is a member, while the nested array represents the group ids to which the principal is a member at that depth
 * @api private
 */
var _getAuthzGroupMembershipAncestry = function(principalIds, callback, _groupIdsCache, _groupIdsByDepth) {
    ...
    getRolesForPrincipalsAndResourceType(principalIds, AuthzConstants.resourceTypes.GROUP, 10000, function(err, principalResourceRoles) {
        if (err) {
            return callback(err);
        }

        // We don't really care about the principal ids, so we can just drop them and merge them
        // into a single array that enumerates all roles found for each resource
        var resourceRoles = {};
        _.each(principalResourceRoles, function(resourceRole, principalId) {
            _.each(resourceRole, function(role, resourceId) {
                resourceRoles[resourceId] = resourceRoles[resourceId] || {};
                resourceRoles[resourceId][role] = true;
            });
        });

        // Filter the ancestors by those we haven't seen yet to avoid duplicates under multiple depths
        var newAncestorIds = _.chain(resourceRoles)
            .keys()
