...
var _getBecomeUserAuthenticationRequestInfo = function(req, res) {
    AuthenticationUtil.getSignedBecomeUserAuthenticationRequest(req.ctx, req.query.becomeUserId, function(err, requestInfo) {
        if (err) {
            return res.send(err.code, err.msg);
        }

        return res.send(requestInfo);
    });
};

/*!
 * Get request information witch which a global administrator can authenticate themself to
 * a user tenant
 */
OAE.globalAdminRouter.on('get', '/api/auth/signed/tenant', function(req, res) {
    AuthenticationUtil.getSignedTenantAuthenticationRequest(req.ctx, req.query.tenant, function(err, requestInfo) {
        if (err) {
            return res.send(err.code, err.msg);
        }
...
/*!
 * Get request information with which an administrator user can authenticate to a tenant
 * and become a specified user
 */
...
/*!
 * Perform a signed authentication request with a signed POST body, as acquired from the
 * `GET /api/auth/signed/*` endpoints
 */
