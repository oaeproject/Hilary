...
var _setContentPermissions = function(contentObj, newPermissions, callback) {
    ...
    PrincipalUtil.getPrincipals(_.keys(newPermissions), function(err, principals) {
        if (err) {
            return callback(err);
        }
...
var _rowToContent = function (row) {
    return new Content(row.colHash.tenant, row.colHash.contentId, row.colHash.visibility, row.colHash.name, row.colHash.description, row.colHash.contentType, row.colHash.lastModified);
};