 ...
 * @param  {Object}             callback.err         MessageBoxAPI#getMessagesById
 * @param  {}
 */
...
var _getMessageThreadKey = function(messageId, callback) {
    // The message id is not specified, so simply return with nothing.
    if (!messageId) {
        return callback();
    }
...
var _leafDelete = function(message, callback) {
    ...
    _getThreadKeysFromMessageBox(message.messageBoxId, message.threadKey, 1, function(err, threadKeys) {
        ...
        if (hasReply) {
            _softDelete(message, callback);
        } else {
            _hardDelete(message, callback);
        }
    });
};
