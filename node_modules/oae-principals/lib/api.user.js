...
/**
 * Get all the connections older than today - "num".
 *
 * @param  {Context}    ctx                 Standard context object containing the current user and the current tenant
 ...
 */
var getLog = module.exports.getLog = function(ctx, num, tenantAlias, callback) {
    ...
    // Get log
    PrincipalsDAO.getAllUsersForTenant(tenantAlias, function(err, log) {
        if (err) {
            return callback(err);
        }
        if (_.isEmpty(log)) {
        ...
        // Get only the connexion older than today - "num"
        var today = new Date();
        today.setMonth(today.getMonth() - num);
        var newList = [];

        _.each(log, function(element) {
            if (element) {
                if (today >= new Date(element.lastLog)) {
        ...
        newList.sort(function(a,b){
          return new Date(a.lastLog) - new Date(b.lastLog);
        });

        // Create array
        var csv = ['principalId,tenantAlias,email,publicAlias,lastLog'];
        _.each(newList, function(element) {
            csv.push(element.id + ',' +
                element.tenant.alias + ',' +
                element.email + ',' +
                element.publicAlias + ',' +
                element.lastLog);
        });
 ...
 * @param  {String}     callback.csv        A CSV with all last connection
 */
var getLogCreation = module.exports.getLogCreation = function(ctx, tenantAlias, callback) {
    callback = callback || function() {};
    ...
    PrincipalsDAO.getAllUsersForTenant(tenantAlias, function(err, log) {
        ...
        // Create array
        var csv = ['principalId,tenantAlias,email,publicAlias,created'];
        _.each(log, function(element) {
            csv.push(element.id + ',' +
                element.tenant.alias + ',' +
                element.email + ',' +
