...
var routeActivity = module.exports.routeActivity = function(activitySeed, callback) {
    ...
    _produceAllEntities(activitySeed, function(err, actor, object, target) {
        ...
        _produceAllRoutes(activitySeed, actor, object, target, function(err, actorRoutes, actorPropagation, objectRoutes, objectPropagation, targetRoutes, targetPropagation) {
            ...
            var allActorRoutes = (actorRoutes.activity) ? actorRoutes.activity.slice(0) : [];
            var allObjectRoutes = (objectRoutes.activity) ? objectRoutes.activity.slice(0) : [];
            var allTargetRoutes = (targetRoutes.activity) ? targetRoutes.activity.slice(0) : [];

            // Collect all notification routes into a discriminated activity route
            _.each(actorRoutes.notification, function(notificationRoute) {
                allActorRoutes.push(notificationRoute + '#notification');
            });

            _.each(objectRoutes.notification, function(notificationRoute) {
                allObjectRoutes.push(notificationRoute + '#notification');
            });

            _.each(targetRoutes.notification, function(notificationRoute) {
                allTargetRoutes.push(notificationRoute + '#notification');
            });
            ...
            for (var i = 0; i < allActorRoutes.length; i++) {
                var actorRoute = allActorRoutes[i];
            ...
            for (i = 0; i < allObjectRoutes.length; i++) {
                var objectRoute = allObjectRoutes[i];
            ...
            for (i = 0; i < allTargetRoutes.length; i++) {
                var targetRoute = allTargetRoutes[i];
            ...
            if (activitySeed.actorResource && actorPropagation.type === ActivityConstants.entityPropagation.SPECIFY && actorPropagation.specify) {
                actorPropagation.specify.forEach(function(route) {
                    ...
                    var notificationRoute = route + '#notification';
                    if (routedActivities[notificationRoute]) {
                        routedActivities[notificationRoute].actor = actor;
                    }
                });
            }
            ...
            if (activitySeed.objectResource && objectPropagation.type === ActivityConstants.entityPropagation.SPECIFY && objectPropagation.specify) {
                objectPropagation.specify.forEach(function(route) {
                    ...
                    var notificationRoute = route + '#notification';
                    if (routedActivities[notificationRoute]) {
                        routedActivities[notificationRoute].object = object;
                    }
                });
            }
            ...
            if (activitySeed.targetResource && targetPropagation.type === ActivityConstants.entityPropagation.SPECIFY && targetPropagation.specify) {
                targetPropagation.specify.forEach(function(route) {
                    ...
                    var notificationRoute = route + '#notification';
                    if (routedActivities[notificationRoute]) {
                        routedActivities[notificationRoute].target = target;
                    }
                });
            }

            // Ensure that only users are delivered notifications, and no user receives a notification for any activity of
            // which they are the actor.
            _.each(routedActivities, function(routedActivity, route) {
                if (route.slice(-13) === '#notification') {
                    var userId = route.split('#').slice(0, -1).join('#');
                    if (userId === routedActivity.actor[ActivityConstants.properties.OAE_ID]) {
                        delete routedActivities[route];
                    }
                }
            });
