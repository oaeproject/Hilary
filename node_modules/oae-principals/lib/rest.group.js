 ...
 */
OAE.tenantRouter.on('post', '/api/group/:groupId/join-request', function(req, res) {
 ...
 * @HttpResponse                401                     You have to be logged in to be able to create a request to join a group
 */
OAE.tenantRouter.on('get', '/api/group/:groupId/join-request', function(req, res) {
    
    PrincipalsAPI.getJoinGroupRequest(req.ctx, req.params.groupId, function(err, request) {
        if (err) {
            return res.status(err.code).send(err.msg);
        }
 ...
 * @HttpResponse                401                     You have to be logged in to be able to create a request to join a group
 */
OAE.tenantRouter.on('get', '/api/group/:groupId/join-request/all', function(req, res) {

    var limit = OaeUtil.getNumberParam(req.query.limit, 10, 1, 25);

    PrincipalsAPI.getJoinGroupRequests(req.ctx, req.params.groupId, req.query.start, limit, function(err, requests, nextToken) {
        if (err) {
            return res.status(err.code).send(err.msg);
        }
 ...
 */
OAE.tenantRouter.on('put', '/api/group/:groupId/join-request', function(req, res) {

    PrincipalsAPI.updateJoinGroupByRequest(req.ctx, req.params.groupId, req.body.principalId, req.body.role, req.body.status, function(err) {
        if (err) {
            return res.status(err.code).send(err.msg);
        }
        return res.status(200).end();
