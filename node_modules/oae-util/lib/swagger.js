 ...
 */
var documentModule = (module.exports.documentModule = function(
  moduleName,
  callback,
) {
  var files = [];
  ...
  readdirp({
    root: util.format('%s/../../%s/lib', __dirname, moduleName),
    fileFilter: '*.js',
  })
    ...
    .on('error', function(err) {
      ...
      if (err.code !== 'ENOENT') {
        log().warn(
          { err: err },
          'Problem recursing directories while documenting ' + moduleName,
        );
      }
      return callback();
    })
      ...
      _.each(files, function(file) {
        register(file, function(err) {
          if (err) {
            log().warn(
              { err: err },
              'Problem opening a file while documenting ' + moduleName,
            );
          }
          return done();
        });
...
var register = (module.exports.register = function(filePath, callback) {
  fs.readFile(filePath, function(err, data) {
    ...
    try {
      ...
      _.each(doc.endpoints, function(endpoint) {
        ...
        _.each(endpoint.server.split(','), function(server) {
          if (server === 'tenant') {
            ...
            log().warn(
              'Tried to register swagger docs for unknown server "' +
                endpoint.server +
                '"',
            );
          }
      ...
      log().warn(
        { err: ex },
        util.format('Could not parse restjsdoc in %s', filePath),
      );
    }
    return callback();
  });
...
var _appendToApi = function(rootResource, api, spec) {
  ...
  validator
    .check(spec.nickname, { path: api.path, msg: 'Nickname must exist' })
    .notEmpty();
  validator
    .check(spec.nickname, {
      path: api.path,
...
var _recurseModel = function(resource, modelName) {
  ...
  _.each(model.properties, function(property) {
    ...
    if (type === 'array') {
      ...
      !_.contains(Constants.primitives, type) &&
      !resource.models[type]
    ) {
