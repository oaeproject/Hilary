...
var updateAggregateStatus = module.exports.updateAggregateStatus = function(statusUpdates, callback) {
    ...
    _.each(statusUpdates, function(status, aggregateKey) {
         ...
         * It's a bit unfortunate that both the activity stream and the aggregate keys use `#` as their separators though
         * e.g.:
         *     content-create#u:cam:lygZT9hhis#email#user:u:cam:lygZT9hhis##
         *                    \____________________/ is the activity stream
         */
        var aggregateKeyParts = aggregateKey.split('#');
        var activityStream = aggregateKeyParts[1] + '#' + aggregateKeyParts[2];
 ...
 *
 * @param  {String[]}   activityStreams     The set of activity streams that need to have their aggregation cycle reset
...
var resetAggregationForActivityStreams = module.exports.resetAggregationForActivityStreams = function(activityStreams, callback) {
    ...
    getActiveAggregateKeysForActivityStreams(activityStreams, function(err, activeAggregateKeysForActivityStreams) {
        if (err) {
        ...
        _.each(activeAggregateKeysForActivityStreams, function(activeAggregateKeys, index) {
            var activityStream = activityStreams[index];
        ...
        var activeAggregateKeysToDelete = [];
        _.each(allActiveAggregateKeys, function(activeAggregateKey) {
            activeAggregateKeysToDelete.push(_createAggregateStatusCacheKey(activeAggregateKey));
            activeAggregateKeysToDelete.push(_createAggregateEntityCacheKey(activeAggregateKey, 'actors'));
            activeAggregateKeysToDelete.push(_createAggregateEntityCacheKey(activeAggregateKey, 'objects'));
            activeAggregateKeysToDelete.push(_createAggregateEntityCacheKey(activeAggregateKey, 'targets'));
        });

        // Delete all the active aggregate keys
        multi.del(activeAggregateKeysToDelete);
        multi.exec(function(err) {
            if (err) {
                log().error({'err': err, 'activityStreams': activityStreams}, 'Failed to reset aggregation for a set of activity streams');
