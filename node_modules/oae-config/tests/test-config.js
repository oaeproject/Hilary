...
describe('Configuration', function() {
    ...
    describe('REST Configuration API', function() {
        ...
        /**
         * Test that verifies that the guests tenant config can be updated just like any other tenant
         */
        it('verify the guests tenant config can be updated', function(callback) {
            // Update the discussion visibility on the guests tenant
            var config = {
                'oae-discussions/visibility/discussion': 'private',
                'oae-principals/recaptcha/enabled': false
            };
            ConfigTestUtil.updateConfigAndWait(globalAdminRestContext, 'guests', config, function(err) {
                assert.ok(!err);

                // Create a user on the guests tenant
                TestsUtil.generateGuestUsers(1, function(guest) {

                    // Verify the change persisted by creating a discussion on the guests tenant
                    RestAPI.Discussions.createDiscussion(guest.restContext, 'displayName', 'description', null, null, null, function(err, discussion) {
                        assert.ok(!err);
                        assert.strictEqual(discussion.visibility, 'private');

                        // Reset the config
                        ConfigTestUtil.clearConfigAndWait(globalAdminRestContext, 'guests', _.keys(config), function(err) {
                            assert.ok(!err);
                            return callback();
                        });
                    });
