...
var init = module.exports.init = function(callback) {
    return MessageBoxSearch.registerMessageSearchDocument(FoldersConstants.search.MAPPING_FOLDER_MESSAGE, ['folder'], function(resources, callback) {
 ...
 */
FoldersAPI.on(FoldersConstants.events.UPDATED_FOLDER_MEMBERS, function(ctx, folder, roleChanges, newMemberIds, updatedMemberIds, removedMemberIds) {
    ...
    var principalIds = _.union(newMemberIds, updatedMemberIds, removedMemberIds);
 ...
 */
FoldersAPI.on(FoldersConstants.events.CREATED_MESSAGE, function(ctx, message, folder) {
    var resource = {
        'id': folder.groupId,
        'messages': [message]
    };
 ...
 */
FoldersAPI.on(FoldersConstants.events.DELETED_MESSAGE, function(ctx, message, folder, deleteType) {
    return MessageBoxSearch.deleteMessageSearchDocument(FoldersConstants.search.MAPPING_FOLDER_MESSAGE, folder.groupId, message);
});
 ...
 *
 * @param  {Object[]}   resources       The resources representing the folder whose messages to index
 * @param  {Function}   callback        Standard callback function
 * @param  {Object}     callback.err    An error that occurred, if any
 * @api private
 */
var _produceFolderMessageDocuments = function(resources, callback, _documents) {
    ...
    if (resource.messages) {
        var documents = MessageBoxSearch.createMessageSearchDocuments(FoldersConstants.search.MAPPING_FOLDER_MESSAGE, resource.id, resource.messages);
    ...
    MessageBoxSearch.createAllMessageSearchDocuments(FoldersConstants.search.MAPPING_FOLDER_MESSAGE, resource.id, resource.id, function(err, documents) {
        if (err) {
