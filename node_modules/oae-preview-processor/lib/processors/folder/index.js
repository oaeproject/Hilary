...
var generatePreviews = module.exports.generatePreviews = function(folderId, callback) {
    _getData(folderId, function(err, folder, contentItems) {
        ...
        _generatePreviews(folder, contentItems, callback);
    });
};
...
var _generatePreviews = function(folder, contentItems, callback) {
    ...
    _downloadThumbnails(ctx, contentItems.slice(), function(err, paths) {
        ...
        _createMontages(paths, function(err, thumbnail, wide) {
            ...
            // Store the files
            ContentUtil.getStorageBackend(ctx).store(ctx, thumbnail, {'filename': 'thumbnail.jpg', 'resourceId': folder.id}, function(err, thumbnailUri) {
                if (err) {
                    return callback(err);
                }

                ContentUtil.getStorageBackend(ctx).store(ctx, wide, {'filename': 'wide.jpg','resourceId': folder.id}, function(err, wideUri) {
                    if (err) {
                        return callback(err);
                    }
