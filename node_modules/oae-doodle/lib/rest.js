...
OAE.tenantRouter.on('post', '/api/doodle/:id/schedule', function (req, res) {
    DoodleAPI.addSchedule(req.ctx, req.params.id, req.body, function (err) {
        res.end(err);
    });
});
...
OAE.tenantRouter.on('post', '/api/doodle/:id/user', function (req, res) {
    DoodleAPI.addMemberWithVotes(req.ctx, req.params.id, req.body.firstName, req.body.lastName, req.body.votes, function (err, data) {
        if (err) return res.end(err);
        res.json(data);
    });
});
