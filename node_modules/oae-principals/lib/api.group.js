    ...
    PrincipalsDAO.getPrincipal(groupId, function(err, group) {
        ...
        ResourceActions.setRoles(ctx, group, changes, function(err, members) {
            ...
            PrincipalsDAO.updatePrincipal(group.id, {'lastModified': newLastModified}, function(err) {
                ...
                // TODO: The process of updating libraries can be moved into a `when` listener. See `oae-principals/lib/librararies/members.js`
                // for an example on `UPDATED_GROUP_MEMBERS`
                PrincipalsUtil.touchMembershipsLibraries(group, oldLastModified, addedMemberIds.slice(), removedMemberIds.slice(), function(err) {
                    if (err) {
                        return callback(err);
                    }
    ...
    PrincipalsDAO.getPrincipal(groupId, function(err, group) {
        ...
        AuthzPermissions.canRemoveRole(ctx, ctx.user(), group, function(err, changes, roles) {
            ...
            AuthzAPI.updateRoles(group.id, changes, function(err) {
                ...
                // relevant activity
                PrincipalsUtil.touchMembershipsLibraries(group, null, [], [ctx.user().id], function(err) {
                    if (err) {
                        return callback(err);
                    }
        ...
        AuthzPermissions.canJoin(ctx, group, function(err, changes, roles) {
            ...
            AuthzAPI.updateRoles(group.id, changes, function(err) {
                ...
                PrincipalsDAO.updatePrincipal(group.id, updatedProfileFields, function(err) {
                    ...
                    PrincipalsUtil.touchMembershipsLibraries(group, oldLastModified, [ctx.user().id], [], function(err) {
                        if (err) {
                            log().error({
                                'err': err,
                                'group': group,
...
var createGroup = module.exports.createGroup = function(ctx, displayName, description, visibility, joinable, roles, callback) {
    ...
    ResourceActions.create(ctx, roles, createFn, function(err, group, members) {
        ...
        PrincipalsUtil.touchMembershipsLibraries(group, null, _.keys(members.changes), [], function(err) {
            if (err) {
                log().error({
                    'err': err,
                    'group': group,
        ...
        AuthzPermissions.canManage(ctx, oldStorageGroup, function(err) {
            ...
            PrincipalsDAO.updatePrincipal(groupId, profileFields, function(err) {
                ...
                PrincipalsUtil.getPrincipal(ctx, groupId, function(err, updatedGroup) {
                    ...
                    PrincipalsUtil.touchMembershipsLibraries(updatedGroup, oldStorageGroup.lastModified, [], [], function(err) {
                        if (err) {
                            log().error({
                                'err': err,
