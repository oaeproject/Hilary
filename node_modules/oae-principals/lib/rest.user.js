    ...
    var createUser = function() {
        AuthenticationAPI.getOrCreateUser(ctx, AuthenticationConstants.providers.LOCAL, req.body.username, {'password': req.body.password}, req.body.displayName, opts, function(err, newUser, loginId, created) {
            if (err) {
                ...
                return res.status(400).send('A user with that username already exists').end();
            }
 ...
 * @api private
 */
var _handleDeleteUser = function(req, res) {
    PrincipalsAPI.deleteUser(req.ctx, req.params.userId, function(err) {
        if (err) {
            return res.status(err.code).send(err.msg);
        }
 ...
 * @api private
 */
var _handleRestoreUser = function(req, res) {
    PrincipalsAPI.restoreUser(req.ctx, req.params.userId, function(err) {
        if (err) {
            return res.status(err.code).send(err.msg);
        }
...
OAE.tenantRouter.on('get', '/api/user/:userId/memberships', function(req, res) {
    ...
    PrincipalsAPI.getMembershipsLibrary(req.ctx, req.params.userId, req.query.start, limit, function(err, memberships, nextToken) {
        ...
        return res.status(200).send({'results': memberships, 'nextToken': nextToken});
    });
});
 ...
 */
var _getUsersForTenant = function(req, res) {
    PrincipalsAPI.getAllUsersForTenant(req.ctx, req.params.tenantAlias, function(err, users) {
        if (err) {
            return res.status(err.code).send(err.msg);
        }
 ...
 */
OAE.tenantRouter.on('get', '/api/user/:userId/export/:exportType', function(req, res) {
    PrincipalsAPI.exportData(req.ctx, req.params.userId, req.params.exportType, function(err, zipFile) {
        if (err) {
            return res.status(err.code).send(err.msg);
        }

        var filename = 'myData.zip';
 ...
 * @param       {String}                months          Number of months that a user has not logged in
 * @param       {String}                type            Two values are possible :   `created` to get the users creation date log 
 *                                                                                  `auth` to get the last login date log
 * @FormParam   {File}                  file            CSV file downloaded by the admin
 * @Return      {log}                                   The log list in csv format
 * @HttpResponse                        200             Terms and conditions available
 */
var _getUsersDateLog = function(req, res) {
    PrincipalsAPI.getUsersDateLog(req.ctx, req.params.tenant, req.params.type, req.params.months, function(err, log) {
        if (err) {
            return res.status(err.code).send(err.msg);
        }

        var filePath = ContentUtil.getStorageBackend(req.ctx, null).getRootDirectory();
        var csvName = req.params.tenant + '-' + req.params.type + '-' + Date.now() + '.csv';

        // Create directory - if not exist - inside the files directory
        if (!fs.existsSync(filePath + '/downloadUserLogs')) {
            fs.mkdirSync(filePath + '/downloadUserLogs');
        }

        fs.writeFile(filePath + '/downloadUserLogs/' + csvName, log, {encoding: "utf8"}, function (err, csv) {
            if (err) {
                return res.status(err.code).send(err.msg);
            }
            
            res.download(filePath + '/downloadUserLogs/' + csvName);

            // Remove the csv file one minute after the start of the download to be sure to keep it alive 
            // as long as the csv is not download
            setTimeout(function() { fs.unlinkSync(filePath + '/downloadUserLogs/' + csvName); }, 60000);
        });
    });
};

OAE.globalAdminRouter.on('get', '/api/users/logs/:tenant/:type/:months', _getUsersDateLog);
OAE.tenantRouter.on('get', '/api/users/logs/:tenant/:type/:months', _getUsersDateLog);
