        ...
        _.each(cachedGlobalSchema, function(module, moduleKey) {
            ...
            _.each(module, function(feature, featureKey) {
                ...
                _.each(feature.elements, function(element, elementKey) {
                    ...
                    // If the value is not suppressed and not private, we add it to the cached suppressed config object
                    if (!element.suppress && !element.private) {
 ...
 * Tenant administrators cannot update values marked as `private`; regular users cannot update any configuration values.
 *
 * @param  {Context}   ctx           Standard context object, representing the currently logged in user and its tenant
...
var canUpdateConfigValue = function(ctx, tenantAlias, moduleId, feature, element) {
    ...
    // If `private` is specified and the user is a not a global admin, the user
    // is not allowed to change the configuration option.
    if (configElement.private && !ctx.user().isGlobalAdmin()) {
        return false;
    }
