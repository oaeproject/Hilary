...
var canSetDiscussionPermissions = module.exports.canSetDiscussionPermissions = function(ctx, discussion, addMembers, callback) {
    ...
    // We need to grab them from the DAO as we might need the full object in the authz API
    PrincipalsDAO.getPrincipals(addMembers, null, function(err, principals) {
        if (err) {
            return callback(err);
        }
 ...
 * @param  {Boolean}    callback.isMember   `true` if the user in context is a member of the discussion. `false` otherwise.
 */
var resolveEffectiveDiscussionAccess = module.exports.resolveEffectiveDiscussionAccess = function(ctx, discussion, callback) {
    AuthzAPI.resolveEffectiveRole(ctx, discussion.id, discussion.tenant.alias, discussion.visibility, DiscussionsConstants.roles.ALL_PRIORITY, function(err, effectiveRole, canInteract) {
        ...
        // Return if the user is anonymous or a manager
        if (canManage || !ctx.user()) {
            return callback(null, canView, canManage, canShare, canPost, canManage);
        // If not a manager do an extra check to see if the user is a member
        } else {
            AuthzAPI.hasRole(ctx.user().id, discussion.id, DiscussionsConstants.roles.MEMBER, function(err, _isMember) {
                return callback(null, canView, canManage, canShare, canPost, _isMember);
            });
        }
    });
};
