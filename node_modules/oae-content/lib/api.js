...
var ContentAuthz = require('./authz');
...
var getContent = module.exports.getContent = function(ctx, contentId, callback) {
    ...
    ContentDAO.Content.getContent(contentId, function(err, contentObj) {
        ...
        ContentAuthz.canViewContent(ctx, contentObj, function(err, canView) {
            if (err) {
                return callback(err);
            } else if (!canView) {
...
var getFullContentProfile = module.exports.getFullContentProfile = function(ctx, contentId, callback) {
    ...
    getContent(ctx, contentId, function(err, contentObj) {
        ...
        ContentAuthz.canManageContent(ctx, contentObj, function(err, isManager) {
            if (err) {
                return callback(err);
            }
 ...
 * @param  {Object}         [additionalMembers]     Object where the keys represent principal ids or email addresses that need to be added to the content upon creation and the values represent the role that principal will have. Possible values are "viewer" and "manager"
...
var createLink = module.exports.createLink = function(ctx, displayName, description, visibility, link, additionalMembers, folders, callback) {
    ...
    _createContent(ctx, contentId, revisionId, 'link', displayName, description, visibility, additionalMembers, folders, {'link': link}, {}, function(err, content, revision) {
        ...
        ContentAPI.emit(ContentConstants.events.CREATED_CONTENT, ctx, content, revision, additionalMembers, folders);
...
var _createFile = function(ctx, displayName, description, visibility, file, additionalMembers, folders, callback) {
    ...
    ContentUtil.getStorageBackend(ctx).store(ctx.tenant().alias, file, options, function(err, uri) {
        ...
        _createContent(ctx, contentId, revisionId, 'file', displayName, description, visibility, additionalMembers, folders, otherValues, revisionData, function(err, contentObj, revision) {
            if (err) {
                return callback(err);
            }
            ...
            ContentAPI.emit(ContentConstants.events.CREATED_CONTENT, ctx, contentObj, revision, additionalMembers, folders);
            return callback(null, contentObj);
        });
    });
};
 ...
 * @param  {Object}         [additionalMembers]     Object where the keys represent principal ids or email addresses that need to be added to the content upon creation and the values represent the role that principal will have. Possible values are "viewer", "editor" and "manager"
...
var createCollabDoc = module.exports.createCollabDoc = function(ctx, displayName, description, visibility, additionalMembers, folders, callback) {
    ...
    Etherpad.createPad(contentId, function(err, ids) {
        ...
        _createContent(ctx, contentId, revisionId, 'collabdoc', displayName, description, visibility, additionalMembers, folders, ids, {}, function(err, content, revision) {
            if (err) {
                return callback(err);
            }
 ...
 * @param  {String}         visibility              The visibility of the collaborative document. One of `public`, `loggedin`, `private`
 * @param  {Object}         additionalMembers       Object where the keys represent principal ids or email addresses that need to be added to the content upon creation and the values represent the role that principal will have. Possible values are "viewer" and "manager", as well as "editor" for collabdocs
 ...
 * @api private
 */
var _createContent = function(ctx, contentId, revisionId, resourceSubType, displayName, description, visibility, additionalMembers, folders, otherValues, revisionData, callback) {
    callback = callback || function() {};
    ...
    // Get the principal ids, creating guest users if necessary
    PrincipalsAPI.getOrCreateGuestUsers(ctx, _.keys(additionalMembers), function(err, principalIds) {
        if (err) {
            return callback(err);
        }

        var permissions = {};
        _.each(additionalMembers, function(role, principal) {
            permissions[principalIds[principal]] = role;
        });

        var additionalMemberIds = _.values(principalIds);
        ...
        PrincipalsDAO.getPrincipals(additionalMemberIds, null, function(err, principals) {
            ...
            AuthzAPI.canInteract(ctx, ctx.tenant().alias, principals, function(err, canInteract, illegalPrincipalIds) {
                ...
                canManageFolders(ctx, folders, function(err, folders) {
                    if (err) {
                        return callback(err);
                    }

                    ContentDAO.Content.createContent(contentId, revisionId, ctx.user().id, resourceSubType, displayName, description, visibility, otherValues, revisionData, function(err, contentObj, revision) {
                        if (err) {
                            return callback(err);
                        }

                        // Make the current user a manager of the content
                        permissions[ctx.user().id] = ContentConstants.roles.MANAGER;
                        _setContentPermissions(ctx, contentObj, permissions, function(err, contentObj) {
                            if (err) {
                                return callback(err);
                            }

                            var allMembers = _.union(principals, [ctx.user()]);
                            allMembers = _.uniq(allMembers, 'id');
                            ContentMembersLibrary.insert(contentObj, allMembers, function(err) {
                                ...
                                _addContentItemToFolders(ctx, contentObj, folders, function(err) {
                                    if (err) {
                                        log().warn({'err': err, 'contentId': contentObj.id, 'folders': folders}, 'Could not add a content item to a folder');
                                    }
...
var _canManageAllFolders = function(ctx, folders, callback) {
    ...
    require('oae-folders/lib/authz').canManageFolder(ctx, folder, function(err, canManage) {
        if (err) {
            return callback(err);
        } else if (!canManage) {
            return callback({'code': 401, 'msg': 'You cannot add items to this folder'});
        }
...
var deleteContent = module.exports.deleteContent = function(ctx, contentId, callback) {
    ...
    canManage(ctx, contentId, function(err, canManage, contentObj) {
        ...
        ContentDAO.Content.deleteContent(contentObj, function(err, members) {
            ...
            ContentAPI.emit(ContentConstants.events.DELETED_CONTENT, ctx, contentObj, members);
            return callback();
        });
    });
};
 ...
 * @param  {String[]}  principals        Array of principal ids or email addresses with whom the content will be shared. By default, they will all be made members
 * @param  {Function}  callback          Standard callback function
 * @param  {Object}    callback.err      An error that occurred, if any
 */
var shareContent = module.exports.shareContent = function(ctx, contentId, principals, callback) {
    callback = callback || function() {};
    ...
    validator.check(contentId, {'code': 400, 'msg': 'A content id must be provided'}).isResourceId();
    validator.check(principals.length, {'code': 400, 'msg': 'The content must at least be shared with 1 user, group or email address'}).min(1);
    _.each(principals, function(principal) {
        validator.check(principal, {'code': 400, 'msg': 'Invalid principal id or email address has been specified'}).isPrincipalIdOrEmailAddress();
    });
    if (validator.hasErrors()) {
        return callback(validator.getFirstError());
    }
    ...
    ContentDAO.Content.getContent(contentId, function(err, contentObj) {
        ...
        _canShareContent(ctx, contentObj, function(err, canShare) {
            ...
            // Get the principal ids, creating guest users if necessary
            PrincipalsAPI.getOrCreateGuestUsers(ctx, principals, function(err, principalIds) {
                if (err) {
                    return callback(err);
                }
                principalIds = _.values(principalIds);
    ...
    AuthzAPI.resolveImplicitRole(ctx, contentObj.id, contentObj.tenant.alias, contentObj.visibility, ContentConstants.roles.ALL_PRIORITY, function(err, implicitRole, canInteract) {
        if (implicitRole === ContentConstants.roles.MANAGER) {
            ...
            return AuthzAPI.getAllRoles(ctx.user().id, contentObj.id, function(err, roles) {
                ...
                var isEditorOrManager = _.some(roles, function(role) {
                    return _.contains([ContentConstants.roles.EDITOR, ContentConstants.roles.MANAGER], role);
                });
                return callback(null, isEditorOrManager);
            });
        }
...
var canManage = module.exports.canManage = function(ctx, contentId, callback) {
    ...
    getContent(ctx, contentId, function(err, contentObj) {
        ...
        ContentAuthz.canManageContent(ctx, contentObj, function(err, canManage) {
            if (err) {
                return callback(err);
            }
...
var setContentPermissions = module.exports.setContentPermissions = function(ctx, contentId, permissionChanges, callback) {
    ...
    _.each(permissionChanges, function(role, principalId) {
        validator.check(principalId, {'code': 400, 'msg': 'Invalid principal id or email address has been specified'}).isPrincipalIdOrEmailAddress();
    ...
    getContent(ctx, contentId, function(err, contentObj) {
        ...
        ContentAuthz.canManageContent(ctx, contentObj, function(err, canManage) {
            ...
            // Get the principal ids, creating guests if necessary
            PrincipalsAPI.getOrCreateGuestUsers(ctx, _.keys(permissionChanges), function(err, principalIds) {
                if (err) {
                    return callback(err);
                }

                var changes = {};
                _.each(permissionChanges, function(role, principal) {
                    changes[principalIds[principal]] = role;
                });

                // Ensure that we're not crossing tenant boundaries or violating principal visibility settings
                _checkNewContentPermissions(ctx, contentObj, changes, function(err, membershipAfterChanges, newMemberIds, updatedMemberIds, removedMemberIds, newMembers) {
                    if (err) {
                        return callback(err);
                    }

                    // Apply the permission changes to the content. This will then update the content libraries as well
                    _setContentPermissions(ctx, contentObj, changes, function(err, newContentObj) {
                        if (err) {
                            return callback(err);
                        }
                        ...
                        ContentMembersLibrary.insert(contentObj, newMembers, function(err) {
                            ...
                            ContentMembersLibrary.remove(contentObj, removedMemberIds, function(err) {
                                ...
                                ContentAPI.emit(ContentConstants.events.UPDATED_CONTENT_MEMBERS, ctx, newContentObj, changes, newMemberIds, updatedMemberIds, removedMemberIds);
                                return callback();
                            });
                        });
                    });
    ...
    AuthzAPI.updateRoles(contentObj.id, newPermissions, function(err) {
        if (err) {
            log().error({
                'err': err,
                'contentId': contentObj.id,
                'principalIds': _.pluck(newPermissions, 'id')
            }, 'An error occurred while changing a set of principal roles on a piece of content');
            return callback(err);
        }
...
var deleteComment = module.exports.deleteComment = function(ctx, contentId, commentCreatedDate, callback) {
    ...
    getContent(ctx, contentId, function(err, content) {
        ...
        MessageBoxAPI.getMessages(contentId, [commentCreatedDate], null, function(err, messages) {
            ...
            ContentAuthz.canManageContent(ctx, content, function(err, canManage) {
                if (err) {
                    return callback(err);
                } else if (!canManage) {
