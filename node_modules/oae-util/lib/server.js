...
module.exports.setupServer = function(port, config) {
    ...
    server.use(express.bodyParser(config.files));
    ...
    server.use(express.session({'secret': config.cookie.secret, 'store': redisStore}));
     ...
     */
    server.use(function(req, res, next) {
        if (!_isSafeMethod(req) && !_isSafePath(req) && !_isSameOrigin(req)) {
            log().warn({
                'method': req.method,
...
var addSafePathPrefix = module.exports.addSafePathPrefix = function(pathPrefix) {
    log().info('Adding %s to list of paths that are not CSRF-protected.');
...
module.exports.postInitializeServer = function(server) {
    ...
    var serverTelemetry = TelemetryAPI.telemetry('server');
...
var _isSafePath = function(req) {
    ...
    _.each(safePathPrefixes, function(safePathPrefix) {
        if (!isSafePath) {
            if (path.indexOf(safePathPrefix) === 0) {
                isSafePath = true;
            }
        }
    });
...
/**
 * Determine whether or not this request is considered "safe"
 *
 * @param  {Request}    req     The express request object to test
 * @return {Boolean}            `true` if the request is safe (e.g., GET, HEAD), `false` otherwise
 * @api private
 */
var _isSafeMethod = function(req) {
