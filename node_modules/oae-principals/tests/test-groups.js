...
describe('Groups', function() {
    ...
    describe('Join group by requests', function() {

        /**
         * Test that verifies that a request is correctly created
         */
        it('create request to join a group', function(callback) {

            // Create a user on another tenant
            TestsUtil.generateTestUsers(gtAdminRestContext, 2, function(err, users, orodan, brecke) {
                assert.ok(!err);

                // Create a group and add orodan as a manager
                RestAPI.Group.createGroup(orodan.restContext, 'Test Group', 'Group', 'public', 'request', [], [], function(err, groupObj) {
                    assert.ok(!err);

                    // Make sure the query isn't created if group id is invalid
                    PrincipalsTestUtil.assertCreateRequestJoinGroupFails(brecke.restContext, 'invalidGroupId', 400, function(err) {
                        assert.ok(!err);

                        // Create request to join a group
                        PrincipalsTestUtil.assertCreateRequestJoinGroupSucceeds(brecke.restContext, groupObj.id, function(err) {
                            assert.ok(!err);

                            // Make sure the query isn't created if group id is invalid
                            PrincipalsTestUtil.assertGetJoinGroupRequestFails(brecke.restContext, 'invalidGroupId', 400, function(err) {
                                assert.ok(!err);

                                // Get request to ensure that a request is created
                                PrincipalsTestUtil.assertGetJoinGroupRequestSucceeds(brecke.restContext, groupObj.id, function(request) {
                                    assert.ok(request);
         ...
         */
        it('get all requests to join a specific group', function(callback) {

            // Create a user on another tenant
            TestsUtil.generateTestUsers(gtAdminRestContext, 3, function(err, users, orodan, brecke, nicolaas) {
                assert.ok(!err);
                ...
                RestAPI.Group.createGroup(orodan.restContext, 'Test Group', 'Group', 'public', 'request', [], [], function(err, groupObj) {
                    ...
                    PrincipalsTestUtil.assertCreateRequestJoinGroupSucceeds(brecke.restContext, groupObj.id, function(err) {
                        ...
                        // Make sure the query isn't created if group id is invalid
                        PrincipalsTestUtil.assertCreateRequestJoinGroupSucceeds(nicolaas.restContext, groupObj.id, function(err) {
                            assert.ok(!err);

                            // Make sure the query isn't created if group id is invalid
                            PrincipalsTestUtil.assertGetJoinGroupRequestsFails(orodan.restContext, 'invalidGroupId', 400, function(err) {
                                assert.ok(!err);

                                // Get request to ensure that a request is created
                                PrincipalsTestUtil.assertGetJoinGroupRequestsSucceeds(orodan.restContext, groupObj.id, function(requests) {
                                    assert.ok(requests);
         ...
         */
        it('accept request to join a group', function(callback) {
                ...
                RestAPI.Group.createGroup(orodan.restContext, 'Test Group', 'Group', 'public', 'request', [], [], function(err, groupObj) {
                    ...
                    PrincipalsTestUtil.assertCreateRequestJoinGroupSucceeds(brecke.restContext, groupObj.id, function(err) {
                        ...
                        PrincipalsTestUtil.assertCreateRequestJoinGroupSucceeds(nicolaas.restContext, groupObj.id, function(err) {
                            ...
                            PrincipalsTestUtil.assertUpdateJoinGroupByRequestFails(orodan.restContext, groupObj.id, brecke.user.id, AuthzConstants.role.MANAGER, 'invalidStatus', 400, function(err) {
                                assert.ok(!err);
                                PrincipalsTestUtil.assertUpdateJoinGroupByRequestFails(orodan.restContext, groupObj.id, brecke.user.id, 'invalidRole', PrincipalsConstants.requestStatus.ACCEPT, 400, function(err) {
                                    assert.ok(!err);
                                    PrincipalsTestUtil.assertUpdateJoinGroupByRequestFails(orodan.restContext, 'invalidGroupId', brecke.user.id, AuthzConstants.role.MANAGER, PrincipalsConstants.requestStatus.ACCEPT, 400, function(err) {
                                        assert.ok(!err);
                                        PrincipalsTestUtil.assertUpdateJoinGroupByRequestFails(orodan.restContext, groupObj.id, 'invalidUserId', AuthzConstants.role.MANAGER, PrincipalsConstants.requestStatus.ACCEPT, 400, function(err) {
                                            assert.ok(!err);

                                            // Accept requests
                                            PrincipalsTestUtil.assertUpdateJoinGroupByRequestSucceeds(orodan.restContext, groupObj.id, brecke.user.id, AuthzConstants.role.MANAGER, PrincipalsConstants.requestStatus.ACCEPT, function(err) {
                                                assert.ok(!err);
                                                PrincipalsTestUtil.assertUpdateJoinGroupByRequestSucceeds(orodan.restContext, groupObj.id, nicolaas.user.id, AuthzConstants.role.MEMBER, PrincipalsConstants.requestStatus.ACCEPT, function(err) {
                                                    assert.ok(!err);

                                                    // Make sure the request is marked as accepted
                                                    PrincipalsDAO.getJoinGroupRequest(groupObj.id, brecke.user.id, function(err, request) {
                                                        assert.equal(request.status, PrincipalsConstants.requestStatus.ACCEPT);
                                                        ...
                                                        PrincipalsTestUtil.assertGetJoinGroupRequestsSucceeds(orodan.restContext, groupObj.id, function(requests) {
                                                            ...
                                                            // Get the members of this group
                                                            RestAPI.Group.getGroupMembers(orodan.restContext, groupObj.id, undefined, undefined, function(err, members) {
                                                                assert.ok(!err);
                                                                assert.equal(members.results.length, 3);

                                                                // Morph results to hash for easy access
                                                                var hash = _.groupBy(members.results, function(member) { return member.profile.id; });
                                                                assert.equal(hash[brecke.user.id][0].role, AuthzConstants.role.MANAGER);
                                                                assert.equal(hash[nicolaas.user.id][0].role, AuthzConstants.role.MEMBER);
         ...
         */
        it('reject request to join a group', function(callback) {
            ...
            TestsUtil.generateTestUsers(gtAdminRestContext, 2, function(err, users, orodan, brecke) {
                ...
                RestAPI.Group.createGroup(orodan.restContext, 'Test Group', 'Group', 'public', 'request', [], [], function(err, groupObj) {
                    ...
                    PrincipalsTestUtil.assertCreateRequestJoinGroupSucceeds(brecke.restContext, groupObj.id, function(err) {
                        ...
                        PrincipalsTestUtil.assertUpdateJoinGroupByRequestSucceeds(orodan.restContext, groupObj.id, brecke.user.id, null, PrincipalsConstants.requestStatus.REJECT, function(err) {
                            assert.ok(!err);
         ...
         */
        it('cancel request to join a group', function(callback) {
            ...
            TestsUtil.generateTestUsers(gtAdminRestContext, 2, function(err, users, orodan, brecke) {
                ...
                RestAPI.Group.createGroup(orodan.restContext, 'Test Group', 'Group', 'public', 'request', [], [], function(err, groupObj) {
                    ...
                    PrincipalsTestUtil.assertCreateRequestJoinGroupSucceeds(brecke.restContext, groupObj.id, function(err) {
                        ...
                        PrincipalsTestUtil.assertUpdateJoinGroupByRequestSucceeds(brecke.restContext, groupObj.id, brecke.user.id, null, PrincipalsConstants.requestStatus.CANCEL, function(err) {
                            assert.ok(!err);
