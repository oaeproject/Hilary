...
var LocalBackend = require('oae-content/lib/backends/local');
...
describe('Content Backends', function() {
  ...
  describe('Remote backend', function() {
    /**
    ...
    it('verify remote backend is able to return a download link', function(callback) {
      ...
      var downloadStrategy = RemoteBackend.getDownloadStrategy(null, uri);
      assert.equal(downloadStrategy.strategy, 'direct');
      assert.equal(downloadStrategy.target, 'http://www.apereo.org/favicon.ico');
      return callback();
    });
     ...
     */
    it('verify storing content items is not implemented', function(callback) {
      RemoteBackend.store(null, { name: 'foo' }, null, function(err) {
     ...
     */
    it('verify getting content items is not implemented', function(callback) {
      RemoteBackend.get(null, 'remote#www.google.com', function(err) {
        assert.equal(err.code, 501);
     ...
     */
    it('verify removing content items is not implemented', function(callback) {
      RemoteBackend.remove(null, 'remote#www.google.com', function(err) {
        assert.equal(err.code, 501);
  ...
  describe('Local backend', function() {
    ...
    afterEach(function(callback) {
      ...
      LocalBackend.init(_originalRootDir, function(err) {
        assert.ok(!err);
        return callback();
      });
    });
    ...
    describe('#init()', function() {
      ...
      it('verify error handling', function(callback) {
        LocalBackend.init('\0', function(err) {
    ...
    describe('#get()', function() {
       ...
       */
      it('verify error handling', function(callback) {
        var uri = 'local#/non/existing/file';

        LocalBackend.get(null, uri, function(err, file) {
          assert.ok(err);
          assert.ok(!file);
