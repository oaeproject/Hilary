...
describe('Authentication', function() {
    ...
    before(function() {
        ...
        anonymousTenantRestContext = TestsUtil.createTenantRestContext(global.oaeTests.tenants.signed.host);
    ...
    /**
     * We've changed the expires configuration value, revert it back so we don't accidentilly fail any other tests.
     */
    after(function(callback) {
        RestAPI.Config.updateConfig(globalAdminRestContext, 'signedtest', 'oae-authentication/signed/expires', 120000, function(err) {
            assert.ok(!err);
            callback();
        });
    });
    ...
    describe('Signed authentication', function() {
        ...
        describe('Login', function() {
            ...
            var performSignedTokenRequest = function(expires, signature, userId, isLoggedIn, callback) {
                ...
                var restCtx = new RestContext('http://' + global.oaeTests.tenants.signed.host, userId, null, global.oaeTests.tenants.signed.host);
                RestUtil.setupEmptyJar(restCtx);
                RestAPI.Authentication.loginWithSignedToken(restCtx, token, function(err) {
                    assert.ok(!err);
                    RestAPI.User.getMe(restCtx, function(err, user) {
                        assert.ok(!err);
                        if (isLoggedIn) {
             ...
             */
            it('verify parameter validation', function(callback) {
                RestAPI.Authentication.getSignedToken(globalAdminRestContext, 'signedtest', function(err, token) {
                    assert.ok(!err);
            ...
            it('verify expiring signature', function(callback) {
                RestAPI.Config.updateConfig(globalAdminRestContext, 'signedtest', 'oae-authentication/signed/expires', 2, function() {
                    RestAPI.Authentication.getSignedToken(globalAdminRestContext, 'signedtest', function(err, token) {
                        assert.ok(!err);
                        performSignedTokenRequest(token.expires, token.signature, token.userId, false, callback);
                    });
                });
            });
