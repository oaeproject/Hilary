  ...
  TaskQueue.bind(
    ...
    function(err) {
      if (err) {
        log().error(
          { err: err },
          'Could not bind to the generate previews queue',
        );
        return callback(err);
      }

      log().info(
        'Bound the preview processor to the generate previews task queue',
      );
      ...
      TaskQueue.bind(
        ...
        function(err) {
          if (err) {
            log().error(
              { err: err },
              'Could not bind to the generate folder previews queue',
            );
            return callback(err);
          }

          log().info(
            'Bound the preview processor to the generate folder previews task queue',
          );
          ...
          TaskQueue.bind(
            ...
            function(err) {
              if (err) {
                log().error(
                  { err: err },
                  'Could not bind to the regenerate previews queue',
                );
                return callback(err);
              }

              log().info(
                'Bound the preview processor to the regenerate previews task queue',
              );
              return callback();
            },
          );
    ...
    TaskQueue.unbind(
      ...
      function(err) {
        if (err) {
          log().error(
            { err: err },
            'Could not unbind from the folder previews queue',
          );
          return callback(err);
        }

        log().info(
          'Unbound the preview processor from the folder generate previews task queue',
        );
        ...
        TaskQueue.unbind(PreviewConstants.MQ.TASK_REGENERATE_PREVIEWS, function(
          ...
          if (err) {
            log().error(
              { err: err },
              'Could not unbind from the regenerate previews queue',
            );
            return callback(err);
          }
 ...
 */
var registerProcessor = (module.exports.registerProcessor = function(
  processorId,
  processor,
) {
  ...
  if (processorId) {
    validator
      .check(_processors[processorId], 'This processor is already registerd')
      .isNull();
  }
  validator.check(null, 'Missing processor').isObject(processor);
  if (processor) {
    validator
      .check(processor.test, 'The processor has no test method')
      .notNull();
    validator
      .check(
        processor.generatePreviews,
        'The processor has no generatePreviews method',
      )
      .notNull();
  }
 ...
 */
var unregisterProcessor = (module.exports.unregisterProcessor = function(
  processorId,
) {
  if (!processorId) {
 ...
 */
var getProcessor = (module.exports.getProcessor = function(
  ctx,
  contentObj,
  callback,
) {
    ...
    processor.test(ctx, contentObj, function(err, score) {
      ...
      if (scoredProcessors.length === processors.length) {
        ...
        if (scoredProcessors[0].score === scoredProcessors[1].score) {
          log().warn(
            { contentId: contentId },
            'Has 2 processors with an equal score',
          );
        }
 ...
 */
var submitForProcessing = (module.exports.submitForProcessing = function(
  contentId,
  revisionId,
) {
  log().trace(
 ...
 */
var submitFolderForProcessing = (module.exports.submitFolderForProcessing = function(
  folderId,
) {
  log().trace(
    { folderId: folderId },
    'Submitting for folder preview processing',
  );
  TaskQueue.submit(PreviewConstants.MQ.TASK_GENERATE_FOLDER_PREVIEWS, {
    folderId: folderId,
  });
});
 ...
 * @param  {Object}     [callback.err]                      An error that occurred, if any
 */
var reprocessPreviews = (module.exports.reprocessPreviews = function(
  ctx,
  filters,
  callback,
) {
  callback =
    ...
    function(err) {
      if (err) {
        log().error(
          { err: err, statuses: statuses },
          'Failed to invoke reprocess previews task',
        );
      }
    };
...
var reprocessPreview = (module.exports.reprocessPreview = function(
  ...
  validator
    ...
    .isResourceId();
  validator
    .check(revisionId, { code: 400, msg: 'A revision id must be provided' })
    .isResourceId();
  validator
  ...
  if (!ctx.user().isAdmin(contentTenantAlias)) {
    return callback({
      code: 401,
      msg:
        "You must be admin of the content item's tenant to reprocess its previews",
    });
  }
...
var _handleGenerateFolderPreviewsTask = function(data, callback) {
  callback =
    ...
    function(err) {
      if (err) {
        log().error(
          { err: err, data: data },
          'Error handling folder preview generation',
        );
      }
    };
  ...
  log().info(
    ...
    'Starting preview folder generation process',
  );
  FolderProcessor.generatePreviews(data.folderId, function(err) {
    if (err) {
      log().error(
        { err: err, folderId: data.folderId },
        'Error when trying to process a folder',
      );
...
var _handleGeneratePreviewsTask = function(data, callback) {
  callback =
    ...
    function(err) {
      if (err) {
        log().error(
          { err: err, data: data },
          'Error handling preview generation',
        );
      }
    };
  ...
  var start = Date.now();
  log().info(
    { contentId: data.contentId, data: data },
...
var _handleRegeneratePreviewsTask = function(data, callback) {
  callback =
    ...
    function(err) {
      if (err) {
        log().error(
          { err: err, opts: opts },
          'Error reprocessing all previews',
        );
      }
    };
  ...
  var _onEach = function(contentRows, done) {
    ...
    if (!filterGenerator.needsRevisions() || _.isEmpty(filteredContent)) {
      ...
      ContentDAO.Revisions.getAllRevisionsForContent(contentIds, function(
        err,
        revisionsByContent,
      ) {
        if (err) {
          log().error(
            { err: err },
            'Error trying to retrieve revisions for content',
          );
        }
  ...
  ContentDAO.Content.iterateAll(
    filterGenerator.getContentColumnNames(),
    1000,
    _onEach,
    function(err) {
      if (err) {
        log().error(
          { err: err },
          'Error scanning content items for preview reprocessing',
        );
        return callback(err);
      }
