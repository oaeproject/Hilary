 ...
 * @param  {String}         role                The role that should be assigned to the members
 * @param  {Number}         httpCode            The expected failure HTTP code of the share content request
 * @param  {Function}       callback            Standard callback function
 * @throws {AssertionError}                     Thrown if there is an error ensuring that the request fails in the specified manner
 */
var assertShareContentFails = module.exports.assertShareContentFails = function(managerRestContext, actorRestContext, contentId, memberIds, role, httpCode, callback) {
    // Get the members library so we can ensure it does not change after the failure
    getAllContentMembers(managerRestContext, contentId, null, function(membersBeforeUpdate) {
        membersBeforeUpdate = AuthzTestUtil.getMemberRolesFromResults(membersBeforeUpdate);

        // Perform the update and ensure it fails as expected
        RestAPI.Content.shareContent(actorRestContext, contentId, memberIds, role, function(err) {
            assert.ok(err);
            assert.strictEqual(err.code, httpCode);
 ...
 * @param  {String[]}       memberIds           The ids of the members with which to share the content item
 * @param  {String}         role                The role that should be given to the members
 * @param  {Function}       callback            Standard callback function
 * @throws {AssertionError}                     Thrown if there is an error verifying that the content item is successfully shared
 */
var assertShareContentSucceeds = module.exports.assertShareContentSucceeds = function(managerRestContext, actorRestContext, contentId, memberIds, role, callback) {
    // Ensure the members library is currently built
    getAllContentMembers(managerRestContext, contentId, null, function(membersBeforeUpdate) {
        membersBeforeUpdate = AuthzTestUtil.getMemberRolesFromResults(membersBeforeUpdate);
        ...
        _.each(memberIds, function(memberId) {
            if (!membersBeforeUpdate[memberId] || ContentConstants.roles.ALL_PRIORITY.indexOf(membersBeforeUpdate[memberId]) < ContentConstants.roles.ALL_PRIORITY.indexOf(role)) {
                roleChange[memberId] = role;
            }
        });
