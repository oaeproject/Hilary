...
var setupPassportSerializers = function(cookieSecret) {
    ...
    passport.serializeUser(function(oaeAuthInfo, done) {
        ...
        if (oaeAuthInfo.user) {
            ...
            if (oaeAuthInfo.imposter) {
                AuthenticationAPI.emit(AuthenticationConstants.events.USER_IMPOSTERED, oaeAuthInfo.imposter, oaeAuthInfo.user);
            } else {
                var strategyName = AuthenticationUtil.parseStrategyId(oaeAuthInfo.strategyId).strategyName;
                AuthenticationAPI.emit(AuthenticationConstants.events.USER_LOGGED_IN, oaeAuthInfo.user, strategyName);
            }
        }

        // Encrypt the serialized information so it cannot be tampered with
        var cookieData = _encryptCookieData(JSON.stringify(toSerialize), cookieSecret);
...
var ensureSchema = function(callback) {
    Cassandra.createColumnFamilies({
        'AuthenticationLoginId': 'CREATE TABLE "AuthenticationLoginId" ("loginId" text PRIMARY KEY, "userId" text, "password" text, "secret" text)',
        ...
        'ShibbolethMetadata': 'CREATE TABLE "ShibbolethMetadata" ("loginId" text PRIMARY KEY, "persistentId" text, "identityProvider" text, "affiliation" text, "unscopedAffiliation" text)'
    }, callback);
};
