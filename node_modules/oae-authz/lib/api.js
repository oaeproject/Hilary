...
var _updateRoles = function(resourceId, changes, callback) {
    ...
    if (AuthzUtil.isGroupId(resourceId)) {
        // Collect the user ids that need invalidating
        var invalidatedUsers = {};

        // Collect the group ids for which the members need to be invalidated
        var groupsToInvalidate = [];

        // Keep track of the groups that we've already invalidated as we walk through the
        // memberships. We already invalidate the `resourceId` group previously
        var invalidatedGroups = {};
        invalidatedGroups[resourceId] = true;

        _.each(changes, function(change, principalId) {
            if (AuthzUtil.isUserId(principalId)) {
                // We simply invalidate all user rows from the AuthzMembershipsCache
                invalidatedUsers[principalId] = true;
                queries.push({
                    'query': 'DELETE FROM "AuthzMembershipsCache" WHERE "principalId" = ?',
                    'parameters': [principalId]
                });
            } else if (AuthzUtil.isGroupId(principalId)) {
                // Queue groups to have their member users (direct and indirect) invalidated from
                // the AuthzMembershipsCache
                groupsToInvalidate.push(principalId);
            }
        });

        /*!
         * Perform the process of recursively aggregating the queries to invalidate all member users
         * (direct or indirect) of the list of `groupsToInvalidate`
         */
        var _invalidateGroupMembers = function() {
            if (_.isEmpty(groupsToInvalidate)) {
                // When there are no more groups to invalidate, we run the final list of queries and
                ...
                Cassandra.runBatchQuery(queries, function(err) {
                    ...
                    return callback(null, _.keys(invalidatedUsers));
                });
                return;
            }

            // Get all of the members of the next group, so they can be invalidated
            ...
            getAuthzMembers(groupId, null, 10000, function(err, members) {
                ...
                _.each(members, function(member) {
                    ...
                    // Invalidate all member users
                    if (AuthzUtil.isUserId(memberId) && !invalidatedUsers[memberId]) {
                        invalidatedUsers[memberId] = true;
                        queries.push({
                            'query': 'DELETE FROM "AuthzMembershipsCache" WHERE "principalId" = ?',
                            'parameters': [memberId]
                        });
                    ...
                    } else if (AuthzUtil.isGroupId(memberId) && !invalidatedGroups[memberId] && !_.contains(groupsToInvalidate, memberId)) {
                        groupsToInvalidate.push(memberId);
                        invalidatedGroups[memberId] = true;
                    }
                });

                // Recursively aggregate the queries to invalidate the group members of the next group in
                // the `groupsToInvalidate` list
                return _invalidateGroupMembers();
            });
        };

        // Start aggregating queries to invalidate group members if there are any groups to
        // invalidate
        return _invalidateGroupMembers();
    } else {
        // The resource was not a group, so we don't have to worry about membership cache
        // invalidations
        Cassandra.runBatchQuery(queries, function(err) {
            if (err) {
                return callback(err);
    ...
    Cassandra.runQuery('SELECT "groupId" FROM "AuthzMembershipsCache" WHERE "principalId" = ? LIMIT 1', [userId], function(err, rows) {
        ...
        // There are no columns, we need to explode the user's group memberships
            ...
            _explodeGroupMemberships(userId, function(err, graph) {
                ...
                // removed
                var allMemberships = _.chain(graph.getNodes()).pluck('id').without(userId).value().sort();
...
var _explodeGroupMemberships = function(principalId, callback) {
    ...
    _getAuthzGroupMembershipGraph([principalId], function(err, graph) {
        if (err) {
            return callback(err);
        }

        // Generate a sorted set of all the Authz groups the principal is a member of
        var allMemberships = [];
        var indirectMemberships = [];
        graph.forEachNode(function(node, nodeId) {
            if (nodeId !== principalId) {
                allMemberships.push(nodeId);
                if (!graph.getEdge(principalId, nodeId)) {
                    // A group memberships is indirect if there is no membership edge that links it
                    // directly to the target principal
                    indirectMemberships.push(nodeId);
                }
            }
        });

        // We should not cache anything for groups into the AuthzMembershipsCache or the
        // AuthzMembershipsIndirectCache. So break early to avoid that
        ...
        var authzMembershipsCacheQueries = _.map(allMemberships, function(groupId) {
            // For each group, get the direct members that are relevant in the memberships graph and
            // their roles. We will JSON encode and store them in the cache so that we can
            // reconstruct the graph quickly
            var memberRoles =
                _.chain(graph.getInEdgesOf(groupId))
                    .map(function(edge) {
 ...
 *  * The resulting graph will always contain a node representing the given `principalId`, even if they have no memberships
 *  * Each node has an `id` property which indicates the principal id (e.g., "u:oae:branden", "g:oae:developers")
 *  * The inbound edges of each node denote a "has member" relationship
 *      * i.e., (Branden) --> (OAE Developers) indicates that the "OAE Developers" group "has member" "Branden"
 *  * The outbound edges then obviously denote a "is member of" relationship
 *      * i.e., (Branden) --> (OAE Developers) indicates that "Branden" "is member of" the "OAE Developers" group
 *  * Each edge has a `role` property which indicates the role the source principal has in the target principal
 *
...
var getPrincipalMembershipsGraph = module.exports.getPrincipalMembershipsGraph = function(principalId, callback) {
    ...
    Cassandra.runAllPagesQuery('AuthzMembershipsCache', 'principalId', principalId, 'groupId', null, function(err, rows) {
        if (err) {
            ...
            // If we have no data, go to the memberships CFs to try and resolve it recursively
        ...
        _.each(rows, function(row) {
            ...
            var memberRoles = JSON.parse(row.get('value').value);

            graph.addNode(groupId);
            _.each(memberRoles, function(memberRole) {
                group.addNode(memberRole.memberId);

                var edge = group.addEdge(memberRole.memberId, groupId);
                edge.role = memberRole.role;
            });
        });
...
var getPrincipalMemberships = module.exports.getPrincipalMemberships = function(principalId, start, limit, callback) {
    ...
    Cassandra.runPagedQuery('AuthzMembershipsCache', 'principalId', principalId, 'groupId', start, limit, null, function(err, rows, nextToken, startMatched) {
        ...
        _explodeGroupMemberships(principalId, function(err, graph) {
            ...
            var allMemberships = _.chain(graph.getNodes()).pluck('id').without(principalId).value().sort();
            var startIndex = 0;
            if (start) {
...
var _getAuthzGroupMembershipGraph = function(principalIds, callback, _graph) {
    if (!_graph) {
    ...
    getRolesForPrincipalsAndResourceType(principalIds, AuthzConstants.resourceTypes.GROUP, function(err, principalParentRole) {
        ...
        _.each(principalParentRole, function(parentRole, principalId) {
            _.each(parentRole, function(role, parentGroupId) {
                ...
                var edge = _graph.addEdge(principalId, parentGroupId);
                edge.role = role;
            });
        });
        ...
        // principals
        return _getAuthzGroupMembershipGraph(nextPrincipalIds, callback, _graph);
    });
};
 ...
 * @param  {Object[]}   targetResources                         The standard resources to which the user is trying to associate the resource (e.g., Adding a user to a group, or adding a content item to a folder)
 * @param  {Function}   callback                                Standard callback function
 * @param  {Object}     callback.err                            An error that occurred, if any
