...
var getActivities = module.exports.getActivities = function(activityStreamId, start, limit, callback) {
    ...
    Cassandra.runQuery('SELECT FIRST ' + paging.limit + ' REVERSED ? .. \'\' FROM ActivityStreams USING CONSISTENCY ONE WHERE activityStreamId = ?', [paging.start, activityStreamId], function(err, rows) {
        if (err) {
            return callback(err);
        }
...
var deliverActivities = module.exports.deliverActivities = function(routedActivities, callback) {
    ...
    _.keys(routedActivities).forEach(function(route) {
        ...
        queries.push({
            'query': 'UPDATE ActivityStreams USING TTL ' + ActivityConfig.getConfig().activityTtl + ' SET ' + sets.join(', ') + ' WHERE activityStreamId = ?',
            'parameters': params
        });
    });
    ...
    for (var i = 0; i < activityBuckets.length; i++) {
        ...
        if (activityBucket && activityBucket.length) {
            ...
            activityBucket.forEach(function(routedActivity) {
                sets.push('? = ?');
                // Order by time so we can prioritize collection of older activities over newer activities
...
var getQueuedActivities = module.exports.getQueuedActivities = function(bucketNumber, limit, callback) {
    limit = OaeUtil.getIntParam(limit, ActivityConfig.getConfig().collectionBatchSize);
 ...
 * are currently aggregating, expired, the id of the last activity delivery, etc...
 *
...
/**
 * Get all the entities that have been aggregated for the aggregate key.
 *
...
var saveAggregatedEntities = module.exports.saveAggregatedEntities = function(aggregates, callback) {
    ...
    // Append the update queries for each aggregate key
    aggregateKeys.forEach(function(aggregateKey) {
        var aggregate = aggregates[aggregateKey];
        _appendAggregatedEntityQueries(aggregateKey, aggregate, 'actors', queries);
        _appendAggregatedEntityQueries(aggregateKey, aggregate, 'objects', queries);
        _appendAggregatedEntityQueries(aggregateKey, aggregate, 'targets', queries);
    });
...
/**
 * For an aggregate, append the Cassandra query needed to update the aggregate with the new entities of the specified `entityType`.
 *
 ...
 * @param   {String}    entityType      The type of entity to create the query for. One of 'actors', 'objects' or 'targets'
 * @param   {Object[]}  queries         The array to which to append the update queries
 * @api private
 */
var _appendAggregatedEntityQueries = function(aggregateKey, aggregate, entityType, queries) {
    var sets = [];
    var params = [];
    var entities = aggregate[entityType];
    var keys = _.keys(entities);
    if (keys.length > 0) {
        keys.forEach(function(entityKey) {
            sets.push('? = ?');
            params.push(entityKey, JSON.stringify(entities[entityKey]));
        });

        // We post-fix the aggregate with entity type, because it's just the entities for that type
        params.push(util.format('%s#%s', aggregateKey, entityType));
