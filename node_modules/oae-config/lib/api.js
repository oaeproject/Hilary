...
module.exports.config = function(moduleId) {
    ...
    return {
         ...
         * @return {Date}       cachedConfiguration   The timestamp when the config value was last updated. If the element could not be found, the epoch date will be returned
         */
...
var updateConfig = module.exports.updateConfig = function(ctx, tenantAlias, configValues, callback) {
    ...
    // Convert the provided values to the correct storage value
    var storageValues = {};
    var temp = {};
    _.each(configValues, function(newValue, key) {
        ...
        storageValues[columnName] = cachedGlobalSchema[module][feature].elements[element].toDBValue(previousValue, optionalKey, newValue);

        // Convert that output back in to a JSON value, so we can use it for the next key (if it needs it)
        temp[columnName] = cachedGlobalSchema[module][feature].elements[element].toJSONValue(storageValues[columnName]);
    });
