...
/**
 * Determine which visibility level of library the user in context should receive from the target library owner.
 * The following visibility matrix will be tested.
 * Assume that Tenant A en B both have enabled tenant permeability and Tenant C has it disabled.
 * The column headers defined the status of the current user (whether he is anonymous, an authenticated user or if he's a member),
 * the rows express the visibility setting of the library owner.
 * Each cell will then express which library stream should be returned. A `/` means the user is not allowed to see the library.
 *
 *
 *                        ||                    Tenant A                     ||                    Tenant B                    ||           Tenant C
 *                        ||     Anon    |   Loggedin user   |   Member      ||      Anon    |   Loggedin user   |   Member    ||      Anon    |   Loggedin user
 *         ------------------------------------------------------------------------------------------------------------------- ----------------------------------
 *         public group   ||     public  |     logged in     |  private      ||     public   |      public       |   private   ||     public   |      public
 *                        ||             |                                   ||              |                   |             ||              |
 *         loggedin group ||       /     |     logged in     |  private      ||        /     |        /          |   private   ||        /     |        /
 *                        ||             |                                   ||              |                   |             ||              |
 *         private group  ||       /     |        /          |  private      ||        /     |        /          |   private   ||        /     |        /
 *
 *
 ...
 * @param  {String}     callback.visibility The maximum visibility (as enumerated in `LibraryConstants.visibility.ALL_PRIORITY`) of library the user in context can see from the target library owner. If the user has no access, this value will be null
 */
