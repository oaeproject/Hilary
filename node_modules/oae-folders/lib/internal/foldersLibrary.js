...
var list = module.exports.list = function(principal, visibility, opts, callback) {
    LibraryAPI.Index.list(FoldersConstants.library.FOLDERS_LIBRARY_INDEX_NAME, principal.id, visibility, {'start': opts.start, 'limit': opts.limit}, callback);
};
...
var insert = module.exports.insert = function(principalIds, folder, callback) {
    ...
    var effectiveLibraryVisibilities = LibraryAPI.Authz.resolveEffectiveLibraryVisibilities(principalIds, folder);
    LibraryAPI.Index.insert(FoldersConstants.library.FOLDERS_LIBRARY_INDEX_NAME, effectiveLibraryVisibilities, folder.id, folder.lastModified, callback);
};
...
var update = module.exports.update = function(principalIds, folder, oldLastModified, callback) {
    ...
    OaeUtil.invokeIfNecessary(touchFolder, FoldersDAO.updateFolder, folder, {}, function(err, updatedFolder) {
        ...
        var effectiveLibraryVisibilities = LibraryAPI.Authz.resolveEffectiveLibraryVisibilities(principalIds, folder);
        LibraryAPI.Index.update(FoldersConstants.library.FOLDERS_LIBRARY_INDEX_NAME, effectiveLibraryVisibilities, updatedFolder.id, updatedFolder.lastModified, oldLastModified, function(err) {
            if (err) {
                return callback(err);
            }

            // Return the most recent version of the folder to the caller. If we updated the
            // timestamp of the folder, we want to make sure it has the most recent version
            return callback(null, updatedFolder || folder);
        });
    });
};
...
var remove = module.exports.remove = function(principalIds, folder, callback) {
    ...
    LibraryAPI.Index.remove(FoldersConstants.library.FOLDERS_LIBRARY_INDEX_NAME, principalIds, folder.id, folder.lastModified, callback);
};
