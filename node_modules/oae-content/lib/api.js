...
var createCollabDoc = module.exports.createCollabDoc = function(ctx, displayName, description, visibility, additionalMembers, callback) {
    ...
    Etherpad.createPad(contentId, function(err, ids) {
        ...
        _createContent(ctx, contentId, 'collabdoc', displayName, description, visibility, additionalMembers, ids, {'etherpadHtml': ''}, function(err, content, revision) {
            if (err) {
                return callback(err);
            }
 ...
 */
var verifySignedPreviewItem = module.exports.verifySignedPreviewItem = function(ctx, id, previewItem, signature, callback) {
    var validator = new Validator();
    validator.check(id, {'code': 400, 'msg': 'Missing content ID'}).notEmpty();
    ...
    if (_.isObject(signature)) {
        ...
        var tenantAlias = (id) ? AuthzUtil.getResourceFromId(id).tenantAlias : '';
        ...
        var body = principalId + ':' + id + ':' + lastModified;
    ...
    ContentDAO.Previews.getContentPreview(id, previewItem, function(err, preview) {
        if (err) {
            return callback(err);
        }
            ...
            _.each(revisions, function(revision) {
                if (users[revision.createdBy]) {
                    ...
                    if (revision.thumbnailUrl) {
                        ...
                        revision.thumbnailUrl = ContentUtil.getDownloadUrlForUri(ctx, tenantAlias, revision.thumbnailUrl);
                    }
                }
            });
...
var _augmentContent = function(ctx, contentObj) {
    ...
    if (contentObj.previews && contentObj.previews.thumbnailUri) {
        contentObj.previews.thumbnailUrl = ContentUtil.getDownloadUrlForUri(ctx, contentObj.tenant, contentObj.previews.thumbnailUri);
    }
};
