...
OAE.globalAdminRouter.on('get', '/api/auth/signed', function(req, res) {
...
OAE.tenantRouter.on('post', '/api/auth/signed', function(req, res, next) {
    ...
    passport.authenticate(strategyId, { 'successRedirect': redirectUrl,
...
var _getSignedToken = function(ctx, tenantAlias, callback) {
    ...
    // Create a signature that's valid for at most 2 minutes and can be used to log on the tenant
    var userId = ctx.user().id;
    var signature = Signature.createExpiringSignature({'tenantAlias': tenantAlias, 'userId': userId}, TIME_1_MINUTE_IN_SECONDS, TIME_1_MINUTE_IN_SECONDS);
