...
var getDirectRoles = module.exports.getDirectRoles = function(principalIds, resourceId, callback) {
    ...
    var placeholders = _.map(principalIds, function() {
        return '?';
    });
    var parameters = _.union([resourceId], principalIds);
    var cql = util.format('SELECT "memberId", "role" FROM "AuthzMembers" WHERE "resourceId" = ? AND "memberId" IN (%s)', placeholders.join(','));

    Cassandra.runQuery(cql, parameters, {'cache': false}, function(err, rows) {
        if (err) {
            return callback(err);
        }
...
var _checkGroupMembershipsForUser = function(userId, groupIds, callback) {
    ...
    Cassandra.runQuery('SELECT "groupId" FROM "AuthzMembershipsCache" WHERE "principalId" = ? LIMIT 1', [userId], function(err, rows) {
        ...
        if (rows.length === 1) {
            var placeholders = _.map(groupIds, function() {
                return '?';
            });
            var parameters = _.union([userId], groupIds);
            var cql = util.format('SELECT "groupId" FROM "AuthzMembershipsCache" WHERE "principalId" = ? AND "groupId" IN (%s)', placeholders.join(','));
            ...
            Cassandra.runQuery(cql, parameters, {'cache': false}, function(err, rows) {
                ...
                return callback(null, _.pluck(rows, 'groupId'));
            });
...
var getPrincipalMembershipsGraph = module.exports.getPrincipalMembershipsGraph = function(principalId, callback) {
    ...
    Cassandra.runAllPagesQuery('AuthzMembershipsCache', 'principalId', principalId, 'groupId', null, function(err, rows) {
        ...
        _.each(rows, function(row) {
            ...
            var memberRoles = {};
            try {
                memberRoles = JSON.parse(row.value);
            } catch (err) {
...
var getPrincipalMemberships = module.exports.getPrincipalMemberships = function(principalId, start, limit, callback) {
    ...
    Cassandra.runPagedQuery('AuthzMembershipsCache', 'principalId', principalId, 'groupId', start, limit, null, function(err, rows, nextToken, startMatched) {
        if (err) {
            ...
            return callback(null, _.pluck(rows, 'groupId'), nextToken);
        }
 ...
 */
var computeMemberRolesAfterChanges = module.exports.computeMemberRolesAfterChanges = function(resourceId, memberRoles, opts, callback) {
