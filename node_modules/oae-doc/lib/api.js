...
module.exports.getDoc = function(moduleId, callback) {
    ...
    // Check if a cached version is available to return
     ...
     * @param   {Object}     callback.lib    The name of the file, returned for reference purpose
     */
    var readFile = function(lib, callback) {
        fs.readFile(dir + "/" + lib, 'utf-8', function(err, data) {
            if (err) {
                return callback(err, data, lib);
            }
            callback(err, data, lib);
        });
    };
    ...
    IO.getFileListForFolder(dir, function(libs) {
        ...
        for (var i=0; i < libs.length; i++) {
            ...
            readFile(lib, function(err, data, lib) {
                done++;

                if (!err) {
                    doc[lib] = dox.parseComments(data);
                    cachedDocs[moduleId] = cachedDocs[moduleId] || {};
                    cachedDocs[moduleId][lib] = doc[lib];
                } else {
