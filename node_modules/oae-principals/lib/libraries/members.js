 ...
 */
const list = module.exports.list = function(group, visibility, opts, callback) {
    LibraryAPI.Index.list(PrincipalsConstants.library.MEMBERS_INDEX_NAME, group.id, visibility, opts, (err, entries, nextToken) => {
        if (err) {
            return callback(err);
        }
...
LibraryAPI.Index.registerLibraryIndex(PrincipalsConstants.library.MEMBERS_INDEX_NAME, {
    'pageResources'(libraryId, start, limit, callback) {
        AuthzAPI.getAuthzMembers(libraryId, start, limit, (err, memberEntries) => {
            ...
            PrincipalsDAO.getPrincipals(_.pluck(memberEntries, 'id'), ['principalId', 'tenantAlias', 'visibility', 'smallPictureUri'], (err, members) => {
                if (err) {
                    return callback(err);
                }
...
const _updatePrincipalInMembersLibraries = function(ctx, updatedPrincipal, oldPrincipal) {
    ...
    // any updates
    if (_hasProfilePicture(updatedPrincipal) === _hasProfilePicture(oldPrincipal) &&
    ...
    AuthzAPI.getAllRolesForPrincipalAndResourceType(updatedPrincipal.id, AuthzConstants.resourceTypes.GROUP, (err, allRoles) => {
        if (err) {
            return log().error({err, 'userId': user.id}, 'An error occurred while trying to update all members libraries to which a user belongs');
        }
 ...
 */
PrincipalsEmitter.when(PrincipalsConstants.events.UPDATED_GROUP_MEMBERS, (ctx, group, oldGroup, memberChangeInfo, opts, callback) => {
...
PrincipalsEmitter.when(PrincipalsConstants.events.JOINED_GROUP, (ctx, group, oldGroup, memberChangeInfo, callback) => {
    ...
    LibraryAPI.Index.insert(PrincipalsConstants.library.MEMBERS_INDEX_NAME, [entry], (err) => {
        if (err) {
            log().error({err, 'groupId': group.id, 'userId': ctx.user().id}, 'An unexpected error occurred trying to insert a user into a group members library');
            return callback(err);
        }
 ...
 * @api private
 */
var _getMembersLibraryRank = function(groupId, principal, role) {
    ...
    } else if (principal.visibility === AuthzConstants.visibility.LOGGEDIN && principal.tenant.alias === libraryTenantAlias) {
 ...
 */
var _hasProfilePicture = function(principal) {
