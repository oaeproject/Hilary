...
var generatePreviews = module.exports.generatePreviews = function(ctx, content, callback) {
    ...
    log().info('Generating previews for media file: %s', content.id);
    ...
    ZencoderUtil.createJob(tenant.alias, downloadPath, content.id, ctx.revisionId, revision.mime, function(err, res, body) {
        if (err) {
            log().error({'err': err, 'msg': 'Error occurred when posting file to Zencoder'});
            return callback(err);
        } else if (res.statusCode >= 400) {
            var responseErr = {'code': res.statusCode, 'body': body};
            log().error({'err': responseErr, 'msg': util.format('Received status code %s when posting file to Zencoder', res.statusCode)});
            return callback(responseErr);
        } else if (!body.outputs) {
            var bodyError = {'err': body, 'msg': 'Zencoder response is missing outputs field'};
            log().error(bodyError);
