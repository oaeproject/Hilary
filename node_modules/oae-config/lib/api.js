        ...
        _.each(cachedGlobalSchema, function(module, moduleKey) {
            ...
            _.each(module, function(feature, featureKey) {
                ...
                _.each(feature.elements, function(element, elementKey) {
                    ...
                    if (tenantConfig[key]) {
                        cachedGlobalConfig[tenantAlias][moduleKey][featureKey][elementKey] = Util.castToBoolean(tenantConfig[key]);
                    }
...
var canUpdateConfigValue = function(ctx, tenantAlias, moduleId, feature, element) {
    ...
    // Check if the current user is a tenant administrator and the configuration option was not marked as `tenantOverride=false`
    // If the user is a tenant and the option is marked as false, return false as the user is not allowed to update this configuration value
    if (ctx.user().isTenantAdmin(tenantAlias) && !cachedGlobalSchema[moduleId][feature].elements[element].tenantOverride) {
        return false;
    }

    // Verify that the configuration option is not updated by a non-admin and marked as `globalAdminOnly` and
    // verify that if it is marked as `globalAdminOnly` the user is a global administrator.
    var globalAdminOnly = cachedGlobalSchema[moduleId][feature].elements[element].globalAdminOnly === true;
    if (!globalAdminOnly || (globalAdminOnly && ctx.user().isGlobalAdmin())) {
        return true;
    }
    return false;
};
