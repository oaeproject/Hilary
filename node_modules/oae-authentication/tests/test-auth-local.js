...
describe('Authentication', function() {
                        ...
                        RestAPI.Authentication.exists(anonymousCamRestContext, username.toLowerCase(), function(err) {
                            ...
                            RestAPI.Authentication.exists(anonymousCamRestContext, username.toUpperCase(), function(err) {
                                ...
                                RestAPI.Authentication.exists(anonymousGtRestContext, username, function(err) {
                                    ...
                                    // Go through the same steps for creating users through the admin UI
                                    var username = TestsUtil.generateTestUserId() + 'Bb';

                                    // Verify that the username doesn't exist yet
                                    RestAPI.Authentication.exists(globalAdminRestContext, username, function(err) {
                                        assert.equal(err.code, 404);

                                        // Create a user with this login id
                                        RestAPI.User.createUser(globalAdminRestContext, username, 'password', 'Test User', {'visibility': 'public', 'tenantAlias': global.oaeTests.tenants.cam.alias}, function(err, createdUser) {
                                            assert.ok(!err);
                                            assert.ok(createdUser);

                                            // Verify that the username exists
                                            RestAPI.Authentication.existsOnTenant(globalAdminRestContext, username, global.oaeTests.tenants.cam.alias, function(err) {
                                                assert.ok(!err);

                                                // Verify that the username exists, even if we change the text case
                                                RestAPI.Authentication.existsOnTenant(globalAdminRestContext, username.toLowerCase(), global.oaeTests.tenants.cam.alias, function(err) {
                                                    assert.ok(!err);
                                                    RestAPI.Authentication.existsOnTenant(globalAdminRestContext, username.toUpperCase(), global.oaeTests.tenants.cam.alias, function(err) {
                                                        assert.ok(!err);

                                                        // Verify that the username is still available on different tenants
                                                        RestAPI.Authentication.existsOnTenant(camAdminRestContext, username, global.oaeTests.tenants.gt.alias, function(err) {
                                                            assert.equal(err.code, 404);
                                                            callback();
                                                        });
                                                    });
    ...
    describe('#createGlobalAdminUser', function() {
        ...
        it('verify authorization of creating a global admin user', function(callback) {
            ...
            RestAPI.Authentication.createGlobalAdminUser(anonymousCamRestContext, userId, userId, userId, function(err, user) {
                assert.ok(err);
                assert.equal(err.code, 404);
                assert.ok(!user);

                // Ensure anonymous on global admin tenant cannot create a global admin user
                RestAPI.Authentication.createGlobalAdminUser(anonymousGlobalRestContext, userId, userId, userId, function(err, user) {
                    assert.ok(err);
                    assert.equal(err.code, 401);
                    assert.ok(!user);
                    ...
                    // only mounted on the global tenant
                    RestAPI.Authentication.createGlobalAdminUser(camAdminRestContext, userId, userId, userId, function(err, user) {
                        assert.ok(err);
                        assert.equal(err.code, 404);
                        assert.ok(!user);
                        ...
                        RestAPI.Authentication.login(testGlobalUserRestContext, userId, userId, function(err) {
                            ...
                            RestAPI.User.getMe(testGlobalUserRestContext, function(err, me) {
                                ...
                                RestAPI.Authentication.createGlobalAdminUser(globalAdminRestContext, userId, userId, userId, function(err, user) {
                                    assert.ok(!err);
                                    assert.ok(user);
        ...
        it('verify validation of creating a global admin user', function(callback) {
            ...
            RestAPI.Authentication.createGlobalAdminUser(globalAdminRestContext, null, userId1, userId1, function(err, user) {
                assert.ok(err);
                assert.equal(err.code, 400);
                assert.ok(!user);

                // Ensure you cannot create a global admin user without a password
                RestAPI.Authentication.createGlobalAdminUser(globalAdminRestContext, userId1, null, userId1, function(err, user) {
                    assert.ok(err);
                    assert.equal(err.code, 400);
                    assert.ok(!user);

                    // Ensure you cannot create a global admin user with a password that is too short
                    RestAPI.Authentication.createGlobalAdminUser(globalAdminRestContext, userId1, 'a', userId1, function(err, user) {
                        assert.ok(err);
                        assert.equal(err.code, 400);
                        assert.ok(!user);

                        // Ensure you cannot create a global admin user without a displayName
                        RestAPI.Authentication.createGlobalAdminUser(globalAdminRestContext, userId1, userId1, null, function(err, user) {
                            assert.ok(err);
                            assert.equal(err.code, 400);
                            assert.ok(!user);
                            ...
                            // Ensure that the credentials do not authenticate a value global administrator
                            RestAPI.Authentication.login(testGlobalUserRestContext, userId1, userId1, function(err) {
                                assert.ok(err);
                                assert.equal(err.code, 401);
                                ...
                                RestAPI.User.getMe(testGlobalUserRestContext, function(err, me) {
                                    ...
                                    RestAPI.Authentication.createGlobalAdminUser(globalAdminRestContext, userId1, userId1, userId1, function(err, user) {
                                        assert.ok(!err);
                                        assert.ok(user);
