 ...
 * @api private
 */
var _getContent = function(contentId, callback) {
...
/**
 * Get the content object and members of a content item. If the `contentObj` is specified, it will simply be returned as the content item. If
 * `members` is specified, it will simply be returned as the members.
 *
 *  TODO: This is necessary as access to the 'internal' methods of content are required. The refactoring is a significant conflict with other
 *        work in progress, so this API method exists for now. Do not use it as it is not permission protected.
 * 
 * @param   {String}    contentId           The id of the content item we're indexing
 * @param   {Content}   [contentObj]        The content object we're indexing. If not specified, it will be fetched using the `contentId`.
 * @param   {String[]}  [members]           The content memembers. If not specified, it will be fetched using the `contentId`.
 * @param   {Function}  callback            Invoked when the process completes
 * @param   {Object}    callback.err        An error that occurred, if any
 * @param   {Object}    callback.contentObj The content item
 * @param   {String[]}  callback.members    The principal ids that are members of the content item
 */
var getContentWithMembers = module.exports.getContentWithMembers = function(contentId, contentObj, members, callback) {
    _getContentIfNecessary(contentId, contentObj, function(err, contentObj) {
        if (err) {
            return callback(err);
        }

        _getMembersIfNecessary(contentId, members, function(err, members) {
            if (err) {
                return callback(err);
            }

            return callback(null, contentObj, members);
        });
    });
}
...
/**
 * Get a content's basic profile information based on a pooled content id, only if it is not provided as `contentObj`. This is simply
 * a convenience method for conditionally
 * 
 * @param  {String}         contentId           The id of the content object we want to retrieve
 * @param  {Content}        [contentObj]        The content item we're trying to get. If this is specified, it will simply be returned in the `callback`.
 * @param  {Function}       callback            Standard callback function
 * @param  {Object}         callback.err        Error object containing the error message
 * @param  {Content}        callback.contentObj Retrieved content object
 */
var _getContentIfNecessary = function(contentId, contentObj, callback) {
    if (contentObj) {
        return callback(null, contentObj);
    }

    _getContent(contentId, callback);
}

/**
 * Internal function that gets all of the principals that are directly associated to a piece of content, only if they are not found in
 * the `members` parameter. This is simply a convenience function for conditionally accessing the membership.
 * 
 * @param  {String}         contentId           The id of the content object for which we want to get the members
 * @param  {String[]}       [members]           An array of principal ids that represent the members of the content item. If specified, it will simply be returned in the `callback`.
 * @param  {Function}       callback            Standard callback function takes arguments `err` and `members`
 * @param  {Object}         callback.err        Error object containing the error message
 * @param  {Object}         callback.members    Object for which the keys are principal ids and values are their roles on the content
 * @api private
 */
var _getMembersIfNecessary = function(contentId, members, callback) {
    if (members) {
    ...
    _getAllContentMembers(contentId, function(err, members) {
        ...
        return callback(null, _.keys(members));
    });
}
