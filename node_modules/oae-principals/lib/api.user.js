...
/**
 * Get all the users creation date within a tenant
 *
 * @param  {Context}    ctx                 Standard context object containing the current user and the current tenant
 ...
 * @param  {String}     months              Number of months that a user has not logged in - if the type of the request is `auth`
 * @param  {Function}   callback            Standard callback function
 * @param  {Object}     callback.err        An error that occurred, if any
 * @param  {String}     callback.csv        A CSV with all creation dates
 */
const getUsersDateLog = module.exports.getUsersDateLog = function(ctx, tenantAlias, type, months, callback) {

    callback = callback || function() {};

    let userNotFoundErr = {'code': 404, 'msg': 'No users were found'};
    let numberOfMonths = parseInt(months);

    let isAdmin = (ctx.user() && ctx.user().isAdmin());
    if (!isAdmin) {
        return callback({'code': 401, 'msg': 'You are not authorised to obtain user connection logs'});
    }
    ...
    PrincipalsDAO.getAllUsersForTenant(tenantAlias, function(err, users) {
        ...
        if (type === PrincipalsConstants.downloadLogType.CREATED) {
            _getCsv(users, type, function(err, csv) {
                return callback(null, csv);
            });
        
        // If the type of connection is `auth` get only the connection older than someTimeAgo - {numberOfMonths}
        } else {
            if (!Number.isInteger(numberOfMonths)) {
                return callback({'code': 400, 'msg': 'The month should be a number'});
            }
            
            _getOnlyUsersWhoHaventLoggedInInAWhile(users, numberOfMonths, type, function(err, users) {
                if (err) {
                    return callback(err);
                }
 ...
 * @param  {Function}   callback            Standard callback function
 * @param  {Object}     callback.err        An error that occurred, if any
 * @param  {Object}     callback.users      List of users from the current tenant who haven't logged in {numberOfMonths} month(s)
 * @api private
 */
const _getCsv = function(users, type, callback) {

    // Sort row by older date
    users.sort((a,b) => { return a.created - b.created; });

    // Create and return the csv file
    let csv = ['principalId,tenantAlias,email,publicAlias,'];

    if (type === PrincipalsConstants.downloadLogType.CREATED) {
        csv[0] += 'created';
    } else if (type === PrincipalsConstants.downloadLogType.AUTH) {
        csv[0] += 'lastLogin';
    }

    _.each(users, function(user) {
        if (user !== undefined) {
            let txt = `${user.id},${user.tenant.alias},${user.email},${user.publicAlias},`;
            if (type === PrincipalsConstants.downloadLogType.CREATED) {
                csv.push(txt + `${user.created}`);
            } else if (type === PrincipalsConstants.downloadLogType.AUTH) {
                csv.push(txt + `${user.lastLogin}`);
            }
        }
    });
    csv = csv.join('\n');
...
/**
 * Get only the connection older than someTimeAgo - {numberOfMonths}
 *
 * @param  {Object}     users             List of users from the current tenant
 ...
 * @api private
 */
const _getOnlyUsersWhoHaventLoggedInInAWhile = function(users, numberOfMonths, type, callback) {
    let now = new Date();
    let someTimeAgo = now.setMonth(now.getMonth() - numberOfMonths);
    let usersWhoHaventLoggedInInAWhile = _.reject(users, function(user) { if (user !== undefined) { return user.lastLogin == undefined || user.lastLogin > someTimeAgo; } });
