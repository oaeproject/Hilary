 ...
 */
var Cleaner = (module.exports = new EmitterAPI.EventEmitter());
...
var start = (module.exports.start = function(directory, interval) {
  ...
  cleaners[directory] = setInterval(
    cleanDirectory,
    interval,
    interval,
    directory,
...
var stop = (module.exports.stop = function(directory) {
  if (cleaners[directory]) {
    ...
    log().warn(
      { directory: directory },
      'A request to stop an unknown cleaning job was made.',
    );
  }
});
...
var checkFile = function(path, time, callback) {
  fs.stat(path, function(err, stats) {
    ...
    if (err && err.code === 'ENOENT') {
      ...
      log().error(
        { err: err, path: path },
        'Could not get the metadata for a file.',
      );
      return callback(err);
    }
    ...
    if (stats && stats.isFile() && stats.atime.getTime() < time) {
      log().info(
        { path: path, lastModified: stats.atime.getTime(), expires: time },
      ...
      fs.unlink(path, function(err) {
        ...
        if (err && err.code !== 'ENOENT') {
          log().error(
            { err: err, path: path },
            'Could not delete an expired temporary file.',
          );
          return callback(err);
        }
        callback();
