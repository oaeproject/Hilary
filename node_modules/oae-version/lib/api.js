...
function _getUIVersion(callback) {
    _getVersionInfo(_uiPath, callback);
}
    ...
    gift.init(directory, function(err, repo) {
        ...
        // Get the tag info
         repo.tags(function(err, tags) {
            ...
            var tagsByCommitId = _.indexBy(tags, function(tag) {
                return tag.commit.id;
            });

            var hasTriedSubmodule = false;
            var innerCallback = function(err, branch) {
                if (err) {
                    // 3akai-ux might be a submodule so we need to try that
                    if(repo.path === _uiPath && !hasTriedSubmodule) {
                        log().warn('Tried fetching git branch information as a regular repo, will try as a submodule next');
                        var pathElements = repo.dot_git.split('/');
                        var dot_git = pathElements.pop();
                        var submodule = pathElements.pop();
                        pathElements.push(dot_git, 'modules', submodule);
                        repo.dot_git = pathElements.join('/');

                        hasTriedSubmodule = true;
                        return repo.branch(innerCallback);
                    }
...
function _getGitDescribeVersion(repo, branchName, tagsByCommitId, callback, _nrOfCommits, _lastCommitSha) {
    ...
    repo.commits(branchName, 30, _nrOfCommits, function(err, commits) {
        ...
        if (lastTag) {
            var describeVersion = util.format('%s+%d+%s', lastTag, _nrOfCommits, _lastCommitSha);
