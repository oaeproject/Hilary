...
/*!
 * Specifies the interval in which the same email can't be sent out again
 */
var deduplicationInterval = 7 * 24 * 60 * 60;
...
/**
 * Sends a templated email to a user if, and only if, we haven't sent out that exact email.
 * This is to prevent that the server ends up in a loop and sends out thousands of e-mails.
 *
 * There are two ways to identify an e-mail.
 *   1.  By passing in an `id` in the additional options.
 *   2.  When the `id` field is left undefined, a hash will be generated by concatenating the following fields:
 *           *  to
 *           *  subject
 *           *  txt body
 *           *  html body
 ...
 * @param  {String}     [opts.id]           A unique identifier for this email. If an email with this id has already been sent out in the last `DEDUPLICATION_INTERVAL` seconds, it will not be sent out. If left null, a hash of the mail will be used to attempt de-duplication
...
var sendEmail = module.exports.sendEmail = function(templateModule, templateId, toUser, data, opts, callback) {
    ...
    var emailInfo = _.extend({'from': EmailConfig.getValue(toUser.tenant.alias, 'general', 'from')}, renderedTemplates['meta.json'], {
        'to': toUser.email,
...
/**
 * Sends an email if, and only if, we haven't sent out that exact email.
 * This is to prevent that the server ends up in a loop and sends out thousands of e-mails.
 *
 * There are two ways to identify an e-mail.
 *   1.  By passing in an `id` in the additional options.
 ...
 * @param  {Object}     opts            Additional options
 * @param  {String}     [opts.id]       If specified, it will be used to identify this email
...
var _sendEmail = function(emailInfo, opts, callback) {
    ...
    if (opts.id) {
        md5sum.update(opts.id.toString());
    // If no unique id was specified by the user, we will generate one based on the mail data that is available
    } else {
    ...
    // We lock the mail for a sufficiently long time. If this ever turns out to be a problem we can probably scale it back to 3 days
    var lockKey = util.format('oae-emails:%s', hash);
    Locking.acquire(lockKey, deduplicationInterval, function(err, token) {
        if (err) {
            log().error({'err': err, 'hash': hash, 'emailInfo': emailInfo}, 'Unable to lock hash');
            return callback(err);
        } else if (!token) {
            log().error({'hash': hash, 'emailInfo': emailInfo}, 'A lock was already in place, this indicates we\'re sending out the same mail twice');
            return callback({'code': 403, 'msg': 'This email has already been sent out'});
        }

        // We got a lock, send our mail
        if (debug) {
