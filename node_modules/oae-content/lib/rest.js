...
OAE.tenantServer.post('/api/content/create', function(req, res) {
    ...
    } else if (req.body.resourceSubType === 'document') {
        ContentAPI.createDocument(req.ctx, req.body.displayName, req.body.description, req.body.visibility, additionalMembers, function(err, contentObj) {
            if (err) {
                return res.send(err.code, err.msg);
            }
            res.send(201, contentObj);
    ...
    // Not a recognized file type
        ...
        res.send(400, 'Unrecognized resourceSubType. Accepted values are "link", "file" and "document"');
    }
});
