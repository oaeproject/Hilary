...
var store = module.exports.store = function(ctx, file, callback) {
    ...
    var uri = util.format('%d/%d/%d/%d/%d/%s/%s', date.getFullYear(), date.getMonth() + 1, date.getDate(), date.getHours(), date.getMinutes(), id, file.name);
    ...
    _getClient(ctx).PutObject(options, function(err, data) {
        ...
        fs.unlink(file.path, function(unlinkError) {
            if (unlinkError) {
                log().error({'err': unlinkError}, 'Could not remove the temporary file.');
            }
...
var get = module.exports.get = function(ctx, uri, callback) {
    ...
    var path = uri.split(':')[1];
    var filename = Path.basename(path);
    var tempPath = Temp.path({'suffix': filename});
    ...
    log().trace('Downloading %s from S3.', path);
...
var _getBucketName = function(ctx) {
    return Config.getValue(ctx, 'storage', 'amazons3-bucket');
};
...
var _getClient = function(ctx) {
    ...
    var accessKey = Config.getValue(ctx, 'storage', 'amazons3-access-key');
    var secretKey = Config.getValue(ctx, 'storage', 'amazons3-secret-key');
    var region = Config.getValue(ctx, 'storage', 'amazons3-region');
