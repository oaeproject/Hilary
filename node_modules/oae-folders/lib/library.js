...
/*!
 * When a folder gets updated we need to update the folder libraries it's placed in
 */
FoldersAPI.on(FoldersConstants.events.UPDATED_FOLDER, function(ctx, folder, updatedFolder) {
    ...
    AuthzAPI.getAuthzMembers(folder.groupId, null, 10000, function(err, memberRoles, nextToken) {
        if (err) {
            log().error({'err': err, 'folder': folder}, 'Could not retrieve the members for a folder, their folder library will NOT be updated');
            return;
        }
        ...
        FoldersFoldersLibrary.update(principalIds, updatedFolder, folder.lastModified, function(err) {
            if (err) {
                log().error({'err': err, 'folder': folder}, 'Could not update the folder libraries for a set of users');
                return;
            }
...
ContentAPI.on(ContentConstants.events.UPDATED_CONTENT, function(ctx, newContentObj, oldContentObj) {
    ...
    FoldersAuthz.getFoldersForContent(newContentObj.id, function(err, folders) {
        ...
        // Purge all the folder content libraries for the authz folders. Note that
        // some of the items in the `folders array, may point to groups rather than
        // folders. This is OK, as they would not have a folder content library and
        // purging a non-existant library is harmless
...
ContentAPI.on(ContentConstants.events.DELETED_CONTENT, function(ctx, contentObj, members) {
    ...
    var folders = FoldersAuthz.authzPrincipalsToFolders(members);
