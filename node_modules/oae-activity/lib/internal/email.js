...
var _collectImmediateBucket = function(bucketNumber, callback) {
    var now = new Date();
    collectMails(bucketNumber, PrincipalsConstants.emailPreferences.IMMEDIATE, now.getDay(), now.getHours() + 1, callback);
};
...
var _collectDailyBucket = function(bucketNumber, callback) {
    if (_isDailyCycle()) {
        var now = new Date();
        return collectMails(bucketNumber, PrincipalsConstants.emailPreferences.DAILY, now.getDay(), now.getHours() + 1, callback);
    } else {
...
var _createEmailBucketIdForUser = function(user) {
    ...
    if (emailPreference !== PrincipalsConstants.emailPreferences.IMMEDIATE) {
         ...
         *
         * For example, assume the server is in Holland (UTC+1), the given timezone is Miami (UTC-5) and mails need
         * to be delivered at 13h. The mail needs to leave the server at 19h (UTC+1 time) so it arrives when
         * it's 13h in Miami
         *
         * For weekly emails, it's entirely possible we need to schedule email delivery on the previous/next day.
         * For example, assume the server is in Holland (UTC+1), the user's timezone is Islamabad (UTC+5) and mails
         * need to be delivered at 2am. The mails needs to leave the server at 22h (UTC+1 time) so it arrives when
         * it's 2am in Islamabad
         */
        ...
        // which we can round as we can be a bit flexible about when the email arrives. `getTimezoneOffset` returns
        // the offset against *UTC*. This is fine as we expect servers to be configured and run in UTC time
        ...
        hour = mailConfig[emailPreference].hour + offsetInHours;
        if (hour < 0) {
            dayOfWeek = mailConfig.weekly.day - 1;
        } else if (hour >= 24) {
            dayOfWeek = mailConfig.weekly.day + 1;
        } else {
        ...
        // Ensure that we're dealing with valid 24-hour values
        hour = hour % 24;
    }

    return _createEmailBucketId(bucketNumber, emailPreference, dayOfWeek, hour);
};
