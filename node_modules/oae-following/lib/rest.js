 ...
 */
OAE.tenantRouter.on('get', '/api/following/:userId/followers', function(
  req,
  res,
) {
  ...
  FollowingAPI.getFollowers(
    ...
    req.query.start,
    limit,
    function(err, followers, nextToken) {
      if (err) {
        return res.status(err.code).send(err.msg);
      }

      return res.status(200).send({ results: followers, nextToken: nextToken });
    },
  );
});
 ...
 */
OAE.tenantRouter.on('get', '/api/following/:userId/following', function(
  req,
  res,
) {
  var limit = OaeUtil.getNumberParam(req.query.limit, 10, 1, 25);
  FollowingAPI.getFollowing(
    ...
    req.query.start,
    limit,
    function(err, following, nextToken) {
      if (err) {
        return res.status(err.code).send(err.msg);
      }
 ...
 */
OAE.tenantRouter.on('post', '/api/following/:userId/follow', function(
  req,
  res,
) {
 ...
 */
OAE.tenantRouter.on('post', '/api/following/:userId/unfollow', function(
  req,
  res,
) {
