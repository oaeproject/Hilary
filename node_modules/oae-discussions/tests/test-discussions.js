    ...
    describe('Creating Discussions', function() {
        ...
        it('verify create discussion validation', function(callback) {
            ...
            TestsUtil.generateTestUsers(camAdminRestCtx, 1, function(err, user) {
                ...
                RestAPI.Discussions.createDiscussion(camAnonymousRestCtx, displayName, description, visibility, null, null, null, function(err, discussion) {
                    assert.ok(err);
                    assert.equal(err.code, 401);

                    // Verify cannot create discussion with null displayName
                    RestAPI.Discussions.createDiscussion(user.restContext, null, description, visibility, null, null, null, function(err, discussion) {
                        assert.ok(err);
                        assert.equal(err.code, 400);

                        // Verify cannot create discussion with invalid visibility
                        RestAPI.Discussions.createDiscussion(user.restContext, displayName, description, 'not-a-visibility', null, null, null, function(err, discussion) {
                            assert.ok(err);
                            assert.equal(err.code, 400);

                            // Verify with no visibility
                            RestAPI.Discussions.createDiscussion(user.restContext, displayName, description, null, null, null, null, function(err, discussion) {
                                assert.ok(err);
                                assert.equal(err.code, 400);

                                // Verify with an invalid manager id
                                RestAPI.Discussions.createDiscussion(user.restContext, displayName, description, visibility, ['not-an-id'], null, null, function(err, discussion) {
                                    assert.ok(err);
                                    assert.equal(err.code, 400);

                                    // Verify with multiple invalid manager ids
                                    RestAPI.Discussions.createDiscussion(user.restContext, displayName, description, visibility, ['not-an-id', 'another-one'], null, null, function(err, discussion) {
                                        assert.ok(err);
                                        assert.equal(err.code, 400);

                                        // Verify with an invalid member id
                                        RestAPI.Discussions.createDiscussion(user.restContext, displayName, description, visibility, null, ['not-an-id'], null, function(err, discussion) {
                                            assert.ok(err);
                                            assert.equal(err.code, 400);

                                            // Verify with multiple invalid member ids
                                            RestAPI.Discussions.createDiscussion(user.restContext, displayName, description, visibility, null, ['not-an-id', 'another-one'], null, function(err, discussion) {
                                                assert.ok(err);
                                                assert.equal(err.code, 400);

                                                // Sanity check a good discussion creation
                                                RestAPI.Discussions.createDiscussion(user.restContext, displayName, description, visibility, null, null, null, function(err, discussion) {
                                                    assert.ok(!err);
    ...
    describe('Updating Discussions', function() {
        ...
        it('verify update discussion validation', function(callback) {
            ...
            TestsUtil.generateTestUsers(camAdminRestCtx, 1, function(err, user) {
                ...
                RestAPI.Discussions.createDiscussion(user.restContext, displayName, description, visibility, null, null, null, function(err, createdDiscussion) {
                    assert.ok(!err);
    ...
    describe('Discussions Library', function() {
        ...
        it('verify validation when listing discussion library', function(callback) {
            TestsUtil.generateTestUsers(camAdminRestCtx, 1, function(err, user) {
                ...
                RestAPI.Discussions.getDiscussionsLibrary(user.restContext, 'not-a-valid-id', null, null, function(err, items) {
                    ...
                    RestAPI.Discussions.getDiscussionsLibrary(user.restContext, user.user.id, null, 'not-a-valid-limit', function(err, items) {
                        assert.ok(err);
                        assert.equal(err.code, 400);
        ...
        it('verify discussion share validation', function(callback) {
            ...
            TestsUtil.generateTestUsers(camAdminRestCtx, 2, function(err, user) {
                ...
                RestAPI.Discussions.createDiscussion(user1.restContext, displayName, description, visibility, null, null, null, function(err, discussion) {
                    assert.ok(!err);
        ...
        it('verify library auto-repair on duplicate items', function(callback) {
            TestsUtil.generateTestUsers(camAdminRestCtx, 1, function(err, user) {
                ...
                RestAPI.Discussions.createDiscussion(user.restContext, displayName, description, visibility, null, null, null, function(err, discussion1) {
                    assert.ok(!err);

                    RestAPI.Discussions.createDiscussion(user.restContext, displayName, description, visibility, null, null, null, function(err, discussion2) {
                        assert.ok(!err);
