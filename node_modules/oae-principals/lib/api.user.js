...
var exportData = module.exports.exportData = function(ctx, userId, exportType, callback) {
    ...
    PrincipalsDAO.getPrincipal(userId, function(err, principal) {
        ...
        var avatar = principal.picture;

        _exportContentData(ctx, userId, exportType, function(err, txtDiscussion, txtMeeting, txtLink, contentsSplited) {
            if (err) {
                return callback(err);
            }
            
            return callback(null, {'txtFile': txtFile, 'avatar': avatar, 'txtDiscussion': txtDiscussion, 'txtMeeting': txtMeeting, 'txtLink': txtLink, 'contentsSplited': contentsSplited});
        });
    });
};
...
var _exportContentData = function(ctx, userId, exportType, callback) {
    ...
    ContentAPI.getContentLibraryItems(ctx, userId, null, null, function(err, contents, nextToken) {
        if (err) {
            return callback(err);
        }
        ...
        _linkToTxt(ctx, contentsSplited.link, function(err, txtLink) { 
            if (err) {
                return callback(err);
            }

            // Convert collabdocs into txt file
            _collabdocToTxt(ctx, contentsSplited.collabdoc, function(err, txtCollabdoc) { 
                if (err) {
                    return callback(err);
                }

                // Get meetings library
                MeetingsAPI.Meetings.getMeetingsLibrary(ctx, userId, null, null, function(err, meetings, nextToken) {
                    if (err) {
                        return callback(err);
                    }
                    ...
                    _meetingToTxt(ctx, meetings, function(err, txtMeeting) {
                        if (err) {
                            return callback(err);
                        }

                        // Get discussions library
                        DiscussionsAPI.Discussions.getDiscussionsLibrary(ctx, userId, null, null, function(err, discussions, nextToken) {
                            if (err) {
                                return callback(err);
                            }
                            ...
                            // Convert meetings into txt file
                            _discussionToTxt(ctx, discussions, function(err, txtDiscussion) {
                                if (err) {
                                    return callback(err);
                                }

                                return callback(null, txtDiscussion, txtMeeting, txtLink, contentsSplited);
                            });
                        });
                    });
                });
    ...
    async.mapSeries(collabdocs, function(collabdoc, outerCB) {
        ...
        ContentAPI.getComments(ctx, collabdoc.id, null, null, function(err, messages) {
            ...
            ContentAPI.getRevision(ctx, collabdoc.id, collabdoc.latestRevisionId, function(err, revision) {
                ...
                txtCollabdoc += 'Name of the collabdoc : ' + collabdoc.displayName
                    + '\nPath of the collabdoc : ' + collabdoc.profilePath
                    + '\nVisibility of the collabdoc : ' + collabdoc.visibility
                ...
                _getMessages(txtCollabdoc, messages, function(txtCollabdocWithMessages) {
                    txtCollabdoc = txtCollabdocWithMessages;
                    outerCB(null);
                });
            });
        });
    }, function(err) {
        return callback(null, txtCollabdoc);
    });
};
...
var _linkToTxt = function(ctx, links, callback) {
    ...
    async.mapSeries(links, function(link, outerCB) {
        ContentAPI.getComments(ctx, link.id, null, null, function(err, messages) {
            ...
            txtLink += 'Name of the link : ' + link.displayName
                + '\nPath of the link : ' + link.profilePath
                + '\nLink : ' + link.link
                + '\nVisibility of the link : ' + link.visibility
                + '\nTenant name : ' + link.tenant.displayName
                + '\n\n';

            _getMessages(txtLink, messages, function(txtLinkWithMessages) {
                txtLink = txtLinkWithMessages;
                outerCB(null);
            });
        });
    }, function(err) {
        return callback(null, txtLink);
    });
};
...
var _meetingToTxt = function(ctx, meetings, callback) {
    ...
    async.mapSeries(meetings, function(meeting, outerCB) {
        MeetingsAPI.Meetings.getMessages(ctx, meeting.id, null, null, function(err, messages) {
            ...
            txtMeeting += 'Name of the meeting : ' + meeting.displayName
                + '\nPath of the meeting : ' + meeting.tenant.host + meeting.profilePath
                + '\nVisibility of the meeting : ' + meeting.visibility
                + '\nTenant name : ' + meeting.tenant.displayName
                + '\n\n';

            _getMessages(txtMeeting, messages, function(txtMeetingWithMessages) {
                txtMeeting = txtMeetingWithMessages;
                outerCB(null);
            });
        });
    }, function(err) {
        return callback(null, txtMeeting);
    });
};
...
var _discussionToTxt = function(ctx, discussions, callback) {
    ...
    async.mapSeries(discussions, function(discussion, outerCB) {
        DiscussionsAPI.Discussions.getMessages(ctx, discussion.id, null, null, function(err, messages) {
            ...
            txtDiscussion += 'Name of the discussion : ' + discussion.displayName
                    + '\nDescription of the discussion : ' + discussion.description
                    + '\nPath of the discussion : ' + discussion.tenant.host + discussion.profilePath
                    + '\nVisibility of the discussion : ' + discussion.visibility
                    + '\nTenant name : ' + discussion.tenant.displayName
                    + '\n';

            _getMessages(txtDiscussion, messages, function(txtDiscussionWithMessages) {
                txtDiscussion = txtDiscussionWithMessages;
                outerCB(null);
            });
        });
    }, function(err) {
        return callback(null, txtDiscussion);
    });
};
