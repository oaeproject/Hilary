...
var deleteAnnotation = module.exports.deleteAnnotation = function(ctx, revisionId, annotationId, callback){
    ...
    validator.check(revisionId, {'code': 400, 'msg': 'Need a valid revisionId'}).isResourceId();
    validator.check(annotationId, {'code': 400, 'msg': 'Need a valid annotationId'}).isResourceId();
...
var updateAnnotation = module.exports.updateAnnotation = function(ctx, revisionId, page, annotationId, data, callback){
    var validator = new Validator();
    validator.check(revisionId, {'code': 400, 'msg': 'Need a valid revisionId'}).isResourceId();
    validator.check(page, {'code': 400, 'msg': 'Need a valid page number'}).isShortString();
    validator.check(annotationId, {'code': 400, 'msg': 'Need a valid annotationId'}).isResourceId();
    validator.check(data, {'code': 400, 'msg': 'Need a annotation'}).isObject();
    validator.check(null, {'code': 401, 'msg': 'Only authenticated users can delete an annotation').isLoggedInUser(ctx);
    if (validator.hasErrors()) {
        return callback(validator.getFirstError());
    }
...
var getAnnotations = module.exports.getAnnotations = function(ctx, revisionId, page, callback){
     ...
     * the current page.
     */ 
    Cassandra.runQuery('SELECT ? .. ? FROM Annotations WHERE revisionId = ?', [page, nextPage, revisionId], function(err, rows){
        if (err) {
            return callback(err);
        }
        /**
