...
var init = (module.exports.init = function(config, callback) {
  ...
  exec(cmd, { timeout: config.timeout }, function(err, stdout, stderr) {
    ...
    if (err || (stderr && stderr.indexOf('Error') !== -1)) {
      ...
      errorMessage +=
        'Please run the command in your terminal of choice and ensure that:\n';
      errorMessage +=
        '    1.  The path to the soffice binary is configured properly.\n';
      errorMessage +=
      ...
      return callback({
        code: 500,
        msg:
          'The path for the office binary or temporary directory is misconfigured',
      });
    }
    ...
    fs.exists(pdfPath, function(exists) {
      if (!exists) {
        ...
        return callback({
          code: 500,
          msg:
            'Unable to convert the office file to pdf. Office failed silently.',
        });
      }
