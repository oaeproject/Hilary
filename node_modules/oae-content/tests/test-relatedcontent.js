...
describe('Related content', function() {
    ...
    var createContentForUser = function(restContext, tenant, callback) {
        ...
        RestAPI.Content.createLink(restContext, TITLES[0], null, 'public', 'http://www.oaeproject.org', [], [], function(err, contentObj) {
            assert.ok(!err);
            contentItems.BY_TENANT[tenant].PUBLIC.push(contentObj.id);

            RestAPI.Content.createLink(restContext, TITLES[1], null, 'loggedin', 'http://www.oaeproject.org', [], [], function(err, contentObj) {
                assert.ok(!err);
                contentItems.BY_TENANT[tenant].LOGGEDIN.push(contentObj.id);

                RestAPI.Content.createLink(restContext, TITLES[2], null, 'private', 'http://www.oaeproject.org', [], [], function(err, contentObj) {
                    assert.ok(!err);
                    contentItems.BY_TENANT[tenant].PRIVATE.push(contentObj.id);

                    RestAPI.Content.createLink(restContext, TITLES[3], null, 'public', 'http://www.oaeproject.org', [], [], function(err, contentObj) {
                        assert.ok(!err);
                        contentItems.BY_TENANT[tenant].PUBLIC.push(contentObj.id);

                        RestAPI.Content.createLink(restContext, TITLES[4], null, 'loggedin', 'http://www.oaeproject.org', [], [], function(err, contentObj) {
                            assert.ok(!err);
                            contentItems.BY_TENANT[tenant].LOGGEDIN.push(contentObj.id);

                            RestAPI.Content.createLink(restContext, TITLES[5], null, 'private', 'http://www.oaeproject.org', [], [], function(err, contentObj) {
                                assert.ok(!err);
                                contentItems.BY_TENANT[tenant].PRIVATE.push(contentObj.id);
                                callback();
                            });
    ...
    /**
     * Test that verifies only valid content ids return results
     */
    it('verify the content id gets validated', function(callback) {
        SearchTestsUtil.searchAll(camAdminRestContext, 'relatedcontent', [''], null, function(err, results) {
            assert.equal(err.code, 400);
            assert.ok(!results);

            SearchTestsUtil.searchAll(camAdminRestContext, 'relatedcontent', ['invalid-content-id'], null, function(err, results) {
                assert.equal(err.code, 400);
                assert.ok(!results);

                SearchTestsUtil.searchAll(camAdminRestContext, 'relatedcontent', ['u:cam:bleh'], null, function(err, results) {
                    assert.equal(err.code, 400);
                    assert.ok(!results);
                    callback();
                });
    ...
    it('verify anonymous users can only see public content', function(callback) {
        ...
        RestAPI.Search.search(anonymousCamRestContext, 'relatedcontent', [contentItems.BY_TENANT.CAM.PUBLIC[0]], null, function(err, results) {
            ...
            RestAPI.Search.search(anonymousCamRestContext, 'relatedcontent', [contentItems.BY_TENANT.CAM.PUBLIC[0]], {'scope': '_network'}, function(err, results) {
                assert.ok(!err);
    ...
    it('verify logged in users can see both public and logged in content', function(callback) {
        ...
        RestAPI.Search.search(loggedinCamRestContexts[1], 'relatedcontent', [contentItems.BY_TENANT.CAM.PUBLIC[0]], null, function(err, results) {
            ...
            RestAPI.Search.search(loggedinCamRestContexts[1], 'relatedcontent', [contentItems.BY_TENANT.CAM.PUBLIC[0]], {'scope': '_network'}, function(err, results) {
                assert.ok(!err);
    ...
    it('verify limit handling', function(callback) {
        SearchTestsUtil.searchRefreshed(anonymousCamRestContext, 'relatedcontent', [contentItems.BY_TENANT.CAM.PUBLIC[0]], {'limit': 1, 'scope': '_network'}, function(err, data) {
            assert.ok(!err);
            ...
            SearchTestsUtil.searchRefreshed(anonymousCamRestContext, 'relatedcontent', [contentItems.BY_TENANT.CAM.PUBLIC[0]], {'limit': 2, 'scope': '_network'}, function(err, data) {
                assert.ok(!err);
                assert.equal(data.total, 7);
                ...
                SearchTestsUtil.searchRefreshed(anonymousCamRestContext, 'relatedcontent', [contentItems.BY_TENANT.CAM.PUBLIC[0]], {'limit': 'non-numeric', 'scope': '_network'}, function(err, data) {
                    assert.ok(!err);
                    assert.equal(data.total, 7);
                    assert.equal(data.results.length, 5);

                    SearchTestsUtil.searchRefreshed(anonymousCamRestContext, 'relatedcontent', [contentItems.BY_TENANT.CAM.PUBLIC[0]], {'scope': '_network'}, function(err, data) {
                        assert.ok(!err);
                        assert.equal(data.total, 7);
                        assert.equal(data.results.length, 5);
                        callback();
    ...
    /**
     * Verifies that data residing on private tenants cannot be found via related content searches.
     */
    it('verify tenant separation', function(callback) {
        // Create a private tenant.
        var privTenant = 'related-content-' + Math.random();
        TestsUtil.createTenantWithAdmin(privTenant, privTenant, function(err, privateTenant, privateTenantAdminRestCtx) {
            assert.ok(!err);
            ConfigTestUtil.updateConfigAndWait(globalAdminRestContext, privateTenant.alias, {'oae-tenants/tenantprivacy/tenantprivate': true}, function(err) {
                assert.ok(!err);

                // Create a piece of content that is similar to one in the cam tenant.
                RestAPI.Content.createLink(privateTenantAdminRestCtx, TITLES[0], null, 'public', 'http://www.oaeproject.org', [], [], function(err, privateContentObj) {
                    assert.ok(!err);

                    // Search for it in the cam tenant.
                    SearchTestsUtil.searchRefreshed(loggedinCamRestContexts[1], 'relatedcontent', [contentItems.BY_TENANT.CAM.PUBLIC[0]], {'scope': '_network'}, function(err, results) {
                        assert.ok(!err);
                        ...
                        RestAPI.Search.search(privateTenantAdminRestCtx, 'relatedcontent', [contentItems.BY_TENANT.CAM.PUBLIC[0]], {'scope': '_network'}, function(err, results) {
                            assert.ok(!err);
                            var docsFromCamTenant = _.filter(results.results, function(searchDoc) { return searchDoc.tenant.alias === global.oaeTests.tenants.cam.alias; });
