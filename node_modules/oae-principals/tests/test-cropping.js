...
describe('Users', function() {
    ...
    before(function(callback) {
        ...
        // Depending on whether we're running in coverage mode or not, files might be stored somewhere else.
        // Get the root files dir from the config so we can use it to verify if old images are removed.
        RestAPI.Config.getTenantConfig(camAdminRestContext, null, function(err, data) {
            assert.ok(!err);
            rootFilesDir = data['oae-content']['storage']['local-dir'];
            callback();
        });
    });
    ...
    var verifyCropping = function(ctx, principalId, selectedArea, expectedHttpCode, callback) {
        RestAPI.Crop.cropPicture(ctx, principalId, selectedArea, function(err, data) {
            ...
            verifySize(data.smallPictureUri, PrincipalsConstants.picture.size.SMALL, PrincipalsConstants.picture.size.SMALL, function() {
                verifySize(data.mediumPictureUri, PrincipalsConstants.picture.size.MEDIUM, PrincipalsConstants.picture.size.MEDIUM, function() {
                    callback();
                });
            });
        });
    ...
    describe('Profile picture', function() {
                            ...
                            RestAPI.User.getUser(anonymousRestContext, contexts['simon'].user.id, function(err, user) {
                                ...
                                RestAPI.User.updateUser(contexts['simon'].restContext, contexts['simon'].user.id, {'visibility': 'loggedin'}, function(err) {
                                    ...
                                    RestAPI.User.getUser(contexts['nicolaas'].restContext, contexts['simon'].user.id, function(err, user) {
                                        ...
                                        // The signed large picture is only exposed to the owner
                                        assert.equal(user.largePicture, undefined);

                                        // The user who owns the pictures can see verything
        ...
        /*!
         * Verify that when you upload/crop a picture, the old profile pictures get removed.
         */
        it('verify that old pictures are removed on updates', function(callback) {
            createUser(function(ctx) {
                var selectedArea = createSelectedArea(10, 10, 200, 200);
                RestAPI.User.uploadPicture(ctx, ctx.user.id, getPictureStream, selectedArea, function(err) {
                    assert.ok(!err);
        ...
        var verifySearchThumbnails = function(restContext, groupId, canPublic, canLoggedIn, canPrivate, publicUserId, loggedinUserId, privateUserId, callback) {
            RestAPI.Search.search(restContext, 'members', [groupId], null, function(err, results) {
                assert.ok(!err);
                                ...
                                RestAPI.Group.updateGroup(simon.restContext, uiTeam.id, { 'visibility': 'loggedin'}, function(err) {
                                    ...
                                    RestAPI.Group.updateGroup(simon.restContext, qaTeam.id, { 'visibility': 'private'}, function(err) {
                                        ...
                                        RestAPI.Group.setGroupMembers(simon.restContext, oaeTeam.id, changes, function(err) {
                                            ...
                                            RestAPI.Search.search(simon.restContext, 'members', [ oaeTeam.id ], { 'q': 'Group' }, function(err, results) {
                                                assert.ok(!err);
                                                assert.equal(results.total, 3);
