 ...
 */
OAE.globalAdminServer.post('/api/search/reindexAll', function(req, res) {
 ...
 */
OAE.globalAdminServer.get(/\/api\/search\/([^\/]+)(\/.*)?/, function(req, res) {
    var searchType = req.params[0];
    if (searchType) {
        req.telemetryUrl = '/api/search/' + searchType;
    }

    var tenantAlias = req.body.tenantAlias;
    if (tenantAlias) {
        req.ctx = new Context(TenantsAPI.getTenant(tenantAlias), req.ctx.user());
    }
