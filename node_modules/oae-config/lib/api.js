...
Pubsub.on('oae-config', function(receivedMessage) {
    ...
    if (tenantAlias === config.servers.globalAdminAlias) {
        cacheAllTenantConfigs();
    } else {
        cacheTenantConfig(tenantAlias);
    }
});
...
module.exports.config = function(moduleId) {
    ...
    return {
        /**
         * Get a configuration value for the current module. This will return a cached value.
         * 
         ...
         * @return  {Boolean|String|Number|Object}    cachedConfiguration   The requested config value. This will be null if the config element cannot be found. e.g. `true`
         */
        getConfigValue: function(ctx, feature, element) {
            // Parameter validation
            var validator = new Validator();
...
/**
 * Get the full cached config schema
 * 
 * @return {Object}     JSON object representing the full cached config schema
 */
var getSchema = module.exports.getSchema = function(callback) {
    return cachedSchema;
};

/**
 * Get the full config feed for a tenant from cache, containing all the config values for that tenant
 * 
 * @return {Object}     JSON object representing the full cached tenant config
 */
var getTenantConfig = module.exports.getTenantConfig = function(tenantAlias, callback) {
    return cachedConfig[tenantAlias];
};
...
var cacheSchema = function(callback) {
    ...
    var done = 0;
    // Reset the schema that's already set
    cachedSchema = {};    
...
var cacheAllTenantConfigs = function(callback) {
    ...
    TenantsAPI.getAllTenants(function(err, tenants) {
        tenantsToCache.concat(_.keys(tenants));
...
var cacheTenantConfig = function(tenantAlias, callback) {
    ...
    Cassandra.runQuery('SELECT * FROM Config USING CONSISTENCY QUORUM WHERE tenantAlias = ?', [tenantAlias], function(err, rows) {
        ...
        for (var module in cachedSchema) {
            cachedConfig[tenantAlias][module] = {};
            // Get all of the top level properties of the modules. Only the ones that are
            // object are configuration properties
            for (var feature in cachedSchema[module]) {
                if (_.isObject(cachedSchema[module][feature])) {
                    ...
                    for (var element in cachedSchema[module][feature].elements) {
                        ...
                        if (tenantConfig[key]) {
                            cachedConfig[tenantAlias][module][feature][element] = tenantConfig[key];
                        }
                    }
                }
            }
