 ...
 * @return {String}         the link with the base url in it
 */
var ensureAbsoluteLink = module.exports.ensureAbsoluteLink = function(link, baseUrl) {
    ...
    // Similarly, if the base url is already at the front there is nothing we need to do
    } else if (link.indexOf(baseUrl) === 0) {
        return link;
 ...
 * @return {String}                 The html in which each link has to be absolute
 */
var ensureAbsoluteLinks = module.exports.ensureAbsoluteLinks = function(str, baseUrl) {
    return str.replace(/href="(.*?)"/gm, function(match, link) {
        var fullUrl = ensureAbsoluteLink(link, baseUrl);
        return 'href="' + fullUrl + '"';
    });
};
 ...
 *
 * @param  {Object}         The template utility that can be used to encode HTML, translate keys, etc
 * @param  {String}         The email preference for which to generate an email summary
 * @param  {Activity[]}     The activities for which to generate a summary
