    ...
    describe('Export personal data', () => {
        ...
        it('verify get personal data', function(callback) {
            ...
            TestsUtil.generateTestUsers(camAdminRestContext, 1, function(err, users, brecke) {
                ...
                brecke.restContext.tenant = () => { return brecke.user.tenant; }
                brecke.restContext.user = () => { return brecke.user; }
         ...
         */
        it('verify resources with same names', function(callback) {
            ...
            TestsUtil.generateTestUsers(camAdminRestContext, 1, function(err, users, brecke) {
                ...
                RestAPI.Discussions.createDiscussion(brecke.restContext, 'name', 'description', 'public', null, null, function(err, discussion) {
                    ...
                    RestAPI.Discussions.createDiscussion(brecke.restContext, 'name', 'description', 'public', null, null, function(err, secondDiscussion) {
                        assert.ok(!err);

                        // Create one new discussion with the same name
                        RestAPI.Discussions.createDiscussion(brecke.restContext, 'name', 'description', 'public', null, null, function(err, thirdDiscussion) {
                            assert.ok(!err);
                            ...
                            PrincipalsAPI.exportData(brecke.restContext, brecke.user.id, 'content', function(err, zip) {
                                ...
                                .then(function(zipDiscussion) {
                                    ...
                                    .then(function(zipDiscussion) {
                                        ...
                                        assert.equal(secondDiscussion.displayName, element[0]);

                                        // Verify the personal data on the zip file
                                        zip.file('discussion_data/'+ discussion.displayName +'(1).txt').async('string')
                                        .then(function(zipDiscussion) {
                                            var lines = zipDiscussion.split('\n');
                                            var element = [];
                                            ...
                                            // Verify the personal data on the zip file
                                            zip.file('discussion_data/'+ discussion.displayName +'(2).txt').async('string')
                                            .then(function(zipDiscussion) {
                                                var lines = zipDiscussion.split('\n');
                                                var element = [];
                                                ...
                                                assert.equal(thirdDiscussion.displayName, element[0]);
            ...
            ContentsTestUtil.createCollabDoc(camAdminRestContext, 1, 1, function(collabdoc, users, brecke) {
                ...
                ContentsTestUtil.assertCreateLinkSucceeds(brecke.restContext, 'name', '', 'private', 'http://google.com', [], [], [], function(link) {
                    ...
                    RestAPI.User.uploadPicture(brecke.restContext, brecke.user.id, getPictureStream, cropArea, function(err) {
                        ...
                        // Get the object 
                        PrincipalsAPI.exportContentData(brecke.restContext, brecke.user.id, 'content', function(err, data) {
                            assert.ok(!err);
                                ...
                                .then(function(zipLink) {
                                    ...
                                    .then(function(zipCollabdoc) {
                                        ...
                                        .then(function(zipPicture) {
                                            ...
                                            assert.equal(zipCollabdoc, data.collabdocData[0].text);
                                            assert.equal(zipLink, data.linkData[0].text);
                    ...
                    PrincipalsAPI.exportContentData(brecke.restContext, brecke.user.id, 'content', function(err, data) {
                        ...
                        PrincipalsAPI.exportData(brecke.restContext, brecke.user.id, 'content', function(err, zip) {
                            ...
                            .then(function(zipDiscussion) {
                                ...
                                // Compare the object with the zip content
                                assert.equal(zipDiscussion, data.discussionData[0].text);
                    ...
                    PrincipalsAPI.exportContentData(brecke.restContext, brecke.user.id, 'content', function(err, data) {
                        ...
                        PrincipalsAPI.exportData(brecke.restContext, brecke.user.id, 'content', function(err, zip) {
                            ...
                            .then(function(zipMeeting) {
                                ...
                                // Compare the object with the zip content
                                assert.equal(zipMeeting, data.meetingData[0].text);
                        ...
                        RestAPI.Content.createComment(simon.restContext, collabdoc.id, 'Another comment', null, function(err, anotherComment) {
                            ...
                            PrincipalsAPI.exportData(brecke.restContext, brecke.user.id, 'content', function(err, zip) {
                                ...
                                .then(function(zipCollabdoc) {
                                    ...
                                    var messageCreatedComment = dateFormat(new Date(parseInt(comment.created)), 'dd-mm-yyyy, h:MM:ss TT');
                                    var messageCreatedAnotherComment = dateFormat(new Date(parseInt(comment.created)), 'dd-mm-yyyy, h:MM:ss TT');
                                    
                                    // Get the message level
                                    var levelComment = element[8][1].split(' ');
                                    var levelAnotherComment = element[7][1].split(' ');

                                    // Verify if the comment and the author of the comment are the same 
                                    assert.equal(element[8][3], comment.body);
                                    assert.ok(element[8][2].includes(comment.createdBy.publicAlias));
                                    assert.equal(levelComment[1], comment.level);
                                    assert.ok(element[8][0].includes(messageCreatedComment));

                                    // Verify if the comment and the author of the comment are the same 
                                    assert.equal(element[7][3], anotherComment.body);
                                    assert.ok(element[7][2].includes(anotherComment.createdBy.publicAlias));
                                    assert.equal(levelAnotherComment[1], anotherComment.level);
