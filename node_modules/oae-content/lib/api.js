...
var _createContent = function(ctx, contentId, revisionId, resourceSubType, displayName, description, visibility, additionalMembers, folders, otherValues, revisionData, callback) {
    ...
    _getExistingPrincipals(additionalMemberIds, null, function(err, principalsById) {
        if (err) {
            return callback(err);
        }

        var targets = {
            'principalRoles': _.map(additionalMembers, function(role, principalId) {
                return {
            ...
            canManageFolders(ctx, folders, function(err, folders) {
                ...
                ContentDAO.Content.createContent(contentId, revisionId, ctx.user().id, resourceSubType, displayName, description, visibility, otherValues, revisionData, function(err, contentObj, revision) {
                    ...
                    _setContentPermissions(ctx, contentObj, additionalMembers, function(err, contentObj) {
                        ...
                        var allMembers = _.chain(principalsById).values().union([ctx.user()]).value();
