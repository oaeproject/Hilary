...
var init = module.exports.init = function(config, callback) {
    ...
    PreviewOffice.init(config);
    PreviewPDF.init(config);
...
var enable = module.exports.enable = function(callback) {
    ...
    MQ.bind(ContentConstants.mq.GENERATE_PREVIEWS, options, _handleGeneratePreviewsTask, function(err) {
        if (err) {
            return log().error({'err': err}, 'Could not bind too the previews queue.');
            callback(err);
        }
...
var disable = module.exports.disable = function(callback) {
    ...
    MQ.unbind(ContentConstants.mq.GENERATE_PREVIEWS, function(err) {
        if (err) {
            return log().error({'err': err}, 'Could not unbind from the previews queue.');
            callback(err);
        }
...
var unregisterProcessor = module.exports.unregisterProcessor = function(processorId) {
    ...
    var contentTypes = Object.keys(_processors);
    for (var i = 0; i < contentTypes.length; i++) {
        var mimeTypes = Object.keys(_processors[contentTypes[i]]);
        for (var ii = 0; ii < mimeTypes.length; ii++) {
            var processors = _processors[contentTypes[i]][mimeTypes[ii]];
            var newProcessors = [];
            for (var iii = 0; iii < processors.length; iii++) {
                if (processors[iii].id !== processorId) {
                    newProcessors.push(processors[iii]);
                }
            }
            _processors[contentTypes[i]][mimeTypes[ii]] = newProcessors;
        }
    }
};
...
var _handleGeneratePreviewsTask = function(data, callback) {
    ...
    if (!data.id) {
        log().error({ 'err': data }, 'An invalid task was submitted to the generate previews task queue.');
...
var generatePreviews = module.exports.generatePreviews = function(contentId, callback) {
    log().info('%s - Starting preview generation process.', contentId);
    ...
    var cleanCallback = function(err) {
        ctx.cleanup();
        if (err) {
            log().error({'err': err}, 'Error when trying to process %s', contentId);
            // If the process fails, we set the status to 'error'.
            ctx.setStatus(ContentConstants.previews.ERROR, callback);
        } else {
            // We're done.
            log().info('%s - Preview processing done', contentId);
            callback();
        }
    };
    ...
    ctx.login(function(err) {
        ...
        _generatePreviews(ctx, function(err) {
            if (err) {
                log().error({'err': err}, 'Error trying to generate previews');
...
var _generatePreviews = function(ctx, callback) {
    ...
    RestAPI.Content.getContent(ctx.tenantRestContext, ctx.contentId, function(err, content) {
        if (err) {
            log().error({'err': err}, 'Could not get the content profile.');
            return callback(err);
        }
        ...
        var mimeType = content.mime || 'application/octet-stream';
        var processor = getProcessor(content.contentType, mimeType);
        if (processor) {
            processor(ctx, callback);
        } else {
            log().info('Ignoring %s as the type %s is not supported for now.', ctx.contentId, content.contentType);
            callback(null);
        }
    });
};
