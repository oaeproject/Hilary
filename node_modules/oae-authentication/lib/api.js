...
ConfigAPI.on('cleared', _configUpdate);
...
var _getOrCreateUser = function(ctx, loginId, displayName, opts, callback) {
    ...
    _getUserIdFromLoginId(loginId, function(err, userId) {
        if (err && err.code !== 404) {
            ...
            // If we got an email address, we try to look up the user by their email address
            if (opts.email) {
                PrincipalsDAO.getUserByEmail(opts.email, function(err, user) {
                    if (err && err.code !== 404) {
                        return callback(err);

                    // The email was not in the database, create the user account
                    } else if (err && err.code === 404) {
                        return _createAndReturn();
                    }

                    // At this point, we found a user with the given email address.
                    // If that user belongs to a different tenant, we bail out
                    if (user.tenant.alias !== ctx.tenant().alias) {
                        return callback({'code': 401, 'msg': 'The email address matches that of a user on a different tenant'});
                    }

                    // If this user is trying to create an account from an authentication
                    // source we don't trust, we bail out of the user creation process
                    else if (opts.emailVerified !== true) {
                        return callback({'code': 400, 'msg': 'This email address is already used by another user'});

                    // If we do trust the source however, we associate the new login Id with
                    // the retrieved user account
                    } else {
                        log().trace({
                            'userId': user.id,
                            'email': opts.email,
                            'loginId': loginId
                        }, 'Associating a user through their email address');
                ...
                }, 'Creating a user without an e-mail address');
                return _createAndReturn();
            }
        } else {
...
var _associateLoginId = function(loginId, userId, callback) {
    ...
    log().trace({'loginId': loginId, 'userId': userId}, 'Associating a login id to a user');
