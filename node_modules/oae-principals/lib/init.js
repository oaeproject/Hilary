...
var _ensureSchema = function(callback) {
    ...
    Cassandra.createColumnFamily('Principals', 'CREATE TABLE "Principals" ("principalId" text PRIMARY KEY, "tenantAlias" text, "displayName" text, "description" text, "email" text, "visibility" text, "joinable" text, "lastModified" text, "locale" text, "timezone" text, "publicAlias" text, "largePictureUri" text, "mediumPictureUri" text, "smallPictureUri" text, "admin:global" text, "admin:tenant" text, "notificationsUnread" text, "notificationsLastRead" text, "acceptedTC" text)', callback);
};

/**
 * Ensure that the default global administrative user exists with username "username", and create
 * them if they do not
 *
...
var _ensureGlobalAdmin = function(config, callback) {
    // Mock a global admin request context so we can create a proper global administrator in the
    // system
    ...
    AuthenticationAPI.getOrCreateGlobalAdminUser(globalContext, 'administrator', 'administrator', 'Global Administrator', callback);
};
