...
var resetAggregationForActivityStreams = module.exports.resetAggregationForActivityStreams = function(activityStreamIds, callback) {
    ...
    ActivityDAO.resetAggregationForActivityStreams(activityStreamIds, function(err) {
        if (err) {
            log().error({'err': err, 'activityStreamIds': activityStreamIds}, 'Failed to reset aggregation for activity streams');
            return callback(err);
        }
...
var collectAllBuckets = module.exports.collectAllBuckets = function(callback) {
    ...
    ActivityBuckets.collectAllBuckets('activity', numberOfBuckets, maxConcurrentCollections, collectionExpiry, _collectBucket, callback);
};
...
var _collectBucket = function(bucketNumber, callback) {
    ...
    var limit = ActivitySystemConfig.getConfig().collectionBatchSize;
    ...
    ActivityDAO.getQueuedActivities(bucketNumber, limit, function(err, routedActivities) {
        ...
        var allAggregates = createAggregates(_.values(routedActivities));
        ...
        ActivityDAO.getAggregateStatus(allAggregateKeys, function(err, statusByAggregateKey) {
            ...
            _.each(allAggregateKeys, function(aggregateKey) {
            ...
            ActivityDAO.deleteAggregateData(_.keys(expiredAggregates), function(err) {
                if (err) {
                    return callback(err);
                }
                ...
                ActivityDAO.getAggregatedEntities(_.keys(activeAggregates), function(err, fetchedEntities) {
                    ...
                    // Keeps track of what activities have already been "claimed" by an aggregate so that an activity doesn't get
                    // delivered twice
                    ...
                    _.each(allAggregateKeys, function(aggregateKey) {
                        var aggregate = allAggregates[aggregateKey];
                        var isMultiAggregateInQueue = (aggregate.activityIds.length > 1);
                        var isMultiAggregateInFeed = (fetchedEntities[aggregateKey] && !_isSingleAggregate(fetchedEntities[aggregateKey]));

                        // If the activity is a multi-aggregate in the activity feed already, mark all routed activities to
                        // indicate that they have aggregated. This is used by consumers of the push API to know if an activity
                        // has incremented the overall count of notifications
                        if (isMultiAggregateInFeed) {
                            _.each(aggregate.activityIds, function(activityId) {
                                if (routedActivities[activityId]) {
                                    routedActivities[activityId].aggregated = true;
                                }
                            });
                        }

                        // If the activity is a multi-aggregate within the routed activity queue, then all but the first of them are
                        // considered to have merged with an aggregate
                        if (isMultiAggregateInQueue) {
                            _.each(aggregate.activityIds.slice(1), function(activityId) {
                                if (routedActivities[activityId]) {
                                    routedActivities[activityId].aggregated = true;
                                }
                            });
                        ...
                        if (isMultiAggregateInQueue || isMultiAggregateInFeed) {
                            ...
                            if (status && status.lastActivity) {
                                ...
                                // Indicate that a routed activity associated to this aggregate
                            } else {
                                // If this aggregate is not deleting a previous aggregate, it counts as one new
                    ...
                    _.each(allAggregateKeys, function(aggregateKey) {
                        ...
                        var isClaimed = (claimedRouteActivities[aggregate.route] && claimedRouteActivities[aggregate.route][activityId]);
                        if (!isClaimed && activeAggregates[aggregateKey]) {
                            ...
                            if (status && status.lastActivity) {
                                ...
                                // Indicate that this activity aggregated with another that already existed in the feed
                                if (routedActivities[activityId]) {
                                    routedActivities[activityId].aggregate = true;
                                }
                            } else {
                                // If this aggregate is not deleting a previous aggregate, it counts as one new
                                // activity in the feed
                    ...
                    _.each(allAggregateKeys, function(aggregateKey) {
                        ...
                        if (!isClaimed) {
                            ...
                            // Each new non-aggregating activity counts as a new activity in the route
                            numNewActivitiesByRoute[aggregate.route] = numNewActivitiesByRoute[aggregate.route] || 0;
                            numNewActivitiesByRoute[aggregate.route]++;
                        }
                    });
                    ...
                    ActivityDAO.saveAggregatedEntities(allAggregates, function(err) {
                        ...
                        _.chain(aggregatesToDeliver).keys().each(function(aggregateKey) {
                            var aggregate = allAggregates[aggregateKey];
                            ...
                            var actor = createActivityEntity(_.values(aggregate.actors));
                            var object = createActivityEntity(_.values(aggregate.objects));
                            var target = createActivityEntity(_.values(aggregate.targets));
                        ...
                        ActivityDAO.deliverActivities(activityStreamUpdates, function(err) {
                            ...
                            _.each(activityStreamUpdates, function(routedActivities, route) {
                                _.each(routedActivities, function(activity, activityId) {
                            ...
                            _.chain(activitiesToDelete).keys().each(function(route) {
                            ...
                            ActivityDAO.deleteActivities(activitiesToDelete, function(err) {
                                ...
                                var statusUpdatesByActivityStreamId = {};
                                _.each(allAggregateKeys, function(aggregateKey) {
                                    var aggregate = allAggregates[aggregateKey];
                                    statusUpdatesByActivityStreamId[aggregate.route] = statusUpdatesByActivityStreamId[aggregate.route] || {};
                                    ...
                                    if (aggregate[ActivityConstants.properties.OAE_ACTIVITY_ID]) {
                                        statusUpdatesByActivityStreamId[aggregate.route][aggregateKey].lastActivity = aggregate[ActivityConstants.properties.OAE_ACTIVITY_ID];
                                    }
                                });

                                // Step #11: Update the activity statuses, indicating they have just been updated and collected, where applicable
                                ActivityDAO.indexAggregateData(statusUpdatesByActivityStreamId, function(err) {
                                    if (err) {
                                        return callback(err);
                                    }
                                    ...
                                    ActivityDAO.deleteQueuedActivities(bucketNumber, numCollected, function(err) {
                                        ...
                                        var deliveredActivityInfos = {};
                                        _.each(routedActivities, function(routedActivity) {
                                            var activityStreamIdParts = routedActivity.route.split('#');
                                            var resourceId = activityStreamIdParts[0];
                                            var streamType = activityStreamIdParts[1];
                                            deliveredActivityInfos[resourceId] = deliveredActivityInfos[resourceId] || {};
                                            deliveredActivityInfos[resourceId][streamType] = deliveredActivityInfos[resourceId][streamType] || {
 ...
 */
var createAggregates = module.exports.createAggregates = function(routedActivities) {
    var aggregates = {};
    _.each(routedActivities, function(routedActivity) {
        ...
        _.each(groupBy, function(pivot) {
            ...
            var hasNew = _contributesNewEntity(aggregate, actorKey, objectKey, targetKey);
 ...
 * @api private
 */
var _contributesNewEntity = function(aggregate, actorKey, objectKey, targetKey) {
 ...
 * @api private
 */
var _isSingleAggregate = function(aggregate) {
    return (_.keys(aggregate.actors).length <= 1 && _.keys(aggregate.objects).length <= 1 && _.keys(aggregate.targets).length <= 1);
};
 ...
 */
var createActivityEntity = module.exports.createActivityEntity = function(entities) {
    if (!entities) {
        return undefined;
...
var _flattenActivity = function(aggregate) {
    ...
    var actorsKeys = _.chain(aggregate.actors)
        .values()
    ...
    var objectsKeys = _.chain(aggregate.objects)
        .values()
        .map(_createEntityKey)
        .value()
        .sort()
    ...
    var targetsKeys = _.chain(aggregate.targets)
        .values()
        .map(_createEntityKey)
        .value()
        .sort()
