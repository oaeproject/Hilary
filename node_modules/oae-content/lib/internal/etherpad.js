...
var refreshConfiguration = module.exports.refreshConfiguration = function(_etherpadConfig) {
    ...
    _.each(etherpadConfig.hosts, function(host) {
        // No need to make the URL any uglier then it already is.
        var externalPort = (host.externalPort === 80 || host.externalPort === 443) ? '' : ':' + host.externalPort;
        ...
        var etherpad = {
            ...
            'externalProtocol': host.externalProtocol,
...
var joinPad = module.exports.joinPad = function(ctx, contentId, padId, groupId, callback) {
    ...
    client.createAuthorIfNotExistsFor(args, function(err, author) {
        ...
        client.createSession(session, function(err, data) {
            ...
            // Construct the URL.
            var url = getPadUrl(contentId, padId, data.sessionID);
 ...
 */
var getPadUrl = module.exports.getPadUrl = function(contentId, padId, sessionId) {
    var server = _getServer(contentId);
    return Url.format({
        'protocol': server.externalProtocol,
        'hostname': server.externalHost,
        'port': server.externalPort,
        'pathname': '/p/' + padId,
