 ...
 */
var getTermsAndConditions = (module.exports.getTermsAndConditions = function(
  ctx,
  locale,
) {
  ...
  var termsAndConditions = PrincipalsConfig.getValue(
    ctx.tenant().alias,
    'termsAndConditions',
    'text',
  );
...
var acceptTermsAndConditions = (module.exports.acceptTermsAndConditions = function(
  ...
  var isEnabled = PrincipalsConfig.getValue(
    ctx.tenant().alias,
    'termsAndConditions',
    'enabled',
  );
  if (!isEnabled) {
    return callback({
      code: 400,
      msg:
        'The Terms and Conditions are not enabled, there is no need to accept them',
    });
  }
  ...
  validator
    .check(userId, { code: 400, msg: 'Invalid userId passed in' })
    .isUserId();
  validator
    .check(null, {
      code: 401,
  ...
  if (ctx.user().id === userId || ctx.user().isAdmin(userTenant)) {
    ...
    return callback({
      code: 401,
      msg:
        'You are not authorized to accept the Terms and Conditions on behalf of this user',
    });
  }
});
 ...
 */
var needsToAcceptTermsAndConditions = (module.exports.needsToAcceptTermsAndConditions = function(
  ctx,
) {
  ...
  var lastUpdated = PrincipalsConfig.getLastUpdated(
    ctx.tenant().alias,
    'termsAndConditions',
    'text',
  );
