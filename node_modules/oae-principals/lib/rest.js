...
var log = require('oae-logger').logger('principals');
...
module.exports = function(tenant) {
    log().info('Initializing principals rest endpoints for tenant %s', tenant.alias);
    ...
    tenant.server.post('/api/user/create', function(req, res) {
        ...
        var createUserEnabled = PrincipalsConfig.getValue(req.ctx.tenant().alias, 'internalaccountcreationenabled', 'enabled');

        if (req.ctx.user()) {
            // If creation of users is enabled and the user is an admin, create the user and don't bother with recaptcha.
            if (createUserEnabled && req.ctx.user().isAdmin(tenant.alias)) {
                return createUser();

            // If creation of users is disabled but the user is a global admin, create the user and don't bother with recaptcha.
            } else if (!createUserEnabled && req.ctx.user().isGlobalAdmin()) {
                return createUser();

            // If creation of users is disabled and the user is not a global admin, don't create the user as non-admins cannot create accounts.
            } else {
                return res.send(401);
            }
        }
        ...
        var needsReCaptcha = PrincipalsConfig.getValue(req.ctx.tenant().alias, 'recaptcha', 'enabled');
        ...
        recaptcha.checkAnswer(PrincipalsConfig.getValue(req.ctx.tenant().alias, 'recaptcha', 'private-key'), req.connection.remoteAddress, req.body.recaptchaChallenge, req.body.recaptchaResponse);
    });
    ...
    tenant.server.post('/api/user/:id/password', function(req, res) {
        req.telemetryUrl = '/api/user/id/passwords';
