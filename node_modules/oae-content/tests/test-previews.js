    ...
    it('verify preview download parameter validation', function(callback) {
        createPreviews(function(contexts, contentObj, previews) {
            ...
            RestAPI.Content.downloadPreviewItem(contexts['nicolaas'].restContext, contentObj.id, contentObj.latestRevisionId, previews.files[0].filename, previews.signature, function(err, body, response) {
                ...
                // Missing parameters
                RestAPI.Content.downloadPreviewItem(contexts['nicolaas'].restContext, contentObj.id, contentObj.latestRevisionId, previews.files[0].filename, {'signature': previews.signature.signature}, function(err, body, response) {
                    assert.equal(err.code, 401);
                    assert.ok(!body);

                    RestAPI.Content.downloadPreviewItem(contexts['nicolaas'].restContext, contentObj.id, contentObj.latestRevisionId, previews.files[0].filename, {'expires': previews.signature.expires}, function(err, body, response) {
                        assert.equal(err.code, 401);
                        assert.ok(!body);

                        // Wrong signature
                        RestAPI.Content.downloadPreviewItem(contexts['nicolaas'].restContext, contentObj.id,contentObj.latestRevisionId, previews.files[0].filename, {'signature': 'wrong', 'expires': previews.signature.expires}, function(err, body, response) {
                                ...
                                verifySignedUriDownload(bert.restContext, contentObjOnCamTenant.previews.thumbnailUrl, function() {
                                    ...
                                    RestAPI.Content.updateContent(contexts['nicolaas'].restContext, contentObj.id, {'displayName': randomText}, function(err, updatedContentObj) {
                                        ...
                                        SearchTestsUtil.searchAll(bert.restContext, 'general', null, {'resourceTypes': 'content', 'q': randomText}, function(err, results) {
                                            ...
                                            return verifySignedUriDownload(bert.restContext, doc.thumbnailUrl, callback);
                                        });
                                    });
                                });
                            });
    ...
    it('verify thumbnail and medium URLs are present on the revision object', function(callback) {
        createPreviews(function(contexts, contentObj, previews) {
            ...
            RestAPI.Content.getRevisions(contexts['nicolaas'].restContext, contentObj.id, null, null, function(err, revisions) {
                ...
                verifySignedUriDownload(contexts['nicolaas'].restContext, revisions.results[0].thumbnailUrl, function() {
                    verifySignedUriDownload(contexts['nicolaas'].restContext, revisions.results[0].mediumUrl, function() {
                        ...
                        RestAPI.Content.getRevision(contexts['nicolaas'].restContext, contentObj.id, revisions.results[0].revisionId, function(err, revision) {
                            ...
                            verifySignedUriDownload(contexts['nicolaas'].restContext, revision.thumbnailUrl, function() {
                                verifySignedUriDownload(contexts['nicolaas'].restContext, revision.mediumUrl, function() {
                                    ...
                                    RestAPI.Content.restoreRevision(contexts['nicolaas'].restContext, contentObj.id, revisions.results[0].revisionId, function(err, restoredRevisionObj) {
                                        assert.ok(!err);
