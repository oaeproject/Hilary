...
// Caches the global administration and guest tenant objects
var globalTenant = null;
var guestTenant = null;
...
/**
 * Get a tenant whose configured email domain matches the specified email domain. If there is none,
 * then the guest tenant is returned as it implicitly matches all email domains.
 *
 * @param  {String}     emailDomain     The email domain with which to lookup the tenant
 * @return {Tenant}                     The tenant that matches the domain, if any
 */
var getTenantByEmailDomain = module.exports.getTenantByEmailDomain = function(emailDomain) {
    var tenant = _.find(getTenants(), function(tenant) {
        ...
        return (tenant.emailDomain && _matchesEmailDomainSuffix(emailDomain.toLowerCase(), tenant.emailDomain));
    });

    // Default to the guest tenant
    tenant = tenant || guestTenant;
...
var _cacheTenants = function(callback) {
    ...
    Cassandra.runQuery('SELECT * FROM "Tenant"', false, function(err, rows) {
        ...
        guestTenant = new Tenant(serverConfig.guestTenantAlias, 'Guest tenant', serverConfig.guestTenantHost);
        ...
        _.chain(rows)
            ...
            // Include the global and guest tenants
            .union([globalTenant, guestTenant])
