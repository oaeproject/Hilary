...
describe('Group Push', function() {
  ...
  describe('Authorization', function() {
    ...
    it('verify signatures must be valid', function(callback) {
      TestsUtil.generateTestUsers(localAdminRestContext, 2, function(
        err,
        users,
        simong,
        branden,
        ...
        RestAPI.User.getMe(simong.restContext, function(err, simonFull) {
          ...
          ActivityTestsUtil.getFullySetupPushClient(data, function(client) {
            ...
            RestAPI.Group.createGroup(
              ...
              function(err, group) {
                assert.ok(!err);
                RestAPI.Group.getGroup(simong.restContext, group.id, function(
                  err,
                  group,
                ) {
                  assert.ok(!err);
        ...
        RestAPI.Group.createGroup(
          ...
          function(err, group) {
            ...
            RestAPI.Group.getGroup(simong.restContext, group.id, function(
              ...
              RestAPI.Group.getGroup(branden.restContext, group.id, function(
                err,
                group,
              ) {
                assert.ok(!err);
                assert.ok(!group.signature);
                RestAPI.Group.getGroup(anonymousRestContext, group.id, function(
                  err,
                  group,
                ) {
                  assert.ok(!err);
  ...
  describe('Notifications', function() {
    ...
    var setupFixture = function(callback) {
      TestsUtil.generateTestUsers(localAdminRestContext, 3, function(
        err,
        users,
        branden,
        simon,
        ...
        RestAPI.User.getMe(contexts['simon'].restContext, function(
          err,
          simonFull,
        ) {
          assert.ok(!err);
    ...
    it('verify updates trigger a push notification', function(callback) {
      setupFixture(function(contexts, group, client) {
        ...
        client.on('message', function(message) {
          if (
            message.resourceId === group.id &&
            message.streamType === 'activity'
          ) {
            ...
            assert.equal(
              message.activities[0].object.displayName,
              'Laaike whatevs',
            );
