...
LibraryAPI.Index.registerLibraryIndex(
  PrincipalsConstants.library.MEMBERS_INDEX_NAME,
  {
    pageResources: function(libraryId, start, limit, callback) {
      AuthzAPI.getAuthzMembers(libraryId, start, limit, function(
        err,
        memberEntries,
      ) {
        if (err) {
 ...
 */
var _updatePrincipalInMembersLibraries = function(
  ctx,
  updatedPrincipal,
  oldPrincipal,
) {
  ...
  AuthzAPI.getAllRolesForPrincipalAndResourceType(
    ...
    function(err, allRoles) {
      ...
      var entries = _.map(allRoles, function(groupRole) {
        return {
          id: groupRole.id,
          oldRank: _getMembersLibraryRank(
            groupRole.id,
            oldPrincipal,
            groupRole.role,
          ),
          newRank: _getMembersLibraryRank(
            groupRole.id,
            updatedPrincipal,
            groupRole.role,
          ),
      ...
      return LibraryAPI.Index.update(
        PrincipalsConstants.library.MEMBERS_INDEX_NAME,
        entries,
      );
    },
 ...
 */
PrincipalsEmitter.on(
  PrincipalsConstants.events.UPDATED_USER,
  _updatePrincipalInMembersLibraries,
);
PrincipalsEmitter.on(
...
PrincipalsEmitter.when(PrincipalsConstants.events.JOINED_GROUP, function(
  ...
  LibraryAPI.Index.insert(
    ...
    function(err) {
      if (err) {
        log().error(
          { err: err, groupId: group.id, userId: ctx.user().id },
 ...
 */
PrincipalsEmitter.on(PrincipalsConstants.events.LEFT_GROUP, function(
  ctx,
  group,
  memberChangeInfo,
) {
  ...
  LibraryAPI.Index.remove(PrincipalsConstants.library.MEMBERS_INDEX_NAME, [
    entry,
  ]);
});
 ...
 * @api private
 */
var _handleInvalidateMembersLibrary = function(
  group,
  membershipsGraph,
  membersGraph,
  callback,
...
PrincipalsDelete.registerGroupDeleteHandler(
  'members-library',
  _handleInvalidateMembersLibrary,
);
