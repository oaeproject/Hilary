...
module.exports = function() {
    ...
    passport.use(new BearerStrategy(function(accessToken, callback) {
        OAuthDAO.AccessTokens.getAccessToken(accessToken, function(err, token) {
            ...
            var tenantAlias = AuthzUtil.getPrincipalFromId(token.userId).tenantAlias;
            var tenant = TenantsAPI.getTenant(tenantAlias);
            var ctx = new Context(tenant, new User(tenantAlias, token.userId));
            PrincipalsUtil.getPrincipal(ctx, token.userId, function (err, user) {
                if (err && err.code === 404) {
                    return callback(null, false);
                } else if (err) {
                    return callback(err);
