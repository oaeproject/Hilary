...
var roleModel = require('oae-roles/lib/model');
...
module.exports.createUser = function(context, username, password, visibility, locale, timezone, firstName, lastName, displayName, callback) {
    ...
    var id = roleUtil.toUuid('u', context.tenant().alias, username);
    module.exports.getUser(context, id, function(err, user) {
        ...
        utils.hashPassword(password, function(hash) {
            ...
            // Create the group.
            cassandra.runQuery('INSERT INTO Principals (principal_id, tenant, visibility, locale, timezone, user_username, user_password, user_first_name, user_last_name, user_display_name) \
                    VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)', [id, context.tenant().alias, visibility, locale, timezone, username, hash, firstName, lastName, displayName], function (err) {
                if (err) {
...
module.exports.getTenantUser = function(context, username, callback) {
    var user_id = roleUtil.toUuid('u', context.tenant().alias, username);
...
module.exports.isTenantAdmin = function(tenantAlias, user, callback) {
    var tenantAdminResource = new roleModel.Resource('r', tenantAlias, 'system');
    roleAPI.hasRole(user.id, tenantAdminResource.toUuid(), 'admin', callback);
};
...
module.exports.makeUserTenantAdmin = function(context, user, callback) {
    var tenantAdminResource = new roleModel.Resource('r', context.tenant().alias, 'system');
    roleAPI.setRole(user.id, tenantAdminResource.toUuid(), 'admin', callback)
};
...
module.exports.isGlobalAdmin = function(user, callback) {
    var globalAdminResource = new roleModel.Resource('r', 'global', 'system');
    roleAPI.hasRole(user.id, globalAdminResource.toUuid(), 'admin', callback);
};