...
describe('Search Util', function() {
    ...
    describe('#filterOr', function() {
        ...
        it('verify one param', function(callback) {
            ...
            assert.equal(filter.or[0].param, 'Arbitrary object should be returned');
            return callback();
        });
    ...
    describe('#filterAnd', function() {
        ...
        it('verify one param', function(callback) {
            ...
            assert.equal(filter.and[0].param, 'Arbitrary object should be returned');
            return callback();
        });
    ...
    describe('#filterMemberships', function() {

        /**
         * Test that verifies a single parameter filter object is returned in the filter
         */
        it('verify with a membership', function(callback) {
            var filter = SearchUtil.filterMemberships(['membership']);
            assert.ok(filter.has_child);

            assert.equal(filter.has_child.type, 'resource_memberships');
            assert.ok(filter.has_child.query);
            assert.ok(filter.has_child.query.terms);
            assert.equal(filter.has_child.query.terms['direct_memberships'].length, 1);
            assert.equal(filter.has_child.query.terms['direct_memberships'][0], 'membership');
            return callback();
        });
         ...
         * Test that verifies filter terms with unspecified terms array returns a falsey result
         */
        it('verify with unspecified memberships', function(callback) {
            assert.ok(!SearchUtil.filterMemberships());
            return callback();
        });
    });
