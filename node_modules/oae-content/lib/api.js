 ...
 * @param  {String}    visibility         The visibility setting for the piece of content. Possible values are "public", "loggedin" and "private" [optional]
 * @param  {Object}    additionalMembers  Object where the keys represent principal ids that need to be added to the content upon creation and the values represent the role that principal will have. Possible values are "viewer" and "manager"
 * @param  {Object}    file               A file object as returned by one of the storage backends their `store` method.
 * @param  {Function}  callback           Standard callback function takes arguments `err` and `content`
 * @param  {Object}    callback.err       Error object containing the error message
...
var createFile = module.exports.createFile = function(ctx, name, description, visibility, additionalMembers, file, callback) {
    callback = callback || function() {};
    _createContent(ctx, 'file', name, description, visibility, additionalMembers, {'file': file}, callback);
};
...
var _createContent = function(ctx, contentType, name, description, visibility, additionalMembers, otherValues, callback) {
    ...
    // Parameter validation
    var validationErrors = validateContentCreationParameters(ctx, contentType, name, description, visibility, additionalMembers, otherValues);
    if (validationErrors) {
    ...
    Cassandra.runQuery(q.query, q.parameters, function(err) {
        ...
        _setContentPermissions(ctx, contentObj, additionalMembers, function(err) {
            ...
            store(ctx, stream, headers, function(err, uri) {
                if (err) {
                    return callback(err);
                }

                // Create the revision.
                createRevision(ctx, contentObj.contentId, uri, file.filename, file.mime, function(err, revision) {
                    if (err) {
                        return callback(err);
                    }

                    // Call the callback function and return the content object.
 ...
 * @param  {Object}         additionalMembers  Object where the keys represent principal ids that need to be added to the content upon creation and the values represent the role that principal will have. Possible values are "viewer" and "manager"
 * @param  {Object}         otherValues        JSON object where the keys represent other metadata values that need to be stored, and the values represent the metadata values
 * @return {Object}                            An error object if the parameters don't pass validation or null if they do.
 */
var validateContentCreationParameters = module.exports.validateContentCreationParameters = function(ctx, contentType, name, description, visibility, additionalMembers, otherValues) {
    // Setting content to public if no privacy setting is provided
    visibility = visibility || ContentConstants.visibility.PUBLIC;
...
/**
 * @borrows Interface.store as Storage.store
 */
var store = module.exports.store = function(ctx, stream, headers, callback) {
    _getBackend(ctx).store(ctx, stream, headers, callback);
};

/**
 * @borrows Interface.remove as Storage.remove
 */
var remove = module.exports.remove = function(ctx, uri, callback) {
    _getBackend(ctx, uri).remove(ctx, uri, callback);
};

/**
 * @borrows Interface.getDownloadLink as Storage.getDownloadLink
 */
var getDownloadLink = module.exports.getDownloadLink = function(ctx, uri) {
    return _getBackend(ctx, uri).getDownloadLink(ctx, uri);
};
 ...
 * @param {String}      thingId             The ID of the thing this revision is for.
 * @param {String}      uri                 An URI for this revision
 * @param {String}      [filename]          An optional filename
 * @param {String}      [mimetype]          An optional mimetype
 * @param {Function}    [callback]          An optional callback method.
 * @param {Object}      callback.err        An error object (if any)
 * @param {Revision}    callback.revision   A revision object.
 */
var createRevision = module.exports.createRevision = function(ctx, thingId, uri, filename, mimetype, callback) {
    callback = callback || function() {};
    uri = uri || '';
    filename = filename || '';
    mimetype = mimetype || '';
    ...
    var validator = new Validator();
    validator.check(thingId, {'code': 400, 'msg': 'There must be a thing id associated to a revision'}).notEmpty();
    ...
    var revision = new Revision(thingId, Date.now(), ctx.user().id, uri, filename, mimetype);
    var values = {};
    values[revision.created] = _serializeRevision(revision);
    var q = Cassandra.constructUpsertCQL('Revisions', 'id', revision.thingId, values, 'QUORUM');
    Cassandra.runQuery(q.query, q.parameters, function(err) {
        if (err) {
            return callback(err);
        }
...
/**
 * Get the revisions that are associated to a 'thing' (piece of content, page, ..)
 * @param  {Context}        ctx                 Standard context object, representing the currently logged user and its tenant
 * @param  {String}         thingId             The id of the object for which we want to get the revisions
 ...
 */
var getRevisions = module.exports.getRevisions = function(ctx, thingId, start, limit, callback) {
    limit = limit || 10;

    var validator = new Validator();
    validator.check(thingId, {'code': 400, 'msg': 'A thing id must be provided'}).notEmpty();
    validator.check(limit, {'code': 400, 'msg': 'A valid limit should be passed in'}).isInt();
    if (validator.hasErrors()) {
        return callback(validator.getFirstError());
    }
    ...
    // Page the query.
    var paging = Cassandra.getPagingParameters(start, limit);
    Cassandra.runQuery('SELECT FIRST ' + paging.limit + ' REVERSED ?..\'\' FROM Revisions USING CONSISTENCY QUORUM WHERE id = ?', [paging.start, thingId], function (err, rows) {
        if (err) {
            return callback(err);
        }
        ...
        for (var c = paging.startIndex; c < rows[0].count; c++) {
            if (rows[0][c].name !== 'id') {
                var revision = _columnToRevision(thingId, rows[0][c]);
 ...
 */
var getLatestRevision = module.exports.getLatestRevision = function(ctx, thingId, callback) {
    getRevisions(ctx, thingId, null, 1, function(err, revisions) {
        if (err) {
            return callback(err);
        }

        if (revisions.length === 0) {
            return callback({'code': 404, 'msg': 'No revision found for ' + thingId});
        }
...
/**
 * Serializes a revision into a string.
 * @param  {Revision} revision The revision to serialize.
 * @return {String}            The serialized revision.
 */
var _serializeRevision = function(revision) {
    return util.format('%s#%s#%s#%s', revision.createdBy, revision.uri, revision.filename || '', revision.mimetype || '');
};

/**
 * Converts a Cassandra Column to a Revision object.
 * @param  {String}     thingId  The row key (`thingId`) for this column.
 * @param  {Column}     column   A Helenus Column
 * @return {Revision}            A revision object.
 */
var _columnToRevision = function(thingId, column) {
    var created = parseInt(column.name, 10);
    var parts = column.value.split('#');
    var createdBy = parts[0];
    var uri = parts[1];
    var filename = parts[2];
    var mimetype = parts[3];
    return new Revision(thingId, created, createdBy, uri, filename, mimetype);
};
