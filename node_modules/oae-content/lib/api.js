...
var createLink = module.exports.createLink = function(ctx, displayName, description, visibility, link, additionalMembers, callback) {
    ...
    _createContent(ctx, contentId, revisionId, 'link', displayName, description, visibility, additionalMembers, {'link': link}, {'link': link}, function(err, content, revision) {
        if (err) {
            return callback(err);
        }
        ...
        Etherpad.getHTML(contentObj.id, contentObj.etherpadPadId, function(err, html) {
            ...
            _getRevision(ctx, contentObj, contentObj.latestRevisionId, function(err, revision) {
                if (err) {
                    ...
                    // If there is no real update or this is a brand new doc (no etherpadHtml on the revision) and the user didn't enter
                    // anything, we can return early
                    contentObj.latestRevision = revision;
                    return callback(null, contentObj);
                }
...
var joinCollabDoc = module.exports.joinCollabDoc = function(ctx, contentId, callback) {
    ...
    canManage(ctx, contentId, function(err, canManage, contentObj) {
        ...
        // Join the pad
        Etherpad.joinPad(ctx, contentObj.id, contentObj.etherpadPadId, contentObj.etherpadGroupId, function(err, data) {
            if (err) {
                return callback(err);
            }
    ...
    canManage(ctx, contentId, function(err, isManager, contentObj) {
        ...
        ContentUtil.getStorageBackend(ctx).store(ctx, file, options, function(err, uri) {
            ...
            ContentDAO.Revisions.createRevision(revisionId, contentObj.id, ctx.user().id, opts, function(err, revision) {
                ...
                return _updateContentMetadata(ctx, contentObj, opts, revision, ContentConstants.events.UPDATED_CONTENT_BODY, callback);
            });
        });
    });
};
...
var updateContentMetadata = module.exports.updateContentMetadata = function(ctx, contentId, profileFields, callback) {
    ...
    canManage(ctx, contentId, function(err, canManage, oldContentObj) {
        ...
        // If we're updating a link, we also need to create a new revision
        if (profileFields.link && profileFields.link !== oldContentObj.link) {
            if (oldContentObj.resourceSubType !== 'link') {
                return callback({'code': 400, 'msg': 'This piece of content is not a link'});
            }

            // Create a revision for this new link
            var revisionId = _generateRevisionId(contentId);
            ContentDAO.Revisions.createRevision(revisionId, oldContentObj.id, ctx.user().id, {'link': profileFields.link}, function(err, revision) {
                if (err) {
                    return callback(err);
                }
                ...
                // Store the id of this new revision on the content record
                profileFields.latestRevisionId = revisionId;

                return _updateContentMetadata(ctx, oldContentObj, profileFields, revision, ContentConstants.events.UPDATED_CONTENT, callback);
            });

        // Plain content metadata update
        } else {
            return _updateContentMetadata(ctx, oldContentObj, profileFields, null, ContentConstants.events.UPDATED_CONTENT, callback);
        }
    });
};

/**
 * Update a content record in the database and return the new piece of content
 *
 * @param  {Context}    ctx                 Standard context object, representing the currently logged in user and its tenant
 * @param  {Content}    oldContentObj       The content object that will be updated
 * @param  {Object}     profileFields       The hash containing the content object fields that need updating and their new values
 * @param  {Revision}   [revision]          An optional revision that goes along with this update. If specified the `latestRevisionId` for the content item will be set to the `revisionId` value of this revision
 * @param  {String}     [type]              Type of content update. One of ContentConstants.events.UPDATED_CONTENT or ContentConstants.events.UPDATED_CONTENT_BODY. Defaults to UPDATED_CONTENT
 * @param  {Function}   callback            Standard callback function
 * @param  {Object}     callback.err        An error object, if any
 * @param  {Content}    callback.content    The new content object
 */
var _updateContentMetadata = function(ctx, oldContentObj, profileFields, revision, type, callback) {
    type = type || ContentConstants.events.UPDATED_CONTENT;
    if (revision) {
        profileFields.latestRevisionId = revision.revisionId;
    }
    ...
    ContentDAO.Content.updateContent(oldContentObj, profileFields, true, function(err, newContentObj) {
        ...
        ContentAPI.emit(type, ctx, newContentObj, oldContentObj, revision);

        // Output a full content profile
        return _getFullContentProfile(ctx, newContentObj, true, callback);
    });
};
