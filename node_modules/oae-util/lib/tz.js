 ...
 */
var getTimezoneFromRails = (module.exports.getTimezoneFromRails = function(
  zone,
) {
 ...
 */
var getClosestSupportedTimezone = (module.exports.getClosestSupportedTimezone = function(
  zone,
) {
  ...
  try {
    ...
    _.each(RailsMappings, function(value, key) {
      ...
      var mappingOffset = mappingDate.getTimezoneOffset();
    ...
    // No matching offset, find the zones with the closest offset and most similar DST rules
 ...
 * @api private
 */
var _getMostSimilarZone = function(orig, candidates) {
  if (candidates.length === 1) {
    return candidates[0];
  }

  var highScore = { score: 0, zones: [] };
  var origZoneInfo = _getRelevantZoneInfo(orig);
  _.each(candidates, function(candidate, index) {
    var candidateZoneInfo = _getRelevantZoneInfo(candidate);
    var score = 0;
    ...
    _.each(origZoneInfo.rules, function(rule, index) {
    ...
    if (score > highScore.score) {
      highScore = { score: score, zones: [candidate] };
 ...
 * @api private
 */
var _getRelevantZoneInfo = function(zone) {
  var zoneInfo = { zone: _.last(tz.timezone.zones[zone]) };
  zoneInfo.rules = _.filter(tz.timezone.rules[zoneInfo.zone[1]], function(
    rule,
  ) {
    return rule[1] === 'max';
  });

  _.each(zoneInfo.rules, function(rule, index) {
    // Cut out just the rule entries we care about
    var trimmedRule = rule.slice(2, 7);
