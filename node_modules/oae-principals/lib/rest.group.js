 ...
 * @PathParam   {string}        groupId                 The id of the group to store the large picture for
 * @PathParam   {string}        text                    The text written by the user who wants to join the group
 * @HttpResponse                200                     Email sent
 * @HttpResponse                400                     Fail to send an email
 */
OAE.tenantRouter.on('post', '/api/group/:groupId/request-join', function(req, res) {
    PrincipalsAPI.sendRequestJoinGroup(req.ctx, req.params.groupId, req.body.text, function(err) {
        if (err) {
            return res.status(err.code).send(err.msg);
        }
...
/**
 * @REST postGroupGroupIdRequestJoinPrincipalId
 *
 * Join a group
 *
 * @Server      tenant
 * @Method      POST
 * @Path        /group/{groupId}/request-join/{principalId}
 * @PathParam   {string}        groupId                 The id of the group to join
 ...
 * @HttpResponse                401                     You have to be logged in to be able to join a group
 */
OAE.tenantRouter.on('post', '/api/group/:groupId/request-join/:principalId', function(req, res) {
    console.log('ici')
    PrincipalsAPI.joinGroupByRequest(req.ctx, req.params.groupId, req.params.principalId, function(err) {
        if (err) {
            return res.status(err.code).send(err.msg);
        }
