...
var _setupTenantAdmin = function(tenant, callback) {
  ...
  AuthenticationAPI.createUser(ctx, adminLoginId, displayName, { email: email }, function(
    err,
    createdUser
  ) {
    assert.ok(!err);
...
var generateTestUsers = (module.exports.generateTestUsers = function(
  ...
  _ensureAuthenticated(restCtx, function(err) {
    ...
    RestAPI.Tenants.getTenant(restCtx, null, function(err, tenant) {
      ...
      RestAPI.User.createUser(restCtx, username, 'password', displayName, email, {}, function(
        err,
        user
      ) {
        if (err) {
...
var _createMultiPrivacyGroups = function(tenant, callback) {
  _createGroupWithVisibility(tenant, 'public', tenant.publicUser.user.id, function(publicGroup) {
    _createGroupWithVisibility(tenant, 'loggedin', tenant.loggedinUser.user.id, function(
      loggedinGroup
    ) {
      _createGroupWithVisibility(tenant, 'private', tenant.privateUser.user.id, function(
        privateGroup
      ) {
...
var _createPublicTenant = function(tenantAlias, hostSeed, callback) {
  createTenantWithAdmin(tenantAlias, TenantsTestUtil.generateTestTenantHost(hostSeed), function(
    err,
    tenant,
    tenantAdminRestCtx,
    tenantAdminUser
...
var _bindRequestLogger = function() {
  ...
  RestUtil.emitter.on('request', function(restCtx, url, method, data) {
    requestLog().trace(
      {
        restCtx: restCtx,
        url: url,
  ...
  RestUtil.emitter.on('response', function(body, res) {
    requestLog().trace({ res: res, body: body }, 'REST Request complete');
  });

  RestUtil.emitter.on('error', function(err, body, res) {
    requestLog().error(
      {
        err: err,
        res: res,
