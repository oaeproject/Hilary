...
var shareContent = module.exports.shareContent = function(ctx, contentId, principalIds, callback) {
    ...
    getContent(ctx, contentId, function(err, contentObj) {
        ...
        _canManage(ctx, contentId, function(err, canManage) {
            if (canManage || contentObj.visibility === ContentConstants.permissions.LOGGEDIN || contentObj.visibility === ContentConstants.permissions.PUBLIC) {
                ...
                _getContentMembers(contentId, function(err, members) {
                    ...
                    for (var p = 0; p < principalIds.length; p++) {
                        shareObject[p] = ContentConstants.roles.MEMBER;
                    }
                    _setContentPermissions(contentObj, shareObject, function(err) {
                        if (err) {
                            callback(err);
                        }
                        callback();
                ...
                callback({'code': 403, 'msg': 'You are not allowed to share this content'});
            }
        });
    });
};
 ...
 *                                                          an empty string is passed in, the permissions for that principal will be removed
 *                                                          e.g. {'user1': 'manager', 'user2': 'member', 'user3': null}
 * @param  {Function(err, canManage)}   callback            Standard callback function
 * @param  {Object}                     callback.err        Error object containing the error message
 * @param  {Boolean}                    callback.canManage  Whether or not the user has access to the content
 */
var setContentPermissions = module.exports.setContentPermissions = function(ctx, contentId, newPermissions, callback) {
    ...
    var validator = new Validator();
    validator.check(contentId, {'code': 400, 'msg': 'A content id must be provided'}).notEmpty();
    validator.check(_.keys(newPermissions).length, {'code': 403, 'msg': 'You should specify at least 1 user/group to set content permissions on'}).min(1);
    ...
    getContent(ctx, contentId, function(err, contentObj) {
        ...
        _canManage(ctx, contentId, function(err, canManage) {
            ...
            var todo = _.keys(newPermissions).length; var done = 0;
        ...
        for (var member in members) {
            if (members.hasOwnProperty(member)) {
                ...
                if (oldLastModified) {
                    ...
                    queries.push({
                        'query': 'DELETE ? FROM LibraryByPrinciple WHERE principalId = ?',
                        'parameters': [oldLastModified + ':' + contentObj.contentId, member + ':loggedin']
                    });
                    // Remove the old entry in the public row
                    queries.push({
                        'query': 'DELETE ? FROM LibraryByPrinciple WHERE principalId = ?',
                        'parameters': [oldLastModified + ':' + contentObj.contentId, member + ':public']
                    });
                }
                ...
                if (contentObj.visibility === ContentConstants.permissions.LOGGEDIN || contentObj.visibility === ContentConstants.permissions.PUBLIC) {
                    queries.push({
                        'query': 'UPDATE LibraryByPrinciple SET ? = ? WHERE principalId = ?',
                        'parameters': [contentObj.lastModified + ':' + contentObj.contentId, contentObj.contentId, member + ':loggedin']
                    });
                }
                // Add it back to the public row
                if (contentObj.visibility === ContentConstants.permissions.PUBLIC) {
                    queries.push({
                        'query': 'UPDATE LibraryByPrinciple SET ? = ? WHERE principalId = ?',
                        'parameters': [contentObj.lastModified + ':' + contentObj.contentId, contentObj.contentId, member + ':public']
                    });
                }
            }
        };
