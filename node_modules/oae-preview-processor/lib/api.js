...
/**
 * Submits a task that triggers all erroneous preview items to be regenerated.
 *
 * @param  {Context}    ctx             The context of the request to reprocess the previews
 * @param  {String[]}   [statuses]      An array of statuses of previews that indicate it needs to be regenerated (Default: `['error']`)
...
var _generatePreviews = function(ctx, callback) {
    ...
    RestAPI.Content.getContent(ctx.tenantRestContext, ctx.contentId, function(err, content) {
        ...
        if (processor) {
            ...
            log().info({'contentId': ctx.contentId, 'contentType': content.contentType, 'mimeType': mimeType}, 'Ignoring as this type of content is not supported for now.');
...
var _handleRegeneratePreviewsTask = function(data, callback) {
    ...
    ContentDAO.iterateAll(['contentId', 'previews'], 1000, function(err, contentObjs, complete) {
        if (err) {
        ...
        log().info('Scanning %s content items to see if previews need to be reprocessed.', contentObjs.length);
        totalScanned += contentObjs.length;
        ...
        _.each(contentObjs, function(contentObj) {
            ...
            if (contentObj.previews) {
                try {
                    previews = JSON.parse(contentObj.previews);
                } catch (ex) {
                    ...
                    log().warn({'contentObj': contentObj}, 'Found invalid JSON for content item. Forcing regeneration of previews.');
                    reprocess = true;
                }
                ...
                log().warn({'contentId': contentObj.contentId}, 'Found no previews object for content item. Forcing regeneration of previews.');
                reprocess = true;
            }

            if (reprocess) {
                contentIdsToBeReprocessed.push(contentObj.contentId);
            }
        });
