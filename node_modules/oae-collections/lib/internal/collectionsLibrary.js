 ...
 * @param  {Number}         [oldLastModified]       The previous collection last modified time. If this is unspecified, this function will treat the last modified time of the provided collection as the "old last modified", and will update the collection's last modified time itself
 * @param  {Function}       callback                Standard callback function
 * @param  {Object}         callback.err            An error that occurred, if any
 * @param  {Collection}     callback.collection     The most recent version of the collection. As this function may conditionally update the last modified timestamp of the collection, this parameter will contain the updated version if it did. If it did not update the timestamp, this parameter will be the same collection that was provided in the `collection` parameter
 */
var update = module.exports.update = function(principalIds, collection, oldLastModified, callback) {
    ...
    if (!_.isNumber(oldLastModified)) {
        // If no old last modified date was provided, we touch the collection to update it and treat
        // the provided collection as the old version
        oldLastModified = collection.lastModified;
    } else {
        // If the old last modified date was provided, we do not touch the collection. We update
        // the libraries using the old last modified and the last modified that is on the provided
        // collection
        touchCollection = false;
    }

    // If the caller specified we should "touch" the collection, we simply update its last modified
    ...
    OaeUtil.invokeIfNecessary(touchCollection, CollectionsDAO.updateCollection, collection.id, {}, function(err, updatedCollection) {
        ...
        var effectiveLibraryVisibilities = LibraryAPI.Authz.resolveEffectiveLibraryVisibilities(principalIds, collection);
        LibraryAPI.Index.update(CollectionsConstants.library.COLLECTIONS_LIBRARY_INDEX_NAME, effectiveLibraryVisibilities, updatedCollection.id, updatedCollection.lastModified, oldLastModified, function(err) {
            if (err) {
                return callback(err);
            }

            // Return the most recent version of the collection to the caller. If we updated the
            // timestamp of the collection, we want to make sure it has the most recent version
            return callback(null, updatedCollection || collection);
        });
    });
};
