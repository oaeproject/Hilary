...
var _handleSignedDownload = function(req, res) {
  ContentAPI.verifySignedDownloadQueryString(req.ctx, req.query, function(
    err,
    downloadInfo,
  ) {
    if (err) {
...
var _createContent = function(
  ...
  if (resourceSubType === 'link') {
    ...
    return callback({
      code: 400,
      msg:
        'Unrecognized resourceSubType. Accepted values are "link", "file" and "collabdoc"',
    });
  }
};
...
OAE.tenantRouter.on('get', '/api/content/:contentId', function(req, res) {
  ContentAPI.getFullContentProfile(req.ctx, req.params.contentId, function(
    err,
    contentProfile,
  ) {
    if (err) {
 ...
 */
OAE.tenantRouter.on('get', '/api/content/:contentId/download', function(
  req,
  res,
) {
...
OAE.tenantRouter.on(
  'get',
  '/api/content/:contentId/download/:revisionId',
  function(req, res) {
    ContentAPI.getRevisionDownloadInfo(
      req.ctx,
      req.params.contentId,
 ...
 * @HttpResponse                    404                     Content not available
 */
OAE.tenantRouter.on('post', '/api/content/:contentId/newversion', function(
  req,
  res,
) {
  if (!req.files || !req.files.file) {
  ...
  function(req, res) {
    ...
    try {
      ...
      return res
        .status(400)
        .send(
          'Malformed metadata object. Expected proper JSON for: ' +
            invalidField,
        );
    }
 ...
 * @HttpResponse                    404                 Content not available
 */
OAE.tenantRouter.on('get', '/api/content/:contentId/revisions', function(
  req,
  res,
) {
  ...
  ContentAPI.getRevisions(
    ...
    req.query.start,
    limit,
    function(err, revisions, nextToken) {
      if (err) {
        return res.status(err.code).send(err.msg);
      }
      res.status(200).send({ results: revisions, nextToken: nextToken });
    },
  );
});
 ...
 * @HttpResponse                    404                 Content not available
 */
OAE.tenantRouter.on('get', '/api/content/:contentId/members', function(
  req,
  res,
) {
  var limit = OaeUtil.getNumberParam(req.query.limit, 10, 1, 25);
  ContentAPI.getContentMembersLibrary(
    ...
    req.query.start,
    limit,
    function(err, members, nextToken) {
      if (err) {
        return res.status(err.code).send(err.msg);
      }
 ...
 * @HttpResponse                        404                 Content not available
 */
OAE.tenantRouter.on('post', '/api/content/:contentId/members', function(
  req,
  res,
) {
  ...
  ContentAPI.setContentPermissions(
    req.ctx,
    req.params.contentId,
    req.body,
    function(err) {
 ...
 * @HttpResponse                        404                 Content not available
 */
OAE.tenantRouter.on('get', '/api/content/:contentId/invitations', function(
  req,
  res,
) {
  ContentAPI.getContentInvitations(req.ctx, req.params.contentId, function(
    err,
    invitations,
  ) {
    if (err) {
...
OAE.tenantRouter.on(
  'post',
  '/api/content/:contentId/invitations/:email/resend',
  function(req, res) {
    ContentAPI.resendContentInvitation(
      req.ctx,
      req.params.contentId,
      req.params.email,
      function(err) {
 ...
 * @HttpResponse                404                 Content not available
 */
OAE.tenantRouter.on('post', '/api/content/:contentId/share', function(
  req,
  res,
) {
  // Make sure viewers is an array
  req.body.viewers = OaeUtil.toArray(req.body.viewers);
  ContentAPI.shareContent(
    req.ctx,
    req.params.contentId,
    req.body.viewers,
    function(err) {
 ...
 */
OAE.tenantRouter.on('post', '/api/content/:contentId/messages', function(
  req,
  res,
) {
 ...
 * @HttpResponse                    400                 Invalid content resource id provided
 */
OAE.tenantRouter.on('get', '/api/content/:contentId/messages', function(
  req,
  res,
) {
  var limit = OaeUtil.getNumberParam(req.query.limit, 10, 1, 25);
  ContentAPI.getComments(
    ...
    function(err, messages, nextToken) {
      ...
      res.status(200).send({ results: messages, nextToken: nextToken });
    },
  );
});
...
OAE.tenantRouter.on(
  'delete',
  '/api/content/:contentId/messages/:created',
  function(req, res) {
 ...
 */
OAE.tenantRouter.on('get', '/api/content/library/:principalId', function(
  req,
  res,
) {
  ...
  ContentAPI.getContentLibraryItems(
    ...
    req.query.start,
    limit,
    function(err, items, nextToken) {
      if (err) {
        return res.status(err.code).send(err.msg);
      }
      res.status(200).send({ results: items, nextToken: nextToken });
    },
  );
});
...
OAE.tenantRouter.on(
  'delete',
  '/api/content/library/:principalId/:contentId',
  function(req, res) {
...
var _handleDownload = function(res, downloadInfo, expiresMax) {
  ...
  if (
    ...
    downloadStrategy.strategy ===
    ContentConstants.backend.DOWNLOAD_STRATEGY_REDIRECT
  ) {
    ...
    downloadStrategy.strategy ===
    ContentConstants.backend.DOWNLOAD_STRATEGY_TEST
  ) {
