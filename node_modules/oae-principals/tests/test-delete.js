                ...
                PrincipalsTestUtil.assertGetUserSucceeds(user1.restContext, user1.user.id, function() {
                    PrincipalsTestUtil.assertGetUserSucceeds(user2.restContext, user2.user.id, function() {
                        PrincipalsTestUtil.assertGetUserSucceeds(user3.restContext, user3.user.id, function() {
                            ...
                            DisableUsersMigration.doMigration(globalAdminContext, global.oaeTests.tenants.cam.alias, function(err, affectedUsers) {
                                assert.ok(!err);
                                // Update redis and search since we updated outside the scope of the API
                                Redis.flush(function(err) {
                                    assert.ok(!err);
                                    PrincipalsTestUtil.assertGetUserFails(user1.restContext, user1.user.id, 404, function(err, user1) {
                                        assert.ok(!err);
                                        PrincipalsTestUtil.assertGetUserFails(user2.restContext, user2.user.id, 404, function(err, user2) {
                                            assert.ok(!err);
                                            PrincipalsTestUtil.assertGetUserFails(user3.restContext, user3.user.id, 404, function(err, user3) {
                                                assert.ok(!err);
                                                PrincipalsAPI.deleteOrRestoreUsersByTenancy(globalAdminContext, global.oaeTests.tenants.cam.alias, false, function(err) {
                                                    assert.ok(!err);
                                                    return callback();
                                                });
                                            });
