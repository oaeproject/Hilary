...
OAE.globalAdminRouter.on('post', '/api/tenant/create', function(req, res) {
    ...
    // Apply the email domain if it is specified
    if (_.isString(req.body.emailDomain)) {
        opts.emailDomain = req.body.emailDomain;
    }
...
OAE.globalAdminRouter.on('get', '/api/tenants', function(req, res, next) {
    res.send(200, TenantsAPI.getTenants());
});
...
OAE.tenantRouter.on('get', '/api/tenant/landingPage', function(req, res) {
    ...
    return res.send(200, landingPage);
});
