...
describe('Utilities', function() {
    ...
    describe('Cassandra', function() {
        ...
        it('Test casting to a Boolean', function(callback) {
            Cassandra.runQuery('INSERT INTO testQuery (key_id, testbool, teststring) VALUES (?, ?, ?) USING CONSISTENCY QUORUM', ['testkey', 'true', 'notaboolean'], function (err) {
                assert.ok(!err);
                Cassandra.runQuery('SELECT testbool, teststring FROM testQuery USING CONSISTENCY QUORUM WHERE key_id = ?', ['testkey'], function (err, rows) {
                    assert.ok(!err);
                    assert.equal(typeof rows[0][0].value, 'boolean');
                    assert.equal(typeof rows[0][1].value, 'string');
                    callback();
                });
            });
        });
