 ...
 */
ActivityAPI.registerActivityRouter('user', function(activitySeed, entityType, entity, callback) {
    // Route the activity to the user's self
    var userId = entity[ActivityConstants.properties.OAE_ID];
    var activityRoutes = [userId];
    var notificationRoutes = [userId];

    // Route the activity to the followers of this user if appropriate
    _routeFollowers(activitySeed, entityType, entity, function(err, followers) {
        if (err) {
            return callback(err);
        } else if (followers && followers.length > 0) {
            activityRoutes = activityRoutes.concat(followers);
        }
...
/*!
 * Identify the follower routes. If the activity is not a create, share or "add to group" type of activity, it will not be routed to
 * the user's followers.
 */
var _routeFollowers = function(activitySeed, entityType, entity, callback) {
    var userId = entity[ActivityConstants.properties.OAE_ID];
    if (entity.visibility !== 'private' && _.contains([ActivityConstants.verbs.CREATE, ActivityConstants.verbs.ADD, ActivityConstants.verbs.SHARE], activitySeed.verb)) {
 ...
 */
ActivityAPI.registerActivityRouter('group', function(activitySeed, entityType, persistentEntity, callback) {
    ...
    ActivityUtil.getAllAuthzMembersByRole(groupId, function(err, specify) {
        ...
        // Resolve routes
        var activityRoutes = null;
        var notificationRoutes = null;

        var isGroupCreateActivity = (activitySeed.verb === ActivityConstants.verbs.CREATE);
        var isMembershipActivity = _.contains([ActivityConstants.verbs.JOIN, activitySeed.verb === ActivityConstants.verbs.ADD], activitySeed.verb);

        // Determine the activity routes. Membership activities only go to the managers of the content item
        if (isMembershipActivity) {
            // If the activity is that a user became a member of the group, only managers are interested
            activityRoutes = specify['manager'] || [];
        } else {
            // Union all members of all roles into one array as the routes
            activityRoutes = _.flatten(_.values(specify));
        }

        // Determine the notification routes. If this group was just created, we must notify all user members
        if (isGroupCreateActivity) {
        ...
        // Include the group itself as an activity route
        activityRoutes.push(groupId);
        activityRoutes = _.uniq(activityRoutes);

        // Groups always show for everyone
        return callback(null, {'activity': activityRoutes, 'notification': notificationRoutes}, {'type': ActivityConstants.entityPropagation.ALL});
    });
});
