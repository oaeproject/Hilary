...
var rowToGroup = function (row) {
    ...
    var group = new Group(hash.tenant, hash.principalId, hash.alias, hash);

    // This only modifies the group object if the group has a picture.
    setPictureOnPrincipalObject(group, 'small', hash.smallPictureUri);
    setPictureOnPrincipalObject(group, 'medium', hash.mediumPictureUri);
    setPictureOnPrincipalObject(group, 'large', hash.largePictureUri);
    return group;
};
...
var hashToUser = function(hash) {
    ...
    // This only modifies the user object if the user has a picture.
    setPictureOnPrincipalObject(user, 'small', hash.smallPictureUri);
    setPictureOnPrincipalObject(user, 'medium', hash.mediumPictureUri);
    setPictureOnPrincipalObject(user, 'large', hash.largePictureUri);
    return user;
};
...
/**
 * Sets a picture on a principal model object.
 * This method will only set a picture if all the parameters are defined.
 * Note: this does *NOT* persist anything.
 *
 * @param {Group|User}  principal   The User or Group object on which the picture metadata should be set.
 * @param {String}      size        The size of this image. One of small, medium or large.
 * @param {String}      uri         The URI.
 */
var setPictureOnPrincipalObject = module.exports.setPictureOnPrincipalObject = function(principal, size, uri) {
    if (principal && size && uri) {
        // Having 32 as the day will automatically make the month (and/or year) roll over.
        // This allows us to have a small sliding window.
        var now = new Date();
        var expiresAt = new Date(now.getFullYear(), now.getMonth(), 32);
        var sig = Signature.createExpiresAtSignature(principal.tenant, expiresAt, uri);
        principal[size + 'Picture'] = util.format('/api/download/signed?uri=%s&signature=%s&expires=%s', uri, sig.signature, sig.expires);
        principal[size + 'PictureUri'] = uri;
    }
};
