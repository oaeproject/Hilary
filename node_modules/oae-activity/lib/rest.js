...
var _handleGetActivities = function(activityStreamId, req, res) {
    ...
    var limit = isNaN(req.query.limit) ? null : req.query.limit;
    var start = isNaN(req.query.from) ? null : req.query.from;
...
/**
 * Request handler to force a collection of aggregate activities. It will return the collection cycle completes.
 *
 * @param   {Request}   req     The express request object
 * @param   {Response}  res     The express response object
 */
var _handlePostCollection = function(req, res) {
    ActivityAPI.collect(req.ctx, function(err) {
        if (err) {
            return res.send(err.code, err.msg);
        }
...
/*!
 * Force an aggregate activity collection.
 */
OAE.globalAdminServer.post('/api/activity/collect', _handlePostCollection);
 ...
 * Force an aggregate activity collection.
 */
OAE.tenantServer.post('/api/activity/collect', _handlePostCollection);
