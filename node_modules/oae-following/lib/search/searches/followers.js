...
module.exports = function(ctx, opts, callback) {
    ...
    validator.check(opts.userId, {'code': 400, 'msg': 'Must specificy an id of a user to search their followers'}).isUserId();
    ...
    PrincipalsDAO.getPrincipal(opts.userId, function(err, user) {
        ...
        FollowingAuthz.canViewFollowers(ctx, user, function(err, canViewFollowers) {
            if (err) {
                return callback(err);
            } else if (!canViewFollowers) {
                return callback({'code': 401, 'msg': 'You are not authorized to view this user\' followers'});
            }
...
var _search = function(ctx, opts, callback) {
    ...
    var filter = SearchUtil.filterAnd(
        ...
        SearchUtil.createHasChildQuery('resource_following', SearchUtil.filterTerms('following', [opts.userId]))
    );
