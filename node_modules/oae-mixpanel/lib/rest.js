...
var _getFileSize = function(req, res) {
    MixPanelAPI.getUploadedFilesFileSize(req.ctx, function(err, data) {
        if (err) {
 ...
 *
 * Get the total file size of uploaded files stats for all tenants for the current month
 *
 * @Server      admin,tenant
 * @Method      GET
 ...
 *
 * Get the total file size of uploaded files stats for all tenants for the current month for a specific tenant
 *
 * @Server      admin
 * @Method      GET
...
OAE.globalAdminRouter.on('get', '/api/mixpanel/stats/fileSize/:tenantAlias', function(req, res) {
    MixPanelAPI.getUploadedFilesFileSizeForTenant(req.ctx, req.params.tenantAlias, function(err, data) {
        if (err) {
            return res.status(err.code).send(err.msg);
        }
...
var _getUniqueUsers = function(req, res) {
    MixPanelAPI.getUniqueUsers(req.ctx, function(err, data) {
        if (err) {
            return res.status(err.code).send(err.msg);
        }
 ...
 * @REST getUniqueUsers
 *
 * Get the unique user count stats for all tenants for the current month
 *
 * @Server      admin,tenant
 * @Method      GET
...
OAE.globalAdminRouter.on('get', '/api/mixpanel/stats/uniqueUsers/:tenantAlias', function(req, res) {
    MixPanelAPI.getUniqueUsersForTenant(req.ctx, req.params.tenantAlias, function(err, data) {
        if (err) {
            return res.status(err.code).send(err.msg);
        }
