 ...
 */
var createIndex = (module.exports.createIndex = function(
  indexName,
  settings,
  callback,
) {
  indexExists(indexName, function(err, exists) {
    ...
    if (!exists) {
      ...
      _exec(
        'createIndex',
        client.createIndex(indexName, settings, null),
        callback,
      );
    } else {
      return callback();
...
var indexExists = (module.exports.indexExists = function(indexName, callback) {
  _exec('indexStatus', client.status(indexName, null), function(err) {
    if (
      err &&
      err.error === 'IndexMissingException[[' + indexName + '] missing]'
    ) {
 ...
 * @param  {Object}    callback.err        An error that occurred, if any
 */
var putMapping = (module.exports.putMapping = function(
  typeName,
  fieldProperties,
  opts,
  callback,
  ...
  mappingExists(typeName, function(err, exists) {
    ...
    return _exec(
      'putMapping',
      client.putMapping(index, typeName, data),
      callback,
    );
  });
});
 ...
 * @param  {Object}    callback.err    An error that occurred, if any
 */
var mappingExists = (module.exports.mappingExists = function(
  typeName,
  callback,
) {
  _exec('getMapping', client.getMapping(index, typeName, null), function(
    err,
    data,
  ) {
    if (err) {
 ...
 * @param  {Object}    callback.err    An error that occurred, if any
 */
var index = (module.exports.index = function(
  typeName,
  id,
  doc,
  options,
  ...
  log().trace(
    { typeName: typeName, id: id, document: doc, options: options },
    'Indexing a document',
  );
  return _exec(
    'index',
    client.index(index, typeName, doc, id, options),
    callback,
  );
});
...
var bulk = (module.exports.bulk = function(operations, callback) {
  ...
  for (var i = 0; i < operations.length; i++) {
    ...
    if (
      keys.length === 1 &&
      (meta['create'] || meta['index'] || meta['delete'])
    ) {
      numOps++;
      var opName = keys[0];
  ...
  log().trace(
    ...
    'Performing a bulk set of %s operations.',
    numOps,
  );
...
var del = (module.exports.del = function(typeName, id, callback) {
  log().trace(
    { typeName: typeName, documentId: id },
    'Deleting an index document.',
  );
  return _exec(
    'delete',
    client.deleteDocument(index, typeName, id, null),
    callback,
  );
});
 ...
 * @param  {Object}     callback.err    An error that occurred, if any
 */
var deleteByQuery = (module.exports.deleteByQuery = function(
  typeName,
  query,
  options,
  callback,
  ...
  log().trace(
    { typeName: typeName, query: query, options: options },
    'Deleting search documents by query',
  );
  return _exec(
    'deleteByQuery',
    client.deleteByQuery(index, typeName, query, options),
    callback,
  );
});
