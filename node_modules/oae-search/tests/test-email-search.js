...
describe('Email Search', function() {
    ...
    var _runEmailSearchInteractionTests = function(tests, callback) {
        ...
        fn(test.restCtx, 'email', null, {'email': test.target.email, 'scope': '_interact'}, [test.target.id], function(response) {
    ...
    it('verify authorization of email search', function(callback) {
        TestsUtil.generateTestUsers(camAdminRestContext, 1, function(err, users, user) {
            ...
            SearchTestUtil.assertSearchFails(anonymousRestContext, 'email', null, {'scope': '_interact', 'email': user.user.email}, 401, function() {
                SearchTestUtil.assertSearchEquals(user.restContext, 'email', null, {'scope': '_interact', 'email': user.user.email}, [user.user.id], function() {
                    SearchTestUtil.assertSearchEquals(camAdminRestContext, 'email', null, {'scope': '_interact', 'email': user.user.email}, [user.user.id], function() {
                        SearchTestUtil.assertSearchEquals(globalAdminOnTenantRestContext, 'email', null, {'scope': '_interact', 'email': user.user.email}, [user.user.id], function() {
                            return callback();
                        });
                    });
                });
    ...
    it('verify validation of email search', function(callback) {
        TestsUtil.generateTestUsers(camAdminRestContext, 1, function(err, users, user) {
            ...
            SearchTestUtil.assertSearchFails(user.restContext, 'email', null, null, 400, function() {
                SearchTestUtil.assertSearchFails(user.restContext, 'email', null, {'email': 'notanemail'}, 400, function() {

                    // Sanity check user can perform an email search
                    SearchTestUtil.assertSearchEquals(user.restContext, 'email', null, {'email': user.user.email}, [user.user.id], function() {
                        return callback();
                    });
                });
            });
    ...
    /**
     * Test that verifies that email _interact search does not violate interaction boundaries
     */
    it('verify email search does not violate tenant interaction boundaries', function(callback) {
        TestsUtil.setupMultiTenantPrivacyEntities(function(publicTenant0, publicTenant1, privateTenant0, privateTenant1) {
            _runEmailSearchInteractionTests([
                ...
                {'restCtx': publicTenant0.publicUser.restContext, 'target': publicTenant0.privateUser.user, 'visible': false},
                ...
                {'restCtx': publicTenant0.publicUser.restContext, 'target': publicTenant1.loggedinUser.user, 'visible': false},
                {'restCtx': publicTenant0.publicUser.restContext, 'target': publicTenant1.privateUser.user, 'visible': false},
                ...
                {'restCtx': privateTenant0.publicUser.restContext, 'target': privateTenant0.privateUser.user, 'visible': false},
                                ...
                                FoldersTestUtil.assertCreateFolderSucceeds(userInfo0.restContext, userInfo0.user.email, userInfo0.user.email, 'public', null, null, function(folder) {
                                    SearchTestUtil.whenIndexingComplete(function() {
                                        ...
                                        SearchTestUtil.assertSearchContains(userInfo0.restContext, 'general', null, {'q': userInfo0.user.email}, allResourceIds, function() {
                                            ...
                                            SearchTestUtil.assertSearchEquals(userInfo0.restContext, 'email', null, {'email': userInfo0.user.email}, allUserIds, function() {
                                                return callback();
                                            });
                                        });
                                    });
