...
describe('Email Search', function() {
    ...
    var _runEmailSearchInteractionTests = function(tests, callback) {
        ...
        fn(test.restCtx, 'email', null, {'q': test.target.email, 'scope': '_interact'}, [test.target.id], function(response) {
    ...
    it('verify authorization of email search', function(callback) {
        TestsUtil.generateTestUsers(camAdminRestContext, 1, function(err, users, user) {
            ...
            SearchTestUtil.assertSearchFails(anonymousRestContext, 'email', null, {'scope': '_interact', 'q': user.user.email}, 401, function() {
                SearchTestUtil.assertSearchEquals(user.restContext, 'email', null, {'scope': '_interact', 'q': user.user.email}, [user.user.id], function() {
                    SearchTestUtil.assertSearchEquals(camAdminRestContext, 'email', null, {'scope': '_interact', 'q': user.user.email}, [user.user.id], function() {
                        SearchTestUtil.assertSearchEquals(globalAdminOnTenantRestContext, 'email', null, {'scope': '_interact', 'q': user.user.email}, [user.user.id], function() {
                            return callback();
                        });
                    });
                });
