...
OaeEmitter.on('ready', function() {
    ...
    if (getTenant(serverConfig.guestsTenantAlias)) {
        return;
    }

    // Create the guests tenant
    _createTenant(serverConfig.guestsTenantAlias, 'Guests tenant', serverConfig.guestsTenantHost, function(err) {
        if (err) {
...
var getGuestsTenant = module.exports.getGuestsTenant = function() {
    return getTenant(serverConfig.guestsTenantAlias);
};
...
var updateTenant = module.exports.updateTenant = function(ctx, alias, tenantUpdates, callback) {
    ...
    validator.check(alias.toLowerCase(), {'code': 400, 'msg': 'The guests tenant cannot be updated'}).not(serverConfig.guestsTenantAlias);
