...
describe('UI', function() {
    ...
    describe('Skinning', function() {
            ...
            TestsUtil.createTenantWithAdmin(testTenantAlias, testTenantHost, function(err, testTenant, testTenantAdminRestContext) {
                ...
                UITestUtil.updateSkinAndWait(globalAdminRestContext, testTenantAlias, skinConfig, function(err) {
                    ...
                    RestAPI.UI.getSkinVariables(globalAdminRestContext, testTenantAlias, function(err, variables) {
                        ...
                        var institutionalLogoUrlValue = _getSkinVariableValue('institutional-logo-url', variables);
                        assert.strictEqual(institutionalLogoUrlValue, '\'/assets/${tenantAlias}/logo/${tenantAlias}.png\'');

                        // Get the rendered skin and ensure the tenant alias is
                        // placed in the institutional logo url
                        RestAPI.UI.getSkin(testTenantAdminRestContext, function(err, css) {
                            assert.ok(!err);

                            // Ensure the `.oae-institutiona-logo` class
                            // contains the dynamic value
                            var expectedInstitutionalLogoStr = util.format('.oae-institutional-logo{background-image:url(/assets/%s/logo/%s.png)}', testTenantAlias, testTenantAlias);
                            assert.notEqual(css.indexOf(expectedInstitutionalLogoStr), -1);
                            return callback();
                        });
                    });
                });
        ...
        it('verify logo can be uploaded for a tenant', function(callback) {
            ...
            var fileStream = fs.createReadStream(__dirname + '/data/oae-logo.png');

            // Assert that the global admin can change the logo for a tenant
            RestAPI.UI.uploadLogo(globalAdminRestContext, fileStream, tenantAlias, function(err, data) {
                assert.ok(!err);
                assert.ok(data);
                assert.ok(data.url.indexOf('signed') !== -1);

                fileStream = fs.createReadStream(__dirname + '/data/oae-logo.png');
                // Assert that a tenant admin can change the logo for a tenant
                RestAPI.UI.uploadLogo(camAdminRestContext, fileStream, tenantAlias, function(err, data) {
                    assert.ok(!err);
                    assert.ok(data);
                    assert.ok(data.url.indexOf('signed') !== -1);
                    ...
                    RestAPI.UI.uploadLogo(anonymousCamRestContext, fileStream, tenantAlias, function(err, data) {
                        assert.ok(err);
                        ...
                        TestsUtil.generateTestUsers(camAdminRestContext, 1, function(err, users) {
                            ...
                            RestAPI.UI.uploadLogo(user.restContext, fileStream, tenantAlias, function(err, data) {
                                assert.ok(err);
                                assert.strictEqual(err.code, 401);

                                fileStream = fs.createReadStream(__dirname + '/data/video.mp4');
