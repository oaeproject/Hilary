...
var init = module.exports.init = function(callback) {
    return MessageBoxSearch.registerMessageSearchDocument('discussion_message', function(resources, callback) {
...
DiscussionsAPI.on(DiscussionsConstants.events.DELETED_DISCUSSION, function(ctx, discussionId) {
    TaskQueue.submit(SearchConstants.mq.TASK_DELETE_DOCUMENT, {'id': discussionId});
});
...
DiscussionsAPI.on(DiscussionsConstants.events.DELETED_DISCUSSION_MESSAGE, function(ctx, discussion, deleteType, messageCreatedDate, message) {
    return MessageBoxSearch.deleteMessageSearchDocument('discussion_message', message);
});
...
var _produceDiscussionMessageDocuments = function(resources, callback, _documents) {
    ...
    if (resource.messages) {
        return callback(null, MessageBoxSearch.createMessageSearchDocuments('discussion_message', resource.id, resource.messages));
    }

    // If there were no messages stored on the resource object, we go ahead and index all messages for the discussion
    return MessageBoxSearch.createAllMessageSearchDocuments('discussion_message', resource.id, resource.id, callback);
};
...
SearchAPI.registerReindexAllHandler('discussion', function(callback) {
    ...
    var _onEach = function(discussionRows, done) {
        ...
        TaskQueue.submit(SearchConstants.mq.TASK_INDEX_DOCUMENT, {'resources': discussionResources});
