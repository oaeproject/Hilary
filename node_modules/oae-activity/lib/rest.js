...
var _handleGetActivities = function(activityStreamId, req, res) {
    ...
    ActivityAPI.getActivityStream(req.ctx, activityStreamId, 'activity', start, limit, req.query.transformer, function(err, activityStream) {
        if (err) {
...
OAE.tenantRouter.on('get', '/api/notifications', function(req, res) {
    var limit = OaeUtil.getNumberParam(req.query.limit, 10, 1, 25);
    var userId = req.ctx.user() ? req.ctx.user().id : null;
    ActivityAPI.getActivityStream(req.ctx, userId, 'notification', req.query.start, limit, req.query.transformer, function(err, notificationStream) {
        if (err) {
            return res.send(err.code, err.msg);
        }
