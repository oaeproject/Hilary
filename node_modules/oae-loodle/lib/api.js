...
exports.create = function (ctx, currentUser, contentId, name, description, visibility, additionalMembers, folders, callback) {
    ...
    async.series([
        ...
        /**
        // Add the members of the folders the loodle is created in
        function (end) {
            addMembersOfFolders(ctx, folders, loodleId, token, end);
        },
         **/
    ], function (err) {
        return callback(err);
...
/**
 * Add the members of multiple folders to a loodle
 *
 * @param ctx
 * @param folders
 * @param loodleId
 ...
 * @param callback
 */
var addMembersOfFolders = function (ctx, folders, loodleId, token, callback) {

    async.eachSeries(folders, function (folder, done) {
        addMembersOfFolder(ctx, folder, loodleId, token, done);
    }, callback);

};

/**
 * Add the members of a folder to a loodle
 *
 * @param ctx
 * @param folder
 * @param loodleId
 ...
 * @param callback
 */
var addMembersOfFolder = function (ctx, folder, loodleId, token, callback) {

    // Require the folders api in this local scope
    // avoid a circle dependency problem when creating a folder
    var FolderAPI = require('oae-folders');

    // Get the members of the folders
    FolderAPI.getFolderMembers(ctx, folder, 0, null, function (err, members) {

        async.each(members, function (member, done) {
            // Create a mirror loodle account if the user does not have one yet
            UserDAO.createUserIfNeeded(ctx, member.profile.id, function (err) {
                if (err) return done(err);

                // Add the user to the specified loodle if he/she does not participate yet
                LoodleDAO.addUserIfNeeded(ctx, loodleId, member.profile.id, token, done);
            });
        }, callback);
