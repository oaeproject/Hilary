...
describe('Utilities', function() {
    ...
    describe('Cassandra', function() {
            ...
            Cassandra.createColumnFamily('VerifyPagedColumnQueryStartAndEnd', 'CREATE TABLE "VerifyPagedColumnQueryStartAndEnd" ("keyId" text, "columnName" text, "value" text, PRIMARY KEY("keyId", "columnName")) WITH COMPACT STORAGE', function(err, created) {
                ...
                Cassandra.runBatchQuery(batch, function(err) {
                    ...
                    Cassandra.runPagedQuery('VerifyPagedColumnQueryStartAndEnd', 'keyId', 'key', 'columnName', null, 8, {'end': 'a'}, function(err, rows, nextToken, startMatched) {
                        ...
                        Cassandra.runPagedQuery('VerifyPagedColumnQueryStartAndEnd', 'keyId', 'key', 'columnName', 'a', 8, {'end': 'a'}, function(err, rows, nextToken, startMatched) {
                            assert.ok(!err);
                            assert.ok(rows);
                            ...
                            Cassandra.runPagedQuery('VerifyPagedColumnQueryStartAndEnd', 'keyId', 'key', 'columnName', 'a', 8, {'end': 'b'}, function(err, rows, nextToken, startMatched) {
                                assert.ok(!err);
                                assert.ok(rows);
                                assert.strictEqual(startMatched, true);

                                assert.equal(rows.length, 1);
                                assert.equal(rows[0].get('columnName').value, 'b');
                                ...
                                Cassandra.runPagedQuery('VerifyPagedColumnQueryStartAndEnd', 'keyId', 'key', 'columnName', null, 8, {'end': 'j'}, function(err, rows, nextToken, startMatched) {
                                    ...
                                    Cassandra.runPagedQuery('VerifyPagedColumnQueryStartAndEnd', 'keyId', 'key', 'columnName', 'g', 8, {'end': 'j'}, function(err, rows, nextToken, startMatched) {
                                        assert.ok(!err);
                                        assert.ok(rows);
                                        assert.strictEqual(startMatched, true);

                                        assert.equal(rows.length, 3);
                                        assert.equal(rows[0].get('columnName').value, 'h');
                                        assert.equal(rows[1].get('columnName').value, 'i');
                                        assert.equal(rows[2].get('columnName').value, 'j');
                                        assert.ok(!nextToken);
                                        ...
                                        Cassandra.runPagedQuery('VerifyPagedColumnQueryStartAndEnd', 'keyId', 'key', 'columnName', null, 8, {'reversed': true, 'end': 'm'}, function(err, rows, nextToken, startMatched) {
                                            ...
                                            Cassandra.runPagedQuery('VerifyPagedColumnQueryStartAndEnd', 'keyId', 'key', 'columnName', 'm', 8, {'reversed': true, 'end': 'm'}, function(err, rows, nextToken, startMatched) {
                                                assert.ok(!err);
                                                assert.ok(rows);
                                                assert.strictEqual(startMatched, true);
                                                ...
                                                Cassandra.runPagedQuery('VerifyPagedColumnQueryStartAndEnd', 'keyId', 'key', 'columnName', 'm', 8, {'reversed': true, 'end': 'l'}, function(err, rows, nextToken, startMatched) {
                                                    assert.ok(!err);
                                                    assert.ok(rows);
                                                    assert.strictEqual(startMatched, true);

                                                    assert.equal(rows.length, 1);
                                                    assert.equal(rows[0].get('columnName').value, 'l');
                                                    ...
                                                    Cassandra.runPagedQuery('VerifyPagedColumnQueryStartAndEnd', 'keyId', 'key', 'columnName', null, 8, {'reversed': true, 'end': 'c'}, function(err, rows, nextToken, startMatched) {
                                                        ...
                                                        // Verify inclusive end works with bounded start, multple results partial page (forward)
                                                        Cassandra.runPagedQuery('VerifyPagedColumnQueryStartAndEnd', 'keyId', 'key', 'columnName', 'f', 8, {'reversed': true, 'end': 'c'}, function(err, rows, nextToken, startMatched) {
                                                            assert.ok(!err);
                                                            assert.ok(rows);
                                                            assert.strictEqual(startMatched, true);

                                                            assert.equal(rows.length, 3);
                                                            assert.equal(rows[0].get('columnName').value, 'e');
                                                            assert.equal(rows[1].get('columnName').value, 'd');
                                                            assert.equal(rows[2].get('columnName').value, 'c');
                                                            assert.ok(!nextToken);
        ...
        /**
         * Test that ensures the existence of the CQL3 bug: https://issues.apache.org/jira/browse/CASSANDRA-6330. When this bug no longer exists,
         * the Cassandra.runPagedQuery method can be removed in favour of simple CQL queries, as this bug is the only thing it is now
         * abstracting
         */
        it('verify a strict upper bound on range query results in one less item that requested with limit', function(callback) {
            Cassandra.createColumnFamily('VerifyCassandra6330', 'CREATE TABLE "VerifyCassandra6330" ("keyId" text, "column" text, "value" text, PRIMARY KEY ("keyId", "column")) WITH COMPACT STORAGE', function(err, created) {
                ...
                Cassandra.runBatchQuery(batch, function(err) {
                    ...
                    Cassandra.runQuery('SELECT "column" FROM "VerifyCassandra6330" WHERE "keyId" = ? AND "column" < ? ORDER BY "column" DESC LIMIT 2', ['key', 'c'], function(err, rows) {
                        ...
                        // Even though we asked for 2 items, and there were 2 to fetch (a and b), we only get one. This is the bug
