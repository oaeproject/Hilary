...
var getAllRoles = module.exports.getAllRoles = function(principalUuid, resourceUuid, callback) {
    ...
    getAllUserRoles(principalUuid, resourceUuid, callback);
};

// TODO
getAllUserRoles = function(principalUuid, groupId, callback) {
    var allRoles = {};
    // Direct check
    Cassandra.runQuery('SELECT ? FROM AuthzRoles USING CONSISTENCY QUORUM WHERE principalId = ?', [groupId, principalUuid], function(err, rows) {
        if (err) {
            return callback(err);
        }
        if (rows[0].get(groupId) && rows[0].get(groupId).value) {
            allRoles[rows[0].get(groupId).value] = true;
        }
        ...
        var cql = 'SELECT ? .. ? FROM AuthzGroupMembers USING CONSISTENCY QUORUM WHERE groupId = ?';
        Cassandra.runQuery(cql, [start, end, groupId], function(err, rows) {
            if (err) {
                return callback(err);
            }
                ...
                var explodeGroupMemberships = function(groupIds) {
                    ...
                    Cassandra.runQuery(cql, [start, end, _.keys(groupIds)], function(err, rows) {
                        ...
                        if (_.keys(newToInvestigate).length === 0) {
                            ...
                            Cassandra.runQuery('SELECT ? FROM AuthzRoles USING CONSISTENCY QUORUM WHERE principalId = ?', [_.keys(finalGroupIds), principalUuid], function(err, rows) {
                                if (err) {
                                    return callback(err);
                                }
                                if (rows[0].count > 0) {
                                    ...
                                    for (var g in groups) {
                                        if (groups[g] && finalGroupIds[g]) {
                                            allRoles[finalGroupIds[g]] = true;
                                        }
                                    }
 ...
 * @param  {Object}      callback.err            An error that occurred, if any
 * @param  {String[]}    callback.members        The principal UUIDs for all members of the group
 */
...
var isMemberOf = module.exports.isMemberOf = function(principalUuid, groupUuid, callback) {
    ...
    checkGroupMember(principalUuid, groupUuid, callback);
    //isAllowedAny(principalUuid, groupUuid, callback);
};

//TODO
var checkGroupMember = function(userId, groupId, callback) {
    // Direct check
    Cassandra.runQuery('SELECT ? FROM AuthzRoles USING CONSISTENCY QUORUM WHERE principalId = ?', [groupId, userId], function(err, rows) {
        if (err) {
            return callback(err);
        }
        ...
        if (rows[0].get(groupId) && rows[0].get(groupId).value) {
            ...
            // Get the groups associated to the group
            //var limit = 1000;
            var columnPrefix = 'g:';
            var start = columnPrefix;
            var end = start+'|';
                    ...
                    var explodeGroupMemberships = function(groupIds) {
                        ...
                        Cassandra.runQuery(cql, [start, end, groupIds], function(err, rows) {
                            ...
                            if (newToInvestigate.length === 0) {
                                ...
                                var start = 'g:';
                                var end = start + '|';
                                Cassandra.runQuery('SELECT ? FROM AuthzRoles USING CONSISTENCY QUORUM WHERE principalId = ?', [finalGroupIds, userId], function(err, rows) {
                                    if (err) {
                                        return callback(err);
                                    }
                                    if (rows[0].count > 0) {
 ...
 * @param  {Object}         changes         A JSON object keyed by a principalUuid, with value either the role to apply, or 'false' to indicate removal from the group.
    ...
    hasRole(principalUuid, resourceUuid, action, function(err, hasRole) {
        if (err) {
            ...
            getMemberOf(principalUuid, function(err, groupUuids) {
                ...
                _isAllowed(groupUuids, action, resourceUuid, function(err, isAllowed) {
                    if (err) {
                        Telemetry.incr('isAllowed.err.count');
                        return callback(err);
                    }
    ...
    getRole(principalUuid, resourceUuid, function(err, role) {
        ...
        if (role) {
            ...
            getMemberOf(principalUuid, function(err, groupUuids) {
                ...
                _isAllowed(groupUuids, null, resourceUuid, function(err, isAllowed) {
                    if (err) {
                        Telemetry.incr('isAllowed.err.count');
                        return callback(err);
                    }
 ...
 * resource specified.
 *
 * @param  {String[]}      principalUuids       The array of principals to check
