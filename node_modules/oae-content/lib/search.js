 ...
 * @api private
 */
var _produceContentCommentDocuments = function(resources, callback, _documents, _errors) {
    _documents = _documents || [];
    _errors = _errors || [];
    if (_.isEmpty(resources)) {
        return callback(_errors, _documents);
    }
    ...
    if (resource.comments) {
        ...
        return _produceContentCommentDocuments(resources, callback, _documents, _errors);
    }

    // If there were no messages stored on the resource object, we go ahead and index all comments for the content item
    MessageBoxSearch.createAllMessageSearchDocuments(ContentConstants.search.MAPPING_CONTENT_COMMENT, resource.id, resource.id, function(errors, documents) {
        if (!_.isEmpty(errors)) {
            _errors = _.union(_errors, errors);
        }
 ...
 * @api private
 */
var _produceContentBodyDocuments = function(resources, callback, _documents, _errors) {
    _documents = _documents || [];
    _errors = _errors || [];
    if (_.isEmpty(resources)) {
        return callback(_errors, _documents);
    ...
    ContentDAO.Revisions.getRevisions(resource.id, null, 1, null, function(err, revisions) {
        if (err) {
            _errors.push(err);
            return _produceContentBodyDocuments(resources, callback, _documents, _errors);
        }
            ...
            ContentUtil.getStorageBackend(null, preview.uri).get(tenantAlias, preview.uri, function(err, file) {
                ...
                fs.readFile(file.path, function(err, data) {
                    ...
                    fs.unlink(file.path, function(err) {
                        if (err) {
                            log().error({'err': err}, 'Could not remove the plain text file when indexing content');
                        }

                        // Move on to the next file
                        _produceContentBodyDocuments(resources, callback, _documents);
                    });
                });
            });
        });
