...
module.exports = function(config) {
    ...
    strategy.getPassportStrategy = function(tenant) {
        ...
        var externalIdAttribute = AuthenticationConfig.getValue(tenant.alias, AuthenticationConstants.providers.SHIBBOLETH, 'externalIdAttribute');
        externalIdAttribute = _.chain(externalIdAttribute.split(' ')).compact().uniq().value();
        ...
        var passportStrategy = new ShibbolethStrategy({
            ...
            // The `remote_user` header is configured in shib to identify this user. We use
            ...
            for (var i = 0; ((externalId === headers['remote_user']) && (i < externalIdAttribute.length)); i++) {
                var attribute = externalIdAttribute[i];
