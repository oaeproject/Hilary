...
var generatePictureURLs = function(ctx, principal) {
    if (principal.smallPictureUri) {
        principal.smallPicture = getSignedPictureUrl(ctx, ctx.tenant().alias, principal.smallPictureUri);
    }
    if (principal.mediumPictureUri) {
        principal.mediumPicture = getSignedPictureUrl(ctx, ctx.tenant().alias, principal.mediumPictureUri);
    }
    ...
    if (principal.largePictureUri && ctx.user() && ctx.user().id == principal.id) {
        principal.largePicture = getSignedPictureUrl(ctx, ctx.tenant().alias, principal.largePictureUri);
    }
};
...
var getSignedPictureUrl = module.exports.getSignedPictureUrl = function(ctx, tenant, uri) {
    ...
    var sig = Signature.createRollingSignature(tenant, settings.unit, settings.value, settings.offsetUnit, settings.offsetValue, body);
...
var _getProfilePictureExpireSettings = function(tenantAlias) {
    return {
        'unit': Config.getValue(tenantAlias, 'picture', 'unit'),
        'value': parseInt(Config.getValue(tenantAlias, 'picture', 'value'), 10),
        'offsetUnit': Config.getValue(tenantAlias, 'picture', 'offsetUnit'),
        'offsetValue': parseInt(Config.getValue(tenantAlias, 'picture', 'offsetValue'), 10)
    };
};
