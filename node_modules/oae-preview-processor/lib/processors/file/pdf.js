...
// LESS template used to adapt the CSS files generated by pdf2htmlEX into
// a form compatible with embedding in an OAE page (single style sheet
// plus scoped style rules)
...
var init = module.exports.init = function(config, callback) {
    ...
    exec(cmd, {'timeout': 10000}, function (err, stdout, stderr) {
        ...
        exec(cmd, {'timeout': 10000}, function (err, stdout, stderr) {
            ...
            // Get the template used to construct CSS stylesheet. LESS will adopt the
            // multiple CSS style sheets created by pdf2htmlEX into a single style sheet,
            // and it will introduce scoping into the pdf2htmlEX rules so those rules
            // don't apply to elements on the OAE page other than the document.
            fs.readFile(__dirname + _pdf2htmlLessTemplatePath, 'utf8', function(err, templateContent) {
                if (err) {
...
var _convertPDFToHTMLPages = function(ctx, path, pagesDir, callback) {
    ...
    exec(cmd, {'timeout': _pdf2htmlEXTimeout}, function (err, stdout, stderr) {
        ...
        fs.readdir(pagesDir, function(err, files) {
            ...
            // Create a single CSS file to hold styles required for document and,
            // at the same time, scope the style rules so they don't interfere
            // with other elements on the OAE page
            var combinedCssPath = pagesDir + '/combined.css';

            // Process the CSS files using LESS. We have to replace any
            // colons in the parameters to ensure valid LESS syntax.
            var lessSource = _pdf2htmlLessTemplate({
                contentId: ctx.contentId.replace(/:/g, '-'),
                revisionId: ctx.revisionId.replace(/:/g, '-')
            });

            // Compile the LESS code with include paths set to location of style sheets
