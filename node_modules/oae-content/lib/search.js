...
var _produceContentBodyDocuments = function(resources, callback, _documents) {
    ...
    ContentDAO.Revisions.getRevisions(resource.id, null, 1, null, function(err, revisions) {
        ...
        var _done = _.after(revision.previews.pageCount, function() {
            _documents.push(SearchUtil.createChildSearchDocument(ContentConstants.search.MAPPING_CONTENT_BODY, resource.id, {'content_body': body}));
            return _produceContentBodyDocuments(resources, callback, _documents);
        });

        var _appendPreviewToBody = function(err, preview) {
            if (err) {
                return callback(err);
            }
        ...
        for (var i = 1; i <= revision.previews.pageCount; i++) {
            ContentDAO.Previews.getContentPreview(revision.previewsId, util.format('page.%s.html', i), _appendPreviewToBody);
        }
    });
};
