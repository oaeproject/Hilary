 ...
 * @param  {String}     contentId               The content id of the pad that should be joined
 * @param  {String}     padId                   The unique identifier of the pad that should be joined
 * @param  {String}     groupId                 The unique identifier of the group that is associated to this pad
 * @param  {Function}   callback                Standard callback method
 * @param  {Object}     callback.err            An error that occurred, if any
...
var joinPad = module.exports.joinPad = function(ctx, contentId, padId, groupId, callback) {
    ...
    client.createAuthorIfNotExistsFor(args, function(err, author) {
        if (err) {
            log().error({'err': err, 'contentId': contentId, 'principalId': ctx.user().id, 'etherpad': client.options.host}, 'Could not create an etherpad author');
        ...
        client.createSession(session, function(err, data) {
            if (err) {
                log().error({'err': err, 'contentId': contentId, 'principalId': ctx.user().id, 'etherpad': client.options.host}, 'Could not create an etherpad session');
...
/**
 * Manually mark a user as a recent author of a pad
 *
 * @param  {String}     contentId           The ID of the collaborative document in OAE
 * @param  {String}     padId               The ID of the collaborative document in etherpad
 * @param  {String}     userId              The ID of the OAE user to add as a recent author
 * @param  {Function}   callback            Standard callback function
 * @param  {Object}     callback.err        Standard error object, if any
 */
var addRecentAuthor = module.exports.addRecentAuthor = function(contentId, padId, userId, callback) {
    var client = getClient(contentId);

    var protocol = (client.options.port === 443 ? 'https' : 'http');
    var options = {
        'qs': {
            'apikey': client.options.apikey,
            'userId': userId
        },
        'uri': util.format('%s://%s:%s/oae/%s/recentAuthors/add', protocol, client.options.host, client.options.port, padId)
    };
    request(options, function(err, response, body) {
        if (err) {
            log().error({'err': err, 'etherpad': client.options.host}, 'Unable to communicate with the etherpad server');
            return callback({'code': 500, 'msg': 'Unable to communicate with the etherpad server'});
 ...
 * @param  {String}     contentId           The ID of the collaborative document in OAE
 * @param  {String}     padId               The ID of the collaborative document in etherpad
 * @param  {String}     userId              The ID of the OAE user to check
 * @param  {Function}   callback            Standard callback function
 * @param  {Object}     callback.err        Standard error object, if any
 * @param  {Boolean}    callback.recent     `true` if the user made a change recently, `false` otherwise
 */
var isRecentAuthor = module.exports.isRecentAuthor = function(contentId, padId, userId, callback) {
    var client = getClient(contentId);
    ...
    var options = {
        ...
        'uri': util.format('%s://%s:%s/oae/%s/recentAuthors', protocol, client.options.host, client.options.port, padId)
    };
    request(options, function(err, response, body) {
        if (err) {
            log().error({'err': err, 'etherpad': client.options.host}, 'Unable to communicate with the etherpad server');
        ...
        try {
            var isRecent = JSON.parse(body).recent || false;
            return callback(null, isRecent);
        } catch (err) {
            log().error({'err': err, 'body': body}, 'Unable to convert response body to a json object');
            return callback({'code': 500, 'msg': 'Unable to convert response body to a json object'});
        }
    });
};
