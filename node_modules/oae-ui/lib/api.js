...
var uploadLogoFile = module.exports.uploadLogoFile = function(ctx, file, tenantAlias, callback) {
    // Need to require this here to avoid circular dependency issues
    ...
    ContentAPI.createFile(ctx, util.format('%s logo', tenantAlias), util.format('Logo displayed on %s landing page', tenantAlias), AuthzConstants.visibility.PRIVATE, file, {}, [], function(err, content) {
        if (err) {
            return callback({'code': 500, 'err': err});
        }
        ContentAPI.getRevision(ctx, content.id, content.latestRevisionId, function(err, revision) {
            if (err) {
                return callback({'code': 500, 'err': err});
            }
            var signedUrl = ContentUtil.getSignedDownloadUrl(ctx, revision.uri, -1, -1);
