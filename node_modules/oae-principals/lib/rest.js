...
module.exports = function(tenant) {
    ...
    tenant.server.post('/api/user/create', function(req, res) {
        ...
        if (req.ctx.user()) {
            if (req.ctx.user().isAdmin(tenant.alias)) {
    ...
    tenant.server.get('/api/user/captcha', function(req, res) {
        ConfigAPI.getConfig(req.ctx.tenant().alias, function(err, config) {
            res.send(200, {'public-key': config['oae-principals']['recaptcha']['elements']['public-key'].defaultValue});
        });
    });
    ...
    tenant.server.get('/api/user/exists/:username', function(req, res) {
        req.telemetryUrl = '/api/user/exists/username';
        PrincipalsAPI.getTenantUser(req.ctx, req.params.username, function(err, user) {
            if (err) {
                return res.send(err.code, err.msg);
            }
            res.send(200, user);
