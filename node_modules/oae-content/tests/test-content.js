...
describe('Content', function() {
    ...
    var setUpGroups = function(contexts, callback) {
        ...
        // Create UI Dev Group
        // Make Bert a member
        RestAPI.Group.createGroup(contexts['bert'].restContext, 'UI Dev Team', 'UI Dev Group', 'public', 'yes', [], [contexts['nicolaas'].user.id], function(err, groupObj) {
            assert.ok(!err);
    ...
    var checkPieceOfContent = function(restCtx, libraryToCheck, contentObj, expectAccess, expectManager, expectInLibrary, expectCanShare, callback) {
        ...
        RestAPI.Content.getContent(restCtx, contentObj.id, function(err, retrievedContent) {
            if (expectAccess) {
                ...
                assert.equal(_.keys(retrievedContent.tenant).length, 3);
     ...
     */
    it('verify collabdoc and collabsheet editors can\'t delete', function(callback) {
        setUpUsers(function(contexts) {
            RestAPI.Content.createCollabDoc(contexts['nicolaas'].restContext, 'Test CollabDoc', 'Doc description', 'public', [], [contexts['stuart'].user.id], [], [], function(err, contentObj) {
                ...
                RestAPI.Content.deleteContent(contexts['stuart'].restContext, contentObj.id, function(err) {
                    ...
                    RestAPI.Content.deleteContent(contexts['nicolaas'].restContext, contentObj.id, function(err) {
                        assert.ok(!err);
                        RestAPI.Content.createCollabsheet(contexts['nicolaas'].restContext, 'Test collabsheet', 'Description', 'public', [], [contexts['stuart'].user.id], [], [], function(err, contentObj) {
                            assert.ok(!err);
                            RestAPI.Content.deleteContent(contexts['stuart'].restContext, contentObj.id, function(err) {
                                assert.equal(err.code, 401);
    ...
    describe('Content permissions', function() {
        ...
        it('verify validation on setting content permissions', function(callback) {
            setUpUsers(function(contexts) {
                ...
                RestAPI.Content.createLink(contexts['nicolaas'].restContext, 'Test Content 1', 'Test content description 1', 'public', 'http://www.oaeproject.org/', [], [], [], function(err, contentObj) {
                    ...
                    var validPermissions = AuthzTestUtil.createRoleChange([contexts['simon'].user.id], 'manager');

                    // Invalid content id
                    RestAPI.Content.updateMembers(contexts['nicolaas'].restContext, 'invalidContentId', validPermissions, function(err) {
                        assert.equal(err.code, 400);
                        ...
                        RestAPI.Content.updateMembers(contexts['nicolaas'].restContext, contentObj.id, {}, function(err) {
                            ...
                            // Invalid principal
                            RestAPI.Content.updateMembers(contexts['nicolaas'].restContext, contentObj.id, {'invalid-id': 'manager'}, function(err) {
                                assert.equal(err.code, 400);

                                // Invalid role change
                                var permissions = AuthzTestUtil.createRoleChange([contexts['simon'].user.id], 'totally-wrong-role');
                                RestAPI.Content.updateMembers(contexts['nicolaas'].restContext, contentObj.id, permissions, function(err) {
                                    assert.equal(err.code, 400);
                                    ...
                                    RestAPI.Content.updateMembers(contexts['nicolaas'].restContext, contentObj.id, permissions, function(err) {
                                        ...
                                        RestAPI.Content.updateMembers(contexts['nicolaas'].restContext, contentObj.id, permissions, function(err) {
                                            ...
                                            // Sanity check
                                            RestAPI.Content.updateMembers(contexts['nicolaas'].restContext, contentObj.id, validPermissions, function(err) {
                                                assert.ok(!err);
        ...
        /**
         * Test that verifies collabdoc or collabsheet editors can't change permissions, but can share
         */
        it('verify collabdoc or collabsheet editors can\'t change permissions', function(callback) {
            setUpUsers(function(contexts) {
                RestAPI.Content.createCollabDoc(contexts['nicolaas'].restContext, 'Test CollabDoc', 'Doc description', 'public', [], [contexts['stuart'].user.id], [], [], function(err, contentObj) {
                    assert.ok(!err);
                    var members = {};
                    ...
                    ContentTestUtil.assertUpdateContentMembersFails(contexts['nicolaas'].restContext, contexts['stuart'].restContext, contentObj.id, members, 401, function() {
                        ContentTestUtil.assertShareContentSucceeds(contexts['nicolaas'].restContext, contexts['stuart'].restContext, contentObj.id, _.keys(members), function() {
                            // Make sure same is true for collaborative spreadsheets
                            RestAPI.Content.createCollabsheet(contexts['stuart'].restContext, 'Test collabsheet', 'Sheet description', 'public', [], [contexts['nicolaas'].user.id], [], [], function(err, contentObj) {
                                assert.ok(!err);
                                var members = {};
                                members[contexts['anthony'].user.id] = 'viewer';
                                ContentTestUtil.assertUpdateContentMembersFails(contexts['stuart'].restContext, contexts['nicolaas'].restContext, contentObj.id, members, 401, function() {
                                    ContentTestUtil.assertShareContentSucceeds(contexts['stuart'].restContext, contexts['nicolaas'].restContext, contentObj.id, _.keys(members), callback)
                                });
                            });
                        });
                    });
         ...
         * as a member
         */
        it('verify setting permissions with validated user id adds it to their library', function(callback) {
            TestsUtil.generateTestUsers(camAdminRestContext, 2, function(err, users, creatingUserInfo, targetUserInfo) {
                assert.ok(!err);

                // Create a content item on which to set roles
                var randomString = TestsUtil.generateRandomText(1);
                ContentTestUtil.assertCreateLinkSucceeds(creatingUserInfo.restContext, randomString, randomString, 'public', 'http://www.oaeproject.org', [], [], [], function(content) {

                    // Set the roles of the content item
                    var roleChanges = _.object([[util.format('%s:%s', targetUserInfo.user.email, targetUserInfo.user.id), 'manager']]);
                    ContentTestUtil.assertUpdateContentMembersSucceeds(creatingUserInfo.restContext, creatingUserInfo.restContext, content.id, roleChanges, function() {
                        return callback();
                    });
                });
            });
         ...
         */
        it('verify setting permissions with an email associated to a unique email account adds it to their library', function(callback) {
            TestsUtil.generateTestUsers(camAdminRestContext, 4, function(err, users, creatingUserInfo, targetUserInfoA, targetUserInfoB1, targetUserInfoB2) {
                assert.ok(!err);
                ...
                ContentTestUtil.assertCreateLinkSucceeds(creatingUserInfo.restContext, randomString, randomString, 'public', 'http://www.oaeproject.org', [], [], [], function(content) {
                    ...
                    // Set the roles of the content item
                    var roleChanges = _.object([[targetUserInfoA.user.email, 'manager']]);
                    // restCtx, contentId, updatedMembers, callback
                    RestAPI.Content.updateMembers(creatingUserInfo.restContext, content.id, roleChanges, function(err) {
                        assert.ok(!err);

                        // Ensure the invitations list is empty
                        AuthzTestUtil.assertGetInvitationsSucceeds(creatingUserInfo.restContext, 'content', content.id, function(result) {
                            assert.ok(result);
                            ...
                            ContentTestUtil.getAllContentMembers(creatingUserInfo.restContext, content.id, null, function(members) {
                                var targetMember = _.find(members, function(member) {
                                ...
                                ContentTestUtil.assertGetAllContentLibrarySucceeds(targetUserInfoA.restContext, targetUserInfoA.user.id, null, function(contentItems) {
                                    assert.ok(_.findWhere(contentItems, {'id': content.id}));

                                    // Update the B target users to have the same emails
                                    PrincipalsTestUtil.assertUpdateUserSucceeds(targetUserInfoB2.restContext, targetUserInfoB2.user.id, {'email': targetUserInfoB1.user.email}, function(updatedUser, token) {
                                        PrincipalsTestUtil.assertVerifyEmailSucceeds(targetUserInfoB2.restContext, targetUserInfoB2.user.id, token, function() {
        ...
        it('verify sharing adds users to content members library', function(callback) {
            ...
            TestsUtil.generateTestUsers(camAdminRestContext, 3, function(err, users) {
                ...
                RestAPI.Content.createLink(actor.restContext, 'Test Content 1', 'Test content description 1', 'private', 'http://www.oaeproject.org/', [], [], [], function(err, link) {
                    ...
                    return ContentTestUtil.assertShareContentSucceeds(actor.restContext, actor.restContext, link.id, memberIds, callback);
                });
            });
        });
