...
var createDiscussion = module.exports.createDiscussion = function(ctx, displayName, description, visibility, roles, opts, callback) {
    ...
    ResourceActions.create(ctx, roles, createFn, function(err, discussion, changes) {
        if (err) {
            return callback(err);
        }

        // Index the discussion into user libraries
        _insertLibrary(_.keys(changes), discussion, function(err) {
            if (err) {
                return callback(err);
            }

            DiscussionsAPI.emit(DiscussionsConstants.events.CREATED_DISCUSSION, ctx, discussion, changes);
...
var shareDiscussion = module.exports.shareDiscussion = function(ctx, discussionId, principalIds, callback) {
    ...
    _getDiscussion(discussionId, function(err, discussion) {
        ...
        ResourceActions.share(ctx, discussion, principalIds, AuthzConstants.role.MEMBER, function(err, changes, roles, addedMembers) {
            if (err) {
                return callback(err);
            }

            var addedMemberIds = _.pluck(addedMembers, 'id');
            ...
            OaeUtil.invokeIfNecessary(_testDiscussionUpdateThreshold(discussion), _touch, discussion, function(err, touchedDiscussion) {
                ...
                _insertLibrary(addedMemberIds, discussion, function(err) {
                    ...
                    DiscussionsAPI.emit(DiscussionsConstants.events.UPDATED_DISCUSSION_MEMBERS, ctx, discussion, changes, addedMemberIds);
                    return callback();
                });
            });
        });
...
var setDiscussionPermissions = module.exports.setDiscussionPermissions = function(ctx, discussionId, changes, callback) {
    ...
    _getDiscussion(discussionId, function(err, discussion) {
        ...
        ResourceActions.setRoles(ctx, discussion, changes, function(err, changes, roles, members) {
            if (err) {
                return callback(err);
            }

            var addedMemberIds = _.pluck(members.added, 'id');
            var updatedMemberIds = _.pluck(members.updated, 'id');
            var removedMemberIds = _.pluck(members.removed, 'id');
            ...
            _removeLibrary(removedMemberIds, discussion, function(err) {
                ...
                OaeUtil.invokeIfNecessary(touchDiscussion, _touch, discussion, function(err, touchedDiscussion) {
                    ...
                    _insertLibrary(addedMemberIds, discussion, function(err) {
                        ...
                        var libraryUpdateIds = _.keys(roles.before);
            ...
            DiscussionsAPI.emit(DiscussionsConstants.events.UPDATED_DISCUSSION_MEMBERS, ctx, discussion, changes, addedMemberIds, updatedMemberIds, removedMemberIds);
            return callback(null, roles.after);
        });
    });
};
