    ...
    describe('Skinning', function() {
        ...
        var checkVariables = function(tenantAlias, expectedBackgroundColor, callback) {
            RestAPI.UI.getSkinVariables(globalAdminRestContext, tenantAlias, function(err, data) {
                ...
                assert.equal(data.results[0].name, 'Page background color');
                assert.ok(data.results[0].variables.length > 0);
                assert.equal(data.results[0].variables[0].type, UIConstants.variables.types.COLOR);
                assert.equal(data.results[0].variables[0].value, expectedBackgroundColor);
                callback();
            });
        };
