...
/**
 * Handles a Passport error.
 * Logs a statement to the logs or returns a 401 if the user tried
 * to log in via a disabled authentication strategy.
 *
...
var handlePassportError = module.exports.handlePassportError = function(req, res, next) {
    return function(err) {
        if (err && err.message && err.message.indexOf('Unknown authentication strategy') === 0) {
...
/**
 * Authenticates the user with the passed in strategyId.
...
var handleExternalSetup = module.exports.handleExternalSetup = function(strategyId, passportOptions, req, res, next) {
    ...
    // Get the URL where we should redirect the user towards and store it in a cookie,
...
/**
 * Validates a URL that should be used to redirect the user within OAE after authentication.
...
var handleExternalCallback = module.exports.handleExternalCallback = function(strategyId, req, res, next) {
    ...
    // Get the URL where we should redirect the user towards
    ...
    passport.authenticate(strategyId, {}, function(err, user, challenges, status) {
        if (err) {
            ...
            // The user his credentials didn't check out. This would rarely occur in a
            ...
            log().error({'challenges': challenges, 'status': status}, 'Possible tampering of external callback request detected');
            return res.send(401, 'Unauthorized');
        }

        // The user his authentication credentials are correct
