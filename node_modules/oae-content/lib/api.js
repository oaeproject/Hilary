    ...
    getContent(ctx, contentId, function(err, contentObj) {
        ...
        PrincipalsUtil.getPrincipal(ctx, contentObj.createdBy, function(err, createdBy) {
            ...
            if (!ctx.user()) {
                ...
                // Add the isViewer and isManager properties
                AuthzAPI.isAllowed(ctx.user().id, 'manager', contentId, function(err, isManager) {
                    if (err) {
                        return callback(err);
                    }
...
var _getFullContentProfileLink = function(contentObj, callback) {
    ...
    Cassandra.runQuery('SELECT link FROM Content WHERE contentId = ?', [contentObj.contentId], function (err, rows) {
        if (err) {
            return callback(err);
        }
...
/**
 * Utility function that will be used to further decorate a Sakai Docs's full content profile. 
 * @param  {Content}                        contentObj                  Content object representing the content profile that's decorated
 * @param  {Function(err, contentProfile)}  callback                    Standard callback function
 * @param  {Object}                         callback.err                Error object containing the error message
 * @param  {Content}                        callback.contentProfile     Retrieved content profile
