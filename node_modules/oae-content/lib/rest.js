 ...
 * @param  {Boolean}            expiresMax              Whether an expires header can be sent that is far in the future
 * @api private
 */
var _handleDownload = function(res, downloadInfo, expiresMax) {
    ...
    if (downloadStrategy.strategy === ContentConstants.backend.DOWNLOAD_STRATEGY_INTERNAL) {
        ...
        if (expiresMax) {
            // Add the cache headers manually as some webservers seem to struggle
            // with both setting cache headers and dealing with internal redirects
