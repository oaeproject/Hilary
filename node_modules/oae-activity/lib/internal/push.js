...
var init = (module.exports.init = function(callback) {
  ...
  MQ.declareExchange(
    ...
    QueueConstants.exchange.OPTIONS,
    function(err) {
      if (err) {
        return callback(err);
      }
...
var registerConnection = (module.exports.registerConnection = function(socket) {
  ...
  connectionInfo.authenticationTimeout = setTimeout(function() {
    if (!connectionInfo.ctx) {
      ...
      _writeResponse(connectionInfo, 0, {
        code: 400,
        msg:
          'Authentication should happen within 5 seconds after opening the socket',
      });
      socket.close();
    } else {
      log().trace(
  ...
  socket.on('close', function() {
    log().trace(
      { sid: socket.id, streams: connectionInfo.streams },
      'Closing socket',
    );
    ...
    if (todo === 0) {
      log().trace(
        { sid: socket.id },
        'Not registered for any streams, not doing anything',
      );
      return;
    }
...
var _authenticate = function(connectionInfo, message) {
  ...
  validator
    .check(data.tenantAlias, {
      code: 400,
      msg: 'A tenant needs to be provided',
    })
    .notEmpty();
  validator
    .check(data.userId, { code: 400, msg: 'A userId needs to be provided' })
    .isUserId();
  validator
  ...
  PrincipalsDAO.getPrincipal(data.userId, function(err, user) {
    if (err) {
      ...
      log().error(
        { err: err, userId: data.userId, sid: socket.id },
...
var _subscribe = function(connectionInfo, message) {
  ...
  authzHandler(connectionInfo.ctx, data.stream.resourceId, data.token, function(
    err,
  ) {
    if (err) {
    ...
    var activityStreamId = ActivityUtil.createActivityStreamId(
      data.stream.resourceId,
      data.stream.streamType,
    );
    log().trace(
      { sid: socket.id, activityStreamId: activityStreamId },
      'Registering socket for stream',
    );
    ...
    var finish = function() {
      ...
      var transformerType =
        data.format || ActivityConstants.transformerTypes.INTERNAL;
    ...
    if (connectionInfo.streams.indexOf(activityStreamId) > -1) {
      ...
      _bindQueue(activityStreamId, function(err) {
        if (err) {
          log().error(
            { sid: socket.id, err: err, activityStreamId: activityStreamId },
        ...
        connectionInfosPerStream[activityStreamId] =
          connectionInfosPerStream[activityStreamId] || [];
...
var _getAuthorizationHandler = function(activityStreamId) {
  var streamOptions = ActivityRegistry.getRegisteredActivityStreamType(
    activityStreamId,
  );
...
var _handlePushActivity = function(data, callback) {
  ...
  _.each(connectionInfosPerStream[activityStreamId], function(connectionInfo) {
    var socket = connectionInfo.socket;
    _.each(connectionInfo.transformerTypes[activityStreamId], function(
      transformerType,
    ) {
      todo++;
      ...
      var activities = clone(data.activities);
      ActivityTransformer.transformActivities(
        ...
        function(err) {
          ...
          log().trace(
            { data: msgData, sid: socket.id },
            'Pushing message to socket',
          );
 ...
 */
ActivityEmitter.on(ActivityConstants.events.ROUTED_ACTIVITIES, function(
  routedActivities,
) {
  // Iterate over each target resource
  _.each(routedActivities, function(streamTypeActivities, resourceId) {
    // For each resource, there could be a number of stream types to which it was routed (e.g., activity and notification)
    _.each(streamTypeActivities, function(activity, streamType) {
      ...
      if (selectn('push.delivery.phase', streamOptions) !== 'aggregation') {
        ...
        _push(
          ActivityUtil.createActivityStreamId(resourceId, streamType),
          data,
        );
      }
    });
