...
describe('Related content', function() {
    ...
    var globalAdminRestContent = null;
    ...
    before(function(callback) {
        ...
        globalAdminRestContent = TestsUtil.createGlobalAdminRestContext();

        RestAPI.User.getMe(camAdminRestContext, function(err, meData) {
            assert.ok(!err);

            RestAPI.User.getMe(gtAdminRestContext, function(err, meData) {
                assert.ok(!err);
     ...
     * It will create 3 content items per user, each with a different visibility setting.
     *
    ...
    var createContent = function(restContexts, tenant, callback) {
        ...
        RestAPI.Content.createLink(restContexts[0], TITLES[0], null, 'public', 'http://www.sakaiproject.org', [], [], function(err, contentObj) {
            assert.ok(!err);
            contentItems.BY_TENANT[tenant].PUBLIC.push(contentObj.id);

            RestAPI.Content.createLink(restContexts[0], TITLES[1], null, 'loggedin', 'http://www.sakaiproject.org', [], [], function(err, contentObj) {
                assert.ok(!err);
                contentItems.BY_TENANT[tenant].LOGGEDIN.push(contentObj.id);

                RestAPI.Content.createLink(restContexts[0], TITLES[2], null, 'private', 'http://www.sakaiproject.org', [], [], function(err, contentObj) {
                    assert.ok(!err);
                    contentItems.BY_TENANT[tenant].PRIVATE.push(contentObj.id);

                    RestAPI.Content.createLink(restContexts[1], TITLES[0], null, 'public', 'http://www.sakaiproject.org', [], [], function(err, contentObj) {
                        assert.ok(!err);
                        contentItems.BY_TENANT[tenant].PUBLIC.push(contentObj.id);

                        RestAPI.Content.createLink(restContexts[1], TITLES[1], null, 'loggedin', 'http://www.sakaiproject.org', [], [], function(err, contentObj) {
                            assert.ok(!err);
                            contentItems.BY_TENANT[tenant].LOGGEDIN.push(contentObj.id);

                            RestAPI.Content.createLink(restContexts[1], TITLES[2], null, 'private', 'http://www.sakaiproject.org', [], [], function(err, contentObj) {
                                assert.ok(!err);
                                contentItems.BY_TENANT[tenant].PRIVATE.push(contentObj.id);
                                callback();
                            });
    ...
    /**
     * Verifies that you need access to the base content item before you can search for related items.
     */
    it('verify access on base content item', function(callback) {
        // Anonymous can't see private/loggedin items.
        RestAPI.Search.search(anonymousCamRestContext, 'relatedcontent', [ contentItems.BY_TENANT.CAM.PRIVATE[0] ], null, function(err, results) {
            assert.ok(err);
            assert.equal(err.code, 401);
            assert.ok(!results);
            RestAPI.Search.search(anonymousCamRestContext, 'relatedcontent', [ contentItems.BY_TENANT.CAM.LOGGEDIN[0] ], null, function(err, results) {
                assert.ok(err);
                assert.equal(err.code, 401);
                assert.ok(!results);
                callback();
     ...
     *  * User A created documents with displayName alfa
     *  * User B created a document with displayName beta but without a description
     *  * User C created a document with displayName beta and a description
     *  * The displaynames alfa and beta are similar.
     * If we search for related content against alfa, we should get beta documents.
     * However, the beta-document of user C should be of a higher rank than the beta-document of user B as it has a description
     */
    it('verify description boosting', function(callback) {
        TestsUtil.generateTestUsers(camAdminRestContext, 3, function(err, users) {
            ...
            var alfa = "Cows eat grass but love to drink whiskey";
            var beta = "Cows eat weeds but love to drink vodka";

            RestAPI.Content.createLink(userA, alfa, null, 'public', 'http://www.sakaiproject.org', [], [], function(err, baseContent) {
                assert.ok(!err);

                RestAPI.Content.createLink(userB, beta, null, 'public', 'http://www.sakaiproject.org', [], [], function(err, noDescriptionContent) {
                    assert.ok(!err);
                    RestAPI.Content.createLink(userC, beta, 'description or some such', 'public', 'http://www.sakaiproject.org', [], [], function(err, withDescriptionContent) {
                        assert.ok(!err);

                        SearchTestsUtil.searchAll(anonymousCamRestContext, 'relatedcontent', [ baseContent.id ], {'_refresh': true}, function(err, results) {
                            assert.ok(!err);
    ...
    /**
     * Assume the situation:
     *  * User A created documents with displayName alfa and beta
     *  * User B created a document with displayName beta
     *  * The displaynames alfa and beta are similar.
     * If we search for related content against alfa, we should get beta documents.
     * However, the beta-document of user A should be of a higher rank than the beta-document of user B
     */
