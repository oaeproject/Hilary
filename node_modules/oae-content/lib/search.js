...
var _produceContentBodyDocuments = function(resources, callback, _documents) {
    ...
    ContentDAO.Revisions.getRevisions(resource.id, null, 1, null, function(err, revisions) {
        ...
        var _appendPreviewToBody = function(err, preview, index) {
            if (err) {
                return callback(err);
            }
            ContentUtil.getStorageBackend(null, preview.uri).get(null, preview.uri, function(err, file) {
                if (err) {
                    return callback(err);
                }
        ...
        for (var i = 1; i <= revision.previews.pageCount; i++) {
            ContentDAO.Previews.getContentPreview(revision.previewsId, util.format('page.%s.html', i), function(err, preview) {
                _appendPreviewToBody(err, preview, i);
            });
        }
    });
};
