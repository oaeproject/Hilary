...
describe('MixPanel', function() {
    ...
    describe('Content', function() {
        ...
        it('verify that an error is thrown when an invalid tenant alias is passed in', function(callback) {
            ...
            RestAPI.MixPanel.getUploadedFilesFileSizeForTenant(globalAdminRestContext, 1234, function(err, data) {
                assert.ok(err);

                // Verify that an error is thrown when an invalid tenant alias was provided
                RestAPI.MixPanel.getUploadedFilesFileSizeForTenant(globalAdminRestContext, 'invalid_tenant_alias', function(err, data) {
                    assert.ok(err);
                    callback();
                });
            });
        ...
        it('verify that only administrators of the specified tenant can request stats', function(callback) {
            ...
            RestAPI.MixPanel.getUploadedFilesFileSizeForTenant(anonymousGlobalRestContext, global.oaeTests.tenants.cam.alias, function(err, data) {
                ...
                // Verify that anonymous users are not allowed to request stats on the tenant router
                RestAPI.MixPanel.getUploadedFilesFileSizeForTenant(anonymousCamRestContext, global.oaeTests.tenants.cam.alias, function(err, data) {
                    assert.ok(err);
                    ...
                    TestsUtil.generateTestUsers(camAdminRestContext, 1, function(err, user) {
                        ...
                        // Verify that normal user are not allowed to request stats
                        RestAPI.MixPanel.getUploadedFilesFileSizeForTenant(user.restContext, global.oaeTests.tenants.cam.alias, function(err, data) {
                            assert.ok(err);
                            assert.equal(err.code, 404);

                            // Verify that administrators are allowed to request stats
                            MixPanelTestUtil.mockRequests();
                            RestAPI.MixPanel.getUploadedFilesFileSizeForTenant(camAdminRestContext, global.oaeTests.tenants.cam.alias, function(err, data) {
                                assert.ok(err);
                                assert.equal(err.code, 404);
