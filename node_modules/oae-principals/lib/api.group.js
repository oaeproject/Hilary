 ...
 * @param  {String[]}       newMemberIds            The ids of the members who were added to the group
 * @param  {Function}       callback                Standard callback function
 * @param  {Object}         callback.err            An error that occurred, if any
...
var _insertMembershipsLibraries = function(groups, newMemberIds, callback) {
    ...
    _getAllChildren(newMemberIds, [], function(err, allChildren) {
        ...
        // The principals in whose libraries we'll insert the groups
        var principalIds = allChildren.concat(newMemberIds);

        // Add all the groups to the libraries of the new members
        var done = _.after(groups.length, function() {
            return callback(null, allChildren);
        });
        _.each(groups, function(group) {
            var libraryVisibilities = LibraryAPI.Authz.resolveEffectiveLibraryVisibilities(principalIds, group);
...
var _updateMembershipsLibraries = function(group, excludePrincipals, callback) {
    ...
    _getAllChildren([group.id], excludePrincipals, function(err, allChildren) {
        ...
        // The principals in whose libraries we'll update the groups
        ...
        var libraryVisibilities = LibraryAPI.Authz.resolveEffectiveLibraryVisibilities(principalIdsToUpdate, group);
...
var _removeMembershipsLibraries = function(removedMemberIds, groups, callback) {
    ...
    _getAllChildren(removedMemberIds, [], function(err, allChildren) {
        ...
        // The principals frome whose libraries we'll remove the groups
        ...
        _.each(groups, function(group) {
            ...
            LibraryAPI.Index.remove(PrincipalsConstants.library.MEMBERSHIPS_INDEX_NAME, principalIds, group.id, lastModified, function(err) {
                if (err) {
                    log().error({
                        'err': err,
                        'group': group
...
var _getAncestors = function(group, callback) {
    ...
    AuthzAPI.getPrincipalMemberships(group.id, null, 10000, function(err, memberships) {
        if (err) {
            return callback(err);
        }
