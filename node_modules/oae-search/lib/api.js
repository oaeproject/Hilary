 ...
 */
var SearchAPI = (module.exports = new EmitterAPI.EventEmitter());
 ...
 */
var registerSearchDocumentTransformer = (module.exports.registerSearchDocumentTransformer = function(
  typeName,
  transformer,
) {
  if (searchDocumentTransformers[typeName]) {
    throw new Error(
      'Document transformer for type ' + typeName + ' already exists',
    );
  }
 ...
 */
var registerSearchDocumentProducer = (module.exports.registerSearchDocumentProducer = function(
  typeName,
  producer,
) {
  if (searchDocumentProducers[typeName]) {
    throw new Error(
      'Document producer for type ' + typeName + ' already exists',
    );
  }
 ...
 */
var registerSearch = (module.exports.registerSearch = function(
  typeName,
  queryBuilder,
  postProcessor,
) {
 ...
 */
var registerReindexAllHandler = (module.exports.registerReindexAllHandler = function(
  handlerId,
  handler,
) {
  if (reindexAllHandlers[handlerId]) {
    throw new Error(
      'Reindex-all handler with id ' + handlerId + ' already exists',
    );
  }
 ...
 */
var registerChildSearchDocument = (module.exports.registerChildSearchDocument = function(
  name,
  options,
  callback,
) {
  if (childSearchDocuments[name]) {
    var err = new Error(
      'Child search document mapping with name "' + name + '" already exists',
    );
    log().error(
      { err: err, name: name },
      'Attempted to register duplicate child search document',
    );
 ...
 */
var refreshSearchConfiguration = (module.exports.refreshSearchConfiguration = function(
  searchConfig,
  callback,
) {
 ...
 */
var search = (module.exports.search = function(
  ctx,
  searchType,
  opts,
  callback,
  ...
  if (!registeredSearch) {
    return callback({
      code: 400,
      msg: 'Search "' + searchType + '" is not a valid search type.',
    });
  }

  // Invoke the search plugin to get the query object
  registeredSearch.queryBuilder(ctx, opts, function(err, queryData) {
    if (err) {
      return callback(err);
    } else if (!queryData) {
    ...
    client.search(queryData, null, function(err, elasticSearchResponse) {
      if (err) {
        log().error(
          { err: err },
          'An unexpected error occurred performing a search',
        );
        return callback({
          code: 500,
          msg: 'An unexpected error occurred performing the search',
        });
      }

      // We pull the '_extra' field out and parse it into JSON
      _.each(elasticSearchResponse.hits.hits, function(hit) {
        try {
          hit.fields._extra[0] = JSON.parse(hit.fields._extra[0]);
        } catch (parseErr) {
          log().warn(
            { err: parseErr, hit: hit },
      ...
      SearchUtil.transformSearchResults(
        ...
        function(err, transformedResults) {
          if (err) {
            return callback(err);
          }
          ...
          return registeredSearch.postProcessor(
            ctx,
            opts,
            transformedResults,
            callback,
 ...
 * @param  {Object}     callback.err    An error that occurred, if any
 */
var postReindexAllTask = (module.exports.postReindexAllTask = function(
  ctx,
  callback,
) {
  if (!ctx.user() || !ctx.user().isGlobalAdmin()) {
    return callback({
      code: 401,
      msg: 'Only global administrator can trigger a full reindex.',
    });
  }
 ...
 * @param  {Object}             [callback.err]      An error that occurred, if any
 */
var postIndexTask = (module.exports.postIndexTask = function(
  resourceType,
  resources,
  index,
  callback,
  ...
  validator
    .check(resourceType, { code: 400, msg: 'Must specify a resource type' })
    .notEmpty();
  validator
    .check(null, { code: 400, msg: '"resources" parameter must be an array' })
    .isArray(resources);
  validator
    .check(null, { code: 400, msg: '"index" parameter must be an object' })
    .isObject(index);
  validator
  ...
  return TaskQueue.submit(
    SearchConstants.mq.TASK_INDEX_DOCUMENT,
    { resourceType: resourceType, resources: resources, index: index },
    callback,
  );
});
 ...
 * @param  {Object}     callback.err    An error that occurred, if any
 */
var postDeleteTask = (module.exports.postDeleteTask = function(
  id,
  children,
  callback,
) {
    ...
    client.deleteIndex(indexName, function(err) {
      ...
      client.createIndex(indexName, indexSettings, function(err) {
        if (err) {
          log().error(
            { err: err },
            'Error recreating index "%s" after deletion.',
            indexName,
          );
          return callback(err);
        }
...
var _ensureSearchSchema = function(callback, _names) {
  ...
  return _createChildSearchDocumentMapping(
    name,
    childSearchDocuments[name].schema,
    function(err) {
      if (err) {
        return callback(err);
      }
...
var _createChildSearchDocumentMapping = function(name, schema, callback) {
  /*!
     ...
     */
  var opts = { _parent: SearchConstants.search.MAPPING_RESOURCE };
  ...
  callback =
    ...
    function(err) {
      if (err) {
        log().error(
          { err: err, data: data },
          'Error handling reindex-all task',
        );
      }
    };
  ...
  callback =
    ...
    function(err) {
      if (err) {
        log().error(
          { err: err, data: data },
          'Error handling search document delete task.',
        );
      }
    };
...
var _deleteAll = function(deletes, callback) {
  ...
  var _handleDocumentsDeleted = function(err) {
    if (err) {
      log().error(
        { err: err, operation: del },
        'Error deleting a document from the search index',
      );
    }
...
var _handleIndexDocumentTask = function(data, callback) {
  callback =
    ...
    function(err) {
      if (err) {
        log().error(
          { err: err, data: data },
          'Error handling search indexing task.',
        );
      }
    };
  ...
  _.each(data.resources, function(resource) {
    ...
    if (data.index.children === true) {
      ...
      _.each(childSearchDocuments, function(options, documentType) {
        ...
        if (
          ...
          options.resourceTypes[data.resourceType]
        ) {
    ...
    if (data.index.resource) {
      resourcesToIndex[data.resourceType] =
        resourcesToIndex[data.resourceType] || [];
    ...
    _.chain(data.index.children)
      ...
      .each(function(documentType) {
        resourceChildrenToIndex[documentType] =
          resourceChildrenToIndex[documentType] || [];
  ...
  _produceAllResourceDocuments(resourcesToIndex, function(err, resourceDocs) {
    ...
    log().trace(
      { data: data, resourceDocs: resourceDocs },
      'Produced top-level resource docs',
    );

    _produceAllChildDocuments(resourceChildrenToIndex, function(
      err,
      childResourceDocs,
    ) {
      if (err) {
      ...
      log().trace(
        { data: data, childResourceDocs: childResourceDocs },
        'Produced child resource docs',
      );
      ...
      if (_.isEmpty(allDocs)) {
        ...
        client.index(doc['_type'], id, doc, opts, function(err) {
          if (err) {
            log().error(
              { err: err, id: id, doc: doc, opts: opts },
 ...
 * @api private
 */
var _produceAllResourceDocuments = function(
  resourcesToIndex,
  callback,
  _resourceTypes,
  _documents,
  ...
  if (searchDocumentProducer) {
    searchDocumentProducer(resourcesToIndex[resourceType], function(
      errs,
      documents,
    ) {
      ...
      _.each(errs, function(err) {
        log().error(
          { err: err },
          'Error producing search documents from resources',
        );
      });
      ...
      documents = _.map(documents, function(doc) {
        ...
        log().trace(
          { before: doc, after: newDoc },
          'Converted resource document',
        );
        return newDoc;
      });
 ...
 * @api private
 */
var _produceAllChildDocuments = function(
  ...
  if (childSearchDocumentProducer) {
    childSearchDocumentProducer(resourceChildrenToIndex[documentType], function(
      errs,
      documents,
    ) {
      // Some resources might have triggered an error. We log those here,
      ...
      _.each(errs, function(err) {
        log().error(
          { err: err },
          'Error producing child search documents from resources',
        );
      });
