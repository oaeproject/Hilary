...
var generatePreviews = module.exports.generatePreviews = function(ctx, contentObj, callback) {
    ...
    request(options, function(err, response) {
        if (err) {
            log().warn({'err': err, 'contentId': ctx.contentId}, 'An error occurred while checking if this URL allows for iframe embedding');
            ctx.addPreviewMetadata('embeddable', false);
        } else {

            // If the Content-Disposition header is set to attachment the link isn't embeddable because the browser will prompt to save the target
            var forcedDownload = response.headers['content-disposition'] && response.headers['content-disposition'].split(';')[0] === 'attachment';
        ...
        var generateThumbnail = function() {
            var imgPath = ctx.baseDir + '/webshot.png';
            // If the link target is an image just grab it instead of screenshotting it
            if (contentType && contentType.substring(0, 6) === 'image/') {
                var image = fs.createWriteStream(imgPath);
                image.on('close', function() {
                    LinkProcessorUtil.generatePreviewsFromImage(ctx, imgPath, null, callback);
                });
                request(contentObj.link).on('error', function(err){
                    log().error({'err': err, 'contentId': ctx.contentId}, 'Could not fetch an image');
                    return callback();
                }).pipe(image);
            } else {
                ...
                webshot.getImage(contentObj.link, imgPath, webshotOptions, function (err) {
                    if (err) {
                    ...
                    gm.compare(imgPath, path.resolve(__dirname, '../../../static/link/blank.png'), 0.0, function(err, isEqual, equality, raw) {
                        if (err) {
                            log().error({'err': err, 'contentId': ctx.contentId}, 'Could not compare image');
                            return callback(err);
                        }

                        log().trace({'contentId': ctx.contentId, 'equality': equality, 'raw': raw});
        ...
        var urlParts = url.parse(contentObj.link);
        ...
        if (urlParts.protocol === 'https:') {
            var httpsAccessible = (err ? false : true);
        ...
        // Otherwise we need to do an extra request
            ...
            var link = url.format(urlParts);
            _checkHttps(link, function(httpsAccessible) {
                ctx.addPreviewMetadata('httpsAccessible', httpsAccessible);
                return generateThumbnail();
            });
        }
 ...
 * @api private
 */
var _checkHttps = function(link, callback) {
    var options = {
        'url': link,
        'method': 'HEAD',
        'timeout': webshotOptions.embeddableCheckTimeout
