...
var LoodleAPI = require('oae-loodle/lib/api');
    ...
    ContentDAO.Content.getContent(contentId, function(err, contentObj) {
        ...
        AuthzPermissions.canView(ctx, contentObj, function(err) {
            ...
            if (contentObj.resourceSubType === 'loodle') {
                if (contentObj.visibility === 'private') {
                    LoodleAPI.getFromContentId(ctx, currentUser, contentObj.id, function (err, data) {
                        if (err) return callback(err);
                        contentObj.loodleData = data;

                        // We check if the user participate in the loodle or has only a lecture access
                        // (for example if a loodle is created within a folder)
                        LoodleAPI.userIsInLoodle(ctx, currentUser.id, data.id, function (err, isInLoodle) {
                            if (err) return callback(err);

                            LoodleAPI.getUser(ctx, currentUser.id, function (err, data) {
                                // We create the currentUser object only if the user is explicitely invated
                                // to participate to the loodle
                                if (isInLoodle) {
                                    contentObj.loodleData.currentUser = data;
                                }
                    ...
                    // User connected
                    // If the user connected has been explicitly invited to this loodle (or created it)
                    // we specify his/her data as the current user of the loodle so that he can interact with it
                    if (currentUser) {

                        LoodleAPI.getFromContentId(ctx, currentUser, contentObj.id, function (err, loodle) {
                            if (err) return callback(err);

                            // Check if the user participate to the loodle and add him/her as the current user if
                            // that's the case
                            LoodleAPI.userIsInLoodle(ctx, currentUser.id, loodle.id, function (err, isInLoodle) {
                                if (err) return callback(err);

                                contentObj.loodleData = loodle;
                                ...
                                LoodleAPI.getUser(ctx, currentUser.id, function (err, user) {
                                    if (err) return callback(err);

                                    contentObj.loodleData.currentUser = user;
                                    ContentUtil.augmentContent(ctx, contentObj);
                    ...
                    // Otherwise just get the loodle data
                    else {
                        // Just get the loodle data
                        LoodleAPI.getPublicLoodleFromContentId(ctx, contentObj.id, function (err, data) {
                            if (err) return callback(err);
...
var createDoodle = module.exports.createDoodle = function (ctx, displayName, description, visibility, additionalMembers, folders, callback) {

    callback = callback || function() {};

    // Setting content to default if no visibility setting is provided
    visibility = visibility || Config.getValue(ctx.tenant().alias, 'visibility', 'loodle');

    var contentId = _generateContentId(ctx.tenant().alias);
    var revisionId = _generateRevisionId(contentId);
    var currentUser = ctx.user();

    // Create loodle in the loodle application
    LoodleAPI.create(ctx, currentUser, contentId, displayName, description, visibility, additionalMembers, folders, function (err) {
        if (err) return callback(err);

        // Create content in oae
        _createContent(ctx, contentId, revisionId, 'loodle', displayName, description, visibility, additionalMembers, folders, {}, {}, function(err, content, revision, memberChangeInfo) {
            if (err) {
                return callback(err);
            }
...
var _createContent = function(ctx, contentId, revisionId, resourceSubType, displayName, description, visibility, roles, folderIds, otherValues, revisionData, callback) {
    ...
    validator.check(resourceSubType, {'code': 400, 'msg': 'A valid resourceSubType must be provided. This can be "file", "collabdoc", "link" or "loodle"'}).isIn(ContentConstants.resourceSubTypes);
    validator.check(null, {'code': 401, 'msg': 'You have to be logged in to be able to create a content item'}).isLoggedInUser(ctx);
...
var deleteContent = module.exports.deleteContent = function(ctx, contentId, callback) {
    ...
    _canManage(ctx, contentId, function(err, contentObj) {
        ...
        ContentDAO.Content.deleteContent(contentObj, function(err, members) {
            ...
            if (contentObj.resourceSubType === 'loodle') {
                LoodleAPI.delete(ctx, currentUser, contentObj.id, function (err) {
                    if (err)
                        return callback(err);
    ...
    getContent(ctx, contentId, function(err, content) {
        ...
        ResourceActions.setRoles(ctx, content, changes, function(err, memberChangeInfo) {
            ...
            if (content.resourceSubType === 'loodle') {
                ...
                LoodleAPI.updateMembers(ctx, contentId, changes, function (err) {
                    if (err)
                        return callback(err);
...
var updateContentMetadata = module.exports.updateContentMetadata = function(ctx, contentId, profileFields, callback) {
    ...
    _canManage(ctx, contentId, function(err, oldContentObj) {
        ...
        ContentDAO.Content.updateContent(oldContentObj, profileFields, true, function(err, newContentObj) {
            ...
            LoodleAPI.updateVisibility(ctx, contentId, profileFields, function (err) {
                if (err) return callback(err);
