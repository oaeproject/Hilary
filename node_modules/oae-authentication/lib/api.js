 ...
 */
var getOrCreateUser = function(context, username, locale, timezone, firstName, lastName, displayName, callback) {
    // Check if this user already exists in our DB.
    PrincipalAPI.getTenantUser(context, username, function(err, user) {
        if (err && err.code === 404) {
            // new user, create an account.
            var password = crypto.randomBytes(128).toString('hex');
            PrincipalAPI.createUser(context, username, password, "public", locale, timezone, firstName, lastName, displayName, function(err, principal_id) {
                if (err) {
                    return callback(err);
                } else {
                    // Get the meta data for this user.
                    context = new Context(context.tenant(), new User(context.tenant().alias, principal_id));
                    PrincipalAPI.getUser(context, principal_id, callback);
                }
            });
        } else if (err) {
            return callback(err);
...
module.exports.LocalCallbackWrapper = function(tenant) {
    var that = {};
...
module.exports.LocalStrategyWrapper = function(tenant, validate) {
    var options = {
        'passReqToCallback': true,
        'failureRedirect': false
    };

    LocalStrategy.call(this, options, function(req, username, password, done) {
        PrincipalAPI.checkPassword(req.tenant, username, password, function(err, basic_user) {
            if (err && err.code !== 401) {
                return done(err);
            ...
            req.ctx = new Context(tenant, new User(tenant.alias, basic_user.id));
            PrincipalAPI.getUser(req.ctx, basic_user.id, function(err, user) {
                if (err) {
                    return done(err);
                }
    ...
    this.name = 'local-' + tenant.alias;
};
util.inherits(module.exports.LocalStrategyWrapper, LocalStrategy);
