 ...
 * @api private
 */
var _getContentWithPreviews = function(
  folder,
  callback,
  _contentWithPreviews,
  _start,
) {
  ...
  FoldersDAO.getContentItems(
    ...
    function(err, contentItems, nextToken) {
      if (err) {
        return callback(err);
      }
      ...
      _contentWithPreviews = _.chain(contentItems)
        ...
        .filter(function(contentItem) {
          ...
          // library)
          var targetBucketVisibility = LibraryAPI.Authz.resolveLibraryBucketVisibility(
            folder.id,
            contentItem,
          );
          var targetBucketVisibilityPriority = AuthzConstants.visibility.ALL_PRIORITY.indexOf(
          ...
          var implicitBucketVisibility = folder.visibility;
          var implicitBucketVisibilityPriority = AuthzConstants.visibility.ALL_PRIORITY.indexOf(
          ...
          return (
            targetBucketVisibilityPriority <= implicitBucketVisibilityPriority
          );
        })
      ...
      if (!nextToken) {
        ...
        return _getContentWithPreviews(
          folder,
          callback,
          _contentWithPreviews,
          nextToken,
...
var _generatePreviews = function(folder, contentItems, callback) {
  ...
  _retrieveThumbnails(ctx, contentItems.slice(), function(err, paths) {
    if (err) {
      return callback(err);
    }
      ...
      _removeOldPreviews(ctx, folder, function(err) {
        if (err) {
          log().error(
            ...
            'Unable to remove the old folder previews',
          );
          return callback(err);
        }

        _storeNewPreviews(ctx, folder, thumbnail, wide, function(
          err,
          thumbnailUri,
          wideUri,
        ) {
          if (err) {
            log().error(
              { err: err, folderId: folder.id },
              'Unable to store the new folder previews',
            );
            return callback(err);
          }
...
var _createMontages = function(paths, callback) {
  ...
  _createMontage(
    ...
    function(err, thumbnail) {
      ...
      _createMontage({ columns: 4, rows: 2 }, wideSize, paths.slice(), function(
        err,
        wide,
      ) {
        if (err) {
 ...
 * @api private
 */
var _retrieveThumbnails = function(ctx, contentItems, callback, _paths) {
  _paths = _paths || [];
  ...
  var contentItem = contentItems.shift();
...
var _removeOldPreview = function(ctx, folder, type, callback) {
  if (folder.previews && folder.previews[type]) {
    ContentUtil.getStorageBackend(ctx).remove(
      ctx,
...
var _storeNewPreviews = function(ctx, folder, thumbnail, wide, callback) {
  ...
  ContentUtil.getStorageBackend(ctx).store(
    ctx,
    thumbnail,
    { filename: filename, resourceId: folder.id },
    function(err, thumbnailUri) {
      if (err) {
        return callback(err);
      }
