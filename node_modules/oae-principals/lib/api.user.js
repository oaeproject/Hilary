...
module.exports.createUser = function(ctx, username, password, visibility, locale, timezone, firstName, lastName, displayName, callback) {
    ...
    var id = AuthzUtil.toUuid('u', ctx.tenant().alias, username);
...
module.exports.updateUser = function(ctx, userId, profileFields, callback) {
    ...
    // Check that at least one of the accepted values have been provided
    var acceptedFieldNames = ['visibility', 'user_first_name', 'user_last_name', 'user_display_name', 'locale', 'timezone'];
    var fieldNames = profileFields ? _.keys(profileFields) : [];
    validator.check(fieldNames.length, {'code': 400, 'msg': 'You should at least specify one updated value'}).min(1);
    fieldNames.forEach(function(fieldName) {
        validator.check(fieldName, {'code': 400, 'msg': fieldName + ' is not a recognized user profile field'}).isIn(acceptedFieldNames);
    });
    ...
    module.exports.getUser(ctx, userId, function(err, user) {
        ...
        if (ctx.user().id === userId || ctx.user().isAdmin(principalResource.tenantId)) {
            var q = Cassandra.constructUpdateCQL('Principals', profileFields, 'principal_id', userId);
...
module.exports.getTenantUser = function(ctx, username, callback) {
    var user_id = AuthzUtil.toUuid('u', ctx.tenant().alias, username);
    module.exports.getUser(ctx, user_id, callback);
};
 ...
 */
module.exports.getBasicProfile = function(ctx, user_id, callback) {
    // Anonymous is a bit of a special case.
    module.exports.getUser(ctx, user_id, function(err, user) {
        if (err) {
            return callback(err);
        }
        var data = {
 ...
 */
module.exports.setUserVisibility = function(ctx, user_id, visibility, callback) {
    callback = callback || function() {};
    ...
    if (ctx.user().id === user_id || ctx.user().isAdmin(principalResource.tenantId)) {
        module.exports.getUser(ctx, user_id, function(err, user) {
            if (err) {
                return callback(err);
            }
            Cassandra.runQuery('UPDATE Principals USING CONSISTENCY QUORUM SET visibility=? WHERE principal_id = ?', [visibility, user_id], callback);
        });
    } else {
 ...
 */
module.exports.changePassword = function(ctx, user_id, oldPassword, newPassword, callback) {
 ...
 * @param  {User}           principal_id    The user that needs the flag
 * @param  {Boolean}        isAdmin         Flag that indicates whether the provided user should be a tenant admin.
 * @param  {Function(err)}  callback        Callback method
 * @param  {Object}         callback.err    Error object.
 */
module.exports.setTenantAdmin = function(ctx, principal_id, isAdmin, callback) {
    ...
    if (ctx.user() && (ctx.user().isTenantAdmin(principalResource.tenantId) || ctx.user().isGlobalAdmin())) {
        _setAdmin(ctx, 'admin:tenant', isAdmin, principal_id, callback);
    } else {
 ...
 * @param  {Object}         callback.err    Error object.
 */
module.exports.setGlobalAdmin = function(ctx, principal_id, isAdmin, callback) {
    if (ctx.user() && ctx.user().isGlobalAdmin()) {
        _setAdmin(ctx, 'admin:global', isAdmin, principal_id, callback);
    } else {
        return callback({'code': 401, 'msg': 'You do not have sufficient rights to make someone an admin.'});
    }
};
 ...
 * @private
 */
var _setAdmin = function(ctx, adminType, isAdmin, principal_id, callback) {
    ...
    module.exports.getUser(ctx, principal_id, function(err, user) {
        ...
        Cassandra.runQuery("UPDATE Principals USING CONSISTENCY QUORUM SET ?=? WHERE principal_id = ?", [adminType, isAdmin, principal_id], callback);
    });
};
