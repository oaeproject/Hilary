...
var runPagedQuery = module.exports.runPagedQuery = function(tableName, keyColumnName, keyColumnValue, rangeColumnName, start, limit, opts, callback) {
    ...
    var itemsToFetch = limit + 1;

    var cql = util.format('SELECT * FROM "%s" WHERE "%s" = ? AND "%s" %s ? AND "%s" %s ?', tableName, keyColumnName, rangeColumnName, startOperator, rangeColumnName, endOperator);
    var params = [keyColumnValue, start, opts.end];

    if (opts.reversed) {
        cql += util.format(' ORDER BY "%s" DESC', rangeColumnName);
    }

    cql += util.format(' LIMIT %s', itemsToFetch);
    ...
    runQuery(cql, params, function(err, rows) {
        ...
        var results = null;
        var nextToken = null;
        var startMatched = (rows[0].get(rangeColumnName).value === start);

        if (startMatched) {
            // If we had a match on the start element, we fetch rows starting from the second result
            results = rows.slice(1);
            nextToken = (results.length === limit) ? _.last(results).get(rangeColumnName).value : null;
        } else {
            // If there was no match on the starting, simply select from the start
            results = rows.slice(0, limit);
            nextToken = (rows.length === itemsToFetch) ? _.last(results).get(rangeColumnName).value : null;
        }

        return callback(null, results, nextToken, startMatched);
    });
};
