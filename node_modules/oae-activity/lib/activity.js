...
ActivityAPI.registerActivityStreamType('activity', {
  ...
  authorizationHandler: function(ctx, resourceId, token, callback) {
    ...
    if (ctx.user() && ctx.user().isAdmin(resource.tenantAlias)) {
      ...
      return _authorizeDiscussionActivityStream(
        ctx,
        resourceId,
        token,
        callback,
      ...
      // Jitsi streams
    } else if (resource.resourceType === 'm') {
...
ActivityAPI.registerActivityStreamType('message', {
  ...
  authorizationHandler: function(ctx, resourceId, token, callback) {
    ...
    if (ctx.user() && ctx.user().isAdmin(resource.tenantAlias)) {
      ...
      // Folder streams
    } else if (resourceId[0] === 'f') {
      return _authorizeFolderActivityStream(ctx, resourceId, token, callback);

      // Meeting streams
    } else if (resourceId[0] === 'm') {
      return _authorizeJitsiActivityStream(ctx, resourceId, token, callback);
...
ActivityAPI.registerActivityStreamType('notification', {
  ...
  authorizationHandler: function(ctx, resourceId, token, callback) {
    if (!AuthzUtil.isUserId(resourceId)) {
      return callback({
        code: 400,
        msg: 'Only users can have notification streams',
      });
    }
    ...
    if (!ctx.user()) {
      ...
      return callback({
        code: 401,
        msg: 'You can only request your own notification stream',
      });
    }
...
var _authorizeGroupActivityStream = function(ctx, groupId, token, callback) {
  if (!ctx.user()) {
    return callback({
      code: 401,
      msg: 'Must be a member of a group to see its activity stream',
    });
 ...
 * @api private
 */
var _authorizeContentActivityStream = function(
  ctx,
  contentId,
  token,
  callback,
 ...
 * @api private
 */
var _authorizeDiscussionActivityStream = function(
  ctx,
  discussionId,
  token,
  callback,
  ...
  if (_.isObject(token)) {
    ...
    DiscussionsAPI.Discussions.getDiscussion(ctx, discussionId, function(
      err,
      discussion,
    ) {
      if (err) {
...
var _authorizeFolderActivityStream = function(ctx, folderId, token, callback) {
  if (_.isObject(token)) {
    ...
    FoldersAPI.getFolder(ctx, folderId, function(err, folder) {
      if (err) {
        return callback(err);
      }
 ...
 * @api private
 */
var _authorizeJitsiActivityStream = function(ctx, meetingId, token, callback) {
  if (_.isObject(token)) {
    if (
      !Signature.verifyExpiringResourceSignature(
        ctx,
    ...
    MeetingAPI.getMeeting(ctx, meetingId, function(err, meeting) {
      if (err) {
        return callback(err);
      }
