...
describe('Inviting guests', function() {
     ...
     * Test that verifies that guests can log in on the guests tenant with a local authentication strategy
     */
    it('verify guests on regular tenants with SSO are associated with existing users', function(callback) {
        TenantTestUtil.generateTestTenants(globalAdminRestContext, 1, function(tenant) {
            var domain = util.format('%s.%s', TestsUtil.generateRandomText(1), TestsUtil.generateRandomText(1));
            var domainRegex = util.format('^%s$', domain);
            ConfigTestUtil.updateConfigAndWait(globalAdminRestContext, tenant.alias, {'oae-tenants/domains/email': domainRegex}, function(err) {
    ...
    it('verify guests on regular tenants without SSO can associate a local authentication strategy', function(callback) {
        TestsUtil.setupMultiTenantPrivacyEntities(function(publicTenant0, publicTenant1, privateTenant0, privateTenant1) {
            ...
            var domainRegex = util.format('^%s$', domain);
            var config = {};
            config['oae-tenants/domains/email'] = domainRegex;

            // Enable local authentication, but disable all the others strategies
            config['oae-authentication/local/enabled'] = true;
            config['oae-authentication/twitter/enabled'] = false;
            config['oae-authentication/facebook/enabled'] = false;
            config['oae-authentication/google/enabled'] = false;
            config['oae-authentication/cas/enabled'] = false;
            config['oae-authentication/shibboleth/enabled'] = false;
            ConfigTestUtil.updateConfigAndWait(globalAdminRestContext, publicTenant0.tenant.alias, config, function(err) {
                assert.ok(!err);

                // Create a piece of content and share it with a guest
                var email = 'foo@' + domain;
                RestAPI.Content.createLink(publicTenant0.publicUser.restContext, 'Apereo', 'Apereo', 'private', 'http://www.apereo.org', [], [email], [], function(err, link) {
                    assert.ok(!err);
                });
