...
var createPad = (module.exports.createPad = function(contentId, callback) {
  ...
  client.createGroupIfNotExistsFor(args, function(err, groupData) {
    if (err) {
      log().error(
        { err: err, contentId: contentId, etherpad: client.options.host },
    ...
    log().trace(
      { contentId: contentId, groupID: groupData.groupID },
      'Creating etherpad group pad',
    );
...
var getHTML = (module.exports.getHTML = function(contentId, padId, callback) {
  ...
  client.getHTML({ padID: padId }, function(err, data) {
    if (err) {
      ...
      return callback({
        code: 500,
        msg: 'Could not grab the HTML from etherpad',
      });
    }
 ...
 * @param  {Object}     callback.err    An error that occurred, if any
 */
var setHTML = (module.exports.setHTML = function(
  contentId,
  padId,
  html,
  callback,
  ...
  try {
    ...
    log().error(
      { err: err, html: html, contentId: contentId },
  ...
  client.setHTML({ padID: padId, html: html }, function(err, data) {
    if (err) {
      ...
      return callback({
        code: 500,
        msg: 'Could not set the html on the etherpad instance',
      });
    }
...
var joinPad = (module.exports.joinPad = function(ctx, contentObj, callback) {
  ...
  client.createAuthorIfNotExistsFor(args, function(err, author) {
    if (err) {
      ...
      return callback({
        code: 500,
        msg: 'Could not create an author in the etherpad system',
      });
    }
    ...
    client.createSession(session, function(err, data) {
      if (err) {
        ...
        return callback({
          code: 500,
          msg: 'Could not create an etherpad session for this user',
        });
      }
 ...
 */
var getOnlineAuthors = (module.exports.getOnlineAuthors = function(
  contentId,
  padId,
  callback,
) {
  ...
  client.padUsers({ padID: padId }, function(err, data) {
    if (err) {
      log().error(
        { err: err, contentId: contentId, padId: padId },
        'Could not get the online users of a pad',
      );
      return callback({
        code: 500,
        msg: 'Could not get the online users of a pad',
      });
    }
 ...
 */
var getPadUrl = (module.exports.getPadUrl = function(
  contentObj,
  userId,
  sessionId,
  authorId,
