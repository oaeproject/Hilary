...
// Save the date of the last connection into Cassandra 
AuthenticationAPI.on(AuthenticationConstants.events.USER_LOGGED_IN, function(user, strategyName) {
    PrincipalsDAO.updateLastLogin(user, Date.now(), function(err) {
        if(err) {
            return log().error({'user': user}, 'Error saving the user last connection date into cassandra');
        }
    });
});
