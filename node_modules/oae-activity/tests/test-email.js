...
describe('Activity Email', function() {
    ...
    /**
     * Test that verifies that the default tenant timezone is used when a user doesn't specify one
     * We test this by creating 24 tenants, each with a different timezone (1 hour difference between two zones),
     * creating a user in each tenant, triggering an activity for all 24 users, collecting the emails and asserting only one received an e-mail.
     */
    it('verify sending emails to users with no configured timezone defaults to the configured tenant default timezone', function(callback) {
        // Enable daily mails
        refreshConfiguration(true, false, {}, function() {
            ...
            var createTenant = function() {
                ...
                TestsUtil.createTenantWithAdmin(alias, host, function(err, tenant, restContext, user) {
                    ...
                    RestAPI.User.updateUser(restContext, user.id, profile, function(err) {
                        ...
                        timezone++;
                        var zone = 'Etc/GMT' + ((timezone < 0) ? timezone : ('+' + timezone));
                        ConfigTestUtil.updateConfigAndWait(restContext, null, {'oae-principals/user/timezone': zone}, function(err) {
                            assert.ok(!err);
    ...
    /**
     * Test that verifies that marking notifications only clears the activity stream when the user’s email preference is set to immediate.
     */
                    ...
                    ActivityTestsUtil.collectAndGetActivityStream(nico.restContext, null, null, function(err, activityStream) {
                        ...
                        ActivityAPI.once(ActivityConstants.events.UPDATED_USER, function() {
                            ...
                            EmailTestsUtil.collectAndFetchEmails(function(messages) {
                                ...
                                // as that moves the hour on which the daily email gets collected. Since email gets scheduled
                                // for an hour when the activity gets triggered, we would miss it. Therefor we do a manual check
                                // if he is queued for the correct slot
                                ActivityEmail.collectMails(0, 'daily', config.mail.daily.hour, function(err, isEmpty, mailedUsers) {
                                    assert.ok(!err);
                                    assert.ok(_.find(mailedUsers, function(user) { return (user.id === nico.user.id); }));
                                    return callback();
                                });
                            });
                        });
     ...
     *  - Activities can only aggregate when they happen within 24 hours of each other (aggregateMaxExpiry = 1 day)
                                ...
                                setTimeout(RestAPI.Content.createComment, 1100, nico.restContext, contentObj.id, 'Comment C', null, function(err, commentC) {
                                    ...
                                    RestAPI.Content.createComment(branden.restContext, contentObj.id, 'Comment D', null, function(err, commentD) {
                                        ...
                                        EmailTestsUtil.collectAndFetchEmails(function(messages) {
                                            ...
                                            // 1 activity
                                            assert.equal(simongMessage['_message'].html.match(/class="activity"/g).length, 1);
                                            // 1 content item that was commented on
                                            assert.equal(simongMessage['_message'].html.match(/class="thumbnail thumbnail-resource"/g).length, 1);
                                            // 4 comments
