 ...
 *
 * @param  {Context}    ctx                 Standard context object, representing the currently logged user and its tenant
 * @param  {String}     contentId           The ID of the content to get the comment from
 ...
 * @param  {Object}     callback.err        Error object containing the error message
 * @param  {Object}     callback.comment    The retrieved comment object
 * @api private
 */
var _getComment = module.exports._getComment = function(ctx, contentId, created, callback) {
    Cassandra.runQuery('SELECT ? FROM Comments USING CONSISTENCY QUORUM WHERE contentId = ?', [created, contentId], function (err, rows) {
        if (err) {
            return callback(err);
        }

        var comment = Cassandra.rowToHash(rows[0]);

        Cassandra.runQuery('SELECT * FROM CommentsByCommentId USING CONSISTENCY QUORUM WHERE commentId = ?', [comment[created]], function (err, detailRows) {

            // Get comment from the result set
            comment = Cassandra.rowToHash(detailRows[0]);

            // Get information on the commenter
            var userId = detailRows[0].get('commenter').value;
            PrincipalsUtil.getPrincipal(ctx, userId, function(err, principal) {
                comment = new Comment(comment.comment, comment.commentId, principal, created, comment.lastModified, comment.level, comment.replyTo);
...
/**
 * Posts a comment on the specified content item. Returns an error if saving the comment goes wrong.
 *
 * @param  {Context}    ctx             Standard context object, representing the currently logged user and its tenant
 * @param  {String}     contentId       The ID of the content to place the comment on
 * @param  {String}     comment         The comment to place on the content item
 * @param  {Function}   callback        Standard callback function takes argument `err`
 * @param  {Object}     callback.err    Error object containing the error message
 */
var postComment = module.exports.postComment = function(ctx, contentId, comment, callback) {
    callback = callback || function() {};

    // Parameter validation
    var validator = new Validator();
    validator.check(contentId, {'code': 400, 'msg': 'A content id must be provided'}).not('null');
    validator.check(contentId, {'code': 400, 'msg': 'A content id must be provided'}).notEmpty();
    validator.check(comment, {'code': 400, 'msg': 'A comment must be provided'}).notEmpty();
    ...
    var commentId = AuthzUtil.toId('c', ctx.tenant().alias, ShortId.generate());

    // Add comment to Comments CF
    var parameters = {};
    parameters[Date.now() + '|'] = commentId;
    var q = Cassandra.constructUpsertCQL('Comments', 'contentId', contentId, parameters, 'QUORUM');
    Cassandra.runQuery(q.query, q.parameters, function(err) {
        ...
        // Add comment to CommentsByCommentId CF
        q = Cassandra.constructUpsertCQL('CommentsByCommentId', 'commentId', commentId, {'comment': comment, 'commenter': ctx.user().id}, 'QUORUM');
        Cassandra.runQuery(q.query, q.parameters, callback);
    });
};

/**
 * Update a comment on the specified content item. Returns an error if saving the comment goes wrong.
 *
 * @param  {Context}    ctx             Standard context object, representing the currently logged user and its tenant
 * @param  {String}     contentId       The ID of the content to update the comment on
 * @param  {String}     created         The creation date of the comment to update
 * @param  {String}     comment         The updated comment for the content item
 * @param  {Function}   callback        Standard callback function takes argument `err`
 * @param  {Object}     callback.err    Error object containing the error message
 */
var updateComment = module.exports.updateComment = function(ctx, contentId, created, comment, callback) {
    callback = callback || function() {};
    ...
    validator.check(contentId, {'code': 400, 'msg': 'A content id must be provided'}).not('null');
    validator.check(contentId, {'code': 400, 'msg': 'A content id must be provided'}).notEmpty();
    validator.check(created, {'code': 400, 'msg': 'A comment id must be provided'}).not('null');
    validator.check(created, {'code': 400, 'msg': 'A comment id must be provided'}).notEmpty();
    validator.check(comment, {'code': 400, 'msg': 'A comment must be provided'}).notEmpty();
    ...
    _getComment(ctx, contentId, created, function(err, commentToUpdate) {
        q = Cassandra.constructUpsertCQL('CommentsByCommentId', 'commentId', commentToUpdate.commentId, {'comment': comment, 'lastModified': new Date().getTime()}, 'QUORUM');
        Cassandra.runQuery(q.query, q.parameters, callback);
    });
};

/**
 * Posts a reply to a comment comment on the specified content item. Returns an error if saving the comment goes wrong.
 *
 * @param  {Context}    ctx             Standard context object, representing the currently logged user and its tenant
 * @param  {String}     contentId       The ID of the content to place the comment on
 * @param  {String}     created         The creation date of the comment to reply to
 * @param  {String}     comment         The reply on a comment to place on the content item
 * @param  {Function}   callback        Standard callback function takes argument `err`
 * @param  {Object}     callback.err    Error object containing the error message
 */
var postReply = module.exports.postReply = function(ctx, contentId, created, comment, callback) {
    callback = callback || function() {};
    ...
    _getComment(ctx, contentId, created, function(err, commentReplyingTo) {
        if (err) {
            return callback(err);
        }
        ...
        // Add comment to Comments CF
        var parameters = {};
        parameters[created.split('|')[0] + '#' + Date.now() + '|'] = commentId;
        var q = Cassandra.constructUpsertCQL('Comments', 'contentId', contentId, parameters, 'QUORUM');
        Cassandra.runQuery(q.query, q.parameters, function(err) {
            ...
            // Add comment to CommentsByCommentId CF
            var level = parseInt(commentReplyingTo.level);
            q = Cassandra.constructUpsertCQL('CommentsByCommentId', 'commentId', commentId, {
                'comment': comment,
 ...
 * @param  {Function}   callback             Standard callback function takes argument `err`
 * @param  {Object}     callback.err         Error object containing the error message
 * @param  {Object[]}   callback.comments    An array of Comment objects
 */
var getComments = module.exports.getComments = function(ctx, contentId, start, limit, callback) {
    ...
    validator.check(limit, {'code': 400, 'msg': 'A valid limit should be passed in'}).notEmpty();

    if (validator.hasErrors()) {
        return callback(validator.getFirstError());
    }

    // commentArr will be returned to the user
    var commentArr = [];

    // Arrays used to keep data about the comments before being aggregated into a Comment model and returned
    var comments = [];
    var timestamps = [];
    ...
    // Select first X comments from the Comments CF
    Cassandra.runQuery('SELECT FIRST ' + paging.limit + ' REVERSED ?..\'\' FROM Comments USING CONSISTENCY QUORUM WHERE contentId IN (?)', [paging.start, contentId], function (err, pagedRows) {
        if (!pagedRows[0].count) {
            return callback(null, []);
        }
        // Get detailed comment information
        var commentIds = [];
        var i = 0;
        pagedRows[0].forEach(function(name, value) {
            if (name !== 'contentId' && i < paging.limit) {
                i++;
        ...
        Cassandra.runQuery('SELECT * FROM CommentsByCommentId USING CONSISTENCY QUORUM WHERE commentId IN (?)', [commentIds], function (err, detailRows) {

            // Add comments to the Array to return
            for (var i = 0; i < detailRows.length; i++) {
                comments.push(Cassandra.rowToHash(detailRows[i]));
            }

            // Get information on the commenters
            var userIds = [];
            detailRows.forEach(function(row){
                userIds.push(row.get('commenter').value);
            });

            PrincipalsUtil.getPrincipals(ctx, userIds, function(err, principals) {
                // Aggregate the data to return to the user
                for (var i = 0; i < detailRows.length; i++) {
                    var comment = new Comment(comments[i].comment, comments[i].commentId, principals[comments[i].commenter], timestamps[i], comments[i].lastModified, comments[i].level, comments[i].replyTo);
 ...
 *
 * @param  {Context}    ctx             Standard context object, representing the currently logged user and its tenant
 * @param  {String}     contentId       The ID of the content to remove the comment from
 * @param  {String}     created         The creation date of the comment
 * @param  {Function}   callback        Standard callback function takes argument `err`
 * @param  {Object}     callback.err    Error object containing the error message
 */
var deleteComment = module.exports.deleteComment = function(ctx, contentId, created, callback) {
    // Parameter validation
    var validator = new Validator();
    validator.check(contentId, {'code': 400, 'msg': 'A content id must be provided'}).not('null');
    validator.check(contentId, {'code': 400, 'msg': 'A content id must be provided'}).notEmpty();
    ...
    _getComment(ctx, contentId, created, function(err, comment) {

        if (err) {
            return callback(err);
        }

        Cassandra.runQuery('DELETE ? FROM Comments WHERE contentId = ?', [created, contentId], function(err) {

            if (err) {
                return callback(err);
            }

            Cassandra.runQuery('DELETE FROM CommentsByCommentId WHERE commentId = ?', [comment.created], function(err) {
                callback(err);
            });
        });
    });
