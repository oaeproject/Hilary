...
describe('Content', function() {
    ...
    var tenant2 = new Tenant('gt', 'Georgia Tech', 2002);
    ...
    var setUpUsers = function(callback) {
        ...
        var contexts = [new Context(tenant1, new User(tenant1, 'u:cam:nm417-' + r, 'public', 'en_GB', 'Europe/London', 'Nicolaas Matthijs')),
                        new Context(tenant1, new User(tenant1, 'u:cam:simong-' + r, 'loggedin', 'en_GB', 'Europe/London', 'Simon Gaeremynck')),
                        new Context(tenant1, new User(tenant1, 'u:cam:bp323-' + r, 'private', 'en_GB', 'Europe/London', 'Bert Pareyn')),
                        new Context(tenant1, new User(tenant1, 'u:cam:branden-' + r, 'private', 'en_GB', 'Europe/London', 'Branden Visser')),
                        new Context(tenant1, new User(tenant1, 'u:cam:arwhyte-' + r, 'public', 'en_GB', 'Europe/London', 'Anthony Whyte')),
                        new Context(tenant2, new User(tenant2, 'u:gt:stuart-' + r, 'public', 'en_GB', 'Europe/London', 'Stuart Freeman'))];
                        
        var loginIds = [
            new LoginId('local', tenant1.alias, 'nm417-' + r),
            new LoginId('local', tenant1.alias, 'simong-' + r),
            new LoginId('local', tenant1.alias, 'bp323-' + r),
            new LoginId('local', tenant1.alias, 'branden-' + r),
            new LoginId('local', tenant1.alias, 'arwhyte-' + r),
            new LoginId('local', tenant2.alias, 'stuart-' + r)
          ];
          
        var done = 0;
        var createUserDone = function(err, userId, index) {
            if (err) {
                throw err;
            }
            done++;
        ...
        for (var u = 0; u < contexts.length; u++) {
            ...
            (function() {
                // need to scope the index in a function here so we don't use the outer-context 'u' after it's already fully incremented
                var index = u;
                PrincipalsAPI.createUser(contexts[u], loginIds[u], 'password' + u, user.displayName, opts, function(err, userId) {
                    createUserDone(err, userId, index);
                });
            })();
        }
    };
     ...
     */ 
     var setUpGroups = function(contexts, callback) {
        var r = Math.floor(Math.random() * 10000);
        var groups = [new Group(contexts[0].tenant(), null, 'ui-team-' + r, 'UI Dev Team ' + r, 'UI Dev Group', 'public', 'yes'),
                      new Group(contexts[3].tenant(), null, 'backend-team-' + r, 'Back-end Dev Team ' + r, 'Back-end Dev Group', 'public', 'yes'),
                      new Group(contexts[4].tenant(), null, 'oae-team-' + r, 'OAE Team ' + r, 'OAE Team Group', 'public', 'yes')];
        // Create UI Dev Group
        // Make Bert a member
        var bertMember = {};
        bertMember[contexts[2].user().id] = 'manager';
        PrincipalsAPI.createGroup(contexts[0], groups[0].alias, groups[0].name, groups[0].description, 'public', 'yes', bertMember, {}, function(err, principalId) {
            groups[0].id = principalId;
            // Create Back-end Dev Group
            // Make Simon a member
            var simonMember = {};
            simonMember[contexts[1].user().id] = 'member';
            PrincipalsAPI.createGroup(contexts[3], groups[1].alias, groups[1].name, groups[1].description, 'public', 'yes', simonMember, {}, function(err, principalId) {
                ...
                members[contexts[5].user().id] = 'member';
                PrincipalsAPI.createGroup(contexts[4], groups[2].alias, groups[2].name, groups[2].description, 'public', 'yes', members, {}, function(err, principalId) {
     ...
     */
    var checkPieceOfContent = function(ctx, libraryToCheck, contentObj, expectAccess, expectManager, expectInLibrary, callback) {
        ...
        ContentAPI.getContent(ctx, contentObj.contentId, function(err, retrievedContent) {
            if (expectAccess) {
                ...
                assert.equal(retrievedContent.createdBy, contentObj.createdBy);
            ...
            ContentAPI.hasAccess(ctx, contentObj.contentId, function(err, hasAccess) {
                ...
                ContentAPI.canManage(ctx, contentObj.contentId, function(err, canManage) {
                    ...
                    // Check if the getFullContentProfile function brings back the same information
                    ContentAPI.getFullContentProfile(ctx, contentObj.contentId, function(err, contentProfile) {
                        if (expectAccess) {
                            assert.ok(!err);
                            assert.equal(contentProfile.isManager, expectManager);
                        } else {
                            assert.ok(err);
                        }
     ...
     * and valid way
     */
    it('Test get content', function(callback) {
        setUpUsers(function(contexts) {
            // Create a piece of content
            ContentAPI.createLink(contexts[0], 'Test Content 1', 'Test content description 1', 'public', 'http://www.sakaiproject.org/',  {}, function(err, contentObj) {
                assert.ok(!err);
                assert.ok(contentObj);
                // Get invalid piece of content
                ContentAPI.getContent(contexts[0], null, function(err, retrievedContentObj) {
                    assert.ok(err);
                    assert.ok(!retrievedContentObj);
                    // Get the piece of content
                    ContentAPI.getContent(contexts[0], contentObj.contentId, function(err, retrievedContentObj) {
                        assert.ok(!err);
     ...
     */
    it('Test create link', function(callback) {
        setUpUsers(function(contexts) {
            // Create one as anon user
            ContentAPI.createLink(new Context(tenant1), 'Test Content 1', 'Test content description 1', 'public', 'http://www.sakaiproject.org/', {}, function(err, contentObj) {
                assert.ok(err);
                assert.ok(!contentObj);
                // Create one with all required fields
                ContentAPI.createLink(contexts[0], 'Test Content 2', 'Test content description 2', 'public', 'http://www.sakaiproject.org/', {}, function(err, contentObj) {
                    assert.ok(!err);
                    assert.ok(contentObj.contentId);
                    // Create one without description
                    ContentAPI.createLink(contexts[0], 'Test Content 3', null, 'public', 'http://www.sakaiproject.org/', {}, function(err, contentObj) {
                        assert.ok(!err);
                        assert.ok(contentObj.contentId);
                        // Create one without URL
                        ContentAPI.createLink(contexts[0], 'Test Content 4', 'Test content description 4', 'public', null, {}, function(err, contentObj) {
                            assert.ok(err);
                            assert.ok(!contentObj);
                            // Create one without a valid URL
                            ContentAPI.createLink(contexts[0], 'Test Content 5', 'Test content description 5', 'public', 'Just a string', {}, function(err, contentObj) {
                                assert.ok(err);
                                assert.ok(!contentObj);
                                // Create one without title
                                ContentAPI.createLink(contexts[0], null, 'Test content description 6', 'public', 'http://www.sakaiproject.org/', {}, function(err, contentObj) {
                                    assert.ok(err);
                                    assert.ok(!contentObj);
                                    // Create one without visibility
                                    ContentAPI.createLink(contexts[0], 'Test Content 7', 'Test content description 7', null, 'http://www.sakaiproject.org/', {}, function(err, contentObj) {
                                        assert.ok(!err);
                                        assert.ok(contentObj.contentId);
     ...
     */
    it('Test create file', function(callback) {
        setUpUsers(function(contexts) {
            // Create one as anon user
            ContentAPI.createFile(new Context(tenant1), 'Test Content 1', 'Test content description 1', 'public', {}, function(err, contentObj) {
                assert.ok(err);
                assert.ok(!contentObj);
                // Create one with all required fields
                ContentAPI.createFile(contexts[0], 'Test Content 2', 'Test content description 2', 'public', {}, function(err, contentObj) {
                    assert.ok(!err);
                    assert.ok(contentObj.contentId);
                    // Create one without description
                    ContentAPI.createFile(contexts[0], 'Test Content 3', null, 'public', {}, function(err, contentObj) {
                        assert.ok(!err);
                        assert.ok(contentObj.contentId);
                        // Create one without title
                        ContentAPI.createFile(contexts[0], null, 'Test content description 4', 'public', {}, function(err, contentObj) {
                            assert.ok(err);
                            assert.ok(!contentObj);
                            // Create one without visibility
                            ContentAPI.createFile(contexts[0], 'Test Content 5', 'Test content description 6', null, {}, function(err, contentObj) {
                                assert.ok(!err);
                                assert.ok(contentObj.contentId);
                                // Check if the visibility has been set to public (default)
                                ContentAPI.getContent(contexts[0], contentObj.contentId, function(err, contentObj) {
     ...
     */
    it('Test create sakai doc', function(callback) {
        setUpUsers(function(contexts) {
            // Create one as anon user
            ContentAPI.createSakaiDoc(new Context(tenant1), 'Test Content 1', 'Test content description 1', 'public', {}, function(err, contentObj) {
                assert.ok(err);
                assert.ok(!contentObj);
                // Create one with all required fields
                ContentAPI.createSakaiDoc(contexts[0], 'Test Content 2', 'Test content description 2', 'public', {}, function(err, contentObj) {
                    assert.ok(!err);
                    assert.ok(contentObj.contentId);
                    // Create one without description
                    ContentAPI.createSakaiDoc(contexts[0], 'Test Content 3', null, 'public', {}, function(err, contentObj) {
                        assert.ok(!err);
                        assert.ok(contentObj.contentId);
                        // Create one without title
                        ContentAPI.createSakaiDoc(contexts[0], null, 'Test content description 4', 'public', {}, function(err, contentObj) {
                            assert.ok(err);
                            assert.ok(!contentObj);
                            // Create one without permission
                            ContentAPI.createSakaiDoc(contexts[0], 'Test Content 5', 'Test content description 5', null, {}, function(err, contentObj) {
                                assert.ok(!err);
                                assert.ok(contentObj.contentId);
     ...
     * for various people
     */
    it('Test create public content item', function(callback) {
        setUpUsers(function(contexts) {
            ...
            ContentAPI.createLink(contexts[0], 'Test Content 1', 'Test content description 1', 'public', 'http://www.sakaiproject.org/', {}, function(err, contentObj) {
                ...
                checkPieceOfContent(contexts[0], contexts[0].user().id, contentObj, true, true, true, function() {
                    ...
                    checkPieceOfContent(contexts[1], contexts[0].user().id, contentObj, true, false, true, function() {
                        ...
                        checkPieceOfContent(new Context(contexts[0].tenant()), contexts[0].user().id, contentObj, true, false, true, function() {
                            callback();
                        });
                    });
                });
     ...
     * for various people
     */
    it('Test create logged in content item', function(callback) {
        setUpUsers(function(contexts) {
            // Create a logged in content item
            ContentAPI.createLink(contexts[0], 'Test Content 1', 'Test content description 1', 'loggedin', 'http://www.sakaiproject.org/', {}, function(err, contentObj) {    
                assert.ok(!err);
                assert.ok(contentObj.contentId);
                // Get the piece of content as the person who created the content
                checkPieceOfContent(contexts[0], contexts[0].user().id, contentObj, true, true, true, function() {
                    ...
                    checkPieceOfContent(contexts[1], contexts[0].user().id, contentObj, true, false, true, function() {
                        // Get the piece of content as an anonymous user
                        checkPieceOfContent(new Context(contexts[0].tenant()), contexts[0].user().id, contentObj, false, false, false, function() {
                            callback();
                        });
                    });
                });
     ...
     * for various people
     */
    it('Test create private content item', function(callback) {
        setUpUsers(function(contexts) {
            // Create a private content item
            ContentAPI.createLink(contexts[0], 'Test Content 1', 'Test content description 1', 'private', 'http://www.sakaiproject.org/', {}, function(err, contentObj) {    
                assert.ok(!err);
                assert.ok(contentObj.contentId);
                // Get the piece of content as the person who created the content
                checkPieceOfContent(contexts[0], contexts[0].user().id, contentObj, true, true, true, function() {
     ...
     */
    it('Test create content with default members', function(callback) {
        setUpUsers(function(contexts) {
            // Create a private content item and share with 2 people
            var additionalMembers = {};
            additionalMembers[contexts[1].user().id] = 'manager';
            additionalMembers[contexts[2].user().id] = 'viewer';
            ContentAPI.createLink(contexts[0], 'Test Content 1', 'Test content description 1', 'private', 'http://www.sakaiproject.org/', additionalMembers, function(err, contentObj) {   
                assert.ok(!err);
                assert.ok(contentObj.contentId);
                // Get the piece of content as the person who created the content
                checkPieceOfContent(contexts[0], contexts[0].user().id, contentObj, true, true, true, function() {
    ...
    /**
     * Utility function that will be used to convert the members array that comes back from the getContentMembers function
     * [
     *   {
     *     'role': 'manager',
     ...
     *   }
     * ]
     * into an object that makes it easier for the tests to check that all of the expected members are part of the list
     * {
     *   'principalId': {
     *     'role': 'manager',
     *     'profile': {principalProfile}
     ...
     * }
     */
    var convertMemberArrayToObject = function(members) {
        var memberMap = {};
        _.each(members, function(member) { 
            memberMap[member.profile.id] = member;
        });
        return memberMap;
    };
    ...
    var setUpContentPermissions = function(contexts, privacy, callback) {
        // Create a public content item
        ContentAPI.createLink(contexts[0], 'Test Content 1', 'Test content description 1', privacy, 'http://www.sakaiproject.org/', {}, function(err, contentObj) {    
            assert.ok(!err);
            assert.ok(contentObj.contentId);
            // Get the piece of content as the person who created the content
            checkPieceOfContent(contexts[0], contexts[0].user().id, contentObj, true, true, true, function() {
                // Check the list of content members
                ContentAPI.getContentMembers(contexts[0], contentObj.contentId, null, null, function(err, members) {
                    assert.ok(!err);
                    assert.equal(members.length, 1);
                    members = convertMemberArrayToObject(members);
                    assert.equal(members[contexts[0].user().id].role, 'manager');
                    // Try an invalid set permissions with no principals passed in
                    ContentAPI.setContentPermissions(contexts[0], contentObj.contentId, {}, function(err) {
                        assert.ok(err);
                        ...
                        ContentAPI.setContentPermissions(contexts[0], contentObj.contentId, permissions, function(err) {
                            ...
                            checkPieceOfContent(contexts[1], contexts[1].user().id, contentObj, true, true, true, function() {
                                ContentAPI.getContentMembers(contexts[0], contentObj.contentId, null, null, function(err, members) {
                                    ...
                                    members = convertMemberArrayToObject(members);
                                    assert.equal(members[contexts[0].user().id].role, 'manager');
                                    assert.equal(members[contexts[1].user().id].role, 'manager');
                                    ...
                                    ContentAPI.setContentPermissions(contexts[0], contentObj.contentId, permissions, function(err) {
                                        ...
                                        checkPieceOfContent(contexts[2], contexts[2].user().id, contentObj, true, false, true, function() {
                                            ContentAPI.getContentMembers(contexts[0], contentObj.contentId, null, null, function(err, members) {
                                                ...
                                                assert.equal(members[contexts[0].user().id].role, 'manager');
                                                assert.equal(members[contexts[1].user().id].role, 'manager');
                                                assert.equal(members[contexts[2].user().id].role, 'viewer');
                                                    ...
                                                    checkPieceOfContent(contexts[4], contexts[4].user().id, contentObj, privacy === 'private' ? false : true, false, false, function() {
                                                        ContentAPI.getContentMembers(contexts[0], contentObj.contentId, null, null, function(err, members) {
                                                            ...
                                                            ContentAPI.setContentPermissions(contexts[0], contentObj.contentId, permissions, function(err) {
                                                                ...
                                                                checkPieceOfContent(contexts[1], contexts[1].user().id, contentObj, privacy === 'private' ? false : true, false, false, function() {
                                                                    ContentAPI.getContentMembers(contexts[0], contentObj.contentId, null, null, function(err, members) {
                                                                        assert.ok(!err);
                                                                        members = convertMemberArrayToObject(members);
                                                                        assert.equal(members[contexts[0].user().id].role, 'manager');
                                                                        assert.equal(members[contexts[1].user().id], undefined);
                                                                        assert.equal(members[contexts[2].user().id].role, 'viewer');
                                                                        // Try setting the permissions with an undefined role
                                                                        permissions = {};
                                                                        permissions[contexts[3].user().id] = undefined;
                                                                        ContentAPI.setContentPermissions(contexts[0], contentObj.contentId, permissions, function(err) {
                                                                            assert.ok(err);
                                                                            // Try to set the permissions on the content as a non-manager of the content
                                                                            permissions = {};
                                                                            permissions[contexts[3].user().id] = 'viewer';
                                                                            ContentAPI.setContentPermissions(contexts[1], contentObj.contentId, permissions, function(err) {
                                                                                assert.ok(err);
                                                                                callback(contentObj);
                                                                            });
                                                                        });
     ...
     */
    it('Test public content permissions', function(callback) {
        setUpUsers(function(contexts) {
     ...
     */
    it('Test logged in content permissions', function(callback) {
        setUpUsers(function(contexts) {
     ...
     */
    it('Test private content permissions', function(callback) {
        setUpUsers(function(contexts) {
            ...
            ContentAPI.createLink(contexts[0], 'Test Content 1', 'Test content description 1', 'public', 'http://www.sakaiproject.org/', {}, function(err, contentObj) {    
                ...
                ContentAPI.setContentPermissions(contexts[0], contentObj.contentId, permissions, function(err) {
                    ...
                    ContentAPI.getContentMembers(contexts[0], contentObj.contentId, null, null, function(err, members) {
                        ...
                        assert.equal(members[contexts[1].user().id].role, 'manager');
                        assert.equal(members[contexts[2].user().id].role, 'viewer');
                        assert.equal(members[contexts[3].user().id].role, 'viewer');
                        ...
                        ContentAPI.setContentPermissions(contexts[0], contentObj.contentId, permissions, function(err) {
                            ...
                            ContentAPI.getContentMembers(contexts[0], contentObj.contentId, null, null, function(err, members) {
                                ...
                                assert.equal(members[contexts[0].user().id].role, 'manager');
                                assert.equal(members[contexts[3].user().id].role, 'viewer');
                                assert.equal(members[contexts[4].user().id].role, 'viewer');
                ...
                testSharing(contentObj, contexts[0], contexts[1], true, true, false, true, expectedMembers, function() {
                    ...
                    testSharing(contentObj, contexts[1], contexts[2], true, true, false, true, expectedMembers, function() {
                        ...
                        testSharing(contentObj, contexts[2], contexts[3], true, true, false, true, expectedMembers, function() {
                            ...
                            testSharing(contentObj, contexts[3], contexts[0], true, true, true, true, expectedMembers, function() {
                                // Share as anonymous
                                testSharing(contentObj, new Context(contexts[4].tenant()), contexts[4], false, true, false, false, expectedMembers, function() {
                                    callback();
                                }); 
                            });
                        }); 
                ...
                testSharing(contentObj, contexts[0], contexts[1], true, true, false, true, expectedMembers, function() {
                    ...
                    testSharing(contentObj, contexts[1], contexts[2], false, false, false, false, null, function() {
                        ...
                        testSharing(contentObj, contexts[3], contexts[3], false, false, false, false, null, function() {
                            // Share with the content manager, making sure that he's still the content manager after sharing
                            testSharing(contentObj, contexts[1], contexts[0], false, true, true, true, expectedMembers, function() {
                                // Share as anonymous
                                testSharing(contentObj, new Context(contexts[4].tenant()), contexts[4], false, false, false, false, null, function() {
                                    callback();
                                });
                            });
                        }); 
    ...
    var testGroupAccess = function(contexts, groups, privacy, callback) {
        ...
        ContentAPI.createLink(contexts[4], 'Test Content 1', 'Test content description 1', privacy, 'http://www.sakaiproject.org/', {}, function(err, contentObj) {    
            assert.ok(!err);
            assert.ok(contentObj.contentId);
            ...
            ContentAPI.setContentPermissions(contexts[4], contentObj.contentId, permissions, function(err) {
                ...
                checkPieceOfContent(contexts[2], groups[0].id, contentObj, true, false, true, function() {
                    checkPieceOfContent(contexts[0], contexts[0].user().id, contentObj, true, false, false, function() {
                        checkPieceOfContent(contexts[2], contexts[2].user().id, contentObj, true, false, false, function() {
                            // Check that it shows in UI Dev Team's library
                            ContentAPI.getLibraryItems(contexts[0], groups[0].id, null, 10, function(err, contentItems) {
                                assert.ok(!err);
                                assert.equal(contentItems.length, 1);
                                assert.equal(contentItems[0].contentId, contentObj.contentId);
                                ...
                                ContentAPI.getLibraryItems(contexts[1], contexts[1].user().id, null, 10, function(err, contentItems) {
                                    ...
                                    ContentAPI.getLibraryItems(contexts[0], contexts[0].user().id, null, 10, function(err, contentItems) {
                                        ...
                                        ContentAPI.getLibraryItems(contexts[2], contexts[2].user().id, null, 10, function(err, contentItems) {
                                            ...
                                            ContentAPI.getLibraryItems(contexts[4], groups[1].id, null, 10, function(err, contentItems) {
                                                assert.ok(!err);
                                                assert.equal(contentItems.length, 0);
                                                ContentAPI.getLibraryItems(contexts[4], groups[2].id, null, 10, function(err, contentItems) {
                                                    assert.ok(!err);
                                                    assert.equal(contentItems.length, 0);
                                                    // Check that Stuart doesn't have access
                                                    checkPieceOfContent(contexts[5], contexts[5].user().id, contentObj, privacy === 'private' ? false : true, false, false, function() {
                                                        // Check that Branden doesn't have access
                                                        checkPieceOfContent(contexts[3], contexts[3].user().id, contentObj, privacy === 'private' ? false : true, false, false, function() {
    
                                                            // Share with the OAE Team group
                                                            ContentAPI.shareContent(contexts[4], contentObj.contentId, [groups[2].id], function(err) {
                                                                // Check that Stuart has access
                                                                checkPieceOfContent(contexts[5], contexts[5].user().id, contentObj, true, false, false, function() {
                                                                    ...
                                                                    checkPieceOfContent(contexts[3], contexts[3].user().id, contentObj, true, false, false, function() {
                                                                        ...
                                                                        ContentAPI.getLibraryItems(contexts[4], groups[2].id, null, 10, function(err, contentItems) {
                                                                            ...
                                                                            ContentAPI.getLibraryItems(contexts[0], groups[0].id, null, 10, function(err, contentItems) {
                                                                                ...
                                                                                assert.equal(contentItems.length, 1);
                                                                                assert.equal(contentItems[0].contentId, contentObj.contentId);
                                                                                ContentAPI.getLibraryItems(contexts[1], groups[1].id, null, 10, function(err, contentItems) {
                                                                                    assert.ok(!err);
                                                                                    assert.equal(contentItems.length, 0);
                                                                                    ...
                                                                                    ContentAPI.setContentPermissions(contexts[4], contentObj.contentId, permissions, function(err) {
                                                                                        ...
                                                                                        checkPieceOfContent(contexts[1], contexts[1].user().id, contentObj, true, true, true, function() {
                                                                                            checkPieceOfContent(contexts[3], contexts[3].user().id, contentObj, true, true, false, function() {
                                                                                                checkPieceOfContent(contexts[5], contexts[5].user().id, contentObj, true, false, false, function() {
                        ...
                        checkPieceOfContent(contexts[2], contexts[2].user().id, contentObj, true, false, true, function() {
                            ...
                            ContentAPI.shareContent(contexts[1], contentObj.contentId, [contexts[3].user().id], function(err) {
                                ...
                                checkPieceOfContent(contexts[3], contexts[3].user().id, contentObj, true, false, true, function() {
                                    ...
                                    ContentAPI.deleteContent(new Context(contexts[3].tenant()), contentObj.contentId, function(err) {
                                        assert.ok(err);
                                                                    ...
                                                                    checkPieceOfContent(contexts[2], contexts[2].user().id, contentObj, false, false, false, function() {
                                                                        checkPieceOfContent(contexts[4], contexts[4].user().id, contentObj, false, false, false, function() {
                                                                            checkPieceOfContent(new Context(contexts[0].tenant()), contexts[0].user().id, contentObj, false, false, false, function() {
                                                                                ...
                                                                                AuthzAPI.getRole(contexts[0].user().id, contentObj.contentId, function(err, role) {
                                                                                    assert.ok(!role);
                                                                                    AuthzAPI.getRole(contexts[2].user().id, contentObj.contentId, function(err, role) {
                                                                                        assert.ok(!role);
                                                                                        AuthzAPI.getRole(contexts[4].user().id, contentObj.contentId, function(err, role) {
                                                                                            assert.ok(!role);
                                                                                            // Check whether the list of members is no longer accessible
                                                                                            ContentAPI.getContentMembers(contexts[0], contentObj.contentId, null, null, function(err) {
                                                                                                assert.ok(err);
                                                                                                callback();
                                                                                            });
                                                                                        });
    ...
    var checkNameAndDescription = function(contexts, contentId, expectedName, expectedDescription, callback) {
        // Check as user 0
        ContentAPI.getContent(contexts[0], contentId, function(err, contentObj) {
            assert.ok(!err);
            ...
            // Check as user 1
            ContentAPI.getContent(contexts[1], contentId, function(err, contentObj) {
                assert.ok(!err);
                assert.ok(contentObj.contentId, contentId);
                assert.ok(contentObj.name, expectedName);
                assert.ok(contentObj.description, expectedDescription);
    ...
    it('Test update content profile', function(callback) {
        ...
        ContentAPI.createLink(contexts[0], 'Test Content 1', 'Test content description 1', 'public', 'http://www.sakaiproject.org/', {}, function(err, contentObj) {    
                ...
                ContentAPI.shareContent(contexts[0], contentObj.contentId, [contexts[1].user().id], function(err) {
                    ...
                    ContentAPI.updateContentMetadata(contexts[0], contentObj.contentId, {}, function(err) {
                        assert.ok(err);
                        // Invalid content metadata update (unexisting field)
                        ContentAPI.updateContentMetadata(contexts[0], contentObj.contentId, {'name': 'New Test Content 1', 'nonExisting': 'Non-existing field'}, function(err) {
                            assert.ok(err);
                            ...
                            checkNameAndDescription(contexts, contentObj.contentId, 'Test Content 1', 'Test content description 1', function() {
                                // Change the name
                                ContentAPI.updateContentMetadata(contexts[0], contentObj.contentId, {'name': 'New Test Content 1'}, function(err) {
                                    assert.ok(!err);
                                    ...
                                    checkNameAndDescription(contexts, contentObj.contentId, 'New Test Content 1', 'Test content description 1', function() {
                                        ...
                                        ContentAPI.updateContentMetadata(contexts[0], contentObj.contentId, {'description': 'New test content description 1'}, function(err) {
                                            assert.ok(!err);
                                            ...
                                            checkNameAndDescription(contexts, contentObj.contentId, 'New Test Content 1', 'New test content description 1', function() {
                                                ...
                                                ContentAPI.updateContentMetadata(contexts[0], contentObj.contentId, {'name': 'New Test Content 2', 'description': 'New test content description 2'}, function(err) {
                                                    assert.ok(!err);
                                                    ...
                                                    checkNameAndDescription(contexts, contentObj.contentId, 'New Test Content 2', 'New test content description 2', function() {
                                                        ...
                                                        ContentAPI.updateContentMetadata(contexts[1], contentObj.contentId, {'name': 'New Test Content 3'}, function(err) {
                                                            assert.ok(err);
    ...
    var checkAccessAndLibrary = function(contexts, contentId, expectLoggedInAccess, expectAnonAccess, callback) {
        ...
        ContentAPI.hasAccess(contexts[0], contentId, function(err, hasAccess) {
            assert.ok(!err);
            assert.ok(hasAccess);
            // Check that it's part of the content manager's library
            ContentAPI.getLibraryItems(contexts[0], contexts[0].user().id, null, 10, function(err, items) {
                assert.ok(!err);
                assert.equal(items.length, 1);
                assert.equal(items[0].contentId, contentId);
                // Check for the content viewer
                ContentAPI.hasAccess(contexts[1], contentId, function(err, hasAccess) {
                    assert.ok(!err);
                    assert.ok(hasAccess);
                    // Check that it is part of his library
                    ContentAPI.getLibraryItems(contexts[1], contexts[1].user().id, null, 10, function(err, items) {
                        assert.ok(!err);
                        assert.equal(items.length, 1);
                        assert.equal(items[0].contentId, contentId);
                        // Check that it is visible in the manager's library
                        ContentAPI.getLibraryItems(contexts[1], contexts[0].user().id, null, 10, function(err, items) {
                            assert.ok(!err);
                            ...
                            ContentAPI.hasAccess(contexts[2], contentId, function(err, hasAccess) {
                                assert.ok(!err);
                                assert.equal(hasAccess, expectLoggedInAccess);
                                // Check that it isn't part of his library
                                ContentAPI.getLibraryItems(contexts[2], contexts[2].user().id, null, 10, function(err, items) {
                                    assert.ok(!err);
                                    assert.equal(items.length, 0);
                                    // Check that it is visible in the manager's library
                                    ContentAPI.getLibraryItems(contexts[2], contexts[0].user().id, null, 10, function(err, items) {
                                        assert.ok(!err);
                                        if (expectLoggedInAccess) {
                                            assert.equal(items.length, 1);
                                            assert.equal(items[0].contentId, contentId);
                                        ...
                                        ContentAPI.hasAccess(new Context(), contentId, function(err, hasAccess) {
                                            ...
                                            // Check that it is visible in the manager's library
                                            ContentAPI.getLibraryItems(new Context(), contexts[0].user().id, null, 10, function(err, items) {
                                                assert.ok(!err);
                                                if (expectAnonAccess) {
                                                    assert.equal(items.length, 1);
                                                    assert.equal(items[0].contentId, contentId);
            ...
            ContentAPI.createLink(contexts[0], 'Test Content 1', 'Test content description 1', 'public', 'http://www.sakaiproject.org/', {}, function(err, contentObj) {    
                ...
                ContentAPI.shareContent(contexts[0], contentObj.contentId, [contexts[1].user().id], function(err) {
                    ...
                    checkAccessAndLibrary(contexts, contentObj.contentId, true, true, function() {
                        ...
                        ContentAPI.updateContentMetadata(contexts[0], contentObj.contentId, {'visibility': null}, function(err) {
                            assert.ok(err);
                            // Check that the access remains unchanged
                            checkAccessAndLibrary(contexts, contentObj.contentId, true, true, function() {
                                // Try an unknown visibility update
                                ContentAPI.updateContentMetadata(contexts[0], contentObj.contentId, {'visibility': 'unknown-option'}, function(err) {
                                    assert.ok(err);
                                    // Check that the access remains unchanged
                                    checkAccessAndLibrary(contexts, contentObj.contentId, true, true, function() {
                                        // Make the content logged in only
                                        ContentAPI.updateContentMetadata(contexts[0], contentObj.contentId, {'visibility': 'loggedin'}, function(err) {
                                            assert.ok(!err);
                                            ...
                                            checkAccessAndLibrary(contexts, contentObj.contentId, true, false, function() {
                                                ...
                                                ContentAPI.updateContentMetadata(contexts[0], contentObj.contentId, {'visibility': 'private'}, function(err) {
                                                    assert.ok(!err);
                                                    // Check that everyone can get the content as expected, check library presence as expected
                                                    checkAccessAndLibrary(contexts, contentObj.contentId, false, false, function() {
                                                        // Try update as non-manager
                                                        ContentAPI.updateContentMetadata(contexts[1], contentObj.contentId, {'visibility': 'public'}, function(err) {
                                                            assert.ok(err);
                                                            // Check that everyone can get the content as expected, check library presence as expected
                                                            checkAccessAndLibrary(contexts, contentObj.contentId, false, false, function() {
                                                                callback();
