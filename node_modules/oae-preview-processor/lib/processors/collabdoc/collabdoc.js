...
// Collaborative document or collaborative spreadsheet?
let type = null;

// The absolute path to the html & css files. Same template is used for both collabdocs and collabsheets
const basePath = Path.normalize(__dirname + '/../../../static/collabdoc/');
 ...
 */
const test = module.exports.test = function(ctx, contentObj, callback) {
 ...
 */
const generatePreviews = module.exports.generatePreviews = function(ctx, contentObj, callback) {
    ...
    RestAPI.Content.getRevisions(ctx.tenantRestContext, contentObj.id, null, 2, function(err, revisions) {
        if (err) {
            ...
            // Only 1 revision => unpublished document. Ignore it for now.
            return callback(null, true);
        }

        // Store whether this document is a collaborative document or spreadsheet
        type = contentObj.resourceSubType;
        ...
        _writeCollabHtml(ctx, contentObj.latestRevision[html], function(err, collabFilePath) {
            if (err) {
                return callback(err);
            }

            // Generate a screenshot that is suitable to display in the activity feed.
            const collabFileUri = 'file://' + collabFilePath;
            const path = ctx.baseDir + '/wide.png';
            webshot.getImage(collabFileUri, path, options, function (err) {
                if (err) {
 ...
 * @api private
 */
const _writeCollabHtml = function(ctx, collabHtml, callback) {
    _getWrappedCollabHtml(ctx, collabHtml, function(err, wrappedHtml) {
        if (err) {
            return callback(err);
        }

        // Write the resulting HTML to a temporary file on disk
        const collabFilePath = type === 'collabdoc' ? ctx.baseDir + '/etherpad.html' : ctx.baseDir + '/ethercalc.html';
        fs.writeFile(collabFilePath, wrappedHtml, function(err) {
            if (err) {
                log().error({'err': err, 'contentId': ctx.contentId}, 'Could not write the collaborative file preview HTML to disk');
 ...
 *
 * @param  {Context}    ctx             Standard context object containing the current user and the current tenant
 * @param  {String}     collabHtml      The HTML as retrieved from the API
 * @param  {Function}   callback        Standard callback function
 * @param  {Object}     callback.err    An error that occurred, if any
...
const _getWrappedCollabHtml = function(ctx, collabHtml, callback) {
    ...
    try {
        ...
        log().error({'err': err, 'collabHtml': collabHtml, 'contentId': ctx.contentId}, 'Unable to parse collaborative file preview HTML');
        return callback({'code': 500, 'msg': 'Unable to parse collaborative file preview HTML'});
    }
    ...
    if (wrapperHtml) {
        ...
        fs.readFile(HTML_FILE, 'utf8', function(err, content) {
            if (err) {
                log().error({'err': err, 'contentId': ctx.contentId}, 'Could not read the collaborative file preview wrapper HTML');
