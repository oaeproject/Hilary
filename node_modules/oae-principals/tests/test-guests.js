...
describe('Inviting guests', function() {
    ...
    /**
     * Test that verifies that private tenants cannot interact with guests
     */
    it('verify private tenants cannot interact with guests', function(callback) {
        TestsUtil.setupMultiTenantPrivacyEntities(function(publicTenant1, publicTenant2, privateTenant1) {

            // Try to create some content and inviting a guest
            var members = [TestsUtil.generateTestEmailAddress()];
            RestAPI.Content.createLink(privateTenant1.adminRestContext, 'Apereo', 'Apereo', 'private', 'http://www.apereo.org', [], members, [], function(err, link) {
                assert.ok(err);
                assert.strictEqual(err.code, 401);
                return callback();
            });
