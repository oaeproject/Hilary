 ...
 * * `deletedContent(ctx, contentObj, removedMemberIds)`: A content item was deleted. The `ctx` and the `contentObj` of the deleted item are provided. The `removedMemberIds` holds the ids of the principals who were a member of this content item
...
var deleteContent = module.exports.deleteContent = function(ctx, contentId, callback) {
    ...
    canManage(ctx, contentId, function(err, canManage, contentObj) {
        ...
        // Delete the content
        ContentDAO.Content.deleteContent(contentObj, function(err, removedMemberIds) {
            if (err) {
                return callback(err);
            }
            ContentAPI.emit(ContentConstants.events.DELETED_CONTENT, ctx, contentObj, removedMemberIds);
            return callback();
        });
    });
};
    ...
    ContentDAO.Content.getContent(contentId, function(err, contentObj) {
        ...
        _canShareContent(ctx, contentObj, function(err, canShare) {
            ...
            _checkNewContentPermissions(ctx, contentObj, testPermissionChanges, function(err, membershipAfterChanges, newMemberIds, updatedMemberIds, removedMemberIds, newMembers) {
                ...
                _setContentPermissions(ctx, contentObj, realPermissionChanges, function(err, contentObj) {
                    if (err) {
                        return callback(err);
                    }
...
/**
 * Get the content items inside a principal's library. If the user requests their own library or the library of a group they're a member of,
