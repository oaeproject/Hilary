...
var updateUser = module.exports.updateUser = function(ctx, userId, profileFields, callback) {
    ...
    PrincipalsDAO.getPrincipal(userId, function(err, oldUser) {
        ...
        // If the user updated the email address and is not an admin we will need to verify it
        if (profileFields.email && !ctx.user().isAdmin(oldUser.tenant.alias) && oldUser.email !== profileFields.email) {
            profileFields.emailVerified = false;

            // TODO: Remove old email -> user mapping
            // TODO: Check if a user with the given email address already exists
        }
