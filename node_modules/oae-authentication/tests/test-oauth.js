...
describe('Authentication', function() {
    ...
    before(function(callback) {
        ...
        RestAPI.Admin.loginOnTenant(globalAdminRestContext, 'localhost', function(err, restContext) {
            assert.ok(!err);
    ...
    describe('OAuth', function() {
        ...
        describe('Authentication', function() {
            ...
            /**
             * Test that verifies that CSRF is not required when authenticated via OAuth
             */
            it('verify CSRF is not applied when authenticated via OAuth', function(callback) {
                _setupOAuth(function(simong, client, accessToken) {
                    // Set the referer header to something external and assert that the CSRF protection is not applied
                ...
                _setupOAuth(function(simong, client, accessToken) {
                    ...
                    RestAPI.User.setTenantAdmin(globalAdminRestContext, simong.user.id, true, function(err) {
                        ...
                        RestAPI.User.getMe(simong.oauthRestContext, function(err, data) {
                            ...
                            // Assert that we can do tenant admin things (such as import a CSV file)
                            var getCSV = function() {
                                var file = __dirname + '/../../oae-principals/tests/data/users-with-password.csv';
                                return fs.createReadStream(file);
                            };
                            PrincipalsTestsUtil.importUsers(simong.oauthRestContext, global.oaeTests.tenants.localhost.alias, getCSV, 'cas', function(err) {
                                assert.ok(!err);
