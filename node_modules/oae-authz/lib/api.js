    ...
    _getDirectRole(principalId, resourceId, function(err, directRole) {
        ...
        _getResourceGroupMembers(resourceId, function(err, groups) {
            ...
            if (groupIds.length === 0) {
                ...
                ensureCachedMemberships(principalId, function(err) {
                    if (err) {
                        return callback(err);
                    }
                    
                    //Check if any of these groups is in the cached list of group memberships
                    Cassandra.runQuery('SELECT ? FROM AuthzMembershipsCache USING CONSISTENCY QUORUM WHERE principalId = ?', [groupIds, principalId], function(err, rows) {
                        if (err) {
                            return callback(err);
                        }
                        ...
                        for (var m in memberships) {
                            if (memberships[m] && !_.contains(allRoles, groups[m])) {
                                allRoles.push(groups[m]);
                            }
                        }
                        return callback(null, allRoles);
                    });
...
var _updateRoles = function(resourceId, changes, callback) {
    ...
    if (AuthzUtil.isGroupId(resourceId)) {
        ...
        var groupsToInvalidate = [resourceId];
...
/**
 * TODO
 * Checks whether or not the cached exploded memberships for a given user are present. If not, the exploded principals
 * of the user are fetched and stored
 */
var ensureCachedMemberships = function(principalId, callback) {
    var cql = 'SELECT * FROM AuthzMembershipsCache USING CONSISTENCY QUORUM WHERE principalId = ? LIMIT 2';
    Cassandra.runQuery(cql, [principalId], function(err, rows) {
        if (err) {
            return callback(err);
        }
        //console.log(rows[0].count);
        //console.log(rows[0].toString);
        // We found elements, this means the exploded list is there
        if (rows[0].count > 1) {
            callback(null, true);
        // No exploded memberships have been found
        } else {
            _getAuthzGroupMembershipAncestry(principalId, function(err, memberships) {
                ...
                if (_.keys(memberships).length > 0) {
                    ...
                    Cassandra.runQuery(publishMemberships.query, publishMemberships.parameters, callback);
                } else {
                    callback(null, true);
                }
            });
...
var getPrincipalMemberships = module.exports.getPrincipalMemberships = function(principalId, start, limit, callback) {
    ...
    ensureCachedMemberships(principalId, function(err) {
        ...
        Cassandra.runQuery('SELECT FIRST ' + paging.limit + ' ?..\'\' FROM AuthzMembershipsCache USING CONSISTENCY QUORUM WHERE principalId = ?', [paging.start, principalId], function (err, rows) {
            if (err) {
                return callback(err);
            }
            ...
            var row = rows[0];
            for (var i = paging.startIndex, j = row.count; i < j; i++) {
                if (row[i].name !== "principalId") {
                    memberships.push(row[i].name);
                }
            }
