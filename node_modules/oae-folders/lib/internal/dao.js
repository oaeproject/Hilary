...
var Folder = require('oae-folders/lib/model').Folder;
 ...
 */
var createFolder = module.exports.createFolder = function(createdBy, displayName, description, visibility, callback) {
    var tenantAlias = AuthzUtil.getPrincipalFromId(createdBy).tenantAlias;
    var folderId = _createFolderId(tenantAlias);
    var groupId = PrincipalsUtil.createGroupId(tenantAlias);
    var created = Long.fromNumber(Date.now());
    var storageHash = {
    ...
    var insertGroupIdIndexQuery = Cassandra.constructUpsertCQL('FoldersGroupId', 'groupId', groupId, {'folderId': folderId});
    var insertFolderQuery = Cassandra.constructUpsertCQL('Folders', 'id', folderId, storageHash);

    // Insert the surrogate group id index entry
    Cassandra.runBatchQuery([insertGroupIdIndexQuery, insertFolderQuery], function(err) {
        if (err) {
            return callback(err);
        }
 ...
 */
var getFoldersByIds = module.exports.getFoldersByIds = function(folderIds, callback) {
    if (_.isEmpty(folderIds)) {
        return callback(null, []);
    }

    Cassandra.runInQuery('SELECT * FROM "Folders"', null, 'id', folderIds, function(err, rows) {
        if (err) {
            return callback(err);
        }

        // Assemble the folders array, ensuring it is in the same order as the original ids
        var foldersById = _.chain(rows).map(_rowToFolder).indexBy('id').value();
        var folders = _.chain(folderIds)
 ...
 */
var getFoldersByGroupIds = module.exports.getFoldersByGroupIds = function(groupIds, callback) {
    if (_.isEmpty(groupIds)) {
        return callback(null, []);
    }

    Cassandra.runInQuery('SELECT * FROM "FoldersGroupId"', null, 'groupId', groupIds, function(err, rows) {
        if (err) {
            return callback(err);
        }

        // Assemble the folder ids, ensuring the original ordering is maintained
        var folderIdsByGroupIds = _.chain(rows).map(Cassandra.rowToHash).indexBy('groupId').value();
        var folderIds = _.chain(groupIds)
 ...
 */
var getFolder = module.exports.getFolder = function(folderId, callback) {
    getFoldersByIds([folderId], function(err, folders) {
        if (err) {
            return callback(err);
 ...
 * @param  {Object}     callback.err        An error that occurred, if any
 */
var deleteFolder = module.exports.deleteFolder = function(folderId, callback) {
 ...
 */
var updateFolder = module.exports.updateFolder = function(folder, profileFields, callback) {
    var storageHash = _.extend({}, profileFields);
    storageHash.lastModified = storageHash.lastModified || Date.now();
    storageHash.lastModified = Long.fromNumber(storageHash.lastModified);

    var query = Cassandra.constructUpsertCQL('Folders', 'id', folder.id, storageHash);
 ...
 */
var getContentItems = module.exports.getContentItems = function(folderGroupId, opts, callback) {
    opts = opts || {};
    ...
    AuthzAPI.getRolesForPrincipalAndResourceType(folderGroupId, 'c', opts.start, opts.limit, function(err, roles, nextToken) {
        if (err) {
            return callback(err);
        }
        ...
        ContentDAO.Content.getMultipleContentItems(ids, opts.fields, function(err, contentItems) {
            if (err) {
                return callback(err);
            }
 ...
 * @param  {Folder}     callback.folder     The updated folder
 */
var setPreviews = module.exports.setPreviews = function(folder, previews, callback) {
    var query = Cassandra.constructUpsertCQL('Folders', 'id', folder.id, {'previews': previews});
    Cassandra.runQuery(query.query, query.parameters, function(err) {
        if (err) {
            return callback(err);
        }

        return callback(null, _createUpdatedFolderFromStorageHash(folder, {'previews': previews}));
    });
};
...
var iterateAll = module.exports.iterateAll = function(properties, batchSize, onEach, callback) {
    ...
    Cassandra.iterateAll(properties, 'Folders', 'id', {'batchSize': batchSize}, _iterateAllOnEach, callback);
};
 ...
 * @api private
 */
var _createUpdatedFolderFromStorageHash = function(folder, updatedStorageHash) {
    return new Folder(
        folder.tenant,
        folder.id,
        folder.groupId,
 ...
 * @api private
 */
var _storageHashToFolder = function(folderId, storageHash) {
    return new Folder(
        ...
        // The `created` and `lastModified` values are `Long` instances. This is because we're storing
        // them as `bigint`s and the new cassandra driver doesn't return these as Numbers. As we won't
        // really be dealing with big integers we can simply stringify them and parse them back into regular ints
        parseInt(storageHash.created.toString(), 10),
        parseInt(storageHash.lastModified.toString(), 10),
        storageHash.previews
    );
};
 ...
 * @api private
 */
var _createFolderId = function(tenantAlias) {
