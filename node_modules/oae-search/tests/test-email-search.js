...
describe('Email Search', () => {
    ...
    it('verify authorization of email search', (callback) => {
        TestsUtil.generateTestUsers(camAdminRestContext, 1, (err, users, user) => {
            ...
            SearchTestUtil.assertSearchFails(anonymousRestContext, 'email', null, {'q': user.user.email}, 401, () => {
                SearchTestUtil.assertSearchEquals(user.restContext, 'email', null, {'q': user.user.email}, [user.user.id], () => {
                    SearchTestUtil.assertSearchEquals(camAdminRestContext, 'email', null, {'q': user.user.email}, [user.user.id], () => {
                        SearchTestUtil.assertSearchEquals(globalAdminOnTenantRestContext, 'email', null, {'q': user.user.email}, [user.user.id], () => {
                            return callback();
                        });
                    });
                });
    ...
    it('verify email search returns all users that match a particular email', (callback) => {
        TestsUtil.generateTestUsers(camAdminRestContext, 3, (err, userInfos, userInfo0, userInfo1, userInfo2) => {
            ...
            const allUserIds = _.chain([userInfo0, userInfo1, userInfo2]).pluck('user').pluck('id').value();
            const updateUserIds = _.chain([userInfo1, userInfo2]).pluck('user').pluck('id').value();
            PrincipalsTestUtil.assertUpdateUsersSucceeds(camAdminRestContext, updateUserIds, {'email': userInfo0.user.email}, (users, tokens) => {
                ...
                PrincipalsTestUtil.assertVerifyEmailsSucceeds(userInfoTokens, () => {
                    ...
                    RestAPI.Content.createLink(userInfo0.restContext, userInfo0.user.email, userInfo0.user.email, 'public', 'google.com', [], [], [], (err, link) => {
                        assert.ok(!err);
                        RestAPI.Group.createGroup(userInfo0.restContext, userInfo0.user.email, userInfo0.user.email, 'public', 'yes', null, null, (err, group) => {
                            assert.ok(!err);
                            RestAPI.Discussions.createDiscussion(userInfo0.restContext, userInfo0.user.email, userInfo0.user.email, 'public', null, null, (err, discussion) => {
                                assert.ok(!err);
                                FoldersTestUtil.assertCreateFolderSucceeds(userInfo0.restContext, userInfo0.user.email, userInfo0.user.email, 'public', null, null, (folder) => {
                                    SearchTestUtil.whenIndexingComplete(() => {
                                        ...
                                        SearchTestUtil.assertSearchContains(userInfo0.restContext, 'general', null, {'q': userInfo0.user.email}, allResourceIds, () => {

                                            // Now ensure that only the users come out of the email search
                                            SearchTestUtil.assertSearchEquals(userInfo0.restContext, 'email', null, {'q': userInfo0.user.email}, allUserIds, () => {
                                                return callback();
                                            });
                                        });
                                    });
    ...
    it('verify email search returns the tenant that matches the email domain', (callback) => {
        TestsUtil.setupMultiTenantPrivacyEntities((publicTenant0, publicTenant1, privateTenant0, privateTenant1) => {
            SearchTestUtil.whenIndexingComplete(() => {
                SearchTestUtil.assertSearchSucceeds(publicTenant0.publicUser.restContext, 'email', null, {'q': privateTenant1.publicUser.user.email}, (data) => {
                    ...
                    SearchTestUtil.assertSearchSucceeds(publicTenant0.publicUser.restContext, 'email', null, {'q': 'an.email@ends.up.on.the.guest.tenant.com'}, (data) => {
                        assert.ok(_.isObject(data.tenant));
                        assert.ok(data.tenant.isGuestTenant);
                        return callback();
                    });
