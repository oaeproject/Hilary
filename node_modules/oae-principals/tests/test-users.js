...
describe('Users', function() {
    ...
    describe('Import users', function() {
        ...
        it('verify import local users and display name override', function(callback) {
            ...
            RestAPI.Admin.importUsers(globalAdminRestContext, global.oaeTests.tenants.cam.alias, getCSVWithPasswordStream, 'local', function(err) {
                assert.ok(!err);
                        ...
                        RestAPI.User.getMe(stuartRestContext, function(err, stuartMeObj) {
                            ...
                            RestAPI.User.updateUser(nicolaasRestContext, nicolaasMeObj.id, {'displayName': 'user-zfdHliPLa'}, function (err, user) {
                                ...
                                RestAPI.User.getMe(nicolaasRestContext, function(err, nicolaasMeObj) {
                                    ...
                                    RestAPI.Admin.importUsers(camAdminRestContext, null, getCSVWithPasswordStream, 'local', function(err) {
                                        assert.ok(!err);
        ...
        it('verify import external users', function(callback) {
            ...
            RestAPI.Admin.importUsers(globalAdminRestContext, global.oaeTests.tenants.cam.alias, getCSVWithoutPasswordStream, 'cas', function(err) {
                assert.ok(!err);
                                                ...
                                                RestAPI.User.getUser(camAdminRestContext, stuartUserId, function(err, stuartUserObj) {
                                                    ...
                                                    RestAPI.User.updateUser(camAdminRestContext, nicolaasUserId, {'displayName': 'user-TGdDSdadW'}, function (err, user) {
                                                        ...
                                                        RestAPI.User.getUser(camAdminRestContext, nicolaasUserId, function(err, nicolaasUserObj) {
                                                            ...
                                                            // Re-import the users to verify that the displayName is reverted back to the display name
                                                            // provided in the CSV file. This time, the import is attempted as a tenant admin
                                                            RestAPI.Admin.importUsers(camAdminRestContext, null, getCSVWithoutPasswordStream, 'cas', function(err) {
                                                                assert.ok(!err);
        ...
        it('verify parameter validation', function(callback) {
            ...
            RestAPI.Admin.importUsers(globalAdminRestContext, 'foobar', getCSVWithoutPasswordStream, 'cas', function(err) {
                assert.ok(err);
                assert.ok(err.code, 400);

                // Verify that an authentication method is required
                RestAPI.Admin.importUsers(globalAdminRestContext, global.oaeTests.tenants.cam.alias, getCSVWithoutPasswordStream, null, function(err) {
                    assert.ok(err);
                    assert.ok(err.code, 400);

                    // Verify that an existing authentication method is required
                    RestAPI.Admin.importUsers(globalAdminRestContext, global.oaeTests.tenants.cam.alias, getCSVWithoutPasswordStream, 'foobar', function(err) {
                        assert.ok(err);
                        assert.ok(err.code, 400);

                        // Verify that a CSV file is required
                        RestAPI.Admin.importUsers(globalAdminRestContext, global.oaeTests.tenants.cam.alias, null, 'cas', function(err) {
                            assert.ok(err);
                            assert.ok(err.code, 400);
                            callback();
                        });
        ...
        it('verify only admins can import users', function(callback) {
            ...
            RestAPI.Admin.importUsers(anonymousGlobalRestContext, global.oaeTests.tenants.cam.alias, getCSVWithoutPasswordStream, 'cas', function(err) {
                assert.ok(err);
                assert.ok(err.code, 401);

                // Verify that an anonymous user on a user tenant cannot import users
                RestAPI.Admin.importUsers(anonymousRestContext, null, getCSVWithoutPasswordStream, 'cas', function(err) {
                    assert.ok(err);
                    assert.ok(err.code, 401);
                    ...
                    RestAPI.User.createUser(camAdminRestContext, userId, 'password', 'Test User', {'visibility': 'public'}, function(err, createdUser) {
                        ...
                        RestAPI.Admin.importUsers(userRestContext, null, getCSVWithoutPasswordStream, 'cas', function(err) {
                            assert.ok(err);
                            assert.ok(err.code, 401);
                            callback();
                        });
