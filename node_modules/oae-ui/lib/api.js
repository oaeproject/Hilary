...
// The path that holds the skin and it's defined variables.
var BASE_SKIN_PATH = '/shared/oae/css/oae.skin.less';
...
var updateFileCaches = function(filename) {
    ...
    } else if (filename === BASE_SKIN_PATH) {
...
/**
 * Get the skin file for a tenant.
 *
...
var getSkinVariables = module.exports.getSkinVariables = function(ctx, tenantAlias, callback) {
    ...
    var applyTenantValues = function(err, skinVariables) {
        ...
        // Add in the value.
        _.each(tenant_variables, function(value, key) {
        ...
        // Sort them like the groups appear in the less file.
...
var _parseLessFile = function(variables, callback) {
    ...
    // Read the skin file.
    getStaticFile(BASE_SKIN_PATH, function(err, skin) {
        if (err) {
...
var _cacheSkinVariables = function(callback) {
    _parseLessFile(null, function(err, tree) {
        ...
        //       @body-background-color: #ECEAE5;           --> variable declaration
        ...
        for (var i = 0; i < tree.rules.length; i++) {
            ...
            if (rule.value && typeof rule.value === 'string' && (groupMatch = rule.value.match(groupRegex))) {
                ...
                // Create this group if it doesn't exist and add the variable.
                variables[name] = {
                    'name': name,
...
/**
 * Retrieves the CSS values that a tenant has specified he wishes to use in the skin value.
