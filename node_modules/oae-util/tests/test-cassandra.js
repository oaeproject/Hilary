...
describe('Utilities', function() {
    ...
    describe('Cassandra', function() {
        ...
        /**
         * Test whether the constructInsertCQL works as expected, making sure that invalid parameters
         * are handled appropriately
         */
        it('Test construct insert', function(callback) {
            // Test an invalid call with no provided cf
            var query1 = Cassandra.constructInsertCQL(null, {'key1': 'value1'});
            assert.ok(!query1); 
            // Test an invalid call with no provided values
            var query2 = Cassandra.constructInsertCQL('testCF', {});
            assert.ok(!query2);
            // Test a valid insert with one key-value pair
            var query3 = Cassandra.constructInsertCQL('testCF', {'key1': 'value1'});
            assert.ok(query3);
            assert.equal(query3.query, 'INSERT INTO testCF (key1) VALUES (?)');
            assert.equal(query3.parameters[0], 'value1');
            // Test a valid update with multiple key-value pairs
            var query4 = Cassandra.constructInsertCQL('testCF', {'key1': 'value1', 'key2': 'value2', 'key3': 'value3'});
            assert.ok(query4);
            assert.equal(query4.query, 'INSERT INTO testCF (key1, key2, key3) VALUES (?, ?, ?)');
            assert.equal(query4.parameters[0], 'value1');
            assert.equal(query4.parameters[1], 'value2');
            assert.equal(query4.parameters[2], 'value3');
            callback();
        });
