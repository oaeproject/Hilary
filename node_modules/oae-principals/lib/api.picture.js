 ...
 */
const storePicture = module.exports.storePicture = function(ctx, principalId, file, callback) {
 ...
 */
const generateSizes = module.exports.generateSizes = function(ctx, principalId, x, y, width, callback) {
    callback = callback || function(err) {
        if (err) {
...
var _generateSizes = function(ctx, principal, x, y, width, callback) {
    ...
    ContentUtil.getStorageBackend(ctx, principal.picture.largeUri).get(ctx.tenant().alias, principal.picture.largeUri, (err, file) => {
        if (err) {
            return callback(err);
        }
...
var _storeCroppedPictures = function(ctx, principal, files, callback) {
    ...
    let options = _getProfilePictureStorageOptions(principal.id, now, 'small', ImageUtil.getImageExtension(smallImage.name, '.jpg'));
