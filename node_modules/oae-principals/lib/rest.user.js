 ...
 * @FormParam   {string}        email                       The email to verify
...
OAE.tenantRouter.on('post', '/api/user/email/verify', function(req, res) {
    PrincipalsAPI.verifyEmail(req.ctx, req.body.email, req.body.token, function(err, user) {
        if (err) {
            return res.send(err.code, err.msg);
        }
        ...
        if (!req.ctx.user()) {
            return AuthenticationUtil.handleLogin('local', user, req, res);
 ...
 * @Server      admin,tenant
 * @Method      POST
 * @Path        /user/email/{userId}/resend-email
...
OAE.globalAdminRouter.on('post', '/api/user/:userId/resend-email', _handleResendEmailToken);
OAE.tenantRouter.on('post', '/api/user/:userId/resend-email', _handleResendEmailToken);
