 ...
 * };
 *
 * > getObjectProperty(data, 'options.fruit.yellow.banana', 0)
 * 4
 *
 * > getObjectProperty(data, 'non.existing.expression', 0);
 * 0
 * ```
 *
 * @param  {Object}     obj             The object to search for the expression in
 * @param  {String}     expression      The expression to look for
 * @param  {Object}     defaultVal      In case the expression could not be found, this value will be returned
 */
var getObjectProperty = module.exports.getObjectProperty = function(obj, expression, defaultVal) {
    if (!expression) {
        return defaultVal;
    }
    ...
    if (keys.length === 1) {
        ...
        if (_.has(obj, key)) {
            ...
            return getObjectProperty(obj[key], keys.slice(1).join('.'), defaultVal);
        } else {
