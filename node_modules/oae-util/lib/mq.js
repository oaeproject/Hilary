...
var init = module.exports.init = function(mqConfig, callback) {
    ...
    if (!connection) {
        ...
        connection.on('ready', function() {
            ...
            // FIXME: Analyzing the debug output of amqp, it appears this exchange call here may be faulty in the context
            // of a "reconnect", as the original channel will remain open. This always seems to create duplicate channels
            // open to the exchange which go unused. The additional channel(s) (presumably one per reconnect) seem
            // more-or-less harmless and has been happening for quite some time, and at the moment is not worth doing the
            // testing needed to tighten this up
...
var bind = module.exports.bind = function(name, listener, options, callback) {
    ...
    var queueOptions = _.defaults(options, Constants.DEFAULT_TASK_QUEUE_OPTS);
    connection.queue(name, queueOptions, function(queue) {
        log().info({'queueName': name}, 'Created/Retrieved a RabbitMQ task queue');
        ...
        queue.bind(Constants.DEFAULT_TASK_EXCHANGE_NAME, name, function() {
            ...
            if (purgeQueuesOnStartup && !startupPurgeStatus[name]) {
                // Ensure this queue only gets purged the first time we connect
                startupPurgeStatus[name] = true;

                // Purge the queue before subscribing the handler to it if we are configured to do so.
                purge(name, function(err) {
                    if (err) {
                        return callback(err);
                    }

                    return _subscribe(queue, name, subscribeOptions, listener, callback);
                });
            } else {
                return _subscribe(queue, name, subscribeOptions, listener, callback);
            }
 ...
 *
 * @param  {Queue}      queue               An AMQP Queue to which we'll subscribe the listener
 * @param  {String}     name                The name that the queue was created under
 * @param  {Object}     subscribeOptions    The options with which we wish to subscribe to the queue
 * @param  {Function}   listener            The function that will handle tasks delivered from the queue
 * @param  {Object}     listener.data       The data that was sent in the job. This is different depending on the type of job
 * @param  {Function}   callback            Invoked when the listener has been subscribed
 * @param  {Object}     callback.err        An error that occurred, if any
 * @api private
 */
var _subscribe = function(queue, name, subscribeOptions, listener, callback) {
...
var submit = module.exports.submit = function(name, data, callback) {
    ...
    queueExchange.publish(name, data, _.extend({}, Constants.DEFAULT_TASK_QUEUE_PUBLISH_OPTS));
    return callback();
};
