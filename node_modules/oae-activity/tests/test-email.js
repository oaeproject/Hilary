...
describe('Activity Email', function() {
  ...
  var refreshConfiguration = function(
    ...
    var now = timezoneString
      ? new TZ.Date(timezoneString)
      : new TZ.Date('Etc/UTC');
      ...
      TestsUtil.generateTestUsers(camAdminRestContext, 3, function(
        ...
        EmailTestUtil.collectAndFetchAllEmails(function() {
          ...
          var _assertEmailFooter = function(message, assertions) {
            ...
            assert.notEqual(
              message.html.indexOf('Open Academic Environment</a>'),
              -1,
            );
            ...
            if (assertions.expectInstanceURL) {
              ...
              assert.notEqual(
                message.html.indexOf(util.format('%s</a>', instanceName)),
                -1,
              );
            } else {
              assert.strictEqual(
                message.html.indexOf(
                  util.format('<a href="%s"\n ', instanceURL),
                ),
                -1,
              );
              assert.strictEqual(
            ...
            if (assertions.expectHostingOrganizationURL) {
              ...
              assert.notEqual(
                message.html.indexOf(
                  util.format('%s</a>', hostingOrganization),
                ),
                -1,
              );
            } else {
                      ...
                      RestAPI.Discussions.createDiscussion(
                        ...
                        function(err, firstDiscussion) {
                          ...
                          EmailTestUtil.collectAndFetchAllEmails(function(
                            ...
                            assert.ok(
                              messages[0].html.indexOf(
                                simong.user.profilePath,
                              ) > 0,
                            );

                            // Assert the order of activities is correct (oldest at the bottom)
                            var contentIndex = messages[0].html.indexOf(
                              firstLink.profilePath,
                            );
                            var discussionIndex = messages[0].html.indexOf(
                              firstDiscussion.profilePath,
                            );
                              ...
                              function(err, secondDiscussion) {
                                ...
                                EmailTestUtil.collectAndFetchAllEmails(function(
                                  ...
                                  _.each(messages, function(message) {
                                    ...
                                    assert.equal(
                                      messages[0].html.indexOf(
                                        secondLink.profilePath,
                                      ),
                                      -1,
                                    );
                                    assert.equal(
                                      messages[0].html.indexOf(
                                        thirdLink.profilePath,
                                      ),
                                      -1,
                                    );
                                    assert.equal(
                                      messages[0].html.indexOf(
                                        fourthLink.profilePath,
                                      ),
                                      -1,
                                    );
                                    assert.equal(
      ...
      RestAPI.Content.createLink(
        ...
        function(err, firstContentObj) {
          ...
          EmailTestUtil.collectAndFetchAllEmails(function(messages) {
            ...
            assert.ok(
              messages[0].html.indexOf(firstContentObj.displayName) > 0,
            );
            assert.ok(
              messages[0].html.indexOf(firstContentObj.profilePath) > 0,
            );
            ...
            RestAPI.Content.createLink(
              ...
              function(err, secondContentObj) {
                ...
                EmailTestUtil.collectAndFetchAllEmails(function(messages) {
                  ...
                  assert.equal(
                    messages[0].html.indexOf(firstContentObj.profilePath),
                    -1,
                  );
                  assert.ok(
                    messages[0].html.indexOf(secondContentObj.displayName) > 0,
                  );
                  ...
                  RestAPI.Discussions.createDiscussion(
                    ...
                    function(err, discussion) {
                      ...
                      EmailTestUtil.collectAndFetchAllEmails(function(
                        ...
                        assert.equal(
                          messages[0].html.indexOf(
                            secondContentObj.displayName,
                          ),
                          -1,
                        );
                        assert.ok(
                          messages[0].html.indexOf(discussion.displayName) > 0,
                        );
                ...
                function(err) {
                  ...
                  RestAPI.User.updateUser(
                    ...
                    function(err) {
                      ...
                      refreshConfiguration(null, false, false, {}, function(
                        config,
                      ) {
                        // Trigger an activity
                        RestAPI.Content.createLink(
  ...
  it('verify sending emails uses the configured tenant default timezone', function(callback) {
    ...
    refreshConfiguration(null, true, false, {}, function(config) {
      ...
      var createTenant = function() {
        ...
        TestsUtil.createTenantWithAdmin(alias, host, function(
          err,
          tenant,
          restContext,
          user,
    ...
    TestsUtil.createTenantWithAdmin(alias, host, function(
      ...
      ConfigTestUtil.updateConfigAndWait(
        ...
        function(err) {
          ...
          TestsUtil.generateTestUsers(restContext, 2, function(
            err,
            users,
            simong,
            nico,
            ...
            RestAPI.User.updateUser(
              ...
              function(err) {
                ...
                refreshConfiguration('Etc/GMT+5', false, false, {}, function(
                  config,
                ) {
                                            ...
                                            function(err) {
                                              ...
                                              ActivityEmail.collectMails(
                                                ...
                                                function(err, empty, users) {
                                                  ...
                                                  assert.equal(users.length, 1);
                                                  assert.ok(
                                                    _.find(users, function(
                                                      user,
                                                    ) {
                                                      return (
                                                        nico.user.id === user.id
                                                      );
                                                    }),
                                                  );
                                                  return callback();
                ...
                RestAPI.Content.createLink(
                  ...
                  function(err, link) {
                    ...
                    EmailTestUtil.collectAndFetchAllEmails(function(messages) {
                      ...
                      assert.ok(
                        mail.subject.indexOf(simong.user.displayName) !== -1,
                      );
                          ...
                          RestAPI.Discussions.createDiscussion(
                            ...
                            function(err, firstDiscussion) {
                              ...
                              EmailTestUtil.collectAndFetchAllEmails(function(
                                ...
                                assert.equal(
                                  secondMail.subject.indexOf('__MSG__'),
                                  -1,
                                );
              ...
              RestAPI.Discussions.createMessage(
                ...
                function(err, comment) {
                  ...
                  EmailTestUtil.collectAndFetchAllEmails(function(messages) {
                    ...
                    var mail = _.find(messages, function(message) {
                    ...
                    _.each(htmlBody, function(htmlFragment) {
                      assert.notEqual(
                        mail.html.indexOf(htmlFragment),
                        -1,
                        htmlFragment,
                      );
                    });
              ...
              RestAPI.Content.createLink(
                ...
                function(err, link) {
                  ...
                  EmailTestUtil.collectAndFetchAllEmails(function(messages) {
                    ...
                    var utcMessage = _.find(messages, function(message) {
                      return message.to[0].address === nico.user.email;
                    });
                    var plus5Message = _.find(messages, function(message) {
                      return message.to[0].address === bert.user.email;
                    });

                    // Parse the dates out of the messages
                    var utcDate = utcMessage.html.match(
                      /\d\d\d\d-\d\d-\d\d \d\d:\d\d:\d\d/g,
                    )[0];
                    var plus5Date = plus5Message.html.match(
                      /\d\d\d\d-\d\d-\d\d \d\d:\d\d:\d\d/g,
                    )[0];
                ...
                function(err) {
                  ...
                  RestAPI.User.updateUser(
                    ...
                    function(err) {
                      ...
                      refreshConfiguration(null, true, true, {}, function(
                        config,
                      ) {
                                ...
                                function(err, commentD) {
                                  ...
                                  EmailTestUtil.collectAndFetchAllEmails(
                                    function(messages) {
                                      ...
                                      assert.equal(
                                        simongMessage.html.match(
                                          /activity-comment-container/g,
                                        ).length,
                                        4,
                                      );
                                      assert.ok(
                                        simongMessage.html.indexOf(
                                          commentA.body,
                                        ) > 0,
                                      );
                                      assert.ok(
            ...
            ActivityTestUtil.collectAndGetActivityStream(
              ...
              function(err, activityStream) {
                ...
                EmailTestUtil.collectAndFetchAllEmails(function(messages) {
                  ...
                  assert.notEqual(
                    html.indexOf(
                      util.format('%s...', displayName.slice(0, 30)),
                    ),
                    -1,
                  );
  ...
  it('verify mail polling frequency is bounded', function(callback) {
    ...
    refreshConfiguration(
      ...
      false,
      false,
      { mail: { pollingFrequency: 7200 } },
      function() {
        assert.strictEqual(
          ActivitySystemConfig.getConfig().mail.pollingFrequency,
          3600,
        );
                  ...
                  RestAPI.Content.createLink(
                    ...
                    function(err) {
                      ...
                      EmailTestUtil.collectAndFetchAllEmails(function(
                        ...
                        assert.strictEqual(messages.length, 1);
                        assert.notEqual(
                          messages[0].html.indexOf('in the group'),
                          -1,
                        );
                        return callback();
                      });
