...
var createReply = module.exports.createReply = function(contentId, tenantAlias, commentReplyingTo, creatorId, body, callback) {
    ...
    // These queries cannot be run in a batch query as when the first one fails, the second should *NOT* be persisted.
    ...
    Cassandra.runQuery(q, [created, commentId, contentId], function(err) {
        ...
        Cassandra.runQuery(q, [commentId, body, creatorId, commentReplyingTo.commentId, commentReplyingTo.level + 1, contentId, created], function(err) {
            if (err) {
                return callback(err);
            }

            var comment = new Comment(commentId, body, contentId, creatorId, created, commentReplyingTo.level + 1, commentReplyingTo.commentId, false);
            return callback(null, comment);
        });
    });
};
