...
var _produceContentBodyDocuments = function(resources, callback, _documents) {
    ...
    ContentDAO.Revisions.getRevisions(resource.id, null, 1, null, function(err, revisions) {
        ...
        if (!revision.previews || !revision.previews.pageCount) {
            // This content doesn't have text pages to index
        ...
        var _done = _.after(revision.previews.pageCount, function() {
            _documents.push(SearchUtil.createChildSearchDocument(ContentConstants.search.MAPPING_CONTENT_BODY, resource.id, {'content_body': body.join('\n')}));
        ...
        var _getPage = function(index) {
            ContentDAO.Previews.getContentPreview(revision.previewsId, util.format('page.%s.html', index), function(err, preview) {
                if (err) {
                    return callback(err);
                }
        ...
        _.range(1, revision.previews.pageCount).forEach(_getPage);
    });
};
