...
Pubsub.on('oae-config', function(tenantAlias) {
    ...
    if (tenantAlias === config.servers.globalAdminAlias) {
        _cacheAllTenantConfigs(_handleCachedConfig);
    } else {
        _cacheTenantConfig(tenantAlias, _handleCachedConfig);
    }
});
...
var _handleTenantEvent = function(tenant) {
    _cacheTenantConfig(tenant.alias);
};
...
var _cacheAllTenantConfigs = function(callback) {
    ...
    _getAllPersistentTenantConfigs(function(err, adminPersistentConfig, persistentConfigsByTenantAlias) {
        if (err) {
            return callback(err);
        }

        // First cache the admin config as the user tenants will inherit from it
        var adminTenantAlias = config.servers.globalAdminAlias;
        ConfigAPI.emit('preCache', adminTenantAlias);
        _cachePersistentTenantConfig(adminTenantAlias, adminPersistentConfig);
        ConfigAPI.emit('cached', adminTenantAlias);

        _.each(persistentConfigsByTenantAlias, function(persistentConfig, tenantAlias) {
            // Indicate that config values are about to be re-cached
            ConfigAPI.emit('preCache', tenantAlias);

            _cachePersistentTenantConfig(tenantAlias, persistentConfig);
...
var _cacheTenantConfig = function(tenantAlias, callback) {
    callback = callback || function(err) {
        if (err) {
...
var _getAllPersistentTenantConfigs = function(callback) {
    Cassandra.runQuery('SELECT "tenantAlias", "configKey", "value", WRITETIME("value") FROM "Config"', null, function(err, rows) {
        ...
        // Distinguish between the admin persistent config and the other persistent configs
        var persistentConfigsByTenantAlias = _rowsToConfig(rows);
        var adminPersistentConfig = persistentConfigsByTenantAlias[config.servers.globalAdminAlias] || {};
        delete persistentConfigsByTenantAlias[config.servers.globalAdminAlias];

        // Ensure we have a non-null config for each tenant, if they don't have any configuration,
        // then it will just be an empty object
        _.chain(TenantsAPI.getTenants())
            .pluck('alias')
            .each(function(tenantAlias) {
                persistentConfigsByTenantAlias[tenantAlias] = persistentConfigsByTenantAlias[tenantAlias] || {};
            })
            .value();

        return callback(null, adminPersistentConfig, persistentConfigsByTenantAlias);
    });
};
...
/**
 * Apply the given persistent tenant config to the cache for the specified tenant
 *
 * @param  {String}     tenantAlias         The ID of the tenant for which to cache the configuration
 * @param  {Object}     persistentConfig    The stored configuration for the specified tenant
 * @api private
 */
var _cachePersistentTenantConfig = function(tenantAlias, persistentConfig) {
...
var _rowsToConfig = function(rows) {
    ...
    _.each(rows, function(row) {
        ...
        persistentConfig[hash.tenantAlias][hash.configKey] = {
            'value': hash.value,
            'timestamp': new Date(Math.floor(hash['writetime(value)'] / 1000))
        };
    });
