...
OAE.tenantServer.post('/api/group/create', function(req, res) {
    ...
    PrincipalsAPI.createGroup(req.ctx, req.body.displayName, req.body.description, req.body.visibility, req.body.joinable, memberHash, {}, function(err, groupId) {
        if (err) {
...
OAE.tenantServer.post('/api/group/:id', function(req, res) {
    ...
    PrincipalsAPI.updateGroup(req.ctx, req.params.id, req.body, function(err, newGroup) {
        if (err) {
            return res.send(err.code, err.msg);
        }
        res.send(200, newGroup);
    });
});
