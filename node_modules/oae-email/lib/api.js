 ...
 * @param  {Boolean}    [opts.ignoreVerification]       If `true` will ignore the `toUser.emailVerified` flag and send the email regardless. Defaults to `false`
 * @param  {Function}   [callback]                      Invoked when the email has been sent
 * @param  {Object}     [callback.err]                  An error that occurred, if any
 * @param  {Boolean}    [callback.ignored]              `true` if no email was sent to the user because his email address wasn't verified yet
 */
var sendEmail = module.exports.sendEmail = function(templateModule, templateId, toUser, data, opts, callback) {
    ...
    // Unless we were told to ignore the `emailVerified` flag, we don't send
    // emails to users who haven't verified their email address yet
    if (!opts.ignoreVerification && !toUser.emailVerified) {
