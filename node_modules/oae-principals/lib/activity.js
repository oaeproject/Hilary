...
/////////////////////////////////////////////////////////
// GROUP-JOIN / GROUP-ADD-MEMBER / GROUP-CHANGE-MEMBER //
/////////////////////////////////////////////////////////
...
ActivityAPI.registerActivityType(PrincipalsConstants.activity.ACTIVITY_GROUP_CHANGE_MEMBER, {
    'groupBy': [{'actor': true, 'target': true}],
    'routers': {
        'activity': {
            'actor': ['self'],
 ...
 */
PrincipalsAPI.on(PrincipalsConstants.events.UPDATED_GROUP_MEMBERS, function(ctx, group, members, computedPermissions, newMemberIds, updatedMemberIds, removedMemberIds) {
    var millis = Date.now();
    var actorResource = new ActivityModel.ActivitySeedResource('user', ctx.user().id, {'user': ctx.user()});
    ...
    for (var i = 0; i < principalIds.length; i++) {
        ...
        if (members[principalId]) {
            ...
            // When the member has ben added
            if (newMemberIds.indexOf(principalId) > -1) {
            ...
            // When the member has been updated
            if (updatedMemberIds.indexOf(principalId) > -1) {
                ActivityAPI.postActivity(ctx, new ActivityModel.ActivitySeed(PrincipalsConstants.activity.ACTIVITY_GROUP_CHANGE_MEMBER, millis, ActivityConstants.verbs.UPDATE, actorResource, objectResource, targetResource));
            }
        }
    }
});
