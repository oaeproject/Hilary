...
var setUpTenants = function(err, callback) {
    ...
    TenantsAPI.createTenant(globalContext, 'camtest', 'Cambridge University Test', 'cambridge.oae.com', function(err, tenant) {
        ...
        TenantsAPI.createTenant(globalContext, 'gttest', 'Georgia Tech Test', 'gt.oae.com', function(err, tenant) {
            ...
            TenantsAPI.createTenant(globalContext, 'signedtest', 'Test for cross tenant authentication', 'localhost:2001', function(err, tenant) {
                if (err) {
                    return callback(err);
                }
                global.oaeTests.tenants.signed = tenant;
...
var setupTenantAdmin = function(tenant, callback) {
    ...
    var adminUser = new User(tenant.alias, null, 'The admin User', {
        'isGlobalAdmin': false,
        'isTenantAdmin': true
    });
...
var setUpTenantAdmins = function(callback) {
    ...
    var signedTenant = global.oaeTests.tenants.signed;

    setupTenantAdmin(camTenant, function() {
        setupTenantAdmin(gtTenant, function() {
            setupTenantAdmin(signedTenant, callback);
        });
    });
};
            ...
            Cassandra.close(function() {
                ...
                Redis.flush(function(err) {
                    OAE.init(config, function(err) {
                        _bindRequestLogger();
                        setUpTenants(err, function(previewErr) {
                            log().info('Disabling the preview processor during tests.');
                            PreviewAPI.disable(callback);
                        });
                    });
                });
            });
