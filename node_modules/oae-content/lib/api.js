...
var LoodleAPI = require('oae-doodle/lib/api').doodles;
var UserAPI = require('oae-doodle/lib/api').users;
...
var publishLoodleNotification = module.exports.publishLoodleNotification = function (ctx, contentId, callback) {

    ContentDAO.Content.getContent(contentId, function(err, contentObj) {
        if (err)
            return callback(err);

        ContentAPI.emit(ContentConstants.events.EDITED_DOODLE, ctx, contentObj);
        return callback();
    });
    ...
    ContentDAO.Content.getContent(contentId, function(err, contentObj) {
        ...
        AuthzPermissions.canView(ctx, contentObj, function(err) {
            ...
            if (contentObj.resourceSubType === 'doodle') {
                LoodleAPI.get(currentUser, contentObj.id, function (err, data) {
                    if (err)
                        return callback(err);

                    contentObj.loodleData = data;

                    UserAPI.get(currentUser.id, function (err, data) {
...
var createDoodle = module.exports.createDoodle = function (ctx, displayName, description, visibility, additionalMembers, folders, callback) {
    ...
    LoodleAPI.create(ctx, currentUser, contentId, displayName, description, additionalMembers, function (err) {
        if (err)
            return callback(err);
...
var deleteContent = module.exports.deleteContent = function(ctx, contentId, callback) {
    ...
    _canManage(ctx, contentId, function(err, contentObj) {
        ...
        ContentDAO.Content.deleteContent(contentObj, function(err, members) {
            ...
            if (contentObj.resourceSubType === 'doodle') {
                LoodleAPI.delete(currentUser, contentObj.id, function (err) {
                    if (err)
                        return callback(err);
