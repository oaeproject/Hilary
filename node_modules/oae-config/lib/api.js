 ...
 * Tenant administrators and regular users cannot update values marked as `globalAdminOnly`.
 *
 * @param  {Context}   ctx        Standard context object, representing the currently logged in user and its tenant
 ...
 */
var canUpdateConfigValue = function(ctx, moduleId, feature, element) {
    var globalAdminOnly = cachedGlobalSchema[moduleId] &&
                          ...
                          cachedGlobalSchema[moduleId][feature].elements[element].globalAdminOnly === true;
...
var updateConfig = module.exports.updateConfig = function(ctx, tenantAlias, configValues, callback) {
    ...
    for (var i = 0; i < configFields.length; i++) {
        ...
        var element = configFields[i].split('/');
        var canUpdateValue = canUpdateConfigValue(ctx, configFields[i].split('/')[0], configFields[i].split('/')[1], configFields[i].split('/')[2]);
        if (!canUpdateValue) {
            return callback({'code': 401, 'msg': 'Only global administrators can change this config value'});
        }
    }
    if (validator.hasErrors()) {
        return callback(validator.getFirstError());
