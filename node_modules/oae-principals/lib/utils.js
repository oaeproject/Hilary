...
module.exports.getPrincipal = function(principal_id, callback) {
    cassandra.runQuery('SELECT * FROM Principals WHERE principal_id = ?', [principal_id], function (err, rows) {
        ...
        if (rows[0]._colCount === 1) {
            return callback({'code': 404, 'msg': "Couldn't find principal: " + principal_id}, null);
        }
        ...
        if (principal.privacy_setting === 'private') {
            return callback({'code': 404, 'msg': "Couldn't find that principal: " + principal_id}, null);
        }
...
var rowToGroup = function (row) {
    return new Group(row.colHash.tenant, row.colHash.principal_id, row.colHash.privacy_setting, row.colHash.group_title, row.colHash.group_description);
};
...
var rowToUser = function(row) {
    return new User(row.colHash.tenant, row.colHash.principal_id, row.colHash.user_username, row.colHash.privacy_setting, row.colHash.user_first_name, row.colHash.user_last_name, row.colHash.user_display_name);
};