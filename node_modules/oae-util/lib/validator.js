...
Validator.prototype.isGlobalAdministratorUser = function(ctx) {
  if (!_.isObject(ctx)) {
    ...
    !_.isFunction(ctx.tenant) ||
    !_.isObject(ctx.tenant()) ||
    !ctx.tenant().alias
  ) {
    this.error(this.msg || 'The context is not associated to a tenant');
  } else if (
    !_.isFunction(ctx.user) ||
    !_.isObject(ctx.user()) ||
    !ctx.user().id
  ) {
    this.error(this.msg || 'The user is not logged in');
...
Validator.prototype.isDefined = function(val) {
  var isDefined = !_.isNull(val) && !_.isUndefined(val);
  if (!isDefined) {
...
Validator.prototype.isIso3166Country = function() {
  if (!_.isString(this.str)) {
    ...
    this.error(
      this.msg ||
        'Provided country code is not associated to any known country',
    );
  }
};
 ...
 * @api private
 */
var _hasCountryCode = function(code) {
  if (!countriesByCode) {
    // Lazy initialize the country code array so as to not form an cross-
    // dependency on `oae-ui`
    countriesByCode = _.chain(
      require('oae-ui').getIso3166CountryInfo().countries,
    )
      .indexBy('code')
      .mapObject(function() {
        return true;
