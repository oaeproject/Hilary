    ...
    _produceAllEntities(activitySeed, function(err, actor, object, target) {
        ...
        _produceAllRoutes(activitySeed, actor, object, target, function(err, routes) {
            ...
            _.each(routes, function(route) {
                ...
                var stream = ActivityRegistry.getRegisteredActivityStreamType(route.streamType);
                if (stream && stream.visibilityBucketing) {
                    ...
                    if (_canRouteToPublicStream(actor, object, target)) {
                        ...
                        if (publicGroupEntity) {
                            ...
                            if (!route.transient) {
                                activityStreamId = ActivityUtil.createActivityStreamId(publicGroupEntity.id, route.streamType + '#public');
                                allDeliveredActivities[activityStreamId] = activity;
                            }
                        }
                    }
                    ...
                    if (_canRouteToLoggedinStream(actor, object, target)) {
                        ...
                        if (AuthzUtil.isUserId(route.resourceId) && route.resourceId === actor.id) {
                            ...
                            if (!route.transient) {
                                activityStreamId = ActivityUtil.createActivityStreamId(route.resourceId, route.streamType + '#loggedin');
                                allDeliveredActivities[activityStreamId] = activity;
                            }
                        }
                        ...
                        if (loggedinGroupEntity) {
                            ...
                            if (!route.transient) {
                                activityStreamId = ActivityUtil.createActivityStreamId(loggedinGroupEntity.id, route.streamType + '#loggedin');
                                allDeliveredActivities[activityStreamId] = activity;
                            }
                        }
                    }
            ...
            log().trace({'activitySeed': activitySeed, 'allRoutedActivities': allRoutedActivities}, 'Finished routed activities');

            // Queue the activity entities to be collected and aggregated
            _queueActivities(activitySeed, allDeliveredActivities, function(err, buckets) {
                if (err) {
                    return callback(err);
                }
                        ...
                        _producePropagation(associationsContexts.target, target, function(err, targetPropagationRules) {
                            ...
                            var allRoutes = _.chain(actorRoutes)
                                ...
                                .filter(function(route) {
                                    if (route.streamType === ActivityConstants.streams.NOTIFICATION || route.streamType === ActivityConstants.streams.EMAIL) {
                                        // If this is a notification or email route, we can only accept users other than the actor
                                        var isPrincipalRoute = (AuthzUtil.isUserId(route.resourceId) || AuthzUtil.isEmail(route.resourceId));
                            ...
                            _applyPropagations(ActivityConstants.entityTypes.ACTOR, actorPropagationRules, allRoutes, associationsContexts, actor, actorRoutes, function(err, includedRoutes) {
                                ...
                                _applyPropagations(ActivityConstants.entityTypes.OBJECT, objectPropagationRules, includedRoutes, associationsContexts, object, objectRoutes, function(err, includedRoutes) {
                                    ...
                                    _applyPropagations(ActivityConstants.entityTypes.TARGET, targetPropagationRules, includedRoutes, associationsContexts, target, targetRoutes, function(err, includedRoutes) {
                                        ...
                                        // TODO: Figure out if private resource propagation will thwart emails from being sent
...
var _canRouteToPublicStream = function(actor, object, target) {
    return (actor['oae:visibility'] === 'public' &&
...
var _canRouteToLoggedinStream = function(actor, object, target) {
    return ((actor['oae:visibility'] === 'public' || actor['oae:visibility'] === 'loggedin') &&
