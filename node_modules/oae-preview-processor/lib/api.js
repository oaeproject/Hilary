 ...
 * ### Events
 *
 *  * `processed(contentObj, revision)` - Indicates a revision for a piece of content has finished processing
 */
var PreviewProcessorAPI = module.exports = new events.EventEmitter();
...
var _handleGeneratePreviewsTask = function(data, callback) {
    ...
    ctx.login(function(err) {
        ...
        _generatePreviews(ctx, function(err) {
            ctx.cleanup();
            Telemetry.appendDuration('process.time', start);
            PreviewProcessorAPI.emit(PreviewConstants.EVENTS.PREVIEWS_FINISHED, ctx.content, ctx.revision);
            if (err) {
