...
describe('Utilities', function() {
    ...
    describe('Cassandra', function() {
        ...
        it('Test column family', function(callback) {
            ...
            Cassandra.createColumnFamily(name, "CREATE COLUMNFAMILY " + name + " (key_id text PRIMARY KEY)", function(err, created) {
                assert.ok(!err);
                assert.ok(created);
        ...
        it('Test multiple column families', function(callback) {
            ...
            Cassandra.createColumnFamilies({
                name1: 'CREATE COLUMNFAMILY ' + name1 + ' (key_id text PRIMARY KEY)',
                name2: 'CREATE COLUMNFAMILY ' + name2 + ' (key_id text PRIMARY KEY)'
            }, function(err) {
                assert.ok(!err);
                // Check if it exists.
        ...
        it('Test run query', function(callback) {
            // Create a CF first
            Cassandra.createColumnFamily('testQuery', "CREATE COLUMNFAMILY testQuery (key_id text PRIMARY KEY)", function(err, created) {
                assert.ok(!err);
                assert.ok(created);
                ...
                Cassandra.columnFamilyExists('testQuery', function(err, exists) {
                    ...
                    Cassandra.runQuery('INSERT INTO testQuery (key_id, c1, c2) VALUES (?, ?, ?) USING CONSISTENCY QUORUM', ['key1', 'value1', 'value2'], function (err) {
                        assert.ok(!err);
                        // Try to run an invalid insert
                        Cassandra.runQuery('INSERT INTO testQuery (key_id, c1, c2) VALUES (?, ?, ?) USING CONSISTENCY QUORUM', ['key2', 'value', null], function (err) {
                            assert.ok(err);
                            // Try to run a simple select
                            Cassandra.runQuery('SELECT * FROM testQuery USING CONSISTENCY QUORUM WHERE key_id = ?', ['key1'], function (err, rows) {
                                assert.ok(!err);
                                assert.equal(rows.length, 1);
                                assert.equal(rows[0].key, 'key1');
                                // Try to run an invalid select
                                Cassandra.runQuery('SELECT * FROM testQuery USING CONSISTENCY QUORUM WHERE key_id = ?', [null], function (err, rows) {
                                    assert.ok(err);
                                    callback();
                                });
                            });
         ...
         */
        it('Test casting to a Boolean', function(callback) {
            Cassandra.createColumnFamily('testBooleans', "CREATE COLUMNFAMILY testBooleans (key_id text PRIMARY KEY)", function(err, created) {
                assert.ok(!err);
                assert.ok(created);
                Cassandra.runQuery('INSERT INTO testBooleans (key_id, testbool, testnumbool, teststring) VALUES (?, ?, ?, ?) USING CONSISTENCY QUORUM', ['testkey', 'true', '0', 'notaboolean'], function (err) {
                    assert.ok(!err);
                    Cassandra.runQuery('SELECT testbool, testnumbool, teststring FROM testBooleans USING CONSISTENCY QUORUM WHERE key_id = ?', ['testkey'], function (err, rows) {
                        assert.ok(!err);
                        assert.equal(typeof Cassandra.castToBoolean(rows[0][0].value), 'boolean');
                        assert.equal(typeof Cassandra.castToBoolean(rows[0][1].value), 'boolean');
                        assert.equal(typeof Cassandra.castToBoolean(rows[0][2].value), 'string');
                        callback();
                    });
                });
            });
         ...
         */
        it('Test construct upser', function(callback) {
