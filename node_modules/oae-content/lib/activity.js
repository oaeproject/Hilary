 ...
 */
ContentAPI.on(ContentConstants.events.CREATED_CONTENT, function(ctx, content, revision, members, folderIds) {
    var millis = Date.now();
    ...
    if (principalIds.length === 1) {
        // If there's only one principal in the `members` hash it means the user
        // created some content but didn't add anyone else. In that case we simply
        // generate an activity without a target
        ...
        // Generate an activity for each member. The current user does not get added
        // as a target but he'll receive an activity as we're including him in the actor
        // routing rules
        _.each(members, function(role, principalId) {
    ...
    // Generate an activity for each folder
    _.each(folderIds, function(folderId) {
        var targetResource = new ActivityModel.ActivitySeedResource('folder', folderId);
        var activitySeed = new ActivityModel.ActivitySeed(ContentConstants.activity.ACTIVITY_CONTENT_CREATE, millis, ActivityConstants.verbs.CREATE, actorResource, objectResource, targetResource);
        ActivityAPI.postActivity(ctx, activitySeed);
    });
});
 ...
 */
ContentAPI.on(ContentConstants.events.UPDATED_CONTENT_MEMBERS, function(ctx, content, permissionChanges, newMemberIds, updatedMemberIds) {
