...
/*!
 * When a batch of activities are delivered, we check if there are any notifications in there,
 * increment all the target user notifications and fire off emails.
 */
    ...
    incrementNotificationsUnread(userIdsIncrBy, function(err) {
        ...
        _.each(deliveredNotifications, function(activities, userId) {
            ...
            var activitiesToEmail = _.filter(activities, function(activity) {
                ...
                return (activityTypeOptions.streams.notification && activityTypeOptions.streams.notification.email);
            });

            if (_.isEmpty(activitiesToEmail)) {
            ...
            PrincipalsDAO.getPrincipal(userId, function(err, user) {
                ...
                // The user model contains a "compact" version of a tenant which only contains the alias and display name. The
                // notification context however needs the full tenant object
                ...
                ActivityTransformer.transformActivities(ctx, activitiesToEmail,  ActivityConstants.transformerTypes.ACTIVITYSTREAMS, function(err) {
                    if (err) {
                        _decrementNotifications();
                        return log().error({'err': err, 'userId': userId}, 'Failed to transform activities for notifications.');
                    }

                    _sendEmails(user, activitiesToEmail, function() {
                        _decrementNotifications();
                        return;
                    });
                });
...
var markNotificationsRead = module.exports.markNotificationsRead = function(userId, callback) {
    ...
    ActivityDAO.clearNotificationsUnreadCount(userId, function(err) {
        ...
        PrincipalsDAO.updatePrincipal(userId, profileFields, function(err) {
            ...
            // We can return here as resetting the activity aggregation can happen asynchronously
            ...
            var activityStreamId = ActivityUtil.createActivityStreamId(userId, 'notification');
            ActivityAggregator.resetAggregationForActivityStreams([activityStreamId]);
        });
    });
};
...
/**
 * Send an email notification for all the activities in the provided `activitiesToEmail` list.
 *
 * @param  {User}           user                The user to which to send the email
 * @param  {Activity[]}     activitiesToEmail   The activity objects for which to send notification emails
 * @param  {Function}       callback            Invoked when the process completes
 * @api private
 */
var _sendEmails = function(user, activitiesToEmail, callback) {
    if (_.isEmpty(activitiesToEmail)) {
        return callback();
    }

    // Grab the next activity from the list
    var activity = activitiesToEmail.pop();

    // Determine which module and template id should be used for this activity
    var notificationOptions = _getRegisteredActivityOptions(activity).streams.notification.email;
    ...
    var emailHash = util.format('%s:%s:%s', user.id, activity['oae:activityType'], activity.published);

    // Finally send the email
    EmailAPI.sendEmail(templateModule, templateId, user, {'activity': activity}, {'hash': emailHash}, function(err) {
        if (err) {
            log().error({'err': err}, 'Error sending e-mail to user');
        }

        // Continue to the next activity to email
        return _sendEmails(user, activitiesToEmail, callback);
    });
};
