...
/**
 * Determines if the specified config value is a known config value
 *
...
var updateConfig = module.exports.updateConfig = function(ctx, tenantAlias, configValues, callback) {
    ...
    _.each(configValues, function(configValue, configField) {
        validator.check(null, {'code': 400, 'msg': util.format('The configuration value for "%s" must be specified', configField)}).isDefined(configValue);

        var parts = configField.split('/');
        if (!_configValueExists(parts[0], parts[1], parts[2])) {
            return callback({'code': 404, 'msg': util.format('Config key "%s" does not exist', configField)});
        } else if (!_canUpdateConfigValue(ctx, tenantAlias, parts[0], parts[1], parts[2])) {
            return callback({'code': 401, 'msg': util.format('User is not allowed to update config value "%s"', configField)});
        }
    });
    if (validator.hasErrors()) {
        return callback(validator.getFirstError());
