...
describe('Content', function() {
    ...
    describe('Update content', function() {
        ...
        /**
         * Test that verifies that updating a link creates a new revision
         */
        it('verify updating a link creates a new revision', function(callback) {
            TestsUtil.generateTestUsers(camAdminRestContext, 1, function(err, users, simon) {
                assert.ok(!err);

                RestAPI.Content.createLink(simon.restContext, 'Apereo Foundation', 'The Apereo Foundation', 'private', 'http://www.apereo.org', [], [], function(err, originalLink) {
                    assert.ok(!err);
                        assert.strictEqual(originalLink.displayName, 'Apereo Foundation');
                        assert.strictEqual(originalLink.description, 'The Apereo Foundation');
                        assert.strictEqual(originalLink.link, 'http://www.apereo.org');

                    // Update the link, also update the displayName and description so we can assert that all updated metadata is persisted
                    RestAPI.Content.updateContent(simon.restContext, originalLink.id, {'displayName': 'Yahoo!', 'link': 'http://www.yahoo.com', 'description': 'Yahoo Portal'}, function(err, updatedLink) {
                        assert.ok(!err);
                        assert.strictEqual(updatedLink.displayName, 'Yahoo!');
                        assert.strictEqual(updatedLink.description, 'Yahoo Portal');
                        assert.strictEqual(updatedLink.link, 'http://www.yahoo.com');

                        // A new revision should be created
                        RestAPI.Content.getRevisions(simon.restContext, updatedLink.id, null, null, function(err, revisions) {
                            assert.ok(!err);
                            assert.ok(_.isArray(revisions.results));
                            assert.equal(revisions.results.length, 2);
                            assert.equal(revisions.results[0].revisionId, updatedLink.latestRevisionId);
                            assert.equal(revisions.results[0].link, 'http://www.yahoo.com');
                            assert.equal(revisions.results[1].revisionId, originalLink.latestRevisionId);
                            assert.equal(revisions.results[1].link, 'http://www.apereo.org');

                            // Revisions should be sorted as the most recent one first
                            RestAPI.Content.getRevision(simon.restContext, updatedLink.id, revisions.results[0].revisionId, function(err, revision) {
                                assert.ok(!err);
                                assert.equal(revision.revisionId, updatedLink.latestRevisionId);
                                assert.equal(revision.link, 'http://www.yahoo.com');
                                RestAPI.Content.getRevision(simon.restContext, updatedLink.id, revisions.results[1].revisionId, function(err, revision) {
                                    assert.ok(!err);
                                    assert.equal(revision.revisionId, originalLink.latestRevisionId);
                                    assert.equal(revision.link, 'http://www.apereo.org');
