...
var setupRouter = module.exports.setupRouter = function(app) {
    ...
    var _appendToApi = function(rootResource, api, spec) {
        ...
        _.each(spec.params, function (param) {
            ...
            validator.check(param.paramType, {'path': api.path, 'name': param.name, 'msg': 'Invalid param type: ' + param.paramType}).isIn([
                'path',
                'query',
                'body',
                'form',
        ...
        if (validator.hasErrors()) {
            _.each(validator.getErrors(), function(error) {
                log().warn({'swaggerValidationError': error});
            });
            log().warn('Some swagger documentation could not be parsed, the server will start but those routes may be unavailable or undocumented');
        }
