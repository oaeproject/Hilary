    ...
    getGroup(ctx, groupId, function(err, group) {
        ...
        var accessCheck = function(roles) {
            ...
            if (group.isMember) {
                ...
                var body = ctx.user().id + ':' + groupId;
                group.signature = Signature.createRollingSignature(tenantAlias, 2 * 60, 20, body);
            }
    ...
    canCreateGroup(ctx, groupId, function(err) {
        ...
        PrincipalsDAO.getPrincipals(_.keys(permissions), null, function(err, principals) {
            ...
            AuthzAPI.canInteract(ctx, tenantAlias, principals, function(err, canInteract, illegalPrinicpalIds) {
                ...
                var lastModified = Date.now().toString();

                // Create the group
                Cassandra.runQuery('INSERT INTO "Principals" ("principalId", "tenantAlias", "displayName", "description", "visibility", "joinable", "lastModified") VALUES (?, ?, ?, ?, ?, ?, ?)',
