...
var storePicture = module.exports.storePicture = function(ctx, principalId, file, callback) {
    ...
    _canManagePrincipal(ctx, principalId, function(err, principal) {
        ...
        // Rather than trusting the browser/user provided filetype
        // we try to determine it ourselves.
        mime(file.path, function(err, type) {
            if (err) {
                log().error({'err': err, 'file': file}, 'Could not determine the file type');
                return callback({'code': 500, 'msg': 'Could not determine the mime type of this file'});
            }
