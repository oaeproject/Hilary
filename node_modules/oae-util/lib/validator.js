...
/*!
 * Wrapper function around node-validator that allows use of an object for the
 * error message parameter.
 */
var originalCheck = Validator.prototype.check;
Validator.prototype.check = function (str, failMsg) {
    var result = originalCheck.apply(this, arguments);

    // If the `failMsg` was an object and did not contain a property for the function
    // name that failed validation, the `msg` will be null. To ensure we get the
    // object error message, we swap it from the error dictionary which is where
    // node-validator places the fail message if it is an object
    if (result.msg === null) {
        result.msg = result.errorDictionary;
    }
    return result;
};
...
/**
 * Check whether or not the passed in object is an array
 *
 * Usage:
 * ```
 * var validator = new Validator();
 ...
 * ```
 *
 * @param  {Array}     arr   Array that needs to be checked for validity
 */
 ...
 *     * At least 1 character long
 *     * At most 10.000 characters long
 *
 * Usage:
 * ```
 * var validator = new Validator();
 ...
 *     * At least 1 character long
 *     * At most 100.000 characters long
 *
 * Usage:
 * ```
 * var validator = new Validator();
