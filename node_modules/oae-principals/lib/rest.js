...
module.exports = function(tenant) {
    ...
    tenant.server.post('/api/user/create', function(req, res) {
        ...
        ConfigAPI.getConfig(req.ctx.tenant().alias, function(err, config) {
            var needsReCaptcha = config['oae-principals']['recaptcha']['elements']['enabled'].defaultValue;
            if (!needsReCaptcha) {
                return createUser();
            }
            ...
            recaptcha.checkAnswer(config['oae-principals']['recaptcha']['elements']['private-key'].defaultValue, req.connection.remoteAddress, req.body.recaptchaChallenge, req.body.recaptchaResponse);
        });
    });
