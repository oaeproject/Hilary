...
/**
 * Initiate a new transfer and return it.
 *
...
var initiateTransfer = module.exports.initiateTransfer = function (ctx, code, originalUserId, originalEmail, targetEmail, callback) { 
    ...
    _getTTLTransfer(ctx, function (err, TTL) {
        ...
        // Create transfer
        Cassandra.runQuery('INSERT INTO "Transfer" ("originalUserId", "code", "originalEmail", "targetEmail", "targetUserId", "status", "dateLastAction") VALUES (?, ?, ?, ?, ?, ?, ?) USING TTL ?', [originalUserId, code, originalEmail, targetEmail, '', TransferConstants.status.ONGOING, date.toString(), TTL], function(err) {
            if (err) {
                return callback(err);
            }
            Cassandra.runQuery('INSERT INTO "TransferByEmail" ("originalUserId", "code", "originalEmail", "targetEmail", "targetUserId", "status", "dateLastAction") VALUES (?, ?, ?, ?, ?, ?, ?) USING TTL ?', [originalUserId, code, originalEmail, targetEmail, '', TransferConstants.status.ONGOING, date.toString(), TTL], function(err) {
                if (err) {
                    return callback(err);
                }
                var transfer = new Transfer(originalUserId, code, originalEmail, targetEmail, null, TransferConstants.status.ONGOING, date);
...
var cancelTransfer = module.exports.cancelTransfer = function (originalEmail, code, originalUserId, callback) {
    var date = Date.now();
    Cassandra.runQuery('UPDATE "Transfer" SET "status" = ?, "dateLastAction" = ? WHERE "originalUserId" = ? AND "code" = ?', [TransferConstants.status.CANCELED, date.toString(), originalUserId, code], function(err) {
        if (err) {
            return callback(err);
        }
        Cassandra.runQuery('UPDATE "TransferByEmail" SET "status" = ?, "dateLastAction" = ? WHERE "originalEmail" = ? AND "code" = ?', [TransferConstants.status.CANCELED, date.toString(), originalEmail, code], function(err) {
            if (err) {
                return callback(err);
            }
            return callback();
...
var completeTransfer = module.exports.completeTransfer = function (code, originalUserId, originalEmail, targetEmail, targetUserId, callback) {
    var date = Date.now();
    Cassandra.runQuery('UPDATE "Transfer" SET "status" = ?, "dateLastAction" = ? WHERE "originalUserId" = ? AND "code" = ?', [TransferConstants.status.COMPLETED, date.toString(), originalUserId, code], function(err) {
        if (err) {
            return callback(err);
        }
        Cassandra.runQuery('UPDATE "TransferByEmail" SET "status" = ?, "dateLastAction" = ? WHERE "originalEmail" = ? AND "code" = ?', [TransferConstants.status.COMPLETED, date.toString(), originalUserId, code], function(err) {
            if (err) {
                return callback(err);
            }
            var transfer = new Transfer(originalUserId, code, originalEmail, targetEmail, targetUserId, TransferConstants.status.COMPLETED, date);
            return callback(null, transfer);
        });
    });
};
...
var _getTTLTransfer = function (ctx, callback) {
    ...
    Cassandra.runQuery('SELECT "value" FROM "Config" WHERE "tenantAlias" = ? AND "configKey" = ?', [ctx.tenant().alias, configkey], function(err, result) {
        ...
        if (_.isEmpty(result)) {
            ...
            return callback(null, result[0].value * SECONDS_IN_A_DAY);
        }
    });
};
