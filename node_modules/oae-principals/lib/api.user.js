 ...
 * Note that you still need to give this user a way of authenticating in to the system.
 *
...
var createUser = module.exports.createUser = function(ctx, displayName, opts, callback) {
    ...
    // Rather than storing if we accepted the T&C, we store the timestamp when we agreed to it
    // This allows for updating the T&C and making the user re-accept
                ...
                require('oae-authentication').getOrCreateUser(adminCtx, authenticationStrategy, externalId, displayName, opts, function(err, user) {
                    ...
                    if (data.length % 25 === 0) {
                        log(ctx).info({
                            'authenticationStrategy': authenticationStrategy,
                            'tenantAlias': tenantAlias
                        }, 'Importing users. ' + data.length + ' users left to import');
                    }
...
var updateUser = module.exports.updateUser = function(ctx, userId, profileFields, callback) {
    ...
    // Make sure the "Accepted T&C"-field is a boolean
    if (profileFields['acceptedTC']) {
        profileFields['acceptedTC'] = sanitize(profileFields['acceptedTC']).toBooleanStrict();
        if (profileFields['acceptedTC'] !== true) {
            return callback({'code': 400, 'msg': 'You must accept the terms and conditions'});
        }
        profileFields['acceptedTC'] = Date.now();
    }
