 ...
 */
DiscussionsAPI.on(DiscussionsConstants.events.CREATED_DISCUSSION, function(
  ctx,
  discussion,
) {
  var millis = Date.now();
 ...
 */
DiscussionsAPI.on(DiscussionsConstants.events.UPDATED_DISCUSSION, function(
  ctx,
  newDiscussion,
  oldDiscussion,
) {
...
DiscussionsAPI.on(
  ...
  function(ctx, message, discussion) {
    var millis = Date.now();
    var actorResource = new ActivityModel.ActivitySeedResource(
      'user',
      ctx.user().id,
    ...
    var objectResource = new ActivityModel.ActivitySeedResource(
      'discussion-message',
      message.id,
      { discussionId: discussion.id, message: message },
    );
...
DiscussionsAPI.on(
  ...
  function(ctx, discussion, memberChangeInfo, opts) {
    if (opts.invitation) {
    ...
    var discussionResource = new ActivityModel.ActivitySeedResource(
      'discussion',
      discussion.id,
      { discussion: discussion },
    );
    ...
    _.each(addedPrincipalIds, function(principalId) {
      if (principalId === ctx.user().id) {
        ...
        var principalResourceType = PrincipalsUtil.isGroup(principalId)
          ? 'group'
          : 'user';
...
var _discussionProducer = function(resource, callback) {
  ...
  if (discussion) {
    return callback(
      null,
      _createPersistentDiscussionActivityEntity(discussion),
    );
  }
...
var _transformPersistentDiscussionActivityEntity = function(ctx, entity) {
  ...
  var profileUrl =
    baseUrl + '/discussion/' + resource.tenantAlias + '/' + resource.resourceId;
...
var _discussionMessageTransformer = function(ctx, activityEntities, callback) {
  ...
  _.keys(activityEntities).forEach(function(activityId) {
    ...
    _.keys(activityEntities[activityId]).forEach(function(entityId) {
      ...
      var profilePath =
        '/discussion/' + resource.tenantAlias + '/' + resource.resourceId;
 ...
 * @see ActivityAPI#registerActivityEntityType
 */
var _discussionMessageInternalTransformer = function(
  ctx,
  activityEntities,
  callback,
) {
 ...
 */
ActivityAPI.registerActivityEntityAssociation('discussion', 'self', function(
  associationsCtx,
  entity,
  callback,
) {
 ...
 */
ActivityAPI.registerActivityEntityAssociation('discussion', 'members', function(
  associationsCtx,
  entity,
  callback,
) {
