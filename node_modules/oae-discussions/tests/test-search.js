...
describe('Discussion Search', function() {
    ...
    describe('Indexing', function() {
            ...
            TestsUtil.generateTestUsers(camAdminRestContext, 1, function(err, user) {
                ...
                RestAPI.Discussions.createDiscussion(user.restContext, randomText, randomText, 'public', null, null, function(err, discussion) {
                    ...
                    SearchTestsUtil.searchAll(user.restContext, 'general', null, {'resourceTypes': 'discussion', 'q': randomText}, function(err, results) {
                        ...
                        var resourceId = AuthzUtil.getResourceFromId(discussion.id).resourceId;
                        assert.equal(doc.profilePath, '/discussion/' + global.oaeTests.tenants.cam.alias + '/' + resourceId);
                        callback();
                    });
                });
            });
        ...
        /**
         * Verifies that messages get indexed and are searchable.
         */
        it('verify indexing of a message', function(callback) {
            TestsUtil.generateTestUsers(camAdminRestContext, 1, function(err, user) {
                ...
                RestAPI.Discussions.createDiscussion(user.restContext, randomText, randomText, 'public', null, null, function(err, discussion) {
                    ...
                    var randomMessageBody = TestsUtil.generateRandomText(5);
                    RestAPI.Discussions.createMessage(user.restContext, discussion.id, randomMessageBody, null, function(err, message) {
                        assert.ok(!err);

                        // When we search for the message, the discussion should be returned
                        SearchTestsUtil.searchAll(user.restContext, 'general', null, {'resourceTypes': 'discussion', 'q': randomMessageBody}, function(err, results) {
                            assert.ok(!err);
                            var doc = _getDocument(results.results, discussion.id);
                            assert.ok(doc);
                            assert.equal(doc.displayName, randomText);
