...
/**
 * Get an array of hosts (lower-cased) that are currently occupied by tenants
 *
 * @return {String[]}                           The tenant hosts for all the known tenants
 * @api private
 */
var _getTenantHosts = function() {
    return _.chain(tenantsByHost).keys().map(function(host) {
...
var updateTenant = module.exports.updateTenant = function(ctx, alias, tenantUpdates, callback) {
    ...
    _.each(tenantUpdates, function(updateValue, updateField) {
        ...
        if (updateField === 'displayName') {
            ...
            validator.check(updateValue.toLowerCase(), {'code': 400, 'msg': 'The hostname has already been taken'}).notIn(_getTenantHosts());
        }
    });
    if (validator.hasErrors()) {
        return callback(validator.getFirstError());
