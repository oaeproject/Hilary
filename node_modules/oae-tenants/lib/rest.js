 ...
 */
OAE.globalAdminRouter.on('post', '/api/tenantNetwork/create', function(
  req,
  res,
) {
  TenantNetworksAPI.createTenantNetwork(req.ctx, req.body.displayName, function(
    err,
    tenantNetwork,
  ) {
    if (err) {
 ...
 */
OAE.globalAdminRouter.on('delete', '/api/tenantNetwork/:id', function(
  req,
  res,
) {
 ...
 * @HttpResponse                            401         Must be a global administrator user to update a tenant network
 */
OAE.globalAdminRouter.on('post', '/api/tenantNetwork/:id/addTenants', function(
  req,
  res,
) {
  var tenantAliases = _.isString(req.body.alias)
    ? [req.body.alias]
    : req.body.alias;
...
OAE.globalAdminRouter.on(
  'post',
  '/api/tenantNetwork/:id/removeTenants',
  function(req, res) {
    var tenantAliases = _.isString(req.body.alias)
      ? [req.body.alias]
      : req.body.alias;
...
OAE.globalAdminRouter.on('get', '/api/tenant/:alias', function(req, res) {
  ...
  if (!tenant) {
    return res
      .status(404)
      .send('There is no tenant with alias ' + req.params.alias);
  }
  res.status(200).send(tenant);
});
...
OAE.tenantRouter.on('post', '/api/tenant', function(req, res) {
  var update = _.oaeExtendDefined(
    {},
    _.pick(req.body, 'displayName', 'host', 'countryCode'),
  );
  if (_.has(req.body, 'emailDomains')) {
    update.emailDomains = OaeUtil.toArray(req.body.emailDomains);
  }
  TenantsAPI.updateTenant(req.ctx, req.ctx.tenant().alias, update, function(
    err,
  ) {
    if (err) {
      return res.status(err.code).send(err.msg);
