 ...
 */
var getClients = (module.exports.Clients.getClients = function(
  ctx,
  userId,
  callback,
) {
  ...
  validator
    ...
    .isLoggedInUser(ctx);
  validator
    .check(userId, { code: 400, msg: 'An invalid userId was passed in' })
    .isUserId();
 ...
 */
var createClient = (module.exports.Clients.createClient = function(
  ctx,
  userId,
  displayName,
  callback,
  ...
  validator
    ...
    .isLoggedInUser(ctx);
  validator
    .check(userId, { code: 400, msg: 'A client must be bound to a user' })
    .isUserId();
  validator
    .check(displayName, { code: 400, msg: 'Missing client displayName' })
    .notEmpty();
  if (validator.hasErrors()) {
    return callback(validator.getFirstError());
  }
  ...
  if (!ctx.user().isAdmin(userTenantAlias)) {
    return callback({
      code: 401,
      msg: 'Only administrators can create OAuth clients',
    });
  }
 ...
 */
var updateClient = (module.exports.Clients.updateClient = function(
  ctx,
  clientId,
  displayName,
  secret,
  ...
  if (validator.hasErrors()) {
    ...
    return callback({
      code: 400,
      msg: 'A displayName and/or secret has to be provided',
    });
  }
  ...
  OAuthDAO.Clients.getClientById(clientId, function(err, client) {
    ...
    var userTenantAlias = AuthzUtil.getResourceFromId(client.userId)
      .tenantAlias;
    if (
      client.userId !== ctx.user().id &&
      !ctx.user().isAdmin(userTenantAlias)
    ) {
      return callback({
        code: 401,
        msg: 'You cannot update a client that you do not own',
      });
    }
 ...
 * @param  {Object}     callback.err    An error that occurred, if any
 */
var deleteClient = (module.exports.Clients.deleteClient = function(
  ctx,
  clientId,
  callback,
) {
  ...
  OAuthDAO.Clients.getClientById(clientId, function(err, client) {
    ...
    if (
      ...
      return callback({
        code: 401,
        msg: 'You cannot delete a client that you do not own',
      });
    }
