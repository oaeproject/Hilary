...
/**
 * Determine if the requesting user's role is high enough to make the requested permissions changes
 *
 * @param  {Context}        ctx                         Standard context object containing the current user and the current tenant
 * @param  {Content}        contentObj                  The content object that permissions are to be changed on
 * @param  {Object}         permissionChanges           An object with keys that are the userIds and values that are the roles that should be assigned
 * @param  {Function}       callback                    Standard callback function
 * @param  {Error}          callback.err                An error that occurred, if any
 * @param  {Boolean}        callback.isAuthorized       Whether or not the user is authorized to make the requested changes
 */
var canSetContentPermissions = module.exports.canSetContentPermissions = function(ctx, contentObj, permissionChanges, callback) {
    ...
    AuthzAPI.resolveEffectiveRole(ctx, contentObj.id, contentObj.tenant.alias, contentObj.visibility, ContentConstants.roles.ALL_PRIORITY, function(err, effectiveRole, canInteract) {
        if (err) {
            ...
            return callback(null, false, 'You are not allowed to interact with this piece of content');
        }

        AuthzAPI.getDirectRoles(_.keys(permissionChanges), contentObj.id, function(err, existingRoles) {
            if (effectiveRole !== ContentConstants.roles.MANAGER) {
                if (_.contains(permissionChanges, false)) {
                    return callback(null, false, 'Only managers can remove access');
                }
                ...
                if (settingHigherRole) {
                    return callback(null, false, 'You cannot set another user\'s role higher than your own');
                }
                var hasHigherRole = _.find(existingRoles, function(role, principalId) {
                    return ContentConstants.roles.ALL_PRIORITY.indexOf(effectiveRole) < ContentConstants.roles.ALL_PRIORITY.indexOf(role);
                });
                if (hasHigherRole) {
                    return callback(null, false, 'You cannot change the role of someone with a higher role than your own');
                }
            }
            return callback(null, true);
        });
