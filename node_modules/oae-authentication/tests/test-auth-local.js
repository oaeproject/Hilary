...
describe('Authentication', function() {
    ...
    describe('#createGlobalAdminUser', function() {
        ...
        it('verify validation of creating a global admin user', function(callback) {
            ...
            RestAPI.Authentication.createGlobalAdminUser(globalAdminRestContext, null, username, username, function(err, user) {
                assert.ok(err);
                assert.equal(err.code, 400);
                assert.ok(!user);

                // Ensure you cannot create a global admin user without a password
                RestAPI.Authentication.createGlobalAdminUser(globalAdminRestContext, username, null, username, function(err, user) {
                    assert.ok(err);
                    assert.equal(err.code, 400);
                    assert.ok(!user);

                    // Ensure you cannot create a global admin user with a password that is too short
                    RestAPI.Authentication.createGlobalAdminUser(globalAdminRestContext, username, 'a', username, function(err, user) {
                        assert.ok(err);
                        assert.equal(err.code, 400);
                        assert.ok(!user);

                        // Ensure you cannot create a global admin user without a displayName
                        RestAPI.Authentication.createGlobalAdminUser(globalAdminRestContext, username, username, null, function(err, user) {
                            assert.ok(err);
                            assert.equal(err.code, 400);
                            assert.ok(!user);
                            ...
                            RestAPI.Authentication.login(testGlobalUserRestContext, username, username, function(err) {
                                ...
                                RestAPI.User.getMe(testGlobalUserRestContext, function(err, me) {
                                    ...
                                    RestAPI.Authentication.createGlobalAdminUser(globalAdminRestContext, username, username, username, function(err, user) {
                                        assert.ok(!err);
                                        assert.ok(user);

                                        // Ensure when we try and create one with the same loginId, we get a 400 error
                                        RestAPI.Authentication.createGlobalAdminUser(globalAdminRestContext, username, username, username, function(err, secondUser) {
                                            assert.ok(err);
                                            assert.equal(err.code, 400);
