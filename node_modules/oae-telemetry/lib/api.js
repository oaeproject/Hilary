 ...
 */
var TelemetryAPI = module.exports = new events.EventEmitter();
...
module.exports.request = function(req, res) {
    ...
    serverTelemetry.incr(req.method + '.count');
...
var _resetTelemetryCounts = function(callback) {
    ...
    log().trace('Attempting to acquire telemetry reset lock');
    Locking.acquire(_getTelemetryCountResetLock(), telemetryConfig.resetInterval, function(err, token) {
        if (err) {
            log().error({'err': err}, 'Error acquiring lock to reset telemetry data');
            return callback();
        } else if (!token) {
            // We didn't acquire the lock, so don't bother resetting
            log().trace('Failed to acquire lock to reset counts');
            return callback();
        }
        ...
        log().trace('Acquired telemetry reset lock. Resetting counts.');
...
var _pushCountsToRedis = function(callback) {
    if (_.isEmpty(stats.counts)) {
        log().trace('Counts were empty when trying to push to redis');
        return callback();
    }
    ...
    log().trace({'counts': stats.counts}, 'Pushing counts to redis');
...
var _lockAndGetCounts = function(callback) {
    ...
    Locking.acquire(_getTelemetryCountPublishLock(), telemetryConfig.publishInterval, function(err, token) {
        if (err) {
