 ...
 *
 * @param  {String}     id                  The ID of the piece of content OR the id of a revision for which the preview items should be retrieved. If the contentId is specified, the latest preview items will be retrieved.
 * @param  {Function}   callback            Invoked when the process compeletes
 * @param  {Object}     callback.err        An error that occurred, if any
 * @param  {Object[]}   callback.previews   The preview objects.
 */
var getContentPreviews = module.exports.getContentPreviews = function(id, callback) {
    Cassandra.runQuery('SELECT * FROM PreviewItems WHERE id = ?', [id], function(err, rows) {
        if (err) {
            return callback(err);
        }
 ...
 *
 * @param  {String}     id                  The ID of the piece of content OR the id of a revision for which a preview item should be retrieved. If the contentId is specified, the latest preview item will be retrieved.
 ...
 */
var getContentPreview = module.exports.getContentPreview = function(id, previewItem, callback) {
    Cassandra.runQuery('SELECT ? FROM PreviewItems WHERE id = ?', [previewItem, id], function(err, rows) {
        if (err) {
            return callback(err);
        }
        ...
        if (!col || col.value === null) {
            return callback({'code': 404, 'msg': "Couldn't find item " + previewItem + " for id: " + id});
        }
 ...
 * @param  {Object}   callback.previews     Object where each key is a revision id and the value is another object with the thumbnailUri and activityUri.
 */
var getPreviewUris = module.exports.getPreviewUris = function(revisionIds, callback) {
    ...
    Cassandra.runQuery('SELECT thumbnailUri, activityUri FROM Revisions WHERE revisionId IN (?)', [revisionIds], function(err, rows) {
        if (err) {
            return callback(err);
        }
        ...
        rows.forEach(function(row) {
            ...
            var activityUri = row.get('activityUri');
            if (activityUri && activityUri.value) {
                previews[row.key].activityUri = activityUri.value;
            }
        });
...
var storeMetadata = module.exports.storeMetadata = function(contentObj, revisionId, status, thumbnailUri, contentMetadata, previewMetadata, fileData, callback) {
    ...
    if (contentObj.previews.total && contentObj.previews.total > 0) {
        queries.push({
            'query': 'DELETE FROM PreviewItems WHERE id = ?',
    ...
    if (nrOfFiles > 0) {
        queries.push(Cassandra.constructUpsertCQL('PreviewItems', 'id', contentObj.id, fileData));
        queries.push(Cassandra.constructUpsertCQL('PreviewItems', 'id', revisionId, fileData));
    }
    ...
    var activityFile = _.find(fileData, function(val, key) { return val.indexOf('activity#') === 0; });
    if (activityFile) {
        revisionUpdate.activityUri = activityFile.split('#')[1];
    }
