...
/**
 * Get the tenants whose configured email domain matches a given domain
 *
 * @param  {String}         domain      The domain to test against
 * @return {Tenant[]}                   The tenants that matched the given domain
 */
var getTenantsForEmailDomain = module.exports.getTenantsForEmailDomain = function(domain) {
    ...
    // Iterate over the enabled tenants and retain a copy of matching tenants. We cannot use
    // `_.filter` here as we don't want to return a reference to the cached tenant object
    _.each(TenantsAPI.getTenants(true), function(tenant, tenantAlias) {
        ...
        if (emailDomainRegex && domain.match(emailDomainRegex)) {
            matchingTenants.push(_.extend({}, tenant));
        }
    });
