...
describe('Content guests', function() {
    ...
    describe('Creating content', function() {

        /**
         * Test that verifies that content can be created and shared immediately with a guest
         */
        it('verify a piece of content can be created and shared immediately with a guest', function(callback) {
    ...
    describe('Sharing', function() {
        ...
        /**
         * Test that verifies that a piece of content can be shared with both a guest and regular users
         */
        it('verify a piece of content can be shared with both a guest and regular users', function(callback) {
            TestsUtil.generateTestUsers(camAdminRestContext, 3, function(err, users, simong, nico, stuart) {
                ...
                RestAPI.Content.createLink(simong.restContext, 'Apereo', 'Apereo', 'private', 'apereo.org', [], [], [], function(err, link) {
                    ...
                    // Try to share it with some users and a guest
                    var email = TestsUtil.generateTestEmailAddress();
                    ContentTestUtil.assertShareWithUsersAndGuest(simong.restContext, link.id, [nico.user.id, email, stuart.user.id], function(token) {
                        ...
                        PrincipalsTestUtil.signIntoGuestAccountWithToken(token, function(loggedinGuest) {
                            ...
                            RestAPI.Content.getLibrary(loggedinGuest.restContext, loggedinGuest.user.id, null, null, function(err, data) {
                                ...
                                // Verify that the link is also in the regular user's their libraries
                                RestAPI.Content.getLibrary(nico.restContext, nico.user.id, null, null, function(err, data) {
                                    assert.ok(!err);
                                    assert.strictEqual(data.results.length, 1);
                                    assert.strictEqual(data.results[0].id, link.id);

                                    RestAPI.Content.getLibrary(stuart.restContext, stuart.user.id, null, null, function(err, data) {
                                        assert.ok(!err);
                                        assert.strictEqual(data.results.length, 1);
                                        assert.strictEqual(data.results[0].id, link.id);
            ...
            ContentTestUtil.setupMultiTenantPrivacyEntities(function(publicTenantA, publicTenantB, privateTenant) {
                ...
                RestAPI.Content.shareContent(publicTenantA.publicUser.restContext, publicTenantA.publicContent.id, [publicTenantB.publicUser.user.email], function(err) {
                    ...
                    RestAPI.Content.shareContent(publicTenantA.publicUser.restContext, publicTenantA.publicContent.id, [privateTenant.publicUser.user.email], function(err) {
                        ...
                        // Sharing from a private tenant to a user from another tenant is not allowed
                        RestAPI.Content.shareContent(privateTenant.publicUser.restContext, privateTenant.publicContent.id, [publicTenantA.publicUser.user.email], function(err) {
                            ...
                            // For completeness, sharing from a private tenant to a user from the same tenant is not allowed on the basis
                            // that private tenants shouldn't be using inviting guests logic
                            RestAPI.Content.shareContent(privateTenant.publicUser.restContext, privateTenant.publicContent.id, [privateTenant.loggedinUser.user.email], function(err) {
                                assert.ok(err);
                                assert.strictEqual(err.code, 401);
                                return callback();
                            });
