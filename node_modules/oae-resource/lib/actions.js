...
var create = (module.exports.create = function(ctx, roles, createFn, callback) {
  ...
  var firstManagerRole = _.find(roles, function(role, memberId) {
    return (
      AuthzUtil.isPrincipalId(memberId) && role === AuthzConstants.role.MANAGER
    );
  });
  validator
    .check(firstManagerRole, {
      code: 400,
      msg:
        'There must be at least one manager specified when creating a resource',
    })
    .isValidRole();
  ...
  _getTargetRoles(roles, function(err, targetRolesById) {
    ...
    AuthzPermissions.canCreate(ctx, targetRoles, function(
      err,
      memberChangeInfo,
      emailChangeInfo,
    ) {
      ...
      createFn(function(err, resource) {
        ...
        _applyMemberChanges(ctx, resource, memberChangeInfo, function(err) {
          ...
          _applyInvitationChanges(ctx, resource, emailChangeInfo, function(
            err,
          ) {
            if (err) {
              return callback(err);
            ...
            var args = _.union([null], createFnResults, [
              memberChangeInfo,
              emailChangeInfo,
            ]);
 ...
 */
var share = (module.exports.share = function(
  ctx,
  resource,
  targetIds,
  role,
  ...
  validator
    ...
    .isLoggedInUser(ctx);
  validator
    .check(role, { code: 400, msg: 'Must specify a valid role' })
    .isValidRole();
  validator
  ...
  _getTargets(targetIds, function(err, targetsByTargetId) {
    ...
    AuthzPermissions.canShare(ctx, resource, targets, role, function(
      err,
      memberChangeInfo,
      emailChangeInfo,
    ) {
 ...
 * @param  {EmailChangeInfo}    callback.emailChangeInfo    Describes the resource invitation updates
 */
var setRoles = (module.exports.setRoles = function(
  ctx,
  resource,
  roles,
  callback,
  ...
  _.each(roles, function(role, memberId) {
    ...
    validator
      ...
      .not(resourceId);
    validator
      .check(role, { code: 400, msg: 'An invalid role was provided' })
      .isValidRoleChange();
  });
  ...
  _getTargetRoles(roles, function(err, targetRolesById) {
    ...
    AuthzPermissions.canSetRoles(ctx, resource, targetRoles, function(
      err,
      memberChangeInfo,
      emailChangeInfo,
    ) {
 ...
 * @param  {Object}             callback.err    An error that occurred, if any
 */
var resendInvitation = (module.exports.resendInvitation = function(
  ctx,
  resource,
  email,
  callback,
  ...
  validator
    ...
    .isResource(resource);
  validator
    .check(email, { code: 400, msg: 'A valid email must be provided' })
    .isEmail();
  if (validator.hasErrors()) {
    return callback(validator.getFirstError());
  }
  ...
  AuthzPermissions.canManage(ctx, resource, function(err) {
    ...
    AuthzInvitationsDAO.getInvitation(resourceAuthzId, email, function(
      err,
      invitationHash,
    ) {
      if (err) {
      ...
      AuthzInvitationsDAO.getOrCreateTokensByEmails([email], function(
        err,
        tokensByEmail,
      ) {
        if (err) {
 ...
 */
var acceptInvitation = (module.exports.acceptInvitation = function(
  ctx,
  token,
  callback,
) {
  ...
  validator
    ...
    .isLoggedInUser(ctx);
  validator
    .check(token, { code: 400, msg: 'An invitation token must be specified' })
    .notEmpty();
  if (validator.hasErrors()) {
    return callback(validator.getFirstError());
  }
  ...
  _acceptInvitation(ctx, token, function(
    err,
    email,
    invitationHashes,
    ...
    PrincipalsDAO.getPrincipals(inviterUserIds, null, function(
      err,
      inviterUsersById,
    ) {
      if (err) {
        ...
        ResourceActions.emit(
          ...
          function(errs, results) {
            ...
            var fullResources = _.chain(results)
              ...
              .value();
            var fullResourcesByAuthzId = _.indexBy(
              fullResources,
              AuthzUtil.getAuthzId,
            );
            ...
            _.each(invitationHashes, function(invitationHash) {
              ...
              if (resource) {
                ResourceActivity.postInvitationAcceptActivity(
                  ctx,
                  resource,
                  inviterUser,
                );
  ...
  AuthzInvitationsDAO.getEmailByToken(token, function(err, email) {
    ...
    AuthzInvitationsDAO.getAllInvitationsByEmail(email, function(
      ...
      _.each(invitationHashes, function(invitationHash) {
        ...
        memberRolesByResourceId[resourceId] =
          memberRolesByResourceId[resourceId] || {};
        memberRolesByResourceId[resourceId][ctx.user().id] =
          invitationHash.role;
      });
      ...
      _applyAllMemberChanges(memberRolesByResourceId, function(
        err,
        ...
        var memberChangeInfosByResourceId = _.mapObject(
          idChangeInfosByResourceId,
          function(idChangeInfo) {
            // Map the id change infos into member change infos
            return AuthzModel.MemberChangeInfo.fromIdChangeInfo(
              idChangeInfo,
              membersById,
            );
          },
...
var _getTargets = function(targetIds, callback) {
  ...
  const innerCallback = function(err, userIdsByEmails) {
    ...
    var principalIds = _.chain(targetsById)
      .pluck('principalId')
      .uniq()
      .compact()
      .value();
    ...
    PrincipalsDAO.getExistingPrincipals(principalIds, null, function(
      err,
      principalsById,
    ) {
      if (err) {
...
var _applyMemberChanges = function(ctx, resource, memberChangeInfo, callback) {
  ...
  return AuthzAPI.updateRoles(
    authzResourceId,
    memberChangeInfo.changes,
    callback,
  );
};
 ...
 * @api private
 */
var _applyInvitationChanges = function(
  ctx,
  resource,
  emailChangeInfo,
  callback,
) {
  var addedEmailRoles = _.pick(
    emailChangeInfo.changes,
    emailChangeInfo.emails.added,
  );
  var updatedEmailRoles = _.omit(
    emailChangeInfo.changes,
    emailChangeInfo.emails.added,
  );
  var authzResourceId = AuthzUtil.getAuthzId(resource);
