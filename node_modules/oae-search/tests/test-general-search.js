    ...
    describe('Group Indexing', function() {
        ...
        it('verify index updated group', function(callback) {
            TestsUtil.generateTestGroups(camAdminRestContext, 1, function(err, groups) {
                ...
                var oaeTeam = _.values(groups)[0];
                // Verify that the group does not match on the term 'Team', this is to sanity check the search before searching for a valid post-update hit later
                RestAPI.Search.search(camAdminRestContext, 'general', ['group'], { 'q': 'Zyx' }, function(err, results) {
                    assert.ok(!err);
                    assert.ok(!_getDocById(results, oaeTeam.id));
    
                    // update name to "OAE Team"
                    RestAPI.Group.updateGroup(camAdminRestContext, oaeTeam.id, { 'name': 'Zyx Team' }, function(err) {
                        assert.ok(!err);
