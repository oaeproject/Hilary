...
ActivityAPI.registerActivityType(ContentConstants.activity.ACTIVITY_CONTENT_CREATE, {'groupBy': [
    // "Branden Visser uploaded 5 files"
    {'actor': true}
]});
...
ActivityAPI.registerActivityType(ContentConstants.activity.ACTIVITY_CONTENT_COMMENT, {'groupBy': [
    // "Content Item was commented on 5 times"
    {'target': true}
]});
...
ActivityAPI.registerActivityType(ContentConstants.activity.ACTIVITY_CONTENT_SHARE, {'groupBy': [
...
ActivityAPI.registerActivityEntityTransformer('content', function(ctx, activityEntities, callback) {
    ...
    _.keys(activityEntities).forEach(function(activityId) {
        ...
        _.keys(activityEntities[activityId]).forEach(function(entityId) {
            ...
            transformedActivityEntities[activityId][entityId] = ContentUtil.transformPersistentContentActivityEntity(ctx, entity.content);
        });
    });
...
ActivityAPI.registerActivityEntityProducer('content-comment', function(activityType, entityType, resource, callback) {
    ...
    ContentAPI._getContent(comment.contentId, function(err, content) {
        ...
        // TODO: When the 'created' no longer represents the thread timestamp hierarchy, this will have to be switched to the new property
        // that does.
        var siblingStart = comment.created;
        ContentAPI._getComments(comment.contentId, siblingStart, 1, function(err, siblings) {
            context.siblings = siblings;

            if (!comment.replyTo) {
                return _getUsersAndProduceEntity(content, comment, context, callback);
            }

            // Get the parent if it exists for context in the entity
            ContentAPI._getComment(comment.replyTo, function(err, parent) {
                if (err) {
                    return callback(err);
                }
...
var _getUsersAndProduceEntity = function(content, comment, context, callback) {
    var userIds = {};
    if (context.siblings) {
        context.siblings.forEach(function(sibling) {
            userIds[sibling.createdBy] = true;
        });
    }

    if (context.parent) {
        userIds[context.parent.createdBy] = true;
    }

    if (context.parentSiblings) {
        context.parentSiblings.forEach(function(parentSibling) {
            userIds[parentSibling.createdBy] = true;
        });
    }

    PrincipalsDAO.getPrincipals(_.keys(userIds), function(err, authorsById) {
        if (err) {
            return callback(err);
        }
        ...
        if (context.siblings) {
            context.siblings.forEach(function(sibling) {
                sibling.createdBy = authorsById[sibling.createdBy];
            });
        }

        if (context.parent) {
            context.parent.createdBy = authorsById[context.parent.createdBy];
        }

        if (context.parentSiblings) {
            context.parentSiblings.forEach(function(parentSibling) {
                parentSibling.createdBy = authorsById[parentSibling.createdBy];
            });
        }
...
ActivityAPI.registerActivityRouter('content-comment', function(activityType, entityType, entity, callback) {
    ...
    return _routeContent(activityType, entityType, entity.comment.contentId, entity.comment.visibility, callback);
});
...
var _routeContent = function(activityType, entityType, contentId, visibility, callback) {
    ...
    ActivityUtil.getAllAuthzMembersByRole(contentId, function(err, specify) {
        ...
        if (isShareActivity && entityType === ActivityConstants.entityTypes.OBJECT) {
            ...
            delete routes[ContentConstants.roles.VIEWER];
        }
        routes = _.uniq(_.flatten(_.values(routes)));

        // Resolve the propagation of the content item. If the content is private, we propagate to all users
