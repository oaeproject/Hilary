 ...
 * @param  {String}        [opts.resourceTypes]    A comma-separated list of resource types to search (e.g., content,user), or "all". If "all", then the search will not filter on resource type. Possible resource types are enumerated by SearchConstants.general.RESOURCE_TYPES. Default: "all"
...
module.exports = function(ctx, opts, callback) {
    ...
    opts = opts || {};
    opts.resourceTypes = opts.resourceTypes || SearchConstants.general.RESOURCE_TYPE_ALL;
    opts.includeExternal = (opts.includeExternal === 'true');
    opts.includeIndirect = (opts.includeIndirect !== 'false');

    // Resolve the resource types that we'll search for, including whether or not we just need to search for "all" resources
    opts.searchAll = (opts.resourceTypes === SearchConstants.general.RESOURCE_TYPE_ALL);
    if (!opts.searchAll) {
        var resourceTypes = _.compact(opts.resourceTypes.split(','));
        
        // If we find that the array of resource types have no valid values, default to searching all
        if (resourceTypes.length > 0) {
            opts.resourceTypes = resourceTypes;
        } else {
            opts.searchAll = true;
        }
    }
