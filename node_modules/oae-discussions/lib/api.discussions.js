...
var removeDiscussionFromLibrary = module.exports.removeDiscussionFromLibrary = function(ctx, libraryOwnerId, discussionId, callback) {
    var validator = new Validator();
    validator.check(null, {'code': 401, 'msg': 'You must be authenticated to remove a discussion from a library'});
    validator.check(libraryOwnerId, {'code': 400, 'msg': 'A user or group id must be provided'}).isPrincipalId();
    validator.check(discussionId, {'code': 400, 'msg': 'A valid discussion id "' + discussionId + '" was provided'});
    if (validator.hasErrors()) {
        return callback(validator.getFirstError());
    }
    ...
    _getDiscussion(discussionId, function(err, discussion) {
        ...
        DiscussionsAPI.Authz.canRemoveDiscussionFromLibrary(ctx, libraryOwnerId, function(err, canRemove) {
            if (err) {
                return callback(err);
            } else if (!canRemove) {
...
var createMessage = module.exports.createMessage = function(ctx, discussionId, body, replyToCreatedTimestamp, callback) {
    ...
    _getDiscussion(discussionId, function(err, discussion) {
        ...
        DiscussionsAPI.Authz.canPostDiscussionMessage(ctx, discussion, function(err, canPost) {
            ...
            MessageBoxAPI.createMessage(_createDiscussionMessageBoxId(discussionId), ctx.user().id, body, {'replyToCreated': replyToCreatedTimestamp}, callback);
        });
    });
};
...
var deleteMessage = module.exports.deleteMessage = function(ctx, discussionId, messageCreatedDate, callback) {
    ...
    _getDiscussion(discussionId, function(err, discussion) {
        ...
        MessageBoxAPI.getMessages(messageBoxId, [messageCreatedDate], {'scrubDeleted': false}, function(err, message) {
            if (err) {
                return callback(err);
            }
