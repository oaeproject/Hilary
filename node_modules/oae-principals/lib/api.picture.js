...
var _storePicture = function(ctx, principal, file, callback) {
    ...
    ImageUtil.autoOrient(file.path, function(err, orientedFile) {
        if (err) {
            return cleanupOnError(err, file, callback);
        }

        // Remove the original file as we won't be using it any longer.
        fs.unlink(file.path, function(err) {
            if (err) {
                log().warn({'err': err}, 'Unable to remove the original uploaded image.');
                return callback(err);
            }
...
var _storeAndSave = function(ctx, principal, files, callback) {
    ...
    backend.store(ctx, files[key], options, function(err, smallPictureUri) {
        if (err) {
            return callback(err);
        }
        ...
        backend.store(ctx, files[key], options, function(err, mediumPictureUri) {
            if (err) {
                return callback(err);
            }
