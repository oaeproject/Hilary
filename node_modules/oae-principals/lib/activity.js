...
ActivityAPI.registerActivityType(PrincipalsConstants.activity.ACTIVITY_GROUP_ADD_MEMBER,
    {
        'groupBy': [
            // "5 people were added to OAE Team by Simon"
            {'actor': true, 'object': true},


            // "Simon added 5 people to the OAE Team"
...
PrincipalsAPI.on(PrincipalsConstants.events.UPDATED_GROUP_MEMBERS, function(ctx, group, members) {
    ...
    for (var i = 0; i < principalIds.length; i++) {
        ...
        if (members[principalId]) {
            ...
            var targetResource = new ActivityModel.ActivitySeedResource(objectResourceType, principalId);
...
ActivityAPI.registerActivityRouter('user', function(activityType, entityType, entity, callback) {
    // Route the activity to the user's self, but no one else
    // TODO: When user 'following' is implemented, we'll be routing to the user's followers here as well
    var activityRoutes = [entity[ActivityConstants.properties.OAE_ID]];
    var notificationRoutes = null;
    if (entityType !== 'actor') {
        notificationRoutes = [entity[ActivityConstants.properties.OAE_ID]];
    }
...
ActivityAPI.registerActivityRouter('group', function(activityType, entityType, persistentEntity, callback) {
    ...
    ActivityUtil.getAllAuthzMembersByRole(groupId, function(err, specify) {
        ...
        var isGroupCreateActivity = (activityType === PrincipalsConstants.activity.ACTIVITY_GROUP_CREATE);
