 ...
 * @param  {String}    [opts.tenantAlias]      The alias of the tenant to create the user in. Defaults to the current tenant if no value is provided
 * @param  {Boolean}   [opts.isGlobalAdmin]    Whether or not the new user should be a global admin
 * @param  {Boolean}   [opts.isTenantAdmin]    Whether or not the new user should be a tenant admin
...
var createUser = module.exports.createUser = function(ctx, displayName, opts, callback) {
    ...
    var tenantAlias = opts.tenantAlias || ctx.tenant().alias;
    ...
    Cassandra.runQuery(q.query, q.parameters, function (err) {
        ...
        // If the newly created user is a global admin mark it that way
        if (OaeUtil.castToBoolean(opts.isGlobalAdmin)) {
            setGlobalAdmin(ctx, id, OaeUtil.castToBoolean(opts.isGlobalAdmin), function(err) {
                if (err) {
                    return callback(err);
                }

                createdUser.isGlobalAdmin = true;

                PrincipalsEmitter.emit(PrincipalsConstants.events.CREATED_USER, ctx, createdUser);
                return callback(null, createdUser);
            });
        // If the newly created user is a tenant admin mark it that way
        } else if (OaeUtil.castToBoolean(opts.isTenantAdmin)) {
            setTenantAdmin(ctx, id, OaeUtil.castToBoolean(opts.isTenantAdmin), function(err) {
                if (err) {
                    return callback(err);
                }

                createdUser.isTenantAdmin = true;
