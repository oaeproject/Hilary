  ...
  describe('Verification', () => {
        ...
        (user1, tokenUser1) => {
          AuthenticationTestUtil.assertLocalLoginSucceeds(
            ...
            () => {
              ...
              const restContextUser2 = TestsUtil.createTenantRestContext(
    ...
    it('verify an email address can be re-used by other users', callback => {
      ...
      TestsUtil.createTenantWithAdmin(
        tenantAlias,
        tenantHost,
        (err, tenant, tenantAdminRestContext, tenantAdmin) => {
          assert.ok(!err);
                            ...
                            () => {
                              PrincipalsTestUtil.assertVerifyEmailSucceeds(
                                ...
                                () => {
                                  ...
                                  PrincipalsTestUtil.assertUserEmailMappingEquals(
                                    email,
                                    [user1.id, user2.id],
                                    () => {
                                      return callback();
        ...
        RestAPI.Authentication.login(restContext, params.username, 'password', err => {
          ...
          PrincipalsTestUtil.assertResendEmailTokenFails(anonymousRestContext, user.id, 401, () => {
            ...
            TestsUtil.generateTestUsers(camAdminRestContext, 1, (err, users, otherUser) => {
              assert.ok(!err);
              PrincipalsTestUtil.assertResendEmailTokenFails(
                otherUser.restContext,
                user.id,
                401,
                () => {
  ...
  describe('Has pending email verification token', () => {
    ...
    it('verify validation when checking for a pending email verification token', callback => {
      TestsUtil.generateTestUsers(camAdminRestContext, 1, (err, users, simong) => {
        ...
        // Invalid user id
        PrincipalsTestUtil.assertGetEmailTokenFails(
          simong.restContext,
          'not a user id',
          400,
          () => {
          ...
          () => {
            ...
            PrincipalsTestUtil.assertDeleteEmailTokenSucceeds(
              ...
              () => {
                ...
                PrincipalsTestUtil.assertDeleteEmailTokenFails(
                  simong.restContext,
                  simong.user.id,
                  404,
                  () => {
