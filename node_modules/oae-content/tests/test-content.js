...
describe('Content', function() {
    ...
    var setUpUsers = function(callback) {
        ...
        createUser('dave', 'public', 'David Stringer');
    };
                    ...
                    RestAPI.Content.getContent(contexts['nicolaas'].restContext, contentObj.id, function(err, contentObj) {
                        ...
                        RestAPI.Content.download(contexts['nicolaas'].restContext, contentObj.id, null, path, function(err, response) {
                            ...
                            RestAPI.Content.download(contexts['simon'].restContext, contentObj.id, null, path, function(err, body) {
                                ...
                                // Share it.
                                RestAPI.Content.shareContent(contexts['nicolaas'].restContext, contentObj.id, [contexts['simon'].user.id], ContentConstants.roles.VIEWER, function(err) {
                                    assert.ok(!err);
        ...
        it('verify create link', function(callback) {
            setUpUsers(function(contexts) {
                ...
                RestAPI.Content.createLink(anonymousRestContext, 'Test Content 1', 'Test content description 1', 'public', 'http://www.oaeproject.org/', [], [], [], function(err, contentObj) {
                    ...
                    RestAPI.Content.createLink(contexts['nicolaas'].restContext, 'Test Content 2', 'Test content description 2', 'public', 'http://www.oaeproject.org/', [], [], [], function(err, contentObj, response) {
                        assert.ok(!err);
                        assert.ok(contentObj.id);
        ...
        it('verify create file', function(callback) {
            setUpUsers(function(contexts) {
                ...
                RestAPI.Content.createFile(anonymousRestContext, 'Test Content 1', 'Test content description 1', 'public', getFileStream, [], [], [], function(err, contentObj) {
                    ...
                    // Create one with all required fields
                    RestAPI.Content.createFile(contexts['nicolaas'].restContext, 'Test Content 2', 'Test content description 2', 'public', getFileStream, [], [], [], function(err, contentObj, response) {
                        assert.ok(!err);
                        assert.ok(contentObj.id);
                    ...
                    RestAPI.Content.updateMembers(contexts['nicolaas'].restContext, contentObj.id, permissions, function(err) {
                        ...
                        checkPieceOfContent(contexts['simon'].restContext, contexts['simon'].user.id, contentObj, true, true, true, true, function() {
                            checkPieceOfContent(contexts['ian'].restContext, contexts['ian'].user.id, contentObj, true, false, true, (privacy !== 'private'), function() {
                                ...
                                RestAPI.Content.shareContent(contexts['simon'].restContext, contentObj.id, [contexts['stuart'].user.id], ContentConstants.roles.VIEWER, function(err) {
                                    assert.ok(!err);
                                                                                    ...
                                                                                    AuthzAPI.getAllRoles(contexts['nicolaas'].user.id, contentObj.id, function(err, roles) {
                                                                                        ...
                                                                                        AuthzAPI.getAllRoles(contexts['ian'].user.id, contentObj.id, function(err, roles) {
                                                                                            ...
                                                                                            AuthzAPI.getAllRoles(contexts['anthony'].user.id, contentObj.id, function(err, roles) {
                                                                                                ...
                                                                                                return ContentTestUtil.assertGetContentMembersFails(contexts['nicolaas'].restContext, contentObj.id, null, null, 404, callback);
                                                                                            });
                                                                                        });
                                                                                    });
                                                                                });
    ...
    describe('Content permissions', function() {
        ...
        it('verify multiple content permissions', function(callback) {
            setUpUsers(function(contexts) {
                ...
                RestAPI.Content.createLink(contexts['nicolaas'].restContext, 'Test Content 1', 'Test content description 1', 'public', 'http://www.oaeproject.org/', [], [], [], function(err, contentObj) {
                    ...
                    permissions[contexts['stuart'].user.id] = 'viewer';
                    permissions[contexts['ian'].user.id] = 'viewer';
                    ContentTestUtil.assertUpdateContentMembersSucceeds(contexts['nicolaas'].restContext, contexts['nicolaas'].restContext, contentObj.id, permissions, function() {
                        ...
                        ContentTestUtil.assertUpdateContentMembersSucceeds(contexts['nicolaas'].restContext, contexts['nicolaas'].restContext, contentObj.id, permissions, function() {
                            ...
                            ContentTestUtil.assertUpdateContentMembersFails(contexts['nicolaas'].restContext, contexts['nicolaas'].restContext, contentObj.id, permissions, 400, function() {

                                // Remove permissions from multiple people at the same time, some existing and some non-existing. This
                                // should pass because even if a user doesn't exist in the database, there is no harm in trying to remove
                                // some existing membership with them. An issue only arises if you try and add a non-existing user
                                permissions = {};
            ...
            TestsUtil.generateTestUsers(camAdminRestContext, 2, function(err, users) {
                ...
                RestAPI.Content.createLink(nico.restContext, 'Test Content', 'Test content description', 'public', 'http://www.google.com', [], [bert.user.id], [], function(err, contentObj) {
                    ...
                    RestAPI.User.updateUser(bert.restContext, bert.user.id, {'visibility': 'private'}, function(err) {
                        ...
                        var update = {};
                        update[bert.user.id] = 'manager';
                        ContentTestUtil.assertUpdateContentMembersSucceeds(nico.restContext, nico.restContext, contentObj.id, update, function() {

                            // Removing a private user (that was already a member) should work
                            update[bert.user.id] = false;
                            return ContentTestUtil.assertUpdateContentMembersSucceeds(nico.restContext, nico.restContext, contentObj.id, update, callback);
                        });
                    });
                });
            });
        ...
        it('verify getting content members paging', function(callback) {
            TestsUtil.generateTestUsers(camAdminRestContext, 10, function(err, users) {
                ...
                RestAPI.Content.createLink(simon.restContext, 'Test Content', 'Test content description', 'public', 'http://www.oaeproject.org/', [], memberIds, [], function(err, contentObj) {
                    ...
                    ContentTestUtil.getAllContentMembers(simon.restContext, contentObj.id, {'batchSize': 3}, function(members, responses) {
        ...
        it('verify getting content members validation', function(callback) {
            TestsUtil.generateTestUsers(camAdminRestContext, 1, function(err, users) {
                ...
                RestAPI.Content.createLink(ctx, 'Test Content', 'Test content description', 'public', 'http://www.oaeproject.org/', [], [], [], function(err, contentObj) {
                    ...
                    ContentTestUtil.assertGetContentMembersFails(ctx, ' ', null, null, 400, function() {
                        ContentTestUtil.assertGetContentMembersFails(ctx, 'invalid-id', null, null, 400, function() {

                            // Sanity check the base parameters results in success
                            ContentTestUtil.assertGetContentMembersSucceeds(ctx, contentObj.id, null, null, function(members) {
        ...
        /**
         * Test that verifies collabdoc editors can't add managers
         */
        it('verify collabdoc editors have limited permission changing ability', function(callback) {
            setUpUsers(function(contexts) {
                RestAPI.Content.createCollabDoc(contexts['nicolaas'].restContext, 'Test CollabDoc', 'Doc description', 'public', [contexts['dave'].user.id], [contexts['stuart'].user.id], [], [], function(err, contentObj) {
                    assert.ok(!err);
                    var members = {};
                    members[contexts['anthony'].user.id] = 'viewer';
                    members[contexts['ian'].user.id] = 'editor';
                    // Editor can add new viewers and editors
                    ContentTestUtil.assertUpdateContentMembersSucceeds(contexts['nicolaas'].restContext, contexts['stuart'].restContext, contentObj.id, members, function() {
                        members = {};
                        members[contexts['anthony'].user.id] = 'manager';
                        // Editor cannot promote a user to manager
                        ContentTestUtil.assertUpdateContentMembersFails(contexts['nicolaas'].restContext, contexts['stuart'].  restContext, contentObj.id, members, 401, function() {
                            ...
                            members = {};
                            members[contexts['anthony'].user.id] = false;
                            ContentTestUtil.assertUpdateContentMembersFails(contexts['nicolaas'].restContext, contexts['stuart']  .  restContext, contentObj.id, members, 401, function() {
                                // Editor can't demote managers
                                members = {};
                                members[contexts['dave'].user.id] = 'editor';
                                ContentTestUtil.assertUpdateContentMembersFails(contexts['nicolaas'].restContext, contexts['stuart']  .  restContext, contentObj.id, members, 401, function() {
                                    // Editor can share
                                    ContentTestUtil.assertShareContentSucceeds(contexts['nicolaas'].restContext, contexts['stuart'].restContext, contentObj.id, _.keys(members), ContentConstants.roles.VIEWER, function() {
                                        ContentTestUtil.assertShareContentSucceeds(contexts['nicolaas'].restContext, contexts['stuart'].restContext, contentObj.id, [contexts['anthony'].user.id], ContentConstants.roles.EDITOR, callback);
                                    });
                                });
                            });
                        });
    ...
    describe('Content sharing', function() {
        ...
        var testSharing = function(contentObj, sharer, shareWith, expectShare, expectAccess, expectManager, expectInLibrary, expectedMembers, expectCanShare, callback) {
            RestAPI.Content.shareContent(sharer.restContext, contentObj.id, [shareWith.user.id], ContentConstants.roles.VIEWER, function(err) {
                if (expectShare) {
                    assert.ok(!err);
                } else {
                    assert.ok(err);
        ...
        it('verify multiple sharing', function(callback) {
            setUpUsers(function(contexts) {
                ...
                prepareSharing(contexts, 'private', function(contentObj) {
                    ...
                    RestAPI.Content.shareContent(contexts['nicolaas'].restContext, contentObj.id, toShare, ContentConstants.roles.VIEWER, function(err) {
                        assert.ok(!err);
         ...
         */
        it('verify sharing adds users to content members library', function(callback) {
            // Create users to test with
            TestsUtil.generateTestUsers(camAdminRestContext, 3, function(err, users) {
                users = _.values(users);
                var actor = users[0];
                var memberIds = _.chain(users).pluck('user').pluck('id').value().slice(1);

                // Create a content item to share
                RestAPI.Content.createLink(actor.restContext, 'Test Content 1', 'Test content description 1', 'private', 'http://www.oaeproject.org/', [], [], [], function(err, link) {
                    assert.ok(!err);

                    // Ensure sharing the content item is successful
                    return ContentTestUtil.assertShareContentSucceeds(actor.restContext, actor.restContext, link.id, memberIds, ContentConstants.roles.VIEWER, callback);
                });
            });
        });

        /**
         * Test that verifies sharing content doesn't violate role hierarchy
         */
        it('verify sharing respects role hierarchy', function(callback) {
            // Create users to test with
            TestsUtil.generateTestUsers(camAdminRestContext, 10, function(err, users) {
                users = _.values(users);
                var actor = users[0];
                RestAPI.Content.createCollabDoc(actor.restContext, 'Test CollabDoc', 'Doc description', 'public',   [], [], [], [], function(err, contentObj) {
                    // Manager can add viewer
                    ContentTestUtil.assertShareContentSucceeds(actor.restContext, actor.restContext, contentObj.id, [users[1].user.id], ContentConstants.roles.VIEWER, function() {
                        // Manager can add editor
                        ContentTestUtil.assertShareContentSucceeds(actor.restContext, actor.restContext, contentObj.id, [users[2].user.id], ContentConstants.roles.EDITOR, function() {
                            // Manager can add manager
                            ContentTestUtil.assertShareContentSucceeds(actor.restContext, actor.restContext, contentObj.id, [users[3].user.id], ContentConstants.roles.MANAGER, function() {
                                // Editor can add viewer
                                ContentTestUtil.assertShareContentSucceeds(actor.restContext, users[2].restContext, contentObj.id, [users[4].user.id], ContentConstants.roles.VIEWER, function() {
                                    // Editor can add editor
                                    ContentTestUtil.assertShareContentSucceeds(actor.restContext, users[2].restContext, contentObj.id, [users[5].user.id], ContentConstants.roles.EDITOR, function() {
                                        // Editor cannot add manager
                                        ContentTestUtil.assertShareContentFails(actor.restContext, users[2].restContext, contentObj.id, [users[6].user.id], ContentConstants.roles.MANAGER, 401, function() {
                                            // Viewer can add viewer
                                            ContentTestUtil.assertShareContentSucceeds(actor.restContext, users[1].restContext, contentObj.id, [users[7].user.id], ContentConstants.roles.VIEWER, function() {
                                                // Viewer cannot add editor
                                                ContentTestUtil.assertShareContentFails(actor.restContext, users[1].restContext, contentObj.id, [users[8].user.id], ContentConstants.roles.EDITOR, 401, function() {
                                                    // Viewer cannot add manager
                                                    ContentTestUtil.assertShareContentFails(actor.restContext, users[1].restContext, contentObj.id, [users[9].user.id], ContentConstants.roles.MANAGER, 401, callback);
                                                });
                                            });
                                        });
                                    });
                                                    ...
                                                    RestAPI.Content.getLibrary(contexts['anthony'].restContext, groups['oae-team'].id, null, 10, function(err, contentItems) {
                                                        ...
                                                        checkPieceOfContent(contexts['stuart'].restContext, contexts['stuart'].user.id, contentObj, privacy === 'private' ? false : true, false, false, (privacy !== 'private'), function() {
                                                            ...
                                                            checkPieceOfContent(contexts['branden'].restContext, contexts['branden'].user.id, contentObj, privacy === 'private' ? false : true, false, false, (privacy !== 'private'), function() {
                                                                ...
                                                                RestAPI.Content.shareContent(contexts['anthony'].restContext, contentObj.id, [groups['oae-team'].id], ContentConstants.roles.VIEWER, function(err) {
