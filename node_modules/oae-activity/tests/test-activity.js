...
describe('Activity', function() {
    ...
    describe('Activity Routing', function() {
        ...
        it('verify standard resource propagation for joinable resources', function(callback) {
            // Mock a resource type that is joinable to verify its routing behaviour
            ...
            TestsUtil.setupMultiTenantPrivacyEntities(function(publicTenant0, publicTenant1, privateTenant0, privateTenant1) {
                ...
                ConfigTestsUtil.updateConfigAndWait(TestsUtil.createGlobalAdminRestContext(), privateTenant1.tenant.alias, 'oae-tenants/tenantprivacy/tenantprivate', false, function(err) {
                    ...
                    FollowingTestsUtil.followAll(publicTenant0.publicUser.user.id, followers, function(err) {
                        assert.ok(!err);

                        // Make privateTenant1 private now that the association has been made
                        ConfigTestsUtil.updateConfigAndWait(TestsUtil.createGlobalAdminRestContext(), privateTenant1.tenant.alias, 'oae-tenants/tenantprivacy/tenantprivate', true, function(err) {
                            ...
                            // Fabricate an activity with a resource that will invoke the "interacting_tenants". To do this, we will use a content
                            // create activity. The propagation on the `testEntityType` resource will indicate that only interacting tenants (and
                            // 'member' association) can see it, whereas the routing for the actor will attempt to route to all followers
        ...
        it('verify default resource propagation of "routes"', function(callback) {
            TestsUtil.setupMultiTenantPrivacyEntities(function(publicTenant0, publicTenant1, privateTenant0, privateTenant1) {
                ...
                FollowingTestsUtil.followAll(publicTenant0.publicUser.user.id, followers, function(err) {
                    ...
                    // Ensure that the publicTenant0 public user got it, since they were in the routes of the testEntityType entity
                    ActivityTestsUtil.collectAndGetActivityStream(publicTenant0.publicUser.restContext, null, null, function(err, response) {
                        assert.ok(!err);
                        ActivityTestsUtil.assertActivity(response.items[0], 'content-create', 'create', publicTenant0.publicUser.user.id, testId);

                        // Ensure that the user from the same tenant did not receive it, because they were not in the routes of the test entity
                        ActivityTestsUtil.collectAndGetActivityStream(publicTenant0.privateUser.restContext, null, null, function(err, response) {
                            assert.ok(!err);
