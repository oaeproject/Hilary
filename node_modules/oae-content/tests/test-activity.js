...
describe('Content Activity', function() {
                        ...
                        RestAPI.Content.createComment(jackCtx, link.contentId, 'Comment B', null, function(err, commentB) {
                            ...
                            RestAPI.Content.createComment(jackCtx, link.contentId, 'Reply Comment A', commentA.commentId, function(err, replyCommentA) {
                                ...
                                RestAPI.Activity.getActivityStream(jackCtx, jack.id, null, function(err, activityStream) {
                                    ...
                                    /*!
                                     * Verifies the model of a content.
                                     *
                                     ...
                                     * @param   {Comment}           [commentSiblingsComment]    Indicates the entity should have this comment as its first oae:commentSiblings. If unspecified, the entity should have no comment siblings.
                                     */
                                    var _validateComment = function(entity, comment, replyToComment, commentSiblingsComment) {
                                        ...
                                        // Should have no parent or sibling comments
                                        if (commentSiblingsComment) {
                                            _validateComment(entity['oae:commentSiblings'][0], commentSiblingsComment);
                                        } else {
                                            assert.ok(!entity['oae:commentSiblings']);
                                        }
                                    };
                                    ...
                                    activity.object['oae:collection'].forEach(function(entity) {
                                        if (entity.content === 'Comment A') {
                                            ...
                                            // Ensures that comment A has correct data, and no siblings or parents
                                            _validateComment(entity, commentA);
                                        } else if (entity.content === 'Comment B') {
                                            hadCommentB = true;

                                            // Verify that commant B has correct data, no parent, and a sibling (comment A)
                                            _validateComment(entity, commentB, null, commentA);
                                        } else if (entity.content === 'Reply Comment A') {
                                            hadReplyCommentA = true;

                                            // Verify that the reply to comment A has the right data, the parent (comment A) and no sibling
                                            _validateComment(entity, replyCommentA, commentA, null);
                                        }
                                    });
    ...
    describe('Activity Aggregation', function() {
         ...
         */
        it('verifies duplicate content-update-visibility activities are not duplicated in the feed', function(callback) {
            var jackUsername = TestsUtil.generateTestUserId('jack');
         ...
         * for display.
         */
        it('verifies that content-comment activity aggregates both actor and object entities while pivoting on target', function(callback) {
            var jackUsername = TestsUtil.generateTestUserId('jack');
         ...
         * activity feed.
         */
        it('verifies duplicate content-share activities do not result in redundant activities', function(callback) {
            var jackUsername = TestsUtil.generateTestUserId('jack');
            var janeUsername = TestsUtil.generateTestUserId('jane');
        ...
        it('verifies content-share activities aggregate and are branched properly when all collected at once', function(callback) {
            var jackUsername = TestsUtil.generateTestUserId('jack');
            var janeUsername = TestsUtil.generateTestUserId('jane');
        ...
        it('verifies content-share activities aggregate and are branched properly when collected after first share', function(callback) {
            var jackUsername = TestsUtil.generateTestUserId('jack');
            var janeUsername = TestsUtil.generateTestUserId('jane');
            var brandenUsername = TestsUtil.generateTestUserId('mrvisser');
        ...
        it('verifies content-share activities aggregate and are branched properly when collected before last share', function(callback) {
            var jackUsername = TestsUtil.generateTestUserId('jack');
            var janeUsername = TestsUtil.generateTestUserId('jane');
            var brandenUsername = TestsUtil.generateTestUserId('mrvisser');
        ...
        it('verifies content-share activities aggregate and are branched properly when collected after each share', function(callback) {
            var jackUsername = TestsUtil.generateTestUserId('jack');
            var janeUsername = TestsUtil.generateTestUserId('jane');
            var brandenUsername = TestsUtil.generateTestUserId('mrvisser');
