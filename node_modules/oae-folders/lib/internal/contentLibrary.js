 ...
 * @param  {Object}         callback.err    An error that occurred, if any
 */
var remove = module.exports.remove = function(folder, contentItems, callback) {
    callback = callback || function(err) {
        if (err) {
            log().error({
                'err': err,
...
var _insert = function(folder, contentItems, callback) {
    ...
    LibraryAPI.Index.insert(FoldersConstants.library.CONTENT_LIBRARY_INDEX_NAME, effectiveLibraryVisibilities, contentItem.id, contentItem.lastModified, function(err) {
        if (err) {
            log().warn({
                'err': err,
 ...
 * destructive to the `contentItems` parameter as it iterates through content items to remove
 *
 * @param  {Folder}         folder          The folder from which to remove the content items
 * @param  {Content[]}      contentItems    The content items to remove from the folder
 * @param  {Function}       callback        Standard callback function
 ...
 * @api private
 */
var _remove = function(folder, contentItems, callback) {
    if (_.isEmpty(contentItems)) {
    ...
    var contentItem = contentItems.pop();
    LibraryAPI.Index.remove(FoldersConstants.library.CONTENT_LIBRARY_INDEX_NAME, [folder.groupId], contentItem.id, contentItem.lastModified, function(err) {
        if (err) {
            log().warn({
                'err': err,
                'indexName': FoldersConstants.library.CONTENT_LIBRARY_INDEX_NAME,
        ...
        return _remove(folder, contentItems, callback);
    });
};
