...
OAE.tenantServer.post('/api/content/create', function(req, res) {
    ...
    // File creation
        ...
        ContentAPI.createFile(req.ctx, req.body.name, req.body.description, req.body.visibility, additionalMembers, uploadedFile, function(err, contentObj) {
            if (err) {
...
OAE.tenantServer.post('/api/content/:contentId/previews', function(req, res) {
    ...
    ContentAPI.addPreviewItems(req.ctx, req.params.contentId, req.body.status, req.files, req.body, function(err, files) {
        if (err) {
            return res.send(err.code, err.msg);
        }
        res.send(201);
...
OAE.tenantServer.get('/api/content/:contentId/previews/:item', function(req, res) {
    ...
    ContentAPI.verifySignedPreviewItem(req.ctx, req.params.contentId, req.params.item, req.query.signature, req.query.expires, function(err, result) {
        if (err) {
            return res.send(err.code, err.msg);
        }
