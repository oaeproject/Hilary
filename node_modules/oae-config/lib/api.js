...
// Cached global config schema
var cachedSchema = {};
// Cached tenant config schema
var cachedTenantSchema = {};
// Cached config values for global admin users
var cachedConfig = {};
// Cached config values for tenant admin users
    ...
    var getModuleSchema = function(module) {
        ...
        IO.getFileListForFolder(dir, function(configFiles) {
            ...
            if (done === toDo) {
                ...
                cachedTenantSchema = clone(cachedSchema);
                var moduleKeys = _.keys(cachedTenantSchema);
                // Loop over all modules
                for (var m = 0; m < moduleKeys.length; m++) {
                    var mod = moduleKeys[m];
                    var featureKeys = _.keys(cachedTenantSchema[mod]);
                    // Loop over all features per module
                    for (var f = 0; f < featureKeys.length; f++) {
                        var feature = featureKeys[f];
                        if (_.isObject(cachedTenantSchema[mod][feature])) {
                            var elementKeys = _.keys(cachedTenantSchema[mod][feature].elements);
                            ...
                            for (var e = 0; e < elementKeys.length; e++) {
                                ...
                                if (cachedTenantSchema[mod][feature].elements[element].globaladminonly) {
                                    delete cachedTenantSchema[mod][feature].elements[element];
                                }
                            }
                        }
                    }
...
var cacheTenantConfig = function(tenantAlias, callback) {
    ...
    Cassandra.runQuery('SELECT * FROM Config USING CONSISTENCY QUORUM WHERE tenantAlias = ?', [tenantAlias], function(err, rows) {
        ...
        for (var m = 0; m < moduleKeys.length; m++) {
            ...
            cachedTenantConfig[tenantAlias][module] = {};
            ...
            for (var f = 0; f < featureKeys.length; f++) {
                ...
                if (_.isObject(cachedSchema[module][feature])) {
                    ...
                    cachedTenantConfig[tenantAlias][module][feature] = {};
                    ...
                    for (var e = 0; e < elementKeys.length; e++) {
                        ...
                        // If the value is not only available for global admins but also for tenant admins, add it to the cached tenant config
                        if (!cachedSchema[module][feature].elements[element].globaladminonly) {
                            cachedTenantConfig[tenantAlias][module][feature][element] = cachedConfig[tenantAlias][module][feature][element];
                        }
