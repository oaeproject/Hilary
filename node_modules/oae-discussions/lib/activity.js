 ...
 */
DiscussionsAPI.on(DiscussionsConstants.events.CREATED_DISCUSSION, function(ctx, discussion) {
    var millis = Date.now();
 ...
 */
DiscussionsAPI.on(DiscussionsConstants.events.UPDATED_DISCUSSION, function(ctx, newDiscussion, oldDiscussion) {
    var millis = Date.now();
    var actorResource = new ActivityModel.ActivitySeedResource('user', ctx.user().id, {'user': ctx.user()});
 ...
 */
DiscussionsAPI.on(DiscussionsConstants.events.UPDATED_DISCUSSION_MEMBERS, function(ctx, discussion, memberChangeInfo, opts) {
    if (opts.invitation) {
...
var _createPersistentDiscussionActivityEntity = function(discussion) {
    return new ActivityModel.ActivityEntity('discussion', discussion.id, discussion.visibility, {'discussion': discussion});
};
