        ...
        AuthzInvitationsDAO.getAllInvitationsByEmail(email, function(err, invitationHashes) {
            ...
            _applyAllMemberChanges(memberRolesByResourceId, function(err, changesByResourceId) {
                ...
                AuthzInvitationsDAO.deleteInvitations(email, function(err) {
                    ...
                    ResourceActions.emit(ResourceConstants.events.INVITATION_ACCEPTED, ctx, invitationHashes, changesByResourceId, function() {
                        return callback();
                    });
                });
            });
...
var _applyAllMemberChanges = function(memberChangesByResourceId, callback) {
    ...
    _.each(memberRolesByResourceId, function(changes, resourceId) {
        // First determine what changes to actually apply. If someone accepts an old invitation
        // somehow and they already have manager role, an invitation that invited them as viewer
        // should not demote their role. Therefore, we only take into consideration promotions,
        // similar to share
        AuthzAPI.computeMemberRolesAfterChanges(resourceId, changes, {'promotionOnly': true}, function(err, changes, roles, members) {
            if (err) {
                log().warn({'err': err}, 'An error occurred computing member role changes when an invitation was accepted');
                return _done();
            }

            // Perform the actual changes in the resource roles
