...
OAE.init(config, function(err) {
  ...
  _groupUsersByEmail(tenantAlias, function(usersByEmail) {
    ...
    log().info(
      'Begin writing %s batches of user activity to CSV',
      batches.length,
    );
...
function _writeUserRows(userHashBatches) {
  ...
  log().info(
    'There are %d batches remaining to process',
    userHashBatches.length,
  );
  ...
  var _done = _.after(userHashBatch.length, function() {
    log().info(
      'Successfully completed batch of %d users',
      userHashBatch.length,
    );
  ...
  _.each(userHashBatch, function(userHash) {
    _createUserRow(userHash, function(err, row) {
      if (err) {
        log().warn(
          { err: err, user: userHash },
          'Failed to create row from a user hash',
        );
        return _done();
      }
...
function _groupUsersByEmail(tenantAlias, callback) {
  ...
  function _aggregateUsers(principalHashes, callback) {
    log().info(
      'Analyzing %s principals to index by email',
      principalHashes.length,
    );
    _.chain(principalHashes)
      .filter(function(principalHash) {
        return (
          principalHash.email &&
          (!tenantAlias || principalHash.tenantAlias === tenantAlias)
        );
      })
      .tap(function(userHashes) {
        log().info(
...
function _createUserRow(userHash, callback) {
  ...
  _findExternalId(userHash, function(err, externalId) {
    ...
    _findLatestActedActivityMillis(userHash, function(
      err,
      latestActedActivityMillis,
    ) {
      if (err) {
        return callback(err);
      }
      ...
      _findActivityStreamCount(userHash, function(err, activityStreamCount) {
        ...
        _findLatestContentInLibraryMillis(userHash, function(
          err,
          latestContentInLibraryMillis,
        ) {
          if (err) {
            return callback(err);
          }
          ...
          _findContentLibraryCount(userHash, function(
            err,
            contentLibraryCount,
          ) {
            if (err) {
            ...
            _findAuthzMembershipsCacheCount(userHash, function(
              err,
              authzMembershipsCacheCount,
            ) {
              // Find how many memberships to which the user
              // belongs directly
              _findResourceMembershipsCount(userHash, function(
                err,
                resourceMembershipsCount,
              ) {
                if (err) {
                  return callback(err);
                }
                ...
                _.extend(row, {
                  ...
                  content_item_latest_estimate:
                    latestContentInLibraryMillis || 0,
...
function _findLatestActedActivityMillis(userHash, callback, _nextToken) {
  ...
  ActivityDAO.getActivities(
    ...
    _nextToken,
    30,
    function(err, activities, nextToken) {
      if (err) {
        return callback(err);
      } else if (!nextToken) {
