 ...
 * @param  {String[]}   callback.userIds    The OAE user ids in the same order as the `authorIds`
 */
var getUserIds = module.exports.getUserIds = function(authorIds, callback) {
    ...
    var keys = _.map(authorIds, _getMappingKey);
    Redis.getClient().mget(keys, function(err, userIds) {
        if (err) {
            log().error({'err': err, 'authorIds': authorIds}, 'Failed to retrieve OAE users for a set of etherpad author ids');
            return callback({'code': 500, 'msg': 'Failed to retrieve OAE users for a set of etherpad author ids'});
        }
...
var saveAuthorId = module.exports.saveAuthorId = function(authorId, userId, callback) {
    var key = _getMappingKey(authorId);
    Redis.getClient().set(key, userId, function(err) {
        if (err) {
            log().error({'err': err, 'authorId': authorId, 'userId': userId}, 'Failed to store Etherpad author mapping');
...
/**
 * Returns the Redis key where an OAE user ID can be found, given an Etherpad author ID.
 *
 * @param  {String}     authorId    An Etherpad author ID
 * @return {String}                 The key in Redis where the OAE user ID for that author can be found
 * @api private
 */
var _getMappingKey = function(authorId) {
