...
/**
 * Determine if the user can create a resource with the specified target roles
 */
...
var canSetRoles = module.exports.canSetRoles = function(ctx, resource, targetRoles, callback) {
    canManage(ctx, resource, function(err) {
        ...
        _validateRoleChanges(ctx, resource, targetRoles, function(err, memberRoles, newMembers, updatedMembers, removedMembers) {
            if (err) {
                return callback(err);
            } else if (!_.chain(memberRoles).values().contains(AuthzConstants.role.MANAGER).value()) {
                return callback({'code': 400, 'msg': 'The requested change will result in leaving the resource without a manager'});
 ...
 * @param  {Object[]}       [targets.emailRoles]                    The emails that are being invited into the resource
