...
describe('Authentication', function() {
    ...
    afterEach(function(callback) {
        // Reenable local authentication for the Cambridge tenant and verify that it has been reenabled
        ...
        RestAPI.User.createUser(camAdminRestContext, userId, 'password', 'Test User', null, function(err, createdUser) {
            RestAPI.Config.updateConfig(camAdminRestContext, null, 'oae-authentication/local/enabled', true, function(err) {
                assert.ok(!err);
                assert.equal(AuthenticationConfig.getValue(global.oaeTests.tenants.cam.alias, 'local', 'enabled'), true);
                callback();
            });
        });
    });
                                ...
                                RestAPI.User.getMe(userRestContext, function(err, meObj) {
                                    ...
                                    RestAPI.Authentication.logout(userRestContext, function(err) {
                                        ...
                                        RestAPI.User.getMe(userRestContext, function(err, meObj) {
                                            ...
                                            // Disable local authentication and verify that it has been disabled
                                            RestAPI.Config.updateConfig(camAdminRestContext, null, 'oae-authentication/local/enabled', false, function(err) {
                                                assert.ok(!err);
                                                assert.equal(AuthenticationConfig.getValue(global.oaeTests.tenants.cam.alias, 'local', 'enabled'), false);

                                                // Log in with the correct password and userId
                                                RestAPI.Authentication.login(userRestContext, userId, 'password', function(err) {
                                                    assert.ok(err);
                                                    assert.ok(err.code, 401);

                                                    // Verify that we are still logged out because local authentication was disabled
                                                    RestAPI.User.getMe(userRestContext, function(err, meObj) {
                                                        assert.ok(!err);
                                                        assert.ok(meObj);
                                                        assert.equal(meObj.anon, true);
    ...
    describe('#createUser', function() {
        ...
        it('verify create with undefined display name', function(callback) {
            var ctx = new Context(global.oaeTests.tenants.cam);
            var userId = TestsUtil.generateTestUserId();
        ...
        it('verify create with null display name', function(callback) {
            var ctx = new Context(global.oaeTests.tenants.cam);
            var userId = TestsUtil.generateTestUserId();
            var loginId = new LoginId(ctx.tenant().alias, AuthenticationConstants.providers.LOCAL, userId, { password: '12345' });
