...
var storeMetadata = module.exports.storeMetadata = function(contentObj, revisionId, status, thumbnailUri, contentMetadata, previewMetadata, fileData, callback) {
    ...
    // Get the URI for the wide file (if any).
    var wideFile = _.find(fileData, function(val, key) { return val.indexOf('wide#') === 0; });
    if (wideFile) {
        // Get the uri out of the wide file data string.
        // wide file data is of the form '<name>#<uri>' where name will be 'wide'.
        // Rather than doing wideFile.split('#')[1], slice of the name and return the rest of string
        // to prevent not getting the full URI if it would ever contain a '#'.
        revisionUpdate.wideUri = wideFile.split('#').slice(1).join('#');
    }
