...
const TemporaryFile = function (path, size) {
  ...
  that.remove = function (callback) {
    fs.unlink(that.path, (error) => {
      if (error) {
        log().error({ err: error }, 'Could not remove temporary file: %s', that.path);
  ...
  that.update = function (callback) {
    fs.stat(that.path, (error, stat) => {
      if (error) {
        log().error({ err: error }, 'Could not stat the file: %s', that.path);
 ...
 * @return {TempFile}
 */
const createTemporaryFile = function (options) {
  options = options || {};
  const temporaryPath = Temp.path(options);
  return new TemporaryFile(temporaryPath, 0);
};

export { init, TemporaryFile as TempFile, createTemporaryFile as createTempFile };
