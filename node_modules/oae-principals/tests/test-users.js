...
describe('Users', function() {
    ...
    describe('Get user', function() {
        ...
        it('verify get users for tenant', function(callback) {
            ...
            TestsUtil.generateTestUsers(camAdminRestContext, 3, function(err, users, salla, miguel, jordan) {
                assert.ok(!err);

                RestAPI.Admin.getAllUsersForTenant(camAdminRestContext, global.oaeTests.tenants.cam.tenantAlias, function(err, users) {
                    assert.ok(!err);
                    assert.deepEqual([jordan, miguel, salla], users.slice().sort());
                    // Try and get users for a non-existing tenantAlias
                    RestAPI.Admin.getAllUsersForTenant(camAdminRestContext, 'totally-not-a-tenant-alias', function(err, users) {
                        assert.ok(err);
                        assert.equal(err.code, 404);
                        // Try and get users as a non-admin user
                        RestAPI.Admin.getAllUsersForTenant(anonymousRestContext, global.oaeTests.tenant.gt.tenantAlias, function(err, users) {
                            assert.ok(err);
                            assert.equal(err.code, 401);
                            // Try and get users for a tenant with no users
                            RestAPI.Admin.getAllUsersForTenant(gtAdminRestContext, global.oaeTest.tenant.gt.tenantAlias, function(err, users) {
                                assert.ok(err);
                                assert.equal(err.code, 404);
                                return callback();
                            });
