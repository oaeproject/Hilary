...
module.exports.createUser = function(ctx, loginId, password, displayName, opts, callback) {
    ...
    _getUserIdFromLoginId(loginId, function(err, existingUserId) {
        if (err) {
            return callback(err);
        ...
        var userId = AuthzUtil.toUuid('u', ctx.tenant().alias, ShortId.generate());
        var hash = PrincipalsUtil.hashPassword(password);
        var queries = [
            {
                'query': 'INSERT INTO Principals (principalId, tenant, visibility, locale, timezone, password, displayName, loginId) VALUES (?, ?, ?, ?, ?, ?, ?, ?)',
                'parameters': [userId, ctx.tenant().alias, opts.visibility, opts.locale, opts.timezone, hash, displayName, _loginIdToRowKey(loginId)]
            },
            {
                'query': 'INSERT INTO PrincipalsLoginId (loginId, userId) VALUES (?, ?)',
                'parameters': [_loginIdToRowKey(loginId), userId]
            }
        ];
 ...
 */
var getLoginIdFromUserId = module.exports.getLoginIdFromUserId = function(userId, callback) {
    var validator = new Validator();
    ...
    Cassandra.runQuery('SELECT loginId FROM Principals USING CONSISTENCY QUORUM WHERE principalId = ?', [userId], function(err, rows) {
        if (err) {
            return callback(err);
        }

        var row = rows[0];
        var loginIdCol = row.get('loginId');
        var loginId = null;
        if (loginIdCol) {
            var loginId = _rowKeyToLoginId(loginIdCol.value);
        }
...
var _getUserIdFromLoginId = function(loginId, callback) {
    Cassandra.runQuery('SELECT userId FROM PrincipalsLoginId USING CONSISTENCY QUORUM WHERE loginId = ?', [ _loginIdToRowKey(loginId) ], function(err, rows) {
        if (err) {
            return callback(err);
        }

        var row = rows[0];
        var userIdCol = row.get('userId');
...
/**
 * Convert a loginId object to a cassandra row key.
 *
 * @param   {LoginId} loginId   The loginId object to convert to a row key
 * @return  {String}            The row key representation of this loginId
 */
var _loginIdToRowKey = function(loginId) {
    return loginId.provider + ':' + loginId.tenantId + ':' + loginId.externalId;
}

/**
 * Explode a row-key representation of a login id into a LoginId object.
 *
 * @param   {String}  loginIdStr    A row-key representation of a login id (e.g., local:oae:branden, <provider>:<tenant>:<externalId>)
 * @return  {LoginId}               The structured representation of the login id. If the login ID is not valid, then `null` is returned.
 */
var _rowKeyToLoginId = function(loginIdStr) {
    if (!loginIdStr) {
    ...
    if (parts.length !== 3) {
        log().warn('Login ID "%s" was attempted to be parsed, but is not valid.', loginIdStr);
        return null;
    }

    // make sure the resulting login id is valid
    var loginId = new PrincipalsModel.LoginId(parts[0], parts[1], parts[2]);
    var validator = new Validator();
