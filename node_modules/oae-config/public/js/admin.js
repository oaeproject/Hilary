...
(function(){
    ...
    var addBinding = function() {
        ...
        $('.createtenant_container > form').on('submit', function() {
            return false;
        });
        ...
        $adminModulesContainer.on('keypress', function(ev) {
            if (ev.which === 13) {
                return false;
            }
        });
        /* TODO: REMOVE BBQ ONCE THE ADMIN UI IS PART OF 3AKAI-UX */
    ...
    var initializeModules = function() {
        ...
        $.each($('.module_configuration_form'), function(i, formEl) {
            $(formEl).validate({
                submitHandler: function(form) {
                    writeConfig(form, adminContext);
                }
            });
        });

        // Run over the configuration to see if there are any elements that need special attention after they've been rendered.
        // e.g. treeviews need to be initialized and given configuration
        $.each(configuration, function(i, module) {
            $.each(module, function(o, option) {
                if (option.elements) {
                    $.each(option.elements, function(e, element) {
                        if (element.type === 'treestructure') {
                            createTree(i + '_' + o + '_' + e, configuration[i][o].elements[e].defaultValue);
                        }
                    });
                }
            });
        ...
        if (adminContext.tenant.tenantId === 'global') {
            getTenants(function(tenants) {
                preProcessTenants(tenants, function(tenants) {
                    ...
                    $('.createtenant_container > form').validate({
                        submitHandler: function(form) {
