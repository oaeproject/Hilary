...
describe('Content Activity', function() {
    ...
    describe('Routes', function() {
         ...
         */
        it('verify that collaborative document content is not persisted to cassandra', function(callback) {
            TestsUtil.generateTestUsers(camAdminRestContext, 3, function(err, users, simon, branden, nico) {
                ...
                RestAPI.Content.createCollabDoc(simon.restContext, TestsUtil.generateTestUserId('collabdoc'), 'description', 'public', [branden.user.id], [], [], [], function(err, contentObj) {
                    assert.ok(!err);
                    ...
                    RestAPI.Content.joinCollabDoc(branden.restContext, contentObj.id, function(err, data) {
                        ...
                        etherpadClient.setText(args, function(err) {
                            ...
                            ContentTestUtil.publishCollabDoc(contentObj.id, branden.user.id, function() {
                                ...
                                ActivityTestsUtil.collectAndGetActivityStream(branden.restContext, branden.user.id, null, function(err) {
                                    assert.ok(!err);

                                    // Query branden's activity stream to get the item that was persisted
                                    var activityStreamId = util.format('%s#activity', branden.user.id);
                                    Cassandra.runQuery('SELECT * FROM "ActivityStreams" WHERE "activityStreamId" = ?', [activityStreamId], function(err, rows) {
                                        assert.ok(!err);
                                        assert.strictEqual(rows.length, 2);

                                        // Ensure we get the revision activity, and that there is no latest
                                        // revision content
                                        var hash = Cassandra.rowToHash(rows[1]);
                                        var activity = JSON.parse(hash.activity);
                                        ...
                                        RestAPI.Content.createComment(branden.restContext, contentObj.id, 'Comment A', null, function(err, commentA) {
                                            assert.ok(!err);
                                            ActivityTestsUtil.collectAndGetActivityStream(branden.restContext, branden.user.id, null, function(err) {
                                                Cassandra.runQuery('SELECT * FROM "ActivityStreams" WHERE "activityStreamId" = ?', [activityStreamId], function(err, rows) {
                                                    assert.ok(!err);
                                                    ...
                                                    // revision content
                                                    var hash = Cassandra.rowToHash(rows[2]);
                                                    var activity = JSON.parse(hash.activity);
                                                    ...
                                                    RestAPI.Content.shareContent(branden.restContext, contentObj.id, [nico.user.id], function(err) {
                                                        assert.ok(!err);
                                                        ActivityTestsUtil.collectAndGetActivityStream(branden.restContext, branden.user.id, null, function(err) {
                                                            Cassandra.runQuery('SELECT * FROM "ActivityStreams" WHERE "activityStreamId" = ?', [activityStreamId], function(err, rows) {
                                                                assert.ok(!err);
                                                                ...
                                                                // revision content
                                                                var hash = Cassandra.rowToHash(rows[3]);
                                                                var activity = JSON.parse(hash.activity);
                                            ...
                                            ActivityTestsUtil.collectAndGetActivityStream(jack.restContext, jack.user.id, null, function(err, activityStream) {
                                                ...
                                                RestAPI.Content.setPreviewItems(globalTenantAdminRestContext, link.id, revisionId, 'done', suitableFiles, suitableSizes, {}, {}, function(err) {
                                                    ...
                                                    ActivityTestsUtil.collectAndGetActivityStream(jack.restContext, jack.user.id, null, function(err, activityStream) {
                                                        ...
                                                        RestUtil.performRestRequest(anonymousGtRestContext, signedDownloadUrl.pathname, 'GET', signedDownloadUrl.query, function(err, body, response) {
                                                            assert.ok(!err);
            ...
            TestsUtil.generateTestUsers(camAdminRestContext, 3, function(err, users, mrvisser, simong, nicolaas) {
                ...
                PrincipalsTestUtil.assertUpdateUserSucceeds(simong.restContext, simong.user.id, simongUpdate, function() {
                    ...
                    RestAPI.Content.createLink(mrvisser.restContext, 'Google', 'Google', 'public', 'http://www.google.ca', [], [], [], function(err, link) {
                        ...
                        RestAPI.Content.createComment(simong.restContext, link.id, '<script>Nice link.</script>\n\nWould click again', null, function(err, simongComment) {
                            assert.ok(!err);
