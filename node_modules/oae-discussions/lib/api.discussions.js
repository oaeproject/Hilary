...
var createDiscussion = module.exports.createDiscussion = function(ctx, displayName, description, visibility, members, opts, callback) {
    ...
    _.each(members, function(role, memberId) {
        validator.check(memberId, {'code': 400, 'msg': 'Invalid principal id or email address has been specified'}).isPrincipalIdOrEmailAddress();
        ...
        PrincipalsDAO.getPrincipals(memberIds, null, function(err, principals) {
            ...
            AuthzAPI.canInteract(ctx, ctx.tenant().alias, principals, function(err, canInteract, illegalPrincipalIds) {
                ...
                DiscussionsDAO.createDiscussion(ctx.user().id, displayName, description, visibility, null, function(err, discussion) {
                    ...
                    permissions[ctx.user().id] = DiscussionsConstants.roles.MANAGER;

                    // Grant the requested users access to the discussion
                    AuthzAPI.updateRoles(discussion.id, permissions, function(err) {
                        if (err) {
                            return callback(err);
                        }

                        // Index the discussion into user libraries
                        _insertLibrary(memberIds, discussion, function(err) {
                            if (err) {
                                return callback(err);
                            }

                            DiscussionsAPI.emit(DiscussionsConstants.events.CREATED_DISCUSSION, ctx, discussion, permissions);
        ...
        DiscussionsAPI.Authz.canManageDiscussion(ctx, discussion, function(err, canManage) {
            ...
            _getAllMemberIds(discussion.id, function(err, memberIds) {
                ...
                DiscussionsDAO.updateDiscussion(discussion, profileFields, function(err, updatedDiscussion) {
                    ...
                    // the last modified timestamp always gets updated when a discussion is directly modified
                ...
                AuthzAPI.updateRoles(discussion.id, roleChanges, function(err) {
                    ...
                    _removeLibrary(memberIds, discussion, function(err) {
                        ...
                        DiscussionsDAO.deleteDiscussion(discussion.id, function(err) {
                            ...
                            DiscussionsAPI.emit(DiscussionsConstants.events.DELETED_DISCUSSION, ctx, discussion);
                            return callback();
                        });
                    });
                });
...
var getDiscussionsLibrary = module.exports.getDiscussionsLibrary = function(ctx, principalId, start, limit, callback) {
    ...
    PrincipalsDAO.getPrincipal(principalId, function(err, principal) {
        ...
        LibraryAPI.Authz.resolveTargetLibraryAccess(ctx, principal.id, principal.visibility, function(err, hasAccess, visibility) {
            ...
            LibraryAPI.Index.list(DiscussionsConstants.library.DISCUSSIONS_LIBRARY_INDEX_NAME, principalId, visibility, {'start': start, 'limit': limit}, function(err, discussionIds, nextToken) {
                if (err) {
                    return callback(err);
                }
...
var shareDiscussion = module.exports.shareDiscussion = function(ctx, discussionId, principalIds, callback) {
    ...
    _getDiscussion(discussionId, function(err, discussion) {
        ...
        // Get the principal ids, creating guest users if necessary
        PrincipalsAPI.getOrCreateGuestUsers(ctx, principalIds, function(err, principalIds) {
            if (err) {
                return callback(err);
            }
            ...
            AuthzAPI.getDirectRoles(principalIds, discussionId, function(err, roles) {
                ...
                DiscussionsAPI.Authz.canShareDiscussion(ctx, discussion, principalIds, function(err, canShare, illegalPrincipalIds) {
                    ...
                    AuthzAPI.updateRoles(discussionId, roleChanges, function(err) {
                        ...
                        // Update the discussion `lastModified` date if it has not been updated within a designated threshold. This constitutes an
                        // update that is by-passing discussion permissions, but that is ok since it is only the `lastModified` date as a result
                        // of this share
                        OaeUtil.invokeIfNecessary(_testDiscussionUpdateThreshold(discussion), _touch, discussion, function(err, touchedDiscussion) {
                            if (err) {
                                log().warn({
                                    'err': err,
...
var setDiscussionPermissions = module.exports.setDiscussionPermissions = function(ctx, discussionId, permissionChanges, callback) {
    ...
    _getDiscussion(discussionId, function(err, discussion) {
        ...
        PrincipalsAPI.getOrCreateGuestUsers(ctx, _.keys(permissionChanges), function(err, principalIds) {
            ...
            AuthzAPI.computeMemberRolesAfterChanges(discussionId, permissions, function(err, membershipAfterChanges, newMemberIds, updatedMemberIds, removedMemberIds) {
                if (err) {
                    return callback(err);
                }
                ...
                DiscussionsAPI.Authz.canSetDiscussionPermissions(ctx, discussion, newMemberIds, function(err, canSetPermissions, illegalMemberIds) {
                    ...
                    AuthzAPI.updateRoles(discussionId, permissions, function(err) {
                        if (err) {
                            return callback(err);
                        }
                        ...
                        // Remove from the library of removed members before we touch the discussion to update the lastModified
                        _removeLibrary(removedMemberIds, discussion, function(err) {
                            ...
                            OaeUtil.invokeIfNecessary(touchDiscussion, _touch, discussion, function(err, touchedDiscussion) {
                                ...
                                } else if (touchDiscussion) {
                                    // Use the recently touched discussion if we actually did a touch
                                    discussion = touchedDiscussion;
                                }

                                // Always insert the discussion into the added user libraries
                                _insertLibrary(newMemberIds, discussion, function(err) {
                                    if (err) {
                                        log().warn({
                                            'err': err,
                                            'principalIds': principalIds,
                                    ...
                                    // For all existing members of the discussion, we update the discussion in their library but only
                                    // if the discussion last modified time was actually updated. Here we use the `touchedDiscussion`
                                    // object because even if `touchDiscussion` was true, we could have failed to touch the discussion,
                                    // in which case we would not want to update the discussion in libraries
                        ...
                        DiscussionsAPI.emit(DiscussionsConstants.events.UPDATED_DISCUSSION_MEMBERS, ctx, discussion, permissions, newMemberIds, updatedMemberIds, removedMemberIds);
        ...
        DiscussionsAPI.Authz.canPostDiscussionMessage(ctx, discussion, function(err, canPost) {
            ...
            MessageBoxAPI.createMessage(discussionId, ctx.user().id, body, {'replyToCreated': replyToCreatedTimestamp}, function(err, message) {
                ...
                PrincipalsUtil.getPrincipal(ctx, ctx.user().id, function(err, createdBy) {
                    ...
                    // Pass the message to the caller but do not return yet as we still might have to update the discussion libraries
                    callback(null, message);

                    // The following `if` block is an asynchronous block. Check to see if we are in a threshold to perform a
                    // discussion lastModified update. If so, we will update the lastModified date and the discussion's libraries
...
var _insertLibrary = function(principalIds, discussion, callback) {
    ...
    LibraryAPI.Index.insert(DiscussionsConstants.library.DISCUSSIONS_LIBRARY_INDEX_NAME, effectiveLibraryVisibilities, discussion.id, discussion.lastModified, callback);
};
...
var _updateLibrary = function(principalIds, discussion, oldLastModified, callback) {
    ...
    var effectiveLibraryVisibilities = LibraryAPI.Authz.resolveEffectiveLibraryVisibilities(principalIds, discussion);
    LibraryAPI.Index.update(DiscussionsConstants.library.DISCUSSIONS_LIBRARY_INDEX_NAME, effectiveLibraryVisibilities, discussion.id, discussion.lastModified, oldLastModified, callback);
};
...
var _removeLibrary = function(principalIds, discussion, callback) {
    ...
    LibraryAPI.Index.remove(DiscussionsConstants.library.DISCUSSIONS_LIBRARY_INDEX_NAME, principalIds, discussion.id, discussion.lastModified, callback);
};
