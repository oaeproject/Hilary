 ...
 * @param  {Object}     callback.err                An error that occured, if any
 */
var getJoinGroupRequests = module.exports.getJoinGroupRequests = function(ctx, groupId, start, limit, callback) {
    limit = OaeUtil.getNumberParam(limit, 10, 1);
    ...
    PrincipalsDAO.getJoinGroupRequests(groupId, function(err, requests) {
        if (err) {
            return callback(err);
        }
        ...
        var users = _.filter(requests, function(hash) { return hash.status === PrincipalsConstants.requestStatus.PENDING; });

        // Return principal(s)
        if (users.length === 1) {
            PrincipalsDAO.getPrincipal(users[0].principalId, function(err, principal) {
                if (err) {
                    return callback(err);
                }
            ...
            var userIds = _.map(users, function(hash) { return hash.principalId; });
 ...
 * @param  {Object}     callback.err                An error that occured, if any
 */
var getJoinGroupRequest = module.exports.getJoinGroupRequest = function(ctx, groupId, callback) {
    ...
    // If the request exists, return 
    PrincipalsDAO.getJoinGroupRequest(groupId, ctx.user().id, function(err, request) {
        if (err) {
            return callback(err);
        }
 ...
 * @param  {Object}     callback.err    An error that occurred, if any
 */
var updateJoinGroupByRequest = module.exports.updateJoinGroupByRequest = function(ctx, groupId, principalId, role, status, callback) {
    var principalId =  principalId || ctx.user().id;
    var validator = new Validator();
    if (role) {
    ...
    PrincipalsDAO.updateJoinGroupByRequest(principalId, groupId, status, function(err) {
        if (err) {
            return callback(err);
        }
