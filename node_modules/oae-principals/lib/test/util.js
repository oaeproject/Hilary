...
/**
 * Assert that we get the users creation/connection date log
 *
 * @param  {Object}     camAdminRestContext         The REST context of a admin who will download the user logs
 ...
 *
 * @return {Object}                                 An object containing the logs
 */
var assertGetUsersDateLogSucceeds = module.exports.assertGetUsersDateLogSucceeds = function(camAdminRestContext, tenantAlias, months, type, callback) {
    PrincipalsAPI.getUsersDateLog(camAdminRestContext, tenantAlias, type, months, function(err, log) {
        assert.ok(!err);
        assert.ok(log);
        csvToJson(log, function(err, principals) {
            assert.ok(!err);
...
/**
 * Assert that we fail getting the users creation/connection date log
 *
 * @param  {Object}     camAdminRestContext         The REST context of a admin who will download the user logs
 * @param  {String}     tenantAlias                 The tenant alias
 * @param  {Number}     httpCode                    The expected HTTP code of the failed has pending email token operation
 ...
 * @return {Object}                                 An object containing the logs
 */
var assertGetUsersDateLogFails = module.exports.assertGetUsersDateLogFails = function(camAdminRestContext, tenantAlias, httpCode, months, type, callback) {
    PrincipalsAPI.getUsersDateLog(camAdminRestContext, tenantAlias, type, months, function(err, log) {
        assert.ok(err);
        assert.ok(!log);
        assert.strictEqual(err.code, httpCode);
 ...
 *
 * @return {Object}     csv                         An object
 */
 var csvToJson = module.exports.csvToJson = function(csv, callback) {
    var result = [];
    var lines = csv.split('\n');
    var headers = lines[0].split(',');

    _.each(lines, function(line, eachLine) {
        var obj = {};
        var currentline = lines[eachLine].split(',');

        if (currentline.length !== headers.length) {
            return callback({'code': 400, 'msg': 'An error occurred while creating the csv file'});
        }

        _.each(headers, function(header, eachHeader) {
            obj[headers[eachHeader]] = currentline[eachHeader];
            if (headers.length === eachHeader + 1) {
                result.push(obj);
                if (lines.length === eachLine + 1) {
