...
var getOrCreateUser = function(context, username, locale, timezone, firstName, lastName, displayName, callback) {
    ...
    PrincipalAPI.getTenantUser(context, username, function(err, user) {
        if (err && err.code === 404) {
            ...
            PrincipalAPI.createUser(context, username, password, "public", locale, timezone, firstName, lastName, displayName, function(err, principalId) {
                if (err) {
                    return callback(err);
                } else {
                    // Get the meta data for this user.
                    context = new Context(context.tenant(), new User(context.tenant().alias, principalId));
                    PrincipalAPI.getUser(context, principalId, callback);
                }
            });
        } else if (err) {
            return callback(err);
...
module.exports.LocalStrategyWrapper = function(tenant) {
    ...
    var strat = new LocalStrategy(options, function(req, username, password, done) {
        PrincipalAPI.checkPassword(tenant, username, password, function(err, basicUser) {
            if (err && err.code !== 401) {
                return done(err);
