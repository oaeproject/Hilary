                        ...
                        RestAPI.Authentication.login(testGlobalUserRestContext, userId1, userId1, function(err) {
                            ...
                            RestAPI.User.getMe(testGlobalUserRestContext, function(err, me) {
                                ...
                                RestAPI.Authentication.createGlobalAdminUser(globalAdminRestContext, userId1, userId1, userId1, function(err, user) {
                                    ...
                                    // Ensure you cannot create a global admin user without a provider
                                    AuthenticationAPI.createGlobalAdminUser(TestsUtil.createGlobalAdminContext(), null, userId2, userId2, {'password': userId2}, function(err, user) {
                                        assert.ok(err);
                                        assert.equal(err.code, 400);
                                        assert.ok(!user);

                                        // Ensure you cannot create a global admin user without an external id
                                        AuthenticationAPI.createGlobalAdminUser(TestsUtil.createGlobalAdminContext(), 'local', null, userId2, {'password': userId2}, function(err, user) {
                                            assert.ok(err);
                                            assert.equal(err.code, 400);
                                            assert.ok(!user);

                                            // Ensure the user still cannot be authenticated
                                            testGlobalUserRestContext = TestsUtil.createGlobalRestContext();

                                            // Ensure that the credentials do not authenticate a value global administrator
                                            RestAPI.Authentication.login(testGlobalUserRestContext, userId2, userId2, function(err) {
                                                assert.ok(err);
                                                assert.equal(err.code, 401);
                                                ...
                                                RestAPI.User.getMe(testGlobalUserRestContext, function(err, me) {
                                                    ...
                                                    // Sanity check that a user can be created directly through the REST endpoints
                                                    AuthenticationAPI.createGlobalAdminUser(TestsUtil.createGlobalAdminContext(), 'local', userId2, userId2, {'password': userId2}, function(err, user) {
                                                        assert.ok(!err);
                                                        assert.ok(user);
