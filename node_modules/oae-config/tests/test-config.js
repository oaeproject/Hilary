...
describe('Configuration', function() {
    ...
    describe('Schema', function() {
        ...
        /**
         * Test that verifies that the configuration schema for a global admin has `globaladminonly` values
         */
        it('verify get schema global returns `globaladminonly` values', function(callback) {
            RestAPI.Config.getSchema(globalAdminRestContext, function(err, schema) {
                assert.ok(!err);
                assert.ok(schema);
                assert.ok(schema['oae-content'].title);
                assert.ok(schema['oae-content']['storage'].elements['amazons3-access-key'], true);
                callback();
            });
        });

        /**
         * Test that verifies that the configuration schema for a tenant admin does not have `globaladminonly` values
         */
        it('verify get schema tenant does not return `globaladminonly` values', function(callback) {
            RestAPI.Config.getSchema(camAdminRestContext, function(err, schema) {
                assert.ok(!err);
                assert.ok(schema);
                assert.ok(schema['oae-content'].title);
