...
var setUpTenants = function(err, callback) {
    ...
    tenantAPI.createTenant("camtest", "Cambridge University Test", "Cambridge University Description", 2001, "localhost:2001", function(err, tenant) {
        global.oaeTests.tenants.cam = tenant;
        tenantAPI.createTenant("gttest", "Georgia Tech Test", "Georgia Tech Description", 2002, "localhost:2002", function(err, tenant) {
...
before(function(callback) {
    ...
    Cassandra.init(config.cassandra, function(err) {
        ...
        console.log('Dropping keyspace "%s" to clean up before tests.', config.cassandra.keyspace);
        Cassandra.dropKeyspace(config.cassandra.keyspace, function(err) {
            Cassandra.close(function() {
                console.log('Flushing redis DB index "%d" to clean up before tests.', config.redis.dbIndex);
                Pubsub.flush(function(err) {
