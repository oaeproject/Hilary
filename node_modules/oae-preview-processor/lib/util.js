...
var downloadRemoteFile = module.exports.downloadRemoteFile = function(url, path, callback) {
    ...
    stream.on('error', function(err) {
        IO.destroyStream(stream);
        log().error({'err': err, 'url': url}, 'Unable to download the file due to a streaming error');
        if (!called) {
            called = true;
    ...
    var req = request(opts, function(err, response) {
        if (err) {
            log().error({'err': err, 'url': url}, 'Unable to download the file due to a request error.');
            if (!called) {
                called = true;
 ...
 */
var generatePreviewsFromImage = module.exports.generatePreviewsFromImage = function(ctx, path, options, callback) {
    options = options || {};
    options.removeInput = (_.isBoolean(options.removeInput)) ? options.removeInput : false;

    // Generate an image that has its orientation fixed up.
    // We do this up front to make all the cropping/resizing logic easier
    // and to make the whole preview process more performant
    // (orienting images is very slow).
    var fixedPath = ctx.baseDir + '/fixed.jpg';
    var opts = {
        'outputPath': fixedPath,
        'removeInput': options.removeInput
    };
    ImageUtil.autoOrient(path, opts, function(err, fixedFile) {
        if (err) {
            return callback(err);
        }
        ...
        _resizeImages(ctx, fixedPath, sizes, function(err) {
            if (err) {
                return callback(err);
            }
 ...
 * @api private
 */
var _resizeImages = function(ctx, path, sizes, callback) {
 ...
 * The image will be stored at basedir/previews/filename.<size name>.jpg
 * The filename will be a concatenation of `size.prefix`, `size.size` and `.jpg`
 *
 * @param  {PreviewContext}      ctx             The preview context associated to this file.
 * @param  {String}              path            The path where the image can be found on disk.
 ...
 * @param  {String}              [size.prefix]   The prefix that should be used in the filename. The end filename will look like <prefix><size>.jpg . If no prefix is specified, an empty string will be used.
 * @param  {Function}            callback        Standard callback method.
 * @param  {Object}              callback.err    Standard error object (if any.)
 * @api private
 */
var _resize = function(ctx, path, size, callback) {
    ...
    ImageUtil.resizeImage(path, size, function(err, file) {
        ...
        var resizedPath = ctx.baseDir + '/' + prefix + size.size + '.jpg';
...
var _cropThumbnail = function(ctx, path, cropMode, callback) {
    ...
    _cropIntelligently(ctx, path, PreviewConstants.SIZES.IMAGE.THUMBNAIL, PreviewConstants.SIZES.IMAGE.THUMBNAIL, opts, 'thumbnail.jpg', function(err, thumbnailPath) {
        if (err) {
            return callback(err);
        ...
        _cropIntelligently(ctx, path, PreviewConstants.SIZES.IMAGE.WIDE_WIDTH, PreviewConstants.SIZES.IMAGE.WIDE_HEIGHT, opts, 'wide.jpg', function(err, widePath) {
            if (err) {
                return callback(err);
            }
...
/**
 * Given an input video, this function will trascode it to h.264 and
 ...
 *
 * @param  {PreviewContext}     ctx                     The current preview context. It allows you to make requests to the app server to retrieve extra metadata
 * @param  {Content}            path                    The path that points towards the input video.
 * @param  {Object}             [options]               A set of options that can be specified
 * @param  {String}             [options.cropMode]      The crop mode that should be used when cropping the image to generate a thumbnail. Either 'TOP' or 'CENTER'. If left undefined, top will be chosen for portrait images and center for landscape images
 * @param  {Boolean}            [options.removeInput]   Whether or not the input video should be removed. Defaults to false.
 * @param  {Function}           callback                Standard callback method.
 * @param  {Object}             callback.err            An error object (if any).
 */
var generatePreviewsFromVideo = module.exports.generatePreviewsFromVideo = function(ctx, path, options, callback) {
    options = options || {};
    options.removeInput = (_.isBoolean(options.removeInput)) ? options.removeInput : false;

    _screenshot(ctx, path, 'large', function(err) {
        if (err) {
            return callback(err);
        }
        _screenshot(ctx, path, 'medium', function(err) {
            if (err) {
                return callback(err);
            }
            _screenshot(ctx, path, 'small', function(err) {
                if (err) {
                    return callback(err);
                }
                _transcode(ctx, path, callback);
            });
        });
    });
...
var _screenshot = function(ctx, path, size, callback) {
    // Setup for the ffmpeg processor
    var ffmpegOpts = {
        'source': path,
        'timeout': 30,
        'priority': 0,
    ...
    var proc = new ffmpeg(ffmpegOpts);

    // Screenshot the video and generate an image preview
    proc.withSize(PreviewConstants.SIZES.VIDEO[size.toUpperCase()] + 'x?').takeScreenshots({'count': 1, 'timemarks': ['20%'], filename: size}, ctx.baseDir, function(err, filenames) {
        if (err) {
            log().error(err);
            return callback(err);
        }
        // Add it to the set of previews that should be attached to it.
        var previewPath = ctx.baseDir + '/' + size + '.jpg';
        ctx.addPreview(previewPath, size);
        callback();
    });
};

var _transcode = function(ctx, path, callback) {
    // Setup for the ffmpeg processor
    var ffmpegOpts = {
        'source': path,
        'timeout': 30,
    ...
    var proc = new ffmpeg(ffmpegOpts);
    var transcodePath = ctx.baseDir + '/transcoded.m4v';
    proc.usingPreset('podcast').saveToFile(transcodePath, function(stdout, stderr) {
        log().info('Video transcode complete');
        log().info(stdout);
        log().error(stderr);
        ctx.addPreviewMetadata('m4v', transcodePath);
