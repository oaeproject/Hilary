...
var _createContent = function(ctx, contentId, revisionId, resourceSubType, displayName, description, visibility, roles, folderIds, otherValues, revisionData, callback) {
    ...
    canManageFolders(ctx, folderIds, function(err, folders) {
        ...
        ResourceActions.create(ctx, roles, createFn, function(err, content, revision, members) {
            if (err) {
                return callback(err);
            }
            ...
            _updateContentLibraries(content, [], function(err, content) {
                ...
                ContentMembersLibrary.insert(content, members.members.added, function(err) {
                    if (err) {
                        log().warn({
                            'err': err,
                            'contentId': content.id,
                            'principalIds': _.keys(members.changes)
...
var shareContent = module.exports.shareContent = function(ctx, contentId, principalIds, callback) {
    ...
    ContentDAO.Content.getContent(contentId, function(err, content) {
        ...
        ResourceActions.share(ctx, content, principalIds, AuthzConstants.role.VIEWER, function(err, members) {
            if (err) {
                return callback(err);
            ...
            _updateContentLibraries(content, [], function(err, content) {
                ...
                ContentMembersLibrary.insert(content, members.members.added, function(err) {
                    ...
                    ContentAPI.emit(ContentConstants.events.UPDATED_CONTENT_MEMBERS, ctx, content, members.changes, addedMemberIds, [], []);
                    return callback();
                });
            });
        });
...
var setContentPermissions = module.exports.setContentPermissions = function(ctx, contentId, changes, callback) {
    ...
    getContent(ctx, contentId, function(err, content) {
        ...
        ResourceActions.setRoles(ctx, content, changes, function(err, members) {
            if (err) {
                return callback(err);
            }
            ...
            _updateContentLibraries(content, removedMemberIds, function(err, content) {
                ...
                ContentMembersLibrary.insert(content, members.members.added, function(err) {
                    ...
                    ContentMembersLibrary.remove(content, removedMemberIds, function(err) {
                        ...
                        ContentAPI.emit(ContentConstants.events.UPDATED_CONTENT_MEMBERS, ctx, content, members.changes, addedMemberIds, updatedMemberIds, removedMemberIds);
                        return callback();
                    });
                });
            });
