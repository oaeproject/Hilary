...
module.exports = function(config) {
    ...
    strategy.getPassportStrategy = function(tenant) {
        ...
        var passportStrategy = new ShibbolethStrategy({
            ...
            var externalId = _getBestAttributeValue(tenant.alias, 'externalIdAttributes', headers, headers['remote_user']);
            if (!externalId) {
            ...
            var displayName = _getBestAttributeValue(tenant.alias, 'mapDisplayName', headers, headers['remote_user']);
            ...
            var email = _getBestAttributeValue(tenant.alias, 'mapEmail', headers);

            // Only use the email address if its valid. Don't fail the
            // request if it's invalid
            if (email) {
            ...
            req.tenant = tenant;
            AuthenticationUtil.handleExternalGetOrCreateUser(req, AuthenticationConstants.providers.SHIBBOLETH, externalId, null, displayName, opts, function(err, user, loginId, created) {
                if (err) {
                    return callback(err);
 ...
 * @api private
 */
var _getBestAttributeValue = function(tenantAlias, configKey, headers, defaultValue) {
    // Get the priority list from the config
    var priorityList = AuthenticationConfig.getValue(tenantAlias, AuthenticationConstants.providers.SHIBBOLETH, configKey);
