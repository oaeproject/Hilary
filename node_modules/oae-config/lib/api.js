...
var writeConfig = module.exports.writeConfig = function(ctx, tenantId, configValues, callback) {
    ...
    var keys = _.keys(configValues);
    validator.check(tenantId, {'code': 400, 'msg': 'Missing tenantid'}).notEmpty();
    validator.check(keys.length, {'code': 400, 'msg': 'Missing configuration. Example configuration: {"oae-authentication/google-authentication/google-authentication-enabled": {"tenantid": "global","value": false}}'}).min(1);
    for (var i = 0; i < keys.length; i++) {
        validator.check(keys[i], {'code': 400, 'msg': 'The configuration field cannot be null'}).not('null');
        validator.check(configValues[keys[i]], {'code': 400, 'msg': 'The configuration value cannot be null'}).notNull();
    }
