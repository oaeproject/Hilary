...
describe('Authz Groups', function() {
    ...
    describe('#computeMemberRolesAfterChanges()', function() {
         ...
         */
        it('verify functionality when taking into account only adds', function(callback) {
            ...
            AuthzAPI.updateRoles(groupId, rolesBefore, function(err) {
                ...
                var addBertChange = makeMembershipChange(bert, 'member');
                AuthzAPI.computeMemberRolesAfterChanges(groupId, addBertChange, {'onlyAdd': true}, function(err, changes, roles, members) {
                    assert.ok(!err);
                    assert.deepEqual(changes, addBertChange);
                    assert.deepEqual(roles.before, rolesBefore);
                    assert.deepEqual(roles.after, _.extend({}, rolesBefore, addBertChange));
                    ...
                    // Verify a role change by making mrvisser a manager, should result in no change
                    // as we are only taking into consideration adds (shares)
                    var mrvisserManagerChange = makeMembershipChange(mrvisser, 'manager');
                    AuthzAPI.computeMemberRolesAfterChanges(groupId, mrvisserManagerChange, {'onlyAdd': true}, function(err, changes, roles, members) {
                        assert.ok(!err);
                        assert.deepEqual(changes, {});
                        assert.deepEqual(roles.before, rolesBefore);
                        assert.deepEqual(roles.after, rolesBefore);
                        ...
                        var simonMemberChange = makeMembershipChange(simong, 'member');
                        AuthzAPI.computeMemberRolesAfterChanges(groupId, simonMemberChange, {'onlyAdd': true}, function(err, changes, roles, members) {
                            assert.ok(!err);
                            assert.deepEqual(changes, {});
                            assert.deepEqual(roles.before, rolesBefore);
                            assert.deepEqual(roles.after, rolesBefore);
                            ...
                            // change
                            var simonRemoveChange = makeMembershipChange(simong, false);
                            AuthzAPI.computeMemberRolesAfterChanges(groupId, simonRemoveChange, {'onlyAdd': true}, function(err, changes, roles, members) {
                                assert.ok(!err);
                                assert.deepEqual(changes, {});
                                assert.deepEqual(roles.before, rolesBefore);
                                assert.deepEqual(roles.after, rolesBefore);
                                ...
                                var unknownUserRemoveChange = makeMembershipChange(unknownUser, false);
                                AuthzAPI.computeMemberRolesAfterChanges(groupId, unknownUserRemoveChange, {'onlyAdd': true}, function(err, changes, roles, members) {
                                    assert.ok(!err);
                                    assert.deepEqual(changes, {});
                                    assert.deepEqual(roles.before, rolesBefore);
                                    assert.deepEqual(roles.after, rolesBefore);
