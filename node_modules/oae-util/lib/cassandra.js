...
var _iterateAll = function(columnNames, columnFamily, keyColumnName, returnKeyColumn, batchSize, onEach, callback, fromKey) {
    ...
    runQuery(query.query, query.parameters, function(err, rows) {
        ...
        _.each(rows, function(row) {
            ...
            fromKey = row.get(keyColumnName).value;

            // Clean the key off the row if it was not requested
            if (!returnKeyColumn) {
                delete row.get(keyColumnName).value;
            }
        });
...
var executeQuery = function(query, parameters, callback) {
    ...
    pool.cql(query, parameters, {'consistencyLevel': 2}, function(err, results) {
        if (err) {
            log().error(_shrinkLogParameters(err, query, logParams), 'An error occurred executing a cassandra query');
...
var _shrinkLogParameters = function(err, query, params) {
