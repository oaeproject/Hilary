 ...
 */
var createLink = module.exports.createLink = function(ctx, visibility, name, description, link, callback) {
    ...
    _createContent(visibility, name, description, 'link', {'link': link}, callback);
};
 ...
 * @param  {Object}                  callback.content  JSON object containing the pool id of the created content
 */
var _createContent = function(ctx, visibility, name, description, contentType, otherValues, callback) {
    callback = callback || function() {};
    ...
    // Make sure the otherValues is a valid object
    ...
    //var parameters
    // Create the content
    Cassandra.runQuery('INSERT INTO Content (contentId, tenant, visibility, name, description, contentType, createdBy, created, lastModified) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)', [contentId, ctx.tenant().alias, visibility, name, description, contentType, createdBy, created, lastModified], function (err) {
        if (err) {
            return callback(err);
        }
        
        var contentObj = new Content(ctx.tenant().alias, contentId, visibility, name, description, contentType, createdBy, created, lastModified);
...
var updateContentProfile = module.exports.updateContentProfile = function(ctx, contentId, profileFields, callback) {
    ...
    fieldNames.forEach(function(fieldName) {
        validator.check(fieldName, {'code': 400, 'msg': fieldName + ' is not a recognized content profile field'}).isIn(['name', 'description', 'visibility']);
    });
...
var _updateContentMetadata = function(ctx, contentId, metadata, callback) {
    ...
    canManage(ctx, contentId, function(err, canManage, contentObj) {
        ...
        var q = constructUpdateCQL('Content', metadata, 'contentId', contentId);
