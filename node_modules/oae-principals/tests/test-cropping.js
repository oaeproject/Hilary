...
describe('Profile pictures', function() {
  ...
  var _verifyCropping = function(
    ...
    RestAPI.Crop.cropPicture(restCtx, principal.id, selectedArea, function(
      err,
      data,
    ) {
      ...
      RestAPI.User.uploadPicture(
        ...
        function(err) {
          ...
          _verifyCropping(
            ctx,
            ctx.user,
            _createSelectedArea('foo', 10, 200),
            400,
            function() {
              _verifyCropping(
                ctx,
                ctx.user,
                _createSelectedArea(10, 'foo', 200),
                400,
                function() {
                  _verifyCropping(
                    ctx,
                    ctx.user,
                    _createSelectedArea(10, 10, 'foo'),
                    400,
                    function() {
                      _verifyCropping(
                        ctx,
                        ctx.user,
                        _createSelectedArea(10, 10, NaN),
                        400,
                        callback,
                      );
                    },
      ...
      RestAPI.User.uploadPicture(
        ...
        function(err) {
          ...
          _verifyCropping(
            ctx,
            ctx.user,
            _createSelectedArea(10, 10, 20000),
            400,
            callback,
          );
        },
      ...
      RestAPI.User.uploadPicture(
        ...
        function(err) {
          ...
          _verifyCropping(
            ctx,
            ctx.user,
            _createSelectedArea(20000, 10, 100),
            400,
            function() {
              _verifyCropping(
                ctx,
                ctx.user,
                _createSelectedArea(10, 200000, 100),
                400,
                callback,
              );
            },
  ...
  it('verify uploading or cropping a picture for a non-managed group is not allowed', function(callback) {
    _createUsers(function(contexts) {
      TestsUtil.generateTestGroups(contexts['simon'].restContext, 1, function(
        group,
      ) {
        group = group.group;
        RestAPI.Group.uploadPicture(
    ...
    _createUser(function(ctx) {
      ...
      RestAPI.User.uploadPicture(
        ...
        function(err) {
          ...
          RestAPI.User.getUser(ctx, ctx.user.id, function(
            err,
            firstRequestUser,
          ) {
            assert.ok(!err);
            RestAPI.User.getUser(ctx, ctx.user.id, function(
              ...
              assert.ok(!err);
              assert.equal(
                firstRequestUser.smallPicture,
                secondRequestUser.smallPicture,
              );
              assert.equal(
                firstRequestUser.mediumPicture,
                secondRequestUser.mediumPicture,
              );
              assert.equal(
                firstRequestUser.largePicture,
                secondRequestUser.largePicture,
              );
              return callback();
            });
          });
            ...
            RestAPI.User.uploadPicture(
              ...
              function(err) {
                ...
                RestAPI.User.getUser(ctx, ctx.user.id, function(
                  ...
                  var smallPicturePath =
                    rootFilesDir +
                    '/' +
                    _getUriFromDownloadUrl(
                      firstRequestUser.picture.small,
                    ).split(':')[1];
                  var mediumPicturePath =
                    rootFilesDir +
                    '/' +
                    _getUriFromDownloadUrl(
                      firstRequestUser.picture.medium,
                    ).split(':')[1];
                  assert.equal(
  ...
  it('verify that uploading and cropping responds with the expected principal model and no back-end picture URIs', function(callback) {
    ...
    TestsUtil.generateTestUsers(camAdminRestContext, 1, function(
      ...
      PrincipalsTestUtil.uploadAndCropPicture(
        ...
        _getPictureStream,
        { x: 10, y: 10, width: 200 },
        function(uploadPrincipal, cropPrincipal) {
          // Ensure the user model of the "upload large picture" response
          assert.equal(
            uploadPrincipal.tenant.alias,
            mrvisser.user.tenant.alias,
          );
          assert.equal(
            uploadPrincipal.tenant.displayName,
          ...
          assert.equal(
            cropPrincipal.tenant.displayName,
            mrvisser.user.tenant.displayName,
          );
          ...
          TestsUtil.generateTestGroups(mrvisser.restContext, 1, function(
            group,
          ) {
            group = group.group;
   ...
   * @api private
   */
  var _verifySearchThumbnails = function(
    restContext,
    groupId,
    canPublic,
    canLoggedIn,
    ...
    SearchTestsUtil.searchAll(
      ...
      function(err, results) {
        ...
        assert.equal(
          users[publicUserId].hasOwnProperty('thumbnailUrl'),
          canPublic,
        );
        assert.equal(
          users[loggedinUserId].hasOwnProperty('thumbnailUrl'),
          canLoggedIn,
        );
        assert.equal(
          users[privateUserId].hasOwnProperty('thumbnailUrl'),
          canPrivate,
        );
        callback();
      },
  ...
  it('verify the thumbnail is present in group search results', function(callback) {
    ...
    TestsUtil.generateTestUsers(camAdminRestContext, 1, function(err, users) {
      ...
      // Create some groups
      TestsUtil.generateTestGroups(simon.restContext, 4, function(
        oaeTeam,
        backendTeam,
        uiTeam,
        qaTeam,
