 ...
 *
 * Lets say you want to notify all the managers of a piece of content (object) except for the recent collaborators:
 *
 * ```javascript
 * options.streams = {
 *      'notification': {
 *          'router': {
 *              'object': ['managers', '^recent-collaborators']
 *          }
 *      }
 *  }
 * ```
 ...
 * and the recent collaborators should be excluded from the result. Note that switching the parameters, will give different results.
 *
 * ```javascript
 *    ...
 *    'object': ['^recent-collaborators', 'managers']
 *    ...
 * ```
 *
 * In this routing configuration, we would exclude the recent collaborators from the empty set followed by adding all the managers.
