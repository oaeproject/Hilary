...
describe('Content Activity', function() {
    ...
    describe('Routes', function() {
        ...
        it('verify that publishing a collaborative document generates a notification', function(callback) {
            TestsUtil.generateTestUsers(camAdminRestContext, 3, function(err, users) {
                assert.ok(!err);
                var simonUser = _.values(users)[0].user;
                ...
                RestAPI.Content.createCollabDoc(simonCtx, name, 'description', 'public', [brandenId], [nicoId], function(err, contentObj) {
                    assert.ok(!err);
                    ...
                    etherpadClient.setText(args, function(err) {
                        ...
                        // Set the email of simon so he gets an email
                        RestAPI.User.updateUser(simonCtx, simonUser.id, {'email': 'simong@email.blah'}, function(err) {
                            assert.ok(!err);
                            ...
                            RestAPI.Content.publishCollabDoc(brandenCtx, contentObj.id, function(err, data) {
                                ...
                                EmailTestsUtil.collectAndFetchEmails(function(messages) {
                                    ...
                                    assert.equal(messages[0]._message.to, 'simong@email.blah');
                                    assert.notEqual(messages[0]._message.html.indexOf(contentObj.profilePath));
                                    ...
                                    ActivityTestsUtil.collectAndGetActivityStream(jackCtx, jack.id, null, function(err, activityStream) {
                                        ...
                                        RestAPI.Content.setPreviewItems(globalTenantAdminRestContext, link.id, revisionId, 'ignored', {}, {}, {}, {}, function(err) {
                                            ...
                                            ActivityTestsUtil.collectAndGetActivityStream(jackCtx, jack.id, null, function(err, activityStream) {
                                                ...
                                                RestAPI.Content.setPreviewItems(globalTenantAdminRestContext, link.id, revisionId, 'done', suitable_files, suitable_sizes, {}, {}, function(err) {
                                                    assert.ok(!err);
    ...
    describe('Email', function() {
                        ...
                        RestAPI.Content.createLink(mrvisser.restContext, 'Google', 'Google', 'public', 'http://www.google.ca', [], [], function(err, link) {
                            ...
                            RestAPI.Content.createComment(simong.restContext, link.id, '<b>Nice link.</b>\n\nWould click again', null, function(err, simongComment) {
                                ...
                                EmailTestsUtil.collectAndFetchEmails(function(messages) {
                                    ...
                                    // \n's into <br/>'s
                                    assert.notEqual(stringEmail.indexOf('&lt;b&gt;Nice link.&lt;&#x2f;b&gt;<br/><br/>'), -1);
        ...
        /**
         * Test that verifies the email content is escaped
         */
        it('verify content is escaped', function(callback) {
            TestsUtil.generateTestUsers(camAdminRestContext, 2, function(err, users) {
                assert.ok(!err);

                var coenego = _.values(users)[0];
                var simong = _.values(users)[1];

                // Update the Coenego user
                RestAPI.User.updateUser(coenego.restContext, coenego.user.id, {'email': 'coenego' + Math.random() + '@email.com'}, function(err) {
                    assert.ok(!err);

                    // Update the Simong user
                    RestAPI.User.updateUser(simong.restContext, simong.user.id, {'email': 'simong' + Math.random() + '@email.com'}, function(err) {
                        assert.ok(!err);

                        // Create a new content item
                        RestAPI.Content.createLink(coenego.restContext, 'Google', 'Google', 'public', 'http://www.google.be', [], [], function(err, link) {
                            assert.ok(!err);

                            // Create a new commment with the Simong user. This will send an email to the Coenego user
                            var comment = '<script>window.alert("hello world!");</script><p class="test"><span>Nice</span> link, would <a href="http://www.google.be" target="_blank"><a>click</a></b> again</p>';
                            RestAPI.Content.createComment(simong.restContext, link.id, comment, null, function(err, simongComment) {
                                assert.ok(!err);
                                ...
                                EmailTestsUtil.collectAndFetchEmails(function(messages) {
                                    ...
                                    // Assure the HTML string wasn't removed from the message body
                                    assert.ok(message.body.indexOf(comment));

                                    // Sanity check that HTML tags have been escaped
                                    assert.ok(message.html.indexOf(Sanitization.encodeForHTML(comment)));
