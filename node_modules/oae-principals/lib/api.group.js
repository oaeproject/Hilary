 ...
 * @param  {Function}       callback                        Standard callback function
 * @param  {Obect}          callback.err                    An error that occured, if any
...
/**
 * Given an authz change on a given group, update all the membership libraries that are involved
 *
 ...
 * @param  {String[]}       removedMemberIds        The ids of the principals who were removed from the group
 * @param  {Function}       callback                Standard callback function
 * @param  {Object}         callback.err            An error object, if any
 * @api private
 */
var _touchMembershipLibraries = function(group, oldLastModified, newMemberIds, removedMemberIds, callback) {
    // Get the ancestors of this group as these can be used by all of the functions below
    _getAncestors(group, function(err, parentGroups) {
        ...
        // Insert the group (and its ancesters) into the new members their membership libraries
        _insertMembershipsLibraries(groups, newMemberIds, function(err, explodedInsertedPrincipals) {
            if (err) {
                return callback(err);
            }

            // Remove the group (and its ancestors) from the removed principals their membership libraries
            _removeMembershipsLibraries(removedMemberIds, groups, function(err) {
                if (err) {
                    return callback(err);
                }

                // Update the membership library of all the other members of the changed group so it comes to the top
                if (oldLastModified) {
...
/**
 * This method will insert the given `groups` into the memberships libraries of `newMemberIds` AND
 * all the children of `newMemberIds`.
 *
 * @param  {Group}          groups                  The groups to insert into the membership libraries
 * @param  {String[]}       newMemberIds            The ids of the members who were added to a group
 * @param  {Function}       callback                Standard callback function
 * @param  {Object}         callback.err            An error that occurred, if any
 * @param  {String[]}       callback.principals     The ids of the principals whose membership libraries were updated
 * @api private
 */
var _insertMembershipsLibraries = function(groups, newMemberIds, callback) {
    ...
    _getAllChildren(newMemberIds, [], function(err, allChildren) {
        ...
        // Add all the groups to all the new members their libraries
        var done = _.after(groups.length, function() {
            return callback(null, allChildren);
        });
        _.each(groups, function(group) {
            var libraryVisibilities = LibraryAPI.Authz.resolveEffectiveLibraryVisibilities(allChildren, group);
            LibraryAPI.Index.insert(PrincipalsConstants.library.MEMBERSHIPS_INDEX_NAME, libraryVisibilities, group.id, group.lastModified, function(err) {
                if (err) {
                    log().error({
                        'err': err,
                        'group': group
 ...
 * @param  {String[]}       excludePrincipals       The ids of principals whose membership libraries should not be updated
 * @param  {Function}       callback                Standard callback function
 * @param  {Object}         callback.err            An error that occurred, if any
 * @api private
 */
var _updateMembershipsLibraries = function(group, excludePrincipals, callback) {
    ...
    _getAllChildren([group.id], excludePrincipals, function(err, allChildren) {
        ...
        // Update all the groups in all the updated members (and their children) their libraries
        var libraryVisibilities = LibraryAPI.Authz.resolveEffectiveLibraryVisibilities(allChildren, group);
        LibraryAPI.Index.update(PrincipalsConstants.library.MEMBERSHIPS_INDEX_NAME, libraryVisibilities, group.id, group.lastModified, group.oldLastModified, function(err) {
            if (err) {
                log().error({
                    'err': err,
                    'group': group
 ...
 *
 * @param  {String[]}   removedMemberIds    The ids of the members from whose libraries the groups needs to be removed
 * @param  {Group[]}    groups              The groups who should be removed from the `removedMemberIds` their membership libraries
 * @param  {Function}   callback            Standard callback function
 * @param  {Object}     callback.err        An error that occurred, if any
 * @api private
 */
var _removeMembershipsLibraries = function(removedMemberIds, groups, callback) {
    ...
    _getAllChildren(removedMemberIds, [], function(err, allChildren) {
        ...
        // Remove all the group from all the removed members (and their children) their libraries
        var done = _.after(groups.length, callback);
        _.each(groups, function(group) {
            var lastModified = group.oldLastModified || group.lastModified;
            LibraryAPI.Index.remove(PrincipalsConstants.library.MEMBERSHIPS_INDEX_NAME, allChildren, group.id, lastModified, function(err) {
                if (err) {
                    log().error({
                        'err': err,
                        'group': group
                    }, 'Unable to remove a group from a principal\'s membership library');
                }
                done();
            });
        });
 ...
 *
 * @param  {Group}          group               The group whose ancestors to retrieve
 * @param  {Function}       callback            Standard callback function
 * @param  {Object}         callback.err        An error object, if any
 * @param  {Group[]}        callback.groups     The ancestor groups AND the passed in group
 * @api private
 */
var _getAncestors = function(group, callback) {
    // Get all the ancestors of the group
    AuthzAPI.getPrincipalMemberships(group.id, 0, 10000, function(err, memberships) {
        if (err) {
            return callback(err);
        }

        // Get a light-weight group representation for each ancestor
        PrincipalsDAO.getPrincipals(memberships, ['principalId', 'tenantAlias', 'lastModified', 'visibility'], function(err, parentGroups) {
            if (err) {
                return callback(err);
            }
 ...
 *
 * @param  {String[]}   principalIds        The ids of the principals for which to retrieve all the children
 * @param  {Function}   callback            Standard callback function
 * @param  {Object}     callback.err        An error object, if any
 * @param  {String[]}   callback.children   The ids of the principals which are a direct or indirect member of any of the given principal ids. The passed in principal ids will be included in this result set
 * @api private
 */
var _getAllChildren = function(principalIds, excludePrincipals, callback, _groupsToInvalidate, _allChildren) {
    _allChildren = _allChildren || principalIds;
    _groupsToInvalidate = _groupsToInvalidate || _.filter(principalIds, AuthzUtil.isGroupId);

    // If there are no more groups to explode, we can remove the group from all the affected
    // members their library
