...
describe('Utilities', function() {
  describe('Cassandra', function() {
          ...
          Cassandra.iterateAll(
            ...
            function(rows, done) {
              ...
              assert.equal(
                ++numInvoked,
                1,
                'Expected onEach to only be invoked once',
              );
              done();
            },
            function(err) {
        ...
        function(err, created) {
          ...
          Cassandra.runBatchQuery(batch, function(err) {
            ...
            var _onEach = function(rows, done) {
              ...
              assert.ok(
                rows,
                'Expected there to be rows provided to the onEach',
              );
              assert.equal(
                rows.length,
                1,
                'Expected there to be exactly one row',
              );
              ...
              assert.equal(
                rows[0].get('keyId'),
                'key1',
                'Invalid value for keyId',
              );
              assert.equal(
                rows[0].get('colOne'),
                'one',
                'Invalid value for colOne',
              );
              assert.equal(
                rows[0].get('colTwo'),
                'two',
                'Invalid value for colTwo',
              );
            ...
            Cassandra.iterateAll(
              ...
              function(err) {
                ...
                var _onEach = function(rows, done) {
                  ...
                  assert.ok(
                    !rows[0].get('keyId'),
                    'Expected the keyId not to be fetched',
                  );
                  assert.ok(
                    !rows[0].get('colTwo'),
                    'expected no colTwo column to be fetched',
                  );
                  assert.equal(
                    rows[0].get('colOne'),
                    'one',
          ...
          for (var i = 0; i < 10; i++) {
            batch.push(
              Cassandra.constructUpsertCQL(
                ...
                'keyId',
                'key' + i,
                { colOne: 'colOne' + i, colTwo: 'colTwo' + i },
              ),
            );
          }
          ...
          Cassandra.runBatchQuery(batch, function(err) {
            ...
            var _onEach = function(rows, done) {
              ...
              assert.equal(
                rows.length,
                1,
                'Expected to only get 1 row at a time',
              );
            ...
            Cassandra.iterateAll(
              ...
              function(err) {
                ...
                assert.equal(
                  numInvoked,
                  10,
                  'Expected to have exactly 10 batches of data',
                );

                // Verify the contents of all the rows
                assert.equal(
                  _.keys(allRows).length,
                  10,
                  'Expected exactly 10 distinct rows',
                );
                for (var i = 0; i < 10; i++) {
                  var key = 'key' + i;
                  assert.ok(
                    allRows[key],
                    'Expected to get a row with key ' + key,
                  );
                  assert.equal(
        ...
        function(err, created) {
          ...
          Cassandra.columnFamilyExists('testBatchQuery', function(err, exists) {
            ...
            var queries = [
              {
                query:
                  'INSERT INTO "testBatchQuery" ("keyId", "c1", "c2") VALUES (?, ?, ?)',
            ...
            function(err) {
              ...
              Cassandra.runQuery(
                ...
                function(err, rows) {
                  ...
                  assert.equal(
                    typeof OaeUtil.castToBoolean(
                      _.first(rows).get('testnumbool'),
                    ),
                    'boolean',
                  );
                  assert.equal(
                    typeof OaeUtil.castToBoolean(
                      _.first(rows).get('teststring'),
                    ),
                    'string',
                  );
                  callback();
    ...
    it('verify construct upsert', function(callback) {
      ...
      var query1 = Cassandra.constructUpsertCQL(null, 'testId', 'testValue', {
        key1: 'value1',
      });
      assert.ok(!query1);

      // Test an invalid call with no provided values
      var query2 = Cassandra.constructUpsertCQL(
        'testCF',
        'testId',
        'testValue',
        {},
      ...
      var query3 = Cassandra.constructUpsertCQL(
        'testCF',
        'testId',
        'testValue',
        { key1: 'value1' },
      ...
      assert.equal(
        query3.query,
        'UPDATE "testCF" SET "key1" = ? WHERE "testId" = ?',
      );
      ...
      var query5 = Cassandra.constructUpsertCQL(
        'testCF',
        'testId',
        'testValue',
      ...
      var query7 = Cassandra.constructUpsertCQL(
        'testCF',
        'testId',
        'testValue',
        { key1: 'value1', key2: 'value2', key3: 'value3' },
      ...
      assert.ok(query7);
      assert.strictEqual(
        query7.query.indexOf('UPDATE "testCF" USING TTL 500 SET '),
        0,
      );
      ...
      var query8 = Cassandra.constructUpsertCQL(
        'testCF',
        'testId',
        'testValue',
        {
          key1: { anobject: 'a value' },
          key2: ['index0', 'index1'],
          key3: 'value3',
        },
        500,
      ...
      assert.ok(query8);
      assert.strictEqual(
        query8.query.indexOf('UPDATE "testCF" USING TTL 500 SET '),
        0,
      );
    ...
    it('verify truncation of cassandra query log entries', function(callback) {
      ...
      var longQueryMoreThan10 =
        invalidQueryLongerThan300 + '? ? ? ? ? ? ? ? ? ? ? ?';
      ...
      // the test
      Cassandra.runQuery(longQueryMoreThan10, longParamsMoreThan10, function(
        err,
      ) {
        assert.ok(err);
