...
/**
 * Adds the signature and thumbnail url (if the content item has a thumbnail)
 *
 ...
 * the target using a signature. If the uri represents a target that has a "direct" download strategy, the target will be
 * provided directly rather than a signed request that comes back through the /api/download/signed endpoint. Therefore
 * the expiry parameters `duration` and `offset` are invalid concepts and have no effect in that case
 *
 * @param  {Context}    ctx         The context of the user request for which we are signing the download
