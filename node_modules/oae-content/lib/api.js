...
var createFile = module.exports.createFile = function(ctx, name, description, visibility, additionalMembers, file, callback) {
    ...
    var validator = new Validator();
    validator.check(file.mime, {'code': 400, 'msg': 'Missing mimetype on the file object.'}).notEmpty();
    validator.check(file.size, {'code': 400, 'msg': 'Missing size on the file object.'}).notEmpty();
    validator.check(file.filename, {'code': 400, 'msg': 'Missing filename on the file object.'}).notEmpty();
    if (validator.hasErrors()) {
        return callback(validator.getFirstError());
    }
    ...
    _createContent(ctx, 'file', name, description, visibility, additionalMembers, otherValues, function(err, contentObj) {
        ...
        _getStorageBackend(ctx).store(ctx, opts, function(err, uri) {
            if (err) {
                return callback(err);
            }
            ...
            _createRevision(ctx, contentObj.contentId, uri, opts, function(err) {
                if (err) {
                    return callback(err);
                }
...
var _getStorageBackend = function(ctx, uri) {
    ...
    if (uri) {
        ...
        if (backendName.code) {
            // In case something went wrong in the config api, an error object will be returned.
            // This should never happen, so something is seriously wrong at this point.
            log().error({'err': err}, 'Config error, aborting.');
            throw new Error('Retrieving the backend from the config resulted in an error.');
        }
    }
