...
describe('Loodle update', function () {
    ...
    describe('update the role of an user', function () {
        ...
        it('should correctly update the role of the user for the loodle', function (callback) {

            setUpUsers(function (contexts) {

                // Create a loodle
                RestAPI.Loodles.createLoodle(contexts['branden'].restContext, 'Mon loodle', 'test', 'private', [contexts['simon'].user.id], [], [], function (err, data) {
                    if (err) return callback(err);

                    // Update the role a loodle user
                    var changes = {};
                    changes[contexts['simon'].user.id] = 'viewer';
                    RestAPI.Content.updateMembers(contexts['branden'].restContext, data.id, changes, function (err) {
                        if (err) return callback(err);

                        // Check if the role has been correcly updated
                        RestAPI.Content.getContent(contexts['simon'].restContext, data.id, function (err, content) {
                            assert.equal(err, null);
                            assert.equal(content.isManager, false);
        ...
        it('should handle the case where the user has another access to the loodle through a group : the user keeps/gains the "higher" role', function (callback) {
            ...
            setUpUsers(function (contexts) {
                ...
                RestAPI.Group.createGroup(contexts['branden'].restContext, 'Mon groupe', 'test', 'private', 'no', [contexts['simon'].user.id], [], function (err, group) {
                    ...
                    // Create a loodle with the group and one group user as members
                    RestAPI.Loodles.createLoodle(contexts['branden'].restContext, 'Mon loodle', 'test', 'private', [], [group.id, contexts['simon'].user.id], [], function (err, data) {
                        if (err) return callback(err);

                        // Update the role of the loodle user
                        var changes = {};
                        changes[contexts['simon'].user.id] = 'manager';
                        RestAPI.Content.updateMembers(contexts['branden'].restContext, data.id, changes, function (err) {
                            if (err) return callback(err);

                            // Check if the updated user has the correct role
                            RestAPI.Content.getContent(contexts['simon'].restContext, data.id, function (err, content) {
                                assert.equal(err, null);
                                assert.equal(content.isManager, true);
        ...
        it('should handle the case where the user has another access to the loodle through a folder : the user role is still updated', function (callback) {
