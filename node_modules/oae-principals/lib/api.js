 ...
 * * `updatedUser(ctx, newUser, oldUser)`: A user was updated. The `ctx`, updated user and previous user object are provided
 * * `createdGroup(ctx, group)`: A group was created. The `ctx` and the `group` object that was created are both provided.
 * * `updatedGroup(ctx, newGroup, oldGroup)`: A group was updated. The `ctx`, old group (before update) and the newly updated group data are provided.
