...
var resolveTargetLibraryAccess = module.exports.resolveTargetLibraryAccess = function(ctx, libraryId, libraryOwner, callback) {
    var roleHigh = 'manager';
    var roleLow = 'member';
    var rolesPriority = [roleLow, roleHigh];

    // We only need to know "high role" and "low role" for this, so the `rolesPriority` we use is
    // rather arbitrary
    AuthzAPI.resolveImplicitRole(ctx.user(), libraryOwner, rolesPriority, function(err, implicitRole) {
        if (err) {
            return callback(err);
        } else if (implicitRole === roleHigh) {
        ...
        // cases
        var needsExplicitCheck = (!AuthzUtil.isUserId(libraryId) && ctx.user());
        let innerCallback = function(err, hasAnyRole) {
        ...
        if(needsExplicitCheck) {
            console.log("Invoking hasAnyRole with " + (ctx.user() && ctx.user().id) + "," + libraryId);
            AuthzAPI.hasAnyRole((ctx.user() && ctx.user().id), libraryId, innerCallback);
        } else {
            console.log("returning innerCallback()!");
            return innerCallback();
        }
        // OaeUtil.invokeIfNecessary(needsExplicitCheck, AuthzAPI.hasAnyRole, (ctx.user() && ctx.user().id), libraryId, callback);
    });
};
 ...
 */
var resolveLibraryBucketVisibility = module.exports.resolveLibraryBucketVisibility = function(libraryId, resource) {
