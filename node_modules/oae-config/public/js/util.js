            ...
            $.ajax({
                ...
                success: function(data) {
                    ...
                    if (err.status === 401) {
                        showError({
                            'title': 'Login failed',
                            'message': 'The password and username do not match.'
                        });
                    }
                }
            });
    ...
    var showLogin = function(adminContext, configuration) {
        ...
        $.each($('#admin_login_container form'), function(i, formEl) {
            $(formEl).validate({
    ...
    var switchView = function(hash) {
        ...
        $('#admin_lhnav li').removeClass('active');
        $('#admin_lhnav li#' + hash).addClass('active');
    ...
    /**
     * Mark a table row as deleted and disable the input fields inside of the row
     */
    var setTableRowDeleted = function() {
        var $tr = $(this).parents('tr');
        $(this).siblings().toggle();
        $(this).toggle();
        if ($tr.hasClass('error')) {
            $tr.removeClass('error');
            $tr.find('input').removeAttr('data-disabled');
            $tr.find('input').removeAttr('disabled');
        } else {
            $tr.addClass('error');
            $tr.find('input').attr('data-disabled', true);
            $tr.find('input').attr('disabled', true);
        }
    };

    /**
     * Adds a row to the table before the 'add row' button.
     * The row is based on a predefined template. The template can be built using a schema in the configuration.
     */
    var addTableRow = function() {
        var $table = $(this).prev('table');
        $table.show();
        var $template = $table.find('.new_table_row_template').clone().removeClass('new_table_row_template').show();
        // make ID's unique
        var time = new Date().getTime();
        $.each($template.find('input'), function(i, input) {
            $(input).attr('id', $(input).attr('id').replace('rndnumber', 'id' + time));
        });
        $table.children('tbody').children(':last-child').before($template);
    };

    /**
     * Creates a treeview using the jstree plugin
     * @param {String}    id               The id of the container the treeview will be rendered in
     * @param {Array{}}   treeStructure    The treeview structure as returned from the server to render on the page
     */
    var createTree = function(id, treeStructure) {
        $('#' + id).tree({
            selected: treeStructure[0].attributes.id || '',
            ui: {
                theme_name: "oae"
            },
            data: {
            ...
            callback: {
                oncreate: function(el, ref, html) {
                    $(el).attr('id', 'id' + new Date().getTime());
                }
            }
        });

        $('#' + id + '_rename').on('click', function() {
            $.tree.reference($(this).attr('data-for-id')).rename();
        });

        $('#' + id + '_remove').on('click', function() {
            $.tree.reference($(this).attr('data-for-id')).remove();
        });

        $('#' + id + '_create').on('click', function() {
            $.tree.reference($(this).attr('data-for-id')).create();
        });
    };
