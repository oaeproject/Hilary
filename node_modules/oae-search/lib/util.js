    ...
    if (!hits || !hits.hits || !hits.hits.length) {
        ...
        var _monitorTransformers = function(err, resourceType, docs) {
            if (transformErr) {
                ...
                // If we aren't using the default transformer, ensure the _type and resourceType by merging them over the doc, those should never change and don't necessarily need to be known by an extension transformer
                if (resourceType !== '*') {
