...
module.exports.init = function(config, callback) {
    ...
    pool.connect(function(err) {
        ...
        var keyspaceMetadata = getKeyspace(keyspace);
        if (keyspaceMetadata) {
            return _reloadPool(callback);
        }
 ...
 */
var getKeyspace = module.exports.getKeyspace = function(name) {
    return pool.metadata.keyspaces[name];
};
...
var columnFamilyExists = module.exports.columnFamilyExists = function(name, callback) {
    runQuery('SELECT "columnfamily_name" FROM "system"."schema_columnfamilies" WHERE "keyspace_name" = ? AND "columnfamily_name" = ?', [CONFIG.keyspace, name], function(err, rows) {
        if (err) {
            return callback(err);
        }
...
var dropColumnFamily = module.exports.dropColumnFamily = function(name, callback) {
    ...
    columnFamilyExists(name, function(err, exists) {
        ...
        runQuery(util.format('DROP TABLE "%s"', name), [], function(err) {
            if (err) {
                return callback(err);
            }

            // The Cassandra drivers caches the CF schema internally and there is no way of
            // refreshing it synchronously. To refresh it, we close and re-open the connection pool,
            // which makes this function not suitable to use in production runtime!
    ...
    columnFamilyExists(name, function(err, exists) {
        ...
        if (!exists) {
            runQuery(cql, false, function(err) {
                ...
                // The Cassandra driver caches the CF schema internally and there is no way of
                // refreshing it synchronously. To refresh it, we close and re-open the connection
                // pool, which makes this function not suitable to use in production runtime!
                _reloadPool(function(err) {
                    if (err) {
                        return callback(err);
                    }
...
var runPagedQuery = module.exports.runPagedQuery = function(tableName, keyColumnName, keyColumnValue, rangeColumnName, start, limit, opts, callback) {
    ...
    runQuery(cql, params, function(err, rows) {
        ...
        var nextToken = (results.length === limit) ? _.last(results).get(rangeColumnName) : null;
...
var constructUpsertCQL = module.exports.constructUpsertCQL = function(cf, rowKey, rowValue, values, ttl) {
    ...
    _.each(values, function(value, key) {
        ...
        if (!_.isDate(value) && _.isObject(value) && !(value instanceof Long)) {
...
var executeQuery = function(query, parameters, callback) {
    ...
    // Use consistency of QUORUM (2) for all queries
    pool.execute(query, parameters, {'consistencyLevel': 2}, function(err, result) {
        if (err) {
            log().error({
                'err': err,
                'query': query,
...
var _createPool = function(config) {
    ...
    var _pool = new cassandra.Client(cassandraConfig);
