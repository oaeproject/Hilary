...
describe('Users', function() {
    ...
    before(function(callback) {
        ...
        anonymousRestContext = TestsUtil.createAnonymousRestContext(global.oaeTests.tenants.cam.host);
        // Fill up Cam tenant admin rest context
        camAdminRestContext = TestsUtil.createAdminRestContext(global.oaeTests.tenants.cam.host);
        // Fill up GT tenant admin rest context
        gtAdminRestContext = TestsUtil.createAdminRestContext(global.oaeTests.tenants.gt.host);
    ...
    describe('Create user', function() {
        ...
        it('verify create user', function(callback) {
            ...
            RestAPI.User.createUser(anonymousRestContext, userId, 'password', 'Test User', {'visibility': 'public'}, function(err, userObj) {
                assert.ok(err);
                ...
                RestAPI.User.createUser(camAdminRestContext, userId, 'password', 'Test User', {'visibility': 'public'}, function(err, createdUser) {
                    ...
                    var userRestContext = new RestContext(global.oaeTests.tenants.cam.host, userId, 'password');
    ...
    describe('Get user', function() {
        ...
        it('verify get user', function(callback) {
            // Create a test user
            var userId = TestsUtil.generateTestUserId();
            RestAPI.User.createUser(camAdminRestContext, userId, 'password', 'Test User', {'visibility': 'public'}, function(err, userObj) {
                assert.ok(!err);
                assert.ok(userObj);
                ...
                RestAPI.User.getUser(anonymousRestContext, userObj.id, function(err, retrievedUser) {
                    ...
                    assert.equal(retrievedUser.profile.displayName, 'Test User');
                    callback();
                });
            });
        });
        ...
        it('verify get user by ugly username', function(callback) {
            ...
            RestAPI.User.createUser(camAdminRestContext, userId1, 'password', 'Test User', {'visibility': 'public'},  function(err, userObj1) {
                ...
                RestAPI.User.getUser(anonymousRestContext, userObj1.id, function(err, retrievedUser1) {
                    ...
                    assert.equal(retrievedUser1.profile.displayName, 'Test User');
                    ...
                    RestAPI.User.createUser(camAdminRestContext, userId2, 'password', 'Кругом шумел', {'visibility': 'public'}, function(err, userObj2) {
                        ...
                        RestAPI.User.getUser(anonymousRestContext, userObj2.id, function(err, retrievedUser2) {
                            ...
                            assert.equal(retrievedUser2.profile.displayName, 'Кругом шумел');
                            callback();
                        });
                    });
                });
        ...
        /**
         * Test that verifies that a user can be retrieved by its username and tenant. This will use the internal API
         * as the functionality has not yet been exposed through the REST API.
         */
        it('verify get user by username and tenant', function(callback) {
            // Create a test user with an ugly user name
            var userId = TestsUtil.generateTestUserId();
            RestAPI.User.createUser(camAdminRestContext, userId, 'password', 'Test User', null, function(err, userObj) {
                assert.ok(!err);
                ...
                // Try and get that user by username and tenant
                var ctx = new Context(global.oaeTests.tenants.cam, userObj);
                PrincipalsAPI.getTenantUser(ctx, userId, function(err, user) {
                    assert.ok(!err);
                    assert.ok(user);
    ...
    describe('Update user', function() {
        ...
        it('verify update user', function(callback) {
            ...
            RestAPI.User.createUser(camAdminRestContext, testUserId, 'password', 'Test User', {'visibility': 'public', 'locale': 'en_GB', 'timezone': 'Europe/London'}, function(err, userObj) {
                ...
                var testUserRestContext = new RestContext(global.oaeTests.tenants.cam.host, testUserId, 'password');
            ...
            RestAPI.User.createUser(camAdminRestContext, testUserId, 'password', 'Test User', {'visibility': 'public'}, function(err, userObj) {
                ...
                RestAPI.User.updateUser(testUserRestContext, userObj.id, updateValues, function (err) {
                    ...
                    RestAPI.User.createUser(camAdminRestContext, updaterUserId, 'password', 'Test User', null, function(err, updaterUserObj) {
                        ...
                        var updateUserRestContext = new RestContext(global.oaeTests.tenants.cam.host, updaterUserId, 'password');
                        updateValues = {
                            'firstName': 'John'
                        };
        ...
        it('verify user permissions', function(callback) {
            ...
            var verifyProfilePermissions = function(restContext, userToCheck, expectAccess, expectedVisibility, checkDone) {
                ...
                RestAPI.User.getUser(restContext, userToCheck, function(err, userObj) {
                    ...
                    if (expectAccess) {
                        assert.equal(userObj.profile.firstName, 'FirstName');
                        assert.equal(userObj.profile.lastName, 'LastName');
                    } else {
            ...
            RestAPI.User.createUser(camAdminRestContext, jackUserId, 'password', 'Jack Doe', {'visibility': 'public'}, function(err, jack) {
                assert.ok(!err);
                assert.ok(jack);
                var jackRestContext = new RestContext(global.oaeTests.tenants.cam.host, jackUserId, 'password');
                ...
                RestAPI.User.createUser(camAdminRestContext, janeUserId, 'password', 'Jane Doe', {'visibility': 'public'}, function(err, jane) {
                    assert.ok(!err);
                    assert.ok(jane);
                    var janeRestContext = new RestContext(global.oaeTests.tenants.cam.host, janeUserId, 'password');
    ...
    describe('User password', function() {
        ...
        it('verify admin change password', function(callback) {
            ...
            RestAPI.User.createUser(camAdminRestContext, testUserId, 'password', 'Test User', null, function(err, userObj) {
                ...
                RestAPI.User.changePassword(anonymousRestContext, userObj.id, 'password', 'totally-new-password', function(err) {
                    assert.ok(err);
                
                    // Try to change the password as the tenant admin
                    RestAPI.User.changePassword(camAdminRestContext, userObj.id, 'password', 'totally-new-password', function(err) {
                        assert.ok(!err);
                        callback();
                    });
                });
    ...
    describe('Global and tenant admin', function() {
                            ...
                            PrincipalsAPI.setGlobalAdmin(jackContext, jack.id, true, function(err) {
                                ...
                                PrincipalsAPI.setGlobalAdmin(jackContext, jane.id, true, function(err) {
                                    ...
                                    PrincipalsAPI.setGlobalAdmin(globalAdminContext, jack.id, true, function(err) {
                                        ...
                                        PrincipalsAPI.getTenantUser(new Context(global.oaeTests.tenants.cam), jackUserId, function(err, user) {
                                            assert.ok(!err);
                                            ...
                                            PrincipalsAPI.setGlobalAdmin(jackContext, jane.id, true, function(err) {
                                                ...
                                                PrincipalsAPI.getTenantUser(new Context(global.oaeTests.tenants.cam), janeUserId, function(err, user) {
                                                    assert.ok(!err);
                                                    assert.equal(user.isGlobalAdmin(), true);
                                                    assert.equal(user.isAdmin(global.oaeTests.tenants.cam.alias), true);
                                                    assert.equal(user.isTenantAdmin(global.oaeTests.tenants.cam.alias), false);
        ...
        it('Verify tenant admin separation', function(callback) {
            // We create 3 users: jack, jane and goe. Jack and Joe are in tenant A, Jane is in tenant B.
            ...
            RestAPI.User.createUser(camAdminRestContext, jackUserId, 'password', 'Jack Doe', null, function(err, jack) {
                ...
                RestAPI.User.createUser(camAdminRestContext, joeUserId, 'password', 'Joe Doe', null, function(err, joe) {
                    assert.ok(!err);
                    assert.ok(joe);
                    var joeRestContext = new RestContext(global.oaeTests.tenants.cam.host, joeUserId, 'password');
                    
                    var janeUserId = TestsUtil.generateTestUserId('jane');
                    RestAPI.User.createUser(gtAdminRestContext, janeUserId, 'password', 'Jane Doe', null, function(err, jane) {
                        assert.ok(!err);
                        assert.ok(jane);
                        var janeRestContext = new RestContext(global.oaeTests.tenants.gt.host, janeUserId, 'password');
                            ...
                            PrincipalsAPI.getTenantUser(new Context(global.oaeTests.tenants.cam), jackUserId, function(err, user) {
                                ...
                                RestAPI.User.updateUser(jackRestContext, joe.id, updateData, function(err) {
                                    ...
                                    RestAPI.User.getMe(joeRestContext, function(err, meObj) {
                                        assert.ok(!err);
                                        assert.ok(meObj);
                                        assert.equal(meObj.profile.displayName, 'Foo Bar');
                                        ...
                                        RestAPI.User.updateUser(jackRestContext, jane.id, updateData, function(err) {
                                            ...
                                            RestAPI.User.getMe(janeRestContext, function(err, meObj) {
                                                assert.ok(!err);
                                                assert.ok(meObj);
                                                assert.equal(meObj.profile.displayName, 'Jane Doe');
