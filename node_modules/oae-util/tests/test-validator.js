...
/**
 * Test whether or not the validator used to check for valid Sakai OAE
 * User objects is working as intended
 * @param {Object} test     Standard nodeunit test object
 */
module.exports.testUserValidator = function(test) {
    var tenant = new Tenant('cam', 'Cambridge University', 'Cambridge University Description', 2001);
    // Valid users
    var user1 = new User(tenant, 'u:cam:nm417', 'nm417', 'public', 'Nicolaas', 'Matthijs', 'Nicolaas Matthijs');
    var user2 = new User(tenant, 'u:cam:nm417', 'nm417', 'private', 'Nicolaas', 'Matthijs', 'Nicolaas Matthijs');
    var user3 = new User(null, 'u:cam:nm417', null, null, null, null, null);
    // Invalid users
    var user4 = new User(tenant, null, 'nm417', 'public', 'Nicolaas', 'Matthijs', 'Nicolaas Matthijs');
    var user5 = new User(tenant, '', 'nm417', 'public', 'Nicolaas', 'Matthijs', 'Nicolaas Matthijs');

    test.expect(10);

    // Single test successful
    var validator = new Validator();
    validator.check().isUser(user1);
    test.ok(!validator.hasErrors());
    test.ok(!validator.getErrors());

    var validator = new Validator();
    validator.check().isUser(user3);
    test.ok(!validator.hasErrors());
    test.ok(!validator.getErrors());

    // Single test failed
    validator = new Validator();
    validator.check().isUser(user4);
    test.ok(validator.hasErrors());
    test.ok(validator.getErrors());
