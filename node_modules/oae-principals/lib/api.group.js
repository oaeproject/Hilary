...
var getFullGroupProfile = module.exports.getFullGroupProfile = function(ctx, groupId, callback) {
    getGroup(ctx, groupId, function(err, group) {
        ...
        _getAllRoles(ctx, groupId, function(err, roles) {
            ...
            group.isMember = (isAdmin || _.contains(roles, Constants.roles.MEMBER) || _.contains(roles, Constants.roles.MANAGER));
            group.isManager = (isAdmin || _.contains(roles, Constants.roles.MANAGER));
...
var updateGroup = module.exports.updateGroup = function(ctx, groupId, profileFields, callback) {
    ...
    PrincipalsDAO.getPrincipal(groupId, function(err, oldStorgaeGroup) {
        if (err) {
            return callback(err);
        }

        // Check if we can update this group
        _canManage(ctx, oldStorgaeGroup, function(err, canManage) {
            if (err) {
                return callback(err);
            } else if (!canManage) {
            ...
            PrincipalsDAO.updatePrincipal(groupId, profileFields, function(err) {
                ...
                var updatedStorageGroup = _.extend({}, oldStorgaeGroup, profileFields);
                ...
                PrincipalsUtil.getPrincipal(ctx, groupId, function(err, updatedGroup) {
                     ...
                     */
                    var _callback = function() {
                        PrincipalsEmitter.emit(PrincipalsConstants.events.UPDATED_GROUP, ctx, updatedStorageGroup, oldStorgaeGroup);
                    ...
                    AuthzAPI.getAuthzMembers(groupId, null, 10000, function(err, memberRoles) {
                        ...
                        _updateMembershipsLibraries(memberIds, updatedGroup, oldStorgaeGroup.lastModified, function(err) {
                            if (err) {
                                log().error({
                                    'err': err,
                                    'oldGroup': oldStorgaeGroup,
