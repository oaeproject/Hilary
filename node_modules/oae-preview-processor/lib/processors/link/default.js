...
// The amount of time after which we consider a request to be timed out when checking
// if a link is embeddable in an iframe.
var EMBEDDABLE_CHECK_TIMEOUT = 15000;
 ...
 * @param  {String}     _config.renderDelay     Defines the timeout (in ms) that should be waited between loading the page and taking a screencapture
 * @param  {Number}     _config.timeout         Defines the timeout (in ms) when the link processing should be stopped
 * @param  {Function}   callback                Standard callback function
 * @param  {Object}     callback.err            Standard error object (if any)
 */
var init = module.exports.init = function(_config, callback) {
    if (!_config || !_config.renderDelay || !_config.timeout) {
        return callback({'code': 400, 'msg': 'Missing configuration for the Link Preview Processor, required fields are `renderDelay` and `timeout`.'});
    }

    webshotOptions.renderDelay = _config.renderDelay;
    webshotOptions.timeout = _config.timeout;
    callback();
};
...
var generatePreviews = module.exports.generatePreviews = function(ctx, contentObj, callback) {
    ...
    var options = {
        'url': contentObj.link,
        'method': 'HEAD',
        'timeout': EMBEDDABLE_CHECK_TIMEOUT
    };
