            ...
            setUpUsers(function(contexts) {
                ...
                RestAPI.Content.createLink(contexts['bert'].restContext, 'Test Content 1', 'Test content description 1', 'public', 'http://www.sakaiproject.org/',  [], [], function(err, publicContentObj) {
                    ...
                    RestAPI.Content.createLink(contexts['nicolaas'].restContext, 'Test Content 2', 'Test content description 2', 'private', 'http://www.sakaiproject.org/',  [], [contexts['bert'].user.id], function(err, privateContentObj) {
                        ...
                        // Create a comment as bert on the private link
                        RestAPI.Content.createComment(contexts['bert'].restContext, privateContentObj.contentId, 'This is a comment on the private content.', null, function(err) {
                            assert.ok(!err);
                            ...
                            RestAPI.Content.getComments(contexts['bert'].restContext, privateContentObj.contentId, null, 10, function(err, comments) {
                                ...
                                assert.equal(comments.results[0].body, 'This is a comment on the private content.');
                                var privateContentCommentId = comments.results[0].commentId;
                                ...
                                RestAPI.Content.updateMembers(contexts['nicolaas'].restContext, privateContentObj.contentId, permissions, function(err) {
                                    ...
                                    RestAPI.Content.deleteComment(contexts['bert'].restContext, publicContentObj.contentId, privateContentCommentId, function(err) {
                                        assert.ok(err);
                                        assert.ok(err.code, 400);
                                        // Get the comment verify that it wasn't deleted
                                        RestAPI.Content.getComments(contexts['nicolaas'].restContext, privateContentObj.contentId, null, 10, function(err, comments) {
                                            assert.ok(!err);
                                            assert.equal(comments.results.length, 1);
                                            assert.equal(comments.results[0].createdBy.id, contexts['bert'].user.id);
                                            assert.equal(comments.results[0].body, 'This is a comment on the private content.');
                                            // try to create a comment on the private content item by using the public content item's ID and the ID of the comment that was just created
                                            RestAPI.Content.createComment(contexts['bert'].restContext, publicContentObj.contentId, 'This reply on the comment shouldn\'t be accepted', privateContentCommentId, function(err) {
                                                assert.ok(err);
                                                assert.ok(err.code, 400);
                                                // Get the comment verify that it wasn't created
                                                RestAPI.Content.getComments(contexts['nicolaas'].restContext, privateContentObj.contentId, null, 10, function(err, comments) {
                                                    assert.ok(!err);
                                                    assert.equal(comments.results.length, 1);
                                                    assert.equal(comments.results[0].createdBy.id, contexts['bert'].user.id);
                                                    ...
                                                    RestAPI.Content.deleteComment(contexts['nicolaas'].restContext, publicContentObj.contentId, null, function(err) {
                                                        assert.ok(err);
                                                        assert.ok(err.code, 400);
                                                        ...
                                                        RestAPI.Content.getComments(contexts['nicolaas'].restContext, privateContentObj.contentId, null, 10, function(err, comments) {
                                                            ...
                                                            RestAPI.Content.deleteComment(contexts['bert'].restContext, null, privateContentCommentId, function(err) {
                                                                assert.ok(err);
                                                                assert.ok(err.code, 400);
