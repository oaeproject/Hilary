        ...
        'hosts': [
            {
                ...
                'externalPort': 80,
                'internalHost': '0.etherpad.oae.com',
                'internalPort': 80
            },
            {
                'externalProtocol': 'https',
                'externalHost': '1.etherpad.oae.com',
                'externalPort': 80,
                'internalHost': '1.etherpad.oae.com',
                'internalPort': 80
            }
        ]
    };
        ...
        TestsUtil.generateTestUsers(camAdminRestContext, 1, function(err, users) {
            ...
            RestAPI.Content.createCollabDoc(simonCtx, name, 'description', 'private', [], [], function(err, contentObj) {
                ...
                RestAPI.Content.getContent(simonCtx, contentObj.id, function(err, contentObj) {
                    assert.ok(!err);
                    assert.ok(!contentObj.etherpadHtml);
                    ...
                    editAndPublish(simonCtx, contentObj, [text], function() {
                        ...
                        RestAPI.Content.getContent(simonCtx, contentObj.id, function(err, updatedContentObj) {
                            assert.ok(!err);
                            assert.ok(updatedContentObj.etherpadHtml);
                            assert.notEqual(updatedContentObj.etherpadHtml, contentObj.etherpadHtml);

                            // Remove linebreaks and check if the text is correct.
                            var html = updatedContentObj.etherpadHtml.replace('<br>', '');
                            ...
                            setEtherpadText(contentObj, 'There are no facts, only interpretations.', function(err) {
                                ...
                                RestAPI.Content.getContent(simonCtx, contentObj.id, function(err, latestContentObj) {
                                    assert.ok(!err);
                                    assert.ok(latestContentObj.etherpadHtml);
                                    assert.equal(latestContentObj.etherpadHtml, updatedContentObj.etherpadHtml);
                                    callback();
                                });
                            });
                        });
                ...
                RestAPI.Content.getContent(simonCtx, contentObj.id, function(err, contentObj) {
                    ...
                    editAndPublish(simonCtx, contentObj, texts, function() {
                        RestAPI.Content.getRevisions(simonCtx, contentObj.id, null, null, function(err, revisions) {
                            ...
                            // We published our document 3 times, this should result in 3 revisions.
                            ...
                            RestAPI.Content.restoreRevision(simonCtx, contentObj.id, revisions[1].revisionId, function(err) {
                                ...
                                RestAPI.Content.getContent(simonCtx, contentObj.id, function(err, updatedContent) {
                                    ...
                                    assert.equal(updatedContent.etherpadHtml, texts[1] + '<br>');
    ...
    it('verify that etherpad related properties cannot be set on the content object', function(callback) {
        TestsUtil.generateTestUsers(camAdminRestContext, 1, function(err, users) {
            ...
            RestAPI.Content.createCollabDoc(simonCtx, name, 'description', 'public', [], [], function(err, contentObj) {
                ...
                RestAPI.Content.getContent(simonCtx, contentObj.id, function(err, baselineContentObj) {
                    assert.ok(!err);
                    ...
                    RestAPI.Content.updateContent(simonCtx, contentObj.id, {'etherpadGroupId': 'bleh'}, function(err) {
                        ...
                        RestAPI.Content.updateContent(simonCtx, contentObj.id, {'etherpadPadId': 'bleh'}, function(err) {
                            assert.equal(err.code, 400);
                            RestAPI.Content.updateContent(simonCtx, contentObj.id, {'etherpadHtml': 'bleh'}, function(err) {
                                assert.equal(err.code, 400);

                                // Double-check the the content item didn't change.
                                RestAPI.Content.getContent(simonCtx, contentObj.id, function(err, latestContentObj) {
                                    assert.ok(!err);
                                    assert.equal(baselineContentObj.etherpadGroupId, latestContentObj.etherpadGroupId);
                                    assert.equal(baselineContentObj.etherpadPadId, latestContentObj.etherpadPadId);
                                    assert.equal(baselineContentObj.etherpadHtml, latestContentObj.etherpadHtml);
                                    callback();
                                });
                            });
                        });
