...
ContentAPI.on(ContentConstants.events.CREATED_CONTENT, function(ctx, content, revision, members) {
    ...
    // Loop all the members to see if the content was created in a group
    ...
    for (var i = 0; i < principalIds.length; i++) {
        ...
        if (members[principalId]) {
            ...
            if (principalId !== ctx.user().id) {
                ...
                var activitySeed = new ActivityModel.ActivitySeed(ContentConstants.activity.ACTIVITY_CONTENT_SHARE, millis, ActivityConstants.verbs.SHARE, actorResource, objectResource, targetResource);
                ActivityAPI.postActivity(ctx, activitySeed);
            }
        }
    }
