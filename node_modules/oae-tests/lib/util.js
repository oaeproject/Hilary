...
/**
 * Returns a function that returns a readable stream that will emit data untill
 * `bytes` have been emitted. This stream can be used in requestjs to perform
 ...
 */
var createFakeFileReadableStream = module.exports.createFakeFileReadableStream = function(filename, size) {
    return function() {
        // We'll implement a readable stream that generates the data
        var rs = new stream.Stream();

        // RequestJS uses the `path` property of `fs.streams` to determine the filename, fake one
        ...
        rs.resume = function() {
             ...
             * Generates some data
             *
             * @param  {Number} toGen   The amount of data to generate
             * @return {String}         A string with exactly `toGen` characters in it
             */
            var genData = function(toGen) {
                var data = '';
            ...
            var toGen = (size > 1024 ? 1024 : 1);
            var data = genData(toGen);
            ...
            if (bytes < size) {
                rs.emit('data', genData(size - bytes));
            }
