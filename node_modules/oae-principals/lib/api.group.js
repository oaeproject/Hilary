...
var PrincipalsSearchAPI = require('./search');
...
var getGroupMembers = module.exports.getGroupMembers = function(ctx, groupId, start, limit, callback) {
    ...
    getGroup(ctx, groupId, function(err, group) {
        ...
        if (!ctx.user() && (group.visibility === Visibility.LOGGEDIN || group.visibility === Visibility.PRIVATE)) {
...
var setGroupMembers = module.exports.setGroupMembers = function(ctx, groupId, members, callback) {
    ...
    canManage(ctx, groupId, function(err, canManage) {
        ...
        _setGroupMembers(ctx, groupId, members, function(err) {
            _reindexGroup(ctx, groupId);
            return callback(err);
        });
    });
};
        ...
        if (!err) {
            ...
            Cassandra.runQuery('INSERT INTO Principals (principalId, alias, tenant, name, description, visibility, joinable) VALUES (?, ?, ?, ?, ?, ?, ?) USING CONSISTENCY QUORUM',
                ...
                _setGroupMembers(ctx, id, members, function(err) {
                    // reindex the group, regardless of the result here
                    _reindexGroup(ctx, id);
...
/**
 * Add or reindex the group identified by the given groupId.
 *
 * @param   {Context}       ctx             The context of the current request.
 * @param   {String}        groupId         The ID of the group to index
 * @param   {Function}      callback        Invoked when the process completes
 * @param   {Object}        callback.err    An error that occurred, if any
 */
var _reindexGroup = function(ctx, groupId, callback) {
    callback = callback || function(err) {
        if (err) {
            log().error({
                err: err,
                groupId: groupId
            }, 'Error updating index for group.');
        }
    }

    PrincipalsUtil.getPrincipal(ctx, groupId, function(err, group) {
        if (err) {
            return callback(err);
        }

        AuthzAPI.getAuthzMembers(groupId, null, 10000, function(err, memberships) {
            if (err) {
                return callback(err);
            }

            var members = _.map(memberships, function(membership) {
                return membership.id;
            });

            PrincipalsSearchAPI.indexGroup(group, members, callback);
        });
    });
}
