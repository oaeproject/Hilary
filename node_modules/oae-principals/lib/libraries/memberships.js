...
LibraryAPI.Search.registerLibrarySearch('memberships-library', ['group'], {
  searches: {
    private: function(ctx, libraryOwner, opts, callback) {
      ...
      _getAllGroupMembershipsFromLibrary(libraryOwner.id, function(
        err,
        groupIds,
      ) {
        if (err) {
 ...
 * @api private
 */
var _getAllGroupMembershipsFromLibrary = function(
  principalId,
  callback,
  _groupIds,
  _nextToken,
  ...
  LibraryAPI.Index.list(
    ...
    function(err, entries, nextToken) {
      ...
      return _getAllGroupMembershipsFromLibrary(
        principalId,
        callback,
        _groupIds,
        nextToken,
 ...
 */
PrincipalsEmitter.when(PrincipalsConstants.events.CREATED_GROUP, function(
  ctx,
  group,
  memberChangeInfo,
  callback,
 ...
 * belongs
 */
PrincipalsEmitter.on(PrincipalsConstants.events.UPDATED_GROUP, function(
  ctx,
  updatedGroup,
  oldGroup,
) {
  _touchMembershipLibraries(updatedGroup, oldGroup.lastModified, null, function(
    err,
  ) {
    if (err) {
      log().warn(
 ...
 */
PrincipalsEmitter.when(PrincipalsConstants.events.LEFT_GROUP, function(
  ctx,
  group,
  memberChangeInfo,
  callback,
 ...
 */
PrincipalsEmitter.when(PrincipalsConstants.events.JOINED_GROUP, function(
  ctx,
  group,
  oldGroup,
  memberChangeInfo,
  ...
  _touchMembershipLibraries(
    group,
    oldGroup.lastModified,
    memberChangeInfo,
    function(err) {
      if (err) {
        log().warn(
...
PrincipalsEmitter.when(
  ...
  function(ctx, group, oldGroup, memberChangeInfo, opts, callback) {
 ...
 * @api private
 */
var _touchMembershipLibraries = function(
  group,
  oldLastModified,
  memberChangeInfo,
  callback,
) {
  var addedMemberIds = memberChangeInfo
    ? _.pluck(memberChangeInfo.members.added, 'id')
    : [];
  var removedMemberIds = memberChangeInfo
    ? _.pluck(memberChangeInfo.members.removed, 'id')
    : [];
  ...
  _getGroupAncestorsIncludingDeleted(group, function(err, ancestorGroups) {
    ...
    _insertMembershipsLibraries(groups, addedMemberIds, function(
      err,
      explodedInsertedPrincipals,
    ) {
      if (err) {
        return callback(err);
      }
      ...
      _removeMembershipsLibraries(removedMemberIds, groups, function(err) {
        ...
        if (oldLastModified) {
          return _updateMembershipsLibraries(
            changedGroup,
            explodedInsertedPrincipals,
            callback,
          );
 ...
 * @api private
 */
var _updateMembershipsLibraries = function(
  group,
  excludePrincipalIds,
  callback,
) {
  ...
  _getAllGroupChildren([group.id], excludePrincipalIds, function(
    err,
    allChildren,
  ) {
    if (err) {
...
var _getGroupAncestorsIncludingDeleted = function(group, callback) {
  ...
  AuthzAPI.getPrincipalMembershipsGraph(group.id, function(err, graph) {
    ...
    var membershipIds = _.chain(graph.getNodes())
      .pluck('id')
      .without(group.id)
      .value();
    ...
    PrincipalsDAO.getPrincipals(
      ...
      function(err, parentGroupsByGroupId) {
        if (err) {
          return callback(err);
        }
 ...
 * @api private
 */
var _getAllGroupChildren = function(
  principalIds,
  excludePrincipals,
  callback,
  _groupsToExplode,
  ...
  _groupsToExplode =
    _groupsToExplode || _.filter(principalIds, AuthzUtil.isGroupId);
 ...
 * @api private
 */
var _handleInvalidateMembershipsLibraries = function(
  group,
  membershipsGraph,
  membersGraph,
  callback,
