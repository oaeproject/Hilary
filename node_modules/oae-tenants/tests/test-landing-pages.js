        ...
        TestsUtil.setupMultiTenantPrivacyEntities(function(tenant) {
            ...
            ConfigTestUtil.updateConfigAndWait(globalAdminRestContext, tenant.tenant.alias, configUpdate, function() {
                ...
                RestAPI.Tenants.getLandingPage(anonymousRestContext, function(err, landingPage) {
                    ...
                    TestsUtil.generateTestUsers(tenant.adminRestContext, 2, function(err, users, simong, nico) {

                        // Set a user's locale to French
                        RestAPI.User.updateUser(simong.restContext, simong.user.id, {'locale': 'fr_FR'}, function(err) {
                            assert.ok(!err);

                            // Get the landing page information with the French user
                            RestAPI.Tenants.getLandingPage(simong.restContext, function(err, landingPage) {
                                assert.ok(!err);
                                ...
                                RestAPI.Tenants.getLandingPage(nico.restContext, function(err, landingPage) {
                                    assert.ok(!err);
