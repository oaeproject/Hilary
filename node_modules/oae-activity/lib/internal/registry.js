...
var registerActivityStreamType = (module.exports.registerActivityStreamType = function(
  ...
  if (activityStreams[activityStreamType]) {
    throw new Error(
      util.format(
        'Attempted to register duplicate activity stream type "%s"',
        activityStreamType,
      ),
    );
  }
...
var registerActivityType = (module.exports.registerActivityType = function(
  ...
  if (activityTypes[activityType]) {
    throw new Error(
      util.format(
        'Attempted to register duplicate activity type of type "%s"',
        activityType,
      ),
    );
  }
  ...
  _.each(options.streams, function(streamConfig, streamName) {
    if (!streamConfig || _.isEmpty(streamConfig.router)) {
      throw new Error(
        util.format('Missing or empty router for stream "%s"', streamName),
      );
    }
    ...
    _.each(options.streams[streamName].router, function(
      assocations,
      entityName,
    ) {
