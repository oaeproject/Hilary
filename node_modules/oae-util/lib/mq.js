...
var init = module.exports.init = function(mqConfig, callback) {
    ...
    if (!connection) {
        ...
        let arrayOfHostsToConnectTo = _.map([mqConfig.connection.host], eachHost => {
...
var declareExchange = module.exports.declareExchange = function(exchangeName, exchangeOptions, callback) {
    ...
    channel.assertExchange(exchangeName, exchangeOptions.type, exchangeOptions).then((exchange) => {
        ...
        log().error({'exchangeName': exchangeName, 'err': new Error('Unable to declare an exchange') });
    });
};
...
var declareQueue = module.exports.declareQueue = function(queueName, queueOptions, callback) {
    ...
    channel.assertQueue(queueName, queueOptions).then(queue => {
        ...
        log().error({ 'queueName': queueName, 'err': new Error('Unable to declare a queue') });
    });
};
...
var unsubscribeQueue = module.exports.unsubscribeQueue = function(queueName, callback) {
    ...
    channel.cancel(consumerTag).then((ok) => {
        ...
        log().error({ 'queueName': queueName, 'err': new Error('Unable to unsubscribe the queue') });
    });
};
...
var purge = module.exports.purge = function(queueName, callback) {
    ...
    channel.purgeQueue(queueName).then(data => {
        ...
        log().error({ 'queueName': queueName, 'err': new Error('Unable to purge queue') });
    });
};
