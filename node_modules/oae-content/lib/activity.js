...
ContentAPI.on(ContentConstants.events.CREATED_COMMENT, function(ctx, comment, content) {
    var millis = Date.now();
    var actorResource = new ActivityModel.ActivitySeedResource('user', ctx.user().id, {'user': ctx.user()});
    var objectResource = new ActivityModel.ActivitySeedResource('content-comment', comment.id, {'contentId': content.id, 'comment': comment});
...
ActivityAPI.registerActivityEntityProducer('content-comment', function(activityType, entityType, resource, callback) {
    ...
    ContentDAO.Content.getContent(contentId, function(err, content) {
        ...
        if (comment.replyTo) {
            ...
            MessageBoxAPI.getMessages(comment.messageBoxId, [comment.replyTo], null, function(err, parent) {
                if (err) {
                    return callback(err);
                }

                parent = parent[comment.replyTo];
...
ActivityAPI.registerActivityRouter('content-comment', function(activityType, entityType, entity, callback) {
    ...
    _routeContent(activityType, entityType, contentId, entity.comment.visibility, function(err, routes, propagation) {
        ...
        MessageBoxAPI.getRecentContributions(entity.comment.messageBoxId, null, null, function(err, recentContributions) {
            if (err) {
                return callback(err);
            ...
            // Get a unique list of the userIds that contributed to the comments of this content item
            var contributorIds = [];
            _.each(recentContributions, function(contribution) {
                contributorIds.push(contribution.userId);
            });
...
ActivityAPI.registerActivityEntityTransformer('content-comment', function(ctx, activityEntities, callback) {
    ...
    _.keys(activityEntities).forEach(function(activityId) {
        ...
        _.keys(activityEntities[activityId]).forEach(function(entityId) {
            ...
            transformedActivityEntities[activityId][entityId] = ContentUtil.transformPersistentCommentActivityEntity(ctx, entity.contentId, entity.comment, entity.commentContext);
        });
    });
    return callback(null, transformedActivityEntities);
});
