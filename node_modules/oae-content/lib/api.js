...
const createCollabSheet = module.exports.createCollabSheet = function(ctx, displayName, description, visibility, additionalMembers, folders, callback) {
    ...
    Ethercalc.createRoom(contentId, function(err, snapshot, roomId) {
        if (err) {
            return callback(err);
        }

        _createContent(ctx, contentId, revisionId, 'collabsheet', displayName, description, visibility, additionalMembers, folders, {'ethercalcSnapshot': snapshot, 'ethercalcRoomId': roomId}, {}, function(err, content, revision, memberChangeInfo) {
            if (err) {
                return callback(err);
            }

            content.ethercalcRoomId = roomId;
            content.ethercalcSnapshot = snapshot;
            ...
            PrincipalsDAO.getPrincipal(data.userId, function(err, principal) {
                ...
                Ethercalc.getHTML(roomId, function(err, html) {
                    ...
                    ContentDAO.Revisions.getRevision(contentObj.latestRevisionId, function(err, revision) {
                        ...
                        // Otherwise we create a new revision
                        const newRevisionId = _generateRevisionId(contentObj.id);
                        ContentDAO.Revisions.createRevision(newRevisionId, contentObj.id, data.userId, {'ethercalcHtml': html}, function(err, revision) {
                            if (err) {
    ...
    _canManage(ctx, contentId, function(err, contentObj) {
        ...
        ContentDAO.Revisions.getRevision(revisionId, function(err, revision) {
            ...
            ContentDAO.Revisions.createRevision(newRevisionId, contentId, ctx.user().id, revision, function(err, newRevision) {
                 ...
                 * to the Content CF.
                 */
                const blacklist = ['revisionId', 'contentId', 'createdBy', 'created', 'etherpadHtml', 'previewsId', 'downloadPath'];
