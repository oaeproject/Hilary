    ...
    _screenshot(ctx, path, 'large', function(err) {
        ...
        _screenshot(ctx, path, 'medium', function(err) {
            ...
            _screenshot(ctx, path, 'small', function(err) {
                ...
                _screenshot(ctx, path, 'thumbnail', function(err) {
                    if (err) {
                        return callback(err);
                    }
...
var _screenshot = function(ctx, path, size, callback) {
    ...
    proc.withSize(PreviewConstants.SIZES.VIDEO[size.toUpperCase()] + 'x?').takeScreenshots({'count': 1, filename: size}, ctx.baseDir, function(err, filenames) {
        if (err) {
            log().error(err);
            return callback(err);
        }
...
var _transcode = function(ctx, path, callback) {
    ...
    proc.toFormat('m4v')
        ...
        .withAudioChannels(1)
        .saveToFile(transcodePath, function(stdout, stderr) {
        log().info({
            'transcode': 'complete',
            'stdout': stdout,
            'stderr': stderr
        });
        ctx.addPreview(transcodePath, 'm4v');
        callback();
    });
};
