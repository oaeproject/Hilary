        ...
        return callback(null, createHasChildQuery(
            ...
            createFilteredQuery(
                ...
                filterOr(
                    ...
                    // Include all resources that has the current user or the user's *indirect* group membership as
                    // a direct member
                    filterTerms('direct_members', _.union([ctx.user().id], indirectGroupIds)),

                    // Additionally, include all resources that has the current user's *direct* group membership as
                    // a direct member. This is an optimization that avoids the requirement of sending the user's
                    // direct membership in the ElasticSearch query, only their indirect group membership is
                    // necessary to send by using the ElasticSearch Terms Lookup filter
