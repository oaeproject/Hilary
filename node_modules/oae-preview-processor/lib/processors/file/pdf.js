        ...
        fs.readdir(pagesDir, function(err, files) {
            ...
            fs.rename(baseMinCssPath, baseCssPath, function(err) {
                ...
                fs.rename(fancyMinCssPath, fancyCssPath, function(err) {
                    ...
                    // Combine the individual CSS files and scope their rules using LESS
                    var parent = '.' + ctx.contentId.replace(/:/g, '-') + '-' + ctx.revisionId.replace(/:/g, '-') + '-pdf2html { ' +
                                      '@import (less) "base.css"; ' +
                                      '@import (less) "fancy.css"; ' +
                                      '@import (less) "lines.css"; ' +
                                  '}';
                    var parser = less.Parser({ paths: [pagesDir] });
                    parser.parse(parent, function(err, tree) {
                        if (err) {
                        ...
                        fs.writeFile(combinedCssPath, tree.toCSS({
                            ...
                            // Keep the individual files for backwards compatibility
