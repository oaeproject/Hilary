 ...
 var _completeTransfer = function (ctx, originalEmail, code, targetEmail, targetUserId, transfer, callback) {
    ...
    TransferDAO.getAuthzRoles(transfer.originalUserId, function(err, authzRoles) {
        ...
        if (_.isEmpty(authzRoles)) {
            var err = {'code': 400, 'msg': 'The user has no roles to transfer'};
            return callback(err);
        }
        ...
        _transferRoles(ctx, authzRoles, targetUserId, transfer.originalUserId, function(err, newAuthzRoles) {
            ...
            TransferDAO.completeTransfer(code, transfer.originalUserId, originalEmail, targetEmail, targetUserId, function (err, recordedTransfer) {
                if (err) {
                    return callback(err);
                }
                if (!recordedTransfer) {
...
/**
 * This function have the objectif to restructure the variable memberHashes. The actual structure are not the 
 * good one the send emails. We want to send only one email per user and not one email per resource per user.
 *
