    ...
    getContent(ctx, contentId, function(err, contentObj) {
        ...
        PrincipalsUtil.getPrincipal(ctx, contentObj.createdBy, function(err, createdBy) {
            ...
            if (!ctx.user()) {
                ...
                checkRole(ctx, contentObj, ContentConstants.roles.MANAGER, function(err, isManager) {
 ...
 * @param  {Function(err, contentProfile)}  callback                    Standard callback function
 * @param  {Object}                         callback.err                Error object containing the error message
 * @param  {Content}                        callback.contentProfile     Retrieved content profile
 */
 ...
 * @param  {Object}                  additionalMembers  Object where the keys represent principal ids that need to be added to the content upon
 *                                                      creation and the values represent the role that principal will have. Possible values are
 *                                                      "viewer" and "manager"
 * @param  {Function(err, content)}  callback           Standard callback function
 * @param  {Object}                  callback.err       Error object containing the error message
 ...
 *                                                      creation and the values represent the role that principal will have. Possible values are
 *                                                      "viewer" and "manager"
 * @param  {Object}                  otherValues        JSON object where the keys represent other metadata values that need to be stored,
 *                                                      and the values represent the metadata values
 * @param  {Function(err, content)}  callback           Standard callback function
 * @param  {Object}                  callback.err       Error object containing the error message
 * @param  {Object}                  callback.content   JSON object containing the pool id of the created content
 */
...
var deleteContent = module.exports.deleteContent = function(ctx, contentId, callback) {
    ...
    canManage(ctx, contentId, function(err, canManage, contentObj) {
        ...
        _getAllContentMembers(contentId, function(err, members) {
            ...
            _setContentPermissions(ctx, contentObj, members, function(err) {
                if (err) {
                    return callback(err);
                }
...
var _shareContent = function(ctx, contentObj, principalIds, callback) {
    ...
    _getAllContentMembers(contentObj.contentId, function(err, members) {
        ...
        principalIds = _.difference(principalIds, _.keys(members));
...
var canManage = module.exports.canManage = function(ctx, contentId, callback) {
    ...
    // TODO: Return reversed content list
};
...
var _canManage = function(ctx, contentId, callback) {
    AuthzAPI.isAllowed(ctx.user().id, ContentConstants.roles.MANAGER, contentId, callback);
};
...
var _hasAccess = function(ctx, contentObj, callback) {
    ...
    // If the content is private
        ...
        //AuthzAPI.isAllowedAny(ctx.user().id, contentObj.contentId, callback);
        checkAccess(ctx, contentObj, callback);
    } else {
        callback({'code': 401, 'msg': "You don't have access to this piece of content. Please try logging in"});
    }
};
...
// TODO
var checkAccess = function(ctx, contentObj, callback) {
    // Direct check
    Cassandra.runQuery('SELECT ? FROM AuthzRoles USING CONSISTENCY QUORUM WHERE principalId = ?', [contentObj.contentId, ctx.user().id], function(err, rows) {
        if (err) {
            return callback(err);
        }

        if (rows[0].get(contentObj.contentId) && rows[0].get(contentObj.contentId).value) {
            callback(null, contentObj);
        } else {
            // Get the groups associated to the content
            //var limit = 1000;
            var columnPrefix = 'g:';
            var start = columnPrefix;
            var end = start+'|';
        
            //var cql = 'SELECT FIRST ' + limit + ' ? .. ? FROM ContentMembers USING CONSISTENCY QUORUM WHERE contentId = ?';
            var cql = 'SELECT ? .. ? FROM ContentMembers USING CONSISTENCY QUORUM WHERE contentId = ?';
            Cassandra.runQuery(cql, [start, end, contentObj.contentId], function(err, rows) {
                if (err) {
                    return callback(err);
                }
        
                var entries = Cassandra.rowToHash(rows[0]);
                if (_.keys(entries).length === 0) {
                    callback({'code': 401});
                } else {
                    
                    // Explode the group memberships
                    var finalGroupIds = _.keys(entries);
                    var explodeGroupMemberships = function(groupIds) {
                        var start = 'g:';
                        var end = start + '|';
                        var cql = 'SELECT ? .. ? FROM AuthzGroupMembers USING CONSISTENCY QUORUM WHERE groupId IN (?)';
                        Cassandra.runQuery(cql, [start, end, groupIds], function(err, rows) {
                            var newToInvestigate = [];
                            for (var i = 0; i < rows.length; i++) {
                                var groups = Cassandra.rowToHash(rows[i]);
                                for (var g in groups) {
                                    if (finalGroupIds.indexOf(g) === -1) {
                                        newToInvestigate.push(g);
                                        finalGroupIds.push(g);
                                    }
                                }
                            }
                            if (newToInvestigate.length === 0) {
                                
                                // Check if the user has any of these groups as a direct membership
                                var start = 'g:';
                                var end = start + '|';
                                Cassandra.runQuery('SELECT ? FROM AuthzRoles USING CONSISTENCY QUORUM WHERE principalId = ?', [finalGroupIds, ctx.user().id], function(err, rows) {
                                    if (err) {
                                        return callback(err);
                                    }
                                    if (rows[0].count > 0) {
                                ...
                                explodeGroupMemberships(newToInvestigate);
                            }
                        });
                    }
                    explodeGroupMemberships(_.keys(entries));
...
// TODO
var checkRole = function(ctx, contentObj, role, callback) {
    // Direct check
    Cassandra.runQuery('SELECT ? FROM AuthzRoles USING CONSISTENCY QUORUM WHERE principalId = ?', [contentObj.contentId, ctx.user().id], function(err, rows) {
        if (err) {
            return callback(err);
        }
        if (rows[0].get(contentObj.contentId) && rows[0].get(contentObj.contentId).value === role) {
            callback(null, true);
        } else {
            // Get the groups associated to the content
            //var limit = 1000;
            ...
            Cassandra.runQuery(cql, [start, end, contentObj.contentId], function(err, rows) {
                ...
                if (_.keys(entries).length === 0) {
                    ...
                    // Explode the group memberships
                    var finalGroupIds = entries;
                    var explodeGroupMemberships = function(groupIds) {
                        var start = 'g:';
                        var end = start + '|';
                        var cql = 'SELECT ? .. ? FROM AuthzGroupMembers USING CONSISTENCY QUORUM WHERE groupId IN (?)';
                        Cassandra.runQuery(cql, [start, end, _.keys(groupIds)], function(err, rows) {
                            if (err) {
                                callback(err);
                            }
                            var newToInvestigate = {};
                            for (var i = 0; i < rows.length; i++) {
                                ...
                                for (var g in groups) {
                                    if (!finalGroupIds[g]) {
                                        newToInvestigate[g] = true;
                                        finalGroupIds[g] = finalGroupIds[rows[i].key];
                                    }
                                }
                            }
                            if (_.keys(newToInvestigate).length === 0) {
                                
                                // Check if the user has any of these groups as a direct membership
                                var start = 'g:';
                                var end = start + '|';
                                Cassandra.runQuery('SELECT ? FROM AuthzRoles USING CONSISTENCY QUORUM WHERE principalId = ?', [_.keys(finalGroupIds), ctx.user().id], function(err, rows) {
                                    if (err) {
                                        return callback(err);
                                    }
                                    if (rows[0].count > 0) {
                                        var groups = Cassandra.rowToHash(rows[0]);
                                        for (var g in groups) {
                                            if (groups[g] && finalGroupIds[g] === role) {
                    ...
                    explodeGroupMemberships(entries);
...
var _setContentPermissions = function(ctx, contentObj, newPermissions, callback) {
    ...
    PrincipalsUtil.getPrincipals(ctx, _.keys(newPermissions), function(err, principals) {
        ...
        // access
        AuthzAPI.applyRoleChanges(contentObj.contentId, newPermissions, function(err) {
            if (err) {
                return callback(err);
            }
 ...
 * @param  {String}                     start               Determines the point at which content members are returned for paging purposes.
...
var getContentMembers = module.exports.getContentMembers = function(ctx, contentId, start, limit, callback) {
    ...
    getContent(ctx, contentId, function(err, contentObj) {
        ...
        Cassandra.runQuery("SELECT first " + paging.limit + " ?..\'\' FROM ContentMembers USING CONSISTENCY QUORUM WHERE contentId = ?", [paging.start, contentId], function (err, rows) {
            if (err) {
                return callback(err);
            }

            var tempMembers = {};
            for (var r = 0; r < rows[0].count; r++) {
                var name = rows[0][r].name;
                if (name !== 'contentId') {
                    tempMembers[name] = rows[0][r].value;
                }
            }

            // Get the basic profiles for all of these principals
            PrincipalsUtil.getPrincipals(ctx, _.keys(tempMembers), function(err, memberProfiles) {
                if (err) {
                    return callback(err);
                }
                
                // Merge profiles and roles into one object
                var members = [];
                for (var member in tempMembers) {
                    if (tempMembers.hasOwnProperty(member)) {
                        members.push({
                            'role': tempMembers[member],
...
var _getAllContentMembers = function(contentId, callback) {
    Cassandra.runQuery("SELECT * FROM ContentMembers USING CONSISTENCY QUORUM WHERE contentId = ?", [contentId], function (err, rows) {
        if (err) {
            return callback(err);
        }
 ...
 *                                               e.g. {'name': 'New content name', 'description': 'New content description', 'visibility': 'private'}
 * @param  {Function(err)}   callback            Standard callback function
 * @param  {Object}          callback.err        Error object containing the error message
 */
 ...
 * @param  {Object}          metadata            Object where the keys represent the profile fields that need to be updated and the
 *                                               values represent the new values for those profile fields.
 * @param  {Function(err)}   callback            Standard callback function
 * @param  {Object}          callback.err        Error object containing the error message
 */
 ...
 * @param  {Array<String>}              removedMembers      Array of principal ids that contains all principal that have been
 *                                                          removed as a member of the content, and therefore their libraries
 *                                                          should be updated so the content is removed.
 * @param  {Function(err, members)}     callback            Standard callback function
 * @param  {Object}                     callback.err        Error object containing the error message
 */
...
var getLibraryItems = module.exports.getLibraryItems = function(ctx, principalId, start, limit, callback) {
    ...
    // Check if we are looking at a group library
        ...
        if (!ctx.user()) {
            ...
            AuthzAPI.isMemberOf(ctx.user().id, principalId, function(err, isMemberOf) {
            //checkGroupMember(ctx, principalId, function(err, isMemberOf) {
                if (err) {
                    return callback(err);
                }
 ...
 * @param  {String}                  principalId       The principal id of the library we're requesting. This is the processed library
 *                                                     principal id, meaning that it will be the principal id when direct access is allowed,
 *                                                     principalId:loggedin if the user is logged in or principalId:public for anonymous users
 * @param  {String}                  start             Determines the point at which content items are returned for paging purposed.
 *                                                     If not provided, the first x elements will be returned
 * @param  {Integer}                 limit             Number of items to return. Will default to 10 if not provided
 * @param  {Function(err, content)}  callback          Standard callback function
