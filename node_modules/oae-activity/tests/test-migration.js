...
describe('Activity Migration', function() {
    ...
    var setUp = function(userId, callback) {
        ...
        activities[userId] = {};
        for (var i = 0; i < 5; i++) {
            var oldActivityId = Date.now() + ':' + i;
            activities[userId][oldActivityId] = _mockActivity(oldActivityId, userId);
        }

        // Generate 5 activitities under the `u:cam:foo#activity` activity stream
        activities[userId + '#activity'] = {};
        for (i = 5; i < 10; i++) {
            var newActivityId = Date.now() + ':' + i;
            activities[userId + '#activity'][newActivityId] = _mockActivity(newActivityId, userId);
        }
