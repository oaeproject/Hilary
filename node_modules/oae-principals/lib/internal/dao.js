...
var createGroup = module.exports.createGroup = function(groupId, tenantAlias, displayName, description, visibility, joinable, createdBy, callback) {
    ...
    var parameters = [groupId, tenantAlias, displayName, description, visibility, joinable, '' + lastModified.getTime(), createdBy, lastModified];
...
var getPrincipals = module.exports.getPrincipals = function(principalIds, fields, callback) {
    ...
    if (fields) {
        ...
        query = 'SELECT ' + columns.join(',') + ' FROM "Principals"';
    } else {
        query = 'SELECT * FROM "Principals"';
    }

    Cassandra.runInQuery(query, parameters, 'principalId', principalIds, function(err, rows) {
        if (err) {
            return callback(err);
        }
...
var deletePrincipal = module.exports.deletePrincipal = function(principalId, callback) {
    ...
    AuthzDelete.isDeleted([principalId], function(err, wasDeleted) {
        ...
        AuthzDelete.setDeleted(principalId, function(err) {
            ...
            _updatePrincipal(principalId, {'deleted': new Date()}, function(err) {
                if (err) {
...
var getEmailToken = module.exports.getEmailToken = function(userId, callback) {
    Cassandra.runQuery('SELECT * FROM "PrincipalsEmailToken" WHERE "principalId" = ?', [userId], function(err, rows) {
        ...
        var token = _.first(rows)['token'];
        var email = _.first(rows)['email'];
...
var getUserIdsByEmails = module.exports.getUserIdsByEmails = function(emails, callback) {
    Cassandra.runInQuery('SELECT * FROM "PrincipalsByEmail"', null, 'email', emails, function(err, rows) {
        if (err) {
            return callback(err);
        }
...
var _getPrincipalFromCassandra = function(principalId, callback) {
    Cassandra.runQuery('SELECT * FROM "Principals" WHERE "principalId" = ?', [principalId], function (err, rows) {
        ...
        if (isUser(principalId)) {
            ...
            // is safe
            _updateCachedUser(principalId, rows[0]);
        }
...
var _getPrincipalFromRow = function(row) {
    ...
    return isGroup(row.principalId) ? _getGroupFromRow(row) : _getUserFromRow(row);
};
...
var _getGroupFromRow = function(row) {
    // Format the timestamp columns as millis since the epoch
    _timestampToMillis(row, 'created');
    _timestampToMillis(row, 'deleted');

    // If the `deleted` column is empty, we take it out of the row entirely
    if (!row.deleted) {
        delete row.deleted;
    }

    return new Group(row.tenantAlias, row.principalId, row.displayName, row);
};
...
var _getUserFromRow = function(row) {
    _transformUserFieldTypes(row);

    return _hashToUser(row);
};
...
var _transformUserFieldTypes = function(hash) {
    ...
    // If the `deleted` column is empty, we take it out of the hash entirely
    if (!hash.deleted) {
        delete hash.deleted;
    }
};
