...
var setSection = module.exports.setSection = function(context, principal_id, sectionName, sectionVisibility, sectionData, overwrite, callback) {
    ...
    getSection(context, principal_id, sectionName, function(err, oldProfileSection) {
        ...
        // Only the section owner or admins can update a profile section.
        context.isAdmin(function(err, isAdmin) {
            if (err) {
                return callback(err);
            }
            if (context.user().id === principal_id || isAdmin) {
...
var getSection = module.exports.getSection = function(context, principal_id, sectionName, callback) {
    ...
    Cassandra.runQuery("SELECT ?, ? from Profiles WHERE principal_id = ?", [columnVisibilityName, columnDataName, principal_id], function(err, rows) {
        ...
        if (rows.rowCount() !== 1 || rows[0]._colCount === 0) {
            return callback({'code': 404, 'msg': 'Section not found'});
        }

        var visibility = rows[0].colHash[columnVisibilityName];
        var data = rows[0].colHash[columnDataName];
        data = JSON.parse(data);
        var profileSection = new ProfileSection(principal_id, sectionName, data, visibility);
...
var getAllSections = module.exports.getAllSections = function(context, principal_id, callback) {
    Cassandra.runQuery("SELECT * from Profiles WHERE principal_id = ?", [principal_id], function(err, rows) {
        ...
        if (rows.rowCount() !== 1) {
            return callback({'code': 404, 'msg': 'No sections found for this principal'});
        }
        ...
        var allSections = [];
        var checkAccess = function(profileSection) {
            _hasAccess(context, profileSection, function(err, hasAccess) {
                retrievedSections++;
                if (!err && hasAccess) {
                ...
                if (retrievedSections === Math.floor(rows[0].cols.length / 2)) {
                    callback(false, allSections);
                }
            });
        };

        // Because of the ordering of columns we'll get the following data
        // principal_id, xxx#data, xxx#visibility, yyy#data, yyy#visibility ...
        for (var i = 1; i < rows[0].cols.length; i+=2) {
            var sectionName = rows[0].cols[i].name.split('#')[0];
            var sectionData = rows[0].cols[i].value;
            var visibility = rows[0].cols[i+1].value;
            var profileSection = new ProfileSection(principal_id, sectionName, sectionData, visibility);
            checkAccess(profileSection);
        }
    });
};
...
var _hasAccess = function(context, profileSection, callback) {
    ...
    context.isAdmin(function(err, isAdmin) {
        if (isAdmin || context.user().id === profileSection.principal_id || profileSection.visibility !== Constants.visibility.PRIVATE) {
