    ...
    it('verify users need to accept the Terms and Conditions when creating an account', function(callback) {
        ...
        ConfigTestUtil.updateConfigAndWait(camAdminRestContext, null, {'oae-principals/recaptcha/enabled': false}, function(err) {
            ...
            enableAndSetTC(camAdminRestContext, 'default', 'legalese', true, function() {
                ...
                RestAPI.User.createUser(anonymousCamRestContext, username, 'password', 'Test User', {'email': email}, function(err, userObj) {
                    assert.equal(err.code, 400);
                    RestAPI.User.createUser(anonymousCamRestContext, username, 'password', 'Test User', {'email': email, 'acceptedTC': false}, function(err, userObj) {
                        assert.equal(err.code, 400);
                        RestAPI.User.createUser(anonymousCamRestContext, username, 'password', 'Test User', {'email': email, 'acceptedTC': 'wrong'}, function(err, userObj) {
                            assert.equal(err.code, 400);

                            RestAPI.User.createUser(anonymousCamRestContext, username, 'password', 'Test User', {'email': email, 'acceptedTC': true}, function(err, userObj) {
                                assert.ok(!err);
