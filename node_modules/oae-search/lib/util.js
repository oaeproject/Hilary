 ...
 *
 * @param   {String}    field   The field the full-text query should be applied to
...
var createQueryStringQuery = module.exports.createQueryStringQuery = function(q) {
    if (!q || q === SearchConstants.query.ALL) {
        ...
        // Build a query that will do a full-text search on the title and general field, giving favour to matches on the title
        return {
            'multi_match': {
                'fields': ['title^2.0', 'general^0.5'],
                'query': q
            }
        };
    }
...
var _orderDocs = function(docsHash, orderSpec) {
    ...
    docs.sort(function(one, other) {
        return orderSpec[one.id] - orderSpec[other.id];
    });
    return docs;
};
