...
PrincipalsAPI.on(PrincipalsConstants.events.UPDATED_GROUP_MEMBERS, function(ctx, group, oldGroup, memberChangeInfo, opts) {
    ...
    _.each(addedMemberIds, function(memberId) {
        var objectResourceType = PrincipalsUtil.isGroup(memberId) ? 'group' : 'user';
        var objectResource = new ActivityModel.ActivitySeedResource(objectResourceType, memberId);
        var activitySeed = new ActivityModel.ActivitySeed(PrincipalsConstants.activity.ACTIVITY_GROUP_ADD_MEMBER, millis, ActivityConstants.verbs.ADD, actorResource, objectResource, targetResource);
        ActivityAPI.postActivity(ctx, activitySeed);
    });
    ...
    _.each(updatedMemberIds, function(memberId) {
        var objectResourceType = PrincipalsUtil.isGroup(memberId) ? 'group' : 'user';
        var objectResource = new ActivityModel.ActivitySeedResource(objectResourceType, memberId);
        var activitySeed = new ActivityModel.ActivitySeed(PrincipalsConstants.activity.ACTIVITY_GROUP_UPDATE_MEMBER_ROLE, millis, ActivityConstants.verbs.UPDATE, actorResource, objectResource, targetResource);
        ActivityAPI.postActivity(ctx, activitySeed);
    });
});
...
/*!
 * Fire the request-group-join activity when someone whants to join a group
 */
PrincipalsAPI.on(PrincipalsConstants.events.REQUEST_TO_JOIN_GROUP, function(ctx, group, oldGroup, memberChangeInfo) {
    var millis = Date.now();
    var actorResource = new ActivityModel.ActivitySeedResource('user', ctx.user().id, {'user': ctx.user()});
    var objectResource = new ActivityModel.ActivitySeedResource('group', group.id, {'group': group});
    ActivityAPI.postActivity(ctx, new ActivityModel.ActivitySeed(PrincipalsConstants.activity.ACTIVITY_REQUEST_TO_JOIN_GROUP, millis, ActivityConstants.verbs.REQUEST, actorResource, objectResource));
});
 ...
 * @see ActivityAPI#registerActivityEntityType
 */
var _userInternalTransformer = function(ctx, activityEntities, callback) {
    var transformedActivityEntities = {};
    _.keys(activityEntities).forEach(function(activityId) {
        transformedActivityEntities[activityId] = transformedActivityEntities[activityId] || {};
        _.keys(activityEntities[activityId]).forEach(function(entityId) {
 ...
 * @see ActivityAPI#registerActivityEntityType
 */
var _groupInternalTransformer = function(ctx, activityEntities, callback) {
    var transformedActivityEntities = {};
    _.keys(activityEntities).forEach(function(activityId) {
        transformedActivityEntities[activityId] = transformedActivityEntities[activityId] || {};
        _.keys(activityEntities[activityId]).forEach(function(entityId) {
...
ActivityAPI.registerActivityEntityType('group', {
    ...
    'propagation': function(associationsCtx, entity, callback) {
        ActivityUtil.getStandardResourcePropagation(entity.group.visibility, entity.group.joinable, function(err, propagation) {
            if (err) {
                return callback(err);
            }
