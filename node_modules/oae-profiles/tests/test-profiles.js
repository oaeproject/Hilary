...
var log = require('oae-logger').logger('test-profiles');
...
describe('Profiles', function() {
    ...
    describe('Profile section visibility', function() {
        ...
        it('verify public profile section is accessible from external tenant', function(callback) {
            var tenantAliasB = TestsUtil.generateTestUserId('tenant');
            RestAPI.Tenant.createTenant(globalAdminRestContext, tenantAliasB, tenantAliasB, tenantAliasB, function(err) {
                assert.ok(!err);

                TestsUtil.generateTestUsers(TestsUtil.createTenantRestContext(tenantAliasB), 1, function(err, userB) {
                    assert.ok(!err);
        ...
        it('verify loggedin profile section is not accessible when getting all sections', function(callback) {
            var tenantAliasB = TestsUtil.generateTestUserId('tenant');
            RestAPI.Tenant.createTenant(globalAdminRestContext, tenantAliasB, tenantAliasB, tenantAliasB, function(err) {
                assert.ok(!err);
                    ...
                    TestsUtil.generateTestUsers(camAdminRestContext, 1, function(err, userA) {
                        ...
                        RestAPI.Profile.setSection(userB.restContext, userB.user.id, 'aboutme', 'loggedin', {'aboutme': 'awesome guy'}, null, function(err) {
                            ...
                            RestAPI.Profile.setSection(userB.restContext, userB.user.id, 'publications', 'public', {'publications': 'the art of war'}, null, function(err) {
                                ...
                                RestAPI.Profile.getAllSections(userA.restContext, userB.user.id, function(err, sections) {
                                    assert.ok(!err);
                                    assert.ok(!sections['aboutme']);
                                    assert.ok(sections['publications']);
                                    callback();
                                });
                            });
                        });
