...
var transferDataToArchive = module.exports.transferDataToArchive = function (ctx, user, archiveUsers, callback) {
    ...
    async.eachSeries(types, function iterator(type, callback) {
        ...
        // Delete/update roles
        _transferRight(ctx, user, archiveUsers, listEmail, listIdElement, type, function(err, listEmail, listIdElement) {
            if (err) {
                return callback(err);
            }
            return callback();
        ...
        _addToArchive(archiveUsers.archiveId, user.id, listIdElement, function(err) {
            ...
            _deleteFollowers(ctx, user, function(err) {
                ...
                _deleteFollowing(ctx, user, function(err) {
                    ...
                    // Send all emails
                    _sendEmail(ctx, listEmail, archiveUsers, function(err, listEmail) {
                        if (err) {
                            return callback(err);
                        }
...
var _parseData = function(data, idUserDeleted, callback) {
    ...
    listElementByMember.users.push({'contentWillBeDeleted': [], 'userJustLeaving': [], 'profile': data[0].memberList[0].profile});
    
    var exist = false;

    // For each member/editor/... of a resource
    _.each(data, function(d, i, arrI) {

        // There is a member list
        _.each(d.memberList, function(member, k, arrK) {

            exist = false;

            // Create another list to sort by user the list of resource
            _.each(listElementByMember.users, function(memberNewList, j, arrJ) { 
                if (member.profile.id !== idUserDeleted) {

                    // Add resource to user 
                    if ((memberNewList.profile.id === member.profile.id)) {
                        exist = true;
                        if (d.action === true && (memberNewList.contentWillBeDeleted.indexOf(d.resource) === -1)) {
                            memberNewList.contentWillBeDeleted.push(d.resource);
                        } else if (d.action === false && (memberNewList.userJustLeaving.indexOf(d.resource) === -1)) {
                            memberNewList.userJustLeaving.push(d.resource);
                        }
                        // If end loop, return callback
                        if (arrI.length === i+1) {
                    ...
                    else if ((arrJ.length === j+1) && (exist === false)) {
                        listElementByMember.users.push({'contentWillBeDeleted': [], 'userJustLeaving': [], 'profile': member.profile});
                        if (d.action === true) {
                            arrJ[j+1].contentWillBeDeleted.push(d.resource);
                        } else if (d.action === false) {
                            arrJ[j+1].userJustLeaving.push(d.resource);
                        }
                        // If end loop, return callback
                        if (arrI.length === i+1) {
                            return callback(null, listElementByMember);
 ...
 * @param  {Object}     callback.err    An error that occurred, if any
 */
var _sendEmail = function(ctx, data, archiveUser, callback) {
    ...
    PrincipalsDAO.getPrincipal(archiveUser.archiveId, function(err, archiveUser) {
        ...
        _parseData(data, userDeleted.id, function(err, parseData) {
            ...
            async.eachSeries(parseData.users, function(user, innerCallback) {
 ...
 * @api private
 */
var _transferRight = function(ctx, user, archiveUser, listEmail, listIdElement, type, callback) {
    ...
    _getLibrary(ctx, user.id, type, function(err, elements) {
        ...
        // Recursive loop
        _transferRightLoop(elements, ctx, user, archiveUser, listEmail, listIdElement, type, function(err, listEmail, listIdElement) {
            if (err) {
                return callback(err);
            }
            return callback(null, listEmail, listIdElement);
...
var _transferRightLoop = function(hashes, ctx, user, archiveUser, listEmail, listIdElement, type, callback, previousElement) {

    // Delete all his right on the previous element and remove it from library
    if (previousElement) {
        _removeFromLibrary(ctx, user.id, previousElement, type, function(err) {
            if (err) {
                return callback(err);
            }
        });
    ...
    // Return when there are no more element in the array
    if (!hashes || _.isEmpty(hashes)) {
        return callback(null, listEmail, listIdElement);
    }

    var element = hashes.shift();
                ...
                if (isManager) {
                    ...
                    if (_.isEmpty(newMemberList)) { 
                        ...
                        _updateRoles(ctx, element, archiveUser, type, function(err) {
                            ...
                            return _transferRightLoop(hashes, ctx, user, archiveUser, listEmail, listIdElement, type, callback, element);
                        });
                    ...
                    // We will just notify the members that the user will remove his account
                    listEmail.push({'userDeleted': user, 'resource': element, 'memberList': memberList, 'action': false});
                    // Recursive step inside the callback
                    return _transferRightLoop(hashes, ctx, user, archiveUser, listEmail, listIdElement, type, callback, element);
                }
            });
 ...
 * @param  {Context}    ctx                 Standard context object containing the current user and the current tenant
 * @param  {String}     userId              The user id 
 * @param  {String}     previousElement     Element to remove from library
 * @param  {String}     type                Type can be : 'content', 'folder', 'discussion', 'meeting' or 'group'
 * @param  {Function}   callback            Standard callback function
 * @param  {Object}     callback.err        An error that occured, if any
 * @api private
 */
var _removeFromLibrary = function(ctx, userId, previousElement, type, callback) {
    if (!previousElement) {
        return callback();
    }
    ...
    switch (type) {
        case 'content':
            ContentAPI.removeContentFromLibrary(ctx, userId, previousElement.id, function(err) {
                if (err) {
                    return callback(err);
                }
                return callback();
        ...
        case 'folder':
            FolderAPI.removeFolderFromLibrary(ctx, userId, previousElement.id, function(err) {
                if (err) {
                    return callback(err);
                }
                return callback();
        ...
        case 'discussion':
            DiscussionAPI.removeDiscussionFromLibrary(ctx, userId, previousElement.id, function(err) {
                if (err) {
                    return callback(err);
                }
                return callback();
        ...
        case 'meeting':
            MeetingsAPI.Meetings.removeMeetingFromLibrary(ctx, userId, previousElement.id, function(err) {
                if (err) {
                    return callback(err);
                }
                return callback();
        ...
        case 'group':
            ...
            ResourceActions.setRoles(ctx, previousElement, changes, function(err, memberChangeInfo) {
                if (err) {
                    return callback(err)
                } 

                PrincipalsEmitter.emit(PrincipalsConstants.events.LEFT_GROUP, ctx, previousElement, memberChangeInfo, function(errs) {
                    if (errs) {
