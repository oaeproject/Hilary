...
module.exports = function(tenant) {
    ...
    tenant.server.configure(function() {
        ...
        tenant.server.use(express.session({ secret: 'this secret is so secret that its totally secret', store: new CassandraStore()}));
    ...
    tenant.passport.deserializeUser(function(principal_id, done) {
        principalAPI.getUser(principal_id, function (err, user) {
            done(err, user);
        });
    });
    ...
    tenant.passport.use(new LocalStrategy({'passReqToCallback': true},
        function(req, username, password, done) {
            principalAPI.checkPassword(req.tenant, username, password, function(err, user) {
    ...
    tenant.server.post('/api/auth/login', tenant.passport.authenticate('local'), function(req, res) {
        if (!req.user) {
            return res.send(401);
        }
        principalAPI.getUser(req.user.id, function(err, user) {
            if (!err) {
    ...
    tenant.server.get('/api/auth/logout', function(req, res){
        req.logOut();
        res.redirect('/');
    });
    ...
    tenant.server.get('/api/auth/google/return',
        tenant.passport.authenticate(googleStrategy.name, { 'successRedirect': '/api/auth/me',
                                                            'failureRedirect': '/'  })
    );
    ...
    tenant.server.get('/api/auth/twitter/callback',
        tenant.passport.authenticate(twitterStrategy.name, { 'successRedirect': '/api/auth/me',
                                                             'failureRedirect': '/' })
    );
    ...
    tenant.server.get('/api/auth/facebook/callback',
        tenant.passport.authenticate(facebookStrategy.name, { 'successRedirect': '/api/auth/me',
                                                              'failureRedirect': '/' })
    );
    ...
    tenant.server.get('/api/auth/me', function(req, res) {
        if (req.user) {
            res.send(req.user);
        }
        else {
