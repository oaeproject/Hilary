 ...
 */
FoldersAPI.on(FoldersConstants.events.CREATED_FOLDER, function(
  ctx,
  folder,
  memberChangeInfo,
) {
  ...
  if (extraMembers.length === 1) {
    var targetResourceType = PrincipalsUtil.isGroup(extraMembers[0])
      ? 'group'
      : 'user';
    targetResource = new ActivityModel.ActivitySeedResource(
      targetResourceType,
      extraMembers[0],
    );
  }
 ...
 */
FoldersAPI.on(FoldersConstants.events.ADDED_CONTENT_ITEMS, function(
  ctx,
  actionContext,
  folder,
  contentItems,
  ...
  if (actionContext !== 'content-create') {
    ...
    var targetResource = new ActivityModel.ActivitySeedResource(
      'folder',
      folder.id,
      { folder: folder },
    );
 ...
 */
FoldersAPI.on(FoldersConstants.events.UPDATED_FOLDER, function(
  ctx,
  oldFolder,
  updatedFolder,
) {
 ...
 */
FoldersAPI.on(FoldersConstants.events.CREATED_COMMENT, function(
  ctx,
  message,
  folder,
) {
  ...
  var objectResource = new ActivityModel.ActivitySeedResource(
    'folder-comment',
    message.id,
    { folderId: folder.id, message: message },
  );
  var targetResource = new ActivityModel.ActivitySeedResource(
    'folder',
    folder.id,
...
FoldersAPI.on(FoldersConstants.events.UPDATED_FOLDER_MEMBERS, function(
  ctx,
  folder,
  memberChangeInfo,
  opts,
  ...
  var folderResource = new ActivityModel.ActivitySeedResource(
    'folder',
    folder.id,
    { folder: folder },
  );
  ...
  _.each(addedMemberIds, function(memberId) {
    if (memberId === ctx.user().id) {
      ...
      var principalResourceType = PrincipalsUtil.isGroup(memberId)
        ? 'group'
        : 'user';
      var principalResource = new ActivityModel.ActivitySeedResource(
...
var _folderProducer = function(resource, callback) {
  var folder =
    resource.resourceData && resource.resourceData.folder
      ? resource.resourceData.folder
      : null;
...
var _folderCommentProducer = function(resource, callback) {
  ...
  FoldersDAO.getFolder(folderId, function(err, folder) {
    ...
    MessageBoxUtil.createPersistentMessageActivityEntity(message, function(
      err,
      entity,
    ) {
      if (err) {
...
var _folderCommentTransformer = function(ctx, activityEntities, callback) {
  ...
  _.keys(activityEntities).forEach(function(activityId) {
    ...
    _.keys(activityEntities[activityId]).forEach(function(entityId) {
      ...
      var profilePath =
        '/folder/' + resource.tenantAlias + '/' + resource.resourceId;
 ...
 * @see ActivityAPI#registerActivityEntityType
 */
var _folderCommentInternalTransformer = function(
  ctx,
  activityEntities,
  callback,
) {
  ...
  _.keys(activityEntities).forEach(function(activityId) {
    ...
    _.keys(activityEntities[activityId]).forEach(function(entityId) {
      ...
      transformedActivityEntities[activityId][
        entityId
      ] = MessageBoxUtil.transformPersistentMessageActivityEntityToInternal(
        ctx,
        entity.message,
      );
    });
 ...
 */
ActivityAPI.registerActivityEntityAssociation('folder', 'self', function(
  associationsCtx,
  entity,
  callback,
) {
 ...
 */
ActivityAPI.registerActivityEntityAssociation('folder', 'members', function(
  associationsCtx,
  entity,
  callback,
) {
 ...
 */
ActivityAPI.registerActivityEntityAssociation('folder', 'managers', function(
  associationsCtx,
  entity,
  callback,
) {
 ...
 */
var _transformPersistentFolderActivityEntity = function(
  ctx,
  entity,
  foldersById,
) {
  ...
  if (folder.previews && folder.previews.thumbnailUri) {
    var thumbnailUrl = ContentUtil.getSignedDownloadUrl(
      ctx,
  ...
  if (folder.previews && folder.previews.wideUri) {
    var wideUrl = ContentUtil.getSignedDownloadUrl(
      ctx,
      folder.previews.wideUri,
      -1,
    );
    opts.ext[
      ContentConstants.activity.PROP_OAE_WIDE_IMAGE
