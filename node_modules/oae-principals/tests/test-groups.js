...
describe('Groups', function() {
                    ...
                    function(err) {
                      ...
                      RestAPI.Config.getTenantConfig(
                        ...
                        function(err, config) {
                          ...
                          assert.ok(config);
                          assert.equal(
                            config['oae-principals']['group']['joinable'],
                            'no',
                          );
                          return callback();
                        },
  ...
  describe('Get group', function() {
        ...
        function(err, createdGroup) {
          ...
          PrincipalsTestUtil.uploadAndCropPicture(
            ...
            function() {
              ...
              RestAPI.Group.getGroup(johnRestContext, createdGroup.id, function(
                err,
                fetchedGroup,
              ) {
                assert.ok(!err);
                ...
                assert.equal(
                  fetchedGroup.createdBy.id.substr(0, 10),
                  'u:camtest:',
                );
                assert.equal(
                      ...
                      function(err, fetchedGroup) {
                        ...
                        assert.equal(
                          ...
                          '/group/' +
                            createdGroup.tenant.alias +
                            '/' +
                            AuthzUtil.getResourceFromId(createdGroup.id)
                              .resourceId,
                        );
                        assert.ok(_.isObject(createdGroup.tenant));
                        assert.equal(_.keys(fetchedGroup.tenant).length, 3);
    ...
    it('verify non existing group', function(callback) {
      ...
      RestAPI.Group.getGroup(johnRestContext, 'totally-unknown', function(
        err,
        groupData,
      ) {
        assert.ok(err);
      ...
      TestsUtil.generateTestUsers(camAdminRestContext, 3, function(
        ...
        RestAPI.Group.createGroup(
          ...
          function(err, newGroup) {
            ...
            RestAPI.Group.getGroup(johnRestContext, newGroup.id, function(
              err,
              groupData,
            ) {
              assert.ok(!err);
              ...
              RestAPI.Group.getGroup(jack.restContext, newGroup.id, function(
                err,
                groupData,
              ) {
                assert.ok(!err);
                ...
                RestAPI.Group.getGroup(jane.restContext, newGroup.id, function(
                  err,
                  groupData,
                ) {
                  assert.ok(!err);
                  ...
                  RestAPI.Group.getGroup(joe.restContext, newGroup.id, function(
                    err,
                    groupData,
                  ) {
                    assert.ok(!err);
        ...
        RestAPI.Group.createGroup(
          ...
          function(err, groupOne) {
            ...
            RestAPI.Group.createGroup(
              ...
              [],
              [jane.user.id],
              function(err, groupTwo) {
                assert.ok(!err);
                // Get the group profile
                RestAPI.Group.getGroup(jane.restContext, groupOne.id, function(
                  err,
                  fetchedGroup,
                ) {
                  assert.ok(!err);
                        ...
                        function(err, fetchedGroup) {
                          ...
                          RestAPI.User.getRecentlyVisitedGroups(
                            ...
                            function(err, recentGroups) {
                              assert.ok(!err);
                              assert.equal(
                                recentGroups.results[0].id,
                                groupTwo.id,
                              );
                              assert.equal(
                                recentGroups.results[1].id,
                                groupOne.id,
                              );
                              return callback();
                            },
  ...
  describe('Update group', function() {
    ...
    it('verify updating a group with no parameters fails', function(callback) {
      RestAPI.Group.createGroup(
        ...
        function(err, newGroup) {
          assert.ok(!err);
          RestAPI.Group.updateGroup(johnRestContext, newGroup.id, {}, function(
            err,
            updatedGroup,
          ) {
            assert.equal(err.code, 400);
          ...
          RestAPI.Group.updateGroup(
            ...
            function(err) {
              ...
              RestAPI.Group.getGroup(johnRestContext, createdGroup.id, function(
                ...
                assert.ok(!err);
                assert.strictEqual(
                  createdGroup.deleted,
                  afterUpdateGroup.deleted,
                );
                return callback();
              });
            },
              ...
              function(err, updatedGroup) {
                ...
                RestAPI.Group.updateGroup(
                  ...
                  function(err, updatedGroup) {
                    ...
                    RestAPI.Group.updateGroup(
                      TestsUtil.createTenantRestContext(
                        global.oaeTests.tenants.cam.host,
                      ),
                      newGroup.id,
                      { visibility: 'public' },
  ...
  describe('Set group members', function() {
              ...
              function(err) {
                ...
                RestAPI.Group.getGroupMembers(
                  ...
                  function(err, members) {
                    ...
                    assert.equal(
                      hash[johnRestContext.user.id][0].role,
                      'manager',
                    );
                    assert.equal(hash[jack.user.id][0].role, 'member');
                    assert.equal(hash[jane.user.id][0].role, 'manager');
                          ...
                          function(err) {
                            ...
                            RestAPI.Group.getGroupMembers(
                              ...
                              function(err, members) {
                                ...
                                assert.equal(members.results.length, 3);
                                // Morph results to hash for easy access.
                                var hash = _.groupBy(members.results, function(
                                  member,
                                ) {
                                  return member.profile.id;
                                });
                                ...
                                assert.equal(
                                  hash[jack.user.id][0].role,
                                  'member',
                                );
                                assert.equal(
                                  hash[jane.user.id][0].role,
                                  'member',
                                );
    ...
    it('verify add members indirect access', function(callback) {
      TestsUtil.generateTestUsers(camAdminRestContext, 2, function(
        ...
        RestAPI.Group.createGroup(
          ...
          [],
          [],
          function(err, managedByCambridge) {
            assert.ok(!err);
              ...
              function(err) {
                ...
                RestAPI.Group.getGroupMembers(
                  ...
                  function(err, members) {
                    ...
                    assert.equal(
                      members.results[0].profile.id,
                      johnRestContext.user.id,
                    );
                    return callback();
                  },
                );
            ...
            RestAPI.Group.getGroupMembers(
              ...
              function(err, members) {
                ...
                TestsUtil.createTenantWithAdmin(
                  tenantAliasB,
                  tenantHost,
                  function(err, tenantB, adminRestCtxB) {
                    assert.ok(!err);

                    // Create user in tenant B
                    TestsUtil.generateTestUsers(adminRestCtxB, 1, function(
                      err,
                      users,
                      userB,
                    ) {
    ...
    it('verify that a group that is part of a private tenant cannot add a user from an external public tenant', function(callback) {
      ...
      TestsUtil.generateTestUsers(camAdminRestContext, 2, function(
        ...
        var tenantAliasB = TenantsTestUtil.generateTestTenantAlias();
        var tenantHost = TenantsTestUtil.generateTestTenantHost();
        TestsUtil.createTenantWithAdmin(tenantAliasB, tenantHost, function(
          err,
          tenantB,
          adminRestCtxB,
        ) {
              ...
              function() {
                ...
                PrincipalsTestUtil.assertJoinGroupSucceeds(
                  ...
                  function() {
                    ...
                    RestAPI.Group.getGroup(jack.restContext, group.id, function(
                      err,
                      groupAfterJoin,
                    ) {
                ...
                function(err, memberships) {
                  ...
                  RestAPI.Group.getMembershipsLibrary(
                    ...
                    function(err, memberships) {
                      ...
                      // Assert we only retrieved groups
                      assert.ok(
                        _.contains(groupIds, memberships.results[0].id),
                      );
                      assert.ok(
                        _.contains(groupIds, memberships.results[1].id),
                      );

                      // Assert that we've not seen these groups before
                      assert.ok(
                        !_.contains(seenGroupIds, memberships.results[0].id),
                      );
                      assert.ok(
                        !_.contains(seenGroupIds, memberships.results[1].id),
                      );

                      // Add the retrieved groups to the set of seen groups
                      seenGroupIds = seenGroupIds.concat(
                        _.pluck(memberships.results, 'id'),
                      );
    ...
    it('verify order', function(callback) {
      TestsUtil.generateTestUsers(camAdminRestContext, 3, function(
        ...
        /*
                 ...
                 *          simon           nico
                 */
        TestsUtil.generateTestGroups(camAdminRestContext, 5, function(
          topGroup,
          simonParentParent,
          simonParent,
          nicoParentParent,
  ...
  describe('Simple group structure', function() {
                  ...
                  function() {
                    assertGetMembershipsLibrary(
                      ...
                      function() {
                        ...
                        var membersToAdd = {};
                        membersToAdd[createdPrincipals['branden'].id] =
                          'member';
                        RestAPI.Group.setGroupMembers(
                          johnRestContext,
                          ...
                          function(err) {
                            ...
                            membersToAdd = {};
                            membersToAdd[createdPrincipals['simon'].id] =
                              'member';
                            RestAPI.Group.setGroupMembers(
                              johnRestContext,
                              ...
                              function(err) {
                                ...
                                RestAPI.Group.setGroupMembers(
                                  ...
                                  function(err) {
                                    ...
                                    membersToAdd = {};
                                    membersToAdd[createdPrincipals['bert'].id] =
                                      'member';
                                    RestAPI.Group.setGroupMembers(
                                      johnRestContext,
                                      createdPrincipals['west-flemish'].id,
    ...
    assertGroupMembers = function(
      ...
      RestAPI.Group.getGroupMembers(
        ...
        function(err, members) {
          ...
          // Morph results to hash for easy access.
          var hash = _.groupBy(members.results, function(principal) {
     ...
     *  @param  {Function}         callback             Standard callback function
     */
    assertGetMembershipsLibrary = function(
      createdPrincipals,
      userIdentifier,
      expectedGroups,
      callback,
      ...
      RestAPI.Group.getMembershipsLibrary(
        ...
        function(err, memberships) {
          ...
          // Morph results to hash for easy access
          var hash = _.groupBy(memberships.results, function(membership) {
  ...
  describe('Group visibility', function() {
    ...
    it('verify group visibility', function(callback) {
      var assertGroupVisibility = function(
        restContext,
        groupObj,
        expectedAccess,
        expectedMembers,
        ...
        RestAPI.Group.getGroup(restContext, groupObj.id, function(
          err,
          retrievedGroupObj,
        ) {
          if (expectedAccess) {
