 ...
 */
OAE.tenantRouter.on('delete', '/api/group/:groupId', function(req, res) {
    PrincipalsAPI.deleteGroup(req.ctx, req.params.groupId, function(err) {
        if (err) {
            return res.status(err.code).send(err.msg);
        }
 ...
 * @HttpResponse                404                 The group did not exist
 */
OAE.tenantRouter.on('post', '/api/group/:groupId/restore', function(req, res) {
    PrincipalsAPI.restoreGroup(req.ctx, req.params.groupId, function(err) {
        if (err) {
            return res.status(err.code).send(err.msg);
        }
...
OAE.tenantRouter.on('get', '/api/group/:groupId/members', function(req, res) {
    ...
    PrincipalsAPI.getMembersLibrary(req.ctx, req.params.groupId, req.query.start, limit, function(err, members, nextToken) {
        if (err) {
            return res.status(err.code).send(err.msg);
        }
        res.status(200).send({'results': members, 'nextToken': nextToken});
    });
});
 ...
 */
OAE.tenantRouter.on('get', '/api/group/:groupId/invitations', function(req, res) {
    PrincipalsAPI.getGroupInvitations(req.ctx, req.params.groupId, function(err, invitations) {
        if (err) {
            return res.status(err.code).send(err.msg);
        }
 ...
 */
OAE.tenantRouter.on('post', '/api/group/:groupId/invitations/:email/resend', function(req, res) {
    PrincipalsAPI.resendGroupInvitation(req.ctx, req.params.groupId, req.params.email, function(err) {
        if (err) {
            return res.status(err.code).send(err.msg);
        }
...
/**
 * @REST postGroupGroupIdRequestJoin
 *
 * Get a request
 *
 * @Server      tenant
 * @Method      POST
 * @Path        /group/{groupId}/request-join
 * @PathParam   {string}        groupId                 The id of the group to store the large picture for
 * @PathParam   {string}        role                    The role ask by the user who wants to join the group
 * @Return      {Request}       request                 A request
 * @HttpResponse                200                     Request returned
 * @HttpResponse                401                     You have to be logged in to be able to leave a group
 * @HttpResponse                402                     Invalid groupId specified
 */
OAE.tenantRouter.on('get', '/api/group/:groupId/request-join', function(req, res) {
    
    PrincipalsAPI.getRequestJoinGroup(req.ctx, req.params.groupId, function(err, request) {
        if (err) {
            return res.status(err.code).send(err.msg);
        }
...
/**
 * @REST postGroupGroupIdRequestJoinAll
 *
 * Create a request
 *
 * @Server      tenant
 * @Method      GET
 * @Path        /group/{groupId}/request-join/all
 * @PathParam   {string}        groupId                 The id of the group to store the large picture for
 * @PathParam   {string}        role                    The role ask by the user who wants to join the group
 * @Return      {Requests}      requests                List of requests for a group 
 * @HttpResponse                200                     List returned
 * @HttpResponse                401                     You have to be logged in to be able to leave a group
 * @HttpResponse                402                     Invalid groupId specified
 */
OAE.tenantRouter.on('get', '/api/group/:groupId/request-join/all', function(req, res) {

    var limit = OaeUtil.getNumberParam(req.query.limit, 10, 1, 25);

    PrincipalsAPI.getRequestsJoinGroup(req.ctx, req.params.groupId, req.query.start, limit, function(err, requests, nextToken) {
        if (err) {
            return res.status(err.code).send(err.msg);
        }
        return res.status(200).send({'results': requests, 'nextToken': nextToken});
    });
});
 ...
 * @Server      tenant
 * @Method      POST
 * @Path        /group/{groupId}/request-join/create
 * @PathParam   {string}        groupId                 The id of the group to store the large picture for
 * @PathParam   {string}        role                    The role ask by the user who wants to join the group
 * @Return      {void}
 ...
 * @HttpResponse                402                     Invalid groupId specified
 */
OAE.tenantRouter.on('post', '/api/group/:groupId/request-join/create', function(req, res) {
    
    PrincipalsAPI.createRequestJoinGroup(req.ctx, req.params.groupId, req.body.role, function(err) {
        if (err) {
            return res.status(err.code).send(err.msg);
        }
        return res.status(200).end();
...
/**
 * @REST postGroupGroupIdRequestJoinUpdate
 *
 * Update the request role
 *
 * @Server      tenant
 * @Method      POST
 * @Path        /group/{groupId}/request-join/update
 * @PathParam   {string}        groupId                 The id of the group to store the large picture for
 * @PathParam   {string}        role                    The role ask by the user who wants to join the group
 * @Return      {void}
 * @HttpResponse                200                     Request updated
 * @HttpResponse                400                     Invalid role specified
 * @HttpResponse                401                     You have to be logged in to be able to leave a group
 * @HttpResponse                402                     Invalid groupId specified
 */
OAE.tenantRouter.on('post', '/api/group/:groupId/request-join/update', function(req, res) {
    
    PrincipalsAPI.updateRequestJoinGroup(req.ctx, req.params.groupId, req.body.role, function(err) {
        if (err) {
            return res.status(err.code).send(err.msg);
        }
        return res.status(200).end();
...
/**
 * @REST postGroupGroupIdRequestJoinRemove
 *
 * Remove a request from a user
 *
 * @Server      tenant
 * @Method      POST
 * @Path        /group/{groupId}/request-join/remove
 * @PathParam   {string}        groupId                 The id of the group to join
 * @Return      {void}
 * @HttpResponse                200                     Request removed
 * @HttpResponse                401                     You have to be logged in to be able to leave a group
 * @HttpResponse                402                     Invalid groupId specified
 */
OAE.tenantRouter.on('delete', '/api/group/:groupId/request-join/remove', function(req, res) {

    PrincipalsAPI.removeJoinGroupByRequest(req.ctx, req.params.groupId, function(err) {
        if (err) {
            return res.status(err.code).send(err.msg);
        }
        return res.status(200).end();
...
/**
 * @REST postGroupGroupIdRequestJoinAccept
 *
 * Accept a request
 *
 * @Server      tenant
 * @Method      POST
 * @Path        /group/{groupId}/request-join/accept
 * @PathParam   {string}        groupId                 The id of the group to join
 * @PathParam   {string}        principalId             The id of the principal who wants to join this group
 * @PathParam   {string}        role                    The role ask by the user who wants to join the group
 * @Return      {void}
 ...
 * @HttpResponse                402                     Invalid groupId specified
 */
OAE.tenantRouter.on('post', '/api/group/:groupId/request-join/accept', function(req, res) {

    PrincipalsAPI.acceptJoinGroupByRequest(req.ctx, req.params.groupId, req.body.principalId, req.body.role, function(err) {
        if (err) {
            return res.status(err.code).send(err.msg);
        }
        return res.status(200).end();
...
/**
 * @REST postGroupGroupIdRequestJoinReject
 *
 * Reject a request
 *
 * @Server      tenant
 * @Method      POST
 * @Path        /group/{groupId}/request-join/reject
 * @PathParam   {string}        groupId                 The id of the group to join
 * @PathParam   {string}        principalId             The id of the principal who wants to join this group
 * @Return      {void}
 * @HttpResponse                200                     Request rejected
 * @HttpResponse                401                     You have to be logged in to be able to leave a group
 * @HttpResponse                402                     Invalid groupId specified
 */
OAE.tenantRouter.on('delete', '/api/group/:groupId/request-join/reject', function(req, res) {

    PrincipalsAPI.rejectJoinGroupByRequest(req.ctx, req.params.groupId, req.body.principalId, function(err) {
        if (err) {
            return res.status(err.code).send(err.msg);
        }
        return res.status(200).end();
