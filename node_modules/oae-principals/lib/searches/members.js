...
module.exports = function(ctx, opts, callback) {
    ...
    PrincipalsAPI.getGroup(ctx, opts.resourceId, function(err, group) {
        if (err) {
            ...
            return callback({'code': 404, 'msg': util.format('Couldn\'t find group: %s', opts.resourceId)});
        }
...
/**
 * Perform the search that searches a group or content item's list of members
 *
 * @param  {Context}       ctx                 Standard context object containing the current user and the current tenant
 * @param  {Object}        opts                General search options
 * @param  {Function}      callback            Standard callback function
...
var _search = function(ctx, opts, callback) {
    ...
    var filter = SearchUtil.filterAnd(
        SearchUtil.filterResources(['user', 'group'], SearchConstants.deleted.BOTH),
        SearchUtil.createHasChildQuery(AuthzConstants.search.MAPPING_RESOURCE_MEMBERSHIPS, SearchUtil.filterTerms('direct_memberships', [opts.resourceId]))
    );
