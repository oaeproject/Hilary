...
var saveQueuedActivities = module.exports.saveQueuedActivities = function(activityBuckets, callback) {
    ...
    _.each(activityBuckets, function(activityBucket, bucketNumber) {
        ...
        log().trace({'activityBucket': activityBucket}, 'Persisting queued activities to bucket.');

        var redisZaddInput = [_createBucketKey(bucketNumber)];
        _.each(activityBucket, function(routedActivity) {
            redisZaddInput.push(routedActivity.activity.published);
            redisZaddInput.push(JSON.stringify(routedActivity));
        });

        bucketRedisOps[bucketNumber] = redisZaddInput;
    });

    log().trace({'data': bucketRedisOps}, 'Soring data in redis.');

    var numBuckets = _.keys(bucketRedisOps).length;
    var done = false;
    var numComplete = 0;
    _.each(bucketRedisOps, function(ops) {
        Redis.getClient().zadd(ops, function(err) {
            if (done) {
                // Alread completed, don't do anything
                return;
            } else if (err) {
                done = true;
                return callback(err);
...
var updateAggregateStatus = module.exports.updateAggregateStatus = function(statusUpdates, callback) {
    ...
    _.each(keysToExpire, function(key) {
        multi.expire(key, ActivitySystemConfig.getConfig().aggregateMaxExpiry);
    });
...
var getAggregatedEntities = module.exports.getAggregatedEntities = function(aggregateKeys, callback) {
    ...
    multi.exec(function(err, results) {
        ...
        var objects = results[1];
        var targets = results[2];

        _.each(results, function(result, i) {
            if (result) {
                // Each aggregate key is permutated into 3 keys (one for actors, objects and targets). So figure out which aggregateKey and entityType this is for
...
var saveAggregatedEntities = module.exports.saveAggregatedEntities = function(aggregates, callback) {
    ...
    _.each(aggregates, function(aggregate, aggregateKey) {
        ...
        // Generate the arguments for the hmset requests
        ...
        // Bind the requests to the multi
        if (hmsetActorArgs.length > 0) {
            updatedAggregate = true;
        ...
        if (updatedAggregate) {
            // If we've update the aggregate, reset the expiry on the entity content
            multi.expire(aggregateActorsKey, ActivitySystemConfig.getConfig().aggregateMaxExpiry);
            multi.expire(aggregateObjectsKey, ActivitySystemConfig.getConfig().aggregateMaxExpiry);
            multi.expire(aggregateTargetsKey, ActivitySystemConfig.getConfig().aggregateMaxExpiry);
        }
    });
