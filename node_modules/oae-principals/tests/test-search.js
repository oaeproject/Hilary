    ...
    describe('Indexing', () => {
        ...
        it('verify indexing without full user item', (callback) => {
            TestsUtil.generateTestUsers(camAdminRestContext, 2, (err, users, doer, jack) => {
                ...
                SearchTestsUtil.searchAll(doer.restContext, 'general', null, {'resourceTypes': 'user', 'q': jack.user.displayName}, (err, results) => {
                    assert.ok(!err);
        ...
        it('verify indexing without full group item', (callback) => {
            TestsUtil.generateTestUsers(camAdminRestContext, 1, (err, users, doer) => {
                ...
                RestAPI.Group.createGroup(doer.restContext, groupText, groupText, 'public', 'no', [], [], (err, group) => {
                    ...
                    // Verify the content item exists
                    SearchTestsUtil.searchAll(doer.restContext, 'general', null, {'resourceTypes': 'group', 'q': groupText}, (err, results) => {
                        assert.ok(!err);
                ...
                SearchTestsUtil.deleteAll(() => {
                    ...
                    TestsUtil.generateTestUsers(camAdminRestContext, 1, (err, users, user) => {
                        ...
                        TestsUtil.generateTestGroups(user.restContext, 1, (group) => {
                            ...
                            SearchTestsUtil.searchAll(user.restContext, 'general', null, {'resourceTypes': 'user', 'q': user.user.displayName}, (err, results) => {
                                assert.ok(!err);
                                assert.ok(_getDocById(results, user.user.id));

                                SearchTestsUtil.searchAll(user.restContext, 'general', null, {'resourceTypes': 'group', 'q': group.displayName}, (err, results) => {
                                    assert.ok(!err);
                                    assert.ok(_getDocById(results, group.id));

                                    SearchTestsUtil.searchAll(user.restContext, 'memberships-library', [user.user.id], null, (err, results) => {
                                        assert.ok(!err);
                                        assert.ok(_getDocById(results, group.id));

                                        SearchTestsUtil.searchAll(user.restContext, 'members-library', [group.id], null, (err, results) => {
                                            assert.ok(!err);
                                            assert.ok(_getDocById(results, user.user.id));
