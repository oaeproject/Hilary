...
var _createEmailBucketIdForUser = function(user) {
    ...
    if (user.emailPreference !== PrincipalsConstants.emailPreferences.IMMEDIATE) {
         ...
         *
         * For example, the server is always configured in UTC time, the given timezone is Miami (UTC-5) and mails need
         * to be delivered at 13h. The mail needs to leave the server at 18h (UTC time) so it arrives when it's 13h in Miami.
         *
         * For weekly emails, it's entirely possible we need to schedule email delivery on the previous/next day.
         * For example, the server is always configured in UTC time, the user's timezone is Islamabad (UTC+5) and mails
         * need to be delivered at 2am. The mails needs to leave the server at 21h (UTC time) so it arrives when
