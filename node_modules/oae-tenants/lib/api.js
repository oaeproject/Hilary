...
var getTenantByEmail = module.exports.getTenantByEmail = function(emailOrDomain) {
    ...
    var tenant = tenants[tenantAlias] || tenants[serverConfig.guestTenantAlias];
...
var _updateCachedTenant = function(tenantAlias, callback) {
    ...
    Cassandra.runQuery('SELECT * FROM "Tenant" WHERE "alias" = ?', [tenantAlias], function(err, rows) {
        ...
        tenantEmailDomainIndex.update(tenantAlias, tenant.emailDomain, (oldTenant && oldTenant.emailDomain));
        ...
        // Update the tenant in the email domain index
        var conflictingTenantAlias = tenantEmailDomainIndex.update(tenant.alias, tenant.emailDomain);
...
var _createTenant = function(alias, displayName, host, opts, callback) {
    ...
    _.each(opts, function(val, key) {
        ...
        } else if (key === 'countryCode' && val) {
            validator.check(val, {'code': 400, 'msg': 'The country code must be a valid ISO-3166 country code'}).isIso3166Country();
        }
    });
    if (validator.hasErrors()) {
        return callback(validator.getFirstError());
...
var updateTenant = module.exports.updateTenant = function(ctx, alias, tenantUpdates, callback) {
    ...
    _.each(tenantUpdates, function(updateValue, updateField) {
        ...
        } else if (updateField = 'countryCode' && updateValue) {
            validator.check(updateValue, {'code': 400, 'msg': 'The country code must be a valid ISO-3166 country code'}).isIso3166Country();
        }
    });
    if (validator.hasErrors()) {
        return callback(validator.getFirstError());
