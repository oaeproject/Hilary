...
describe('Collaborative documents', function() {
            ...
            RestAPI.Content.createCollabDoc(
              ...
              function(err, contentObj) {
                ...
                RestAPI.Content.joinCollabDoc(ctx, ' ', function(err) {
                  ...
                  RestAPI.Content.joinCollabDoc(ctx, 'invalid-id', function(
                    err,
                  ) {
                    assert.equal(err.code, 400);

                    // Sanity check
                    RestAPI.Content.joinCollabDoc(ctx, contentObj.id, function(
                      err,
                      data,
                    ) {
                      assert.ok(!err);
  ...
  it('verify different servers get selected depending on the content ID', function() {
    ...
    for (var i = 0; i < total; i++) {
      ...
      var etherpadUrl = Etherpad.getPadUrl(
        contentObjC,
        'userId',
        'sesionId',
        'authorId',
    ...
    assert.ok(
      devA > 0.95,
      'Expected a maximum deviation of 5%, deviation was: ' +
        Math.round((1 - devA) * 100) +
        '%',
    );
    assert.ok(
      devB > 0.95,
      'Expected a maximum deviation of 5%, deviation was: ' +
        Math.round((1 - devB) * 100) +
        '%',
    );
    ...
    TestsUtil.generateTestUsers(camAdminRestContext, 3, function(err, users) {
      ...
      RestAPI.Content.createCollabDoc(
        ...
        function(err, contentObj) {
          ...
          RestAPI.Content.joinCollabDoc(simonCtx, contentObj.id, function(
            err,
            data,
          ) {
            assert.ok(!err);
            ...
            RestAPI.Content.joinCollabDoc(brandenCtx, contentObj.id, function(
              err,
              data,
            ) {
  ...
  it('verify the correct HTML is retrieved when publishing', function(callback) {
    ...
    ContentTestUtil.createCollabDoc(camAdminRestContext, 1, 1, function(
      contentObj,
      users,
      simon,
    ) {
      // Verify there is no HTML present yet
      getContentWithLatestRevision(simon.restContext, contentObj.id, function(
        contentObj,
        revision,
      ) {
        ...
        editAndPublish(simon, contentObj, [text], function() {
          getContentWithLatestRevision(
            simon.restContext,
            contentObj.id,
            function(updatedContentObj, updatedRevision) {
              ...
              assert.notEqual(
                updatedRevision.etherpadHtml,
                revision.etherpadHtml,
              );

              // Remove linebreaks and check if the text is correct
              ContentTestUtil.assertEtherpadContentEquals(
                updatedRevision.etherpadHtml,
                text,
              );
              ...
              setEtherpadText(
                ...
                function(err) {
                  ...
                  getContentWithLatestRevision(
                    simon.restContext,
                    contentObj.id,
                    function(latestContentObj, latestRevision) {
            ...
            RestAPI.Content.restoreRevision(
              ...
              function(err) {
                ...
                getContentWithLatestRevision(
                  simon.restContext,
                  contentObj.id,
                  function(updatedContent, updatedRevision) {
