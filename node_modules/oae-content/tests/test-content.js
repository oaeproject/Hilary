    ...
    describe('Download content', function() {
        ...
        it('verify validation of signed urls', function(callback) {
            setUpUsers(function(contexts) {
                ...
                // Generate a signed download url for Branden that Simon will try to use.
                var tenant = TenantsAPI.getTenant(contexts['branden'].user.tenant.alias);
                var signedDownloadUrl = ContentUtil.getDownloadUrl(new Context(tenant, contexts['branden'].user), 'local:2012/12/06/file.doc');
                var parsedUrl = url.parse(signedDownloadUrl, true);

                // Branden should be able to download it.
                // (In this case, downloading = 204)
                RestUtil.RestRequest(contexts['branden'].restContext, '/api/download/signed', 'GET', parsedUrl.query, function(err, body, response) {
                    ...
                    // If Simon tries to download the file with the same parameters,
                    // it should result in a 401 as the signature would not be correct.
                    // (It would not match his principal ID.)
                    RestUtil.RestRequest(contexts['simon'].restContext, '/api/download/signed', 'GET', parsedUrl.query, function(err, body, response) {
                        assert.equal(err.code, 401);

                        // An anonymous user can't download this either.
                        RestUtil.RestRequest(anonymousRestContext, '/api/download/signed', 'GET', parsedUrl.query, function(err, body, response) {
                            assert.equal(err.code, 401);

                            // The following requests test missing or invalid parameters
                            var params = _.extend({}, parsedUrl.query);

                            // Missing uri.
                            delete params.uri;
                            RestUtil.RestRequest(contexts['branden'].restContext, '/api/download/signed', 'GET', params, function(err, body, request) {
                                assert.equal(err.code, 400);

                                // Missing signature.
                                params = _.extend({}, parsedUrl.query);
