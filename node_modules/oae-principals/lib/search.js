...
var _transformUserDocuments = function(ctx, docs, callback) {
    ...
    for (var i = 0; i < docIds.length; i++) {
        ...
        if (user.mediumPictureUri) {
            var sig = Signature.createExpiresAtSignature(fields.tenantAlias, expiresAt, fields.thumbailUrl);
            result.thumbnailUrl = util.format('/api/download/signed?uri=%s&signature=%s&expires=%s', user.mediumPictureUi, sig.signature, sig.expires);
        }
...
var _transformGroupDocuments = function(ctx, docs, callback) {
    ...
    for (var i = 0; i < docIds.length; i++) {
        ...
        var doc = docs[docId];
        var fields = doc.fields
        var group = new Group(fields.tenantAlias, docId, fields.alias, {
            'visibility': fields.visibility
        });
        if (fields.thumbnailUrl) {
            group.mediumPictureUri = fields.thumbnailUrl;
        }

        // hide information that is sensitive to the current session, then transform it back into a search document using the producer
        var result = _produceGroupSearchDocument(group);

        if (group.mediumPictureUri) {
            var sig = Signature.createExpiresAtSignature(fields.tenantAlias, expiresAt, fields.thumbailUrl);
            result.thumbnailUrl = util.format('/api/download/signed?uri=%s&signature=%s&expires=%s', group.mediumPictureUi, sig.signature, sig.expires);
        }
