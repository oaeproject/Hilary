...
var AuthzGraph = require('oae-authz/lib/internal/graph');
...
var _updateRoles = function(resourceId, changes, callback) {
    ...
    var queries = _.chain(changes)
        .map(function(role, principalId) {
            if (role) {
    ...
    var usersToInvalidate = [];
    var groupsToInvalidate = {};
    if (AuthzUtil.isGroupId(resourceId)) {
        _.each(changes, function(change, principalId) {
    ...
    // queries we have
    getAuthzMembersGraph(_.keys(groupsToInvalidate), function(err, graph) {
        if (err) {
            return callback(err);
        }
 ...
 */
var getAllAuthzMembers = module.exports.getAllAuthzMembers = function(resourceId, callback, _members, _nextToken) {
    Cassandra.runAllPagesQuery('AuthzMembers', 'resourceId', resourceId, 'memberId', null, function(err, rows) {
        if (err) {
            return callback(err);
        }

        var members = _.chain(rows)
...
var _checkGroupMembershipsForUser = function(userId, groupIds, callback) {
    ...
    Cassandra.runQuery('SELECT "groupId" FROM "AuthzMembershipsCache" WHERE "principalId" = ? LIMIT 1', [userId], function(err, rows) {
        if (err) {
            return callback(err);
        }
        ...
        // There are no columns, we need to explode the user's group memberships
            ...
            _explodeGroupMemberships(userId, function(err, graph) {
                if (err) {
                    return callback(err);
                }
                ...
                // removed
                var allMemberships = _.chain(graph.getNodes())
                    .pluck('id')
                    .without(userId)
                    .value()
                    .sort();
...
var _explodeGroupMemberships = function(principalId, callback) {
    ...
    _getAuthzGroupMembershipsGraph([principalId], function(err, graph) {
        if (err) {
            return callback(err);
        }
        ...
        var authzMembershipsCacheQueries = _.map(allMemberships, function(groupId) {
            ...
            var memberRoles = _.chain(graph.getInEdgesOf(groupId))
                .map(function(edge) {
 ...
 */
var getAuthzMembersGraph = module.exports.getAuthzMembersGraph = function(resourceIds, callback, _graph, _resourceIds) {
    ...
    var resourceId = _resourceIds.shift();
    ...
    getAuthzMembers(resourceId, null, 10000, function(err, members) {
        if (err) {
            return callback(err);
        }
        ...
        _.each(members, function(member) {
            var node = _graph.addNode(member.id);
            var edge = _graph.addEdge(member.id, resourceId, {'role': member.role});
 ...
 */
var getPrincipalMembershipsGraph = module.exports.getPrincipalMembershipsGraph = function(principalId, callback) {
    var validator = new Validator();
    validator.check(principalId, {'code': 400, 'msg': 'Invalid principal id provided'}).isPrincipalId();
    if (validator.hasErrors()) {
        return callback(validator.getFirstError());
    ...
    Cassandra.runAllPagesQuery('AuthzMembershipsCache', 'principalId', principalId, 'groupId', null, function(err, rows) {
        if (err) {
            return callback(err);
        ...
        if (!_.isEmpty(graphParseErrs)) {
            _.each(graphParseErrs, function(graphParseErr) {
...
var getPrincipalMemberships = module.exports.getPrincipalMemberships = function(principalId, start, limit, callback) {
    ...
    Cassandra.runPagedQuery('AuthzMembershipsCache', 'principalId', principalId, 'groupId', start, limit, null, function(err, rows, nextToken, startMatched) {
        ...
        _explodeGroupMemberships(principalId, function(err, graph) {
            if (err) {
                return callback(err);
            }
...
var getIndirectPrincipalMemberships = module.exports.getIndirectPrincipalMemberships = function(principalId, start, limit, callback) {
    ...
    // Note that indirect group memberships aren't cached at the moment, so this will still function
    // properly but be less efficient than it could be. Since there is no use-case currently to get
    // indirect memberships of a group, this inefficiency is not an issue
 ...
 * @api private
 */
var _getIndirectPrincipalMembershipsFromCache = function(principalId, start, limit, callback) {
 ...
 * @api private
 */
var _getIndirectPrincipalMembershipsByExplosion = function(principalId, start, limit, callback) {
    _explodeGroupMemberships(principalId, function(err, graph) {
        if (err) {
            return callback(err);
        }
        ...
        var indirectMemberships = _.chain(graph.getNodes())
            .filter(function(node) {
                var isPrincipalId = (node.id === principalId);
                var isDirect = graph.getEdge(principalId, node.id);
        ...
        // We don't want to include the start element, so pick the next element as the start
        var startIndex = (start) ? _.indexOf(indirectMemberships, start) + 1 : 0;
 ...
 * @api private
 */
var _getAuthzGroupMembershipsGraph = function(principalIds, callback, _graph) {
    if (!_graph) {
    ...
    getRolesForPrincipalsAndResourceType(principalIds, AuthzConstants.resourceTypes.GROUP, function(err, principalParentRole) {
        if (err) {
            return callback(err);
        }
        ...
        var nextPrincipalIds = [];
        _.each(principalParentRole, function(parentRole, principalId) {
            _.each(parentRole, function(role, parentGroupId) {
                // Add the group as a node in the graph
                var node = _graph.addNode(parentGroupId);
                if (node) {
                ...
                _graph.addEdge(principalId, parentGroupId, {'role': role});
            });
        });
 ...
 * @api private
 */
var _getInvalidateMembershipsCacheQueries = function(userIds) {
 ...
 */
var getAllRolesForPrincipalAndResourceType = module.exports.getAllRolesForPrincipalAndResourceType = function(principalId, resourceType, callback) {
    var validator = new Validator();
    validator.check(principalId, {'code': 400, 'msg': 'Invalid principal id specified: ' + principalId}).isPrincipalId();
    ...
    // CQL.
    var start = util.format('%s:', resourceType);
    var end = util.format('%s:|', resourceType);

    Cassandra.runAllPagesQuery('AuthzRoles', 'principalId', principalId, 'resourceId', {'start': start, 'end': end}, function(err, rows) {
        if (err) {
            return callback(err);
        }
 ...
 */
var getRolesForPrincipalsAndResourceType = module.exports.getRolesForPrincipalsAndResourceType = function(principalIds, resourceType, callback) {
 ...
 * @param  {Function}   callback                        Standard callback function
 * @param  {Object}     callback.err                    An error that occurred, if any
 * @param  {Object}     callback.changes                The applicable changes that were actually applied. This omits and pointless changes such as setting a user to the role they already have, or removing a user that wasn't a member already
 * @param  {Object}     callback.roles                  Indicates the full membership roles before and after the changes are applied
 * @param  {Object}     callback.roles.before           An object keyed by principal id, whose value is the role the principal had before the changes are made
 * @param  {Object}     callback.roles.after            An object keyed by principal id, whose value is the role the principal had after the changes are made
 * @param  {Object}     callback.members                Indicates the principal ids that were added, updated and removed as a result of these changes
 * @param  {String[]}   callback.members.addedIds       The principal ids that would be new members to the resource
 * @param  {String[]}   callback.members.updatedIds     The principal ids whose role existed before and will change
 * @param  {String[]}   callback.members.removedIds     The principal ids whose role existed before and will be removed
 */
var computeMemberRolesAfterChanges = module.exports.computeMemberRolesAfterChanges = function(resourceId, changes, opts, callback) {
    // When the resource id is not specified, we perform the computation as though the members list is empty
    OaeUtil.invokeIfNecessary(resourceId, getAuthzMembers, resourceId, null, 10000, function(err, memberIdsWithRoleBefore) {
        if (err) {
            return callback(err);
        }

        // Convert the member ids + role array into the permission change object
        var memberRolesBefore = _.chain(memberIdsWithRoleBefore)
            .indexBy('id')
            .mapObject(function(memberIdWithRole) {
                return memberIdWithRole.role;
            })
            .value();
        var changeResult = AuthzUtil.computeRoleChanges(memberRolesBefore, changes, opts);

        // Extract the results in the way we want them
        var members = {
            'addedIds': changeResult.ids.added,
            'updatedIds': changeResult.ids.updated,
            'removedIds': changeResult.ids.removed
        };

        return callback(null, changeResult.changes, changeResult.roles, members);
    });
};
 ...
 */
var resolveEffectiveRole = module.exports.resolveEffectiveRole = function(user, resource, rolesPriority, callback) {
    resolveImplicitRole(user, resource, rolesPriority, function(err, implicitRole, canInteract) {
        if (err) {
            return callback(err);
        ...
        getAllRoles(user.id, AuthzUtil.getAuthzId(resource), function(err, roles) {
            if (err) {
                return callback(err);
 ...
 */
var resolveImplicitRole = module.exports.resolveImplicitRole = function(principal, resource, rolesPriority, callback) {
    if (!principal) {
    ...
    var principalId = AuthzUtil.getAuthzId(principal);
    var resourceId = AuthzUtil.getAuthzId(resource);
    ...
    if (AuthzUtil.isUserId(principalId)) {
        ...
        } else if (principal.isAdmin(resource.tenant.alias)) {
    ...
    var tenantsCanInteract = TenantsUtil.canInteract(principal.tenant.alias, resource.tenant.alias);
    if (resource.visibility === AuthzConstants.visibility.PUBLIC) {
        ...
        if (AuthzUtil.isUserId(principalId) && resource.joinable === AuthzConstants.joinable.YES && tenantsCanInteract) {
            ...
            return callback(null, _.first(rolesPriority), tenantsCanInteract);
        } else if (resource.visibility === AuthzConstants.visibility.LOGGEDIN && principal.tenant.alias === resource.tenant.alias) {
