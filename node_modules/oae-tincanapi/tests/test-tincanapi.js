...
describe('TinCanAPI', function() {
	...
	/**
	 * Test that verifies that expapi statements are sent to a configurable LRS.
	 */
	it('Verify if Tin Can works', function(callback) {
			...
			TestsUtil.generateTestUsers(camAdminRestContext, 1, function(err, users) {
				...
				_.each(users, function(user, id) {
					...
					onRequest = function(req) {
						...
						if (tincanSent && activitiesCollected) {
							callback();
						}
					};
	...
	/**
	 * Test that verifies if statements still could be sent to LRS when disabled in tenant
	 */
	it('Verify if Tin Can is enabled in the tenant', function(callback) {
		...
		ConfigTestUtil.updateConfigAndWait(camAdminRestContext, null, 'oae-tincanapi/lrs/enabled', false, function(err) {
			...
			TestsUtil.generateTestUsers(camAdminRestContext, 1, function(err, users) {
				...
				_.each(users, function(user, id) {
					...
					onRequest = function(req) {
						assert.fail(null, null, 'No statements should be sent when LRS is disabled');
					};
