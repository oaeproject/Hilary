  ...
  var _assertAllEmailsInvite = function(restContext, emails, opts, callback) {
    ContentTestUtil.assertCreateLinkSucceeds(
      ...
      function(link) {
        ...
        RestAPI.Content.shareContent(restContext, link.id, emails, function(
          err,
        ) {
          assert.ok(!err);
      ...
      var queries = _.chain(userIdEmails)
        .map(function(email, userId) {
          return [
            {
              query:
                'UPDATE "Principals" SET "email" = ? WHERE "principalId" = ?',
                ...
                _assertAllEmailsSearch(
                  ...
                  function() {
                    _assertAllEmailsInvite(
                      ...
                      function() {
                        // Run the migration
                        LowerCaseEmailsMigrator.doMigration(function(
                          err,
                          stats,
                        ) {
                          assert.ok(!err);
                          assert.strictEqual(
                            stats.nUpdated,
                            emailsWithUpperCase.length,
                          );
                          assert.strictEqual(stats.nFailed, 0);

                          // Refresh the search index
                          SearchTestUtil.reindexAll(
