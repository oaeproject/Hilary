 ...
 */
var getOrCreateTokensByEmails = (module.exports.getOrCreateTokensByEmails = function(
  emails,
  callback,
) {
 ...
 * @param  {Object}     callback.emailTokens    An object keyed by email, whose value is the invitation token associated to it
 */
var getTokensByEmails = (module.exports.getTokensByEmails = function(
  emails,
  callback,
) {
 ...
 */
var getEmailByToken = (module.exports.getEmailByToken = function(
  token,
  callback,
) {
  Cassandra.runQuery(
    ...
    function(err, rows) {
      if (err) {
        ...
        return callback({
          code: 404,
          msg: util.format(
            'There is no email associated to the email token "%s"',
            token,
          ),
        });
      }
 ...
 */
var getAllInvitationsByEmail = (module.exports.getAllInvitationsByEmail = function(
  email,
  callback,
) {
  _getAllInvitationResourceIdsByEmail(email, function(
    err,
    invitationResourceIds,
  ) {
    if (err) {
      return callback(err);
    }
 ...
 */
var getAllInvitationsByResourceId = (module.exports.getAllInvitationsByResourceId = function(
  resourceId,
  callback,
  _invitations,
  _nextToken,
 ...
 */
var getInvitation = (module.exports.getInvitation = function(
  resourceId,
  email,
  callback,
) {
    ...
    function(err, rows) {
      if (err) {
        ...
        return callback({
          code: 404,
          msg:
            'An invitation could not be found for the given resource and email',
        });
      }
 ...
 */
var createInvitations = (module.exports.createInvitations = function(
  resourceId,
  emailRoles,
  inviterUserId,
  callback,
 ...
 * @param  {Object}     callback.err    An error that occurred, if any
 */
var updateInvitationRoles = (module.exports.updateInvitationRoles = function(
  resourceId,
  emailRoles,
  callback,
) {
  ...
  _.each(emailRoles, function(role, email) {
    if (_.isString(role)) {
      queries.push({
        query:
          'UPDATE "AuthzInvitations" SET "role" = ? WHERE "resourceId" = ? AND "email" = ?',
      ...
      queries.push(
        {
          query:
            'DELETE FROM "AuthzInvitations" WHERE "resourceId" = ? AND "email" = ?',
 ...
 * @param  {Object}     callback.err    An error that occurred, if any
 */
var deleteInvitationsByResourceId = (module.exports.deleteInvitationsByResourceId = function(
  resourceId,
  callback,
) {
 ...
 * @param  {Object}     callback.err    An error that occurred, if any
 */
var deleteInvitationsByEmail = (module.exports.deleteInvitationsByEmail = function(
  email,
  callback,
) {
  var validator = new Validator();
  validator
    .check(email, { code: 400, msg: 'Specified email is not valid' })
    .isEmail();
  if (validator.hasErrors()) {
    return callback(validator.getFirstError());
  }
  ...
  getOrCreateTokensByEmails([email], function(err, tokenByEmail) {
    ...
    _getAllInvitationResourceIdsByEmail(email, function(err, resourceIds) {
      ...
      var queries = [
        ...
        {
          query:
            'DELETE FROM "AuthzInvitationsResourceIdByEmail" WHERE "email" = ?',
          parameters: [email],
        },
      ];
 ...
 * @api private
 */
var _getAllInvitationResourceIdsByEmail = function(
  email,
  callback,
  _resourceIds,
  _nextToken,
  ...
  var cql =
    'SELECT "resourceId" FROM "AuthzInvitationsResourceIdByEmail" WHERE "email" = ?';
  var params = [email];
  if (_nextToken) {
    cql += ' AND "resourceId" > ?';
  ...
  Cassandra.runQuery(cql, params, function(err, rows) {
    ...
    return _getAllInvitationResourceIdsByEmail(
      email,
      callback,
      _resourceIds,
      _.last(resourceIds),
