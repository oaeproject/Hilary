...
var _createFile = function(ctx, displayName, description, visibility, additionalMembers, file, callback) {
    ...
    _createContent(ctx, 'file', displayName, description, visibility, additionalMembers, otherValues, function(err, contentObj) {
        ...
        getStorageBackend(ctx).store(ctx, file, options, function(err, uri) {
            ...
            // Create the revision.
            otherValues.uri = uri;
            ContentDAO.Revisions.createRevision(contentObj.id, ctx.tenant().alias, ctx.user().id, otherValues, function(err) {
                if (err) {
                    return callback(err);
                }
...
var createCollabDoc = module.exports.createCollabDoc = function(ctx, displayName, description, visibility, additionalMembers, callback) {
    ...
    _createContent(ctx, 'collabdoc', displayName, description, visibility, additionalMembers, {}, function(err, content) {
        ...
        Etherpad.createPad(content, function(err) {
            if (err) {
                return callback(err);
            }
...
var publishCollabDoc = module.exports.publishCollabDoc = function(ctx, contentId, callback) {
    ...
    ContentAPI.getContent(ctx, contentId, function(err, contentObj) {
        if (err) {
            ...
            return callback({'code': 400, 'msg': 'This is not a collaboration document.'});
        }

        // Get the latest html from etherpad.
        Etherpad.getHTML(contentObj, function(err, html) {
            if (err) {
                return callback(err);
            }

            // Create a new revision.
            var opts = {'collabdocHtml': html};
            ContentDAO.Revisions.createRevision(contentId, ctx.tenant().alias, ctx.user().id, opts, function(err, revision) {
                if (err) {
                ...
                ContentDAO.Content.updateContent(contentObj, opts, true, function(err, newContentObj) {
                    ...
                    newContentObj.collabdoc.html = html;
 ...
 * Only users who have manager permissions on the collaboration document can join the pad.
 *
 * @param  {Context}    ctx             Standard context object containing the current user and the current tenant
 * @param  {String}     contentId       The ID of the collaboration document that should be joined.
 * @param  {Function}   callback        Standard callback function takes arguments `err` and `content`
 * @param  {Object}     callback.err    Error object containing the error message
...
var joinCollabDoc = module.exports.joinCollabDoc = function(ctx, contentId, callback) {
    ...
    canManage(ctx, contentId, function(err, canManage, contentObj) {
        ...
        // Join the pad
        Etherpad.joinPad(ctx, contentObj, callback);
    });
};
...
var _updateFileBody = function(ctx, contentId, file, callback) {
    ...
    canManage(ctx, contentId, function(err, isManager, contentObj) {
        ...
        getStorageBackend(ctx).store(ctx, file, options, function(err, uri) {
            ...
            ContentDAO.Revisions.createRevision(contentObj.id, ctx.tenant().alias, ctx.user().id, opts, function(err, revision) {
                if (err) {
                    return callback(err);
                }
    ...
    canManage(ctx, contentId, function(err, canManage, contentObj) {
        ...
        _getRevision(ctx, contentId, revisionId, function(err, revision) {
            if (err) {
                ...
                return callback({'code': 401, 'msg': 'Manager reights are required to restore a revision.'});
            }

            // Create a new revision, by copying the other one.
            var tenantAlias = AuthzUtil.getResourceFromId(contentId).tenantAlias;
            ContentDAO.Revisions.createRevision(contentId, tenantAlias, ctx.user().id, revision, function(err, newRevision) {
                if (err) {
                    return callback(err);
                }
                ...
                ContentDAO.Content.updateContent(contentObj, profileFields, true, function(err) {
                    ...
                    // If this piece of content is a collaboration document,
                    // we need to set the text in etherpad.
                    if (contentObj.resourceSubType === 'collabdoc') {
                        Etherpad.setHTML(contentObj, profileFields.collabdocHtml, function(err) {
                            if (err) {
                                return callback(err);
                            }
