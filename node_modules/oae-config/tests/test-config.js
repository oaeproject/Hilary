...
describe('Configuration', function() {
    ...
    describe('Internal Configuration API', function() {
        ...
        it('verify get single config value', function(callback) {
            ...
            assert.equal(AuthenticationConfig.getConfigValue(johnContext, 'sso', 'enabled'), null);
            // Retrieve a boolean value
            assert.equal(AuthenticationConfig.getConfigValue(johnContext, 'twitter', 'enabled'), true);
            // Retrieve a string value
            assert.equal(PrincipalsConfig.getConfigValue(johnContext, 'user', 'defaultLanguage'), 'en_GB');
            callback();
        });
        ...
        it('verify validation', function(callback) {
            // Verify that initializing a config factory needs a module name
            assert.equal(ConfigAPI.config().code, 400);
            // Verify that a feature needs to be provided when getting a config value
            var AuthenticationConfig = ConfigAPI.config('oae-authentication');
            assert.equal(AuthenticationConfig.getConfigValue(johnContext).code, 400);
            // Verify that an element needs to be provided when getting a config value
            assert.equal(AuthenticationConfig.getConfigValue(johnContext, 'twitter').code, 400);
            callback();
        });
    });
    ...
    describe('REST Configuration API', function() {
        ...
        /**
         * Test that verifies that the configuration can be retrieved on the global and tenant server
         */
        it('verify get config', function(callback) {
            RestAPI.Config.getTenantConfig(globalAdminRestContext, null, function(err, config) {
                ...
                // Verify that the tenants have the same value as their default
                RestAPI.Config.getTenantConfig(camAdminRestContext, null, function(err, config) {
                    assert.ok(!err);
                    assert.ok(config);
                    assert.equal(config['oae-authentication']['twitter']['enabled'], true);
                    
                    // Verify that the tenant value can also be retrieved through the global admin
                    console.log(global.oaeTests.tenants.cam.alias);
            ...
            RestAPI.Config.updateConfig(camAdminRestContext, null, 'oae-authentication/twitter/enabled', true, function(err) {
                ...
                RestAPI.Config.getTenantConfig(camAdminRestContext, null, function(err, config) {
                    ...
                    RestAPI.Config.getTenantConfig(globalAdminRestContext, global.oaeTests.tenants.cam.alias, function(err, config) {
                        ...
                        // Validate that the global admin still has the old value
                        RestAPI.Config.getTenantConfig(globalAdminRestContext, null, function(err, config) {
                            assert.ok(!err);
                            assert.ok(config);
                            assert.equal(config['oae-authentication']['twitter']['enabled'], false);
