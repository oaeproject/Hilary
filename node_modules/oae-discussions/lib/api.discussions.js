...
var Validator = require('oae-authz/lib/validator');
...
// When updating discussions, only do it at most every hour
var LIBRARY_UPDATE_THRESHOLD_SECONDS = 360;
...
var createDiscussion = module.exports.createDiscussion = function(ctx, displayName, description, visibility, members, opts, callback) {
    visibility = visibility || DiscussionsConfig.getValue(ctx.tenant().alias, 'visibility', 'discussions');
    members = members || {};
    opts = opts || {};
    ...
    validator.check(visibility, {'code': 400, 'msg': 'An invalid discussion visibility option has been provided. Must be one of: ' + discussionsConstants.visibility.ALL.join(', ')}).isIn(DiscussionsConstants.visibility.ALL);

    // Verify each memberId and role is valid
    _.each(members, function(memberId, role) {
        validator.check(memberId, {'code': 400, 'msg': 'The memberId: ' + memberId + ' is not a valid member id'}).isPrincipalId();
        validator.check(role, {'code': 400, 'msg': 'The role: ' + role + ' is not a valid member role for a discussion'}).isIn(DiscussionConstants.roles.ALL_PRIORITY);
    });
...
var updateDiscussion = module.exports.updateDiscussion = function(ctx, discussionId, profileFields, callback) {
    var validator = new Validator();
    validator.check(discussionId, {'code': 400, 'msg': 'A content id must be provided'}).isResourceId();
    validator.check(null, {'code': 401, 'msg': 'You must be authenticated to update a discussion'}).isLoggedInUser(ctx);
    validator.check(fieldNames.length, {'code': 400, 'msg': 'You should at least one profile field to update'}).min(1);
    _.each(profileFields, function(value, field) {
        ...
        if (field === 'visibility') {
            validator.check(value, {'code': 400, 'msg': 'An invalid visibility was specified. Must be one of: ' + DiscussionsConstants.visibility.ALL.join(', ')}).isIn(DiscussionsConstants.visibility.ALL);
        }
    });
 ...
 */
var getDiscussions = module.exports.getDiscussions = function(ctx, principalId, start, limit, callback) {
    ...
    DiscussionsAPI.Authz.resolveTargetLibraryVisibility(ctx, principalId, function(err, visibility) {
        if (err) {
            return callback(err);
        }
        ...
        LibraryAPI.Index.list(LIBRARY_INDEX_NAME, principalId, visibility, start, limit, null, function(err, discussionIds, nextToken) {
            ...
            DiscussionsDAO.getDiscussionsById(discussionIds, function(err, discussions) {
                ...
                var orderedDiscussion = [];
                _.each(discussionIds, function(discussionId) {
                    var discussion = discussions[discussionId];
                    if (discussion) {
                        orderedDiscussion.push(discussion);
                    }
                });

                return callback(null, orderedDiscussions, nextToken);
            });
        });
    });
};
...
var getFullDiscussionProfile = module.exports.getFullDiscussionProfile = function(ctx, discussionId, callback) {
    ...
    _getDiscussion(discussionId, function(err, discussion) {
        ...
        DiscussionsAPI.Authz.resolveEffectiveDiscussionAccess(ctx, discussion, function(err, canView, canManage, canShare, canPost) {
            ...
            discussions.isManager = canManage;
            discussions.canShare = canShare;
            discussions.canPost = canPost;

            // Populate the creator of the discussion
            PrincipalsAPI.getUser(ctx, discussion.createdBy, function(err, creator) {
                if (err && err.code === 404) {
                    log().warn({
                        'err': err,
...
var shareDiscussion = module.exports.shareDiscussion = function(ctx, discussionId, principalIds, callback) {
    ...
    _getDiscussion(ctx, discussionId, function(err, discussion) {
        if (err) {
            return callback(err);
        }

        canShareDiscussion(ctx, discussion, function(err, canShare) {
            if (err) {
                return callback(err);
            } else if (!canShare) {
            ...
            AuthzAPI.getDirectRoles(principalIds, discussionId, function(err, roles) {
                ...
                // Ensure none of the principals we'll be sharing with violate tenant privacy boundaries
                var hasInvalidUser = false;
                _.each(principalIds, function(principalId) {
                    if (!DiscussionsAPI.Authz.canBecomeMemberOnDiscussion(principalId, discussion)) {
                        hasInvalidUser = true;
                    }
                });
                ...
                AuthzAPI.setRoles(discussionId, roleChanges, function(err) {
                    if (err) {
                        return callback(err);
                    }
                    ...
                    _touch(discussion, function(err, touchedDiscussion) {
                        if (err) {
                            log().warn({
                                ...
                                'discussionId': discussion.id
                            }, 'Error touching the discussion when sharing content. Ignoring.');
                        }
...
var setDiscussionPermissions = module.exports.setDiscussionPermissions = function(ctx, discussionId, permissionChanges, callback) {
    ...
    _getDiscussion(ctx, discussionId, function(err, discussion) {
        ...
        canSetDiscussionPermissions(ctx, discussion, function(err, canSetPermissions) {
            if (err) {
                return callback(err);
            ...
            // Ensure we aren't removing all managers from the discussion
            _computeMembershipAfterChanges(discussionId, permissionChanges, function(err, membershipAfterChanges, newMembers, updatedMembers, removedMembers) {
                if (err) {
                    return callback(err);
            ...
            if (_testDiscussionUpdateThreshold(discussion)) {
                ...
                _getAllMemberIds(discussion.id, function(err, memberIds) {
                    ...
                    _touch(discussion, function(err, updatedDiscussion) {
                        ...
                        _updateLibrary(memberIds, discussion, oldLastModified);
                    });
                });
            }

            // While the libraries are updating, we can create the message and return to the user
            MessageBoxAPI.createMessage(messageBoxId, ctx.user().id, body, {'replyToCreated': replyToCreatedTimestamp}, callback);
        });
    });
};
...
var deleteMessage = module.exports.deleteMessage = function(ctx, discussionId, messageCreatedDate, callback) {
    ...
    _getDiscussion(ctx, discussionId, function(err, discussion) {
        ...
        var messageBoxId = _createDiscussionMessageBoxId(discussionId);
        MessageBoxAPI.getMessages(messageBoxId, [messageCreatedDate], null, function(err, message) {
            if (err) {
                return callback(err);
            }

            message = message[messageCreatedDate];
            if (!message) {
            ...
            DiscussionsAPI.Authz.canDeleteDiscussionMessage(ctx, discussion, message, function(err, canDelete) {
                ...
                MessageBoxAPI.deleteMessage(messageBoxId, messageCreatedDate, {'deleteType': MessageBoxConstants.deleteTypes.LEAF}, function(err, deleteType, message) {
                    if (err) {
                        return callback(err);
                    }
 ...
 * @param  {Function}       callback                Invoked when the process completes
 * @param  {Object}         callback.err            An error that occurred, if any
 * @param  {Discussion[]}   callback.messages       The list of messages fetched
 */
var getMessages = module.exports.getMessages = function(ctx, discussionId, start, limit, callback) {
    ...
    getDiscussion(ctx, discussionId, function(err, discussion) {
        ...
        var messageBoxId = _createDiscussionMessageBoxId(discussionId);
        MessageBoxAPI.getMessagesFromMessageBox(messageBoxId, start, limit, null, function(err, messages) {
            if (err) {
                return callback(err);
            }
...
/**
 * Determine what the membership of a discussion would be after applying the specified permission changes to it.
 *
 * @param  {String}     discussionId            The id of the discussion to check
 * @param  {Object}     permissionChanges       An object that describes the permission changes to apply to the discussion. The key is the id of the principal to which to apply the change, and the value is the role to apply to the principal. If the value is `false`, the principal will be revoked access.
 * @param  {Function}   callback                Invoked when the test is complete
 * @param  {Object}     callback.err            An error that occurred, if any
 * @param  {Object}     callback.permissions    An object that describes the permissions on the discussion after the changes have been applied. The key is the id of the principal, and the value is the role of the principal
 * @param  {String[]}   callback.newMembers     The principal ids that would be new members to the discussion
 ...
 * @api private
 */
var _computeMembershipAfterChanges = function(discussionId, permissionChanges, callback) {
    AuthzAPI.getAuthzMembers(discussionId, null, 10000, function(err, memberIdsWithRolesArray) {
        if (err) {
            return callback(err);
        }
        ...
        _.each(permissionChanges, function(change, memberId) {
            if (change === false && memberIdsWithRoles[memberId]) {
                // The member `memberId` will be removed
                delete memberIdsWithRoles[memberId];
                removedMembers.push(memberId);
            } else if (memberIdsWithRoles[memberId] && memberIdsWithRoles[memberId] !== change) {
...
var _getAllMemberIds = function(discussionId, callback) {
    AuthzAPI.getAuthzMembers(discussionId, start, 10000, function(err, members) {
        if (err) {
            return callback(err);
        }
...
var _insertLibrary = function(principalIds, discussion, callback) {
    ...
    var effectiveLibraryVisibilities = DiscussionsAPI.Authz.resolveEffectiveLibraryVisibilities(principalIds, discussion);
...
/**
 * Create a messagebox id from a discussio id. It looks of the form:
 *
 *  "d:cam:dfjOSdfij#discussion"
 *
 * Where "d:cam:dfjOSdfij" is the id of the discussion.
 *
 * @param  {String}     discussionId    The id of the discussion from which to create a messagebox id
 * @return {String}                     The unique messagebox id for the discussion's messages
 */
var _createDiscussionMessageBoxId = function(discussionId) {
    return util.format('%s#discussion', discussionId);
};
