 ...
 * changes afterward.
 *
 *  TODO:   Passing in a `createFn` is not the ideal pattern. Would be better to have a central
...
var create = module.exports.create = function(ctx, roles, createFn, callback) {
    ...
    _getTargetRoles(roles, function(err, targetRoles) {
        if (err) {
            return callback(err);
        }
...
var share = module.exports.share = function(ctx, resource, targetIds, role, callback) {
    ...
    _getTargets(targetIds, function(err, targets) {
        if (err) {
            return callback(err);
        }
 ...
 * @param  {Function}       callback                Standard callback function
 * @param  {Object}         callback.err            An error that occurred, if any
 * @param  {ShareTarget[]}  callback.targetRoles    The share targets with their desired roles applied
 */
var _getTargetRoles = function(roles, callback) {
    _getTargets(_.keys(roles), function(err, targets) {
        if (err) {
            return callback(err);
        }
        ...
        _.each(targets, function(target) {
            var targetId = (target.principal) ? target.principal.id : target.email;
 ...
 * @param  {Function}       callback            Standard callback function
 * @param  {Object}         callback.err        An error that occurred, if any
 * @param  {ShareTarget[]}  callback.targets    The share targets that were parsed in the `targetIds` list
 * @api private
 */
var _getTargets = function(targetIds, callback) {
    var targets = _.chain(targetIds)
        .map(AuthzUtil.parseShareTarget)
        .compact()
        .value();
    var principalIds = _.chain(targets).pluck('principalId').compact().value();
