 ...
 */
var downloadRemoteFile = (module.exports.downloadRemoteFile = function(
  url,
  path,
  callback,
) {
  ...
  stream.on('error', function(err) {
    ...
    log().error(
      { err: err, url: url },
      'Unable to download the file due to a streaming error',
    );
    if (!called) {
      called = true;
      callback({
  ...
  var req = request(opts, function(err, response) {
    if (err) {
      log().error(
        { err: err, url: url },
        'Unable to download the file due to a request error.',
      );
      if (!called) {
        called = true;
 ...
 * @param  {Object}             callback.err            An error that occurred, if any
 */
var generatePreviewsFromImage = (module.exports.generatePreviewsFromImage = function(
  ctx,
  path,
  options,
  callback,
  ...
  options = options || {};
  options.removeInput = _.isBoolean(options.removeInput)
    ? options.removeInput
    : false;
...
var _resizeImages = function(ctx, path, sizes, callback) {
  ...
  gm(path).size(function(err, sourceSize) {
    if (err) {
      called = true;
      log().error(
        { err: err, path: path, contentId: ctx.content.id },
 ...
 * @api private
 */
var _cropIntelligently = function(
  ctx,
  path,
  width,
  height,
