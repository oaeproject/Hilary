...
var store = module.exports.store = function(ctx, opts, callback) {
    ...
    var uri = util.format('%d/%d/%d/%d/%d/%s/%s', date.getFullYear(), date.getMonth() + 1, date.getDate(), date.getHours(), date.getMinutes(), id, opts.filename);
...
var remove = module.exports.remove = function(ctx, uri, callback) {
    ...
    _getClient(ctx).DeleteObject(_getBucketName(ctx), uri, function(err) {
        if (err) {
            log().error({'err': err}, 'Could not delete from S3.');
            return callback(err);
        }
        callback(null, uri);
    });
