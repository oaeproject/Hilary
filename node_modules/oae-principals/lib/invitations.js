...
ResourceActions.when(ResourceConstants.events.ACCEPTED_INVITATION, function(ctx, invitationHashes, memberChangeInfosByResourceId, inviterUsersById, callback) {
    ...
    // Note that some of these group ids could be folder's authz ids. Therefore we need to limit to
    ...
    PrincipalsDAO.getPrincipals(groupIds, null, function(err, groupsById) {
        if (err) {
            ...
            return callback();
        } else if (_.isEmpty(groupsById)) {
            // If none of the group ids were principal groups, no need to continue
            return callback();
        }

        groupIds = _.keys(groupsById);
        var invitationHashesByResourceId = _.chain(invitationHashes)
            .indexBy('resourceId')
            .pick(_.keys(groupsById))
            .value();
        ...
        _touchAllGroups(groups, function(updatedGroupsById) {
            ...
            _.each(groups, function(group) {
                ...
                var invitationCtx = Context.fromUser(inviterUsersById[invitationHash.inviterUserId]);
                var memberChangeInfo = memberChangeInfosByResourceId[group.id];
                return PrincipalsEmitter.emit(PrincipalsConstants.events.UPDATED_GROUP_MEMBERS, invitationCtx, updatedGroup, group, memberChangeInfo);
            });
        });
    });
});
...
var _touchAllGroups = function(groups, callback) {
    ...
    var _done = _.chain(groups).size().after(function() {
