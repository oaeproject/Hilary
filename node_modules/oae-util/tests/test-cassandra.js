        ...
        it('verify paged column query start and end', function(callback) {
            ...
            Cassandra.createColumnFamily('VerifyPagedColumnQueryStartAndEnd', 'CREATE COLUMNFAMILY VerifyPagedColumnQueryStartAndEnd (keyId text PRIMARY KEY)', function(err, created) {
                ...
                Cassandra.runQuery(query.query, query.parameters, function(err) {
                    ...
                    Cassandra.runPagedColumnQuery('VerifyPagedColumnQueryStartAndEnd', 'keyId', 'key', null, 8, {'end': 'a'}, function(err, row, nextToken, startMatched) {
                        assert.ok(!err);
                        assert.ok(row);
                        ...
                        Cassandra.runPagedColumnQuery('VerifyPagedColumnQueryStartAndEnd', 'keyId', 'key', 'a', 8, {'end': 'a'}, function(err, row, nextToken, startMatched) {
                            assert.ok(!err);
                            assert.ok(row);
                            ...
                            Cassandra.runPagedColumnQuery('VerifyPagedColumnQueryStartAndEnd', 'keyId', 'key', 'a', 8, {'end': 'b'}, function(err, row, nextToken, startMatched) {
                                assert.ok(!err);
                                assert.ok(row);
                                assert.strictEqual(startMatched, true);
                                ...
                                Cassandra.runPagedColumnQuery('VerifyPagedColumnQueryStartAndEnd', 'keyId', 'key', null, 8, {'end': 'j'}, function(err, row, nextToken, startMatched) {
                                    assert.ok(!err);
                                    assert.ok(row);
                                    assert.strictEqual(startMatched, false);
                                    ...
                                    Cassandra.runPagedColumnQuery('VerifyPagedColumnQueryStartAndEnd', 'keyId', 'key', 'g', 8, {'end': 'j'}, function(err, row, nextToken, startMatched) {
                                        assert.ok(!err);
                                        assert.ok(row);
                                        assert.strictEqual(startMatched, true);
                                        ...
                                        Cassandra.runPagedColumnQuery('VerifyPagedColumnQueryStartAndEnd', 'keyId', 'key', null, 8, {'reversed': true, 'end': 'm'}, function(err, row, nextToken, startMatched) {
                                            assert.ok(!err);
                                            assert.ok(row);
                                            assert.strictEqual(startMatched, false);
                                            ...
                                            Cassandra.runPagedColumnQuery('VerifyPagedColumnQueryStartAndEnd', 'keyId', 'key', 'm', 8, {'reversed': true, 'end': 'm'}, function(err, row, nextToken, startMatched) {
                                                assert.ok(!err);
                                                assert.ok(row);
                                                assert.strictEqual(startMatched, true);
                                                ...
                                                Cassandra.runPagedColumnQuery('VerifyPagedColumnQueryStartAndEnd', 'keyId', 'key', 'm', 8, {'reversed': true, 'end': 'l'}, function(err, row, nextToken, startMatched) {
                                                    assert.ok(!err);
                                                    assert.ok(row);
                                                    assert.strictEqual(startMatched, true);
                                                    ...
                                                    // Verify inclusive end works with bounded start, multiple results full page (forward)
                                                    Cassandra.runPagedColumnQuery('VerifyPagedColumnQueryStartAndEnd', 'keyId', 'key', null, 8, {'reversed': true, 'end': 'c'}, function(err, row, nextToken, startMatched) {
                                                        assert.ok(!err);
                                                        assert.ok(row);
                                                        assert.strictEqual(startMatched, false);
                                                        ...
                                                        // Verify inclusive end works with bounded start, multple results partial page (forward)
                                                        Cassandra.runPagedColumnQuery('VerifyPagedColumnQueryStartAndEnd', 'keyId', 'key', 'f', 8, {'reversed': true, 'end': 'c'}, function(err, row, nextToken, startMatched) {
                                                            assert.ok(!err);
                                                            assert.ok(row);
                                                            assert.strictEqual(startMatched, true);
        ...
        it('verify paged column query paging forward and reverse', function(callback) {
            ...
            Cassandra.createColumnFamily('VerifyPagedColumnQueryPaging', 'CREATE COLUMNFAMILY VerifyPagedColumnQueryPaging (keyId text PRIMARY KEY)', function(err, created) {
                ...
                Cassandra.runQuery(query.query, query.parameters, function(err) {
                    ...
                    // Page by 8
                    Cassandra.runPagedColumnQuery('VerifyPagedColumnQueryPaging', 'keyId', 'key', null, 8, null, function(err, row, nextToken, startMatched) {
                        assert.ok(!err);
                        assert.ok(row);
                        assert.strictEqual(startMatched, false);
                        ...
                        // Start from 'h'
                        Cassandra.runPagedColumnQuery('VerifyPagedColumnQueryPaging', 'keyId', 'key', 'h', 8, null, function(err, row, nextToken, startMatched) {
                            assert.ok(!err);
                            assert.ok(row);
                            assert.strictEqual(startMatched, true);
                            ...
                            // Start from 'm'
                            Cassandra.runPagedColumnQuery('VerifyPagedColumnQueryPaging', 'keyId', 'key', 'm', 8, null, function(err, row, nextToken, startMatched) {
                                assert.ok(!err);
                                assert.ok(row);
                                assert.strictEqual(startMatched, true);
                                ...
                                Cassandra.runPagedColumnQuery('VerifyPagedColumnQueryPaging', 'keyId', 'key', null, 8, {'reversed': true}, function(err, row, nextToken, startMatched) {
                                    assert.ok(!err);
                                    assert.ok(row);
                                    assert.strictEqual(startMatched, false);
                                    ...
                                    // Start from 'f'
                                    Cassandra.runPagedColumnQuery('VerifyPagedColumnQueryPaging', 'keyId', 'key', 'f', 8, {'reversed': true}, function(err, row, nextToken, startMatched) {
                                        assert.ok(!err);
                                        assert.ok(row);
                                        assert.strictEqual(startMatched, true);
                                        ...
                                        // Start from 'a'
                                        Cassandra.runPagedColumnQuery('VerifyPagedColumnQueryPaging', 'keyId', 'key', 'a', 8, {'reversed': true}, function(err, row, nextToken, startMatched) {
                                            assert.ok(!err);
                                            assert.ok(row);
                                            assert.strictEqual(startMatched, true);
        ...
        it('verify multi-byte character in paged column query', function(callback) {
            Cassandra.createColumnFamily('VerifyMultiBytePagedColumnQuery', 'CREATE COLUMNFAMILY VerifyMultiBytePagedColumnQuery (keyId text PRIMARY KEY, colOne text, colTwo text)', function(err, created) {
                ...
                Cassandra.runQuery('INSERT INTO VerifyMultiBytePagedColumnQuery (keyId, colOne) VALUES (?, ?) USING CONSISTENCY QUORUM', ['key1', stringWithMultiByte], function(err) {
                    ...
                    Cassandra.runPagedColumnQuery('VerifyMultiBytePagedColumnQuery', 'keyId', 'key1', null, 10, null, function(err, row) {
                        assert.ok(!err);
                        assert.equal(row.count, 1);
                        assert.equal(row[0].value, stringWithMultiByte);
                        return callback();
                    });
                });
            });
