...
var setupPassportSerializers = function() {
    ...
    passport.deserializeUser(function(principalId, callback) {
        // We store the user ID in the session store, which is stored as an encrypted cookie
        principalId = _decryptCookieData(principalId);

        // We retrieve the user object via the principal utility as that will include the signed
        // profile picture URLs
        var tenantAlias = AuthzUtil.getPrincipalFromId(principalId).tenantAlias;
        var tenant = TenantsAPI.getTenant(tenantAlias);
        var ctx = new Context(tenant, new User(tenantAlias, principalId));
        PrincipalsUtil.getPrincipal(ctx, principalId, function (err, user) {
            if (err && err.code === 404) {
