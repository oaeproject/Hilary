...
module.exports.setupServer = function(port, config) {
    ...
    _applyShutdownHandling(server, app, port);
    ...
    // We pass in the config.files hash so we can configure where
    // the uploaded files will be temporarily stored.
    app.use(express.bodyParser(config.files));
    ...
    app.use(express.session({'secret': config.cookie.secret, 'store': redisStore}));
 ...
 */
module.exports.postInitializeServer = function(app) {
...
/**
 * Apply the logic and request handling required to gracefully shut down the web server such that user requests
 * complete safely and new requests fail over to other application nodes.
 *
 ...
 */
var _applyShutdownHandling = function(server, app, port) {
    ...
    // Notify the front-end proxy that we're shutting down, this should cause persistent connections to force-close
    // and a proxy to gracefully push bounce the request to a new node
    app.use(function(req, res, next) {
        if (isShuttingDown) {
