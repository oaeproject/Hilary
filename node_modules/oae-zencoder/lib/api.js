...
var ZencoderAPI = module.exports = new EmitterAPI.EventEmitter();
...
var encodingCompleted = module.exports.encodingCompleted = function(notification, callback) {
    ...
    if (job.state !== 'finished') {
        return callback({'code': 500,'msg': util.format('Zencoder could not transcode file with zencoderId %s', zencoderId)});
    } else if (!job.pass_through) {
        return callback({'code': 500, 'msg': util.format('Zencoder notification for zencoderId %s is missing S3 file path', zencoderId)});
    } else if (output.state === 'failed') {
        return callback({'code': 500, 'err': output.error_class ,'msg': util.format('Zencoder job for zencoderId %s failed with the following message: %s', zencoderId, output.error_message)});
    }
...
var _createAndSavePreviews = function(contentRevision, output, key, callback) {
    ...
    var revisionId = contentRevision.revisionId;
    var ctx = new PreviewContext(PreviewProcessorAPI.getConfiguration(), contentId, revisionId);

    log().info({'contentId': contentId, 'revisionId': revisionId}, 'Received encoding completed notification from Zencoder');
    ...
    ctx.login(function(err) {
        ...
        ctx.getContentData(function(err) {
            if (err) {
                return callback(err);
            } else if (ctx.revision.previews && ctx.revision.previews.total > 0) {
                return callback({'code': 500, 'msg': 'Previews have already been created for this revision'});
            }

            ctx.addPreviewMetadata('videoKey', key);
            if (output.duration_in_ms) {
                ctx.addPreviewMetadata('duration', (output.duration_in_ms / 1000));
            }

            var thumbnail = output.thumbnails ? _.findWhere(output.thumbnails, {'label': 'base'}) : null;

            if (thumbnail && _.isArray(thumbnail.images)) {
                return _createThumbnails(ctx, _.first(thumbnail.images), callback);
            }
            // If there are no thumbnail images, just save the video url
            log().info('No thumbnails found for ' + contentId);
            ctx.savePreviews(function(err) {
                if (err) {
                    callback(err);
                }
                ZencoderAPI.emit(PreviewConstants.EVENTS.PREVIEWS_FINISHED, ctx.content, ctx.revision, ctx.getStatus());
                return callback();
            });
        });
    });
...
var _createThumbnails = function(ctx, thumbnailImage, callback) {
    ...
    PreviewUtil.downloadRemoteFile(thumbnailImage.url, util.format('%s/base.png', ctx.baseDir), function(err, path) {
        ...
        PreviewUtil.generatePreviewsFromImage(ctx, path, {'removeInput': true}, function(err) {
            ...
            log().info({'contentId': ctx.contentId, 'revisionId': ctx.revisionId}, 'Saved preview and thumbnails for video content');
            ctx.savePreviews(function(err) {
                if (err) {
                    callback(err);
                }
