...
describe('Principals', function() {
    ...
    describe('Utilities', function() {
        ...
        var createGroups = function(ctx, userids, callback) {
            var r = Math.round(Math.random() * 1000000);
            var groupids = [];
            PrincipalsAPI.createGroup(ctx, 'oae-team-' + r, 'OAE Team', 'public', function(err, groupId) {
                assert.ok(!err);
                groupids.push(groupId);
                PrincipalsAPI.createGroup(ctx, 'backend-team-' + r, 'Back-end Team', 'public', function(err, groupId) {
                    assert.ok(!err);
                    groupids.push(groupId);
                    PrincipalsAPI.createGroup(ctx, 'ui-team-' + r, 'UI Team', 'public', function(err, groupId) {
                        assert.ok(!err);
                        groupids.push(groupId);
                        callback(groupids);
                    });
                    ...
                    PrincipalsUtil.getPrincipals(ctx, [userids[0], userids[1], userids[2]], function(err, users) {
                        ...
                        PrincipalsUtil.getPrincipals(ctx, [groupids[0], groupids[1]], function(err, groups) {
                            ...
                            PrincipalsUtil.getPrincipals(ctx, [groupids[0], userids[0], userids[1], groupids[1], userids[3]], function(err, principals) {
                                ...
                                PrincipalsUtil.getPrincipals(ctx, [userids[0], 'non-existing-user', userids[2]], function(err, users, missingUsers) {
                                    assert.ok(!err);
                                    assert.equal(_.keys(users).length, 2);
                                    assert.equal(missingUsers.length, 1);
                                    // Get existing groups, of which some don't exist
                                    PrincipalsUtil.getPrincipals(ctx, [groupids[0], 'non-existing-group'], function(err, groups, missingGroups) {
                                        assert.ok(!err);
                                        ...
                                        PrincipalsUtil.getPrincipals(ctx, [groupids[0], userids[0], 'non-existing-user', userids[1], 'non-existing-group'], function(err, principals, missingPrincipals) {
                                            assert.ok(!err);
                                            assert.equal(_.keys(principals).length, 3);
                                            assert.equal(missingPrincipals.length, 2);
                                            callback();
                                        });
                                    });
                                });
