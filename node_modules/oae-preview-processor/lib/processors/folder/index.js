...
var _getContentWithPreviews = function(folder, callback, _contentWithPreviews, _start) {
    ...
    FoldersDAO.getContentItems(folder.groupId, {'start': _start, 'limit': 20}, function(err, contentItems, nextToken) {
        if (err) {
            return callback(err);
        }
        ...
        _contentWithPreviews = _.chain(contentItems)
            ...
            .filter(function(contentItem) {
                ...
                // library)
                var targetBucketVisibility = LibraryAPI.Authz.resolveLibraryBucketVisibility(folder.id, contentItem);
                var targetBucketVisibilityPriority = AuthzConstants.visibility.ALL_PRIORITY.indexOf(targetBucketVisibility);
                ...
                var implicitBucketVisibilityPriority = AuthzConstants.visibility.ALL_PRIORITY.indexOf(implicitBucketVisibility);
