...
describe('Emails', function() {
    ...
    describe('Email de-duplication', function() {
        ...
        /**
         * Test that verifies that a unique id can be used to perform de-duplication
         */
        it('verify an id can be used to perform de-duplication', function(callback) {
            EmailTestsUtil.collectAndFetchEmails(function() {
                TestsUtil.generateTestUsers(camAdminRestContext, 1, function(err, users, simong) {
                    ...
                    EmailTestsUtil.sendEmail('oae-tests', 'TestTemplate', simong.user, null, {'id': 'u:cam:simong#123456'}, function(err, message) {
                        assert.ok(!err);
                        assert.ok(message);

                        // Re-using the same id should result in a failure
                        EmailTestsUtil.sendEmail('oae-tests', 'TestTemplate', simong.user, null, {'id': 'u:cam:simong#123456'}, function(err) {
                            assert.ok(err);
                            assert.equal(err.code, 403);

                            // Re-using the same id, but with the same mail should result in a failure
                            // We generate a "different" mail by passing in a data object
                            EmailTestsUtil.sendEmail('oae-tests', 'TestTemplate', simong.user, {'data': 'test'}, {'id': 'u:cam:simong#123456'}, function(err) {
                                assert.ok(err);
                                assert.equal(err.code, 403);

                                // Using another id (but otherwise the same mail) should work
                                EmailTestsUtil.sendEmail('oae-tests', 'TestTemplate', simong.user, null, {'id': 'u:cam:mrvisser#000000'}, function(err, message) {
                                    assert.ok(!err);
                                    assert.ok(message);
                                    return callback();
                                });
         ...
         */
        it('verify omitting the id uses the email info for deduplication', function(callback) {
            TestsUtil.generateTestUsers(camAdminRestContext, 1, function(err, users, simong) {
                assert.ok(!err);
        ...
        it('verify deduplication is limited to a configurable interval', function(callback) {
            TestsUtil.generateTestUsers(camAdminRestContext, 1, function(err, users, simong) {
                assert.ok(!err);
                ...
                EmailAPI.init(_createDefaultConfig({'deduplicationInterval': 5}), function(err) {
                    assert.ok(!err);
                    // Send out the first e-mail
                    EmailTestsUtil.sendEmail('oae-tests', 'TestTemplate', simong.user, null, null, function(err, message) {
                        assert.ok(!err);
