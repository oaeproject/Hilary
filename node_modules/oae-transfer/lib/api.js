...
var initiateTransfer = module.exports.initiateTransfer = function (ctx, originalUserId, originalEmail, targetEmail, callback) {
    ...
    TransferDAO.initiateTransfer(ctx, code, originalUserId, originalEmail, targetEmail, function(err) {
        if (err) {
            return callback(err);
        }

        getTransferById(ctx, originalUserId, function(err, transfer) {
            if (err) {
                return callback(err);
            }
...
var getTransferById = module.exports.getTransferById = function (ctx, originalUserId, callback) {
    ...
    TransferDAO.getTransferById(originalUserId, function(err, transfer) {
        ...
        if (_.isEmpty(transfer)) {
            var err = {'code': 412, 'msg': 'Precondition failed'};
            return callback(err);
        }
        ...
        _completeTransfer(ctx, originalEmail, code, targetEmail, targetUserId, transfer, function(err, members) {
            ...
            if (_.isEmpty(members)) { 
                ...
                _parseData(members, function(err, parsedData) {
                    ...
                    _sendEmail(ctx, parsedData, targetUserId, originalEmail, targetEmail, function(err) {
                        if (err) {
                            return callback(err);
                        }
                    });
 ...
 * @param  {Object[]}       callback.members            All the members of user resource
 */
 var _completeTransfer = function (ctx, originalEmail, code, targetEmail, targetUserId, transfer, callback) {
    ...
    TransferDAO.getAuthzRoles(transfer.originalUserId, function(err, authzRoles) {
        ...
        _transferRoles(ctx, authzRoles, targetUserId, transfer.originalUserId, function(err, newAuthzRoles) {
            ...
            TransferDAO.completeTransfer(code, transfer.originalUserId, originalEmail, targetEmail, targetUserId, function (err) {
                if (err) {
                    return callback(err);
                }
...
var _getDocument = function (ctx, resourceId, callback) {
    ...
    switch (resourceType) {
        case 'c':
            ...
            break;
        case 'g':
            TransferDAO.getFolderOrGroupByGroupId(ctx, resourceId, function(err, result) {
                if (err) {
                    return callback(err);
                }
                return callback(null, result);                
 ...
 */
var _transferRoles = function(ctx, authzRoles, targetUserId, originalUserId, callback) {
    ...
    async.eachSeries(authzRoles, function (authzRole, innerCallback) {
            ...
            if (doUpdate) {
                ...
                AuthzAPI.updateRoles(authzRole.resourceId, memberToUpdate, function(err, usersToInvalidate) {
                    ...
                    _transferResourceLibrary(ctx, authzRole.resourceId, targetUserId, originalUserId, function (err, result) {
                        ...
                        newAuthzRole['resourceId'] = authzRole.resourceId;
                        newAuthzRole['role'] = authzRole.role;
                        newAuthzRole['principalId'] = targetUserId;
                        newAuthzRoles.push(newAuthzRole);
        ...
        return callback(null, newAuthzRoles);
    });
};
...
var _transferResourceLibrary = function (ctx, resourceId, targetUserId, originalUserId, callback) {
    ...
    _getDocument(ctx, resourceId, function (err, resource) {
        ...
        // Insert to library 
        _addToLibrary(resource, targetUserId, libraryIndexName, function(err) {
            if (err) {
                return callback(err);
            }
 ...
 */
var _addToLibrary = function(obj, principalId, constant, callback) {
    var entries = [{
        'id': principalId,
 ...
 * @param  {Function}   callback                            Standard callback function
 * @param  {Object}     callback.err                        An error that occurred, if any
 * @param  {Object}     callback.listOfResourcesByMember   List of resources by members
 */
var _parseData = function(associationMemberResource, callback) {

    // Create another list make a list of resource by user 
    var listOfResourcesByMember = [];

    async.eachSeries(associationMemberResource, function (memberAndResource, outerCallback) {

        var member = memberAndResource[0];
        var resource = memberAndResource[1];

        if (_.isEmpty(listOfResourcesByMember)) {
            listOfResourcesByMember.push({'profile': member.profile, 'resources': []});
        }

        var userFoundOnTheList = false;
        var iterate = 0;
        
        async.eachSeries(listOfResourcesByMember, function (element, innerCallback) {
            
            // If the user is found on 'listOfResourcesByMember', add the resource to user 
            ...
            if (!userFoundOnTheList) {
                listOfResourcesByMember.push({'profile': member.profile, 'resources': []});
                listOfResourcesByMember[iterate].resources.push(resource);   
            }

            return outerCallback();
        });
    }, function () {
        return callback(null, listOfResourcesByMember);
    });
};
