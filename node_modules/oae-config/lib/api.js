...
var cacheTenantConfig = function(tenantAlias, callback) {
    ...
    Cassandra.runQuery('SELECT "configKey", "value", writetime("value") as wt FROM "Config" WHERE "tenantAlias" = ?', [tenantAlias], function(err, rows) {
        if (err) {
            return callback(err);
        }
        ...
        _.each(rows, function(row) {
            persistentConfig[row.configKey] = {
                'value': row.value,
                'timestamp': Cassandra.bigintTimestampToDate(row.wt)
            };
        });
