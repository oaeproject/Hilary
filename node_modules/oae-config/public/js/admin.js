...
(function(){
    ...
    var preProcessTenants = function(tenants, callback) {
        ...
        // Check if the tenant can change the details of a server
        if (configuration['oae-tenants'] &&
            configuration['oae-tenants'].config &&
            configuration['oae-tenants'].config.options &&
            configuration['oae-tenants'].config.options['allow-tenant-change-details'] &&
            configuration['oae-tenants'].config.options['allow-tenant-change-details'].elements &&
            configuration['oae-tenants'].config.options['allow-tenant-change-details'].elements['allow-tenant-change-details-details']) {
                tenants.info.canChangeDetails = configuration['oae-tenants'].config.options['allow-tenant-change-details'].elements['allow-tenant-change-details-details'].value;
        }
    ...
    var getCtx = function(meData, callback) {
        ...
        if (meData.tenant === 'global') {
            // If the tenant is global, do a check to see if a tenant is loaded
            ...
            if (adminContext.tenant.context === adminContext.tenant.tenantId) {
                adminContext.tenant.isEditingGlobal = true;
            }

        } else {
            // If not global, it can only be a tenant administration page
            adminContext.tenant = {
                'context': 'tenant',
                'tenantId': meData.tenant
            }
        }

        // Returns true if the server accessed is not global server
        adminContext.tenant.isNotGlobalServer = function() {
            return adminContext.tenant.context !== 'tenant' &&
                       (adminContext.tenant.context === 'global' &&
                        adminContext.tenant.context !==  adminContext.tenant.tenantId);
        };

        if (meData.anon) {
            return callback(meData);
        }

        if (adminContext.tenant.tenantId === 'global') {
            // We don't need to fetch the global tenant information
            // since we'll only show global modules and other tenants, not global server info
            callback(adminContext)
        } else {
            var url = '/api/tenant';
            if (adminContext.tenant.isNotGlobalServer()) {
                url += '/' + adminContext.tenant.tenantId;
            }
            $.ajax({
                url: url,
                success: function(data) {
                    adminContext.tenant.active = data.active,
                    adminContext.tenant.baseUrl = data.baseUrl,
                    adminContext.tenant.name = data.name || '',
                    adminContext.tenant.description = data.description || '',
                    adminContext.tenant.port = data.port || ''
    ...
    var addBinding = function() {
        ...
        $adminModulesContainer.on('click', '.expandableobject_trash', setTableRowDeleted);
        $adminModulesContainer.on('click', '.expandableobject_undo_trash', setTableRowDeleted);
        $adminModulesContainer.on('click', '.expandableobject_add_new', addTableRow);
    };
    ...
    var initializeFooter = function() {
        if (cachedTenants.length) {
            ...
            $('#admin_content').css('padding-bottom', $('#admin_footer_container').height() + 20);
        } else if (adminContext.tenant.context === 'global') {
    ...
    var initializeTenants = function(callback) {
        if (adminContext.tenant.tenantId === 'global') {
            ...
            preProcessTenants([{
                ...
                'baseUrl': adminContext.tenant.baseUrl,
