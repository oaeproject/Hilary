...
let ConfigAPI = require('oae-config');
let log = require('oae-logger').logger('oae-authentication');
let TenantsUtil = require('oae-tenants/lib/util');

let AuthenticationAPI = require('oae-authentication');

let AuthenticationConfig = ConfigAPI.config('oae-authentication');
let AuthenticationConstants = require('oae-authentication/lib/constants').AuthenticationConstants;
let AuthenticationUtil = require('oae-authentication/lib/util');

let CasStrategy = require('passport-cas').Strategy;
...
module.exports = function() {
  ...
  strategy.getPassportStrategy = function(tenant) {
    ...
    var casHost = AuthenticationConfig.getValue(
      tenant.alias,
      AuthenticationConstants.providers.CAS,
      'url'
    );
    var loginPath = AuthenticationConfig.getValue(
      tenant.alias,
      AuthenticationConstants.providers.CAS,
      'loginPath'
    );
    var validatePath = AuthenticationConfig.getValue(
      tenant.alias,
      AuthenticationConstants.providers.CAS,
      'validatePath'
    );
    var mapDisplayName = AuthenticationConfig.getValue(
      tenant.alias,
      AuthenticationConstants.providers.CAS,
      'mapDisplayName'
    ).toLowerCase();
    var mapEmail = AuthenticationConfig.getValue(
      tenant.alias,
      AuthenticationConstants.providers.CAS,
      'mapEmail'
    ).toLowerCase();
    var mapLocale = AuthenticationConfig.getValue(
      tenant.alias,
      AuthenticationConstants.providers.CAS,
      'mapLocale'
    ).toLowerCase();
    var useSaml = AuthenticationConfig.getValue(
      tenant.alias,
      AuthenticationConstants.providers.CAS,
      'useSaml'
    );

    let serverBase = TenantsUtil.getBaseUrl(tenant);
    ...
    var passportStrategy = new CasStrategy(
      ...
      (req, casResponse, done) => {
        ...
        if (casResponse.attributes) {
          ...
          var mappedDisplayName = AuthenticationUtil.renderTemplate(
            mapDisplayName,
  ...
  strategy.logout = function(req, res) {
    ...
    var logoutUrl = AuthenticationConfig.getValue(
      tenant.alias,
      AuthenticationConstants.providers.CAS,
      'logoutUrl'
    );
