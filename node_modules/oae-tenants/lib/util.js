 ...
 *
 *  * The specified user and any principals in the list of principalIds; OR
 *  * The specified tenant and any principals in the list of principalIds.
 *
 * This does not resolve tenant boundary violations between the specified user and specified tenant
 * as that is more complex and handled differently.
 *
 * This only checks if there are any violations between two *tenants*. This does *NOT* take the
 * principal visibility into account.
 *
 ...
 */
var resolveTenantBoundaryViolations = module.exports.resolveTenantBoundaryViolations = function(user, tenantAlias, principalIds) {
    // Check tenant privacy between the current user in context, discussion and target principals
    return _.filter(principalIds, function(principalId) {
        ...
        return (!canInteract(user.tenant.alias, principalTenantAlias) || !canInteract(tenantAlias, principalTenantAlias));
    });
};
