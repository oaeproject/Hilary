...
AuthzGraph.prototype = Object.create(Graph.prototype);
 ...
 *                              id already existed
 */
AuthzGraph.prototype.addNode = function(nodeId) {
 ...
 *                              existed
 */
AuthzGraph.prototype.addEdge = function(fromId, toId) {
 ...
 *                                          be the last id returned to the `forEachCallback`
 * @param  {Function}   forEachCallback     Invoked for every node traversed over in the inbound
 *                                          edge search
 */
AuthzGraph.prototype.inboundForEach = function(nodeId, forEachCallback, _visited) {
    if (_visited[nodeId]) {
        return;
    } else {
    ...
    _.each(edges, function(edge) {
        inboundForEach(edge.from.id, forEachCallback, _visited);
    });

    // Finally invoke the callback with the current node
    forEachCallback(nodeId);
};
