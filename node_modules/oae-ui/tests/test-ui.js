...
describe('UI', function() {
  ...
  describe('Widget config aggregation', function() {
    ...
    it('verify widget configs', function(callback) {
      ...
      RestAPI.UI.getWidgetManifests(anonymousGlobalRestContext, function(
        err,
        data,
      ) {
        assert.ok(!err);
        ...
        RestAPI.UI.getWidgetManifests(anonymousCamRestContext, function(
          err,
          data,
        ) {
          assert.ok(!err);
  ...
  describe('Batch static files', function() {
    ...
    it('verify batch static get', function(callback) {
      var files = [
        '/ui/index.html',
        '/node_modules/oae-core/footer/js/footer.js',
        '/nonexisting',
      ];
      // Get these files on the global admin server
      RestAPI.UI.getStaticBatch(anonymousGlobalRestContext, files, function(
        err,
        batch1,
      ) {
        assert.ok(!err);
        ...
        RestAPI.UI.getStaticBatch(anonymousCamRestContext, files, function(
          err,
          batch2,
        ) {
          assert.ok(!err);
          ...
          files = [
            '/node_modules/oae-core/footer/css/footer.css',
            '/ui/index.html',
          ];
          // Get these files on the global admin server
          RestAPI.UI.getStaticBatch(anonymousGlobalRestContext, files, function(
    ...
    it('verify batch single file', function(callback) {
      ...
      RestAPI.UI.getStaticBatch(anonymousGlobalRestContext, file, function(
        err,
        data,
      ) {
        assert.ok(!err);
        ...
        RestAPI.UI.getStaticBatch(anonymousCamRestContext, file, function(
          err,
          data,
        ) {
          assert.ok(!err);
    ...
    it('verify validation', function(callback) {
      ...
      RestAPI.UI.getStaticBatch(anonymousGlobalRestContext, null, function(
        err,
        data,
      ) {
        assert.ok(err);
        ...
        assert.ok(!data);
        RestAPI.UI.getStaticBatch(anonymousGlobalRestContext, [], function(
          err,
          data,
        ) {
          assert.ok(err);
          ...
          RestAPI.UI.getStaticBatch(anonymousGlobalRestContext, file, function(
            ...
            RestAPI.UI.getStaticBatch(anonymousCamRestContext, null, function(
              err,
              data,
            ) {
              assert.ok(err);
              assert.equal(err.code, 400);
              assert.ok(!data);
              RestAPI.UI.getStaticBatch(anonymousCamRestContext, [], function(
                err,
                data,
              ) {
                assert.ok(err);
  ...
  describe('Skinning', function() {
    ...
    var checkSkin = function(restCtx, expectedBackgroundColor, callback) {
      RestAPI.UI.getSkin(restCtx, function(err, css, response) {
        ...
        var bodyBackgroundColorRegex = new RegExp(
          'body{background-color:#([0-9a-zA-Z]+)}',
        );
     ...
     * @api private
     */
    var updateSkinAndCheck = function(
      restCtx,
      skinConfig,
    ...
    var checkVariables = function(
      ...
      RestAPI.UI.getSkinVariables(globalAdminRestContext, tenantAlias, function(
        err,
        data,
      ) {
        assert.ok(!err);
    ...
    it('verify only administrators can retrieve skin variabes', function(callback) {
      RestAPI.UI.getSkinVariables(
        ...
        function(err, data) {
          ...
          RestAPI.UI.getSkinVariables(anonymousCamRestContext, null, function(
            err,
            data,
          ) {
            assert.equal(err.code, 401);
            RestAPI.UI.getSkinVariables(
              ...
              function(err, data) {
                assert.ok(!err);
                RestAPI.UI.getSkinVariables(camAdminRestContext, null, function(
                  err,
                  data,
                ) {
                  assert.ok(!err);
    ...
    it('verify skin url variables are overridden by hash file mappings', function(callback) {
      ...
      TestsUtil.createTenantWithAdmin(testTenantAlias, testTenantHost, function(
        err,
        testTenant,
        testTenantAdminRestContext,
      ) {
        assert.ok(!err);
        ...
        RestAPI.UI.getSkinVariables(
          ...
          function(err, variables) {
            ...
            var defaultLogoUrl = _getSkinVariableValue(
                  ...
                  UITestUtil.updateSkinAndWait(
                    ...
                    function(err) {
                      ...
                      RestAPI.UI.getSkin(testTenantAdminRestContext, function(
                        ...
                        skinConfig = {
                          'institutional-logo-url': "  '  /test/directory  '  ",
                        };
                        UITestUtil.updateSkinAndWait(
                          globalAdminRestContext,
                          testTenantAlias,
     ...
     */
    it('verify institutional-logo-url is templated', function(callback) {
          ...
          function(err) {
            ...
            RestAPI.UI.getSkinVariables(
              ...
              function(err, variables) {
                ...
                var institutionalLogoUrlValue = _getSkinVariableValue(
                  'institutional-logo-url',
                  variables,
                );
                assert.strictEqual(
                ...
                RestAPI.UI.getSkin(testTenantAdminRestContext, function(
                  ...
                  var expectedInstitutionalLogoStr = util.format(
                    '.oae-institutional-logo{background-image:url(/assets/%s/logo/%s.png)}',
                    testTenantAlias,
                    testTenantAlias,
                  );
                  assert.notEqual(
                    css.indexOf(expectedInstitutionalLogoStr),
                    -1,
                  );
                  ...
                  RestAPI.UI.getLogo(
                    ...
                    (err, logoURL) => {
                      ...
                      assert.notEqual(
                        expectedInstitutionalLogoStr.indexOf(logoURL),
                        -1,
                      );
                      return callback();
                    },
     ...
     */
    it('verify logo can be uploaded for a tenant', function(callback) {
      var tenantAlias = global.oaeTests.tenants.cam.alias;
      var filePath = __dirname + '/data/oae-logo.png';
      var fileStream = fs.createReadStream(filePath);

      // Assert that the global admin can change the logo for a tenant
      RestAPI.UI.uploadLogo(
        globalAdminRestContext,
        fileStream,
        tenantAlias,
        function(err, data) {
