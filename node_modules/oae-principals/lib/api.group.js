...
/**
 * Performs an access check on whether or not the current user should be able to see the group.
 * If the current user has access to the group, the group object will be augmented
 * with fields that indicate whether the current user is a member and/or manager
 * of the specified group or if he can join it.
 *
 * @param  {Context}   ctx             The current context.
...
var createGroup = module.exports.createGroup = function(ctx, displayName, description, visibility, joinable, members, otherValues, callback) {
    ...
    var groupId = AuthzUtil.toId(Constants.PRINCIPAL_TYPE_GROUP, tenant.alias, ShortId.generate());
    ...
    canCreateGroup(ctx, groupId, function(err) {
        ...
        Cassandra.runQuery('INSERT INTO Principals (principalId, tenantAlias, displayName, description, visibility, joinable) VALUES (?, ?, ?, ?, ?, ?) USING CONSISTENCY QUORUM',
            [groupId, tenant.alias, displayName, description, visibility, joinable], function (err) {
            if (err) {
                return callback(err);
            }
            ...
            var group = new Group(tenant.alias, groupId, displayName, opts);
            _setGroupMembers(ctx, group, members, function(err) {
