...
var createContent = module.exports.createContent = function(contentId, revisionId, createdBy, resourceSubType, displayName, description, visibility, otherValues, revisionData, callback) {
    ...
    RevisionsDAO.createRevision(revisionId, contentId, createdBy, revisionData, function(err, revision) {
        ...
        Cassandra.runQuery(q.query, q.parameters, function(err) {
            ...
            var contentObj = new Content(tenantAlias, contentId, visibility, displayName, description, resourceSubType, parameters.createdBy, parameters.created, null, revision.revisionId, {'status': 'pending'}, otherValues.extra);
...
var _updateContentLibraries = function(contentObj, oldLastModified, newLastModified, removedMembers, callback) {
    ...
    getAllContentMembers(contentObj.id, function(err, members) {
        ...
        LibraryAPI.Index.update(ContentConstants.library.CONTENT_LIBRARY_INDEX_NAME, libraryVisibility, contentObj.id, newLastModified, oldLastModified, function(err) {
            if (err) {
                errs.push(err);
            }
            ...
            if (!_.isEmpty(removedMembers)) {
                ...
                LibraryAPI.Index.remove(ContentConstants.library.CONTENT_LIBRARY_INDEX_NAME, removedMembers, contentObj.id, oldLastModified, function(err) {
                    if (err) {
                        errs.push(err);
                    }
...
var _rowToContent = function(row) {
    ...
    var contentObj = new Content(hash.tenantAlias, hash.contentId, hash.visibility, hash.displayName, hash.description, hash.resourceSubType, hash.createdBy, hash.created, hash.lastModified, hash.latestRevisionId, hash.previews, hash.extra);
 ...
 * @api private
 */
var _parseExtra = function(hash) {
    try {
        hash.extra = JSON.parse(hash.extra);
    } catch (ex) {
        hash.extra = {};
//        log().warn({'hash': hash}, 'Could not parse the content extra object');
    }
};
