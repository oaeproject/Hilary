...
OAE.tenantRouter.on('post', '/api/auth/shibboleth', function(req, res, next) {
  ...
  var redirectUrl = AuthenticationUtil.validateRedirectUrl(
    req.param('redirectUrl'),
  );
...
OAE.tenantRouter.on('get', '/api/auth/shibboleth/sp', function(req, res, next) {
  if (ShibbolethAPI.getSPHost() !== req.hostname) {
    return res
      .status(501)
      .send('This endpoint is not enabled on a regular tenant');
  }
  ...
  ShibbolethAPI.validateInitiateParameters(
    tenantAlias,
    signature,
    expires,
    function(err, tenant) {
      ...
      var strategyId = AuthenticationUtil.getStrategyId(
        tenant,
 ...
 * @Return      {void}
 */
OAE.tenantRouter.on('get', '/api/auth/shibboleth/sp/callback', function(
  req,
  res,
  next,
) {
    ...
    passport.authenticate(strategyId, {}, function(
      ...
      if (err) {
        log().error(
          ...
          'Error during Shibboleth authentication',
        );
        ...
        return res.redirect(
          tenantUrl + '/?authentication=failed&reason=tampering',
        );
      }
      ...
      // along their user id
      var redirectUrl = ShibbolethAPI.getAuthenticatedUserRedirectUrl(
        tenant,
        user,
      );
 ...
 * @Return      {void}
 */
OAE.tenantRouter.on('get', '/api/auth/shibboleth/callback', function(
  req,
  res,
  next,
) {
  ...
  var signature = req.query.signature;
  var expires = req.query.expires;
  ShibbolethAPI.getUser(req.tenant, userId, signature, expires, function(
    err,
    user,
  ) {
    if (err) {
