...
describe('Emails', () => {
  ...
  describe('Templates', () => {
    ...
    it('verify sendEmail validation', callback => {
      generateTestUsers(asCambridgeTenantAdmin, 1, (err, users) => {
        ...
        sendEmail(EMAIL_MODULE, TEST, johnDoeUser, NO_DATA, NO_OPTS, (err /* , message */) => {
          assert.ok(err);
          ...
          sendEmail(EMAIL_MODULE, TEST, johnDoeUser, NO_DATA, NO_OPTS, (err /* , message */) => {
            ...
            sendEmail(EMAIL_MODULE, TEST, null, NO_DATA, NO_OPTS, (err /* , message */) => {
              assert.ok(err);
              assert.strictEqual(err.code, 400);
              ...
              sendEmail(null, TEST, johnDoeUser, NO_DATA, NO_OPTS, (err /* , message */) => {
                assert.ok(err);
                assert.strictEqual(err.code, 400);

                // Verify error when there is no template id
                sendEmail(EMAIL_MODULE, null, johnDoeUser, NO_DATA, NO_OPTS, (err /* , message */) => {
                  assert.ok(err);
                  assert.strictEqual(err.code, 400);

                  // Verify error with non-existent module
                  sendEmail('oae-non-existent', TEST, johnDoeUser, NO_DATA, NO_OPTS, (err /* , message */) => {
                    assert.ok(err);
    ...
    it('verify email templates are internationalized', callback => {
      generateTestUsers(asCambridgeTenantAdmin, 2, (err, users) => {
        ...
        sendEmail(EMAIL_MODULE, 'test_locale', miguel.user, NO_DATA, NO_OPTS, (err, info) => {
          assert.notExists(err);
          ...
          sendEmail(EMAIL_MODULE, 'test_locale', rita.user, NO_DATA, NO_OPTS, (err, info) => {
            assert.notExists(err);
    ...
    it('verify error with no meta template', callback => {
      generateTestUsers(asCambridgeTenantAdmin, 1, (err, users) => {
        ...
        sendEmail(EMAIL_MODULE, 'TestNoMeta', johnDoe.user, NO_DATA, NO_OPTS, (err /* , message */) => {
          assert.ok(err);
          assert.strictEqual(err.code, 500);
    ...
    it('verify error with no html or txt template', callback => {
      generateTestUsers(asCambridgeTenantAdmin, 1, (err, users) => {
        ...
        // Verify error when there is no email
        sendEmail(EMAIL_MODULE, 'test_meta_only', johnDoe.user, NO_DATA, NO_OPTS, (err /* , message */) => {
          assert.ok(err);
    ...
    it('verify html and txt templates', callback => {
      generateTestUsers(asCambridgeTenantAdmin, 1, (err, users) => {
        ...
        // Verify HTML only
        sendEmail(EMAIL_MODULE, 'test_html_only', johnDoe.user, NO_DATA, NO_OPTS, (err, info) => {
          assert.notExists(err);

          const message = JSON.parse(info.message);
          assert.strictEqual(message.from.name, 'Cambridge University Test');
          assert.strictEqual(message.from.address, util.format('noreply@%s', johnDoe.restContext.hostHeader));
          ...
          // Verify text only
          sendEmail(EMAIL_MODULE, 'test_txt_only', johnDoe.user, NO_DATA, NO_OPTS, (err, info) => {
            assert.notExists(err);
            ...
            sendEmail(EMAIL_MODULE, 'test_html_and_txt', johnDoe.user, NO_DATA, NO_OPTS, (err, info) => {
              assert.notExists(err);
    ...
    it("verify html doesn't include the HTML5 entity for the apostrophe", callback => {
      generateTestUsers(asCambridgeTenantAdmin, 1, (err, users) => {
        ...
        // Verify HTML only
        sendEmail(EMAIL_MODULE, 'test_html_with_apostrophes_only', johnDoe.user, NO_DATA, NO_OPTS, (err, info) => {
          assert.notExists(err);
    ...
    it('verify exception handling from template rendering', callback => {
      generateTestUsers(asCambridgeTenantAdmin, 1, (err, users) => {
        ...
        sendEmail(EMAIL_MODULE, 'test_throw_error', johnDoe.user, _mailData(true, false, false, false), null, err => {
          assert.ok(err);
          assert.strictEqual(err.code, 500);
    ...
    it('verify shared logic can be used', callback => {
      generateTestUsers(asCambridgeTenantAdmin, 1, (err, users) => {
        ...
        sendEmail(EMAIL_MODULE, 'test_shared', johnDoe.user, NO_DATA, NO_OPTS, (err, info) => {
          assert.notExists(err);
          const message = JSON.parse(info.message);
    ...
    it('verify other template files can be included', callback => {
      generateTestUsers(asCambridgeTenantAdmin, 1, (err, users) => {
        ...
        sendEmail(EMAIL_MODULE, 'test_include', johnDoe.user, NO_DATA, NO_OPTS, (err, info) => {
          assert.notExists(err);
  ...
  describe('Email configuration', () => {
    ...
    it('verify transport validation', callback => {
      init(_createDefaultConfig({ debug: false, transport: 'wrong' }), err => {
        assert.strictEqual(err.code, 400);
        ...
        init(_createDefaultConfig(mailConfigOverrides), err => {
          assert.notExists(err);
    ...
    it('verify noreply from header', callback => {
      generateTestUsers(asCambridgeTenantAdmin, 2, (err, users) => {
        ...
        updateConfigAndWait(asCambridgeTenantAdmin, null, config, err => {
          assert.notExists(err);
          ...
          createLink(
            ...
            (err, link) => {
              ...
              collectAndFetchAllEmails(messages => {
                ...
                clearConfig(asCambridgeTenantAdmin, null, configToClear, err => {
                  assert.notExists(err);
                            ...
                            (err, comment) => {
                              ...
                              collectAndFetchAllEmails(messages => {
                                ...
                                clearConfig(asCambridgeTenantAdmin, null, configToClear, err => {
                                  ...
                                  updateConfigAndWait(globalAdminRestContext, null, config, err => {
                                    assert.notExists(err);
  ...
  describe('Email de-duplication', () => {
    ...
    it('verify a provided hash can be used to perform de-duplication', callback => {
      collectAndFetchAllEmails(() => {
        generateTestUsers(asCambridgeTenantAdmin, 1, (err, users) => {
          ...
          sendEmail(EMAIL_MODULE, TEST, johnDoe.user, null, { hash }, (err, message) => {
            assert.notExists(err);
            assert.ok(message);

            // Re-using the same hash should result in test failure
            sendEmail(EMAIL_MODULE, TEST, johnDoe.user, null, { hash }, err => {
              assert.ok(err);
               ...
               */
              sendEmail(EMAIL_MODULE, TEST, johnDoe.user, { data: TEST }, { hash }, err => {
                assert.ok(err);
                assert.strictEqual(err.code, 403);

                // Using another hash (but otherwise the same mail) should work
                sendEmail(EMAIL_MODULE, TEST, johnDoe.user, null, { hash: 'u:cam:janeDoe#000000' }, (err, message) => {
                  assert.notExists(err);
                  assert.ok(message);
    ...
    it('verify omitting the hash uses the email info for de-duplication', callback => {
      generateTestUsers(asCambridgeTenantAdmin, 1, (err, users) => {
        ...
        // Send out the first e-mail
        sendEmail(EMAIL_MODULE, TEST, johnDoe.user, NO_DATA, NO_OPTS, (err, message) => {
          assert.notExists(err);
          assert.ok(message);

          // Sending out the same email should result in a failure
          sendEmail(EMAIL_MODULE, TEST, johnDoe.user, NO_DATA, NO_OPTS, err => {
            assert.ok(err);
            assert.strictEqual(err.code, 403);

            // Sanity check that sending out a different email works
            sendEmail(EMAIL_MODULE, TEST, johnDoe.user, { data: TEST }, NO_OPTS, err => {
              assert.ok(err);
              assert.strictEqual(err.code, 403);
    ...
    it('verify de-duplication is limited to a configurable interval', callback => {
      generateTestUsers(asCambridgeTenantAdmin, 1, (err, users) => {
        ...
        init(_createDefaultConfig({ deduplicationInterval: 2 }), err => {
          assert.notExists(err);
    ...
    it('verify the same email can be sent to multiple users', callback => {
      generateTestUsers(asCambridgeTenantAdmin, 2, (err, users) => {
        ...
        sendEmail(EMAIL_MODULE, TEST, johnDoe.user, NO_DATA, NO_OPTS, (err, message) => {
          ...
          sendEmail(EMAIL_MODULE, TEST, janeDoe.user, NO_DATA, NO_OPTS, (err, message) => {
            assert.notExists(err);
            assert.ok(message);
  ...
  describe('Throttling', () => {
    ...
    it('verify email throttling', callback => {
       ...
       */
      init(_createDefaultConfig({ throttling: { timespan: 2, count: 2 } }), err => {
        assert.notExists(err);
        ...
        generateTestUsers(asCambridgeTenantAdmin, 1, (err, users) => {
          ...
          sendEmail(EMAIL_MODULE, 'test', johnDoe.user, NO_DATA, { hash: generateUniqueHash() }, (
            ...
            sendEmail(EMAIL_MODULE, TEST, johnDoe.user, null, { hash: generateUniqueHash() }, (err /* , message */) => {
              assert.notExists(err);
