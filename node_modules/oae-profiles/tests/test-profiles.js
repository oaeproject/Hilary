...
describe('Profiles', function() {
    ...
    var createUsers = function(callback) {
        ...
        var simon = new User(_tenant, undefined, 'public', 'en_GB', 'Europe/London', 'simong');
        var bert = new User(_tenant, undefined, 'public', 'en_GB', 'Europe/London', 'PhysX');
        
        var simonOpts = {
          privacy: simon.privacy,
          locale: simon.locale,
          timezone: simon.timezone
        }
        
        var bertOpts = {
          privacy: bert.privacy,
          locale: bert.locale,
          timezone: bert.timezone
        }
        
        PrincipalAPI.createUser(_anonymousContext, loginId(_tenant.alias, 'Simon-' + r), 'password', simon.displayName, simonOpts, function(err, userId) {
            assert.ok(!err);
            simon.id = userId;
            var simonContext = new Context(_tenant, simon);

            PrincipalAPI.createUser(_anonymousContext, loginId(_tenant.alias, 'Bert-' + r), 'password', bert.displayName, bertOpts, function(err, userId) {
                assert.ok(!err);
                bert.id = userId;
    ...
    var login = function(user, callback) {
        logout(function(err, response, body) {
            ...
            PrincipalsAPI.getLoginIdFromUserId(user.id, function(err, loginId) {
                // Login
                request.post({
                    'url': 'http://localhost:2001/api/auth/login',
                    'method': 'POST',
                    'form': { 'username': loginId.externalId, 'password': 'password' }
                },function(err, response, body) {
                    assert.ok(!err);
                    callback(err, response, body);
                });
    ...
    var setSection = function(context, principalId, section, visibility, data, overwrite, callback) {
        login(context.user(), function(err, response, body) {
            assert.ok(!err);

            // Set the section.
            request.post({
                'url': 'http://localhost:2001/api/user/' + principalId + '/profile',
                'method': 'POST',
    ...
    var getSection = function(context, principalId, section, callback) {
        login(context.user(), function(err, response, body) {
            assert.ok(!err);
            request.get('http://localhost:2001/api/user/' + principalId + '/profile/' + section, function(err, response, body) {
                handleHttpResponse(err, response, body, callback);
            });
        });
    };

    var getAllSections = function(context, principalId, callback) {
        login(context.user(), function(err, response, body) {
            assert.ok(!err);
            request.get('http://localhost:2001/api/user/' + principalId + '/profile/', function(err, response, body) {
                handleHttpResponse(err, response, body, callback);
            });
        });
    };
