...
module.exports = function(tenant) {
    ...
    tenant.server.get('/api/content/:contentId/download', function(req, res) {
        ...
        ContentAPI.getLatestRevision(req.ctx, req.params.contentId, function(err, revision) {
            ...
            if (revision.link.code === 200) {
                res.setHeader('X-Accel-Redirect', revision.link.url);
                res.send(200, revision.link.url);
            } else if (revision.link.code === 301 || reversion.link.code === 302) {
                res.setHeader('Location', revision.link.url);
                res.send(revision.link.code);
            } else {
                res.send(404);
            }
        });
    ...
    tenant.server.post('/api/content/:contentId/newversion', function(req, res) {
        ...
        ContentAPI.updateFileBody(req.ctx, req.params.contentId, req.files.file, function(err, revision) {
            if (err) {
                return res.send(err.code, err.msg);
            }
