 ...
 */
var exportData = module.exports.exportData = function(ctx, userId, exportType, callback) {
    var validator = new Validator();

    validator.check(userId, {'code': 400, 'msg': 'A valid user id must be provided'}).isUserId();
    validator.check(null, {'code': 401, 'msg': 'You have to be logged in to be able to delete a pending email token'}).isLoggedInUser(ctx);
    validator.check(exportType, {'code': 402, 'msg': 'An invalid exportType has been specified'}).isIn(_.values(PrincipalsConstants.exportType));
    ...
    // Export data
    PrincipalsDAO.getPrincipal(userId, function(err, principal) {
        if (err) {
            return callback(err);
        }

        var personalDetails = 'Personal id : ' + principal.id
                    + '\nPersonal name : ' + principal.displayName
                    + '\nPersonal e-mail : ' + principal.email; 

        // Get profile picture if exist
        _extractProfilePicture(ctx, principal, function(err, profilePicture) {
            if (err) {
                return callback(err);
            }

            // Get personal data 
            exportContentData(ctx, userId, exportType, function(err, data) {
                if (err) {
                    return callback(err);
                }

                // Create an object by assembling all personal data
                _assemblePersonalData(personalDetails, profilePicture, data, function(err, personalData) {
                    if (err) {
                        return callback(err);
                    }

                    // Convert an object to a zip file
                    _objectToZipFile(personalData, function(err, zipFile) {
                        if (err) {
                            log().error({'err': err, 'displayName': principal.displayName}, 'An error occurred while creating the zip file');
                            return callback(err);
                        }
...
const _extractProfilePicture = function(ctx, principal, callback) {
    if (_.isEmpty(principal.picture)) {
        ...
        var pathSplited = principal.picture.largeUri.split('/');
        var imageName = pathSplited[pathSplited.length - 1];

        var profilePicture = {'path': path + '/' + pathLargePicture[1], 'imageName': imageName};
 ...
 */
const exportContentData = module.exports.exportContentData = function(ctx, userId, exportType, callback) {
    if (exportType === PrincipalsConstants.exportType.PERSONAL_DATA) {
        ...
        ContentAPI.getContentLibraryItems(ctx, userId, null, null, function(err, contents) {
            ...
            if (exportType === PrincipalsConstants.exportType.CONTENT_DATA) {
                contents = _.reject(contents, function(content) { return content.createdBy !== userId; });
            }

            // Group all content by type
            var contentsSplited = _.groupBy(contents, function(content) { return content.resourceSubType; });

            // get uploaded files
            _getUploadedFiles(ctx, contentsSplited.file, function(err, uploadData) { 
                if (err) {
                    return callback(err);
                }

                // Convert links into txt file
                _linkToTxt(ctx, contentsSplited.link, function(err, linkData) { 
                    if (err) {
                        return callback(err);
                    }

                    // Convert collabdocs into txt file
                    _collabdocToTxt(ctx, contentsSplited.collabdoc, function(err, collabdocData) { 
                        if (err) {
                            return callback(err);
                        }
                        ...
                        MeetingsAPI.Meetings.getMeetingsLibrary(ctx, userId, null, null, function(err, meetings) {
                            ...
                            if (exportType === PrincipalsConstants.exportType.CONTENT_DATA) {
                                meetings = _.reject(meetings, function(meeting) { return meeting.createdBy !== userId; });
                            }

                            // Convert meetings into txt file
                            _meetingToTxt(ctx, meetings, function(err, meetingData) {
                                if (err) {
                                    return callback(err);
                                }

                                // Get discussions library
                                DiscussionsAPI.Discussions.getDiscussionsLibrary(ctx, userId, null, null, function(err, discussions) {
                                    if (err) {
                                        return callback(err);
                                    }

                                    if (exportType === PrincipalsConstants.exportType.CONTENT_DATA) {
                                        discussions = _.reject(discussions, function(discussion) { return discussion.createdBy !== userId; });
                                    }

                                    // Convert meetings into txt file
                                    _discussionToTxt(ctx, discussions, function(err, discussionData) {
                                        if (err) {
                                            return callback(err);
                                        }

                                        return callback(null, {uploadData, linkData, collabdocData, meetingData, discussionData});
                                    });
                                });
                            });
                        });
...
const _getUploadedFiles = function(ctx, uploadedFiles, callback) {
    ...
    // Get files path
    let filePath = ContentUtil.getStorageBackend(ctx, null).getRootDirectory();
    let uploadData = [];

    _.each(uploadedFiles, uploadedFile => {
        var uploadedFileId = uploadedFile.id.split(':');
        var path = uploadedFileId[0] + '/' + uploadedFileId[1] + '/' + uploadedFileId[2].substring(0, 2) + '/' + uploadedFileId[2].substring(2, 4) + '/' + uploadedFileId[2].substring(4, 6) + '/' + uploadedFileId[2].substring(6, 8) + '/' + uploadedFileId[2] + '/' + uploadedFile.latestRevisionId.split(':').join('-');
        var nameWithoutSpace = uploadedFile.filename.split(/ /g).join('-');
        
        uploadData.push({'path': filePath + '/' + path + '/' + nameWithoutSpace, 'title': nameWithoutSpace});
    });
...
const _collabdocToTxt = function(ctx, collabdocs, callback) {
    ...
    async.eachSeries(collabdocs, function(collabdoc, callback) {

        ContentAPI.getComments(ctx, collabdoc.id, null, null, function(err, messages) {
            if (err) {
                return callback(err);
            }

            ContentAPI.getRevision(ctx, collabdoc.id, collabdoc.latestRevisionId, function(err, revision) {
                if (err) {
                    return callback(err);
                }

                txtCollabdoc = 'Collabdoc name: ' + collabdoc.displayName
                    + '\nCollabdoc path: ' + collabdoc.profilePath
                    + '\nCollabdoc visibility: ' + collabdoc.visibility
                    + '\nTenant name : ' + collabdoc.tenant.displayName
                    + '\nContent of the document : ' + revision.etherpadHtml
                    + '\n\n';

                _getMessages(txtCollabdoc, messages, function(err, txtCollabdocWithMessages) {
                    if (err) {
                        return callback(err);
                    }

                    _clearName(collabdoc.displayName, function(err, fileName) {
                        if (err) {
                            return callback(err);
                        }
...
const _linkToTxt = function(ctx, links, callback) {
    ...
    async.eachSeries(links, function(link, callback) {
        ContentAPI.getComments(ctx, link.id, null, null, function(err, messages) {
            if (err) {
                return callback(err);
            }

            txtLink = 'Link name : ' + link.displayName
                + '\nLink path : ' + link.profilePath
                + '\nLink : ' + link.link
                + '\nLink visibility: ' + link.visibility
                + '\nTenant name : ' + link.tenant.displayName
                + '\n\n';

            _getMessages(txtLink, messages, function(err, txtLinkWithMessages) {
                if (err) {
                    return callback(err);
                }

                _clearName(link.displayName, function(err, fileName) {
                    if (err) {
                        return callback(err);
                    }
...
const _meetingToTxt = function(ctx, meetings, callback) {
    ...
    async.eachSeries(meetings, function(meeting, callback) {
        MeetingsAPI.Meetings.getMessages(ctx, meeting.id, null, null, function(err, messages) {
            if (err) {
                return callback(err);
            }

            txtMeeting = 'Meeting name : ' + meeting.displayName
                + '\nMeeting description : ' + meeting.description
                + '\nMeeting path : ' + meeting.tenant.host + meeting.profilePath
                + '\nMeeting visibility: ' + meeting.visibility
                + '\nTenant name : ' + meeting.tenant.displayName
                + '\n\n';

            _getMessages(txtMeeting, messages, function(err, txtMeetingWithMessages) {
                if (err) {
                    return callback(err);
                }

                _clearName(meeting.displayName, function(err, fileName) {
                    if (err) {
                        return callback(err);
                    }
...
const _discussionToTxt = function(ctx, discussions, callback) {
    ...
    async.eachSeries(discussions, function(discussion, callback) {
        DiscussionsAPI.Discussions.getMessages(ctx, discussion.id, null, null, function(err, messages) {
            if (err) {
                return callback(err);
            }

            txtDiscussion = 'Discussion name : ' + discussion.displayName
                    + '\nDiscussion description: ' + discussion.description
                    + '\nDiscussion path : ' + discussion.tenant.host + discussion.profilePath
                    + '\nDiscussion visibility : ' + discussion.visibility
                    + '\nTenant name : ' + discussion.tenant.displayName
                    + '\n';

            _getMessages(txtDiscussion, messages, function(err, txtDiscussionWithMessages) {
                if (err) {
                    return callback(err);
                }

                _clearName(discussion.displayName, function(err, fileName) {
                    if (err) {
                        return callback(err);
                    }
...
const _getMessages = function(txt, messages, callback) {
    ...
    _.each(messages, message => {
        var messageCreated = dateFormat(new Date(parseInt(message.created)), 'dd-mm-yyyy, h:MM:ss TT');
        txt += '\n\t' + messageCreated + ' : level ' + message.level + ' : ' + message.createdBy.displayName + ' says : ' + message.body;
    });
...
const _clearName = function(nameResource, callback) {
    ...
    var fileName = nameResource.replace(HTTP_PROTOCOL + '://', '');
...
const _objectToZipFile = function(personalData, callback) {
    ...
    const compressProfilePicture = (callback) => {
        if (personalData.profilePicture) {
            ...
            fs.readFile(personalData.profilePicture.path, function (err, data) {
                if (err) {
                    return callback(err);
                }
    ...
    const compressUploadedData = (callback) => {
        if (personalData.upload) {
            ...
            var uploadFolder = zipFile.folder('upload_data');

            async.eachSeries(personalData.upload, function(uploadedFile, callback) {
                fs.readFile(uploadedFile.path, function (err, data) {
                    if (err) {
                        return callback(err);
                    }
                    var fileExt = uploadedFile.title.split('.').pop();
                    var text = uploadedFile.title.split('.');
                    var fileName = text.slice(0, text.length - 1).join('.');
                    var newName = _getNewFileName(fileExt, fileName, uploadFolder);
    ...
    const compressCollabDocs = (callback) => {
        if (personalData.collabdoc) {
            ...
            var collabdocFolder = zipFile.folder('collabdoc_data');

            _.each(personalData.collabdoc, collabdoc => {
                var fileExt = collabdoc.title.split('.').pop();
                var text = collabdoc.title.split('.');
                var fileName = text.slice(0, text.length - 1).join('.');
                var newName = _getNewFileName(fileExt, fileName, collabdocFolder);
    ...
    const compressLinks = (callback) => {
        if (personalData.link) {
            ...
            var linkFolder = zipFile.folder('link_data');

            _.each(personalData.link, link => {
                var fileExt = link.title.split('.').pop();
                var text = link.title.split('.');
                var fileName = text.slice(0, text.length - 1).join('.');
                var newName = _getNewFileName(fileExt, fileName, linkFolder);
    ...
    const compressMeetings = (callback) => {
        if (personalData.meeting) {
            ...
            var meetingFolder = zipFile.folder('meeting_data');

            _.each(personalData.meeting, meeting => {
                var fileExt = meeting.title.split('.').pop();
                var text = meeting.title.split('.');
                var fileName = text.slice(0, text.length - 1).join('.');
                var newName = _getNewFileName(fileExt, fileName, meetingFolder);
    ...
    const compressDiscussions = (callback) => {
        if (personalData.discussion) {
            ...
            var discussionFolder = zipFile.folder('discussion_data');
            
            _.each(personalData.discussion, discussion => {
                var fileExt = discussion.title.split('.').pop();
                var text = discussion.title.split('.');
                var fileName = text.slice(0, text.length - 1).join('.');
                var newName = _getNewFileName(fileExt, fileName, discussionFolder);
...
/**
 * Get all the users connection date of a tenant older than someTimeAgo - "num".
 *
 * @param  {Context}    ctx                 Standard context object containing the current user and the current tenant
 * @param  {Context}    num                 Number of months
 * @param  {String}     tenantAlias         The tenant
 * @param  {Function}   callback            Standard callback function
 * @param  {Object}     callback.err        An error that occurred, if any
 * @param  {String}     callback.csv        A CSV with all last connection
 */
var getUsersConnectionDateLog = module.exports.getUsersConnectionDateLog = function(ctx, num, tenantAlias, callback) {

    callback = callback || function() {};

    var isAdmin = (ctx.user() && ctx.user().isAdmin());
    if (!isAdmin) {
        return callback({'code': 401, 'msg': 'You are not authorized to get log'});
    }

    // Get all the users for a given tenancy
    PrincipalsDAO.getAllUsersForTenant(tenantAlias, function(err, users) {
        if (err) {
            return callback(err);
        }
        ...
        // Get only the connection older than someTimeAgo - "num"
        var someTimeAgo = new Date();
        someTimeAgo.setMonth(someTimeAgo.getMonth() - num);
        var usersWhoHaventLoggedInInAWhile = [];

        _.each(users, function(element) {
            if (element) {
                if (element.lastLogin <= someTimeAgo) {
                    usersWhoHaventLoggedInInAWhile.push(element);
                }
            }
        });

        // Sort row by older date
        usersWhoHaventLoggedInInAWhile.sort(function(a, b){
            return a.lastLogin - b.lastLogin;
        });

        // Create and return the csv file
        var csv = ['principalId, tenantAlias, email, publicAlias, lastLogin'];

        _.each(usersWhoHaventLoggedInInAWhile, function(element) {
            csv.push(`${element.id}, ${element.tenant.alias}, ${element.email}, ${element.publicAlias}, ${element.lastLogin}`);
        });
        csv = csv.join('\n');
...
/**
 * Get all the users creation date of a tenant
 *
 * @param  {Context}    ctx                 Standard context object containing the current user and the current tenant
 * @param  {String}     tenantAlias         The tenant for which users should be retrieved.
 * @param  {String}     type                It can be `created` to get all the users creation date of a tenant 
 *                                          or `auth` to get all the users connection date of a tenant older than someTimeAgo - {months}.
 * @param  {Context}    months                 Number of months that a user has not logged in - if the type of the request is `auth`
 * @param  {Function}   callback            Standard callback function
 * @param  {Object}     callback.err        An error that occurred, if any
 * @param  {String}     callback.csv        A CSV with all creation dates
 */
var getUsersDateLog = module.exports.getUsersDateLog = function(ctx, tenantAlias, type, months, callback) {

    callback = callback || function() {};

    var userNotFoundErr = {'code': 402, 'msg': 'No users were found'};
    ...
    PrincipalsDAO.getAllUsersForTenant(tenantAlias, function(err, users) {
        ...
        // If the type of connexion is `auth` get only the connection older than someTimeAgo - {months}
        // Else return the users list
        _getOnlyUsersWhoHaventLoggedInInAWhile(users, months, type, function(err, users) {
            if (err) {
                return callback(err);
            }
            ...
            var csv = [];
            if (type === 'created') {
                csv = ['principalId,tenantAlias,email,publicAlias,created'];
            } else if (type === 'auth') {
                csv = ['principalId,tenantAlias,email,publicAlias,lastLogin'];
            }

            _.each(users, function(element) {
                if (type === 'created') {
                    csv.push(`${element.id},${element.tenant.alias},${element.email},${element.publicAlias},${element.created}`);
                } else if (type === 'auth') {
                    csv.push(`${element.id},${element.tenant.alias},${element.email},${element.publicAlias},${element.lastLogin}`);
                }
            });
...
/**
 * Get only the connection older than someTimeAgo - {months}
 *
 * @param  {Object}     users             List of users from the current tenant
 * @param  {String}     months            Number of months
 * @param  {Function}   callback          Standard callback function
 * @param  {Object}     callback.err      An error that occurred, if any
 * @param  {Object}     callback.users    List of users from the current tenant who haven't logged in {months} month(s)
 * @api private
 */
var _getOnlyUsersWhoHaventLoggedInInAWhile = function(users, months, type, callback) {
    ...
    // Get only the connection older than someTimeAgo - {months}
    var someTimeAgo = new Date();
    someTimeAgo.setMonth(someTimeAgo.getMonth() - months);
    var usersWhoHaventLoggedInInAWhile = [];

    _.each(users, function(element, i, arr) {
        if (element.lastLogin <= someTimeAgo) {
            usersWhoHaventLoggedInInAWhile.push(element);
            if (arr.length === i + 1) {
                return callback(null, usersWhoHaventLoggedInInAWhile);
            }
        }
        else {
            if (arr.length === i + 1) {
