...
/**
 * Returns a query that can be used to search trough all top level resources (content, users, groups, discussions, ..)
 * that will search trough messages as well.
 *
 * @param  {String}     q   The string that we're searching for.
 * @return {Object}         An object that can be used in the query part of the ElasticSearch Query DSL
 */
var createGeneralQuerySupportingMessages = module.exports.createGeneralQuerySupportingMessages = function(q) {
    // We need to wrap the bool query in a custom_boost_factor as the hits would otherwise
    // not reach our cutoff point.
    return {
        'bool' : {
            'should': [
                createQueryStringQuery(q, ['q_high^2.0', 'q_low^0.75']),
                createHasChildQuery('message', createQueryStringQuery(q, ['body']), 'avg')
            ]
        }
    };
};
...
var filterMembers = module.exports.filterMembers = function(members) {
    ...
    return createHasChildQuery(SearchConstants.resourceMembersMappingName, filterTerms('direct_members', members));
};
...
var filterMemberships = module.exports.filterMemberships = function(memberships) {
    ...
    return createHasChildQuery(SearchConstants.resourceMembershipsMappingName, filterTerms('direct_memberships', memberships));
};
 ...
 * @param  {String[]}  fields  The fields of the documents that should be searched through
 * @return {Object}            A Query object that can be used in the query portion of the ElasticSearch Query DSL
 */
var createQueryStringQuery = module.exports.createQueryStringQuery = function(q, fields) {
    q = getQueryParam(q);
