 ...
 * @param  {Object}         additionalMembers       Object where the keys represent principal ids that need to be added to the content upon creation and the values represent the role that principal will have. Possible values are "viewer" and "manager", as well as "editor" for collabdocs and collabsheets
...
var _createContent = function(ctx, resourceSubType, displayName, description, visibility, link, uploadedFile, additionalMembers, folderIds, callback) {
    ...
    } else if (resourceSubType === 'collabsheet') {
        return ContentAPI.createCollabSheet(ctx, displayName, description, visibility, additionalMembers, folderIds, callback);

    // Not a recognized file type
    } else {
        return callback({'code': 400, 'msg': 'Unrecognized resourceSubType. Accepted values are "link", "file", "collabdoc" and "collabsheet"'});
    }
};
 ...
 * @Return      {CollabdocJoinInfo}                         Information on how to join the collaborative document or spreadsheet
 * @HttpResponse                    200                     Joined collabdoc or collabsheet
