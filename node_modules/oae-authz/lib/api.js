    ...
    _getDirectRole(principalId, resourceId, function(err, directRole) {
        ...
        _getResourceGroupHierarchy(resourceId, function(err, groups) {
            ...
            if (groupIds.length === 0) {
                ...
                Cassandra.runQuery('SELECT ? FROM AuthzRoles USING CONSISTENCY QUORUM WHERE principalId = ?', [groupIds, principalId], function(err, rows) {
                    if (err) {
                        return callback(err);
                    }
...
var _getResourceGroupHierarchy = function(resourceId, callback) {
    ...
    Cassandra.runQuery('SELECT ? .. ? FROM AuthzMembers USING CONSISTENCY QUORUM WHERE resourceId = ?', [start, end, resourceId], function(err, rows) {
        ...
        var groupHierarchyIds = _.keys(groupHierarchy);
        if (groupHierarchyIds.length === 0) {
            callback(null, {});
        } else {
            ...
            explodeGroupMemberships(groupHierarchyIds);
