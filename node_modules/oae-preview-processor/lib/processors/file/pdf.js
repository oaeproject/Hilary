...
var _pdftotextBinary = null;
var _pdftotextTimeout = 120000;
...
// the full OAE page
var _pdf2htmlLessTemplate = null;
var _pdf2htmlLessTemplatePath = util.format('%s/../../../lessTemplates/pdf2html.less.jst', __dirname);
...
var init = module.exports.init = function(config, callback) {
    if (!config || !config.pdftotext || !config.pdftotext.binary || !config.pdf2htmlEX || !config.pdf2htmlEX.binary) {
        return callback({'code': 400, 'msg': 'Missing configuration for the pdftotext and/or pdf2htmlEX binary'});
    }

    // Try to execute `pdf2htmlEX --version`
    var cmd = util.format('"%s" --version', config.pdf2htmlEX.binary);
    ...
    exec(cmd, {'timeout': _pdf2htmlEXTimeout}, function (err, stdout, stderr) {
        ...
        log().trace('Executing %s to verify if the path to the pdftotext binary is configured correctly.', cmd);
        exec(cmd, {'timeout': _pdftotextTimeout}, function (err, stdout, stderr) {
            if (err) {
                log().error({'err': err, 'stdout': stdout, 'stderr': stderr}, 'Could not properly execute the pdftotext binary');
            ...
            fs.readFile(_pdf2htmlLessTemplatePath, 'utf8', function(err, templateContent) {
                if (err) {
                    log().error({'err': err, 'stdout': stdout, 'stderr': stderr}, 'Could not read the pdf2htmlEX LESS template');
                ...
                if (templateContent) {
                    try {
                        ...
                        log().error({'err': ex, 'templateContent': templateContent}, 'Could not generate the pdf2html combined LESS template');
                    ...
                    return callback({'code': 500, 'msg': 'pdf2htmlEX LESS template file ' + _pdf2htmlLessTemplatePath + ' had no content'});
                }
            });
        });
    });
...
var previewPDF = module.exports.previewPDF = function(ctx, path, callback) {
    ...
    fs.mkdir(pagesDir, function(err) {
        ...
        _convertPDFToHTMLPages(ctx, path, pagesDir, function(err) {
            ...
            _convertToText(ctx, path, pagesDir, function(err) {
                if (err) {
                    return callback(err);
                }
...
var _convertPDFToHTMLPages = function(ctx, path, pagesDir, callback) {
    var cmd = util.format('"%s" --split-pages=1 --page-filename=page..html --embed-css=0 --css-filename=lines.css --embed-javascript=0 --fit-width=700 --no-drm=1 --dest-dir "%s" "%s"', _pdf2htmlEXBinary, pagesDir, path);
    exec(cmd, {'timeout': _pdf2htmlEXTimeout}, function (err, stdout, stderr) {
        if (err) {
            log().error({'err': err, 'cmd': cmd, 'stdout': stdout, 'stderr': stderr}, 'Could not convert page to html');
        ...
        fs.readdir(pagesDir, function(err, files) {
            ...
            var combinedCssPath = pagesDir + '/combined.css';
            ...
            var parser = less.Parser({paths: [pagesDir]});
            parser.parse(lessSource, function(err, tree) {
                if (err) {
                    log().error({'err': err}, 'Error combining the CSS files');
                    return callback({'code': 500, 'msg': 'Error combining the CSS files'});
                }

                // Save the resulting output
                fs.writeFile(combinedCssPath, tree.toCSS({
                    'cleancss': true,
                    'compress': true
                }), function(err) {
                    if (err) {
                        log().error({'err': err}, 'Error saving the combined CSS file');
...
var _generateThumbnail = function(ctx, path, pagesDir, callback) {
    ...
    var width = PreviewConstants.SIZES.PDF.LARGE;
    var output = pagesDir + '/page.1.png';
    gm(path + '[0]')
        .adjoin()
        ...
        .write(output, function(err) {
            if (err) {
                log().error({'err': err, 'contentId': ctx.contentId}, 'Could not convert a PDF page to a PNG');
 ...
 * @api private
 */
var _convertToText = function(ctx, input, pagesDir, callback) {
    var output = pagesDir + '/plain.txt';
    var cmd = util.format('"%s" -q "%s" "%s"', _pdftotextBinary, input, output);
     // Execute the command
    ...
    exec(cmd, options, function (err, stdout, stderr) {
        if (err) {
            log().error({'err': err, 'contentId': ctx.contentId, 'stdout': stdout, 'stderr': stderr}, 'Could not convert the PDF to plain text');
