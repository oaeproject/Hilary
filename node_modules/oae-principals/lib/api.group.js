...
var getFullGroupProfile = module.exports.getFullGroupProfile = function(ctx, groupId, callback) {
    getGroup(ctx, groupId, function(err, group) {
        ...
        /**
         * Checks if the current user can view the group's profile information.
         *
         * @param  {String[]} roles A set of roles the current user has in the group.
         */
        var accessCheck = function(roles) {
            ...
            group.isMember = (_.contains(roles, Constants.roles.MEMBER) || _.contains(roles, Constants.roles.MANAGER) || isAdmin);
            group.isManager = (_.contains(roles, Constants.roles.MANAGER) || isAdmin);
            group.canJoin = _canJoin(ctx, group, hasRole);
            callback(null, group);
        };

        // Avoid doing an authz check if the user is anonymous.
        if (!ctx.user()) {
            accessCheck([]);
        } else {
    ...
    getGroup(ctx, groupId, function(err, group) {
        ...
        _canManage(ctx, group, function(err, canManage) {
            ...
            AuthzAPI.computeMemberRolesAfterChanges(groupId, memberChanges, function(err, computedPermissions, newMemberIds, updatedMemberIds, removedMemberIds) {
                if (err) {
                    return callback(err);
                } else if (!_.chain(computedPermissions).values().contains(Constants.roles.MANAGER).value()) {
                ...
                AuthzAPI.canInteract(ctx, group.tenant.alias, newMemberIds, function(err, canInteract, illegalPrinicpalIds) {
                    if (err) {
                        return callback(err);
                    } else if (!canInteract) {
                    ...
                    AuthzAPI.updateRoles(group.id, memberChanges, function(err) {
                        ...
                        PrincipalsDAO.updatePrincipal(group.id, {'lastModified': newLastModified}, function(err) {
                            ...
                            // Remove from libraries of users who have been removed from the group
                            _removeMembershipsLibraries(removedMemberIds, group.id, oldLastModified, function(err) {
                                ...
                                // Update all the other user libraries, as the lastModified of the group has been updated
                                _updateMembershipsLibraries(_.keys(computedPermissions), group, oldLastModified, function(err) {
                                    if (err) {
                                        log().warn({
                                            'err': err,
                                            'group': group,
                                            'removedMemberIds': _.keys(computedPermissions)
                                        }, 'An error occurred while updating memberships libraries');
                                    }

                                    PrincipalsEmitter.emit(PrincipalsConstants.events.UPDATED_GROUP_MEMBERS, ctx, group, memberChanges, computedPermissions, newMemberIds, updatedMemberIds, removedMemberIds);
                                    return callback();
                                });
                            });
                        });
    ...
    AuthzAPI.getDirectRoles([ctx.user().id], groupId, function(err, userRole) {
        ...
        PrincipalsDAO.getPrincipal(groupId, function(err, group) {
            ...
            AuthzAPI.computeMemberRolesAfterChanges(groupId, memberChanges, function(err, computedPermissions, newMemberIds, updatedMemberIds, removedMemberIds) {
                if (err) {
                    return callback(err);
                } else if (userRole === Constants.roles.MANAGER && !_.chain(computedPermissions).values().contains(Constants.roles.MANAGER).value()) {
                    return callback({'code': 400, 'msg': 'The requested operation would leave the group without a manager'});
                }
        ...
        PrincipalsDAO.getPrincipal(groupId, function(err, group) {
            ...
            AuthzAPI.computeMemberRolesAfterChanges(group.id, changeMembers, function(err, computedPermissions) {
                ...
                AuthzAPI.updateRoles(group.id, changeMembers, function(err) {
                    ...
                    /*!
                     * A callback method that, when invoked, will signal the successful completion of this method. Used for
                     * convenience so that when operations occur afterward that are not tied to the success of this method are
                     * invoked they can easily return without returning an error code
                     */
                    ...
                    PrincipalsDAO.updatePrincipal(group.id, {'lastModified': newLastModified}, function(err) {
                        ...
                        _updateMembershipsLibraries(_.keys(computedPermissions), group, oldLastModified, _callback);
                    });
                });
            });
        });
...
var _canJoin = function(ctx, group, hasRole) {
    // You cannot join a group you're already a member of.
    if (hasRole) {
        return false;
    ...
    // settings allow it AND the group's visibility setting is set too public.
 ...
 * @param  {Object}    permissions          A hash where each key is a principal id and the value is one of 'manager', 'member' or false. If the value is set to false, the principal will be removed from the group.
 ...
 */
var createGroup = module.exports.createGroup = function(ctx, displayName, description, visibility, joinable, permissions, otherValues, callback) {
    ...
    var groupId = AuthzUtil.toId(Constants.PRINCIPAL_TYPE_GROUP, tenantAlias, ShortId.generate());
    ...
    canCreateGroup(ctx, groupId, function(err) {
        ...
        permissions[currentUser] = Constants.roles.MANAGER;

        // Ensure we don't violate tenant boundaries by adding users from other private tenants
        AuthzAPI.canInteract(ctx, tenantAlias, _.keys(permissions), function(err, canInteract, illegalPrinicpalIds) {
            if (err) {
                return callback(err);
            }
            ...
            // Create the group
            Cassandra.runQuery('INSERT INTO Principals (principalId, tenantAlias, displayName, description, visibility, joinable, lastModified) VALUES (?, ?, ?, ?, ?, ?, ?) USING CONSISTENCY QUORUM',
                [groupId, tenantAlias, displayName, description, visibility, joinable, lastModified], function (err) {
                if (err) {
                    return callback(err);
                }
    ...
    getFullGroupProfile(ctx, groupId, function(err, oldGroup) {
        ...
        _canManage(ctx, oldGroup, function(err, canManage) {
            ...
            PrincipalsDAO.updatePrincipal(groupId, profileFields, function(err) {
                ...
                var updatedGroup = _.extend({}, oldGroup, profileFields);
                 ...
                 */
                var _callback = function() {
                    PrincipalsEmitter.emit(PrincipalsConstants.events.UPDATED_GROUP, ctx, updatedGroup, oldGroup);
                ...
                AuthzAPI.getAuthzMembers(groupId, null, 10000, function(err, memberRoles) {
                    ...
                    _updateMembershipsLibraries(memberIds, updatedGroup, oldGroup.lastModified, function(err) {
                        if (err) {
                            log().error({
                                'err': err,
                                'oldGroup': oldGroup,
...
var canCreateGroup = function(ctx, groupId, callback) {
    if (getUserId(ctx) !== null) {
        getGroup(ctx, groupId, function(err, group) {
            ...
            return callback({'code': 400, 'msg': 'This group id is not available.'});
        });
    } else {
