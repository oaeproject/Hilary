...
var getOrCreateTokenByEmail = module.exports.getInvitationToken = function(email, callback) {
    Cassandra.runQuery('SELECT * FROM "AuthzInvitationsTokenByEmail" WHERE "email" = ?', [email], function(err, rows) {
        if (err) {
            return callback(err);
        ...
        var queries = [
            ...
            Cassandra.constructUpsertCQL('AuthzInvitationsTokenByEmail', 'token', token, {'email': email})
        ];
...
var saveInvitation = module.exports.saveInvitation = function(resourceId, email, inviterUserId, role, callback) {
    ...
    var queries = [
        Cassandra.constructUpsertCQL('AuthzInvitations', 'resourceId', resourceId, invitation.toStorage()),
        Cassandra.constructUpsertCQL('AuthzInvitationsResourceIdByEmail', 'email', email, {'reosurceId': resourceId})
    ];
...
var getInvitationsByResourceId = module.exports.getInvitationsByResourceId = function(resourceId, opts, callback) {
    ...
    Cassandra.runQuery('SELECT * FROM "AuthzInvitations" WHERE "resourceId" = ? AND "email" > ? ORDER BY "email" ASC LIMIT ' + opts.limit, [resourceId, email], function(err, rows) {
        if (err) {
            return callback(err);
        } else if (_.isEmpty(rows)) {
