...
// Users have 24 hours to verify an email address
var EMAIL_MAPPING_TTL = 24 * 60 * 60;
...
var createUser = module.exports.createUser = function(user, callback) {
    ...
    // If the user contained an email address, map the email to the user id
    if (user.email) {
        var ttl = EMAIL_MAPPING_TTL;

        // If the email is already verified, there's no need for the TTL
        if (user.emailVerified) {
            ttl = null;
        }
        queries.push(Cassandra.constructUpsertCQL('PrincipalsByEmail', 'email', user.email, {'principalId': user.id}, ttl));
    }
 ...
 * @param  {Function}   callback                            Standard callback function
 * @param  {Object}     callback.err                        An error that occurred, if any
 * @param  {Object}     callback.err.existingPrincipals     A hash of principals keyed by the principal id, whose value is a principal (either user or group) that existed in storage
 ...
 * @param  {String}     [oldEmail]      The old email address that was associated with the user. If this is defined and different from the `profileFields.email` address, the email to principal mapping will be updated
 * @param  {Function}   callback        Standard callback function
 * @param  {Object}     callback.err    An error that occurred, if any
 */
var updatePrincipal = module.exports.updatePrincipal = function(principalId, profileFields, oldEmail, callback) {
    ...
    // If a user's email address needs to be updated, we remove the old one from the mapping
    if (profileFields.email && (profileFields.email !== oldEmail)) {
        queries.push({'query': 'DELETE FROM "PrincipalsByEmail" WHERE "email" = ?', 'parameters': [oldEmail]});
    }

    // If a user's email address is updated, we add it to the mapping
    if (profileFields.email && (profileFields.email !== oldEmail)) {
        var ttl = EMAIL_MAPPING_TTL;

        // If the email is already verified, there's no need for the TTL
        if (profileFields.emailVerified) {
            ttl = null;
        }
        queries.push(Cassandra.constructUpsertCQL('PrincipalsByEmail', 'email', profileFields.email, {'principalId': principalId}, ttl));
    }
...
/**
 * Get a user via an email address. If no user could be found
 * for a given email address, a 404 error will be returned
 *
 * @param  {String}     email           The email that is associated with a user
 * @param  {Function}   callback        Standard callback function
 * @param  {Object}     callback.err    An error object, if any
 * @param  {User}       callback.user   The user that was associated with the email
 */
var getUserViaEmail = module.exports.getUserViaEmail = function(email, callback) {
 ...
 * @param  {string}     userId          The id of the user with whom the token is associated
 * @param  {Function}   callback        Standard callback function
 * @param  {Object}     callback.err    An error object, if any
 */
var storeEmailToken = module.exports.storeEmailToken = function(token, userId, callback) {
    var q = Cassandra.constructUpsertCQL('EmailTokens', 'token', token, {'userId': userId});
    Cassandra.runQuery(q.query, q.parameters, callback);
};

/**
 * Get a user via an email token. If no user could be found
 * for a given token, a 404 error will be returned
 *
 * @param  {String}     token           The token that is associated with a user
 * @param  {Function}   callback        Standard callback function
 * @param  {Object}     callback.err    An error object, if any
 * @param  {User}       callback.user   The user that was associated with the token
 */
var getUserViaEmailToken = module.exports.getUserViaEmailToken = function(token, callback) {
    Cassandra.runQuery('SELECT * FROM "EmailTokens" WHERE "token" = ?', [token], function(err, rows) {
        if (err) {
            return callback(err);
        } else if (rows.length === 0) {
            return callback({'code': 404, 'msg': 'No user found by that token'});
        }

        var userId = rows[0].get('userId').value;
        return getPrincipal(userId, callback);
    });
};

/**
 * Validate a token and user by removing the token from the database and
 * marking the user's email address as verified
 *
 * @param  {String}     token           The token to remove
 * @param  {User}       user            The user whose email address should be marked as verified
 * @param  {Function}   callback        Standard callback function
 * @param  {Object}     callback.err    An error object, if any
 */
var validateToken = module.exports.validateToken = function(token, user, callback) {
    var queries = [];

    // Delete the token from the database as it's no longer needed
    queries.push({'query': 'DELETE FROM "EmailTokens" WHERE "token" = ?', 'parameters': [token]});

    // Mark the email address as valid for the given user
    queries.push(Cassandra.constructUpsertCQL('Principals', 'principalId', user.id, {'emailVerified': true}));

    // Update the email mapping to get rid of any TTL
    ...
    Cassandra.runBatchQuery(queries, function(err) {
        ...
        // Invalidate the cached user object
        invalidateCachedUsers([user.id], callback);
    });
};
...
var hashToUser = function(hash) {
    var user = new User(hash.tenantAlias, hash.principalId, hash.displayName, {
        ...
        'emailVerified': sanitize(hash.emailVerified).toBooleanStrict(),
        'guest': sanitize(hash['guest']).toBooleanStrict(),
    });
    return user;
};
