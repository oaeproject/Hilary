...
OAE.globalAdminRouter.on('post', '/api/user/createGlobalAdminUser', (req, res) => {
    ...
    AuthenticationAPI.getOrCreateGlobalAdminUser(req.ctx, req.body.username, req.body.password, req.body.displayName, opts, (err, user, loginId, created) => {
        if (err) {
            return res.status(err.code).send(err.msg);
        } if (!created) {
...
const _handleCreateTenantAdminUser = function(req, res) {
    ...
    const loginId = new LoginId(tenantAlias, AuthenticationConstants.providers.LOCAL, req.body.username, {'password': req.body.password});
    const opts = _getOptionalProfileParameters(req.body);
...
OAE.globalAdminRouter.on('post', '/api/user/:tenantAlias/create', (req, res) => {
    const loginId = new LoginId(req.params.tenantAlias, AuthenticationConstants.providers.LOCAL, req.body.username, {'password': req.body.password});
    const opts = {
...
OAE.tenantRouter.on('post', '/api/user/create', (req, res) => {
    ...
    const createUser = function() {
        AuthenticationAPI.getOrCreateUser(ctx, AuthenticationConstants.providers.LOCAL, req.body.username, {'password': req.body.password}, req.body.displayName, opts, (err, newUser, loginId, created) => {
            if (err) {
                return res.status(err.code).send(err.msg);
            } if (!created) {
...
const _handleImportUsers = function(req, res) {
    ...
    PrincipalsAPI.importUsers(req.ctx, req.body.tenantAlias, req.files.file, req.body.authenticationStrategy, forceProfileUpdate, (err) => {
        if (err) {
            return res.status(err.code).send(err.msg);
        }
...
OAE.tenantRouter.on('get', '/api/user/:userId/memberships', (req, res) => {
    ...
    PrincipalsAPI.getMembershipsLibrary(req.ctx, req.params.userId, req.query.start, limit, (err, memberships, nextToken) => {
        if (err) {
            return res.status(err.code).send(err.msg);
        }
