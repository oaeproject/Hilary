...
/**
 * Transforms a persisted user activity entity that can be used in an activity stream.
 * The returned activity entity will be outputted in the activitystrea.ms-compliant data model.
 *
...
/**
 * Transforms a persisted user activity entity that can be used in an activity stream.
 * The user property will be taken from the persisted entity, scrubbed and returned as the "transformed entity".
 *
 * @param  {Context}    ctx         The current context
 * @param  {String}     userId      The id of the user
 * @param  {User}       [user]      The user object. If not specified, the generated entity with be abbreviated with just the information available
...
/**
 * Transforms a persisted group activity entity that can be used in an activity stream.
 *
 * For more details on the transformed entity model, @see ActivityAPI#registerActivityEntityType
 *
 * @param  {Context}           ctx         The current context
...
/**
 * Transforms a persisted group activity entity that can be used in an activity stream.
 * The group property will be taken from the persisted entity, scrubbed and returned as the "transformed entity".
 *
 * For more details on the transformed entity model, @see ActivityAPI#registerActivityEntityType
 *
 * @param  {Context}           ctx         The current context
...
var _generatePictureURLs = function(ctx, principal) {
    if (principal.picture.smallUri) {
        principal.picture.small = getSignedPictureUrl(ctx, principal.picture.smallUri);
    }
    if (principal.picture.mediumUri) {
        principal.picture.medium = getSignedPictureUrl(ctx, principal.picture.mediumUri);
    }
    if (principal.picture.largeUri) {
        principal.picture.large = getSignedPictureUrl(ctx, principal.picture.largeUri);
    }
};

/**
 * Generates a url at which the uri can be downloaded.
 ...
 */
var getSignedPictureUrl = module.exports.getSignedPictureUrl = function(ctx, uri) {
