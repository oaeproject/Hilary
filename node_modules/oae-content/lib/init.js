...
module.exports = function(config, callback) {
    ...
    // Create the Cassandra column families.
    ensureSchema(function(err) {
        if (err) {
            return callback(err);
        }

        // Create the directory where uploaded files can be temporarily be stored.
        fs.exists(config.files.uploadDir, function(exists) {
            if (!exists) {
                fs.mkdir(config.files.uploadDir, callback)
            } else {
                callback();
            }
        });
