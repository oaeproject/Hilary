...
module.exports = function(grunt) {
  ...
  grunt.initConfig({
    ...
    replace: {
      'check-style': {
        src: [
          'node_modules/oae-*/**/*.js',
          '!node_modules/oae-*/node_modules/**/*.js',
        ],
        overwrite: true,
        replacements: [
          {
  ...
  grunt.registerTask('test-instrumented', function(report) {
    ...
    var excludeDirectoriesParameters = _.map(excludeDirectories, function(
      module,
    ) {
  ...
  grunt.registerTask('coveralls', function() {
    ...
    shell.exec(
      'cat ./target/lcov.info | ./node_modules/coveralls/bin/coveralls.js',
    );
  });
  ...
  grunt.registerTask('release', function(outputDir) {
    if (!outputDir) {
      return grunt.log.writeln(
        'Please provide a path where the files should be copied to'.red,
      );
    }
