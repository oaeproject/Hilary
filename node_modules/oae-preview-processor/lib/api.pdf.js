 ...
 * @param  {String}     config.previews.binaries.pdftk      The path to the Libre Office executable. This should either be a direct path or the filename that's on the `PATH` environment.
 */
...
var previewPDF = module.exports.previewPDF = function(ctx, path, callback) {
    ...
    var pagesDir = ctx.dirs.base + '/pages';
    fs.mkdir(pagesDir, function(err) {
        if (err) {
            log().error({'err': err}, '%s - Could not create a directory %s to store the pages in.', ctx.contentId, pagesDir);
            return callback({'code': 500, 'msg': 'Could not create a directory to store the splitted pages in.'});
        }

        // Split the PDF pages.
        _splitPDF(ctx, path, pagesDir, function(err, pages) {
            if (err) {
                return callback(err);
            }
            ...
            var called = false;
            var sizes = [
                { 'width': PreviewConstants.SIZES.LARGE, 'height': PreviewConstants.SIZES.LARGE, 'name': 'large' },
            ...
            var resize = function() {
                ...
                PreviewImages.resizeImages(ctx, pagePath, sizes, function(err) {
                    if (err) {
                        return callback(err);
                    }
            ...
            // Start resizing.
            // We only resize 3 images at a time as doing (n pages) * (3 images per page)
            // would be too cpu intensive otherwise.
            resize();
        });
    });
};
...
var _generateThumbnail = function(ctx, pagesDir, callback) {
    ...
    fs.exists(path, function(exists) {
        if (!exists) {
            log().error('%s - The first page could not be found at %s', ctx.contentId, path);
...
var _splitPDF = function(ctx, path, pagesDir, callback) {
    var cmd = _pdftkBinary + ' "' + path + '" burst output ' + pagesDir + '/page.%d.pdf';
     // Execute the command.
    log().trace('%s - executing %s', ctx.contentId, cmd);
    var child = exec(cmd, { 'timeout': _timeout }, function (err, stdout, stderr) {
        if (err) {
            log().error({'err': err}, '%s - Could not split the PDF: %s - %s', ctx.contentId, stdout, stderr);
        ...
        fs.readdir(pagesDir, function(err, files) {
            if (err) {
                log().error({'err': err}, '%s - Could not read the %s directory to list the files', ctx.contentId, pagesDir);
