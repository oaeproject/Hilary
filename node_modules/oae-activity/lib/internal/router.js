...
var routeActivity = module.exports.routeActivity = function(activitySeed, callback) {
    ...
    _produceAllEntities(activitySeed, function(err, actor, object, target) {
        ...
        _produceAllRoutes(activitySeed, actor, object, target, function(err, activityRoutes, notificationRoutes) {
            if (err) {
                return callback(err);
            }
            ...
            _.each(activityRoutes, function(route) {
            ...
            _.each(notificationRoutes, function(route) {
 ...
 * @param  {Function}       callback                    Invoked when the process completes
 * @param  {Object}         callback.err                An error that occurred, if any
 * @param  {String[]}       callback.activityRoutes     The activity routes to which to route the activity
 * @param  {String[]}       callback.notificationRoutes The notification routes to which to route the activity
 * @api private
 */
var _produceAllRoutes = function(activitySeed, actor, object, target, callback) {
    ...
    var actorRouteConfig = _getRouterConfigForObjectType(activityConfig, 'actor');
    var objectRouteConfig = _getRouterConfigForObjectType(activityConfig, 'object');
    var targetRouteConfig = _getRouterConfigForObjectType(activityConfig, 'target');
    ...
    var actorAssociationsCtx = associationsSession.createAssociationsContext(actor.objectType, actor[ActivityConstants.properties.OAE_ID]);
    var objectAssociationsCtx = (object) ? associationsSession.createAssociationsContext(object.objectType, object[ActivityConstants.properties.OAE_ID]) : null;
    var targetAssociationsCtx = (target) ? associationsSession.createAssociationsContext(target.objectType, target[ActivityConstants.properties.OAE_ID]) : null;

    // Route the actor entity
    _produceRoutes(actorAssociationsCtx, actor, actorRouteConfig, function(err, actorRoutes) {
        if (err) {
            return callback(err);
        }

        // Route the object entity
        _produceRoutes(objectAssociationsCtx, object, objectRouteConfig, function(err, objectRoutes) {
            if (err) {
                return callback(err);
            }

            // Route the target entity
            _produceRoutes(targetAssociationsCtx, target, targetRouteConfig, function(err, targetRoutes) {
                if (err) {
                    return callback(err);
                }

                // Determine the propagation rules for the actor
                _producePropagation(actorAssociationsCtx, actor, function(err, actorPropagationRules) {
                    if (err) {
                        return callback(err);
                    }

                    // Determine the propagation rules for the object
                    _producePropagation(objectAssociationsCtx, object, function(err, objectPropagationRules) {
                        if (err) {
                            return callback(err);
                        }

                        // Determine the propagation rules for the target
                        _producePropagation(targetAssociationsCtx, target, function(err, targetPropagationRules) {
                            if (err) {
                                return callback(err);
                            }
                            ...
                            // For notifications, we can immediately trim down to only user routes, and pluck out the actor user themself
                            allNotificationRoutes = _.filter(allNotificationRoutes, function(notificationRoute) {
                                return (AuthzUtil.isUserId(notificationRoute) && notificationRoute !== actor[ActivityConstants.properties.OAE_ID]);
                            });

                            // Merge all the activity and notification routes into one array, and run them through all the entity propagations to see which
                            // ones survive the filters
                            var allRoutes = _.union(allActivityRoutes, allNotificationRoutes);

                            // First filter down the routes by the actor propagation
                            _applyPropagations(actorPropagationRules, allRoutes, actorAssociationsCtx, actor, actorRoutes, function(err, includedRoutes) {
                                if (err) {
                                    return callback(err);
                                } else if (includedRoutes.length === 0) {
                                ...
                                _applyPropagations(objectPropagationRules, includedRoutes, objectAssociationsCtx, object, objectRoutes, function(err, includedRoutes) {
                                    if (err) {
                                        return callback(err);
                                    } else if (includedRoutes.length === 0) {
                                    ...
                                    _applyPropagations(targetPropagationRules, includedRoutes, targetAssociationsCtx, target, targetRoutes, function(err, includedRoutes) {
                                        if (err) {
                                            return callback(err);
                                        }

                                        // Index all routes in a hash so we have quick access to filter out the unsafe activity and notification routes
                                        var allRoutesHash = {};
                                        _.each(includedRoutes, function(route) {
                                            allRoutesHash[route] = true;
                                        });

                                        // Trim the activity and notification routes down to only those that are safe, as determined by our propagation chain
                                        allActivityRoutes = _.filter(allActivityRoutes, function(route) {
                                            return allRoutesHash[route];
                                        });

                                        allNotificationRoutes = _.filter(allNotificationRoutes, function(route) {
                                            return allRoutesHash[route];
                                        });

                                        // Return the final list of routes
                                        return callback(null, allActivityRoutes, allNotificationRoutes);
                                    });
                                });
                            });
                        });
 ...
 * @param  {AssociationsContext}    associationsCtx         The associations context for the entity whose propagation we are applying
 ...
 * @api private
 */
var _applyPropagations = function(propagations, routes, associationsCtx, entity, entityRoutes, callback, _includeRoutes, _excludeRoutes) {
    if (!entity) {
    ...
    _applyPropagation(propagation, _excludeRoutes, associationsCtx, entity, entityRoutes, function(err, includeRoutes, excludeRoutes) {
        if (err) {
        ...
        if (excludeRoutes.length === 0) {
            ...
            return _applyPropagations(propagations, routes, associationsCtx, entity, entityRoutes, callback, _includeRoutes, _excludeRoutes);
        }
    });
};
 ...
 * @api private
 */
var _applyPropagation = function(propagation, routes, associationsCtx, entity, entityRoutes, callback) {
    ...
    if (propagation.type === ActivityConstants.entityPropagation.ALL) {
        ...
        _.each(routes, function(route) {
            var routeTenantAlias = AuthzUtil.getResourceFromId(route).tenantAlias;
        ...
        _.each(entityRoutes, function(entityRoute) { entityRoutesHash[entityRoute] = true; });

        // Split the routes into those who are in the entity routes and those who are not
        _.each(routes, function(route) {
            if (entityRoutesHash[route]) {
                includedRoutes.push(route);
            } else {
                excludedRoutes.push(route);
            }
        ...
        associationsCtx.get(propagation.association, function(err, associations) {
            ...
            _.each(routes, function(route) {
                if (associationRoutesHash[route]) {
                    includedRoutes.push(route);
                } else {
                    excludedRoutes.push(route);
                }
 ...
 * @param  {String[]}               [routeConfig.activity]          The configured activity routing associations
 * @param  {String[]}               [routeConfig.notification]      The configured notification routing associations
 * @param  {Function}               callback                        Standard callback function
 * @param  {Object}                 callback.err                    An error that occurred, if any
 * @param  {Object}                 callback.routes                 The routes derived from the routers
 * @param  {String[]}               callback.routes.activity        The activity routes derived from the activity associations
 * @param  {String[]}               callback.routes.notification    The notification routes derived from the activity associations
 * @api private
 */
var _produceRoutes = function(associationsCtx, entity, routeConfig, callback) {
    if (!entity || !routeConfig) {
        return callback(null, {});
    } else if ((!routeConfig.activity || routeConfig.activity.length === 0) && (!routeConfig.notification || routeConfignotification.length === 0)) {
        return callback(null, {});
    }

    routeConfig.activity = routeConfig.activity || [];
    routeConfig.notification = routeConfig.notification || [];

    // Determine the activity routes from the configured associations
    _routeAssociations(associationsCtx, routeConfig.activity.slice(), function(err, activityRoutes) {
        if (err) {
            return callback(err);
        }

        // Determine the notification routes from the configured associations
        _routeAssociations(associationsCtx, routeConfig.notification.slice(), function(err, notificationRoutes) {
            if (err) {
                return callback(err);
            }

            var routes = {'activity': activityRoutes, 'notification': notificationRoutes};
            log().trace({'entity': entity, 'routes': routes, 'routeConfig': routeConfig}, 'Generated routes for activity entity');
 ...
 * @return {Object}                         An object with fields `activity` and `notification`, whose value is an array that represents the associations on which to route
 */
var _getRouterConfigForObjectType = function(activityConfig, objectType) {
    return {
        'activity': activityConfig && activityConfig.routers && activityConfig.routers.activity && activityConfig.routers.activity[objectType],
        'notification': activityConfig && activityConfig.routers && activityConfig.routers.notification && activityConfig.routers.notification[objectType]
    };
};
