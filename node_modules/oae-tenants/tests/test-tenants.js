    ...
    describe('Tenant domains', function() {
        ...
        it('verify tenants can be retrieved by a configured email domain', function(callback) {
            TestsUtil.setupMultiTenantPrivacyEntities(function(tenant1, tenant2, tenant3) {
                ...
                var domain1 = util.format('foo.%d.com', Date.now());
                var domainRegex1 = util.format('^%s$', domain1);
                var configUpdate1 = {'oae-tenants/domains/email': domainRegex1};
                ConfigTestUtil.updateConfigAndWait(globalAdminRestContext, tenant1.tenant.alias, configUpdate1, function() {
                    ...
                    var domain2 = util.format('foo.%d.com', Date.now());
                    var domainRegex2 = util.format('^%s$', domain2);
                    var configUpdate2 = {'oae-tenants/domains/email': domainRegex2};
                    ConfigTestUtil.updateConfigAndWait(globalAdminRestContext, tenant2.tenant.alias, configUpdate2, function() {
                        ...
                        var tenants = TenantsUtil.getTenantsForEmailDomain(domain1);
                        assert.strictEqual(tenants.length, 1);
                        assert.strictEqual(tenants[0].alias, tenant1.tenant.alias);

                        // Verify no matches
                        tenants = TenantsUtil.getTenantsForEmailDomain('no.no.no.no');
                        assert.strictEqual(tenants.length, 0);

                        // Verify bad input
                        tenants = TenantsUtil.getTenantsForEmailDomain(null);
                        assert.strictEqual(tenants.length, 0);
                        tenants = TenantsUtil.getTenantsForEmailDomain('');
                        assert.strictEqual(tenants.length, 0);

                        // Configure a third tenant that responds to both domains
                        var domainRegex3 = util.format('^(%s|%s)$', domain1, domain2);
                        var configUpdate3 = {'oae-tenants/domains/email': domainRegex3};
                        ConfigTestUtil.updateConfigAndWait(globalAdminRestContext, tenant3.tenant.alias, configUpdate3, function() {

                            // If we now retrieve either of the domains, the third tenant should always be included in the results
                            tenants = TenantsUtil.getTenantsForEmailDomain(domain1);
                            assert.strictEqual(tenants.length, 2);
                            assert.ok(_.contains([tenant1.tenant.alias, tenant3.tenant.alias], tenants[0].alias));
                            assert.ok(_.contains([tenant1.tenant.alias, tenant3.tenant.alias], tenants[1].alias));
                            return callback();
                        });
                    });
                });
