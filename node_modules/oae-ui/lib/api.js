 ...
 * @param  {String}     hashes          A mapping that will map pre-optimized paths to hashed, post-optimized static asset paths
...
var _generateSkin = function(tenantAlias, callback) {
    ...
    if (hashes) {
        _.each(tenantVariables, function(value, key) {
            ...
            // Do not try to apply optimized paths if it is not a URL
            if (!variableMetadata) {
                return;
            } else if (variableMetadata.type !== UIConstants.variables.types.URL) {
                return;
            }

            // Parse the URL value and replace it with the hash result, if any
            var urlRegex = /'(.*)'/g;
            if (urlRegex.test(value)) {
                var url = value.replace(urlRegex, '$1').trim();
                if (hashes[url]) {
                    tenantVariables[key] = util.format('\'%s\'', hashes[url]);
                }
            }
        });
    }
    ...
    _parseLessFile(tenantVariables, function(err, tree) {
        if (err) {
            return callback(err);
        }
