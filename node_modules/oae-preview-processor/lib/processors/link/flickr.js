...
var generatePreviews = module.exports.generatePreviews = function(ctx, contentObj, callback) {
    ...
    var handleDownload = function(err, opts, ignore) {
        if (err) {
            ...
            return callback(null, ContentConstans.previews.IGNORED);
        }
...
var _getFlickrPhoto = function(ctx, id, callback) {
    ...
    var url = util.format('%s?method=flickr.photos.getInfo&api_key=%s&photo_id=%s&format=json&nojsoncallback=1', apiUrl, config.apiKey, id);
...
var _getFlickrSet = function(ctx, id, callback) {
    var config = _getConfig();
    var url = util.format('%s?method=flickr.photosets.getInfo&api_key=%s&photoset_id=%s&format=json&nojsoncallback=1', apiUrl, config.apiKey, id);
    request(url, function(err, response, body) {
        if (err) {
