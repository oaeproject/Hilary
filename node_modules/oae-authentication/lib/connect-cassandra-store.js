...
module.exports = function (connect) {
    ...
    CassandraStore.prototype.get = function (sid, callback) {
        cassandra.runQuery('SELECT * FROM Sessions USING CONSISTENCY QUORUM WHERE sid = ?', [sid], function(err, rows) {
            if (err) {
                return callback(err);
            }
            ...
            return callback(null, JSON.parse(rows[0].get('data').value));
        });
    };
    ...
    CassandraStore.prototype.set = function (sid, session, callback) {
        ...
        cassandra.runQuery('UPDATE Sessions USING CONSISTENCY QUORUM AND TTL ' + ttl + ' SET data=? WHERE sid=?', [session, sid], function(err) {
            callback(err);
        });
    };

    CassandraStore.prototype.destroy = function (sid, callback) {
        cassandra.runQuery('DELETE data FROM Sessions USING CONSISTENCY QUORUM WHERE sid=?', [sid], function(err) {
            callback(err);
        });
    };
