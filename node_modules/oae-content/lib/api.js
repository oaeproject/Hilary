 ...
 * @param  {String}    role              The role that should be assigned to the principals
 * @param  {Function}  callback          Standard callback function
 * @param  {Object}    callback.err      An error that occurred, if any
 */
var shareContent = module.exports.shareContent = function(ctx, contentId, principalIds, role, callback) {
    callback = callback || function() {};
    role = role || ContentConstants.roles.VIEWER;
    ...
    ContentDAO.Content.getContent(contentId, function(err, contentObj) {
        ...
        _canShareContent(ctx, contentObj, function(err, canShare) {
            ...
            _checkTargetUserAccess(ctx, contentObj, testPermissionChanges, function(err, membershipAfterChanges, newMemberIds, updatedMemberIds, removedMemberIds, newMembers) {
                if (err) {
                    return callback(err);
                } else if (_.isEmpty(newMembers) && role === ContentConstants.roles.VIEWER) {
                ...
                // If we're making anything other than viewers we need to include the updatedMembers
                // as they could have been viewers before
                if (role !== ContentConstants.roles.VIEWER) {
                    newMemberIds = newMemberIds.concat(updatedMemberIds);
                }

                // Set the permissions for only users who previously had no role on the item or whose role is updated
                var realPermissionChanges = _makeAllPermissionChanges(newMemberIds, role);

                // Check that the user isn't trying to assign any role higher than their own
                ContentAuthz.canSetContentPermissions(ctx, contentObj, realPermissionChanges, function(err, canSetPermissions, reason) {
                    if (err) {
                        return callback(err);
                    }

                    if (!canSetPermissions) {
                        return callback({'code': 401, 'msg': reason});
                    }
...
var setContentPermissions = module.exports.setContentPermissions = function(ctx, contentId, permissionChanges, callback) {
    ...
    getContent(ctx, contentId, function(err, contentObj) {
        ...
        // Check that the user isn't trying to assign any role higher than their own
        ContentAuthz.canSetContentPermissions(ctx, contentObj, permissionChanges, function(err, canSetPermissions, reason) {
            if (err) {
                return callback(err);
            }
            ...
            _checkTargetUserAccess(ctx, contentObj, permissionChanges, function(err, membershipAfterChanges, newMemberIds, updatedMemberIds, removedMemberIds, newMembers) {
                if (err) {
                    return callback(err);
                }
 ...
 * @api private
 */
var _checkTargetUserAccess = function(ctx, contentObj, newPermissions, callback) {
        ...
        LibraryAPI.Authz.canRemoveFromLibrary(ctx, libraryOwnerId, function(err, canRemove) {
            ...
            AuthzAPI.computeMemberRolesAfterChanges(contentId, permissionChanges, function(err, membershipAfterChanges, newMemberIds, updatedMemberIds, removedMemberIds) {
                ...
                AuthzAPI.updateRoles(contentId, permissionChanges, function(err) {
                    ...
                    LibraryAPI.Index.remove(ContentConstants.library.CONTENT_LIBRARY_INDEX_NAME, [entry], function(err) {
                        if (err) {
                            log().error({
                                'err': err,
