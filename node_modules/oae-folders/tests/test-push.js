    ...
    describe('Notifications', function() {
        ...
        it('verify comments trigger a push notification', function(callback) {
            setupFixture(function(contexts, folder, client) {
                ...
                // Create a comment
                FoldersTestUtil.assertCreateMessageSucceeds(contexts['branden'].restContext, folder.id, 'Message body', null, function(_comment) {
                    comment = _comment;
                });

                client.on('message', function(message) {
                    ActivityTestsUtil.assertActivity(message.activities[0], FoldersConstants.activity.ACTIVITY_FOLDER_COMMENT, ActivityConstants.verbs.POST, contexts['branden'].user.id, comment.id, folder.id);

                    // Verify the message is present on the activity object
                    assert.strictEqual(message.activities[0].object.body, comment.body);
                    assert.strictEqual(message.activities[0].object.created, comment.created);
