...
describe('Related content', function() {
    ...
    it('verify limit handling', function(callback) {
        RestAPI.Search.search(anonymousCamRestContext, 'relatedcontent', [ contentItems.BY_TENANT.CAM.PUBLIC[0] ], {'limit': 1, 'includeExternal': true}, function(err, data) {
            assert.ok(!err);
            assert.equal(data.results.length, 1);

            RestAPI.Search.search(anonymousCamRestContext, 'relatedcontent', [ contentItems.BY_TENANT.CAM.PUBLIC[0] ], {'limit': 2, 'includeExternal': true}, function(err, data) {
                assert.ok(!err);
                assert.equal(data.results.length, 2);

                // The default limit is 5 and should be fallen back to when the
                // request parameter has been omitted or is not an int.
                // (there are only 3 other similar pieces of content in the system though)
                RestAPI.Search.search(anonymousCamRestContext, 'relatedcontent', [ contentItems.BY_TENANT.CAM.PUBLIC[0] ], {'limit': 'non-numeric', 'includeExternal': true}, function(err, data) {
                    assert.ok(!err);
                    assert.equal(data.results.length, 3);

                    RestAPI.Search.search(anonymousCamRestContext, 'relatedcontent', [ contentItems.BY_TENANT.CAM.PUBLIC[0] ], {'includeExternal': true}, function(err, data) {
                        assert.ok(!err);
                        assert.equal(data.results.length, 3);
                        callback();
                    });
