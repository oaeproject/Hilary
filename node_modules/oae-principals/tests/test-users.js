...
describe('Users', function() {
    ...
    describe('Import users', function() {
        ...
        /*!
         * Test that verifies that a CSV user file can be imported using a local authentication strategy
         */
        it('verify import local users', function(callback) {
                        ...
                        RestAPI.User.getMe(stuartRestContext, function(err, stuartMeObj) {
                            ...
                            RestAPI.User.updateUser(nicolaasRestContext, nicolaasMeObj.id, {'displayName': 'user-zfdHliPLa'}, function (err, user) {
                                ...
                                RestAPI.User.getMe(nicolaasRestContext, function(err, nicolaasMeObj) {
                                    ...
                                    RestAPI.Admin.importUsers(camAdminRestContext, global.oaeTests.tenants.cam.alias, getCSVWithPasswordStream, 'local', function(err) {
                                        assert.ok(!err);

                                        // Verify that the displayName is correctly reverted
                                        RestAPI.User.getMe(nicolaasRestContext, function(err, nicolaasMeObj) {
                                            assert.ok(!err);
                                            assert.ok(nicolaasMeObj);
                                            assert.equal(nicolaasMeObj.displayName, 'Nicolaas Matthijs');
                                            assert.equal(nicolaasMeObj.email, 'nicolaas@test.com');
                                            // TODO: Change displayName to something else and verify it doesn't get overwritten
                                            callback();
                                        });
                                    });
                                });
        ...
        it('verify import external users', function(callback) {
            // Import users as a global admin
            // TODO
            console.log(TestsUtil.generateTestUserId());
            callback();
        });
        ...
        // Test permissions
        //  Test as anon on global admin
        //  Test as anon on tenant admin
        //  Test as non-admin on tenant admin

        // Test local import
        //   Successful import
        //   Check that users exist
        //   Check that sign in as user
        //   Check that a username is overriden
