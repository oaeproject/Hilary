...
module.exports = function(app, callback) {
    ensureSchema(function() {
        ConfigAPI.initializeConfig(function() {
            callback();
        });
    });
...
var registerConfigAdminRestEndpoints = function(app) {
    ...
    // Look for match on /api/config or /api/config/tenantid
    app.server.get(/^\/api\/config\/?(\w*)/, function(req, res, next) {
        ConfigAPI.getConfig(req.params[0] || OAE.serverTenant.alias, function(err, configuration) {
            if (err) {
                return res.send(err.code, err.msg);
            }
    ...
    // Look for match on /api/config or /api/config/tenantid
    app.server.post(/^\/api\/config\/?(\w*)/, function(req, res, next) {
        if (!req.user || !req.user.isAdmin(OAE.serverTenant.alias)) {
            return res.send(401, 'Unauthorized');
        }

        ConfigAPI.writeConfig(req.params[0] || OAE.serverTenant.alias, req.body, function(err) {
            if (err) {
                return res.send(err.code, err.msg);
            }
            res.send(200);
