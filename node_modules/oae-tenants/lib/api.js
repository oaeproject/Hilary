        ...
        if (!tenant) {
            getTenantByAlias(alias, function(err, tenant) {
                if (!tenant) {
                    ...
                    Cassandra.runQuery('UPDATE Tenant USING CONSISTENCY QUORUM SET alias = ?, name = ?, description = ?, baseUrl = ?, active = ? WHERE port = ?',
...
var registerAPI = function(tenant, callback) {
    ...
    globalServer = OAE.getGlobalServer();

    globalServer.post('/api/tenant/create', function(req, res, next) {
        createTenant(req.body.id, req.body.name, req.body.description, req.body.port, req.body.baseurl, function(err) {
            if (err) {
    ...
    globalServer.post('/api/tenant/start', function(req, res, next) {
        putTenantsInMaintenanceMode(req.body.tenants, false, function(err) {
            if (err) {
                return res.send(err.code, err.msg);
            }
            res.send(200);
    ...
    globalServer.post('/api/tenant/stop', function(req, res, next) {
        putTenantsInMaintenanceMode(req.body.tenants, true, function(err) {
            if (err) {
                return res.send(err.code, err.msg);
            }
            res.send(200);
    ...
    globalServer.post('/api/tenant/delete', function(req, res, next) {
        deleteTenants(req.body.tenants, function(err) {
            if (err) {
                return res.send(err.code, err.msg);
            }
            res.send(200);
    ...
    globalServer.get('/api/tenants', function(req, res, next) {
        getAllTenants(function(err, tenants) {
            if (err) {
                return res.send(err.code, err.msg);
            }
