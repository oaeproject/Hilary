...
var LoodleAPI = require('oae-doodle/lib/api');
...
var getContent = module.exports.getContent = function(ctx, contentId, callback) {
    ...
    ContentDAO.Content.getContent(contentId, function(err, contentObj) {
        ...
        AuthzPermissions.canView(ctx, contentObj, function(err) {
            ...
            if (contentObj.resourceSubType === 'doodle') {
                if (contentObj.visibility === 'private') {
                    ...
                    // User connected
                    if (currentUser) {
                        // If we get to this point, it means an user is trying to access a loodle
                        // accessible by tenant, which means even if he/she doesn't have a mirror account
                        // in the loodle application, he/she have access to it, so we should get or create
                        // the corresponding mirror account and him/her to the loodle.
                        LoodleAPI.getOrCreateUser(ctx, currentUser.id, function (err, user) {
                            if (err) return callback(err);

                            LoodleAPI.addUserIfNeeded(ctx, contentObj.id, currentUser.id, function (err) {
                                if (err) return callback(err);
...
var createDoodle = module.exports.createDoodle = function (ctx, displayName, description, visibility, additionalMembers, folders, callback) {
    ...
    // Setting content to default if no visibility setting is provided
    visibility = visibility || Config.getValue(ctx.tenant().alias, 'visibility', 'doodles');
    ...
    LoodleAPI.create(ctx, currentUser, contentId, displayName, description, visibility, additionalMembers, function (err) {
        ...
        _createContent(ctx, contentId, revisionId, 'doodle', displayName, description, visibility, additionalMembers, folders, {}, {}, function(err, content, revision, memberChangeInfo) {
            if (err) {
                return callback(err);
            }
...
var _createContent = function(ctx, contentId, revisionId, resourceSubType, displayName, description, visibility, roles, folderIds, otherValues, revisionData, callback) {
    ...
    validator.check(resourceSubType, {'code': 400, 'msg': 'A valid resourceSubType must be provided. This can be "file", "collabdoc", "link" or "doodle"'}).isIn(ContentConstants.resourceSubTypes);
    validator.check(null, {'code': 401, 'msg': 'You have to be logged in to be able to create a content item'}).isLoggedInUser(ctx);
...
var setContentPermissions = module.exports.setContentPermissions = function(ctx, contentId, changes, callback) {
    ...
    getContent(ctx, contentId, function(err, content) {
        ...
        ResourceActions.setRoles(ctx, content, changes, function(err, memberChangeInfo) {
            ...
            if (content.resourceSubType === 'doodle') {
