    ...
    getGroup(ctx, groupId, function(err, group) {
        ...
        _getAllRoles(ctx, groupId, function(err, roles) {
            ...
            OaeUtil.invokeIfNecessary(group.createdBy, PrincipalsUtil.getPrincipal, ctx, group.createdBy, function(err, createdBy) {
                ...
                // As part of the group profile, get the top 9 members in the members library
                getMembersLibrary(ctx, groupId, null, 9, function(err, members) {
                    if (err) {
                        return callback(err);
                    }
...
var getMembersLibrary = module.exports.getMembersLibrary = function(ctx, groupId, start, limit, callback) {
    ...
    getGroup(ctx, groupId, function(err, group) {
        ...
        LibraryAPI.Authz.resolveTargetLibraryAccess(ctx, group.id, group.visibility, function(err, hasAccess, visibility) {
            if (err) {
                return callback(err);
            } else if (!hasAccess) {
            ...
            // Finally get the members from the members library and their basic profile
