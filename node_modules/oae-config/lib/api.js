...
Pubsub.on('oae-config', function(receivedMessage) {
    ...
    getFullConfig(tenantId, false, function() {});
});
 ...
 * @param {Boolean}   getCached  If set to true will try to return cached config before reading from Cassandra
 * @param {Function}  callback   Function executed after config has been read
 */
var getFullConfig = module.exports.getFullConfig = function(tenantId, getCached, callback) {
    var validator = new Validator();
    validator.check(tenantId, {'code': 400, 'msg': 'Missing tenantid'}).notEmpty();
    if (validator.hasErrors()) {
        return callback(validator.getFirstError());
    ...
    if (getCached && cachedConfiguration[tenantId]) {
        log().info('return ' + tenantId + ' from cache');
        return callback(false, cachedConfiguration[tenantId]);
    }

    Cassandra.runQuery('SELECT * FROM Config USING CONSISTENCY QUORUM WHERE tenantId = ?', [OAE.serverTenant.alias], function(err, globalRows) {
        if (!err) {
                            ...
                            Cassandra.runQuery('SELECT * FROM Config USING CONSISTENCY QUORUM WHERE tenantId = ?', [tenantId], function(err, tenantRows) {
                                mergeConfigIntoOriginal(tenantRows[0], config, function(config) {
                                    if (!err) {
                                        ...
                                        cachedConfiguration[tenantId] = config;
...
var filterModulesByConfig = function(modules, callback) {
    ...
    /**
     * Does the check to see if all modules and their config files have been read and executes the callback if so
     */
     ...
     * @param {String}    module    the name of the module to read configuration files for
     */
            ...
            if (!err && files) {
                ...
                for (var i = 0; i < files.length; i++) {
                    if (files[i].indexOf('.json') !== -1) {
                        fs.readFile(dir + files[i], 'binary', function(err, content){
                            c++;
                            if (!err) {
                                content = JSON.parse(content);
                                if (configurableModules[module] && configurableModules[module].config) {
                                    configurableModules[module].config = OAEUtil.extend(true, configurableModules[module].config, content);
                                } else {
