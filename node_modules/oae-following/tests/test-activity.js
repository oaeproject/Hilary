...
var DiscussionsConstants = require('oae-discussions/lib/constants')
...
var FollowingConstants = require('oae-following/lib/constants')
  .FollowingConstants;
...
describe('Following Activity', function() {
  ...
  /*!
     ...
     */
  var _assertFollowActivity = function(
    followActivity,
    actorUserId,
    objectUserId,
  ) {
  ...
  it('verify following user and followed user get a follow activity', function(callback) {
    ...
    FollowingTestsUtil.createFollowerAndFollowed(camAdminRestContext, function(
      follower,
      followed,
    ) {
                                  ...
                                  function(err) {
                                    ...
                                    ActivityTestsUtil.collectAndGetActivityStream(
                                      ...
                                      function(err, response) {
                                        ...
                                        updateMembers = {};
                                        updateMembers[followed.user.id] =
                                          'member';
                                        RestAPI.Group.setGroupMembers(
                                          nico.restContext,
                                          group1.id,
  ...
  it('verify followers from external tenants no longer receive activity for users who become loggedin', function(callback) {
    TestsUtil.generateTestUsers(camAdminRestContext, 1, function(err, users) {
      ...
      TestsUtil.generateTestUsers(gtAdminRestContext, 1, function(err, users) {
        ...
        RestAPI.Following.follow(gtUser.restContext, camUser.user.id, function(
          err,
        ) {
          assert.ok(!err);
  ...
  it('verify user gets notification and email when a user follows them', function(callback) {
    TestsUtil.generateTestUsers(camAdminRestContext, 3, function(err, users) {
      ...
      // Follow the user
      RestAPI.Following.follow(follower.restContext, followed.user.id, function(
        err,
      ) {
        assert.ok(!err);
        ...
        EmailTestsUtil.collectAndFetchAllEmails(function(messages) {
          ...
          assert.notEqual(
            messages[0].html.indexOf(follower.user.profilePath),
            -1,
          );
                  ...
                  function(err) {
                    ...
                    ActivityTestsUtil.collectAndGetNotificationStream(
                      ...
                      function(err, response) {
                        ...
                        RestAPI.User.getMe(followed.restContext, function(
                          err,
                          me,
                        ) {
                          assert.ok(!err);
  ...
  it('verify users get a single aggregated activity in their email when 1 or multiple users follow them', function(callback) {
    TestsUtil.generateTestUsers(camAdminRestContext, 4, function(
      ...
      RestAPI.Following.follow(branden.restContext, simon.user.id, function(
        err,
      ) {
        assert.ok(!err);
        RestAPI.Following.follow(nico.restContext, simon.user.id, function(
          err,
        ) {
          assert.ok(!err);
          ...
          EmailTestsUtil.collectAndFetchAllEmails(function(messages) {
            ...
            RestAPI.Following.follow(bert.restContext, simon.user.id, function(
              err,
            ) {
              assert.ok(!err);
              ...
              EmailTestsUtil.collectAndFetchAllEmails(function(messages) {
                ...
                assert.equal(messages[0].to[0].address, simon.user.email);
                assert.equal(
                  messages[0].html.match(/data-activity-id/g).length,
                  1,
                );

                // Assert both Branden and Nico are not mentioned in the email
                assert.strictEqual(
                  messages[0].html.indexOf(branden.user.profilePath),
                  -1,
                );
                assert.strictEqual(
                  messages[0].html.indexOf(nico.user.profilePath),
                  -1,
                );
  ...
  it('verify following a user, following another, then refollowing the first results in only 1 activity in the activity stream', function(callback) {
    TestsUtil.generateTestUsers(camAdminRestContext, 3, function(err, users) {
      ...
      /*!
             ...
             */
      RestAPI.Following.follow(simon.restContext, branden.user.id, function(
        err,
      ) {
        assert.ok(!err);
        ...
        ActivityTestsUtil.collectAndGetActivityStream(
          ...
          function(err, response) {
            ...
            /*!
                     ...
                     */
            RestAPI.Following.follow(simon.restContext, bert.user.id, function(
              err,
            ) {
              assert.ok(!err);
