 ...
 *                                  keyspace creation
 * @param {Function}    callback    Standard callback function that should be called when the tenants
 *                                  have been created and have started up  
 */
var setUpTenants = function(err, callback) {
    ...
    var globalAdmin = new User('global', null, 'administrator', 'private', 'en_GB', 'Europe/London', 'Admin', 'User', 'The global admin User', true, true);
    var globalContext = new Context(global.oaeTests.tenants.global, globalAdmin);

    // Create the Cambridge tenant
    TenantsAPI.createTenant(globalContext, 'camtest', 'Cambridge University Test', 2001, 'http://localhost:2001', function(err, tenant) {
        global.oaeTests.tenants.cam = tenant;
        // Create the Georgia Tech tenant
        TenantsAPI.createTenant(globalContext, 'gttest', 'Georgia Tech Test', 2002, 'http://localhost:2002', function(err, tenant) {
            ...
            setTimeout(setUpTenantAdmins, 2000, callback);
        });
    });
};
...
var setUpTenantAdmins = function(callback) {
    ...
    var camAdmin = new User(global.oaeTests.tenants.cam.alias, null, 'administrator', 'private', 'en_GB', 'Europe/London', 'Admin', 'User', 'The Cambridge admin User', false, true);
    var camContext = new Context(global.oaeTests.tenants.cam, camAdmin);
    PrincipalsAPI.createUser(camContext, camAdmin.username, 'administrator', camAdmin.visibility, camAdmin.locale, camAdmin.timezone, camAdmin.firstName, camAdmin.lastName, camAdmin.displayName, function(err, createdUser) {
        PrincipalsAPI.setTenantAdmin(camContext, createdUser.id, true, function(err) {
            // Create the Georgia Tech admin
            var gtAdmin = new User(global.oaeTests.tenants.gt.alias, null, 'administrator', 'private', 'en_GB', 'Europe/London', 'Admin', 'User', 'The Georgia Tech admin User', false, true);
            var gtContext = new Context(global.oaeTests.tenants.gt, gtAdmin);
            PrincipalsAPI.createUser(gtContext, gtAdmin.username, 'administrator', gtAdmin.visibility, gtAdmin.locale, gtAdmin.timezone, gtAdmin.firstName, gtAdmin.lastName, gtAdmin.displayName, function(err, createdUser) {
