...
describe('OAE Util', function() {
  describe('#getNumberParam', function() {
    it('verify a variety of inputs for getNumberParam', function(callback) {
      ...
      assert.equal(
        OaeUtil.getNumberParam(null, { test: 'worked' }).test,
        'worked',
      );
  ...
  describe('#invokeIfNecessary', function() {
     ...
     */
    it('verify the method is not invoked with a falsey isNecessary parameter', function(callback) {
      // Ensure _toInvoke is not called with `false`
      OaeUtil.invokeIfNecessary(false, _toInvoke, 'invoked', function(
        toReturn,
      ) {
        assert.ok(!toReturn);

        // Ensure _toInvoke is not called with `null`
        OaeUtil.invokeIfNecessary(null, _toInvoke, 'invoked', function(
          toReturn,
        ) {
          assert.ok(!toReturn);

          // Ensure _toInvoke is not called with `undefined`
          OaeUtil.invokeIfNecessary(undefined, _toInvoke, 'invoked', function(
            toReturn,
          ) {
            assert.ok(!toReturn);

            // Ensure _toInvoke is not called with the empty string
            OaeUtil.invokeIfNecessary('', _toInvoke, 'invoked', function(
              toReturn,
            ) {
              assert.ok(!toReturn);
     ...
     * "isNecessary" parameter
     */
    it('verify the method is invoked with a truthy isNecessary parameter', function(callback) {
      ...
      OaeUtil.invokeIfNecessary(true, _toInvoke, 'invoked', function(toReturn) {
        ...
        OaeUtil.invokeIfNecessary(
          ...
          function(toReturn) {
            ...
            OaeUtil.invokeIfNecessary(1, _toInvoke, 'invoked', function(
              toReturn,
            ) {
              assert.equal(toReturn, 'invoked');
  ...
  describe('#toArray', function() {
     ...
     */
    it('verify the method validates incoming values properly and returns an Array', function(callback) {
      var testObj = {
        key1: 'value1',
        key2: 'value2',
      };
