    ...
    describe('Has pending email verification token', function() {
        ...
        it('verify has pending email verification token functionality', function(callback) {
            TestsUtil.generateTestUsers(camAdminRestContext, 1, function(err, users, simong) {
                ...
                PrincipalsTestUtil.assertHasPendingEmailTokenFails(simong.restContext, simong.user.id, 404, function() {
                    ...
                    PrincipalsTestUtil.assertUpdateUserSucceeds(simong.restContext, simong.user.id, {'email': email}, function() {
                        ...
                        PrincipalsTestUtil.assertHasPendingEmailTokenSucceeds(simong.restContext, simong.user.id, function(emailForToken) {
        ...
        it('verify validation when checking for a pending email verification token', function(callback) {
            TestsUtil.generateTestUsers(camAdminRestContext, 1, function(err, users, simong) {
                ...
                // Invalid user id
                PrincipalsTestUtil.assertHasPendingEmailTokenFails(simong.restContext, 'not a user id', 400, function() {
                    PrincipalsTestUtil.assertHasPendingEmailTokenFails(simong.restContext, 'g:camtest:1234234', 400, function() {
                        return callback();
                    });
                });
            });
        ...
        it('verify authorization when checking for a pending email verification token', function(callback) {
            TestsUtil.generateTestUsers(camAdminRestContext, 2, function(err, users, simong, mrvisser) {
                ...
                PrincipalsTestUtil.assertUpdateUserSucceeds(simong.restContext, simong.user.id, {'email': email}, function() {
                    ...
                    PrincipalsTestUtil.assertHasPendingEmailTokenFails(anonymousRestContext, simong.user.id, 401, function() {

                        // Users cannot check other users their email tokens
                        PrincipalsTestUtil.assertHasPendingEmailTokenFails(mrvisser.restContext, simong.user.id, 401, function() {

                            // Tenant administrator cannot check users from another tenant their email token
                            PrincipalsTestUtil.assertHasPendingEmailTokenFails(gtAdminRestContext, simong.user.id, 401, function() {
                                ...
                                PrincipalsTestUtil.assertHasPendingEmailTokenSucceeds(simong.restContext, simong.user.id, function(emailForToken) {
                                    ...
                                    PrincipalsTestUtil.assertHasPendingEmailTokenSucceeds(camAdminRestContext, simong.user.id, function(emailForToken) {
                                        return callback();
                                    });
                                });
                            });
