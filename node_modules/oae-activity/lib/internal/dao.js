 ...
 *
 * @param  {String}         userId              The id of the user whose notifications we should fetch
 * @param  {Number|String}  [start]             Number of millis since the epoc (stringified or number version) from which to start returning notifications. Only notifications older than this timestamp will be returned. By default, starts from the newest.
 * @param  {Number}         [limit]             The maximum number of notifications to return. Default: 25
 * @param  {Function}       callback            Invoked when the process completes
 * @param  {Object}         callback.err        An error that occurred, if any
 * @param  {Activity[]}     callback.activities The list of notifications
 */
var getNotifications = module.exports.getNotifications = function(userId, start, limit, callback) {
    // Simply delegate to getActivities, abstracting the #notification tag
    getActivities(userId + '#notification', start, limit, callback);
};
