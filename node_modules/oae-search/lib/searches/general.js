...
var RESOURCE_TYPES_ACCESS_SCOPED = [
 ...
 * @param  {Boolean}        [opts.resourceCanManage]    Required for `_share` scope. Specifies if the current user in context can manage the resource being shared
 * @param  {String}         [opts.resourceTenantAlias]  Required for `_share` scope. Specifies the tenant alias of the resource being shared
 * @param  {String}         [opts.resourceVisibility]   Required for `_share` scope. Specifies the visibility of the resource being shared
...
module.exports = function(ctx, opts, callback) {
    ...
    if (opts.scope === SearchConstants.general.SCOPE_SHARE) {
        opts.resourceCanManage = OaeUtil.castToBoolean(opts.resourceCanManage);
        validator.check(null, {'code': 401, 'msg': 'Anonymous users are not authorized to share'}).isLoggedInUser(ctx);
        validator.check(null, {'code': 400, 'msg': 'When using "_share" scope, true/false must be provided for "resourceCanManage"'}).isBoolean(opts.resourceCanManage);
        validator.check(opts.resourceTenantAlias, {'code': 400, 'msg': 'When using "_share" scope, a "resourceTenantAlias" must be specified'}).notEmpty();
        validator.check(opts.resourceVisibility, {'code': 400, 'msg': 'When using "_share" scope, a valid "resourceVisibility" must be specified'}).isValidVisibility();
    ...
    opts.resourceInfo = {
        'canManage': opts.resourceCanManage,
        'tenantAlias': opts.resourceTenantAlias,
        'visibility': opts.resourceVisibility
    };
...
var _search = function(ctx, opts, callback) {
    ...
    var filterScopeAndAccessOpts = {
        'explicit': _needsFilterByExplicitAccess(ctx, opts),
        'resourceInfo': opts.resourceInfo
    };

    SearchUtil.filterScopeAndAccess(ctx, opts.scope, filterScopeAndAccessOpts, function(err, filterScopeAndAccess) {
        if (err) {
            return callback(err);
        }
...
var _createQuery = function(ctx, opts) {
    ...
    var boost = (hasContent || hasDiscussion || hasFolder) ? 5 : null;
    var query = {
        'bool': {
            'should': [
...
var _needsFilterByExplicitAccess = function(ctx, opts) {
    ...
    var includesAccessScopedResources = (
    ...
    return isAuthenticated && isNotGlobalAdmin && includesAccessScopedResources && hasTextQuery;
};
