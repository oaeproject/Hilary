 ...
 * TODO
 */
var createTenantAndWait = module.exports.createTenantAndWait = function(restCtx, alias, displayName, host, callback) {
    ...
    RestAPI.Tenants.createTenant(restCtx, tenantAlias, update, function(err) {
        if (err) {
    ...
    /*!
     * Handles the 'update' tenants event, simply notifying the `_callback` that the tenant has been created
     *
     * @see TenantsAPI events for parameter description
     */
    var _updateListener = function(tenant) {
        tenantCreated = tenant;
        _callback();
    };

    TenantsAPI.once('update', _updateListener);
