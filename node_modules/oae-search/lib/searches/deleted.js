...
module.exports = function(ctx, opts, callback) {
  ...
  if (!user || !user.isAdmin(user.tenant.alias)) {
    return callback({
      code: 401,
      msg: 'You are not authorized to search deleted items',
    });
  }
  ...
  var filterResources = SearchUtil.filterResources(
    opts.resourceTypes,
    SearchConstants.deleted.ONLY,
  );

  // Apply the scope and access filters for the deleted search
  SearchUtil.filterScopeAndAccess(ctx, opts.scope, false, function(
    err,
