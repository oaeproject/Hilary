...
var _createContent = function(ctx, contentId, revisionId, resourceSubType, displayName, description, visibility, additionalMembers, folders, otherValues, revisionData, callback) {
    ...
    _getExistingPrincipals(additionalMemberIds, null, function(err, principals) {
        if (err) {
            return callback(err);
        }
...
var shareContent = module.exports.shareContent = function(ctx, contentId, principalIds, callback) {
    ...
    ContentDAO.Content.getContent(contentId, function(err, contentObj) {
        ...
        _getExistingPrincipals(principalIds, null, function(err, principals) {
            if (err) {
                return callback(err);
            }

            AuthzPermissions.canShare(ctx, contentObj, {'principals': principals}, function(err, newMembers) {
                if (err) {
                    return callback(err);
                } else if (_.isEmpty(newMembers)) {
                    // There are no actual shares to perform, don't do anything
                    return callback();
                }
    ...
    getContent(ctx, contentId, function(err, contentObj) {
        ...
        _getExistingPrincipals(principalIds, null, function(err, principals) {
            ...
            var principalRoles = _.map(principals, function(principal) {
                return {
                    'principal': principal,
                    'role': permissionChanges[principal.id]
                };
            });

            AuthzPermissions.canSetRoles(ctx, contentObj, {'principalRoles': principalRoles}, function(err, memberRoles, newMembers, updatedMembers, removedMembers) {
                if (err) {
                    return callback(err);
                }
