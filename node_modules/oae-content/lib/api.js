...
var setContentPermissions = module.exports.setContentPermissions = function(ctx, contentId, permissionChanges, callback) {
    ...
    getContent(ctx, contentId, function(err, contentObj) {
        ...
        // Check that the user isn't trying to assign any role higher than his own
        AuthzAPI.resolveEffectiveRole(ctx, contentObj.id, contentObj.tenant.alias, contentObj.visibility, ContentConstants.roles.ALL_PRIORITY, function(err, effectiveRole, canInteract) {
            if (err) {
                return callback(err);
            } else if (!canInteract) {
                return callback({'code': 401, 'msg': 'You are not allowed to interact with this piece of content'});
            }
            ...
            ContentAuthz.canManageContent(ctx, contentObj, function(err, canManage) {
                ...
                AuthzAPI.getDirectRoles(_.keys(permissionChanges), contentObj.id, function(err, existingRoles) {
                    if (!canManage) {
                        if (_.contains(permissionChanges, false)) {
                            return callback({'code': 401, 'msg': 'Only managers can remove access'});
                        }

                        var settingHigherRole = _.find(permissionChanges, function(role, principalId) {
                            return ContentConstants.roles.ALL_PRIORITY.indexOf(effectiveRole) < ContentConstants.roles.ALL_PRIORITY.indexOf(role);
                        });
                        if (settingHigherRole) {
                            return callback({'code': 401, 'msg': 'You cannot set another user\'s role higher than your own'});
                        }
                        var hasHigherRole = _.find(existingRoles, function(role, principalId) {
                            return ContentConstants.roles.ALL_PRIORITY.indexOf(effectiveRole) < ContentConstants.roles.ALL_PRIORITY.indexOf(role);
                        });
                        if (hasHigherRole) {
                            return callback({'code': 401, 'msg': 'You cannot change the role of someone with a higher role than your own'});
                        }
                    }
