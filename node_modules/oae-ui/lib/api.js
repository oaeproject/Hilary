...
/**
 * Caches all the bundle files in-memory
 *
 * @param  {Function}   callback        Standard callback function
...
var _cacheI18nKeys = function(callback) {
    ...
    readdirp({
        'root': uiDirectory,
        'directoryFilter': ['!.bin', '!custom', '!docs', '!etc', '!tests', '!shared', '!tools'],
        'fileFilter': '*.properties'
    })
    .on('data', function (entry) {
        _cacheBundleFile('/' + entry.path, function(err) {
            if (err) {
                return done(err);
            }
        });
    ...
    .on('error', function (err) { log().error({'err': err}, 'A fatal error occured whilst eading the i18n bundles'); })
    .on('end', done);
};
...
var _cacheBundleFile = function(bundlePath, callback) {
    ...
    getStaticFile(bundlePath, function(err, data) {
        if (err) {
            log().error({'err': err, 'path': entry.path}, 'Could not read an i18n bundle');
 ...
 * @param  {String}     locale  The locale that should be used to translate any i18n keys
 * @return {String}             The rendered and translated template
 */
var renderTemplate = module.exports.renderTemplate = function(tpl, data, locale) {
    data = data || {};
    data.util = {
        /**
        ...
        /**
         * Extracts the URI from a signed URL.
         *
         * @param  {String}     str     The signed URL (/api/download/signed?uri=....&signature=..) to extract the URI from
         * @return {String}             The URI or an empty string if no uri was found
         */
        'uriFromURL': function(str) {
            if (!str) {
                return '';
            }
            var parsed = url.parse(str, true);
            if (parsed.query && parsed.query.uri) {
                return parsed.query.uri;
            }
            return '';
        }
    };

    // Translate the template first
    var translatedTemplate = translate(tpl, locale);

    // Parse and process it with Underscore
    try {
        var compiledTemplate = _.template(translatedTemplate);
