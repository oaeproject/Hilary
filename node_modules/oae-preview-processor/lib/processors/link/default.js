...
var generatePreviews = (module.exports.generatePreviews = function(
  ...
  request(options, function(err, response) {
    if (err) {
      ...
      forcedDownload =
        forcedDownload ||
        response.headers['content-type'] === PreviewConstants.TYPES.DEFAULT;
    ...
    var generateThumbnail = function() {
      ...
      if (
        contentType &&
        _.contains(PreviewConstants.TYPES.IMAGE, contentType)
      ) {
        var image = fs.createWriteStream(imgPath);
        image.on('close', function() {
          LinkProcessorUtil.generatePreviewsFromImage(
            ctx,
            imgPath,
            null,
            callback,
        ...
        webshot.getImage(contentObj.link, imgPath, webshotOptions, function(
          err,
        ) {
          if (err) {
            log().error(
          ...
          gm.compare(
            ...
            function(err, isEqual, equality, raw) {
              ...
              if (isEqual) {
                log().info(
                  { contentId: ctx.contentId },
                  'Not attaching blank screenshot',
                );
                return callback();
              } else {
