...
describe('Group Push', function() {
                    ...
                    ActivityTestsUtil.getFullySetupClient(data, function(client) {
                        ...
                        RestAPI.Group.createGroup(simong.restContext, 'displayName', 'description', 'public', 'yes', [branden.user.id], null, function(err, group) {
                            ...
                            RestAPI.Group.getGroup(simong.restContext, group.id, function(err, group) {
                                ...
                                client.subscribe(group.id, null, group.signature, function(err) {
                                    assert.equal(err.code, 400);

                                    // Ensure we get a 400 error with a missing resource id
                                    client.subscribe(null, 'activity', group.signature, function(err) {
                                        assert.equal(err.code, 400);

                                        // Ensure we get a 401 error with an invalid token
                                        client.subscribe(group.id, 'activity', {'signature': group.signature.signature}, function(err) {
                                            assert.equal(err.code, 401);
                                            client.subscribe(group.id, 'activity', {'expires': group.signature.expires}, function(err) {
                                                assert.equal(err.code, 401);
                                                client.subscribe(group.id, 'activity', {'expires': Date.now() + 10000, 'signature': 'foo', 'lastModified': Date.now()}, function(err) {
                                                    assert.equal(err.code, 401);
                                                    ...
                                                    RestAPI.Group.getGroup(branden.restContext, group.id, function(err, groupForBranden) {
                                                        assert.ok(!err);
                                                        client.subscribe(group.id, 'activity', groupForBranden.signature, function(err) {
                                                            assert.equal(err.code, 401);
                                                            callback();
                                                        });
                                                    });
    ...
    describe('Notifications', function() {
        ...
        var setupFixture = function(callback) {
            TestsUtil.generateTestUsers(localAdminRestContext, 3, function(err, users) {
                assert.ok(!err);

                var contexts = {
                    'branden': _.values(users)[0],
                    'simon': _.values(users)[1],
                    'nico': _.values(users)[2]
                };
