...
var removeContentItemsFromFolder = module.exports.removeContentItemsFromFolder = function(ctx, folderId, contentIds, callback) {
    ...
    _getFolder(folderId, function(err, folder) {
        ...
        FoldersAuthz.canRemoveItemsFromFolder(ctx, folder, function(err, canRemoveItems) {
            ...
            // Grab the actual content item objects to ensure they exist
            ContentDAO.Content.getMultipleContentItems(contentIds, null, function(err, contentItems) {
                if (err) {
                    return callback(err);
                }

                // Ensure we could retrieve all items
                var knownContentIds = _.chain(contentItems).compact().pluck('id').value();
                var unknownContentIds = _.difference(contentIds, knownContentIds);
                if (unknownContentIds.length > 0) {
                    return callback({'code': 404, 'msg': 'Some content items could not be found: ' + unknownContentIds.join(', ')});
                }

                // Remove the items from the authz group which is the canonical source
                _removeContentItemsFromFolder(folder, contentIds.slice(), function(err) {
                    if (err) {
                        return callback(err);
                    }

                    // Remove the items from the library buckets for this folder
        ...
        LibraryAPI.Authz.resolveTargetLibraryAccess(ctx, folder.groupId, folder.visibility, function(err, hasAccess, visibility) {
            ...
            FoldersContentLibrary.list(folder, visibility, {'start': start, 'limit': limit}, function(err, contentIds, nextToken) {
                ...
                ContentDAO.Content.getMultipleContentItems(contentIds, null, function(err, contentItems) {
                    ...
                    // In case a content item could not be found (due to a faulty library or a slight delay)
                    // between a content delete and library update, we filter out null values
