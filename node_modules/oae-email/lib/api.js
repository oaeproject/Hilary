        ...
        EmailRateLimiter.count(emailInfo.to, throttleConfig.timespan, function(err, count) {
            ...
            EmailRateLimiter.add(emailInfo.to, function(err) {
                ...
                emailTransport.sendMail(emailInfo, function(err, response) {
                    if (err) {
                        log().error({'err': err, 'emailInfo': emailInfo}, 'Error sending email to recipient.');
                        return callback(err);
                    }
                    // If we're debugging we log the mail that would have been sent
                    if (debug) {
                        log().info(response, 'Sending email to %s', emailInfo.to);
                        var mailparser = new MailParser();
                        mailparser.on('end', function(email_object){
                            EmailAPI.emit('debugSent', email_object);
                            return callback();
                        });
                        mailparser.write(response.response);
                        mailparser.end();
                    } else {
                        return callback();
                    }
