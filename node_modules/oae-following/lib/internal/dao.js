...
/**
 * Given a list of potential followed user ids, determine which of them are being followed by the following user.
 *
...
var isFollowing = module.exports.isFollowing = function(followingUserId, followedUserIds, callback) {
    ...
    Cassandra.runQuery('SELECT' + paramFollowingPlaceholders.join(',') + ' FROM FollowingUsersFollowing USING CONSISTENCY QUORUM WHERE userId = ?', paramValues, function(err, row) {
        if (err) {
            return callback(err);
        }
...
var saveFollows = module.exports.saveFollows = function(followerUserId, followedUserIds, callback) {
    ...
    // Create the string that will represent the set fields in the CQL query: "? = '1', ? = '1', ..."
    // Each column's name is the id of the user being followed, where the value is just the value 1
    var placeholders = _.map(followedUserIds, function(followedUserId) { return '? = \'1\''; }).join(', ');
    var queries = [{
        'query': 'UPDATE FollowingUsersFollowing SET ' + placeholders + ' WHERE userId = ?',
