    ...
    getContent(ctx, contentId, function(err, contentObj) {
        ...
        MessageBoxAPI.getMessages(contentId, [commentCreatedDate], null, function(err, messages) {
            ...
            if (ctx.user().id === message.createdBy) {
                return _deleteComment(ctx, contentObj, commentCreatedDate, callback);
            }
 ...
 *
 * @param  {Context}        ctx                     Standard context object, representing the currently logged in user and its tenant
 * @param  {Content}        contentObj              The Content object on which a comment should be deleted
 ...
 */
var _deleteComment = function(ctx, contentObj, commentCreatedTimestamp, callback) {
    var messageBoxId = contentObj.id;
    ...
    MessageBoxAPI.deleteMessage(messageBoxId, commentCreatedTimestamp, {'deleteType': MessageBoxConstants.deleteTypes.LEAF}, function(err, deleteType, message) {
        ...
        ContentAPI.emit(ContentConstants.events.DELETED_COMMENT, ctx, contentObj, commentCreatedTimestamp, deleteType, message);
...
var _augmentContent = function(ctx, contentObj) {
    ...
    if (contentObj.previews) {
        if (contentObj.previews.thumbnailUri) {
            contentObj.previews.thumbnailUrl = _getDownloadUrlFromUri(ctx, contentObj.previews.thumbnailUri, contentObj.id);
        }

        if (contentObj.previews.smallUri) {
            contentObj.previews.smallUrl = _getDownloadUrlFromUri(ctx, contentObj.previews.smallUri, contentObj.id);
        }

        if (contentObj.previews.mediumUri) {
            contentObj.previews.mediumUrl = _getDownloadUrlFromUri(ctx, contentObj.previews.mediumUri, contentObj.id);
        }

        if (contentObj.previews.largeUri) {
            contentObj.previews.largeUrl = _getDownloadUrlFromUri(ctx, contentObj.previews.largeUri, contentObj.id);
        }
    }
};
