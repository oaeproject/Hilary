...
var generatePreviews = (module.exports.generatePreviews = function(
  ...
  RestAPI.Content.getRevisions(
    ...
    function(err, revisions) {
      ...
      _writeEtherpadHtml(ctx, contentObj.latestRevision.etherpadHtml, function(
        err,
        etherpadFilePath,
      ) {
        if (err) {
  ...
  _getWrappedEtherpadHtml(ctx, etherpadHtml, function(err, wrappedHtml) {
    ...
    fs.writeFile(etherpadFilePath, wrappedHtml, function(err) {
      if (err) {
        log().error(
          { err: err, contentId: ctx.contentId },
          'Could not write the etherpad HTML to disk',
        );
        return callback({
          code: 500,
          msg: 'Could not write the etherpad HTML to disk',
        });
      }
...
var _getWrappedEtherpadHtml = function(ctx, etherpadHtml, callback) {
  ...
  try {
    ...
    log().error(
      { err: err, etherpadHtml: etherpadHtml, contentId: ctx.contentId },
  ...
  if (wrapperHtml) {
    ...
    fs.readFile(HTML_FILE, 'utf8', function(err, content) {
      if (err) {
        log().error(
          { err: err, contentId: ctx.contentId },
          'Could not read the collabdoc wrapper HTML',
        );
        return callback({
          code: 500,
          msg: 'Could not read the collabdoc wrapper HTML',
        });
      }
