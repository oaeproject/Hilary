 ...
 */
const MQ = module.exports = new EmitterAPI.EventEmitter();
 ...
 */
const rejectMessage = module.exports.rejectMessage = function(message, requeue, callback) {
 ...
 */
const init = module.exports.init = function(mqConfig, callback) {
 ...
 * @param  {Function}   callback            Standard callback function
 */
const declareExchange = module.exports.declareExchange = function(exchangeName, exchangeOptions, callback) {

    if (!exchangeName) {
        log().error({exchangeName, 'err': new Error('Tried to declare an exchange without providing an exchange name')});
    ...
    channel.assertExchange(exchangeName, exchangeOptions.type, exchangeOptions).then((exchange) => {
 ...
 * @param  {Function}   callback            Standard callback function
 */
const declareQueue = module.exports.declareQueue = function(queueName, queueOptions, callback) {
    if (!queueName) {
        log().error({queueName, queueOptions, 'err': new Error('Tried to declare a queue without providing a name')});
    ...
    channel.assertQueue(queueName, queueOptions).then(queue => {
 ...
 */
const isQueueDeclared = module.exports.isQueueDeclared = function(queueName) {
...
var _processBindQueue = function() {
    ...
    if (queuesToBind.length > 0 && !isWorking) {
        ...
        channel.bindQueue(todo.queueName, todo.exchangeName, todo.routingKey).then(queue => {
            ...
            log().error({ 'queueName': todo.queueName, 'err': new Error('Unable to bind queue to an exchange') });
        });
    }
};
 ...
 * @param  {Object}     callback.err    An error that occurred, if any
 */
const bindQueueToExchange = module.exports.bindQueueToExchange = function(queueName, exchangeName, routingKey, callback) {
    if (!queues[queueName]) {
        log().error({queueName, exchangeName, routingKey, 'err': new Error('Tried to bind a non existing queue to an exchange, have you declared it first?')});
        return callback({'code': 400, 'msg': 'Tried to bind a non existing queue to an exchange, have you declared it first?'});
    } if (!exchanges[exchangeName]) {
        log().error({queueName, exchangeName, routingKey, 'err': new Error('Tried to bind a queue to a non-existing exchange, have you declared it first?')});
        return callback({'code': 400, 'msg': 'Tried to bind a queue to a non-existing exchange, have you declared it first?'});
    } if (!routingKey) {
        log().error({queueName, exchangeName, routingKey, 'err': new Error('Tried to bind a queue to an existing exchange without specifying a routing key')});
 ...
 * @param  {Object}     callback.err    An error that occurred, if any
 */
const unbindQueueFromExchange = module.exports.unbindQueueFromExchange = function(queueName, exchangeName, routingKey, callback) {
    if (!queues[queueName]) {
        log().error({queueName, exchangeName, routingKey, 'err': new Error('Tried to unbind a non existing queue from an exchange, have you declared it first?')});
        return callback({'code': 400, 'msg': 'Tried to unbind a non existing queue from an exchange, have you declared it first?'});
    } if (!exchanges[exchangeName]) {
        log().error({queueName, exchangeName, routingKey, 'err': new Error('Tried to unbind a queue from a non-existing exchange, have you declared it first?')});
        return callback({'code': 400, 'msg': 'Tried to unbind a queue from a non-existing exchange, have you declared it first?'});
    } if (!routingKey) {
        log().error({queueName, exchangeName, routingKey, 'err': new Error('Tried to unbind a queue from an exchange without providing a routingKey')});
 ...
 * @param  {Object}     callback.err        An error that occurred, if any
 */
const subscribeQueue = module.exports.subscribeQueue = function(queueName, subscribeOptions, listener, callback) {
    ...
    if (!queues[queueName]) {
        log().error({queueName, subscribeOptions, 'err': new Error('Tried to subscribe to an unknown queue')});
    ...
    channel.consume(queueName, (msg) => {
        ...
        if (deliveryInfo.redelivered) {
            ...
            MQ.submit(MqConstants.REDELIVER_EXCHANGE_NAME, MqConstants.REDELIVER_QUEUE_NAME, redeliveryData, MqConstants.REDELIVER_SUBMIT_OPTIONS, (err) => {
                if (err) {
...
var unsubscribeQueue = module.exports.unsubscribeQueue = function(queueName, callback) {
    ...
    if (!queue || !queue.queue) {
        log().warn({queueName, 'err': new Error('Attempted to unbind listener from non-existant job queue. Ignoring')});
        return callback();
    }
    const consumerTag = queue.consumerTag;
    queue = queue.queue;

    channel.cancel(consumerTag).then((ok) => {
 ...
 */
const submit = module.exports.submit = function(exchangeName, routingKey, data, options, callback) {
    options = options || {};
    callback = callback || function() {};

    if (!exchanges[exchangeName]) {
        log().error({exchangeName, routingKey, 'err': new Error('Tried to submit a message to an unknown exchange')});
        return callback({'code': 400, 'msg': 'Tried to submit a message to an unknown exchange'});
    } if (!routingKey) {
        log().error({exchangeName, routingKey, 'err': new Error('Tried to submit a message without specifying a routingKey')});
        return callback({'code': 400, 'msg': 'Tried to submit a message without specifying a routingKey'});
    }

    MQ.emit('preSubmit', routingKey);

    channelWrapper.publish(exchangeName, routingKey, data, options, (err, ok) => {
        if (err) {
 ...
 */
const getBoundQueueNames = module.exports.getBoundQueueNames = function() {
    return _.chain(queues).keys().filter((queueName) => { return !_.isUndefined(queues[queueName].consumerTag); }).value();
};
...
var purge = module.exports.purge = function(queueName, callback) {
    ...
    channel.purgeQueue(queueName).then(data => {
        if (!data) {
            log().error({err, queueName}, 'Error purging queue');
 ...
 * @param  {Object}     [callback.err]  An error that occurred purging the queue, if any
 */
const purgeAll = module.exports.purgeAll = function(callback) {
    callback = callback || function(err) {
        if (err) {
