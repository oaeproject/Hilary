/*!
 * Copyright 2012 Sakai Foundation (SF) Licensed under the
 * Educational Community License, Version 2.0 (the "License"); you may
 * not use this file except in compliance with the License. You may
 * obtain a copy of the License at
 *
 *     http://www.osedu.org/licenses/ECL-2.0
 *
 * Unless required by applicable law or agreed to in writing,
 * software distributed under the License is distributed on an "AS IS"
 * BASIS, WITHOUT WARRANTIES OR CONDITIONS OF ANY KIND, either express
 * or implied. See the License for the specific language governing
 * permissions and limitations under the License.
 */

var fs = require('fs');
var _ = require('underscore');
var dox = require('dox');

var IO = require('oae-util/lib/io');
var Modules = require('oae-util/lib/modules');
var Elements = require('oae-util/lib/elements');
var Validator = require('oae-util/lib/validator').Validator;
var log = require('oae-logger').logger('oae-doc');

var cachedDocs = {};
var cachedUIDocs = {};

/**
 * Retrieves the docs for the requested module
 * 
 * @param  {String}     moduleId        The module to get docs for
 * @param  {Function}   callback        The function to call when complete
 * @param  {Object}     callback.doc    The documentation for the module requested
 */
var getDoc = module.exports.getDoc = function(moduleId, callback) {
    var validator = new Validator();
    validator.check(moduleId, {'code': 400, 'msg': 'Missing moduleId'}).notEmpty();
    validator.check(moduleId.substring(0, 4), {'code': 400, 'msg': 'No such OAE module'}).equals('oae-');
    validator.check(moduleId, {'code': 400, 'msg': 'ModuleId can\'t contain \'.\''}).notContains('.');
    if (validator.hasErrors()) {
        return callback(validator.getFirstError());
    }

    if (cachedDocs[moduleId]) {
        return callback(null, cachedDocs[moduleId]);
    } else {
        return callback({'code': 404, 'msg': 'No documentation for this module was found'});
    }
};

/**
 * Retrieves the docs for the requested UI API Element
 * 
 * @param  {String}     uiElementId        The UI API Element to get docs for
 * @param  {Function}   callback        The function to call when complete
 * @param  {Object}     callback.doc    The documentation for the module requested
 */
var getUIDoc = module.exports.getUIDoc = function(uiElementId, callback) {
    var validator = new Validator();
    validator.check(uiElementId, {'code': 400, 'msg': 'Missing uiElementId'}).notEmpty();
    if (validator.hasErrors()) {
        return callback(validator.getFirstError());
    }
    if (cachedUIDocs[uiElementId]) {
        return callback(null, cachedUIDocs[uiElementId]);
    } else {
        return callback({'code': 404, 'msg': 'No documentation for this module was found'});
    }
};

/**
 * Parse module doc into json
 *
 * @param   {string}      moduleId        The module to get docs for
 * @param   {Function}    callback        The function to call when complete
 * @param   {Object}      callback.doc    The documentation for the module requested
 * @api private
 */
var readDoc = function(moduleId, callback) {

    var dir = 'node_modules/' + moduleId + '/lib';

    /*!
     * Filters out all non-javascript files and folders, as we don't want to generate documentation for these, 
     * and returns the filtered array.
     * 
     * @param  {String[]}   libs       The unfiltered Array of file names in the module lib directory
     * @return {String[]}              The returned, filtered, Array of file names that can be used to generate documentation for
     */
    var filterLibs = function(libs) {
        var allowedLibs = [];
        for (var i = 0; i < libs.length; i++) {
            if (libs[i].indexOf('.js') !== -1) {
                allowedLibs.push(libs[i]);
            }
        }
        return allowedLibs;
    };

    IO.getFileListForFolder(dir, function(libs) {
        // Filter out all non-javascript files
        libs = filterLibs(libs);

        var toDo = libs.length;
        var done = 0;
        var doc = {};

        for (var i=0; i < libs.length; i++) {
            var lib = libs[i];
            (function(lib) {
                IO.readFile(dir + '/' + lib, function(err, data) {
                    done++;
                    if (!err) {
                        try {
                            doc[lib] = dox.parseComments(data);
                        } catch (err) {
                            log().warn({
                                err: err,
                                data: data
                            }, 'Failed parsing comment data with dox for file %s. Ignoring.', dir + '/' + lib);
                        }
                    } else {
                        log().error('Failed reading ' + dir + '/' + lib);
                    }

                    if (done === toDo) {
                        return callback(err, doc);
                    }
                });
            })(lib);
        }

        if (!libs.length) {
            callback({'code': 404, 'msg': 'No documentation for this module was found'});
        }
    });
};
/**
 * Parse UI API Element doc into json
 *
 * @param   {string}      uiElementId       The UI API Element to get docs for
 * @param   {Function}    callback          The function to call when complete
 * @param   {Object}      callback.doc      The documentation for the module requested
 * @api private
 */
var readUIDoc = function(uiElementId, callback) {

    var dir = '../3akai-ux/shared/oae/api';
    var doc = {};
    IO.readFile(dir + '/oae.api.' + uiElementId+'.js', function(err, data) {
        if (!err) {
            try {
                doc[uiElementId] = dox.parseComments(data);
            } catch (err) {
                log().warn({
                    err: err,
                    data: data
                }, 'Failed parsing comment data with dox for file');
            }
        } else {
            log().error('Failed reading'+uiElementId);
        }
            return callback(err, doc);
    });
};

/**
 * Initializes the documentation by fetching all modules + configuration for those modules and caching it
 * 
 * @param   {Function}   callback              The function to call when complete
 */
var initializeDocs = module.exports.initializeDocs = function(callback) {
    var modules = Modules.getAvailableModules();
    var toDo = modules.length;
    var done = 0;

    for (var i = 0; i < toDo; i++) {
        var moduleId = modules[i];

        (function(moduleId) {
            readDoc(moduleId, function(err, doc) {
                done++;
                cachedDocs[moduleId] = doc;
            });
        })(moduleId);
    }

    var elements = Elements.getAvailableUIAPIElements();
    toDo = elements.length;
    done = 0;

    for (var i = 0; i < toDo; i++) {
        var elementId = elements[i];

        (function(elementId) {
            readUIDoc(elementId, function(err, doc) {
                done++;
                cachedUIDocs[elementId] = doc;
            });
        })(elementId);
    }
    callback();
};
