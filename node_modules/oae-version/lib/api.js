      ...
      gift.init(path.resolve(hilaryDirectory, _uiPath), (err, submodule) => {
        submodule.current_commit((err, lastCheckedOutCommit) => {
          if (submodulePointer === lastCheckedOutCommit.id) {
            ...
            return callback({
              code: 500,
              msg: "The submodule hasn't been checked out properly",
            });
          }
        });
      });
...
function _getBuildVersion(buildInfoPath, callback) {
  fs.readFile(buildInfoPath, function(err, buildInfo) {
    ...
    try {
      ...
      return callback({
        code: 500,
        msg: 'Unable to parse the build info file',
      });
    }
    ...
    repo.tags(function(err, tags) {
      ...
      repo.branch(function(err, branch) {
        if (err) {
          ...
          return callback({
            code: 500,
            msg: 'Could not get the branch information',
          });
        }
 ...
 * @api private
 */
function _getGitDescribeVersion(
  repo,
  branchName,
  tagsByCommitId,
  callback,
  ...
  repo.commits(branchName, 30, _nrOfCommits, function(err, commits) {
    ...
    if (lastTag) {
      let describeVersion = util.format(
        '%s+%d+%s',
        lastTag,
        _nrOfCommits,
        _lastCommitSha,
