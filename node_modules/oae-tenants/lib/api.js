...
module.exports.startTenant = function(tenant, callback) {
    Pubsub.publish('oae-tenants', 'start ' + tenant.port, callback);
};
    ...
    if (tenant) {
        if (runningServers[tenant.port]) {
            ...
            registerAPI(tenant, function() {
                Server.postInitializeServer(tenant);
                callback(false, !tenant.active);
            });
        }
    } else {
 ...
 * @param  {Number[]}     ports           An array of ports that should be stopped.
 ...
 * @param  {Object}       callback.err    An error object (if any.)
 */
var disableTenants = module.exports.disableTenants = function(ctx, ports, disabled, callback) {
    callback = callback || function() {};

    if (!ctx.user() || !ctx.user().isAdmin(OAE.serverTenant.alias)) {
 ...
 * @param  {Function}  callback    Callback function executed when request is completed.
 */
var deleteTenants = module.exports.deleteTenants = function(ctx, ports, callback) {
    callback = callback || function() {};
    ...
    Cassandra.runBatchQuery(queries, 'QUORUM', function(err) {
        ...
        for (var i = 0; i < ports.length; i++) {
            Pubsub.publish('oae-tenants', 'stop ' + ports[i], callback);
        }
    });
};
...
var getTenantByPort = module.exports.getTenantByPort = function(port, callback) {
    Cassandra.runQuery('SELECT * FROM Tenant USING CONSISTENCY QUORUM WHERE port = ?', [ port ], function(err, rows) {
        if (err) {
            return callback(err);
        }
...
var getTenantByAlias = module.exports.getTenantByAlias = function(alias, callback) {
    getAllTenants(function(err, tenants) {
        ...
        if (tenant) {
            ...
            callback({'code': 404, 'msg': 'There is no tenant with the alias ' + alias});
        }
    });
};
 ...
 */
var createTenant = module.exports.createTenant = function(ctx, alias, name, port, baseUrl, callback) {
    callback = callback || function() {};
        ...
        if (!tenant) {
            getTenantByAlias(alias, function(err, tenant) {
                if (!tenant) {
                    ...
                    tenant = new model.Tenant(alias, name, port, baseUrl, true, false);
                    // no tenant by this alias or port exist, create one
                    Cassandra.runQuery('UPDATE Tenant USING CONSISTENCY QUORUM SET alias = ?, name = ?, baseUrl = ?, active = ? WHERE port = ?',
                        [tenant.alias, tenant.name, baseUrl, tenant.active, tenant.port], function(err) {
                        if (err) {
                            return callback(err);
                        }
            ...
            callback({'code': 400, 'msg': 'A tenant is already running on port ' + port}, tenant);
        }
    });
};
 ...
 * @param  {Context}    ctx            The current context.
 * @param   {Number}    port           The port on which the tenant listens
 ...
 * @param   {Object}    callback.err   An error object (if any)
 */
var updateTenant = module.exports.updateTenant = function(ctx, port, name, callback) {
    ...
    Cassandra.runQuery('UPDATE Tenant USING CONSISTENCY QUORUM SET name = ? WHERE port = ?', [name, port], callback);
};

/**
 * Register all of the REST end points from all modules against a tenant
 *
 * @param  {Tenant}      tenant      The tenant object representing the tenant for which the REST endpoints need to be registered
 * @param  {Function}    callback    Standard callback function execute when all of the REST endpoints have finished registering
 */
 ...
 *
 * @param  {Row[]}   rows   The rows to check for a tenant
 * @return {Boolean}        True if there is at least one tenant, False otherwise
 */
...
var mapToTenant = function(row, returnDeleted) {
    ...
    return new model.Tenant(hash.alias, hash.name, hash.port, hash.baseUrl, hash.active, hash.deleted);
};
