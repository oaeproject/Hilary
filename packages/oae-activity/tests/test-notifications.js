...
describe('Notifications', () => {
      ...
      TestsUtil.generateTestUsers(camAdminRestContext, 2, (err, createdUsers) => {
        ...
        RestAPI.Content.createLink(
          ...
          (err, content) => {
            ...
            ActivityTestsUtil.collectAndGetNotificationStream(simong.restContext, null, (err, notificationStream) => {
              assert.ok(!err);
                                      ...
                                      (err, link3) => {
                                        ...
                                        ActivityTestsUtil.collectAndGetNotificationStream(
                                          ...
                                          (err, notificationStream) => {
                                            ...
                                            assert.ok(_.isArray(notificationStream.items[0].object['oae:collection']));
          ...
          RestAPI.Content.createLink(
            ...
            (err, firstContentObj) => {
              ...
              ActivityTestsUtil.collectAndGetNotificationStream(simong.restContext, null, (err, notificationStream) => {
                ...
                assert.strictEqual(notificationStream.items.length, 1);
                assert.strictEqual(notificationStream.items[0].actor['oae:id'], mrvisser.user.id);
                assert.strictEqual(notificationStream.items[0].object['oae:id'], firstContentObj.id);
  ...
  describe('Mail deduplication', () => {
    ...
    it('verify mails for the same activity and the same user are only sent once', callback => {
      TestsUtil.generateTestUsers(camAdminRestContext, 3, (err, createdUsers, simong, mrvisser, nico) => {
        assert.ok(!err);
