...
var _getFlickrPhoto = function(ctx, id, callback) {
    ...
    var apiUrl = util.format('https://api.flickr.com/services/rest/?method=flickr.photos.getInfo&api_key=%s&photo_id=%s&format=json&nojsoncallback=1', config.apiKey, id);
    request(apiUrl, function(err, response, body) {
        if (err) {
            log().error({'err': err, 'body': body}, 'An unexpected error occurred getting a Flickr photo');
            return callback(err);
        } else if (response.statusCode !== 200) {
            err = {'code': response.statusCode, 'msg': body};
            log().error({'err': err}, 'An unexpected error occurred getting a Flickr photo');
            return callback(err);
        }
        ...
        try {
            ...
            log().error({'err': ex, 'contentId': ctx.contentId}, 'Could not parse flickr response');
...
var _getFlickrSet = function(ctx, id, callback) {
    var config = _getConfig();
    var apiUrl = util.format('https://api.flickr.com/services/rest/?method=flickr.photosets.getInfo&api_key=%s&photoset_id=%s&format=json&nojsoncallback=1', config.apiKey, id);
    request(apiUrl, function(err, response, body) {
        if (err) {
            log().error({'err': err, 'body': body}, 'An unexpected error occurred getting a Flickr photo set');
            return callback(err);
        } else if (response.statusCode !== 200) {
            err = {'code': response.statusCode, 'msg': body};
            log().error({'err': err}, 'An unexpected error occurred getting a Flickr photo set');
            return callback(err);
        }
