...
module.exports.getPrincipal = function(context, principal_id, callback) {
    cassandra.runQuery('SELECT * FROM Principals USING CONSISTENCY QUORUM WHERE principal_id = ?', [principal_id], function (err, rows) {
        if (err) {
            return callback(err);
        }
...
var getPrincipalFromRow = function(context, row, callback) {
    if (module.exports.isGroup(row.get('principal_id').value)) {
        ...
        if (context.user() && context.user().isAdmin(groupResource.tenantId)) {
            ...
            AuthzAPI.isMemberOf(context.user().id, group.id, function(err, isMemberOf) {
                if (err) {
                    return callback(err);
                } else if (!isMemberOf) {
...
module.exports.getPrincipals = function(context, principal_ids, callback) {
    ...
    cassandra.runQuery("SELECT * FROM Principals USING CONSISTENCY QUORUM WHERE principal_id IN (?)", [principal_ids], function(err, rows) {
        if (err) {
            return callback(err);
        }
...
module.exports.isGroup = function(group_id) {
    var principal = AuthzUtil.getPrincipalFromUuid(group_id);
...
module.exports.isUser = function(user_id) {
    var principal = AuthzUtil.getPrincipalFromUuid(user_id);
...
var rowToGroup = function (row) {
    ...
    return new Group(hash.tenant, hash.principal_id, hash.name, hash.description, hash.visibility, hash.joinable);
};
