...
var _updateRoles = function(resourceId, changes, callback) {
    ...
    if (AuthzUtil.isGroupId(resourceId)) {
        _.each(changes, function(change, principalId) {
            if (AuthzUtil.isUserId(principalId)) {
                ...
                usersToInvalidate[principalId] = true;
    ...
    getAuthzMembersGraph(_.keys(groupsToInvalidate), function(err, graph) {
        ...
        // Extract just the user ids from the nodes. We will be invalidating them all from the authz
        // caches
        _.chain(graph.getNodes())
            .pluck('id')
            .filter(AuthzUtil.isUserId)
            .each(function(userId) {
                usersToInvalidate[userId] = true;
            });

        // For each user to invalidate, add their cache invalidation queries to the final queries
        // array
        usersToInvalidate = _.keys(usersToInvalidate);
        _.each(usersToInvalidate, function(userId) {
                queries.push(
                    {
 ...
 *
 *  * The nodes in the graph represent principals, except for potentially the specified resources
 *  * The resulting graph will always contain the nodes representing the given `resourceIds`, even if they have no members
 *  * Each node has an `id` property which indicates the id (e.g., "c:cam:Foo.docx", "u:oae:branden")
 *  * The inbound edges of each node denote a "has member" relationship
 *      * i.e., (Branden) --> (Foo.docx) indicates that the "Foo.docx" document "has member" "Branden"
 *  * The outbound edges then obviously denote a "is member of" relationship
 *      * i.e., (Branden) --> (Foo.docx) indicates that "Branden" "is a member of" the "Foo.docx" document
 *  * Each edge has a `role` property which indicates the role the source principal has in the target resource/principal
 *
 * @param  {String[]}       resourceId      The id of the resources whose full members graph to get
 * @param  {Function}       callback        Standard callback function
 * @param  {Object}         callback.err    An error that occurred, if any
    ...
    Cassandra.runAllPagesQuery('AuthzMembershipsCache', 'principalId', principalId, 'groupId', null, function(err, rows) {
        ...
        _.each(rows, function(row) {
            ...
            _.each(memberRoles, function(memberRole) {
                ...
                group.addEdge(memberRole.memberId, groupId, {'role': memberRole.role});
            });
        });
