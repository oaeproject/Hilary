...
describe('Preview processor', function() {
    ...
    after(function(callback) {
        ...
        ConfigTestUtil.updateConfigAndWait(globalAdminRestContext, 'admin', update, function(err) {
            assert.ok(!err);
            ZencoderTestUtil.clearZencoderConfig(globalAdminRestContext, global.oaeTests.tenants.localhost.alias, callback);
        });
    });
    ...
    /**
     * @return {Stream} Returns a stream that points to a video file
     */
    ...
    /**
     * @return {Stream} Returns a stream that points to an audio file
     */
    var getAudioStream = function() {
        return fs.createReadStream(__dirname + '/data/audio.mp3');
    };
    ...
    describe('Preview generation', function() {
        ...
        /**
         * Mock the Zencoder REST API
         *
         * @api private
         */
        var _mockZencoderPost = function(extension) {
            var nock = require('nock');
            nock.enableNetConnect();

            // Expect POST requests to:
            ...
            nock('https://app.zencoder.com/api/v2')
                .post('/jobs', function(body) {
                    return _.matcher(body, {
                        'private': true,
                        'height': 600
                        }) && _.matcher(body.output, {
                            'notifications': util.format('https://%s/api/zencoder/encodingNotification', global.oaeTests.tenants.localhost.host),
                            'format': extension
                        });
                    })
                .reply(200, {
                    ...
                    'outputs': [
                            {
                                'id': 101,
                                'label': null,
                                'url': util.format('http://bucket.s3.amazonaws.com/fake:content:id/fake-revision-id.%s', extension)
                            }
                        ]
                    });
        };

        var _mockZencodingNotification = function(restCtx, content, revision, extension) {
            var revisionIdWithDashes = revision.revisionId.replace(/:/g, '-');
            var outputUrl = util.format('http://bucket.s3.amazonaws.com/%s', content.id);
            var notification = {
                'job':
                {
                    'id': 100,
                    'state': 'finished',
                    'pass_through': util.format('%s/%s.%s', content.id, revisionIdWithDashes, extension)
                },
                'input':
                {
                    'id': 102,
                ...
                {
                    'id': 101,
                    'url': util.format('%s/%s.%s', outputUrl, revisionIdWithDashes, extension),
                    'label': null,
                    'state': 'finished',
                    'md5_checksum': null
                }
        ...
        /**
         * Test that verifies that video files are sent to Zencoder for processing.
         */
        it('verify video processing works', function(callback) {
            // Ignore this test if the PP is disabled
            if (!defaultConfig.previews.enabled) {
                return callback();
            }
            ...
            var extension = 'mp4';
            _mockZencoderPost(extension);
            ZencoderTestUtil.enableZencoder(globalAdminRestContext, 'globalKey', function() {
                _createContentAndWait('file', null, getVideoStream, function(restCtx, content) {
                    assert.equal(content.previews.status, 'pending');
                    RestAPI.Content.getRevision(restCtx, content.id, content.latestRevisionId, function(err, revision) {
                        var notification = _mockZencodingNotification(restCtx, content, revision, extension);
                        RestAPI.Zencoder.encodingComplete(restCtx, notification, function(err) {
                            assert.ok(!err);
                            ContentAPI.on(ContentConstants.events.UPDATED_CONTENT_PREVIEW, function(contentObj) {
                                console.log(contentObj.previews)
                                assert.equal(contentObj.previews.status, 'done');
                                assert.equal(contentObj.previews.videoKey, util.format('%s/%s.%s', content.id, revision.revisionId.replace(/:/g, '-'), extension));
                                return callback();
                            });
                        });
                    });
        ...
        it('verify audio processing works', function(callback) {
            // Ignore this test if the PP is disabled
            if (!defaultConfig.previews.enabled) {
                return callback();
            }
            ...
            _mockZencoderPost(extension);
            ZencoderTestUtil.enableZencoder(globalAdminRestContext, 'globalKey', function() {
                _createContentAndWait('file', null, getAudioStream, function(restCtx, content) {
                    assert.equal(content.previews.status, 'pending');
                    RestAPI.Content.getRevision(restCtx, content.id, content.latestRevisionId, function(err, revision) {
                        var notification = _mockZencodingNotification(restCtx, content, revision, extension);
                        RestAPI.Zencoder.encodingComplete(restCtx, notification, function(err) {
