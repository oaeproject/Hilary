...
var createTenantWithAdmin = module.exports.createTenantWithAdmin = function(tenantAlias, tenantHost, callback) {
    ...
    RestAPI.Tenants.createTenant(adminCtx, tenantAlias, tenantAlias, tenantHost, function(err, tenant) {
        if (err) {
            return callback(err);
        }

        // Disable recaptcha so we can create a user
        ConfigTestsUtil.updateConfigAndWait(adminCtx, tenantAlias, 'oae-principals/recaptcha/enabled', false, function(err) {
            if (err) {
                return callback(err);
            }
            ...
            RestAPI.User.createUser(anonymousCtx, 'administrator', 'administrator', 'Tenant Administrator', null, function(err, tenantAdmin) {
                ...
                RestAPI.User.setTenantAdmin(adminCtx, tenantAdmin.id, true, function(err) {
                    ...
                    ConfigTestsUtil.updateConfigAndWait(tenantAdminRestCtx, null, 'oae-principals/recaptcha/enabled', true, function(err) {
                        if (err) {
                            return callback(err);
                        }
 ...
 *
 * @param  {Number} [nrOfWords]     The amount of words you wish to generate. Default: 1
...
var _createPrivateTenant = function(tenantAlias, callback) {
    _createPublicTenant(tenantAlias, function(tenant, tenantAdminRestCtx) {
        ...
        ConfigTestsUtil.updateConfigAndWait(createGlobalAdminRestContext(), tenant.alias, 'oae-tenants/tenantprivacy/tenantprivate', true, function(err) {
            assert.ok(!err);
