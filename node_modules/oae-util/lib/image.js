    ...
    .write(outputPath, function(err) {
        if (err) {
            fs.unlink(outputPath, function(err) {
                if (err) {
                    log().error({'err': err, 'path': outputPath}, 'Could not unlink a file');
                }
            });
...
var _cropImage = function(path, selectedArea, callback) {
    ...
    gm(path).size(function (err, size) {
        ...
        var tempPath = temp.path({'suffix': getImageExtension(path, '.jpg')});
        gm(path)
        ...
        .write(tempPath, function(err) {
            if (err) {
                fs.unlink(tempPath, function() {
                    if (err) {
                        log().error({'err': err, 'path': tempPath}, 'Could not unlink a file');
                    }
                });
...
var _resizeImage = function(path, size, callback) {
    var suffix = size.width + 'x' + size.height + getImageExtension(path, '.jpg');
