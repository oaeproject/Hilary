...
const createRoom = module.exports.createRoom = function(content, callback) {
    ...
    .then(data => {
        ...
        .then(data => {
            _mapContentIdToRoomId(content, roomId, function(err) {
                if (err) {
                    return callback(err);
                }
...
const joinRoom = module.exports.joinRoom = function(ctx, contentObj, callback) {
    ...
    _mapContentIdToRoomId(contentObj, contentObj.ethercalcRoomId, function(err) {
        if (err) {
            return callback(err);
        }
...
/**
 * Map Ethercalc spreadsheet to OAE content item
 *
 * @param  {Content}    contentObj              The OAE content object that should be mapped to a room
 * @param  {String}     roomId                  The Ethercalc room ID
 * @param  {Function}   callback                Standard callback function
 * @param  {Object}     callback.err            An error that occurred, if any
 * @api private
 */
const _mapContentIdToRoomId = function(contentObj, roomId, callback) {
    ContentDAO.Ethercalc.setContentId(contentObj.id, contentObj.ethercalcRoomId, function(err) {
        if (err) {
            return callback(err);
        }
        return callback(null);
