...
const AuthzConstants = require('oae-authz/lib/constants').AuthzConstants;
...
const MessageBoxConstants = require('oae-messagebox/lib/constants').MessageBoxConstants;
 ...
 */
const ContentAPI = module.exports = new EmitterAPI.EventEmitter();
 ...
 */
const getContent = module.exports.getContent = function(ctx, contentId, callback) {
 ...
 */
const getFullContentProfile = module.exports.getFullContentProfile = function(ctx, contentId, callback) {
    // Parameter validation
    const validator = new Validator();
    validator.check(contentId, {'code': 400, 'msg': 'A content id must be provided'}).isResourceId();
    if (validator.hasErrors()) {
...
const _getFullContentProfile = function(ctx, contentObj, isManager, callback) {
    ...
    PrincipalsUtil.getPrincipal(ctx, contentObj.createdBy, function(err, createdBy) {
        ...
        AuthzPermissions.canShare(ctx, contentObj, null, AuthzConstants.role.VIEWER, function(err) {
            ...
            // For any other than collabdoc or -sheet, we simply return with the share information
 ...
 */
const createLink = module.exports.createLink = function(ctx, displayName, description, visibility, link, additionalMembers, folders, callback) {
 ...
 */
const createFile = module.exports.createFile = function(ctx, displayName, description, visibility, file, additionalMembers, folders, callback) {
    // Wrap the callback function into a function that cleans up the file in case something went wrong
    const cleanUpCallback = _getCleanUpCallback({'file': file}, callback);
...
const _createFile = function(ctx, displayName, description, visibility, file, additionalMembers, folders, callback) {
    ...
    ContentUtil.getStorageBackend(ctx).store(ctx.tenant().alias, file, options, function(err, uri) {
        ...
        const revisionData = _.extend({}, otherValues, {'uri': uri});
 ...
 */
const createCollabDoc = module.exports.createCollabDoc = function(ctx, displayName, description, visibility, additionalMembers, folders, callback) {
    callback = callback || function() {};
 ...
 */
const createCollabSheet = module.exports.createCollabSheet = function(ctx, displayName, description, visibility, additionalMembers, folders, callback) {
    callback = callback || function() {};
    ...
    Ethercalc.createRoom(contentId, function(err, roomId) {
        ...
        _createContent(ctx, contentId, revisionId, 'collabsheet', displayName, description, visibility, additionalMembers, folders, {'ethercalcRoomId': roomId}, {}, function(err, content, revision, memberChangeInfo) {
            if (err) {
                return callback(err);
            }
 ...
 * @param  {String}         resourceSubType     The content item type. One of `file`, `collabdoc`, `collabsheet` or `link`
 ...
 * @api private
 */
const _createContent = function(ctx, contentId, revisionId, resourceSubType, displayName, description, visibility, roles, folderIds, otherValues, revisionData, callback) {
    callback = callback || function() {};
    ...
    validator.check(resourceSubType, {'code': 400, 'msg': 'A valid resourceSubType must be provided. This can be "file", "collabdoc", "collabsheet" or "link"'}).isIn(ContentConstants.resourceSubTypes);
    validator.check(null, {'code': 401, 'msg': 'You have to be logged in to be able to create a content item'}).isLoggedInUser(ctx);

    // Ensure all roles applied are valid. Editor is only valid for collabdocs and -sheets
    ...
    canManageFolders(ctx, folderIds, function(err, folders) {
        ...
        // Create the resource
        const createFn = _.partial(ContentDAO.Content.createContent, contentId, revisionId, ctx.user().id, resourceSubType, displayName, description, visibility, otherValues, revisionData);
 ...
 * @param  {Object}     callback.err        An error that occurred, if any
 */
const handlePublish = module.exports.handlePublish = function(data, callback) {
 ...
 * @param  {Object}     callback.err        An error that occurred, if any
 */
const ethercalcPublish = module.exports.ethercalcPublish = function(data, callback) {
    callback = callback || function(err) {
        if (err) {
            log().error({'err': err, 'data': data}, 'Error handling ethercalc edit');
        }
    };

    ContentDAO.Ethercalc.getEdited(data.contentId, data.userId, function(err, hasEdited) {
        if (err) {
            callback(err);
        }
        if (!hasEdited) {
        ...
        log().trace({'data': data}, 'Got an ethercalc edit');
        ContentDAO.Content.getContent(data.contentId, function(err, contentObj) {
            if (err) {
                return callback(err);
            }
            ...
            PrincipalsDAO.getPrincipal(data.userId, function(err, principal) {
                ...
                Ethercalc.getHTML(roomId, function(err, html) {
                    ...
                    ContentDAO.Revisions.getRevision(contentObj.latestRevisionId, function(err, revision) {
                        ...
                        if (Ethercalc.isContentEqual(revision.ethercalcHtml, html) || (!revision.ethercalcHtml && Ethercalc.isContentEmpty(html))) {
                            // Spreadsheet has had edits but content has not changed - no further action necessary
                            ContentAPI.emit(ContentConstants.events.EDITED_COLLABSHEET, ctx, contentObj);
                            return callback();
                        }
                        ...
                        Ethercalc.getRoom(roomId, function(err, snapshot) {
                            ...
                            ContentDAO.Revisions.createRevision(newRevisionId, contentObj.id, data.userId, {'ethercalcHtml': html, 'ethercalcSnapshot': snapshot}, function(err, revision) {
                                if (err) {
                                    log().error({'err': err, 'contentId': contentObj.id}, 'Could not create a revision for this collaborative spreadsheet');
                                    return callback({'code': 500, 'msg': 'Could not create a revision for this collaborative spreadsheet'});
                                }
                                Ethercalc.getRoom(roomId, function(err, snapshot) {
                                    if (err) {
                                        log().error({'err': err, 'contentId': contentObj.id}, 'Failed to fetch Ethercalc snapshot for this collaborative spreadsheet');
                                        return callback({'code': 500, 'msg': 'Failed to fetch Ethercalc snapshot for this collaborative spreadsheet'});
                                    }
                                    // Update the content so we can easily retrieve it.
                                    // This will also bump the collabsheet to the top of the library lists
                                    ContentDAO.Content.updateContent(contentObj, {'latestRevisionId': revision.revisionId}, true, function(err, newContentObj) {
                                        if (err) {
                                            log().error({'err': err, 'contentId': contentObj.id}, 'Could not update the main content this collaborative spreadsheet');
                                            return callback({'code': 500, 'msg': 'Could not update this collaborative spreadsheet'});
                                        }
...
const ethercalcPublish = module.exports.ethercalcPublish = function(data, callback) {
    log().info(data)
    ContentDAO.Ethercalc.getContentId(data.roomId, function(err, contentId) {
        if (err) {
            return callback({code: 500, msg: `Failed to check edits for Ethercalc room ${data.roomId}`});
        }
        ContentDAO.Content.getContent(contentId, function(err, contentObj) {
            if (err) {
                return callback(err);
            }
            // Workaround until I finish mapping between Ethercalc and OAE users
            PrincipalsDAO.getPrincipal(contentObj.createdBy, function(err, principal) {
                const ctx = Context.fromUser(principal);
                Ethercalc.getHTML(contentObj.ethercalcRoomId, function(err, html) {
                    // Get the latest OAE revision and compare the html that in Ethercalc.
                    // We only need to create a new revision if there is an actual update
                    ContentDAO.Revisions.getRevision(contentObj.latestRevisionId, function(err, revision) {
                        if (err) {
                            return callback(err);
                        } else if (Ethercalc.isContentEqual(revision.ethercalcHtml, html) || (!revision.ethercalcHtml && Ethercalc.isContentEmpty(html))) {
                            ContentAPI.emit(ContentConstants.events.EDITED_COLLABSHEET, ctx, contentObj);
                            return callback();
                        }
                        ...
                        // Otherwise we create a new revision
                        const newRevisionId = _generateRevisionId(contentObj.id);
                        ContentDAO.Revisions.createRevision(newRevisionId, contentObj.id, null, {'ethercalcHtml': html}, function(err, revision) {
                            if (err) {
                                log().error({'err': err, 'contentId': contentObj.id}, 'Could not create a revision for this collaborative spreadsheet');
                                return callback({'code': 500, 'msg': 'Could not create a revision for this collaborative spreadsheet'});
                            }
                            Ethercalc.getRoom(data.roomId, function(err, snapshot) {
                                if (err) {
                                    log().error({'err': err, 'contentId': contentObj.id}, 'Failed to fetch Ethercalc snapshot for this collaborative spreadsheet');
                                    return callback({'code': 500, 'msg': 'Failed to fetch Ethercalc snapshot for this collaborative spreadsheet'});
                                }
                                // Update the content so we can easily retrieve it.
                                // This will also bump the collabsheet to the top of the library lists
                                ContentDAO.Content.updateContent(contentObj, {'latestRevisionId': revision.revisionId, 'ethercalcSnapshot': snapshot}, true, function(err, newContentObj) {
                                    if (err) {
                                        log().error({'err': err, 'contentId': contentObj.id}, 'Could not update the main content this collaborative spreadsheet');
                                        return callback({'code': 500, 'msg': 'Could not update this collaborative spreadsheet'});
                                    }
 ...
 * Only users who have manager permissions on the collaborative document or spreadsheet can join the pad/room.
 *
 * @param  {Context}    ctx             Standard context object containing the current user and the current tenant
 * @param  {String}     contentId       The ID of the collaborative  or spreadsheet that should be joined
 * @param  {Function}   callback        Standard callback function
 * @param  {Object}     callback.err    An error that occurred, if any
 * @param  {Object}     callback.url    JSON object containing the url where the pad/room is accessible
 */
const joinCollabDoc = module.exports.joinCollabDoc = function(ctx, contentId, callback) {
    // Parameter validation
    const validator = new Validator();
    validator.check(contentId, {'code': 400, 'msg': 'A content id must be provided'}).isResourceId();
    if (validator.hasErrors()) {
    ...
    _canEdit(ctx, contentId, function(err, contentObj) {
        if (err) {
            ...
            return callback({'code': 400, 'msg': 'This is not a collaborative document or spreadsheet'});
        }
    });
};
 ...
 * @param  {Object}    callback.err      An error that occurred, if any
 */
const deleteContent = module.exports.deleteContent = function(ctx, contentId, callback) {
    callback = callback || function() {};
        ...
        ContentDAO.Content.deleteContent(contentObj, function(err, members) {
            ...
            if (contentObj.resourceSubType === 'collabsheet') {
                Ethercalc.deleteRoom(contentObj.ethercalcRoomId, function(err) {
                    ...
                    log().info({'contentId': contentId}, 'Deleted an Ethercalc room');
 ...
 * @param  {Object}    callback.err      An error that occurred, if any
 */
const shareContent = module.exports.shareContent = function(ctx, contentId, principalIds, callback) {
    // Parameter validation
    const validator = new Validator();
    validator.check(contentId, {'code': 400, 'msg': 'A content id must be provided'}).isResourceId();
 ...
 * @param  {Object}          callback.err   An error that occurred, if any
 */
const setContentPermissions = module.exports.setContentPermissions = function(ctx, contentId, changes, callback) {
    // Parameter validation
    const validator = new Validator();
    validator.check(null, {'code': 401, 'msg': 'You have to be logged in to be able to create a content item'}).isLoggedInUser(ctx);
    validator.check(contentId, {'code': 400, 'msg': 'A content id must be provided'}).isResourceId();
 ...
 * @param  {Object}     callback.err    An error that occurred, if any
 */
const removeContentFromLibrary = module.exports.removeContentFromLibrary = function(ctx, libraryOwnerId, contentId, callback) {
    const validator = new Validator();
 ...
 */
const getContentMembersLibrary = module.exports.getContentMembersLibrary = function(ctx, contentId, start, limit, callback) {
 ...
 */
const getContentInvitations = module.exports.getContentInvitations = function(ctx, contentId, callback) {
    const validator = new Validator();
 ...
 * @param  {Object}         callback.err    An error that occurred, if any
 */
const resendContentInvitation = module.exports.resendContentInvitation = function(ctx, contentId, email, callback) {
    const validator = new Validator();
    validator.check(contentId, {'code': 400, 'msg': 'A valid resource id must be specified'}).isResourceId();
    if (validator.hasErrors()) {
        return callback(validator.getFirstError());
 ...
 */
const updateFileBody = module.exports.updateFileBody = function(ctx, contentId, file, callback) {
    callback = callback || function(err) {
        if (err) {
 ...
 * @param  {Object}      callback.err        An error that occurred, if any
 */
const setPreviewItems = module.exports.setPreviewItems = function(ctx, contentId, revisionId, status, files, sizes, contentMetadata, previewMetadata, callback) {
    files = files || {};
    sizes = sizes || {};
 ...
 */
const getSignedPreviewDownloadInfo = module.exports.getSignedPreviewDownloadInfo = function(ctx, contentId, revisionId, previewItem, signatureData, callback) {
    const validator = new Validator();
    ...
    ContentDAO.Previews.getContentPreview(revisionId, previewItem, function(err, preview) {
        ...
        const downloadStrategy = ContentUtil.getStorageBackend(ctx, preview.uri).getDownloadStrategy(ctx.tenant().alias, preview.uri);
 ...
 */
const getPreviewItems = module.exports.getPreviewItems = function(ctx, contentId, revisionId, callback) {
    getContent(ctx, contentId, function(err, content) {
        if (err) {
            return callback(err);
        }
 ...
 */
const updateContentMetadata = module.exports.updateContentMetadata = function(ctx, contentId, profileFields, callback) {
    callback = callback || function() {};
 ...
 */
const createComment = module.exports.createComment = function(ctx, contentId, body, replyToCreatedTimestamp, callback) {
    callback = callback || function() {};
 ...
 * @param  {String}     callback.nextToken   The value to provide in the `start` parameter to get the next set of results
 */
const getComments = module.exports.getComments = function(ctx, contentId, start, limit, callback) {
    limit = OaeUtil.getNumberParam(limit, 10, 1);
 ...
 */
const deleteComment = module.exports.deleteComment = function(ctx, contentId, commentCreatedDate, callback) {
    const validator = new Validator();
 ...
 * @param  {String}         callback.nextToken  The value to provide in the `start` parameter to get the next set of results
 */
const getContentLibraryItems = module.exports.getContentLibraryItems = function(ctx, principalId, start, limit, callback) {
    limit = OaeUtil.getNumberParam(limit, 10, 1);
 ...
 * @param  {String}         callback.nextToken  The value to provide in the `start` parameter to get the next set of results
 */
const getRevisions = module.exports.getRevisions = function(ctx, contentId, start, limit, callback) {
    limit = OaeUtil.getNumberParam(limit, 10, 1);
...
const _getRevisions = function(ctx, contentObj, start, limit, opts, callback) {
    ...
    ContentDAO.Revisions.getRevisions(contentObj.id, start, limit, opts, function(err, revisions, nextToken) {
        ...
        const userIds = _.map(revisions, function(revisions) { return revisions.createdBy; });
 ...
 */
const getRevision = module.exports.getRevision = function(ctx, contentId, revisionId, callback) {
    const validator = new Validator();
 ...
 */
const getRevisionDownloadInfo = module.exports.getRevisionDownloadInfo = function(ctx, contentId, revisionId, callback) {
    const validator = new Validator();
    validator.check(contentId, {'code': 400, 'msg': 'A valid contentId must be provided'}).isResourceId();
    if (revisionId) {
        validator.check(revisionId, {'code': 400, 'msg': 'A valid revisionId must be provided'}).isResourceId();
 ...
 */
const restoreRevision = module.exports.restoreRevision = function(ctx, contentId, revisionId, callback) {
    const validator = new Validator();
    validator.check(contentId, {'code': 400, 'msg': 'A valid contentId must be provided'}).isResourceId();
    validator.check(revisionId, {'code': 400, 'msg': 'A valid revisionId must be provided'}).isResourceId();
    if (validator.hasErrors()) {
    ...
    _canManage(ctx, contentId, function(err, contentObj) {
        ...
        ContentDAO.Revisions.getRevision(revisionId, function(err, revision) {
            ...
            ContentDAO.Revisions.createRevision(newRevisionId, contentId, ctx.user().id, revision, function(err, newRevision) {
                 ...
                 * to the Content CF.
                 */
                const blacklist = ['revisionId', 'contentId', 'createdBy', 'created', 'etherpadHtml', 'previewsId', 'downloadPath', 'ethercalcHtml', 'ethercalcSnapshot'];
                ...
                ContentDAO.Content.updateContent(contentObj, updates, true, function(err, newContentObj) {
                    ...
                    if (contentObj.resourceSubType === 'collabdoc') {
                        ...
                        Ethercalc.setSheetContents(contentObj.ethercalcRoomId, revision.ethercalcSnapshot, function(err) {
                            if (err) {
                                return callback(err);
                            }
 ...
 */
const verifySignedDownloadQueryString = module.exports.verifySignedDownloadQueryString = function(ctx, qs, callback) {
...
const _generateRevisionId = function(contentId) {
    const tenantAlias = AuthzUtil.getResourceFromId(contentId).tenantAlias;
