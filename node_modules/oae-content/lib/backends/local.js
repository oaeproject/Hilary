...
var store = module.exports.store = function(ctx, file, callback) {
    ...
    _ensureDirectoryExists(localDir, function(err) {
        ...
        fs.rename(file.path, newPath, function(err) {
            if (err) {
                if (err.code !== 'EXDEV') {
                    log().error({'err': err}, "Wasn't able to relocate the file %s to %s.", file.path, newPath);
                    return callback(err);
                } else {
                    // file.path and newpath are on different partitions
                    // we'll have to copy then delete
                    var ins = fs.createReadStream(file.path);
                    var outs = fs.createWriteStream(newPath);
                    ...
                    outs.once('close', function(){
                        fs.unlink(file.path, callback(null, 'local:' + uri));
                    });
                    ins.pipe(outs);
                }
            } else {
...
var _getRootDir = function(ctx) {
    return Config.getValue(ctx, 'storage', 'local-dir');
};
