 ...
 *  * `prePurge(name)`                                      - Invoked just before a queue is purged. All mesages which are not awaiting acknowledgement will be removed.
 *  * `postPurge(name, count)`                              - Invoked after the queue has been purged. `count` is the number of messages that are purged from the queue.
...
var declareExchange = module.exports.declareExchange = function(exchangeName, exchangeOptions, callback) {
    if (exchanges[exchangeName]) {
        log().error({'exchangeName': exchangeName}, 'Tried to declare an exchange twice');
...
var declareQueue = module.exports.declareQueue = function(queueName, queueOptions, callback) {
    if (queues[queueName]) {
        log().error({'queueName': queueName, 'queueOptions': queueOptions}, 'Tried to declare a queue twice');
        return callback({'code': 400, 'msg': 'Tried to declare a queue twice'});
    }

    connection.queue(queueName, queueOptions, function(queue) {
        log().info({'queueName': queueName}, 'Created/Retrieved a RabbitMQ task queue');
...
var bindQueueToExchange = module.exports.bindQueueToExchange = function(queueName, exchangeName, routingKey, callback) {
    if (!queues[queueName]) {
        log().error({'queueName': queueName, 'exchangeName': exchangeName, 'routingKey': routingKey}, 'Tried to bind a non existing queue to an exchange, have you declared it first?');
        return callback({'code': 400, 'msg': 'Tried to bind a non existing queue to an exchange, have you declared it first?'});
    }
    if (!exchanges[exchangeName]) {
        log().error({'queueName': queueName, 'exchangeName': exchangeName, 'routingKey': routingKey}, 'Tried to bind a queue to a non-existing exchange, have you declared it first?');
        return callback({'code': 400, 'msg': 'Tried to bind a queue to a non-existing exchange, have you declared it first?'});
    }
    if (!routingKey) {
        log().error({'queueName': queueName, 'exchangeName': exchangeName, 'routingKey': routingKey}, 'Tried to bind a queue to an existing exchange without specifying a routing key');
...
var unbindQueueFromExchange = module.exports.unbindQueueFromExchange = function(queueName, exchangeName, routingKey, callback) {
    ...
    if (!routingKey) {
        log().error({'queueName': queueName, 'exchangeName': exchangeName, 'routingKey': routingKey}, 'Tried to unbind a queue from an exchange without providing a routingKey');
...
var subscribeQueue = module.exports.subscribeQueue = function(queueName, subscribeOptions, listener, callback) {
    if (!queues[queueName]) {
        log().error({'queueName': queueName, 'subscribeOptions': subscribeOptions}, 'Tried to subscribe to an unknown queue');
    ...
    queues[queueName].queue.subscribe(subscribeOptions, function(data, headers, deliveryInfo, message) {
        ...
        MQ.emit('preHandle', queueName, data, headers, deliveryInfo);
        ...
        if (!ok) {
            log().error({'err': err, 'queueName': queueName}, 'Error binding worker for task queue.');
            return _destroyQueue(queueName, function(err) {
                // Don't overwrite the original error with any binding errors
                return callback({'code': 500, 'msg': 'Error binding a worker for task queue'});
            });
        }
 ...
 *
 * @param  {String}    name            The name of the message queue to unsubscribe from
 * @param  {Function}  callback        Invoked when the process completes
 * @param  {Object}    callback.err    An error that occurred, if any
 */
var unsubscribeQueue = module.exports.unsubscribeQueue = function(name, callback) {
...
var submit = module.exports.submit = function(exchangeName, routingKey, data, options, callback) {
    ...
    if (!exchanges[exchangeName]) {
        log().error({'exchangeName': exchangeName, 'routingKey': routingKey}, 'Tried to submit a message to an unknown exchange');
        return callback({'code': 400, 'msg': 'Tried to submit a message to an unknown exchange'});
    }
    if (!routingKey) {
        log().error({'exchangeName': exchangeName, 'routingKey': routingKey}, 'Tried to submit a message without specifying a routingKey');
        return callback({'code': 400, 'msg': 'Tried to submit a message without specifying a routingKey'});
    }

    MQ.emit('preSubmit', routingKey);
    exchanges[exchangeName].publish(routingKey, data, options);
    return callback();
};
 ...
 * Purge a queue.
 *
 * @param  {String}     name            The name of the queue to purge.
...
var purge = module.exports.purge = function(name, callback) {
    ...
    if (!queues[name]) {
        log().warn({'queueName': name}, 'Tried purging unknown queue.');
        return callback({'code': 400, 'msg': 'Missing queue name'});
    }

    log().info({'queueName': name}, 'Purging queue.');
    MQ.emit('prePurge', name);
    queues[name].queue.purge().addCallback(function(data) {
        if (!data) {
            log().error({'err': err, 'queueName': name}, 'Error purging task queue.');
            return callback({'code': 500, 'msg': 'Error purging task queue: ' + name});
        }

        MQ.emit('postPurge', name, data.messageCount);
