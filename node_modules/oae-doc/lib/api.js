...
var pathTo3akaiux = '../3akai-ux/shared/oae/api';
...
var getDoc = module.exports.getDoc = function(moduleId, type, callback) {
    ...
    if (type === 'backend') {  
        ...
        validator.check(moduleId, {'code': 400, 'msg': 'Module name can\'t contain \'.\''}).notContains('.');
    }
 ...
 * @api private
 */
var readUIDoc = function(moduleId, callback) {
    var doc = {};
    IO.readFile(pathTo3akaiux + '/oae.api.' + moduleId + '.js', function(err, data) {
        if (!err) {
            try {
...
var loadAvailableFrontendModules = function (callback) {
    var _modules = [];
    IO.getFileListForFolder(pathTo3akaiux, function(err, modules) {
        if (err) {
            log().error(err);
        } else {
            var toDo = modules.length;
...
/**
 * Returns the available frontend module names from cache INVULLEN
 *
