...
module.exports = function(app, callback) {
    ensureSchema(function(err) {
        ...
        PrincipalsAPI.createUser(globalContext, globalUser.username, 'administrator', globalUser.visibility, globalUser.locale, globalUser.timezone, globalUser.firstName, globalUser.lastName, globalUser.displayName, function(err, userId) {
            ...
            if (!err) {
                ...
                PrincipalsAPI.setGlobalAdmin(globalContext, userId, true, function(err) {
                    ...
                    log().info("Admin account with user_id '%s' created.", userId);
                    callback();
                });
...
var ensureSchema = function(callback) {
    Cassandra.createColumnFamilies({
        'Principals': "CREATE COLUMNFAMILY Principals (principal_id text PRIMARY KEY, alias text, tenant text, visibility text, alias text, user_institution_userId text, user_first_name text, user_last_name text, user_email text, user_picture text, name text, description text, joinable text) WITH comment='Principal information';"
    }, callback);
};