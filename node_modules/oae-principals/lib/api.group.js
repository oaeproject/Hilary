...
var setGroupMembers = module.exports.setGroupMembers = function(ctx, groupId, changes, callback) {
    ...
    PrincipalsDAO.getPrincipal(groupId, function(err, group) {
        ...
        ResourceActions.setRoles(ctx, group, changes, function(err, changes, roles, members) {
            if (err) {
                return callback(err);
            }
            ...
            PrincipalsDAO.updatePrincipal(group.id, {'lastModified': newLastModified}, function(err) {
                ...
                _touchMembershipLibraries(group, oldLastModified, addedMemberIds.slice(), removedMemberIds.slice(), function(err) {
                    ...
                    PrincipalsEmitter.emit(PrincipalsConstants.events.UPDATED_GROUP_MEMBERS, ctx, group, changes, addedMemberIds, updatedMemberIds, removedMemberIds, function(errs) {
                        if (errs) {
...
var createGroup = module.exports.createGroup = function(ctx, displayName, description, visibility, joinable, roles, callback) {
    ...
    ResourceActions.create(ctx, roles, createFn, function(err, group, changes) {
        if (err) {
            return callback(err);
        }

        // Update the user's libraries
        _touchMembershipLibraries(group, null, _.keys(changes), [], function(err) {
            if (err) {
                log().error({
                    'err': err,
                    'group': group,
            ...
            PrincipalsEmitter.emit(PrincipalsConstants.events.CREATED_GROUP, ctx, group, changes);
            return callback(null, group);
        });
    });
};
