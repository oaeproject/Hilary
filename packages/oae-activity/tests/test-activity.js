...
describe('Activity', () => {
          ...
          (err, loggedinDiscussion) => {
            ...
            RestAPI.Discussions.createDiscussion(
              ...
              (err, privateDiscussion) => {
                ...
                RestAPI.Discussions.shareDiscussion(actorRestContext, publicDiscussion.id, [target], err => {
                  assert.ok(!err);
                  RestAPI.Discussions.shareDiscussion(actorRestContext, loggedinDiscussion.id, [target], err => {
                    assert.ok(!err);
                    RestAPI.Discussions.shareDiscussion(actorRestContext, privateDiscussion.id, [target], err => {
                      assert.ok(!err);
  ...
  const _setupUsersAndDiscussions = function(callback) {
    TestsUtil.setupMultiTenantPrivacyEntities((publicTenant0, publicTenant1) => {
      ...
      TestsUtil.generateTestUsers(publicTenant0.adminRestContext, 1, (err, users, extraPublicUser) => {
        assert.ok(!err);
          ...
          TestsUtil.generateTestUsers(camAdminRestContext, 1, (err, users, jack) => {
            ...
            RestAPI.Content.createLink(
              ...
              err => {
                ...
                ActivityTestUtil.collectAndGetActivityStream(jack.restContext, null, null, (err, activityStream) => {
                  assert.ok(!err);
          ...
          RestAPI.Content.createLink(
            ...
            (err, link) => {
              ...
              ActivityAPI.postActivity(anonymousCamApiContext, _createActivitySeed(), err => {
                ...
                ActivityAPI.postActivity(anonymousCamApiContext, _createActivitySeed({ activityType: '' }), err => {
                  assert.ok(err);
                  assert.strictEqual(err.code, 400);

                  // Verify no verb
                  ActivityAPI.postActivity(anonymousCamApiContext, _createActivitySeed({ verb: '' }), err => {
                    assert.ok(err);
                    assert.strictEqual(err.code, 400);

                    // Verify no publish date
                    ActivityAPI.postActivity(anonymousCamApiContext, _createActivitySeed({ published: '' }), err => {
                      assert.ok(err);
                      assert.strictEqual(err.code, 400);
  ...
  describe('Activity Routing', () => {
            ...
            FollowingTestsUtil.followByAll(publicTenant0.publicUser.user.id, followers, err => {
              ...
              ConfigTestsUtil.updateConfigAndWait(
                ...
                err => {
                  ...
                  const testId = util.format('a:%s:%s', publicTenant0.tenant.alias, TestsUtil.generateTestUserId());
    ...
    it('verify activity association exclusion', callback => {
      TestsUtil.generateTestUsers(camAdminRestContext, 3, (err, users, nico, branden, simon) => {
        ...
        ActivityTestUtil.collectAndGetActivityStream(branden.restContext, null, null, (err, activityStream) => {
          assert.ok(!err);
          assert.strictEqual(activityStream.items.length, 0);
          ...
          ActivityTestUtil.collectAndGetActivityStream(nico.restContext, null, null, (err, activityStream) => {
            ...
            ActivityTestUtil.collectAndGetActivityStream(simon.restContext, null, null, (err, activityStream) => {
              assert.ok(!err);
              assert.strictEqual(activityStream.items.length, 1);
              assert.strictEqual(activityStream.items[0].object['oae:id'], testId);
          ...
          ActivityTestUtil.assertGetActivityStreamFails(jack.restContext, 'c:cam:someContent', null, 400, () => {
            ...
            ActivityTestUtil.assertGetActivityStreamFails(
              ...
              () => {
                ...
                ActivityTestUtil.collectAndGetActivityStream(jack.restContext, jack.user.id, null, err => {
                  assert.ok(!err);
                  return callback();
                });
              }
    ...
    it('verify activities with collections have tenant information', callback => {
      TestsUtil.generateTestUsers(camAdminRestContext, 3, (err, users, jack, jane, jill) => {
        ...
        RestAPI.Content.createLink(
          ...
          (err, link) => {
            assert.ok(!err);
            RestAPI.Content.shareContent(jack.restContext, link.id, [jane.user.id, jill.user.id], err => {
              assert.ok(!err);
  ...
  describe('Activity Stream Aggregation', () => {
      ...
      ActivityAggregator.collectAllBuckets(err => {
        ...
        TestsUtil.generateTestUsers(camAdminRestContext, 1, (err, users, jack) => {
          ...
          ActivityDAO.getAggregateStatus = function(allAggregateKeys, callback) {
            const brokenAggregateKeyPrefix = util.format('content-create#%s#activity', jack.user.id);
          ...
          RestAPI.Content.createLink(
            ...
            (err, linkA) => {
              ...
              ActivityTestUtil.collectAndGetActivityStream(jack.restContext, null, null, (err, activityStream) => {
                ...
                RestAPI.Content.createComment(jack.restContext, linkA.id, 'Comment Comment', null, err => {
                  assert.ok(!err);
                  ActivityTestUtil.collectAndGetActivityStream(jack.restContext, null, null, (err, activityStream) => {
                    assert.ok(!err);
                    assert.strictEqual(activityStream.items.length, 1);
    ...
    it('verify aggregation max expiry time', callback => {
      ...
      ActivityTestUtil.refreshConfiguration({ aggregateIdleExpiry: 5, aggregateMaxExpiry: 1 }, err => {
        assert.ok(!err);
    ...
    it('verify aggregated data is automatically deleted after the idle expiry time', callback => {
      ...
      ActivityTestUtil.refreshConfiguration({ aggregateIdleExpiry: 1, aggregateMaxExpiry: 5 }, err => {
        assert.ok(!err);
              ...
              ActivityTestUtil.collectAndGetActivityStream(jack.restContext, null, null, err => {
                ...
                ActivityDAO.getAggregateStatus([aggregateKey], (err, aggregateStatus) => {
                  ...
                  ActivityDAO.getAggregatedEntities([aggregateKey], (err, aggregatedEntities) => {
                    ...
                    setTimeout(ActivityDAO.getAggregateStatus, 1100, [aggregateKey], (err, aggregateStatus) => {
                      assert.ok(!err);
    ...
    it('verify aggregation can be reset', callback => {
      TestsUtil.generateTestUsers(camAdminRestContext, 2, (err, users, simong, mrvisser) => {
        ...
        RestAPI.Content.createLink(
          ...
          (err, firstContentObj) => {
            assert.ok(!err);
            ActivityTestUtil.collectAndGetActivityStream(mrvisser.restContext, null, null, (err, activityStream) => {
              assert.ok(!err);
              ...
              ActivityAggregator.resetAggregationForActivityStreams([mrvisser.user.id + '#activity'], err => {
                assert.ok(!err);
                RestAPI.Content.createLink(
                  simong.restContext,
                  'Second item',
                                ...
                                ActivityTestUtil.collectAndGetNotificationStream(
                                  ...
                                  (err, notificationStream) => {
                                    ...
                                    ActivityTestUtil.assertActivity(
                                      ...
                                      'create',
                                      simong.user.id,
                                      [firstContentObj.id, secondContentObj.id, thirdContentObj.id],
                                      mrvisser.user.id
                                    );
                ...
                (err, activeAggregateKeysForActivityStream) => {
                  ...
                  ActivityDAO.getActiveAggregateKeysForActivityStreams(
                    ...
                    (err, activeAggregateKeysForNotificationStream) => {
                      ...
                      const allNotificationKeys = _.flatten(activeAggregateKeysForNotificationStream[0][1]);
                      assert.ok(_.isEmpty(_.intersection(allActivityKeys, allNotificationKeys)));

                      // Reset simon's "activity" activity stream
                      ActivityAggregator.resetAggregationForActivityStreams([simong.user.id + '#activity'], err => {
                        assert.ok(!err);
            ...
            RestAPI.Content.createComment(
              ...
              (err, comment) => {
                ...
                ActivityTestUtil.collectAndGetActivityStream(simong.restContext, null, null, (err, activityStream) => {
                  ...
                  ActivityDAO.getActivities(contentObj.id + '#message', null, 20, (err, activities) => {
                    assert.ok(!err);
