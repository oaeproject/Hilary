...
AuthzGraph.prototype.toString = function() {
    var nodeStrs = _.pluck(this.getNodes(), 'id');
    var edgeStrs = _.map(this.getEdges(), function(edge) {
 ...
 * and doing a breadth-first traversal
 *
...
AuthzGraph.prototype._traverse = function(nodeId, getEdgeFn, nextNodeProperty, _nodes, _visitedIds) {
    ...
    // Traverse inbound edges, recursively visiting the `from` nodes that we haven't visited yet
    _.chain(getEdgeFn.call(self, nodeId))
        .pluck(nextNodeProperty)
        .pluck('id')
        .filter(function(fromId) {
            return (!_visitedIds[fromId]);
        })
        .each(function(fromId) {
            self._traverse(fromId, getEdgeFn, nextNodeProperty, _nodes, _visitedIds);
        });
