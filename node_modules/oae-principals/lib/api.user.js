        ...
        if (personalData.upload) {
            ...
            async.mapSeries(personalData.upload, function(uploadedFile, callback) {
                fs.readFile(uploadedFile.path, function (err, data) {
                    ...
                    _getNewFileName(fileExt, fileName, uploadFolder, function(err, newName) {
    ...
    const compressCollabDocs = (callback) => {
        if (personalData.collabdoc) {
            ...
            async.mapSeries(personalData.collabdoc, function(collabdoc, callback) {
                ...
                _getNewFileName(fileExt, fileName, collabdocFolder, function(err, newName) {
    ...
    const compressLinks = (callback) => {
        if (personalData.link) {
            ...
            async.mapSeries(personalData.link, function(link, callback) {
                ...
                _getNewFileName(fileExt, fileName, linkFolder, function(err, newName) {
    ...
    const compressMeetings = (callback) => {
        if (personalData.meeting) {
            ...
            async.mapSeries(personalData.meeting, function(meeting, callback) {
                ...
                _getNewFileName(fileExt, fileName, meetingFolder, function(err, newName) {
    ...
    const compressDiscussions = (callback) => {
        if (personalData.discussion) {
            ...
            async.mapSeries(personalData.discussion, function(discussion, callback) {
                ...
                _getNewFileName(fileExt, fileName, discussionFolder, function(err, newName) {
 ...
 * @param  {Function}   callback            Standard callback function
 * @param  {Object}     callback.err        An error that occurred, if any
 * @param  {String}     callback.fileName   The new file name
 */
const _getNewFileName = function(fileExt, fileName, folder, callback, index) {
    index = index || 0;
    var file = '';
    ...
    try {
        folder.file(file).async('uint8array').then(function (data) {
            return _getNewFileName(fileExt, fileName, folder, callback, index + 1);
        });
    } catch (err) {
        if (index !== 0) {
            return callback(null, fileName + '(' + index + ').' + fileExt);
        } else {
            return callback(null, fileName + '.' + fileExt);
        }
    }
};
