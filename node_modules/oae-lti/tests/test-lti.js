        ...
        it('verify retrieved LTI tool launch data', function(callback) {
            RestAPI.Group.createGroup(johnRestContext, 'This is a group', null, 'public', 'yes', [], [], function(err, group) {
                ...
                RestAPI.LtiTool.createLtiTool(johnRestContext, group.id, launchUrl, secret, key, title, description, function(err, tool) {
                    ...
                    RestAPI.LtiTool.getLtiTool(johnRestContext, group.id, tool.id, function(err, data) {
                        assert.ok(!err);
        ...
        it('verify non-managers are given role \'Learner\'', function(callback) {
            RestAPI.Group.createGroup(johnRestContext, 'This is a group', null, 'public', 'yes', [], [], function(err, group) {
                RestAPI.LtiTool.createLtiTool(johnRestContext, group.id, 'http://lti.launch.url', 'secret', '12345', 'LTI tool title', 'LTI tool description', function(err, tool) {
                    assert.ok(!err);
                    // Get the LTI tool and verify its model
                    RestAPI.LtiTool.getLtiTool(anonymousRestContext, group.id, tool.id, function(err, data) {
                        assert.ok(!err);
                        var ltiLaunchData = data.launchParams;
    ...
    describe('Delete LTI tool', function() {
        ...
        it('verify that LTI tools can be deleted', function(callback) {
            RestAPI.Group.createGroup(johnRestContext, 'This is a group', null, 'public', 'yes', [], [], function(err, group) {
                ...
                RestAPI.LtiTool.createLtiTool(johnRestContext, group.id, launchUrl, secret, key, 'LTI tool title', 'LTI tool description', function(err, tool) {
                    ...
                    RestAPI.LtiTool.getLtiTool(johnRestContext, group.id, id, function(err, data) {
                        assert.ok(!err);
                        var ltiLaunchData = data.launchParams;
                        assert.equal(ltiLaunchData['oauth_consumer_key'], key);
                        assert.equal(ltiLaunchData['resource_link_id'], id);
                        RestAPI.LtiTool.deleteLtiTool(johnRestContext, group.id, id, function(err) {
                            assert.ok(!err);
                            // Assert the tool can no longer be fetched
                            RestAPI.LtiTool.getLtiTool(johnRestContext, group.id, id, function(err, data) {
                                assert.ok(err) ;
                                assert.equal(err.code, 404);
                                assert.equal(err.msg, util.format('Could not find LTI tool %s for group %s', id, group.id));
                                return callback();
                            });
                        });
                    });
        ...
        it('verify that a non-manager can not delete LTI tool', function(callback) {
            RestAPI.Group.createGroup(johnRestContext, 'This is a group', null, 'public', 'yes', [], [], function(err, group) {
                ...
                RestAPI.LtiTool.createLtiTool(johnRestContext, group.id, launchUrl, secret, key, 'LTI tool title', 'LTI tool description', function(err, tool) {
                    assert.ok(!err);
                    RestAPI.LtiTool.deleteLtiTool(anonymousRestContext, group.id, tool.id, function(err) {
                        assert.ok(err) ;
                        assert.equal(err.code, 401);
                        assert.equal(err.msg, 'The current user does not have access to manage this resource');
                        return callback();
        ...
        it('verify that a LTI tool can not be deleted in a deleted group', function(callback) {
            RestAPI.Group.createGroup(johnRestContext, 'This is a group', null, 'public', 'yes', [], [], function(err, group) {
                ...
                RestAPI.LtiTool.createLtiTool(johnRestContext, group.id, 'http://lti.launch.url', 'secret', '12345', 'LTI tool title', 'LTI tool description', function(err, tool) {
                    ...
                    RestAPI.Group.deleteGroup(johnRestContext, group.id, function(err) {
                        assert.ok(!err);
                        RestAPI.LtiTool.deleteLtiTool(johnRestContext, group.id, tool.id, function(err) {
                            assert.ok(err) ;
                            assert.equal(err.code, 404);
