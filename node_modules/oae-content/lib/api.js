...
var _getContent = module.exports._getContent = function(contentId, callback) {
    // Retrieve the content item from Cassandra
...
var createFile = module.exports.createFile = function(ctx, displayName, description, visibility, additionalMembers, file, callback) {
    ...
    var otherValues = {
        ...
        'size': file.size,
        'filename': file.name,
        'previews_status': ContentConstants.previews.PENDING
    };
    _createContent(ctx, 'file', displayName, description, visibility, additionalMembers, otherValues, function(err, contentObj) {
        ...
        contentObj.previews.status = ContentConstants.previews.PENDING;
...
var createSakaiDoc = module.exports.createSakaiDoc = function(ctx, displayName, description, visibility, additionalMembers, callback) {
    ...
    _createContent(ctx, 'sakaidoc', displayName, description, visibility, additionalMembers, { 'previews_status': ContentConstants.previews.PENDING }, function(err, content) {
        if (!err) {
            emitter.emit(ContentConstants.events.CREATED_CONTENT, ctx, content, additionalMembers);
        }
        callback(err, content);
        ...
        getStorageBackend(ctx).store(ctx, file, function(err, uri) {
            ...
            _createRevision(ctx, contentObj.contentId, uri, opts, function(err, revision) {
                ...
                Cassandra.runQuery(q.query, q.parameters, function(err) {
                    ...
                    emitter.emit(ContentConstants.events.UPDATED_CONTENT, contentObj);
                    callback();
                });
            });
        });
 ...
 */
var addPreviewItems = module.exports.addPreviewItems = function(ctx, contentId, status, files, sizes, metadata, callback) {
    files = files || {};
    sizes = sizes || {};
    metadata = metadata || {};
    ...
    getContent(ctx, contentId, function(err, contentObj) {
        ...
        if (status === ContentConstants.previews.ERROR || todo === 0) {
            return _setPreviewMetadata(ctx, contentObj, status, null, metadata, callback);
        }
        ...
        fileKeys.forEach(function(key) {
            ...
            getStorageBackend(ctx).store(ctx, files[key], function(err, uri) {
                ...
                if (todo === 0 && !called) {
                    ...
                    called = true;
                    log().trace({'data': values}, 'Storing %d content preview files.', fileKeys.length);

                    // Remove any old previews that we had and store the new ones.
                    var queries = [
                        {
                            'query': 'DELETE FROM ContentPreviews WHERE contentId = ?',
                            'parameters': [contentId]
                        },
                        Cassandra.constructUpsertCQL('ContentPreviews', 'contentId', contentId, values)
                    ];
                    Cassandra.runBatchQuery(queries, 'QUORUM', function(err) {
                        ...
                        _setPreviewMetadata(ctx, contentObj, status, thumbnailUri, metadata, callback);
                    });
                }
            });
        });
...
var _setPreviewMetadata = function(ctx, contentObj, status, thumbnailUri, metadata, callback) {
    // Store all the extra metadata we might want to pass along to the content object.
    var values = {};
    Object.keys(metadata).forEach(function(key) {
        values['previews_' + key] = metadata[key];
        contentObj.previews[key] = metadata[key];
    });

    // Set the thumbnail URI if we have one.
    if (thumbnailUri) {
        values['previews_thumbnailUri'] = thumbnailUri;
        contentObj.previews.thumbnailUri = thumbnailUri;
    }

    // Remember the status.
    values['previews_status'] = status;

    var q = Cassandra.constructUpsertCQL('Content', 'contentId', contentObj.contentId, values, 'QUORUM');
    if (!q) {
        log().error({'data': values}, 'Could not create a correct query to set preview metadata.');
    ...
    Cassandra.runQuery(q.query, q.parameters, function(err) {
        ...
        if (status === ContentConstants.previews.DONE) {
            emitter.emit(ContentConstants.events.UPDATED_CONTENT_PREVIEW, ctx, contentObj.contentId, contentObj);
        }
 ...
 * @param  {String}     signature       A signature that validates this call.
 * @param  {Number}     expires         When the signature expires (in millis since epoch.)
 * @param  {Function}   callback        Standard callback method.
 * @param  {Object}     callback.err    Standard error object (if any)
...
var _rowToContent = function(row) {
    ...
    // Each content object should always have a previews object.
    // All the columns that are prefixed with `previews_` should be passed into
    // this object.
    contentObj.previews = {};
    _.each(hash, function(value, key, list) {
        if (key.indexOf('previews_') === 0) {
            contentObj.previews[key.substr(9)] = value;
        }
    });

    if (contentObj.previews.thumbnailUri) {
        contentObj.previews.thumbnailUrl = getThumbnailUrl(contentObj.contentId);
    }
    return contentObj;
};
