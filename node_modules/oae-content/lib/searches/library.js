...
var _search = function(ctx, fullAccess, opts, callback) {
    ...
    var baseFilter = SearchUtil.filterAnd(
        ...
        SearchUtil.filterTerms('access', [opts.principalId])
    );

    var authzPrincipal = AuthzUtil.getPrincipalFromId(opts.principalId);
    if (fullAccess) {
        // being granted full access to this content, we are allowed to ignore visibility filters
        filter = baseFilter;
    } else if (ctx.user() && ctx.user().tenant === authzPrincipal.tenantAlias) {
