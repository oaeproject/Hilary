...
describe('UI', function() {
    ...
    describe('Widget config aggregation', function() {
        ...
        it('verify widget configs', function(callback) {
            ...
            RestAPI.UI.getWidgetConfigs(anonymousGlobalRestContext, function(err, data) {
                assert.ok(!err);
                ...
                RestAPI.UI.getWidgetConfigs(anonymousRestContext, function(err, data) {
                    assert.ok(!err);
                    assert.ok(data['topnavigation']);
                    assert.equal(data['topnavigation'].id, 'topnavigation');
                    assert.equal(data['topnavigation'].path, 'oae-core/topnavigation');
    ...
    describe('Batch static files', function() {
        ...
        it('verify batch static get', function(callback) {
            ...
            RestAPI.UI.staticBatch(anonymousGlobalRestContext, files, function(err, batch1) {
                assert.ok(!err);
                ...
                RestAPI.UI.staticBatch(anonymousRestContext, files, function(err, batch2) {
                    assert.ok(!err);
                    ...
                    // Get these files on the global admin server
                    RestAPI.UI.staticBatch(anonymousGlobalRestContext, files, function(err, batch3) {
                        assert.ok(!err);
                        ...
                        // Get these files on the tenant server
                        RestAPI.UI.staticBatch(anonymousRestContext, files, function(err, batch4) {
                            assert.ok(!err);
        ...
        it('verify batch single file', function(callback) {
            ...
            RestAPI.UI.staticBatch(anonymousGlobalRestContext, file, function(err, data) {
                assert.ok(!err);
                ...
                RestAPI.UI.staticBatch(anonymousRestContext, file, function(err, data) {
                    assert.ok(!err);
                    assert.ok(_.keys(data).length, 1);
                    assert.ok(data[file]);
                    assert.equal(typeof data[file], 'string');
        ...
        it('verify validation', function(callback) {
            ...
            RestAPI.UI.staticBatch(anonymousGlobalRestContext, null, function(err, data) {
                assert.ok(err);
                assert.equal(err.code, 400);
                assert.ok(!data);
                RestAPI.UI.staticBatch(anonymousGlobalRestContext, [], function(err, data) {
                    assert.ok(err);
                    assert.equal(err.code, 400);
                    assert.ok(!data);
                    
                    // Test on the tenant server
                    RestAPI.UI.staticBatch(anonymousRestContext, null, function(err, data) {
                        assert.ok(err);
                        assert.equal(err.code, 400);
                        assert.ok(!data);
                        RestAPI.UI.staticBatch(anonymousRestContext, [], function(err, data) {
                            assert.ok(err);
                            assert.equal(err.code, 400);
                            assert.ok(!data);
                            callback();
