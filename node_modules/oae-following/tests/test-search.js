...
describe('Following Search', function() {
    ...
    it('verify followers and following searches reflect follows and unfollows', function(callback) {
        ...
        FollowingTestUtil.createFollowerAndFollowed(camAdminRestContext, function(follower, followed) {
            ...
            FollowingTestUtil.searchFollowerAndFollowing(follower.user.id, follower.restContext, followed.user.id, followed.restContext, function(followerUserDoc, followedUserDoc) {
        ...
        TestsUtil.generateTestUsers(camAdminRestContext, 4, function(err, testUsers) {
            ...
            RestAPI.User.updateUser(privateUser.restContext, privateUser.user.id, {'visibility': 'private'}, function(err) {
                ...
                RestAPI.User.updateUser(loggedinUser.restContext, loggedinUser.user.id, {'visibility': 'loggedin'}, function(err) {
                    ...
                    FollowingTestUtil.assertNoSearchFeedAccess(camAnonymousRestContext, [privateUser.user.id, loggedinUser.user.id], function() {
                        FollowingTestUtil.assertHasFollowFeedAccess(camAnonymousRestContext, [publicUser.user.id], function() {
                           ...
                           FollowingTestUtil.assertNoSearchFeedAccess(gtAdminRestContext, [privateUser.user.id, loggedinUser.user.id], function() {
                                FollowingTestUtil.assertHasSearchFeedAccess(gtAdminRestContext, [publicUser.user.id], function() {

                                    // Verify bert can see only public and loggedin follow searches
                                    FollowingTestUtil.assertNoSearchFeedAccess(bert.restContext, [privateUser.user.id], function() {
                                        FollowingTestUtil.assertHasSearchFeedAccess(bert.restContext, [publicUser.user.id, loggedinUser.user.id], function() {

                                            // Verify private user can see follow searches
                                            FollowingTestUtil.assertHasSearchFeedAccess(privateUser.restContext, [publicUser.user.id, loggedinUser.user.id, privateUser.user.id], function() {

                                                // Verify cam admin can see follow searches
                                                FollowingTestUtil.assertHasSearchFeedAccess(camAdminRestContext, [publicUser.user.id, loggedinUser.user.id, privateUser.user.id], function() {

                                                    // Verify global admin can see follow searches
                                                    FollowingTestUtil.assertHasSearchFeedAccess(globalAdminOnTenantRestContext, [publicUser.user.id, loggedinUser.user.id, privateUser.user.id], callback);
                                                });
                                            });
                                        });
                                    });
