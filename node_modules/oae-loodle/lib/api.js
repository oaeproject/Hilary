...
var getGroupMembers = function (ctx, groupId, role, callback) {
    ...
    groupAPI.getMembersLibrary(ctx, groupId, 0, null, function (err, data) {
        ...
        data.forEach(function (member) {
            groupMembers[member.profile.id] = role;
        });
...
var getGroupsMembers = function (ctx, groups, callback) {
    ...
    async.forEachOf(groups, function (role, groupId, done) {
        getGroupMembers(ctx, groupId, role, function (err, groupMembers) {
            if (err) return done(err);

            for (var k in groupMembers) {
                // If the user was already in another group
                if (k in groupsMembers) {
                    // We keep for the user the "higher" role
                    if (groupsMembers[k] === 'manager' || groupMembers[k] === 'manager') {
                        groupsMembers[k] = 'manager';
                    }
                    else {
                        groupsMembers[k] = 'viewer';
                    }
                }
                else {
                    groupsMembers[k] = groupMembers[k];
                }
            }
    ...
    async.series([
        ...
        function createLoodle (end) {
            LoodleDAO.createLoodle(ctx, token, name, description, visibility, function (err, data) {
                ...
                delete additionalMembers[currentUser.id]
        ...
        function getGroupMembers (end) {
            async.forEachOf(additionalMembers, function (role, id, finish) {
                if(util.isGroupId(id)) {
                    groupAPI.getMembersLibrary(ctx, id, 0, null, function (err, groupMembers) {
                        if (err) return done (err);

                        groupMembers.forEach(function (groupMember) {
                            if (!(groupMember.profile.id in additionalMembers) && groupMember.profile.id != currentUser.id) {
                                additionalMembers[groupMember.profile.id] = 'manager';
                            }
                        });

                        delete additionalMembers[id];
        ...
        function (end) {
            addUsers(ctx, additionalMembers, token, loodleId, end);
        }
    ], callback);
...
var addUsers = function (ctx, additionalMembers, token, loodleId, callback) {
    ...
    async.forEachOf(additionalMembers, function (role, id, done) {
        addUser(ctx, id, token, loodleId, callback);
    }, callback);
