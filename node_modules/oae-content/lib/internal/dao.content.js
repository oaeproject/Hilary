...
var getMultipleContentItems = module.exports.getMultipleContentItems = function(contentIds, fields, callback) {
    ...
    if (fields) {
        ...
        query = util.format('SELECT "%s" FROM "Content"', fields.join('","'));
    } else {
        query = 'SELECT * FROM "Content"';
    }

    Cassandra.runInQuery(query, parameters, 'contentId', contentIds, function(err, rows) {
        if (err) {
            return callback(err);
        }
...
var _rowToContent = function(row) {
    // Try and parse and apply the previews object of the row
    _parsePreviews(row);

    var contentObj = new Content(row.tenantAlias, row.contentId, row.visibility, row.displayName, row.description, row.resourceSubType, row.createdBy, row.created, row.lastModified, row.latestRevisionId, row.previews);
    if (contentObj.resourceSubType === 'file') {
        contentObj.filename = row.filename;
        contentObj.size = (row.size) ? parseInt(row.size, 10) : 0;
        contentObj.mime = row.mime;
    } else if (contentObj.resourceSubType === 'link') {
        contentObj.link = row.link;
    } else if (contentObj.resourceSubType === 'collabdoc') {
        contentObj.etherpadGroupId = row.etherpadGroupId;
        contentObj.etherpadPadId = row.etherpadPadId;
    }
