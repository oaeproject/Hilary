...
SearchConstants.general = {
    ...
    // A search scope that narrows `_share` down to only users and groups who the current user in
    // context can share with, and those that can share with specified resource visibility
    // information
    'SCOPE_SHARE': '_share',

    // A search scope that includes resources for only the current tenant in addition to resources
    // the user has access to outside the tenant
    'SCOPE_TENANT': '_tenant',

    // An array that contains all known internal scope types
    'SCOPES_ALL': ['_all', '_network', '_share', '_tenant']
};
