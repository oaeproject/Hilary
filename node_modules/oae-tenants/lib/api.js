 ...
 */
ConfigAPI.eventEmitter.on('update', function(alias) {
...
var searchTenants = (module.exports.searchTenants = function(q, opts) {
  ...
  if (q) {
    results = _
      .chain(tenantSearchIndex.search(q))
      .sortBy('ref')
      .sortBy('score')
      .pluck('ref')
      .map(getTenant)
...
var _updateCachedTenant = function(tenantAlias, callback) {
  ...
  // Get the available tenants
  Cassandra.runQuery('SELECT * FROM "Tenant" WHERE "alias" = ?', [tenantAlias], function(
    err,
    rows
  ) {
    if (err) {
...
var _tenantToStorageHash = function(tenant) {
  var hash = _.pick(tenant, 'displayName', 'host', 'emailDomains', 'countryCode', 'active');
  if (hash.emailDomains) {
