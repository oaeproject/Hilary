 ...
 *
 * @param   {String}         activityType                     The type of activity this producer is meant to produce activity objects for. If '*', then this is meant to produce activity objects for all types of activities, so long as the `resourceType` is a match
 ...
 */
var registerActivityObjectProducer = module.exports.registerActivityObjectProducer = function(activityType, resourceType, producer) {
    activityProducers[activityType] = activityProducers[activityType] || {};
    if (activityProducers[activityType][resourceType]) {
        throw new Error('Attempted to register duplicate activity producer');
    }
    activityProducers[activityType][resourceType] = producer;
};
 ...
 * and converting it into an ActivityObject that can be used in an activity.
 *
 * @param   {String}         activityType               The type of activity this router is meant to route. If '*', then this is meant to route all types of activities
 ...
 */
var registerActivityRouter = module.exports.registerActivityRouter = function(activityType, resourceType, router) {
    activityRouters[activityType] = activityRouters[activityType] || {};
    if (activityRouters[activityType][resourceType]) {
        throw new Error('Attempted to register duplicate activity router');
    }
    activityRouters[activityType][resourceType] = router;
};
...
var getActivityStream = module.exports.getActivityStream = function(ctx, activityStreamId, start, limit, callback) {
    ...
    if (AuthzUtil.isGroupId(activityStreamId)) {
...
var _handleActivityTask = function(activitySeed, callback) {
    ...
    _produceAllEntities(activitySeed, function(err, actor, object, target) {
        ...
        _produceAllRoutes(activitySeed, actor, object, target, function(err, actorRoutes, objectRoutes, targetRoutes) {
            ...
            // 1. create all routes and apply the "OWN" propagation entities. this is how private entities get delivered feeds
            ...
            // 2. back-fill all entities that are set to propagate on "all". apply the minimum the data (i.e., the resource type and id)
            // where appropriate.
...
var _produceRoutes = function(activityType, entityType, resource, activityObject, callback) {
    ...
    var router = _resolveHelper(activityRouters, activityType, resource.resourceType);
    if (!router) {
        log().trace({
...
var _produceEntity = function(activityType, entityType, resource, callback) {
    ...
    var producer = _resolveHelper(activityProducers, activityType, resource.resourceType);
    if (!producer) {
        log().warn({
            'activityType': activityType,
            'entityType': entityType,
...
var _resolveHelper = function(helpersHash, activityType, resourceType) {
    var catchAllActivities = helpersHash[ActivityConstants.activityTypes.ALL];
    var exactActivities = helpersHash[activityType];

    if (exactActivities && exactActivities[resourceType]) {
        // try the most specific match possible first
        return exactActivities[resourceType];
    } else if (catchAllActivities && catchAllActivities[resourceType]) {
        // fall back to the catch-all for activities, with a match on resource type
        return catchAllActivities[resourceType];
    }

    // if we get here, there is no helper for this combination. probably a warning, but let the caller decide
    return null;
};
