...
var createComment = module.exports.createComment = function(ctx, contentId, body, replyToCreatedTimestamp, callback) {
    ...
    getContent(ctx, contentId, function(err, contentObj) {
        ...
        var messageBoxId = ContentUtil.getContentCommentMessageBoxId(contentId);
        MessageBoxAPI.createMessage(messageBoxId, ctx.user().id, body, {'replyToCreated': replyToCreatedTimestamp}, function(err, message) {
            if (err) {
                return callback(err);
            }
...
var getComments = module.exports.getComments = function(ctx, contentId, start, limit, callback) {
    ...
    getContent(ctx, contentId, function(err, contentObj) {
        ...
        var messageBoxId = ContentUtil.getContentCommentMessageBoxId(contentId);
        MessageBoxAPI.getMessagesFromMessageBox(messageBoxId, start, limit, null, function(err, comments) {
            if (err) {
                return callback(err);
            }
...
var deleteComment = module.exports.deleteComment = function(ctx, contentId, commentCreatedDate, callback) {
    ...
    getContent(ctx, contentId, function(err, contentObj) {
        ...
        var messageBoxId = ContentUtil.getContentCommentMessageBoxId(contentId);
        MessageBoxAPI.getMessages(messageBoxId, [commentCreatedDate], null, function(err, message) {
            if (err) {
                return callback(err);
            }

            message = message[commentCreatedDate];
            if (!message) {
                return callback({'code': 404, 'msg': 'The specified comment does not exist'});
            } else if (ctx.user().id === message.createdBy) {
                return _deleteComment(messageBoxId, commentCreatedDate, callback);
            }
...
var getLibraryItems = module.exports.getLibraryItems = function(ctx, principalId, start, limit, callback) {
    ...
    // FIXME: Content should not reference discussions directly for this, but this method better belongs somewhere
    // more generic like a `LibraryAPI.Authz` object. This work should also accompany consolidation of visibility
    // definitions into oae-authz, but is too substantial of a change to group together with discussions+messages
    // work that this change is a part of.
            ...
            _.each(revisions, function(revision) {
                if (users[revision.createdBy]) {
                    ...
                    if (revision.thumbnailUrl) {
                        var tenantAlias = AuthzUtil.getResourceFromId(revision.revisionId).tenantAlias;
                        var principalId = (ctx.user() ? ctx.user().id : '');
                        revision.thumbnailUrl = ContentUtil.getDownloadUrlForUri(principalId, tenantAlias, revision.thumbnailUrl);
                    }
                }
            });
...
var _augmentContent = function(ctx, contentObj) {
    ...
    if (contentObj.previews && contentObj.previews.thumbnailUri) {
        ...
        contentObj.previews.thumbnailUrl = ContentUtil.getDownloadUrlForUri(principalId, contentObj.tenant.alias, contentObj.previews.thumbnailUri);
    }
};
