...
var getTenantSkinVariables = module.exports.getTenantSkinVariables = function(tenantAlias) {
    ...
    _renderDynamicValue(resolvedVariables, 'institutional-logo-url', {'tenantAlias': tenantAlias});
 ...
 * @api private
 */
var _renderDynamicValue = function(obj, key, data) {
    var value = obj[key];
    if (value) {
        try {
            obj[key] = _.template(value)(data);
        } catch (err) {
            log().warn({
                'err': err,
                'key': key,
                'value': value
            }, 'An error occurred while rendering a templated dynamic value');
        }
    }
};
