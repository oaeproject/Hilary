...
OAE.globalAdminServer.get('/api/tenant/:alias', function(req, res) {
    ...
    TenantsAPI.getTenantByAlias(req.params.alias, function(err, tenant) {
        if (err) {
...
OAE.globalAdminServer.post('/api/tenant', function(req, res) {
    req.telemetryUrl = '/api/tenant';
    TenantsAPI.updateTenant(req.ctx, req.body.alias, req.body.displayName, function(err) {
        if (err) {
            return res.send(err.code, err.msg);
        }
        res.send(200);
...
OAE.globalAdminServer.post('/api/tenant/create', function(req, res) {
    ...
    TenantsAPI.createTenant(req.ctx, req.body.alias, req.body.displayName, req.body.host, function(err) {
        if (err) {
            return res.send(err.code, err.msg);
        }
        res.send(200, 'New tenant "' + req.body.displayName + '" has been fired up on host ' + req.body.host);
    });
});
...
OAE.tenantServer.post('/api/tenant', function(req, res) {
    req.telemetryUrl = '/api/tenant';
    TenantsAPI.updateTenant(req.ctx, req.ctx.tenant().alias, req.body.displayName, function(err) {
        if (err) {
            return res.send(err.code, err.msg);
        }
        res.send(200);
...
OAE.tenantServer.get('/api/tenants', function(req, res, next) {
    req.telemetryUrl = '/api/tenants';
    TenantsAPI.getAllTenants(function(err, tenants) {
        if (err) {
            return res.send(err.code, err.msg);
