...
var log = require('oae-logger').logger('IO');
 ...
 */
var getFileListForFolder = (module.exports.getFileListForFolder = function(
  foldername,
  callback,
) {
...
var copyFile = (module.exports.copyFile = function(source, dest, callback) {
  ...
  ins.once('error', function(err) {
    ...
    log().error(
      { err: err },
      "Wasn't able to copy the file %s to %s.",
      source,
      dest,
    );
  ...
  fs.rename(source, dest, function(err) {
    if (err) {
      ...
      if (err.code !== 'EXDEV') {
        log().error(
          { err: err },
          "Wasn't able to rename the file  %s to %s.",
          source,
          dest,
        );
 ...
 */
var exists = (module.exports.exists = function(path, callback) {
  fs.open(path, 'r', function(err) {
    if (err) {
      if (err.code === 'ENOENT') {
        ...
        return callback({
          code: 500,
          msg: 'Could not check whether a file or folder exists',
        });
      }
    } else {
