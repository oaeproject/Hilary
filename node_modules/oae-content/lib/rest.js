...
module.exports = function(tenant) {
    ...
    tenant.server.post('/api/content/create', function(req, res) {
        ...
        // File creation
            ...
            if (!req.files.file) {
                // No file was found, which means a bad request.
    ...
    tenant.server.get('/api/content/:contentId/download', function(req, res) {
        ...
        ContentAPI.hasAccess(req.ctx, req.params.contentId, function(err, hasAccess) {
            if (err) {
                return res.send(err.code, err.msg);
            }
            if (!hasAccess) {
            ...
            ContentAPI.getLatestRevision(req.ctx, req.params.contentId, function(err, revision) {
                ...
                var link = ContentAPI.getDownloadLink(req.ctx, revision.uri);

                if (link.code === 200) {
                    var mimetype = revision.mimetype || 'application/octet-stream';
