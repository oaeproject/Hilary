    ...
    if (processIndexJobs && !boundIndexWorkers) {
        ...
        TaskQueue.bind(SearchConstants.mq.TASK_INDEX_DOCUMENT, _handleIndexDocumentTask, null, function() {
            TaskQueue.bind(SearchConstants.mq.TASK_DELETE_DOCUMENT, _handleDeleteDocumentTask, null, function() {
                TaskQueue.bind(SearchConstants.mq.TASK_REINDEX_ALL, _handleReindexAllTask, null, callback);
            });
        });
        ...
        TaskQueue.unbind(SearchConstants.mq.TASK_INDEX_DOCUMENT, function() {
            TaskQueue.unbind(SearchConstants.mq.TASK_DELETE_DOCUMENT, function() {
                TaskQueue.unbind(SearchConstants.mq.TASK_REINDEX_ALL, callback);
            });
        });
    } else {
 ...
 * @api private
 */
var _ensureSearchSchema = function(callback) {
    ...
    client.putMapping(SearchConstants.resourceMappingName, resourceSchema, {'_source': false}, function(err) {
        if (err) {
            return callback(err);
        }
        ...
        client.putMapping(SearchConstants.resourceMembersMappingName, resourceMembersSchema, childOpts, function(err) {
            if (err) {
                return callback(err);
            }

            return client.putMapping(SearchConstants.resourceMembershipsMappingName, resourceMembershipsSchema, childOpts, callback);
        });
    });
};
