...
// A dictionary that maps a widget name on it's configfile.
var widgetConfigs = {};
 ...
 * It will alsp place watches on the necessary files so if anything changes
 ...
 */
var getFile = module.exports.getFile = function(url, callback) {
    if (fileCache[url]) {
        callback(false, fileCache[url]);
    } else {
...
/**
 * @return {Object} An object where each key is the widgetname and
 *                  the value is the widget config.
 */
...
/**
 * @return {Object} The config object for the specific widget or null if no
 *                  config was found for that widget.
 */
var getWidgetConfig = module.exports.getWidgetConfig = function(widget) {
    return widgetConfigs[widget];
};
