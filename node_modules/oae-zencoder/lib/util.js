...
var getConfig = module.exports.getConfig = function(tenantAlias) {
    ...
    if (tenantEnabled && apiKey) {
        return {
            'apiKey': apiKey,
            'amazons3-region': ZencoderConfig.getValue(tenantAlias, 'zencoder', 'amazons3-region'),
            'amazons3-bucket': ZencoderConfig.getValue(tenantAlias, 'zencoder', 'amazons3-bucket'),
            'amazons3-access-key': ZencoderConfig.getValue(tenantAlias, 'zencoder', 'amazons3-access-key'),
            'amazons3-secret-key': ZencoderConfig.getValue(tenantAlias, 'zencoder', 'amazons3-secret-key')
        };
    } else {
        return config;
    }
...
var createJob = module.exports.createJob = function(tenantAlias, path, contentId, revisionId, mime, callback) {
    ...
    var uriEncodedContentId = encodeURIComponent(contentId);

    var tenant = TenantsAPI.getTenant(tenantAlias);
    var baseUrl = util.format('s3://s3-%s.amazonaws.com/%s/%s', config['amazons3-region'], config['amazons3-bucket'], uriEncodedContentId);
    ...
    var options = {
        ...
        'pass_through': util.format('%s/%s', uriEncodedContentId, filename),
        'private': true,
        'height': 600,
        'output': {
