...
describe('Content', function() {
    ...
    describe('Download content', function() {
        ...
        it('verify validation of signed urls', function(callback) {
            setUpUsers(function(contexts) {
                ...
                RestUtil.performRestRequest(contexts['branden'].restContext, '/api/download/signed', 'GET', parsedUrl.query, function(err, body, response) {
                    assert.ok(!err);
                    assert.strictEqual(response.statusCode, 204);

                    // Simon should be able to download the content item using the same signature
                    RestUtil.performRestRequest(contexts['simon'].restContext, '/api/download/signed', 'GET', parsedUrl.query, function(err, body, response) {
                        assert.ok(!err);

                        // Global admin should be able to download the content item using the same signature
                        RestUtil.performRestRequest(globalAdminRestContext, '/api/download/signed', 'GET', parsedUrl.query, function(err, body, response) {
                            assert.ok(!err);

                            // An anonymous user can download it using the same signature as well
                            RestUtil.performRestRequest(anonymousRestContext, '/api/download/signed', 'GET', parsedUrl.query, function(err, body, response) {
                                assert.ok(!err);

                                // Missing uri
                                RestUtil.performRestRequest(contexts['branden'].restContext, '/api/download/signed', 'GET', _.omit(parsedUrl.query, 'uri'), function(err, body, request) {
                                    assert.equal(err.code, 401);

                                    // Different uri has an invalid signature
                                    RestUtil.performRestRequest(contexts['branden'].restContext, '/api/download/signed', 'GET', _.extend({}, parsedUrl.query, {'uri': 'blahblahblah'}), function(err, body, request) {
                                        assert.equal(err.code, 401);

                                        // Missing signature parameter
                                        RestUtil.performRestRequest(contexts['branden'].restContext, '/api/download/signed', 'GET', _.omit(parsedUrl.query, 'signature'), function(err, body, request) {
                                            assert.equal(err.code, 401);

                                            // Different signature should fail assertion
                                            RestUtil.performRestRequest(contexts['branden'].restContext, '/api/download/signed', 'GET', _.extend({}, parsedUrl.query, {'signature': 'ATTACK LOL!!'}), function(err, body, request) {
                                                assert.equal(err.code, 401);

                                                // Missing expires parameter
                                                RestUtil.performRestRequest(contexts['branden'].restContext, '/api/download/signed', 'GET', _.omit(parsedUrl.query, 'expires'), function(err, body, request) {
                                                    assert.equal(err.code, 401);

                                                    // Missing signature parameter
                                                    RestUtil.performRestRequest(contexts['branden'].restContext, '/api/download/signed', 'GET', _.extend({}, parsedUrl.query, {'expires': 2345678901}), function(err, body, request) {
                                                        assert.equal(err.code, 401);
    ...
    describe('Create content', function() {
        ...
        it('verify cannot create content of unknown resourceSubType', function(callback) {
            ...
            RestUtil.performRestRequest(camAdminRestContext, '/api/content/create', 'POST', {'resourceSubType': 'unicorns'}, function(err) {
                assert.equal(err.code, 400);
                callback();
            });
        });
