...
const createRoom = module.exports.createRoom = function(content, callback) {
    ...
    .then(data => {
        ...
        .catch(err => {
            ...
            return callback({'code': 500, 'msg': 'Could not create Ethercalc room'});
        });

    })
    .catch(err => {
        log().error({'err': err, 'contentId': contentId, 'ethercalcRoomId': roomId, 'ethercalc': ethercalcConfig.host}, 'Could not create Ethercalc room');
        return callback(err);
    });
};
 ...
 * @api private
 */
const setEdited = module.exports.setEdited = function(data, callback) {
    if (data.contentId && data.userId) {
        ContentDAO.Ethercalc.setEdited(data.contentId, data.userId, function(err) {
            if (err) {
                return callback(err);
            }
            return callback(null);
