...
var createMessage = module.exports.createMessage = function(messageBoxId, createdBy, body, opts, callback) {
    ...
    _getMessageThreadKey(replyToMessageId, function(err, replyToThreadKey) {
        ...
        var lockUniqueTimestamp = function(timestamp, callback) {
            Locking.acquire(timestamp, 1, function(err, lockToken) {
                if (err || !lockToken) {
                    return lockUniqueTimestamp(Date.now());
                }
                return callback(timestamp, lockToken);
            });
        };
        lockUniqueTimestamp(Date.now(), function(created, lockToken) {
...
var _getThreadKeysFromMessageBox = function(messageBoxId, start, limit, callback) {
    ...
    Cassandra.runPagedColumnQuery('MessageBoxMessages', 'messageBoxId', messageBoxId, start, limit, {'reversed': true}, function(err, row, nextToken) {
        if (err) {
            return callback(err);
        }
