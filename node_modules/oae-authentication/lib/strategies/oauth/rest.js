...
server.exchange(
  oauth2orize.exchange.clientCredentials(
    ...
    function(client, scope, callback) {
      ...
      OAuthDAO.AccessTokens.getAccessTokenForUserAndClient(
        ...
        function(err, accessToken) {
          ...
          OAuthDAO.AccessTokens.createAccessToken(
            token,
            client.userId,
            client.id,
            function(err) {
...
var verifyClientAuthentication = function(clientId, clientSecret, callback) {
  OAuthDAO.Clients.getClientById(clientId, function(err, client) {
    if (err) {
      ...
      log().warn(
        { client: client.id },
        'A client attempted to authenticate with the wrong secret',
      );
      return callback(null, false);
    }
 ...
 * @HttpResponse                    401             Unauthorized
 */
OAE.tenantRouter.on('post', '/api/auth/oauth/clients/:userId', function(
  req,
  res,
) {
 ...
 * @HttpResponse                    401             Unauthorized
 */
OAE.tenantRouter.on('get', '/api/auth/oauth/clients/:userId', function(
  req,
  res,
) {
  OAuthAPI.Clients.getClients(req.ctx, req.params.userId, function(
    err,
    clients,
  ) {
    if (err) {
...
OAE.tenantRouter.on(
  'post',
  '/api/auth/oauth/clients/:userId/:clientId',
  function(req, res) {
