...
const PrincipalsModel = require('./model');
 ...
 * @api private
 */
var getAllUsersForTenant = function(ctx, tenantAlias, callback) {
  tenantAlias = tenantAlias || ctx.user().tenant.alias;
  const tenant = TenantsAPI.getTenant(tenantAlias);
 ...
 */
var canDeleteUser = function(ctx, userId, callback) {
  if (!ctx.user()) {
 ...
 * @param  {User}       callback.user       The user that was fetched to perform the checks. Will not be specified if the authorization fails
 */
var canRestoreUser = function(ctx, userId, callback) {
  if (!ctx.user()) {
    return callback(null, false);
  }
 ...
 */
var getUser = function(ctx, userId, callback) {
  const validator = new Validator();
...
const _getMessages = function(txt, messages, callback) {
  ...
  _.each(messages, message => {
    const messageCreated = dateFormat(
      new Date(parseInt(message.created)),
