 ...
 *
 * @param  {User[]}     users                           The users that should be queued
 * @param  {Function}   callback                        Standard callback function
 * @param  {Object}     callback.err                    Standard error object, if any
 */
var queueUsersForEmail = module.exports.queueUsersForEmail = function(users, callback) {
    ...
    _.each(users, function(user) {
        var bucketId = createEmailBucketIdForUser(user.id, user.emailPreference, user.timezone);
        queries.push({'query': 'INSERT INTO "EmailBuckets" ("bucketId", "userId") VALUES (?, ?)', 'parameters': [bucketId, user.id]});
    });
 ...
 * @param  {Function}   callback                Standard callback function
 * @param  {Object}     callback.err            Standard error object, if any
 * @param  {String}     callback.bucketId       The fully specified ID of the bucket from which the users were retrieved
 * @param  {Object}     callback.userIds        The IDs of the users that were in this bucket
...
var getQueuedUsersForEmail = module.exports.getQueuedUsersForEmail = function(bucketNumber, emailPreference, hour, start, limit, callback) {
    ...
    Cassandra.runPagedQuery('EmailBuckets', 'bucketId', bucketId, 'userId', start, limit, {'end': 'z'}, function(err, rows, nextToken) {
        if (err) {
            return callback(err);
        }
...
/**
 * Unqueues a set of users from a bucket.
 *
...
var unqueueUsersForEmail = module.exports.unqueueUsersForEmail = function(bucketId, userIds, callback) {
    ...
    _.each(userIds, function(userId) {
        queries.push({'query': 'DELETE FROM "EmailBuckets" WHERE "bucketId" = ? AND "userId" = ?', 'parameters': [bucketId, userId]});
    });
    Cassandra.runBatchQuery(queries, callback);
};

/**
 * Given a user ID and his email preference, return the approriate email bucket ID
 *
 ...
 * @param  {String}     timezone            The user's timezone. Defaults to `Etc/UTC`
 * @return {String}                         The ID of the email bucket that this user should go in
 */
var createEmailBucketIdForUser = module.exports.createEmailBucketIdForUser = function(userId, emailPreference, timezone) {
    ...
    if (emailPreference !== PrincipalsConstants.emailPreferences.IMMEDIATE) {
        /*
         * Take the user's timezone into account for daily and/or weekly mails. We try to deliver an e-mail
         * at the configured hour in the user's timezone. In order to do this we need the timezone offset
         * between the user and the server's timezone.
         *
         * For example, assume the server is in Holland (GMT+1), the user is in Miami (GMT-4) and mails need
        ...
        var userTimezoneOffsetInHours = Math.round(new TZ.Date(timezone).getTimezoneOffset() / 60);

        // Add (or subtract) the offset and ensure that we don't go over 24 (as there are only 24 hours in a day)
        hour = (mailConfig[emailPreference].hour + userTimezoneOffsetInHours) % 24;
    }
 ...
 *
 * @param  {String}     bucket      The bucket ID from which to retrieve the email preference
 * @return {String}                 The email preference
 * @api private
 */
var _getEmailPreferenceFromBucketId = function(bucketId) {
    return bucketId.split(':')[2];
};
