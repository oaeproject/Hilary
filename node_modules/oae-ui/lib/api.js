...
// A dictionary that will hold the widget configs for all widgets. This will be filled upon
// initialization.
 ...
 *
 * @param  {String}     uiDir       The absolute path to the directory containing the UI repository.
 */
var init = module.exports.init = function(uiDir) {
    // Cache the ui directory path
    uiDirectory = uiDir;

    // Cache all of the widget config files
    cacheWidgetConfigs();
 ...
 * widget config file, and refresh the widget config cache if that's the case
 * 
...
var updateFileCaches = function(filename) {
    ...
    // config files
    if (/^\/node_modules\/(.*?)\/config.json$/.test(filename)) {
        cacheWidgetConfigs();
    }
};
...
/**
 * Get the aggregated list of widget configs.
 * 
 * @return {Object}     An object where each key is the widget id and the value is the widget config.
 */
...
/**
 * Cache all widget configs under the UI's node_modules directory
 */
var cacheWidgetConfigs = function() {
    widgetConfigCache = {};
    // Cache all of the widget config files under node_modules
    readdirp({ root: uiDirectory + '/node_modules', fileFilter: 'config.json' })
      .on('data', function (entry) {
          var widgetId = entry.parentDir.split('/')[entry.parentDir.split('/').length - 1];
          widgetConfigCache[widgetId] = require(entry.fullPath);
          widgetConfigCache[widgetId].id = widgetId;
          widgetConfigCache[widgetId].path = entry.parentDir;
      });
}
 ...
 */
var staticBatch = module.exports.staticBatch = function(files, callback) {
    ...
    for (var i = 0; i < files.length; i++) {
        validator.check(files[i], {'code': 400, 'msg': 'A valid file URL needs to be provided'}).notEmpty();
    }
...
var getStaticFile = function(path, callback) {
    ...
    if (staticFileCache[path]) {
        ...
        cacheFile(path, function(err, data) {
            if (err) {
                return callback(err);
            }
