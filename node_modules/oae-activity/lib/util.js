 ...
 */
var getStandardResourcePropagation = module.exports.getStandardResourcePropagation = function(resourceVisibility, resourceJoinable, callback) {
    var propagation = [];
    ...
    // If the resource is "joinable", then we do allow others to see it, but only if their tenant can interact with the resource's tenant. This
    // includes resources that are private, so we apply this propagation regardless if the resource is private or not
    if (resourceJoinable === AuthzConstants.joinable.YES) {
        propagation.push({'type': ActivityConstants.entityPropagation.INTERACTING_TENANTS});
    }

    // Always allow routing to the self association
    propagation.push({'type': ActivityConstants.entityPropagation.ASSOCIATION, 'association': 'self'});

    // Always allow routing to the members of the item whether it's loggedin or private
    propagation.push({'type': ActivityConstants.entityPropagation.ASSOCIATION, 'association': 'members'});
    return callback(null, propagation);
};
...
var getAllAuthzMembersByRole = module.exports.getAllAuthzMembersByRole = function(resourceId, callback) {
    AuthzAPI.getAuthzMembers(resourceId, null, 10000, function(err, members) {
        ...
        _getAllAuthzGroupMembersByRole(groupMembersByRole, function(err, indirectMembersByRole) {
            ...
            _.each(indirectMembersByRole, function(indirectMembers, role) {
 ...
 */
var createActivityStreamId = module.exports.createActivityStreamId = function(resourceId, activityStreamType) {
 ...
 */
var parseActivityStreamId = module.exports.parseActivityStreamId = function(activityStreamId) {
    var parts = activityStreamId.split('#');
    return {
        'resourceId': parts[0],
        'streamType': parts[1]
    };
