...
describe('Search', function() {
    ...
    describe('Indexing', function() {
         ...
         */
        it('verify mime property where appropriate', function(callback) {
            ...
            RestAPI.User.createUser(camAdminRestContext, doerUsername, 'password', 'Doer', null, function(err, doer) {
                ...
                RestAPI.Content.createLink(doerRestContext, 'Test Content 1', uniqueString, 'public', 'http://www.sakaiproject.org/',  [], [], function(err, link) {
                    assert.ok(!err);

                    SearchTestsUtil.searchAll(doerRestContext, 'general', null, {'resourceTypes': 'content', 'q': uniqueString}, function(err, results) {
                        assert.ok(!err);
                        var contentDoc = _getDocById(results, link.id);
                        assert.ok(contentDoc);
                        ...
                        var file = fs.createReadStream( __dirname + '/data/sakaiger.png');
                        var uniqueString = TestsUtil.generateTestUserId('resourcetype-test');
                        RestAPI.Content.createFile(doerRestContext, 'Test Content 2', uniqueString, 'public', file,  [], [], function(err, contentObj) {
                            assert.ok(!err);
