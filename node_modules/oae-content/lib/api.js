...
var getContent = module.exports.getContent = function(ctx, contentId, callback) {
    ...
    ContentDAO.Content.getContent(contentId, function(err, contentObj) {
        ...
        AuthzPermissions.canView(ctx, contentObj, function(err) {
            ...
            ContentAPI.emit(ContentConstants.events.GET_CONTENT, ctx, contentObj, function(errs, results) {
                if (errs) return callback(_.first(errs));

                // This function always return an array of errs and results
                contentObj = results[0];
...
/**
 * Create a loodle as a pooled content item
 *
 * @param  {Context}        ctx                     Standard context object containing the current user and the current tenant
 * @param  {String}         displayName             The display name of the loodle
 * @param  {String}         [description]           A longer description for the loodle
 * @param  {String}         [visibility]            The visibility of the loodle. One of `public`, `loggedin`, `private`
 * @param  {Object}         [additionalMembers]     Object where the keys represent principal ids that need to be added to the content upon creation and the values represent the role that principal will have. Possible values are "viewer", "editor" and "manager"
 * @param  {String[]}       [folders]               The ids of the folders to which this loodle should be added
 * @param  {Function}       callback                Standard callback function
 * @param  {Object}         callback.err            An error that occurred, if any
 * @param  {Content}        callback.content        The created loodle
 */
var createLoodle = module.exports.createLoodle = function (ctx, displayName, description, visibility, additionalMembers, folders, callback) {
...
var updateContentMetadata = module.exports.updateContentMetadata = function(ctx, contentId, profileFields, callback) {
    ...
    _canManage(ctx, contentId, function(err, oldContentObj) {
        ...
        ContentDAO.Content.updateContent(oldContentObj, profileFields, true, function(err, newContentObj) {
            ...
            ContentAPI.emit(ContentConstants.events.UPDATED_CONTENT, ctx, newContentObj, oldContentObj, function(errs) {
                if (errs) {
                    return callback(_.first(errs));
                }
