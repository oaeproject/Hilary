...
describe('Configuration', function() {
    ...
    describe('Internal Configuration API', function() {
        ...
        it('verify get single config value', function(callback) {
            ...
            assert.equal(AuthenticationConfig.getValue(johnContext, 'sso', 'enabled'), null);
            // Retrieve a boolean value
            assert.equal(AuthenticationConfig.getValue(johnContext, 'twitter', 'enabled'), true);
            // Retrieve a string value
            assert.equal(PrincipalsConfig.getValue(johnContext, 'user', 'defaultLanguage'), 'en_GB');
            // Retrieve a suppressed value
            assert.equal(PrincipalsConfig.getValue(johnContext, 'recaptcha', 'privateKey'), '6LcFWdYSAAAAANrHjt2Y5VJXoICHa95PFDarVcGs');
            callback();
        });
        ...
        it('verify validation', function(callback) {
            // Verify that initializing a config factory needs a module name. This will throw an error,
            // so we surround it with a try catch
            try {
                assert.equal(ConfigAPI.config().code, 400);
                // No error has been thrown if we make it this far
                assert.fail();
            } catch (err) {
                assert.ok(err);
            }
            ...
            var AuthenticationConfig = ConfigAPI.config('oae-authentication');
            assert.equal(AuthenticationConfig.getValue(johnContext), null);
            // Verify that an element needs to be provided when getting a config value
            assert.equal(AuthenticationConfig.getValue(johnContext, 'twitter'), null);
            callback();
        });
    });
