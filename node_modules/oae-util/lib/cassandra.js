...
var nativeCassandra = require('cassandra-native-driver');
...
// Variable that will hold the cached prepared queries
var cachedPreparedQueries = {};
...
module.exports.init = function(config, callback) {
    ...
    client = _createCassandraClient(CONFIG);
    client.connect({'address': config.hosts}, function(err) {
        if (err) {
        ...
        createKeyspace(keyspace, function(err) {
            if (err) {
                log().error({'err': err}, 'Failed to create the keyspace');
                return callback({'code': 500, 'msg': 'Failed to create the keyspace'});
            }

            var query = util.format('USE "%s"', keyspace);
            client.execute(query, function(err) {
                if (err) {
                    log().error({'err': err}, 'Error connecting to the configured keyspace');
                    return callback({'code': 500, 'msg': 'Error connecting to the configured keyspace'});
                }
...
var createKeyspace = module.exports.createKeyspace = function(name, callback) {
    var replication = util.format('{\'class\': \'%s\', \'replication_factor\': \'%s\'}', CONFIG.strategyClass, CONFIG.replication);
    runQuery(util.format('CREATE KEYSPACE "%s" WITH REPLICATION = %s', name, replication), null, {'cache': false}, function(err) {
        if (err && (!err.cause || err.cause.message.indexOf('Cannot add existing keyspace') === -1)) {
            return callback(err);
        }
 ...
 */
var dropKeyspace = module.exports.dropKeyspace = function(name, callback) {// TODO: Implement
    callback = callback || function() {};
    runQuery(util.format('DROP KEYSPACE "%s"', name), null, {'cache': false}, function(err) {
        if (err) {
            return callback(err);
        }
...
var columnFamilyExists = module.exports.columnFamilyExists = function(name, callback) {
    var cql = 'SELECT "columnfamily_name" FROM "system"."schema_columnfamilies" WHERE "keyspace_name" = ? AND columnfamily_name = ?';
    runQuery(cql, [CONFIG.keyspace, name], {'cache': true}, function(err, rows) {
        if (err) {
            return callback(err);
        }
...
var dropColumnFamily = module.exports.dropColumnFamily = function(name, callback) {
    ...
    columnFamilyExists(name, function(err, exists) {
        if (err) {
            ...
            return callback({'code': 400, 'msg': 'The table ' + name + ' could not be dropped as it does not exist'});
        }

        return runQuery(util.format('DROP TABLE "%s"', name), [], {'cache': true}, callback);
    });
};
...
var createColumnFamily = module.exports.createColumnFamily = function(name, cql, callback) {
    ...
    columnFamilyExists(name, function(err, exists) {
        ...
        if (!exists) {
            runQuery(cql, false, {'cache': false}, function(err) {
                if (err) {
                    return callback(err);
                }
...
var createColumnFamilies = module.exports.createColumnFamilies = function(families, callback) {
    callback = callback || function() {};
    var keys = _.keys(families).slice();
 ...
 * @param  {Object}   callback.err  An error that occurred, if any
 */
var runQuery = module.exports.runQuery = function(query, parameters, opts, callback) {
    ...
    executeQuery(query, parameters, opts, callback);
};

/**
 * Run a `.. WHERE x IN (...)` query
 *
 * @param  {String}     queryWithoutWhere           The first part of the query
 * @param  {String[]}   parametersWithoutWhere      The parameters  for the first part of the query
 * @param  {String}     key                         The key to filter by
 * @param  {String[]}   inArray                     The values to filter by
 * @param  {Function}   callback                    Standard callback function
 * @param  {Object}     callback.err                An error that occurred, if any
 * @param  {Row[]}      callback.rows               The returned Cassandra rows
 */
var runInQuery = module.exports.runInQuery = function(queryWithoutWhere, parametersWithoutWhere, key, inArray, callback) {
    parametersWithoutWhere = parametersWithoutWhere || [];

    // Build the query up to the where
    var parameters = parametersWithoutWhere.slice();
    var inPlaceholders = [];
    _.each(inArray, function(inItem) {
        parameters.push(inItem);
        inPlaceholders.push('?');
    });

    // To avoid leaking memory, we only cache the generated query if there's less than 10 parameters
    var cache = false;
    if ((parametersWithoutWhere.length + inPlaceholders.length) < 10) {
        cache = true;
    }

    var cql = util.format('%s WHERE "%s" IN (%s)', queryWithoutWhere, key, inPlaceholders.join(','));
    return runQuery(cql, parameters, {'cache': cache}, callback);
};
 ...
 * @param  {Object}     callback.err        An error that occurred, if any
 */
var runBatchQuery = module.exports.runBatchQuery = function(queries, callback) {
    ...
    query += 'APPLY BATCH;';
    executeQuery(query, parameters, {'cache': false}, callback);
};
 ...
 */
var runPagedQuery = module.exports.runPagedQuery = function(tableName, keyColumnName, keyColumnValue, rangeColumnName, start, limit, opts, callback) {
    limit = OaeUtil.getNumberParam(limit, 25);
    opts = opts || {};

    var startOperator = (opts.reversed) ? '<' : '>';
    var endOperator = (opts.reversed) ? '>=' : '<=';

    var cql = util.format('SELECT * FROM "%s" WHERE "%s" = ?', tableName, keyColumnName);
    var params = [keyColumnValue];
    ...
    runQuery(cql, params, {'cache': false}, function(err, rows) {
        if (err) {
            return callback(err);
        ...
        var results = rows.slice(0, limit);
        var nextToken = (results.length === limit) ? _.last(results)[rangeColumnName] : null;
 ...
 */
var runAllPagesQuery = module.exports.runAllPagesQuery = function(tableName, keyColumnName, keyColumnValue, rangeColumnName, opts, callback, _nextToken, _rows) {
    _rows = _rows || [];
    opts = opts || {};
    ...
    var start = opts.start || _nextToken;
    runPagedQuery(tableName, keyColumnName, keyColumnValue, rangeColumnName, start, opts.batchSize, {'end': opts.end}, function(err, rows, nextToken) {
        if (err) {
            return callback(err);
        }
        ...
        return runAllPagesQuery(tableName, keyColumnName, keyColumnValue, rangeColumnName, opts, callback, nextToken, _rows);
    });
};
 ...
 * @param  {Row[]}      onEach.rows             A set of `Row` objects, holding all the rows fetched from storage in this iteration.
 ...
 * @api private
 */
var _iterateAll = function(columnNames, columnFamily, keyColumnName, returnKeyColumn, batchSize, onEach, callback, fromKey) {
    var query = _buildIterateAllQuery(columnNames, columnFamily, keyColumnName, batchSize, fromKey);
    ...
    runQuery(query.query, query.parameters, {'cache': false}, function(err, rows) {
        if (err) {
            return callback(err);
        } else if (_.isEmpty(rows)) {
        ...
        _.each(rows, function(row) {
            ...
            fromKey = row[keyColumnName];

            // Clean the key off the row if it was not requested
            if (!returnKeyColumn) {
                delete row[keyColumnName];
            }
        });
 ...
 * @api private
 */
var _buildIterateAllQuery = function(columnNames, columnFamily, keyColumnName, batchSize, fromKey) {
    var cql = 'SELECT ';
    var params = [];

    if (!_.isEmpty(columnNames)) {
        // Quote all the column names and join with a comma
        cql += _.map(columnNames, function(columnName) {
...
var rowToHash = module.exports.rowToHash = function(row) {
    // TODO: Rows are already hash objects, get rid of this method
    return row;
};

/**
 * Convert a bigint value that holds an epoch timestamp (in microseconds) to a Date object
 *
 * @param  {Object}     bigint      The bigint object to convert
 * @return {Date}                   The converted date object
 */
var bigintTimestampToDate = module.exports.bigintTimestampToDate = function(bigint) {
    var l = new Long(bigint.low, bigint.high);
    return new Date(Math.floor(l.toNumber() / 1000));
};
 ...
 */
var constructUpsertCQL = module.exports.constructUpsertCQL = function(cf, rowKey, rowValue, values, ttl) {
 ...
 * Prepare a CQL query
 *
 * @param  {String}     query                       The CQL query to prepare
 * @param  {Object}     opts                        A set of additional options
 * @param  {Object}     opts.cache                  Whether the prepared query can be retrieved from or stored in the cache
 * @param  {Function}   callback                    Standard callback function
 * @param  {Object}     callback.err                An error that occurred, if any
 * @param  {Object}     callback.preparedQuery      The prepared query instance
 */
var prepare = function(query, opts, callback) {
    opts = opts || {};
    if (opts.cache && cachedPreparedQueries[query]) {
        return callback(null, cachedPreparedQueries[query]);
    } else {
        client.prepare(query, function(err, preparedQuery) {
            if (err) {
                log().error({
                    'err': err,
                    'query': query
                }, 'Could not prepare a query');
                return callback({'code': 500, 'msg': 'Could not prepare a query'});
            }

            // Cache the prepared statement only if:
            //  - we're told the query is cache-able
            //  - there are less than 10 arguments being passed in. This is a simple heuristic to
            //    avoid leaking memory when someone builds a dynamic query but forgets to set the
            //    `opts.cache` value to false
            if (opts.cache && query.split('?').length < 10) {
                cachedPreparedQueries[query] = preparedQuery;
            }

            return callback(null, preparedQuery);
        });
    }
};
 ...
 * @api private
 */
var executeQuery = function(query, parameters, opts, callback) {
    ...
    prepare(query, opts, function(err, preparedQuery) {
        if (err) {
            log().error({
                'err': err,
                'query': query
            }, 'Could not prepare a query');
            return callback({'code': 500, 'msg': 'Could not prepare a query', 'cause': err});
        }

        var q = preparedQuery.query();
        q.bind(parameters, {});
        q.execute({}, function(err, result) {
            if (err) {
                ...
                return callback({'code': 500, 'msg': 'An error occurred executing a query', 'cause': err});
            }
 ...
 *
 * @return {Client}         A Cassandra client
 * @api private
 */
var _createCassandraClient = function(config) {
    var cassandraConfig = {
        // Exceeded write bytes water mark
        'write_bytes_high_water_mark': 65536 * 8
    };

    return new nativeCassandra.Client(cassandraConfig);
};
