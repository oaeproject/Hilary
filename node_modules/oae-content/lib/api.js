...
var _createFile = function(ctx, displayName, description, visibility, additionalMembers, file, callback) {
    ...
    validator.check(displayName, {'code': 400, 'msg': 'A displayName can only be a 1000 characters long'}).isShortString();
    if (description) {
    ...
    mime(file.path, function(err, type) {
        if (err) {
            log().warn({'err': err, 'file': file}, 'Could not determine the file type, defaulting to application/octet-stream');
...
var updateContentMetadata = module.exports.updateContentMetadata = function(ctx, contentId, profileFields, callback) {
    ...
    for (var i = 0; i < fieldNames.length; i++) {
        ...
        if (fieldName === 'displayName') {
            ...
            validator.check(profileFields['displayName'], {'code': 400, 'msg': 'A displayName can only be a 1000 characters long'}).isShortString();
...
var createComment = module.exports.createComment = function(ctx, contentId, body, replyToCreatedTimestamp, callback) {
    ...
    validator.check(body, {'code': 400, 'msg': 'A comment must be provided'}).isLongString();
...
var _getRevisions = function(ctx, contentObj, start, limit, opts, callback) {
    // Page the query.
    ContentDAO.Revisions.getRevisions(contentObj.id, start, limit, opts, function(err, revisions) {
        if (err) {
            return callback(err);
        }
