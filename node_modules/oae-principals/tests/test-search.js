...
describe('Search', function() {
    ...
    describe('Indexing', function() {
        ...
        /**
         * Test that verifies when a user is indexed with just the user id, it still indexes the user
         */
        it('verify indexing without full user item', function(callback) {
            TestsUtil.generateTestUsers(camAdminRestContext, 2, function(err, users, doer, jack) {
                ...
                SearchTestsUtil.searchAll(doer.restContext, 'general', null, {'resourceTypes': 'user', 'q': jack.user.displayName}, function(err, results) {
                    ...
                    // Verify the email address is indexed
                    SearchTestsUtil.searchAll(doer.restContext, 'general', null, {'resourceTypes': 'user', 'q': jack.user.email}, function(err, results) {
                        assert.ok(!err);
                        var userDoc = _getDocById(results, jack.user.id);
                        assert.ok(userDoc);

                        // Delete the user from the index under the hood, this is to avoid the automatic index events invalidating the test
                        ElasticSearch.del('resource', jack.user.id, function(err) {
                            assert.ok(!err);

                            // Verify the user item no longer exists
                            SearchTestsUtil.searchAll(doer.restContext, 'general', null, {'resourceTypes': 'user', 'q': jack.user.displayName}, function(err, results) {
                                assert.ok(!err);
                                var userDoc = _getDocById(results, jack.user.id);
                                assert.ok(!userDoc);

                                // Verify we cannot find the user via their email neither
                                SearchTestsUtil.searchAll(doer.restContext, 'general', null, {'resourceTypes': 'user', 'q': jack.user.email}, function(err, results) {
                                    assert.ok(!err);
                                    var userDoc = _getDocById(results, jack.user.id);
                                    assert.ok(!userDoc);
                                    ...
                                    SearchAPI.postIndexTask('user', [{'id': jack.user.id}], {'resource': true}, function(err) {
                                        ...
                                        // Ensure that the full user item is now back in the search index
                                        SearchTestsUtil.searchAll(doer.restContext, 'general', null, {'resourceTypes': 'user', 'q': jack.user.displayName}, function(err, results) {
                                            assert.ok(!err);
                                            var userDoc = _getDocById(results, jack.user.id);
                                            assert.ok(userDoc);
        ...
        /**
         * Test that verifies when a group is indexed with just the group id, it still indexes the group
         */
            ...
            TestsUtil.generateTestUsers(camAdminRestContext, 1, function(err, users, doer) {
                ...
                RestAPI.Group.createGroup(doer.restContext, groupText, groupText, 'public', 'no', [], [], function(err, group) {
                    ...
                    SearchTestsUtil.searchAll(doer.restContext, 'general', null, {'resourceTypes': 'group', 'q': groupText}, function(err, results) {
                        ...
                        // Delete the group from the index under the hood, this is to avoid the automatic index events invalidating the test
                        ElasticSearch.del('resource', group.id, function(err) {
                            ...
                            SearchTestsUtil.searchAll(doer.restContext, 'general', null, {'resourceTypes': 'group', 'q': groupText}, function(err, results) {
                                ...
                                SearchAPI.postIndexTask('group', [{'id': group.id}], {'resource': true}, function(err) {
                                    ...
                                    // Ensure that the full group is now back in the search index
                                    SearchTestsUtil.searchAll(doer.restContext, 'general', null, {'resourceTypes': 'group', 'q': groupText}, function(err, results) {
                                        assert.ok(!err);
                                        var groupDoc = _getDocById(results, group.id);
                                        assert.ok(groupDoc);
