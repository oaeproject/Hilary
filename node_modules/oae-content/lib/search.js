 ...
 */
ContentAPI.on(ContentConstants.events.UPDATED_CONTENT_PREVIEW, function(
  content,
) {
  SearchAPI.postIndexTask('content', [{ id: content.id }], {
    resource: true,
 ...
 */
ContentAPI.on(ContentConstants.events.DELETED_CONTENT, function(
  ctx,
  contentObj,
) {
 ...
 */
ContentAPI.on(ContentConstants.events.DELETED_COMMENT, function(
  ctx,
  comment,
  content,
) {
 ...
 * @api private
 */
var _produceContentCommentDocuments = function(
  resources,
  callback,
  _documents,
  _errs,
  ...
  if (resource.comments) {
    var documents = MessageBoxSearch.createMessageSearchDocuments(
      ContentConstants.search.MAPPING_CONTENT_COMMENT,
      resource.id,
      resource.comments,
    );
    _documents = _.union(_documents, documents);
    return _produceContentCommentDocuments(
      resources,
      callback,
      _documents,
      _errs,
 ...
 * @api private
 */
var _produceContentBodyDocuments = function(
  resources,
  callback,
  _documents,
  _errs,
  ...
  ContentDAO.Revisions.getRevisions(resource.id, null, 1, null, function(
    err,
    revisions,
  ) {
    if (err) {
      _errs = _.union(_errs, [err]);
      return _produceContentBodyDocuments(
        resources,
        callback,
        _documents,
        _errs,
    ...
    if (
      ...
      log().trace(
        { id: resource.id, previews: revision.previews },
        'No text to index',
      );
    ...
    ContentDAO.Previews.getContentPreview(
      revision.previewsId,
      'plain.txt',
      function(err, preview) {
        ...
        var tenantAlias = AuthzUtil.getResourceFromId(revision.previewsId)
          .tenantAlias;
        ContentUtil.getStorageBackend(null, preview.uri).get(
          ...
          function(err, file) {
            ...
            fs.readFile(file.path, function(err, data) {
              if (!err) {
                var childDoc = SearchUtil.createChildSearchDocument(
                  ContentConstants.search.MAPPING_CONTENT_BODY,
                  resource.id,
  ...
  _getContentItems(resources, function(err, contentItems) {
    ...
    _getRevisionItems(contentItems, function(err, revisionsById) {
      ...
      _.each(contentItems, function(contentItem) {
        docs.push(
          _produceContentSearchDocument(
            contentItem,
            revisionsById[contentItem.latestRevisionId],
          ),
        );
      });
...
var _getContentItems = function(resources, callback) {
  ...
  ContentDAO.Content.getMultipleContentItems(contentIdsToFetch, null, function(
    err,
    extraContentItems,
  ) {
    if (err) {
...
SearchAPI.registerSearchDocumentProducer(
  'content',
  _produceContentSearchDocuments,
);
...
SearchAPI.registerReindexAllHandler('content', function(callback) {
  ...
  var _onEach = function(contentRows, done) {
    ...
    log().info(
      'Firing re-indexing task for %s content items.',
      contentResources.length,
    );
    SearchAPI.postIndexTask('content', contentResources, {
      resource: true,
      children: true,
    });
