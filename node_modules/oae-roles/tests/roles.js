...
module.exports.testTenantSeparation = function(test) {
    ...
    api.setRole(principalUuidA, resourceUuid, 'manager', function(err) {
        ...
        api.getRole(principalUuidB, resourceUuid, function(role, err) {
            test.ok(!err);
            test.ok(!role);
            ...
            api.setRole(principalUuidB, resourceUuid, 'viewer', function(err) {
                ...
                api.hasRole(principalUuidA, resourceUuid, 'manager', function(hasRole, err) {
                    test.ok(!err);
                    test.ok(hasRole);

                    // ensure user from context B is a viewer, not manager
                    api.hasRole(principalUuidB, resourceUuid, 'viewer', function(hasRole, err) {
                        test.ok(!err);
                        test.ok(hasRole);
                        test.done();
                    });
...
module.exports.testHasRole = function(test) {
    ...
    api.setRole(principalUuid, resourceUuid, 'manager', function(err) {
        ...
        api.hasRole(principalUuid, resourceUuid, 'manager', function(hasRole, err) {
            test.ok(!err);
            test.ok(hasRole);
...
module.exports.testGetRolesForResourceType = function(test) {
    ...
    loadContentRoles(principalUuid, baseViewerContentId, ResourceTypes.CONTENT, 300, 'viewer', function() {
        loadContentRoles(principalUuid, baseManagerContentId, ResourceTypes.CONTENT, 300, 'manager', function() {
            ...
            // grab the first 250
            api.getRolesForResourceType(principalUuid, ResourceTypes.CONTENT, null, 250, function(entries250_1, err) {
                test.ok(!err);
                test.equal(entries250_1.length, 250);
                
                // get the next 250
                api.getRolesForResourceType(principalUuid, ResourceTypes.CONTENT, entries250_1[249].uuid, 250, function(entries250_2, err) {
                    test.ok(!err);
                    ...
                    api.getRolesForResourceType(principalUuid, ResourceTypes.CONTENT, entries250_2[249].uuid, 250, function(entries100, err) {
                        test.ok(!err);
...
module.exports.testGetAssociationsForPrincipalsAndResourceType = function(test) {
    ...
    loadContentRoles(principalUuid1, baseViewerContentId, ResourceTypes.GROUP, 300, 'viewer', function() {
        loadContentRoles(principalUuid2, baseManagerContentId, ResourceTypes.GROUP, 300, 'manager', function() {
            ...
            api.getAssociationsForPrincipalsAndResourceType([principalUuid1, principalUuid2], ResourceTypes.GROUP, 1000, function(entries, err) {
                test.ok(!err);
                test.equal(Object.keys(entries).length, 600);
                
                // make sure they work individually
                api.getAssociationsForPrincipalsAndResourceType([principalUuid1], ResourceTypes.GROUP, 1000, function(entries, err) {
                    test.ok(!err);
                    test.equal(Object.keys(entries).length, 300);

                    api.getAssociationsForPrincipalsAndResourceType([principalUuid2], ResourceTypes.GROUP, 1000, function(entries, err) {
                        test.ok(!err);
                        test.equal(Object.keys(entries).length, 300);

                        // test per-principal limitations
                        api.getAssociationsForPrincipalsAndResourceType([principalUuid1, principalUuid2], ResourceTypes.GROUP, 100, function(entries, err) {
                            test.ok(!err);
...
function explodeGroupAncestry(tenantId, principalUuids, results, callback) {
    ...
    api.getAssociationsForPrincipalsAndResourceType(principalUuids, ResourceTypes.GROUP, 1000, function(entries, err) {
        if (err) {
            throw err;
        }
