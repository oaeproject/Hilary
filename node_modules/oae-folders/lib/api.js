    ...
    FoldersDAO.getFolder(folderId, function(err, folder) {
        ...
        FoldersAuthz.canManageFolder(ctx, folder, function(err, canManage) {
            ...
            FoldersDAO.updateFolder(folder, updates, function(err, updatedFolder) {
                ...
                // Update the folder libraries of the principals that
                // are a member of this folders
                AuthzAPI.getAuthzMembers(folder.groupId, null, 10000, function(err, memberRoles, nextToken) {
                    ...
                    FoldersFoldersLibrary.update(principalIds, updatedFolder, folder.lastModified, function(err) {
                        if (err) {
                            return callback(err);
                        }
 ...
 * @api private
 */
var _updateFolderContentVisibility = function(ctx, folder, visibility, callback, _start, _failedContent) {
        ...
        ContentDAO.Content.getMultipleContentItems(contentIds, null, function(err, contentItems) {
            ...
            var done = function() {
                FoldersContentLibrary.purge(folder, function(err) {
                    ...
                    _.each(_failedContent, function(content) {
            ...
            var updateBatch = function() {
                ...
                // Only Once all content items in this batch have been updated,
                // do we move on to the next batch
    ...
    FoldersDAO.getFolder(folderId, function(err, folder) {
        ...
        FoldersAuthz.canManageFolder(ctx, folder, function(err, canManage) {
            ...
            _deleteFolder(folder, function(err, memberIds) {
                ...
                // Get all the content that was in this folder so we can either remove the
                // content items or remove the authz link
                FoldersAuthz.getContentInFolder(folder, function(err, contentIds) {
                    if (err) {
                        return callback(err);
                    }
 ...
 * @param  {Function}       callback            Standard callback function
 * @param  {String[]}       _failedContent      The ids of the content items that could not be removed
 * @api private
 */
var _deleteContent = function(ctx, contentIds, callback, _failedContent) {
    _failedContent = _failedContent || [];

    // If there are no more items to delete, we can return to the caller
    ...
    FoldersDAO.getFolder(folderId, function(err, folder) {
        ...
        ContentDAO.Content.getMultipleContentItems(contentIds, null, function(err, contentItems) {
            ...
            FoldersAuthz.canAddItemsToFolder(ctx, folder, contentItems, function(err, canAddItem, illegalContentIds) {
                ...
                addContentItemsToFolderImpl(folder, contentItems.slice(), function(err) {
                    if (err) {
                        return callback(err);
                    }
 ...
 * @param  {Object}         callback.err        An error object, if any
 */
var addContentItemsToFolderImpl = module.exports.addContentItemsToFolderImpl = function(folder, contentItems, callback) {
