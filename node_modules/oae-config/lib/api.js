...
var getConfigFromCassandra = function(tenantId, callback) {
    var globalAdminAlias = 'admin';
    var cqlParams = [globalAdminAlias];
    if (tenantId !== globalAdminAlias) {
...
var initializeConfig = module.exports.initializeConfig = function(_config, callback) {
    callback = callback || function(err) {
        if (err) {
            log().error({'err': err}, 'Error when initializing the config');
        }
    };
    var globalAdminAlias = 'admin'
        ...
        Cassandra.runQuery('SELECT * FROM Config USING CONSISTENCY QUORUM WHERE tenantId IN (?)', [cqlParams], function(err, rows) {
            ...
            getDefaultConfiguration(function(defaultConfig) {
                ...
                for (var r = 0; r < rows.length; r++) {
                    if (rows[r].get('tenantId').value === globalAdminAlias) {
                        cachedConfiguration[globalAdminAlias] = mergeStoredConfigIntoOriginal([rows[r]], defaultConfig);
                        break;
                    }
                }
                ...
                for (var rr = 0; rr < rows.length; rr++) {
                    if (rows[rr].get('tenantId').value !== globalAdminAlias) {
                        cachedConfiguration[rows[rr].get('tenantId').value] = mergeStoredConfigIntoOriginal([rows[rr]], cachedConfiguration[globalAdminAlias]);
                    }
                }
