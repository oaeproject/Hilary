...
describe('Search Util', function() {
  ...
  describe('#filterOr', function() {
    ...
    it('verify one param', function(callback) {
      var filter = SearchUtil.filterOr({
        param: 'Arbitrary object should be returned',
      });
      assert.ok(filter);
      assert.ok(!filter.or);
    ...
    it('verify mixed null params', function(callback) {
      var filter = SearchUtil.filterOr(null, { key: 'value' }, undefined, {
        key: 'value',
      });
      assert.ok(filter);
      assert.ok(filter.or);
  ...
  describe('#filterAnd', function() {
     ...
     */
    it('verify mixed null params', function(callback) {
      var filter = SearchUtil.filterAnd(null, { key: 'value' }, undefined, {
        key: 'value',
      });
      assert.ok(filter);
      assert.ok(filter.and);
  ...
  describe('#filterResources', function() {
    ...
    it('verify creating a resource filter', function(callback) {
      ...
      assert.strictEqual(
        filterUnspecifiedResources.and[0].term._type,
        'resource',
      );
      assert.strictEqual(
        filterUnspecifiedResources.and[1].not.exists.field,
        'deleted',
      );
      ...
      assert.strictEqual(
        filterEmptyResources.and[1].not.exists.field,
        'deleted',
      );
      ...
      assert.strictEqual(
        filterOneResource.and[1].terms.resourceType[0],
        'content',
      );
      assert.strictEqual(filterOneResource.and[2].not.exists.field, 'deleted');

      var filterAllDeletedResources = SearchUtil.filterResources(
        null,
        SearchConstants.deleted.ONLY,
      );
      assert.strictEqual(filterAllDeletedResources.and.length, 2);
      assert.strictEqual(
        filterAllDeletedResources.and[0].term._type,
        'resource',
      );
      assert.strictEqual(
        filterAllDeletedResources.and[1].exists.field,
        'deleted',
      );

      var filterAllDeletedAndExistingResources = SearchUtil.filterResources(
        null,
        SearchConstants.deleted.BOTH,
      );
      assert.strictEqual(
        filterAllDeletedAndExistingResources.term._type,
        'resource',
      );
  ...
  describe('#filterExplicitAccess', function() {
    ...
    it('verify anonymous and global admin user receive no filter for explicit access', function(callback) {
      SearchUtil.filterExplicitAccess(
  ...
  describe('#createHasChildQuery', function() {
    ...
    it('verify creating a has_child query', function(callback) {
      ...
      var filter = SearchUtil.createHasChildQuery(
        'type',
        'childQuery',
        'scoreType',
      );
  ...
  describe('Others', function() {
    ...
    it('verify getQueryParam', function(callback) {
      ...
      assert.equal(
        SearchUtil.getQueryParam(null, null),
        SearchConstants.query.ALL,
      );
    ...
    it('verify getSortParam', function(callback) {
      ...
      assert.equal(
        SearchUtil.getSortParam('not-valid', 'not-valid'),
        SearchConstants.sort.ASC,
      );
      assert.equal(SearchUtil.getSortParam(validType), validType);
      assert.equal(SearchUtil.getSortParam(null, validType), validType);
      assert.equal(
        SearchUtil.getSortParam(null, null),
        SearchConstants.sort.ASC,
      );
      assert.equal(SearchUtil.getSortParam(validType), validType);
    ...
    it('verify getScopeParam', function(callback) {
      ...
      assert.equal(
        SearchUtil.getScopeParam('invalid'),
        SearchConstants.general.SCOPE_ALL,
      );
      assert.equal(
      ...
      assert.equal(
        SearchUtil.getScopeParam('invalid', tenantAlias),
        tenantAlias,
      );
