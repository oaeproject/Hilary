...
var createAccessToken = module.exports.createAccessToken = function(token, userId, clientId, callback) {
    ...
    var queries = [
        ...
        Cassandra.constructUpsertCQL('OAuthAccessTokenByUser', ['userId', 'clientId'], [userId, clientId], {'token': token})
    ];
...
var getAccessToken = module.exports.getAccessToken = function(token, callback) {
    ...
    Cassandra.runQuery('SELECT * FROM "OAuthAccessToken" WHERE "token" = ?', [token], function(err, rows) {
        if (err) {
            return callback(err);
...
var getAccessTokenForUserAndClient = module.exports.getAccessTokenForUserAndClient = function(userId, clientId, callback) {
    // TODO: As this gets called on every OAuth authenticated call, it might not be a bad idea to cache this in Redis
    Cassandra.runQuery('SELECT "token" FROM "OAuthAccessTokenByUser" WHERE "userId" = ? AND "clientId" = ?', [userId, clientId], function(err, rows) {
        if (err) {
            return callback(err);
        } else if (_.isEmpty(rows)) {
            return callback(null, null);
        ...
        var accessToken = new AccessToken(rows[0].token, userId, clientId);
        return callback(null, accessToken);
    });
};
