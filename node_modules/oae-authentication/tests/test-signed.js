...
describe('Authentication', function() {
    ...
    var anonymousCamRestContxt = null;
    ...
    describe('Signed Authentication', function() {
         ...
         */
        var _performSignedAuthenticationRequest = function(requestInfoUrl, body, isLoggedIn, callback) {
            var parsedUrl = url.parse(requestInfoUrl);
            var restCtx = new RestContext('http://' + global.oaeTests.tenants.localhost.host, {'hostHeader': parsedUrl.host});
        ...
        describe('Get Signed Tenant Authentication Request', function() {
            ...
            it('verify only global administrators can request a signed tenant authentication', function(callback) {
                ...
                TestsUtil.generateTestUsers(camAdminRestContext, 1, function(err, users, mrvisser) {
                    ...
                    RestAPI.Admin.getSignedTenantAuthenticationRequestInfo(anonymousGlobalRestContext, 'localhost', function(err, requestInfo) {
                        assert.ok(err);
                        assert.equal(err.code, 401);
                        assert.ok(!requestInfo);

                        // Verify a tenant admin cannot request signed authentication to another tenant
                        RestAPI.Admin.getSignedTenantAuthenticationRequestInfo(camAdminRestContext, 'localhost', function(err, requestInfo) {
                            assert.ok(err);
                            ...
                            RestAPI.Admin.getSignedTenantAuthenticationRequestInfo(mrvisser.restContext, 'localhost', function(err, requestInfo) {
                                assert.ok(err);
                                ...
                                RestAPI.Admin.getSignedTenantAuthenticationRequestInfo(globalAdminRestContext, 'localhost', function(err, requestInfo) {
                                    assert.ok(!err);
                                    assert.ok(requestInfo);
                                    return callback();
                                });
            ...
            it('verify parameter validation', function(callback) {
                ...
                RestAPI.Admin.getSignedTenantAuthenticationRequestInfo(globalAdminRestContext, null, function(err, requestInfo) {
                    assert.ok(err);
                    assert.equal(err.code, 400);
                    assert.ok(!requestInfo);

                    // Ensure we cannot get a signed request with a non-existing tenant alias
                    RestAPI.Admin.getSignedTenantAuthenticationRequestInfo(globalAdminRestContext, 'some non existing tenant alias', function(err, requestInfo) {
                        assert.ok(err);
                        assert.equal(err.code, 404);
                        assert.ok(!requestInfo);
        ...
        describe('Get Signed Become User Authentication Request', function() {
            ...
            it('verify users can only become users from tenants on which they are administrators', function(callback) {
                ...
                TestsUtil.generateTestUsers(camAdminRestContext, 2, function(err, users, mrvisser, nico) {
                    assert.ok(!err);

                    // Verify anonymous global user cannot request signed authentication for mrvisser
                    RestAPI.Admin.getSignedBecomeUserAuthenticationRequestInfo(anonymousGlobalRestContext, mrvisser.user.id, function(err, requestInfo) {
                        assert.ok(err);
                        assert.equal(err.code, 401);
                        assert.ok(!requestInfo);

                        // Verify anonymous user on the cam tenant cannot request signed authentication for mrvisser
                        RestAPI.Admin.getSignedBecomeUserAuthenticationRequestInfo(anonymousCamRestContext, mrvisser.user.id, function(err, requestInfo) {
                            assert.ok(err);
                            assert.equal(err.code, 401);
                            assert.ok(!requestInfo);

                            // Verify regular tenant user cannot request signed authentication for mrvisser
                            RestAPI.Admin.getSignedBecomeUserAuthenticationRequestInfo(nico.restContext, mrvisser.user.id, function(err, requestInfo) {
                                assert.ok(err);
                                assert.equal(err.code, 401);
                                assert.ok(!requestInfo);

                                // Verify tenant administrator from another tenant cannot request signed authentication for mrvisser
                                RestAPI.Admin.getSignedBecomeUserAuthenticationRequestInfo(gtAdminRestContext, mrvisser.user.id, function(err, requestInfo) {
                                    assert.ok(err);
                                    assert.equal(err.code, 401);
                                    assert.ok(!requestInfo);

                                    // Make nico a tenant administrator for the cam tenant
                                    RestAPI.User.setTenantAdmin(camAdminRestContext, nico.user.id, true, function(err) {
                                        assert.ok(!err);

                                        // Verify a tenant administrator cannot impersonate another tenant administrator
                                        RestAPI.Admin.getSignedBecomeUserAuthenticationRequestInfo(camAdminRestContext, nico.user.id, function(err, requestInfo) {
                                            assert.ok(err);
                                            assert.equal(err.code, 401);
                                            assert.ok(!requestInfo);

                                            // Verify a global administrator can impersonate a tenant administrator
                                            RestAPI.Admin.getSignedBecomeUserAuthenticationRequestInfo(globalAdminRestContext, nico.user.id, function(err, requestInfo) {
                                                assert.ok(!err);
                                                assert.ok(requestInfo);
                                                assert.equal(requestInfo.body.becomeUserId, nico.user.id);

                                                // Verify a global administrator can impersonate a regular user
                                                RestAPI.Admin.getSignedBecomeUserAuthenticationRequestInfo(globalAdminRestContext, mrvisser.user.id, function(err, requestInfo) {
                                                    assert.ok(!err);
                                                    assert.ok(requestInfo);
                                                    assert.equal(requestInfo.body.becomeUserId, mrvisser.user.id);
            ...
            it('verify parameter validation', function(callback) {
                ...
                TestsUtil.generateTestUsers(camAdminRestContext, 1, function(err, users, mrvisser) {
                    ...
                    RestAPI.Admin.getSignedBecomeUserAuthenticationRequestInfo(globalAdminRestContext, null, function(err, requestInfo) {
                        assert.ok(err);
                        assert.equal(err.code, 400);
                        assert.ok(!requestInfo);

                        // Ensure `becomeUserId` must be a valid user id
                        RestAPI.Admin.getSignedBecomeUserAuthenticationRequestInfo(globalAdminRestContext, 'invalid user id', function(err, requestInfo) {
                            assert.ok(err);
                            assert.equal(err.code, 400);
                            assert.ok(!requestInfo);
                            RestAPI.Admin.getSignedBecomeUserAuthenticationRequestInfo(globalAdminRestContext, 'g:cam:notauserid', function(err, requestInfo) {
                                assert.ok(err);
                                assert.equal(err.code, 400);
                                assert.ok(!requestInfo);

                                // Ensure `becomeUserId` must be a user id of a user that exists
                                RestAPI.Admin.getSignedBecomeUserAuthenticationRequestInfo(globalAdminRestContext, 'u:cam:doesnotexist', function(err, requestInfo) {
                                    assert.ok(err);
                                    assert.equal(err.code, 404);
                                    assert.ok(!requestInfo);
            ...
            it('verify imposter cannot get another signed become user request', function(callback) {
                ...
                TestsUtil.generateTestUsers(camAdminRestContext, 2, function(err, users, mrvisser, simon) {
                    ...
                    RestAPI.User.setTenantAdmin(camAdminRestContext, simon.user.id, true, function(err) {
                        ...
                        RestAPI.Admin.getSignedBecomeUserAuthenticationRequestInfo(simon.restContext, mrvisser.user.id, function(err, requestInfo) {
                            assert.ok(!err);
                            assert.ok(requestInfo);

                            // Imposter simon as a the global admin
                            RestAPI.Admin.loginAsUser(globalAdminRestContext, simon.user.id, 'http://' + global.oaeTests.tenants.localhost.host, function(err, globalAdminImposteringSimonRestContext) {
                                assert.ok(!err);

                                // Ensure it is indeed an impostering context
                                RestAPI.User.getMe(globalAdminImposteringSimonRestContext, function(err, me) {
                                    assert.ok(!err);
                                    assert.ok(me);
                                    ...
                                    RestAPI.Admin.getSignedBecomeUserAuthenticationRequestInfo(globalAdminImposteringSimonRestContext, mrvisser.user.id, function(err, requestInfo) {
                                        assert.ok(err);
                                        assert.equal(err.code, 401);
                                        assert.ok(!requestInfo);
                                        return callback();
        ...
        describe('Signed Tenant Login', function() {
            ...
            it('verify login on tenant works', function(callback) {
                RestAPI.Admin.loginOnTenant(globalAdminRestContext, 'localhost', null, function(err, restCtx) {
                    assert.ok(!err);
            ...
            it('verify parameter validation', function(callback) {
                RestAPI.Admin.getSignedTenantAuthenticationRequestInfo(globalAdminRestContext, 'localhost', function(err, requestInfo) {
                    ...
                    _performSignedAuthenticationRequest(requestInfo.url, requestInfo.body, true, function() {

                        // Permutations of missing parameters
                        _performSignedAuthenticationRequest(requestInfo.url, _.omit(requestInfo.body, 'userId'), false, function() {
                            _performSignedAuthenticationRequest(requestInfo.url, _.omit(requestInfo.body, 'expires'), false, function() {
                                _performSignedAuthenticationRequest(requestInfo.url, _.omit(requestInfo.body, 'signature'), false, function() {

                                    // All parameters are present, but some are invalid
                                    _performSignedAuthenticationRequest(requestInfo.url, _.extend({}, requestInfo.body, {'userId': 'u:admin:badid'}), false, function() {
                                        _performSignedAuthenticationRequest(requestInfo.url, _.extend({}, requestInfo.body, {'expires': 1234567890}), false, function() {
                                            return _performSignedAuthenticationRequest(requestInfo.url, _.extend({}, requestInfo.body, {'signature': 'bad signature'}), false, callback);
                                        });
                                    });
                                });
                            });
        ...
        describe('Signed Become User Login', function() {
            ...
            it('verify login as user creates an impersonating request context', function(callback) {
                ...
                TestsUtil.generateTestUsers(camAdminRestContext, 1, function(err, users, mrvisser) {
                    ...
                    RestAPI.Admin.loginAsUser(camAdminRestContext, mrvisser.user.id, util.format('http://%s', global.oaeTests.tenants.localhost.host), function(err, impersonatingMrvisserRestCtx) {
                        assert.ok(!err);

                        // Ensure we are authenticated as mrvisser and impostering
                        RestAPI.User.getMe(impersonatingMrvisserRestCtx, function(err, me) {
                            assert.ok(!err);
                            assert.ok(me);
            ...
            it('verify parameter validation', function(callback) {
                ...
                TestsUtil.generateTestUsers(camAdminRestContext, 2, function(err, users, mrvisser, simon) {
                    ...
                    RestAPI.Admin.getSignedBecomeUserAuthenticationRequestInfo(camAdminRestContext, mrvisser.user.id, function(err, requestInfo) {
                        assert.ok(!err);
                        ...
                        _performSignedAuthenticationRequest(requestInfo.url, requestInfo.body, true, function() {
                            ...
                            _performSignedAuthenticationRequest(requestInfo.url, _.omit(requestInfo.body, 'userId'), false, function() {
                                _performSignedAuthenticationRequest(requestInfo.url, _.omit(requestInfo.body, 'expires'), false, function() {
                                    _performSignedAuthenticationRequest(requestInfo.url, _.omit(requestInfo.body, 'signature'), false, function() {
                                        _performSignedAuthenticationRequest(requestInfo.url, _.omit(requestInfo.body, 'becomeUserId'), false, function() {

                                            // Permutations of tampered data
                                            _performSignedAuthenticationRequest(requestInfo.url, _.extend(requestInfo.body, {'userId': simon.user.id}), false, function() {
                                                _performSignedAuthenticationRequest(requestInfo.url, _.extend(requestInfo.body, {'expires': 1234567890}), false, function() {
                                                    _performSignedAuthenticationRequest(requestInfo.url, _.extend(requestInfo.body, {'signature': 'different'}), false, function() {
                                                        return _performSignedAuthenticationRequest(requestInfo.url, _.extend(requestInfo.body, {'becomeUserId': simon.user.id}), false, callback);
                                                    });
                                                });
                                            });
                                        });
