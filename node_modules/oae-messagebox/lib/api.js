 ...
 */
var MessageBoxAPI = (module.exports = new EmitterAPI.EventEmitter());
 ...
 * @api private
 */
var replaceLinks = (module.exports.replaceLinks = function(body) {
  body = body || '';

  // Replace any matched URLs with relative links in markdown format
  return body.replace(REGEXP_LINK, function(
    fullMatch,
    preURLChar,
    host,
    path,
...
var createMessage = (module.exports.createMessage = function(
  ...
  if (opts.replyToCreated) {
    validator
      .check(opts.replyToCreated, {
        code: 400,
        msg:
          'If the replyToCreated optional parameter is specified, it should not be null.',
      })
      .notNull();
    validator
      .check(opts.replyToCreated, {
        code: 400,
        msg:
          'If the replyToCreated optional parameter is specified, it should be an integer.',
      })
      .isInt();
    validator
      .check(opts.replyToCreated, {
        code: 400,
        msg:
          'If the replyToCreated optional parameter is specified, it cannot be in the future.',
      })
      .max(Date.now());
  }
  ...
  var replyToMessageId = replyToCreated
    ? _createMessageId(messageBoxId, replyToCreated)
    : null;
  ...
  _getMessageThreadKey(replyToMessageId, function(err, replyToThreadKey) {
    ...
    _lockUniqueTimestamp(id, Date.now(), function(created, lockKey, lockToken) {
      ...
      var createMessageQuery = Cassandra.constructUpsertCQL(
        'Messages',
        'id',
        messageId,
        messageStorageHash,
      ...
      Cassandra.runQuery(
        ...
        function(err) {
          ...
          Cassandra.runQuery(
            indexMessageQuery.query,
            indexMessageQuery.parameters,
            function(err) {
              if (err) {
                return callback(err);
              }
              ...
              Cassandra.runQuery(
                recentContributionsQuery.query,
                recentContributionsQuery.parameters,
              );

              // Get the expanded Message object, emit it so it can be indexed, and return it to the caller.
              var message = _storageHashToMessage(
                messageId,
                messageStorageHash,
              );
              MessageBoxAPI.emit(
                MessageBoxConstants.events.CREATED_MESSAGE,
                message,
              );
 ...
 * @param  {Object}         callback.err    An error that occurred, if any
 */
var updateMessageBody = (module.exports.updateMessageBody = function(
  messageBoxId,
  created,
  newBody,
  callback,
  ...
  validator
    .check(created, {
      code: 400,
      msg:
        'The created parameter must be a valid timestamp (integer) that is not in the future.',
    })
    .max(Date.now());
  validator
    .check(newBody, {
  ...
  Cassandra.runQuery(
    ...
    function(err) {
      ...
      MessageBoxAPI.emit(
        MessageBoxConstants.events.UPDATED_MESSAGE,
        messageId,
        newBody,
      );
      callback();
 ...
 */
var getMessagesFromMessageBox = (module.exports.getMessagesFromMessageBox = function(
  messageBoxId,
  start,
  limit,
  opts,
  ...
  _getThreadKeysFromMessageBox(messageBoxId, start, limit, function(
    err,
    threadKeys,
    nextToken,
  ) {
...
var getMessages = (module.exports.getMessages = function(
  ...
  _.each(createdTimestamps, function(timestamp) {
    validator
      .check(timestamp, { code: 400, msg: 'A timestamp cannot be null.' })
      .notNull();
    validator
      .check(timestamp, { code: 400, msg: 'A timestamp should be an integer.' })
      .isInt();
    validator
      .check(timestamp, {
        code: 400,
 ...
 */
var getMessagesById = (module.exports.getMessagesById = function(
  messageIds,
  opts,
  callback,
) {
  ...
  if (opts.deleteType) {
    ...
    validator
      .check(opts.deleteType, {
        code: 400,
        msg:
          'If the deleteType is specified it should be one of: ' +
          deleteValues.join(', '),
      })
      .isIn(deleteValues);
  }
  ...
  getMessages(
    ...
    function(err, messages) {
      ...
      if (message) {
        ...
        return callback({
          code: 404,
          msg: 'The specified message did not exist',
        });
      }
    },
  );
 ...
 */
var getRecentContributions = (module.exports.getRecentContributions = function(
  messageBoxId,
  start,
  limit,
  callback,
...
var _leafDelete = function(message, callback) {
  ...
  _getThreadKeysFromMessageBox(
    message.messageBoxId,
    message.threadKey,
    1,
    function(err, threadKeys) {
      if (err) {
        return callback(err);
      }
 ...
 * @api private
 */
var _getThreadKeysFromMessageBox = function(
  messageBoxId,
  start,
  limit,
  callback,
