 ...
 */
var localUsernameExists = (module.exports.localUsernameExists = function(
  ctx,
  tenantAlias,
  username,
  callback,
  ...
  validator
    .check(username, { code: 400, msg: 'Please specify a username' })
    .notEmpty();
 ...
 */
var getOrCreateGlobalAdminUser = (module.exports.getOrCreateGlobalAdminUser = function(
  ctx,
  username,
  password,
  displayName,
  ...
  validator
    .check(null, {
      code: 401,
      msg:
        'You must be a global administrator to create a global administrator user',
    })
    .isGlobalAdministratorUser(ctx);
  validator
    .check(username, { code: 400, msg: 'You must provide a username' })
    .notEmpty();
  validator
    .check(password, { code: 400, msg: 'You must provide a password' })
    .notEmpty();
  validator
    .check(displayName, { code: 400, msg: 'You must provide a display name' })
    .notEmpty();
  validator
    .check(displayName, {
      code: 400,
 ...
 */
var getOrCreateUser = (module.exports.getOrCreateUser = function(
  ctx,
  authProvider,
  externalId,
  providerProperties,
  ...
  _getUserIdFromLoginId(loginId, function(err, userId) {
    if (err && err.code !== 404) {
      ...
      if (opts.authoritative && opts.email) {
        var validator = new Validator();
        validator
          .check(opts.email, { code: 400, msg: 'Invalid email' })
          .isEmail();
        if (validator.hasErrors()) {
          delete opts.email;
        }
      ...
      OaeUtil.invokeIfNecessary(
        ...
        function(err, email) {
          ...
          var shouldCheckEmail =
            !_.isEmpty(ctx.tenant().emailDomains) &&
            !isAdmin &&
            !opts.authoritative;
      ...
      PrincipalsDAO.getPrincipal(userId, function(err, user) {
        if (err) {
          ...
          return callback({
            code: 401,
            msg: 'Provided login id belongs to a deleted user',
          });
        }
...
var _validateEmailBelongsToTenant = function(ctx, email, callback) {
  ...
  if (!email) {
    return callback({
      code: 400,
      msg:
        'An email address was not provided when a tenant email domain has been configured',
  ...
  if (ctx.tenant().alias !== tenant.alias) {
    return callback({
      code: 400,
      msg:
        'An email address was provided that does not match the configured email domain',
 ...
 */
var createTenantAdminUser = (module.exports.createTenantAdminUser = function(
  ctx,
  loginId,
  displayName,
  opts,
  ...
  var validator = new Validator();
  validator
    .check(null, { code: 400, msg: 'A LoginId must be provided' })
    .isObject(loginId);
  validator
    .check(displayName, { code: 400, msg: 'You must provide a display name' })
    .notEmpty();
  ...
  if (!targetTenant) {
    ...
    return callback({
      code: 400,
      msg:
        'A tenant administrator cannot be created on the global admin tenant',
    });
  } else if (!ctx.user() || !ctx.user().isAdmin(targetTenant.alias)) {
    return callback({
      code: 401,
      msg: 'Only administrators can create new tenant administrators',
    });
  }
...
var createUser = (module.exports.createUser = function(
  ...
  if (targetTenant.isGlobalAdminServer && !isGlobalAdmin) {
    ...
    return callback({
      code: 401,
      msg:
        'Only global administrators may create a user on the global admin tenant',
    });
    ...
    return callback({
      code: 401,
      msg:
        'Only global administrators may create users on a tenant that is not the current',
    });
  }
    ...
    _getUserIdFromLoginId(loginId, function(err, userId) {
      if (err && err.code !== 404) {
        ...
        return callback({
          code: 400,
          msg:
            'This login id already exists and is already associated to a user',
        });
      }

      // Hash the user password
      if (loginId.provider === AuthenticationConstants.providers.LOCAL) {
        loginId.properties.password = AuthenticationUtil.hashPassword(
          loginId.properties.password,
        );
      }

      // Create the user and immediately associate the login id
      PrincipalsAPI.createUser(
        ctx,
        loginId.tenantAlias,
        displayName,
        opts,
 ...
 * @param  {Object}    callback.err    An error that occurred, if any
 */
var associateLoginId = (module.exports.associateLoginId = function(
  ctx,
  loginId,
  userId,
  callback,
  ...
  validator
    ...
    .isLoggedInUser(ctx);
  validator
    .check(userId, { code: 400, msg: 'You must specify a user id' })
    .notEmpty();
  if (validator.hasErrors()) {
    return callback(validator.getFirstError());
  }
  ...
  _getUserIdFromLoginId(loginId, function(err, existingUserIdMapping) {
    if (err && err.code !== 404) {
      ...
      return callback({
        code: 401,
        msg: 'Login ID is already associated to a user',
      });
    }
 ...
 * @param  {Object}     callback.err      An error that occurred, if any
 */
var changePassword = (module.exports.changePassword = function(
  ctx,
  userId,
  oldPassword,
  newPassword,
  ...
  validator
    ...
    .isLoggedInUser(ctx);
  validator
    .check(userId, { code: 400, msg: 'A user id must be provided' })
    .isUserId();
  validator
    .check(newPassword, { code: 400, msg: 'A new password must be provided' })
    .notEmpty();
  if (validator.hasErrors()) {
    return callback(validator.getFirstError());
  }
  ...
  PrincipalsAPI.getUser(ctx, userId, function(err, user) {
    ...
    _getUserLoginIds(userId, function(err, loginIds) {
      ...
      if (!localLoginId) {
        return callback({
          code: 400,
          msg: 'User does not have a local account mapping',
        });
      }
      ...
      if (!isAdmin && !isTargetUser) {
        ...
        return callback({
          code: 401,
          msg: "You're not authorized to change this user's password",
        });
      } else if (isAdmin) {
        // If the user is admin we don't care about the old password
        log().info(
          'User %s is changing the password for user %s',
          ctx.user().id,
          userId,
        );
      ...
      checkPassword(
        localLoginId.tenantAlias,
        localLoginId.externalId,
        oldPassword,
        function(err, user) {
          if (err) {
 ...
 */
var checkPassword = (module.exports.checkPassword = function(
  tenantAlias,
  username,
  password,
  callback,
  ...
  var validator = new Validator();
  validator
    .check(tenantAlias, { code: 401, msg: 'A tenant must be provided' })
    .notEmpty();
  validator
    .check(username, { code: 400, msg: 'A username must be provided' })
    .notEmpty();
  validator
    .check(password, { code: 400, msg: 'A password must be provided' })
    .notEmpty();
  if (validator.hasErrors()) {
    return callback(validator.getFirstError());
  }
  ...
  Cassandra.runQuery(
    ...
    function(err, rows) {
      if (err) {
        ...
        return callback({
          code: 401,
          msg: 'No password found for this principal',
        });
      }
 ...
 */
var getUserIdFromLoginId = (module.exports.getUserIdFromLoginId = function(
  tenantAlias,
  provider,
  externalId,
  callback,
 ...
 */
var getResetPasswordSecret = (module.exports.getResetPasswordSecret = function(
  ctx,
  username,
  callback,
) {
  ...
  _getUserIdFromLoginId(loginId, function(err, userId) {
    ...
    PrincipalsDAO.getPrincipal(userId, function(err, user) {
      if (err) {
        ...
        log().warn(
          { userId: userId },
          'Used asked for password reset but has no email address',
        );
        return callback({
          code: 400,
          msg:
            'This user has no email address, you will have to ask an admin to reset the password',
        });
      }
 ...
 * @param  {Object}     callback.err   An error that occurred, if any
 */
var resetPassword = (module.exports.resetPassword = function(
  ctx,
  username,
  secret,
  newPassword,
  ...
  validator
    ...
    .notEmpty();
  validator
    .check(secret, { code: 400, msg: 'A secret must be provided' })
    .notEmpty();
  validator
    .check(newPassword, { code: 400, msg: 'A new password must be provided' })
    .notEmpty();
  ...
  Cassandra.runQuery(
    ...
    function(err, rows) {
      ...
      if (!dbSecret || dbSecret !== secret) {
        log().warn(
          { loginId: loginId },
          'Someone tried to use an expired password reset secret',
        );
        return callback({
          code: 401,
          msg: 'The secret was not found or incorrect',
        });
...
var _associateLoginId = function(loginId, userId, callback) {
  ...
  if (query) {
    ...
    return callback({
      code: 500,
      msg: 'Error associating login id to user ' + userId,
    });
  }
};
 ...
 */
var getUserLoginIds = (module.exports.getUserLoginIds = function(
  ctx,
  userId,
  callback,
) {
 ...
 */
var registerStrategy = (module.exports.registerStrategy = function(
  strategyName,
  strategy,
) {
...
var refreshStrategies = (module.exports.refreshStrategies = function(tenant) {
  _.each(strategies, function(strategy, strategyName) {
    ...
    var passportStrategyName = AuthenticationUtil.getStrategyId(
      tenant,
      strategyName,
    );
      ...
      log().debug(
        {
          ...
          passportStrategyName: passportStrategyName,
        },
        'Enabling strategy',
      );
    }
...
var logout = (module.exports.logout = function(req, res) {
  if (!req.ctx.user()) {
    return res
      .status(400)
      .send('You need to be logged in, in order to log out');
  }
  ...
  AuthenticationAPI.emit(
    AuthenticationConstants.events.USER_LOGGED_OUT,
    req.ctx,
  );
  ...
  var passportStrategyName = AuthenticationUtil.parseStrategyId(
    authInfo.strategyId,
  ).strategyName;
