...
var getRevisions = module.exports.getRevisions = function(contentId, start, limit, opts, callback) {
    ...
    Cassandra.runPagedQuery('RevisionByContent', 'contentId', contentId, 'created', start, limit, {'reversed': true}, function(err, rows, nextToken) {
        if (err) {
            return callback(err);
        ...
        var revisionsToRetrieve = _.pluck(rows, 'revisionId');
...
var getMultipleRevisions = module.exports.getMultipleRevisions = function(revisionIds, opts, callback) {
    ...
    Cassandra.runInQuery(util.format('SELECT %s FROM "Revisions"', columns), null, 'revisionId', revisionIds, function(err, rows) {
        if (err) {
            return callback(err);
        }
...
var getAllRevisionsForContent = module.exports.getAllRevisionsForContent = function(contentIds, callback) {
    Cassandra.runInQuery('SELECT "contentId", "revisionId" FROM "RevisionByContent"', null, 'contentId', contentIds, function(err, rows) {
        if (err) {
            return callback(err);
        } else if (_.isEmpty(rows)) {
            return callback(null, {});
...
var createRevision = module.exports.createRevision = function(revisionId, contentId, createdBy, revisionProperties, callback) {
    ...
    var values = _.omit(revisionProperties, 'revisionId', 'downloadPath');
    ...
    var oldRevisionId = revisionProperties.revisionId;
    ...
    _copyPreviewItemsIfNecessary(revisionProperties.revisionId, revisionId, function(err) {
        ...
        var q = Cassandra.constructUpsertCQL('Revisions', 'revisionId', revisionId, values);
        Cassandra.runQuery(q.query, q.parameters, function(err) {
            ...
            Cassandra.runQuery('INSERT INTO "RevisionByContent" ("contentId", "created", "revisionId") VALUES (?, ?, ?)', [contentId, values.created, revisionId], function(err) {
                if (err) {
                    return callback(err);
                }
 ...
 *
 * @param  {Row}        row     A Cassandra row object
...
var _rowToRevision = function(row) {
    ...
    return new Revision(row.contentId, row.revisionId, row.createdBy, row.created, row);
};
