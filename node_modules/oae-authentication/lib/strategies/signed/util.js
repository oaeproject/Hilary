 ...
 */
var getSignedTenantAuthenticationRequest = (module.exports.getSignedTenantAuthenticationRequest = function(
  ctx,
  tenantAlias,
  callback,
) {
  ...
  validator
    .check(null, {
      code: 401,
      msg:
        'Only global administrators are allowed to authenticate to other tenants',
    })
    .isGlobalAdministratorUser(ctx);
  validator
    .check(tenantAlias, { code: 400, msg: 'Missing target tenant alias' })
    .notEmpty();
  ...
  var signedData = Signature.createExpiringSignature(
    data,
 ...
 * @param  {Object}     callback.requestInfo.body   The signed body of the POST request to send in order to verify the authenticity of the authentication request
 */
var getSignedBecomeUserAuthenticationRequest = (module.exports.getSignedBecomeUserAuthenticationRequest = function(
  ctx,
  becomeUserId,
  callback,
) {
  ...
  if (validator.hasErrors()) {
    ...
    return callback({
      code: 401,
      msg: 'Only administrators can become a user',
    });
  } else if (ctx.imposter()) {
    ...
    return callback({
      code: 401,
      msg: 'You cannot become a user while impostering another user',
    });
  }
  ...
  PrincipalsDAO.getPrincipal(becomeUserId, function(err, becomeUser) {
    if (err) {
      ...
      return callback({
        code: 401,
        msg: 'You are not authorized to become this user',
      });
    } else if (
      becomeUser.isAdmin(becomeUser.tenant.alias) &&
 ...
 */
var verifySignedAuthenticationBody = (module.exports.verifySignedAuthenticationBody = function(
  ctx,
  body,
  callback,
) {
