...
var _updateRoles = function(resourceId, changes, callback) {
    ...
    getAuthzMembersGraph(_.keys(groupsToInvalidate), function(err, graph) {
        ...
        // Extract just the user ids from the nodes and apply them to the hash of users we wish to
        // invalidate. We will be invalidating them all from the authz memberships cache
...
/**
 * Create a group memberships graph for all the provided principals. The resulting graph is
 * guaranteed to contain all principal ids, even if some or all of them don't have any memberships.
 * The result is a graph that is possibly several disjoint graphs, as it's not necessary that all
 * provided principals have a memberships graph that intersect with one another
 *
