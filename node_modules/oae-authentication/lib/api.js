...
var localUsernameExists = module.exports.localUsernameExists = function(ctx, username, callback) {
    // #594 - All usernames for local authentication should be lowercase
 ...
 * @param  {String}       [opts.visibility]  The user's visibility. One of: @see PrincipalsConstants.visibility
...
var getOrCreateUser = module.exports.getOrCreateUser = function(ctx, authProvider, externalId, displayName, opts, callback) {
    // #594 - All usernames for local authentication should be lowercase
    if (authProvider === AuthenticationConstants.providers.LOCAL) {
...
var createUser = module.exports.createUser = function(ctx, loginId, displayName, opts, callback) {
    var validator = new Validator();
    validator.check(displayName, {'code': 400, 'msg': 'Please specify a display name.'}).isShortString();
    ...
    if (loginId.provider === AuthenticationConstants.providers.LOCAL) {
        loginId.externalId = loginId.externalId.toLowerCase();
    }
...
var getUserIdFromLoginId = module.exports.getUserIdFromLoginId = function(tenantAlias, provider, externalId, callback) {
    // #594 - All usernames for local authentication should be lowercase
    if (provider === AuthenticationConstants.providers.LOCAL) {
        externalId = externalId.toLowerCase();
    }
...
var refreshStrategies = module.exports.refreshStrategies = function(tenant) {
    _.each(strategies, function(strategy, strategyName) {
        // Get the name we used to register this strategy with passport.
        // This is a combination of the tenant and strategy name.
        ...
        // Only re-enable it, if the tenant wants it enabled.
        // We also get a new instance of the passport strategy so any updates to the
        // configuration are taken into account.
...
var _refreshAllTenantStrategies = function() {
    TenantsAPI.getAllTenants(function(err, tenants) {
        if (err) {
            return log().error({'err': err}, 'Error fetching tenants to enable authentication strategies.');
        }
