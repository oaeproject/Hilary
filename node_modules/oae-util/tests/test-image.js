...
describe('Image', function() {
  ...
  describe('#cropImage()', function() {
    ...
    it('verify parameter validation', function(callback) {
      ImageUtil.cropImage(undefined, generateArea(10, 10, 200, 200), function(
        err,
      ) {
        assert.equal(err.code, 400);
        ImageUtil.cropImage(
          'some/path',
          generateArea(undefined, 10, 200, 200),
          function(err) {
            assert.equal(err.code, 400);
            ImageUtil.cropImage(
              'some/path',
              generateArea(10, undefined, 200),
              function(err) {
                assert.equal(err.code, 400);
                ImageUtil.cropImage(
                  'some/path',
                  generateArea(10, 10, undefined, 200),
                  function(err) {
                    assert.equal(err.code, 400);
                    ImageUtil.cropImage(
                      'some/path',
                      generateArea(10, 10, 200, undefined),
                      function(err) {
                        assert.equal(err.code, 400);
                        ...
                        ImageUtil.cropImage(
                          path,
                          generateArea(10000, 10000, 10, 10),
                          function(err, file) {
                            assert.equal(err.code, 400);
    ...
    it('verify error handling', function(callback) {
      ...
      ImageUtil.cropImage('some/path', generateArea(10, 10, 200, 200), function(
        err,
      ) {
  ...
  describe('#resizeImage()', function() {
    ...
    it('verify parameter validation', function(callback) {
      ImageUtil.resizeImage(undefined, generateSize(200, 200), function(err) {
        assert.equal(err.code, 400);
        ImageUtil.resizeImage('some/path', generateSize(-10, 200), function(
          err,
        ) {
          assert.equal(err.code, 400);
          ImageUtil.resizeImage('some/path', generateSize(10, -200), function(
  ...
  describe('#convertToJPG()', function() {
    ...
    it('verify images get properly converted to JPG', function(callback) {
      var path = Path.resolve(
        __dirname + '/../../oae-preview-processor/tests/data/image.gif',
      );
