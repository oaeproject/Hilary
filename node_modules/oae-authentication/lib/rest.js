 ...
 * @Server      Admin
 * @Method      GET
 * @Path        /auth/local/reset/init/{tenantAlias}/{username}
 ...
 */
OAE.globalAdminRouter.on('post', '/api/auth/local/reset/init/:tenantAlias/:username', _getResetPasswordSecret);
...
var _resetPassword = function(req, res){
	AuthenticationAPI.resetPassword(req.ctx, req.params.tenantAlias, req.params.username, req.params.secret, req.body.newPassword, function(err, changed) {
	    if (err) {
	        return callback(err);
	    }
...
/**
 * @REST GlobalResetChange
 *
 * Reset an user's password based on a existing username
 *
 * @Server      admin
 * @Method      POST
 * @Path        /auth/local/reset/change/{tenantAlias}/{username}
 * @PathParam   {string}        username                The username for the user
 ...
 * @HttpResponse                404                     Config value ... does not exist
 */
OAE.globalAdminRouter.on('post', '/api/auth/local/reset/change/:tenantAlias/:username/', _resetPassword);
