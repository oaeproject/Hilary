...
describe('Library Search', function() {
     ...
     */
    it('verify comments are included when searching through libraries', function(callback) {
        TestsUtil.generateTestUsers(camAdminRestContext, 1, function(err, users, simon) {

            // Create content with a comment on it
            RestAPI.Content.createLink(simon.restContext, 'Apereo Website', 'The website of the Apereo Foundation', 'public', 'http://www.apereo.org', [], [], [], function(err, content) {
                assert.ok(!err);
                RestAPI.Content.createComment(simon.restContext, content.id, 'abcdefghi', null, function(err, comment) {
                    assert.ok(!err);
                    ...
                    SearchTestsUtil.searchAll(simon.restContext, 'content-library', [simon.user.id], {'q': 'abcdefghijklmn'}, function(err, results) {
                        assert.ok(!err);
                        assert.ok(_.find(results.results, {'id': content.id}));


                        // Create a discussion with a message on it
                        RestAPI.Discussions.createDiscussion(simon.restContext, 'A talk', 'about the moon', 'public', [], [], function(err, discussion) {
                            assert.ok(!err);
                            RestAPI.Discussions.createMessage(simon.restContext, discussion.id, 'stuvwxyz', null, function(err, message) {
                                assert.ok(!err);
                                ...
                                // a test string or use an md5 hash as those are probably not going to contain
                                // substrings of 5 characters
                                SearchTestsUtil.searchAll(simon.restContext, 'discussion-library', [simon.user.id], {'q': 'stuvwxyz'}, function(err, results) {
                                    assert.ok(!err);
    ...
    describe('User Libraries', function() {
        ...
        it('verify the principal id gets validated', function(callback) {
            SearchTestsUtil.searchAll(camAdminRestContext, 'content-library', [''], null, function(err, results) {
                assert.equal(err.code, 400);
                assert.ok(!results);

                SearchTestsUtil.searchAll(camAdminRestContext, 'content-library', ['invalid-user-id'], null, function(err, results) {
                    assert.equal(err.code, 400);
                    assert.ok(!results);
                    ...
                    RestAPI.User.createUser(camAdminRestContext, jackUsername, 'password', 'Jack McJackerson', null, function(err, jack) {
                        ...
                        RestAPI.User.createUser(camAdminRestContext, janeUsername, 'password', 'Jane McJaneFace', null, function(err, jane) {
                            ...
                            RestAPI.Content.createLink(doerRestContext, 'Apereo Website', 'The website of the Apereo Foundation', 'public', 'http://www.apereofoundation.org', [], [], [], function(err, content) {
                                ...
                                RestAPI.Content.shareContent(doerRestContext, content.id, [jack.id], ContentConstants.roles.VIEWER, function(err) {
                                    assert.ok(!err);
                            ...
                            RestAPI.Group.setGroupMembers(doerRestContext, group.id, membership, function(err) {
                                ...
                                RestAPI.User.createUser(camAdminRestContext, janeUsername, 'password', 'Jane McJaneFace', null, function(err, jane) {
                                    ...
                                    RestAPI.Content.createLink(doerRestContext, 'Apereo Website', 'The website of the Apereo Foundation', 'public', 'http://www.apereofoundation.org', [], [], [], function(err, content) {
                                        ...
                                        RestAPI.Content.shareContent(doerRestContext, content.id, [group.id], ContentConstants.roles.VIEWER, function(err) {
                                            assert.ok(!err);
