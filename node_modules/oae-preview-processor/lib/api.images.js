...
var generatePreviews = module.exports.generatePreviews = function(ctx, callback) {
    ...
    ctx.download(function(err, path) {
        ...
        var sizes = [
            { 'width': 700, 'height': 700, 'name': 'normal' },
            { 'width': 174, 'height': 174, 'name': 'small' }
        ];
...
var _resize = function(ctx, path, size, callback) {
    ...
    // Generate a "normal" and a "small" sized image.
...
var cropThumbnail = module.exports.cropThumbnail = function(ctx, path, callback) {
    ...
    gm(path).size(function (err, size) {
        ...
        if (width === height) {
            ImageUtil.resizeImage(path, { 'width': 150, 'height': 150}, function(err, thumbnailFile) {
                if (err) {
                    return callback(err);
                }
            ...
            // crop it.
            ImageUtil.cropAndResize(path, selectedArea, [150], function(err, files) {
                if (err) {
                    return callback(err);
                }
                ...
                var thumbnailPath = ctx.dirs.base + '/thumbnail.png';
                shell.mv(files['150'].path, thumbnailPath);
                ctx.setThumbnail(thumbnailPath);
                callback(null);
            });
        }
