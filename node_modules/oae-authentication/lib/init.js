...
var contextMiddleware = function(req, res, next) {
    ...
    if (req.user && req.user.user) {
        user = req.user.user;
        imposter = req.user.imposter;
    }

    req.ctx = new Context(req.tenant, user, null, imposter);
    return next();
};
...
var setupPassportSerializers = function() {
    ...
    passport.serializeUser(function(data, done) {
        var toSerialize = {};
        if (data.user) {
            toSerialize.userId = data.user.id;
            if (data.imposter) {
                toSerialize.imposterId = data.imposter.id;
            }
        }

        return done(null, _encryptCookieData(JSON.stringify(toSerialize)));
    });
    ...
    passport.deserializeUser(function(toDeserialize, callback) {
        var deserializedData = _decryptCookieData(toDeserialize);

        // Backward compatibility before "imposter" was implemented. If this doesn't exist,
        // all users will lose their sessions after an upgrade which could be lame.
        //
        // TODO: Is this really necessary?
        if (deserializedData[0] === '{') {
            // If this session was started with the imposter feature installed, the cookie is
            // a serialized JSON object
            deserializedData = JSON.parse(deserializedData);
        } else {
            // If this session was started before the imposter feature was available, the cookie
            // is a plain string that is the id of the user
            deserializedData = {'userId': deserializedData};
        }

        if (!deserializedData.userId) {
            return callback(null, false);
        }

        // Get the effective user of the session
        PrincipalsDAO.getPrincipal(deserializedData.userId, function(err, user) {
            if (err && err.code === 404) {
                ...
                return callback(err);
            } else if (!deserializedData.imposterId) {
            ...
            PrincipalsDAO.getPrincipal(deserializedData.imposterId, function(err, imposterUser) {
                if (err && err.code === 404) {
                    // If the user does not exist, the session is toast
                    return callback(null, false);
                } else if (err) {
                ...
                return callback(null, {'user': user, 'imposter': imposterUser});
            });
        });
    });
};
