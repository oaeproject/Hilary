...
var ActivityCluster = require('oae-activity/lib/internal/cluster');
...
describe('Activity', function() {
    ...
    describe('Activity API', function() {
                    ...
                    RestAPI.User.createUser(camAdminRestContext, jackUsername, 'password', 'Jack McJackerson', null, function(err, jack) {
                        ...
                        RestAPI.Content.createLink(jackCtx, 'Google', 'Google', 'public', 'http://www.google.ca', [], [], function(err, link) {
                            ...
                            ActivityTestsUtil.collectAndGetActivityStream(jackCtx, null, null, function(err, activityStream) {
                                ...
                                setTimeout(ActivityTestsUtil.collectAndGetActivityStream, 1000, jackCtx, null, null, function(err, activityStream) {
                                    assert.ok(!err);
                                    assert.ok(activityStream);
                                    assert.ok(activityStream.items);
                                    assert.equal(activityStream.items.length, 0);
        ...
        describe('#registerActivityEntityRouter()', function() {
            ...
            it('verify default producer persists just the activity seed resource data', function(callback) {
                ...
                ActivityAPI.registerActivityRouter(testResourceType, function(activityType, entityType, persistentEntity, routerCallback) {
                    ...
                    if (hadActor && hadObject && hadTarget) {
                        // Purposely don't call the routerCallback after we're verified, we don't need to continue routing
                        return callback();
                    } else {
        ...
        describe('#registerActivityTransformer()', function() {
            ...
            /**
             * Test that verifies that the default activity transformer will return just the oae:id and objectType of an entity
             */
            it('verify default activity transformer returns objectType and oae:id', function(callback) {
                ...
                RestAPI.User.createUser(camAdminRestContext, jackUsername, 'password', 'Jack McJackerson', null, function(err, jack) {
                    ...
                    RestAPI.Content.createLink(jackCtx, 'Google', 'Google', 'public', 'http://www.google.ca', [], [], function(err, link) {
                        ...
                        var _createActivitySeed = function(seedOverlay, actorOverlay, objectOverlay, targetOverlay) {
                            ...
                            var object = {'resourceType': 'content', 'resourceId': link.contentId};
                        ...
                        // Verify no seed
                        ActivityAPI.postActivity(_createActivitySeed(), function(err) {
                            assert.ok(err);
                            assert.equal(err.code, 400);

                            // Verify no activity type
                            ActivityAPI.postActivity(_createActivitySeed({'activityType': ''}), function(err) {
                                assert.ok(err);
                                assert.equal(err.code, 400);

                                // Verify no verb
                                ActivityAPI.postActivity(_createActivitySeed({'verb': ''}), function(err) {
                                    assert.ok(err);
                                    assert.equal(err.code, 400);

                                    // Verify no publish date
                                    ActivityAPI.postActivity(_createActivitySeed({'published': ''}), function(err) {
                                        assert.ok(err);
                                        assert.equal(err.code, 400);
                                        ...
                                        ActivityAPI.postActivity(_createActivitySeed({}), function(err) {
                                            ...
                                            ActivityAPI.postActivity(_createActivitySeed({}, {'resourceType': ''}), function(err) {
                                                assert.ok(err);
                                                assert.equal(err.code, 400);

                                                // Verify no actor resource id
                                                ActivityAPI.postActivity(_createActivitySeed({}, {'resourceId': ''}), function(err) {
                                                    assert.ok(err);
                                                    assert.equal(err.code, 400);
    ...
    describe('Activity Stream Permissions and Validation', function() {
        ...
        it('verify getActivityStream validation', function(callback) {
            ...
            RestAPI.User.createUser(camAdminRestContext, jackUsername, 'password', 'Jack McJackerson', null, function(err, jack) {
                ...
                // Try anonymous user
                ActivityTestsUtil.collectAndGetActivityStream(anonymousCamRestContext, jack.id, {'_delay': false}, function(err, activityStream) {
                    assert.ok(err);
                    assert.equal(err.code, 401);
                    assert.ok(!activityStream);

                    // Try empty id
                    ActivityTestsUtil.collectAndGetActivityStream(jackCtx, '  ', {'_delay': false}, function(err, activityStream) {
                        assert.ok(err);
                        assert.equal(err.code, 400);
                        assert.ok(!activityStream);

                        // Try invalid principal id
                        ActivityTestsUtil.collectAndGetActivityStream(jackCtx, 'c:cam:someContent', {'_delay': false}, function(err, activityStream) {
                            assert.ok(err);
                            assert.equal(err.code, 400);
                            assert.ok(!activityStream);

                            // Sanity-check valid query
                            ActivityTestsUtil.collectAndGetActivityStream(jackCtx, jack.id, {'_delay': false}, function(err, activityStream) {
                                assert.ok(!err);
                                callback();
                            });
                        });
                ...
                RestAPI.Content.createLink(jackCtx, 'Google', 'Google', 'public', 'http://www.google.ca', [], [], function(err, link) {
                    ...
                    ActivityTestsUtil.collectAndGetActivityStream(jackCtx, null, null, function(err, activityStream) {
                        ...
                        RestAPI.User.createUser(camAdminRestContext, janeUsername, 'password', 'Jane', null, function(err, jane) {
                            ...
                            ActivityTestsUtil.collectAndGetActivityStream(janeCtx, jack.id, {'_delay': false}, function(err, activityStream) {
                                assert.ok(err);
                                assert.equal(err.code, 401);
                                assert.ok(!activityStream);
                                callback();
                    ...
                    RestAPI.Group.createGroup(jackCtx, groupAlias, groupAlias, groupAlias, 'public', 'no', [], [], function(err, group) {
                        ...
                        RestAPI.Content.createLink(jackCtx, 'Google', 'Google', 'public', 'http://www.google.ca', [], [group.id], function(err, link) {
                            ...
                            ActivityTestsUtil.collectAndGetActivityStream(jackCtx, group.id, null, function(err, activityStream) {
                                ...
                                ActivityTestsUtil.collectAndGetActivityStream(janeCtx, group.id, {'_delay': false}, function(err, activityStream) {
                                    assert.ok(err);
                                    assert.equal(err.code, 401);
                                    assert.ok(!activityStream);
                                    callback();
    ...
    describe('Activity Stream Aggregation', function() {
                    ...
                    RestAPI.Content.createLink(jackCtx, 'A', 'A', 'public', 'http://www.google.ca', [], [], function(err, linkA) {
                        ...
                        RestAPI.Content.createLink(jackCtx, 'B', 'B', 'public', 'http://www.google.ca', [], [], function(err, linkB) {
                            ...
                            ActivityTestsUtil.collectAndGetActivityStream(jackCtx, null, null, function(err, activityStream) {
                                ...
                                setTimeout(RestAPI.Content.createLink, 1000, jackCtx, 'C', 'C', 'public', 'http://www.google.ca', [], [], function(err, linkC) {
                                    assert.ok(!err);
            ...
            ActivityAPI.refreshConfiguration(createDefaultConfig({'aggregateIdleExpiry': 5, 'aggregateMaxExpiry': 1}), function(err) {
                ...
                RestAPI.User.createUser(camAdminRestContext, jackUsername, 'password', 'Jack McJackerson', null, function(err, jack) {
                    ...
                    RestAPI.Content.createLink(jackCtx, 'A', 'A', 'public', 'http://www.google.ca', [], [], function(err, linkA) {
                        ...
                        // Drop an aggregate in. The when collected the aggregate is 500ms old
                        setTimeout(RestAPI.Content.createLink, 500, jackCtx, 'B', 'B', 'public', 'http://www.google.ca', [], [], function(err, linkB) {
                            assert.ok(!err);
                            ...
                            ActivityTestsUtil.collectAndGetActivityStream(jackCtx, null, null, function(err, activityStream) {
                                ...
                                setTimeout(RestAPI.Content.createLink, 800, jackCtx, 'C', 'C', 'public', 'http://www.google.ca', [], [], function(err, linkC) {
                                    assert.ok(!err);
        ...
        /**
         * Test that verifies that when the maxAggregateExpiry expires, the aggregate data disappears from Cassandra.
         */
        it('verify aggregated data is automatically deleted after the maxAggregateExpiry time', function(callback) {
            var jackUsername = TestsUtil.generateTestUserId('jack');
                        ...
                        ActivityTestsUtil.collectAndGetActivityStream(jackCtx, null, null, function(err, activityStream) {
                            ...
                            ActivityDAO.getAggregateStatus([aggregateKey], function(err, aggregateStatus) {
                                ...
                                ActivityDAO.getAggregatedEntities([aggregateKey], function(err, aggregatedEntities) {
                                    ...
                                    assert.ok(aggregatedEntities[aggregateKey]['objects']['content:' + link.contentId]);
    ...
    describe('Activity Clustering', function() {

        /**
         * Verifies locking an activity processing bucket stops other nodes from being able to acquire it. Also verifies that releasing
         * the bucket then allows others to acquire the bucket.
         */
        it('verify bucket acquisition and release', function(callback) {
            // Get a lock, make sure it works
            ActivityCluster.acquireBucket(99, 5000, function(err, token) {
                assert.ok(!err);
                assert.ok(token);

                // Try again, make sure we don't get a token for it
                ActivityCluster.acquireBucket(99, 5000, function(err, tokenBad) {
                    assert.ok(!err);
                    assert.ok(!tokenBad);

                    // Release the bucket
                    ActivityCluster.releaseBucket(99, token, function(err, hadLock) {
                        assert.ok(!err);
                        assert.ok(hadLock);

                        // Try again, we should get it this time around
                        ActivityCluster.acquireBucket(99, 5000, function(err, token2) {
                            assert.ok(!err);
                            assert.ok(token2);

                            // Release the bucket again to continue.
                            ActivityCluster.releaseBucket(99, token2, function(err, hadLock) {
                                assert.ok(!err);
                                assert.ok(hadLock);
                                callback();
                            });
        ...
        /**
         * Verifies that locking an activity bucket for longer than the lock expiry results in other nodes from being able to steal the
         * lock.
         */
        it('verify a bucket expires and stealing an expired lock', function(callback) {
            // Get a lock, make sure it works
            ActivityCluster.acquireBucket(99, 1, function(err, token) {
                assert.ok(!err);
                assert.ok(token);

                // Try again, make sure we don't get a token for it
                ActivityCluster.acquireBucket(99, 5, function(err, tokenBad) {
                    assert.ok(!err);
                    assert.ok(!tokenBad);

                    // Wait until it expires then try and steal it
                    setTimeout(ActivityCluster.acquireBucket, 1100, 99, 5, function(err, tokenGood) {
                        assert.ok(!err);
                        assert.ok(tokenGood);
                        ...
                        ActivityCluster.releaseBucket(99, token, function(err, hadLock) {
                            ...
                            // Make sure that the invalid release token failed to release the bucket
                            ActivityCluster.acquireBucket(99, 5, function(err, tokenBad2) {
                                assert.ok(!err);
                                assert.ok(!tokenBad2);

                                // Release it successfully to continue.
                                ActivityCluster.releaseBucket(99, tokenGood, function(err, hadLock) {
                                    assert.ok(!err);
                                    assert.ok(hadLock);
                                    callback();
                                });
        ...
        it('verify releasing expired lock reports that no lock was released', function(callback) {
            ...
            ActivityCluster.acquireBucket(99, 1, function(err, token) {
                ...
                ActivityCluster.acquireBucket(99, 5, function(err, tokenBad) {
                    ...
                    setTimeout(ActivityCluster.releaseBucket, 1100, 99, token, function(err, hadLock) {
                        assert.ok(!err);
                        assert.ok(!hadLock);
                        callback();
                    });
