...
describe('Members Library Search', function() {
    ...
    it('verify cannot search for invalid or non-existent group', function(callback) {
        SearchTestsUtil.searchAll(anonymousRestContext, 'group-members-library', ['not-a-group-id'], null, function(err, results) {
            assert.ok(err);
            assert.equal(err.code, 400);
            assert.ok(!results);

            SearchTestsUtil.searchAll(anonymousRestContext, 'group-members-library', [util.format('g:%s:nonexistent-group-id', global.oaeTests.tenants.cam.alias)], null, function(err, results) {
                assert.ok(err);
            ...
            RestAPI.User.createUser(camAdminRestContext, jackUsername, 'password', 'Jack McJackerson', null, function(err, jack) {
                ...
                RestAPI.Group.setGroupMembers(doerRestContext, targetPublicGroup.id, changes, function(err) {
                    ...
                    RestAPI.User.createUser(camAdminRestContext, janeUsername, 'password', 'Jane McJaneFace', null, function(err, jane) {
                        ...
                        SearchTestsUtil.searchAll(anonymousRestContext, 'group-members-library', [targetPublicGroup.id], null, function(err, results) {
                            assert.ok(!err);
                            ...
                            SearchTestsUtil.searchAll(darthVaderRestContext, 'group-members-library', [targetPublicGroup.id], null, function(err, results) {
                                assert.ok(!err);
                                var publicUserResult = _getDocById(results, publicUserMember.id);
                                var loggedinUserResult = _getDocById(results, loggedinUserMember.id);
                                var privateUserResult = _getDocById(results, privateUserMember.id);
                                ...
                                SearchTestsUtil.searchAll(janeRestContext, 'group-members-library', [targetPublicGroup.id], null, function(err, results) {
                                    assert.ok(!err);
                                    var publicUserResult = _getDocById(results, publicUserMember.id);
                                    var loggedinUserResult = _getDocById(results, loggedinUserMember.id);
                                    var privateUserResult = _getDocById(results, privateUserMember.id);
                                    ...
                                    SearchTestsUtil.searchAll(jackRestContext, 'group-members-library', [targetPublicGroup.id], null, function(err, results) {
                                        assert.ok(!err);
                                        var publicUserResult = _getDocById(results, publicUserMember.id);
                                        var loggedinUserResult = _getDocById(results, loggedinUserMember.id);
                                        var privateUserResult = _getDocById(results, privateUserMember.id);
            ...
            RestAPI.User.createUser(camAdminRestContext, jackUsername, 'password', 'Jack McJackerson', null, function(err, jack) {
                ...
                RestAPI.Group.setGroupMembers(doerRestContext, targetLoggedinGroup.id, changes, function(err) {
                    ...
                    RestAPI.User.createUser(camAdminRestContext, janeUsername, 'password', 'Jane McJaneFace', null, function(err, jane) {
                        ...
                        SearchTestsUtil.searchAll(anonymousRestContext, 'group-members-library', [targetLoggedinGroup.id], null, function(err, results) {
                            assert.ok(err);
                            ...
                            SearchTestsUtil.searchAll(darthVaderRestContext, 'group-members-library', [targetLoggedinGroup.id], null, function(err, results) {
                                assert.ok(err);
                                assert.equal(err.code, 401);
                                assert.ok(!results);

                                // Verify results and visibility for loggedin user
                                SearchTestsUtil.searchAll(janeRestContext, 'group-members-library', [targetLoggedinGroup.id], null, function(err, results) {
                                    assert.ok(!err);
                                    var publicUserResult = _getDocById(results, publicUserMember.id);
                                    var loggedinUserResult = _getDocById(results, loggedinUserMember.id);
                                    var privateUserResult = _getDocById(results, privateUserMember.id);
                                    ...
                                    // Verify results and visibility for member user
                                    SearchTestsUtil.searchAll(jackRestContext, 'group-members-library', [targetLoggedinGroup.id], null, function(err, results) {
                                        assert.ok(!err);
                                        var publicUserResult = _getDocById(results, publicUserMember.id);
                                        var loggedinUserResult = _getDocById(results, loggedinUserMember.id);
                                        var privateUserResult = _getDocById(results, privateUserMember.id);
            ...
            RestAPI.User.createUser(camAdminRestContext, jackUsername, 'password', 'Jack McJackerson', null, function(err, jack) {
                ...
                RestAPI.Group.setGroupMembers(doerRestContext, targetPrivateGroup.id, changes, function(err) {
                    ...
                    RestAPI.User.createUser(camAdminRestContext, janeUsername, 'password', 'Jane McJaneFace', null, function(err, jane) {
                        ...
                        SearchTestsUtil.searchAll(anonymousRestContext, 'group-members-library', [targetPrivateGroup.id], null, function(err, results) {
                            assert.ok(err);
                            assert.equal(err.code, 401);
                            assert.ok(!results);

                            // Verify cross-tenant user cannot see members of private group
                            SearchTestsUtil.searchAll(darthVaderRestContext, 'group-members-library', [targetPrivateGroup.id], null, function(err, results) {
                                assert.ok(err);
                                assert.equal(err.code, 401);
                                assert.ok(!results);

                                // Verify loggedin user cannot see members of private group
                                SearchTestsUtil.searchAll(janeRestContext, 'group-members-library', [targetPrivateGroup.id], null, function(err, results) {
                                    assert.ok(err);
                                    assert.equal(err.code, 401);
                                    assert.ok(!results);

                                    // Verify results and visibility for member user
                                    SearchTestsUtil.searchAll(jackRestContext, 'group-members-library', [targetPrivateGroup.id], null, function(err, results) {
                                        assert.ok(!err);
                                        var publicUserResult = _getDocById(results, publicUserMember.id);
                                        var loggedinUserResult = _getDocById(results, loggedinUserMember.id);
                                        var privateUserResult = _getDocById(results, privateUserMember.id);
        ...
        RestAPI.User.createUser(camAdminRestContext, jackUsername, 'password', 'Jack McJackerson', null, function(err, jack) {
            ...
            RestAPI.User.createUser(camAdminRestContext, janeUsername, 'password', 'Jane McJaneFace', null, function(err, jane) {
                ...
                RestAPI.Group.setGroupMembers(doerRestContext, targetLoggedinGroup.id, changes, function(err) {
                    ...
                    SearchTestsUtil.searchRefreshed(doerRestContext, 'group-members-library', [targetLoggedinGroup.id], {'limit': 2, 'start': 0}, function(err, results) {
                        assert.ok(!err);
                        ...
                        RestAPI.Search.search(doerRestContext, 'group-members-library', [targetLoggedinGroup.id], {'limit': 1, 'start': 0}, function(err, results) {
                            assert.ok(!err);
                            assert.ok(results.results);
                            ...
                            RestAPI.Search.search(doerRestContext, 'group-members-library', [targetLoggedinGroup.id], {'limit': 1, 'start': 1}, function(err, results) {
                                assert.ok(!err);
                                assert.ok(results.results);
                                assert.equal(results.results.length, 1);
