 ...
 */
var storePicture = (module.exports.storePicture = function(
  ctx,
  principalId,
  file,
  callback,
  ...
  callback =
    ...
    function(err) {
      if (err) {
        log().error(
          { err: err },
          'Unable to store picture %s for %s',
          file.path,
          principalId,
        );
      }
  ...
  validator
    ...
    .isPrincipalId();
  validator
    .check(file, { code: 400, msg: 'A file must be provided' })
    .notNull();
  if (file) {
    validator
...
var _storeLargePicture = function(ctx, principalId, file, callback) {
  ...
  ImageUtil.autoOrient(file.path, { removeInput: true }, function(
    err,
    orientedFile,
  ) {
    if (err) {
    ...
    ImageUtil.convertToJPG(orientedFile.path, function(err, convertedFile) {
      ...
      var options = _getProfilePictureStorageOptions(
        principalId,
        Date.now(),
        'large',
        '.jpg',
      ...
      ContentUtil.getStorageBackend(ctx).store(
        ...
        function(err, largePictureUri) {
          ...
          return PrincipalsDAO.updatePrincipal(
            principalId,
 ...
 */
var generateSizes = (module.exports.generateSizes = function(
  ctx,
  principalId,
  x,
  y,
  ...
  callback =
    ...
    function(err) {
      if (err) {
        log().error(
          { err: err },
          'Unable to crop picture %s for %s',
          fileUri,
          principalId,
        );
      }
  ...
  validator
    ...
    .isPrincipalId();
  validator
    .check(x, { code: 400, msg: 'The x value must be a positive integer' })
    .isInt();
  validator
    .check(x, { code: 400, msg: 'The x value must be a positive integer' })
    .min(0);
  validator
    .check(y, { code: 400, msg: 'The y value must be a positive integer' })
    .isInt();
  validator
    .check(y, { code: 400, msg: 'The y value must be a positive integer' })
    .min(0);
  ...
  validator
    .check(width, {
      code: 400,
      msg:
        'The width value must be a positive integer greater than or equal to 10',
    })
    .min(10);
  if (validator.hasErrors()) {
  ...
  _canManagePrincipal(ctx, principalId, function(err, principal) {
    if (err) {
      ...
      return callback({
        code: 400,
        msg: 'This principal has no large picture',
      });
    }
    ...
    _generateSizes(ctx, principal, x, y, width, function(err, principal) {
      ...
      if (PrincipalsUtil.isUser(principalId)) {
        ...
        PrincipalsEmitter.emit(
          PrincipalsConstants.events.SET_USER_PICTURE,
          ctx,
          principal,
        );
        ...
        PrincipalsEmitter.emit(
          PrincipalsConstants.events.SET_GROUP_PICTURE,
          ctx,
          principal,
        );
...
var _generateSizes = function(ctx, principal, x, y, width, callback) {
  ...
  ContentUtil.getStorageBackend(ctx, principal.picture.largeUri).get(
    ...
    function(err, file) {
      ...
      ImageUtil.cropAndResize(file.path, selectedArea, sizes, function(
        err,
        files,
      ) {
...
var _storeCroppedPictures = function(ctx, principal, files, callback) {
  ...
  backend.store(ctx.tenant().alias, smallImage, options, function(
    err,
    smallPictureUri,
  ) {
    if (err) {
    ...
    backend.store(ctx.tenant().alias, mediumImage, options, function(
      err,
      mediumPictureUri,
    ) {
      if (err) {
      ...
      return _saveCroppedPictureUris(
        ctx,
        principal,
        smallPictureUri,
        mediumPictureUri,
 ...
 * @api private
 */
var _saveCroppedPictureUris = function(
  ctx,
  principal,
  smallPictureUri,
  mediumPictureUri,
...
var _canManagePrincipal = function(ctx, principalId, callback) {
  ...
  PrincipalsDAO.getPrincipal(principalId, function(err, principal) {
    ...
    AuthzPermissions.canManage(ctx, principal, function(err) {
      if (err) {
        return callback(err);
      }
 ...
 * @api private
 */
var _getProfilePictureStorageOptions = function(
  resourceId,
  timestamp,
  size,
  extension,
