...
const SearchConstants = require('oae-search/lib/constants').SearchConstants;
const SearchModel = require('oae-search/lib/model');

const SCOPES_ALL = [SearchConstants.general.SCOPE_ALL, SearchConstants.general.SCOPE_NETWORK, SearchConstants.general.SCOPE_INTERACT,
 ...
 */
const getSearchParams = module.exports.getSearchParams = function(req) {
    if (!req || !req.query) {
        return {};
    }
 ...
 */
const getQueryParam = module.exports.getQueryParam = function(val, defaultVal) {
 ...
 * @return {String}                     `val` if it is valid, `defaultVal` otherwise
 */
const getScopeParam = module.exports.getScopeParam = function(val, defaultVal) {
 ...
 * @return {String}                     `val` if it is valid. `defaultVal` otherwise
 */
const getSortDirParam = module.exports.getSortDirParam = function(val, defaultVal) {
 ...
 * @return {String}                     `val` if it is valid. `defaultVal` otherwise
 */
const getSortFieldParam = module.exports.getSortFieldParam = function(val, defaultVal) {
 ...
 */
const getArrayParam = module.exports.getArrayParam = function(val, defaultVal) {
 ...
 */
const transformSearchResults = module.exports.transformSearchResults = function(ctx, transformers, results, callback) {
    ...
    const _monitorTransformers = function(err, resourceType, docs) {
        if (transformErr) {
            ...
            if (transformersComplete >= transformersToRun) {
                ...
                let orderedDocs = _.values(transformedDocs);
 ...
 */
const createQuery = module.exports.createQuery = function(query, filter, opts) {
    opts = opts || {};
    opts.sortBy = getSortFieldParam(opts.sortBy, SearchConstants.sort.field.SCORE);
    const sortBy = {[opts.sortBy]: getSortDirParam(opts.sort)};
    ...
    // const isLimitedByCreator = opts.createdBy && opts.createdBy === ctx.user().id;
 ...
 */
const createFilteredQuery = module.exports.createFilteredQuery = function(query, filter) {
    const filteredQuery = {
        'filtered': {
            'filter': filter
        }
 ...
 */
const createBulkIndexOperations = module.exports.createBulkIndexOperations = function(docs) {
    const cmds = [];
 ...
 */
const filterIds = module.exports.filterIds = function(ids) {
    return {
        'ids': {
            'values': ids
        }
 ...
 */
const filterExists = module.exports.filterExists = function(field) {
    return {
        'exists': {
            'field': field
        }
 ...
 */
const filterOr = module.exports.filterOr = function(/* filter0, filter1, filter2, ... */) {
    const filters = _.compact(arguments);
 ...
 */
const filterAnd = module.exports.filterAnd = function(/* filter0, filter1, filter2, ... */) {
    const filters = _.compact(arguments);
    if (_.isEmpty(filters)) {
        return null;
    } else if (filters.length === 1) {
 ...
 */
const filterNot = module.exports.filterNot = function(filter) {
    if (!filter) {
        return null;
    }
    return {'not': filter };
 ...
 */
const filterTerms = module.exports.filterTerms = function(field, values) {
    if (_.isEmpty(values)) {
        return null;
    }
 ...
 */
const filterTerm = module.exports.filterTerm = function(field, value) {
    if (!value) {
        return null;
    }
 ...
 */
const filterResources = module.exports.filterResources = function(resourceTypes, deleted) {
 ...
 */
const filterInteractingTenants = module.exports.filterInteractingTenants = function(tenantAlias) {
 ...
 */
const filterScopeAndAccess = module.exports.filterScopeAndAccess = function(ctx, scope, needsFilterByExplicitAccess, callback) {
    ...
    OaeUtil.invokeIfNecessary(needsFilterByExplicitAccess, filterExplicitAccess, ctx, function(err, explicitAccessFilter) {
        ...
        } else if (scope === SearchConstants.general.SCOPE_MY) {
            if (!user) {
                // Anonymous users cannot interact with anything, give an authorization error for this scenario
                return callback({'code': 400, 'msg': 'Anonymous users cannot search for their own resources'});
            }
        ...
        // tenants
        return callback(null, filterAnd(
            filterOr(
                implicitAccessFilter,
                explicitAccessFilter
            ),
 ...
 */
const filterImplicitAccess = module.exports.filterImplicitAccess = function(ctx) {
    const user = ctx.user();
 ...
 */
const filterExplicitAccess = module.exports.filterExplicitAccess = function(ctx, callback) {
    const user = ctx.user();
    if (!user) {
    ...
    AuthzAPI.getAllIndirectPrincipalMemberships(ctx.user().id, function(err, indirectGroupIds) {
        if (err) {
            return callback(err);
        }
...
/**
 * Create an ElasticSearch filter that will filter only the items created by the current user.
 *
 * @param  {Context}    ctx                 Standard context object containing the current user and the current tenant
 * @param  {String}     createdBy           A string containing the user id to filter by. Must be the currently logged in user
 */
const filterCreatedBy = module.exports.filterCreatedBy = function(ctx, createdBy) {
    const user = ctx.user();
 ...
 */
const createQueryStringQuery = module.exports.createQueryStringQuery = function(q, fields, boost) {
    if (_.isEmpty(fields)) {
 ...
 * @return {Object}             A Query object that can be used in the query portion of the ElasticSearch Query DSL
 */
const createEmailQuery = module.exports.createEmailQuery = function(email) {
 ...
 * @return {Object}                     A Query object that can be used in the query portion of the ElasticSearch Query DSL
 */
const createHasChildQuery = module.exports.createHasChildQuery = function(type, childQuery, scoreType, boost) {
    if (!childQuery) {
        return null;
    }
 ...
 * @return {Object}                 A Query object that can be used in the query portion of the ElasticSearch Query DSL
 */
const createMoreLikeThisQuery = module.exports.createMoreLikeThisQuery = function(val, fields) {
 ...
 */
const createChildSearchDocument = module.exports.createChildSearchDocument = function(type, resourceId, fields) {
 ...
 */
const getChildSearchDocumentId = module.exports.getChildSearchDocumentId = function(type, resourceId, childId) {
