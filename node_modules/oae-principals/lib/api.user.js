...
var _createUser = function(ctx, user, callback) {
    PrincipalsDAO.createUser(user, function(err) {
        ...
        // If the user gets created without an email address (or it's considered verified) we can
        // return here
        if ((!user.email && !user.emailChange) || user.emailVerified) {
            return callback(null, user);
        }

        // Send the user a token they can use to verify their email address with
        _sendEmailToken(ctx, user, function(err) {
            if (err) {
                return callback(err);
            }
...
var updateUser = module.exports.updateUser = function(ctx, userId, profileFields, callback) {
    ...
    // Verify that restricted email properties aren't set here. We can't re-use the DAO's
    // restricted fields as we do want to set them in the DAO, not in the API
    var invalidEmailKeys = _.intersection(['emailVerified', 'emailChange'], profileFieldKeys);
    validator.check(invalidEmailKeys.length, {'code': 400, 'msg': 'Restricted email property was attempted to be set.'}).max(0);
    ...
    PrincipalsDAO.getPrincipal(userId, function(err, oldUser) {
        ...
        // desired email address in a separate `emailChange` property and set the `emailVerified`
        // property to `false`. This will allow the UI to show a nag screen to the user asking
        // them to confirm their email
        var isAdmin = ctx.user().isAdmin(oldUser.tenant.alias);
        var isEmailChange = (!_.isUndefined(profileFields.email) && profileFields.email !== oldUser.email);
        if (isEmailChange && !isAdmin) {
            profileFields.emailChange = profileFields.email;
            profileFields.emailVerified = false;
            delete profileFields.email;
        }

        // This adds some handling for the special case where a user tried to change his email
        // address, made a mistake and realised his email address was correct. For example, Jack
        // has an account with the address "jack@delphi.edu". He changes it to "jack.bar@delphi.edu"
        // mistakenly. The UI would keep asking him to confirm that email even though he has no
        // way of doing so. Jack should be able to get rid of this, by reverting back to "jack@delphi.edu"
        if (!_.isUndefined(profileFields.email) && !_.isUndefined(oldUser.email) && profileFields.email === oldUser.email) {
            profileFields.emailChange = '';

            // We can safely revert to `true` as a user can't have an old email address without
            // having verified it
            profileFields.emailVerified = true;
        }
        ...
        PrincipalsDAO.updatePrincipal(userId, profileFields, function(err) {
            ...
            // If the email address changed, we might have to send a verification email
            if (profileFields.emailChange) {
                _sendEmailToken(ctx, newUser, function(err) {
                    if (err) {
                        return callback(err);
                    }
 ...
 * @api private
 */
var _sendEmailToken = function(ctx, user, callback) {
    callback = callback || function(err) {
        if (err) {
    ...
    PrincipalsDAO.storeEmailToken(token, user.id, function(err) {
        ...
        var userToEmail = _.extend({}, user, {'email': user.emailChange});
        ...
        EmailAPI.sendEmail('oae-principals', 'verify', userToEmail, data, {'hash': token, 'ignoreVerification': true}, callback);
    });
};
...
var resendEmailToken = module.exports.resendEmailToken = function(ctx, userId, callback) {
    ...
    PrincipalsDAO.getPrincipal(userId, function(err, user) {
        ...
        return _sendEmailToken(ctx, user, callback);
    });
};
 ...
 * @param  {User}       callback.user   The updated user
 */
var verifyEmail = module.exports.verifyEmail = function(ctx, email, token, callback) {
    var validator = new Validator();
    validator.check(email, {'code': 400, 'msg': 'A valid email address must be provided'}).notEmpty();
    ...
    validator.check(token, {'code': 400, 'msg': 'An invalid guest token was provided'}).regex(/^email-[a-zA-Z0-9]{32}$/);
    if (validator.hasErrors()) {
        return callback(validator.getFirstError());
    }
    ...
    PrincipalsDAO.getUserByEmailToken(token, function(err, user) {
        if (err) {
            return callback(err);
        } else if (user.emailChange !== email) {
            // Although very unlikely, it's better to be safe
            log().warn({'email': email, 'user': user.id}, 'Someone tried to verify their own email address with another user their token');
            return callback({'code': 401, 'msg': 'Wrong token'});
        }

        // Remove the token, set the email address and create the email-to-user mapping
        PrincipalsDAO.setEmailAddress(user, email, token, function(err, user) {
            if (err) {
                return callback(err);
            }

            // Emit an event that an email address has been verified
            PrincipalsEmitter.emit(PrincipalsConstants.events.VERIFIED_EMAIL, ctx, email, user);
