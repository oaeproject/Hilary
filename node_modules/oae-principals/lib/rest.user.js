 ...
 * @Server      tenant
 * @Method      GET
 * @Path        /api/users/logs/{tenantAlias}/{type}/{months}
 ...
 * @Return      {File}                  file            The log list in csv format
 * @HttpResponse                        200             File successfully uploaded
 */
var _getUsersDateLog = function(req, res) {
    PrincipalsAPI.getUsersDateLog(req.ctx, req.params.tenantAlias, req.params.type, req.params.months, function(err, log) {
        if (err) {
            return res.status(err.code).send(err.msg);
        }

        var csvName = req.params.tenantAlias + '-' + req.params.type + '-' + Date.now() + '.csv';

        res.setHeader('Content-disposition', 'attachment; filename=' + csvName);
        res.set('Content-Type', 'text/csv');
        res.writeHead(200);
        res.end(new Buffer(log));
    });
};

OAE.globalAdminRouter.on('get', '/api/users/logs/:tenantAlias/:type/:months', _getUsersDateLog);
OAE.tenantRouter.on('get', '/api/users/logs/:tenantAlias/:type/:months', _getUsersDateLog);
