...
var _canManage = function(ctx, contentObj, callback) {
    ...
    return AuthzAPI.hasRole(ctx.user().id, contentObj.contentId, ContentConstants.roles.MANAGER, callback);
};
...
var _augmentContent = function(ctx, contentObj) {
    ...
    contentObj.signature = ContentUtil.signContent(ctx, contentObj.contentId, contentObj.lastModified);

    if (contentObj.previews.thumbnailUri) {
        contentObj.previews.thumbnailUrl = ContentUtil.getThumbnailUrl(ctx, contentObj.contentId, contentObj.lastModified, contentObj.signature);
    }
};
