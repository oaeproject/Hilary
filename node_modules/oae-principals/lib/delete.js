 ...
 */
var registerGroupDeleteHandler = (module.exports.registerGroupDeleteHandler = function(
  name,
  handler,
) {
  if (_groupDeleteHandlers[name]) {
    throw new Error(
      util.format(
        'Attempted to register multiple group delete handlers for name "%s"',
        name,
      ),
    );
    ...
    throw new Error(
      util.format(
        'Attempted to register non-function group delete handler for name "%s"',
        name,
      ),
    );
  }
 ...
 */
var registerGroupRestoreHandler = (module.exports.registerGroupRestoreHandler = function(
  name,
  handler,
) {
  if (_groupRestoreHandlers[name]) {
    throw new Error(
      util.format(
        'Attempted to register multiple group restore handlers for name "%s"',
        name,
      ),
    );
  } else if (!_.isFunction(handler)) {
    throw new Error(
      util.format(
        'Attempted to register non-function group restore handler for name "%s"',
        name,
      ),
    );
  }
 ...
 */
var invokeGroupDeleteHandlers = (module.exports.invokeGroupDeleteHandlers = function(
  group,
) {
 ...
 * @param  {Group}  group   The group for which to invoke the delete handlers
 */
var invokeGroupRestoreHandlers = (module.exports.invokeGroupRestoreHandlers = function(
  group,
) {
 ...
 */
var whenDeletesComplete = (module.exports.whenDeletesComplete = function(
  callback,
) {
...
var _invokeGroupHandlers = function(log, handlers, group) {
  ...
  AuthzAPI.getPrincipalMembershipsGraph(group.id, function(
    err,
    membershipsGraph,
  ) {
    if (err) {
...
var _invokeHandlers = function(log, handlers, principal /*, args... */) {
  ...
  _.each(handlers, function(handler, name) {
    ...
    var thisHandlerArgs = handlerArgs.concat(function(errs) {
      ...
      if (!_.isEmpty(errs)) {
        return log().error(
          { principalId: principal.id, handlerName: name, errs: errs },
 ...
 * a group is deleted
 */
PrincipalsEmitter.on(PrincipalsConstants.events.DELETED_GROUP, function(
  ctx,
  group,
) {
 ...
 * a group is restored
 */
PrincipalsEmitter.on(PrincipalsConstants.events.RESTORED_GROUP, function(
  ctx,
  group,
) {
