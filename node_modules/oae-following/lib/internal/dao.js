...
var getFollowers = module.exports.getFollowers = function(userId, start, limit, callback) {
    ...
    Cassandra.runPagedQuery('FollowingUsersFollowers', 'userId', userId, 'followerId', start, limit, null, function(err, rows, nextToken) {
        if (err) {
            return callback(err);
        }

        return callback(null, _.pluck(rows, 'followerId'), nextToken);
    });
};
...
var getFollowing = module.exports.getFollowing = function(userId, start, limit, callback) {
    ...
    Cassandra.runPagedQuery('FollowingUsersFollowing', 'userId', userId, 'followingId', start, limit, null, function(err, rows, nextToken) {
        if (err) {
            return callback(err);
        }

        return callback(null, _.pluck(rows, 'followingId'), nextToken);
    });
};
...
var isFollowing = module.exports.isFollowing = function(followingUserId, followedUserIds, callback) {
    ...
    var placeholders = _.map(followedUserIds, function() {
        return '?';
    });
    var parameters = _.union([followingUserId], followedUserIds);
    var cql = util.format('SELECT "followingId", "value" FROM "FollowingUsersFollowing" WHERE "userId" = ? AND "followingId" IN (%s)', placeholders.join(','));
