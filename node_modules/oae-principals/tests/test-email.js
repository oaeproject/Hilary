...
describe('Users', function() {
    ...
    describe('Email tokens', function() {
        ...
        describe('Verifying email tokens', function() {
            ...
            /**
             * Test that verifies that users are automatically logged on when they verify their token
             */
            it('verify users are automatically logged on when they verify their token', function(callback) {
                TestsUtil.generateTestUsers(camAdminRestContext, 1, function(err, users, simong) {
                    assert.ok(!err);
                    ...
                    PrincipalsTestUtil.whenVerificationEmailSent(email, function(token) {
                        ...
                        // Verify the token
                        RestAPI.User.verifyEmailToken(anonymousRestContext, token, function(err) {
                            assert.ok(!err);

                            // Assert the user is now logged in
                            RestAPI.User.getMe(anonymousRestContext, function(err, me) {
                                assert.ok(!err);
                                assert.ok(!me.anon);
                                assert.strictEqual(me.emailVerified, true);
            ...
            /**
             * Test that verifies that email tokens can be verified by anonymous users
             */
            it('verify email tokens can be verified by anonymous users', function(callback) {
                TestsUtil.generateTestUsers(camAdminRestContext, 1, function(err, users, simong) {
                    assert.ok(!err);
                    ...
                    PrincipalsTestUtil.whenVerificationEmailSent(email, function(token) {
                        ...
                        // Let the anonymous user verify the token
                        RestAPI.User.verifyEmailToken(anonymousRestContext, token, function(err) {
                            assert.ok(!err);
        ...
        describe('Resending email tokens', function(callback) {
            ...
            it('verify an email token can be resent by a user', function(callback) {
                TestsUtil.generateTestUsers(camAdminRestContext, 2, function(err, users, simong, nico) {
                    ...
                    RestAPI.User.resendEmailToken(simong.restContext, nico.user.id, function(err) {
                        ...
                        // Users can resend their own email token
                        RestAPI.User.resendEmailToken(simong.restContext, simong.user.id, function(err) {
                            assert.ok(!err);
                        });

                        PrincipalsTestUtil.whenVerificationEmailSent(simong.user.email, function(token) {
                            return callback();
                        });
                    });
                });
            ...
            it('verify an email token can be resent by a tenant admin', function(callback) {
                TestsUtil.generateTestUsers(camAdminRestContext, 1, function(err, users, simong) {
                    ...
                    // Tenant admins from other tenants cannot resent an email token
                    RestAPI.User.resendEmailToken(gtAdminRestContext, simong.user.id, function(err) {
                        assert.strictEqual(err.code, 401);

                        // Tenant admins from the same tenant can resend an email token
        ...
        it('verify an email address is unverified when updated', function(callback) {
            TestsUtil.generateTestUsers(camAdminRestContext, 3, function(err, users, simon, nico, stuart) {
                ...
                RestAPI.User.updateUser(nico.restContext, nico.user.id, {'email': newEmail}, function(err) {
                    ...
                    PrincipalsTestUtil.whenVerificationEmailSent(newEmail, function(token) {
