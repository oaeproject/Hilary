...
var setGroupMembers = module.exports.setGroupMembers = function(ctx, groupId, changes, callback) {
    ...
    for (var i = 0; i < principals.length;i++) {
        ...
        validator.check(changes[principals[i]], {'code': 400, 'msg': 'Invalid principal specified as member for this group.'}).isValidRoleChange();
    }
    if (validator.hasErrors()) {
        return callback(validator.getFirstError());
    }
    ...
    PrincipalsDAO.getPrincipal(groupId, function(err, group) {
        ...
        ResourceActions.setRoles(ctx, group, changes, function(err, members) {
            if (err) {
                return callback(err);
            }
            ...
            PrincipalsDAO.updatePrincipal(group.id, {'lastModified': newLastModified}, function(err) {
                ...
                _touchMembershipLibraries(group, oldLastModified, addedMemberIds.slice(), removedMemberIds.slice(), function(err) {
                    if (err) {
                        return callback(err);
                    }

                    PrincipalsEmitter.emit(PrincipalsConstants.events.UPDATED_GROUP_MEMBERS, ctx, group, members.changes, addedMemberIds, updatedMemberIds, removedMemberIds, function(errs) {
                        if (errs) {
...
var leaveGroup = module.exports.leaveGroup = function(ctx, groupId, callback) {
    ...
    PrincipalsDAO.getPrincipal(groupId, function(err, group) {
        ...
        AuthzPermissions.canRemoveRole(ctx, ctx.user(), group, function(err, changes, roles) {
            if (err) {
                return callback(err);
            }

            AuthzAPI.updateRoles(group.id, changes, function(err) {
                if (err) {
                    return callback(err);
                }
                ...
                _touchMembershipLibraries(group, null, [], [ctx.user().id], function(err) {
                    ...
                    PrincipalsEmitter.emit(PrincipalsConstants.events.LEFT_GROUP, ctx, group, roles.before[ctx.user().id], function(errs) {
                        if (errs) {
                            return callback(_.first(errs));
                        }
...
var joinGroup = module.exports.joinGroup = function(ctx, groupId, callback) {
    ...
    PrincipalsDAO.getPrincipal(groupId, function(err, group) {
        ...
        AuthzPermissions.canJoin(ctx, group, function(err, changes, roles) {
            if (err) {
                return callback(err);
            }
            ...
            AuthzAPI.updateRoles(group.id, changes, function(err) {
                ...
                PrincipalsDAO.updatePrincipal(group.id, updatedProfileFields, function(err) {
                    ...
                    _touchMembershipLibraries(group, oldLastModified, [ctx.user().id], [], function(err) {
                        if (err) {
                            log().error({
                                'err': err,
                                'group': group,
                                'memberIds': _.keys(roles.after)
                ...
                PrincipalsEmitter.emit(PrincipalsConstants.events.JOINED_GROUP, ctx, group, changes[ctx.user().id], function(errs) {
                    if (errs) {
                        return callback(_.first(errs));
                    }
 ...
 * @param  {Object}    [roles]              A hash where each key is a principal id and the value is one of `PrincipalsConstants.roles`
 * @param  {Function}  [callback]           Standard callback function
...
var createGroup = module.exports.createGroup = function(ctx, displayName, description, visibility, joinable, roles, callback) {
    ...
    roles[ctx.user().id] = PrincipalsConstants.roles.MANAGER;
    ...
    ResourceActions.create(ctx, roles, createFn, function(err, group, members) {
        if (err) {
            return callback(err);
        }

        // Update the user's libraries
        _touchMembershipLibraries(group, null, _.keys(members.changes), [], function(err) {
            if (err) {
                log().error({
                    'err': err,
                    'group': group,
                    'roles': members.changes
                }, 'An error occurred trying to insert to the library index while creating a group');
            }

            PrincipalsEmitter.emit(PrincipalsConstants.events.CREATED_GROUP, ctx, group, members.changes);
            return callback(null, group);
        });
    });
};
 ...
 * @param  {Function}   callback                Standard callback function
 * @param  {Object}     callback.err            An error that occurred, if any
 * @param  {Object}     callback.principals     An object keyed by principal id, whose value is the principal object
 * @api private
 */
