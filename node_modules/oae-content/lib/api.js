 ...
 * @param  {Content}   callback.content   JSON object containing the pool id of the created content
 */
var createDocument = module.exports.createDocument = function(ctx, displayName, description, visibility, additionalMembers, callback) {
    callback = callback || function() {};

    // Setting content to default if no visibility setting is provided
    visibility = visibility || Config.getValue(ctx.tenant().alias, 'visibility', 'documents');

    _createContent(ctx, 'document', displayName, description, visibility, additionalMembers, {}, function(err, content) {
        if (!err) {
            ContentAPI.emit(ContentConstants.events.CREATED_CONTENT, ctx, content, additionalMembers);
        }
        callback(err, content);
 ...
 *
 * @param  {Context}        ctx                Standard context object containing the current user and the current tenant
 * @param  {String}         resourceSubType    The content type. Possible values are "file", "document" and "link"
...
var _createContent = function(ctx, resourceSubType, displayName, description, visibility, additionalMembers, otherValues, callback) {
    ...
    validator.check(resourceSubType, {'code': 400, 'msg': 'A valid content type must be provided. This can be "file", "document" or "link"'}).isIn(ContentConstants.resourceSubTypes);
    validator.check(null, {'code': 401, 'msg': 'You have to be logged in to be able to create a content item'}).isLoggedInUser(ctx);
    if (validator.hasErrors()) {
        return callback(validator.getFirstError());
    }
