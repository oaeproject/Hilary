...
var ContentSearchAPI = require('./search');
...
var deleteContent = module.exports.deleteContent = function(ctx, contentId, callback) {
    ...
    canManage(ctx, contentId, function(err, canManage, contentObj) {
        ...
        Cassandra.runQuery('DELETE FROM Content WHERE contentId = ?', [contentId], function(err, rows) {
            ...
            // delete the indexed content item
            _deleteIndexedContent(contentId);
    ...
    getContent(ctx, contentId, function(err, contentObj) {
        ...
        // Check if the user can manage the content
            ...
            _canManage(ctx, contentObj, function(err, canManage) {
                ...
                _shareContent(ctx, contentObj, principalIds, function(err) {
                    // reindex the content item, regardless of its state
                    _reindexContent(contentObj);
                    return callback(err);
                });
            });
...
var _updateContentMetadata = function(ctx, contentId, metadata, callback) {
    ...
    canManage(ctx, contentId, function(err, canManage, contentObj) {
        ...
        Cassandra.runQuery(q.query, q.parameters, function(err) {
            ...
            _updateLibraries(contentObj, [], function(err) {
                _reindexContent(contentObj);
                return callback(err);
            });
        });
...
/**
 * Index the given content item.
 *
 * @param {Content}     contentObj      The content item to (re)index
 * @param {Function}    callback        Invoked when the process completes.
 * @param {Object}      callback.err    An error that occurred, if any
 */
var _reindexContent = function(contentObj, callback) {
    callback = callback || function(err) {
        if (err) {
            log().error({
                err: err,
                contentObj: contentObj
            }, 'Error indexing content item.');
        }
    };
    ...
    _getAllContentMembers(contentObj.contentId, function(err, members) {
        ...
        var members = _.map(members, function(member) {
            return member.id;
        });

        ContentSearchAPI.indexContent(contentObj, members, callback);
    });
}
 ...
 * @param {Object}      callback.err    An error that occurred, if any
 */
var _deleteIndexedContent = function(contentId, callback) {
    callback = callback || function(err) {
        if (err) {
            log().error({ err: err }, 'Error deleting indexed content item with id %s', contentId);
        }
    };

    ContentSearchAPI.deleteContent(contentId, callback);
}
