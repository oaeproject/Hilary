 ...
 */
var startTenant = module.exports.startTenant = function(tenant) {
 ...
 * @param {Function} callback A function(tenant, err) specifying the tenant requested
 */
 ...
 * @param {Function} callback A function(tenant, err) specifying the tenant information that was persisted.
 */
var createTenant = module.exports.createTenant = function(alias, name, description, port, callback) {
    callback = callback || function() {};
    getTenantByPort(port, function(tenant, err) {
        if (!err) {
            if (!tenant) {
                getTenantByAlias(alias, function(tenant, err) {
                    if (!err) {
                        if (!tenant) {
                            ...
                            callback(null, {why: 'Tenant by alias "' + alias + '" alread exists.'});
                        }
                    } else {
                        callback(null, err);
                    }
                ...
                callback(null, {why: 'Tenant by port "' + port + '" already exists.'});
            }
        } else {
            callback(null, err);
        }
