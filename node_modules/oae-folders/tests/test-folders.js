...
describe('Folders', function() {
    ...
    describe('Get Folders Content', function() {

        /**
         * Test that verifies that the correct folder content library is returned
         */
        it('verify users get the appropriate folders content library visibility', function(callback) {
            TestsUtil.setupMultiTenantPrivacyEntities(function(publicTenant, publicTenant1) {

                // Create a public folder that includes a user from another tenant
                FoldersTestUtil.generateTestFoldersWithVisibility(publicTenant.publicUser.restContext, 1, 'public', function(folder) {
                    FoldersTestUtil.assertUpdateFolderMembersSucceeds(publicTenant.publicUser.restContext, folder.id, _memberUpdate(publicTenant1.publicUser, 'manager'), function() {

                        // Add some private, loggedin and public content to the folder
                        RestAPI.Content.createLink(publicTenant.publicUser.restContext, 'test', 'test', 'public', 'http://www.google.ca', null, null, function(err, publicLink) {
                            assert.ok(!err);
                            RestAPI.Content.createLink(publicTenant.publicUser.restContext, 'test', 'test', 'loggedin', 'http://www.google.ca', null, null, function(err, loggedinLink) {
                                assert.ok(!err);
                                RestAPI.Content.createLink(publicTenant.publicUser.restContext, 'test', 'test', 'private', 'http://www.google.ca', null, null, function(err, privateLink) {
                                    assert.ok(!err);
                                    FoldersTestUtil.assertAddContentItemsToFolderSucceeds(publicTenant.publicUser.restContext, folder.id, [publicLink.id, loggedinLink.id, privateLink.id], function() {

                                        // Anonymous users can only see public content in the folder
                                        FoldersTestUtil.assertFolderEquals(publicTenant.anonymousRestContext, folder.id, [publicLink.id], function() {
                                            // Users from the same tenant can see public and logged in content
                                            FoldersTestUtil.assertFolderEquals(publicTenant.loggedinUser.restContext, folder.id, [publicLink.id, loggedinLink.id], function() {
                                                // Folder members can see all the content
                                                FoldersTestUtil.assertFolderEquals(publicTenant.publicUser.restContext, folder.id, [publicLink.id, loggedinLink.id, privateLink.id], function() {
                                                    // Tenant admins from the same tenant can see everything
                                                    FoldersTestUtil.assertFolderEquals(publicTenant.adminRestContext, folder.id, [publicLink.id, loggedinLink.id, privateLink.id], function() {
                                                        // Users from another tenant can only see the public content
                                                        FoldersTestUtil.assertFolderEquals(publicTenant1.loggedinUser.restContext, folder.id, [publicLink.id], function() {
                                                            // Users from another tenant who are a member of the folder can see everything
                                                            FoldersTestUtil.assertFolderEquals(publicTenant1.publicUser.restContext, folder.id, [publicLink.id, loggedinLink.id, privateLink.id], function() {
                                                                // Tenant admins from another tenant can only see the public content
                                                                FoldersTestUtil.assertFolderEquals(publicTenant1.adminRestContext, folder.id, [publicLink.id], callback);
                                                            });
                                                        });
                                                    });
                                                });
        ...
        /**
         * Test that verifies that when retrieving content from a folder it's sorted on its `lastModified` timestamp
         */
        it('verify get folders content library order', function(callback) {
                    ...
                    RestAPI.Content.createLink(camAdminRestContext, 'test', 'test', 'public', 'http://www.google.ca', null, null, function(err, link1) {
                        ...
                        RestAPI.Content.createLink(camAdminRestContext, 'test', 'test', 'public', 'http://www.google.ca', null, null, function(err, link2) {
                            ...
                            RestAPI.Content.createLink(camAdminRestContext, 'test', 'test', 'public', 'http://www.google.ca', null, null, function(err, link3) {
                                assert.ok(!err);
                                FoldersTestUtil.assertAddContentItemsToFolderSucceeds(simong.restContext, folder.id, [link1.id, link2.id, link3.id], function() {

                                    // Get the content in the folder
                                    FoldersTestUtil.getAllFolderContentItems(simong.restContext, folder.id, null, function(contentItems) {
                                        assert.strictEqual(contentItems.length, 3);

                                        // Assert the order in this folder is based on the last modified date
                                        assert.strictEqual(contentItems[0].id, link3.id);
                                        assert.strictEqual(contentItems[1].id, link2.id);
                                        assert.strictEqual(contentItems[2].id, link1.id);

                                        // Update the link
                                        RestAPI.Content.updateContent(camAdminRestContext, link1.id, {'description': 'in a VAN down by the RIVER'}, function(err) {
                                            assert.ok(!err);

                                            FoldersLibrary.whenAllPurged(function() {

                                                // Retrieve the content items in the folder. The link we just updated should be on top
                                                FoldersTestUtil.getAllFolderContentItems(simong.restContext, folder.id, null, function(contentItems) {
                                                    assert.strictEqual(contentItems.length, 3);

                                                    // Assert the order in this folder is based on the last modified date
                                                    assert.strictEqual(contentItems[0].id, link1.id);
                                                    assert.strictEqual(contentItems[1].id, link3.id);
                                                    assert.strictEqual(contentItems[2].id, link2.id);
                                                    return callback();
                                                });
                                            });
                                        });
    ...
    describe('Remove Items from Folder', function() {
        ...
        /**
         * Test that verifies parameters are validated when removing content items from a folder
         */
        it('verify parameter validation', function(callback) {
            ...
            TestsUtil.generateTestUsers(camAdminRestContext, 1, function(err, users, simong) {
                ...
                FoldersTestUtil.generateTestFolders(simong.restContext, 1, function(folder) {
                    ...
                    // Create a content item that they will add to their folder
                    RestAPI.Content.createLink(simong.restContext, 'test', 'test', 'public', 'http://www.google.ca', null, null, function(err, link) {
                        assert.ok(!err);
                        FoldersTestUtil.assertAddContentItemsToFolderSucceeds(simong.restContext, folder.id, [link.id], function() {

                            // Invalid folder id
                            FoldersTestUtil.assertRemoveContentItemsFromFolderFails(simong.restContext, 'invalid folder id', [link.id], 400, function() {

                                // Unknown folder id
                                FoldersTestUtil.assertRemoveContentItemsFromFolderFails(simong.restContext, 'f:camtest:bleh', [link.id], 404, function() {

                                    // Missing content
                                    FoldersTestUtil.assertRemoveContentItemsFromFolderFails(simong.restContext, folder.id, [], 400, function() {

                                        // Invalid content id
                                        FoldersTestUtil.assertRemoveContentItemsFromFolderFails(simong.restContext, folder.id, ['invalid content id'], 400, function() {

                                            // Mix of valid and invalid content ids
                                            FoldersTestUtil.assertRemoveContentItemsFromFolderFails(simong.restContext, folder.id, [link.id, 'invalid content id'], 400, function() {

                                                // Mix of valid and unknown content ids
                                                FoldersTestUtil.assertRemoveContentItemsFromFolderFails(simong.restContext, folder.id, [link.id, 'c:camtest:bleh'], 404, function() {

                                                    // Sanity check that nothing was removed
                                                    FoldersTestUtil.assertFolderEquals(simong.restContext, folder.id, [link.id], callback);
                                                });
                                            });
                                        });
                                    });
        ...
        it('verify removing multiple items from a folder', function(callback) {
            ...
            FoldersTestUtil.setupMultiTenantPrivacyEntities(function(publicTenant, publicTenant1) {
                ...
                // Create 5 content items to add to the folder
                RestAPI.Content.createLink(publicTenant.publicUser.restContext, 'test1', 'test', 'public', 'http://www.google.ca', null, null, function(err, link1) {
                    assert.ok(!err);
                    RestAPI.Content.createLink(publicTenant.publicUser.restContext, 'test2', 'test', 'public', 'http://www.google.ca', null, null, function(err, link2) {
                        assert.ok(!err);
                        RestAPI.Content.createLink(publicTenant.publicUser.restContext, 'test3', 'test', 'public', 'http://www.google.ca', null, null, function(err, link3) {
                            assert.ok(!err);
                            RestAPI.Content.createLink(publicTenant.publicUser.restContext, 'test4', 'test', 'public', 'http://www.google.ca', null, null, function(err, link4) {
                                assert.ok(!err);
                                RestAPI.Content.createLink(publicTenant.publicUser.restContext, 'test5', 'test', 'public', 'http://www.google.ca', null, null, function(err, link5) {
                                    assert.ok(!err);

                                    // Add the items to the folder
                                    FoldersTestUtil.assertAddContentItemsToFolderSucceeds(publicTenant.adminRestContext, publicTenant.publicFolder.id, [link1.id, link2.id, link3.id, link4.id, link5.id], function() {

                                        // Remove the first 2 content items
                                        FoldersTestUtil.assertRemoveContentItemsFromFolderSucceeds(publicTenant.adminRestContext, publicTenant.publicFolder.id, [link1.id, link2.id], function() {

                                            // Ensure that the items have been removed from all libraries
                                            FoldersTestUtil.assertFolderEquals(publicTenant.publicUser.restContext, publicTenant.publicFolder.id, [link3.id, link4.id, link5.id], function() {
                                                FoldersTestUtil.assertFolderEquals(publicTenant.loggedinUser.restContext, publicTenant.publicFolder.id, [link3.id, link4.id, link5.id], function() {
                                                    FoldersTestUtil.assertFolderEquals(publicTenant.privateUser.restContext, publicTenant.publicFolder.id, [link3.id, link4.id, link5.id], function() {
                                                        FoldersTestUtil.assertFolderEquals(publicTenant1.publicUser.restContext, publicTenant.publicFolder.id, [link3.id, link4.id, link5.id], function() {
                                                            FoldersTestUtil.assertFolderEquals(publicTenant.adminRestContext, publicTenant.publicFolder.id, [link3.id, link4.id, link5.id], function() {
                                                                FoldersTestUtil.assertFolderEquals(publicTenant.anonymousRestContext, publicTenant.publicFolder.id, [link3.id, link4.id, link5.id], callback);
                                                            });
                                                        });
                                                    });
                                                });
    ...
    describe('Content Integration', function() {
                            ...
                            FoldersTestUtil.assertAddContentItemToFoldersSucceeds(publicTenant.adminRestContext, [publicTenant.publicFolder.id], link.id, function() {
                                ...
                                FoldersTestUtil.assertFolderEquals(publicTenant.publicUser.restContext, publicTenant.publicFolder.id, [link.id], function() {
                                    FoldersTestUtil.assertFolderEquals(publicTenant1.publicUser.restContext, publicTenant.publicFolder.id, [link.id], function() {
                                        FoldersTestUtil.assertFolderEquals(publicTenant.anonymousRestContext, publicTenant.publicFolder.id, [link.id], function() {
                                                    ...
                                                    FoldersLibrary.whenAllPurged(function() {
                                                        ...
                                                        FoldersTestUtil.assertFolderEquals(publicTenant.publicUser.restContext, publicTenant.publicFolder.id, [link.id], function() {
                                                            FoldersTestUtil.assertFolderEquals(publicTenant1.publicUser.restContext, publicTenant.publicFolder.id, [], function() {
                                                                FoldersTestUtil.assertFolderEquals(publicTenant.anonymousRestContext, publicTenant.publicFolder.id, [], function() {
                                                                    FoldersTestUtil.assertFolderEquals(simong.restContext, publicTenant.publicFolder.id, [link.id], function() {
        ...
        /**
         * Test that verifies that content items are removed from folders when they are deleted
         */
        it('verify content items are removed from folders when they are deleted', function(callback) {
            TestsUtil.generateTestUsers(camAdminRestContext, 1, function(err, users, simong) {
                assert.ok(!err);
                ...
                FoldersTestUtil.generateTestFolders(simong.restContext, 1, function(folder) {
                    ...
                    RestAPI.Content.createLink(simong.restContext, 'test', 'test', 'public', 'http://www.google.ca', null, null, function(err, link) {
                        ...
                        FoldersTestUtil.assertAddContentItemToFoldersSucceeds(simong.restContext, [folder.id], link.id, function() {

                            // Sanity-check the item is in the folder
                            FoldersTestUtil.assertFolderEquals(simong.restContext, folder.id, [link.id], function() {

                                // Delete the item
                                RestAPI.Content.deleteContent(simong.restContext, link.id, function(err) {
                                    assert.ok(!err);

                                    FoldersLibrary.whenAllPurged(function() {

                                        // Sanity check the folder is empty
                                        FoldersTestUtil.assertFolderEquals(simong.restContext, folder.id, [], callback);
                                    });
                                });
                            });
                        });
