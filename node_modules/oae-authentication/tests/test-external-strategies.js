...
describe('Authentication', function() {
    ...
    // Rest contexts that can be used
    ...
    describe('CAS authentication', function() {
        ...
        beforeEach(function(callback) {
            ...
            AuthenticationTestUtil.setupCASServer(global.oaeTests.tenants.localhost.alias, false, externalId, 'Simon', email, function(_validTicket, _closeCallback) {
                validTicket = _validTicket;
            ...
            _enableStrategy('cas', function(done) {
                ...
                RestAPI.Authentication.casCallback(restContext, {'ticket': validTicket}, function(err, body, response) {
                    ...
                    RestAPI.User.getMe(restContext, function(err, me) {
                        ...
                        // CAS is an authoritative source, the user should not
                        // have to verify his email address
                ...
                RestAPI.Authentication.casCallback(restContext, {'ticket': validTicket}, function(err, body, response) {
                    ...
                    RestAPI.User.getMe(restContext, function(err, me) {
                        ...
                        RestAPI.Authentication.logout(restContext, function(err, data, response) {
                            ...
                            var casLocation = url.parse(response.headers.location);
                            assert.equal(casLocation.hostname, 'localhost');
                            assert.equal(casLocation.pathname, '/cas/logout');
    ...
    describe('Shibboleth authentication', function() {
                ...
                _initiateShibbolethAuthFlow('/content/bla', function(tenantRestContext, spRestContext) {
                    ...
                    _callbackShibbolethAuthFlow(tenantRestContext, spRestContext, attributes, '/content/bla', function() {
                        ...
                        RestAPI.User.getMe(tenantRestContext, function(err, me) {
                            ...
                            // Shibboleth is an authoritative source. The email address should not be verified
                            assert.strictEqual(me.emailVerified, true);
                            return done();
                        });
                    });
        ...
        /**
         * Test that verifies that logging in with an existing email from the same tenant associates the loginid with the user
         */
        it('verify logging in with an existing email from the same tenant associates the loginid with the user', function(callback) {
            // Generate a user with a local test account on the `localhost` tenant
            TestsUtil.generateTestUsers(localAdminRestContext, 1, function(err, users, simong) {
                // Generate a user with a local test account on the `Cambridge` tenant
                TestsUtil.generateTestUsers(camAdminRestContext, 1, function(err, users, nico) {

                    // Log in through Shibboleth, but use the same email address as Simon
                    _enableStrategy('shibboleth', function(done) {
                        _initiateShibbolethAuthFlow('/content/bla', function(tenantRestContext, spRestContext) {
                            var attributes = {
                                'shib-session-id': _.random(10000),
                            ...
                            _callbackShibbolethAuthFlow(tenantRestContext, spRestContext, attributes, '/content/bla', function() {
                                ...
                                // Assert that we authenticated as Simon
                                RestAPI.User.getMe(tenantRestContext, function(err, me) {
                                    assert.ok(!err);
                                    assert.ok(!me.anon);
                                    assert.strictEqual(me.id, simong.user.id);
                                    assert.strictEqual(me.authenticationStrategy, 'shibboleth');

                                    // Trying to authenticate with Nico's email should fail as he belongs
                                    ...
                                    _initiateShibbolethAuthFlow('/content/bla', function(tenantRestContext, spRestContext) {
                                        attributes = {
                                            ...
                                            'remote_user': 'simon' + _.random(10000),
                                            'displayname': 'Simon',
                                            'email': nico.user.email
                                        };
                                        RestAPI.Authentication.shibbolethSPCallback(spRestContext, attributes, function(err, body, response) {
                                            assert.ok(!err);
                                            assert.strictEqual(response.statusCode, 302);
                                            assert.ok(response.headers.location);
                                            assert.ok(response.headers.location.indexOf('failed') > 0);
                                            return done();
                                        });
                                    });
                                });
