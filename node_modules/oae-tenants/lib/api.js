...
var init = module.exports.init = function(_serverConfig, callback) {
    ...
    OAE.tenantServer.use(function(req, res, next) {
        ...
        } else if (req.headers.host === serverConfig.guestsTenantHost) {
            req.tenant = getGuestsTenant();
        } else {
...
var getTenant = module.exports.getTenant = function(tenantAlias) {
    if (tenantAlias === serverConfig.guestsTenantAlias) {
        return getGuestsTenant();
    }
...
var getGuestsTenant = module.exports.getGuestsTenant = function() {
    return new Tenant(serverConfig.guestsTenantAlias, 'Guest tenant', serverConfig.guestsTenantHost, {
        'active': true,
        'deleted': false
    });
};
...
var createTenant = module.exports.createTenant = function(ctx, alias, displayName, host, callback) {
    ...
    if (host) {
        ...
        validator.check(host.toLowerCase(), {'code': 400, 'msg': 'This hostname is reserved for the guest tenant'}).not(serverConfig.guestsTenantHost);
    }
...
var updateTenant = module.exports.updateTenant = function(ctx, alias, tenantUpdates, callback) {
    ...
    _.each(tenantUpdates, function(updateValue, updateField) {
        ...
        if (updateField === 'displayName') {
            ...
            validator.check(tenantUpdates.host.toLowerCase(), {'code': 400, 'msg': 'This hostname is reserved for the guest tenant'}).not(serverConfig.guestsTenantHost);
        }
    });
    if (validator.hasErrors()) {
        return callback(validator.getFirstError());
