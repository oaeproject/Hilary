...
module.exports = function(config) {
        ...
        var passportStrategy = new LocalStrategy({'passReqToCallback': true}, function(req, username, password, done) {
            ...
            AuthenticationAPI.checkPassword(tenant.alias, username, password, function(err, userId) {
                ...
                PrincipalsAPI.getUser(ctx, userId, function(err, user) {
                    ...
                    var strategyId = AuthenticationUtil.getStrategyId(tenant, AuthenticationConstants.providers.LOCAL);
                    var authObj = {'user': user, 'strategyId': strategyId};
    ...
    // The local strategy must be enabled for the global admin
    ...
    // The local strategy must be enabled for the guests tenant
    var guestsTenant = TenantsAPI.getGuestsTenant();
    var guestsLocalPassportStrategyName = AuthenticationUtil.getStrategyId(guestsTenant, AuthenticationConstants.providers.LOCAL);
    passport.use(guestsLocalPassportStrategyName, strategy.getPassportStrategy(guestsTenant));
};
