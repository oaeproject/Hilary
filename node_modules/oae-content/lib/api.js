...
/**
 * Publishes a collaborative document. When a document is published the following happens:
 ...
 * this function deals with message coming from RabbitMQ. Producers of those messages
 * are expected to perform the necessary permissions checks. In the typical case
 * where Etherpad is submitting messages, the authorization happens by virtue of the app
            ...
            Etherpad.getHTML(contentObj.id, contentObj.etherpadPadId, function(err, html) {
                ...
                ContentDAO.Revisions.getRevision(contentObj.latestRevisionId, function(err, revision) {
                    if (err) {
                        ...
                        // the other one keeps idling (but NOT making any chances) for a while and then leaves as well. There is no
                        // need to generate another revision as we already have one with the latest HTML. We do however raise another
                        // event so we can generate an "editted document"-activity for this user as well
