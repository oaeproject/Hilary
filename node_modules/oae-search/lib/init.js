 ...
 *
 *  a) If you need to add data that is stored, but not indexed, you can do so using the `extra` schema property. The users resource does this with the publicAlias.
 *  b) If you need to add data that is indexed, but not stored, ES actually lets you just index arbitrary properties, and by default you can search on them
 *  c) If you need advanced control over a field, you can make schema changes on the fly on ES, and you can update the schemaExtension to reflect that for your deployment
 *
 ...
 *          {String}        schema.title                The title of the resource to display
 *          {Object}        schema.extra                A blob of JSON that can be stored (and not indexed) for this resource
