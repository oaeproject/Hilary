...
var _produceAllRoutes = function(activitySeed, actor, object, target, callback) {
    ...
    _produceRoutes(activitySeed, ActivityConstants.entityTypes.ACTOR, actor, function(err, actorRoutes, actorPropagation) {
        if (err) {
            return callback(err);
        }

        _produceRoutes(activitySeed, ActivityConstants.entityTypes.OBJECT, object, function(err, objectRoutes, objectPropagation) {
            if (err) {
                return callback(err);
            }

            _produceRoutes(activitySeed, ActivityConstants.entityTypes.TARGET, target, function(err, targetRoutes, targetPropagation) {
                if (err) {
                    return callback(err);
                }
 ...
 *
 * @param  {ActivitySeed}      activitySeed         The activity seed for the activity seed being routed
 ...
 * @api private
 */
var _produceRoutes = function(activitySeed, entityType, entity, callback) {
    if (!entity) {
    ...
    router(activitySeed, entityType, entity, function(err, routes, propagation) {
        if (err) {
            return callback(err);
        }
