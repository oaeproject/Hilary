...
// A variable we can use for lazy-loading the collabdoc.html template.
...
var generatePreviews = module.exports.generatePreviews = function(ctx, contentObj, callback) {
    ...
    RestAPI.Content.getRevisions(ctx.tenantRestContext, contentObj.id, null, 2, function(err, revisions) {
        ...
        // Write the etherpad HTML to a file so we can generate a screenshot for it.
...
/**
 * Takes the provided HTML, wraps it in some outer HTML and writes it to disk.
 *
 * @param  {Context}    ctx             The context of the request to reprocess the previews
 * @param  {String}     etherpadHtml    The HTML to wrap and write.
 * @param  {Function}   callback        Standard callback method
 * @param  {Object}     callback.err    Standard error object, if any.
 * @param  {String}     callback.path   The path where the file has been written to.
 * @api private
 */
...
/**
 * Wraps the provided etherpad HTML with our own set of HTML in order to make
 * prettier previews.
 *
 * @param  {String}     etherpadHtml    The HTML as retrieved from the API.
 * @param  {Function}   callback        Standard callback method.
 * @param  {Object}     callback.err    Standard error object, if any.
 * @param  {String}     callback.html   Wrapped HTML.
 * @api private
 */
var _getWrappedEtherpadHtml = function(ctx, etherpadHtml, callback) {
    // The absolute path to the CSS file.
    var cssFile = Path.normalize(__dirname + '/../../../static/collabdoc/collabdoc.css');

    // The data that can be used in the collabdoc template.
    var templateData = {
        'ctx': ctx,
    ...
    if (wrapperHtml) {
        ...
        var htmlFile = Path.normalize(__dirname + '/../../../static/collabdoc/collabdoc.html');
        fs.readFile(htmlFile, function(err, content) {
            ...
            wrapperHtml = content.toString('utf8');
            callback(null, _.template(wrapperHtml, templateData));
        });
    }
};
