...
describe('Configuration', function() {
    ...
    describe('REST Configuration API', function() {
        ...
        it('verify get config global', function(callback) {
            ...
            RestAPI.Config.getTenantConfig(globalAdminRestContext, null, function(err, config) {
                ...
                // Verify that a private value is present
                ...
                RestAPI.Config.getTenantConfig(anonymousGlobalRestContext, null, function(err, config) {
                    ...
                    // Verify that a private value is not present
        ...
        it('verify get config tenant', function(callback) {
            ...
            RestAPI.Config.getTenantConfig(camAdminRestContext, null, function(err, config) {
                ...
                assert.equal(config['oae-authentication']['twitter']['enabled'], true);
                // Verify that a suppressed value is present
                assert.equal(config['oae-mediacore']['mediacore']['collectionId'], '');
                // Verify that a private value is not present
                assert.ok(!config['oae-content']['storage'].elements, true);
                ...
                RestAPI.Config.getTenantConfig(johnRestContext, null, function(err, config) {
                    ...
                    RestAPI.Config.getTenantConfig(anonymousCamRestContext, null, function(err, config) {
                        ...
                        assert.equal(config['oae-authentication']['twitter']['enabled'], true);
                        // Verify that a suppressed value is not present
                        assert.equal(config['oae-mediacore']['mediacore']['collectionId'], null);
                        // Verify that a private value is not present
                        assert.ok(!config['oae-content']['storage'].elements, true);
                        callback();
                    });
        ...
        /**
         * Test that verifies config fields on admin can't be cleared
         */
        it('verify clear config global', function(callback) {
            RestAPI.Config.clearConfig(globalAdminRestContext, 'admin', ['oae-authentication/twitter/enabled'], function(err) {
                assert.ok(err);
         ...
         */
        it('verify clear config tenant', function(callback) {
        ...
        it('verify set config value global', function(callback) {
            // Make a change on the global admin as a global admin
            ConfigTestUtil.updateConfigAndWait(globalAdminRestContext, null, {'oae-authentication/twitter/enabled': false}, function(err) {
                ...
                RestAPI.Config.getTenantConfig(globalAdminRestContext, null, function(err, config) {
                    ...
                    RestAPI.Config.getTenantConfig(camAdminRestContext, null, function(err, config) {
                        ...
                        ConfigTestUtil.updateConfigAndWait(globalAdminRestContext, null, {'oae-tenants/tenantprivacy/tenantprivate': true}, function(err) {
                            assert.ok(!err);
                            ...
                            RestAPI.Config.getTenantConfig(globalAdminRestContext, null, function(err, config) {
                                assert.ok(!err);
                                assert.ok(config);
                                assert.equal(config['oae-tenants']['tenantprivacy']['tenantprivate'], true);
                                ...
                                RestAPI.Config.getTenantConfig(camAdminRestContext, null, function(err, config) {
                                    ...
                                    RestAPI.Config.getTenantConfig(johnRestContext, null, function(err, config) {
                                        assert.ok(!err);
                                        assert.ok(config);
                                        assert.equal(config['oae-tenants']['tenantprivacy']['tenantprivate'], null);

                                        // Reset the configuration value for next tests
                                        ConfigTestUtil.updateConfigAndWait(globalAdminRestContext, null, {'oae-tenants/tenantprivacy/tenantprivate': false}, function(err) {
                                            assert.ok(!err);
                                            callback();
                                        });
                                    });
        ...
        it('verify set config value tenant', function(callback) {
            // Make a change on the tenant admin as a tenant admin
            ConfigTestUtil.updateConfigAndWait(camAdminRestContext, null, {'oae-authentication/twitter/enabled': true}, function(err) {
                ...
                RestAPI.Config.getTenantConfig(camAdminRestContext, null, function(err, config) {
                    ...
                    RestAPI.Config.getTenantConfig(globalAdminRestContext, global.oaeTests.tenants.cam.alias, function(err, config) {
                        ...
                        assert.ok(config);
                        assert.equal(config['oae-authentication']['twitter']['enabled'], true);
                        assert.equal(config['oae-tenants']['tenantprivacy']['tenantprivate'], false);

                        // Set a new value for a suppressed config value
                        ConfigTestUtil.updateConfigAndWait(camAdminRestContext, null, {'oae-tenants/tenantprivacy/tenantprivate': true}, function(err) {
                            assert.ok(!err);
                            ...
                            RestAPI.Config.getTenantConfig(camAdminRestContext, null, function(err, config) {
                                ...
                                RestAPI.Config.getTenantConfig(johnRestContext, null, function(err, config) {
                                    ...
                                    RestAPI.Config.getTenantConfig(globalAdminRestContext, null, function(err, config) {
                                        ...
                                        // Clear the tenant config to prevent interference with other tests
                                        ConfigTestUtil.clearConfigAndWait(camAdminRestContext, null, ['oae-authentication/twitter/enabled', 'oae-tenants/tenantprivacy/tenantprivate'], function(err) {
                                            assert.ok(!err);
                                            callback();
                                        });
                                    });
        ...
        it('verify set tenant config value global', function(callback) {
            // Make a change on a tenant admin as a global admin
                                        ...
                                        ConfigTestUtil.updateConfigAndWait(globalAdminRestContext, global.oaeTests.tenants.cam.alias, {'oae-authentication/local/enabled': 'true'}, function(err) {
                                            ...
                                            RestAPI.Config.getTenantConfig(globalAdminRestContext, global.oaeTests.tenants.cam.alias, function(err, config) {
                                                ...
                                                RestAPI.Config.getTenantConfig(globalAdminRestContext, global.oaeTests.tenants.cam.alias, function(err, config) {
                                                    ...
                                                    ConfigTestUtil.updateConfigAndWait(globalAdminRestContext, null, {'oae-authentication/signed/expires': '1'}, function(err) {
                                                        assert.ok(!err);
                                                        RestAPI.Config.getTenantConfig(globalAdminRestContext, null, function(err, config) {
                                                            assert.ok(!err);
                                                            assert.ok(config);
                                                            ...
                                                            ConfigTestUtil.updateConfigAndWait(globalAdminRestContext, null, {'oae-authentication/signed/expires': '0'}, function(err) {
                                                                assert.ok(!err);
                                                                RestAPI.Config.getTenantConfig(globalAdminRestContext, null, function(err, config) {
                                                                    assert.ok(!err);
                                                                    assert.ok(config);
                                                                    ...
                                                                    ConfigTestUtil.updateConfigAndWait(globalAdminRestContext, null, {'oae-authentication/signed/expires': originalValue}, function(err) {
                                                                        assert.ok(!err);
                                                                        callback();
                                                                    });
                                                                });
                                ...
                                ConfigTestUtil.updateConfigAndWait(anonymousCamRestContext, null, {'oae-authentication/twitter/enabled': 'moops'}, function(err) {
                                    ...
                                    ConfigTestUtil.updateConfigAndWait(anonymousGlobalRestContext, null, {'oae-authentication/twitter/enabled': 'moops'}, function(err) {
                                        ...
                                        ConfigTestUtil.updateConfigAndWait(johnRestContext, null, {'oae-content/storage/amazons3-access-key': 'moops'}, function(err) {
                                            ...
                                            ConfigTestUtil.updateConfigAndWait(camAdminRestContext, null, {'oae-content/storage/backend': 'local'}, function(err) {
                                                assert.ok(err);
                                                assert.equal(err.code, 401);
            ...
            RestAPI.Config.getSchema(globalAdminRestContext, function(err, schema) {
                ...
                ConfigTestUtil.updateConfigAndWait(globalAdminRestContext, global.oaeTests.tenants.cam.alias, {'oae-principals/group/visibility': 'private'}, function(err) {
                    ...
                    RestAPI.Config.getTenantConfig(globalAdminRestContext, global.oaeTests.tenants.cam.alias, function(err, config) {
                        ...
                        // Clear the config field's value so we don't impact the other tests
        ...
        it('verify radio field', function(callback) {
            ...
            RestAPI.Config.getSchema(globalAdminRestContext, function(err, schema) {
                ...
                ConfigTestUtil.updateConfigAndWait(globalAdminRestContext, null, {'oae-content/storage/backend': 'amazons3'}, function(err) {
                    assert.ok(!err);
                    RestAPI.Config.getTenantConfig(globalAdminRestContext, null, function(err, config) {
                        assert.ok(!err);
                        assert.equal(config['oae-content']['storage']['backend'], 'amazons3');

                        // Reset the config so we don't impact the other tests
                        ConfigTestUtil.updateConfigAndWait(globalAdminRestContext, null, {'oae-content/storage/backend': 'local'}, function(err) {
                            assert.ok(!err);
                            RestAPI.Config.getTenantConfig(globalAdminRestContext, null, function(err, config) {
                                assert.ok(!err);
    ...
    describe('Configuration options', function() {
        ...
        /**
         * Test that verifies that setting `valueVisibility: private` and `tenantOverride: true` on a configuration value makes sure only the
         * global admin can view this value
         */
        it('verify value with `valueVisibility: private` and `tenantOverride: true` can only be viewed by the global admin', function(callback) {
            // Assert that `valueVisibility` and `tenantOverride` are set for the configuration we'll be using in the test
            RestAPI.Config.getSchema(globalAdminRestContext, function(err, schema) {
                assert.equal(schema['oae-principals']['recaptcha'].elements['privateKey'].valueVisibility, 'private');
                assert.equal(schema['oae-principals']['recaptcha'].elements['privateKey'].tenantOverride, true);

                // Private values don't return for anonymous users
                RestAPI.Config.getTenantConfig(anonymousCamRestContext, null, function(err, config) {
                    assert.ok(!err);
                    assert.equal(config['oae-principals']['recaptcha']['privateKey'], null);

                    // Private values don't return for logged in users
                    RestAPI.Config.getTenantConfig(johnRestContext, null, function(err, config) {
                        assert.ok(!err);
                        assert.equal(config['oae-principals']['recaptcha']['privateKey'], null);

                        // Private values don't return for tenant administrators
                        RestAPI.Config.getTenantConfig(camAdminRestContext, null, function(err, config) {
                            assert.ok(!err);
                            assert.equal(config['oae-principals']['recaptcha']['privateKey'], '');

                            // Private values return for global administrators on the global server
                            RestAPI.Config.getTenantConfig(globalAdminRestContext, null, function(err, config) {
                                assert.ok(!err);
                                assert.equal(config['oae-principals']['recaptcha']['privateKey'], '6LcFWdYSAAAAANrHjt2Y5VJXoICHa95PFDarVcGs');

                                // Private values return for global administrators on the tenant server
                                RestAPI.Config.getTenantConfig(globalAdminRestContext, global.oaeTests.tenants.cam.alias, function(err, config) {
                                    assert.ok(!err);
                                    assert.equal(config['oae-principals']['recaptcha']['privateKey'], '6LcFWdYSAAAAANrHjt2Y5VJXoICHa95PFDarVcGs');
                                    callback();
        ...
        /**
         * Test that verifies that setting `valueVisibility: private` and `tenantOverride: false` on a configuration value makes sure no one except the global administrator
         * can override the value
         */
        it('verify value with `valueVisibility: private` and `tenantOverride: false` can not be overridden on the tenant except by global administrators', function(callback) {
            // Assert that `valueVisibility` and `tenantOverride` are set for the configuration we'll be using in the test
            RestAPI.Config.getSchema(globalAdminRestContext, function(err, schema) {
                assert.equal(schema['oae-content']['storage'].elements['amazons3-access-key'].valueVisibility, 'private');
                assert.equal(schema['oae-content']['storage'].elements['amazons3-access-key'].tenantOverride, false);

                // Try overriding a private configuration value as a tenant admin on the tenant server
                ConfigTestUtil.updateConfigAndWait(camAdminRestContext, null, {'oae-tenants/storage/amazons3-access-key': 'newamazonkey'}, function(err) {
                    assert.ok(err);
                    ...
                    RestAPI.Config.getTenantConfig(globalAdminRestContext, global.oaeTests.tenants.cam.alias, function(err, config) {
                        ...
                        // Try overriding a private configuration value as a logged in user on the tenant server
                        ConfigTestUtil.updateConfigAndWait(johnRestContext, null, {'oae-content/storage/amazons3-access-key': 'newamazonkey'}, function(err) {
                            assert.ok(err);
                            ...
                            RestAPI.Config.getTenantConfig(globalAdminRestContext, global.oaeTests.tenants.cam.alias, function(err, config) {
                                ...
                                // Try overriding a private configuration value as an anonymous user on the tenant server
                                ConfigTestUtil.updateConfigAndWait(anonymousCamRestContext, null, {'oae-content/storage/amazons3-access-key': 'newamazonkey'}, function(err) {
                                    assert.ok(err);
                                    ...
                                    RestAPI.Config.getTenantConfig(globalAdminRestContext, global.oaeTests.tenants.cam.alias, function(err, config) {
                                        ...
                                        // Try overriding a private configuration value as the global administrator on the tenant server
                                        ConfigTestUtil.updateConfigAndWait(globalAdminRestContext, global.oaeTests.tenants.cam.alias, {'oae-content/storage/amazons3-access-key': 'newamazonkey'}, function(err) {
                                            assert.ok(!err);

                                            // Verify that the configuration has changed
                                            RestAPI.Config.getTenantConfig(globalAdminRestContext, global.oaeTests.tenants.cam.alias, function(err, config) {
                                                assert.ok(!err);
                                                assert.equal(config['oae-content']['storage']['amazons3-access-key'], 'newamazonkey');

                                                // Clear the config for next tests
                                                ConfigTestUtil.clearConfigAndWait(globalAdminRestContext, global.oaeTests.tenants.cam.alias, ['oae-content/storage/amazons3-access-key'], function(err) {
                                                    assert.ok(!err);
                                                    callback();
                                                });
                                            });
        ...
        /**
         * Test that verifies that setting `valueVisibility: private` and `tenantOverride: true` on a configuration value still allows tenant admins to
         * override the value with their own configuration
         */
        it('verify value with `valueVisibility: private` and `tenantOverride: true` still allows tenant admins to override with their own values', function(callback) {
            // Assert that `valueVisibility` and `tenantOverride` are set for the configuration we'll be using in the test
            RestAPI.Config.getSchema(globalAdminRestContext, function(err, schema) {
                assert.equal(schema['oae-mediacore']['mediacore'].elements['collectionId'].valueVisibility, 'private');
                assert.equal(schema['oae-mediacore']['mediacore'].elements['collectionId'].tenantOverride, true);

                // Set a global admin value for the mediacore collection ID
                ConfigTestUtil.updateConfigAndWait(globalAdminRestContext, null, {'oae-mediacore/mediacore/collectionId': 'globaladmincollectionid'}, function(err) {
                    assert.ok(!err);

                    // Verify that the configuration is returning for the tenant looking at the tenant configuration on the global admin server
                    RestAPI.Config.getTenantConfig(globalAdminRestContext, global.oaeTests.tenants.cam.alias, function(err, config) {
                        assert.ok(!err);
                        assert.equal(config['oae-mediacore']['mediacore']['collectionId'], 'globaladmincollectionid');

                        // Verify that private configuration isn't returned for tenant admins
                        RestAPI.Config.getTenantConfig(camAdminRestContext, null, function(err, config) {
                            assert.ok(!err);
                            assert.equal(config['oae-mediacore']['mediacore']['collectionId'], '');

                            // Override a private configuration value as a tenant admin on the tenant server
                            ConfigTestUtil.updateConfigAndWait(camAdminRestContext, null, {'oae-mediacore/mediacore/collectionId': 'newcollectionid'}, function(err) {
                                assert.ok(!err);

                                // Verify that the configuration has changed and is returning for the tenant admin
                                RestAPI.Config.getTenantConfig(globalAdminRestContext, global.oaeTests.tenants.cam.alias, function(err, config) {
                                    assert.ok(!err);
                                    assert.equal(config['oae-mediacore']['mediacore']['collectionId'], 'newcollectionid');
                                    callback();
                                });
                            });
                        });
        ...
        /**
         * Test that verifies that setting `valueVisibility: admin` and `tenantOverride: true` on a configuration value makes sure only the
         * global and tenant administrators can view this value
         */
        it('verify value with `valueVisibility: admin` and `tenantOverride: true` can only be viewed by the global and tenant admins', function(callback) {
            // Assert that `valueVisibility` and `tenantOverride` are set for the configuration we'll be using in the test
            RestAPI.Config.getSchema(globalAdminRestContext, function(err, schema) {
                assert.equal(schema['oae-tenants']['tenantprivacy'].elements['tenantprivate'].valueVisibility, 'admin');
                assert.equal(schema['oae-tenants']['tenantprivacy'].elements['tenantprivate'].tenantOverride, true);

                // Admin values don't return for anonymous users
                RestAPI.Config.getTenantConfig(anonymousCamRestContext, null, function(err, config) {
                    assert.ok(!err);
                    assert.equal(config['oae-tenants']['tenantprivacy']['tenantprivate'], null);

                    // Admin values don't return for logged in users
                    RestAPI.Config.getTenantConfig(johnRestContext, null, function(err, config) {
                        assert.ok(!err);
                        assert.equal(config['oae-tenants']['tenantprivacy']['tenantprivate'], null);

                        // Admin values return for tenant administrators
                        RestAPI.Config.getTenantConfig(camAdminRestContext, null, function(err, config) {
                            assert.ok(!err);
                            assert.equal(config['oae-tenants']['tenantprivacy']['tenantprivate'], false);

                            // Admin values return for global administrators on the global server
                            RestAPI.Config.getTenantConfig(globalAdminRestContext, null, function(err, config) {
                                assert.ok(!err);
                                assert.equal(config['oae-tenants']['tenantprivacy']['tenantprivate'], false);

                                // Admin values return for global administrators on the tenant server
                                RestAPI.Config.getTenantConfig(globalAdminRestContext, global.oaeTests.tenants.cam.alias, function(err, config) {
                                    assert.ok(!err);
                                    assert.equal(config['oae-tenants']['tenantprivacy']['tenantprivate'], false);
                                    callback();
        ...
        /**
         * Test that verifies that setting `valueVisibility: public` and `tenantOverride: true` on a configuration value makes sure
         * everyone can view the configuration value
         */
        it('verify value with `valueVisibility: public` and `tenantOverride: true` can be viewed by everyone', function(callback) {
            // Assert that `valueVisibility` and `tenantOverride` are set for the configuration we'll be using in the test
            RestAPI.Config.getSchema(globalAdminRestContext, function(err, schema) {
                assert.equal(schema['oae-authentication']['local'].elements['allowAccountCreation'].valueVisibility, 'public');
                assert.equal(schema['oae-authentication']['local'].elements['allowAccountCreation'].tenantOverride, true);

                // Public values return for anonymous users
                RestAPI.Config.getTenantConfig(anonymousCamRestContext, null, function(err, config) {
                    assert.ok(!err);
                    assert.equal(config['oae-authentication']['local']['allowAccountCreation'], true);

                    // Public values return for logged in users
                    RestAPI.Config.getTenantConfig(johnRestContext, null, function(err, config) {
                        assert.ok(!err);
                        assert.equal(config['oae-authentication']['local']['allowAccountCreation'], true);

                        // Public values return for tenant administrators
                        RestAPI.Config.getTenantConfig(camAdminRestContext, null, function(err, config) {
                            assert.ok(!err);
                            assert.equal(config['oae-authentication']['local']['allowAccountCreation'], true);

                            // Public values return for global administrators on the global server
                            RestAPI.Config.getTenantConfig(globalAdminRestContext, null, function(err, config) {
                                assert.ok(!err);
                                assert.equal(config['oae-authentication']['local']['allowAccountCreation'], true);

                                // Public values return for global administrators on the tenant server
                                RestAPI.Config.getTenantConfig(globalAdminRestContext, global.oaeTests.tenants.cam.alias, function(err, config) {
                                    assert.ok(!err);
                                    assert.equal(config['oae-authentication']['local']['allowAccountCreation'], true);
                                    callback();
