...
describe('Profiles', function() {
    ...
    var setSection = function(context, principal_id, section, visibility, data, overwrite, callback) {
        login(context.user(), function(err, response, body) {
            ...
            // Set the section.
            request.post({
                'url': 'http://localhost:2001/api/user/' + principal_id + '/profile',
                'method': 'POST',
    ...
    var getSection = function(context, principal_id, section, callback) {
        login(context.user(), function(err, response, body) {
            assert.ok(!err);
            request.get('http://localhost:2001/api/user/' + principal_id + '/profile/' + section, function(err, response, body) {
                handleHttpResponse(err, response, body, callback);
            });
        });
    };
    ...
    var getAllSections = function(context, principal_id, callback) {
        login(context.user(), function(err, response, body) {
            assert.ok(!err);
            request.get('http://localhost:2001/api/user/' + principal_id + '/profile/', function(err, response, body) {
                handleHttpResponse(err, response, body, callback);
            });
        });
    };
