 ...
 * @param  {String} config.pdf.binary       The path to the `pdftk` executable. This should either be a direct path or the filename that's on the `PATH` environment.
 * @param  {Number} config.pdf.timeout      Specifies the time (in ms) when the process is considered to be hanging and should be killed. (Default: 120000ms)
 */
var init = module.exports.init = function(config, callback) {
    if (!config || !config.pdf.binary || !config.pdf.timeout) {
...
var _splitPDF = function(ctx, path, pagesDir, callback) {
    ...
    var child = exec(cmd, { 'timeout': _pdftkTimeout }, function (err, stdout, stderr) {
        if (err) {
