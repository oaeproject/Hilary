...
var _search = function(ctx, opts, callback) {
    ...
    if (queryParam === SearchConstants.query.ALL) {
        ...
        if (_includesResourceType(opts, 'content')) {
            query.bool.should.push(SearchUtil.createHasChildQuery('content_comment', SearchUtil.createQueryStringQuery(opts.q, ['body^0.5']), 'max'));
        }

        // For discussions, include their messages
        if (_includesResourceType(opts, 'discussion')) {
            query.bool.should.push(SearchUtil.createHasChildQuery('discussion_message', SearchUtil.createQueryStringQuery(opts.q, ['body^0.5']), 'max'));
        }
    }
    ...
    var accessFilter = (ctx.user() && ctx.user().isTenantAdmin(ctx.tenant().alias)) ?
            SearchUtil.filterTerm('tenantAlias', ctx.tenant().alias) : SearchUtil.createHasChildQuery(SearchConstants.resourceMembersMappingName, SearchUtil.filterTerms('direct_members', opts.access));
