 ...
 * * `updatedContentPreview(content)`: A content item's preview items have been updated
 */
 ...
 */
var createLink = module.exports.createLink = function(ctx, displayName, description, visibility, link, additionalMembers, folderIds, callback) {
    ...
    _createContent(ctx, contentId, revisionId, 'link', displayName, description, visibility, additionalMembers, folderIds, {'link': link}, {}, function(err, content, revision) {
        if (err) {
            return callback(err);
        }

        ContentAPI.emit(ContentConstants.events.CREATED_CONTENT, ctx, content, revision, additionalMembers, folderIds);
 ...
 */
var createFile = module.exports.createFile = function(ctx, displayName, description, visibility, file, additionalMembers, folderIds, callback) {
    ...
    return _createFile(ctx, displayName, description, visibility, file, additionalMembers, folderIds, cleanUpCallback);
};
 ...
 * @api private
 */
var _createFile = function(ctx, displayName, description, visibility, file, additionalMembers, folderIds, callback) {
    callback = callback || function() {};
    ...
    ContentUtil.getStorageBackend(ctx).store(ctx, file, options, function(err, uri) {
        ...
        _createContent(ctx, contentId, revisionId, 'file', displayName, description, visibility, additionalMembers, folderIds, otherValues, revisionData, function(err, contentObj, revision) {
            if (err) {
                return callback(err);
            }
            ...
            ContentAPI.emit(ContentConstants.events.CREATED_CONTENT, ctx, contentObj, revision, additionalMembers, folderIds);
            return callback(null, contentObj);
        });
    });
};
 ...
 */
var createCollabDoc = module.exports.createCollabDoc = function(ctx, displayName, description, visibility, additionalMembers, folderIds, callback) {
    callback = callback || function() {};
    ...
    Etherpad.createPad(contentId, function(err, ids) {
        ...
        _createContent(ctx, contentId, revisionId, 'collabdoc', displayName, description, visibility, additionalMembers, folderIds, ids, {}, function(err, content, revision) {
            if (err) {
                return callback(err);
            }
 ...
 * @api private
 */
var _createContent = function(ctx, contentId, revisionId, resourceSubType, displayName, description, visibility, additionalMembers, folderIds, otherValues, revisionData, callback) {
    callback = callback || function() {};
    ...
    PrincipalsDAO.getPrincipals(additionalMemberIds, null, function(err, principals) {
        ...
        AuthzAPI.canInteract(ctx, ctx.tenant().alias, principals, function(err, canInteract, illegalPrincipalIds) {
            ...
            _getFolders(ctx, folderIds, function(err, folders) {
                if (err) {
                    return callback(err);
                }
                    ...
                    _setContentPermissions(ctx, contentObj, additionalMembers, function(err, contentObj) {
                        ...
                        ContentMembersLibrary.insert(contentObj, allMembers, function(err) {
                            ...
                            _addContentItemToFolders(ctx, contentObj, folders, function(err) {
                                if (err) {
                                    log().warn({'err': err, 'contentId': contentObj.id, 'folderIds': folderIds}, 'Could not add a content item to a set of folders');
                                }
 ...
 * @param  {String[]}       folderIds               The ids of the folders to check
 * @param  {Function}       callback                Standard callback function
 * @param  {Object}         callback.err            An error object, if any
 * @param  {Folder[]}       callback.folders        The basic folder objects for the given folder ids
 * @api private
 */
var _getFolders = function(ctx, folderIds, callback) {
    ...
    _.each(folderIds, function(folderId) {
        ...
        // Validate that we can interact with the tenant that contains the folder
        var folderTenantAlias = AuthzUtil.getResourceFromId(folderId).tenantAlias;
        var canInteract = TenantsUtil.canInteract(ctx.tenant().alias, folderTenantAlias);
        validator.check(canInteract, {'code': 400, 'msg': util.format('Folder "%s%" belongs to a tenant that cannot interact with this tenant', folderId)});
    });
...
var _addContentItemToFolders = function(ctx, content, folders, callback) {
    ...
    // as we'd get a dependency cycle otherwise
    var folder = folders.pop();
    require('oae-folders').addContentItemsToFolderImpl(folder, [content], function(err) {
        if (err) {
            return callback(err);
        }
...
var _canManageAll = function(ctx, contentItems, callback, _cannotManageContentItems) {
    ...
    ContentAuthz.canManageContent(ctx, contentItem, function(err, canManage) {
        if (err) {
            return callback(err);
        } else if (!canManage) {
