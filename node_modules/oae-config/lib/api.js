...
let ConfigDAO = require('./internal/dao.js');
 ...
 *
 */
let getTenantLogo = module.exports.getTenantLogo = function(ctx, tenantAlias, callback) {
    ...
    ConfigDAO.getTenantLogo(tenantAlias, (err, tenantLogo) => {
        if(err) {
            // TODO log here
            return callback(err);
        }

        return callback(null, tenantLogo);
    });
};
...
var _getAllPersistentTenantConfigs = function(callback) {
    Cassandra.runAutoPagedQuery('SELECT "tenantAlias", "configKey", "value", WRITETIME("value") FROM "Config"', null, function(err, rows) {
        if (err) {
            return callback(err);
        }
