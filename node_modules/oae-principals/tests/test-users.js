...
describe('Users', function() {
    ...
    describe('Create user', function() {
        ...
        it('verify the email address has to belong to the configured tenant\'s email domain when creating a local user', function(callback) {
            ...
            TenantsTestUtil.createTenantAndWait(globalAdminRestContext, tenantAlias, tenantAlias, tenantHost, {}, function(err) {
                ...
                ConfigTestUtil.updateConfigAndWait(globalAdminRestContext, tenantAlias, {'oae-principals/recaptcha/enabled': false}, function(err) {
                    ...
                    _verifyCreateUserWithEmailDomain(tenantHost, emailDomain1, true, function() {
                        _verifyCreateUserWithEmailDomain(tenantHost, emailDomain2, true, function() {
                            ...
                            RestAPI.Tenants.updateTenant(globalAdminRestContext, tenantAlias, {'emailDomain': emailDomain1}, function(err) {
                                ...
                                _verifyCreateUserWithEmailDomain(tenantHost, emailDomain1, true, function() {
                                    _verifyCreateUserWithEmailDomain(tenantHost, emailDomain2, false, function() {
                                        return callback();
                                    });
                                });
                            });
        ...
        /**
         * Verify a user can or cannot be created with an email that belongs to a given email domain
         *
         ...
         * @param  {Boolean}        expectSuccess       Whether or not the user should be created
         * @param  {Function}       callback            Standard callback function
         * @api private
         */
        var _verifyCreateUserWithEmailDomain = function(tenantHost, emailDomain, expectSuccess, callback) {
            var username = TestsUtil.generateTestUserId();
            var email = TestsUtil.generateTestEmailAddress(null, emailDomain);
            var anonymousRestContext = TestsUtil.createTenantRestContext(tenantHost);
            RestAPI.User.createUser(anonymousRestContext, username, 'password', 'Test User', email, {'visibility': 'public'}, function(err) {
                if (expectSuccess) {
                    assert.ok(!err);
                } else {
                    assert.ok(err);
