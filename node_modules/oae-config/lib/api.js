...
var clearConfig = module.exports.clearConfig = function(ctx, tenantAlias, configFields, callback) {
    ...
    for (var j = 0; j < configFields.length; j++) {
        ...
        if (!optionalKey) {
            // If we're clearing an entire element, we can simply delete the column
            queries.push({
                'query': 'DELETE ? FROM Config WHERE tenantAlias = ?',
                'parameters': [columnKey, tenantAlias]
            });
        } else {
            // If we're only clearing an optional key, we retrieve the full object, delete the key, stringify it and store it again
            var value = _.extend({}, cachedTenantConfig[tenantAlias][module][feature][element]);
            if (value[optionalKey]) {
                delete value[optionalKey];
            }
            var storageValues = {};
            storageValues[columnKey] = JSON.stringify(value);
