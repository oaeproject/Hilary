 ...
 * @param  {Function}   callback            Standard callback function
 * @throws {AssertionError}                 An assertion error is thrown if the library could not be checked or it was stale
 */
var assertNotStale = module.exports.assertNotStale = function(indexName, libraryId, visibility, callback) {
    LibraryAPI.Index.isStale(indexName, libraryId, visibility, function(err, isStale) {
        assert.ok(!err);
        assert.ok(!isStale);
        return callback();
    });
