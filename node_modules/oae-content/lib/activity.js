 ...
 */
ContentAPI.on(ContentConstants.events.UPDATED_CONTENT_BODY, function(ctx, newContentObj, oldContentObj) {
    var millis = Date.now();
    var actorResource = new ActivityModel.ActivitySeedResource('user', ctx.user().id, {'user': ctx.user()});
...
ActivityAPI.registerActivityRouter('content-comment', function(activityType, entityType, entity, callback) {
    ...
    _routeContent(activityType, entityType, entity.contentId, entity.contentVisibility, function(err, routes, propagation) {
        if (err) {
            return callback(err);
        }
...
var _routeContent = function(activityType, entityType, contentId, visibility, callback) {
    ...
    ActivityUtil.getAllAuthzMembersByRole(contentId, function(err, specify) {
        ...
        var isUpdateActivity = (activityType === ContentConstants.activity.ACTIVITY_CONTENT_UPDATE || activityType === ContentConstants.activity.ACTIVITY_CONTENT_REVISION || activityType === ContentConstants.activity.ACTIVITY_CONTENT_RESTORED_REVISION);
