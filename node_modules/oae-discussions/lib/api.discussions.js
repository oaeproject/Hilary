...
var createDiscussion = module.exports.createDiscussion = function(ctx, displayName, description, visibility, members, opts, callback) {
    ...
    _getExistingPrincipals(memberIds, function(err, principals) {
        if (err) {
            return callback(err);
        }
        ...
        var targets = {
            'principalRoles': _.map(members, function(role, memberId) {
                return {
                    'principal': principalsById[memberId],
                    'role': role
                };
            })
        };
...
var shareDiscussion = module.exports.shareDiscussion = function(ctx, discussionId, principalIds, callback) {
    ...
    _getDiscussion(discussionId, function(err, discussion) {
        ...
        _getExistingPrincipals(principalIds, function(err, principals) {
            if (err) {
                return callback(err);
            }

            // Ensure we only share with those that we are able to share with and don't already have
            // roles
            AuthzPermissions.canShare(ctx, discussion, {'principals': principals}, function(err, sharingPrincipals) {
                if (err) {
                    return callback(err);
                }

                var sharingPrincipalIds = _.pluck(sharingPrincipals, 'id');

                // Apply the membership updates
                var roleChanges = _.chain(sharingPrincipalIds)
                    .invert()
                    .mapObject(function(val) {
                        return AuthzConstants.role.MEMBER;
                    })
                    .value();
                ...
                AuthzAPI.updateRoles(discussionId, roleChanges, function(err) {
                    ...
                    OaeUtil.invokeIfNecessary(_testDiscussionUpdateThreshold(discussion), _touch, discussion, function(err, touchedDiscussion) {
                        if (err) {
                            log().warn({
                                'err': err,
                                'principalIds': sharingPrincipalIds,
                                'discussionId': discussion.id
                        ...
                        _insertLibrary(sharingPrincipalIds, discussion, function(err) {
                            if (err) {
                                log().warn({
                                    'err': err,
                                    'principalIds': sharingPrincipalIds,
                            ...
                            DiscussionsAPI.emit(DiscussionsConstants.events.UPDATED_DISCUSSION_MEMBERS, ctx, discussion, roleChanges, sharingPrincipalIds);
                            return callback();
                        });
                    });
                });
...
var setDiscussionPermissions = module.exports.setDiscussionPermissions = function(ctx, discussionId, permissionChanges, callback) {
    ...
    _getDiscussion(discussionId, function(err, discussion) {
        ...
        _getExistingPrincipals(_.keys(permissionChanges), function(err, principals) {
            if (err) {
                return callback(err);
            }

            var principalRoles = _.map(permissionChanges, function(role, principalId) {
                return {
                    'principal': _.findWhere(principals, {'id': principalId}),
                    'role': role
                };
            });

            AuthzPermissions.canSetRoles(ctx, discussion, {'principalRoles': principalRoles}, function(err, membershipAfterChanges, newMembers, updatedMembers, removedMembers) {
                if (err) {
                    return callback(err);
                }

                var newMemberIds = _.pluck(newMembers, 'id');
                var updatedMemberIds = _.pluck(updatedMembers, 'id');
                var removedMemberIds = _.pluck(removedMembers, 'id');
...
var removeDiscussionFromLibrary = module.exports.removeDiscussionFromLibrary = function(ctx, libraryOwnerId, discussionId, callback) {
    ...
    _getDiscussion(discussionId, function(err, discussion) {
        ...
        PrincipalsDAO.getPrincipal(libraryOwnerId, function(err, principal) {
            ...
            AuthzPermissions.canRemoveRole(ctx, principal, discussion, function(err) {
                if (err) {
                    return callback(err);
                }
                ...
                AuthzAPI.updateRoles(discussionId, permissionChanges, function(err) {
                    ...
                    DiscussionsAPI.emit(DiscussionsConstants.events.UPDATED_DISCUSSION_MEMBERS, ctx, discussion, permissionChanges, [], [], [libraryOwnerId]);
...
/**
 * Get the principals identified by the given ids, ensuring that each principal existed
 *
 * @param  {String[]}       principalIds            The list of principal ids to fetch
 * @param  {Function}       callback                Standard callback function
 * @param  {Object}         callback.err            An error that occurred, if any
 * @param  {Principal[]}    callback.principals     The principals that were fetched in the same order as the provided list of ids
 * @api private
 */
var _getExistingPrincipals = function(principalIds, callback) {
    // Get the principals, ensuring each one exists
    PrincipalsDAO.getPrincipals(principalIds, null, function(err, principals) {
        if (err) {
            ...
            return callback({'code': 400, 'msg': 'One or more target principals do not exist'});
        }

        // Return the principals in the order they were given in the input array
        return callback(null, _.map(principalIds, _.propertyOf(principals)));
    });
};
