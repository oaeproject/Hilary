...
var _setContentPermissions = function(ctx, contentObj, newPermissions, callback) {
    ...
    PrincipalsUtil.getPrincipals(ctx, _.keys(newPermissions), function(err, principals) {
        ...
        for (var p in newPermissions) {
            if (newPermissions[p] === false) {
                removedMembers.push(p);
            } else if (!TenantsUtil.canInteract(contentObj.tenant, principals[p].tenant)) {
                // we shouldn't be sharing this with someone outside of this tenant
                return callback({'code': 400, 'msg': 'This content item cannot be shared with external user "' + principals[p].id + '".'})
            }
        }
