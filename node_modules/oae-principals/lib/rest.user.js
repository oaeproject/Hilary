...
var _getUsersDateLog = function(req, res) {
    PrincipalsAPI.getUsersDateLog(req.ctx, req.params.tenant, req.params.type, req.query.months, function(err, log) {
        if(err) {
            return res.status(err.code).send(err.msg);
        }

        fs.writeFile(__dirname + "/my_file.csv", log, {encoding: "utf8"}, function (err) {
            if (err) {
                return res.status(err.code).send(err.msg);
            }

            var downloadInfo = { filename: 'my_file.csv',
                                 strategy: { strategy: 'direct',
                                             target: __dirname + '/my_file.csv' } };

            var qs = Signature.createExpiringSignature(__dirname + '/my_file.csv', Date.now(), null);
            qs.uri = __dirname + '/my_file.csv';
            
            ContentAPI.verifySignedDownloadQueryString(req.ctx, qs, function(err, downloadInfo) {
                if (err) {
                    return res.status(err.code).send(err.msg);
                }
                ...
                // In all other cases we respond with a 404
                    ...
                    res.status(404).end();
                    // fs.unlinkSync(__dirname + "/my_file.csv");
                }
            });
        });
    });
...
OAE.globalAdminRouter.on('get', '/api/users/logs/:tenant/:type', _getUsersDateLog);
OAE.tenantRouter.on('get', '/api/users/logs/:tenant/:type', _getUsersDateLog);
