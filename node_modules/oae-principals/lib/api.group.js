 ...
 * @param  {Boolean}    callback.canManageAny       Whether or not the current user can manage any of the principals
 */
var sendRequestJoinGroup = module.exports.sendRequestJoinGroup = function(ctx, groupId, text, callback) {
    ...
    AuthzAPI.getAllAuthzMembers(groupId, function(err, memberIdRoles) {
        ...
        PrincipalsDAO.getPrincipal(groupId, function(err, group) {
            ...
            _.each(memberIdRoles, function(member, i, arrI) {

                if (member.role === AuthzConstants.role.MANAGER) {

                    // Get principal
                    PrincipalsDAO.getPrincipal(member.id, function(err, manager) {
                        if (err) {
                            return callback(err);
                        }
                        ...
                        if (i === arrI.length-1) { 
                            _sendRequestEmailToManager(ctx, group, text, managers, callback);
                        }
                    });
                } else {
 ...
 * @api private
 */
var _sendRequestEmailToManager = function(ctx, group, text, managers, callback) {

    var listEmail = [];

    // Write email
    _.each(managers, function(user, i, arrI) {
            
        // Generate a token if none was specified
        var token = ShortId.generate();

        // Store the token
        PrincipalsDAO.storeEmailToken(user.id, user.email, token, function(err) {
            if (err) {
                return callback(err);
            }

            var userToEmail = _.extend({}, user, {'email': user.email});
            var tenant = TenantsAPI.getTenant(user.tenant.alias);
            var groupIdUrl = group.id.split(':').join('/');
            var userIdUrl = user.id.split(':').join('/');
            var validateRequestUrl = TenantsUtil.getBaseUrl(tenant) + '/group/' + groupIdUrl + '/request-join/' + userIdUrl;
            ...
            EmailAPI.sendEmail('oae-principals', 'joinGroupRequest', userToEmail, dataEmail, {'hash': Date.now()});
            listEmail.push(user.id);
 ...
 * @param  {Object}     callback.err    An error that occurred, if any
 */
var joinGroupByRequest = module.exports.joinGroupByRequest = function(ctx, groupId, principalId, callback) {
    var validator = new Validator();
    validator.check(groupId, {'code': 400, 'msg': 'Invalid groupId specified'}).isGroupId();
    ...
    AuthzAPI.hasRole(principalId, groupId, AuthzConstants.role.MANAGER, function(err, isManager) {
        if (err) {
            return callback(err);
        } else if (!isManager) {
        ...
        PrincipalsDAO.getPrincipal(groupId, function(err, group) {
            ...
            PrincipalsDAO.getPrincipal(principalId, function(err, principal) {
                ...
                // Update role
                AuthzAPI.updateRoles(groupId, _.oaeObj(principalId, AuthzConstants.role.MEMBER), function(err, usersToInvalidate) {
                    if (err) {
                        return callback(err);
                    }

                    // Update libraries
                    AuthzPermissions.validateRoleChanges(ctx, group, [principal], {'promoteOnly': true}, function(err, memberChangeInfo, emailChangeInfo) {
                        if (err) {
                            return callback(err);
                        }
                        ...
                        LibraryAPI.Index.insert(PrincipalsConstants.library.MEMBERSHIPS_INDEX_NAME, [entry], callback);
