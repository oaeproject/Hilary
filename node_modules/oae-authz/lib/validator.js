...
/**
 * Checks whether or not the string in context is a valid principal id or email address
 *
 * Usage:
 * ```
 * var validator = new Validator();
 * validator.check(idOrEmailAddress, error).isPrincipalIdOrEmailAddress();
 * ```
 */
Validator.prototype.isPrincipalIdOrEmailAddress = function() {
    if (!this.str) {
        this.error(this.msg || 'An empty value was provided');
    } else if (!(AuthzUtil.isGroupId(this.str) || AuthzUtil.isUserId(this.str)) && !this.isEmail(this.str)) {
        this.error(this.msg || 'A principal id or email address must be provided');
    }
};
...
Validator.prototype.isValidRole = function() {
    if (!this.str || !_.isFunction(this.str.trim) || _.isEmpty(this.str.trim())) {
