...
var rowToUser = function(row) {
    ...
    var user = new User(hash.tenant, hash.principalId, hash.visibility, hash.locale, timezone, hash.displayName, isGlobalAdmin, isTenantAdmin);

    // The row might contain extra data that we wish to pass back.
    user.extra = {};
    var keys = _.keys(hash);
    var blacklist = ['tenant', 'principalId', 'username', 'password', 'visibility', 'locale', 'timezone', 'displayName', 'loginId', 'admin:global', 'admin:tenant'];
