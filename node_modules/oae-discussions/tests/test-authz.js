...
describe('Discussions Authz', function() {
    ...
    describe('#canRemoveDiscussionFromLibrary', function(callback) {

        /**
         * Test that verifies anonymous users cannot share a discussion
         */
        it('verifies anonymous cannot remove a discussion from a library', function(callback) {
            // The actual principalId doesn't matter, as it should know immediately that anonymous cannot share
            DiscussionsAPI.Authz.canRemoveDiscussionFromLibrary(anonymousContext, 'some-prinicpal-id', function(err, canRemove) {
                assert.ok(!err);
                assert.ok(!canRemove);
                return callback();
            });
    ...
    describe('#canDeleteDiscussionMessage', function(callback) {
        ...
        it('verifies anonymous cannot delete a discussion message', function(callback) {
            ...
            DiscussionsAPI.Authz.canPostDiscussionMessage(anonymousContext, {}, {}, function(err, canDelete) {
                assert.ok(!err);
                assert.ok(!canDelete);
                return callback();
            });
