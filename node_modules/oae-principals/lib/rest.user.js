...
OAE.tenantRouter.on('get', '/api/user/termsAndConditions', function(req, res) {
  var termsAndConditions = PrincipalsAPI.getTermsAndConditions(
    req.ctx,
    req.query.locale,
  );
 ...
 */
OAE.tenantRouter.on('post', '/api/user/:userId/termsAndConditions', function(
  req,
  res,
) {
  PrincipalsAPI.acceptTermsAndConditions(req.ctx, req.params.userId, function(
    err,
    user,
  ) {
    if (err) {
 ...
 */
OAE.globalAdminRouter.on('post', '/api/user/createGlobalAdminUser', function(
  req,
  res,
) {
...
var _handleCreateTenantAdminUser = function(req, res) {
  ...
  AuthenticationAPI.createTenantAdminUser(
    ctx,
    loginId,
    req.body.displayName,
    opts,
...
OAE.tenantRouter.on(
  'post',
  '/api/user/createTenantAdminUser',
  _handleCreateTenantAdminUser,
);
 ...
 * @HttpResponse                404                     A non-existing tenant was specified as the target for this user
 */
OAE.globalAdminRouter.on('post', '/api/user/:tenantAlias/create', function(
  req,
  res,
) {
  var loginId = new LoginId(
...
OAE.tenantRouter.on('post', '/api/user/create', function(req, res) {
  ...
  if (!opts.email) {
    return res
      .status(400)
      .send('A valid email address is required when creating a local account');
  }
  ...
  var createUser = function() {
    AuthenticationAPI.getOrCreateUser(
      ...
      req.body.displayName,
      opts,
      function(err, newUser, loginId, created) {
        if (err) {
          return res.status(err.code).send(err.msg);
        } else if (!created) {
          return res
  ...
  var needsTermsAndConditionsAgreement = PrincipalsConfig.getValue(
    tenant.alias,
    'termsAndConditions',
    'enabled',
  );
  ...
  var needsReCaptcha = PrincipalsConfig.getValue(
    tenant.alias,
    'recaptcha',
    'enabled',
  );
  ...
  var recaptchaPublicKey = PrincipalsConfig.getValue(
    tenant.alias,
    'recaptcha',
    'publicKey',
  );
  var recaptchaPrivateKey = PrincipalsConfig.getValue(
    tenant.alias,
    'recaptcha',
    'privateKey',
  );
...
var _handleUpdateUser = function(req, res) {
  PrincipalsAPI.updateUser(req.ctx, req.params.userId, req.body, function(
    err,
    user,
  ) {
    if (err) {
...
var _handleGetFullProfile = function(req, res) {
  PrincipalsAPI.getFullUserProfile(req.ctx, req.params.userId, function(
    err,
    user,
  ) {
    if (err) {
...
OAE.tenantRouter.on('get', '/api/user/:userId/memberships', function(req, res) {
  ...
  PrincipalsAPI.getMembershipsLibrary(
    ...
    req.query.start,
    limit,
    function(err, memberships, nextToken) {
      if (err) {
        return res.status(err.code).send(err.msg);
      }
      ...
      return res
        .status(200)
        .send({ results: memberships, nextToken: nextToken });
    },
  );
});
 ...
 */
OAE.tenantRouter.on('post', '/api/user/:userId/email/verify', function(
  req,
  res,
) {
  PrincipalsAPI.verifyEmail(
    req.ctx,
    req.params.userId,
    req.body.token,
    function(err, user) {
...
OAE.globalAdminRouter.on(
  'post',
  '/api/user/:userId/email/resend',
  _handleResendEmailToken,
);
OAE.tenantRouter.on(
  'post',
  '/api/user/:userId/email/resend',
 ...
 * @HttpResponse                404                         The specified user could not be found
 */
OAE.tenantRouter.on('get', '/api/user/:userId/groups/recent', function(
  req,
  res,
) {
  var limit = 5;
  PrincipalsAPI.getRecentGroupsForUserId(
    req.ctx,
    req.params.userId,
    limit,
...
var _getUsersForTenant = function(req, res) {
  PrincipalsAPI.getAllUsersForTenant(req.ctx, req.params.tenantAlias, function(
    err,
    users,
  ) {
    if (err) {
