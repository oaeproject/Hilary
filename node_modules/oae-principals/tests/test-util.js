...
describe('Principals', function() {
    ...
    describe('Utilities', function() {
        ...
        var createUsers = function(ctx, callback) {
            ...
            PrincipalsAPI.createUser(ctx, loginId(tenantId, 'nm417-' + r), 'password' + r, 'Nicolaas Matthijs', nmOpts, function(err, id) {
                assert.ok(!err);
                userIds.push(id);
                PrincipalsAPI.createUser(ctx, loginId(tenantId, 'simong-' + r), 'password' + r, 'Simon Gaeremynck', simonOpts, function(err, id) {
                    assert.ok(!err);
                    userIds.push(id);
                    PrincipalsAPI.createUser(ctx, loginId(tenantId, 'branden-' + r), 'password' + r, 'Branden Visser', brandenOpts, function(err, id) {
                        assert.ok(!err);
                        userIds.push(id);
                        PrincipalsAPI.createUser(ctx, loginId(tenantId, 'physx-' + r), 'password' + r, 'Bert Pareyn', bertOpts, function(err, id) {
                            assert.ok(!err);
                            userIds.push(id);
                            callback(userIds);
                        });
