 ...
 */
var getArrayParam = module.exports.getArrayParam = function(val, defaultVal) {
    ...
    val = _.compact(val);
    return (!_.isEmpty(val)) ? val : defaultVal;
};
 ...
 * @param  {Object[]}   [opts.sortBy]       A homogenous array (strings and objects) indicating how to sort resources. These sort items are secondary to `_score`, but take presedence over the natural `sort` sorting field of the resource documents. The objects are as per the ElasticSearch query sort API: http://www.elastic.co/guide/en/elasticsearch/reference/current/search-request-sort.html
...
var createQuery = module.exports.createQuery = function(query, filter, opts) {
    ...
    opts = {
        ...
        'sort': _.union(
            // Primary sort on search `_score`, always
            [{'_score': {'order': 'desc'}}],

            // Secondary until N-ary sort is on the specified `sortBy` array
            opts.sortBy,
 ...
 */
var createFilteredQuery = module.exports.createFilteredQuery = function(query, filter) {
    var filteredQuery = {
        'filtered': {
            'filter': filter
        }
...
/**
 * Create an ElasticSearch filter that results in no matches. Only useful for permission logic
 * to ensure negation.
 *
 * @return {Object}     An ElasticSearch filter that won't match any documents
 */
var filterFalse = module.exports.filterFalse = function() {
    return filterTerm('resourceType', '_');
};
...
var filterTerms = module.exports.filterTerms = function(field, values) {
    ...
    if (_.isArray(values)) {
        ...
        // We've been given an ElasticSearch terms lookup path, so apply that rather than a list of
        // values
 ...
 */
var filterResources = module.exports.filterResources = function(resourceTypes, deleted) {
    ...
    var filterDeleted = filterNot(filterExists('deleted'));
 ...
 * @param  {Boolean}    [opts.explicit]                 Whether or not the access filter should take into consideration explicit access. Default: `false`
 * @param  {Object}     [opts.resourceInfo]             Only applicable for `_share`. Specifies resource information needed to do a `_share` scope. If the scope is `_share` and this is not specified, the search will effectively become `_interact`
 * @param  {Boolean}    [opts.resourceInfo.canManage]   Specifies if the user in context can manage the resource being shared. If `true`, the search will operate identical to `_interact` as managers can break interaction boundaries for the resource
 * @param  {String}     [opts.resourceInfo.tenantAlias] Specifies the tenant alias of the resource being shared
 * @param  {String}     [opts.resourceInfo.visibility]  Specifies the visibility of the resource being shared
 * @param  {Function}   callback                        Standard callback function
 * @param  {Object}     callback.err                    An error that occurred, if any
 * @param  {Object}     callback.filter                 The ElasticSearch filter that can be used in the search query
 */
var filterScopeAndAccess = module.exports.filterScopeAndAccess = function(ctx, scope, opts, callback) {
    scope = getScopeParam(scope);
    opts = opts || {};
    opts.resourceInfo = opts.resourceInfo || {};
    ...
    OaeUtil.invokeIfNecessary(opts.explicit, filterExplicitAccess, ctx, function(err, explicitAccessFilter) {
        if (err) {
            return callback(err);
        }
        ...
        } else if (scope === SearchConstants.general.SCOPE_SHARE) {
            if (!user) {
                // Anonymous users cannot share with anyone, give an authorization error for this scenario
                return callback({'code': 401, 'msg': 'Anonymous users are not authorized to share'});
            }

            // Only users and groups are applicable targets for sharing
            var resourceTypeFilter = filterTerms('resourceType', [
                SearchConstants.general.RESOURCE_TYPE_GROUP,
                SearchConstants.general.RESOURCE_TYPE_USER
            ]);

            // When sharing, there is an "actor" (current user in context), the "resource" being
            // shared, and the "target" user or group who is being shared the resource. To filter
            // out "target" user search results we need to determine both: a) That the "actor" can
            // interact with the "target"; and b) when the "actor" is not a manager of the resource,
            // that the "target" can interact with the "resource"

            // Filter down to only users and groups that the user in context can interact with
            var actorInteractionFilter = filterOr(
                filterAnd(
                    implicitAccessFilter,
                    interactingTenantAliasesFilter,

                    // A user cannot interact with any private resource, and they should not have
                    // themself as an option either
                    (!user.isAdmin(user.tenant.alias)) ? filterNot(filterTerm('visibility', AuthzConstants.visibility.PRIVATE)) : null

                ),
                explicitAccessFilter
            );

            // Ensure the resource access information is properly specified
            var resourceInteractionFilter = null;
            var resourceCanManage = opts.resourceInfo.canManage;
            var resourceTenantAlias = opts.resourceInfo.tenantAlias;
            var resourceVisibility = opts.resourceInfo.visibility;
            if (!_.contains(AuthzConstants.visibility.ALL_PRIORITY, resourceVisibility)) {
                return callback({'code': 400, 'msg': 'A valid resource visibility must be specified to search for sharing'});
            } else if (!resourceTenantAlias) {
                return callback({'code': 400, 'msg': 'A valid resource tenant alias must be specified to search for sharing'});
            }

            // If the user in context cannot manage the resource, we must also filter down to only
            // target users and groups that can interact with the resource
            if (!resourceCanManage) {
                if (resourceVisibility === AuthzConstants.visibility.PRIVATE) {
                    // No one can implicitly interact with private resource
                    resourceInteractionFilter = filterFalse();
                } else if (resourceVisibility === AuthzConstants.visibility.LOGGEDIN) {
                    // Only users of the same tenant can implicitly interact with a loggedin resource
                    resourceInteractionFilter = filterTerm('tenantAlias', resourceTenantAlias);
                } else {
                    // For a public resource, all users in the tenant network can interact
                    var resourceInteractingTenantAliases = TenantsUtil.getAllTenantsForInteraction(resourceTenantAlias);
                    resourceInteractionFilter = filterTerms('tenantAlias', _.union([resourceTenantAlias], resourceInteractingTenantAliases));
                }
            }

            // Apply both the actor interaction filter and the resource interaction filter (if
            // applicable)
            filter = filterAnd(
                resourceTypeFilter,
                actorInteractionFilter,
                resourceInteractionFilter
            );
 ...
 */
var filterExplicitAccess = module.exports.filterExplicitAccess = function(ctx, callback) {
    var user = ctx.user();
    if (!user) {
    ...
    // TODO: Use the resource_memberships document in combination with a terms lookup filter
    // See: http://www.elasticsearch.org/guide/en/elasticsearch/reference/current/query-dsl-terms-filter.html
    AuthzAPI.getIndirectPrincipalMemberships(ctx.user().id, null, 10000, function(err, indirectGroupIds) {
        if (err) {
            return callback(err);
        }

        return callback(null, createHasChildQuery(
 ...
 * @return {Object}                     A Query object that can be used in the query portion of the ElasticSearch Query DSL
 */
var createHasChildQuery = module.exports.createHasChildQuery = function(type, childQuery, scoreType, boost) {
    if (!childQuery) {
        return null;
    }
