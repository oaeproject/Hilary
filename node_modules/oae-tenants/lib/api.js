...
var initializeTenantMiddleware = module.exports.initializeTenantMiddleware = function(callback) {
    ...
    // Get all tenants and cache them by their baseUrl
    getAllTenants(function(err, tenants) {
        ...
        for (var t = 0; t < tenants.length; t++) {
            runningServers[tenants[t].baseUrl] = tenants[t];
        }
        callback();
    });
};
...
var _stopTenant = function(tenant, callback) {
    if (tenant && runningServers[tenant.baseUrl]) {
        // Disable that server
        runningServers[tenant.baseUrl].disabled = true;
        callback();
    } else {
 ...
 */
var createTenant = module.exports.createTenant = function(ctx, alias, name, baseUrl, callback) {
    callback = callback || function() {};
    ...
    getTenantByAlias(alias, function(err, tenant) {
        if (!tenant) {
            tenant = new model.Tenant(alias, name, baseUrl, true, false);
            // no tenant by this alias or port exist, create one
            Cassandra.runQuery('UPDATE Tenant USING CONSISTENCY QUORUM SET name = ?, baseUrl = ?, active = ? WHERE alias = ?',
                [tenant.name, baseUrl, tenant.active, tenant.alias], function(err) {
                if (err) {
                    return callback(err);
                }
...
var mapToTenant = function(row, returnDeleted) {
    ...
    return new model.Tenant(hash.alias, hash.name, hash.baseUrl, hash.active, hash.deleted);
};
