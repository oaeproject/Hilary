...
/**
 * Check to see if a user can interact with a given list of resources
 *
 * @param  {Context}    ctx                                     Standard context object containing the current user and the current tenant
 * @param  {String}     resourceTenantAlias                     The tenant alias of the resource the user is performaning an action upon
 * @param  {Object[]}   targetResources                         The standard resources to which the user is trying to associate the resource (e.g., Adding a user to a group, or adding a content item to a folder)
...
var _canExplicitlyInteract = function(ctx, resourceTenantAlias, targetResources, callback, _illegalTargetResourceIds) {
    ...
    if (AuthzUtil.isUserId(targetResource.id)) {
        // You can always interact with guest users (as long as you're not on a private tenant)
        if (targetResource.guest) {
