        ...
        RestAPI.User.createUser(camAdminRestContext, doerUsername, 'password', 'Doer', null, function(err, _doerUser) {
            ...
            RestAPI.User.createUser(camAdminRestContext, publicUserMemberUsername, 'password', 'Public User Member', null, function(err, _publicUserMember) {
                ...
                RestAPI.User.createUser(camAdminRestContext, loggedinUserMemberUsername, 'password', 'Loggedin User Member', loggedinOpts, function(err, _loggedinUserMember) {
                    ...
                    // The loggedin user has an extra property, we need to verify this is visible and hidden in search results where appropriate
                    RestAPI.User.updateUser(camAdminRestContext, loggedinUserMember.id, {'extraProp': 'My extra prop'}, function(err) {
                        ...
                        RestAPI.User.createUser(camAdminRestContext, privateUserMemberUsername, 'password', 'Private User Member', privateOpts, function(err, _privateUserMember) {
                            ...
                            RestAPI.Group.createGroup(doerRestContext, targetPublicGroupAlias, targetPublicGroupAlias, targetPublicGroupAlias, 'public', 'no', [], [], function(err, _targetPublicGroup) {
                                assert.ok(!err);
                                targetPublicGroup = _targetPublicGroup;

                                RestAPI.Group.createGroup(doerRestContext, targetLoggedinGroupAlias, targetLoggedinGroupAlias, targetLoggedinGroupAlias, 'loggedin', 'no', [], [], function(err, _targetLoggedinGroup) {
                                    assert.ok(!err);
                                    targetLoggedinGroup = _targetLoggedinGroup;

                                    RestAPI.Group.createGroup(doerRestContext, targetPrivateGroupAlias, targetPrivateGroupAlias, targetPrivateGroupAlias, 'private', 'no', [], [], function(err, _targetPrivateGroup) {
                                        assert.ok(!err);
                ...
                RestAPI.Group.setGroupMembers(doerRestContext, targetPublicGroup.id, changes, function(err) {
                    ...
                    RestAPI.User.createUser(camAdminRestContext, janeUsername, 'password', 'Jane McJaneFace', null, function(err, jane) {
                        ...
                        SearchTestsUtil.searchAll(anonymousRestContext, 'members', [targetPublicGroup.id], null, function(err, results) {
                            ...
                            SearchTestsUtil.searchAll(darthVaderRestContext, 'members', [targetPublicGroup.id], {'_refresh': false}, function(err, results) {
                                assert.ok(!err);
                                var publicUserResult = _getDocById(results, publicUserMember.id);
                                var loggedinUserResult = _getDocById(results, loggedinUserMember.id);
                                var privateUserResult = _getDocById(results, privateUserMember.id);
                                ...
                                SearchTestsUtil.searchAll(janeRestContext, 'members', [targetPublicGroup.id], {'_refresh': false}, function(err, results) {
                                    assert.ok(!err);
                                    var publicUserResult = _getDocById(results, publicUserMember.id);
                                    var loggedinUserResult = _getDocById(results, loggedinUserMember.id);
                                    var privateUserResult = _getDocById(results, privateUserMember.id);
                                    ...
                                    SearchTestsUtil.searchAll(jackRestContext, 'members', [targetPublicGroup.id], {'_refresh': false}, function(err, results) {
                                        assert.ok(!err);
                                        var publicUserResult = _getDocById(results, publicUserMember.id);
                                        var loggedinUserResult = _getDocById(results, loggedinUserMember.id);
                                        var privateUserResult = _getDocById(results, privateUserMember.id);
                ...
                RestAPI.Group.setGroupMembers(doerRestContext, targetLoggedinGroup.id, changes, function(err) {
                    ...
                    RestAPI.User.createUser(camAdminRestContext, janeUsername, 'password', 'Jane McJaneFace', null, function(err, jane) {
                        ...
                        SearchTestsUtil.searchAll(anonymousRestContext, 'members', [targetLoggedinGroup.id], null, function(err, results) {
                            ...
                            SearchTestsUtil.searchAll(darthVaderRestContext, 'members', [targetLoggedinGroup.id], {'_refresh': false}, function(err, results) {
                                assert.ok(err);
                                assert.equal(err.code, 401);
                                assert.ok(!results);

                                // Verify results and visibility for loggedin user
                                SearchTestsUtil.searchAll(janeRestContext, 'members', [targetLoggedinGroup.id], {'_refresh': false}, function(err, results) {
                                    assert.ok(!err);
                                    var publicUserResult = _getDocById(results, publicUserMember.id);
                                    var loggedinUserResult = _getDocById(results, loggedinUserMember.id);
                                    var privateUserResult = _getDocById(results, privateUserMember.id);
                                    ...
                                    // Verify results and visibility for member user
                                    SearchTestsUtil.searchAll(jackRestContext, 'members', [targetLoggedinGroup.id], {'_refresh': false}, function(err, results) {
                                        assert.ok(!err);
                                        var publicUserResult = _getDocById(results, publicUserMember.id);
                                        var loggedinUserResult = _getDocById(results, loggedinUserMember.id);
                                        var privateUserResult = _getDocById(results, privateUserMember.id);
                ...
                RestAPI.Group.setGroupMembers(doerRestContext, targetPrivateGroup.id, changes, function(err) {
                    ...
                    RestAPI.User.createUser(camAdminRestContext, janeUsername, 'password', 'Jane McJaneFace', null, function(err, jane) {
                        ...
                        SearchTestsUtil.searchAll(anonymousRestContext, 'members', [targetPrivateGroup.id], null, function(err, results) {
                            ...
                            SearchTestsUtil.searchAll(darthVaderRestContext, 'members', [targetPrivateGroup.id], {'_refresh': false}, function(err, results) {
                                assert.ok(err);
                                assert.equal(err.code, 401);
                                assert.ok(!results);

                                // Verify loggedin user cannot see members of private group
                                SearchTestsUtil.searchAll(janeRestContext, 'members', [targetPrivateGroup.id], {'_refresh': false}, function(err, results) {
                                    assert.ok(err);
                                    assert.equal(err.code, 401);
                                    assert.ok(!results);
                                    
                                    // Verify results and visibility for member user
                                    SearchTestsUtil.searchAll(jackRestContext, 'members', [targetPrivateGroup.id], {'_refresh': false}, function(err, results) {
                                        assert.ok(!err);
                                        var publicUserResult = _getDocById(results, publicUserMember.id);
                                        var loggedinUserResult = _getDocById(results, loggedinUserMember.id);
                                        var privateUserResult = _getDocById(results, privateUserMember.id);
