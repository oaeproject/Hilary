...
module.exports = function(ctx, opts, callback) {
    ...
    if (ctx.user() && (ctx.user().isAdmin(authzPrincipal.tenantAlias) || ctx.user().id === opts.principalId)) {
        ...
        _search(ctx, true, opts, callback);
    } else if (ctx.user() && PrincipalsUtil.isGroup(opts.principalId)) {
        // If we're searching a group library, assume full access to all content in the group if the user is a member of the group
        AuthzAPI.hasAnyRole(ctx.user().id, opts.principalId, function(err, hasAnyRole) {
            if (err) {
                return callback(err);
            }
            _search(ctx, hasAnyRole, opts, callback);
        });
    } else {
        // Either we're anonymous or we're searching some other user's library
        _search(ctx, false, opts, callback);
    }
};
 ...
 * @param   {SearchResult}  callback.results    An object that represents the results of the query
 */
var _search = function(ctx, fullAccess, opts, callback) {
    var data = {};
    ...
    // Only look for content resources that are in the user's library
    var baseFilter = SearchUtil.filterAnd(
        SearchUtil.filterTerm('_type', SearchConstants.resourceMappingName),
        SearchUtil.filterTerm('resourceType', 'content'),
    ...
    if (fullAccess) {
        ...
        filter = SearchUtil.filterAnd(
            baseFilter,
            SearchUtil.filterOr(
                SearchUtil.filterTerm('visibility', ContentConstants.visibility.PUBLIC),
                SearchUtil.filterAnd(
                    SearchUtil.filterTerm('tenantAlias', ctx.tenant().alias),
                    SearchUtil.filterTerm('visibility', ContentConstants.visibility.LOGGEDIN)
                )
            )
        );
    } else {
        // For anonymous / cross-tenant users, only show the public library
        filter = SearchUtil.filterAnd(baseFilter, SearchUtil.filterTerm('visibility', ContentConstants.visibility.PUBLIC));
    }
