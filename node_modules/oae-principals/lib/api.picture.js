...
var _storePicture = function(ctx, principal, file, callback) {
    ...
    ImageUtil.autoOrient(file.path, {'removeInput': true}, function(err, orientedFile) {
        ...
        ImageUtil.convertToJPG(orientedFile.path, function(err, convertedFile) {
            ...
            var options = _getProfilePictureStorageOptions(principal.id, Date.now(), 'large', '.jpg');
            ContentUtil.getStorageBackend(ctx).store(ctx, convertedFile, options, function(err, largePictureUri) {
                ...
                PrincipalsDAO.updatePrincipal(principal.id, {'largePictureUri': largePictureUri}, function(err) {
                    ...
                    principal.picture.large = ContentUtil.getSignedDownloadUrl(ctx, largePictureUri);
                    return callback(null, principal);
                });
            });
        });
    ...
    ContentUtil.getStorageBackend(ctx, principal.picture.largeUri).get(ctx, principal.picture.largeUri, function(err, file) {
        ...
        ImageUtil.cropAndResize(file.path, selectedArea, sizes, function(err, files) {
            ...
            file.remove(function(removalError) {
                ...
                return _storeAndSave(ctx, principal, files, callback);
            });
        });
    });
};
...
var _storeAndSave = function(ctx, principal, files, callback) {
    ...
    backend.store(ctx, smallImage, options, function(err, smallPictureUri) {
        ...
        backend.store(ctx, mediumImage, options, function(err, mediumPictureUri) {
            ...
            return _saveUrisToCassandra(ctx, principal, smallPictureUri, mediumPictureUri, callback);
        });
    });
};
...
var _saveUrisToCassandra = function(ctx, principal, smallPictureUri, mediumPictureUri, callback) {
    ...
    PrincipalsDAO.updatePrincipal(principal.id, profileFields, function(err) {
        ...
        newPrincipal.picture.small = ContentUtil.getSignedDownloadUrl(ctx, smallPictureUri);
        newPrincipal.picture.medium = ContentUtil.getSignedDownloadUrl(ctx, mediumPictureUri);
