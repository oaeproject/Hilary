                ...
                _createContentAndWait('file', null, getVideoStream, function(restCtx, content) {
                    ...
                    RestAPI.Content.getRevision(restCtx, content.id, content.latestRevisionId, function(err, revision) {
                        ...
                        RestAPI.Zencoder.encodingComplete(restCtx, notification, function(err) {
                            assert.ok(!err);
                            RestAPI.Content.getPreviewItems(restCtx, content.id, revision.revisionId, function() {
                                assert.equal(content.previews.status, 'done');
                                assert.equal(content.previews.videoKey, util.format('%s/%s', content.id, revision.revisionId));
                                return callback();
                            });
                        });
                    });
