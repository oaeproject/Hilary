...
var SwaggerParam = require('./swaggerParamTypes');
...
var documentModule = module.exports.documentModule = function(module, callback) {
    var files = shell.find('node_modules/' + module + '/lib').filter(function(file) {
 ...
 *
 * @param  {String}          filename      The full path to the file to be documented
 ...
 * @api private
 */
var _register = function(filename, callback) {
        ...
        try {
            ...
            _.each(doc.endpoints, function(endpoint, nickname) {
                ...
                _.each(endpoint.pathParams, function(pathParam) {
                    endpoint.parameters.push(SwaggerParam.path(pathParam.name, pathParam.description, _convertRJDArrayDefToSwagger(pathParam.type), _constructSwaggerValue(pathParam.validValues), pathParam.defaultValue));
                });
                _.each(endpoint.queryParams, function(queryParam) {
                    endpoint.parameters.push(SwaggerParam.query(queryParam.name, queryParam.description, _convertRJDArrayDefToSwagger(queryParam.type), queryParam.required, queryParam.allowMultiple, _constructSwaggerValue(queryParam.validValues), queryParam.defaultValue));
                });
                _.each(endpoint.bodyParams, function(bodyParam) {
                    endpoint.parameters.push(SwaggerParam.body(bodyParam.name, bodyParam.description, _convertRJDArrayDefToSwagger(bodyParam.type), bodyParam.defaultValue));
                });
                _.each(endpoint.headerParams, function(headerParam) {
                    endpoint.parameters.push(SwaggerParam.header(headerParam.name, headerParam.description, _convertRJDArrayDefToSwagger(headerParam.type), headerParam.required));
                });
                _.each(endpoint.formParams, function(formParam) {
                    endpoint.parameters.push(SwaggerParam.form(formParam.name, formParam.description, _convertRJDArrayDefToSwagger(formParam.type), formParam.defaultValue));
                });
            ...
            log().warn('Could not parse restjsdoc in ' + filename + ' ' + error.message);
        }
        return callback();
    });
};
...
var _convertRJDArrayDefToSwagger = function(def) {
    if (def.match(/\[\]$/)) {
        def = 'List[' + def.substr(0, def.length - 2) + ']';
    }
    return def;
};
...
/**
 * Unwrap swagger `List[type]` to return just the `type` this is safe to use on non-lists as well
 *
