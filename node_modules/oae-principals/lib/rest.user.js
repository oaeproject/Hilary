...
OAE.tenantServer.post('/api/user/create', function(req, res) {
    ...
    var needsReCaptcha = PrincipalsConfig.getValue(req.ctx, 'recaptcha', 'enabled');
    ...
    recaptcha.checkAnswer(PrincipalsConfig.getValue(req.ctx, 'recaptcha', 'privateKey'), req.connection.remoteAddress, req.body.recaptchaChallenge, req.body.recaptchaResponse);
});
...
OAE.tenantServer.post('/api/user/:id/picture', function(req, res) {
    ...
    req.files = req.files || {};
    PrincipalsAPI.storePicture(req.ctx, req.params.id, req.files.file, function(err) {
        if (err) {
            return res.send(err.code, err.msg);
        }
