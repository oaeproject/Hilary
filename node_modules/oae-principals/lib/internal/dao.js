...
var getGroupFromRow = function(row) {
    ...
    _setPictureUriOnPrincipalObject(group, 'small', hash.smallPictureUri);
    _setPictureUriOnPrincipalObject(group, 'medium', hash.mediumPictureUri);
    _setPictureUriOnPrincipalObject(group, 'large', hash.largePictureUri);
...
var getUserFromRow = function(row) {
    var hash = Cassandra.rowToHash(row);
    hash.isGlobalAdmin = row.get('admin:global') && !(sanitize(row.get('admin:global').value).toBooleanStrict() === 'false');
    hash.isTenantAdmin = row.get('admin:tenant') && !(sanitize(row.get('admin:tenant').value).toBooleanStrict() === 'false');
    var user = hashToUser(hash);
...
var hashToUser = function(hash) {
    ...
    _setPictureUriOnPrincipalObject(user, 'small', hash.smallPictureUri);
    _setPictureUriOnPrincipalObject(user, 'medium', hash.mediumPictureUri);
    _setPictureUriOnPrincipalObject(user, 'large', hash.largePictureUri);
 ...
 * @api private
 */
var _setPictureUriOnPrincipalObject = function(principal, size, uri) {
