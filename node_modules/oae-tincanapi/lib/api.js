 ...
 */
var initializeTinCanAPI = (module.exports.initializeTinCanAPI = function(
  _config,
  callback,
) {
 ...
 * @api private
 */
var _processActivities = function(routedActivities) {
  ...
  _.each(routedActivities, function(streamTypeActivities, resourceId) {
    ...
    _.each(streamTypeActivities, function(activity, streamType) {
      ...
      if (
        ...
        var tenantAlias = activity.actor.user.tenant.alias;
        ...
        var actor = new TinCanModel.TinCanActor(
          activity.actor.user.displayName,
          homePage,
        );
        var verb = _mapVerb(activity.verb);
        var obj = new TinCanModel.TinCanObject(
        ...
        tenantStatements[tenantAlias].push(
          new TinCanModel.TinCanStatement(actor, verb, obj),
        );
      }
    });
  });
 ...
 */
var sendStatementsToLRS = (module.exports.sendActivitiesToLRS = function(
  statements,
  tenantAlias,
) {
  ...
  if (TinCanConfig.getValue(tenantAlias, 'lrs', 'enabled')) {
    ...
    request(options, function(err, response, body) {
      if (err) {
        log().error(
          { err: err, body: body },
          'An error ocurred whilst sending statements to the LRS',
        );
      }
    });
  }
