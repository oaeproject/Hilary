...
var _cacheTenants = function(callback) {
    ...
    let queryAllTenants = 'SELECT * FROM "Tenant"';
    Cassandra.runAutoPagedQuery(queryAllTenants, false, function(err, rows) {
        if (err) {
            return callback(err);
        }
...
var disableTenants = module.exports.disableTenants = function(ctx, aliases, disabled, callback) {
    ...
    Cassandra.runBatchQuery(queries, function(err) {
        ...
        async.mapSeries(aliases, function(eachAlias, transformed) {
            // disable or restore users from those tenancies too
            UserAPI.deleteOrRestoreUsersByTenancy(ctx, eachAlias, disabled, function(err) {
                if(err) {
                    transformed(err);
                } else {
                    // Indicate that a caching operation is pending
