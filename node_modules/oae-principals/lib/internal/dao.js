 ...
 * @api private
 */
var createRequestJoinGroup = module.exports.createRequestJoinGroup = function(principalId, groupId, role, callback) {
    var queries = [];

    getRequestsJoinGroup(groupId, function(err, requests) {
        if (err) {
            return callback(err);
        }
        ...
        if (!_.isEmpty(requests)) {
            list = JSON.parse(requests.principalId);
        }

        _addObjectToArray(principalId, role, list, function(newPrincipalString) {
        
            // Create query to insert into 'RequestByGroup'
            if (_.isEmpty(requests)) {
                queries.push({'query': 'INSERT INTO "RequestByGroup" ("groupId", "principalId") VALUES (?, ?)', 'parameters': [groupId, newPrincipalString]});
            } else {
                queries.push({'query': 'UPDATE "RequestByGroup" SET "principalId" = ? WHERE "groupId" = ?', 'parameters': [newPrincipalString, groupId]});
            }

            // Create query
            queries.push({'query': 'INSERT INTO "Request" ("principalId", "groupId", "role") VALUES (?, ?, ?)', 'parameters': [principalId, groupId, role]});
 ...
 * @api private
 */
var updateRequestJoinGroup = module.exports.updateRequestJoinGroup = function(principalId, groupId, role, callback) {
    var queries = [];
    ...
    getRequestsJoinGroup(groupId, function(err, requests) {
        ...
        _removeObjectToArray(principalId, JSON.parse(requests.principalId), function(newPrincipalList) {

            _addObjectToArray(principalId, role, JSON.parse(newPrincipalList), function(newPrincipalString) {

                // Create queries
                queries.push({'query': 'UPDATE "Request" SET "role" = ? WHERE "groupId" = ? AND "principalId" = ?', 'parameters': [role, groupId, principalId]});
                queries.push({'query': 'UPDATE "RequestByGroup" SET "principalId" = ? WHERE "groupId" = ?', 'parameters': [newPrincipalString, groupId]});
 ...
 * @api private
 */
var removeJoinGroupByRequest = module.exports.removeJoinGroupByRequest = function(principalId, groupId, callback) {
    var queries = [];
    ...
    getRequestsJoinGroup(groupId, function(err, requests) {
        ...
        _removeObjectToArray(principalId, JSON.parse(requests.principalId), function(newPrincipalString) {

            // Create queries
            queries.push({'query': 'DELETE FROM "Request" WHERE "groupId" = ? AND "principalId" = ?', 'parameters': [groupId, principalId]});
            queries.push({'query': 'UPDATE "RequestByGroup" SET "principalId" = ? WHERE "groupId" = ?', 'parameters': [newPrincipalString, groupId]});
 ...
 * @api private
 */
var getRequestJoinGroup = module.exports.getRequestJoinGroup = function(principalId, groupId, callback) {
    Cassandra.runQuery('SELECT * FROM "Request" WHERE "groupId" = ? AND "principalId" = ?', [groupId, principalId], function (err, row) {
        if (err) {
            return callback(err);
        }
        if (_.isEmpty(row)) {
        ...
        var hash = Cassandra.rowToHash(row[0]);
 ...
 * @api private
 */
var getRequestsJoinGroup = module.exports.getRequestsJoinGroup = function(groupId, callback) {
    Cassandra.runQuery('SELECT * FROM "RequestByGroup" WHERE "groupId" = ?', [groupId], function (err, row) {
        if (err) {
            return callback(err);
        }
        if (_.isEmpty(row)) {
...
var _addObjectToArray = function(principalId, role, list, callback) {

    var newPrincipal = {[principalId]: role};

    if (_.isEmpty(list)) {
        return callback(JSON.stringify([newPrincipal]));
    } else {
        var principalsList = list;
        principalsList.push(newPrincipal);
        return callback(JSON.stringify(principalsList));
    }
};

var _removeObjectToArray = function(principalId, list, callback) {

    var l = list[0];

    if (_.isEmpty(_.omit(l,[principalId]))) {
        return callback(JSON.stringify([]));
    } else {
        return callback(JSON.stringify([_.omit(l,[principalId])]));
    }
};
