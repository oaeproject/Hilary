                            ...
                            MQ.bindQueueToExchange(queueName, exchangeName, routingKey, function(err) {
                                ...
                                _bindPreHandleOnce(queueName, function(_queueName, data, headers, deliveryInfo, message) {
                                    ...
                                    MQ.once('storedRedelivery', function(_queueName) {
                                        ...
                                        MQ.subscribeQueue('oae-util-mq-redeliverqueue', {'prefetchCount': 1}, function(messageObj, listenerCallback) {
                                            assert.ok(messageObj);
                                            // TODO test this below
                                            assert.ok(messageObj.headers);
                                            // assert.equal(data.deliveryInfo.queue, queueName);
                                            assert.equal(messageObj.data.deliveryInfo.exchange, exchangeName);
                                            assert.equal(messageObj.data.deliveryInfo.routingKey, routingKey);
                                            assert.equal(messageObj.data.data.data, 'test');
