...
/**
 * Marks a principal as being deleted
 *
...
var deletePrincipal = module.exports.deletePrincipal = function(principalId, callback) {
    ...
    Cassandra.runQuery('UPDATE "Principals" SET "deleted" = ? WHERE "principalId" = ?', [deleted, principalId], function(err) {
        ...
        // If this is a user, we'll want to invalidate them from the cache, if necessary
        if (isUser(principalId)) {
            return invalidateCachedUsers([principalId], callback);
        } else {
            return callback();
        }
    });
...
/**
 * Restores a principal from its deleted state
 *
...
/**
 * Determine if the given principals are deleted
 *
 * @param  {String[]}   principalIds        An array of principal ids to use to determine if they've been deleted
 * @param  {Function}   callback            Standard callback function
 * @param  {Object}     callback.err        An error that occurred, if any
 * @param  {Object}     callback.deleted    An object whose keys are the principal ids that are deleted. Any principal that is not deleted will not have their id in this obejct
 */
var isDeleted = module.exports.isDeleted = function(principalIds, callback) {
    Cassandra.runQuery('SELECT "deleted" FROM "Principals" WHERE "principalId" IN (?)', [principalIds], function(err, rows) {
        var deleted = {};
        _.each(rows, function(row) {
            if (row.get('deleted') && _.isNumber(row.get('deleted').value)) {
                deleted[row.get('principalId').value] = true;
            }
        });
...
var getPrincipalFromCassandra = function(principalId, callback) {
    Cassandra.runQuery('SELECT * FROM "Principals" WHERE "principalId" = ?', [principalId], function (err, rows) {
        ...
        return callback(null, getPrincipalFromRow(rows[0]));
    });
};
