...
describe('Content Activity', function() {
    ...
    var _getActivity = function(activityStream, activityType, entityType, entityOaeId) {
        ...
        return _.find(activityStream.items, function(activity) {
            return (activity['oae:activityType'] === activityType && activity[entityType] && activity[entityType]['oae:id'] === entityOaeId);
        });
    };
     ...
     */
    var _getEmail = function(emails, to) {
    ...
    describe('Routes', function() {
        ...
        it('verify content propagation to non-route activity feeds', function(callback) {
            ...
            RestAPI.User.createUser(camAdminRestContext, jackUsername, 'password', 'Jack McJackerson', null, function(err, jack) {
                ...
                RestAPI.Content.createLink(camAdminRestContext, 'Google', 'Google', 'private', 'http://www.google.ca', [], [], [], function(err, link) {
                    ...
                    RestAPI.Content.shareContent(camAdminRestContext, link.id, [jack.id], ContentConstants.roles.VIEWER, function(err) {
                        assert.ok(!err);
                    ...
                    RestAPI.Content.joinCollabDoc(branden.restContext, contentObj.id, function(err, data) {
                        ...
                        EmailTestsUtil.collectAndFetchAllEmails(function() {
                            ...
                            etherpadClient.setText(args, function(err) {
                                ...
                                ContentTestUtil.publishCollabDoc(contentObj.id, branden.user.id, function() {

                                    // An email should be sent to Simon and Nico
                                    EmailTestsUtil.collectAndFetchAllEmails(function(emails) {
                                        assert.equal(emails.length, 2);

                                        var simonEmail = _getEmail(emails, simon.user.email);
                                        assert.ok(simonEmail);
                                        assert.equal(simonEmail.to[0].address, simon.user.email);
                                        assert.equal(simonEmail.subject, util.format('%s edited the document "%s"', branden.user.displayName, contentObj.displayName));
                                        assert.notEqual(simonEmail.html.indexOf(contentObj.profilePath), -1);

                                        var nicoEmail = _getEmail(emails, nico.user.email);
                                        assert.ok(nicoEmail);
                                        assert.equal(nicoEmail.to[0].address, nico.user.email);
                                        assert.equal(nicoEmail.subject, util.format('%s edited the document "%s"', branden.user.displayName, contentObj.displayName));
                                        assert.notEqual(nicoEmail.html.indexOf(contentObj.profilePath), -1);

                                        // No email should have been sent to Branden
                                        var brandenEmail = _getEmail(emails, branden.user.email);
                                        ...
                                        ActivityTestsUtil.collectAndGetNotificationStream(simon.restContext, null, function(err, data) {
                                            ...
                                            ActivityTestsUtil.collectAndGetNotificationStream(nico.restContext, null, function(err, data) {
                                                ...
                                                assert.equal(notificationNico['oae:activityType'], 'content-revision');
                                                assert.equal(notificationNico['actor']['oae:id'], branden.user.id);
                                                assert.equal(notificationNico['object']['oae:id'], contentObj.id);

                                                // There should be no notification in Branden's stream as he published the change
                                                ActivityTestsUtil.collectAndGetNotificationStream(branden.restContext, null, function(err, data) {
                                                    assert.ok(!err);
                                                    var notificatioBranden = _getActivity(data, 'content-revision', 'object', contentObj.id);
         ...
         */
        it('verify an activity is generated regardless of whether there was an update to a is collaborative document since the last revision', function(callback) {
            ContentTestUtil.createCollabDoc(camAdminRestContext, 2, 2, function(contentObj, users, simon, nico) {
                ...
                etherpadClient.setText(args, function(err) {
                    ...
                    ContentTestUtil.publishCollabDoc(contentObj.id, simon.user.id, function() {
                        ...
                        ContentTestUtil.publishCollabDoc(contentObj.id, nico.user.id, function() {

                            // Assert that there is an aggregated activity for an updated document that holds
                            // both Simon and Nico as the actors
                            ActivityTestsUtil.collectAndGetActivityStream(nico.restContext, nico.user.id, null, function(err, data) {
                                assert.ok(!err);
                                ActivityTestsUtil.assertActivity(data.items[0], 'content-revision', 'update', [simon.user.id, nico.user.id], contentObj.id);

                                // Sanity-check there are 2 revisions, the initial empty one + the one "published" revision
                                RestAPI.Content.getRevisions(nico.restContext, contentObj.id, null, null, function(err, data) {
                                    assert.ok(!err);
                                    assert.strictEqual(data.results.length, 2);

                                    // Get the latest revision
                                    RestAPI.Content.getRevision(nico.restContext, contentObj.id, data.results[0].revisionId, function(err, revision) {
                                        assert.ok(!err);
                                        assert.ok(revision);
            ...
            TestsUtil.generateTestUsers(camAdminRestContext, 4, function(err, users, simon, nico, bert, stuart) {
                ...
                RestAPI.Content.createLink(simon.restContext, 'Google', 'Google', 'public', 'http://www.google.ca', [], [], [], function(err, contentObj) {
                    ...
                    RestAPI.User.updateUser(nico.restContext, nico.user.id, {'visibility': 'private'}, function(err) {
                        assert.ok(!err);
                        RestAPI.Content.shareContent(nico.restContext, contentObj.id, [nico.user.id], ContentConstants.roles.VIEWER, function(err) {
                            assert.ok(!err);
                            ...
                            ActivityTestsUtil.collectAndGetActivityStream(nico.restContext, null, null, function(err) {
                                ...
                                ActivityDAO.getActivities(contentObj.id + '#activity', null, 25, function(err, activities) {
                                    ...
                                    RestAPI.Content.shareContent(bert.restContext, contentObj.id, [bert.user.id], ContentConstants.roles.VIEWER, function(err) {
                                        assert.ok(!err);
                                        ...
                                        ActivityTestsUtil.collectAndGetActivityStream(bert.restContext, null, null, function(err) {
                                            ...
                                            ActivityDAO.getActivities(contentObj.id + '#activity', null, 25, function(err, activities) {
                                                ...
                                                RestAPI.Content.shareContent(bert.restContext, contentObj.id, ContentConstants.roles.VIEWER, [stuart.user.id], function(err) {
                                                    assert.ok(!err);
        ...
        it('verify a public, loggedin and private content activity entities are propagated only to appropriate users', function(callback) {
            ...
            TestsUtil.setupMultiTenantPrivacyEntities(function(publicTenant0, publicTenant1, privateTenant0, privateTenant1) {
                ...
                FollowingTestsUtil.followByAll(publicTenant0.publicUser.user.id, followers, function() {
                    ...
                    RestAPI.Content.createLink(publicTenant0.publicUser.restContext, 'Google', 'Google', 'public', 'http://www.google.ca', [], [publicTenant1.publicUser.user.id], [], function(err, publicLink) {
                        assert.ok(!err);
                        RestAPI.Content.createLink(publicTenant0.publicUser.restContext, 'Google', 'Google', 'loggedin', 'http://www.google.ca', [], [publicTenant1.publicUser.user.id], [], function(err, loggedinLink) {
                            ...
                            RestAPI.Content.createLink(publicTenant0.publicUser.restContext, 'Google', 'Google', 'private', 'http://www.google.ca', [], [publicTenant1.publicUser.user.id], [], function(err, privateLink) {
                                ...
                                ActivityTestsUtil.collectAndGetActivityStream(publicTenant0.publicUser.restContext, null, null, function(err, result) {
                                    assert.ok(!err);
                                    ActivityTestsUtil.assertActivity(result.items[0], 'content-create', 'create', publicTenant0PublicUserId, [publicLink.id, loggedinLink.id, privateLink.id], publicTenant1.publicUser.user.id);

                                    // Ensure the loggedin user of the same tenant gets 2 of the content items in their feed: public and loggedin
                                    ActivityTestsUtil.collectAndGetActivityStream(publicTenant0.loggedinUser.restContext, null, null, function(err, result) {
                                        assert.ok(!err);
                                        ActivityTestsUtil.assertActivity(result.items[0], 'content-create', 'create', publicTenant0PublicUserId, [publicLink.id, loggedinLink.id], publicTenant1.publicUser.user.id);
                                        ...
                                        ActivityTestsUtil.collectAndGetActivityStream(publicTenant1.publicUser.restContext, null, null, function(err, result) {
                                            assert.ok(!err);
                                            ActivityTestsUtil.assertActivity(result.items[0], 'content-create', 'create', publicTenant0PublicUserId, [publicLink.id, loggedinLink.id, privateLink.id], publicTenant1.publicUser.user.id);
                                            ...
                                            ActivityTestsUtil.collectAndGetActivityStream(publicTenant1.loggedinUser.restContext, null, null, function(err, result) {
                                                assert.ok(!err);
                                                ActivityTestsUtil.assertActivity(result.items[0], 'content-create', 'create', publicTenant0PublicUserId, publicLink.id, publicTenant1.publicUser.user.id);
                                                return callback();
                                            });
                                        });
                                    });
        ...
        it('verify a content-add-to-library activity is generated when a user adds a content item to their own library', function(callback) {
            ...
            RestAPI.User.createUser(camAdminRestContext, jackUsername, 'password', 'Jack McJackerson', null, function(err, jack) {
                ...
                RestAPI.Content.createLink(camAdminRestContext, 'Google', 'Google', 'public', 'http://www.google.ca', [], [], [], function(err, link) {
                    ...
                    RestAPI.Content.shareContent(jackCtx, link.id, [jack.id], ContentConstants.roles.VIEWER, function(err) {
                        assert.ok(!err);
    ...
    describe('Activity Aggregation', function() {
                ...
                RestAPI.Content.createLink(simong.restContext, 'Apereo!', 'Apereo!', 'public', 'http://www.apereo.org', [], [], [], function(err, link) {
                    ...
                    ActivityTestsUtil.assertFeedContainsActivity(simong.restContext, simong.user.id, 'content-create', ActivityConstants.verbs.CREATE, simong.user.id, link.id, null, function() {
                        ...
                        ActivityTestsUtil.assertFeedContainsActivity(nico.restContext, nico.user.id, 'content-create', ActivityConstants.verbs.CREATE, simong.user.id, link.id, null, function() {
                            ...
                            ActivityTestsUtil.assertFeedDoesNotContainActivity(bert.restContext, bert.user.id, 'content-create', function() {
                                ActivityTestsUtil.assertFeedDoesNotContainActivity(stuart.restContext, stuart.user.id, 'content-create', function() {
                                    ActivityTestsUtil.assertFeedDoesNotContainActivity(stephen.restContext, stephen.user.id, 'content-create', function() {
                                        ActivityTestsUtil.assertFeedDoesNotContainActivity(groupMemberA.restContext, groupMemberA.user.id, 'content-create', function() {
                                            ActivityTestsUtil.assertFeedDoesNotContainActivity(groupMemberB.restContext, groupMemberB.user.id, 'content-create', function() {
                                                ActivityTestsUtil.assertFeedDoesNotContainActivity(groupMemberA.restContext, groupA.group.id, 'content-create', function() {
                                                    ActivityTestsUtil.assertFeedDoesNotContainActivity(groupMemberB.restContext, groupB.group.id, 'content-create', callback);
                                                });
                                            });
                                        });
                                    });
        ...
        it('verify one extra user', function(callback) {
            _setup(function(simong, nico, bert, stuart, stephen, groupMemberA, groupMemberB, groupA, groupB) {
                ...
                RestAPI.Content.createLink(simong.restContext, 'Apereo!', 'Apereo!', 'public', 'http://www.apereo.org', [bert.user.id], [], [], function(err, link) {
                    assert.ok(!err);

                    // The actor should receive an activity
                    ActivityTestsUtil.assertFeedContainsActivity(simong.restContext, simong.user.id, 'content-create', ActivityConstants.verbs.CREATE, simong.user.id, link.id, bert.user.id, function() {

                        // Users who follows the actor receive the activity
                        ActivityTestsUtil.assertFeedContainsActivity(nico.restContext, nico.user.id, 'content-create', ActivityConstants.verbs.CREATE, simong.user.id, link.id, bert.user.id, function() {

                            // The user who was made a member gets an activity
                            ActivityTestsUtil.assertFeedContainsActivity(bert.restContext, bert.user.id, 'content-create', ActivityConstants.verbs.CREATE, simong.user.id, link.id, bert.user.id, function() {
        ...
        it('verify one extra group', function(callback) {
            _setup(function(simong, nico, bert, stuart, stephen, groupMemberA, groupMemberB, groupA, groupB) {
                ...
                RestAPI.Content.createLink(simong.restContext, 'Apereo!', 'Apereo!', 'public', 'http://www.apereo.org', [groupA.group.id], [], [], function(err, link) {
                    assert.ok(!err);

                    // The actor should receive an activity
                    ActivityTestsUtil.assertFeedContainsActivity(simong.restContext, simong.user.id, 'content-create', ActivityConstants.verbs.CREATE, simong.user.id, link.id, groupA.group.id, function() {

                        // Users who follow the actor receive the activity
                        ActivityTestsUtil.assertFeedContainsActivity(nico.restContext, nico.user.id, 'content-create', ActivityConstants.verbs.CREATE, simong.user.id, link.id, groupA.group.id, function() {

                            // The group who was made a member gets an activity
                            ActivityTestsUtil.assertFeedContainsActivity(groupMemberA.restContext, groupA.group.id, 'content-create', ActivityConstants.verbs.CREATE, simong.user.id, link.id, groupA.group.id, function() {

                                // Members of the group get an activity
                                ActivityTestsUtil.assertFeedContainsActivity(groupMemberA.restContext, groupMemberA.user.id, 'content-create', ActivityConstants.verbs.CREATE, simong.user.id, link.id, groupA.group.id, function() {
        ...
        it('verify more than one extra member', function(callback) {
            _setup(function(simong, nico, bert, stuart, stephen, groupMemberA, groupMemberB, groupA, groupB) {
                ...
                RestAPI.Content.createLink(simong.restContext, 'Apereo!', 'Apereo!', 'public', 'http://www.apereo.org', [bert.user.id, groupA.group.id], [], [], function(err, link) {
                    assert.ok(!err);
                    ...
                    ActivityTestsUtil.assertFeedContainsActivity(simong.restContext, simong.user.id, 'content-create', ActivityConstants.verbs.CREATE, simong.user.id, link.id, null, function() {
                        ...
                        ActivityTestsUtil.assertFeedContainsActivity(nico.restContext, nico.user.id, 'content-create', ActivityConstants.verbs.CREATE, simong.user.id, link.id, null, function() {
                            ...
                            ActivityTestsUtil.assertFeedContainsActivity(nico.restContext, nico.user.id, 'content-create', ActivityConstants.verbs.CREATE, simong.user.id, link.id, null, function() {
                                ...
                                // The group who was made a member gets an activity
                                ActivityTestsUtil.assertFeedContainsActivity(groupMemberA.restContext, groupA.group.id, 'content-create', ActivityConstants.verbs.CREATE, simong.user.id, link.id, null, function() {

                                    // Members of the group get an activity
                                    ActivityTestsUtil.assertFeedContainsActivity(groupMemberA.restContext, groupMemberA.user.id, 'content-create', ActivityConstants.verbs.CREATE, simong.user.id, link.id, null, function() {
                    ...
                    RestAPI.User.createUser(camAdminRestContext, brandenUsername, 'password', 'Branden Visser', null, function(err, branden) {
                        ...
                        RestAPI.Content.createLink(jackCtx, 'Google', 'Google', 'public', 'http://www.google.ca', [], [], [], function(err, googleLink) {
                            ...
                            RestAPI.Content.createLink(jackCtx, 'Yahoo', 'Yahoo', 'public', 'http://www.yahoo.ca', [], [], [], function(err, yahooLink) {
                                ...
                                RestAPI.Content.shareContent(jackCtx, googleLink.id, [jane.id, branden.id], ContentConstants.roles.VIEWER, function(err) {
                                    assert.ok(!err);

                                    // Share Yahoo link with jane only
                                    RestAPI.Content.shareContent(jackCtx, yahooLink.id, [jane.id], ContentConstants.roles.VIEWER, function(err) {
                                        assert.ok(!err);
                    ...
                    RestAPI.User.createUser(camAdminRestContext, brandenUsername, 'password', 'Branden Visser', null, function(err, branden) {
                        ...
                        RestAPI.Content.createLink(jackCtx, 'Google', 'Google', 'public', 'http://www.google.ca', [], [], [], function(err, googleLink) {
                            ...
                            RestAPI.Content.createLink(jackCtx, 'Yahoo', 'Yahoo', 'public', 'http://www.yahoo.ca', [], [], [], function(err, yahooLink) {
                                ...
                                RestAPI.Content.shareContent(jackCtx, googleLink.id, [jane.id], ContentConstants.roles.VIEWER, function(err) {
                                    assert.ok(!err);
                                    ...
                                    ActivityTestsUtil.collectAndGetActivityStream(jackCtx, null, null, function(err, activityStream) {
                                        ...
                                        RestAPI.Content.shareContent(jackCtx, googleLink.id, [branden.id], ContentConstants.roles.VIEWER, function(err) {
                                            assert.ok(!err);
                    ...
                    RestAPI.User.updateUser(simong.restContext, simong.user.id, simongUpdate, function(err) {
                        ...
                        RestAPI.Content.createLink(simong.restContext, 'Google', 'Google', 'public', 'http://www.google.ca', [], [], [], function(err, link) {
                            ...
                            EmailTestsUtil.collectAndFetchAllEmails(function(messages) {
                                ...
                                RestAPI.Content.shareContent(simong.restContext, link.id, [mrvisser.user.id], ContentConstants.roles.VIEWER, function(err) {
                                    assert.ok(!err);
