...
var registerProcessor = module.exports.registerProcessor = function(processorId, processor) {
    if (!processorId) {
        throw new Error('Missing processor ID');
    } else if (_processors[processorId]) {
        throw new Error('This processor is already registered');
    } else if (!_.isObject(processor)) {
        throw new Error('Must provide a preview processor as the second argument');
    } else if (!_.isFunction(processor.test)) {
        throw new Error('Processor must have a "test" function');
    } else if (!_.isFunction(processor.generatePreviews)) {
        throw new Error('Processor must have a "generatePreviews" method');
    }
...
var reprocessPreviews = module.exports.reprocessPreviews = function(ctx, filters, callback) {
    ...
    MQ.submit(PreviewConstants.MQ.TASK_REGENERATE_PREVIEWS, {'filters': filters}, callback);
};
 ...
 * @param  {String}     contentId   The id of the content item whose revision's preview will be reprocessed
 * @param  {String}     revisionId  The id of the revision to be reprocessed
 */
