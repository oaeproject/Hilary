...
var getContentPreviews = module.exports.getContentPreviews = function(revisionId, callback) {
    Cassandra.runQuery('SELECT "name", "value" FROM "PreviewItems" WHERE "revisionId" = ?', [revisionId], function(err, rows) {
        if (err) {
            return callback(err);
        }

        var previews = _.map(rows, function(row) {
...
var getContentPreview = module.exports.getContentPreview = function(revisionId, previewItem, callback) {
    Cassandra.runQuery('SELECT "value" FROM "PreviewItems" WHERE "revisionId" = ? AND "name" = ?', [revisionId, previewItem], function(err, rows) {
        if (err) {
            return callback(err);
        ...
        var data = rows[0].get('value').value.split('#');
        callback(null, {
            'size': data[0],
            'uri': data[1]
        });
...
var getPreviewUris = module.exports.getPreviewUris = function(revisionIds, callback) {
    ...
    Cassandra.runQuery('SELECT "revisionId", "thumbnailUri", "wideUri" FROM "Revisions" WHERE "revisionId" IN (?)', [revisionIds], function(err, rows) {
        if (err) {
            return callback(err);
        }
        ...
        rows.forEach(function(row) {
            ...
            if (thumbnailUri && thumbnailUri.value) {
                previews[revisionId].thumbnailUri = thumbnailUri.value;
            }

            var wideUri = row.get('wideUri');
            if (wideUri && wideUri.value) {
                previews[revisionId].wideUri = wideUri.value;
            }
        });
...
var storeMetadata = module.exports.storeMetadata = function(contentObj, revisionId, status, thumbnailUri, contentMetadata, previewMetadata, fileData, callback) {
    ...
    _.each(fileData, function(value, name) {
    ...
    var data = {'status': status, 'total': _.keys(fileData).length};
    ...
    _runQueryIfSpecified(deleteQuery, function(err) {
        if (err) {
            return callback(err);
        }
...
var copyPreviewItems = module.exports.copyPreviewItems = function(fromRevisionId, toRevisionId, callback) {
    ...
    Cassandra.runQuery('SELECT * FROM "PreviewItems" WHERE "revisionId" = ?', [fromRevisionId], function(err, rows) {
        if (err) {
            return callback(err);
        } else if (_.isEmpty(rows)) {
            return callback();
        ...
        var queries = _.map(rows, function(row) {
            row = Cassandra.rowToHash(row);
            return {
                'query': 'INSERT INTO "PreviewItems" ("revisionId", "name", "value") VALUES (?, ?, ?)',
 ...
 * @api private
 */
var _runQueryIfSpecified = function(query, callback) {
    if (!query) {
        return callback();
    }
