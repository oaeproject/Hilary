 ...
 * * `userLoggedIn(user, strategyId)`: A user logged into the system. The strategy that was used to log in is provided
 * * `userLoggedOut(user)`: A user logged out of the system
 */
...
var logout = module.exports.logout = function(req, res) {
    if (!req.ctx.user()){
        return res.send(400, 'You need to be logged in order to log out');
    }
    ...
    AuthenticationAPI.emit(AuthenticationConstants.events.USER_LOGGED_OUT, req.ctx.user());
