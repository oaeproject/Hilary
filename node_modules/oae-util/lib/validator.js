...
/**
 * Check whether or not a tenant object represents an actual tenant
 * @param {Tenant} tenant   Tenant object that needs to be checked for validity
 */
Validator.prototype.isTenant = function(tenant) {
    if (!_.isObject(tenant) || !tenant.alias) {
        this.error(this.msg || 'Not a valid tenant');
    }
    return this;
};

/**
 * Check whether or not a user object represents an actual user
 * @param {User} user   User object that needs to be checked for validity
 */
Validator.prototype.isUser = function(user) {
    if (!_.isObject(user) || !user.id) {
        this.error(this.msg || 'Not a valid user');
    }
    return this;
};