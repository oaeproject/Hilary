...
var TenantEmailDomainIndex = require('./internal/emailDomainIndex');
...
var tenantEmailDomainIndex = null;
...
var getTenantByEmail = module.exports.getTenantByEmail = function(emailOrDomain) {
    ...
    var tenantAlias = tenantEmailDomainIndex.match(domain);

    // Default to the guest tenant
    var guestTenant = tenants[serverConfig.guestTenantAlias];
    var tenant = tenants[tenantAlias] || guestTenant;
 ...
 */
var getTenantsForEmailDomains = module.exports.getTenantsForEmailDomains = function(emailsOrDomains) {
    var mappedTenants = {};
    _.each(emailsOrDomains, function(emailOrDomain) {
...
var _createTenant = function(alias, displayName, host, opts, callback) {
    ...
    _.each(opts, function(val, key) {
        ...
        if (key === 'emailDomains') {
            ...
            validator.check(opts[key], {'code': 400, 'msg': 'The country code must be a valid ISO-3166 country code'}).isIso3166Country();
        }
    });
    if (validator.hasErrors()) {
        return callback(validator.getFirstError());
...
var updateTenant = module.exports.updateTenant = function(ctx, alias, tenantUpdates, callback) {
    ...
    _.each(tenantUpdates, function(updateValue, updateField) {
        ...
        if (updateField === 'displayName') {
            ...
            validator.check(tenantUpdates[updateField], {'code': 400, 'msg': 'The country code must be a valid ISO-3166 country code'}).isIso3166Country();
        }
    });
    if (validator.hasErrors()) {
        return callback(validator.getFirstError());
...
var disableTenants = module.exports.disableTenants = function(ctx, aliases, disabled, callback) {
    ...
    Cassandra.runBatchQuery(queries, function(err) {
        ...
        var disableUsersInTenants = function(tenantAliases) {
            var tenantAlias = tenantAliases.pop();
            var UserAPI = require('oae-principals/lib/api.user.js');

            UserAPI.deleteOrRestoreUsersByTenancy(ctx, tenantAlias, disabled, function(err, users) {
                if (err) {
                    var action = disabled ? 'deleting' : 'restoring';
                    log().error({'err': err, 'alias': tenantAlias},
                        'An error occurred when %s users for tenant', action);
                }
                ...
                Pubsub.publish('oae-tenants', cmd + ' ' + tenantAlias, callback);
            });

            if (!_.isEmpty(tenantAliases)) {
                disableUsersInTenants(tenantAliases);
            }
        }
        disableUsersInTenants(aliases)
    });
};
