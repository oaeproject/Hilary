...
var getActivitiesFromStreams = module.exports.getActivitiesFromStreams = function(activityStreamIds, start, callback) {
    var placeholders = _.map(activityStreamIds, function() {
        return '?';
    });
    var query = util.format('SELECT * FROM "ActivityStreams" WHERE "activityStreamId" IN (%s)', placeholders.join(','));
    var parameters = activityStreamIds.slice();
    if (start) {
        query += ' AND "activityId" > ?';
        parameters.push(start + ':');
    }

    Cassandra.runQuery(query, parameters, {'cache': false}, function(err, rows) {
        if (err) {
            return callback(err);
        }
        ...
        _.each(rows, function(row) {
            var activityStreamId = row.activityStreamId;
            var activityId = row.activityId;
            var activityStr = row.activity;
            try {
...
var getQueuedUserIdsForEmail = module.exports.getQueuedUserIdsForEmail = function(bucketId, start, limit, callback) {
    Cassandra.runPagedQuery('EmailBuckets', 'bucketId', bucketId, 'userId', start, limit, {'end': '|'}, function(err, rows, nextToken) {
        ...
        return callback(null, _.pluck(rows, 'userId'), nextToken);
    });
};
