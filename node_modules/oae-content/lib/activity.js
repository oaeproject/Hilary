 ...
 */
ContentAPI.on(ContentConstants.events.UPDATED_CONTENT_BODY, function(ctx, content, revision, fileInfo) {
    var millis = Date.now();
    var actorResource = new ActivityModel.ActivitySeedResource('user', ctx.user().id, {'user': ctx.user()});
    var objectResource = new ActivityModel.ActivitySeedResource('content', content.id, {'content': content});
...
ActivityAPI.registerActivityType(ContentConstants.activity.ACTIVITY_CONTENT_RESTORED_REVISION, {'groupBy': [
    {'actor': true}
]});
...
ContentAPI.on(ContentConstants.events.CREATED_COMMENT, function(ctx, message, content) {
    var millis = Date.now();
    var actorResource = new ActivityModel.ActivitySeedResource('user', ctx.user().id, {'user': ctx.user()});
    var objectResource = new ActivityModel.ActivitySeedResource(ContentConstants.activity.ACTIVITY_CONTENT_COMMENT, message.id, {'contentId': content.id, 'message': message});
...
ActivityAPI.registerActivityEntityProducer('content-comment', function(activityType, entityType, resource, callback) {
    ...
    ContentDAO.Content.getContent(contentId, function(err, content) {
        ...
        MessageBoxUtil.createPersistentMessageActivityEntity(message, function(err, entity) {
            ...
            // Store the content id and visibility on the entity
 ...
 */
ActivityAPI.registerActivityRouter('content', function(activitySeed, entityType, entity, callback) {
    // Extract the OAE ID from the activity object to get the content id
    return _routeContent(activitySeed.activityType, entityType, entity[ActivityConstants.properties.OAE_ID], entity.content.visibility, callback);
});

/*!
 * Routes an activity that occurred for a comment activity entity
 */
ActivityAPI.registerActivityRouter('content-comment', function(activitySeed, entityType, entity, callback) {
    // First collect the routes for the parent content item
    _routeContent(activitySeed.activityType, entityType, entity.contentId, entity.contentVisibility, function(err, routes, propagation) {
        if (err) {
            return callback(err);
        }
 ...
 *
 * @param  {ActivitySeed}   activitySeed            The activity seed
 ...
 * @api private
 */
var _routeContent = function(activitySeed, entityType, contentId, visibility, callback) {
    ...
    ActivityUtil.getAllAuthzMembersByRole(contentId, function(err, specify) {
        ...
        var isContentCreateActivity = (activitySeed.verb === ActivityConstants.verbs.CREATE);
        var isShareActivity = _.contains([ActivityConstants.verbs.ADD, ActivityConstants.verbs.SHARE], activitySeed.verb);
        var isCommentActivity = (activitySeed.verb === ActivityConstants.verbs.POST);
        var isUpdateActivity = (activitySeed.verb === ActivityConstants.verbs.UPDATE);
        if (isShareActivity) {
            ...
            // When a comment is posted or a piece of content is updated,
            // we must notify all the manager users of the parent content item
