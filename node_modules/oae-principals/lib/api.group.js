 ...
 * @param  {Function(err, group)}   callback        A standard callback method.
 * @param  {Object}                 callback.err    An error object (if any.)
 * @param  {Object}                 callback.group  The group object
 */
var getGroup = module.exports.getGroup = function(ctx, groupId, callback) {
    if (!PrincipalsUtil.isGroup(groupId)) {
        return callback({'code': 400, 'msg': 'The provided groupId is not a group identifier.'});
    }
...
var getGroupByAlias = module.exports.getGroupByAlias = function(ctx, alias, callback) {
    var id = AuthzUtil.toUuid(Constants.PRINCIPAL_TYPE_GROUP, ctx.tenant().alias, alias);
 ...
 * @param  {Function(err, group)}   callback        A standard callback method.
 * @param  {Object}                 callback.err    An error object (if any.)
 * @param  {Object}                 callback.group  The agumented group object
 */
var getFullGroupProfile = module.exports.getFullGroupProfile = function(ctx, groupId, callback) {
    getGroup(ctx, groupId, function(err, group) {
        ...
        AuthzAPI.getAllRoles(ctx.user().id, groupId, function(err, roles) {
            ...
            group.isMember = (roles[Constants.roles.MEMBER] || roles[Constants.roles.MANAGER]);
            group.isManager = (roles[Constants.roles.MANAGER] === true);
...
var getGroupMembers = module.exports.getGroupMembers = function(ctx, groupId, start, limit, callback) {
    ...
    validator.check(groupId, {'code': 400,'msg': 'An invalid group id was specified'}).isGroupPrincipal();
    ...
    getGroup(ctx, groupId, function(err, group) {
        ...
        AuthzAPI.getGroupMembers(groupId, start, limit, function(err, members) {
            if (err) {
                return callback(err);
            }
            ...
            PrincipalsUtil.getPrincipals(ctx, principalids, function(err, membersObject, missingMembers) {
                if (err) {
                    return callback(err);
                }
...
var memberOf = module.exports.memberOf = function(ctx, principalId, start, limit, callback) {
    ...
    AuthzAPI.getMemberOf(principalId, function(err, groups) {
        ...
        PrincipalsUtil.getPrincipals(ctx, groups, function(err, principalHash, missingPrincipals) {
            if (err) {
                return callback(err);
            }
...
var setGroupMembers = module.exports.setGroupMembers = function(ctx, groupId, members, callback) {
    ...
    var validator = new Validator();
    validator.check(groupId, {'code': 400, 'msg': 'Invalid groupId specified'}).isGroupPrincipal();
 ...
 *
 * @param {Context}             ctx             The current execution context
 * @param {String}              alias           The alias for this group. This will be prefixed with 'g:tenantId:' to construct the group id. 
 *                                              The only requirement for this parameter is that it doesn't contain any colons (:).
...
var createGroup = module.exports.createGroup = function(ctx, alias, name, description, visibility, joinable, members, otherValues, callback) {
    ...
    var id = AuthzUtil.toUuid(Constants.PRINCIPAL_TYPE_GROUP, tenant.alias, alias);

    canCreateGroup(ctx, id, function(err) {
        if (!err) {
            // Create the group.
            Cassandra.runQuery('INSERT INTO Principals (principal_id, alias, tenant, name, description, visibility, joinable) VALUES (?, ?, ?, ?, ?, ?, ?) USING CONSISTENCY QUORUM',
                ...
                members[currentgetUserId] = Constants.roles.MANAGER;
 ...
 * @param {Context}                         ctx                 The current execution context.
 * @param {String}                          groupId             The id of the group.
 * @param {Function(err, canManage)}        callback            Callback method.
 * @param {Object}                          callback.err        If specified, indicates that the user cannot create the group. The error 'code' property is the http response code.
 * @param {Boolean}                         callback.canManage  Whether or not the user can manage the group.
 */
var _canManage = function(ctx, groupId, callback) {
    AuthzAPI.isAllowed(getUserId(ctx), Constants.roles.MANAGER, groupId, callback);
};
...
var _setGroupMembers = function(ctx, groupId, members, callback) {
    ...
    PrincipalsUtil.getPrincipals(ctx, _.keys(members), function(err, principals, missingPrincipals) {
        ...
        AuthzAPI.applyGroupMembershipChanges(groupId, members, callback);
    });
};
