...
/**
 * Get all last connection of users.
 *
 * @param  {Function}       callback            Standard callback function
 * @param  {Object}         callback.err        An error that occurred, if any
 * @param  [User]           callback.rows       Rows of users
 */
var getLog = module.exports.getLog = function(callback) {
    Cassandra.runQuery('SELECT * FROM "Principals"', [], function(err, rows) {
        if (err) {
            return callback(err);
        } else if (_.isEmpty(rows)) {
            return callback({'code': 404, 'msg': 'No principals found'});
        }
...
/**
 * Save the last connection of a user. 
 *
 * @param  {Object}      user                The user
 * @param  {Function}    callback            Standard callback function
 * @param  {Object}      callback.err        An error that occurred, if any
 */
var saveDateLastLogIn = module.exports.saveDateLastLogIn = function(user, callback) {
    callback = callback || function() {};

    Cassandra.runQuery('UPDATE "Principals" SET "lastLog" = ? WHERE "principalId" = ?', [Date.now(), user.id], function(err) {
        if (err) {
            console.log(err)
            return callback(err);
        }
