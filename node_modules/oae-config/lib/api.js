 ...
 * @param  {Object}             cassandraConfig     Helenus object that contains configuration values stored in Cassandra. There will be one 
 *                                                  row for the global tenant, and also one row for the current tenant if the current tenant 
 *                                                  is not the global tenant
 * @param  {Object}             moduleConfig        Contains the aggregated configurations for all of the available modules
 * @return {Object}                                 All of the configuration values for all available modules, with values overriden if the
 *                                                  current tenant has modifications to that config variable
 */
        ...
        for (var c = 0; c < cassandraConfig[r].count; c++) {
            if (cassandraConfig[r][c].name !== 'tenantId') {
                ...
                if (moduleConfig[colHash[0]] && moduleConfig[colHash[0]][colHash[1]] && moduleConfig[colHash[0]][colHash[1]].elements[colHash[2]]) {
                    moduleConfig[colHash[0]][colHash[1]].elements[colHash[2]].defaultValue = Cassandra.castToBoolean(cassandraConfig[r][c].value);
                }
            }
        }
 ...
 * @param {Function(defaultConfig)}   callback         Callback function executed when filtering is complete. Passes through 
 *                                                     the filtered Array of config objects.
 * @param {Object}                    defaultConfig    Object containing the default configuration for all of the configurable modules
 */
        ...
        var getConfigContent = function(dir, module) {
            IO.getFileListForFolder(dir, function(files) {
                for (var ii = 0; ii < files.length; ii++) {
                    ...
                    configurableModules[module] = _.extend(configurableModules[module] || {}, content.config[module]);
                }
                done++;
                if (done === toDo) {
