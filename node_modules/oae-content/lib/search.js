...
ContentAPI.on(ContentConstants.events.CREATED_COMMENT, function(ctx, content, comment) {
    TaskQueue.submit(SearchConstants.mq.TASK_INDEX_DOCUMENT, {
        'resources': [{
            'id': content.id,
            'comments': [comment],
...
ContentAPI.on(ContentConstants.events.DELETED_COMMENT, function(ctx, content, comment) {
    return MessageBoxSearch.deleteMessageSearchDocument('content_comment', comment);
});
...
var _produceContentCommentDocuments = function(resources, callback, _documents) {
    ...
    if (resource.comments) {
        return callback(null, MessageBoxSearch.createMessageSearchDocuments('content_comment', resource.id, resource.comments));
    }

    // If there were no messages stored on the resource object, we go ahead and index all comments for the content item
    return MessageBoxSearch.createAllMessageSearchDocuments('content_comment', resource.id, resource.id, callback);
};
