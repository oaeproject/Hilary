...
var createUser = (module.exports.createUser = function(user, callback) {
  ...
  queries.push(
    Cassandra.constructUpsertCQL('Principals', 'principalId', user.id, values),
  );
  ...
  if (isUser(principalId)) {
    ...
    _getUserFromRedis(principalId, function(err, user) {
      if (err && err.code !== 404) {
        log().error(
          { err: err },
          'Error occurred when trying to get a user from Redis.',
        );
      } else if (user) {
 ...
 */
var getExistingPrincipals = (module.exports.getExistingPrincipals = function(
  principalIds,
  fields,
  callback,
) {
  ...
  getPrincipals(principalIds, fields, function(err, principalsById) {
    if (err) {
      ...
      return callback({
        code: 400,
        msg: 'One or more provided principals did not exist',
      });
    }
...
var getPrincipals = (module.exports.getPrincipals = function(
  ...
  if (fields) {
    ...
    query =
      'SELECT ' +
      columns.join(',') +
      ' FROM "Principals" WHERE "principalId" IN ?';
  } else {
 ...
 * @param  {Object}     callback.err    An error that occurred, if any
 */
var updatePrincipal = (module.exports.updatePrincipal = function(
  principalId,
  profileFields,
  callback,
) {
 ...
 * @param  {Object}     callback.err    An error that occurred, if any
 */
var deletePrincipal = (module.exports.deletePrincipal = function(
  principalId,
  callback,
) {
 ...
 * @param  {Object}     callback.err    An error that occurred, if any
 */
var restorePrincipal = (module.exports.restorePrincipal = function(
  principalId,
  callback,
) {
 ...
 * @param  {Object}     callback.err    An error that occurred, if any
 */
var acceptTermsAndConditions = (module.exports.acceptTermsAndConditions = function(
  userId,
  callback,
) {
  Cassandra.runQuery(
 ...
 * @param  {Object}         callback.err    An error that occurred, if any
 */
var setAdmin = (module.exports.setAdmin = function(
  adminType,
  isAdmin,
  userId,
  callback,
  ...
  Cassandra.runQuery(
    util.format(
      'UPDATE "Principals" SET "%s" = ? WHERE "principalId" = ?',
      adminType,
    ),
    [
      sanitize(isAdmin)
  ...
  Cassandra.runQuery(
    ...
    function(err, rows) {
      if (err) {
        ...
        return callback({
          code: 404,
          msg: 'No email token found for the given user id',
        });
      }
 ...
 */
var getUserIdsByEmails = (module.exports.getUserIdsByEmails = function(
  emails,
  callback,
) {
  Cassandra.runQuery(
 ...
 * @param  {Object}     callback.err    An error object, if any
 */
var storeEmailToken = (module.exports.storeEmailToken = function(
  userId,
  email,
  token,
  callback,
 ...
 */
var setEmailAddress = (module.exports.setEmailAddress = function(
  user,
  email,
  callback,
) {
  ...
  if (user.email && user.email !== email) {
    queries.push({
      query:
        'DELETE FROM "PrincipalsByEmail" WHERE "email" = ? AND "principalId" = ?',
      parameters: [user.email, user.id],
    });
  }
...
var _updatePrincipal = function(principalId, profileFields, callback) {
  ...
  _isEmailAddressUpdate(principalId, profileFields, function(
    err,
    isEmailAddressUpdate,
    oldEmail,
  ) {
    if (err) {
      return callback(err);
  ...
  Cassandra.runQuery(
    ...
    function(err, rows) {
      if (err) {
        ...
        return callback({
          code: 404,
          msg: "Couldn't find principal: " + principalId,
        });
      }
...
var _getPrincipalFromRow = function(row) {
  ...
  return isGroup(row.get('principalId'))
    ? _getGroupFromRow(row)
    : _getUserFromRow(row);
};
 ...
 * @param  {Object}      callback.err    An error that occurred, if any
 */
var invalidateCachedUsers = (module.exports.invalidateCachedUsers = function(
  userIds,
  callback,
) {
 ...
 * @param  {Object}     callback.err    An error object, if any
 */
var setLatestVisit = (module.exports.setLatestVisit = function(
  user,
  group,
  visit,
  callback,
 ...
 */
var getVisitedGroups = (module.exports.getVisitedGroups = function(
  userId,
  callback,
) {
  if (isUser(userId)) {
