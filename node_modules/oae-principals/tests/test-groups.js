...
describe('Groups', function() {
    ...
    describe('Get group members', function() {
        ...
        it('verify loggedin membership visibility', function(callback) {
            ...
            RestAPI.User.createUser(anonymousRestContext, usernameA, 'password', 'Public User', null, function(err, userA) {
                ...
                RestAPI.User.createUser(anonymousRestContext, usernameA2, 'password', 'Public User', null, function(err, userA2) {
                    assert.ok(!err);
    ...
    describe('Set group members', function() {
        ...
        /**
         * Test that verifies that a non-joinable group cannot be joined
         */
        it('verify that a user cannot join a non-joinable group', function(callback) {
            // Create a first user to use inside of test
            var brandenUserId = TestsUtil.generateTestUserId('mrvisser');
            RestAPI.User.createUser(camAdminRestContext, brandenUserId, 'password', 'Branden Visser', null, function(err, branden) {
                assert.ok(!err);
                ...
                var groupId = TestsUtil.generateTestGroupId();
                RestAPI.Group.createGroup(camAdminRestContext, groupId, 'Test Group', 'Group', 'public', 'request', [], [], function(err, group) {
                    assert.ok(!err);

                    var join = {};
                    join[branden.id] = 'member';
                    RestAPI.Group.setGroupMembers(brandenRestContext, group.id, join, function(err) {
                        assert.ok(err);
                        assert.equal(err.code, 401);

                        // Verify the user was not added
                        RestAPI.Group.getGroupMembers(camAdminRestContext, group.id, null, 10000, function(err, members) {
                            assert.ok(!err);
                            assert.equal(members.length, 1);
                            callback();
                        });
        ...
        /**
         * Test that verifies that a joinable group can only be joined as a member, and that unjoining works
         */
        it('verify joining and unjoining a joinable group', function(callback) {
            // Create a first user to use inside of test
            var brandenUserId = TestsUtil.generateTestUserId('mrvisser');
            RestAPI.User.createUser(camAdminRestContext, brandenUserId, 'password', 'Branden Visser', null, function(err, branden) {
                assert.ok(!err);
                ...
                // Create a group that is not joinable
                var groupId = TestsUtil.generateTestGroupId();
                RestAPI.Group.createGroup(camAdminRestContext, groupId, 'Test Group', 'Group', 'private', 'yes', [], [], function(err, group) {
                    assert.ok(!err);

                    var join = {};
                    join[branden.id] = 'manager';
                    RestAPI.Group.setGroupMembers(brandenRestContext, group.id, join, function(err) {
                        assert.ok(err);
                        assert.equal(err.code, 401);
                        ...
                        RestAPI.Group.getGroupMembers(camAdminRestContext, group.id, null, 10000, function(err, members) {
                            ...
                            RestAPI.Group.setGroupMembers(brandenRestContext, group.id, join, function(err) {
                                ...
                                // Verify the user is a member now
                                RestAPI.Group.getGroupMembers(camAdminRestContext, group.id, null, 10000, function(err, members) {
                                    assert.ok(!err);
                                    assert.equal(members.length, 2);

                                    // Unjoin the group
                                    join[branden.id] = false;
                                    RestAPI.Group.setGroupMembers(brandenRestContext, group.id, join, function(err) {
                                        assert.ok(!err);
