...
describe('Config Aggregator', function() {
    ...
    describe('Admin UI', function() {
        ...
        it('verify access tenant admin', function(callback) {
            RestAPI.Config.getTenantAdminUI(camAdminRestContext, null, function(err, adminui) {
                assert.ok(!err);
                assert.ok(adminui);
                callback();
            });
        ...
        /**
         * Test that verifies that the tenant admin UI can be accessed through the global server
         */
        it('verify access tenant admin through global server', function(callback) {
            RestAPI.Config.getTenantAdminUI(globalAdminRestContext, 'camtest', function(err, adminui) {
                assert.ok(!err);
                assert.ok(adminui);
                callback();
            });
    ...
    describe('Configuration', function() {
        ...
        it('verify set global config value', function(callback) {
            RestAPI.Config.setConfig(globalAdminRestContext, null, 'oae-authentication/twitter/enabled', false, function(err) {
                ...
                RestAPI.Config.getConfig(globalAdminRestContext, null, function(err, config) {
                    assert.ok(!err);
                    assert.ok(config);
                    assert.equal(config['oae-authentication'][AuthenticationConstants.providers.TWITTER].elements['enabled'].defaultValue, false);
        ...
        it('verify set tenant config value', function(callback) {
            RestAPI.Config.setConfig(camAdminRestContext, null, 'oae-authentication/twitter/enabled', true, function(err) {
                ...
                RestAPI.Config.getConfig(camAdminRestContext, null, function(err, config) {
                    assert.ok(!err);
                    assert.ok(config);
                    assert.equal(config['oae-authentication'][AuthenticationConstants.providers.TWITTER].elements['enabled'].defaultValue, true);
                    callback();
                });
            });
        });
        ...
        it('verify get single config value', function(callback) {
            assert.equal(AuthenticationConfig.getValue('camtest', AuthenticationConstants.providers.TWITTER, 'enabled'), false);
            callback();
        });
        ...
        it('verify get single config value requires parameters', function(callback) {
            ...
            assert.equal(AuthenticationConfig.getValue('camtest', AuthenticationConstants.providers.TWITTER).code, 400);
            assert.equal(AuthenticationConfig.getValue('camtest', AuthenticationConstants.providers.TWITTER).msg, 'Missing the element parameter');
            callback();
        });
