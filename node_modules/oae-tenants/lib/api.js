...
var TenantConstants = require('./constants').TenantConstants;
...
/*!
 * Create the guest tenant if it doesn't exist already. This needs to happen when the system
 * is ready, are there are a lot of modules that need to be initialized who listen for `create-tenant` events.
 */
OaeEmitter.on('ready', function() {
    // If the guests tenant already exists, we can return here
    if (getTenant(TenantConstants.aliases.GUESTS)) {
        return;
    }

    // Create the guests tenant
    _createTenant(TenantConstants.aliases.GUESTS, 'Guests tenant', serverConfig.guestsTenantHost, function(err) {
        if (err) {
            log().error({'err': err}, 'Unable to create the guests tenant');
        }
    });
});
...
/**
 * Get the tenant that represents the guests tenant
 *
 * @return {Tenant}         Tenant object associated to the guests tenant
 */
var getGuestsTenant = module.exports.getGuestsTenant = function() {
    return getTenant(TenantConstants.aliases.GUESTS);
};
...
var _cacheTenants = function(callback) {
    ...
    Cassandra.runQuery('SELECT * FROM "Tenant"', false, function(err, rows) {
        ...
        globalTenant = new Tenant(TenantConstants.aliases.ADMIN, 'Global admin server', serverConfig.globalAdminHost, {'isGlobalAdminServer': true});
...
var createTenant = module.exports.createTenant = function(ctx, alias, displayName, host, callback) {
    ...
    if (host) {
        validator.check(host.toLowerCase(), {'code': 400, 'msg': 'This hostname is reserved for the Shibboleth tenant'}).not(serverConfig.shibbolethSPHost);
    }
    ...
    if (!getTenant(alias)) {
        ...
        if (!getTenantByHost(host)) {
            _createTenant(alias, displayName, host, callback);
        } else {
...
/**
 * Internal function that creates and persists a tenant. It's assumed
 * that the tenant parameters have been validated previously.
 *
 * @param  {String}     alias                   The unique alias for the tenant
 * @param  {String}     displayName             A descriptive short name for the tenant
 * @param  {String}     host                    The host on which this tenant will be proxying (e.g. oae.cam.ac.uk or oae.gatech.edu)
 ...
 * @api private
 */
var _createTenant = function(alias, displayName, host, callback) {
    // Create the tenant
...
var updateTenant = module.exports.updateTenant = function(ctx, alias, tenantUpdates, callback) {
    ...
    validator.check(alias.toLowerCase(), {'code': 400, 'msg': 'The guests tenant cannot be updated'}).not(TenantConstants.aliases.GUESTS);
    ...
    _.each(tenantUpdates, function(updateValue, updateField) {
        ...
        if (updateField === 'displayName') {
            ...
            validator.check(tenantUpdates.host.toLowerCase(), {'code': 400, 'msg': 'This hostname is reserved for the Shibboleth tenant'}).not(serverConfig.shibbolethSPHost);
        }
    });
    if (validator.hasErrors()) {
        return callback(validator.getFirstError());
