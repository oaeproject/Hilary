...
var _query = function(indexName, libraryId, visibility, opts, callback) {
    ...
    Cassandra.runPagedColumnQuery('LibraryIndex', 'bucketKey', bucketKey, opts.start, internalLimit, {'reversed': true}, function(err, row, nextToken) {
        if (err) {
            return callback(err);
        } else if (opts.rebuildIfNecessary) {
            // If we are rebuilding, check to see if the query indicates that the library index was not seeded
            if (row.count === 0) {
                // We got 0 items. We should always get something (low and high slugs). Rebuild and reperform the query
                return _rebuildAndQuery(indexName, libraryId, visibility, opts, callback);
            } else if (!opts.start && !row.get(SLUG_HIGH)) {
                ...
                return _rebuildAndQuery(indexName, libraryId, visibility, opts, callback);
            } else if (row.count < internalLimit && !row.get(SLUG_LOW)) {
                ...
                return _rebuildAndQuery(indexName, libraryId, visibility, {'start': opts.start, 'limit': internalLimit}, callback);
            }
        }

        var result = _adjustRowForSlugs(opts.start, opts.limit, row, nextToken);
...
/**
 * Given a row that was queried from a library index, adjust it for the fact that:
 *
 ...
 * @param  {Row}        row         The cassandra row that was returned from the query
 ...
 * @api private
 */
var _adjustRowForSlugs = function(start, limit, row, nextToken) {
    var result = {
        'keys': [],
        'nextToken': nextToken
    };
    ...
    while (result.keys.length > limit) {
        result.keys.pop();
    }
