...
var updatePrincipal = module.exports.updatePrincipal = function(principalId, profileFields, oldEmail, callback) {
    ...
    if (oldEmail && (profileFields.email !== oldEmail)) {
        queries.push({'query': 'DELETE FROM "PrincipalsByEmail" WHERE "email" = ?', 'parameters': [oldEmail]});
    }

    // Note that we only create the mapping for the new email address if the email address
    // can be considered to be verified
    if (profileFields.email && (profileFields.email !== oldEmail) && profileFields.emailVerified) {
