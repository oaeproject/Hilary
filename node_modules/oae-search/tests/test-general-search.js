...
describe('General Search', function() {
    ...
    describe('User Indexing', function() {
        ...
        it('verify index created user', function(callback) {
            TestsUtil.generateTestUsers(camAdminRestContext, 1, function(err, users) {
                ...
                SearchTestsUtil.searchAll(anonymousRestContext, 'general', null, {'resourceTypes': 'user'}, function(err, results) {
                    assert.ok(!err);
        ...
        it('verify index updated user', function(callback) {
            TestsUtil.generateTestUsers(camAdminRestContext, 1, function(err, users) {
                ...
                RestAPI.Authentication.login(anonymousRestContext, mrvisser.restContext.userId, mrvisser.restContext.userPassword, function(err) {
                    ...
                    SearchTestsUtil.searchAll(mrvisser.restContext, 'general', null, {'resourceTypes': 'user', 'q': 'Visser'}, function(err, results) {
                        assert.ok(!err);
                        assert.ok(!_getDocById(results, mrvisser.user.id));
                        ...
                        RestAPI.User.updateUser(mrvisser.restContext, mrvisser.user.id, updateProperties, function(err) {
                            ...
                            SearchTestsUtil.searchAll(anonymousRestContext, 'general', null, {'resourceTypes': 'user', 'q': 'Visser'}, function(err, results) {
                                assert.ok(!err);
                                ...
                                assert.ok(doc.extra);
                                assert.equal(doc.extra.extraProp, 'Some extra property');
    ...
    describe('Group Indexing', function() {
        ...
        it('verify index created group', function(callback) {
            TestsUtil.generateTestGroups(camAdminRestContext, 1, function(err, groups) {
                ...
                SearchTestsUtil.searchAll(anonymousRestContext, 'general', null, {'resourceTypes': 'group'}, function(err, results) {
                    assert.ok(!err);
        ...
        it('verify index updated group', function(callback) {
            ...
            RestAPI.User.createUser(camAdminRestContext, doerUsername, 'password', 'Doer', null, function(err, doer) {
                ...
                TestsUtil.generateTestGroups(doerRestContext, 1, function(err, groups) {
                    assert.ok(!err);
        
                    var xyzTeam = _.values(groups)[0];
                    ...
                    SearchTestsUtil.searchAll(doerRestContext, 'general', null, {'resourceTypes': 'group', 'q': xyzTeam.displayName}, function(err, results) {
                        assert.ok(!err);
                        
                        var doc = _getDocById(results, xyzTeam.id);
                        assert.ok(doc);
                        assert.equal(doc.displayName, xyzTeam.displayName);
        
                        // Update name match the term
                        RestAPI.Group.updateGroup(doerRestContext, xyzTeam.id, {'displayName': 'Team testverifyindexupdatedgroup'}, function(err) {
                            assert.ok(!err);
        
                            // Verify that the group now appears with the search term 'testverifyindexupdatedgroup'
                            SearchTestsUtil.searchAll(doerRestContext, 'general', null, {'resourceTypes': 'group', 'q': 'testverifyindexupdatedgroup'}, function(err, results) {
                                assert.ok(!err);

                                var doc = _getDocById(results, xyzTeam.id);
                                assert.ok(doc);
                                assert.equal(doc.displayName, 'Team testverifyindexupdatedgroup');
    ...
    describe('Content Indexing', function() {
        ...
        it('verify index created content', function(callback) {
            ...
            RestAPI.User.createUser(camAdminRestContext, doerUsername, 'password', 'Doer', null, function(err, doer) {
                ...
                RestAPI.Content.createLink(doerRestContext, 'Sakai Foundation', 'Link to Sakai Foundation Website', 'public', 'http://www.sakaifoundation.org', [], [], function(err, content) {
                    assert.ok(!err);

                    // Verify search term Sakai matches the document
                    SearchTestsUtil.searchAll(doerRestContext, 'general', null, {'resourceTypes': 'content', 'q': 'Sakai'}, function(err, results) {
                        assert.ok(!err);
        ...
        it('verify index updated content', function(callback) {
            ...
            RestAPI.User.createUser(camAdminRestContext, doerUsername, 'password', 'Doer', null, function(err, doer) {
                ...
                RestAPI.Content.createLink(doerRestContext, 'Sakai Foundation', 'Link to Sakai Foundation Website', 'public', 'http://www.sakaifoundation.org', [], [], function(err, content) {
                    ...
                    SearchTestsUtil.searchAll(doerRestContext, 'general', null, {'resourceTypes': 'content', 'q': 'Apereo'}, function(err, results) {
                        assert.ok(!err);
                        assert.ok(!_getDocById(results, content.contentId));

                        // Update the content
                        RestAPI.Content.updateContent(doerRestContext, content.contentId, { 'description': 'Link to Apereo Foundation Website' }, function(err) {
                            assert.ok(!err);
                            
                            // Verify Apereo now matches the updated content item
                            SearchTestsUtil.searchAll(doerRestContext, 'general', null, {'resourceTypes': 'content', 'q': 'Apereo'}, function(err, results) {
                                assert.ok(!err);
                                assert.ok(_getDocById(results, content.contentId));
                                callback();
    ...
    describe('General Search Parameters', function() {
         ...
         */
        it('verifies a variety of valid and invalid values for the resourceTypes parameter', function(callback) {
            ...
            RestAPI.User.createUser(camAdminRestContext, jackUsername, 'password', 'Jacker', null, function(err, jack) {
                assert.ok(!err);
                var jackRestContext = TestsUtil.createTenantRestContext(global.oaeTests.tenants.cam.host, jackUsername, 'password');

                RestAPI.Group.createGroup(jackRestContext, groupAlias, groupAlias, 'A really awesome group', 'public', 'no', [], [], function(err, group) {
                    assert.ok(!err);

                    RestAPI.Content.createLink(jackRestContext, 'Apereo Foundation', 'Link to Apereo Foundation Website', 'public', 'http://www.apereo.org', [], [], function(err, content) {
                        assert.ok(!err);

                        // Verify unspecified resourceTypes searches all
                        SearchTestsUtil.searchAll(jackRestContext, 'general', null, null, function(err, results) {
                            assert.ok(!err);
                            _verifyHasResourceTypes(results, true, true, true);

                            // Verify empty resourceTypes searches all
                            SearchTestsUtil.searchAll(jackRestContext, 'general', null, {'resourceTypes': '', '_refresh': false}, function(err, results) {
                                assert.ok(!err);
                                _verifyHasResourceTypes(results, true, true, true);

                                // Verify non-matching resource type returns nothing
                                SearchTestsUtil.searchAll(jackRestContext, 'general', null, {'resourceTypes': 'not-matching-anything', '_refresh': false}, function(err, results) {
                                    assert.ok(!err);
                                    assert.equal(results.results.length, 0);

                                    // Verify each single resourceType searches just that one
                                    SearchTestsUtil.searchAll(jackRestContext, 'general', null, {'resourceTypes': 'user', '_refresh': false}, function(err, results) {
                                        assert.ok(!err);
                                        _verifyHasResourceTypes(results, true, false, false);

                                        SearchTestsUtil.searchAll(jackRestContext, 'general', null, {'resourceTypes': 'group', '_refresh': false}, function(err, results) {
                                            assert.ok(!err);
                                            _verifyHasResourceTypes(results, false, true, false);

                                            SearchTestsUtil.searchAll(jackRestContext, 'general', null, {'resourceTypes': 'content', '_refresh': false}, function(err, results) {
                                                assert.ok(!err);
                                                _verifyHasResourceTypes(results, false, false, true);

                                                // Verify searching 2 returns just the 2 types
                                                SearchTestsUtil.searchAll(jackRestContext, 'general', null, {'resourceTypes': 'group,content', '_refresh': false}, function(err, results) {
                                                    assert.ok(!err);
                                                    ...
                                                    SearchTestsUtil.searchAll(jackRestContext, 'general', null, {'resourceTypes': ',,content,,', '_refresh': false}, function(err, results) {
                                                        ...
                                                        SearchTestsUtil.searchAll(jackRestContext, 'general', null, {'resourceTypes': ',,user,content,,,', '_refresh': false}, function(err, results) {
                                                            assert.ok(!err);
                                                            ...
                                                            SearchTestsUtil.searchAll(jackRestContext, 'general', null, {'resourceTypes': ',,non-matching,,group,user', '_refresh': false}, function(err, results) {
                                                                assert.ok(!err);
    ...
    describe('Search Paging', function() {
        ...
        it('verify search paging', function(callback) {
            ...
            RestAPI.User.createUser(camAdminRestContext, doerUsername, 'password', 'Doer', null, function(err, doer) {
                ...
                RestAPI.Content.createLink(doerRestContext, 'Sakai Foundation', 'Link to Sakai Foundation Website', 'public', 'http://www.sakaifoundation.org', [], [], function(err, link) {
                    assert.ok(!err);
            ...
            RestAPI.User.createUser(camAdminRestContext, doerUsername, 'password', 'Doer', null, function(err, doer) {
                ...
                RestAPI.Content.createLink(doerRestContext, 'Sakai Foundation', 'Link to Sakai Foundation Website', 'public', 'http://www.sakaifoundation.org', [], [], function(err, content) {
                    ...
                    SearchTestsUtil.searchAll(doerRestContext, 'general', null, {'resourceTypes': 'content', 'q': 'Sakai'}, function(err, results) {
                        ...
                        RestAPI.Content.deleteContent(doerRestContext, content.contentId, function(err) {
                            assert.ok(!err);
                ...
                RestAPI.User.createUser(gtAdminRestContext, darthVaderUsername, 'password', 'Darth Vader', null, function(err, darthVader) {
                    ...
                    RestAPI.User.createUser(camAdminRestContext, jackUsername, 'password', 'Jack McJackerson', null, function(err, jack) {
                        ...
                        RestAPI.User.createUser(camAdminRestContext, janeUsername, 'password', 'Janey McJaneFace', null, function(err, jane) {
                            ...
                            TestsUtil.generateTestGroups(doerRestContext, 5, function(err, groups) {
                                assert.ok(!err);
                                ...
                                TestsUtil.generateGroupHierarchy(doerRestContext, groupIds, 'member', function(err) {
                                    ...
                                    TestsUtil.generateGroupHierarchy(doerRestContext, [groupIds[4], jack.id], 'member', function(err) {
                                        ...
                                        RestAPI.Content.createLink(doerRestContext, 'Test Content 1', 'Test content description 1', 'public', 'http://www.sakaiproject.org/',  [], [groupIds[0]], function(err, contentObj) {
                                            assert.ok(!err);

                                            // Verify anonymous can see it
                                            SearchTestsUtil.searchAll(anonymousRestContext, 'general', null, {'q': 'Content'}, function(err, results) {
                                                assert.ok(!err);

                                                var contentDoc = _getDocById(results, contentObj.contentId);
                                                assert.ok(contentDoc);
                                                assert.equal(contentDoc.resourceSubType, 'link');
                                                assert.equal(contentDoc.displayName, 'Test Content 1');
                                                assert.equal(contentDoc.tenantAlias, 'camtest');
                                                assert.equal(contentDoc.visibility, 'public');
                                                assert.equal(contentDoc.resourceType, 'content');
                                                assert.equal(contentDoc.id, contentObj.contentId);
                                                ...
                                                SearchTestsUtil.searchAll(camAdminRestContext, 'general', null, {'q': 'Content', '_refresh': false}, function(err, results) {
                                                    assert.ok(!err);
                                                    ...
                                                    assert.equal(contentDoc.tenantAlias, contentObj.tenant);
                                                    ...
                                                    SearchTestsUtil.searchAll(janeRestContext, 'general', null, {'q': 'Content', '_refresh': false}, function(err, results) {
                                                        assert.ok(!err);
                                                        ...
                                                        SearchTestsUtil.searchAll(janeRestContext, 'general', null, {'q': 'Content', '_refresh': false}, function(err, results) {
                                                            ...
                                                            SearchTestsUtil.searchAll(jackRestContext, 'general', null, {'q': 'Content', '_refresh': false}, function(err, results) {
                                                                assert.ok(!err);
                            ...
                            TestsUtil.generateTestGroups(doerRestContext, 5, function(err, groups) {
                                ...
                                TestsUtil.generateGroupHierarchy(doerRestContext, groupIds, 'member', function(err) {
                                    ...
                                    TestsUtil.generateGroupHierarchy(doerRestContext, [groupIds[4], jack.id], 'member', function(err) {
                                        ...
                                        RestAPI.Content.createLink(doerRestContext, 'Test Content 1', 'Test content description 1', 'loggedin', 'http://www.sakaiproject.org/',  [], [groupIds[0]], function(err, contentObj) {
                                            assert.ok(!err);
                                            ...
                                            SearchTestsUtil.searchAll(anonymousRestContext, 'general', null, {'q': 'Content'}, function(err, results) {
                                                ...
                                                SearchTestsUtil.searchAll(darthVaderRestContext, 'general', null, {'q': 'Content', 'includeExternal': true, '_refresh': false}, function(err, results) {
                                                    assert.ok(!err);
                                                    assert.ok(!_getDocById(results, contentObj.contentId));
                            ...
                            TestsUtil.generateTestGroups(doerRestContext, 5, function(err, groups) {
                                ...
                                TestsUtil.generateGroupHierarchy(doerRestContext, groupIds, 'member', function(err) {
                                    ...
                                    TestsUtil.generateGroupHierarchy(doerRestContext, [groupIds[4], jack.id], 'member', function(err) {
                                        ...
                                        RestAPI.Content.createLink(doerRestContext, 'Test Content 1', 'Test content description 1', 'private', 'http://www.sakaiproject.org/',  [], [groupIds[0]], function(err, contentObj) {
                                            assert.ok(!err);
            ...
            RestAPI.User.createUser(gtAdminRestContext, darthVaderUsername, 'password', 'Darth Vader', null, function(err, darthVader) {
                ...
                RestAPI.User.createUser(camAdminRestContext, jackUsername, 'password', 'Jack McJackerson', jackOpts, function(err, jack) {
                    ...
                    RestAPI.User.createUser(camAdminRestContext, janeUsername, 'password', 'Janey McJaneFace', null, function(err, jane) {
                        ...
                        SearchTestsUtil.searchAll(darthVaderRestContext, 'general', null, {'resourceTypes': 'user', 'q': 'Jack'}, function(err, results) {
                            assert.ok(!err);
                            var jackDoc = _getDocById(results, jack.id);
                            assert.ok(!jackDoc);

                            // Verify visible for cross-tenant user on external search
                            SearchTestsUtil.searchAll(darthVaderRestContext, 'general', null, {'resourceTypes': 'user', 'q': 'Jack', 'includeExternal': true, '_refresh': false}, function(err, results) {
                                assert.ok(!err);
                                var jackDoc = _getDocById(results, jack.id);
                                assert.ok(jackDoc);
                                assert.equal(jackDoc.id, jack.id);
                                assert.equal(jackDoc.resourceType, 'user');
                                assert.equal(jackDoc.tenantAlias, jack.tenant);
                                ...
                                SearchTestsUtil.searchAll(anonymousRestContext, 'general', null, {'resourceTypes': 'user', 'q': 'Jack', '_refresh': false}, function(err, results) {
                                    assert.ok(!err);
                                    var jackDoc = _getDocById(results, jack.id);
                                    assert.ok(jackDoc);
                                    assert.equal(jackDoc.id, jack.id);
                                    ...
                                    SearchTestsUtil.searchAll(janeRestContext, 'general', null, {'resourceTypes': 'user', 'q': 'Jack', '_refresh': false}, function(err, results) {
                                        assert.ok(!err);
                                        var jackDoc = _getDocById(results, jack.id);
                                        assert.ok(jackDoc);
                                        assert.equal(jackDoc.id, jack.id);
                                        ...
                                        SearchTestsUtil.searchAll(camAdminRestContext, 'general', null, {'resourceTypes': 'user', 'q': 'Jack', '_refresh': false}, function(err, results) {
                                            assert.ok(!err);
                                            var jackDoc = _getDocById(results, jack.id);
                                            assert.ok(jackDoc);
                                            assert.equal(jackDoc.id, jack.id);
    ...
    describe('Group Search Visibility', function() {
                    ...
                    RestAPI.User.createUser(camAdminRestContext, jackUsername, 'password', 'Jack McJackerson', null, function(err, jack) {
                        ...
                        RestAPI.User.createUser(camAdminRestContext, janeUsername, 'password', 'Janey McJaneFace', null, function(err, jane) {
                            ...
                            RestAPI.Group.createGroup(jackRestContext, groupAlias, groupAlias, 'A really awesome group', 'public', 'no', [], [sith.id], function(err, group) {
                                ...
                                SearchTestsUtil.searchAll(anonymousRestContext, 'general', null, {'resourceTypes': 'group', 'q': 'awesome'}, function(err, results) {
                                    assert.ok(!err);
                                    var groupDoc = _getDocById(results, group.id);
                                    assert.ok(groupDoc);
                                    assert.equal(groupDoc.tenantAlias, group.tenant);
                                    ...
                                    SearchTestsUtil.searchAll(darthVaderRestContext, 'general', null, {'resourceTypes': 'group', 'q': 'awesome', '_refresh': false}, function(err, results) {
                                        assert.ok(!err);
                                        var groupDoc = _getDocById(results, group.id);
                                        assert.ok(!groupDoc);

                                        // Verify cross-tenant user can query the group with includeExternal
                                        SearchTestsUtil.searchAll(darthVaderRestContext, 'general', null, {'resourceTypes': 'group', 'q': 'awesome', 'includeExternal': true, '_refresh': false}, function(err, results) {
                                            assert.ok(!err);
                                            var groupDoc = _getDocById(results, group.id);
                                            assert.ok(groupDoc);
                                            assert.equal(groupDoc.tenantAlias, group.tenant);
                                            ...
                                            SearchTestsUtil.searchAll(sithRestContext, 'general', null, {'resourceTypes': 'group', 'q': 'awesome', '_refresh': false}, function(err, results) {
                                                assert.ok(!err);
                                                var groupDoc = _getDocById(results, group.id);
                                                assert.ok(groupDoc);
                                                assert.equal(groupDoc.tenantAlias, group.tenant);
                                                ...
                                                SearchTestsUtil.searchAll(janeRestContext, 'general', null, {'resourceTypes': 'group', 'q': 'awesome', '_refresh': false}, function(err, results) {
                                                    assert.ok(!err);
                                                    var groupDoc = _getDocById(results, group.id);
                                                    assert.ok(groupDoc);
                                                    assert.equal(groupDoc.tenantAlias, group.tenant);
                                                    ...
                                                    SearchTestsUtil.searchAll(jackRestContext, 'general', null, {'resourceTypes': 'group', 'q': 'awesome', '_refresh': false}, function(err, results) {
                                                        assert.ok(!err);
                                                        var groupDoc = _getDocById(results, group.id);
                                                        assert.ok(groupDoc);
                                                        assert.equal(groupDoc.tenantAlias, group.tenant);
        ...
        it('verify loggedin group search visibility', function(callback) {
            ...
            var privateTenantAlias = TestsUtil.generateTestUserId('privateTenant');
            var privateTenantGroupAlias = TestsUtil.generateTestUserId('privateTenantGroup');
            ...
            TestsUtil.createTenantWithAdmin(privateTenantAlias, privateTenantAlias, function(err, privateTenant, privateTenantAdminRestContext) {
                ...
                RestAPI.Config.updateConfig(TestsUtil.createGlobalAdminRestContext(), privateTenantAlias, 'oae-tenants/tenantprivacy/tenantprivate', true, function(err) {
                    assert.ok(!err);
                    ...
                    RestAPI.User.createUser(privateTenantAdminRestContext, lukeSkywalkerUsername, 'password', 'Luke Skywalker', null, function(err, lukeSkywalker) {
                        ...
                        RestAPI.Group.createGroup(lukeSkywalkerRestContext, privateTenantGroupAlias, privateTenantGroupAlias, 'A luke skywalker tenant group', 'loggedin', 'yes', [], [], function(err, privateTenantGroup) {
                            assert.ok(!err);
                                            ...
                                            RestAPI.Group.createGroup(jackRestContext, groupAlias, groupAlias, 'A really awesome group', 'loggedin', 'no', [], [sith.id], function(err, group) {
                                                ...
                                                RestAPI.Group.createGroup(jackRestContext, groupAliasJoinable, groupAliasJoinable, 'A really super joinable group', 'loggedin', 'request', [], [sith.id], function(err, groupJoinable) {
                                                    ...
                                                    SearchTestsUtil.searchAll(anonymousRestContext, 'general', null, {'resourceTypes': 'group', 'q': 'awesome'}, function(err, results) {
                                                        ...
                                                        SearchTestsUtil.searchAll(anonymousRestContext, 'general', null, {'resourceTypes': 'group', 'q': 'joinable', '_refresh': false}, function(err, results) {
                                                            assert.ok(!err);
                                                            ...
                                                            SearchTestsUtil.searchAll(darthVaderRestContext, 'general', null, {'resourceTypes': 'group', 'q': 'awesome', 'includeExternal': true, '_refresh': false}, function(err, results) {
                                                                ...
                                                                SearchTestsUtil.searchAll(darthVaderRestContext, 'general', null, {'resourceTypes': 'group', 'q': 'joinable', 'includeExternal': true, '_refresh': false}, function(err, results) {
                                                                    assert.ok(!err);
                                                                    var groupDoc = _getDocById(results, groupJoinable.id);
                                                                    assert.ok(groupDoc);
                                                                    assert.equal(groupDoc.tenantAlias, groupJoinable.tenant);
                                                                    assert.equal(groupDoc.resourceType, 'group');
                                                                    assert.equal(groupDoc.id, groupJoinable.id);
                                                                    assert.equal(groupDoc.displayName, groupJoinable.displayName);
                                                                    ...
                                                                    SearchTestsUtil.searchAll(sithRestContext, 'general', null, {'resourceTypes': 'group', 'q': 'awesome', '_refresh': false}, function(err, results) {
                                                                        ...
                                                                        SearchTestsUtil.searchAll(janeRestContext, 'general', null, {'resourceTypes': 'group', 'q': 'awesome', '_refresh': false}, function(err, results) {
                                                                            ...
                                                                            SearchTestsUtil.searchAll(jackRestContext, 'general', null, {'resourceTypes': 'group', 'q': 'awesome', '_refresh': false}, function(err, results) {
                                                                                ...
                                                                                SearchTestsUtil.searchAll(lukeSkywalkerRestContext, 'general', null, {'resourceTypes': 'group', 'q': 'skywalker', '_refresh': false}, function(err, results) {
                                                                                    assert.ok(!err);
                                                                                    var groupDoc = _getDocById(results, privateTenantGroup.id);
                                                                                    assert.ok(groupDoc);
                                                                                    assert.equal(groupDoc.tenantAlias, privateTenantGroup.tenant);
                                                                                    assert.equal(groupDoc.resourceType, 'group');
                                                                                    ...
                                                                                    SearchTestsUtil.searchAll(lukeSkywalkerRestContext, 'general', null, {'resourceTypes': 'group', 'q': 'joinable', 'includeExternal': true, '_refresh': false}, function(err, results) {
                                                                                        assert.ok(!err);
                                                                                        var groupDoc = _getDocById(results, groupJoinable.id);
                                                                                        assert.ok(!groupDoc);
                                                                                        ...
                                                                                        SearchTestsUtil.searchAll(jackRestContext, 'general', null, {'resourceTypes': 'group', 'q': 'skywalker', 'includeExternal': true, '_refresh': false}, function(err, results) {
                                                                                            assert.ok(!err);
                                                                                            var groupDoc = _getDocById(results, privateTenantGroup.id);
                                                                                            assert.ok(!groupDoc);
                                                                                            callback();
            ...
            TestsUtil.createTenantWithAdmin(privateTenantAlias, privateTenantAlias, function(err, privateTenant, privateTenantAdminRestContext) {
                ...
                RestAPI.Config.updateConfig(TestsUtil.createGlobalAdminRestContext(), privateTenantAlias, 'oae-tenants/tenantprivacy/tenantprivate', true, function(err) {
                    ...
                    RestAPI.User.createUser(privateTenantAdminRestContext, lukeSkywalkerUsername, 'password', 'Luke Skywalker', null, function(err, lukeSkywalker) {
                        ...
                        RestAPI.Group.createGroup(lukeSkywalkerRestContext, privateTenantGroupAlias, privateTenantGroupAlias, 'A luke skywalker tenant group', 'private', 'yes', [], [], function(err, privateTenantGroup) {
                            assert.ok(!err);
    ...
    describe('Search Analysis', function() {
        ...
        it('verify edgengram analyzer of 3 for auto-suggest', function(callback) {
            ...
            RestAPI.User.createUser(camAdminRestContext, doerUsername, 'password', 'Doer', null, function(err, doer) {
                ...
                RestAPI.Content.createLink(doerRestContext, 'Sakai Xyzforedgengram', 'Link to Sakai Foundation Website', 'public', 'http://www.sakaifoundation.org', [], [], function(err, content) {
                    assert.ok(!err);

                    // Search for just the first 3 characters to ensure it matches "Xyzforedgengram"
                    SearchTestsUtil.searchAll(doerRestContext, 'general', null, {'q': 'Xyz'}, function(err, results) {
                        assert.ok(!err);
                        assert.ok(_getDocById(results, content.contentId));
                        callback();
                    });
        ...
        it('verify search indexing is not case-sensitive', function(callback) {
            ...
            RestAPI.User.createUser(camAdminRestContext, doerUsername, 'password', 'Doer', null, function(err, doer) {
                ...
                RestAPI.Content.createLink(doerRestContext, 'Sakai Xyzforedgengram', 'Link to Sakai Foundation Website', 'public', 'http://www.sakaifoundation.org', [], [], function(err, content) {
                    ...
                    // Search for just the first 3 characters to ensure it matches "Xyzforedgengram"
                    SearchTestsUtil.searchAll(doerRestContext, 'general', null, {'q': 'sakai'}, function(err, results) {
                        assert.ok(!err);
                        assert.ok(_getDocById(results, content.contentId));
                        callback();
                    });
    ...
    describe('Search Options', function() {
        ...
        it('verify resourceType scope param', function(callback) {
            ...
            RestAPI.User.createUser(camAdminRestContext, jackUsername, 'password', 'Jack JickMackerson', null, function(err, jack) {
                ...
                RestAPI.Group.createGroup(camAdminRestContext, groupAlias, groupAlias, 'A really awesome group', 'public', 'no', [], [], function(err, group) {
                    assert.ok(!err);
                    ...
                    RestAPI.Content.createLink(camAdminRestContext, 'Sakai Foundation', 'Link to Sakai Foundation Website', 'public', 'http://www.sakaifoundation.org', [], [], function(err, content) {
                        ...
                        SearchTestsUtil.searchAll(jackCtx, 'general', null, {'resourceTypes': 'user'}, function(err, results) {
                            ...
                            SearchTestsUtil.searchAll(jackCtx, 'general', null, {'resourceTypes': 'group', '_refresh': false}, function(err, results) {
                                assert.ok(!err);
                                ...
                                SearchTestsUtil.searchAll(jackCtx, 'general', null, {'resourceTypes': 'content', '_refresh': false}, function(err, results) {
                                    assert.ok(!err);
                                    assert.ok(_getDocById(results, content.contentId));
                                    assert.equal(_.filter(results.results, function(result) { return result.resourceType === 'user' }).length, 0);
                                    assert.equal(_.filter(results.results, function(result) { return result.resourceType === 'group' }).length, 0);
        ...
        /**
         * Test that verifies when includeIndirect is not specified, private resources by indirect access are returned in general search, and when
         * it is disabled, no indirect resources are returned in general search.
         */
        it('verify includeIndirect option', function(callback) {
            var jackUsername = TestsUtil.generateTestUserId('jack');
            var group1Alias = TestsUtil.generateTestUserId('group1');
            var group2Alias = TestsUtil.generateTestUserId('group2');

            // Create a group membership hierarchy: group2 -> group1 -> jack; to test indirect searches
            RestAPI.User.createUser(camAdminRestContext, jackUsername, 'password', 'Jack JickMackerson', null, function(err, jack) {
                assert.ok(!err);
                var jackCtx = TestsUtil.createTenantRestContext(global.oaeTests.tenants.cam.host, jackUsername, 'password');

                RestAPI.Group.createGroup(camAdminRestContext, group1Alias, group1Alias, 'A really awesome group', 'private', 'no', [], [jack.id], function(err, group1) {
                    assert.ok(!err);

                    RestAPI.Group.createGroup(camAdminRestContext, group2Alias, group2Alias, 'A really awesome group', 'private', 'no', [], [group1.id], function(err, group2) {
                        assert.ok(!err);

                        // Search with jack while including indirect, to sanity check that the indirect group is returned
                        SearchTestsUtil.searchAll(jackCtx, 'general', null, {'q': 'awesome'}, function(err, results) {
                            assert.ok(!err);

                            // Verify that using default (includeIndirect === true), we get both groups
                            assert.ok(_getDocById(results, group1.id));
                            assert.ok(_getDocById(results, group2.id));

                            // Search with jack *exluding* indirect groups, verify group2 is not returned
                            SearchTestsUtil.searchAll(jackCtx, 'general', null, {'q': 'awesome', 'includeIndirect': false, '_refresh': false}, function(err, results) {
                                assert.ok(!err);

                                // Verify that when includeIndirect is disabled, we don't get group2, which is indirectly associated to jack
                                assert.ok(_getDocById(results, group1.id));
                                assert.ok(!_getDocById(results, group2.id));
                                callback();
                            });
