...
var test = module.exports.test = function(ctx, content, callback) {
    if (content.resourceSubType === 'file' && MediaCoreConfig.getValue(content.tenant.alias, 'mediacore', 'enabled')) {
...
var generatePreviews = module.exports.generatePreviews = function(ctx, content, callback) {
    ...
    ctx.download(function(err, path) {
        ...
        var tenantAlias = content.tenant.alias;

        // Create a media item
        MediaCoreUtil.signedRequest(tenantAlias, 'post', '/api2/media', null, {
            ...
            MediaCoreDAO.saveContentRevisionId(mediaId.toString(), ctx.contentId, ctx.revisionId, function(err) {
                if (err) {
                    return callback(err);
                }

                // Ask MediaCore to let us upload a file to the media item
                MediaCoreUtil.signedRequest(tenantAlias, 'post', util.format('/api2/media/%s/files', mediaId), null, {
                    ...
                    MediaCoreUtil.signedRequest(tenantAlias, 'post', util.format('/api2/media/%s/publish', mediaId), null, null, function(err, res, body) {
                        ...
                        var uploadReq = request.post(uploadProtocol.upload_url, function(err, res, body) {
                            ...
                            if (uploadProtocol.postprocess_url) {
                                MediaCoreUtil.signedRequest(tenantAlias, 'post', uploadProtocol.postprocess_url, null, {
 ...
 * @api private
 */
var _addMetadata = function(ctx, content, mediaCoreId, callback) {
    ctx.addPreviewMetadata('mediaCoreId', mediaCoreId);

    // Get thumbnail urls
    MediaCoreUtil.signedRequest(content.tenant.alias, 'get', util.format('/api2/media/%s/thumbs', mediaCoreId), null, null, function(err, res, body) {
        ...
        try {
            ...
            log().error({'err': ex, 'body': body}, 'Received invalid response from MediaCore when getting thumbnail urls');
