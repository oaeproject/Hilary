...
var log = require('oae-logger').logger('lti-rest');
 ...
 * @Server      tenant
 * @Method      GET
 * @Path        /lti/{groupId}/{toolId}
 * @PathParam   {string}                groupId             The id of the group this LTI tool belongs to
 * @PathParam   {string}                toolId              The id of the LTI tool to get
 * @Return      {ToolLaunchParams}                          An object containing information required to launch the LTI tool
 * @HttpResponse                        200                 Tool launch info available
 * @HttpResponse                        404                 The LTI tool could not be found
 */
OAE.tenantRouter.on('get', '/api/lti/:groupId/:toolId', function(req, res) {
    LtiApi.getLtiTool(req.ctx, req.params.toolId, req.params.groupId, function(err, tool) {
        if (err) {
            return res.send(err.code, err.msg);
        }

        return res.send(200, tool);
    });
});
 ...
 * @PathParam   {string}        groupId                 The id of the group this LTI tool belongs to
 * @FormParam   {string}        id                      The id of the new LTI tool (resource_link_id)
 ...
 */
OAE.tenantRouter.on('post', '/api/lti/:groupId/create', function(req, res) {
    var opts = {
        'displayName': req.body.displayName,
        'description': req.body.description
    };
    LtiApi.addLtiTool(req.ctx, req.body.id, req.params.groupId, req.body.url, req.body.secret, req.body.key, opts, function(err, tool) {
        if (err) {
            return res.send(err.code, err.msg);
        }

        return res.send(201, tool);
    });
});
 ...
 */
OAE.tenantRouter.on('get', '/api/lti/:groupId', function(req, res) {
    LtiApi.getLtiTools(req.ctx, req.params.groupId, function(err, ltiTools) {
        if (err) {
            return res.send(err.code, err.msg);
        }

        return res.send(200, ltiTools);
    });
});
 ...
 * @PathParam   {string}        toolId      The id of the LTI tool to delete
 ...
 */
OAE.tenantRouter.on('delete', '/api/lti/:groupId/:toolId', function(req, res) {
    LtiApi.deleteLtiTool(req.ctx, req.params.toolId, req.params.groupId, function(err) {
        if (err) {
            return res.send(err.code, err.msg);
        }
