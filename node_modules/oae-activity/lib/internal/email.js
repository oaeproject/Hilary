...
var _collectImmediateBucket = function(bucketNumber, callback) {
    collectMails(bucketNumber, PrincipalsConstants.emailPreferences.IMMEDIATE, callback);
};
...
var _collectDailyBucket = function(bucketNumber, callback) {
    if (_isDailyCycle()) {
        return collectMails(bucketNumber, PrincipalsConstants.emailPreferences.DAILY, callback);
    } else {
...
var _collectWeeklyBucket = function(bucketNumber, callback) {
    if (_isWeeklyCycle()) {
        return collectMails(bucketNumber, PrincipalsConstants.emailPreferences.WEEKLY, callback);
    } else {
        return callback(null, true);
    }
};
 ...
 */
var collectMails = module.exports.collectMails = function(bucketNumber, emailPreference, callback) {
    ...
    var now = new Date();
    var bucketId = _createEmailBucketId(bucketNumber, emailPreference, now.getDay(), now.getHours() + 1);
...
var _createEmailBucketIdForUser = function(user) {
    ...
    if (emailPreference !== PrincipalsConstants.emailPreferences.IMMEDIATE) {
         ...
         * For example, assume the server is in Holland (GMT+1), the user's timezone is Islamabad (GMT+5) and mails
         * need to be delivered at 2am. The mails needs to leave the server at 22h (GMT+1 time) so it arrives when
