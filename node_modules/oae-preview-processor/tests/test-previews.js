...
describe('Preview processor', function() {
    ...
    describe('Preview generation', function() {
        ...
        before(function(callback) {
            ...
            // Purge anything that is hanging around in the preview processing queue
        ...
        it('verify a push notification is sent out on preview generation completion', function(callback) {
            ...
            MQTestUtil.whenTasksEmpty(PreviewConstants.MQ.TASK_GENERATE_PREVIEWS, function() {
                ...
                TestsUtil.generateTestUsers(signedAdminRestContext, 1, function(err, response) {
                    assert.ok(!err);
                    var mrvisser = _.values(response)[0];
                    ...
                    RestAPI.User.getMe(mrvisser.restContext, function(err, mrvisserFullMeData) {
                        ...
                        RestAPI.Content.createFile(mrvisser.restContext, 'Test Content 1', 'Test content description 1', 'private', getImageStream,  [], [], function(err, contentObj) {
                            ...
                            MQTestUtil.whenTasksEmpty(PreviewConstants.MQ.TASK_GENERATE_PREVIEWS, function() {
                                ...
                                // Trigger a re-index for that piece of content
                                // The test is considered succesfull if we receive a push notification on our client
                                RestAPI.Content.getContent(mrvisser.restContext, contentObj.id, function(err, contentObj) {
                                    ...
                                    ActivityTestsUtil.getFullySetupClient(data, function(client) {
                                        ...
                                        var collectedActivities = false;
                                        var receivedPushMsg = false;

                                        client.on('message', function(message) {
                                            if (isPreviewed && message.activity && message.activity['oae:activityType'] === 'previews-finished') {
                                                receivedPushMsg = true;

                                                // Ensure that there is an image there
                                                assert.ok(_.isObject(message.activity.object.image));
                                                assert.ok(message.activity.object.image.url);

                                                // Sometimes the push message gets delivered before the REST request has returned
                                                if (receivedPushMsg && collectedActivities) {
                                                    return callback();
                                                }
                                            }
                                        });
                                        ...
                                        RestAPI.Previews.reprocessPreview(signedAdminRestContext, contentObj.id, contentObj.latestRevisionId, function(err) {
                                            ...
                                            // Wait till the image has been processed and collect the activites
                                            MQTestUtil.whenTasksEmpty(PreviewConstants.MQ.TASK_GENERATE_PREVIEWS, function() {
                                                isPreviewed = true;

                                                ActivityTestsUtil.collectAndGetActivityStream(mrvisser.restContext, null, null, function(err, activityStream) {
                                                    assert.ok(!err);
                                                    collectedActivities = true;

                                                    // Sometimes the REST requests returns before the push message has been delivered
                                                    if (receivedPushMsg && collectedActivities) {
                                                        return callback();
                                                    }
                                                });
