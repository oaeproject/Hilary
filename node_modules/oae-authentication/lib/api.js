    ...
    _getUserIdFromLoginId(loginId, function(err, userId) {
        if (err && err.code !== 404) {
            ...
            OaeUtil.invokeIfNecessary(opts.invitationToken, AuthzInvitationsDAO.getEmailByToken, opts.invitationToken, function(err, email) {
                ...
                // If an email address was provided by a non authoritative source such as local user
                // sign up or a Google / Twitter sign in we should check whether the email address
                // matches the tenant's configured email domain
                var shouldCheckEmail = (opts.email && !opts.authoritative);
...
var _validateEmailBelongsToTenant = function(ctx, email, callback) {
    // There's nothing to do if the tenant has no configured email domain
    if (!ctx.tenant().emailDomain) {
        return callback();
    }
    ...
    if (ctx.tenant().alias !== tenant.alias) {
        return callback({'code': 400, 'msg': 'The provided email address does not match the configured tenant email domain'});
    }
