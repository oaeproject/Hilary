...
var refreshConfiguration = module.exports.refreshConfiguration = function(config, callback) {
    ...
    // Configure redis. Either the container-managed one, or a dedicated one if config.activity.redis was configured
    ...
    if (config.processActivityJobs && !boundWorker) {
        ...
        return MQ.bind(ActivityConstants.mq.TASK_ACTIVITY, ActivityRouter.routeActivity, {'prefetchCount': config.maxConcurrentRouters}, callback);
 ...
 * @param   {String[]}       router.callback.routes                 The routes to which the activity should be delivered. For routing to user and group feeds, the route is typically their principalId.
