...
DiscussionsAPI.on(DiscussionsConstants.events.DELETED_DISCUSSION_MESSAGE, function(ctx, message, discussion, deleteType) {
    return MessageBoxSearch.deleteMessageSearchDocument('discussion_message', discussion.id, message);
});
...
var _produceDiscussionMessageDocuments = function(resources, callback, _documents) {
    ...
    if (AuthzUtil.getResourceFromId(resource.id).resourceType !== 'd') {
        // Produce only children for discussions
        return _produceDiscussionMessageDocuments(resources, callback, _documents);
    } else if (resource.messages) {
        var documents = MessageBoxSearch.createMessageSearchDocuments('discussion_message', resource.id, resource.messages);
    ...
    MessageBoxSearch.createAllMessageSearchDocuments('discussion_message', resource.id, resource.id, function(err, documents) {
        if (err) {
