 ...
 */
OAE.registerPreShutdownHandler('oae-activity-buckets', null, function(
  callback,
) {
  log().info(
 ...
 */
var getBucketNumber = (module.exports.getBucketNumber = function(
  str,
  numberOfBuckets,
) {
 ...
 */
var collectAllBuckets = (module.exports.collectAllBuckets = function(
  type,
  numberOfBuckets,
  ...
  if (
    ...
    bucketsInfo[type].maxConcurrentCollections
  ) {
    log().trace(
      { type: type },
      'Aborting collection due to max concurrent collections count reached',
    );
    return callback();
  }
  bucketsInfo[type].currentConcurrentCollectionCount++;
  ...
  log().trace(
    { type: type },
    'Beginning collection of %s buckets',
    bucketNumbers.length,
  );
...
var _collectBuckets = function(type, bucketNumbers, callback, _errs) {
  ...
  _collectBucket(type, bucketNumber, function(err) {
    if (err) {
      log().warn(
        { err: err, bucketNumber: bucketNumber, type: type },
        'Error collecting aggregate bucket',
      );
...
var _collectBucket = function(type, bucketNumber, callback) {
  ...
  log().trace(
    'Attempting collection of bucket number %s %s',
    type,
    bucketNumber,
  );
  ...
  Locking.acquire(lockKey, bucketInfo.collectionExpiry, function(err, lockId) {
    ...
    log().trace(
      { lockId: lockId, type: type },
      'Acquired a lock on bucket number %s',
      bucketNumber,
    );
