...
var getConfig = module.exports.getConfig = function(tenantAlias) {
    ...
    var region = ZencoderConfig.getValue(tenantAlias, 'zencoder', 'amazons3-region');
    var bucketName = ZencoderConfig.getValue(tenantAlias, 'zencoder', 'amazons3-bucket');
    var accessKey = ZencoderConfig.getValue(tenantAlias, 'zencoder', 'amazons3-access-key');
    var secretKey = ZencoderConfig.getValue(tenantAlias, 'zencoder', 'amazons3-secret-key');

    // Check for tenant overrides
    var tenantEnabled = ZencoderConfig.getValue(tenantAlias, 'zencoder', 'enabled');
    var apiKey = ZencoderConfig.getValue(tenantAlias, 'zencoder', 'apiKey');

    return {
        'enabled': apiKey && tenantEnabled ? tenantEnabled : config['enabled'],
        'apiKey': apiKey && tenantEnabled ? apiKey : config['apiKey'],
        'amazons3-region': region || config['amazons3-region'],
        'amazons3-bucket': bucketName || config['amazons3-bucket'],
        'amazons3-access-key': accessKey || config['amazons3-access-key'],
        'amazons3-secret-key': secretKey || config['amazons3-secret-key'],
        'watermark': !tenantEnabled ? config['watermark'] : null
    };
};
...
var createJob = module.exports.createJob = function(tenantAlias, path, contentId, revisionId, mime, callback) {
    ...
    var options = {
        ...
        // We store the S3 bucket filepath so it can later be used to create a signed URL
    ...
    log().info({'contentId': contentId, 'revisionId': revisionId}, 'Creating Zencoder job');
