 ...
 */
var getActivities = (module.exports.getActivities = function(
  activityStreamId,
  start,
  limit,
  callback,
 ...
 */
var getActivitiesFromStreams = (module.exports.getActivitiesFromStreams = function(
  activityStreamIds,
  start,
  callback,
) {
 ...
 * @param  {Object}    callback.err        An error that occurred, if any
 */
var deliverActivities = (module.exports.deliverActivities = function(
  routedActivities,
  callback,
) {
  callback =
 ...
 * @param  {Object}    callback.err        An error that occurred, if any
 */
var deleteActivities = (module.exports.deleteActivities = function(
  routeActivityIds,
  callback,
) {
  ...
  _.each(routeActivityIds, function(activityIds, route) {
    ...
    _.each(activityIds, function(activityId) {
      queries.push({
        query:
          'DELETE FROM "ActivityStreams" WHERE "activityStreamId" = ? AND "activityId" = ?',
 ...
 * @param  {Object}     callback.err        An error that occurred, if any
 */
var clearActivityStream = (module.exports.clearActivityStream = function(
  activityStreamId,
  callback,
) {
  Cassandra.runQuery(
 ...
 * @param  {Object}        callback.err        An error that occurred, if any
 */
var saveQueuedActivities = (module.exports.saveQueuedActivities = function(
  activityBuckets,
  callback,
) {
 ...
 */
var getQueuedActivities = (module.exports.getQueuedActivities = function(
  bucketNumber,
  limit,
  callback,
) {
  limit = OaeUtil.getNumberParam(
    limit,
    ActivitySystemConfig.getConfig().collectionBatchSize,
  );
  ...
  redisClient.zrange(_createBucketCacheKey(bucketNumber), 0, limit, function(
    err,
    routedActivities,
  ) {
    if (err) {
    ...
    var numToDelete = _.size(routedActivities);
    ...
    _.each(routedActivities, function(routedActivity) {
      try {
        ...
        queuedActivities[
          _createRoutedActivityKey(routedActivity)
        ] = routedActivity;
      } catch (err) {
        var truncatedRoutedActivity = JSON.stringify(routedActivity);
        if (truncatedRoutedActivity) {
          truncatedRoutedActivity = truncatedRoutedActivity.slice(0, 300);
        }

        log().error(
          { err: err, routedActivity: truncatedRoutedActivity },
 ...
 * @param  {Object}    callback.err        An error that occurred, if any
 */
var deleteQueuedActivities = (module.exports.deleteQueuedActivities = function(
  bucketNumber,
  numberToDelete,
  callback,
) {
  ...
  log().trace(
    { bucketNumber: bucketNumber, numberToDelete: numberToDelete },
  ...
  redisClient.zremrangebyrank(
    _createBucketCacheKey(bucketNumber),
    0,
    numberToDelete - 1,
    callback,
  );
 ...
 */
var getAggregateStatus = (module.exports.getAggregateStatus = function(
  aggregateKeys,
  callback,
) {
    ...
    for (var i = 0; i < results.length; i++) {
      ...
      if (result) {
        try {
          ...
          log().warn(
            { err: ex },
            'Found invalid aggregate status entry. Skipping.',
          );
        }
      }
    }
 ...
 * @param  {Object}    callback.err                         An error that occurred, if any
 */
var indexAggregateData = (module.exports.indexAggregateData = function(
  ...
  var maxExpiryAgo =
    Date.now() - ActivitySystemConfig.getConfig().aggregateMaxExpiry * 1000;

  _.each(statusUpdatesByActivityStreamId, function(
    statusUpdatesByActivityStreamId,
    activityStreamId,
  ) {
    _.each(statusUpdatesByActivityStreamId, function(status, aggregateKey) {
      ...
      var activeAggregatesForActivityStreamKey = _createActiveAggregatesForActivityStreamKey(
        activityStreamId,
      );
      multi.zadd(
 ...
 * @param  {Object}    callback.err    An error that occurred, if any
 */
var deleteAggregateData = (module.exports.deleteAggregateData = function(
  aggregateKeys,
  callback,
) {
  if (_.isEmpty(aggregateKeys)) {
  ...
  var aggregateCacheKeysToDelete = _getAggregateCacheKeysForAggregateKeys(
    aggregateKeys,
  );
 ...
 * @param  {Object}     callback.err        An error that occurred, if any
 */
var resetAggregationForActivityStreams = (module.exports.resetAggregationForActivityStreams = function(
  activityStreamIds,
  callback,
) {
  // 1. Get all the active aggregates for these activity streams
  getActiveAggregateKeysForActivityStreams(activityStreamIds, function(
    err,
    activeAggregateKeysForActivityStreams,
  ) {
    if (err) {
      log().error(
        { err: err, activityStreamIds: activityStreamIds },
    ...
    _.each(activeAggregateKeysForActivityStreams, function(
      activeAggregateKeys,
      index,
    ) {
      ...
      if (!_.isEmpty(activeAggregateKeys)) {
        ...
        allActiveAggregateKeys = allActiveAggregateKeys.concat(
          activeAggregateKeys,
        );
      }
    });
    ...
    var activeAggregateCacheKeysToDelete = _getAggregateCacheKeysForAggregateKeys(
 ...
 */
var getActiveAggregateKeysForActivityStreams = (module.exports.getActiveAggregateKeysForActivityStreams = function(
  activityStreams,
  callback,
) {
  var multi = redisClient.multi();
  ...
  _.each(activityStreams, function(activityStream) {
    var activeAggregatesKey = _createActiveAggregatesForActivityStreamKey(
      activityStream,
    );
  ...
  multi.exec(function(err, activeAggregateKeysForActivityStreams) {
    if (err) {
      log().error(
        { err: err, activityStreams: activityStreams },
        'Failed to get the active aggregate keys for a set of activity streams',
      );
      return callback({
        code: 500,
 ...
 */
var getAggregatedEntities = (module.exports.getAggregatedEntities = function(
  aggregateKeys,
  callback,
) {
  if (_.isEmpty(aggregateKeys)) {
  ...
  _.each(aggregateKeys, function(aggregateKey) {
    multiGetAggregateEntities.hgetall(
  ...
  multiGetAggregateEntities.exec(function(err, results) {
    ...
    _fetchEntitiesByIdentities(_.keys(entityIdentities), function(
      err,
      entitiesByIdentity,
    ) {
      if (err) {
      ...
      _.each(results, function(result, i) {
        if (result) {
          ...
          _.each(result, function(identity, entityKey) {
            ...
            aggregateEntities[aggregateKey][entityType][entityKey] =
              entitiesByIdentity[identity];
          });
        }
      });
...
var _fetchEntitiesByIdentities = function(entityIdentities, callback) {
  ...
  var entityIdentityCacheKeys = _.map(entityIdentities, function(
    entityIdentity,
  ) {
 ...
 * @param  {Object}    callback.err    An error that occurred, if any
 */
var saveAggregatedEntities = (module.exports.saveAggregatedEntities = function(
  aggregates,
  callback,
) {
  ...
  _.each(aggregates, function(aggregate, aggregateKey) {
    ...
    var aggregateActorsKey = _createAggregateEntityCacheKey(
      aggregateKey,
      'actors',
    );
    var aggregateObjectsKey = _createAggregateEntityCacheKey(
      aggregateKey,
      'objects',
    );
    var aggregateTargetsKey = _createAggregateEntityCacheKey(
      aggregateKey,
      'targets',
    );
    ...
    multi.expire(
      aggregateActorsKey,
      ActivitySystemConfig.getConfig().aggregateIdleExpiry,
    );
    multi.expire(
      aggregateObjectsKey,
      ActivitySystemConfig.getConfig().aggregateIdleExpiry,
    ...
    log().trace(
      { msetArgs: msetArgs },
      'Persisting entities in redis by identity.',
    );
 ...
 * @param  {Object}     callback.err    An error that occurred, if any
 */
var saveQueuedUserIdsForEmail = (module.exports.saveQueuedUserIdsForEmail = function(
  emailBuckets,
  callback,
) {
  ...
  _.each(emailBuckets, function(userIds, bucketId) {
    _.each(userIds, function(userId) {
      queries.push({
        query:
          'INSERT INTO "EmailBuckets" ("bucketId", "userId") VALUES (?, ?)',
 ...
 */
var getQueuedUserIdsForEmail = (module.exports.getQueuedUserIdsForEmail = function(
  bucketId,
  start,
  limit,
  callback,
 ...
 * @param  {Object}     callback.err        An error that occurred, if any
 */
var unqueueUsersForEmail = (module.exports.unqueueUsersForEmail = function(
  bucketId,
  userIds,
  callback,
) {
 ...
 * @param  {Object}     callback.err    An error that occurred, if any
 */
var clearNotificationsUnreadCount = (module.exports.clearNotificationsUnreadCount = function(
  userId,
  callback,
) {
 ...
 */
var incrementNotificationsUnreadCounts = (module.exports.incrementNotificationsUnreadCounts = function(
  userIdsIncrBy,
  callback,
) {
  // Filter out those users whose notification count should be "incremented" by 0
  var userIds = _.chain(userIdsIncrBy)
    .keys()
...
var _createAggregateEntityCacheKey = function(aggregateKey, entityType) {
  ...
  return util.format(
    'oae-activity:aggregate:%s:%s:entities',
    aggregateKey,
    entityType,
  );
};
