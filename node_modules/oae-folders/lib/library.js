...
LibraryAPI.Index.registerLibraryIndex(
  FoldersConstants.library.CONTENT_LIBRARY_INDEX_NAME,
  {
    pageResources: function(libraryId, start, limit, callback) {
      ...
      FoldersDAO.getContentItems(libraryId, opts, function(
        err,
        contentItems,
        nextToken,
      ) {
 ...
 */
FoldersAPI.when(FoldersConstants.events.CREATED_FOLDER, function(
  ctx,
  folder,
  memberChangeInfo,
  callback,
 ...
 */
FoldersAPI.when(FoldersConstants.events.DELETED_FOLDER, function(
  ctx,
  folder,
  removedMemberIds,
  callback,
 ...
 */
FoldersAPI.when(FoldersConstants.events.UPDATED_FOLDER_MEMBERS, function(
  ctx,
  folder,
  memberChangeInfo,
  opts,
  ...
  FoldersAuthz.getFoldersForContent(newContentObj.id, function(err, folders) {
    ...
    _.each(folders, function(folder) {
      ...
      FoldersContentLibrary.remove(folder, [oldContentObj], function(err) {
        if (err) {
          log().error(
            { err: err, folderId: folderId, contentId: oldContentObj.id },
  ...
  FoldersDAO.getFoldersByGroupIds(groupIds, function(err, folders) {
    if (err) {
      ...
      log().error(
        ...
        'Unable to remove content from a folder',
      );
      return;
    }
