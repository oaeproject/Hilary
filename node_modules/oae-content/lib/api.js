...
var _getFullContentProfile = function(ctx, contentObj, isManager, callback) {
    ...
    PrincipalsUtil.getPrincipal(ctx, contentObj.createdBy, function(err, createdBy) {
        ...
        var canShare = true;
        AuthzPermissions.canShare(ctx, contentObj, null, function(err) {
            if (err && err.code !== 401) {
                return callback(err);
            } else if (err) {
                canShare = false;
...
var _createContent = function(ctx, contentId, revisionId, resourceSubType, displayName, description, visibility, additionalMembers, folderIds, otherValues, revisionData, callback) {
    ...
    _.each(additionalMembers, function(role) {
    ...
    canManageFolders(ctx, folderIds, function(err, folders) {
        ...
        ResourceActions.create(ctx, additionalMembers, createFn, function(err, content, revision, members) {
            if (err) {
                return callback(err);
            }
...
var shareContent = module.exports.shareContent = function(ctx, contentId, principalIds, callback) {
    ...
    ContentDAO.Content.getContent(contentId, function(err, content) {
        ...
        ResourceActions.share(ctx, content, principalIds, AuthzConstants.role.VIEWER, function(err, newMembers, memberRoles) {
            if (err) {
                return callback(err);
            }

            var newMemberIds = _.pluck(newMembers, 'id');
            var appliedPermissionChanges = _.pluck(memberRoles, newMemberIds);
            ...
            _updateContentLibraries(content, [], function(err, content) {
                ...
                ContentMembersLibrary.insert(content, newMembers, function(err) {
                    if (err) {
                        log().warn({
                            'err': err,
                            'contentId': content.id,
                    ...
                    ContentAPI.emit(ContentConstants.events.UPDATED_CONTENT_MEMBERS, ctx, content, appliedPermissionChanges, newMemberIds, [], []);
                    return callback();
                });
            });
        });
...
var setContentPermissions = module.exports.setContentPermissions = function(ctx, contentId, permissionChanges, callback) {
    ...
    getContent(ctx, contentId, function(err, content) {
        ...
        ResourceActions.setRoles(ctx, content, permissionChanges, function(err, memberRoles, newMembers, updatedMembers, removedMembers) {
            if (err) {
                return callback(err);
            }
...
var removeContentFromLibrary = module.exports.removeContentFromLibrary = function(ctx, libraryOwnerId, contentId, callback) {
    ...
    ContentDAO.Content.getContent(contentId, function(err, content) {
        ...
        PrincipalsDAO.getPrincipal(libraryOwnerId, function(err, libraryOwner) {
            ...
            AuthzPermissions.canRemoveRole(ctx, libraryOwner, content, function(err) {
                if (err) {
                    return callback(err);
                }
                ...
                AuthzAPI.updateRoles(contentId, permissionChanges, function(err) {
                    ...
                    LibraryAPI.Index.remove(ContentConstants.library.CONTENT_LIBRARY_INDEX_NAME, [entry], function(err) {
                        ...
                        ContentAPI.emit(ContentConstants.events.UPDATED_CONTENT_MEMBERS, ctx, content, permissionChanges, [], [], [libraryOwnerId]);
                        return callback();
                    });
                });
            });
