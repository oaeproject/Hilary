...
ActivityEmitter.on(ActivityConstants.events.DELIVERED_ACTIVITIES, function(
  ...
  var userIdsIncrBy = {};
  _.each(deliveredActivityInfos, function(streams, resourceId) {
    _.each(streams, function(activityInfo, streamType) {
  ...
  incrementNotificationsUnread(userIdsIncrBy, function(err) {
    if (err) {
      log().error(
        { err: new Error(err.msg), userIdsIncrBy: userIdsIncrBy },
...
var markNotificationsRead = (module.exports.markNotificationsRead = function(
  ...
  ActivityDAO.clearNotificationsUnreadCount(user.id, function(err) {
    ...
    PrincipalsDAO.updatePrincipal(user.id, profileFields, function(err) {
      ...
      var notificationActivityStreamId = ActivityUtil.createActivityStreamId(
        user.id,
        'notification',
      );
      ActivityAggregator.resetAggregationForActivityStreams([
        notificationActivityStreamId,
      ]);
      ...
      if (
        user.emailPreference === PrincipalsConstants.emailPreferences.IMMEDIATE
      ) {
        ...
        ActivityDAO.clearActivityStream(emailActivityStreamId, function(err) {
          if (err) {
            log().warn(
              { err: err },
              'Could not clear the email activity stream',
            );
          }
        });
      }
 ...
 * @param  {Object}     callback.err        An error that occurred, if any
 */
var incrementNotificationsUnread = (module.exports.incrementNotificationsUnread = function(
  userIdIncrs,
  callback,
) {
  /*!
     ...
     */
  ActivityDAO.incrementNotificationsUnreadCounts(userIdIncrs, function(
    err,
    newValues,
  ) {
    if (err) {
 ...
 */
var whenNotificationsEmpty = (module.exports.whenNotificationsEmpty = function(
  handler,
) {
