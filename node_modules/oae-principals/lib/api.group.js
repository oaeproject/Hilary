...
var PrincipalsGuestsAPI = require('./api.guests');
 ...
 * @param  {Object}      memberChanges   Object where the keys represent the principal ids which should be updated/added/removed or email addresses that should be invited. The value is a string representing the new role. If false is passed in, the membership for that principal will be removed e.g. {'user1': 'manager', 'user2': 'viewer', 'user3': false}
 * @param  {Function}    [callback]      Standard callback function
...
var setGroupMembers = module.exports.setGroupMembers = function(ctx, groupId, memberChanges, callback) {
    ...
    PrincipalsDAO.getPrincipal(groupId, function(err, group) {
        ...
        _canManage(ctx, group.id, function(err, canManage) {
            ...
            PrincipalsGuestsAPI.getPrincipalsAndCreateGuests(ctx, _.keys(memberChanges), function(err, principals) {
                if (err) {
                    return callback(err);
                }

                var permissions = {};
                _.each(memberChanges, function(role, principal) {
                ...
                AuthzAPI.computeMemberRolesAfterChanges(groupId, permissions, function(err, computedPermissions, newMemberIds, updatedMemberIds, removedMemberIds) {
                    if (err) {
                        return callback(err);
                    ...
                    AuthzAPI.canInteract(ctx, group.tenant.alias, _.values(principals), function(err, canInteract, illegalPrinicpalIds) {
                        if (err) {
                            return callback(err);
                        } else if (!canInteract) {
                        ...
                        AuthzAPI.updateRoles(group.id, permissions, function(err) {
                            ...
                            PrincipalsDAO.updatePrincipal(group.id, {'lastModified': newLastModified}, null, function(err) {
                                if (err) {
                                    return callback(err);
                                }
                                ...
                                _touchMembershipLibraries(group, oldLastModified, newMemberIds.slice(), removedMemberIds.slice(), function(err) {
                                    ...
                                    PrincipalsEmitter.emit(PrincipalsConstants.events.UPDATED_GROUP_MEMBERS, ctx, group, permissions, newMemberIds, updatedMemberIds, removedMemberIds);
                                    return callback();
                                });
                            });
                        });
        ...
        PrincipalsDAO.getPrincipal(groupId, function(err, group) {
            ...
            AuthzAPI.computeMemberRolesAfterChanges(group.id, changeMembers, function(err, computedPermissions, newMemberIds, updatedMemberIds, removedMemberIds) {
                ...
                AuthzAPI.updateRoles(group.id, changeMembers, function(err) {
                    ...
                    PrincipalsDAO.updatePrincipal(group.id, updatedProfileFields, null, function(err) {
                        if (err) {
                            return log().error({
                                'err': err,
                                'group': group
 ...
 * @param  {Object}    [permissions]        A hash where each key is a principal id or email address and the value is one of `PrincipalsConstants.roles`
 * @param  {Function}  [callback]           Standard callback function
...
var createGroup = module.exports.createGroup = function(ctx, displayName, description, visibility, joinable, permissions, callback) {
    ...
    // Get the principal ids, creating guest users if necessary
    PrincipalsGuestsAPI.getPrincipalsAndCreateGuests(ctx, _.keys(permissions), function(err, principals) {
        if (err) {
            return callback(err);
        }

        var members = {};
        _.each(permissions, function(role, principal) {
            members[principals[principal].id] = role;
        });
        ...
        AuthzAPI.canInteract(ctx, tenantAlias, principals, function(err, canInteract, illegalPrinicpalIds) {
            ...
            PrincipalsDAO.createGroup(groupId, tenantAlias, displayName, description, visibility, joinable, currentUser, function (err, group) {
                ...
                AuthzAPI.updateRoles(group.id, members, function(err) {
                    if (err) {
                        return callback(err);
                    }

                    // Update the user's libraries
                    _touchMembershipLibraries(group, null, _.pluck(principals, 'id'), [], function(err) {
                        if (err) {
                            log().error({
                                'err': err,
                                'group': group,
...
var updateGroup = module.exports.updateGroup = function(ctx, groupId, profileFields, callback) {
    ...
    PrincipalsDAO.getPrincipal(groupId, function(err, oldStorageGroup) {
        ...
        _canManage(ctx, oldStorageGroup.id, function(err, canManage) {
            ...
            PrincipalsDAO.updatePrincipal(groupId, profileFields, null, function(err) {
                if (err) {
                    return callback(err);
                }
