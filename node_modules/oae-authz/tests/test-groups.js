...
describe('Authz-Groups', function() {
    ...
    var createUserIds = function(tenantId, num) {
        var ids = [];
        for (var i = 0; i < num; i++) {
            ids.push(AuthzUtil.toId('u', tenantId, i));
        }
        return ids;
    };
    ...
    describe('#getPrincipalMemberships()', function() {
        ...
        it('verify invalid principal id', function(callback) {
            AuthzAPI.getPrincipalMemberships('not an id', null, null, function(err) {
                assert.ok(err);
                assert.equal(err.code, 400);
                callback();
            });
        ...
        it('verify non-principal id', function(callback) {
            AuthzAPI.getPrincipalMemberships('c:cam:Foo.docx', null, null, function(err) {
                assert.ok(err);
                assert.equal(err.code, 400);
                callback();
            });
        ...
        it('verify empty membership is empty array', function(callback) {
            ...
            AuthzAPI.getPrincipalMemberships(userId, null, null, function(err, groupIds) {
                assert.ok(!err);
                assert.ok(groupIds);
