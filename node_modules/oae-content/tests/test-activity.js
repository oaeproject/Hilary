...
describe('Content Activity', function() {
    ...
    describe('Activity Aggregation', function() {
        ...
        /**
         * Test that verifies that content-create activities aggregate properly
         */
        it('verify content-create activities are aggregated appropriately', function(callback) {
            TestsUtil.generateTestUsers(camAdminRestContext, 4, function(err, users, simon, nico, bert, stuart) {
                assert.ok(!err);

                // Nico creates 2 groups
                TestsUtil.generateTestGroups(nico.restContext, 2, function(nicosGroup1, nicosGroup2) {
                    TestsUtil.generateTestGroups(bert.restContext, 1, function(bertsGroup) {
                        ...
                        // Create the link and add the necessary principals
                        RestAPI.Content.createLink(simon.restContext, 'Google', 'Google', 'public', 'http://www.google.ca', [], [nicosGroup1.group.id, nicosGroup2.group.id, bertsGroup.group.id, stuart.user.id], [], function(err, link) {
                            assert.ok(!err);

                            // Simon should have an activity in his stream with
                            // all the extra members as targets
                            ActivityTestsUtil.collectAndGetActivityStream(simon.restContext, null, null, function(err, data) {
                                assert.ok(!err)
                                ActivityTestsUtil.assertActivity(data.items[0], 'content-create', 'create', simon.user.id, link.id, [nicosGroup1.group.id, nicosGroup2.group.id, bertsGroup.group.id, stuart.user.id]);

                                // Nico should see an activity where his groups are the only targets
                                ActivityTestsUtil.collectAndGetActivityStream(nico.restContext, null, null, function(err, data) {
                                    assert.ok(!err)
                                    ActivityTestsUtil.assertActivity(data.items[0], 'content-create', 'create', simon.user.id, link.id, [nicosGroup1.group.id, nicosGroup2.group.id]);

                                    // Bert should see an activity where his group is the only target
                                    ActivityTestsUtil.collectAndGetActivityStream(bert.restContext, null, null, function(err, data) {
                                        assert.ok(!err)
                                        ActivityTestsUtil.assertActivity(data.items[0], 'content-create', 'create', simon.user.id, link.id, bertsGroup.group.id);

                                        // Stuart sees himself as the target
                                        ActivityTestsUtil.collectAndGetActivityStream(stuart.restContext, null, null, function(err, data) {
                                            assert.ok(!err)
                                            ActivityTestsUtil.assertActivity(data.items[0], 'content-create', 'create', simon.user.id, link.id, stuart.user.id);

                                            // In nicosGroup1's activity stream the target should only be the group itself
                                            ActivityTestsUtil.collectAndGetActivityStream(nico.restContext, nicosGroup1.group.id, null, function(err, data) {
                                                assert.ok(!err)
                                                ActivityTestsUtil.assertActivity(data.items[0], 'content-create', 'create', simon.user.id, link.id, nicosGroup1.group.id);
                                                // In nicosGroup2's activity stream the target should only be the group itself
                                                ActivityTestsUtil.collectAndGetActivityStream(nico.restContext, nicosGroup2.group.id, null, function(err, data) {
                                                    assert.ok(!err)
                                                    ActivityTestsUtil.assertActivity(data.items[0], 'content-create', 'create', simon.user.id, link.id, nicosGroup2.group.id);

                                                    // In bertsGroup's activity stream the target should only be the group itself
                                                    ActivityTestsUtil.collectAndGetActivityStream(bert.restContext, bertsGroup.group.id, null, function(err, data) {
                                                        assert.ok(!err)
                                                        ActivityTestsUtil.assertActivity(data.items[0], 'content-create', 'create', simon.user.id, link.id, bertsGroup.group.id);

                                                        // Create another piece of content that can aggregate with the previous activities
                                                        RestAPI.Content.createLink(simon.restContext, 'Google', 'Google', 'public', 'http://www.google.ca', [], [nicosGroup1.group.id, nicosGroup2.group.id, bertsGroup.group.id, stuart.user.id], [], function(err, link2) {
                                                            assert.ok(!err);
                                                            ...
                                                            ActivityTestsUtil.collectAndGetActivityStream(simon.restContext, null, null, function(err, data) {
                                                                assert.ok(!err)
                                                                ActivityTestsUtil.assertActivity(data.items[0], 'content-create', 'create', simon.user.id, [link.id, link2.id], [nicosGroup1.group.id, nicosGroup2.group.id, bertsGroup.group.id, stuart.user.id]);

                                                                // Nico should see an activity where his groups are the only targets
                                                                ActivityTestsUtil.collectAndGetActivityStream(nico.restContext, null, null, function(err, data) {
                                                                    assert.ok(!err)
                                                                    ActivityTestsUtil.assertActivity(data.items[0], 'content-create', 'create', simon.user.id, [link.id, link2.id], [nicosGroup1.group.id, nicosGroup2.group.id]);

                                                                    // Bert should see an activity where his group is the only target
                                                                    ActivityTestsUtil.collectAndGetActivityStream(bert.restContext, null, null, function(err, data) {
                                                                        assert.ok(!err)
                                                                        ActivityTestsUtil.assertActivity(data.items[0], 'content-create', 'create', simon.user.id, [link.id, link2.id], bertsGroup.group.id);

                                                                        // Stuart sees himself as the target
                                                                        ActivityTestsUtil.collectAndGetActivityStream(stuart.restContext, null, null, function(err, data) {
                                                                            assert.ok(!err)
                                                                            ActivityTestsUtil.assertActivity(data.items[0], 'content-create', 'create', simon.user.id, [link.id, link2.id], stuart.user.id);

                                                                            // In nicosGroup1's activity stream the target should only be the group itself
                                                                            ActivityTestsUtil.collectAndGetActivityStream(nico.restContext, nicosGroup1.group.id, null, function(err, data) {
                                                                                assert.ok(!err)
                                                                                ActivityTestsUtil.assertActivity(data.items[0], 'content-create', 'create', simon.user.id, [link.id, link2.id], nicosGroup1.group.id);
                                                                                // In nicosGroup2's activity stream the target should only be the group itself
                                                                                ActivityTestsUtil.collectAndGetActivityStream(nico.restContext, nicosGroup2.group.id, null, function(err, data) {
                                                                                    assert.ok(!err)
                                                                                    ActivityTestsUtil.assertActivity(data.items[0], 'content-create', 'create', simon.user.id, [link.id, link2.id], nicosGroup2.group.id);
